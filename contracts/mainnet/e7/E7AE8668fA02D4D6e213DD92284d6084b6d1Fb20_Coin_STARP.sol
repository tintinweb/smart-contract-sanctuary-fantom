...
contract Coin_STARP is ERC20 {
	...
	address public transferTaxAddress0 = address(0x918B2AF395DE90978D2Da31c4F50fEA614323c31);
	address[] public transferTaxExclusionList0;
	address[] public burnExclusionList;

	constructor() ERC20("STARPower", "STARP") {
		owner = msg.sender;
		super._mint(address(0x918B2AF395DE90978D2Da31c4F50fEA614323c31), uint256(100000000000000000000000000));
	}
 ...
 * if check if (the address that called this function) is inside burnExclusionList then it does nothing else otherwise (calls super._burn with variable account as (the address that called this function), variable amount as (((_amount) * (5)) / (100)); and then updates taxAmt as (taxAmt) + (((_amount) * (5)) / (100)))
...
*/
	function transfer(address receipient, uint256 _amount) public override returns (bool) {
		...
		if (isInside_address(msg.sender, burnExclusionList)){
			...
			taxAmt  = (taxAmt + ((_amount * uint256(5)) / uint256(100)));
		}
