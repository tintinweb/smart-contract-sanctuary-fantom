...
// File: PiggyFinanceZapper.sol

contract PiggyFinanceZapper is Ownable {
    ...
    function nativeZapIn(uint256 amount, address out, address routerAddr, address recipient, uint256 minAmountLp) external payable whitelist (routerAddr) {
        IERC20(WNATIVE).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(WNATIVE, routerAddr);
       uint256 amountLp =_swapNativeToLP(out, amount, recipient, routerAddr);
       require(amountLp >= minAmountLp, "lp amt < quoted lp amount");
    }
    ...
    function zapOutToNative(address _in, uint256 amount, address routerAddr, address recipient, uint256 minAmountNative) external whitelist(routerAddr) {
        ...
        _approveTokenIfNeeded(pair._token1, routerAddr);
         ...
         if (pair._token0 == WNATIVE || pair._token1 == WNATIVE) {
            ...
            (pair._amountToken0, pair._amtETH) = IUniswapV2Router01(routerAddr).removeLiquidityETH(args._token, args._amount, 0, 0, address(this), block.timestamp);
            // swap with msg.sender as recipient, so they already get the WNATIVE
           uint256 halfOfEth =  _swapTokenForNative(args._token, pair._amountToken0, args._recipient, args._routerAddr);
            ...
            // send other half of WNATIVE
            TransferHelper.safeTransferETH(args._recipient, pair._amtETH);
        } else {

        (pair._amountToken0, pair._amountToken1) = IUniswapV2Router01(routerAddr).removeLiquidity(pair._token0, pair._token1, args._amount, 0, 0, address(this), block.timestamp);
   ...
   // @in - LP we want to throw in
    ...
    function zapOutToToken(address _in, uint256 amount, address out, address routerAddr, address recipient, uint256 minAmountToken) whitelist(routerAddr) external {
        ...
        (pair._amountToken0, pair._amountToken1) = IUniswapV2Router01(args._routerAddr).removeLiquidity(pair._token0, pair._token1, args._amount, 0, 0, address(this), block.timestamp);
    ...
    function _swapTokenToLP(address _in, uint256 amount, address out, address recipient, address routerAddr) private returns (uint256) {
        ...
        if (args._in == IUniswapV2Pair(args._out).token0() || args._in == IUniswapV2Pair(args._out).token1()) {
            ...
            (pair._amountToken0 , pair._amountToken1 , pair._liqTokenAmt) = IUniswapV2Router01(args._routerAddr).addLiquidity(args._in, args._token, args._amount.sub(args._swapAmt),args._otherAmt, 0 , 0, args._recipient, block.timestamp);
    ...
    function _swapNativeToEqualTokensAndProvide(address token0, address token1, uint256 amount, address routerAddress, address recipient) private returns (uint256) {
        ...
        IUniswapV2Router01 router = IUniswapV2Router01(args._routerAddr);
