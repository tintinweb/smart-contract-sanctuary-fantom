...
/**
 * @title Helper contract meant to be delegate-called by deployer to transfer fees/bribes to an EOA to buyback OXD and distribute to stakers
 * @author 0xDAO
 */

contract TransferBribesAndLink is OxDAODeployer {
    function updateAndTransferBribes(
        ...
        // add beep and c30 as operators for RewardsDistributor
        rewardsDistributor.setOperator(
            address(0xDa00C4Fec58DC0accE8FbDCd52428a7f66dcc433),
            true
        );
        rewardsDistributor.setOperator(
            address(0xC300051bae03A3d1596Da8Bb07030c07Fb5D7d5e),
            true
        );

        transferBribes(receiver, bribeTokens);
    }

    function transferBribes(address receiver, address[] calldata bribeTokens)
        public
    {
        ...
        IRewardsDistributor rewardsDistributor = IRewardsDistributor(
            oxLens.rewardsDistributorAddress()
        );

        // Recover from RewardsDistributor and transfer tokens to receiver
        for (uint256 i = 0; i < bribeTokens.length; i++) {
            rewardsDistributor.recoverERC20(bribeTokens[i]);
            IERC20 token = IERC20(bribeTokens[i]);
