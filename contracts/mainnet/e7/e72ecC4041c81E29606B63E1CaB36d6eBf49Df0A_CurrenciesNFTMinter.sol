...
import "../../../nft/interfaces/INFTMintableERC1155.sol";
import "../../../nft/interfaces/INFTTotalSupplyERC1155.sol";
import "../interfaces/ICurrenciesNFT.sol";
import "../interfaces/ICurrenciesNFTMinter.sol";

contract CurrenciesNFTMinter is Initializable, UUPSUpgradeable, AccessControlEnumerableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ICurrenciesNFTMinter {
    ...
    error Fee(uint256);
    error NotBuyable();
    error MaxSupply(uint256);

    function initialize() public initializer {
        __CurrenciesNFTMinter_init();
    }

    function __CurrenciesNFTMinter_init() internal onlyInitializing {
        ...
        __CurrenciesNFTMinter_init_unchained();
    }

    function __CurrenciesNFTMinter_init_unchained() internal onlyInitializing {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        ...
        interfaceId == type(ICurrenciesNFTMinter).interfaceId;
    }

    function buy(uint256 currencyId_, uint256 amount_) external payable whenNotPaused nonReentrant {
        CurrenciesNFTLib.CurrencyAttributes memory currency = ICurrenciesNFT(currenciesNFT).getCurrencyAttributes(currencyId_);
        uint256 price_ = currency.price;

        if(currency.buyable != 1) revert NotBuyable();
        if(msg.value < price_ * amount_) revert Fee(price_ * amount_);
    ...
    function safeMint(address to_, uint256 currencyId_, uint256 amount_) internal {
        CurrenciesNFTLib.CurrencyAttributes memory currency = ICurrenciesNFT(currenciesNFT).getCurrencyAttributes(currencyId_);
        uint256 totalSupply_ = INFTTotalSupplyERC1155(currenciesNFT).getTotalSupply(currencyId_);
        uint256 maxSupply_ = currency.maxSupply;
        if(totalSupply_ + amount_ > maxSupply_) revert MaxSupply(maxSupply_);
        INFTMintableERC1155(currenciesNFT).mint(to_, currencyId_, amount_);
    }
    ...
    /**
     * @notice Mint while increasing maxSupply
    ...
    function extraMint(address to_, uint256 currencyId_, uint256 amount_) external onlyRole(AccessControlConstants.MINTER_ROLE) {
        ...
        INFTMintableERC1155(currenciesNFT).mint(to_, currencyId_, amount_);
        currency.maxSupply += amount_;
        ICurrenciesNFT(currenciesNFT).setCurrencyAttributes(currencyId_, currency);
    }
...
interface ICurrenciesNFTMinter {
