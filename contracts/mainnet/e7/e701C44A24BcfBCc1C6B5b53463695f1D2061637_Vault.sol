...
contract Vault is IVault, ReentrancyGuardUpgradeable
{
  ...
  {
    require(msg.sender == _REGISTRY.coordinator(), "!coordinator");
    ...
    coo = msg.sender;
    _token = IERC20(token);
    _token.safeApprove(_REGISTRY.depositManager(), type(uint256).max);
    _token.safeApprove(_REGISTRY.borrowManager(), type(uint256).max);
  }
  ...
  {
    require(msg.sender == coo, "!coordinator");


    _token.safeApprove(_REGISTRY.depositManager(), 0);
    _token.safeApprove(_REGISTRY.depositManager(), type(uint256).max);

    _token.safeApprove(_REGISTRY.borrowManager(), 0);
    _token.safeApprove(_REGISTRY.borrowManager(), type(uint256).max);
  }
}
