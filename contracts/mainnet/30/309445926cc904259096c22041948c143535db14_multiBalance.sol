...
contract multiBalance
{
	...
	function balanceOf(address[] memory t, address[] memory h) public view returns (uint256[][] memory b, uint256[] memory n)
	{
		for(uint i;i<t.length;i++)
		{
			for(uint j;j<h.length;j++)
			{
				b[i][j] = LPT(t[i]).balanceOf(h[j]);
				n[i] += b[i][j];
			}
			b[i][h.length] = LPT(t[i]).decimals();
		}
	}

	function tvlOfCoinLPT(address[] memory t) public view returns (uint256[] memory b, uint256 n)
	{
		for(uint i;i<t.length;i++)
		{
	...
	function tvlOfUsdLPT(address[] memory t, address[] memory u) public view returns (uint256[] memory b, uint256 n)
	{
		for(uint i;i<t.length;i++)
		{
	...
	function valueOfTokensCoin(address[][2] memory t, address[] memory h) public view returns (uint256[][] memory b, uint256[][] memory v, uint256[][3] memory n)
	{
		for(uint i;i<t.length;i++)
		{
			n[i][2] == ITVL(tvlGuru).p_t_coin_usd(t[i][1]);
			b[i][h.length] = LPT(t[i][1]).decimals();
			for(uint j;j<h.length;j++)
			{
				b[i][j] = LPT(t[i][0]).balanceOf(h[j]);
				v[i][j] = b[i][j] * n[i][2] / 10**b[i][h.length];
				n[i][0] += b[i][j];
			}
			n[i][1] = n[i][0] * n[i][2] / 10**b[i][h.length];
		}
	}

	function valueOfTokensUsd(address[][3] memory t, address[] memory h) public view returns (uint256[][] memory b, uint256[][] memory v, uint256[][3] memory n)
	{
		for(uint i;i<t.length;i++)
		{
			n[i][2] == ITVL(tvlGuru).p_t_usd(t[i][1],t[i][2]);
			b[i][h.length] = LPT(t[i][2]).decimals();
			for(uint j;j<h.length;j++)
			{
				b[i][j] = LPT(t[i][0]).balanceOf(h[j]);
				v[i][j] = b[i][j] * n[i][2] / 10**b[i][h.length];
