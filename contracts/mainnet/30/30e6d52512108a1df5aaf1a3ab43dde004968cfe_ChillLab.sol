...
contract ChillLab is ChillPillRarity, ReentrancyGuard {
    ...
    uint32 public pillBoost14days = 1200;
    uint32 public pillBoost7days  = 1100;
    ...
    uint public constant MAX_CHILLPILL_POWER = 10627876002;
    uint public MAX_CHILLPILL_STAKE = 2; // Max amount of pills you can stake
 
    uint256 public stakingFeePercent; // percentage of staking fees to be collected
    ...
    function _deposit(uint _pid, uint _amount, address to, uint[] memory tokenIDs) internal {
        ...
        EnumerableSet.UintSet storage staked = _stakedChillPills[_pid][to];
        uint256 totalPills = len + staked.length();//.length;
        require(totalPills <= MAX_CHILLPILL_STAKE, string(abi.encodePacked("deposit: Can only boost with a maximum of ",MAX_CHILLPILL_STAKE," chill pills")));
    ...
    function setChillPillBoost(uint32 newBaseBoost,uint32 newPillBoost7days,uint32 newPillBoost14days) external onlyAdmin {
        require(newBaseBoost < 5000); //5000 = 50%
        require(newPillBoost7days < 5000); //5000 = 50%
        require(newPillBoost14days < 5000); //5000 = 50%
        basePillBoost = newBaseBoost;
        pillBoost7days= newPillBoost7days;
        pillBoost14days = newPillBoost14days;
    }
