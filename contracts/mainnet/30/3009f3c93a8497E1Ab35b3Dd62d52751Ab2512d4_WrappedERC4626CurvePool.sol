...
contract WrappedERC4626CurvePool is ERC4626 {
    ...
    constructor(
    ...
    ) ERC4626(IERC20Metadata(_lpToken)) ERC20(name, symbol) {
        ...
        token.approve(curvePool.poolAddress, type(uint256).max);
        lpToken.approve(curvePool.poolAddress, type(uint256).max);
    }
    ...
    {
        return curvePool.calcTokenAmount(assets, true) * (10 ** decimals()) / (10 ** lpTokenDecimals);
    }
    ...
    {
		return curvePool.calcWithdrawOneCoin(shares * (10 ** lpTokenDecimals) / (10 ** decimals()));
    }
    ...
    function _withdraw(
        ...
        _burn(owner, shares);
		uint256 lpAmount = shares * (10 ** lpTokenDecimals) / (10 ** decimals());
        uint256 amount = curvePool.removeLiquidity(lpAmount);
...
library CurveLibrary {
        ...
        returns (uint256 lpAmount)
    {
        if (pool.assetsCount == 2) {
            ...
            lpAmount = ICurvePool2Assets(pool.poolAddress).add_liquidity(
                amounts,
                0
            );
            ...
            amounts[pool.assetIndex] = amount;
            lpAmount = ICurvePool3Assets(pool.poolAddress).add_liquidity(
                amounts,
                0
            );
            ...
            amounts[pool.assetIndex] = amount;
            lpAmount = ICurvePool4Assets(pool.poolAddress).add_liquidity(
                amounts,
                0
            );
        }
    ...
    function calcTokenAmount(
        ...
        if (pool.assetsCount == 2) {
            uint256[2] memory amounts;
            amounts[pool.assetIndex] = amount;
            lpAmount = ICurvePool2Assets(pool.poolAddress).calc_token_amount(
                amounts,
                isDeposit
            );
        } else if (pool.assetsCount == 3) {
            uint256[3] memory amounts;
            amounts[pool.assetIndex] = amount;
            lpAmount = ICurvePool3Assets(pool.poolAddress).calc_token_amount(
                amounts,
                isDeposit
            );
        } else if (pool.assetsCount == 4) {
            uint256[4] memory amounts;
            amounts[pool.assetIndex] = amount;
            lpAmount = ICurvePool4Assets(pool.poolAddress).calc_token_amount(
                amounts,
                isDeposit
            );
        }
    ...
    {
        amount = ICurvePool(pool.poolAddress).calc_withdraw_one_coin(
            lpAmount,
    ...
    function removeLiquidity(CurvePool storage pool, uint256 lpAmount)
        ...
        returns (uint256 amount)
    {
        amount = ICurvePool(pool.poolAddress).remove_liquidity_one_coin(lpAmount, int128(pool.assetIndex), 1);
    }
}