...
// File contracts/EqualizerFtmScarabvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualizerFtmScarabvAMM is Ownable, Pausable {
    ...
    address public constant scarab = address(0x2e79205648B85485731CFE3025d66cF2d3B059c4);
    address public want;
    ...
    IEqualizerRouter.Routes[] public wrappedToScarabPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToScarabPath
    ) {
        ...
        for (uint i; i < _wrappedToScarabPath.length; ++i) {
           wrappedToScarabPath.push(_wrappedToScarabPath[i]);
        }
   ...
   //Withdraws funds from masterchef
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(wrappedHalf, 0, wrappedToScarabPath, address(this), block.timestamp);

        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(scarab).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(wrapped, scarab, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp); 
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(scarab).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(scarab).safeApprove(router, type(uint).max);
    }
