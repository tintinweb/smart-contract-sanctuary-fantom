...
contract DiceRoll is Ownable, ReentrancyGuard, VRFV2WrapperConsumerBase{
    using SafeMath for uint256;
    ...
    mapping(uint256 => uint256) public maxBetSelection;

    mapping(uint256 => uint256) public payouts;

    mapping(string => uint256) public payoutNumSel;

    event DiceResult(uint256 requestId, uint256 dv1, uint256 dv2, uint256 payout);
    ...
    constructor(address _usdtAddress, address _erc1155) VRFV2WrapperConsumerBase(linkAddress, vrfWrapperAddress){
        treasury = msg.sender;
    ...
    function viewDiceStatus(uint256 requestId) public view returns(uint256 randomWord1,uint256 randomWord2, address player, uint256 dv1, uint256 dv2, bool fulfilled, uint256 payout, uint256[] memory selection, uint256[] memory bet) {
        ...
        payout = statuses[requestId].payout;
        selection = statuses[requestId].selection;
        bet = statuses[requestId].bet;
    }
     
    function rollDice(uint256[] memory _bet, uint256[] memory _selection, uint32 _dice) public nonReentrant returns(uint256) {

        require(msg.sender == tx.origin, "smart contract are not allowed");
        require(_dice == 1 || _dice == 2, "dice can only be 1 or 2" );
        require(_bet.length == _selection.length, "must be same length");
        ...
        if(_dice == 1){
            ...
            for(uint i = 0 ; i < _selection.length ; i++) {
                require(_selection[i] <= 10 && _selection[i] > 0 , "not a valid selection");
            }
            ...
            require(count <= 3, "max selection is 3 for 1 dice");

        }else{
            
            for(uint i = 0 ; i < _selection.length ; i++) {
                require(_selection[i] <= 16 && _selection[i] > 2 , "not a valid selection");
            }
            ...
            require(count <= 6, "max selection is 6 for 2 dice");
        }

        uint256 requestId = requestRandomness(callbackGasLimit, requestConfirmation, _dice);
