...
contract Elf is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 ELF-WETH LP -> ELF pool
    // uint256 public constant INITIAL_ELF_POOL_DISTRIBUTION = 140000 ether;
    ...
    uint256 public sellFee = 0; // see fee is 10%
    ...
    /**
     * @notice Constructs the ELF ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ELF", "ELF") {
        // Mints 1 ELF to contract creator for initial pool setup
    ...
    function _getElfPrice() internal view returns (uint256 _elfPrice) {
        try IOracle(elfOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Elf: failed to fetch ELF price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _elfPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_elfPrice >= taxTiersTwaps[tierId]) {
    ...
    function setElfOracle(address _elfOracle) public onlyOperatorOrTaxOffice {
        require(_elfOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ELF to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ELF to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentElfPrice = _getElfPrice();
            currentTaxRate = _updateTaxRate(currentElfPrice);
            if (currentElfPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_elfPool != address(0), "!_elfPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_elfPool, INITIAL_ELF_POOL_DISTRIBUTION);
