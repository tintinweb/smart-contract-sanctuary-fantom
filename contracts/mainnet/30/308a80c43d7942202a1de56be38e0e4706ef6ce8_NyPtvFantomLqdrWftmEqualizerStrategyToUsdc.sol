/**
 *Submitted for verification at FtmScan.com on 2023-01-21
*/
...
error NyPtvFantomLqdrWftmEqualizerStrategyToUsdc__MaxSlippageOutOfBounds(uint256 maxSlippage);
...
contract NyPtvFantomLqdrWftmEqualizerStrategyToUsdc is NYBaseProfitTakingStrategyV3 {
  ...
  function _swapFarmEmissionTokens() internal {
    ...
    if (equalToUsdcPath.length >= 2 && equalBalance > 0) {
      ...
      uint percentage = (amounts[2] * 9700) / 10000;
  ...
  function updateMaxSlippage(uint256 _maxSlippage) external {
    if (_maxSlippage > 10000) {
      revert NyPtvFantomLqdrWftmEqualizerStrategyToUsdc__MaxSlippageOutOfBounds(_maxSlippage);
    }
    _atLeastRole(ADMIN);
