...
contract SmeltRewardPool is Initializable,
                            OwnableUpgradeable,
    ...
    IERC20Upgradeable smelt;
    IERC20Upgradeable stater;
    ...
    struct PoolInfo {
        ...
        IERC721Upgradeable nft; // Address of nft contract.
    ...
    function initialize(
        ...
        smelt = IERC20Upgradeable(_smelt);
        stater = IERC20Upgradeable(_stater);
    ...
    function UnstakeNft(uint256 _pid, uint256[] calldata _tokenIds) external nonReentrant  {
        ...
        if (_tokenIds.length > 0) {
            ...
            if (pool.givesReceipts == true) {
                IERC20Upgradeable godsRcpt = IERC20Upgradeable(godsReceipt);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant  {
        ...
        if (_amount > 0) {
            ...
            if (pool.givesReceipts == true) {
                IERC20Upgradeable bsdRcpt = IERC20Upgradeable(basedReceipt);
   ...
   for (uint256 i = 0; i < userIds.length; ++i) {
    ...
    function nftCheckPoolDuplicate(IERC721Upgradeable _nft) internal view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
    ...
    function add(
        ...
        require (_depFee < 10000, "cant be > 100%");
         if (_isNftPool) {
            ...
            nftCheckPoolDuplicate(IERC721Upgradeable(_nft));

        } else if (!_isNftPool)  {
            _nft = address(0x0000000000000000000000000000000000000000);
