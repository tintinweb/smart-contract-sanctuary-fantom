...
interface BBondOtcContract {
    function _swapOnOTC(address _in, uint256 _amount) external returns (uint256);
...
interface BasedTreasury {
    function buyBonds(uint256 _basedAmount, uint256 targetPrice) external;
}
...
import '../../interfaces/BasedTreasury.sol';
import '../../interfaces/BBondOtcContract.sol';

/// all LP strategies will inherit this contract
contract BBondMatrixSingleAutocompoundV2 is MatrixStrategyBase, MatrixSwapHelper {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    address internal constant BBOND = 0xC078285F16665B3F4bCe74AbDCF0f4C877de3E9f;

    // additionals
    address internal constant BASED_TREASURY = 0x25430E2ACb1255453135b2c64436C6b1Ff8153C5;
    address internal constant BASED_OTC_CONTRACT = 0xf91249583D7D727CABFf12D36024b8b9Dc50253F;
    ...
    constructor(
        ...
        securityFee = 10;
        totalFee = 600;
        partner = 0x2EDb7f36830Abb77AF9D6Fe1b97b2251E5BE1d66;
        wrapped = WFTM;
    ...
    function _setWhitelistedAddresses() internal virtual {
        ...
        whitelistedAddresses.add(BASED_ROUTER);
        whitelistedAddresses.add(SPOOKYSWAP_ROUTER);
        whitelistedAddresses.add(BASED);
        whitelistedAddresses.add(BBOND);
    }

    function convertBasedToBBond() internal returns (uint256) {
        uint256 basedBalance = IERC20(BASED).balanceOf(address(this));
        if (basedBalance > 0) {
            uint256 pre = IERC20(BBOND).balanceOf(address(this));
            BBondOtcContract(BASED_OTC_CONTRACT)._swapOnOTC(BASED, basedBalance);
            uint256 post = IERC20(BBOND).balanceOf(address(this));
            return post - pre;
        }
    ...
    function _setDefaultSwapPaths() internal virtual {
        // USDC -> WFTM
        address[] memory _usdcWFTM = new address[](2);
        _usdcWFTM[0] = USDC;
        _usdcWFTM[1] = WFTM;
        _setSwapPath(USDC, WFTM, SPOOKYSWAP_ROUTER, _usdcWFTM);

        // WFTM -> BASED
        address[] memory _wftmBased = new address[](2);
        _wftmBased[0] = WFTM;
        _wftmBased[1] = BASED;
        _setSwapPath(WFTM, BASED, BASED_ROUTER, _wftmBased);
    }
    ...
    function _giveAllowances() internal override {
        ...
        IERC20(BASED).safeApprove(BASED_TREASURY, 0);
        IERC20(BASED).safeApprove(BASED_TREASURY, type(uint256).max);

        IERC20(BASED).safeApprove(BASED_OTC_CONTRACT, 0);
        IERC20(BASED).safeApprove(BASED_OTC_CONTRACT, type(uint256).max);
    }

    function _removeAllowances() internal override {
        // removing approve tokens for mastechef
        IERC20(want).safeApprove(masterchef, 0);
        IERC20(BASED).safeApprove(BASED_TREASURY, 0);
        IERC20(BASED).safeApprove(BASED_OTC_CONTRACT, 0);
    }
    ...
    function _swap(
        ...
        // custom routes
        if (_fromToken == USDC && _toToken == BBOND) {
            // first swap to based
            _swap(_fromToken, BASED, _amount);

            // wrap based to bbond
            return convertBasedToBBond();
        }

        if (_fromToken == USDC && _toToken == BASED) {
            uint256 _toTokenBefore = IERC20(_toToken).balanceOf(address(this));

            // first swap to wftm
            uint256 _amountToSwap = _swap(USDC, WFTM, _amount);

            // then swap to based
            _swap(WFTM, BASED, _amountToSwap);

            return IERC20(_toToken).balanceOf(address(this)) - _toTokenBefore;
        }
    ...
    function _harvest() internal virtual override returns (uint256 _wantHarvested, uint256 _wrappedFeesAccrued) {
        ...
        if (_outputBalance > 0) {
            ...
            _wantHarvested = _swap(output, want, _outputBalance);
        }
    }
