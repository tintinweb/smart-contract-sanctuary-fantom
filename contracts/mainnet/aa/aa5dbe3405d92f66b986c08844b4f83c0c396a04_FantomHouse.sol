...
contract FantomHouse is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    function refund() external nonReentrant {
        for (uint256 i = refundedUpTo; i <= ids.length; i++) {
            Bet storage _bet = bets[ids[i]];
            if (_bet.timestamp != 0 && block.timestamp >= _bet.timestamp + 15 minutes) {
                delete _bet.timestamp;
                _bet.pending = 2;
    ...
    function fulfillRandomWords(
        ...
        require(_bet.timestamp != 0, "timestamp should exist");
