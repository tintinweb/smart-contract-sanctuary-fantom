...
contract POL is Ownable {
    ...
    function makePOL() external {
        uint balanceWFTM = wftm.balanceOf(address(this));
        uint balanceUSDC = _swap(balanceWFTM / 2);
        balanceWFTM /= 2;
        wftm.approve(address(router), balanceWFTM);
        usdc.approve(address(router), balanceUSDC);
        router.addLiquidity(address(wftm), address(usdc), balanceWFTM, balanceUSDC, balanceWFTM * 950/1000, balanceUSDC * 950/1000, address(this), block.timestamp + 180);
    }
