...
interface ISaleModel is IInitOwnable {
    event Initiated(uint256 indexed host, address indexed collection, uint256 indexed startTime);
...
interface ISaleFactory {
    ...
    event Initiated(IInitOwnable[] indexed saleModels, address indexed treasury, uint256 indexed treasuryCut, uint256 commission);
...
contract BuyoutFixed is ISaleModel, InitOwnable {
    ...
    ISaleFactory constant public SALE_FACTORY = ISaleFactory(0x7edc8F6A471e13B5AF358F0B5dA4ED48c8100d64);
    ...
    function init(
        ...
        if (_bonuses.length > 0) {
            for (uint256 i; i < _bonuses.length; i++) {
                bulkBonuses.push(_bonuses[i]);
            }
        }

        emit Initiated(_host, address(_collection), block.timestamp);
    }
    ...
    function claimRaised() external override onlyInit {
        ...
        if (commission > 0) {
            ...
            if (host.treasury != theaterTreasury) {
                ...
                // otherwise, give total commission to theater.
