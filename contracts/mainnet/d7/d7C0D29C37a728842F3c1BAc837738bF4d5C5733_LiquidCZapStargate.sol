...
contract LiquidCZapStargate {
    ...
    function LiquidCOut1 (address liquidCVault, uint256 withdrawAmount, uint256 poolID, address lpToken) external {
        ILiquidCVaultV6 vault = ILiquidCVaultV6(liquidCVault);
    ...
    function LiquidCOut2 (address liquidCVault, address token, uint256 withdrawAmount, uint16 poolID, address pool, address lpToken) external {
        _approveTokenIfNeeded(lpToken, stargateRouter);
        _approveTokenIfNeeded(lpToken, pool);
        uint256 amountLiquidity = IERC20(lpToken).balanceOf(address(this));
        IStargateRouter(stargateRouter).instantRedeemLocal(poolID, amountLiquidity, address(this));
        uint256 balance = IERC20(token).balanceOf(address(this));
        IERC20(token).safeTransfer(msg.sender, balance);
    }

    function LiquidCOut3 (address liquidCVault, address token, uint256 withdrawAmount, uint16 poolID, address pool, address lpToken) external {
        _approveTokenIfNeeded(lpToken, pool);
        uint256 amountLiquidity = IERC20(lpToken).balanceOf(address(this));
        IStargateRouter(stargateRouter).instantRedeemLocal(poolID, amountLiquidity, address(this));
        uint256 balance = IERC20(token).balanceOf(address(this));
    ...
    function LiquidCOut4 (address liquidCVault, address token, uint256 withdrawAmount, uint16 poolID, address pool, address lpToken) external {
        _approveTokenIfNeeded(lpToken, stargateRouter);
        uint256 amountLiquidity = IERC20(lpToken).balanceOf(address(this));
        IStargateRouter(stargateRouter).instantRedeemLocal(poolID, amountLiquidity, address(this));
        uint256 balance = IERC20(token).balanceOf(address(this));
