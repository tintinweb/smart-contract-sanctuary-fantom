...
interface IBinaryPunkdApes {
  function mint(address to, uint256 tokenId) external;
}

contract PunkdApes is ERC721,
  ERC721Enumerable,
  ERC721URIStorage,
  Ownable,
  AccessControl,
  ...
  address public bpaAddress;
  bool public mintingEnabled;
  string public baseUrl;
  enum MintType { PUBLIC, AIRDROP, ADMIN }
  ...
  constructor() ERC721("Punk'd Apes", "PA") {
    adminMintCount = 0;
    airdropMintCount = 0;
    mintCount = 0;
    price = 100 * 10**18;
    supply = 5000;
    stopMintingAt = supply;
    royaltyFee = 75; // 7.5%
    royaltyAddress = 0x802fa9C643E3d1770ffFA60195631e1b7002d795;
    startFrom = 0;
    mintBlockSize = 1000;
    mintBlockNr = 1;
    mintOffset = 0;
  ...
  function mint(uint256 quantity) external payable {
    ...
    require(mintCount + quantity <= stopMintingAt, "STOP_MINTING_AT");
    require(msg.value == price * quantity, "WRONG_AMOUNT");
    _doMint(quantity, msg.sender, MintType.PUBLIC);
    _splitPayment();
  }

  function adminMint(uint256 quantity, address to) external {
  ...
  function specificMint(uint256 tokenId, address to) external {
    require(tokenId < mintBlockNr * mintBlockSize, "ID_OUT_OF_MINT_BLOCK");
    require(hasRole(ADMIN_ROLE, msg.sender), "UNAUTHORIZED");
  ...
  function setMintBlock(uint256 _mintBlockNr) external onlyOwner {
    mintBlockNr = _mintBlockNr;
    startFrom = (mintBlockNr - 1) * mintBlockSize;
  }

  function _bpaMint(address to, uint256 tokenId) private {
    IBinaryPunkdApes bpa = IBinaryPunkdApes(bpaAddress);
    bpa.mint(to, tokenId);
  }

  function _doMint(uint256 quantity, address to, MintType mintType) private {
    require(bpaAddress != address(0), "BPA_ADDRESS_NOT_SET");
    require(mintCount + quantity <= supply, "MINTED_OUT");
    for (uint256 i = 0; i < quantity; i++) {
      ...
      if (mintType == MintType.PUBLIC && mintCount == stopMintingAt)
      {
  ...
  function setMintingEnabled(bool value) external {
    require(hasRole(ADMIN_ROLE, msg.sender), "UNAUTHORIZED");
  ...
  function setStopMintingAt(uint256 value) external onlyOwner {
    stopMintingAt = value;
  ...
  function setBpaAddress(address addr) external onlyOwner {
    bpaAddress = addr;
    emit AddressPropertyChange("bpaAddress", addr);
  }
  ...
  function _splitPayment() internal {
    if (feeRecipientCount > 0) {
      ...
      for(uint256 i = 1; i <= feeRecipientCount; i++) {
        uint256 amountForRecipient = amount * feeRecipients[i].basisPoints / totalFeeBasisPoints;
        (bool sent, ) = feeRecipients[i].recipient.call{value: amountForRecipient}("");
