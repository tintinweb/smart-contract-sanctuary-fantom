 ...
 */
contract ReaperVaultNativev1_3 is ERC20, Ownable, ReentrancyGuard {
    ...
    // Wrapped native token the vault accepts and looks to maximize.
    ...
    constructor(
        ...
        wrapped = IWrappedNative(_wrappedNative);
    ...
    /**
     * @dev It calculates the total underlying value of {wrapped} held by the system.
     * It takes into account the vault contract balance, the strategy contract balance
     * and the balance deployed in other contracts as part of the strategy.
     *
     * Note that there shouldn't be any native coins in the vault since there is no
     * "receive" or "fallback" function that is payable.
     */
    function balance() public view returns (uint256) {
        return wrapped.balanceOf(address(this)) + IStrategy(strategy).balanceOf();
    }
    ...
    function depositAll() external {
        deposit(wrapped.balanceOf(msg.sender));
    }
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        uint256 _before = wrapped.balanceOf(address(this));
        wrapped.safeTransferFrom(msg.sender, address(this), _amount);
        uint256 _after = wrapped.balanceOf(address(this));
    ...
    /**
     * Alternative entry point into the vault. User can send native coins
     * and the vault will wrap them before forwarding to the strategy.
     */
    function depositNative() external payable nonReentrant {
        ...
        uint256 _before = wrapped.balanceOf(address(this));
        require(wrapped.deposit{value: _amount}() == wrapped.ERR_NO_ERROR(), "wrapping error");
        uint256 _after = wrapped.balanceOf(address(this));
        _amount = _after - _before;
        uint256 _amountAfterDeposit = (_amount * (PERCENT_DIVISOR - depositFee)) / PERCENT_DIVISOR;
        uint256 shares = 0;
    ...
    function earn() public {
        ...
        wrapped.safeTransfer(strategy, _bal);
        IStrategy(strategy).deposit();
    }

    /**
     * Helper version for FE-ABI consistency that always defaults to
     * using flash loans for withdrawals, and paying msg.sender back in
     * wrapped tokens.
     */
    function withdrawAll() external {
        withdrawWithMode(balanceOf(msg.sender), true, false);
    }

    /**
     * @dev A helper function to call withdraw() with all the sender's funds.
     */
    function withdrawAllWithMode(bool _useFL, bool _useNative) external {
        withdrawWithMode(balanceOf(msg.sender), _useFL, _useNative);
    }
    ...
    function withdraw(uint256 _shares) external {
        withdrawWithMode(_shares, true, false);
    }
    ...
    function withdrawWithMode(
        ...
        uint256 b = wrapped.balanceOf(address(this));
        if (b < r) {
        ...
        if (_useNative) {
            require(wrapped.withdraw(r) == wrapped.ERR_NO_ERROR(), "unwrapping error");
            (bool success, ) = msg.sender.call{value: r}("");
            require(success, "Transfer failed.");
        } else {
            wrapped.safeTransfer(msg.sender, r);
        }
        incrementWithdrawals(r);
    }

    /**
     * Payable receive function to be able to receive native coins from
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        require(_token != address(wrapped), "!wrapped");
