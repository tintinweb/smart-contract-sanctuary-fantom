...
contract PancakeERC20 is IPancakeERC20 {
    ...
    string public constant name = 'Magik LPs';
    string public constant symbol = 'MAGIK-LP';
...
contract MagikFactory is IPancakeFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'Magik: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Magik: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Magik: PAIR_EXISTS'); // single check is sufficient
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'Magik: FORBIDDEN');
        feeTo = _feeTo;
    }
