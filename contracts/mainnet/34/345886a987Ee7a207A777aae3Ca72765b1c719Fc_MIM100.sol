...
import "./HundredStaker.sol";

contract MIM100 is HundredStaker {

    constructor(uint maxGasPrice)
        HundredStaker(
            0x82f0B8B456c1A451378467398982d4834b6829c1,
            0xa8cD5D59827514BCF343EC19F531ce1788Ea48f8,
...
/**
 * @dev Stakes DAI in hundred.finance.
 * @notice Needs approval for FRAX, HND, and any added source token.
 * Wallet also needs to call `toggle_approve_mint` on the `MINTER`
 * to allow this contract to claim/compound the rewards.
 */
contract HundredStaker is GelatoTask, TokenHandler {
    ...
    address public immutable gaugeToken;
    address public immutable minter;

    address public router;

    address[] public HNDSwapPath;
    ...
    constructor(
        ...
        GelatoTask(0x6EDe1597c05A0ca77031cBA43Ab887ccf24cd7e8, maxGasPrice)
    {
        token = token_;
        hToken = hToken_;
        gaugeToken = gaugeToken_;
        minter = 0x42B458056f887Fd665ed6f160A59Afe932e1F559;
    }

    function setHNDSwapPath(address router_, address[] memory path) external onlyOwner {
        require(path[0] == HND, "path not starting with HND");
        require(path[path.length - 1] == token, "invalid swap route");
        router = router_;
        HNDSwapPath = path;
    }
    ...
    function supply(address wallet) external onlyManager(wallet) validGasPrice {
        ...
        IGauge(gaugeToken).transfer(wallet, IGauge(gaugeToken).balanceOf(address(this)));
    }

    function _supply(address wallet, uint amount, bool compound) private {
        _approveMaxIfNecessary(token, hToken, amount);
        uint err = IHundredERC20(hToken).mint(amount);
        require(err == 0, "failed to supply");
        _stake(wallet, compound);
    }

    function _stake(address wallet, bool compound) private {
        uint amount = IERC20(hToken).balanceOf(address(this));
        if (amount > 0) {
            _approveMaxIfNecessary(hToken, gaugeToken, amount);
            IGauge(gaugeToken).deposit(amount);
            if (compound) _compound(wallet);
        }
    }

    function _compound(address wallet) private {
        TokenMinter(minter).mint_for(gaugeToken, wallet);
        uint amount = _transferableAmount(wallet, HND);
        if (amount > 0) {
            IERC20(HND).safeTransferFrom(wallet, address(this), amount);
            _approveMaxIfNecessary(HND, router, amount);
            uint[] memory amountsOut = IRouter(router).swapExactTokensForTokens(amount, 0, HNDSwapPath, address(this), block.timestamp);
