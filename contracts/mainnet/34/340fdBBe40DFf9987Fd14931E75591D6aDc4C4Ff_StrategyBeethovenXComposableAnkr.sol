...
contract StrategyBeethovenXComposableAnkr is StratFeeManagerInitializable {
    ...
    bytes32 public toNativeSwapPoolId;
    address public rewarder;
    address public solidlyRouter;

    // Routes
    ISolidlyRouter.Routes[] public rewardToAnkrFTMRoute;
    ...
    function initialize(
        ...
        bytes32[] calldata _toNativeSwapPoolId,
        ISolidlyRouter.Routes[] calldata _rewardToAnkrFTMRoute,
        ...
        for (uint i; i < _rewardToAnkrFTMRoute.length; ++i) {
            rewardToAnkrFTMRoute.push(_rewardToAnkrFTMRoute[i]);
        }
        ...
        toNativeSwapPoolId = _toNativeSwapPoolId[0];
        secondOutput = rewardToAnkrFTMRoute[0].from;
        ankrFTM = rewardToAnkrFTMRoute[rewardToAnkrFTMRoute.length - 1].to;
    ...
    function swapRewardsToNative() internal {
        ...
        if (ankrFTMBal > 0) {
            balancerSwap(toNativeSwapPoolId, ankrFTM, native, ankrFTMBal);
        }
    ...
    function rewardToAnkrFTM() external view returns (address[] memory) {
        ISolidlyRouter.Routes[] memory _route = rewardToAnkrFTMRoute;
