...
contract Bender is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the BENDER ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("BENDER", "BENDER") {
        // Mints 100 BENDER to contract creator for initial pool setup
    ...
    function _getBenderPrice() internal view returns (uint256 _benderPrice) {
        try IOracle(benderOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Bender: failed to fetch BENDER price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _benderPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_benderPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBenderOracle(address _benderOracle) public onlyOperatorOrTaxOffice {
        require(_benderOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BENDER to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BENDER to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBenderPrice = _getBenderPrice();
            currentTaxRate = _updateTaxRate(currentBenderPrice);
            if (currentBenderPrice < burnThreshold) {
                burnTax = true;
            }
        }
