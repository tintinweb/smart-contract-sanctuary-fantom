...
// File contracts/EqualDaiEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualDaiEqualizerV2  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToDaiPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToDaiPath,
        ...
        for (uint i; i < _equalToDaiPath.length; ++i) {
            equalToDaiPath.push(_equalToDaiPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToDaiPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(equal).balanceOf(address(this));
        uint256 t2Bal = IERC20(dai).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(equal, dai, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(dai).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(dai).safeApprove(router, type(uint).max);
    }
