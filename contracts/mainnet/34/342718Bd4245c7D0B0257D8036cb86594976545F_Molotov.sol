...
// File: Molotov Finance/lib/SafeMath8.sol
...
// File: Zelensky Finance/math/Math.sol
...
// File: Zelensky Finance/utils/Context.sol
...
// File: Zelensky Finance/owner/Ownable.sol
...
// File: Zelensky Finance/GSN/Context.sol



pragma solidity >=0.6.0 <0.8.0;
// File: Zelensky Finance/owner/Operator.sol
...
// File: Zelensky Finance/ERC20/ERC20.sol
...
// File: Zelensky Finance/ERC20/ERC20Burnable.sol
...
// File: Zelensky Finance/Molotov.sol

pragma solidity 0.6.12;

contract Molotov is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 48h genesis pools 
    uint256 public constant INITIAL_RARE_POOL_DISTRIBUTION = 55000 ether;

    // DAO FUND INITIAL ALLOCATION IS 1000 MOLOTOV
    ...
    /**
     * @notice Constructs the MOLOTOV ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MOLOTOV", "MOLOTOV") {
        // Mints 3,000 MOLOTOV to contract creator for initial pool setup & LP
    ...
    function _getRarePrice() internal view returns (uint256 _rarePrice) {
        try IOracle(rareOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Molotov: failed to fetch MOLOTOV price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _rarePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_rarePrice >= taxTiersTwaps[tierId]) {
    ...
    function setRareOracle(address _rareOracle) public onlyOperatorOrTaxOffice {
        require(_rareOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MOLOTOV to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MOLOTOV to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRarePrice = _getRarePrice();
            currentTaxRate = _updateTaxRate(currentRarePrice);
            if (currentRarePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_rarePool != address(0), "!_rarePool");
         require(_daoFund != address(0), "!_treasury");
        rewardPoolDistributed = true;
        _mint(_rarePool, INITIAL_RARE_POOL_DISTRIBUTION);
