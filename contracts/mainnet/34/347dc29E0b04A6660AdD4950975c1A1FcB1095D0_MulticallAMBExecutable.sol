 ...
 */
contract MulticallAMBExecutable is MulticallExecutable, RBAC {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");
    bytes32 public constant FB_EXEC_ROLE = keccak256("FB_EXEC_ROLE");

    constructor(address _admin) RBAC(_admin) {}

    receive() external payable onlyRole(FB_EXEC_ROLE) {}

    fallback() external payable onlyRole(FB_EXEC_ROLE) {}
 ...
 * @dev Provides a function to batch together multiple calls in a single external call.
 */
abstract contract MulticallExecutable {
    struct InputData {
        address dest;
        bytes data;
        uint256 value;
