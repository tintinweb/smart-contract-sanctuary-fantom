...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant elite = address(0xf43Cc235E686d7BC513F53Fbffb61F760c3a1882);
    address internal constant eth = address(0x74b23882a30290451A17c44f4F05243b6b58C76d);
    address[] internal rewardTokens;
    address[4] internal slippageLPs;
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR ELITE-ETH EQUALIZER")),
            string(abi.encodePacked("XpE-ELITE-ETH"))
        )
        {
        gauge = _gauge;
        router = _router;
        ...
        slippageLPs = [address(0x7547d05dFf1DA6B4A2eBB3f0833aFE3C62ABD9a1), address(0x3d6c56f6855b7Cc746fb80848755B0a9c3770122), address(0xE3bD349BDb8203C15426b2d273f57568E658F843), address(0x9B99bBE1EBd7415DBC859e35161a451698c5D331)];
    ...
    function _chargeFees(address caller) internal {                   
        ...
        uint minAmt = getSlippage(equalBal, slippageLPs[1], equal);
        ...
        uint usdProfit = IEqualizerPair(slippageLPs[0]).sample(wftm, toProfit, 1, 1)[0];
    ...
    function _addLiquidity() internal {
        ...
        (uint minAmtEth) = getSlippage(wftmHalf, slippageLPs[2], wftm);
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmtEth, wftm, eth, false, address(this), lastHarvest);

        (uint minAmtElite) = getSlippage(wftmHalf, slippageLPs[3], wftm);
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmtElite, wftm, elite, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(eth, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(elite, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(eth, elite, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(eth, elite, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function _timestamp() internal view returns (uint64 timestamp){
        uint lastBlock = IEqualizerPair(slippageLPs[0]).blockTimestampLast();
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyOwner {
        if(_token == equal || _token == wftm || _token == elite || _token == eth){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(eth, router, 0);
        SafeTransferLib.safeApprove(elite, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(eth, router, type(uint).max);
        SafeTransferLib.safeApprove(elite, router, type(uint).max);
    }
