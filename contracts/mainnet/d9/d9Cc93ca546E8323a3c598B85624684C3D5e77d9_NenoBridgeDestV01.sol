...
contract NenoBridgeDestV01 is Ownable{
    ...
    function anyExecute(bytes memory _data) external onlyAuth returns (bool success, bytes memory result){ //add access control only smpc address can execute this
    ...
    function redeem(address _token, uint256 _amount) external payable returns (bool) { //add prereq
