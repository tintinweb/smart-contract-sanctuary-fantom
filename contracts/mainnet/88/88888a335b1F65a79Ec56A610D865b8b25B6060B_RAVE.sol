...
contract RAVE is ERC20, ERC20Snapshot, Ownable {
  ...
  function _mint(address account, uint256 amount) internal virtual override(ERC20) {
    require(ERC20.totalSupply() + amount <= uint(100_000_000).toDecimals(), "ERC20Capped: cap exceeded");
    super._mint(account, amount);
  }
  ...
  function isClaimed(address account) external view returns(bool) { return claimed[account]; }
  ...
  constructor(address _treasury, bytes32 root, address dev) ERC20("Rave Names", "RAVE") {
    treasury = _treasury;
    merkleRoot = root;
