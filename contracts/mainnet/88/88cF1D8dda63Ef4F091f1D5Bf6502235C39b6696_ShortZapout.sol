...
contract ShortZapout is Ownable {
    ...
    function estimateZapOutToken(address _fromLp, uint256 amount, address _to, address _router ) public view whitelist(_router) returns (uint256) {
        address token0 = IUniswapV2Pair(_fromLp).token0();
        address token1 = IUniswapV2Pair(_fromLp).token1();
        if(_to == WNATIVE) {
            if(token0 == WNATIVE) {
                //address _in, uint256 amount, address out, address routerAddr
                return _estimateSwap(token1, amount, _to, _router);
            } else {
                return _estimateSwap(token0, amount, _to, _router);
            }
        }
        ...
        if(token0 == WNATIVE) {
            ...
            if(_to == token1) {
                ...
                uint256 halfAmountof_to = _estimateSwap(token0, amount, _to, _router);
                uint256 otherhalfAmountof_to = _estimateSwap(token1, amount, _to, _router);
