...
// File contracts/EqualFtmAnkrvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualFtmAnkrvAMM  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public wrappedToAnkrPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToAnkrPath
    ) {
        ...
        for (uint i; i < _wrappedToAnkrPath.length; ++i) {
            wrappedToAnkrPath.push(_wrappedToAnkrPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToAnkrPath, address(this), block.timestamp);  

        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(ankr).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, ankr, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);    
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(ankr).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(ankr).safeApprove(unirouter, type(uint).max);
    }
