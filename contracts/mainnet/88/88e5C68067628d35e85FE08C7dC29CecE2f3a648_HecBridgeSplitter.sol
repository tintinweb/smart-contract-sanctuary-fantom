...
contract HecBridgeSplitter is OwnableUpgradeable, PausableUpgradeable {
	...
	function Bridge(
		...
		for (uint256 i = 0; i < sendingAssetInfos.length; i++) {
			require(sendingAssetInfos[i].feePercentage > minFeePercentage, 'Spltter: invalid asset info');
			...
			if (sendingAssetInfos[i].sendingAssetId != address(0)) {
				...
				uint256 approvedAmount = sendingAssetInfos[i].sendingAmount;
				uint256 feeAmount = (approvedAmount * sendingAssetInfos[i].feePercentage) / 1000;
				uint256 calcBridgeAmount = approvedAmount - feeAmount;
	...
	function _takeFee(SendingAssetInfo memory feeInfo) internal returns (address, uint256) {
		uint256 feeAmount = (feeInfo.sendingAmount * feeInfo.feePercentage) / 1000;
		if (feeInfo.sendingAssetId != address(0)) {
			IERC20Upgradeable token = IERC20Upgradeable(feeInfo.sendingAssetId);
			token.safeTransferFrom(address(this), DAO, feeAmount);
			return (feeInfo.sendingAssetId, feeAmount);
		} else {
			payable(DAO).transfer(feeAmount);
