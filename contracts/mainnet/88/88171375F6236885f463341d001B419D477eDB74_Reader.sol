...
contract Reader is Governable {
    ...
    function getMaxAmountIn(
        ...
        uint256 maxUsdeAmount = _vault.maxUsdeAmounts(_tokenIn);

        if (maxUsdeAmount != 0) {
            if (maxUsdeAmount < _vault.usdeAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdeAmount.sub(_vault.usdeAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10**tokenInDecimals).div(10**USDE_DECIMALS);
    ...
    function getAmountOut(
        ...
        {
            uint256 usdeAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            usdeAmount = usdeAmount.mul(10**USDE_DECIMALS).div(10**tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, usdeAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, usdeAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 2] = vault.usdeAmounts(token);
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _usdeAmount);
    ...
    function getFullVaultTokenInfo(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxUsdeAmounts(token);
