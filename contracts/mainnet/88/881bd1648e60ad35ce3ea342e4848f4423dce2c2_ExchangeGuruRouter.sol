...
library ExchangeGuruLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
            ...
            hex'a647c30b51cf748e0c22cebd9a4ec16845fe40efe4bdb589812cdfabc16e7342' // init code hash
        ))));
    }
    ...
    function getSwapFee(address factory, address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = IExchangeGuruPair(pairFor(factory, tokenA, tokenB)).swapFee();
    }
...
contract ExchangeGuruRouter is IExchangeGuruRouter {
    ...
    function addLiquidity(
        ...
        address pair = ExchangeGuruLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = ExchangeGuruLibrary.pairFor(factory, token, WETH);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? ExchangeGuruLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IExchangeGuruPair(ExchangeGuruLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ExchangeGuruLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(ExchangeGuruLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = ExchangeGuruLibrary.sortTokens(input, output);
            IExchangeGuruPair pair = IExchangeGuruPair(ExchangeGuruLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = ExchangeGuruLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, pair.swapFee());
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ExchangeGuruLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(ExchangeGuruLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    {
        return ExchangeGuruLibrary.getAmountOut(amountIn, reserveIn, reserveOut, swapFee);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return ExchangeGuruLibrary.getAmountsOut(factory, amountIn, path);
    }
