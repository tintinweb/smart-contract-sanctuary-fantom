...
contract REITSale is Ownable {
    using SafeERC20 for ERC20;
    using Address for address;

    uint256 constant USDCdecimals = 10**18;
    uint256 constant REITdecimals = 10**9;

    uint256 public MAX_SOLD = 195000 * REITdecimals;
    uint256 public privateSalePrice = (20 * USDCdecimals) / REITdecimals;
    uint256 public publicSalePrice = (30 * USDCdecimals) / REITdecimals;

    uint256 public constant MAX_PRIVATE_SALE_PER_ACCOUNT = 75 * REITdecimals;
    uint256 public constant MAX_PUBLIC_SALE_PER_ACCOUNT = 50 * REITdecimals; 

    mapping(address => bool) public approvedBuyers;
    mapping(address => uint256) public invested;

    address public REIT; // REIT Token
    address public dev;
    ...
    constructor(address _dev, address usdc_address) {
        dev = _dev;
    ...
    function buyREIT(uint256 amount) public onlyEOA {
        require(REIT == address(0), "No longer for sale");
    ...
    function claimREIT() public onlyEOA {
        require(isClaimable(), "Claiming not active");
        ERC20(REIT).transfer(msg.sender, invested[msg.sender]);
        owed -= invested[msg.sender];
        invested[msg.sender] = 0;
    }

    function setReitAddress(address _REIT) public {
        require(msg.sender == dev, "!dev");
        REIT = _REIT;
    ...
    function setMaxSold(uint256 _MAX_SOLD) public {
        require(msg.sender == dev, "!dev");
        MAX_SOLD = _MAX_SOLD * REITdecimals;
    }

    function setPrivateSalePrice(uint256 _price) public {
        require(msg.sender == dev, "!dev");
        privateSalePrice = (_price * USDCdecimals) / (REITdecimals * 10);
    }

    function setPublicSalePrice(uint256 _price) public {
        require(msg.sender == dev, "!dev");
        publicSalePrice = (_price * USDCdecimals) / (REITdecimals * 10);
    }

    function withdraw(address _token, address receiverAddress) external onlyOwner  {
        require(REIT != address(0), "REIT address not set");
