...
contract TFUSD is IERC20Metadata, Ownable {
  ...
  uint256 private _cap = 100 * 10**9 * 10**18;
  ...
  uint256 public upperPegRate = 1150000;
  uint256 public lowerPegRate = 1150000;
  ...
  event Amounts(uint256 a, uint256 b, address c);
  ...
  /******************                    PEGGING                    ****************************/
  /*********************************************************************************************/
  function addLiquidity2(uint256 pegAmount) public {
    address pairAddr = IUniswapV2Factory(uniFactory).getPair(address(this), peggedToken);
    if (pairAddr == address(0) || pairAddress == address(0) || pairAddr != pairAddress) {
    ...
    emit Amounts(tfusdAmount, pegAmount, pairAddress);
  ...
  function changePeggedToken(address pegToken) public onlyManager {
    ...
    if (pairAddr == address(0) || pairAddress == address(0) || pairAddr != pairAddress) {
      ...
      pairAddr = IUniswapV2Factory(uniFactory).getPair(address(this), pegToken);
    }
   ...
   */
  function peg(bool autoLp) public onlyManager {
    ...
    if (autoLp) {
      ...
      _swapTfusdToDaiAddLiquidity(inAmount);
    }
    else {
      ...
      uint256[] memory amounts = IUniswapV2Router(uniRouter).swapExactTokensForTokens(inAmount, 0, path, address(this), block.timestamp);
  ...
  function requiredAutoLp() public view returns(bool) {
    if (getPeggedRatio() > upperPegRate) return true;
    return false;
  }

  function requiredManualLp() public view returns(bool) {
    if (getDepeggedRatio() > lowerPegRate) return true;
    return false;
  }
  ...
  function _approveTokenIfNeeded(address token, address spender, uint256 allowAmount) private {
    ...
    if (oldAllowance < allowAmount) {
      IERC20(token).approve(spender, allowAmount.sub(oldAllowance));
    }
  }

  function _autoPegAndAddFeeLiquidity(uint256 amount) internal {
    if (requiredAutoLp() == true) {
      peg(true);
    }
    uint256 aLpFee = amount.mul(autoLpFee).div(10 ** manageDecimal);
    _swapTfusdToDaiAddLiquidity(aLpFee);
  }

  function _swapTfusdToDaiAddLiquidity(uint256 inAmount) internal {
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = peggedToken;
  ...
  function _addLiquidity(uint256 tfusdAmount, uint256 pegAmount) public {
  ...
  function _transfer(
    ...
    unchecked {
      ...
      if (excludedAddress[to] == true) {
        ...
        uint256 rFee = amount.mul(reflectFee).div(10 ** manageDecimal);
        uint256 aLpFee = amount.mul(autoLpFee).div(10 ** manageDecimal);
  ...
  function _afterTokenTransfer(
    ...
    _autoPegAndAddFeeLiquidity(amount);
  }
