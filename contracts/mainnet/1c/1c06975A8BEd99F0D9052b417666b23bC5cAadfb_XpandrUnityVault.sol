...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant skull = address(0xfa5992A8A47aF7029e04eC6a95203AD3f301460b);
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR SKULL-FTM EQUALIZER")),
            string(abi.encodePacked("XpE-SKULL-FTM"))
        )
        {
        gauge = _gauge;
        router = _router;
    ...
    function _addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmt, wftm, skull, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(wftm, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(skull, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(wftm, skull, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(wftm, skull, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyOwner {
        if(_token == equal || _token == wftm || _token == skull){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(skull, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(skull, router, type(uint).max);
    }
