...
contract Strategy is BaseStrategy {
	...
	SwapSteps internal beetsSwapSteps;
	...
	constructor(
		...
		toggles = Toggles({
			...
			doSellRewards: true,
			collectFees: true,
			abandonRewards: false
		});
	}

	function approveRewards() internal {
		IERC20(BEETS).approve(address(balancerVault), 0);
		IERC20(BEETS).approve(address(balancerVault), max);
		IERC20(DEUS).approve(solidlyRouter, 0);
		IERC20(DEUS).approve(solidlyRouter, max);
	}
	...
	function name() external view override returns (string memory) {
		return string(abi.encodePacked('Beethoven USDC-DEI v2'));
	}
	...
	function estimateHarvest() external view returns (uint256 _profit) {
		(uint256 _beets, uint256 _deus) = getPendingRewards();

		if (_beets > 0) {
			(uint256 beetUnitPrice, ) =
				ISolidlyRouter(solidlyRouter).getAmountOut(1e18, BEETS, address(want));
			_profit += beetUnitPrice.mul(_beets);
		}

		route[] memory deusToWantRoute = new route[](2);
		deusToWantRoute[0] = route(DEUS, DEI, false);
		deusToWantRoute[1] = route(DEI, address(want), true);
		if (_deus > 0) {
			uint256[] memory results =
				ISolidlyRouter(solidlyRouter).getAmountsOut(_deus, deusToWantRoute);
			_profit += results[2];
		}
	}
	...
	function getPendingRewards() public view returns (uint256 _beets, uint256 _deus) {
		_beets = masterChef.pendingBeets(masterChefPoolId, address(this));
		_deus = rewarder.pendingToken(masterChefPoolId, address(this));
	}

	function balanceOfReward() public view returns (uint256 _beets, uint256 _deus) {
		_beets = IERC20(BEETS).balanceOf(address(this));
		_deus = IERC20(DEUS).balanceOf(address(this));
	}
	...
	function bptsToTokens(uint256 _amountBpt) public view returns (uint256 _amount) {
	...
	function liquidateAllPositions() internal override returns (uint256 liquidated) {
		...
		_sellBpt(balanceOfBpt()); // Sell all bpt
	...
	function _sellBpt(uint256 _amountBpts) internal {
		...
		if (_amountBpts > 0) {
			...
			minAmountsOut[tokenIndex] = bptsToTokens(_amountBpts).mul(basisOne.sub(maxSlippageOut)).div(
				basisOne
			);
			bytes memory userData =
				abi.encode(IBalancerVault.ExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT, _amountBpts, tokenIndex);
	...
	function _sellRewards() internal {
		(uint256 _beetsAmount, uint256 _deusAmount) = balanceOfReward();
		if (_beetsAmount > 0) {
			uint256 length = beetsSwapSteps.poolIds.length;
			...
			limits[0] = int256(_beetsAmount);
			for (uint256 j = 0; j < length; j++) {
				steps[j] = IBalancerVault.BatchSwapStep(
					beetsSwapSteps.poolIds[j],
					j,
					j + 1,
					j == 0 ? _beetsAmount : 0,
					abi.encode(0)
	...
	function _joinPool() internal returns (bool _joined) {
		...
		if (amountIn > 0) {
			uint256 expectedBptOut =
				tokensToBpts(amountIn).mul(basisOne.sub(maxSlippageIn)).div(basisOne);
			uint256[] memory maxAmountsIn = new uint256[](numTokens);
			maxAmountsIn[tokenIndex] = amountIn; // Want
			bytes memory userData =
				abi.encode(
	...
	function _depositIntoMasterChef(uint256 _bpts) internal {
	...
	function prepareMigration(address _newStrategy) internal override {
		...
		(uint256 _beetsAmount, uint256 _deusAmount) = balanceOfReward();
		if (_beetsAmount > 0) {
			IERC20(BEETS).safeTransfer(_newStrategy, _beetsAmount);
		}
		if (_deusAmount > 0) {
			IERC20(DEUS).safeTransfer(_newStrategy, _deusAmount);
		}
	}
	...
	function depositIntoMasterChef(uint256 _bpts) external onlyVaultManagers {
	...
	function addSwapSteps(SwapSteps memory _beetsSteps) public onlyVaultManagers {
		beetsSwapSteps = _beetsSteps;
	}
	...
	function setToggles(
		bool _doClaimRewards,
		bool _doSellRewards,
		bool _doCollectFees,
		bool _abandonRewards
	) external onlyVaultManagers {
		toggles.claimRewards = _doClaimRewards;
		toggles.doSellRewards = _doSellRewards;
		toggles.collectFees = _doCollectFees;
		toggles.abandonRewards = _abandonRewards;
	}
