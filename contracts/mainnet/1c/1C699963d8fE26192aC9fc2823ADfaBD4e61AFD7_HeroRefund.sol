...
contract HeroRefund is AccessControl, IERC721Receiver, Pausable {
    // Library Methods
    ...
    // Contract Configs
    mapping(uint256 => bool) public _giveaway;
    mapping(uint256 => bool) public _isRefunded;
    mapping(uint256 => uint256) public _refundAmt;

    // Events
    event HeroRefunded(uint256 _nftId, uint256 _amount, address _owner);

    // NFT Construction
    constructor(address _fantomHeroesAddress, address _heroFactoryAddress) {
        // setup roles
    ...
    function claimRefund(uint256 _nftId) public payable whenNotPaused {
        SharedStructs.Hero memory heroData = _heroFactory.getHero(_nftId);
        require(_fanthomHeroes.ownerOf(_nftId) == msg.sender, "Must own hero.");
        require(
            heroData.generation == 0,
            "Only Gen0 Heroes allowed for refund"
        );
        require(
            _giveaway[_nftId] == false,
            "Giveaway hero. Not eligible for refund"
        );
        require(_isRefunded[_nftId] == false, "Refund already processed");

        _isRefunded[_nftId] = true;

        require(_refundAmt[_nftId] > 0, "No refund available");

        require(
            address(this).balance >= _refundAmt[_nftId],
            "Not enough funds to process refund"
        );

        _fanthomHeroes.safeTransferFrom(msg.sender, address(this), _nftId);

        (bool success, ) = (msg.sender).call{value: _refundAmt[_nftId]}("");
        require(success, "Refund Failure");

        emit HeroRefunded(_nftId, _refundAmt[_nftId], msg.sender);
    }
    ...
    function manageGiveawayHeroes(uint256[] memory _nftIds, bool _isGiveaway)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < _nftIds.length; i++) {
            _giveaway[_nftIds[i]] = _isGiveaway;
        }
    }

    function manageRefundAmount(
        uint256[] memory _nftIds,
        uint256[] memory _amounts
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_nftIds.length == _amounts.length, "Arrays must be same size");

        for (uint256 i = 0; i < _nftIds.length; i++) {
            _refundAmt[_nftIds[i]] = _amounts[i] * 1 ether;
        }
    }

    function manageRefundedHeroes(uint256[] memory _nftIds, bool _status)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < _nftIds.length; i++) {
            _isRefunded[_nftIds[i]] = _status;
        }
    }
