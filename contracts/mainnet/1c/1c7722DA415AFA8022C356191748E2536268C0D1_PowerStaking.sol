...
contract PowerStaking is OwnableUpgradeable {
    ...
    struct PoolInfo {
        ...
        uint256 accPowerPerShare; // Accumulated SUSHIs per share, times 1e12. See below.
    }
    // The Power TOKEN!
    IERC20 public power;
    // Block number when bonus Power period ends.
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 powerReward =
                multiplier.mul(powerPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accPowerPerShare = accPowerPerShare.add(
                powerReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accPowerPerShare = pool.accPowerPerShare.add(
            powerReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accPowerPerShare).div(1e12).sub(
                    user.rewardDebt
                );
    ...
    // Safe power transfer function, just in case if rounding error causes pool to not have enough SUSHIs.
    ...
    function setPowerPerBlock(uint256 _newValue) external onlyOwner {
        powerPerBlock = _newValue;
    }
}
