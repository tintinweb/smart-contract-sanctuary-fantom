...
import "../cryptoblock/currencyblocks/minters/CurrencyBlocksMinter.sol";
import "../cryptoblock/CryptoBlocksUpgradeable.sol";
import "./lib/RivalsBattleConstants.sol";
import "./lib/RivalsConstants.sol";

/**
 * @dev RivalsBattle must have OPERATOR role for Rivals
 */
contract RivalsBattle is Initializable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, CryptoBlocksUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMathUpgradeable for uint256;
    uint256 private _randomNonce;                   // Nonce for random number generator
    uint256 private _randomSeed;                    // Seed for random number generator
    uint256 private _rewardAmount;                  // rewards for current round of queued participants
    uint256[] private _participants;                // dbIds of rivals participating
    uint256[] private _recentWinners;               // dbIds of recent winners
    uint256[] private _recentLosers;                // dbIds of recent losers
    address[] private _adminFeeAddress;             // Admin(s) fee address
    uint256[] private _adminFeeAmount;              // Admin(s) fee amount / 1000. ex: 1 = 0.1%
    address private _rivalsDb;                      // rivalsDb contract
    mapping(uint256 => address) private _rivalsRVL; // rivalsRVL contract
    address private _currencies;                    // currencies contract
    uint256 private _round;                         // round
    mapping(uint256 => MatchResult) private _matchResults;  // round => matchResult
    uint256[] private _weeklyTopWinners;            // top 5 weekly
    uint256[] private _overallTopWinners;           // top 5 overall
    address private _currenciesMinter;

    // Frontend can pull level and cp
    struct MatchResult {
        uint32 rivalA;          // dbId
        uint32 rivalB;          // dbId
        uint32 fantomMatterA;   // num FantomMatter used
        uint32 fantomMatterB;   // num FantomMatter used
        uint32 placeHolder;
        uint32 placeHolder2;
        uint32 placeHolder3;
        uint32 placeHolder4;
    ...
    event Win(uint256 indexed dbId, uint256 indexed time, uint256 indexed round);
    event Loss(uint256 indexed dbId, uint256 indexed time, uint256 indexed round);
    event MatchResults(uint256 indexed round, uint256 indexed rivalA, uint256 indexed rivalB, uint256 fantomMatterA, uint256 fantomMatterB);
    ...
    function initialize() initializer public {
        ...
        __RivalsBattle_init_unchained();
    }

    function __RivalsBattle_init_unchained() internal initializer {
        _uintMapping[RivalsBattleConstants.UINT_MIN_PARTICIPANTS] = 6;
        _uintMapping[RivalsBattleConstants.UINT_ENTRY_FEE] = 3 ether;
        _adminFeeAddress.push(msg.sender);
        _adminFeeAmount.push(10);
    }
     ...
     * @dev deprecated
     */
    /*function queueRival(uint256 tokenId, uint256 set, uint256 fee) payable external nonReentrant {
        queueRival(tokenId, set, fee, 0);
    }*/

    /**
     * @dev set = rivals set matching index of _rivalsRVL
     */
    function queueRival(uint256 tokenId, uint256 set, uint256 fee, uint256 fantomMatter) payable public nonReentrant {
        require(msg.value >= fee && fee == _uintMapping[RivalsBattleConstants.UINT_ENTRY_FEE], "F");
        require(set <= 1, "S");
        if(_participants.length == 0) {
            require(!paused(), "P");
        }

        require(IERC721Upgradeable(_rivalsRVL[set]).ownerOf(tokenId) == msg.sender, "!O");
        require(ICryptoBlocks(_rivalsRVL[set]).getBlockAttribute(tokenId, RivalsConstants.UINT_QUEUED) == 0, "Q");
        uint256 dbId = ICryptoBlocks(_rivalsRVL[set]).getBlockAttribute(tokenId, RivalsConstants.UINT_DBID);
        ICryptoBlocksAttributes rivalsRVL = ICryptoBlocksAttributes(_rivalsRVL[set]);
        rivalsRVL.setBlockAttribute(tokenId, RivalsConstants.UINT_QUEUED, 1);

        if(fantomMatter > 0) {
            require(fantomMatter <= 10, "FM");
            IERC1155BurnableUpgradeable(_currencies).burn(msg.sender, CurrencyBlocksConstants.ID_FANTOM_MATTER, fantomMatter);
            rivalsRVL.setBlockAttribute(tokenId, RivalsConstants.UINT_USE_FANTOM_MATTER, fantomMatter);
        }
        ...
        // last participant gets paid some fee for high gas cost
            /*
            uint256 totalAmount = fee;
            address[] memory adminFeeAddress = _adminFeeAddress;
            for(uint256 i; i < adminFeeAddress.length; i++) {
                // hardcode 6 in instead of _adminFeeAmount[i]
                // this equates to 0.018 ether
                uint256 balance = (totalAmount * 6) / 1000;
                if(balance > address(this).balance) {
                    balance = address(this).balance;
                }
                (bool sent,) = payable(adminFeeAddress[i]).call{value: balance}("");
                require(sent);
                fee = fee - balance;
            }*/
        // take away 20% to help pay last participant gas
        uint256 baseFee = fee;
        fee = fee - (fee * 20) / 1000;
        if(_participants.length == 5) {
            uint256 balance = ((baseFee * 20) * 6) / 1000;
            (bool sent,) = payable(msg.sender).call{value: balance}("");
            require(sent);
        }
        _rewardAmount += fee;
        _participants.push(dbId);
        _checkParticipants();
    }

    /**
     * @dev Checks participants and if there is enough, then randomly match each other
     * Rivals are tracked by dbId
     */
    function _checkParticipants() internal {
        if(_participants.length >= _uintMapping[RivalsBattleConstants.UINT_MIN_PARTICIPANTS]) {
            uint256[] memory participants = _participants;
            // clear recent winners and losers
            delete _recentWinners;
            delete _recentLosers;

            while(participants.length > 0) {
                uint256 rivalA;
                uint256 rivalB;
                (rivalA, participants) = _selectRandom(participants);
                if(participants.length > 1) {
                    (rivalB, participants) = _selectRandom(participants);
                } else {
                    rivalB = participants[0];
                    delete participants;
                }
                matchParticipants(rivalA, rivalB, _rewardAmount / 3);
            }
    ...
    /**
     * @dev Will implement more factors in future for win conditions
     * Factors that influence win chance
     * Level difference - up to 10%
     * Rarity - up to 10%
     * Fantom Matter - up to 10%
     * Combat Power - up to 10% rounded down
     */
    function matchParticipants(uint256 rivalA, uint256 rivalB, uint256 rewardAmount) internal {
        ICryptoBlocks[2] memory rivalSet;
        uint256[2] memory rivalTokenId;
        (rivalSet[0], rivalTokenId[0]) = _getRivalsRVL(rivalA);
        (rivalSet[1], rivalTokenId[1]) = _getRivalsRVL(rivalB);
        uint256[2] memory level = [rivalSet[0].getBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_LEVEL), rivalSet[1].getBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_LEVEL)];
        uint256[2] memory rarity = [rivalSet[0].getBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_RARITY), rivalSet[1].getBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_RARITY)];
        uint256[2] memory cp = [rivalSet[0].getBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_CP_UV), rivalSet[1].getBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_CP_UV)];
        uint256[2] memory fantomMatter = [rivalSet[0].getBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_USE_FANTOM_MATTER), rivalSet[1].getBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_USE_FANTOM_MATTER)];
        uint256 random = _random(0, 99);
        _round++;
        uint256 round = _round;

        if(int256(random) < 50 +
            (int256(level[0]) - int256(level[1])) +
            ((int256(rarity[0]) - int256(rarity[1])) * 10) +
            ((int256(fantomMatter[0]) - (int256(fantomMatter[1])) * 10) +
            ((int256(cp[0]) - int256(cp[1])) * 10) / 10)) {
            // rivalA won
            uint256 wins = rivalSet[0].getBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_WINS);
            uint256 balance = rivalSet[0].getBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_BALANCE);
            ICryptoBlocksAttributes(address(rivalSet[0])).setBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_WINS, wins + 1);
            ICryptoBlocksAttributes(address(rivalSet[0])).setBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_BALANCE, balance + rewardAmount);
            _recentWinners.push(rivalA);
            _updateTopWinners(rivalA, wins + 1);

            uint256 losses = rivalSet[1].getBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_LOSSES);
            ICryptoBlocksAttributes(address(rivalSet[1])).setBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_LOSSES, losses + 1);
            _recentLosers.push(rivalB);
            emit Win(rivalA, block.timestamp, round);
            emit Loss(rivalB, block.timestamp, round);
            emit MatchResults(round, rivalA, rivalB, fantomMatter[0], fantomMatter[1]);

            // enter match results
            MatchResult memory matchResult;
            matchResult.rivalA = uint32(rivalA);
            matchResult.rivalB = uint32(rivalB);
            matchResult.fantomMatterA = uint32(fantomMatter[0]);
            matchResult.fantomMatterB = uint32(fantomMatter[1]);
            _matchResults[round] = matchResult;

            // mint fantomMatter to loser
            ICurrencyBlocksMinter(_currenciesMinter).freeMint(IERC721Upgradeable(address(rivalSet[1])).ownerOf(rivalTokenId[1]), CurrencyBlocksConstants.ID_FANTOM_MATTER, 1);
        } else {
            // rivalB won
            uint256 wins = rivalSet[1].getBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_WINS);
            uint256 balance = rivalSet[1].getBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_BALANCE);
            ICryptoBlocksAttributes(address(rivalSet[1])).setBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_WINS, wins + 1);
            ICryptoBlocksAttributes(address(rivalSet[1])).setBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_BALANCE, balance + rewardAmount);
            _recentWinners.push(rivalB);
            _updateTopWinners(rivalB, wins + 1);

            uint256 losses = rivalSet[0].getBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_LOSSES);
            ICryptoBlocksAttributes(address(rivalSet[0])).setBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_LOSSES, losses + 1);
            _recentLosers.push(rivalA);
            emit Win(rivalB, block.timestamp, round);
            emit Loss(rivalA, block.timestamp, round);
            emit MatchResults(round, rivalB, rivalA, fantomMatter[0], fantomMatter[1]);

            // enter match results
            MatchResult memory matchResult;
            matchResult.rivalA = uint32(rivalB);
            matchResult.rivalB = uint32(rivalA);
            matchResult.fantomMatterA = uint32(fantomMatter[1]);
            matchResult.fantomMatterB = uint32(fantomMatter[0]);
            _matchResults[round] = matchResult;

            // mint fantomMatter to loser
            ICurrencyBlocksMinter(_currenciesMinter).freeMint(IERC721Upgradeable(address(rivalSet[0])).ownerOf(rivalTokenId[0]), CurrencyBlocksConstants.ID_FANTOM_MATTER, 1);
        }

        ICryptoBlocksAttributes(address(rivalSet[0])).setBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_QUEUED, 0);
        ICryptoBlocksAttributes(address(rivalSet[1])).setBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_QUEUED, 0);
        if(fantomMatter[0] > 0) {
            ICryptoBlocksAttributes(address(rivalSet[0])).setBlockAttribute(rivalTokenId[0], RivalsConstants.UINT_USE_FANTOM_MATTER, 0);
        }
        if(fantomMatter[1] > 0) {
            ICryptoBlocksAttributes(address(rivalSet[1])).setBlockAttribute(rivalTokenId[1], RivalsConstants.UINT_USE_FANTOM_MATTER, 0);
        }
    }

    function _updateTopWinners(uint256 dbId_, uint256 wins_) internal {
        uint256[] memory topWinners = _overallTopWinners;
        if(topWinners.length < 5) {
            for(uint256 i; i < topWinners.length; i++) {
                if(dbId_ == topWinners[i]) {
                    return;
                }
            }
            _overallTopWinners.push(dbId_);
        } else {
            uint256 lowestWin;
            uint256 lowestWinIndex;
            for(uint256 i; i < topWinners.length; i++) {
                ICryptoBlocks winner;
                uint256 winnerId;
                (winner, winnerId) = _getRivalsRVL(topWinners[i]);
                uint256 winnerWins;
                // get wins
                if(topWinners[i] != dbId_) {
                    winnerWins = winner.getBlockAttribute(winnerId, RivalsConstants.UINT_WINS);
                } else {
                    // dbId exist in topWinners so no need to kick anyone
                    return;
//                    winnerWins = wins_;
                }
                // update lowestWin
                if(winnerWins < lowestWin || i == 0) {
                    lowestWin = winnerWins;
                    lowestWinIndex = i;
                }
            }
            // we know lowestWin and index
            // this rival didn't make the cut
            if(wins_ < lowestWin) {
                return;
            } else {
                // we kick lowestWin index
                _overallTopWinners[lowestWinIndex] = dbId_;
            }
        }
    }

    function getOverallTopWinners() external view returns(uint256[] memory) {
        return _overallTopWinners;
    }

    function clearOverallTopWinners() external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        delete _overallTopWinners;
    }

    function _getRivalsRVL(uint256 dbId) internal view returns(ICryptoBlocks, uint256) {
        if(dbId < 800) {
            return (ICryptoBlocks(_rivalsRVL[0]), dbId);
        }
        return (ICryptoBlocks(_rivalsRVL[1]), dbId - 800);
    }
    ...
    function setRound(uint256 round_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _round = round_;
    }

    function getMatchResults(uint256 round_) external view returns(MatchResult memory) {
        return _matchResults[round_];
    }

    function setMatchResults(uint256 round_, MatchResult calldata result_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _matchResults[round_] = result_;
    }

    /**
     * @dev returns payable balance associated with rival
     * unused
     */
    /*
    function getRivalBalance(uint256 dbId) external view returns(uint256) {
        ICryptoBlocks rivalsRVL;
        uint256 tokenId;
        (rivalsRVL, tokenId) = _getRivalsRVL(dbId);
        return ICryptoBlocks(rivalsRVL).getBlockAttribute(tokenId, RivalsConstants.UINT_BALANCE);
    }
    */

    function getAllRivalsBalance(address account) external view returns(uint256) {
        uint256 balance;
        for(uint256 set; set < 2; set++) {
            uint256 total = IERC721EnumerableUpgradeable(_rivalsRVL[set]).balanceOf(account);
            for(uint256 i; i < total; i++) {
                uint256 tokenId = IERC721EnumerableUpgradeable(_rivalsRVL[set]).tokenOfOwnerByIndex(account, i);
                balance += ICryptoBlocks(_rivalsRVL[set]).getBlockAttribute(tokenId, RivalsConstants.UINT_BALANCE);
            }
        }
        if(balance > address(this).balance) {
            balance = address(this).balance;
    ...
    /* unused
    function claimRivalBalance(uint256 dbId, uint256 set) external nonReentrant {
        uint256 balance;
        require(IERC721Upgradeable(_rivalsRVL[set]).ownerOf(dbId) == _msgSender(), "Not owner");
        balance = ICryptoBlocks(_rivalsRVL[set]).getBlockAttribute(dbId, RivalsConstants.UINT_BALANCE);
        ICryptoBlocksAttributes(_rivalsRVL[set]).setBlockAttribute(dbId, RivalsConstants.UINT_BALANCE, 0);
        if(balance > address(this).balance) {
            balance = address(this).balance;
        }
    ...
    */
    ...
    function claimAllRivalsBalance() external nonReentrant {
        uint256 balance;
        for(uint256 set; set < 2; set++) {
            uint256 total = IERC721EnumerableUpgradeable(_rivalsRVL[set]).balanceOf(msg.sender);
            for(uint256 i; i < total; i++) {
                uint256 tokenId = IERC721EnumerableUpgradeable(_rivalsRVL[set]).tokenOfOwnerByIndex(msg.sender, i);
                balance += ICryptoBlocks(_rivalsRVL[set]).getBlockAttribute(tokenId, RivalsConstants.UINT_BALANCE);
                ICryptoBlocksAttributes(_rivalsRVL[set]).setBlockAttribute(tokenId, RivalsConstants.UINT_BALANCE, 0);
            }
        }
        if(balance > address(this).balance) {
            balance = address(this).balance;
    ...
    function getRecentWinners() external view returns(uint256[] memory) {
        return _recentWinners;
    }

    function getRecentLosers() external view returns(uint256[] memory) {
        return _recentLosers;
    }

    function setAdminFeeAddress(uint256 index_, address address_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _adminFeeAddress[index_] = address_;
    }

    function getAdminFeeAddress(uint256 index_) external view returns(address) {
        return _adminFeeAddress[index_];
    }

    function setAdminFeeAmount(uint256 index_, uint256 amount_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _adminFeeAmount[index_] = amount_;
    }

    function getAdminFeeAmount(uint256 index_) external view returns(uint256) {
        return _adminFeeAmount[index_];
    }

    function addAdminFee(address account_, uint256 amount_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        // make sure it doesn't exist
        bool exist = false;
        for(uint256 i; i < _adminFeeAddress.length; i++) {
            if(_adminFeeAddress[i] == account_) {
                exist = true;
            }
        }
        if(!exist) {
            _adminFeeAddress.push(account_);
            _adminFeeAmount.push(amount_);
        }
    }

    function removeAdminFee(address account_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        uint256 index;
        for(uint256 i; i < _adminFeeAddress.length; i++) {
            if(account_ == _adminFeeAddress[i]) {
                index = i;
            }
        }

        uint256 lastIndex = _adminFeeAddress.length - 1;
        address lastAddress = _adminFeeAddress[lastIndex];
        uint256 lastAmount = _adminFeeAmount[lastIndex];
        _adminFeeAddress[index] = lastAddress;
        _adminFeeAmount[index] = lastAmount;
        _adminFeeAddress.pop();
        _adminFeeAmount.pop();
    }

    function getRivalsDb() external view returns(address) {
        return _rivalsDb;
    }

    function setRivalsDb(address rivalsDb_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _rivalsDb = rivalsDb_;
    }

    function getRivals(uint256 set_) external view returns(address) {
        return _rivalsRVL[set_];
    }

    function setRivals(address rivals_, uint256 set_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _rivalsRVL[set_] = rivals_;
    }

    function getCurrencies() external view returns(address) {
        return _currencies;
    }

    function setCurrencies(address currencies_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _currencies = currencies_;
    }

    function getCurrenciesMinter() external view returns(address) {
        return _currenciesMinter;
    }

    function setCurrenciesMinter(address currenciesMinter_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _currenciesMinter = currenciesMinter_;
    }

    function setPause(bool pause_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        if(pause_) {
            super._pause();
        } else {
            super._unpause();
    ...
    /**
     * @dev withdraw all balance if there is an issue
     */
    function withdraw() external onlyRole(AccessControlConstants.ADMIN_ROLE) {
     ...
     * @dev withdraw all balance if there is an issue
     */
    function withdrawERC20(address tokenAddress_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        IERC20Upgradeable token = IERC20Upgradeable(tokenAddress_);
        uint256 balance_ = token.balanceOf(address(this));
        token.safeTransfer(msg.sender, balance_);
    }

    /**
     * @dev Random nonce to prevent node validators exploiting
     */
/*    function setRandomNonce(uint256 nonce_) external onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _randomNonce = nonce_;
    }*/
    ...
    function _random(uint256 min_, uint256 max_) internal view returns (uint256) {
        max_++;
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, gasleft(), _randomSeed))) % (max_ - min_);
...
//        _randomNonce++;
     ...
     */
    function _shuffle(uint256[] memory array_) internal view returns(uint256[] memory) {
        for(uint256 i; i < array_.length; i++) {
            uint256 n = i + uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, gasleft(), _randomSeed))) % (array_.length - i);
            uint256 temp = array_[n];
            array_[n] = array_[i];
            array_[i] = temp;
    //        _randomNonce++;
        }
    ...
    /**
     * @dev shuffle array and select one
     * returns selected value and array without the selected value
     */
    function _selectRandom(uint256[] memory array_) internal view returns(uint256, uint256[] memory) {
        array_ = _shuffle(array_);
        uint256 selected = array_[0];

        // error if it is less than 0
        if(array_.length - 1 <= 0) {
            return (selected, array_);
        }
        uint256[] memory newArray = new uint256[](array_.length - 1);
        for(uint256 i = 1; i < array_.length; i++) {
    ...
    /**
     * @dev removes an index from array
     */
    function _removeFromArray(uint256[] memory array_, uint256 index_) internal pure returns(uint256[] memory) {
        // error if it is less than 0
        if(array_.length - 1 < 0) {
            return array_;
        }
        uint256[] memory newArray = new uint256[](array_.length - 1);
        uint256 j;
        for(uint256 i; i < newArray.length; i++) {
            if(j == index_) {
                // skip
                j++;
            }
...
library RivalsBattleConstants {

    /**
     * UINT Mapping IDs
     */
    uint256 public constant UINT_MIN_PARTICIPANTS = 100;
    uint256 public constant UINT_ENTRY_FEE = 200;
