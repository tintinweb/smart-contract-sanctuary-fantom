...
function setRewardTokenContract(address _rewardTokenContract) external onlyOwner {
    ...
    IERC20(rewardTokenContract).mint(address(this), tokenReward);
    ...
    uint256 tokenBalance = IERC20(rewardTokenContract).balanceOf(address(this));
    if (_amount > tokenBalance) {
        IERC20(rewardTokenContract).transfer(_to, tokenBalance);
    } else {
        IERC20(rewardTokenContract).transfer(_to, _amount);
    }
}
