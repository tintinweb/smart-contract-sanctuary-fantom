    ...
    Ownable
{
    ...
    uint256 public maxNftSupply = 1269;
    ...
    constructor()
        ERC721("NFTMember", "MeDMD")
        RandomlyAssigned(maxNftSupply, 1)
    {}
    ...
    function ReserveNFTS(uint256 _amount) public onlyOwner {
        ...
        require(
            supply + _amount <= maxNftSupply,
            "Can't mint more than max NFT supply"
        );
    ...
    function buyNFT_WithERC20(uint256 _amount)
        public
        ensureAvailability
        nonReentrant
    {
        ...
        uint256 supply = totalSupply();
        require(!pausedERC20, "this function is paused");
        require(msg.sender == tx.origin, "Can't mint through another contract");
        require(
            _amount > 0 && _amount <= maxMintPerTx,
            "exceeds the maximum per tx"
        );
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(
            ownerMintedCount + _amount <= maxMintPerAddress,
            "exceeds the maximum per address"
        );
        require(
            supply + _amount <= maxNftSupply,
        ...
        if (msg.sender != owner()) {
            ...
            require(
                tokenContract.balanceOf(msg.sender) >= _totalprice,
                "ERROR USER 404, wrong price sers"
            );
    ...
    function recoverNFT(address _nft, uint256 _tokenId)
        external
        nonReentrant
        onlyOwner
    {
