...
abstract contract BeefySingleHarvesterBase is ManageableUpgradeable, PausableUpgradeable {
    ...
    {
        (bool canHarvestVault, uint256 gasOverhead) = _canHarvestVault(vault_);
    ...
    function _canHarvestVault(IBeefyVault vault_) internal virtual returns (bool canHarvest_, uint256 harvestGasOverhead_) {
        IBeefyStrategy strategy = vault_.strategy();

        // Make sure strategy isn't paused.
        bool isPaused = strategy.paused();

        // Make sure strategy is harvestable.
        bool didHarvest;
        address callFeeRecipient = address(this);
        // Collect gas overhead for profitablity calculation.
    ...
    function _harvestVault(IBeefyVault vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        IBeefyStrategy strategy = vault_.strategy();
