...
// Pooch Farm v2.0 https://poochfund.com
//
// Dependencies include OpenZeppelin & UniSwap contracts as listed below
//
...
contract PoochFarmV2 is Ownable, ReentrancyGuard {
	 ...
	 * EVENTS
	 *****************************************/
	event Deposit(address indexed from, address indexed user, uint256 indexed pid, uint256 amount);
	...
	function fund(uint256 _amount) external {
		...
		rewardToken.safeTransferFrom(address(msg.sender), rewardWallet, _amount);
	}
	...
	function setEarliestRewards(uint256 _value) external onlyOwner {
		require(_value >= block.number, "Earliest reward block must be greater than the current block");
	...
	function poolExists(IERC20 _depositToken, uint256 _lockSeconds) private view returns (bool) {
		for (uint256 pid = 0; pid < rewardPools.length; ++pid) {
			RewardPool storage pool = rewardPools[pid];
	...
	function userPendingPool(uint256 _pid, address _user) public view returns (uint256) {
		...
		if (block.number > userDeposit.lastPayout) blocksElapsed = block.number.sub(userDeposit.lastPayout);

		uint256 poolOwnership = userDeposit.balance.mul(precision).div(pool.depositSum);
		uint256 rewardsDue = blocksElapsed.mul(pool.rewardPerBlock).mul(poolOwnership).div(precision);
		return rewardsDue;
	}
	...
	function getUserState(address _user) external view returns (UserFarmState memory) {
		...
		for (uint256 pid = 0; pid < rewardPools.length; ++pid) {
			...
			if (rewardPools[pid].uniV2Lp == true && rewardPools[pid].selfStake == false) {
				IUniswapV2Pair pair = IUniswapV2Pair(address(rewardPools[pid].depositToken));
	...
	function setLastPayout(UserDeposit storage _deposit) private {
		...
		if (_deposit.lastPayout < earliestRewards) _deposit.lastPayout = earliestRewards; // fair launch accomodation
	}
	...
	function deposit(
		...
		pool.depositToken.safeTransferFrom(address(msg.sender), address(this), _amount); // do the actual deposit
	...
	function restake(uint256 _pid) external nonReentrant {
		...
		require(pool.depositToken == rewardToken, "Restake is only available on single-sided staking");
		...
		pool.depositToken.safeTransferFrom(rewardWallet, address(this), rewardsDue); // move funds from the rewards to this contract
	...
	function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
		...
		if (pool.lockEnforced) require(userDeposit.unlockTime <= block.timestamp, "withdraw: time lock has not passed");
		require(userDeposit.balance >= _amount, "withdraw: can't withdraw more than deposit");
	...
	function emergencyWithdraw(uint256 _pid) external nonReentrant {
		...
		pool.depositToken.safeTransfer(address(msg.sender), userDeposit.balance); // transfer the deposit back to the user
