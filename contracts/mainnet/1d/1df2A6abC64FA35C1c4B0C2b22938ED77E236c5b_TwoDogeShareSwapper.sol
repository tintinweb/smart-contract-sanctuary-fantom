...
https://fdoge.finance/      https://2doge.finance/
*/

contract TwoDogeShareSwapper is Operator {
    ...
    constructor() public {
        tomb = IERC20(0xEb0a2D1b1a33D95204af5d00f65FD9e349419878);
        tbond = IERC20(0xD5A64D724cfE2B1D83313aC6768a589609c19379);
        tshare = IERC20(0xBda29437C8e5dC8BF6a2305D442A3742da7FB033);
        wftmAddress = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
        tombSpookyLpPair = address(0xd0EE9183F8717819c071bD3BDB77df37B7D4d16B);
        tshareSpookyLpPair = address(0xbc9eF8F482ACf57CDa927f6Af39f5c513593aDFb);
        daoAddress = address(0xc6B73D9B6a9E0373d0483fD4E9a52374c8132f04);
    }
    ...
    function getTShareAmountPerTomb() public view returns (uint256) {
        uint256 tombPrice = IERC20(wftmAddress).balanceOf(tombSpookyLpPair).mul(1e18).div(tomb.balanceOf(tombSpookyLpPair));

        uint256 tsharePrice = IERC20(wftmAddress).balanceOf(tshareSpookyLpPair).mul(1e18).div(tshare.balanceOf(tshareSpookyLpPair));
