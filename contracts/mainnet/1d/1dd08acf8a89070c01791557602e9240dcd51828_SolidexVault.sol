...
interface ISolidlyV1Pair is IERC20 {
...
// File: contracts/vaults/ISolidex.sol
...
// File: contracts/vaults/SolidexVault.sol
...
contract SolidexVault is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
  ...
  function initialize(
    ...
    lp = _lp;
    token0 = IERC20Upgradeable(lp.token0());
    token1 = IERC20Upgradeable(lp.token1());
    stable = lp.stable();
    sex = _sex;
    solid = _solid;
    ...
    __ERC20_init(
      string(abi.encodePacked("Ripae Solidex ", lp.symbol())),
      string(abi.encodePacked("ripSolidex-", lp.symbol()))
    );
    __Ownable_init();
  ...
  function yearlyUsdRewards() public view returns (uint) {
    ...
    rewardRate /= gauge.balanceOf(address(lpDepositor));
