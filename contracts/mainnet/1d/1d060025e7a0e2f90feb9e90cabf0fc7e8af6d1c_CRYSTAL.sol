...
contract CRYSTAL is ERC20Burnable, Operator {
    ...
    // uint256 public constant INITIAL_TEAM_FUND_DISTRIBUTION = 10000 ether;
    ...
    function _getCrystalPrice() internal view returns (uint256 _crystalPrice) {
        try IOracle(crystalOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Error: failed to fetch CRYSTAL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _crystalPrice) internal returns (uint256) {
        if (autoCalculateTax) {
    ...
    function setCrystalOracle(address _crystalOracle) public onlyOperatorOrTaxOffice {
        require(_crystalOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCrystalPrice = _getCrystalPrice();
            currentTaxRate = _updateTaxRate(currentCrystalPrice);
            if (currentCrystalPrice < burnThreshold) {
                burnTax = true;
            }
        }
