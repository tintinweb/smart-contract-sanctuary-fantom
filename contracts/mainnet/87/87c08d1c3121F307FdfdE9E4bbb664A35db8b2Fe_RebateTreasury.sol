...
contract RebateTreasury is Ownable {
    ...
    constructor(address scar2, address scar2Oracle, address treasury) {
        Scar2 = IERC20(scar2);
        Scar2Oracle = IOracle(scar2Oracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Scar2 at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 scar2Amount = getScar2Return(token, amount);
        require(scar2Amount <= Scar2.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient scar2 balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + scar2Amount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += scar2Amount;
    }

    // Claim available Scar2 rewards from bonding
    ...
    function setScar2(address scar2) external onlyOwner {
        Scar2 = IERC20(scar2);
    }

    // Set Scar2 oracle

    function setScar2Oracle(address oracle) external onlyOwner {
        Scar2Oracle = IOracle(oracle);
    }
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyOwner {
        require(getScar2Price() < 1e18, "RebateTreasury: unable to buy back");
    ...
    function _claimVested(address account) internal {
        ...
        Scar2.transfer(account, claimable);
    }
    ...
    // Calculate Scar2 return of bonding amount of token

    function getScar2Return(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 scar2Price = getScar2Price();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / scar2Price;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 scar2Price = getScar2Price();
        if (scar2Price < 1e18) return 0;

        uint256 scar2Premium = scar2Price * DENOMINATOR / 1e18 - DENOMINATOR;
        if (scar2Premium < bondThreshold) return 0;
        if (scar2Premium <= secondaryThreshold) {
            return (scar2Premium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (scar2Premium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get SCAR2 price from Oracle

    function getScar2Price() public view returns (uint256) {
        return Scar2Oracle.consult(address(Scar2), 1e18);
    }
    ...
    // Get claimable vested Scar2 for account

    function claimableScar2(address account) external view returns (uint256) {
