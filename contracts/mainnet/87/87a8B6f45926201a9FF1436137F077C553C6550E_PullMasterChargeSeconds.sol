...
import './MasterChargeSeconds.sol';

contract PullMasterChargeSeconds is MasterChargeSeconds {
	...
	function _fundRewardTokens(address recipient, uint256 amount)
		...
		override
	{
		rewardToken.safeTransferFrom(fundAddress, recipient, amount);
	}
}
 ...
 */
abstract contract MasterChargeSeconds is
	...
	ContractWhitelisted
{
	...
	struct PoolInfo {
		...
		uint256 lastRewardTime; // The time of the last pool update
	...
	event RewardsStop(uint256 time);
	 ...
	 */
	function updateStartAndEndTime(uint256 _startTime, uint256 _bonusEndTime)
		external
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
		...
		require(
			...
			'New start time must be lower than new end time'
		);
		require(
			block.timestamp < _startTime,
			'New start time must be higher than current time'
		);
		...
		returns (uint256)
	{
		...
		PoolInfo storage pool = poolInfo[_pid];
		uint256 stakedTokenSupply = pool.strategy.stakedLockedTotal();
		if (block.timestamp > pool.lastRewardTime && stakedTokenSupply != 0) {
	...
	function APR(uint256 _pid) external view override returns (uint256) {
		...
		uint256 reward = rewardPerSecond.mul(31536000).mul(pool.allocPoint).div(
			totalAllocPoint
		);
