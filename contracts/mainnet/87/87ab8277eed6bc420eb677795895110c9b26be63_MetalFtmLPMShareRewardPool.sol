...
contract MetalFtmLPMShareRewardPool {
    using SafeMath for uint256;
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accTSharePerShare = accTSharePerShare.add(_mshareReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _mshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accTSharePerShare = pool.accTSharePerShare.add(_mshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
