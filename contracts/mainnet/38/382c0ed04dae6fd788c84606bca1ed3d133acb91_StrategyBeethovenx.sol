...
// File contracts/utils/StratManager.sol
...
// File contracts/utils/FeeManager.sol
...
abstract contract FeeManager is StratManager {
    ...
    uint constant public TOTAL_FEE_CAP = 2500;
    uint constant public WITHDRAWAL_FEE_CAP = 200;
    uint constant public WITHDRAWAL_MAX = 10000;
    uint public withdrawalFee = 10;

    uint public partnerFee = 0;
    address public partnerAddress = address(0);
    address public yeastAddress = address(0);
    uint public yeastFee = MAX_FEE - partnerFee;

    uint public totalFee = 500; // Fee % of rewards
    ...
    function setPartnetFee(uint256 _fee) public onlyManager {
        require(_fee <= MAX_FEE, "!cap");

        partnerFee = _fee;
        yeastFee = MAX_FEE - partnerFee;
    }
    ...
    function setYeastAddress(address user) public onlyManager {
        yeastAddress = user;
    }

    function setPartnerAddress(address user) public onlyManager {
        partnerAddress = user;
    }
}
...
interface IUniswapV2Pair is IERC20 {
    ...
    function approve() external view returns (address);
}


// File contracts/utils/IMasterChef.sol
...
// File contracts/utils/IUniswapRouterETH.sol
...
// File contracts/utils/IBalancerVault.sol
...
// File contracts/utils/IBeethovenxChef.sol
...
// File contracts/strats/StrategyBeethovenx.sol
...
contract StrategyBeethovenx is StratManager, FeeManager {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (tx.origin != owner() && !paused()) {
            ...
            IERC20(want).safeTransfer(beefyFeeRecipient, withdrawalFeeAmount);
    ...
    function chargeFees() internal {
        ...
        if (input != native) {
            toNative = toNative.mul(totalFee).div(MAX_FEE);
        }
        ...
        if (input == native) {
            nativeBal = nativeBal.mul(totalFee).div(MAX_FEE);
        }

        uint256 yeastFeeAmount = nativeBal.mul(yeastFee).div(MAX_FEE);
        uint256 partnerFeeAmount = nativeBal.mul(partnerFee).div(MAX_FEE);

        if (yeastAddress != address(0)){
            IERC20(native).safeTransfer(yeastAddress, yeastFeeAmount);
        }
        if (partnerAddress != address(0)){
            IERC20(native).safeTransfer(partnerAddress, partnerFeeAmount);
        }
    }
