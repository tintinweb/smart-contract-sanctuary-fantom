...
contract ENCNFT is ERC721URIStorage {
    ...
    mapping(uint256 => EncryptData[]) private _tokenHistory;
    ...
    function mint(string memory _tokenURI, string memory encryptionKey)
        external  returns (uint256)
    {
        ...
        _tokenHistory[newTokenId].push(
            EncryptData(msg.sender, encryptionKey)
        );
    ...
    function transferTokenWithEncryption( 
        ...
        _tokenHistory[_tokenId].push(EncryptData(_to, string(_data)));
        _tokensByOwner[_to].push(_tokenId);
    }
    ...
    {
        EncryptData[] storage allHistoryTokenInfo = _tokenHistory[
            _tokenId
        ];
...
contract Marketplace is ReentrancyGuard {
    ...
    event FeeReceived(uint256 feeValue, uint256 nftId);
    ...
    function withdraw() public {
        require(
            ...
            "Only the owner can withdraw ethers"
        );
    ...
    function makeBet(
        ...
        require(msg.value != 0, "Your bet must be more than 0");
        require(
            msg.value > LISTING_FEE,
            string(abi.encodePacked("Your bet must be more than contract fee"))
        );
    ...
    function getSellerById(
        uint256 idToken
    ) public view returns (SellerData[] memory) {
    ...
    function getBuyersById(
        uint256 idToken
    ) public view returns (BuyerData[] memory) {
    ...
    function acceptRateAndTransferToken(
        ...
        uint256 buyerIndex = getBuyerIndex(idToken, buyerAddress);
        uint256 sellerIndex = getSellerIndex(idToken, msg.sender);
        uint256 pureBet = buyersBoard[idToken][buyerIndex].buyerBet -
            LISTING_FEE;
        ...
        (bool success, bytes memory data) = _marketOwner.call{
            value: LISTING_FEE
        }("Fee transfered to owner");
        require(success, "Transfer fee failed.");
        emit FeeReceived(LISTING_FEE, idToken);
    ...
    function getBuyerIndex(
        uint256 idToken,
        address buyerAddress
    ) public view returns (uint256 buyerIndex) {
    ...
    function getSellerIndex(
        uint256 idToken,
        address sellerAddress
    ) public view returns (uint256 sellerIndex) {
    ...
    function moveTokenForSell(
        ...
        emit NFTListed(_nftContract, idToken, msg.sender, msg.sender, _price);
        sellersBoard[idToken].push(
            SellerData({
    ...
    function removeTokenFromMarketPlace(
        uint256 idToken,
        string memory message
    ) public {
        // to do for burn case
    ...
    function getOwnerInfo(
        uint256 _tokenId,
        address owner
    ) public view returns (string memory balance) {
