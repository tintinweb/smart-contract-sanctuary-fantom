...
import {EnumerableSetUpgradeable} from "oz-upgradeable/utils/structs/EnumerableSetUpgradeable.sol";
...
/**
 * @dev This strategy will supply tokens to Tarot to earn single sided yield
 */
contract ReaperStrategyTarot is ReaperBaseStrategyv4 {
     ...
     * {usedRouters} - A set of routers addresses which are the authorized routers that can be used
    ...
    EnumerableSetUpgradeable.AddressSet private usedRouters;
    ...
    function initialize(
        ...
        address[] memory _keepers,
        address _want,
        RouterPool[] calldata _initialPools,
        address[] calldata _routersToUse,
        uint256 _minWantToDepositOrWithdraw,
        uint256 _maxWantRemainingToRemovePool
    ) public initializer {
        ...
        addUsedRouters(_routersToUse);
        addUsedPools(_initialPools);
    ...
    function _adjustPosition(uint256 _debt) internal override {
        ...
        if (wantBalance > _debt) {
            ...
            _deposit(toReinvest, depositPool);
        }
    }
     ...
     */
    function _deposit(uint256 _amount, address _pool) internal {
        if (_amount >= minWantToDepositOrWithdraw) {
            IERC20Upgradeable(want).safeTransfer(_pool, _amount);
            IBorrowable(_pool).mint(address(this));
        }
    }
    ...
    function _withdraw(uint256 _amountToWithdraw) internal {
        ...
        for (uint256 index = 0; index < nrOfPools; index++) {
            ...
            uint256 bTokenBalance = IBorrowable(currentPool).balanceOf(
                address(this)
            );
            uint256 exchangeRate = IBorrowable(currentPool).exchangeRate();
            uint256 suppliedToPool = (bTokenBalance * exchangeRate) / 1 ether;

            if (suppliedToPool < minWantToDepositOrWithdraw) {
                continue;
            }

            uint256 poolAvailableWant = IERC20Upgradeable(want).balanceOf(
                currentPool
            );
            uint256 ableToPullInUnderlying = MathUpgradeable.min(
                suppliedToPool,
                poolAvailableWant
            );
            uint256 underlyingToWithdraw = MathUpgradeable.min(
                remainingUnderlyingNeeded,
                ableToPullInUnderlying
            );
            ...
            uint256 bTokenToWithdraw = (underlyingToWithdraw * 1 ether) /
                exchangeRate;

            if((bTokenToWithdraw + minWantToDepositOrWithdraw) > bTokenBalance) {
                // Withdraw the full balance to avoid remaining 1 wei pools that cannot be withdrawn from
                bTokenToWithdraw = bTokenBalance; 
            }
    ...
    function rebalance(PoolAllocation[] calldata _allocations) external {
        ...
        for (uint256 index = 0; index < nrOfAllocations; index++) {
            ...
            uint256 wantAvailable = balanceOfWant();
            if (wantAvailable == 0) {
                return;
            }
            ...
            _deposit(depositAmount, pool);
        }
        uint256 wantBalance = balanceOfWant();
        _deposit(wantBalance, depositPool);
    }
    ...
    function _harvestCore(
        uint256 _debt
    ) internal override returns (int256 roi, uint256 repayment) {
        updateExchangeRates();
    ...
    function balanceOfPools() public view returns (uint256 poolBalance) {
        uint256 nrOfPools = usedPools.length();
        for (uint256 index = 0; index < nrOfPools; index++) {
            uint256 wantSupplied = wantSuppliedToPool(usedPools.at(index));
            if (wantSupplied >= minWantToDepositOrWithdraw) { // Do not count balances that cannot be withdrawn
    ...
    function getUsedPools() external view returns (address[] memory) {
        return usedPools.values();
    }

    /**
     * @dev Returns the address for all the currently used routers
     */
    function getUsedRouters() external view returns (address[] memory) {
        return usedRouters.values();
    }
        ...
        returns (uint256 availableBalance)
    {
        ...
        for (uint256 index = 0; index < nrOfPools; index++) {
            ...
            availableBalance += MathUpgradeable.min(
                suppliedToPool,
                poolAvailableWant
            );
        }
    ...
    function getPoolBalances() external view returns (PoolAllocation[] memory) {
        ...
        for (uint256 index = 0; index < nrOfPools; index++) {
            ...
            PoolAllocation memory poolBalance = PoolAllocation(
                poolAddress,
                wantInPool
            );
     ...
     */
    function wantSuppliedToPool(
        address _pool
    ) public view returns (uint256 wantBal) {
    ...
    /**
     * @dev Adds multiple pools at once, using the Tarot factory index (to ensure only Tarot pools can be added)
     */
    function addUsedPools(RouterPool[] calldata _poolsToAdd) public {
        _atLeastRole(KEEPER);
        uint256 nrOfPools = _poolsToAdd.length;
        for (uint256 index = 0; index < nrOfPools; index++) {
            RouterPool memory pool = _poolsToAdd[index];
            address factory = IRouter(pool.router).factory();
            address lpAddress = IFactory(factory).allLendingPools(
                pool.poolIndex
            );
    ...
    function withdrawFromPool(address _pool) external returns (uint256) {
        ...
        if (wantSupplied != 0) {
            ...
            uint256 ableToPullInbToken = (ableToPullInUnderlying * 1 ether) /
                currentExchangeRate;
    ...
    /**
     * @dev Removes a list of pools that will no longer be used.
     */
    function removeUsedPools(address[] calldata _poolsToRemove) external {
        _atLeastRole(STRATEGIST);
        uint256 nrOfPools = _poolsToRemove.length;
        for (uint256 index = 0; index < nrOfPools; index++) {
            address poolToRemove = _poolsToRemove[index];
            require(usedPools.length() > 1, "Must have at least 1 pool");
            require(
                wantSuppliedToPool(poolToRemove) < maxWantRemainingToRemovePool,
                "Want is still supplied"
            );
            require(usedPools.remove(poolToRemove), "Pool not used");
            if (poolToRemove == depositPool) {
     ...
     */
    function setMinWantToDepositOrWithdraw(
        uint256 _minWantToDepositOrWithdraw
    ) external {
        _atLeastRole(STRATEGIST);
        require(_minWantToDepositOrWithdraw != 0, "Can't deposit/withdraw 0");
        minWantToDepositOrWithdraw = _minWantToDepositOrWithdraw;
    }
     ...
     */
    function setMaxWantRemainingToRemovePool(
        uint256 _maxWantRemainingToRemovePool
    ) external {
        _atLeastRole(STRATEGIST);
     ...
     * @dev Sets the maximum amount of pools that can be used at any time
     */
    function addUsedRouters(address[] calldata _routersToUse) public {
        _atLeastRole(DEFAULT_ADMIN_ROLE);
        require(_routersToUse.length != 0, "Cannot set 0 routers");
        for (uint256 index = 0; index < _routersToUse.length; index++) {
            address router = _routersToUse[index];
            require(usedRouters.add(router), "Router already added");
        }
    }

    /**
     * @dev Removes a list of routers that will no longer be used.
     */
    function removeUsedRouters(address[] calldata _routersToRemove) external {
        _atLeastRole(DEFAULT_ADMIN_ROLE);
        uint256 nrOfRouters = _routersToRemove.length;
        for (uint256 index = 0; index < nrOfRouters; index++) {
            address router = _routersToRemove[index];
            require(usedRouters.length() > 1, "Must have at least 1 router");
            require(usedRouters.remove(router), "Router not used");
        }
    }
}