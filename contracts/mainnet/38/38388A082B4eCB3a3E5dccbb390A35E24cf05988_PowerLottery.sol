...
contract RandomNumberClient is Ownable {
  ...
  function fulfillRandomness(bytes32, uint256 _random) external {
    require (msg.sender == address(oracle) || msg.sender == owner(), "Permission denied");
...
contract PowerLottery is RandomNumberClient {
  ...
  uint64 public lockTimeSpan = 12 hours; // lottery is locked for this timespan before choosing winners
  ...
  function draw(bool openNew) external onlyOperator {
    ...
    require (!isOpen, "DRAW - close first");
    ...
    uint totalPrizePower = powerPerTicket * roundSoldTickets * 7 / 10;
    uint totalPrizeEnergy = energyPerTicket * roundSoldTickets * 7 / 10;

    while (delta <= 50 && delta >= -50) {
      uint num = uint16(int16(rn) + 100 - delta) % 100;

      if (id >= 10) {
        if (ticketToUsers[num].length > 0) delete ticketToUsers[num];
        continue;
      }
      ...
      if (ticketToUsers[num].length > 1) {
        for (uint i = 0; i < ticketToUsers[num].length && id < 10; i++) {
          ...
          power.transfer(ticketToUsers[num][i], totalPrizePower * winnerRewards[id] / 10000);
          energy.transfer(ticketToUsers[num][i], totalPrizeEnergy * winnerRewards[id] / 10000);
        ...
        if (userLotteryPlace[ticketToUsers[num][0]][roundId] == 0) {
          ...
          power.transfer(ticketToUsers[num][0], totalPrizePower * winnerRewards[id] / 10000);
          energy.transfer(ticketToUsers[num][0], totalPrizeEnergy * winnerRewards[id] / 10000);
