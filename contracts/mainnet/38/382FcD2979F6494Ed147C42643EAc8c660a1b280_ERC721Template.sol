...
// File contracts/SquaddiesM/SquaddiesM.sol

contract ERC721Template is ERC721Enumerable, ERC721Royalty, ERC721Pausable, AccessControlEnumerable {
    using SafeERC20 for IERC20;
    using Strings for uint256;
    using Address for address;
    using Clones for address;

    event RoyaltyDeployed(address indexed _contract, uint256 indexed _tokenId, address indexed recipient);
    ...
    // supply settings
    uint256 public immutable MAX_SUPPLY = 420;
    uint256 public GANJA_TO_BURN = 3;

    // URI settings
    string private BASE_URI;
    string private URI_EXTENSION = ".json";

    IERC721Enumerable GANJA = IERC721Enumerable(0xcEdEAb32b1D72bd6d0c853bc1a36ed81D42EaFc6);
    IERC721Enumerable TIMMY = IERC721Enumerable(0x82a0aC751c118c7D4DEe71fBb7436862D339e550);

    // Royalty settings
    address public PAYMENT_SPLITTER = address(0xaab3D332F05Af59A3Ada9Df9b7D3CED6EBe6b919);
    ...
    constructor() ERC721("Stoned Sea Turtles", "SST") {
        ...
        baseRoyaltyReceiver.addPayee(0x3833f4e2dbDbdCCb4da61BFfd8189DC30CA528E0, 125);
        baseRoyaltyReceiver.addPayee(0xda80246ca50b6AdE41d03057FcC18384138e1218, 125);
        baseRoyaltyReceiver.addPayee(_msgSender(), 250);
        baseRoyaltyReceiver.addPayee(0x524b862Cb236A99a85D1d0896A8E6852D1406433, 500);

        emit RoyaltyDeployed(address(baseRoyaltyReceiver), 0, 0x524b862Cb236A99a85D1d0896A8E6852D1406433);
    ...
    function setMaxTokensPerMint(uint256 limit) public onlyRole(DEFAULT_ADMIN_ROLE) {
        MAX_TOKENS_PER_MINT = limit;
    }

    function setGanjaToBurn(uint256 burn) public onlyRole(DEFAULT_ADMIN_ROLE) {
        GANJA_TO_BURN = burn;
    }
    ...
    /**
     * @dev returns if approved to take ownership of Ganja
     */
    function isApproved() public view returns (bool) {
        return GANJA.isApprovedForAll(_msgSender(), address(this));
    }

    /****** MINT METHODS ******/

    /**
     * @dev standard mint function that regular users call
     */
    function mint(uint256 count) public whenNotPaused returns (uint256[] memory) {
        require(count > 0, "must mint at least one");
        require(
            count <= MAX_TOKENS_PER_MINT,
            string(abi.encodePacked("Cannot mint more than ", MAX_TOKENS_PER_MINT, " tokens at once"))
        );
        require(
            GANJA.isApprovedForAll(_msgSender(), address(this)),
            "Not permitted to take ownership of Ganja for minting"
        );
        require(TIMMY.balanceOf(_msgSender()) > 0, "Must hold TiMMY to mint");
        uint256 requiredGanja = GANJA_TO_BURN * count;
        require(
            GANJA.balanceOf(_msgSender()) >= requiredGanja,
            string(abi.encodePacked("Must hold at least ", requiredGanja.toString(), " Ganja to mint"))
        );

        uint256[] memory tokenIds = new uint256[](count);

        for (uint256 j = 0; j < count; j++) {
            for (uint256 i = 0; i < GANJA_TO_BURN; i++) {
                // transfer to a burn address 0x01
                GANJA.safeTransferFrom(_msgSender(), address(1), GANJA.tokenOfOwnerByIndex(_msgSender(), 0));
            }

            tokenIds[j] = _mintERC721(_msgSender());
        }
    ...
    /**
     * @dev standard mint function that regular users call with supplying
     * the tokenIds they would like to burn
     */
    function mintSelect(uint256 count, uint256[] memory tokenIdsToBurn)
        public
        whenNotPaused
        returns (uint256[] memory)
    {
        ...
        require(tokenIdsToBurn.length == requiredGanja, "Caller did not supply enough Ganja");
        ...
        for (uint256 j = 0; j < count; j++) {
            for (i = i; i < GANJA_TO_BURN; i++) {
                // transfer to a burn address 0x01
                GANJA.safeTransferFrom(_msgSender(), address(1), tokenIdsToBurn[i]);
            }
