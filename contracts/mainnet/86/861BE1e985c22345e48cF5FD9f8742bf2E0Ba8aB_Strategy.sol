...
contract Strategy is BaseStrategy {
	...
	IUni internal router;

	IERC20 internal constant wftm = IERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83); // wFTM
	...
	constructor(
		...
		alpacaVault = IVault(_alpacaVault);
		masterChef = IMasterChef(_masterChef);
		masterChefPoolId = _masterChefPoolId;
		router = IUni(_router);
		rewardToken = IERC20(0xaD996A45fd2373ed0B10Efa4A8eCB9de445A4302);

		require(masterChef.stakingToken(masterChefPoolId) == _alpacaVault, 'Wrong pool');

		minDepositPeriod = _minDepositPeriod;
		uint256 wantDecimals = ERC20(address(want)).decimals();
		uint256 rewardDecimals = ERC20(address(rewardToken)).decimals();
		rewardsDust = uint256(1).mul(10**uint256(rewardDecimals));
		wantDust = uint256(10).mul(10**uint256(wantDecimals));
		giveAllowances();
	}
	...
	function name() external view override returns (string memory) {
		return string(abi.encodePacked('slending_USDC_Alpaca_strategy'));
	}
	...
	function balanceOfLPInMasterChef() public view returns (uint256) {
		(uint256 _balanceOfLPInMasterChef, ) = masterChef.userInfo(masterChefPoolId, address(this));
		return _balanceOfLPInMasterChef;
	}

	function balanceOfLpTokens() public view returns (uint256) {
		return alpacaVault.balanceOf(address(this));
	}
	...
	function pendingRewards() public view returns (uint256) {
		return masterChef.pendingAlpaca(masterChefPoolId, address(this));
	}

	function estimatedTotalAssets() public view override returns (uint256) {
		// Look at our staked tokens and any free tokens sitting in the strategy.
		return balanceOfPooled().add(balanceOfWant());
	}

	function balanceOfPooled() public view returns (uint256) {
		uint256 ibTokenAmount = balanceOfLpTokens().add(balanceOfLPInMasterChef());
		return ibTokenAmount.mul(alpacaVault.totalToken()).div(alpacaVault.totalSupply());
	}

	function wantToIbToken(uint256 _amount) public view returns (uint256) {
		return _amount.mul(alpacaVault.totalToken()).div(alpacaVault.totalSupply());
	}
	...
	function adjustPosition(uint256 _debtOutstanding) internal override {
		...
		// Claim alpaca rewards.
		claimRewards();
		// If we have significant rewards to sell, then sell some of it.
		sellAllRewards();

		uint256 amountIn = balanceOfWant();

		if (amountIn > wantDust) {
			alpacaVault.deposit(amountIn);
			uint256 toStake = balanceOfLpTokens();
			masterChef.deposit(address(this), masterChefPoolId, toStake);
		...
		returns (uint256 _liquidatedAmount, uint256 _loss)
	{
		...
		if (_amountNeeded > looseAmount) {
			...
			// Remove liquidity to get want tokens.
	...
	function liquidateAllPositions() internal override returns (uint256 _liquidated) {
		...
		alpacaVault.withdraw(balanceOfLpTokens());
	...
	function collectTradingFees() internal {
		...
		uint256 debt = vault.strategies(address(this)).totalDebt;
		if (eta > debt) {
			// Withdraw all lpTokens from masterChef.
			withdrawFromMasterChef();

			uint256 profit = eta.sub(debt);

			removeLiquidity(profit);

			// Put remaining ibUSDC back into masterChef.
			depositLpIntoMasterChef();
		}
	}
	...
	function claimRewards() internal {
		masterChef.harvest(masterChefPoolId);
	}

	function withdrawFromMasterChef() internal {
		uint256 masterChefBal = balanceOfLPInMasterChef();
		if (masterChefBal > 0) {
			masterChef.withdraw(address(this), masterChefPoolId, masterChefBal);
		}
	}

	function depositLpIntoMasterChef() internal {
		if (balanceOfLpTokens() > 0) {
			masterChef.deposit(address(this), masterChefPoolId, balanceOfLpTokens());
		}
	}

	function removeLiquidity(uint256 _wantToExit) internal {
		uint256 lpExitAmount = wantToIbToken(_wantToExit);
		uint256 totalLpBalance = balanceOfLpTokens().add(balanceOfLPInMasterChef());
		uint256 lpToExit = Math.min(lpExitAmount, totalLpBalance);
		alpacaVault.withdraw(lpToExit); // Get want desired out of the ibUSDC.
	}

	function giveAllowances() internal {
		rewardToken.safeApprove(address(router), 0);
		rewardToken.safeApprove(address(router), MAX);
		want.safeApprove(address(alpacaVault), 0);
		want.safeApprove(address(alpacaVault), MAX);
		alpacaVault.approve(address(masterChef), 0);
		alpacaVault.approve(address(masterChef), MAX);
	}
	...
	function emergencyWithdrawFromMasterChef() external onlyVaultManagers {
		withdrawFromMasterChefAndTransfer(address(this));
	}

	function prepareMigration(address _newStrategy) internal override {
		withdrawFromMasterChefAndTransfer(_newStrategy);
	}

	function withdrawFromMasterChefAndTransfer(address _to) internal {
		if (abandonRewards) {
		...
		alpacaVault.transfer(_to, balanceOfLpTokens());
	}
...
interface IVault {
	...
	// Return the Token debt value given the debt share.
	function debtShareToVal(uint256 debtShare) external view returns (uint256);

	// Return the debt share for the given debt value.
	...
	// Add more token to the lending pool.
	function deposit(uint256 amountToken) external;

	// Withdraw token from the lending and burning ibToken.
	function withdraw(uint256 share) external;

	// Pending interest in ibUSDC position.
...
interface IMasterChef {
	...
	function withdraw(
		...
		uint256 _pid,
		uint256 _amount
	) external; // Also use amount = 0 for harvesting rewards.

	function emergencyWithdraw(uint256 pid) external;

	function pendingAlpaca(uint256 pid, address user) external view returns (uint256); // How much pending reward we have.

	function harvest(uint256 _pid) external;

	function stakingToken(uint256 _pid) external returns (address);
