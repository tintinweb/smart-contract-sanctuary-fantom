...
contract ShareRewardPoolHarvest is Ownable {
    using SafeMath for uint256;
    ...
    constructor (
        ...
        chef = _chef;
        shareRewardPool = IShareRewardPool(_chef);
        setPoolSize(_poolSize);
    }

    function setPoolSize(uint256 _poolSize) public onlyOwner {
        poolSize = _poolSize;
    }
    
    function getPool(uint256 _pid) public returns (IShareRewardPool.PoolInfo memory)  {
        return shareRewardPool.poolInfo(_pid);
    }
    
    function harvest(uint256 _pid) public {
        address _sender = msg.sender;
        shareRewardPool.deposit(_pid, 0, _sender);
    }

    function harvestAllRewards() external {
        for (uint256 pid = 0; pid < poolSize; ++pid) {
            IShareRewardPool.PoolInfo memory pool = getPool(pid);
            if (pool.isStarted) {
                harvest(pid);
            }
        }
