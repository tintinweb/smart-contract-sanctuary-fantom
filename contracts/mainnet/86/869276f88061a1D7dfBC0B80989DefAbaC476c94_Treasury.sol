...
contract Treasury is ContractGuard, ReentrancyGuard {
    ...
    modifier checkOperator {
        require(
            IBasisAsset(peg_).operator() == address(this) &&
    ...
    function getPegPrice() public view returns (uint256 pegPrice) {
        try IOracle(pegOracle).consult(peg_, 1e18) returns (uint144 price) {
            ...
            revert("Treasury: failed to consult SRN price from the oracle");
        }
    }
    ...
    function initialize(
        ...
        supplyTiers = [0 ether, 175 ether, 360 ether, 832 ether, 2966 ether, 9171 ether,  13894 ether, 21700];
        maxExpansionTiers = [100, 150, 250, 300, 200, 100, 75, 50];
    ...
    function buyBonds(uint256 _pegAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator nonReentrant {
        ...
        bool mintBondSuccess = IBasisAsset(sbond_).mint(msg.sender, _bondAmount);
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        bool mintPegSuccess = IBasisAsset(peg_).mint(address(this), _amount);
        require(mintPegSuccess, "Treasury: peg minting failed");
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochPegPrice > pegPriceCeiling) {
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedFosbond = _seigniorage.sub(_savedForBoardroom);
                    if (mintingFactorForPayingDebt > 0) {
                        _savedFosbond = _savedFosbond.mul(mintingFactorForPayingDebt).div(10000);
                    }
                }
                ...
                if (_savedFosbond > 0) {
                    seigniorageSaved = seigniorageSaved.add(_savedFosbond);
                    bool mintPegSuccess = IBasisAsset(peg_).mint(address(this), _savedFosbond);
                    require(mintPegSuccess, "Treasury: peg minting failed");
                    emit TreasuryFunded(block.timestamp, _savedFosbond);
                }
            }
        }
    }
