...
contract EsAnzorBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esAnzor;

    constructor(address _esAnzor) public {
        admin = msg.sender;
        esAnzor = _esAnzor;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsAnzorBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esAnzor).transferFrom(msg.sender, _accounts[i], _amounts[i]);
