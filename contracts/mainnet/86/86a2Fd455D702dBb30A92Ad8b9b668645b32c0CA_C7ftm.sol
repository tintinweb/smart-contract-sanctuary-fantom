...
contract C7ftm is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 10000 ether;
    // Initial distribution for the day 2-5 C7FTM-WFTM LP -> C7FTM pool
    // uint256 public constant INITIAL_C7FTM_POOL_DISTRIBUTION = 0 ether;
    // Distribution for DAO FUND
    uint256 public constant INITIAL_DAOFUND_DISTRIBUTION = 100 ether;
    ...
    /**
     * @notice Constructs the C7FTM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("C7FTM", "C7FTM") {
        // Mints 1 C7FTM to contract creator for initial pool setup
    ...
    function _getC7ftmPrice() internal view returns (uint256 _c7ftmPrice) {
        try IOracle(c7ftmOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("C7ftm: failed to fetch C7FTM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _c7ftmPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_c7ftmPrice >= taxTiersTwaps[tierId]) {
    ...
    function setC7ftmOracle(address _c7ftmOracle) public onlyOperatorOrTaxOffice {
        require(_c7ftmOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints C7FTM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of C7FTM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentC7ftmPrice = _getC7ftmPrice();
            currentTaxRate = _updateTaxRate(currentC7ftmPrice);
            if (currentC7ftmPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        //require(_c7ftmPool != address(0), "!_c7ftmPool");
        require(_daoFund != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        //_mint(_c7ftmPool, INITIAL_C7FTM_POOL_DISTRIBUTION);
