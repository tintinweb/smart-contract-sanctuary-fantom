...
// File: contracts/interfaces/StringUtils.sol
...
// File: contracts/interfaces/IStargateChef.sol
...
// File: contracts/StargateSingleStrat.sol
...
contract StargateSingleStrat is Ownable, Pausable {
    ...
    address[] public outputToStakedRoute;
    ...
    event SetOutputToStakedRoute(address[] indexed route, address indexed router);
    ...
    constructor(
        ...
        outputToWrappedRoute = [output, staked, wrapped];
        outputToStakedRoute = [output, staked];

        pendingRewardsFunctionName = "pendingStargate";
    ...
    function _deposit() internal whenNotPaused {
        ...
        IStargateChef(masterchef).deposit(poolId, wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IStargateChef(masterchef).withdraw(poolId, _amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        IStargateChef(masterchef).deposit(poolId, 0);
    ...
    function chargeFees(address caller) internal {                   
        uint256 toWrapped = IERC20(output).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        ...
        if (staked != output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputBal, 0, outputToStakedRoute, address(this), block.timestamp);
        }
        
        uint256 stakedBal = IERC20(staked).balanceOf(address(this));
        IStargateRouter(liquidityRouter).addLiquidity(liquidityId, stakedBal, address(this));
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        if (outputBal > 0) {
            uint256[] memory amountsOut = IUniswapRouterETH(unirouter).getAmountsOut(outputBal, outputToWrappedRoute);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IStargateChef(masterchef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IStargateChef(masterchef).emergencyWithdraw(poolId);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function _giveAllowances() internal {
        ...
        IERC20(staked).safeApprove(liquidityRouter, 0);
        IERC20(staked).safeApprove(liquidityRouter, type(uint).max);
    }
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external onlyOwner {
        require (_path[0] != output && _path[_path.length - 1] == output, "Bad path");

        IUniswapRouterETH(unirouter).swapExactTokensForTokens(_amount, 0, _path, address(this), block.timestamp + 600);
    ...
    function setOutputToWrappedRoute(address[] calldata _route, address _router) external onlyOwner {
        require(_route[0] == output && _route[_route.length - 1] == wrapped, "Bad path");
    ...
    function setOutputToStakedRoute(address[] calldata _route, address _router) external onlyOwner {
        require (_route[0] == wrapped && _route[_route.length - 1] == staked, "Bad path");

        outputToStakedRoute = _route;
        liquidityRouter = _router;

        emit SetOutputToStakedRoute(_route, _router);
    }

    /** @dev Internal function to approve the transaction if the allowance is below transaction amount */
    function approveTxnIfNeeded(address _token, address _spender, uint256 _amount) public onlyOwner {
