 ...
 * @notice vault for auto-compounding curve tokens on pools using a standard MasterChef contract
 * @author YieldWolf
 */
contract ACMasterChefCurve is AutoCompoundVault {
    ...
    function _earnToStake(uint256 _earnAmount) internal override {
        if (liquidityToken != earnToken) {
            _safeSwap(_earnAmount, address(earnToken), address(liquidityToken));
        }

        uint256 liquidityInAmount = liquidityToken.balanceOf(address(this));
        address liquidityPool = ICurveToken(address(stakeToken)).minter();
        liquidityToken.safeIncreaseAllowance(address(liquidityPool), liquidityInAmount);
        uint256[3] memory amounts;
        amounts[liquidityIndex] = liquidityInAmount;
        ICurveLiquidityPool(liquidityPool).add_liquidity(amounts, 1, true);
    }
