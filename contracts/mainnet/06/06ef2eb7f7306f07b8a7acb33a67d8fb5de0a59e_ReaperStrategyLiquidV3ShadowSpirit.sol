...
contract ReaperStrategyLiquidV3ShadowSpirit is ReaperBaseStrategyv3 {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    ISolidlyRouter public constant SPIRIT_ROUTER_V2 = ISolidlyRouter(0x09855B4ef0b9df961ED097EF50172be3e6F13665);
    IShadowChef public chef;
     ...
     * {LQDR} - Reward token.
     * {SPIRIT} - Required for liquidity routing when doing swaps.
     * {linSPIRIT} - Additional reward token.
     * {want} - Address of LP token.
     * {lpToken0} - First underlying token of the LP.
     * {lpToken1} - Second underlying token of the LP.
     */
    ...
    IERC20Upgradeable public constant LQDR = IERC20Upgradeable(0x10b620b2dbAC4Faa7D7FFD71Da486f5D44cd86f9);
    IERC20Upgradeable public constant SPIRIT = IERC20Upgradeable(0x5Cc61A78F164885776AA610fb0FE1257df78E59B);
    IERC20Upgradeable public constant linSPIRIT = IERC20Upgradeable(0xc5713B6a0F26bf0fdC1c52B90cd184D950be515C);
    ISolidlyPair public want;
    IERC20Upgradeable public lpToken0;
    IERC20Upgradeable public lpToken1;

    /**
     * @dev Paths used to swap tokens:
     * {lqdrToUsdcPath} - to swap {LQDR} to {USDC} for charging fees.
     * {linSpiritToUsdcPath} - to swap {linSPIRIT} to {USDC} for charging fees.
     * {lqdrToLpTokenPath} - to swap {LQDR} to one of the LP tokens.
     * {linSpiritToLpTokenPath} - to swap {linSpirit} to one of the LP tokens.
     *
     * {lqdrToLpTokenPath} and {linSpiritToLpTokenPath} must end in the same token of the LP pair.
     * One of them may be of length 1, which indicates that {LQDR} (or {linSPIRIT}) is part of the
     * LP itself.
     */
    address[] public lqdrToUsdcPath;
    address[] public linSpiritToUsdcPath;
    address[] public lqdrToLpTokenPath;
    address[] public linSpiritToLpTokenPath;
    ...
    function initialize(
        ...
        IShadowChef _chef,
        address[] memory _lqdrToLpTokenPath,
        address[] memory _linSpiritToLpTokenPath
    ) public initializer {
        __ReaperBaseStrategy_init(_vault, _feeRemitters, _strategists, _multisigRoles);
        chef = _chef;
        want = ISolidlyPair(chef.lpToken());
        lpToken0 = IERC20Upgradeable(want.token0());
        lpToken1 = IERC20Upgradeable(want.token1());
        lqdrToUsdcPath = [address(LQDR), address(WFTM), address(USDC)];
        lqdrToLpTokenPath = _lqdrToLpTokenPath;
        linSpiritToUsdcPath = [address(linSPIRIT), address(SPIRIT), address(WFTM), address(USDC)];
        linSpiritToLpTokenPath = _linSpiritToLpTokenPath;

        require(lqdrToLpTokenPath.length >= 2 || linSpiritToLpTokenPath.length >= 2, "Both paths can't be singletons");
        require(
            lqdrToLpTokenPath[lqdrToLpTokenPath.length - 1] ==
                linSpiritToLpTokenPath[linSpiritToLpTokenPath.length - 1],
            "Paths should end in same token"
        );
        require(
            lqdrToLpTokenPath[lqdrToLpTokenPath.length - 1] == address(lpToken0) ||
                lqdrToLpTokenPath[lqdrToLpTokenPath.length - 1] == address(lpToken1),
            "Paths must end in either LP token"
        );
    }
    ...
    function _deposit() internal override {
        ...
        if (wantBalance != 0) {
            IERC20Upgradeable(address(want)).safeIncreaseAllowance(address(chef), wantBalance);
            chef.deposit(wantBalance, address(this));
        }
    }
    ...
    function _withdraw(uint256 _amount) internal override {
        ...
        if (wantBal < _amount) {
            chef.withdraw(_amount - wantBal, address(this));
        }

        IERC20Upgradeable(address(want)).safeTransfer(vault, _amount);
    }

    /**
     * @dev Core function of the strat, in charge of collecting and re-investing rewards.
     *      1. Claims {LQDR} and {linSPIRIT} from the {chef}.
     *      2. Claims fees by swapping {LQDR} and {linSPIRIT} to {USDC} using {SPIRIT_ROUTER_V2}.
     *      3. Makes new LP using the remaining {LQDR} and {linSPIRIT}.
     *      4. Re-deposits.
     */
    function _harvestCore() internal override returns (uint256 callerFee) {
        chef.harvest(address(this));
     ...
     *      Charges fees based on the amount of LQDR gained from reward
     */
    function _chargeFees() internal returns (uint256 callerFee) {
        uint256 lqdrFee = (LQDR.balanceOf(address(this)) * totalFee) / PERCENT_DIVISOR;
        uint256 linSpiritFee = (linSPIRIT.balanceOf(address(this)) * totalFee) / PERCENT_DIVISOR;
        uint256 usdcBalBefore = USDC.balanceOf(address(this));
        _swap(lqdrToUsdcPath, lqdrFee);
        _swap(linSpiritToUsdcPath, linSpiritFee);
    ...
    /// @dev Helper function to swap given a {_path} and {_amount}.
    function _swap(address[] memory _path, uint256 _amount) internal {
        if (_amount != 0) {
            ISolidlyRouter.route[] memory routes = new ISolidlyRouter.route[](_path.length - 1);
            uint256 prevRouteOutput = _amount;

            uint256 numRoutes = routes.length;
            for (uint256 i = 0; i < numRoutes; i = _uncheckedInc(i)) {
                (uint256 output, bool useStable) = SPIRIT_ROUTER_V2.getAmountOut(
                    prevRouteOutput,
                    _path[i],
                    _path[i + 1]
                );
                routes[i] = ISolidlyRouter.route({from: _path[i], to: _path[i + 1], stable: useStable});
                prevRouteOutput = output;
            }

            IERC20Upgradeable(_path[0]).safeIncreaseAllowance(address(SPIRIT_ROUTER_V2), _amount);
            SPIRIT_ROUTER_V2.swapExactTokensForTokens(_amount, prevRouteOutput, routes, address(this), block.timestamp);
        }
    }
    ...
    function _addLiquidity() internal {
        if (lqdrToLpTokenPath.length >= 2) {
            _swap(lqdrToLpTokenPath, LQDR.balanceOf(address(this)));
        }

        if (linSpiritToLpTokenPath.length >= 2) {
            _swap(linSpiritToLpTokenPath, linSPIRIT.balanceOf(address(this)));
        }

        address baseToken = lqdrToLpTokenPath[lqdrToLpTokenPath.length - 1];
        address otherToken = address(lpToken0) == baseToken ? address(lpToken1) : address(lpToken0);
        address[] memory baseToOtherToken = new address[](2);
        baseToOtherToken[0] = baseToken;
        baseToOtherToken[1] = otherToken;

        (uint256 reserveA, uint256 reserveB, ) = want.getReserves();
        uint256 baseTokenBal = IERC20Upgradeable(baseToken).balanceOf(address(this));
        uint256 toSwap;
        if (baseToken == address(lpToken0)) {
            toSwap = _getSwapAmount(want, baseTokenBal, reserveA, reserveB, baseToken);
        } else {
            toSwap = _getSwapAmount(want, baseTokenBal, reserveB, reserveA, baseToken);
        }
        _swap(baseToOtherToken, toSwap);

        uint256 lp0Bal = lpToken0.balanceOf(address(this));
        uint256 lp1Bal = lpToken1.balanceOf(address(this));

        if (lp0Bal != 0 && lp1Bal != 0) {
            lpToken0.safeIncreaseAllowance(address(SPIRIT_ROUTER_V2), lp0Bal);
            lpToken1.safeIncreaseAllowance(address(SPIRIT_ROUTER_V2), lp1Bal);
            SPIRIT_ROUTER_V2.addLiquidity(
                address(lpToken0),
                address(lpToken1),
                want.stable(),
                lp0Bal,
    ...
    function balanceOf() public view override returns (uint256) {
        (uint256 amount, ) = chef.userInfo(address(this));
    ...
    function _reclaimWant() internal override {
        chef.emergencyWithdraw(address(this));
    }

    /// @dev Swap whole balance of one of the lp tokens to the other.
    ///     Should only be used if one of the lp tokens accumulates over harvests
    function guardianSwap(address _token) external {
        _atLeastRole(GUARDIAN);
        require(_token == address(lpToken0) || _token == address(lpToken1), "Invalid token");
        ...
        baseToOtherToken[0] = baseToken;
        baseToOtherToken[1] = otherToken;
        _swap(baseToOtherToken, IERC20Upgradeable(baseToken).balanceOf(address(this)));
    }
}
...
interface IShadowChef {
    ...
    function harvestRewardsFromStrategy() external;
    ...
    function overDistributed() external view returns (uint256);
    ...
    function setOverDistributed(uint256 _overDistributed) external;

    function setRewardPerSecond(uint256 _rewardPerSecond) external;

    function setSecondRewarder(address _rewarder, uint256 _pid) external;

    function setSmartWalletChecker(address _checker) external;
