...
// File contracts/RaveFtmEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract RaveFtmEqualizerV2  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToRavePath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToRavePath,
        ...
        for (uint i; i < _equalToRavePath.length; ++i) {
            equalToRavePath.push(_equalToRavePath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToRavePath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wftm).balanceOf(address(this));
        uint256 t2Bal = IERC20(rave).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(wftm, rave, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
