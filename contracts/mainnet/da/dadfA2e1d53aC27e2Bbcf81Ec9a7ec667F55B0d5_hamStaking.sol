...
contract hamStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    function setStakingDestination(address _stakingDestinationAddress) public onlyOwner {
    ...
    function deposit(uint256[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            timer[tokenIds[i]] = block.timestamp + 90 days;
            
        }
    }

      function showIfStaked90(uint256 tokenId) public view returns (bool) {
       if(timer[tokenId] == 0){
           return false;
       }else if(timer[tokenId] <= block.timestamp){
           return true;
       }else {
           return false;
       }
    ...
    function checkTimeOfTokenId(uint256 tokenId) public view returns (uint256) {
      return timer[tokenId];
    }
