...
/**
 * @notice This is a version of TombLens that contains write transactions
 * and pulls reward speeds from a RewardDistributor. TombLensV2 mainly makes calls
 ...
 */
contract TombLensV2 is Exponential {
    /**
    ...
    /**
     * @notice Represents the symbol of the market for the native gas token, such as tFTM
     */
    string public nativeSymbolMarket;

    struct TTokenMetadata {
        address tToken;
        ...
        uint256 tTokenDecimals;
        uint256 underlyingDecimals;
        TombtrollerV1Storage.Version version;
        ...
        uint256 supplyCap;
        uint256 borrowCap;
        uint256 supplyTombRewardsPerSecond;
        uint256 borrowTombRewardsPerSecond;
        uint256 supplyFtmRewardsPerSecond;
        uint256 borrowFtmRewardsPerSecond;
    }

    struct TTokenBalances {
        address tToken;
     ...
     * @param _nativeSymbolMarket Represents the symbol of the market for the native gas token, such as tFTM
     ...
     * @param _tTokens All markets that metadata is being requested for
     * @return The metadata for all markets
     */
    function tTokenMetadataAll(TToken[] calldata _tTokens) external returns (TTokenMetadata[] memory) {
        uint256 tTokenCount = _tTokens.length;
        require(tTokenCount > 0, "invalid input");
        TTokenMetadata[] memory res = new TTokenMetadata[](tTokenCount);
        Tombtroller tombtroller = Tombtroller(address(_tTokens[0].tombtroller()));
        PriceOracle priceOracle = tombtroller.oracle();
        for (uint256 i = 0; i < tTokenCount; i++) {
            require(address(tombtroller) == address(_tTokens[i].tombtroller()), "mismatch tombtroller");
            res[i] = _tTokenMetadataInternal(_tTokens[i], tombtroller, priceOracle);
        }
        return res;
    }
     ...
     * @param _rewardType 0 = TOMB, 1 = FTM
     * @param _tombtroller The tombtroller address
     * @param _tomb The tomb token address
     * @param _account The account that will receive the rewards
     * @return The amount of tokens claimed
     */
    function getClaimableRewards(
        uint8 _rewardType,
        address _tombtroller,
        address _tomb,
        address payable _account
    ) external returns (uint256) {
        require(_rewardType <= 1, "rewardType is invalid");
        if (_rewardType == 0) {
            uint256 balanceBefore = EIP20Interface(_tomb).balanceOf(_account);
            Tombtroller(_tombtroller).claimReward(0, _account);
            uint256 balanceAfter = EIP20Interface(_tomb).balanceOf(_account);
            return sub_(balanceAfter, balanceBefore);
        } else if (_rewardType == 1) {
            uint256 balanceBefore = _account.balance;
            Tombtroller(_tombtroller).claimReward(1, _account);
     ...
     * @param _tToken The market to get metadata for
     * @return The metadata for a market
     */
    function tTokenMetadata(TToken _tToken) external returns (TTokenMetadata memory) {
        Tombtroller tombtroller = Tombtroller(address(_tToken.tombtroller()));
        PriceOracle priceOracle = tombtroller.oracle();
        return _tTokenMetadataInternal(_tToken, tombtroller, priceOracle);
    }

    /**
     * @notice Get market balances for an account
     * @param _tTokens All markets to retrieve account's balances for
     * @param _account The account who's balances are being retrieved
     * @return An account's balances in requested markets
     */
    function tTokenBalancesAll(TToken[] memory _tTokens, address _account) public returns (TTokenBalances[] memory) {
        uint256 tTokenCount = _tTokens.length;
        TTokenBalances[] memory res = new TTokenBalances[](tTokenCount);
        for (uint256 i = 0; i < tTokenCount; i++) {
            res[i] = tTokenBalances(_tTokens[i], _account);
        }
        return res;
    }
     ...
     * @param _tToken The market to retrieve account's balances for
     * @param _account The account who's balances are being retrieved
     * @return An account's balances in a market
     */
    function tTokenBalances(TToken _tToken, address _account) public returns (TTokenBalances memory) {
        TTokenBalances memory vars;
        Tombtroller tombtroller = Tombtroller(address(_tToken.tombtroller()));

        vars.tToken = address(_tToken);
        vars.collateralEnabled = tombtroller.checkMembership(_account, _tToken);

        if (_compareStrings(_tToken.symbol(), nativeSymbolMarket)) {
            vars.underlyingTokenBalance = _account.balance;
            vars.underlyingTokenAllowance = _account.balance;
        } else {
            TErc20 TErc20 = TErc20(address(_tToken));
            EIP20Interface underlying = EIP20Interface(TErc20.underlying());
            vars.underlyingTokenBalance = underlying.balanceOf(_account);
            vars.underlyingTokenAllowance = underlying.allowance(_account, address(_tToken));
        }

        vars.tTokenBalance = _tToken.balanceOf(_account);
        vars.borrowBalanceCurrent = _tToken.borrowBalanceCurrent(_account);

        vars.balanceOfUnderlyingCurrent = _tToken.balanceOfUnderlying(_account);
        PriceOracle priceOracle = tombtroller.oracle();
        uint256 underlyingPrice = priceOracle.getUnderlyingPrice(_tToken);

        (, uint256 collateralFactorMantissa, ) = tombtroller.markets(address(_tToken));
     ...
     */
    function getAccountLimits(Tombtroller _tombtroller, address _account) external returns (AccountLimits memory) {
        AccountLimits memory vars;
        uint256 errorCode;

        (errorCode, vars.liquidity, vars.shortfall) = _tombtroller.getAccountLiquidity(_account);
        require(errorCode == 0, "Can't get account liquidity");

        vars.markets = _tombtroller.getAssetsIn(_account);
        TTokenBalances[] memory tTokenBalancesList = tTokenBalancesAll(vars.markets, _account);
        for (uint256 i = 0; i < tTokenBalancesList.length; i++) {
            vars.totalCollateralValueUSD = add_(vars.totalCollateralValueUSD, tTokenBalancesList[i].collateralValueUSD);
            vars.totalBorrowValueUSD = add_(vars.totalBorrowValueUSD, tTokenBalancesList[i].borrowValueUSD);
        }
     ...
     */
    function _tTokenMetadataInternal(
        TToken _tToken,
        Tombtroller _tombtroller,
        PriceOracle _priceOracle
    ) private returns (TTokenMetadata memory) {
        (bool isListed, uint256 collateralFactorMantissa, TombtrollerV1Storage.Version version) = _tombtroller.markets(
            address(_tToken)
        );
        address underlyingAssetAddress;
        uint256 underlyingDecimals;
        ...
        if (_compareStrings(_tToken.symbol(), nativeSymbolMarket)) {
            ...
            TErc20 TErc20 = TErc20(address(_tToken));
            underlyingAssetAddress = TErc20.underlying();
            underlyingDecimals = EIP20Interface(TErc20.underlying()).decimals();
        }

        if (version == TombtrollerV1Storage.Version.COLLATERALCAP) {
            collateralCap = TCollateralCapErc20Interface(address(_tToken)).collateralCap();
            totalCollateralTokens = TCollateralCapErc20Interface(address(_tToken)).totalCollateralTokens();
        }

        return
            TTokenMetadata({
                tToken: address(_tToken),
                exchangeRateCurrent: _tToken.exchangeRateCurrent(),
                supplyRatePerSecond: _tToken.supplyRatePerSecond(),
                borrowRatePerSecond: _tToken.borrowRatePerSecond(),
                reserveFactorMantissa: _tToken.reserveFactorMantissa(),
                totalBorrows: _tToken.totalBorrows(),
                totalReserves: _tToken.totalReserves(),
                totalSupply: _tToken.totalSupply(),
                totalCash: _tToken.getCash(),
                totalCollateralTokens: totalCollateralTokens,
                isListed: isListed,
                collateralFactorMantissa: collateralFactorMantissa,
                underlyingAssetAddress: underlyingAssetAddress,
                tTokenDecimals: _tToken.decimals(),
                underlyingDecimals: underlyingDecimals,
                version: version,
                collateralCap: collateralCap,
                underlyingPrice: _priceOracle.getUnderlyingPrice(_tToken),
                supplyPaused: _tombtroller.mintGuardianPaused(address(_tToken)),
                borrowPaused: _tombtroller.borrowGuardianPaused(address(_tToken)),
                supplyCap: _tombtroller.supplyCaps(address(_tToken)),
                borrowCap: _tombtroller.borrowCaps(address(_tToken)),
                supplyTombRewardsPerSecond: IRewardDistributor(rewardDistributor).rewardSupplySpeeds(0, address(_tToken)),
                borrowTombRewardsPerSecond: IRewardDistributor(rewardDistributor).rewardBorrowSpeeds(0, address(_tToken)),
                supplyFtmRewardsPerSecond: IRewardDistributor(rewardDistributor).rewardSupplySpeeds(1, address(_tToken)),
                borrowFtmRewardsPerSecond: IRewardDistributor(rewardDistributor).rewardBorrowSpeeds(1, address(_tToken))
            });
    }
...
/**
 * @title Compound's TErc20 Contract
 * @notice TTokens which wrap an EIP-20 underlying
 * @author Compound
 */
contract TErc20 is TToken, TErc20Interface, TProtocolSeizeShareStorage {
    /**
     * @notice Initialize the new money market
     * @param underlying_ The address of the underlying asset
     * @param tombtroller_ The address of the Tombtroller
    ...
    function initialize(
        address underlying_,
        TombtrollerInterface tombtroller_,
        ...
        super.initialize(tombtroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_);
    ...
    function liquidateBorrow(
        ...
        (uint256 err, ) = liquidateBorrowInternal(borrower, repayAmount, tTokenCollateral, false);
        return err;
    }
    ...
    function transferTokens(
        ...
        uint256 allowed = tombtroller.transferAllowed(address(this), src, dst, tokens);
        if (allowed != 0) {
            return failOpaque(Error.TOMBTROLLER_REJECTION, FailureInfo.TRANSFER_TOMBTROLLER_REJECTION, allowed);
        }
        ...
        // unused function
        // tombtroller.transferVerify(address(this), src, dst, tokens);

        return uint256(Error.NO_ERROR);
    }

    /**
     * @notice Get the account's tToken balances
     * @param account The address of the account
     */
    function getTTokenBalanceInternal(address account) internal view returns (uint256) {
    ...
    function mintFresh(
        ...
        uint256 allowed = tombtroller.mintAllowed(address(this), minter, mintAmount);
        if (allowed != 0) {
            return (failOpaque(Error.TOMBTROLLER_REJECTION, FailureInfo.MINT_TOMBTROLLER_REJECTION, allowed), 0);
        }

        /*
         * Return if mintAmount is zero.
         * Put behind `mintAllowed` for accuring potential TOMB rewards.
         */
        if (mintAmount == 0) {
        ...
        // unused function
        // tombtroller.mintVerify(address(this), minter, vars.actualMintAmount, vars.mintTokens);
     ...
     * @param redeemTokensIn The number of tTokens to redeem into underlying
     * @param redeemAmountIn The number of underlying tokens to receive from redeeming tTokens
     * @param isNative The amount is in native or not
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFresh(
        ...
        uint256 allowed = tombtroller.redeemAllowed(address(this), redeemer, vars.redeemTokens);
        if (allowed != 0) {
            return failOpaque(Error.TOMBTROLLER_REJECTION, FailureInfo.REDEEM_TOMBTROLLER_REJECTION, allowed);
        }

        /*
         * Return if redeemTokensIn and redeemAmountIn are zero.
         * Put behind `redeemAllowed` for accuring potential TOMB rewards.
         */
        ...
        /* We call the defense hook */
        tombtroller.redeemVerify(address(this), redeemer, vars.redeemAmount, vars.redeemTokens);
    ...
    function seizeInternal(
        ...
        uint256 allowed = tombtroller.seizeAllowed(address(this), seizerToken, liquidator, borrower, seizeTokens);
        if (allowed != 0) {
            return failOpaque(Error.TOMBTROLLER_REJECTION, FailureInfo.LIQUIDATE_SEIZE_TOMBTROLLER_REJECTION, allowed);
        }

        /*
         * Return if seizeTokens is zero.
         * Put behind `seizeAllowed` for accuring potential TOMB rewards.
         */
        if (seizeTokens == 0) {
            return uint256(Error.NO_ERROR);
        }
        ...
        /* We call the defense hook */
        // unused function
        // tombtroller.seizeVerify(address(this), seizerToken, liquidator, borrower, seizeTokens);
...
import "./TombtrollerInterface.sol";
...
contract TToken is TTokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(
        ...
        // Set the tombtroller
        uint256 err = _setTombtroller(tombtroller_);
        require(err == uint256(Error.NO_ERROR), "setting tombtroller failed");

        // Initialize block timestamp and borrow index (block timestamp mocks depend on tombtroller being set)
     ...
     * @dev This is used by tombtroller to more efficiently perform liquidity checks.
    ...
    {
        uint256 tTokenBalance = getTTokenBalanceInternal(account);
    ...
    /**
     * @notice Returns the current per-sec borrow interest rate for this tToken
    ...
    /**
     * @notice Returns the current per-sec supply interest rate for this tToken
    ...
    /**
     * @notice Returns the estimated per-sec borrow interest rate for this TToken after some change
     * @return The borrow interest rate per sec, scaled by 1e18
     */
    ...
    /**
     * @notice Returns the estimated per-sec supply interest rate for this TToken after some change
     * @return The supply interest rate per sec, scaled by 1e18
     */
    ...
    function borrowFresh(
        ...
        uint256 allowed = tombtroller.borrowAllowed(address(this), borrower, borrowAmount);
        if (allowed != 0) {
            return failOpaque(Error.TOMBTROLLER_REJECTION, FailureInfo.BORROW_TOMBTROLLER_REJECTION, allowed);
        }

        /*
         * Return if borrowAmount is zero.
         * Put behind `borrowAllowed` for accuring potential TOMB rewards.
         */
        if (borrowAmount == 0) {
        ...
        /* We call the defense hook */
        // unused function
        // tombtroller.borrowVerify(address(this), borrower, borrowAmount);
    ...
    function repayBorrowFresh(
        ...
        uint256 allowed = tombtroller.repayBorrowAllowed(address(this), payer, borrower, repayAmount);
        if (allowed != 0) {
            return (failOpaque(Error.TOMBTROLLER_REJECTION, FailureInfo.REPAY_BORROW_TOMBTROLLER_REJECTION, allowed), 0);
        }

        /*
         * Return if repayAmount is zero.
         * Put behind `repayBorrowAllowed` for accuring potential TOMB rewards.
         */
        if (repayAmount == 0) {
            accountBorrows[borrower].principal = borrowBalanceStoredInternal(borrower);
            accountBorrows[borrower].interestIndex = borrowIndex;
        ...
        /* We call the defense hook */
        // unused function
        // tombtroller.repayBorrowVerify(address(this), payer, borrower, vars.actualRepayAmount, vars.borrowerIndex);
    ...
    function liquidateBorrowInternal(
        ...
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, tTokenCollateral, isNative);
    }
    ...
    function liquidateBorrowFresh(
        ...
        uint256 allowed = tombtroller.liquidateBorrowAllowed(
            address(this),
        ...
        if (allowed != 0) {
            return (failOpaque(Error.TOMBTROLLER_REJECTION, FailureInfo.LIQUIDATE_TOMBTROLLER_REJECTION, allowed), 0);
        }
        ...
        (uint256 amountSeizeError, uint256 seizeTokens) = tombtroller.liquidateCalculateSeizeTokens(
            address(this),
            address(tTokenCollateral),
            actualRepayAmount
        );
        require(amountSeizeError == uint256(Error.NO_ERROR), "LIQUIDATE_TOMBTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
        ...
        /* We call the defense hook */
        // unused function
        // tombtroller.liquidateBorrowVerify(address(this), address(tTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    /**
     * @notice Sets a new tombtroller for the market
     * @dev Admin function to set a new tombtroller
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setTombtroller(TombtrollerInterface newTombtroller) public returns (uint256) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_TOMBTROLLER_OWNER_CHECK);
        }

        TombtrollerInterface oldTombtroller = tombtroller;
        // Ensure invoke tombtroller.isTombtroller() returns true
        require(newTombtroller.isTombtroller(), "marker method returned false");

        // Set market's tombtroller to newTombtroller
        tombtroller = newTombtroller;

        // Emit NewTombtroller(oldTombtroller, newTombtroller)
        emit NewTombtroller(oldTombtroller, newTombtroller);
    ...
    function _reduceReservesFresh(uint256 reduceAmount) internal returns (uint256) {
        ...
        // doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
        // Restrict reducing reserves in native token. Implementations except `TWrappedNative` won't use parameter `isNative`.
...
interface IRewardDistributor {
    /**
     * @notice Get TOMB/FTM reward supply speed for a single market
     * @param rewardType 0 = TOMB, 1 = FTM
     * @param tToken The market whose reward speed to get
     * @return The supply reward speed for the market/type
     */
    function rewardSupplySpeeds(uint8 rewardType, address tToken) external view returns (uint256);

    /**
     * @notice Get TOMB/FTM reward borrow speed for a single market
     * @param rewardType 0 = TOMB, 1 = FTM
     * @param tToken The market whose reward speed to get
     * @return The borrow reward speed for the market/type
     */
    function rewardBorrowSpeeds(uint8 rewardType, address tToken) external view returns (uint256);

    /**
     * @notice Claim all the TOMB/FTM accrued by holder in all markets
     * @param rewardType 0 = TOMB, 1 = FTM
     * @param holder The address to claim TOMB/FTM for
     * @dev This is only for RewardDistributor V1
     */
    function claimReward(uint8 rewardType, address payable holder) external;

    /**
     * @notice The TOMB/FTM accrued but not yet transferred to each user     
     * @param rewardType 0 = TOMB, 1 = FTM
     * @param holder The address to claim TOMB/FTM for
     * @return The TOMB/FTM accrued but not yet transferred to each user 
     */
...
import "./PriceOracle/PriceOracle.sol";
import "./TombtrollerInterface.sol";
import "./TombtrollerStorage.sol";
import "./RewardDistributor.sol";
import "./Unitroller.sol";

/**
 * @title Compound's Tombtroller Contract
 * @author Compound (modified by Cream)
 */
contract Tombtroller is TombtrollerV1Storage, TombtrollerInterface, TombtrollerErrorReporter, Exponential {
    /// @notice Emitted when an admin supports a market
    event MarketListed(TToken tToken);

    /// @notice Emitted when an admin delists a market
    event MarketDelisted(TToken tToken);
    ...
    /// @notice Emitted when tToken version is changed
    event NewTTokenVersion(TToken tToken, Version oldVersion, Version newVersion);
     ...
     */
    function enterMarkets(address[] memory tTokens) public returns (uint256[] memory) {
    ...
    function addToMarketInternal(TToken tToken, address borrower) internal returns (Error) {
        ...
        if (marketToJoin.version == Version.COLLATERALCAP) {
            ...
            TCollateralCapErc20Interface(address(tToken)).registerCollateral(borrower);
        }
     ...
     */
    function exitMarket(address tTokenAddress) external returns (uint256) {
        ...
        Market storage marketToExit = markets[tTokenAddress];

        if (marketToExit.version == Version.COLLATERALCAP) {
            TCollateralCapErc20Interface(tTokenAddress).unregisterCollateral(msg.sender);
        }
     ...
     * @param tTokenAddress The address of the asset to be checked
     * @return Whether or not the market is listed
     */
    function isMarketListed(address tTokenAddress) public view returns (bool) {
        return markets[tTokenAddress].isListed;
    }
    ...
    function mintAllowed(
        ...
        if (!isMarketListed(tToken)) {
            return uint256(Error.MARKET_NOT_LISTED);
        }

        if(enabledSingleSupplier[tToken]){
            require(canSupply[tToken][minter], "minter is not allowed");
        }
        ...
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(tToken, minter);
    ...
    function redeemAllowed(
        ...
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(tToken, redeemer);
        return uint256(Error.NO_ERROR);
    }
    ...
    function borrowAllowed(
        ...
        if (!markets[tToken].accountMembership[borrower]) {
            ...
            Error err = addToMarketInternal(TToken(tToken), borrower);
            if (err != Error.NO_ERROR) {
                return uint256(err);
            }
        ...
        RewardDistributor(rewardDistributor).updateAndDistributeBorrowerRewardsForToken(tToken, borrower, borrowIndex);
    ...
    function liquidateBorrowAllowed(
        ...
        if (!isMarketListed(tTokenBorrowed) || !isMarketListed(tTokenCollateral)) {
            return uint256(Error.MARKET_NOT_LISTED);
        }
    ...
    function seizeAllowed(
        ...
        if (!isMarketListed(tTokenCollateral) || !isMarketListed(tTokenBorrowed)) {
            return uint256(Error.MARKET_NOT_LISTED);
        }

        if (TToken(tTokenCollateral).tombtroller() != TToken(tTokenBorrowed).tombtroller()) {
            return uint256(Error.TOMBTROLLER_MISMATCH);
        }

        // Keep the flywheel moving
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(tTokenCollateral, borrower);
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(tTokenCollateral, liquidator);
    ...
    function transferAllowed(
        ...
        // Keep the flywheel moving
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(tToken, src);
        RewardDistributor(rewardDistributor).updateAndDistributeSupplierRewardsForToken(tToken, dst);
        return uint256(Error.NO_ERROR);
    }
    ...
    function flashloanAllowed(
        ...
        return !flashloanGuardianPaused[tToken];
    }

    /**
     * @notice Update TToken's version.
     * @param tToken Version of the asset being updated
     * @param newVersion The new version
     */
    function updateTTokenVersion(address tToken, Version newVersion) external {
        require(msg.sender == tToken, "only tToken could update its version");

        // This function will be called when a new TToken implementation becomes active.
        // If a new TToken is newly created, this market is not listed yet. The version of
        // this market will be taken care of when calling `_supportMarket`.
        if (isMarketListed(tToken)) {
            Version oldVersion = markets[tToken].version;
            markets[tToken].version = newVersion;

            emit NewTTokenVersion(TToken(tToken), oldVersion, newVersion);
        }
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            (oErr, vars.tTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset.getAccountSnapshot(
                account
            );
            if (oErr != 0) {
            ...
            if (vars.tTokenBalance == 0 && vars.borrowBalance == 0 && asset != tTokenModify) {
                continue;
            }
    ...
    function _setSingleSupplierPool(TToken tToken, bool value) public returns (uint256) {
        if (msg.sender != admin) {
            return uint256(Error.UNAUTHORIZED);
        }

        enabledSingleSupplier[address(tToken)] = value;
        return uint256(Error.NO_ERROR);
    }

    function _addSingleSupplierToPool(TToken tToken, address supplier, bool value) public returns (uint256) {
        if (msg.sender != admin) {
            return uint256(Error.UNAUTHORIZED);
        }

        canSupply[address(tToken)][supplier] = value;
        return uint256(Error.NO_ERROR);
    }
    ...
    /**
     * @notice Sets a new price oracle for the tombtroller
    ...
    function _setPriceOracle(PriceOracle newOracle) public returns (uint256) {
        ...
        // Track the old oracle for the tombtroller
        PriceOracle oldOracle = oracle;

        // Set tombtroller's oracle to newOracle
        oracle = newOracle;
     ...
     */
    function _supportMarket(TToken tToken, Version version) external returns (uint256) {
        require(msg.sender == admin, "only admin may support market");
        require(!isMarketListed(address(tToken)), "market already listed");

        tToken.isTToken(); // Sanity check to make sure its really a TToken

        markets[address(tToken)] = Market({isListed: true, collateralFactorMantissa: 0, version: version});
     ...
     * @param tToken The address of the market (token) to delist
     */
    function _delistMarket(TToken tToken) external {
        require(msg.sender == admin, "only admin may delist market");
        require(isMarketListed(address(tToken)), "market not listed");
        require(tToken.totalSupply() == 0, "market not empty");

        tToken.isTToken(); // Sanity check to make sure its really a TToken

        delete markets[address(tToken)];

        for (uint256 i = 0; i < allMarkets.length; i++) {
            if (allMarkets[i] == tToken) {
    ...
    /**
     * @notice Set the given supply caps for the given tToken markets. Supplying that brings total supplys to or above supply cap will revert.
    ...
    function _setMintPaused(TToken tToken, bool state) public returns (bool) {
        require(isMarketListed(address(tToken)), "cannot pause a market that is not listed");
    ...
    function _setFlashloanPaused(TToken tToken, bool state) public returns (bool) {
        require(isMarketListed(address(tToken)), "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == admin, "only pause guardian and admin can pause");
        require(msg.sender == admin || state == true, "only admin can unpause");

        flashloanGuardianPaused[address(tToken)] = state;
        emit ActionPaused(tToken, "Flashloan", state);
        return state;
    }
    ...
    /**
     * @notice Claim all the TOMB/FTM accrued by holder in the specified markets
     * @param holder The address to claim TOMB/FTM for
     * @param tTokens The list of markets to claim TOMB/FTM in
     */
    function claimReward(
        uint8 rewardType,
        address payable holder,
        TToken[] memory tTokens
    ) public {
        RewardDistributor(rewardDistributor).claimReward(rewardType, holder, tTokens);
    }

    /**
     * @notice Claim all TOMB/FTM  accrued by the holders
     * @param rewardType  0 = TOMB, 1 = FTM
     * @param holders The addresses to claim TOMB/FTM for
     * @param tTokens The list of markets to claim TOMB/FTM in
     * @param borrowers Whether or not to claim TOMB/FTM earned by borrowing
     * @param suppliers Whether or not to claim TOMB/FTM earned by supplying
     */
    function claimReward(
        uint8 rewardType,
        ...
        RewardDistributor(rewardDistributor).claimReward(rewardType, holders, tTokens, borrowers, suppliers);
    }
}
...
contract TombtrollerInterface {
    /// @notice Indicator that this is a Tombtroller contract (for inspection)
    bool public constant isTombtroller = true;
...
interface TombtrollerInterfaceExtension {
    function checkMembership(address account, TToken tToken) external view returns (bool);

    function updateTTokenVersion(address tToken, TombtrollerV1Storage.Version version) external;
...
contract TTokenStorage {
     ...
     */
    TombtrollerInterface public tombtroller;
...
contract TSupplyCapStorage {
    /**
     * @notice Internal cash counter for this TToken. Should equal underlying.balanceOf(address(this)) for CERC20.
     */
    uint256 public internalCash;
}

contract TCollateralCapStorage {
    /**
    ...
    /**
     * @notice Collateral cap for this TToken, zero for no cap.
     */
...
contract TTokenInterface is TTokenStorage {
    ...
    /**
     * @notice Event emitted when tombtroller is changed
     */
    event NewTombtroller(TombtrollerInterface oldTombtroller, TombtrollerInterface newTombtroller);
...
contract TWrappedNativeInterface is TErc20Interface {
    /**
    ...
    function liquidateBorrowNative(address borrower, TTokenInterface tTokenCollateral)
        external
        payable
        returns (uint256);
...
contract TCapableErc20Interface is TErc20Interface, TSupplyCapStorage {
    /**
     * @notice Flash loan fee ratio
     */
    uint256 public constant flashFeeBips = 8;
...
contract TCollateralCapErc20Interface is TCapableErc20Interface, TCollateralCapStorage {
    /*** Admin Events ***/
...
contract TProtocolSeizeShareStorage {
    /**
...
contract TombtrollerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        TOMBTROLLER_MISMATCH,
...
contract TokenErrorReporter {
    enum Error {
        ...
        UNAUTHORIZED,
        BAD_INPUT,
        TOMBTROLLER_REJECTION,
        TOMBTROLLER_CALCULATION_ERROR,
    ...
    enum FailureInfo {
        ...
        BORROW_TOMBTROLLER_REJECTION,
        LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        LIQUIDATE_TOMBTROLLER_REJECTION,
        LIQUIDATE_TOMBTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
        LIQUIDATE_CLOSE_AMOUNT_IS_UINT_MAX,
        LIQUIDATE_CLOSE_AMOUNT_IS_ZERO,
        LIQUIDATE_FRESHNESS_CHECK,
        LIQUIDATE_LIQUIDATOR_IS_BORROWER,
        LIQUIDATE_REPAY_BORROW_FRESH_FAILED,
        LIQUIDATE_SEIZE_TOMBTROLLER_REJECTION,
        LIQUIDATE_SEIZE_LIQUIDATOR_IS_BORROWER,
        LIQUIDATE_SEIZE_TOO_MUCH,
        MINT_ACCRUE_INTEREST_FAILED,
        MINT_TOMBTROLLER_REJECTION,
        MINT_FRESHNESS_CHECK,
        MINT_TRANSFER_IN_FAILED,
        MINT_TRANSFER_IN_NOT_POSSIBLE,
        REDEEM_ACCRUE_INTEREST_FAILED,
        REDEEM_TOMBTROLLER_REJECTION,
        ...
        REPAY_BORROW_TOMBTROLLER_REJECTION,
        REPAY_BORROW_FRESHNESS_CHECK,
        REPAY_BORROW_TRANSFER_IN_NOT_POSSIBLE,
        SET_COLLATERAL_FACTOR_OWNER_CHECK,
        SET_COLLATERAL_FACTOR_VALIDATION,
        SET_TOMBTROLLER_OWNER_CHECK,
        ...
        TRANSFER_TOMBTROLLER_REJECTION,
...
contract TombtrollerV1Storage is UnitrollerAdminStorage {
    /**
    ...
    // @notice L3USD pool must only be available for borrowing 
    mapping (address => bool) public enabledSingleSupplier;

    // @notice defines who can supply to L3USD pool 
    mapping (address => mapping (address => bool)) public canSupply;
...
/**
 * @title TombtrollerCore
 * @dev Storage for the tombtroller is at this address, while execution is delegated to the `implementation`.
 * TTokens should reference this contract as their tombtroller.
 */
contract Unitroller is UnitrollerAdminStorage, TombtrollerErrorReporter {
    /**
    ...
    /**
     * @notice Emitted when pendingImplementation is accepted, which means tombtroller implementation is updated
     */
    event NewImplementation(address oldImplementation, address newImplementation);
    ...
    /**
     * @notice Accepts new implementation of tombtroller. msg.sender must be pendingImplementation
...
contract RewardDistributorStorage {
     ...
     * @notice Active brains of Unitroller
     */
    Tombtroller public tombtroller;
    ...
    /// @notice The TOMB/FTM market supply state for each market
    mapping(uint8 => mapping(address => RewardMarketState)) public rewardSupplyState;

    /// @notice The TOMB/FTM market borrow state for each market
    mapping(uint8 => mapping(address => RewardMarketState)) public rewardBorrowState;

    /// @notice The TOMB/FTM borrow index for each market for each supplier as of the last time they accrued reward
    mapping(uint8 => mapping(address => mapping(address => uint256))) public rewardSupplierIndex;

    /// @notice The TOMB/FTM borrow index for each market for each borrower as of the last time they accrued reward
    ...
    /// @notice TOMB token contract address
    address public tombAddress;
}
...
contract RewardDistributor is RewardDistributorStorage, Exponential {
    ...
    event RewardSupplySpeedUpdated(uint8 rewardType, TToken indexed tToken, uint256 newSpeed);

    /// @notice Emitted when a new reward borrow speed is calculated for a market
    event RewardBorrowSpeedUpdated(uint8 rewardType, TToken indexed tToken, uint256 newSpeed);

    /// @notice Emitted when TOMB/FTM is distributed to a supplier
    ...
    /// @notice Emitted when TOMB/FTM is distributed to a borrower
    ...
    /// @notice Emitted when TOMB is granted by admin
    ...
    function initialize() public {
        ...
        tombtroller = Tombtroller(msg.sender);
        initialized = true;
    }
     ...
     */
    function adminOrInitializing() internal view returns (bool) {
        return msg.sender == admin || msg.sender == address(tombtroller);
    }

    /**
     * @notice Set TOMB/FTM speed for a single market
     * @param rewardType 0 = QI, 1 = FTM
    ...
    function _setRewardSpeed(
        ...
        setRewardSpeedInternal(rewardType, tToken, rewardSupplySpeed, rewardBorrowSpeed);
    }
     ...
     * @param newSupplySpeed New TOMB or FTM supply speed for market
     * @param newBorrowSpeed New TOMB or FTM borrow speed for market
     */
    function setRewardSpeedInternal(
        ...
        if (currentRewardSupplySpeed != 0) {
            // note that TOMB speed could be set to 0 to halt liquidity rewards for a market
            updateRewardSupplyIndex(rewardType, address(tToken));
        } else if (newSupplySpeed != 0) {
            // Add the TOMB market
            require(tombtroller.isMarketListed(address(tToken)), "reward market is not listed");

            if (
                rewardSupplyState[rewardType][address(tToken)].index == 0 &&
                rewardSupplyState[rewardType][address(tToken)].timestamp == 0
            ) {
                rewardSupplyState[rewardType][address(tToken)] = RewardMarketState({
        ...
        if (currentRewardSupplySpeed != newSupplySpeed) {
            rewardSupplySpeeds[rewardType][address(tToken)] = newSupplySpeed;
            emit RewardSupplySpeedUpdated(rewardType, tToken, newSupplySpeed);
        }
        ...
        if (currentRewardBorrowSpeed != 0) {
            ...
            updateRewardBorrowIndex(rewardType, address(tToken), borrowIndex);
        } else if (newBorrowSpeed != 0) {
            // Add the TOMB market
            require(tombtroller.isMarketListed(address(tToken)), "reward market is not listed");

            if (
                rewardBorrowState[rewardType][address(tToken)].index == 0 &&
                rewardBorrowState[rewardType][address(tToken)].timestamp == 0
            ) {
                rewardBorrowState[rewardType][address(tToken)] = RewardMarketState({
                    index: rewardInitialIndex,
                    timestamp: safe32(getBlockTimestamp(), "block timestamp exceeds 32 bits")
                });
            }
        ...
        if (currentRewardBorrowSpeed != newBorrowSpeed) {
            rewardBorrowSpeeds[rewardType][address(tToken)] = newBorrowSpeed;
            emit RewardBorrowSpeedUpdated(rewardType, tToken, newBorrowSpeed);
        }
    }

    /**
     * @notice Accrue TOMB/FTM to the market by updating the supply index
     * @param rewardType  0: TOMB, 1: FTM
     * @param tToken The market whose supply index to update
     */
    function updateRewardSupplyIndex(uint8 rewardType, address tToken) internal {
        require(rewardType <= 1, "rewardType is invalid");
        RewardMarketState storage supplyState = rewardSupplyState[rewardType][tToken];
        uint256 supplySpeed = rewardSupplySpeeds[rewardType][tToken];
        uint256 blockTimestamp = getBlockTimestamp();
        uint256 deltaTimestamps = sub_(blockTimestamp, uint256(supplyState.timestamp));
        if (deltaTimestamps > 0 && supplySpeed > 0) {
            uint256 supplyTokens = TToken(tToken).totalSupply();
            ...
            rewardSupplyState[rewardType][tToken] = RewardMarketState({
    ...
    /**
     * @notice Accrue TOMB/FTM to the market by updating the borrow index
     * @param rewardType  0: TOMB, 1: FTM
    ...
    function updateRewardBorrowIndex(
        ...
        require(rewardType <= 1, "rewardType is invalid");
        RewardMarketState storage borrowState = rewardBorrowState[rewardType][tToken];
        uint256 borrowSpeed = rewardBorrowSpeeds[rewardType][tToken];
        uint256 blockTimestamp = getBlockTimestamp();
        uint256 deltaTimestamps = sub_(blockTimestamp, uint256(borrowState.timestamp));
        if (deltaTimestamps > 0 && borrowSpeed > 0) {
            uint256 borrowAmount = div_(TToken(tToken).totalBorrows(), marketBorrowIndex);
            ...
            rewardBorrowState[rewardType][tToken] = RewardMarketState({
                index: safe224(index.mantissa, "new index exceeds 224 bits"),
                timestamp: safe32(blockTimestamp, "block timestamp exceeds 32 bits")
            });
        } else if (deltaTimestamps > 0) {
    ...
    /**
     * @notice Calculate TOMB/FTM accrued by a supplier and possibly transfer it to them
     * @param rewardType  0: TOMB, 1: FTM
     * @param tToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute TOMB/FTM to
     */
    function distributeSupplierReward(
        ...
        Double memory supplierIndex = Double({mantissa: rewardSupplierIndex[rewardType][tToken][supplier]});
        rewardSupplierIndex[rewardType][tToken][supplier] = supplyIndex.mantissa;
        ...
        uint256 supplierTokens = TToken(tToken).balanceOf(supplier);
        ...
        emit DistributedSupplierReward(rewardType, TToken(tToken), supplier, supplierDelta, supplyIndex.mantissa);
    }

    /**
     * @notice Calculate TOMB/FTM accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param rewardType  0: TOMB, 1: FTM
     * @param tToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute TOMB/FTM to
     * @param marketBorrowIndex Current index of the borrow market
     */
    function distributeBorrowerReward(
        ...
        Double memory borrowerIndex = Double({mantissa: rewardBorrowerIndex[rewardType][tToken][borrower]});
        rewardBorrowerIndex[rewardType][tToken][borrower] = borrowIndex.mantissa;

        if (borrowerIndex.mantissa > 0) {
            Double memory deltaIndex = sub_(borrowIndex, borrowerIndex);
            uint256 borrowerAmount = div_(TToken(tToken).borrowBalanceStored(borrower), marketBorrowIndex);
            ...
            emit DistributedBorrowerReward(rewardType, TToken(tToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
     ...
     */
    function updateAndDistributeSupplierRewardsForToken(address tToken, address supplier) external {
        ...
        for (uint8 rewardType = 0; rewardType <= 1; rewardType++) {
            updateRewardSupplyIndex(rewardType, tToken);
            distributeSupplierReward(rewardType, tToken, supplier);
        }
    }
    ...
    function updateAndDistributeBorrowerRewardsForToken(
        ...
        for (uint8 rewardType = 0; rewardType <= 1; rewardType++) {
            updateRewardBorrowIndex(rewardType, tToken, marketBorrowIndex);
            distributeBorrowerReward(rewardType, tToken, borrower, marketBorrowIndex);
        }
    }
     ...
     */
    function claimReward(uint8 rewardType, address payable holder) public {
        return claimReward(rewardType, holder, tombtroller.getAllMarkets());
    }
    ...
    function claimReward(
        ...
        holders[0] = holder;
        claimReward(rewardType, holders, tTokens, true, true);
    }
        ...
        for (uint256 i = 0; i < tTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint256 j = 0; j < holders.length; j++) {
                    distributeBorrowerReward(rewardType, address(tToken), holders[j], borrowIndex);
            ...
            if (suppliers == true) {
                updateRewardSupplyIndex(rewardType, address(tToken));
                for (uint256 j = 0; j < holders.length; j++) {
                    distributeSupplierReward(rewardType, address(tToken), holders[j]);
                    rewardAccrued[rewardType][holders[j]] = grantRewardInternal(
                        rewardType,
                        holders[j],
                        rewardAccrued[rewardType][holders[j]]
    ...
    /**
     * @notice Transfer TOMB/FTM to the user
     * @dev Note: If there is not enough TOMB/FTM, we do not perform the transfer all.
     * @param rewardType 0 = TOMB, 1 = FTM.
     * @param user The address of the user to transfer TOMB/FTM to
     * @param amount The amount of TOMB/FTM to (possibly) transfer
     * @return The amount of TOMB/FTM which was NOT transferred to the user
     */
    ...
    /**
     * @notice Transfer TOMB to the recipient
     * @dev Note: If there is not enough TOMB, we do not perform the transfer all.
     * @param rewardType 0 = TOMB, 1 = FTM
     * @param recipient The address of the recipient to transfer TOMB to
     * @param amount The amount of TOMB to (possibly) transfer
     */
    function _grantReward(
        ...
        require(adminOrInitializing(), "only admin can grant tomb");
    ...
    /**
     * @notice Set the TOMB token address
     */
    function setTombAddress(address newTombAddress) public {
        require(msg.sender == admin, "only admin can set TOMB");
        tombAddress = newTombAddress;
    }

    /**
     * @notice Set the Tombtroller address
     */
    function setTombtroller(address _tombtroller) public {
        require(msg.sender == admin, "only admin can set Tombtroller");
        tombtroller = Tombtroller(_tombtroller);
    }
