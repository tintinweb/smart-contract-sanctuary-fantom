...
// File contracts/BIFI/strategies/Balancer/StrategyBeethovenXDualRewardChefUniV2.sol

pragma solidity ^0.8.0;

contract StrategyBeethovenXDualRewardChefUniV2 is StratFeeManagerInitializable {
    ...
    function initialize(
        ...
        uniV2Router = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (rewardBal > 0) {
            IUniswapRouterETH(uniV2Router).swapExactTokensForTokens(
                rewardBal,
                0,
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            uint256[] memory amountOut = IUniswapRouterETH(uniV2Router).getAmountsOut(
                outputBal,
                secondOutputToNativeRoute
            );
    ...
    function _giveAllowances() internal {
        ...
        IERC20(secondOutput).safeApprove(uniV2Router, type(uint256).max);
        if (secondOutput != native) {
            IERC20(native).safeApprove(uniV2Router, type(uint256).max);
        }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(secondOutput).safeApprove(uniV2Router, 0);
        IERC20(native).safeApprove(uniV2Router, 0);
        IERC20(input).safeApprove(unirouter, 0);
    }
