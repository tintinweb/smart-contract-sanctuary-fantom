...
// File contracts/IceEqualEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract IceEqualEqualizerV2 is Ownable, Pausable {
    ...
    function chargeFees(address caller) internal {
       uint256 feeBal = IERC20(equal).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        uint256 callFee = feeBal * CALL_FEE / FEE_DIVISOR;
        IERC20(equal).transfer(caller, callFee);
        ...
        IERC20(equal).transfer(treasury, treasuryFee);

        uint256 stratFee = feeBal * STRAT_FEE / FEE_DIVISOR;
        IERC20(equal).transfer(strategist, stratFee);
    }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSimple(equalHalf, 0, equal, ice, stable, address(this), block.timestamp);

        uint256 t1Bal = IERC20(equal).balanceOf(address(this));
        uint256 t2Bal = IERC20(ice).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(equal, ice, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
