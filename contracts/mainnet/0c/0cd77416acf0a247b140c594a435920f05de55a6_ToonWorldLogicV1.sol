...
contract ToonWorldLogicV1 is Ownable{
    ...
    constructor(address _ctraits, address _cglyphs, address _cbucks, address _cskeletoons, address _addressReward, address _uniswapRouter, uint256 _summonFee, uint256 _strFee) 
    {
    ...
    function upgradeSkeletoonPayed(uint256 skeletoonTokenId, uint256 glyphTokenId, uint256 geneTokenId, uint256 addedStrength, uint256 position) public payable{
        ...
        timeLock[skeletoonTokenId] = block.timestamp;
         swapWFTMForToonBuvk( ((boughtPoints * (strPointCost )) / 10)*9 ,  rewardAddress);
         swapWFTMForToonBuvk( ((boughtPoints * (strPointCost )) / 10) ,  address(this));
         toonBucks.burn(address(this), toonBucks.balanceOf(address(this)));
    ...
    function summonSkeletoon(uint256[] memory _tokenIds) public payable{
        ...
        swapWFTMForToonBuvk( ((summonFee ) / 10)*9 ,  rewardAddress);
        swapWFTMForToonBuvk( ((summonFee ) / 10) ,  address(this));
        toonBucks.burn(address(this), toonBucks.balanceOf(address(this)));
