...
contract PEG is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 PEG-WETH LP -> PEG pool
    // uint256 public constant INITIAL_PEG_POOL_DISTRIBUTION = 140000 ether;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("TestPEG", "TestPEG") {
    ...
    function _getPEGPrice() internal view returns (uint256 _PEGPrice) {
        try IOracle(PEGOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("PEG: failed to fetch PEG price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _PEGPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_PEGPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPEGOracle(address _PEGOracle) public onlyOperatorOrTaxOffice {
        require(_PEGOracle != address(0), "oracle address cannot be 0 address");
    ...
    function _transfer(
        ...
        if (autoCalculateTax) {
            uint256 currentPEGPrice = _getPEGPrice();
            currentTaxRate = _updateTaxRate(currentPEGPrice);
            if (currentPEGPrice < burnThreshold) {
                burnTax = true;
            }
        }  
