...
contract Zapper is Ownable, IZapper {
        ...
        uint256 _token1Amount,address _routerAddr, address _recipient, uint256 _minAmountLp ) external whitelist(_routerAddr)
    {
        ...
        if(_token0Amount > amount0){
            IERC20(_token0).safeTransferFrom(address(this),msg.sender, _token0Amount.sub(amount0));
        }
        if(_token1Amount > amount1){
            IERC20(_token1).safeTransferFrom(address(this),msg.sender, _token1Amount.sub(amount1));
        }
    }
    ...
    function zapOutToTokens(address _in, uint256 _amount,
        ...
        IERC20(_token0).safeTransferFrom(address(this),msg.sender, _amount);
        IERC20(_token1).safeTransferFrom(address(this),msg.sender, _amount);
    }
