...
contract Vault is AccessControl {
    ...
    address public immutable baseV1Voter;

    address public platformVoter;
    ...
    bytes32 public constant VE_LENDER_ROLE = keccak256("VE_LENDER_ROLE");

    bytes32 public constant PLATFORM_VOTER_ROLE =
        keccak256("PLATFORM_VOTER_ROLE");
    ...
    function _lockFor(
        uint256 tokenId,
        address user,
        address[] memory poolVote_,
        int256[] memory weights_
    ...
    function lock(
        ...
        return _lockFor(tokenId, msg.sender, poolVote_, weights_);
    }
    ...
    function lockFor(
        ...
        return _lockFor(tokenId, user, poolVote_, weights_);
    }
    ...
    function _sellFor(address user, uint256 tokenId)
        internal
        returns (uint256)
    {
        Ive(votingEscrow).transferFrom(msg.sender, address(this), tokenId);
    ...
    function sell(uint256 tokenId) external returns (uint256) {
        return _sellFor(msg.sender, tokenId);
    }

    function sellFor(address user, uint256 tokenId) external returns (uint256) {
        return _sellFor(user, tokenId);
    }
    ...
    function _vote(address[] memory _poolVote, int256[] memory _weights)
        internal
    {
        require(
        ...
        uint256 i;
        for (i = 0; i < _poolVote.length; i++) {
            _totalVoteWeight += _weights[i] > 0 ? _weights[i] : -_weights[i];
        }

        (address[] memory poolVote, int256[] memory weights) = IPlatformVoter(
            platformVoter
        ).getVotes(_poolVote, _weights);
    ...
    function vote(address[] memory _poolVote, int256[] memory _weights)
        external
    {
    ...
    function voteFor(
        ...
        require(
            ...
            "Vault: you are not platform voter"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(isFree[tokenId] == true, "Vault: NOT_FREE_TOKEN");
            IBaseV1Voter(baseV1Voter).vote(tokenId, _poolVote, _weights);
        }
    }
