...
contract YChain is NonblockingLzApp, AccessControl, Pausable, ReentrancyGuard {
    ...
    function invest() internal nonReentrant whenNotPaused {
        require(
            asset.balanceOf(address(this)) >= data[0],
            "Insufficient Assets"
        );
    ...
    function withdrawfromVault() internal nonReentrant whenNotPaused {
        require(previewRedeemOfContract() >= data[1], "Insufficient Shares");
    ...
    function assetIncreaseAllowance(
        address spender,
        uint256 increment
    ) external onlyAdmin {
        asset.increaseAllowance(spender, increment);
    }

    function checkAllowance() external view returns (uint256) {
        return asset.allowance(address(this), address(painUsdcCrypt));
    }

    function balanceOfContract() public view virtual returns (uint256) {
        return painUsdcCrypt.balanceOf(address(this));
    }

    function previewRedeemOfContract() public view virtual returns (uint256) {
        uint256 balance = balanceOfContract();
    ...
    function previewRedeem(
        ...
        return painUsdcCrypt.previewRedeem(_balance);
    }
