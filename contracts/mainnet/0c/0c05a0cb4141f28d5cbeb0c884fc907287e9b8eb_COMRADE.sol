...
$$ |  $$\ $$ |  $$ |$$ | $$ | $$ |$$ |     $$  __$$ |$$ |  $$ |$$   ____|
\$$$$$$  |\$$$$$$  |$$ | $$ | $$ |$$ |     \$$$$$$$ |\$$$$$$$ |\$$$$$$$\ 
 \______/  \______/ \__| \__| \__|\__|      \_______| \_______| \_______|
                                                                         
                                                                         
Our inspiration for this project is the people. 
Comrade is for the people.  
*/                                                                       
...
contract COMRADE is ERC20Burnable, Operator {
     ...
     */
    constructor() public ERC20("COMRADE", "COMRADE") {
        // Mints 1 COMRADE to contract creator for initial pool setup
    ...
    function _getCOMRADEPrice() internal view returns (uint256 _COMRADEPrice) {
        try IOracle(ComradeOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("COMRADE: failed to fetch COMRADE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _COMRADEPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_COMRADEPrice >= taxTiersTwaps[tierId]) {
    ...
    /**
     * @notice Operator mints COMRADE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of COMRADE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCOMRADEPrice = _getCOMRADEPrice();
            currentTaxRate = _updateTaxRate(currentCOMRADEPrice);
            if (currentCOMRADEPrice < burnThreshold) {
                burnTax = true;
            }
        }
