...
contract BasedTombZap is Ownable {
    ...
    function _swapTokenToLP(address _from, uint256 amount, address _to, address recipient, address routerAddr) public returns (uint256) {
        // get pairs for desired lp
    ...
    function _swapNativeToLP(address _LP, uint256 amount, address recipient, address routerAddress) public returns (uint256) {
        // LP
        IUniswapV2Pair pair = IUniswapV2Pair(_LP);
        address token0 = pair.token0();
        address token1 = pair.token1();
    ...
    function _swapHalfNativeAndProvide(address token, uint256 amount, address routerAddress, address recipient) public returns (uint256, uint256, uint256) {
    ...
    function _swapNativeToEqualTokensAndProvide(address token0, address token1, uint256 amount, address routerAddress, address recipient) public returns (uint256, uint256, uint256) {
        uint256 swapValue = amount.div(2);
    ...
    function _swapNativeForToken(address token, uint256 value, address recipient, address routerAddr) public returns (uint256) {
        address[] memory path;
    ...
    function _swapTokenForNative(address token, uint256 amount, address recipient, address routerAddr) public returns (uint256) {
        address[] memory path;
        IUniswapV2Router router = IUniswapV2Router(routerAddr);
    ...
    function _swap(address _from, uint256 amount, address _to, address recipient, address routerAddr) public returns (uint256) {
        IUniswapV2Router router = IUniswapV2Router(routerAddr);
