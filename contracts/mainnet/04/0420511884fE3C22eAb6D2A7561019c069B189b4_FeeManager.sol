...
{
  function swapExactTokensForTokens (uint256 amount, uint256 min, address[] calldata path, address recipient, uint256 deadline) external returns (uint256[] memory);
}
...
contract FeeManager is IFeeManager, ReentrancyGuard
{
  ...
  IUniRouter private constant _ROUTER = IUniRouter(0xF491e7B69E4244ad4002BC14e878a34207E38c29);

  address private constant _KAE = 0x65Def5029A0e7591e46B38742bFEdd1Fb7b24436;
  address private constant _ANCHOR = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;
  ...
  event Burn(address burner, address token, uint256 amount, uint256 value);


  constructor ()
  {
    address[] memory tokens = ITokenRegistry(_REGISTRY.tokenRegistry()).getWhitelisted();


    for (uint256 i; i < tokens.length; i++)
    {
      IERC20(tokens[i]).safeApprove(address(_ROUTER), type(uint256).max);
    }
  }

  function _kaoracle () internal view returns (address)
  {
    return _REGISTRY.getContract(keccak256(bytes("KAOracle")));
  }


  function setConfig (Fee calldata fee, Epoch calldata epoch) external nonReentrant
  {
    require(_fee.deposit == 0, "set");
  ...
  {
    address kaoracle = _kaoracle();
    uint256 balance = IERC20(token).balanceOf(address(this));

    require(msg.sender == tx.origin, "inhuman");
    require(IKAOracle(kaoracle).lastPrice() > 0, "no price");
    require(IKAOracle(kaoracle).canUpdate(msg.sender), "bad burner");
    require(ITokenRegistry(_REGISTRY.tokenRegistry()).isWhitelisted(token), "!whitelisted");
    require(block.timestamp - IKAOracle(kaoracle).lastTimestamp() < 24 hours, "stale");
    require(balance > 0, "empty");


    if (_epoch.value >= 1000e18)
    {
      require(block.timestamp - _epoch.lastTimestamp > 24 hours, "cooldown");
    ...
    uint256 swapping = Math.min(balance, oracle.convertFromUSD(token, 1010e18));
    uint256 swapInUSD = oracle.convertToUSD(token, swapping);
    ...
    _ROUTER.swapExactTokensForTokens(swapping, IKAOracle(kaoracle).convertToKAE(_calcPercentOf(swapInUSD, 7000)), path, address(this), block.timestamp);


    uint256 amount = IERC20(_KAE).balanceOf(address(this));


    _burned += amount;
    _epoch.value += uint128(swapInUSD);
    _epoch.lastTimestamp = uint32(block.timestamp);


    IKAE(_KAE).burn(amount);


    emit Burn(msg.sender, token, amount, swapInUSD);
  }
  ...
  {
    uint256 fee = _calcPercentOf(_calcPercentOf(deposit, interestRate), IStakingManager(_REGISTRY.stakingManager()).isDiscountedForDeposits(depositor) ? _calcPercentOf(_fee.deposit, 7500) : _fee.deposit);


    return Math.max(fee, IOracle(_REGISTRY.oracle()).convertFromUSD(depositToken, 1e18));
  }
  ...
  function getDepositFeeOnInterest (uint256 deposit, uint256 depositInUSD, uint256 interest, bool isDiscountedForDeposits) external view override returns (uint256, address)
  {
    ...
    return (Math.max(fee, (deposit * _DECIMALS) / depositInUSD), _burner());
  }
