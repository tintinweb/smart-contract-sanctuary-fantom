...
interface IStaking {
    ...
    function sFHM() external view returns (address);
}
...
/*
    ...
    FHM token uses for staking.
    FTM -> DAI -> FHM routing
    Token Helper = "sFHM"
*/
contract Strategy_FHM is Ownable {
    ...
    uint256 public pendingFeeToken;
    address public yelLiquidityRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public routerFee = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public lpToken = 0xd77fc9c4074b56Ecf80009744391942FBFDDd88b;
    address public vault;
    address public DAI = 0x8D11eC38a3EB5E956B052f67Da8Bdc9bef8Abf3E; // token0 - decimals 18
    address public mainToken = 0x5E983ff70DE345de15DbDCf0529640F14446cDfa; // sNHM decimals 9
    address public FHM = 0xfa1FBb8Ef55A4855E5688C0eE13aC3f202486286; // token1 - decimals 9

    // ------------> Important constants <--------------
    address public constant USDT = 0x55d398326f99059fF775485246999027B3197955;
    address public constant YELtoken = 0xD3b71117E6C1558c1553305b44988cd944e97300;
    address public constant wrappedToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant stakingHelper = 0x22bbbA50188799De8BB4c424d6989fd2509496b3; // stake
    address public constant staking = 0x22bbbA50188799De8BB4c424d6989fd2509496b3; // unstake
    // ------------>        END          <--------------
    ...
    function earn() public returns(uint256 _balance) {
        _balance = getBalanceOfToken(FHM);
        if(_balance > 0) {
            _approveToken(FHM, stakingHelper);
    ...
    function emergencyWithdraw(address _receiver) public onlyVaultOrOwner {
        ...
        _transfer(FHM, _receiver, getBalanceOfToken(FHM));
        _transfer(DAI, _receiver, getBalanceOfToken(DAI));
    }

    function deposit() public payable onlyVault returns (uint256) {
        _approveToken(FHM, router);
        _approveToken(DAI, router);
        _checkPrices();
        _swapExactETHForTokens(DAI, address(this).balance);
        _swapExactTokenForToken(router, DAI, FHM, getBalanceOfToken(DAI));
        _updateLastPrices();
    ...
    function depositAsMigrate() public onlyVault {
        ...
        _swapExactTokenForToken(router, wrappedToken, DAI, getBalanceOfToken(wrappedToken));
        _swapExactTokenForToken(router, DAI, FHM, getBalanceOfToken(wrappedToken));
        earn();
    }
    ...
    function getBalanceOfToken(address _token) public view returns (uint256) {
        ...
        if(_token == mainToken) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeToken;
        }
    ...
    function setLP(address _address) public onlyVaultOrOwner {
        require(_address != address(0), "The address can not be zero address");
        require(FHM == IPair(_address).token0() && DAI == IPair(_address).token1(), "Tokens don't match");
    ...
    function _takeFee(uint256 _total) internal {
        ...
        bool result = _swapExactTokenForToken(router, FHM, DAI, _amount + pendingFeeToken);
        if(result) {
            emit TakeFee(pendingFeeToken);
            pendingFeeToken = 0;
        } else {
            pendingFeeToken += _amount;
        }

        _amount = getBalanceOfToken(DAI) + pendingFeeDAI;
        result = _swapExactTokenForToken(router, DAI, wrappedToken, _amount);
        if(result) {
            emit TakeFee(pendingFeeDAI);
            pendingFeeDAI = 0;
        } else {
    ...
    function _swapTokenToWrapped(uint256 _total, uint256 _percent) internal {
        ...
        _swapExactTokenForToken(router, FHM, DAI, getBalanceOfToken(FHM));
        _swapExactTokenForToken(router, DAI, wrappedToken, getBalanceOfToken(DAI));
    }
    ...
    function _getPriceWrappedToToken() internal view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = wrappedToken;
        path[1] = DAI;
    ...
    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
        uint256 d = 1e18;
        uint256 amountDAI = _getToken1Price() * _amount / d; // from mainToken
        uint256 _balance = getBalanceOfToken(DAI) + amountDAI;
        if (_balance > 0) {
            tCI += _getPriceWrappedToToken() * _balance / d;
        }
