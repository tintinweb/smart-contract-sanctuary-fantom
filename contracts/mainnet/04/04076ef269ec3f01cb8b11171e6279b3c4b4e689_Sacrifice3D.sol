...
contract Sacrifice3D {
    ...
    struct Stage {
        ...
        uint256 blocknumber;
        bool finalized;
        mapping (uint256 => address) slotXplayer;
    ...
    function tryFinalizeStage() private {
        ...
        if(block.number - 256 <= stageToFinalize.blocknumber) {
            ...
            //determine sacrifice
            uint256 sacrificeSlot = uint256(blockhash(stageToFinalize.blocknumber)) % maxPlayers;
            ...
            hourglass.buy{value: houseFee}(pricefloorAddress);
        } else {
