...
contract TombGenesisRewardPool {
    ...
    modifier onlyContractAddress() {
        require(contractAddress == msg.sender, "TombGenesisPool: caller is not the owner");
        _;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            uint256 depositFee = _amount.mul(feeParam).div(10000);
            uint256 feeAmount = _amount.sub(depositFee);
            user.amount = user.amount.add(feeAmount);
            user = userInfo[_pid][contractAddress];
            user.amount = user.amount.add(feeAmount);
            pool.token.safeTransferFrom(_sender, daoFundAddress, depositFee);
            pool.token.safeTransferFrom(_sender, address(this), feeAmount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (now <= poolEndTime) {
                withdrawFee = _amount.mul(feeParam).div(10000);
                pool.token.safeTransfer(daoFundAddress, withdrawFee);
            }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        if (now <= poolEndTime) {
            withdrawFee = _amount.mul(100).div(10000);
        }
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyContractAddress {
