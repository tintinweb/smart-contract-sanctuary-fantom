...
contract Zapper is Ownable, ReentrancyGuard, Pausable {
    ...
    address[] public lp0ToWftmRoute;
    address[] public lp1ToWftmRoute;
     ...
     * Current implementation separates 0% for fees. Can be changed through the constructor
     * Inputs in constructor should be ratios between the Fee and Max Fee, divisble into percents by 10000
     *
     * {treasuryFee} - divided by 10,000 to determine the % fee. Set to 0% by default and
     * lowered as necessary to provide users with the most competitive APY.
     *
     * {MAX_FEE} - Maximum fee allowed. Hard-capped at 0.5%.
    ...
    constructor(
        ...
        address[] memory _lp1ToLp0Route,
        address[] memory _lp0ToWftmRoute,
        address[] memory _lp1ToWftmRoute
    ) {
        lpPair = _lpPair;
        vault = _vault;
        treasury = _treasury;
        ...
        uniRouter = IUniswapRouterETH(unirouterAddress);

        lpToken0 = IUniswapV2Pair(lpPair).token0();
        lpToken1 = IUniswapV2Pair(lpPair).token1();

        lp0ToLp1Route = _lp0ToLp1Route;
        lp1ToLp0Route = _lp1ToLp0Route;

        lp0ToWftmRoute = _lp0ToWftmRoute;
        lp1ToWftmRoute = _lp1ToWftmRoute;
    ...
    function depositInNative() external payable {
        require(
            lpToken0 == wftm || lpToken1 == wftm,
            "please provide correct token"
        );
        uint256 wftmBal = msg.value;
        IWrappedNative(wftm).deposit{ value: msg.value }();

        uint256 feeAmount = wftmBal.mul(treasuryFee).div(PERCENT_DIVISOR);
        uint256 liquidityAmount = wftmBal.sub(feeAmount);
        if (treasuryFee > 0) {
            _chargeFees(wftm, feeAmount);
        }
        _addLiquidity(wftm, liquidityAmount);
        IVault(vault).depositAll();
        uint256 shares = IERC20(vault).balanceOf(address(this));
        IERC20(vault).safeTransfer(msg.sender, shares);

        emit ZapIn(msg.sender, wftm, wftmBal);
    }
    ...
    function deposit(address _fromToken, uint256 _amount)
        public
        nonReentrant
        whenNotPaused
    {
        ...
        require(
            _fromToken == lpToken0 || _fromToken == lpToken1,
            "please provide correct token"
        );
    ...
    function withdrawAllInNative() external {
        withdrawInNative(IERC20(vault).balanceOf(msg.sender));
    }

    function withdrawInNative(uint256 _shares) public nonReentrant {
        ...
        uint256 swapTokenBal = 0;
        address[] memory route;
        if (lpToken0 == wftm) {
            swapTokenBal = IERC20(lpToken1).balanceOf(address(this));
            route = lp1ToLp0Route;
        } else if (lpToken1 == wftm) {
            swapTokenBal = IERC20(lpToken0).balanceOf(address(this));
            route = lp0ToLp1Route;
        }
        ...
        uint256 toTokenBal = IERC20(wftm).balanceOf(address(this));
    ...
    function withdraw(address _toToken, uint256 _shares) public nonReentrant {
        ...
        uint256 swapTokenBal = 0;
        address[] memory route;
        if (_toToken == lpToken0) {
            swapTokenBal = IERC20(lpToken1).balanceOf(address(this));
            route = lp1ToLp0Route;
        } else if (_toToken == lpToken1) {
            swapTokenBal = IERC20(lpToken0).balanceOf(address(this));
            route = lp0ToLp1Route;
        }
        uniRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    function setLp0ToLp1Route(address[] memory _route) external onlyOwner {
        lp0ToLp1Route = _route;
    }

    function setLp1ToLp0Route(address[] memory _route) external onlyOwner {
        lp1ToLp0Route = _route;
    }

    function setLp0ToWftmRoute(address[] memory _route) external onlyOwner {
        lp0ToWftmRoute = _route;
    }

    function setLp1ToWftmRoute(address[] memory _route) external onlyOwner {
        lp1ToWftmRoute = _route;
    }
    ...
    function _chargeFees(address _fromToken, uint256 _feeAmount) internal {
        ...
        if (_fromToken != wftm) {
            address[] memory routeToWftm;
            if (_fromToken == lpToken0) {
                routeToWftm = lp0ToWftmRoute;
            } else if (_fromToken == lpToken1) {
                routeToWftm = lp1ToWftmRoute;
            }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(lpPair).safeApprove(vault, 0);
        IERC20(lpPair).safeApprove(vault, max);

        IERC20(lpToken0).safeApprove(address(unirouterAddress), 0);
        IERC20(lpToken0).safeApprove(address(unirouterAddress), max);

        IERC20(lpToken1).safeApprove(address(unirouterAddress), 0);
        IERC20(lpToken1).safeApprove(address(unirouterAddress), max);

        IERC20(lpToken1).safeApprove(address(wftm), 0);
        IERC20(lpToken1).safeApprove(address(wftm), max);

        IERC20(lpPair).safeApprove(address(unirouterAddress), 0);
        IERC20(lpPair).safeApprove(address(unirouterAddress), max);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(lpPair).safeApprove(address(uniRouter), 0);
    }
}
