...
import {ISolPair} from '../interfaces/ISolidlyV1Pair.sol';
...
contract Strategy is BaseStrategy {
    ...
    // Swap vars
    ISolPair public solidlyPool;
    address public swapRouter = address(0xa38cd27185a464914D3046f0AB9d43356B34829D); // SolidlySwap swapRouter
    address public auxSwapRouter = address(0x20dd72Ed959b6147912C2e529F0a0C651c33c9ce); // Beethoven router
    address public router = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29); // Spooky Router
    bool public useAuxRouter = false;

    // Beethoven swap data
    bytes32 balancerPoolId = 0x8b858eaf095a7337de6f9bc212993338773ca34e00020000000000000000023c; // USDC-DEI Pool id
    address bpt = 0x8B858Eaf095A7337dE6f9bC212993338773cA34e; // USDC-DEI Pool token
    ...
    // Rewards
    address public DEUS = address(0xDE5ed76E7c05eC5e4572CfC88d1ACEA165109E44);
    address public BOO = address(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE); // SPOOKY (BOO) Reward Token

    // Loose change and dust Threshold
    uint256 public dust = 1e6;
    uint256 public rewardDust = 1e15;
    ...
    constructor(
        ...
        require(lpToken.token0() == address(want), '!pool');
        require(masterChef.lpToken(_pid) == address(_lpToken), '!pool');
        pairToken1 = lpToken.token1();

        rewarder = _rewarder;
        pid = _pid;
        solidlyPool = ISolPair(_solidlyPool);
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('Spooky USDC-DEI Strategy'));
    }
    ...
    function balanceOfPairToken1() public view returns (uint256) {
        return IERC20(pairToken1).balanceOf(address(this));
    }
    ...
    function balanceOfLPInMasterChef() public view returns (uint256 _amount) {
        (_amount, ) = masterChef.userInfo(pid, address(this));
    }

    function balanceOfReward() public view returns (uint256 _booAmount, uint256 _deusAmount) {
        _booAmount = IERC20(BOO).balanceOf(address(this));
        _deusAmount = IERC20(DEUS).balanceOf(address(this));
    }

    function balanceOfLpComponents(uint256 liquidity)
        public
        view
        returns (uint256 _amountWant, uint256 _amountPairToken1)
    {
        (_amountWant, _amountPairToken1) = quoteRemoveLiquidity(liquidity);
    }
    ...
    function balanceOfPooled() public view returns (uint256 _amount) {
        ...
        (uint256 amountWant, uint256 amountPairToken) = balanceOfLpComponents(lpTokens);
        uint256 totalWant = amountPairToken.add(balanceOfPairToken1());
        uint256 pairTokenAsWant = pairToken1ToWant(totalWant);
        return pairTokenAsWant.add(amountWant);
    }

    function pendingRewards() public view returns (uint256 _booBalance, uint256 _deusBalance) {
        _booBalance = masterChef.pendingBOO(pid, address(this));
        _deusBalance = IRewarder(rewarder).pendingToken(pid, address(this));
    }

    function estimateHarvest() external view returns (uint256 _profit) {
        (uint256 _booBalance, uint256 _deusBalance) = pendingRewards();
        ...
        if (_booBalance > 0) {
            uint256 fromBOO = IUniRouter(router).getAmountsOut(_booBalance, rewardToWant)[2];
            _profit += fromBOO;
        }

        if (_deusBalance > 0) {
            rewardToWant[0] = DEUS;
            uint256 fromDEUS = IUniRouter(router).getAmountsOut(_deusBalance, rewardToWant)[2];
            _profit += fromDEUS;
        }
    }
        ...
        )
    {
        ...
        uint256 initialAssetsBalance = estimatedTotalAssets();
        ...
        if (currentBalance > debt) {
            _loss = 0;
            _profit = _calculateNetProfits(initialWantBalance, initialAssetsBalance);
    ...
    function _calculateNetProfits(uint256 _initialWantBalance, uint256 _initialAssetsBalance)
        internal
        view
        returns (uint256 _profit)
    {
        ...
        uint256 debt = vault.strategies(address(this)).totalDebt;
        uint256 currentBalance = estimatedTotalAssets();
        if (wantBalance > _initialWantBalance) {
            if (debt > _initialAssetsBalance) {
                _profit = currentBalance.sub(debt);
            } else {
                _profit = wantBalance.sub(_initialWantBalance);
            }
        }
    }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > looseAmount) {
            ...
            removeLiquidity(wantToLpTokens(toExitAmount));
            _sellLpPairToken();
            depositLpIntoMasterChef();
    ...
    function getSwapAmount(uint256 investmentA) internal view returns (uint256 _swapAmount) {
        (uint256 reserveA, uint256 reserveB, ) = lpToken.getReserves();
        uint256 halfInvestment = investmentA.div(2);
        uint256 nominator = solidlyPool.getAmountOut(halfInvestment, address(want));

        uint256 denominator = quoteLiquidity(halfInvestment, reserveA, reserveB);
    ...
    function _swap(uint256 _fullInvestment) internal returns (uint256[] memory _swapedAmounts) {
        ...
        if (useAuxRouter) {
            return auxSwap(swapAmountIn);
        }
        return mainSwap(swapAmountIn);
    }

    function _sellLpPairToken() internal {
        uint256 lpToken1Balance = balanceOfPairToken1();
        if (lpToken1Balance > 0) {
            if (!useAuxRouter) {
                _sellPairTokenMain(lpToken1Balance);
            } else {
                _sellPairTokenAux(lpToken1Balance);
            }
        }
    }

    function mainSwap(uint256 swapAmountIn) internal returns (uint256[] memory _swapedAmounts) {
        (, bool isStable) =
            ISolRouter(swapRouter).getAmountOut(swapAmountIn, address(want), pairToken1);

        route[] memory lpPairPath = new route[](1);
        lpPairPath[0] = route(address(want), pairToken1, isStable);

        _swapedAmounts = ISolRouter(swapRouter).swapExactTokensForTokens(
            swapAmountIn,
            swapAmountIn.mul(swapSecurityFactor).div(basisOne).mul(1e12),
            lpPairPath,
            address(this),
            block.timestamp
        );
    ...
    function _sellPairTokenMain(uint256 _lpToken1Balance) internal {
        (, bool isStable) =
            ISolRouter(swapRouter).getAmountOut(_lpToken1Balance, pairToken1, address(want));

        route[] memory lp1TokenToLp0Route = new route[](1);
        lp1TokenToLp0Route[0] = route(pairToken1, address(want), isStable);

        ISolRouter(swapRouter).swapExactTokensForTokens(
            _lpToken1Balance,
            _lpToken1Balance.mul(swapSecurityFactor).div(basisOne).div(1e12),
            lp1TokenToLp0Route,
            address(this),
            block.timestamp
        );
    ...
    function solidlyValue(uint256 _amount) internal view returns (uint256) {
        return solidlyPool.getAmountOut(_amount, pairToken1);
    }

    function _sellRewards() internal {
        (uint256 _booAmount, uint256 _deusAmount) = balanceOfReward();
        ...
        if (_booAmount > rewardDust) {
            IUniRouter(router).swapExactTokensForTokens(
                _booAmount,
                0,
                rewardToWant,
                address(this),
    ...
    function auxSwap(uint256 _swapAmountIn) internal returns (uint256[] memory) {
        uint256 amountOut = _swapAmountIn.mul(swapSecurityFactor).div(basisOne).mul(1e12);
        uint256 tokensOut =
            beethovenSwap(_swapAmountIn, amountOut, IAsset(address(want)), IAsset(pairToken1));
    ...
    function _sellPairTokenAux(uint256 _lpToken1Balance) internal {
        uint256 amountOut = _lpToken1Balance.mul(swapSecurityFactor).div(basisOne).div(1e12);
        beethovenSwap(_lpToken1Balance, amountOut, IAsset(pairToken1), IAsset(address(want)));
    }
    ...
    function beethovenSwap(
        ...
        _tokensOut = IBalancerVault(auxSwapRouter).swap(
            singleSwap,
    ...
    function beethovenValue(uint256 _amount) internal view returns (uint256) {
        (, uint256[] memory totalBalances, uint256 lastChangeBlock) =
            IBalancerVault(auxSwapRouter).getPoolTokens(balancerPoolId);
        ...
        return IBalancerPool(bpt).onSwap(request, totalBalances, 1, 0);
    }

    function quoteRemoveLiquidity(uint256 liquidity)
        internal
        view
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        amountA = liquidity.mul(reserveA).div(_totalSupply); // using balances ensures pro-rata distribution
        amountB = liquidity.mul(reserveB).div(_totalSupply); // using balances ensures pro-rata distribution
    }

    function wantToLpTokens(uint256 _amountWant) public view returns (uint256) {
        (uint256 amountWant, uint256 _amountPairToken) = balanceOfLpComponents(1e18);
        uint256 pairTokenAsWant = pairToken1ToWant(_amountPairToken);
        uint256 amountWantPerLp = pairTokenAsWant.add(amountWant);
        return _amountWant.mul(1e18).div(amountWantPerLp);
    }

    function pairToken1ToWant(uint256 _amount) internal view returns (uint256) {
        if (_amount == 0) return 0;
        if (useAuxRouter) {
            return beethovenValue(_amount);
        }
        return solidlyValue(_amount);
    }
    ...
    function removeLiquidity(uint256 _liquidityToExit) internal {
        uint256 toExit = Math.min(_liquidityToExit, balanceOfLpTokens());
        if (toExit > 0) {
            IUniRouter(router).removeLiquidity(
                address(want),
                pairToken1,
                toExit,
                0,
    ...
    function withdrawFromMasterChef() internal {
        ...
        if (masterChefBal > 0) {
            masterChef.withdraw(pid, masterChefBal);
        }
    }
    ...
    function _withdrawFromMasterChefAndTransfer(address _to) internal {
        if (abandonRewards) {
            ...
            _claimRewards();
            withdrawFromMasterChef();
            _transferRewards(_to);
        }
        _transferLpTokens(_to);
        _transferPairToken1(_to);
    }

    function _transferRewards(address _to) internal {
        (uint256 _booAmount, uint256 _deusAmount) = balanceOfReward();
        if (_booAmount > 0) {
            IERC20(BOO).safeTransfer(_to, _booAmount);
        }
        if (_deusAmount > 0) {
            IERC20(DEUS).safeTransfer(_to, _deusAmount);
        }
    }

    function _transferLpTokens(address _to) internal {
        uint256 lpTokensBalance = balanceOfLpTokens();
        if (lpTokensBalance > 0) {
            lpToken.transfer(_to, lpTokensBalance);
        }
    }

    function _transferPairToken1(address _to) internal {
        uint256 pairToken1Balance = balanceOfPairToken1();
        if (pairToken1Balance > 0) {
            IERC20(pairToken1).safeTransfer(_to, pairToken1Balance);
        }
    }
    ...
    function giveAuxSwapAllowance() internal {
        IERC20(want).safeApprove(auxSwapRouter, 0);
        IERC20(want).safeApprove(auxSwapRouter, max);
        IERC20(pairToken1).safeApprove(auxSwapRouter, 0);
        IERC20(pairToken1).safeApprove(auxSwapRouter, max);
    }
    ...
    function giveAllowances() internal {
        ...
        IERC20(DEUS).safeApprove(router, max);
        IERC20(BOO).safeApprove(router, max);

        IERC20(want).safeApprove(swapRouter, 0);
        IERC20(want).safeApprove(swapRouter, max);
        IERC20(pairToken1).safeApprove(swapRouter, 0);
        IERC20(pairToken1).safeApprove(swapRouter, max);

        IERC20(want).safeApprove(router, 0);
        IERC20(want).safeApprove(router, max);
        IERC20(pairToken1).safeApprove(router, 0);
        IERC20(pairToken1).safeApprove(router, max);

        IERC20(address(lpToken)).safeApprove(router, 0);
    ...
    function sellPairToken1() external onlyVaultManagers {
        _sellLpPairToken();
    }

    function setUseAuxRouter(bool _useAux) external onlyVaultManagers {
        useAuxRouter = _useAux;
        giveAuxSwapAllowance();
    }
...
interface IRewarder {
    ...
    function useInf(uint256 _pid, address _user) external view returns (UserInfo memory userInfo);
}
...
interface IMasterChef {
    ...
    */
    ...
    function safeSpiritTransfer(address _to, uint256 _amount) external;
