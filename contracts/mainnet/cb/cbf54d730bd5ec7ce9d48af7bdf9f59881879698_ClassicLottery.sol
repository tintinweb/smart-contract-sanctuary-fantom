/**
 *Submitted for verification at FtmScan.com on 2023-02-26
*/
...
interface ILotteryStruct {
    struct Lottery {
        uint256 uniqueId;   // Unique ID for all lottery contract
        uint256 lotteryId;    // ID for lottery within this contract
        ...
        uint256 winnersCount;          // !NUMBER or PERCENT! of winners in current lottery 
        ...
        bool isRewardable;             // enable/disable participates rewarding
        LotteryEnums.LotteryStatus lotteryStatus;
        LotteryEnums.LotteryType lotteryType;    // type of the lottery                 
        address[] participants;                  // array of participant's addresses
        address[] winners;                       // array of winner's addresses
        address[] claimers;                     // array of claimer's addresses
        address[] returners;                        // array of entry returners 
        uint256[] randoms;                      // array with random words
    }
}
// File: ILottery.sol
...
interface ILottery is ILotteryStruct {
    function addLottery(
...
contract ClassicLottery is ILottery, AccessControl, VRFConsumerBaseV2 {
    using Counters for Counters.Counter;
    Counters.Counter private currentLotteryId;

    address public comissionsHub;
    ...
    /**
        @notice Manager can add a new lottery.
     */
    function addLottery(
                uint256 _lotteryDuration, 
                uint256 _lotteryEntryPrice, 
        ...
        require(_lotteryEntryPrice > 0 && _lotteryStartAt > block.timestamp 
                    && _lotteryDuration > 0 && _winnersCount > 0,
                    "incorrect param(s)");
        if (_maxParticipantsCount > 0) {
            require(_winnersCount < _maxParticipantsCount, "incorrect max participants count");
        }
        ...
        lottery.lotteryType = LotteryEnums.LotteryType.COMMON;
    ...
    */
    function participate() external override virtual payable {
        ...
        if (lottery.maxParticipantsCount > 0){
            require(lottery.participants.length < lottery.maxParticipantsCount, "participants limit");
        }  
        ...
        addressParticipateCount[msg.sender] += 1;
        if (lottery.isRewardable 
            && address(lotteryPlayersRewardDistributionNode) != address(0)
            && participantsReward != 0) {
            lotteryPlayersRewardDistributionNode.distribute(msg.sender, participantsReward);
        }
    }
    ...
    function returnEntry(uint256 _lotteryId) external override virtual {
        ...
        payable(msg.sender).transfer(lottery.lotteryEntryPrice);
    }
    ...
    function determineWinners() external override virtual onlyRole(LOTTERY_MANAGER) {
        ...
        require(lottery.participants.length > lottery.winnersCount, "not enough participants");
        require(lottery.lotteryStatus == LotteryEnums.LotteryStatus.INITIALIZED, "awarding unreachable");
        lottery.lotteryWinnerAward = (100 - comissionPercent) * lottery.currentLot / 100 / lottery.winnersCount;   // calculate winner award considering the fee
        lottery.randomRange = lottery.participants.length;
        lottery.lotteryStatus = LotteryEnums.LotteryStatus.AWARDING;
        getRandomNumbers(RANDOM_NUMBERS_COUNT);
    }
    ...
    /*
    * Manager should call the function to fill array with winners addresses.
    */
    function fillWinners() external onlyRole(LOTTERY_MANAGER) {
        ...
        for(uint256 i = 0; i < lottery.winnersCount; i++){
            ...
            lottery.participants[randomResult] = lottery.participants[lottery.participants.length - 1];
        ...
        uint256 comission = lottery.currentLot - (lottery.lotteryWinnerAward * lottery.winnersCount);
        payable(comissionsHub).transfer(comission);
    ...
    function claimReward(uint256 _lotteryId) external override virtual {
        ...
        require(isAddressWonInLottery[_lotteryId][msg.sender], "you can't claim a reward");
        isAddressWonInLottery[_lotteryId][msg.sender] = false;
        lottery.claimers.push(msg.sender);
        totalAwardedTokens += lottery.lotteryWinnerAward;
        payable(msg.sender).transfer(lottery.lotteryWinnerAward);
    }
    ...
    function getAddressParicipateCount(address _account) external view returns(uint256){
    ...
    function supportsInterface(bytes4 interfaceId) public view override virtual returns(bool){
        return interfaceId == type(ILottery).interfaceId || super.supportsInterface(interfaceId);
    }
}