...
contract ABcDeFxPair {

    string public name = "ABcDeFx Liquidity Position";
    string public symbol = "ABcDeFx.LP";
    ...
    constructor() {
        factory = msg.sender;
        (address _token0, address _token1) = ABcDeFxFactory(msg.sender).getInitializable();
        (token0, token1) = (_token0, _token1);
        feeTaker = IABcDeFxFactory(msg.sender).feeTaker();
        name = string(abi.encodePacked("ABcDeFx Liquidity Position: ", erc20(_token0).name(), " paired with ", erc20(_token1).name()));
        symbol = string(abi.encodePacked("ABcDeFx.LP-", erc20(_token0).symbol(), "/", erc20(_token1).symbol()));
        fee = IABcDeFxFactory(factory).fee();
    ...
    function getFeeTaker() public view returns(address) {
      ...
      if(lft == address(0)) {
        return IABcDeFxFactory(factory).feeTaker();
      }
      else {
        return lft;
      }
    ...
    function _update0(uint amount) internal {
        _safeTransfer(token0, getFeeTaker(), amount); // transfer the fees out to ABcDeFxFees
        totalFee0 += amount;
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) internal {
        require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, 'ABcDeFxPair: OVERFLOW');
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'ILM'); // ABcDeFx: INSUFFICIENT_LIQUIDITY_MINTED
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'ILB'); // ABcDeFx: INSUFFICIENT_LIQUIDITY_BURNED
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(!ABcDeFxFactory(factory).isPaused());
        require(amount0Out > 0 || amount1Out > 0, 'IOA'); // ABcDeFx: INSUFFICIENT_OUTPUT_AMOUNT
        (uint112 _reserve0, uint112 _reserve1) =  (reserve0, reserve1);
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'IL'); // ABcDeFx: INSUFFICIENT_LIQUIDITY

        uint _balance0;
        uint _balance1;
        { // scope for _token{0,1}, avoids stack too deep errors
        (address _token0, address _token1) = (token0, token1);
        require(to != _token0 && to != _token1, 'IT'); // ABcDeFx: INVALID_TO
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IABcDeFxCallee(to).hook(msg.sender, amount0Out, amount1Out, data); // callback, used for flash loans
        _balance0 = erc20(_token0).balanceOf(address(this));
        _balance1 = erc20(_token1).balanceOf(address(this));
        }
        uint amount0In = _balance0 > _reserve0 - amount0Out ? _balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = _balance1 > _reserve1 - amount1Out ? _balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'IIA'); // ABcDeFx: INSUFFICIENT_INPUT_AMOUNT
        ...
        require(_k(_balance0, _balance1) >= _k(_reserve0, _reserve1), 'K'); // ABcDeFx: K
        }
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'ABcDeFx: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'ABcDeFx: INVALID_SIGNATURE');
...
contract ABcDeFxFactory {

    bool public isPaused;
    address public owner;
    address public pendingOwner;

    uint256 public fee = 100;   //1000: =1/1000 = 0.001 = 0.1%, 250 = 0.4%
    ...
    constructor(address _ft, address _ow) {
        feeTaker = _ft;
        owner = _ow;
        isPaused = false;
    }
    ...
    function setFeeTaker(address _pair, address _ft) external {
        require (msg.sender == owner);
        IABcDeFxPair(_pair).setFeeTaker(_ft);
    }
    ...
    function setFeeTakers(address[] memory _pairs, address[] memory _ft) external {
        ...
        for(uint i;i<_pairs.length;i++) {
          IABcDeFxPair(_pairs[i]).setFeeTaker(_ft[i]);
        }
    }
    ...
    function updateFee(address _pair, uint _f) external {
        require (msg.sender == owner);
        IABcDeFxPair(_pair).updateFee(_f);
    }
    ...
    function updateFee(address[] memory _pairs, uint[] memory _f) external {
        require (msg.sender == owner);
        for(uint i;i<_pairs.length;i++) {
          IABcDeFxPair(_pairs[i]).updateFee(_f[i]);
        }
    }

    function INIT_CODE_PAIR_HASH() external pure returns (bytes32) {
        return keccak256(type(ABcDeFxPair).creationCode);
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'IA'); // ABcDeFx: IDENTICAL_ADDRESSES
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'ZA'); // ABcDeFx: ZERO_ADDRESS
        require(getPair[token0][token1] == address(0), 'PE'); // ABcDeFx: PAIR_EXISTS - single check is sufficient
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        (_temp0, _temp1) = (token0, token1);
        pair = address(new ABcDeFxPair{salt:salt}());
