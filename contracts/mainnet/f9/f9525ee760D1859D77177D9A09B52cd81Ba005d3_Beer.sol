...
contract Beer is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 BEER-WETH LP -> BEER pool
    // uint256 public constant INITIAL_BEER_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the BEER ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("BEER", "BEER") {
        // Mints 1000 BEER to contract creator for initial pool setup
    ...
    function _getMedalPrice() internal view returns (uint256 _beerPrice) {
        try IOracle(beerOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Medal: failed to fetch BEER price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _beerPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_beerPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMedalOracle(address _beerOracle) public onlyOperatorOrTaxOffice {
        require(_beerOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BEER to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BEER to mint to
    ...
    function distributeReward(address _genesisPool) external onlyOperator {
        ...
        // _mint(_cometPool, INITIAL_BEER_POOL_DISTRIBUTION);
