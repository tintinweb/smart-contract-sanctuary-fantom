 ...
 * @notice Interface for the Wagmi fee IFlashLoanReceiver.
 * @author Wagmi
 ...
 * @notice Interface for the Wagmi price oracle.
 **/
...
import {IScaledBalanceToken} from "./IScaledBalanceToken.sol";
import {IInitializableWToken} from "./IInitializableWToken.sol";

interface IWToken is IERC20Upgradeable, IScaledBalanceToken, IInitializableWToken {
  /**
   * @dev Emitted after the mint action
   * @param from The address performing the mint
  ...
  /**
   * @dev Mints `amount` wTokens to `user`
  ...
  /**
   * @dev Emitted after wTokens are burned
   * @param from The owner of the wTokens, getting them burned
  ...
  /**
   * @dev Burns wTokens from `user` and sends the equivalent amount of underlying to `receiverOfUnderlying`
   * @param user The owner of the wTokens, getting them burned
  ...
  /**
   * @dev Mints wTokens to the reserve treasury
   * @param amount The amount of tokens getting minted
   * @param index The new liquidity index of the reserve
   */
  ...
  /**
   * @dev Transfers wTokens in the event of a borrow being liquidated, in case the liquidators reclaims the wToken
   * @param from The address getting liquidated, current owner of the wTokens
   * @param to The recipient
  ...
  /**
   * @dev Invoked to execute actions on the wToken side after a repayment.
  ...
  /**
   * @dev Returns the address of the underlying asset of this wToken (E.g. WETH for aWETH)
   **/
 ...
 * @dev Main point of interaction with an Wagmi protocol"s market
 * - Users can:
 *   # Deposit
 *   # Withdraw
 *   # Borrow
 ...
 * @author Wagmi
 **/
contract LendingPool is ILendingPool, LendingPoolStorage, Initializable {
  ...
  function deposit(
    ...
    address wToken = reserve.wTokenAddress;

    reserve.updateState();
    reserve.updateInterestRates(asset, wToken, amount, 0);

    IERC20(asset).safeTransferFrom(msg.sender, wToken, amount);

    bool isFirstDeposit = IWToken(wToken).mint(onBehalfOf, amount, reserve.liquidityIndex);
  ...
  function withdraw(address asset, uint256 amount, address to) external override whenNotPaused returns (uint256) {
    ...
    uint256 userBalance = IWToken(wToken).balanceOf(msg.sender);
    ...
    reserve.updateInterestRates(asset, wToken, 0, amountToWithdraw);
    ...
    IWToken(wToken).burn(msg.sender, to, amountToWithdraw, reserve.liquidityIndex);
  ...
  function repay(
    ...
    address wToken = reserve.wTokenAddress;
    reserve.updateInterestRates(asset, wToken, paybackAmount, 0);

    if (stableDebt.add(variableDebt).sub(paybackAmount) == 0) {
      _usersConfig[onBehalfOf].setBorrowing(reserve.id, false);
    }

    IERC20(asset).safeTransferFrom(msg.sender, wToken, paybackAmount);

    IWToken(wToken).handleRepayment(msg.sender, paybackAmount);
  ...
  function swapBorrowRateMode(address asset, uint256 rateMode) external override whenNotPaused {
    ...
    reserve.updateInterestRates(asset, reserve.wTokenAddress, 0, 0);
  ...
  function rebalanceStableBorrowRate(address asset, address user) external override whenNotPaused {
    ...
    address wTokenAddress = reserve.wTokenAddress;
    ...
    reserve.updateInterestRates(asset, wTokenAddress, 0, 0);
  ...
  function flashLoan(
    ...
    address[] memory wTokenAddresses = new address[](assets.length);
    uint256[] memory premiums = new uint256[](assets.length);

    vars.receiver = IFlashLoanReceiver(receiverAddress);

    for (vars.i = 0; vars.i < assets.length; vars.i++) {
      wTokenAddresses[vars.i] = _reserves[assets[vars.i]].wTokenAddress;

      premiums[vars.i] = amounts[vars.i].mul(_flashLoanPremiumTotal).div(10000);

      IWToken(wTokenAddresses[vars.i]).transferUnderlyingTo(receiverAddress, amounts[vars.i]);
    }
    ...
    for (vars.i = 0; vars.i < assets.length; vars.i++) {
      ...
      vars.currentWTokenAddress = wTokenAddresses[vars.i];
      vars.currentAmountPlusPremium = vars.currentAmount.add(vars.currentPremium);

      if (DataTypes.InterestRateMode(modes[vars.i]) == DataTypes.InterestRateMode.NONE) {
        _reserves[vars.currentAsset].updateState();
        _reserves[vars.currentAsset].cumulateToLiquidityIndex(
          IERC20(vars.currentWTokenAddress).totalSupply(),
          vars.currentPremium
        );
        _reserves[vars.currentAsset].updateInterestRates(
          vars.currentAsset,
          vars.currentWTokenAddress,
  ...
  /**
   * @dev Validates and finalizes an wToken transfer
   * - Only callable by the overlying wToken of the `asset`
   * @param asset The address of the underlying asset of the wToken
   * @param from The user from which the wTokens are transferred
   * @param to The user receiving the wTokens
   * @param amount The amount being transferred/withdrawn
   * @param balanceFromBefore The wToken balance of the `from` user before the transfer
   * @param balanceToBefore The wToken balance of the `to` user before the transfer
   */
  function finalizeTransfer(
    address asset,
    address from,
    ...
    require(msg.sender == _reserves[asset].wTokenAddress, Errors.LP_CALLER_MUST_BE_AN_ATOKEN);
  ...
  /**
   * @dev Initializes a reserve, activating it, assigning an wToken and debt tokens and an
   * interest rate strategy
   * - Only callable by the LendingPoolConfigurator contract
   * @param asset The address of the underlying asset of the reserve
   * @param wTokenAddress The address of the wToken that will be assigned to the reserve
   * @param stableDebtAddress The address of the StableDebtToken that will be assigned to the reserve
   * @param wTokenAddress The address of the VariableDebtToken that will be assigned to the reserve
  ...
  function initReserve(
    ...
    _reserves[asset].init(wTokenAddress, stableDebtAddress, variableDebtAddress, interestRateStrategyAddress);
  ...
  function _executeBorrow(ExecuteBorrowParams memory vars) internal {
    ...
    reserve.updateInterestRates(vars.asset, vars.wTokenAddress, 0, vars.releaseUnderlying ? vars.amount : 0);

    if (vars.releaseUnderlying) {
      IWToken(vars.wTokenAddress).transferUnderlyingTo(vars.user, vars.amount);
    }
  ...
  function calculateUserAccountData(
    ...
    for (vars.i = 0; vars.i < reservesCount; vars.i++) {
      ...
      if (vars.liquidationThreshold != 0 && userConfig.isUsingAsCollateral(vars.i)) {
        vars.compoundedLiquidityBalance = IERC20(currentReserve.wTokenAddress).balanceOf(user);
...
library ReserveLogic {
   ...
   * @param reserve The reserve object
   * @param wTokenAddress The address of the overlying wtoken contract
   * @param interestRateStrategyAddress The address of the interest rate strategy contract
   **/
  function init(
    DataTypes.ReserveData storage reserve,
    ...
    require(reserve.wTokenAddress == address(0), Errors.RL_RESERVE_ALREADY_INITIALIZED);

    reserve.liquidityIndex = uint128(WadRayMath.ray());
    reserve.variableBorrowIndex = uint128(WadRayMath.ray());
    reserve.wTokenAddress = wTokenAddress;
  ...
  function _mintToTreasury(
    ...
    if (vars.amountToMint != 0) {
      IWToken(reserve.wTokenAddress).mintToTreasury(vars.amountToMint, newLiquidityIndex);
    }
  }
...
library ValidationLogic {
    ...
    if (interestRateMode == uint256(DataTypes.InterestRateMode.STABLE)) {
      ...
      require(
        !userConfig.isUsingAsCollateral(reserve.id) ||
          ...
          amount > IERC20(reserve.wTokenAddress).balanceOf(userAddress),
        Errors.VL_COLLATERAL_SAME_AS_BORROWING_CURRENCY
      );

      vars.availableLiquidity = IERC20(asset).balanceOf(reserve.wTokenAddress);
  ...
  function validateSwapRateMode(
    ...
    if (currentRateMode == DataTypes.InterestRateMode.STABLE) {
      ...
      require(
        !userConfig.isUsingAsCollateral(reserve.id) ||
          reserve.configuration.getLtv() == 0 ||
          stableDebt.add(variableDebt) > IERC20(reserve.wTokenAddress).balanceOf(msg.sender),
        Errors.VL_COLLATERAL_SAME_AS_BORROWING_CURRENCY
      );
    } else {
   ...
   * @param wTokenAddress The address of the wToken contract
   */
  function validateRebalanceStableBorrowRate(
    ...
    uint256 availableLiquidity = IERC20(reserveAddress).balanceOf(wTokenAddress).wadToRay();
  ...
  function validateSetUseReserveAsCollateral(
    ...
    uint256 underlyingBalance = IERC20(reserve.wTokenAddress).balanceOf(msg.sender);
  ...
  /**
   * @dev Validates an wToken transfer
   * @param from The user from which the wTokens are being transferred
   * @param reservesData The state of all the reserves
   * @param userConfig The state of the user for the specific reserve
   * @param reserves The addresses of all the active reserves
   * @param oracle The price oracle
