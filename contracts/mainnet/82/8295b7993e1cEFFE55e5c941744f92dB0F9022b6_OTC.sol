...
contract OTC is Ownable, ContractGuard {
    ...
    // insures only whilelisted assets are allowec to be swapped for NATIVE
    modifier assetWhitelist(address _asset) {
        require(asset[_asset], "asset not whitelisted");
        _;
    }
    ...
    constructor(address _NATIVE, address _otcFund, uint256 _slippage) Ownable() {
        NATIVE = _NATIVE;
        otcFund = _otcFund;
        slippage = _slippage;
    ...
    function swapToken(address _in, uint256 amount, address _router, uint256 estimateAPI) external onlyOneBlock {
       ...
       require (estimateAPI < estimateDex.mul(slippage).div(1000), "no fee < dex fee");
       ...
       IERC20(NATIVE).safeTransfer(msg.sender, estimateDex.mul(slippage).div(1000));
    ...
    function _estimateSwap(address _in, uint256 amount, address routerAddr) public view routerWhitelist (routerAddr) assetWhitelist(_in) returns (uint256) {
        IUniswapV2Router _router = IUniswapV2Router(routerAddr);
      
        address stable = _in == USDC? MAI: _in;
             
        address fromBridge = tokenBridgeForRouter[stable][routerAddr];
