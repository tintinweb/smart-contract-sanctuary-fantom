...
interface IBentoBoxV1 {
    ...
    function totals(IERC20)
        external
        view
        returns (uint256 elastic, uint256 base);
...
contract Limone_ZapIn_V1 is ZapInBaseV3_1 {
    ...
    IUniswapV2Factory private constant spiritFactory =
        IUniswapV2Factory(0xEF45d134b73241eDa7703fa787148D9C9F4950b0);

    IUniswapV2Router02 private constant spiritRouter =
    ...
    constructor(address _DAO) {
        ...
        //allowedBoxes
        allowedBoxes[
            IBentoBoxV1(0xc7a161Cfd0e133d289B13692b636B8e8B5CD8d8c)
        ] = true;
    }
    ...
    function ZapIn(
        ...
        cache.balanceAfter = _fromToken == address(0)
            ? IERC20(wftmTokenAddress).safeBalanceOf(address(this))
            : IERC20(_fromToken).safeBalanceOf(address(this));
        require(
        ...
        _approveToken(_pairAddress, address(_bentoBox), LPBought);
        _bentoBox.deposit(
            IERC20(_pairAddress),
            address(this),
            _to,
    ...
    function performLpToBox(
        ...
        uint256 toInvest0 = _pullTokens(_ToUnipoolToken0, token0Amount, true);
        uint256 toInvest1 = _pullTokens(_ToUnipoolToken1, token1Amount, true);

        if (_ToUnipoolToken0 == address(0)) {
            IWETH(wftmTokenAddress).deposit{value: toInvest0}();
            _ToUnipoolToken0 = wftmTokenAddress;
        }
        if (_ToUnipoolToken1 == address(0)) {
            IWETH(wftmTokenAddress).deposit{value: toInvest1}();
            _ToUnipoolToken1 = wftmTokenAddress;
        }
        ...
        address pair = spiritFactory.getPair(
            _ToUnipoolToken0,
            _ToUnipoolToken1
        );
        _approveToken(pair, address(_bentoBox), LPBought);
    ...
    function _uniDeposit(
        ...
        _approveToken(_ToUnipoolToken0, address(spiritRouter), token0Bought);
        _approveToken(_ToUnipoolToken1, address(spiritRouter), token1Bought);

        (uint256 amountA, uint256 amountB, uint256 LP) = spiritRouter
            .addLiquidity(
                _ToUnipoolToken0,
                _ToUnipoolToken1,
                token0Bought,
    ...
    function _swapIntermediate(
        ...
        IUniswapV2Pair pair = IUniswapV2Pair(
            spiritFactory.getPair(_ToUnipoolToken0, _ToUnipoolToken1)
        );
    ...
    */
    function _token2Token(
        ...
        tokenBought = spiritRouter.swapExactTokensForTokens(
            tokens2Trade,
            1,
            path,
            address(this),
