...
abstract contract AutoCompoundStrategy is Ownable, ReentrancyGuard, Pausable {
    ...
    function _safeSwap(
        ...
        address router = swapRouterEnabled ? address(swapRouter) : address(liquidityRouter);
        IERC20(_path[0]).safeIncreaseAllowance(address(router), _amountIn);
        if (_ignoreErrors) {
            try
                IExcaliburRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    _amountIn,
                    0,
                    _path,
                    _to,
                    yieldWolf.feeAddress(),
                    block.timestamp
                )
            {} catch {}
        } else {
