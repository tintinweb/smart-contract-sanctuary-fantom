...
contract Pool is Ownable, ReentrancyGuard {
        ...
        )
    {
        ...
        uint256 _ftmOutBeforeFee = (_xftmIn * collateralRatio) / PRECISION;
        if (collateralRatio < COLLATERAL_RATIO_MAX) {
            uint256 _fantasmValueOut = _xftmIn - _ftmOutBeforeFee;
            _fantasmOut = ((_fantasmValueOut * PRICE_PRECISION) * (PRECISION - redemptionFee)) / PRECISION / _fantasmPrice; // after fee
        }

        if (collateralRatio > 0) {
            _ftmFee = (_ftmOutBeforeFee * redemptionFee) / PRECISION; // redemption fee in FTM
            _ftmOut = _ftmOutBeforeFee - _ftmFee; // after fee
        }
    }
