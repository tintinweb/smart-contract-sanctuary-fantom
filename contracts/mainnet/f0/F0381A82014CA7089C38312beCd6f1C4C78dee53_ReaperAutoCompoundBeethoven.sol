...
contract ReaperAutoCompoundBeethoven is Ownable, Pausable {
    ...
    function withdraw(uint256 _amount) external onActionHarvest {
        ...
        if (pairBal < _amount) {
            IMasterChefv2(masterChef).withdrawAndHarvest(poolId, _amount - pairBal, address(this));
        ...
        uint256 withdrawFee = (pairBal * securityFee) / PERCENT_DIVISOR;
        IERC20(lpToken).safeTransfer(vault, pairBal - withdrawFee);
    }
    ...
    function harvest() public whenNotPaused {
        ...
        if (secondReward) {
            _swapSecondToken(secondRewardToken, secondRewardRoute);
        }

        if (beetsUnderlying == true) {
            _addLiquidity(rewardToken, beetsPosition);
        } else if (wftmUnderlying == true) {
            _addLiquidity(wftm, wftmPosition);
        } else if (usdcUnderlying == true) {
            _addLiquidity(usdc, usdcPosition);
        }
    ...
    function _chargeFees() internal {
        ...
        if (beetsUnderlying == true || usdcUnderlying == true) {
            fees = (rewardBal * totalFee) / PERCENT_DIVISOR;
    ...
    function _addLiquidity(address token, uint8 position) internal {
        ...
        IVault(BeetVault).joinPool(poolID_bytes, address(this), address(this), request);
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IMasterChefv2(masterChef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function _swapSecondToken(address token, bytes32 route) internal {
        if (wftmUnderlying == true || beetsUnderlying == true) {
            ...
            uint256 fees = ((IERC20(wftm).balanceOf(address(this)) - wftmBal) * totalFee) / PERCENT_DIVISOR;
    ...
    function secondRewardInformation(
        bool status,
        address token,
        bytes32 route
    ) external onlyOwner {
