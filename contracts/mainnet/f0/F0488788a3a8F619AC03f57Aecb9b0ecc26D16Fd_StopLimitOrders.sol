...
import {_getAmountOut} from "./uniswap/FUniswapGeneral.sol";
import {
    UniswapV2Router02Handler
} from "../handlers/uniswap/UniswapV2Router02Handler.sol";
...
function _isFeeOk(
    ...
    address _uniRouter,
    uint256 _maxTxFeeInNative
) view returns (bool) {
    (, , uint256 fee, , address[] memory feePathToNative) = abi.decode(
        _auxData,
        (address, address, uint256, address[], address[])
    );

    bool isOutputNative = _outputToken == _wrappedNative ||
        _outputToken == NATIVE;

    uint256 volFee = (_minReturn * 10000) / (10000 - _feeBps) - _minReturn;

    uint256 volFeeInNative = isOutputNative
        ? volFee
        : _getAmountOut(volFee, feePathToNative, _uniRouter);

    return fee < volFeeInNative + _maxTxFeeInNative;
}
...
import {_isFeeOk} from "../functions/FHandlerUtils.sol";
import {_getAmountIn} from "../functions/uniswap/FUniswapGeneral.sol";
import "../handlers/uniswap/UniswapV2Router02Handler.sol";
    ...
    Proxied
{
    ...
    uint256 public maxTxFeeInNative;
    ...
    function initialize(uint256 _feeBps, uint256 _maxTxFeeInNative)
        external
        initializer
    {
        ...
        slippageBps = 500;
        feeBps = _feeBps;
        maxTxFeeInNative = _maxTxFeeInNative;
    }
    ...
    function execute(
        ...
        // Use user defined Handler so bots can not parsse bad handler
        UniswapV2Router02Handler handler = UniswapV2Router02Handler(_handler);
        ...
        require(
            _isFeeOk(
                ...
                _minReturn,
                feeBps,
                handler.WRAPPED_NATIVE(),
                handler.UNI_ROUTER(),
                maxTxFeeInNative
            ),
            "StopLimitOrders#canExecute:FEE_TO_HIGH"
        );
    ...
    function setFeeBps(uint256 _feeBps) external onlyProxyAdmin {
        feeBps = _feeBps;
    }

    function setMaxTxFeeInNative(uint256 _maxTxFeeInNative)
        external
        onlyProxyAdmin
    {
        maxTxFeeInNative = _maxTxFeeInNative;
    ...
    function canExecute(
        ...
        require(
            ...
            "StopLimitOrders#execute:FEE_TO_HIGH"
        );
