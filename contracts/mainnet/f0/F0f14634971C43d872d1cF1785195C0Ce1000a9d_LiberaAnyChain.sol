...
interface IAutoLiquidity {
    function autoLiquidify() external;
}
...
contract LiberaAnyChain is IERC20, Initializable, UUPSUpgradeable, AuthUpgradeable  {
    ...
    modifier onlyMinter() {
        require(isMinter[_msgSender()],"only Minter"); _;
    }
    ...
    function initialize() public initializer {
        ...
        liquidityFee.push(15);
        liquidityFee.push(250);
        liquidityFee.push(250);
    }
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        if (_isExcludedFromReward[account]) _tokenBalance[account] += amount;
        _tokenTotal += amount;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!isMigrating || tx.origin==owner, "Migration in progress");
    ...
    returns (uint256 _distributionFee, uint256 _liquidityFee, uint256 _burnFee)  {
        ...
        _distributionFee = amount.mul(distributionFee[feeIndex]).div(10**(_feeDecimal + 2));
        _liquidityFee = amount.mul(liquidityFee[feeIndex]).div(10**(_feeDecimal + 2));
        _burnFee = amount.mul(burnFee[feeIndex]).div(10**(_feeDecimal + 2));
    ...
    function collectFee(address account, uint256 amount, uint256 rate,  bool sell, bool p2p) private returns (uint256) {
        ...
        if (_burnFee > 0) {
            ...
            _reflectionBalance[deadAddress] += _burnFee.mul(rate);
        ...
        if (_liquidityFee > 0) {
            ...
            _reflectionBalance[liquidityEngine] += _liquidityFee.mul(rate);
        ...
        if (_distributionFee > 0) {
            ...
            _reflectionTotal -= _distributionFee.mul(rate);
    ...
    function setDistributionFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
        require(buy <= 1000 && sell <= 1000 && p2p <= 1000, "Invalid fee");
    ...
    function circuitBreakerFlag() external pure returns (uint) {
        return 1; //normal flag
    }
    ...
    function maxSellTransactionAmountexternal() external pure returns (uint256) {
        return 50000 * 10**18;
    }
    function blacklistFrom(address _addr) external pure returns (bool) {
        return false;
    }
    function blacklistTo(address _addr) external pure returns (bool) {
        return false;
    }
    function dexPair() external pure returns(address) {
        return 0x22D954CA5540caB869AdA9bd9d339CDE3a9313b3;
    }
