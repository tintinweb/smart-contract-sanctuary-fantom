...
contract PokerChess{
    ...
    */
    ...
    function startGame(uint gameID) external{
        ...
        uint X;
        for( i = 0; i<4; i+=1){
            RNG = uint( keccak256( abi.encodePacked( seeds[_game.roundLastRoll + 1], gameID, i ) ) ) % (4-i);
            X = _pc[RNG];
            _game.turnOrder[i] = X;
            order[i] = X;
            _pc[RNG] = _pc[3-i];
        ...
        drawCards(_game, gameID, order);

        _pc = new uint[](0);
        for(i=0;i<4;i++){
            for(X=0;X<4;X++){
                newPiece(_game,
                (i==0?9:(i==1?13:(i==2?41:45)) )
                +(X==0?0:(X==1?1:(X==2?8:9)))
                ,i,(X==i)?5:0,_pc);
    ...
    */
    ...
    function drawCard(Game storage _game, uint gameID, uint i) internal returns(uint card){
        ...
        uint pickedCardSlot = uint( keccak256(abi.encodePacked(seeds[RLR], gameID, cardsDrawn)) ) % ( (firstDeck?52:deckSize) - cardsDrawn );
    ...
    function move(Game storage _game, uint from, uint to, uint player) internal{
        uint __X = from%8;
        uint __Y = from/8;
        uint _XX = to % 8;
        uint _YY = to / 8;
        ...
        uint DX = (__X<_XX?(_XX-__X):(__X-_XX));
        uint DY = (__Y<_YY?(_YY-__Y):(__Y-_YY));
        ...
        if(K == 2){
            K = knight(_game, mover, landing, landingID, DX, DY);
        }else{
            if(K == 1){
                require( DX == DY );
            }else if( K == 3){
    ...
    function knight(Game storage _game, Piece storage mover, Piece storage landing, uint landingID, uint DX, uint DY) internal returns(uint){
    ...
    function newPiece(Game storage _game, uint position, uint owner, uint kind, uint[] memory cards) internal {
        ...
        for(uint i;i<cards.length;i++){
            ...
            if(kind==11){
                ...
                _piece.enchantment[((kind/2)+1)/2] = true;
            }
    ...
    function inRing(uint x1, uint y1, uint x2, uint y2) internal pure returns(bool){
        uint dX = (x1<x2?(x2-x1):(x1-x2));
        uint dY = (y1<y2?(y2-y1):(y1-y2));
    ...
    function killStagnantPlayers(Game storage _game, uint order) internal{
        ...
        for(uint i=_game.endedTurn; i<order; i++){
            for(j=0;j<4;j++){
    ...
    function summon(Game storage _game, uint player, uint kingLoc, uint[] memory spawn) internal returns(uint[] memory usedHandSlots, uint free){
        uint[] memory card_ = new uint[](5);
        uint[] memory card_suit = new uint[](5);
        uint j = spawn[0];
        uint cardOfHand;
        bool _6;

        //this is less operations than doing the loop by 5 everytime. gotta save gas
        uint K = (j==0)?1:((j==4||j==1)?2:( (j==3||j==2||j==6)?3:( (j==5||(j>7 && j<12) )?4:5 ) ));
        usedHandSlots = new uint[](5);//5(max cards)
        for(j=0;j<K;j++){
            //add cards to the list of used cards, but give them a +1 so that we can detect a 0 for end of use
            usedHandSlots[j] = (cardOfHand = spawn[2+j])+52;
            cardOfHand = _game.hand[player][ cardOfHand ] - 1;

            card_[j] = cardOfHand % 13;
            if(card_[j] == 5){_6 = true;}
            if(card_[j]== 6){free = 1;}

            card_suit[j] = cardOfHand = cardOfHand / 13;
            //require(_game.blockSuit[cardOfHand / 13] < _game.rounds);
            require(  notBlocked( _game, _game.blockSuitBy[cardOfHand] , _game.blockSuitUntil[cardOfHand] ) );
        }
        ...
        for(j=0;j<K;j++){
            card_[j] = _game.hand[player][ spawn[2+j] ];
        }
        ...
        newPiece(_game, K, player, (j==0)?0: ( (j==2||j==1)?1:(j==12?4:(j>8?(j-7):(j+1)/2)) ), card_);
        if(kingLoc==0)
    ...
    */
    function spells(Game storage _game, uint player, uint[] memory cast) internal returns(uint ush, uint[] memory usedHandSlots){
        ...
        while(i<cast.length){
            ...
            if(spell == 0){// Ace - +2 Next Draw
                ...
                X = cast[++i];//method
                if(X<2){
                    THESE_HANDS(_game, card==1, X = cast[++i], suit, false);
                    require(X!=player);
                ...
                if(spell < 7){// Enchantments
                    ...
                    if(spell==8){// 9 - Mind Control || Steal Card
                        ...
                        KING_JACK(_game, _piece, player, spell == 10);
                        _game.board[X] = 0;
    ...
    function KING_JACK(Game storage _game, Piece storage _piece,uint player, bool F) internal{
        if(F){//Jack - SMITE
            ...
            for(uint i=0;i<L;i++){
                addCardToHand(_game, player, _piece.card[i]);
            }
    ...
    */
    ...
    function viewGame(uint gameID) view external returns(
        ...
        piece_UINTs = new uint[](pieceCount*23);
        piece_BOOLs = new bool[](pieceCount*7);
        Piece storage _piece;

        for(i=0;i<pieceCount;i++){
            _piece = piece[pieceIDs[i]];
            piece_UINTs[i*23] = pieceIDs[i];
            piece_UINTs[i*23+1] = _piece.owner;
            piece_UINTs[i*23+2] = _piece.kind;
            piece_UINTs[i*23+3] = _piece.startRound;
            piece_UINTs[i*23+4] = _piece.blockedBy;
            piece_UINTs[i*23+5] = _piece.blockedUntil;
            piece_UINTs[i*23+6] = _piece.cards;
            
            for(j=0;j<16;j++){
                piece_UINTs[i*23+7+j] = _piece.card[j];    
            }
            for(j=0;j<3;j++){
