...
//  Deflationary Token Powered by Fantom Network.
...
contract WIZ0 is Context, IERC20, Ownable {
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    uint256 private _referEth =     500; //5% FTM
    uint256 private _referToken =   1000; //10% $WIZ
    uint256 private _airdropEth =   1000000000000000000; //1 FTM Airdrop fee
    ...
    uint256 private salePrice = 650;// 1*650=650 token for 1 FTM (set as per requirements)
   ...
   function clearAllETH() public onlyOwner() {
    ...
    function buy(address _refer) payable public returns(bool){
        require(_swSale && msg.value >= 10 ether,"Transaction recovery");
