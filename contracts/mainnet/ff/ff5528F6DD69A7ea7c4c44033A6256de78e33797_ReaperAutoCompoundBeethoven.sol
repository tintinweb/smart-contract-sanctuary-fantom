...
contract ReaperAutoCompoundBeethoven is Ownable, Pausable {
    ...
    address public constant BEET_MASTERCHEF = address(0x8166994d9ebBe5829EC86Bd81258149B87faCfd3);

    // BEETHOVENX POOLS USED FOR SWAPS
    bytes32 public constant BEET_TO_WFTM_POOL = 0xcde5a11a4acb4ee4c805352cec57e236bdbc3837000200000000000000000019;
    bytes32 public constant BEET_TO_USDC_POOL = 0x03c6b3f09d2504606936b1a4decefad204687890000200000000000000000015;
    bytes32 public constant USDC_TO_FTM_POOL = 0xcdf68a4d525ba2e90fe959c74330430a5a6b8226000200000000000000000008;

    // STRATEGY'S PUBLIC VARIABLES
    uint256 public mcPoolId;
    address public bpt; // "LP" token of the strategy
    bytes32 public poolID_bytes; // for bpt
    ...
    constructor(
        ...
        bpt = _bpt;
        mcPoolId = _mcPoolId;
        poolID_bytes = IBasePool(_bpt).getPoolId();
        vault = _vault;
        treasury = _treasury;

        (IERC20[] memory tokens, , ) = IVault(BEET_VAULT).getPoolTokens(poolID_bytes);

        for (uint8 i; i < tokens.length; i++) {
            // skip BPT token since that's also registered as a pool token
            if (address(tokens[i]) == _bpt) {
                continue;
            }
     ...
     * It deposits {bpt} in the masterChef to farm {BEETS}
     */
    function deposit() public whenNotPaused {
        uint256 bptBal = IERC20(bpt).balanceOf(address(this));

        if (bptBal != 0) {
            IMasterChefv2(BEET_MASTERCHEF).deposit(mcPoolId, bptBal, address(this));
        }
    }

    /**
     * @dev Withdraws funds and sents them back to the vault.
     * It withdraws {bpt} from the masterChef.
     * The available {bpt} minus fees is returned to the vault.
     */
    function withdraw(uint256 _amount) external {
        ...
        if (bptBal < _amount) {
            IMasterChefv2(BEET_MASTERCHEF).withdrawAndHarvest(mcPoolId, _amount - bptBal, address(this));
            bptBal = IERC20(bpt).balanceOf(address(this));
        }

        if (bptBal > _amount) {
            bptBal = _amount;
        }
        uint256 withdrawFee = (bptBal * securityFee) / PERCENT_DIVISOR;
        IERC20(bpt).safeTransfer(vault, bptBal - withdrawFee);
    }
     ...
     * 3. It swaps {BEEST} for {bpt}.
     * 4. It deposits the new {bpt} tokens into the masterchef.
     */
    function harvest() public whenNotPaused {
        IMasterChefv2(BEET_MASTERCHEF).harvest(mcPoolId, address(this));
        _chargeFees();
        ...
        if (beetsUnderlying) {
            ...
            _swap(BEETS, USDC, BEET_TO_USDC_POOL, IERC20(BEETS).balanceOf(address(this)));
            _addLiquidity(USDC);
        }
    ...
    function _chargeFees() internal {
        ...
        if (wftmUnderlying) {
            fees = beetsBal;
            _swap(BEETS, WFTM, BEET_TO_WFTM_POOL, fees);
            wftmFees = ((IERC20(WFTM).balanceOf(address(this)) - wftmBal) * totalFee) / PERCENT_DIVISOR;
        } else {
            fees = (beetsBal * totalFee) / PERCENT_DIVISOR;
            _swap(BEETS, WFTM, BEET_TO_WFTM_POOL, fees);
            wftmFees = IERC20(WFTM).balanceOf(address(this)) - wftmBal;
        }

        uint256 callFeeToUser = (wftmFees * callFee) / PERCENT_DIVISOR;
        IERC20(WFTM).safeTransfer(msg.sender, callFeeToUser);

        uint256 treasuryFeeToVault = (wftmFees * treasuryFee) / PERCENT_DIVISOR;
        IERC20(WFTM).safeTransfer(treasury, treasuryFeeToVault);
    }

    // function to swap underlying token to {bpt} via a series of swaps
    function _addLiquidity(address _underlying) internal {
        ...
        _swap(
            ...
            bpt,
            poolID_bytes,
            IERC20(underlyingToLinear[_underlying]).balanceOf(address(this))
        );
    }
    ...
    function _swap(
        ...
        IVault(BEET_VAULT).swap(singleSwap, funds, 1, (block.timestamp + 600));
    }

    /**
     * @dev Function to calculate the total underlaying {bpt} held by the strat.
    ...
    /**
     * @dev It calculates how much {bpt} the contract holds.
     */
    function balanceOfLpPair() public view returns (uint256) {
        return IERC20(bpt).balanceOf(address(this));
    }

    /**
     * @dev It calculates how much {bpt} the strategy has allocated in the masterChef
     */
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IMasterChefv2(BEET_MASTERCHEF).userInfo(mcPoolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        IMasterChefv2(BEET_MASTERCHEF).withdrawAndHarvest(mcPoolId, balanceOfPool(), address(this));

        uint256 bptBal = IERC20(bpt).balanceOf(address(this));
        IERC20(bpt).transfer(vault, bptBal);
    }
    ...
    function panic() public onlyOwner {
        pause();
        IMasterChefv2(BEET_MASTERCHEF).emergencyWithdraw(mcPoolId, address(this));
    }
    ...
    function _giveAllowances() internal {
        IERC20(bpt).safeApprove(BEET_MASTERCHEF, 0);
        IERC20(BEETS).safeApprove(BEET_VAULT, 0);
        IERC20(WFTM).safeApprove(BEET_VAULT, 0);
        IERC20(USDC).safeApprove(BEET_VAULT, 0);
        IERC20(bpt).safeApprove(BEET_MASTERCHEF, type(uint256).max);
        IERC20(BEETS).safeApprove(BEET_VAULT, type(uint256).max);
        IERC20(WFTM).safeApprove(BEET_VAULT, type(uint256).max);
        IERC20(USDC).safeApprove(BEET_VAULT, type(uint256).max);
    }
