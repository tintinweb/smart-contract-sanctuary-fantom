...
*/
contract Equation is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the EQUATION ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress)
        public
        ERC20("EQUATION", "EQUATION")
    {
        // Mints 1 EQUATION to contract creator for initial pool setup
    ...
    function _getEquationPrice()
        internal
        view
        returns (uint256 _equationPrice)
    {
        try IOracle(equationOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("Equation: failed to fetch EQUATION price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _equationPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (
                ...
                if (_equationPrice >= taxTiersTwaps[tierId]) {
                    require(
    ...
    function setEquationOracle(address _equationOracle)
        public
        onlyOperatorOrTaxOffice
    {
        require(
            _equationOracle != address(0),
    ...
    /**
     * @notice Operator mints EQUATION to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of EQUATION to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEquationPrice = _getEquationPrice();
            currentTaxRate = _updateTaxRate(currentEquationPrice);
            if (currentEquationPrice < burnThreshold) {
                burnTax = true;
            }
        }
