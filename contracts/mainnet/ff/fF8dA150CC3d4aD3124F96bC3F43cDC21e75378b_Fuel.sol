...
contract Fuel is ERC20, Ownable, ReentrancyGuard{

    // 1 $FUEL = 10 $PEARL
    uint256 public EXCHANGE_RATE = 10 ether;
    ...
    */
    function buy(address buyer, uint256 amountPearl) public nonReentrant{
        uint256 amountInWei = amountPearl * 1 ether;

        require(amountPearl > 0,"Cannot buy from 0");
        require(amountPearl >= 10,"Cannot buy less than 10 $PEARL of $FUEL");
        require(pearl.balanceOf(buyer) >= amountInWei,"No enought $PEARL to buy $FUEL");

        // Burn $PEARL tokens from buyer address
        pearl.burn(buyer, amountInWei);

        // Apply exchange rate
        uint256 fuelToBuy = amountInWei / EXCHANGE_RATE;

        // Send $FUEL tokens to "to"
        _mint(buyer, fuelToBuy * 1 ether);
    }

    /**
    * Mint 40 $FUEL for starting
    */
    function mintStart(address receiver) public nonReentrant{
        require(controllers[msg.sender], "Only controllers mint fuel");
