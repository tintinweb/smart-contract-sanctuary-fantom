...
contract GeistFtmMimLeveraged is DefiiWithCustomEnter {
    ...
    IERC20 constant variableDebtMIM =
        IERC20(0xe6f5b2d4DE014d8fa4c45b744921fFDf13f15D4a);

    IGeistPool constant geistPool =
        IGeistPool(0x9FAD24f572045c7869117160A571B2e50b10d068);
    uint256 constant VARIABLE_INTEREST_RATE_MODE = 2;
    IGeistDataProvider constant dataProvider =
        IGeistDataProvider(0xf3B0611e2E4D2cd6aB4bb3e01aDe211c3f42A8C3);
    ILendingPoolAddressesProvider constant addressesProvider =
        ILendingPoolAddressesProvider(
            0x6c793c628Fe2b480c5e6FB7957dDa4b9291F9c9b
        );
    IGeistChefIncentivesController constant incentiveController =
        IGeistChefIncentivesController(
            0x297FddC5c33Ef988dd03bd13e162aE084ea1fE57
        );

    IGeistChefMultiFeeDistribution constant multiFeeDistributor =
        IGeistChefMultiFeeDistribution(
    ...
    function _enterWithParams(bytes memory params) internal override {
        ...
        for (uint8 i = 0; i < loopCount; i++) {
            ...
            (, , uint256 availableBorrowsEth, , , ) = geistPool
                .getUserAccountData(address(this));
            amount = (((((amount * price) / 1e18) * 1e18) / price) * ltv) / 100;
            amount = amount < (availableBorrowsEth * 1e18) / price
                ? amount
                : (availableBorrowsEth * 1e18) / price;
            geistPool.borrow(
                address(MIM),
                amount,
    ...
    function _claim(bool claimWithPenalty) internal {
        ...
        if (claimWithPenalty) {
            ...
            uint256 unlockedBalance = multiFeeDistributor.unlockedBalance(
                address(this)
            );
        ...
        for (uint8 i = 0; i < assets.length; i++) {
            if (
                assets[i] != address(gMIM) &&
                assets[i] != address(variableDebtMIM)
            ) {
