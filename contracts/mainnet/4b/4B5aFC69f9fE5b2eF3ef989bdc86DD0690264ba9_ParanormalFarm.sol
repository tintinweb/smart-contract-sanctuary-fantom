...
contract ParanormalFarm is Ownable {
...
// How many pools are in the contract
    ...
    // Deposit staking tokens to ParanormalFarm for ICE allocation.
    ...
    // Withdraw staked tokens from ParanormalFarm.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "ParanormalFarm: you cant withdraw that much");
        updatePool(_pid);
        uint256 pending =
            user.amount * pool.accIcePerShare / 1e12 - user.rewardDebt + user.remainingIceTokenReward;
        user.remainingIceTokenReward = safeRewardTransfer(msg.sender, pending);
