...
// File contracts/EliteUsdcEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EliteUsdcEqualizerV2 is Ownable, Pausable {
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalBal, 0, equalToUsdcPath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokens(equalBal, 0, equalToElitePath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(usdc).balanceOf(address(this));
        uint256 t2Bal = IERC20(elite).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(usdc, elite, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
