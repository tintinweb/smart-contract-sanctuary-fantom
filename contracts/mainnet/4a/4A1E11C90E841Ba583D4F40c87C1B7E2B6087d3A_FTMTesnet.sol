...
contract FTMTesnet {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 0.001 ether; // 0.001 FTM
	uint256[] public REFERRAL_PERCENTS = [80, 40, 20];
	uint256 constant public PROJECT_FEE = 75;
	...
	constructor(address payable wallet, address payable _developer) public {
        ...
        plans.push(Plan(120, 19)); // 1.9% per day for 120 days
        plans.push(Plan(160, 16)); // 1.6% per day for 160 days
        plans.push(Plan(200, 10)); // 1% per day for 200 days
		plans.push(Plan(120, 19)); // 1.9% per day for 120 days (at the end, compounding)
        plans.push(Plan(160, 16)); // 1.6% per day for 160 days (at the end, compounding)
        plans.push(Plan(200, 10)); // 1% per day for 200 days (at the end, compounding)
	}
	...
	function getResult(uint8 plan, uint256 deposit) public view returns (uint256 percent, uint256 profit, uint256 finish) {
	 ...
	 function getUserPercentRate(address userAddress) public view returns (uint) {
        ...
        uint256 timeMultiplier = block.timestamp.sub(user.holdBonusCheckpoint).div(TIME_STEP); // +0.01% per day
