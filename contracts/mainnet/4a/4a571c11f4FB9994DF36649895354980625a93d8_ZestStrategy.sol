...
abstract contract StrategyBase is IStrategy, ReentrancyGuard, Initializable {
    ...
    IUniswapV2Router02 public unirouter =
        IUniswapV2Router02(0xF491e7B69E4244ad4002BC14e878a34207E38c29); //mainnet
    IFirebirdRouter public firebirdRouter =
        IFirebirdRouter(0xe0C38b2a8D09aAD53f1C67734B9A95E43d5981c0); //mainnet
    // IUniswapV2Router02 public unirouter = IUniswapV2Router02(0xa6AD18C2aC47803E193F75c3677b14BF19B94883); //testnet
    // IFirebirdRouter public firebirdRouter = IFirebirdRouter(0xe0C38b2a8D09aAD53f1C67734B9A95E43d5981c0); //testnet
    ...
    function setUnirouter(IUniswapV2Router02 _unirouter) external onlyTimelock {
    ...
    function _swapTokens(
        ...
        if (_input == _output || _amount == 0) {
            if (_receiver != address(this) && _amount != 0)
                IERC20(_input).safeTransfer(_receiver, _amount);
            return _amount;
        }
        ...
        if (path.length > 0) {
            ...
            firebirdRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                _input,
                _output,
                _amount,
                1,
    ...
    function harvest(address _mergedStrategy) external virtual override nonReentrant {
        require(
            msg.sender == controller || msg.sender == strategist || msg.sender == governance,
            "!authorized"
        );
        ...
        if (_targetCompoundBal > 0) {
            if (_mergedStrategy != address(0)) {
                ...
                if (_performanceFee > 0 && _reserveFund != address(0)) {
                    ...
                    _reserveFundAmount = _swapTokens(
                        _targetCompoundToken,
                        _targetProfitToken,
                        _reserveFundAmount,
                        _reserveFund
                    );
                ...
                if (_gasFee > 0 && _performanceReward != address(0)) {
                    _swapTokens(
                        _targetCompoundToken,
                        _targetProfitToken,
                        _targetCompoundBal.mul(_gasFee).div(10000),
                        _performanceReward
                    );
                }
    ...
    function executeTransaction(
        ...
        require(
            success,
            string(
                abi.encodePacked(getName(), "::executeTransaction: Transaction execution reverted.")
            )
        );
...
*/
contract ZestStrategy is StrategyBase {
    using Address for address;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public farmPool = 0x0895196562C7868C5Be92459FaE7f877ED450452; //example
    uint256 public poolId;
    address[] public farmingTokens;

    address public token0 = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82; //example
    address public token1 = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; //example
    ...
    function _deposit() internal {
        ...
        if (_baseBal > 0) {
            IZestChef(farmPool).deposit(poolId, _baseBal, address(this));
            emit Deposit(baseToken, _baseBal);
        }
    }

    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        (uint256 _stakedAmount, ) = IZestChef(farmPool).userInfo(poolId, address(this));
        if (_amount > _stakedAmount) {
            _amount = _stakedAmount;
        }

        uint256 _before = IERC20(baseToken).balanceOf(address(this));
        IZestChef(farmPool).withdraw(poolId, _amount, address(this));
    ...
    function _withdrawAll() internal override {
        (uint256 _stakedAmount, ) = IZestChef(farmPool).userInfo(poolId, address(this));
        IZestChef(farmPool).withdrawAndHarvest(poolId, _stakedAmount, address(this));
    }

    function claimReward() public override {
        IZestChef(farmPool).harvest(poolId, address(this));
        IZestStaking(IZestChef(farmPool).rewardMinter()).emergencyWithdraw();
    ...
    function _addLiquidity() internal {
        ...
        if (_amount0 > 0 && _amount1 > 0) {
            IUniswapV2Router02(unirouter).addLiquidity(_token0, _token1, _amount0, _amount1, 1, 1, address(this), block.timestamp + 1);
        }
    }

    function balanceOfPool() public view override returns (uint256) {
        (uint256 amount, ) = IZestChef(farmPool).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function claimable_tokens() external view override returns (address[] memory farmToken, uint256[] memory totalDistributedValue) {
        ...
        totalDistributedValue[0] = IZestChef(farmPool).pendingReward(poolId, address(this));
    }
    ...
    function claimable_token() external view override returns (address farmToken, uint256 totalDistributedValue) {
        ...
        totalDistributedValue = IZestChef(farmPool).pendingReward(poolId, address(this));
    }
    ...
    function retireStrat() external override onlyStrategist {
        IZestChef(farmPool).emergencyWithdraw(poolId, address(this));
