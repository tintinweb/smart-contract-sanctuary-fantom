...
contract FTMStaking is Initializable, OwnableUpgradeable, UUPSUpgradeable {
        ...
        )
    {
        ...
        while (amount > 0) {
            ...
            if (
                ...
                amountToUndelegate == amountToReduce &&
                amountToUnlock != amount
            ) {
                // this vault's unlockable balance is exactly the amount needed
                emptyCount = 1;
            }
        ...
        return (info, emptyCount + index - 1, implicitPenalty);
    }
     ...
     */
    function claimRewardsAll() public {
     ...
     */
    function claimRewards(uint256 vaultIndex) external returns (bool) {
        require(!maintenancePaused, "ERR_THIS_FUNCTION_IS_PAUSED");
    ...
    function _claim(address payable vault) internal returns (bool) {
        uint256 balanceBefore = address(this).balance;
        try Vault(vault).claimRewards() {
            if (protocolFeeBIPS > 0) {
                uint256 protocolFee = ((address(this).balance - balanceBefore) *
        ...
        )
    {
        uint256 toValidatorID = Vault(vault).toValidatorID();
        uint256 vaultLockedAmount = Vault(vault).getLockedStake();
        uint256 amountUnlockable = vaultLockedAmount -
        ...
        if (amountUnlockable > amount) {
            // amount unlockable is more than amount needed, this is the last vault
            uint256 vaultPenalty = SFCPenalty.getUnlockPenalty(
                SFC,
                vault,
                toValidatorID,
                amount,
            ...
            return (amount, amount - vaultPenalty, amount);
        }
        return (vaultLockedAmount, amountUnlockable, amountUnlockable);
    }
