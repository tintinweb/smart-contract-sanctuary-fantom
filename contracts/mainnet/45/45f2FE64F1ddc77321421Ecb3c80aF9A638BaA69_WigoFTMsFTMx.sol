...
// File: contracts/WigoFTMsFTMx.sol
...
contract WigoFTMsFTMx is Ownable, Pausable {
    ...
    address[] public wrappedTosFTMxPath;
    ...
    uint256 public TREASURY_FEE = 720;                       // 70% of Platform fee
    uint256 public CALL_FEE = 100;                           // 10% of Platform fee  
    uint256 public STRATEGIST_FEE = 180;                     // 20% of Platform fee

    // Events
    event Harvest(address indexed harvester);
    event SetVault(address indexed newVault);
    event SetWigoToWrappedPath(address[] indexed path);
    event SetWrappedTosFTMxPath(address[] indexed path);
    ...
    constructor(
        ...
        wrappedTosFTMxPath = [wrapped, sftmx];
    ...
    function addLiquidity() internal {
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedTosFTMxPath, address(this), block.timestamp);
    
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(sftmx).balanceOf(address(this));

        IUniSwapRouter(unirouter).addLiquidity(wrapped, sftmx, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(sftmx).safeApprove(unirouter, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(sftmx).safeApprove(unirouter, type(uint).max);
    }
    ...
    function setWrappedToLp0Route(address[] calldata _path) external onlyAdmin {
        require (_path[0] == wrapped && _path[_path.length - 1] == sftmx, "Bad path || !auth");

        wrappedTosFTMxPath = _path;

        emit SetWrappedTosFTMxPath(_path);
    }
