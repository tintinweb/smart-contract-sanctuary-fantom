...
import "../../immutable/IMasterChef.sol";
import "../../immutable/IStrategy.sol";
 ...
 */
contract CombAutoCompoundTombMai is
    Initializable,
    OwnableUpgradeable,
    PausableUpgradeable
{
     ...
     * {lpToken0, lpToken1} - Tokens that the strategy maximizes. ISpookySwapV2Pair tokens.
     */
    ...
    address public constant tombRouter =
    ...
    /**
     * @dev Comb Contracts:
     * {treasury} - Address of the Comb treasury
    ...
    function initialize(
        ...
        tshareToWftmPath = [rewardToken, wftm];
    ...
    function deposit() public whenNotPaused {
        uint256 pairBal = IERC20Upgradeable(lpPair).balanceOf(address(this));
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {
            ...
            pairBal = IERC20Upgradeable(lpPair).balanceOf(address(this));
        }
        ...
        IERC20Upgradeable(lpPair).safeTransfer(vault, pairBal.sub(withdrawFee));
    }
    ...
    function chargeFees() internal {
        uint256 toWftm = IERC20Upgradeable(rewardToken)
            .balanceOf(address(this))
            .mul(totalFee)
            .div(PERCENT_DIVISOR);
        uint256 stayReward = toWftm.mul(treasuryFee).div(PERCENT_DIVISOR);

        IUniswapRouterETH(uniRouter)
            .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                toWftm.sub(stayReward),
                0,
                tshareToWftmPath,
                address(this),
        ...
        IERC20Upgradeable(wftm).safeTransfer(msg.sender, wftmBal);
        IERC20Upgradeable(rewardToken).safeTransfer(treasury, stayReward);
    }
    ...
    function addLiquidity() internal {
        // reward token to ftm
        uint256 tshareBal = IERC20Upgradeable(rewardToken).balanceOf(
            address(this)
        );
        _swap(tshareBal, tshareToWftmPath, uniRouter);

        uint256 wftmBal = IERC20Upgradeable(wftm).balanceOf(address(this));
        _swap(wftmBal, wftmToTombPath, uniRouter);

        uint256 tombHalf = IERC20Upgradeable(lpToken0).balanceOf(
            address(this)
        ) / 2;
        _swap(tombHalf, tombToMaiPath, tombRouter);

        uint256 lp0Bal = IERC20Upgradeable(lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20Upgradeable(lpToken1).balanceOf(address(this));
        IUniswapRouterETH(tombRouter).addLiquidity(
            lpToken0,
            lpToken1,
            lp0Bal,
            lp1Bal,
    ...
    function balanceOfLpPair() public view returns (uint256) {
        return IERC20Upgradeable(lpPair).balanceOf(address(this));
    }
    ...
    function retireStrat() external {
        ...
        uint256 pairBal = IERC20Upgradeable(lpPair).balanceOf(address(this));
        IERC20Upgradeable(lpPair).transfer(vault, pairBal);
    }
    ...
    function giveAllowances() internal {
        IERC20Upgradeable(lpPair).safeApprove(masterChef, type(uint256).max);
        ...
        IERC20Upgradeable(lpToken0).safeApprove(uniRouter, type(uint256).max);
        IERC20Upgradeable(lpToken0).safeApprove(tombRouter, 0);
        IERC20Upgradeable(lpToken0).safeApprove(tombRouter, type(uint256).max);

        IERC20Upgradeable(lpToken1).safeApprove(uniRouter, 0);
        IERC20Upgradeable(lpToken1).safeApprove(uniRouter, type(uint256).max);
        IERC20Upgradeable(lpToken1).safeApprove(tombRouter, 0);
        IERC20Upgradeable(lpToken1).safeApprove(tombRouter, type(uint256).max);

        IERC20Upgradeable(wftm).safeApprove(uniRouter, 0);
        IERC20Upgradeable(wftm).safeApprove(uniRouter, type(uint256).max);
        IERC20Upgradeable(wftm).safeApprove(tombRouter, 0);
        IERC20Upgradeable(wftm).safeApprove(tombRouter, type(uint256).max);
    }

    function removeAllowances() internal {
        IERC20Upgradeable(lpPair).safeApprove(masterChef, 0);
    ...
    function rescueTokens(address token) external onlyOwner returns (bool) {
