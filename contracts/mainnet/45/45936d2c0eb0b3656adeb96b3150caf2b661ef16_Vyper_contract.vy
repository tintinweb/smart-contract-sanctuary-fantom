...
@external
@view
def locked(_addr: address) -> uint256:
    _local_locked: uint256 = 0 
    _locked: LockedBalance = empty(LockedBalance)

    for i in range(100):
        if(i >= self.voting_escrow_count):
            break
        _locked =  VotingEscrow(self.voting_escrows[i]).locked(msg.sender)
        _local_locked += convert(_locked.amount, uint256)
    ...
    for i in range(100):
        ...
        if(_chainId != 0):
            for j in range(100):
                if(j >= totEscrow):
                    break
                _locked = self.mirrored_locks[msg.sender][_chainId][j]
                _local_locked += convert(_locked.amount, uint256)
