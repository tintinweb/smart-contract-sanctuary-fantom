...
contract DevFundUni is Ownable, IDistributor {
  ...
  constructor(IUni _uni, address _treasuryFund, address[] memory accounts, uint256[] memory points) public {
    uni = _uni;
  ...
  function claim() external onlyOwner {
    _setLock(false);
    uint256 treasuryAmount = uni.unclaimedTreasuryFund();
    uni.setTreasuryFund(address(this));
    uni.claimRewards();
    uni.transfer(treasuryFund, treasuryAmount);
    _setLock(true);
  }
  ...
  function allocate(uint256 amount) external {
    ...
    for (uint256 a; a < allocations.length; a++) {
      uni.transfer(allocations[a].account, amount * 1e18 * allocations[a].points / totalPoints);
    }
  }
