...
contract MyBal is StratManager, FeeManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    function balancerSwap(address addr, bytes32 _poolId, address _tokenIn, address _tokenOut, uint256 _amountIn) public returns (uint256) {
        funds = IBalancerVault.FundManagement(addr, true, payable(addr), true);
        IBalancerVault.SingleSwap memory singleSwap = IBalancerVault.SingleSwap(_poolId, swapKind, _tokenIn, _tokenOut, _amountIn, "");
        return IBalancerVault(unirouter).swap(singleSwap, funds, 1, now);
    }
    ...
    function _giveAllow(address token, address spender) public {
