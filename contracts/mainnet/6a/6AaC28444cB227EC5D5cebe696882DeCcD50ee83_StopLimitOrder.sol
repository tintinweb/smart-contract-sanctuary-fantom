...
contract StopLimitOrder is BoringOwnable, BoringBatchable {
    ...
    constructor(uint256 _externalOrderFee, ICoffinBox _coffinBox) public {
        uint256 chainId;
        assembly {
            chainId := chainid()
        }
    ...
    function _preFillOrder(OrderArgs memory order, IERC20 tokenIn, IERC20 tokenOut, ILimitOrderReceiver receiver) internal returns (bytes32 digest, uint256 amountToBeReturned, uint256 amountToBeFilled) {
        ...
        coffinBox.transfer(tokenIn, order.maker, address(receiver), coffinBox.toShare(tokenIn, amountToBeFilled, true));
    ...
    internal returns(uint256 _feesCollected){
        ...
        require(coffinBox.balanceOf(tokenOut, address(this)) >= coffinBox.toShare(tokenOut, amountToBeReturned.add(fee), false).add(_feesCollected), "Limit: not enough");
    }
    ...
    public {
        ...
        coffinBox.transfer(tokenOut, address(this), order.recipient, coffinBox.toShare(tokenOut, amountToBeReturned, false));
    ...
    public {
        ...
        feesCollected[tokenOut] = _feesCollected.add(coffinBox.toShare(tokenOut, fee, false));
    ...
    external {
        ...
        Rebase memory coffinBoxTotals = coffinBox.totals(tokenOut);

        for(uint256 i = 0; i < order.length; i++) {
            coffinBox.transfer(tokenOut, address(this), order[i].recipient, coffinBoxTotals.toBase(amountToBeReturned[i], false));
        }
    }
    ...
    external {
        ...
        feesCollected[tokenOut] = _feesCollected.add(coffinBox.toShare(tokenOut, totalFee, false));
    ...
    function swipeFees(IERC20 token) public {
        ...
        uint256 balance = coffinBox.balanceOf(token, address(this)).sub(1);
        coffinBox.transfer(token, address(this), feeTo, balance);
