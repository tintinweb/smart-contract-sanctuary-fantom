...
import "../utils/IUniswapRouterETH.sol";
...
contract StrategyCurveLP is StratManager, FeeManager {
    ...
    constructor(
        ...
        address _vault,
        address _unirouter
    ) StratManager(msg.sender, msg.sender, _unirouter, _vault, msg.sender) public {
        want = _want;
        rewardsGauge = _gauge;
        pool = _pool;
        poolSize = _poolSize;
    ...
    function chargeFees() internal {
        ...
        uint256 nativeFeeBal = IERC20(native).balanceOf(address(this)).mul(totalFee).div(MAX_FEE);
        uint256 yeastFeeAmount = nativeFeeBal.mul(yeastFee).div(MAX_FEE);
        uint256 partnerFeeAmount = nativeFeeBal.mul(partnerFee).div(MAX_FEE);
