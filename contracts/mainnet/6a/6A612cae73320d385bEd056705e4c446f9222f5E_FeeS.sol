...
contract FeeS is BalanceManagement {
    ...
    event SetFeeDistributionMTLockersPart(uint256 value);
    ...
    constructor(
        ...
        _setFeeDistributionMTLockersPart(_feeDistributionMTLockersPart);
    ...
    // Value in millipercent
    function setFeeDistributionMTLockersPart(uint256 _value) external onlyManager {
        _setFeeDistributionMTLockersPart(_value);
    }
        ...
        for (uint256 assetIndex; assetIndex < assetListLength; assetIndex++) {
            ...
            if (assetBalance > 0) {
                ...
                uint256 feeDistributionMTLockersAmount = (assetBalance *
                    feeDistributionMTLockersPart) / MILLIPERCENT_FACTOR;
                uint256 feeDistributionLPLockersAmount = (assetBalance *
                    feeDistributionLPLockersPart) / MILLIPERCENT_FACTOR;
                ...
                IAssetReceiver(feeDistributionMTLockersAddress).receiveAsset(
                    assetToken,
                    feeDistributionMTLockersAmount
                );
                safeApprove(assetToken, feeDistributionMTLockersAddress, 0);
                ...
                IAssetReceiver(feeDistributionLPLockersAddress).receiveAsset(
                    assetToken,
                    feeDistributionLPLockersAmount
                );
    ...
    function _setFeeDistributionMTLockersPart(uint256 _value) private {
        if (_value > MILLIPERCENT_FACTOR) {
            revert PartValueError();
        }

        feeDistributionMTLockersPart = _value;

        emit SetFeeDistributionMTLockersPart(_value);
    }
