...
contract MetadataHandler is IMetadataHandler, Ownable {
    ...
    string public constant OR_METADATA = "https://revest.mypinata.cloud/ipfs/QmaTg92jvZK12eq7qTbPRzg8jcWdcydBVKh7RnXqZumKcX";
    string public constant AL_METADATA = "https://revest.mypinata.cloud/ipfs/QmPaJFBaVc3XF8ox8xegZU6C8borsaJhf9WyEnTppqgEYH";
    ...
    function getOutputReceiverBytes(uint fnftId) external view override returns (bytes memory output) {
        ...
        string memory farmString = _formatAsset(farmedAsset);
        string memory payString = _formatAsset(paymentAsset);
        uint interestId = principalId + 1;
        uint dec = IERC20Detailed(farmedAsset).decimals();
        output = abi.encode(
            accruedInterest,    // 0
            poolId,             // 1
            farmString,         // 2
    ...
    function getAddressLockBytes(uint fnftId, uint) external view override returns (bytes memory output) {
        ...
        (,, uint sharesAtDeposit, bytes32 poolId) = instance.activated(fnftId);
