...
contract OrderFiller is Initializable {
    ...
    function fillBuyOrder(
        ...
        info.makerOrdersToFill = uint128(makerTakerTokenFillAmounts.length); // 1000 - 1
        info.makerMakerTokenAmountFilled = new uint128[](info.makerOrdersToFill);// 0
        info.makerTakerTokenAmountFilled = new uint128[](info.makerOrdersToFill);// 0

        // Fill stablecoin orders first
        takerOrder.makerToken.approve(exchangeProxyAddress, _orderData.totalMakerTokensToFill); // 1000
        // For every order submitted
        for (uint256 i = 0; i <= info.makerOrdersToFill - 1; i++) { // one run
            // call exchange proxy and fill orders
            (info._takerFill, info._makerFill) = _fill(
                makerOrders[i], //
                makerSignatures[i],
                makerTakerTokenFillAmounts[i] // 1000
            );
