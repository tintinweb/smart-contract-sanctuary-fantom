...
@external
def transferCapsule(_to: address, _tokenId: uint256, ):
    #transfer capsule from contract to original deployer's wallet
    #throw if msg.sender is not the deployer of the contract
    assert (self.owner == msg.sender)
    ERC721(BEARISHPOD_NFT_CONTRACT).transferFrom(self, _to, _tokenId)

@external
def transferHive(_to: address, _tokenId: uint256, ):
    #transfer hive from contract to original deployer's wallet
    #throw if msg.sender is not the deployer of the contract
    assert (self.owner == msg.sender)
    ERC721(BEARISHHIVE_NFT_CONTRACT).transferFrom(self, _to, _tokenId)