...
import './interfaces/ISkeletonStaker.sol';
import './libraries/IncentiveId.sol';
import './libraries/RewardMath.sol';
import './libraries/NFTPositionInfo.sol';
import './libraries/TransferHelperExtended.sol';

import './interfaces/ISkeletonPoolDeployer.sol';
...
/// @title Skeleton canonical staking interface
contract SkeletonStaker is ISkeletonStaker, Multicall {
    ...
    /// @inheritdoc ISkeletonStaker
    ISkeletonPoolDeployer public immutable override poolDeployer;
    /// @inheritdoc ISkeletonStaker
    ...
    /// @param _poolDeployer the Skeleton poolDeployer
    /// @param _nonfungiblePositionManager the NFT position manager contract address
    /// @param _maxIncentiveStartLeadTime the max duration of an incentive in seconds
    /// @param _maxIncentiveDuration the max amount of seconds into the future the incentive startTime can be set
    constructor(
        ISkeletonPoolDeployer _poolDeployer,
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward, int24 minimumWidth) external override {
        require(reward > 0, 'SkeletonStaker::createIncentive: reward must be positive');
        require(
            block.timestamp <= key.startTime,
            'SkeletonStaker::createIncentive: start time must be now or in the future'
        );
        require(
            key.startTime - block.timestamp <= maxIncentiveStartLeadTime,
            'SkeletonStaker::createIncentive: start time too far into future'
        );
        require(key.startTime < key.endTime, 'SkeletonStaker::createIncentive: start time must be before end time');
        require(
            key.endTime - key.startTime <= maxIncentiveDuration,
            'SkeletonStaker::createIncentive: incentive duration is too long'
        );

        require(minimumWidth > 0, "SkeletonStaker::createIncentive: minWidth must be larger than 0");
    ...
    function endIncentive(IncentiveKey memory key) external override returns (uint256 refund) {
        require(block.timestamp >= key.endTime, 'SkeletonStaker::endIncentive: cannot end incentive before end time');
        ...
        require(refund > 0, 'SkeletonStaker::endIncentive: no refund available');
        require(
            incentive.numberOfStakes == 0,
            'SkeletonStaker::endIncentive: cannot end incentive while deposits are staked'
        );
    ...
    /// @notice Upon receiving a Skeleton ERC721, creates the token deposit setting owner to `from`. Also stakes token
    ...
    function onERC721Received(
        ...
        require(
            ...
            'SkeletonStaker::onERC721Received: not a univ3 nft'
        );
    ...
    function transferDeposit(uint256 tokenId, address to) external override {
        require(to != address(0), 'SkeletonStaker::transferDeposit: invalid transfer recipient');
        address owner = deposits[tokenId].owner;
        require(owner == msg.sender, 'SkeletonStaker::transferDeposit: can only be called by deposit owner');
    ...
    function withdrawToken(
        ...
        require(to != address(this), 'SkeletonStaker::withdrawToken: cannot withdraw to staker');
        Deposit memory deposit = deposits[tokenId];
        require(deposit.numberOfStakes == 0, 'SkeletonStaker::withdrawToken: cannot withdraw token while staked');
        require(deposit.owner == msg.sender, 'SkeletonStaker::withdrawToken: only owner can withdraw token');
    ...
    function stakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        require(deposits[tokenId].owner == msg.sender, 'SkeletonStaker::stakeToken: only owner can stake token');
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        if (block.timestamp < key.endTime) {
            require(
                ...
                'SkeletonStaker::unstakeToken: only owner can withdraw token before incentive end time'
            );
        }
        ...
        require(liquidity != 0, 'SkeletonStaker::unstakeToken: stake does not exist');
    ...
    returns (uint256 reward, uint160 secondsInsideX128)
    {
        ...
        (uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity) = stakes(tokenId, incentiveId);
        require(liquidity > 0, 'SkeletonStaker::getRewardInfo: stake does not exist');
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        require(block.timestamp >= key.startTime, 'SkeletonStaker::stakeToken: incentive not started');
        require(block.timestamp < key.endTime, 'SkeletonStaker::stakeToken: incentive ended');

        bytes32 incentiveId = IncentiveId.compute(key);
        require(
            deposits[tokenId].tickUpper - deposits[tokenId].tickLower >= minWidths[incentiveId],
            "SkeletonStaker::stakeToken: range must be larger than minWidth"
        );
        require(
            incentives[incentiveId].totalRewardUnclaimed > 0,
            'SkeletonStaker::stakeToken: non-existent incentive'
        );
        require(
            _stakes[tokenId][incentiveId].liquidityNoOverflow == 0,
            'SkeletonStaker::stakeToken: token already staked'
        );

        (ISkeletonPool pool, int24 tickLower, int24 tickUpper, uint128 liquidity) =
        NFTPositionInfo.getPositionInfo(poolDeployer, nonfungiblePositionManager, tokenId);

        require(pool == key.pool, 'SkeletonStaker::stakeToken: token pool is not the incentive pool');
        require(liquidity > 0, 'SkeletonStaker::stakeToken: cannot stake token with 0 liquidity');
    ...
    function setMinimumWidth(IncentiveKey memory key, int24 minimumWidth) external override {
        require(msg.sender == key.refundee, "SkeletonStaker::setMinimumWidth: only refundee can set minimumWidth");
        require(minimumWidth > 0, "SkeletonStaker::setMinimumWidth: minWidth must be larger than 0");
        bytes32 incentiveId = IncentiveId.compute(key);
        minWidths[incentiveId] = minimumWidth;
...
/// @title An interface for a contract that is capable of deploying SkeletonSwap V3 Pools
/// @notice A contract that constructs a pool must implement this to pass arguments to the pool
/// @dev This is used to avoid having constructor arguments in the pool contract, which results in the init code hash
/// of the pool being constant allowing the CREATE2 address of the pool to be cheaply computed on-chain
interface ISkeletonPoolDeployer {
...
import './ISkeletonPoolDeployer.sol';
import './ISkeletonPool.sol';
import './IERC20Minimal.sol';

import './INonfungiblePositionManager.sol';
import './IMulticall.sol';

/// @title Skeleton Staker Interface
/// @notice Allows staking nonfungible liquidity tokens in exchange for reward tokens
interface ISkeletonStaker is IERC721Receiver, IMulticall {
    /// @param rewardToken The token being distributed as a reward
    /// @param pool The Skeleton pool
    ...
    /// @notice The Skeleton PoolDeployer
    function poolDeployer() external view returns (ISkeletonPoolDeployer);
    ...
    /// @notice Withdraws a Skeleton LP token `tokenId` from this contract to the recipient `to`
    /// @param tokenId The unique identifier of an Skeleton LP token
    ...
    /// @notice Event emitted when a Skeleton LP token has been staked
    /// @param tokenId The unique identifier of an Skeleton LP token
    /// @param liquidity The amount of liquidity staked
    /// @param incentiveId The incentive in which the token is staking
    event TokenStaked(uint256 indexed tokenId, bytes32 indexed incentiveId, uint128 liquidity);

    /// @notice Event emitted when a Skeleton LP token has been unstaked
    /// @param tokenId The unique identifier of an Skeleton LP token
    /// @param incentiveId The incentive in which the token is staking
...
library IncentiveId {
    ...
    function compute(ISkeletonStaker.IncentiveKey memory key) internal pure returns (bytes32 incentiveId) {
...
library NFTPositionInfo {
    /// @param poolDeployer The address of the Skeleton PoolDeployer used in computing the pool address
    /// @param nonfungiblePositionManager The address of the nonfungible position manager to query
    /// @param tokenId The unique identifier of an Skeleton LP token
    /// @return pool The address of the Skeleton pool
    /// @return tickLower The lower tick of the Skeleton position
    /// @return tickUpper The upper tick of the Skeleton position
    /// @return liquidity The amount of liquidity staked
    function getPositionInfo(
        ISkeletonPoolDeployer poolDeployer,
