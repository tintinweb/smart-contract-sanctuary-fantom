...
// File contracts/ERC721.Burned/ERC721.Burned.sol
...
contract ERC721Template is ERC721Enumerable, ERC721Royalty, Ownable {
    ...
    // supply settings
    uint256 public immutable MAX_SUPPLY = 210;
    uint256 public TOKEN_TO_BURN = 2;
    uint256 public LOADED_POSITION = 1;
    modifier whenReady() {
        require(LOADED_POSITION == MAX_SUPPLY, "not ready");
        _;
    }
    modifier whenNotReady() {
        require(LOADED_POSITION != MAX_SUPPLY, "ready");
        _;
    }

    // URI settings
    string public BASE_URI = "ipfs://QmWsFZyXkb64nxRnvq3vwFVgLudso4TgLKkumEifHKQB72/";
    string public URI_EXTENSION = ".json";

    IERC721Enumerable TOKEN = IERC721Enumerable(0x0DC238932032dE4ac7de445b533Be2a205892984);

    // Royalty settings
    IPaymentSplitter private immutable PAYMENT_SPLITTER;

    EnumerableSet.UintSet private MINTABLE;

    constructor() ERC721("Mutant Skellies", "MSKEL") {
        PAYMENT_SPLITTER = IPaymentSplitter(IPaymentSplitter(0xf9C61d6A9f0190EFeF82Cbb73b24002490966094).clone());
        PAYMENT_SPLITTER.initialize();
        PAYMENT_SPLITTER.addPayee(0xb1a9a840E7ED5b97e559aFCA416C566A6a0ad06F, 80);
        PAYMENT_SPLITTER.addPayee(_msgSender(), 20);
        PAYMENT_SPLITTER.transferOwnership(address(0));
    ...
    function remainingSupply() public view returns (uint256) {
        return MINTABLE.length();
    }

    function init() public whenNotReady {
        uint256 count = 0;
        for (LOADED_POSITION; LOADED_POSITION <= MAX_SUPPLY; LOADED_POSITION++) {
            MINTABLE.add(LOADED_POSITION);
            if (++count >= 50) {
                break;
            }
        }
    ...
    /**
     * @dev returns if approved to take ownership of Token
     */
    ...
    function isApproved(address account) public view returns (bool) {
        return TOKEN.isApprovedForAll(account, address(this));
    }
     ...
     */
    function mint(uint256[] memory tokensToBurn) public whenReady returns (uint256) {
        require(tokensToBurn.length == TOKEN_TO_BURN, "ERC721: must supply two to burn");
        require(TOKEN.isApprovedForAll(_msgSender(), address(this)), "ERC721: not approved");
        require(TOKEN.balanceOf(_msgSender()) >= TOKEN_TO_BURN, "ERC721: balance requirements unmet");

        // burn the tokens
        for (uint256 i = 0; i < tokensToBurn.length; i++) {
            TOKEN.safeTransferFrom(_msgSender(), address(1), tokensToBurn[i]);
        }

        return _mintERC721(_msgSender());
    }

    function _randomTokenId() internal view returns (uint256) {
        return MINTABLE.at(MINTABLE.length().randomize());
    }
