...
contract CollateralReserve is Ownable
{
    ...
    constructor() {
      ...
      // setBalancerV2Vault(_bvault);
      setRedemptionFees(20000);
      // setUniRouterV2(unirouter));
    }
    ...
    function setBalancerV2Vault(address _addr) public onlyOwner {
      require(_addr != address(0), 'Invalid address');
      bvault = _addr;
      emit BalancerV2VaultChanged(_addr);
    }
...
// contract CollateralizedNFT is ERC721,Ownable , ERC721Burnable, ERC721Enumerable , ReentrancyGuard
contract TheNFT is ERC721A, CollateralReserve, ReentrancyGuard // , ERC721ABurnable
{
    using SafeMath for uint256;
    ...
    function publicMint(uint256 quantity) public payable {
        ...
        require(quantity <= maxMintAmount, "max 5 ");
        require(supply + quantity <= maxSupply, "End of supply");
        
        require(balanceOf(msg.sender) + quantity <= maxMintAmount, "Your NFTs can't be over 5 ");
        
        // if (msg.sender != owner()) {
        //     require(msg.value >= cost * quantity, "Not enough funds for mint");
        //     require(balanceOf(msg.sender) + quantity <= maxMintAmount, "Your NFTs can't be over 5 ");
        // }
    ...
    function presaleMint(uint256 quantity) public payable {
        uint256 supply = totalSupply();
        uint256 cost = saleConfig.presalePrice * quantity;
        // mintCheck(quantity, supply, cost);
...
//   }
    ...
    //         {
    //             uint256 alloc = ERC20(collaterals[i]).balanceOf(address(this)).div(supply);
    //             uint256 fee = alloc.mul(burnFeePecent).div(1000000);
    //             ERC20(collaterals[i]).safeTransfer(msg.sender, alloc.sub(fee));
    //         }
        // }
    ...
    function withdrawERC20() public payable {
    ...
    // setPresalePrice
    function setPresalePrice(uint64 _val) public onlyOwner {
        saleConfig.presalePrice = _val;
    }
