...
contract TombGenesisRewardPool {
    ...
    uint256 public harvestTax = 2500;

    // TESTNET
    uint256 public tombPerSecond = 0.14467 ether; // 11000 TOMB / (1h * 60min * 60s)
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (_pending > 0) {
                uint256 _tax = _pending.mul(harvestTax).div(10000); //tax amount 25%
                uint256 _remainingHarvest = _pending.sub(_tax); 
                safeTombTransfer(daoFund, _tax);
                safeTombTransfer(_sender, _remainingHarvest);
