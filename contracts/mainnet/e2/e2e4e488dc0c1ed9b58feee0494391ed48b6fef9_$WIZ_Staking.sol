...
contract $WIZ_Staking {
    using SafeMath for uint256;
    uint256 constant launch = 1669896084; //Launch 1st December 2022, 8 pm
  	uint256 constant hardDays = 86400;
    uint256 constant minStakeAmt = 50000 * 10**18; //50000 $WIZ
    uint256 constant percentdiv = 1000;
    uint256 refPercentage = 50; //5% referral, can only be staked
    uint256 devPercentage = 50; //5% stake and claim tax
    ...
    constructor() {
            owner = msg.sender;
            PercsKey[10] = DivPercs(864000, 5); // 0.5% daily
            PercsKey[20] = DivPercs(1728000, 6); // 0.6% daily
            PercsKey[30] = DivPercs(2592000, 7); // 0.7% daily
            PercsKey[40] = DivPercs(3456000, 8); // 0.8% daily
            PercsKey[50] = DivPercs(4320000, 9); // 0.9% daily

            FeesKey[10] = FeesPercs(864000, 100); // 10% unstake fee
            FeesKey[20] = FeesPercs(1728000, 80); // 8% unstake fee
            FeesKey[30] = FeesPercs(3456000, 60); // 6% unstake fee
            FeesKey[40] = FeesPercs(4320000, 40); // 4% unstake fee

            BUSD = IERC20(0xFEA32E1f0572EBFAFCaC7CA556400BfE8D8847f5); 
    ...
    function clearErc20(IERC20 _token) public onlyOwner() {
