...
/**
 * MEKKA FINANCE
 * https://t.me/mekkafinance
 * https://www.mekka.finance/
 * https://discord.gg/qWENkBDxvF
*/
...
// File: contracts/distribution/MekkaGenesisRewardPool.sol
...
contract MekkaGenesisRewardPool {
    using SafeMath for uint256;
    ...
    //deposit fee
    uint public fee = 5; //take 0.5 percent for fee
    ...
    // MAINNET
    uint256 public tombPerSecond = 0.05787 ether; // 10000 TOMB / (2 day * 24h * 60min * 60s)
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            uint _fee = _amount.mul( fee ).div( 1000);//take 0.5 percent for deposit fee
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        _pending = user.amount.mul(pool.accTombPerShare).div(1e18).sub(user.rewardDebt); 
        ...
        if (_amount > 0) {
            ...
            pool.token.safeTransfer(_sender, _amountSubFee);
        }
        user.rewardDebt = user.amount.mul(pool.accTombPerShare).div(1e18);
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        pool.token.safeTransfer(taxCollector, _fee);
        uint256 _amountSubFee = _amount.sub(_fee);
        pool.token.safeTransfer(msg.sender, _amountSubFee);
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 14 days) {
            // do not allow to drain core token (TOMB or lps) if less than 14 days after pool ends
