...
contract EncrypTomb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 Encrypt-DAI LP -> ENCRYPT pool
    ...
    constructor(uint256 _taxRate, address _taxCollectorAddress)
        public
        ERC20("EncrypTomb Token", "ENCRYPT")
    {
    ...
    function _getEncryptPrice() internal view returns (uint256 _encryptPrice) {
        try IOracle(encryptOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("ENCRYPT: failed to fetch ENCRYPT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _encryptPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (
                ...
                if (_encryptPrice >= taxTiersTwaps[tierId]) {
                    require(
    ...
    function setencryptOracle(address _encryptOracle)
        public
        onlyOperatorOrTaxOffice
    {
        require(
            _encryptOracle != address(0),
    ...
    /**
     * @notice Operator mints ENCRYPT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ENCRYPT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentTombPrice = _getEncryptPrice();
...
import "./upgradeability/OwnerStorage.sol";
/**
 ...
 */
abstract contract Context is OwnerStorage {
