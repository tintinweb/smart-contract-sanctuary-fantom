...
contract WINGUnlocker is Ownable {
  ...
  function _updateLastUnlockBlock(address _account) internal {
    if (_lastUnlockBlock[_account] < startReleaseBlock) {
      _lastUnlockBlock[_account] = startReleaseBlock;
    }
  }

  function lockOf(address _account) public returns (uint256) {
    _updateLastUnlockBlock(_account);
  ...
  function canUnlockAmount(address _account) public returns (uint256) {
    uint256 lockedAmount = lockOf(_account);
    if (block.number < startReleaseBlock) {
      ...
      return lockedAmount.mul(releasedBlock).div(blockLeft);
    }
  }
