...
contract PublicOffering is Ownable {
    ...
    uint256 public constant MIN_BUY = 50 * 10**DECIMALS;
    uint256 public constant MAX_BUY = 50000 * 10**DECIMALS;

    // finalizer address
    address public immutable finalizer;

    // Total amount of USDC tokens for purchase
    ...
    function purchase(uint256 amount) external returns (bool) {
        ...
        require(
            amount.add(purchasedAmount[msg.sender]) <= MAX_BUY,
            "Amount exceeds max allocation: $50000"
        );
        require(
            amount.add(totalPurchased) <= hardCap,
