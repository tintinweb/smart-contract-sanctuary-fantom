...
contract BlifeGenesisWnfts is Ownable, ContractGuard {
    ...
    struct UserInfo {
        ...
        uint256 nftRewardDebt; // Deposit debt. See explanation below.
    }
    ...
    event NftDeposit(address indexed user, uint256 indexed pid, uint256 _tokenIds);
   
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event NftWithdraw(address indexed user, uint256 indexed pid, uint256 _tokenIds);
    ...
    constructor(
        ...
        if (_godNft != address(0)) godNft = IERC721(_godNft);
    ...
    function checkPoolDuplicate(IERC20 _token, IERC721 _nft) internal view {
    ...
    function add(
        ...
        checkPoolDuplicate(_token, _nft);
        if (_withUpdate) {
            massUpdatePools();
        }
...
//==============================deposit fee addition needed=========================================================
    ...
    function pendingBLIFENft(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.nftAmount.mul(accBLifePerShare).div(1e18).sub(user.nftRewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            ...
            if (pool.isNftPool){                       //====check if this is nft pool - calculate rewards using nftSupply
    ...
    function deposit(uint256 _pid, uint256 _amount) public onlyOneBlock {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        require (!pool.isNftPool);      //==================make sure this is not nft pool
    ...
    function nftDeposit(uint256 _pid, uint256 _amount, uint256[] calldata _tokenIds) external onlyOneBlock {
        require(_amount == _tokenIds.length);
        
        address _sender = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        require (pool.isNftPool);                         //==============make sure we stake nfts in pools that support nft staking
        UserInfo storage user = userInfo[_pid][_sender];
        updatePool(_pid);
        if (user.nftAmount > 0) {
            // transfer rewards to user if any pending rewards
            uint256 _pending = user.nftAmount.mul(pool.accBLifePerShare).div(1e18).sub(user.nftRewardDebt);
            if (_pending > 0) {
                // send pending reward to user, if rewards accumulating in _pending
                safeBLifeTransfer(_sender, _pending);
                emit RewardPaid(_sender, _pending);
         ...
         if (_amount > 0) {
            ...
            require( godNft.ownerOf(_tokenIds[i]) == _sender,"Can't stake tokens you don't own!");
            pool.nft.safeTransferFrom(_sender, address(this),  _tokenIds[i]);
            stakerAddress[_tokenIds[i]] = _sender;
            user.nftAmount = user.nftAmount + 1;
            emit NftDeposit(_sender, _pid, _tokenIds[i]);
            }
        }
        user.nftRewardDebt = user.nftAmount.mul(pool.accBLifePerShare).div(1e18);
    }

    // Withdraw tokens.
    function nftwithdraw(uint256 _pid, uint256 _amount, uint256[] calldata _tokenIds) external onlyOneBlock {
        require(_amount == _tokenIds.length);
        address _sender = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
         require (pool.isNftPool);         //==================make sure this is not nft pool
        UserInfo storage user = userInfo[_pid][_sender];
        updatePool(_pid);
        require(user.nftAmount >= _amount, "withdraw: not good");
        uint256 _pending = user.nftAmount.mul(pool.accBLifePerShare).div(1e18).sub(user.nftRewardDebt);
        if (_pending > 0) {
            safeBLifeTransfer(_sender, _pending);
            emit RewardPaid(_sender, _pending);
        ...
        if (_amount > 0) {
            ...
            user.nftAmount = user.nftAmount - 1;
            emit NftWithdraw(_sender, _pid, _tokenIds[i]);
            }
