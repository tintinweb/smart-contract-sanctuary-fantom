...
contract PokerChess{
    ...
    */
    ...
    function move(Game storage _game, uint from, uint to, uint player) internal returns(uint DY){
        ...
        if(K == 2){
            ...
            K = 0;
            DY = 0;//gliders don't get free kill. Which DY is repurposed to indicate
        }
    ...
    function knight(Game storage _game, Piece storage mover, uint landingID, uint DX, uint DY) internal returns(uint){
        ...
        if(landingID>0){
            if(!ally){
                ...
                if(mover.arrows>0 && !hasShield(landing) ){
                    mover.arrows-=1;
                    swapCode=1;
                }
                landingID = 0;
    ...
    */
    ...
    function KING_JACK(Game storage _game, Piece storage _piece,uint player, bool F) internal returns(bool hasAce){
        if(F){//Jack - SMITE
    ...
    */
    ...
    function viewGame(uint gameID) view external returns(
        ...
        for(i=0;i<pieceCount;i++){
            ...
            piece_UINTs[i*24+23] = _piece.arrows;
