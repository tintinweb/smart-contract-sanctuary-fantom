/**
 *Submitted for verification at FtmScan.com on 2023-02-16
*/
...
// File contracts/EqualFtmBrushvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualFtmBrushvAMM  is Ownable, Pausable {
    ...
    address public constant brush = address(0x85dec8c4B2680793661bCA91a8F129607571863d);
    address public want;
    ...
    IEqualizerRouter.Routes[] public wrappedToBrushPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToBrushPath
    ) {
        ...
        for (uint i; i < _wrappedToBrushPath.length; ++i) {
            wrappedToBrushPath.push(_wrappedToBrushPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToBrushPath, address(this), block.timestamp);        

        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(brush).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, brush, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(brush).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(brush).safeApprove(unirouter, type(uint).max);
    }
    ...
    function makeCustomTxn(address [][] memory _tokens, bool[] calldata _stable) external onlyAdmin {
        ...
        IERC20(_tokens[0][0]).safeApprove(unirouter, type(uint).max);
