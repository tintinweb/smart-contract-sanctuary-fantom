...
// Note that this pool has no minter key of PALM (rewards).
// Instead, the governance will call PALM distributeReward method and send reward to this pool at the beginning.
contract PalmRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last time that PALMs distribution occurs.
        uint256 accBSharePerShare; // Accumulated PALMs per share, times 1e18. See below.
    ...
    // The time when PALM mining starts.
    uint256 public poolStartTime;

    // The time when PALM mining ends.
    uint256 public poolEndTime;

    address public daoFundAddress;

    uint256 public bSharePerSecond = 0.003486 ether; // 50000 PALM / (166 days * 24h * 60min * 60s)
    ...
    // COCONUT-TOMB LP: 21500 $PALM
    // PALM-TOMB LP: 21000 $PALM
    // TEAM: 4500 $PALM
    // CURVE STABLES LP (OR GEIST STABLES LP): 3000 $PALM

    // Update the given pool's PALM allocation point. Can only be called by the operator.
    ...
    // Safe palm transfer function, just in case if rounding error causes pool to not have enough PALMs.
