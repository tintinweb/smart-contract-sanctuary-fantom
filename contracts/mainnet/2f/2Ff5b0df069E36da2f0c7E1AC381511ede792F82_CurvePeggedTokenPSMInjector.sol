 ...
 * It converts the reward token (CURVE and others) into target token and sends it
 * to the actual fee collector.
 */
contract CurveFeeCollectorAdapter is Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	constructor(address[] memory _sourceTokens, address _targetToken, address _wrappedToken,
		address _collector, address _treasury, address _exchange)
	{
		for (uint256 _i = 0; _i < _sourceTokens.length; _i++) {
			address _sourceToken = _sourceTokens[_i];
	...
	function _gulp() internal returns (bool _success)
	{
		...
		for (uint256 _i = 0; _i < sourceTokens.length; _i++) {
			address _sourceToken = sourceTokens[_i];
			uint256 _totalSource = Transfers._getBalance(_sourceToken);
			if (_totalSource > 0) {
				require(exchange != address(0), "exchange not set");
				uint256 _factor = IExchange(exchange).oracleAveragePriceFactorFromInput(_sourceToken, targetToken, _totalSource);
				if (_factor < minimalGulpFactor) return false;
				Transfers._approveFunds(_sourceToken, exchange, _totalSource);
				IExchange(exchange).convertFundsFromInput(_sourceToken, targetToken, _totalSource, 1);
			}
		}
		{
	...
	function recoverLostFunds(address _token) external onlyOwner nonReentrant delayed
	{
		for (uint256 _i = 0; _i < sourceTokens.length; _i++) {
			address _sourceToken = sourceTokens[_i];
			require(_token != _sourceToken, "invalid token");
		}
...
contract CurvePeggedTokenPSMInjector is Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	function _gulp() internal returns (bool _success)
	{
		...
		if (_underlyingAmount > 0) {
			...
			CurvePeggedToken(peggedToken).deposit(_reserveAmount, _underlyingAmount * (100e16 - slippage) / 100e16, false);
		}
	...
	function setUnderlyingIndex(uint256 _newIndex) external onlyOwner delayed
	{
