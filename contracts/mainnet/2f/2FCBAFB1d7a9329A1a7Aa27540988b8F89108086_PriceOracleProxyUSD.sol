...
interface ITwapPriceOracle {
    ...
    function getPriceTwap(address)
        external 
        returns (uint256);

    function getPriceTwapLast(address)
        external view
        returns (uint256);
...
contract PriceOracleProxyUSD is PriceOracle, Exponential {
    ...
    /// @notice LP Pairs
    mapping(address => address) public tTokenToPair;

    mapping(address => ITwapPriceOracle) public twaporacles; // used exclusively for L3USD until it has ChainLink oracle
    ...
    /**
     * @notice Get the underlying price of a listed tToken asset
     * @param tToken The tToken to get the underlying price of
     * @return The underlying asset price mantissa (scaled by 1e18)
     */
    function getUnderlyingPrice(TToken tToken) public view returns (uint256) {
        address tTokenAddress = address(tToken);

        AggregatorV3Interface aggregator = aggregators[tTokenAddress];

        ITwapPriceOracle twapOracle = twaporacles[tTokenAddress];

        if (address(twapOracle) != address(0)) {
            uint256 price = getPriceFromTWAPoracle(twapOracle, TErc20(tTokenAddress).underlying());
            uint256 underlyingDecimals = EIP20Interface(TErc20(tTokenAddress).underlying()).decimals();
        ...
        // prioritize Chainlink over twap in case if two oracles set for single tToken
        ...
        address asset = address(TErc20(tTokenAddress).underlying());
    ...
    function getPriceFromTWAPoracle(ITwapPriceOracle _twapOracle, address _underlyingToken) internal view returns (uint256) {
        uint256 price = _twapOracle.getPriceTwapLast(_underlyingToken);
        require(price > 0, "invalid price");

        // Extend the decimals to 1e18.
        return mul_(uint256(price), 10**(18 - uint256(_twapOracle.decimals())));
    }

    /*** Admin or guardian functions ***/

    event AggregatorUpdated(address tTokenAddress, address source);
    ...
    /**
     * @notice Set twaporacle for price oracle proxy
     * @param _twapOracle The new twaporacle
     */
    function _setTwapOracle(address _tToken, ITwapPriceOracle _twapOracle) external {
        require(msg.sender == admin, "only the admin may set new twaporacle");
        twaporacles[_tToken] = _twapOracle;
    }
    ...
    /**
     * @notice Set ChainLink aggregators for multiple tTokens
     * @param tTokenAddresses The list of tTokens
     * @param sources The list of ChainLink aggregator sources
     */
    function _setAggregators(address[] calldata tTokenAddresses, address[] calldata sources) external {
        require(msg.sender == admin || msg.sender == guardian, "only the admin or guardian may set the aggregators");
        require(tTokenAddresses.length == sources.length, "mismatched data");
        for (uint256 i = 0; i < tTokenAddresses.length; i++) {
            if (sources[i] != address(0)) {
                require(msg.sender == admin, "guardian may only clear the aggregator");
            }
            aggregators[tTokenAddresses[i]] = AggregatorV3Interface(sources[i]);
            emit AggregatorUpdated(tTokenAddresses[i], sources[i]);
        }
    }

    /**
     * @notice Set the price of underlying asset
     * @param tToken The tToken to get underlying asset from
     * @param underlyingPriceMantissa The new price for the underling asset
     */
    function _setUnderlyingPrice(TToken tToken, uint256 underlyingPriceMantissa) external {
        require(msg.sender == admin, "only the admin may set the underlying price");
        address asset = address(TErc20(address(tToken)).underlying());
