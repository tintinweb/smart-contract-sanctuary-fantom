...
// No permissions granted before Friday, 2 Aug 2024, then GPL-3.0 after this date.
 ...
 * out quoting reference 'NyPtvFantomUsdcDeiSpookyV3StrategyToWftm'
 *
...
contract NyPtvFantomUsdcDeiSpookyV3StrategyToWftm is NYBaseProfitTakingStrategyV3 {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  uint256 public constant POOL_ID = 2;
  ...
  address public constant LP_TOKEN = address(0x91f7120898b4be26cC1e84F421e76725c07d1361);
  address public constant LP_TOKEN_0 = address(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
  address public constant LP_TOKEN_1 = address(0xDE1E704dae0B4051e80DAbB26ab6ad6c12262DA0);
  ...
  function _swapFarmEmissionTokens() internal {
    ...
    if (booToWftmPath.length >= 2 && booBalance > 0) {
      ...
      uint256[] memory amounts = IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(booBalance, booToWftmPath);
    ...
    IERC20Upgradeable deus = IERC20Upgradeable(DEUS);
    uint256 deusBalance = deus.balanceOf(address(this));
    if (deusToWftmPath.length >= 2 && deusBalance > 0) {
      deus.safeIncreaseAllowance(SPOOKY_ROUTER, deusBalance);

      uint256[] memory amounts = IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(deusBalance, deusToWftmPath);
      uint256 amountOutMin = (amounts[amounts.length - 1] * maxSlippage) / PERCENT_DIVISOR;
  ...
  function estimateHarvest() external view override returns (uint256 profit, uint256 callFeeToUser) {
    ...
    if (totalRewards != 0) {
      uint256[] memory harvestAmounts = IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(totalRewards, booToWftmPath);
