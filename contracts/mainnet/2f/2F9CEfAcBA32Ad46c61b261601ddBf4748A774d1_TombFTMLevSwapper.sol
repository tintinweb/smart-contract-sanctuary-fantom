...
contract TombFTMLevSwapper {
    ...
    IUniswapV2Factory constant public tombSwapFactory = IUniswapV2Factory(0xE236f6890F1824fa0a7ffc39b1597A5A6077Cfe9);
    ...
    function _calculateSwapInAmount(uint256 reserveIn, uint256 userIn) internal view returns (uint256) {
        uint256 fee = tombSwapFactory.swapFee();
        uint256 f1 = 4000 * (1000 - fee);
        uint256 f2 = (2000 - fee);
        uint256 f3 = 2 * (1000 - fee);
        return (Babylonian.sqrt(reserveIn * ((userIn * f1) + (reserveIn * f2 * f2))) - (reserveIn * f2)) / f3;
    ...
    function _getAmountOutTomb(
        ...
        uint256 amountInWithFee = amountIn * (1000 - tombSwapFactory.swapFee());
        uint256 numerator = amountInWithFee * reserveOut;
        uint256 denominator = (reserveIn * 1000) + amountInWithFee;
        amountOut = numerator / denominator;
    }
