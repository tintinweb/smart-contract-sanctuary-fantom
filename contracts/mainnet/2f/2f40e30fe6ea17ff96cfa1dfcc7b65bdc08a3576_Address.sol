...
// _______ _     _ _______ ______   _____  _  _  _ ______ _    _ _______  ______ _______ _______
// |______ |_____| |_____| |     \ |     | |  |  |  ____/  \  /  |______ |_____/ |______ |______
// ______| |     | |     | |_____/ |_____| |__|__| /_____   \/   |______ |    \_ ______| |______
// 
...
// File: contracts/BCLOWNZ.sol



pragma solidity ^0.8.4;

contract CLOWNZ is ERC721, ERC2981, Ownable, ERC721Enumerable {
    ...
    constructor() ERC721("CLOWNZ", "CZ"){ 
        royaltyAddress = payable(msg.sender);   
    }

    //Mint
    function sendMints(address[] memory airdropWallets) external onlyOwner {
        require((airdropWallets.length + totalSupply()) < maxSupply, "Too many wallets for supply");
      
        uint256 tokenID = totalSupply() + 1;
        for(uint256 i = 0; i < airdropWallets.length; i++){
            uint256 id = tokenID + i;
            _safeMint(airdropWallets[i], id);
        }
    }

    function ownerMint(uint256 amount) external onlyOwner {
        require((amount + totalSupply()) < maxSupply, "Not enough left to mint that amount");
