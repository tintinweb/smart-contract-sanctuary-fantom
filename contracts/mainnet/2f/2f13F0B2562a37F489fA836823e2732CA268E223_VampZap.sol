...
import "../interfaces/IBatasmPool.sol";
...
contract VampZap is Ownable {
    ...
    address private BATASM_POOL_ADDRRESS;
    address private BFTM_ADDRESS;
    address private BFTM_VAMP_LP_ADDRESS;
    address private FTM_VSHARE_LP_ADDRESS;
    address private VSHARE_ADDRESS;
    address private VAMP_ADDRESS;
    uint16 FEE_RATE;
    uint16 MIN_AMT;
    ...
    event Mint(uint256 amountIn, uint256 amountOut);
    event MintUser(address indexed user, uint256 amountOut);
    ...
    constructor(address _WNATIVE) Ownable() {
       ...
       FEE_RATE = 0;
       MIN_AMT = 1;
       BATASM_POOL_ADDRRESS = 0xE6741CEcf9879fd6FEFbe5160a4b7c5DAe0eb691;
       BFTM_ADDRESS = 0xdc79AFCe5AE2300834B2bB575bC40cF78EF7b5e3;
       BFTM_VAMP_LP_ADDRESS = 0xBe1F814b44AE9C811332E14bBDA046471777a6c3;
       VAMP_ADDRESS = 0xA91ADe2730bF6041ce9DE1Ba9cB22eE7Fea2B009;
       VSHARE_ADDRESS = 0xF99231D26437a5A24928fbF2cB4424d1F8010EA1;
       FTM_VSHARE_LP_ADDRESS = 0x79a50B4487d120eC0C9634b6729d9983fe5F0439;
    }
    ...
    function zapIn(address _to, address routerAddr, address _recipient) external payable {
      ...
      if(_to == BFTM_VAMP_LP_ADDRESS) {
            uint256 amount = msg.value;
            _mintBftmToLP(amount, _to, routerAddr, _recipient);
        }
        else {
    ...
    function estimateZapIn(address _LP, address _router, uint _amt) public view returns (uint256, uint256) {
        ...
        if(_LP == BFTM_VAMP_LP_ADDRESS) {
            // BFTM
            uint256 mintingFee = IBatasmPool(BATASM_POOL_ADDRRESS).mintingFee();
            uint256 fee =  _amt.mul(mintingFee).div(1000000);
            uint256 bftmAmountAfterMintingFee = _amt.sub(fee);
            uint256 bftmZapAmt = bftmAmountAfterMintingFee.div(2);
            ...
            uint[] memory amounts = router.getAmountsOut(bftmZapAmt, path);
            uint256 vampAmt = amounts[amounts.length - 1];
            return (bftmZapAmt, vampAmt);
        }
    ...
    function zapOut(address _from, uint amount, address routerAddr, address _recipient) external {
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            _swapTokenForNative(token, amtToken, _recipient, routerAddr, address(0));
            ...
            _swapTokenForNative(token0, amt0, _recipient, routerAddr, address(0));
            _swapTokenForNative(token1, amt1, _recipient, routerAddr, address(0));
        }
    }
    ...
    function swapToNative(address _from, uint amount, address routerAddr, address _recipient) external {
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        _approveTokenIfNeeded(_from, routerAddr);
        _swapTokenForNative(_from, amount, _recipient, routerAddr, address(0));
    }
    ...
    function _swapTokenToLP(address _from, uint amount, address _to, address recipient, address routerAddr) private returns (uint) {
        ...
        if (_from == IUniswapV2Pair(_to).token0() || _from == IUniswapV2Pair(_to).token1()) { // check if we already have one of the assets
            ...
            // go through native token for highest liquidity
            uint nativeAmount = _swapTokenForNative(_from, amount, address(this), routerAddr, _to);
            return _swapNativeToLP(_to, nativeAmount, recipient, routerAddr);
        }
    }

    function _swapBFTMToLP(address _LP, uint amount, address recipient, address routerAddress) private returns (uint) {
            // LP
            IUniswapV2Pair pair = IUniswapV2Pair(_LP);
            address token0 = pair.token0();
            address token1 = pair.token1();
            ...
            ( , , liquidity) = _swapHalfBFTMAndProvide(token1, token0, amount, routerAddress, recipient);
    ...
    function _swapNativeToLP(address _LP, uint amount, address recipient, address routerAddress) private returns (uint) {
            ...
            if (token0 == WNATIVE || token1 == WNATIVE) {
                ...
                ( , , liquidity) = _swapNativeToEqualTokensAndProvide(token0, token1, amount, routerAddress, recipient, _LP);
            }
            return liquidity;
    }
    ...
    function _swapHalfBFTMAndProvide(address bftm, address token, uint amount, address routerAddress, address recipient) private returns (uint, uint, uint) {
            _approveTokenIfNeeded(token, routerAddress);
            _approveTokenIfNeeded(bftm, routerAddress);
            uint256 swapValue = amount.div(2);
            uint256 tokenAmount = _swapBFTMForToken(token, swapValue, address(this), routerAddress);
            uint256 balance = amount.sub(swapValue);

            return IUniswapV2Router01(routerAddress).addLiquidity(token, bftm, tokenAmount, balance, 1, 1, recipient, block.timestamp);
    }

    function _swapNativeToEqualTokensAndProvide(address token0, address token1, uint amount, address routerAddress, address recipient, address lp) private returns (uint, uint, uint) {
            address recipientTo = recipient;
            if(lp == BFTM_VAMP_LP_ADDRESS) {
                _mintBftmToLP(amount, lp, routerAddress, recipient);
            }
            else {
                uint swapValue = amount.div(2);
                ...
                IUniswapV2Router01 router = IUniswapV2Router01(routerAddress);
                return router.addLiquidity(token0, token1, token0Amount, token1Amount, 0, 0, recipientTo, block.timestamp);
            }
    }

    function _mintBftmToLP(uint256 amount, address to, address routerAddr, address recipient) private {
        IBatasmPool(BATASM_POOL_ADDRRESS).mint{value: amount}(0);
        (uint256 xTokenBalance,,,) = IBatasmPool(BATASM_POOL_ADDRRESS).userInfo(address(this));
        IBatasmPool(BATASM_POOL_ADDRRESS).collect();
        emit Mint(msg.value, xTokenBalance);
        uint256 mintingFee = IBatasmPool(BATASM_POOL_ADDRRESS).mintingFee();
        uint256 fee =  amount.mul(mintingFee).div(1000000);
        uint256 bftmAmountAfterMintingFee = amount.sub(fee);
        emit MintUser(recipient, bftmAmountAfterMintingFee);
        _swapBFTMToLP(to, bftmAmountAfterMintingFee, recipient, routerAddr);
    }
    ...
    function _swapBFTMForToken(address token, uint amount, address recipient, address routerAddr) private returns (uint) {
        address[] memory path;
        IUniswapV2Router01 router = IUniswapV2Router01(routerAddr);
    ...
    function _swapTokenForNative(address token, uint amount, address recipient, address routerAddr, address lp) private returns (uint) {
        address[] memory path;
        IUniswapV2Router01 router = IUniswapV2Router01(routerAddr);
        if (token == VAMP_ADDRESS && lp == FTM_VSHARE_LP_ADDRESS) {
            path = new address[](3);
            path[0] = token;
            path[1] = BFTM_ADDRESS;
            path[2] = router.WETH();
        }
        else if (tokenBridgeForRouter[token][routerAddr] != address(0)) {
            path = new address[](3);
            path[0] = token;
            path[1] = tokenBridgeForRouter[token][routerAddr];
            path[2] = router.WETH();
    ...
    function _swap(address _from, uint amount, address _to, address recipient, address routerAddr) private returns (uint) {
        ...
        if (_from == VAMP_ADDRESS && _to == WNATIVE) {
            path = new address[](3);
            path[0] = _from;
            path[1] = BFTM_ADDRESS;
            path[2] = _to;
        ...
        if (fromBridge != address(0) && toBridge != address(0)) {
            ...
            path[0] = _from;
            path[1] = _to;
        } else if (_from == VAMP_ADDRESS || _to == VAMP_ADDRESS) {
            path = new address[](2);
            path[0] = _from;
            path[1] = _to;
        } else if (_from == VSHARE_ADDRESS || _to == VSHARE_ADDRESS) {
            path = new address[](2);
            path[0] = _from;
            path[1] = _to;
        } else {
