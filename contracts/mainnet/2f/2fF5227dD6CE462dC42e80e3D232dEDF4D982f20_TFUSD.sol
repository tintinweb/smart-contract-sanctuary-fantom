...
contract TFUSD is IERC20Metadata, Ownable {
  ...
  function zPegAndAddFeeLiquidity1(uint256 amount) public view returns(uint256, uint256, uint256) {
    uint256 aLpFee = amount.mul(autoLpFee).div(10 ** manageDecimal);
    aLpFee = aLpFee.add(_autoLpDebt);
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = peggedToken;
    uint256[] memory amounts = IUniswapV2Router(uniRouter).getAmountsOut(aLpFee, path);
    return(aLpFee, amounts[0], amounts[1]);
  }

  event tEvt1(uint256 tfusdAmount, uint256 pegAmount);
  function zPegAndAddFeeLiquidity2(uint256 inAmount) public {
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = peggedToken;
    ...
    emit tEvt1(tfusdAmount, amounts[1]);
  }

  function zPegAndAddFeeLiquidity3(uint256 a, uint256 b) public {
    _addLiquidity(a, b);
  }
  ...
  function _autoPegAndAddFeeLiquidity(uint256 amount) internal {
    ...
    if (amounts[1] == 0) {
      ...
      if (currentalance < aLpFee) {
        aLpFee = currentalance;
      }
      _burn(address(this), aLpFee);
    }
    else {
      if (currentalance < aLpFee) {
        _mint(address(this), aLpFee.sub(currentalance));
      }
    ...
    unchecked {
      ...
      if (excludedAddress[to] == true) {
        ...
        if (aLpFee > 0) {
          ...
          _balances[address(this)] = balanceInfo({
            balance: thisBalance.add(aLpFee),
            updatedAt: block.timestamp
          });
        }
      }
