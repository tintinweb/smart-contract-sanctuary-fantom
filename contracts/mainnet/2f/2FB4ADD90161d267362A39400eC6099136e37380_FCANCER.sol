...
contract FCANCER is ERC721Enumerable, Ownable, ERC2981 {
  ...
  address[] private team = [
    0x6a8eE763809000dB131bA93F1d46EfA03CdF4ea1 //Need AJ Wallet?
  ];
  ...
  uint256 public immutable maxMintAmount; //20

  bool public publicPaused = true;
  uint16[15000] private ids;
  ...
  //adsress usdc = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75;
  ...
  function mintCustom(address token, uint amount, address collection) external payable {
    //mint is closed
    if(publicPaused)
      revert MintPaused();
    if(amount <= 0)
  ...
  function mint(uint amount) external payable {
    ...
    uint amountFromSender = msg.value;
    uint cost = 1 ether;
    if(amountFromSender < amount * cost)
      revert InsufficientFTM({
        totalCost: amount * cost,
        amtFTM: amountFromSender
      });
    //Need to figure out how to transfer msg.value to contract
    //Actually, I think I can do nothing here, and the payable will pay out ftm to the contract automatically
    //require(transferFrom(msg.sender, msg.value, amount * cost), "Payment not successful");
    _mintInternal(amount);
  }

    function _mintInternal(uint _amount) internal {
        for (uint256 i = 1; i <= _amount; ++i) {
            _safeMint(msg.sender, 1);//_pickRandomUniqueId(_getRandom()) +1);
        }
    }
  ...
  function withdraw(address token) external onlyOwner { 
    if(token == address(0)) {
      ...
      require(amount > 0);  
      IERC20(token).transfer(team[0], amount);
    }
  }
}
