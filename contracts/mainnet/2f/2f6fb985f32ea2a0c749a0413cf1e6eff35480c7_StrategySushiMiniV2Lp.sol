...
contract StrategySushiMiniV2Lp is StrategyBase {
    ...
    function _deposit() internal {
        ...
        if (_baseBal > 0) {
            IFantasmChef(farmPool).deposit(poolId, _baseBal, address(this));
            emit Deposit(baseToken, _baseBal);
        }
    }

    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        (uint256 _stakedAmount, ) = IFantasmChef(farmPool).userInfo(poolId, address(this));
        if (_amount > _stakedAmount) {
            _amount = _stakedAmount;
        }

        uint256 _before = IERC20(baseToken).balanceOf(address(this));
        IFantasmChef(farmPool).withdraw(poolId, _amount, address(this));
    ...
    function _withdrawAll() internal override {
        (uint256 _stakedAmount, ) = IFantasmChef(farmPool).userInfo(poolId, address(this));
        IFantasmChef(farmPool).withdrawAndHarvest(poolId, _stakedAmount, address(this));
    }

    function claimReward() public override {
        IFantasmChef(farmPool).harvest(poolId, address(this));
        IMultiFeeDistribution(IFantasmChef(farmPool).rewardMinter()).emergencyWithdraw();
    ...
    function balanceOfPool() public view override returns (uint256) {
        (uint256 amount, ) = IFantasmChef(farmPool).userInfo(poolId, address(this));
        return amount;
    }
    ...
    function claimable_tokens() external view override returns (address[] memory farmToken, uint256[] memory totalDistributedValue) {
        ...
        totalDistributedValue[0] = IFantasmChef(farmPool).pendingReward(poolId, address(this));
    }
    ...
    function claimable_token() external view override returns (address farmToken, uint256 totalDistributedValue) {
        ...
        totalDistributedValue = IFantasmChef(farmPool).pendingReward(poolId, address(this));
    }
    ...
    function retireStrat() external override onlyStrategist {
        IFantasmChef(farmPool).emergencyWithdraw(poolId, address(this));
