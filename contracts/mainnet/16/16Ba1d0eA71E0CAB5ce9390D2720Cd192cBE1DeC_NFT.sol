...
// File contracts/Radiant.TierX/Radiant.TierX.sol
...
/**
 * This contract is cloneable as a minimal proxy contract so that multiple
 * copies of it can be created at a significantly reduced cost
 */
...
contract NFT is ERC721Enumerable, ERC721Royalty, ERC721Pausable, IERC721Burnable, Ownable, Cloneable {
    ...
    function mint(uint256 count) public whenNotPaused returns (uint256[] memory) {
        ...
        require(TOKEN.isApprovedForAll(_msgSender(), address(this)), "ERC721: Contract not approved for spending");
        uint256 mintCost = MINT_PRICE * count;
        require(mintCost <= TOKEN.balanceOf(_msgSender()), "ERC721: Mint price exceeds balance");

        // loop through and burn the required tokens
        for (uint256 i = 0; i < mintCost; i++) {
            uint256 tokenId = TOKEN.tokenOfOwnerByIndex(_msgSender(), 0); // burn from the top of the pile
            TOKEN.burn(tokenId);
        }
