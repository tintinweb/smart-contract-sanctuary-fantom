    ...
    OwnableUpgradeable
{
    ...
    function initialize(address token_, address devFund_) external initializer {
    ...
    function setTreasuryFundConfig(address _treasuryFund, uint256 _treasuryRate)
        external
        onlyOwner
    {
        require(
            _treasuryFund != address(0) || _treasuryRate == 0,
            "zero address"
        );
        require(_treasuryRate <= 10000, "too high");
    ...
    function setMerkleRoot(
        ...
        if (treasuryRate > 0) {
            _token.mint(treasuryFund, (amount * treasuryRate) / 10000);
        }
    }
    ...
    function claimMultiDay(
        ...
        require(
            IERC20(token).transfer(account, _totalAmount),
            "MerkleDistributor: Transfer failed."
        );
    }
