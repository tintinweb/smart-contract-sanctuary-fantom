...
contract ShareWrapper {
    ...
    function stakeAirdrop(address to, uint256 amount) public virtual {
        _totalSupply = _totalSupply.add(amount);
...
contract Masonry is ShareWrapper, ContractGuard {
    ...
    event StakedAirdrop(address indexed user, uint256 amount);
    ...
    function stakeAirdrop(address to, uint amount) public override onlyOneBlock onlyOperator updateReward(to) {
        require(amount > 0, "Masonry: Cannot stake 0");
        super.stakeAirdrop(to, amount);
        masons[to].epochTimerStart = treasury.epoch(); // reset timer
        emit StakedAirdrop(to, amount);
    }
