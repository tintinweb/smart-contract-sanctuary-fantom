...
contract Traits is Ownable, ITraits {
  ...
  function setHnO(address _hnoNFT) external onlyOwner {
    require(_hnoNFT != address(0), "NFT is zero address");
    hnoNFT = IHnO(_hnoNFT);
  }
   ...
   */
  function uploadTraits(uint256 traitType, uint256[] memory traitIds, Trait[] memory traits) external onlyOwner {
    require(traitIds.length == traits.length, "Mismatched inputs");
    for (uint256 i = 0; i < traits.length; i++) traitData[traitType][traitIds[i]] = traits[i];
  }
   ...
   * @return a valid SVG of the Hamster / Owl
   */
  function drawSVG(uint256 tokenId) internal view returns (string memory) {
    ...
    uint256 shift = s.isHamster ? 0 : 9;
    // Trait data indexes 0 - 7 are reserved for Hamsters
    // Trait data indexew 9 - 17 are reserved for Owls.
    string memory svgString = string(abi.encodePacked(
      drawTrait(traitData[0 + shift][s.body]),
      s.isHamster ? drawTrait(traitData[4 + shift][s.neck]) : '',
      s.isHamster ? drawTrait(traitData[1 + shift][s.head]) : drawTrait(traitData[1 + shift][s.rankIndex]),
      s.isHamster ? drawTrait(traitData[2 + shift][s.spell]) : '',
      drawTrait(traitData[3 + shift][s.eyes]),
      drawTrait(traitData[5 + shift][s.mouth]),
      s.isHamster ? '' : drawTrait(traitData[6 + shift][s.tail]),
      s.isHamster ? drawTrait(traitData[7 + shift][s.wand]) : ''
    ));
    return string(abi.encodePacked(
      '<svg id="hnoNFT" width="100%" height="100%" version="1.1" viewBox="0 0 520 520" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
      svgString,
      "</svg>"
    ));
  }
  ...
  function compileAttributes(uint256 tokenId) internal view returns (string memory) {
    ...
    return string(abi.encodePacked(
      ...
      tokenId <= hnoNFT.getPaidTokens() ? '"Gen 0"' : '"Gen 1"',
      '},{"trait_type":"Type","value":',
      s.isHamster ? '"Hamster"' : '"Owl"',
      '}]'
    ));
  }
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    require(_msgSender() == address(hnoNFT), "hmmmm what doing?");
    IHnO.HamsterOwl memory s = hnoNFT.getTokenTraits(tokenId);
    string memory metadata = string(abi.encodePacked(
      '{"name": "',
      s.isHamster ? 'Hamster #' : 'Owl #',
      tokenId.toString(),
      '", "description": "Thousands of Hamsters and Owls compete in a hideout in the metaverse. A tempting prize of $GEM awaits, with deadly high stakes. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Ethereum blockchain.", "image": "data:image/svg+xml;base64,',
      bytes(drawSVG(tokenId)).base64(),
      '", "attributes":',
