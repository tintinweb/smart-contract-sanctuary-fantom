...
import "./interfaces/INFTBoardroom.sol";
...
/**
 * @title Treasury
 * allows for multiple boardrooms and different expansion systems
 */
contract Treasury is ContractGuard, AccessControlEnumerable, ITreasury {
    ...
    struct Boardroom {
        bool nft;               // nft boardroom
        uint256 alloc;          // allocation out of 10000 for rewards
        uint256 withdrawFee;    // withdraw fee when under price ceiling for ERC20 boardrooms
    }
    ...
    EnumerableSet.AddressSet excludedFromTotalSupply;
    ...
    EnumerableSet.AddressSet boardrooms;            // boardroom address
    mapping(address => Boardroom) boardroomsInfo;   // holds Boardroom struct
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of price
    ...
    uint256 public insuranceFundPercent;
    ...
    EnumerableSet.AddressSet updateHooks; // update hook contracts called on allocateSeigniorage
    ...
    event FundingAdded(uint256 indexed epoch, uint256 timestamp, uint256 price, uint256 expanded, uint256 boardroomFund, uint256 daoFund, uint256 insuranceFund, uint256 devFund);
    ...
    error NotEnoughBonds();
    error InvalidBondRate();
    error OverMaxDebtRatio();
    error TreasuryHasNoBudget();
    error TooHigh();
    error TooLow();
    error NotOperator();
    error Exist();
    ...
    modifier checkOperator {
        if(
            !IAccessControl(pegToken).hasRole(AccessControlConstants.OPERATOR_ROLE, address(this)) ||
            !IAccessControl(bond).hasRole(AccessControlConstants.OPERATOR_ROLE, address(this)) ||
            !IAccessControl(share).hasRole(AccessControlConstants.OPERATOR_ROLE, address(this))
        ) revert NeedPermission();
        for(uint256 i; i < boardrooms.length(); i++) {
            if(!IAccessControl(boardrooms.at(i)).hasRole(AccessControlConstants.OPERATOR_ROLE, address(this))) revert NeedPermission();
        }
        _;
    }
    ...
    function getPegTokenCirculatingSupply() public view override returns (uint256) {
        ...
        for(uint256 entryId; entryId < excludedFromTotalSupply.length(); entryId++) {
            balanceExcluded += pegTokenErc20.balanceOf(excludedFromTotalSupply.at(entryId));
        }
    ...
    function addBoardroom(address boardroom_, uint256 alloc_, uint256 withdrawFee_, bool nft_) external onlyOperator {
        if(boardrooms.contains(boardroom_)) revert Exist();
        if(withdrawFee_ > MAX_WITHDRAW_FEE) revert FeeTooHigh();
        boardrooms.add(boardroom_);
        Boardroom memory room;
        room.nft = nft_;
        room.alloc = alloc_;
        room.withdrawFee = withdrawFee_;
        boardroomsInfo[boardroom_] = room;
        boardroomsTotalAllocPoints += alloc_;
    }

    function removeBoardroom(address boardroom_) external onlyOperator {
        if(!boardrooms.contains(boardroom_)) revert DoesNotExist();
        boardroomsTotalAllocPoints -= boardroomsInfo[boardroom_].alloc;
        delete boardroomsInfo[boardroom_].nft;
        delete boardroomsInfo[boardroom_].alloc;
        delete boardroomsInfo[boardroom_].withdrawFee;
        boardrooms.remove(boardroom_);
    }

    function setBoardroom(address boardroom_, uint256 alloc_, uint256 withdrawFee_) external onlyOperator {
        if(!boardrooms.contains(boardroom_)) revert DoesNotExist();
        if(withdrawFee_ > MAX_WITHDRAW_FEE) revert FeeTooHigh();
        boardroomsTotalAllocPoints = (boardroomsTotalAllocPoints - boardroomsInfo[boardroom_].alloc) + alloc_;
        Boardroom memory room;
        room.nft = boardroomsInfo[boardroom_].nft;
        room.alloc = alloc_;
        room.withdrawFee = withdrawFee_;
        boardroomsInfo[boardroom_] = room;
    }

    // Only for ERC20 Boardroom
    function setBoardroomStakeFee(address boardroom_, uint256 boardroomStakeFee_) external onlyOperator {
        if(!boardrooms.contains(boardroom_)) revert DoesNotExist();
        if(boardroomStakeFee_ > MAX_STAKE_FEE) revert FeeTooHigh();
        if(!ERC165Checker.supportsInterface(boardroom_, type(IBoardroomFee).interfaceId)) revert InterfaceNotSupported();
        IBoardroomFee(boardroom_).setStakeFee(boardroomStakeFee_);
    }
    ...
    function setExtraFunds(address daoFund_, uint256 daoFundPercent_, address insuranceFund_, uint256 insuranceFundPercent_, address devFund_, uint256 devFundPercent_) external onlyOperator {
        if(daoFund_ == address(0)) revert ZeroAddress();
        if(daoFundPercent_ > 2000) revert OutOfRange(); // <= 20%
        if(insuranceFund_ == address(0)) revert ZeroAddress();
        if(insuranceFundPercent_ > 2000) revert OutOfRange(); // <= 20%
        if(devFund_ == address(0)) revert ZeroAddress();
        if(devFundPercent_ > 2000) revert OutOfRange(); // <= 20%
        daoFund = daoFund_;
        daoFundPercent = daoFundPercent_;
        insuranceFund = insuranceFund_;
        insuranceFundPercent = insuranceFundPercent_;
        devFund = devFund_;
    ...
    // update each supported hook contracts
    function _updateHooks() internal {
        for(uint256 i; i < updateHooks.length(); i++) {
            try IUpdate(updateHooks.at(i)).update() {} catch {}
        }
    }

    function addUpdateHook(address hook_) external onlyOperator {
        if(updateHooks.contains(hook_)) revert Exist();
        updateHooks.add(hook_);
    }

    function removeUpdateHook(address hook_) external onlyOperator {
        if(!updateHooks.contains(hook_)) revert DoesNotExist();
        updateHooks.remove(hook_);
    }

    function excludeFromTotalSupply(address exclude_) external onlyOperator {
        if(excludedFromTotalSupply.contains(exclude_)) revert();
        excludedFromTotalSupply.add(exclude_);
    }

    function includeFromTotalSupply(address include_) external onlyOperator {
        if(!excludedFromTotalSupply.contains(include_)) revert();
        excludedFromTotalSupply.remove(include_);
    }

    function buyBonds(uint256 pegTokenAmount_, uint256 targetPrice) external override onlyOneBlock checkCondition checkOperator {
    ...
    function redeemBonds(uint256 bondAmount_, uint256 targetPrice) external override onlyOneBlock checkCondition checkOperator {
    ...
    function _sendToBoardroom(uint256 amount_, uint256 expanded_) internal {
        ...
        uint256 insuranceFundAmount_;
        if(insuranceFundPercent > 0) {
            insuranceFundAmount_ = (amount_ * insuranceFundPercent) / 10000;
            IERC20(pegToken).safeTransfer(insuranceFund, insuranceFundAmount_);
        }
        ...
        amount_ -= (daoFundAmount_ + insuranceFundAmount_ + devFundAmount_);

        uint256 amountTotal = amount_;
        for(uint256 i; i < boardrooms.length(); i++) {
            address room = boardrooms.at(i);
            uint256 amt = (amountTotal * boardroomsInfo[room].alloc) / boardroomsTotalAllocPoints;
            bool success;
            (success,) = amount_.trySub(amt);
            if(success) {
                amount_ -= amt;
            } else {
                amt = amount_;
        ...
        if (regulationStats != address(0)) IRegulationStats(regulationStats).addEpochInfo(epoch + 1, previousEpochPegTokenPrice, expanded_, amount_, daoFundAmount_, insuranceFundAmount_, devFundAmount_);
        emit FundingAdded(epoch + 1, block.timestamp, previousEpochPegTokenPrice, expanded_, amount_, daoFundAmount_, insuranceFundAmount_, devFundAmount_);
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if(epoch < bootstrapEpochs) {
            ...
            if(previousEpochPegTokenPrice > pegTokenPriceCeiling) {
                // set each boardroom withdraw fee to 0 when above price ceiling
                for(uint256 i; i < boardrooms.length(); i++) {
                    address room = boardrooms.at(i);
                    if(ERC165Checker.supportsInterface(room, type(IBoardroomFee).interfaceId)) {
                        IBoardroomFee(room).setWithdrawFee(0);
                    }
                }
                ...
                if(savedForBoardroom_ > 0) {
                    ...
                    if (regulationStats != address(0)) IRegulationStats(regulationStats).addEpochInfo(epoch + 1, previousEpochPegTokenPrice, 0, 0, 0, 0, 0);
                    emit FundingAdded(epoch + 1, block.timestamp, previousEpochPegTokenPrice, 0, 0, 0, 0, 0);
                }
                if(savedForBond_ > 0) {
                ...
                // set withdraw fee when price ceiling for each boardroom
                for(uint256 i; i < boardrooms.length(); i++) {
                    address room = boardrooms.at(i);
                    if(ERC165Checker.supportsInterface(room, type(IBoardroomFee).interfaceId)) {
                        IBoardroomFee(room).setWithdrawFee(boardroomsInfo[room].withdrawFee);
                    }
                }
                if (regulationStats != address(0)) IRegulationStats(regulationStats).addEpochInfo(epoch + 1, previousEpochPegTokenPrice, 0, 0, 0, 0, 0);
                emit FundingAdded(epoch + 1, block.timestamp, previousEpochPegTokenPrice, 0, 0, 0, 0, 0);
    ...
    function initialize(address pegToken_, address bond_, address share_, address oracle_, address boardroom_, uint256 startTime_, uint256 startEpoch_) public notInitialized onlyOperator {
        pegToken = pegToken_;
        bond = bond_;
        share = share_;
        oracle = oracle_;
        boardrooms.add(boardroom_);
        boardroomsInfo[boardroom_].alloc = 10000;    // 100%
        boardroomsInfo[boardroom_].withdrawFee = 20; // 2% withdraw fee when under peg
    ...
    function governanceRecoverUnsupported(IERC20 token_, uint256 amount_, address to_) external onlyOperator {
        ...
        require(address(token_) != address(pegToken), "pegToken");
        require(address(token_) != address(bond), "bond");
        require(address(token_) != address(share), "share");
        token_.safeTransfer(to_, amount_);
    }

    function boardroomAddOperator(address boardroom_, address operator_) external onlyOperator {
        IBoardroom(boardroom_).addOperator(operator_);
    }

    function boardroomRemoveOperator(address boardroom_, address operator_) external onlyOperator {
        IBoardroom(boardroom_).removeOperator(operator_);
    }

    function boardroomSetFeeAddress(address boardroom_, address feeAddress_) external onlyOperator {
        IBoardroomFee(boardroom_).setFeeAddress(feeAddress_);
    }

    function boardroomSetLockUp(address boardroom_, uint256 withdrawLockupEpochs_, uint256 rewardLockupEpochs_) external onlyOperator {
        IBoardroom(boardroom_).setLockUp(withdrawLockupEpochs_, rewardLockupEpochs_);
    }
    ...
    function boardroomGovernanceRecoverUnsupported(address boardroom_, address token_, uint256 amount_, address to_) external onlyOperator {
        IBoardroom(boardroom_).governanceRecoverUnsupported(token_, amount_, to_);
    }

    function boardroomGovernanceRecoverUnsupportedERC721(address boardroom_, address token_, uint256 tokenId_, address to_) external onlyOperator {
        INFTBoardroom(boardroom_).governanceRecoverUnsupportedERC721(token_, tokenId_, to_);
    }
}
...
interface INFTBoardroom {
    function governanceRecoverUnsupportedERC721(address token, uint256 tokenId, address to) external;
}
...
interface IBoardroom {
    ...
    function setLockUp(uint256 withdrawLockupEpochs, uint256 rewardLockupEpochs) external;
    function allocateSeigniorage(uint256 amount) external;
    function governanceRecoverUnsupported(address token, uint256 amount, address to) external;
}
