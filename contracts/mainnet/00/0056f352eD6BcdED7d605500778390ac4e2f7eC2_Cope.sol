...
contract Cope is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Distribution for cope reward pool
    uint256 public constant REWARD_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the COPE ERC-20 contract.
     */
    constructor() public ERC20("COPE", "COPE") {
        excludeAddress(address(this));

        _mint(msg.sender, 1 ether);
        taxRate = 0;
        taxCollectorAddress = address(0);
    }
    ...
    function _getCopePrice() internal view returns (uint256 _copePrice) {
        try IOracle(copeOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Cope: failed to fetch COPE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _copePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_copePrice >= taxTiersTwaps[tierId]) {
    ...
    function setCopeOracle(address _copeOracle) public onlyOperatorOrTaxOffice {
        require(_copeOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints COPE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of COPE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCopePrice = _getCopePrice();
            currentTaxRate = _updateTaxRate(currentCopePrice);
            if (currentCopePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        _mint(_rewardPool, REWARD_POOL_DISTRIBUTION);
