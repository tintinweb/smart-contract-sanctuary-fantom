...
// File @openzeppelin/contracts/token/ERC20/extensions/draft-IERC20[email protected]
...
// File contracts/EqualizerFtmChillvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualizerFtmChillvAMM is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public constant equal = address(0x3Fd3A0c85B70754eFc07aC9Ac0cbBDCe664865A6);
    address public constant chill = address(0xe47d957F83F8887063150AaF7187411351643392);
    address public want;
    ...
    IEqualizerRouter.Routes[] public equalToChillPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToChillPath
    ) {
        ...
        for (uint i; i < _equalToChillPath.length; ++i) {
           equalToChillPath.push(_equalToChillPath[i]);
        }
   ...
   //Withdraws funds from masterchef
    ...
    function chargeFees(address caller) internal {
        uint256 equalBal = IERC20(equal).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        uint256 callFee = equalBal * CALL_FEE / FEE_DIVISOR;
        IERC20(equal).safeTransfer(caller, callFee);

        uint256 recipientFee = equalBal * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(equal).safeTransfer(grimFeeRecipient, recipientFee);

        uint256 treasuryFee = equalBal * TREASURY_FEE / FEE_DIVISOR;
        IERC20(equal).safeTransfer(treasury, treasuryFee);

        uint256 stratFee = equalBal * STRAT_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToChillPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wftm).balanceOf(address(this));
        uint256 t2Bal = IERC20(chill).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(wftm, chill, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(chill).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(chill).safeApprove(router, type(uint).max);
    }
