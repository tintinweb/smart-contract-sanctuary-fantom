...
contract Zap is Ownable, IZap {
    ...
    function estimateZapInToken(
        ...
        if (
            ...
            uint256 nativeAmount = _from == WFTM
                ? _amt
                : _estimateSwap(_from, _amt, WFTM, _router);
            if (WFTM == IUniswapV2Pair(_to).token0()) {
                return (
                    nativeAmount.div(2),
                    _estimateSwap(
                        WFTM,
            ...
            if (WFTM == IUniswapV2Pair(_to).token1()) {
                return (
                    _estimateSwap(
                        WFTM,
                        nativeAmount.div(2),
    ...
    function alienOut(
        ...
        if (token0 == WFTM || token1 == WFTM) {
            ...
            address token = token0 != WFTM ? token0 : token1;
            uint256 amtToken;
            uint256 amtETH;
