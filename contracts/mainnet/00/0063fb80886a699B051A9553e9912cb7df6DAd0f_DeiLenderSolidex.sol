...
contract DeiLenderSolidex is BoringOwnable {
        ...
        returns (uint256)
    {
        ...
        return userCollateralAmount > neededCollateral ? userCollateralAmount - neededCollateral : 0;
    }
    ...
    function isSolvent(address user) internal view returns (bool) {
        ...
        return
            userCollateralAmount.mul(oracle.getPrice()).mul(LIQUIDATION_RATIO) /
                (uint256(1e18).mul(1e18)) > getDebt(user);
    }
    ...
    function removeCollateral(address to, uint256 amount) public {
        ...
        Holder(userHolder[msg.sender]).withdrawERC20(address(collateral), to, amount);
    ...
    function repayElastic(address to, uint256 debt) public returns (uint256 repayAmount) {
        accrue();

        uint256 amount = debt.mul(totalBorrow.base) / totalBorrow.elastic;

        (totalBorrow, repayAmount) = totalBorrow.sub(amount, true);
        require(debt == repayAmount, 'debt is not equal to repayAmount!');
        userBorrow[to] = userBorrow[to].sub(amount);

        dei.burnFrom(msg.sender, repayAmount);
    ...
    function liquidate(
        ...
        dei.burnFrom(msg.sender, totalDeiAmount);
    }
