...
contract Rabbit is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 49_000 ether;
    // Initial distribution for the day 0 RABBIT-WFTM LP -> RABBIT pool
    uint256 public constant INITIAL_RABBIT_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the Rabbit ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("RABBIT", "RABBIT") {
        // Mints 1 RABBIT to contract creator for initial pool setup
    ...
    function _getRabbitPrice() internal view returns (uint256 _rabbitPrice) {
        try IOracle(rabbitOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Rabbit: failed to fetch RABBIT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _rabbitPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_rabbitPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRabbitOracle(address _rabbitOracle) public onlyOperatorOrTaxOffice {
        require(_rabbitOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints RABBIT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of RABBIT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRabbitPrice = _getRabbitPrice();
            currentTaxRate = _updateTaxRate(currentRabbitPrice);
            if (currentRabbitPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_rabbitPool != address(0), "!_rabbitPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_rabbitPool, INITIAL_RABBIT_POOL_DISTRIBUTION);
