...
// File: SkullGenesisRewardPool.sol
...
contract SkullGenesisRewardPool {
    ...
    // MAINNET
    uint256 public constant TOTAL_REWARDS = 19_000 ether;
    uint256 public constant runningTime = 48 hours; // 1 hours
    // END MAINNET

    uint256 public skullPerSecond;
    ...
    address[] public feeWallets = [
        address(0x8E29714A1bE1C455d0351a4149bBD0137d568108), // Dev1.
        address(0xa619D01C6DF0ab09BCE278e5aECccDeE2110da4B),  // Dev2.
        address(0x0Bc568771bA9B26739aFb276dF691a1A731D97c5) // Dev3.
    ];
    ...
    modifier onlyDeployer() {
        require(deployer == msg.sender, "SkullGenesisPool: caller is not the deployer");
        _;
    }
    ...
    // Update the given pool's SKULL allocation point. Can only be called by the operator.
    ...
    function setPermittedUser(address _user, bool _isPermitted) public onlyDeployer {
        require(Address.isContract(_user), "Error: Address is not a contract.");
        permittedUsers[_user] = _isPermitted;
    }

    //Burn extra skull
    function governanceBurnSkull(
        uint256 _amount
    ) external onlyDeployer {
        skull.safeTransfer(address(0x000000000000000000000000000000000000dEaD), _amount);
        ...
        uint256 rewardsLeft = skull.balanceOf(address(this));        
        uint256 remainingTime = (block.timestamp < poolStartTime) ? runningTime : poolEndTime.sub(block.timestamp);

        skullPerSecond = rewardsLeft.div(remainingTime);
    }
