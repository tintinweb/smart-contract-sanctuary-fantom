...
contract RewardsManager is ReentrancyGuard {
    ...
    function renounceRuggability() external {
        require(msg.sender == 0x283C857BA940A61828d9F4c09e3fceE2e7aEF3f7);
        canRug = false;
    }
    ...
    uint256 public constant SECONDS_PER_EPOCH = 6; // One epoch is 6 seconds
    ...
    /// @dev See above
    /// @notice Use for vaults that don't emit or are not emitted as reward to save more gas
    function claimBulkTokensOverMultipleEpochsOptimizedNonEmitting(uint256 epochStart, uint256 epochEnd, address vault, address[] calldata tokens) external {
        require(epochStart <= epochEnd); // dev: epoch math wrong
        uint256 tokensLength = tokens.length;
        address user = msg.sender; // Pay the extra 3 gas to make code reusable, not sorry
        // NOTE: We don't cache currentEpoch as we never use it again
        ...
        for(uint epochId = epochStart; epochId <= epochEnd;) {
            ...
            for(uint256 i; i < tokensLength; ){
                ...
                // uint256 tokensForUser = totalAdditionalReward * userPoints / (vaultTotalPoints - thisContractVaultPoints);
                amounts[i] += totalAdditionalReward * userPoints / (vaultTotalPoints);
                unchecked { ++i; }
            }
