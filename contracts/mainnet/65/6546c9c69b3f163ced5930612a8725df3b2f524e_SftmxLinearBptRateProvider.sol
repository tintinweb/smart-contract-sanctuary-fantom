...
contract SftmxLinearBptRateProvider is IRateProvider {
    ...
    function getSftmxRate() external view returns (uint256 _rate) {
        return IFtmStaking(ftmStaking).getExchangeRate();
    }

    function getLinearBptRate() external view returns (uint256 _rate) {
        return IRateProvider(linearBpt).getRate();
    }
}