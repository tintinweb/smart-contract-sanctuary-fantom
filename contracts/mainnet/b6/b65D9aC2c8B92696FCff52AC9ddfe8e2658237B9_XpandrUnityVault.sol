...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR USDC-FTM EQUALIZER")),
            string(abi.encodePacked("XpE-USDC-FTM"))
        )
        {
        gauge = _gauge;
        router = _router;
    ...
    function _chargeFees(address caller) internal {                   
        ...
        uint usdProfit = IEqualizerPair(address(asset)).sample(wftm, toProfit, 1, 1)[0];
    ...
    function _addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmt, wftm, usdc, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(wftm, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(usdc, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(wftm, usdc, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(wftm, usdc, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
