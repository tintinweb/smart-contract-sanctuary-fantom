...
interface IMigratable {
    error AlreadyMigrated(uint256 id);
    ...
    function addMigration(
        LibMigratable.Migration memory migration,
        bool runPending
    ) external;
    ...
    function getMigration(uint256 id)
        external
        view
        returns (LibMigratable.Migration memory);
...
library LibMigratable {
     ...
     */
    function add(Migration memory migration) internal {
        if (_storage().migrationNames[migration.name]) {
            revert MigrationAlreadyAdded(migration.name);
        }
     ...
     * @dev Returns the migrartion with `id`.
     */
    function getMigration(uint256 id) internal view returns (Migration memory) {
    ...
    function runMigration(uint256 id) internal {
        Migration memory migration = _storage().migrations[id];
        if (_storage().executedMigrationIds.length >= id) {
            revert MigrationAlreadyExecuted(id, migration.name);
        }
