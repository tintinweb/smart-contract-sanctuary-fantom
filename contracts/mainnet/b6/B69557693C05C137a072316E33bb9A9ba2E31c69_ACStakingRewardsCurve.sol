...
/**
 * @title AutoCompound StakingRewards with curve stake token
 * @notice strategy for auto-compounding on pools using a standard StakingRewards contract
 * @author YieldWolf
 */
contract ACStakingRewardsCurve is AutoCompoundStrategy {
    ...
    function _farmEmergencyWithdraw() internal override {
     ...
     function earn(address _bountyHunter) external override onlyOwner returns (uint256 bountyReward) {
        ...
        liquidityPool.add_liquidity(address(stakeToken), amounts, 0);
        
        uint256 receiptAmount = stakeToken.balanceOf(address(this));
        stakeToken.safeIncreaseAllowance(address(stakeToken), receiptAmount);
