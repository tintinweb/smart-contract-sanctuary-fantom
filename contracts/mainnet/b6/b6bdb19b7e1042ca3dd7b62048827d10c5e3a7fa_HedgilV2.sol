...
interface IHedgilV2 is IHedgilPool{
    event OpenHedgil(
        address indexed owner,
    ...
    function openHedgil(address lpToken, uint lpAmount, uint maxPriceChange, uint period, address onBehalfOf) external returns (uint hedgilID, uint strike);
    ...
    function getCurrentPrice(address otherToken) external view returns (uint);
    function getHedgeValue(uint q, uint settlementPrice, uint strike) external view returns (uint);
    function getHedgilPrice(address, uint q, uint maxPriceChange, uint period) external view returns (uint price);
}
...
abstract contract HedgilPool is IHedgilPool, ERC20, Ownable {
    ...
    {
        // Only callable to mint (from == 0x0) and burn (to = 0x0)
        require(from == address(0) || to == address(0), "Hedgil pool shares are non-transferrable");
...
interface IHedgilOracle {
    function getIVRate(address otherToken, address quoteToken)
        external
        view
        returns (uint256);

    function getCurrentPrice(address otherToken, address quoteToken)
        external
        view
        returns (uint256);
}

contract HedgilV2 is IHedgilV2, HedgilPool {
    using SafeERC20 for IERC20;
    // Hedgil Oracle (for IVRate and price)
    // TODO: make oracle changeable
    ...
    constructor(address _quoteToken, address _oracle) ERC20("hedgil-A", "h-A") {
    ...
    function name() public view override returns (string memory) {
        return
            string(
                abi.encodePacked(
                    "hedgil-",
                    IERC20Extended(address(quoteToken)).symbol()
                )
            );
    }
    ...
    function _openHedgil(
        ...
        ) = getLPInfo(address(lpToken), lpAmount);

        address otherToken = address(quoteToken) == token0 ? token1 : token0;
        // get param Q, useful for calculating amount and price
        uint256 q = otherToken == token0 ? token0Amount : token1Amount;

        currentPrice = getCurrentPrice(otherToken);

        // calculate amount to be locked as collateral (which is the maximum payment of the hedge)
        uint256 lockAmount = getLockAmount(
            q,
            maxPriceChange,
            currentPrice,
            otherToken
        ...
        // NOTE: payment for the hedge is done in the quote token
        // NOTE: getHedgilPrice returns a cost in amount otherToken, which we then convert to quoteToken using currentPrice
        uint256 cost = getHedgilPrice(otherToken, q, maxPriceChange, period);
    ...
    function getHedgeValue(
        ...
        return
            (q * // otherToken decimals
            (
                settlementPrice + strike - (2 * strike * sqrt((settlementPrice * PRICE_DECIMALS) / strike, PRICE_DECIMALS)) / PRICE_DECIMALS)  // PRICE_DECIMALS
            )
            / PRICE_DECIMALS;
    }
        ...
        returns (uint256)
    {
        uint256 one = MAX_BPS;
        uint256 two = one * 2;
        // Q * 2 / h  * (2 - sqrt(1 - h) - sqrt(1 + h))
        return (two * (two - sqrt(one - h) - sqrt(one + h)) * q) / h / MAX_BPS; 
    }
    ...
    function getHedgilPrice(
        ...
        uint256 _ivRate = getIVRate(otherToken);
        require(_ivRate > 0, "!IV Rate undefined");
        return
            getHedgilAmount(q, maxPriceChange) * // size of hedge (in otherToken)
            _ivRate * sqrt(period, 1) / PRICE_MODIFIER_DECIMALS * // price factor (no unit)
            (10 ** IERC20Extended(address(quoteToken)).decimals()) * // scale up to quoteToken
            getCurrentPrice(otherToken) / PRICE_DECIMALS / // multiply by price (and remove decimals)
            (10 ** IERC20Extended(otherToken).decimals()); // scale down from otherToken
    }
    ...
    function getLockAmount(
        ...
        uint256 quoteTokenDecimals = 10 **
            uint256(IERC20Extended(address(quoteToken)).decimals());
        uint256 otherTokenDecimals = 10 **
            uint256(IERC20Extended(otherToken).decimals());
        ...
        return
            ...
            ) * quoteTokenDecimals) / otherTokenDecimals;
    }
    ...
    {
        IERC20Extended otherToken = IERC20Extended(hedgil.otherToken);
        uint256 quoteTokenDecimals = 10 **
            uint256(IERC20Extended(address(quoteToken)).decimals());
        uint256 otherTokenDecimals = 10 ** uint256(otherToken.decimals());
        uint256 currentPrice = getCurrentPrice(hedgil.otherToken);
    ...
    // use HedgilOracle to get IVtoken
    function getIVRate(address _token) public view returns (uint256) {
        return IHedgilOracle(oracle).getIVRate(_token, address(quoteToken));
    }

    function getCurrentPrice(address otherToken)
        public
        view
        override
        returns (uint256)
    {
        return
            IHedgilOracle(oracle).getCurrentPrice(
                otherToken,
                address(quoteToken)
            );
    }
    ...
    function _sendPayout(address account, uint256 payout) internal {
        IERC20(quoteToken).safeTransfer(account, payout);
    }
