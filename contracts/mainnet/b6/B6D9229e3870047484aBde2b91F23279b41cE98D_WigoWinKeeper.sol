...
import "./interfaces/IWigoWin.sol";

contract WigoWinKeeper is KeeperCompatibleInterface, Ownable, Pausable {
    address public wigoWinContract;
    ...
    event NewWigoWinContract(address indexed wigoWinContract);
    event NewAheadTimeForCheckUpkeep(uint256 time);
    event NewAheadTimeForPerformUpkeep(uint256 time);

    /**
     * @notice Constructor
     * @param _wigoWinContract: WigoWin Contract address
     */
    constructor(address _wigoWinContract) {
        require(_wigoWinContract != address(0), "Cannot be zero address");
        wigoWinContract = _wigoWinContract;
    }
    ...
    {
        if (!paused()) {
            bool genesisStartOnce = IWigoWin(wigoWinContract)
                .genesisStartOnce();
            bool genesisLockOnce = IWigoWin(wigoWinContract).genesisLockOnce();
            bool paused = IWigoWin(wigoWinContract).paused();
            uint256 currentEpoch = IWigoWin(wigoWinContract).currentEpoch();
            uint256 bufferSeconds = IWigoWin(wigoWinContract).bufferSeconds();
            IWigoWin.Round memory round = IWigoWin(wigoWinContract).rounds(
                currentEpoch
            );
    ...
    function performUpkeep(bytes calldata)
        ...
        whenNotPaused
    {
        require(wigoWinContract != address(0), "wigoWinContract Not Set!");
        bool genesisStartOnce = IWigoWin(wigoWinContract).genesisStartOnce();
        bool genesisLockOnce = IWigoWin(wigoWinContract).genesisLockOnce();
        bool paused = IWigoWin(wigoWinContract).paused();
        uint256 currentEpoch = IWigoWin(wigoWinContract).currentEpoch();
        uint256 bufferSeconds = IWigoWin(wigoWinContract).bufferSeconds();
        ...
        if (paused) {
            // unpause operation
            IWigoWin(wigoWinContract).unpause();
        } else {
            if (!genesisStartOnce) {
                IWigoWin(wigoWinContract).genesisStartRound();
            } else if (!genesisLockOnce) {
                // Too early for locking of round, skip current job (also means previous lockRound was successful)
                if (
                    lockTimestamp == 0 ||
                    ...
                    // Too late to lock round, need to pause
                    IWigoWin(wigoWinContract).pause();
                } else {
                    //run genesisLockRound
                    IWigoWin(wigoWinContract).genesisLockRound();
                }
            } else {
                if (
                    block.timestamp + aheadTimeForPerformUpkeep > lockTimestamp
                    ...
                    if (
                        ...
                        //run executeRound
                        IWigoWin(wigoWinContract).executeRound();
                    }
                }
            }
        }
    ...
    function setWigoWinContract(address _wigoWinContract) external onlyOwner {
        require(_wigoWinContract != address(0), "Cannot be zero address");
        wigoWinContract = _wigoWinContract;
        emit NewWigoWinContract(_wigoWinContract);
    }
