/**
 *Submitted for verification at FtmScan.com on 2022-10-01
*/
...
contract ZapHelperV2 {
    ...
    function swapTokensForBestAmountOutReadOnly(
        IOptiSwap _optiSwap,
        address tokenIn,
        address tokenOut,
        uint256 amountIn
    ...
    function optiSwapExactTokensForTokensReadOnly(
        address tokenIn,
        address tokenOut,
        uint256 amountIn
        ...
        if (nextHop == tokenOut) {
            return swapTokensForBestAmountOutReadOnly(_optiSwap, tokenIn, tokenOut, amountIn);
        }
        ...
        if (nextHop != tokenIn) {
            hopAmountOut = swapTokensForBestAmountOutReadOnly(_optiSwap, tokenIn, nextHop, amountIn);
        } else {
            hopAmountOut = amountIn;
        }
        if (nextHop == waypoint) {
            return swapTokensForBestAmountOutReadOnly(_optiSwap, nextHop, tokenOut, hopAmountOut);
        } else if (waypoint == tokenOut) {
            return this.optiSwapExactTokensForTokensReadOnly(nextHop, tokenOut, hopAmountOut);
        } else {
            uint256 waypointAmountOut = this.optiSwapExactTokensForTokensReadOnly(nextHop, waypoint, hopAmountOut);
            return swapTokensForBestAmountOutReadOnly(_optiSwap, waypoint, tokenOut, waypointAmountOut);
        }
    }
    ...
    function optiSwapExactTokensForTokens(
        ...
        if (nextHop == waypoint) {
            ...
            return this.optiSwapExactTokensForTokens(nextHop, tokenOut, hopAmountOut);
        } else {
            uint256 waypointAmountOut = this.optiSwapExactTokensForTokens(nextHop, waypoint, hopAmountOut);
    ...
    function zapETH(address _LP) public payable returns (uint256 liquidity) {
        ...
        bool swap = withSwap(_LP,WETH,msg.value);
        address tokenA = swap ? IUniswapV2Pair(_LP).token1() : IUniswapV2Pair(_LP).token0();
        address tokenB = swap ? IUniswapV2Pair(_LP).token0() : IUniswapV2Pair(_LP).token1();

        IWETH(WETH).deposit{value: msg.value}();
        
        try this.optiSwapExactTokensForTokens(WETH, tokenA, msg.value) {            

        } catch {
            this.optiSwapExactTokensForTokens(WETH, tokenB, msg.value);
            address intermed = tokenA;
            tokenA = tokenB;
            tokenB = intermed;
        }
        ...
        (uint256 r0,,) = IUniswapV2Pair(_LP).getReserves();
        uint256 reserveA = r0;
    ...
    function quote(address _LP, address _inputToken, uint256 _inputAmount) public view returns (address tokenA, address tokenB, uint256 tokenAestimated, uint256 tokenBestimated) {
        (tokenA,tokenB,tokenAestimated,tokenBestimated) = zapquote(_LP, _inputToken, _inputAmount, withSwap(_LP,_inputToken,_inputAmount));
    }

    function withSwap(address _LP, address _inputToken, uint256 _inputAmount) public view returns (bool withSwapResult) {
        (,,uint256 firstTry,) = zapquote(_LP, _inputToken, _inputAmount, true);
        (,,,uint256 secondTry) = zapquote(_LP, _inputToken, _inputAmount, false);
        if(firstTry >= secondTry){
            withSwapResult = true;
        }else{
            withSwapResult = false;
        }
    }

    function zapquote(address _LP, address _inputToken, uint256 _inputAmount, bool swap) public view returns (address tokenA, address tokenB, uint256 tokenAfinal, uint256 tokenBfinal) {

        tokenA = swap ? IUniswapV2Pair(_LP).token1() : IUniswapV2Pair(_LP).token0();
        tokenB = swap ? IUniswapV2Pair(_LP).token0() : IUniswapV2Pair(_LP).token1();
        uint256 totalAmountA;
        
        try this.optiSwapExactTokensForTokensReadOnly(_inputToken, tokenA, _inputAmount) returns (uint256 amountOut) {            
            totalAmountA = amountOut;
        } catch {
            totalAmountA = this.optiSwapExactTokensForTokensReadOnly(_inputToken, tokenB, _inputAmount);
            address intermed = tokenA;
            tokenA = tokenB;
            tokenB = intermed;
        }
        ...
        uint256 balanceBeforeB = tokenB.myBalance();
        uint256 balanceAfterB = tokenB.myBalance() + router.getAmountOut(swapAmount, swap ? r1 : r0, swap ? r0 : r1);
        return (tokenA, tokenB, totalAmountA.sub(swapAmount), balanceAfterB.sub(balanceBeforeB));
    }

    function zap(address _LP, address _inputToken, uint256 _inputAmount) public returns (uint256 liquidity) {

        bool swap = withSwap(_LP,_inputToken,_inputAmount);
        address tokenA = swap ? IUniswapV2Pair(_LP).token1() : IUniswapV2Pair(_LP).token0();
        address tokenB = swap ? IUniswapV2Pair(_LP).token0() : IUniswapV2Pair(_LP).token1();
        
        IERC20(_inputToken).transferFrom(msg.sender, address(this), _inputAmount);

        try this.optiSwapExactTokensForTokens(_inputToken, tokenA, _inputAmount) {            

        } catch {
            this.optiSwapExactTokensForTokens(_inputToken, tokenB, _inputAmount);
            address intermed = tokenA;
            tokenA = tokenB;
            tokenB = intermed;
        }
