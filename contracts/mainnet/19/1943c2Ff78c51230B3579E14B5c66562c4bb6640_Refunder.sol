...
contract Refunder is Ownable, ReentrancyGuard {
    ...
    uint public exchangeRate = 0.12 ether;

    event Refunded(address asset, address sender, uint amount);

    constructor(
        // address _luxorAddress,
        // address _wftmAddress,
        // uint _exchangeRate
    ) {
        // LUX = IERC20(_luxorAddress);
        // WFTM = IERC20(_wftmAddress);
        // setExchangeRate(_exchangeRate);
    }

    function refund(uint amount) public nonReentrant {
        require(isActive, 'refunds have not yet begun');
        uint refundAmount = getRefundAmount(amount);

        // [c] checks: availability & balances.
        require(_checkAvailability(refundAmount), 'unavailable');
        require(LUX.balanceOf(msg.sender) >= amount, 'insufficient balance');

        // [i] burns LUX.
        LUX.safeTransferFrom(msg.sender, address(this), amount);
        // [i] sends: asset to sender.
        WFTM.safeTransfer(msg.sender, refundAmount);

        emit Refunded(address(WFTM), msg.sender, amount);
    }

    function _checkAvailability(uint amount) internal view returns (bool) {
        require(WFTM.balanceOf(address(this)) >= amount, 'insufficient balance');

        return true;
    }

    function showAvailable() external view returns (uint available) {
        available = WFTM.balanceOf(address(this));
    }

    function getRefundAmount(uint amount) public view returns (uint refundAmount) {
        refundAmount = amount * exchangeRate / 1E9;
    }

    function returnFunds() external onlyOwner {
        WFTM.safeTransfer(owner(), WFTM.balanceOf(address(this)));
    }
  ...
  function unstuck() external onlyOwner {
