...
contract CAKE is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 0 CAKE-WFTM LP -> CAKE pool
    uint256 public constant INITIAL_CAKE_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the CAKE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("CAKE", "CAKE") {
        // Mints 1 CAKE to contract creator for initial pool setup
    ...
    function _getCAKEPrice() internal view returns (uint256 _CAKEPrice) {
        try IOracle(CAKEOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("CAKE: failed to fetch CAKE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _CAKEPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_CAKEPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCAKEOracle(address _CAKEOracle) public onlyOperatorOrTaxOffice {
        require(_CAKEOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CAKE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CAKE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCAKEPrice = _getCAKEPrice();
            currentTaxRate = _updateTaxRate(currentCAKEPrice);
            if (currentCAKEPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_CAKEPool != address(0), "!_CAKEPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_CAKEPool, INITIAL_CAKE_POOL_DISTRIBUTION);
