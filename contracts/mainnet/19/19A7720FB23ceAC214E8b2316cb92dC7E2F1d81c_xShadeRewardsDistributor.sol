...
contract xShadeRewardsDistributor is ReentrancyGuard, Ownable {
	...
	function _checkpointToken() internal {
		...
		uint256 lastWeek = (_lastRewardsTime / WEEK) * WEEK;
		uint256 nextWeek;
		...
		for (uint256 i = 0; i < 104; i++) {
			...
			if (block.timestamp < nextWeek) {
				if (sinceLast == 0) {
					rewardsPerWeek[nextWeek] += rewardsToDistribute;
				} else {
					rewardsPerWeek[nextWeek] += (rewardsToDistribute * (block.timestamp - _lastRewardsTime)) / sinceLast;
				}
				break;
			} else {
				if (sinceLast == 0 && nextWeek == _lastRewardsTime) {
					...
					rewardsPerWeek[nextWeek] += (rewardsToDistribute * (nextWeek - _lastRewardsTime)) / sinceLast;
				}
			}
	...
	function _checkpointTotalSupply() internal {
		...
		uint256 epochStartTimestamp = (block.timestamp / WEEK) * WEEK;

		xShadeToken.checkpoint();

		for (uint256 i = 0; i < 104; i++) {
			if (_timeCursor >= epochStartTimestamp) {
				break;
			} else {
	...
	/// @notice Update the total supply checkpoint
	...
	function _claim(address addr, uint256 _lastRewardsTime) internal returns (uint256) {
		...
		if (weekCursor == 0) {
			weekCursor = ((userPoint.timeStamp - 1) / WEEK) * WEEK;
		}
		...
		for (uint256 i = 0; i < 50; i++) {
			...
			if (weekCursor >= userPoint.timeStamp && userEpoch <= maxUserEpoch) {
				userEpoch += 1;
				oldUserPoint = Point({ bias: userPoint.bias, slope: userPoint.slope, timeStamp: userPoint.timeStamp, blockNumber: userPoint.blockNumber });
	...
	/// @dev Each call to claim look at a maximum of 50 user points. For accounts with many xShade related actions,
