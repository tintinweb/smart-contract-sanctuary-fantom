...
contract DeiUsdcZapper is Ownable {
    ...
    address[] usdc2deiPath;
    address[] usdc2deusPath;
    ...
    /* ========== PUBLIC FUNCTIONS ========== */
    function usdcToDeiWithMinting(ProxyInput memory proxyInput, uint256 totalUsdcAmount)
        internal
    ...
    function usdcToDeiWithoutMinting(uint256 swapAmount, uint256 totalUsdcAmount)
        internal
        returns (uint256 token0Bought, uint256 token1Bought)
    {
        IUniwapV2Pair pair = IUniwapV2Pair(pairAddress);
        if (usdcAddress == pair.token0()) {
            token1Bought = IUniswapV2Router02(uniswapRouter)
                    .swapExactTokensForTokens(
                        ...
                        address(this),
                        deadline
                    )[usdc2deiPath.length - 1];
            token0Bought = totalUsdcAmount - swapAmount;
        } else {
            ...
            token1Bought = totalUsdcAmount - swapAmount;
        }
    }

    function zapInNativecoinWithMinting(
        uint256 minLPAmount,
        ...
        (uint256 token0Bought, uint256 token1Bought) = usdcToDeiWithMinting(
            proxyInput,
            totalUsdcAmount
        );
    ...
    function zapInNativecoinWithoutMinting(
        uint256 minLPAmount,
        bool transferResidual, // Set false to save gas by donating the residual remaining after a Zap
        address[] calldata path,
        uint256 amountToSwap
        ...
        (uint256 token0Bought, uint256 token1Bought) = usdcToDeiWithoutMinting(
            amountToSwap,
            totalUsdcAmount
        );
    ...
    function zapInERC20WithMinting(
        uint256 amountIn,
        uint256 minLPAmount,
        bool transferResidual, // Set false to save gas by donating the residual remaining after a Zap
        ProxyInput memory proxyInput,
        ...
        if (path[0] == deiAddress) {
            ...
            if (path[0] != usdcAddress) {
                ...
                if (
                    IERC20(path[0]).allowance(address(this), uniswapRouter) < amountIn
            ...
            proxyInput.amountIn = amountToSwap;
            (token0Bought, token1Bought) = usdcToDeiWithMinting(
                proxyInput,
                totalUsdcAmount
            );
        }
    ...
    function zapInERC20WithoutMinting(
        uint256 amountIn,
        uint256 minLPAmount,
        bool transferResidual, // Set false to save gas by donating the residual remaining after a Zap
        address[] calldata path,
        ...
        if (path[0] == deiAddress) {
            ...
            (token0Bought, token1Bought) = usdcToDeiWithoutMinting(
                amountToSwap,
                totalUsdcAmount
            );
        }
    ...
    function _uniDeposit(
        ...
        // because always there is extra usdc
    ...
    function getAmountOut(ZapperInput memory input, bool isWithMint)
        public
        view
        returns (
            uint256 percentage,
        ...
        )
    {
        ...
        if (input.toUsdcPath[0] == deiAddress){
            // swap amount: dei to usdc amount
            swapAmount = calculateSwapInAmount(reservedDei, input.amountIn);
			uint dei_pure_amount = input.amountIn - swapAmount; 
			percentage = dei_pure_amount * 1e6 / (reservedDei + dei_pure_amount + swapAmount);
			lp = dei_pure_amount * pair.totalSupply() / (reservedDei + swapAmount);
        }else{
            /// convert token to usdc
            if(input.toUsdcPath[0] != usdcAddress){
                input.amountIn = IUniswapV2Router02(uniswapRouter)
                .getAmountsOut(input.amountIn, input.toUsdcPath)[input.toUsdcPath.length - 1];
            }
            
            if(isWithMint){
                // swap amount: usdc to dei amount
                swapAmount = (input.amountIn * reservedDei) / (reservedDei + reservedUsdc * 1e12);                        
                ...
                // usdc_pure_amount = input.amountIn - swapAmount;
                percentage = (input.amountIn - swapAmount) * 1e6 / (reservedUsdc + (input.amountIn - swapAmount));
                lp = (input.amountIn - swapAmount) * pair.totalSupply() / (reservedUsdc);
                
            }else{
                // swap amount: usdc to dei amount
                swapAmount = calculateSwapInAmount(reservedUsdc, input.amountIn);
                percentage = (input.amountIn - swapAmount) * 1e6 / (reservedUsdc + (input.amountIn - swapAmount) + swapAmount);
			    lp = (input.amountIn - swapAmount) * pair.totalSupply() / (reservedUsdc + swapAmount);
            }
        }
    }
