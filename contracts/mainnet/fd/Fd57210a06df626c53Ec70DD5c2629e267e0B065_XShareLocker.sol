...
contract XShareLocker is Operator, IXShareLocker {
    ...
    uint256 public EXTRA_YSHARE_PER_WEEK = MAX_YSHARE_MINTED_PER_XSHARE.div(MAX_LOCK_DURATION); //Extra yShare for every 1 week longer lock
    ...
    bool unlockAll = false; //Allow to unlock all without waiting lock end, use in emergency cases
    ...
    function lockXShare(uint256 _amount, uint256 _lockDuration) public override isInitialized {
    ...
    function updateAverageLockDuration(uint256 _amount, uint256 _lockDuration) internal {
        averageLockDuration = (totalLockedAmount.mul(averageLockDuration).add(_amount.mul(_lockDuration))).div(totalLockedAmount.add(_amount));
...
interface IYShare {
    ...
    function lockerMintFrom(address _address, uint256 m_amount) external;
}
