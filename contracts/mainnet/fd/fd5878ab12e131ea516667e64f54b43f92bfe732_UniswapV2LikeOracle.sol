...
abstract contract OracleBase is IOracle {
    ...
    function getRate(IERC20 srcToken, IERC20 dstToken, IERC20 connector) public view override returns (uint256 rate, uint256 weight) {
        uint256 balance0;
        uint256 balance1;
        if (connector == _NONE) {
    ...
    function getRates(IERC20[] calldata srcTokens, IERC20[] calldata dstTokens, IERC20[] calldata connectors) external view returns (uint256[] memory rates, uint256[] memory weights) {
        ...
        require(len == dstTokens.length && len == connectors.length, "OracleBase: input arrays length mismatch");

        rates = new uint256[](len);
        weights = new uint256[](len);

        for (uint256 i = 0; i < len; i++) {
            (rates[i], weights[i]) = getRate(srcTokens[i], dstTokens[i], connectors[i]);
        }
    }
...
contract UniswapV2LikeOracle is OracleBase {
    ...
    event FactoryInitCodeHashCalculated(bytes32 indexed initcodeHash);
    ...
    constructor(address _factory, bytes32 _initcodeHash) {
        ...
        require(initcodeHash != bytes32(0), "UniswapV2LikeOracle: initcodeHash cannot be zero");

        emit FactoryInitCodeHashCalculated(initcodeHash);
    }
