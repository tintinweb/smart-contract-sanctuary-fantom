...
contract Temple is ITemple, ERC1155, Ownable, Pausable {
    ...
    modifier requireContractsSet() {
        require(address(gemToken) != address(0), "Contracts not set");
        _;
    }

    function setContracts(address _gem) external onlyOwner {
        require(_gem != address(0), "Gem is zero addresss");
        gemToken = IGem(_gem);
    }
    ...
    */
    function mint(uint256 typeId, uint256 qty, address recipient) external override whenNotPaused {
        require(admins[_msgSender()], "Only admins can call this");
        require(typeInfo[typeId].mints - typeInfo[typeId].burns + qty <= typeInfo[typeId].maxSupply, "All tokens minted");
        // If the ERC1155 is swapped for $GEM, transfer the GEM to this contract in case the swap back is desired.
        // NOTE: This will fail if the origin doesn't have the required amount of $GEM
        if (typeInfo[typeId].gemExchangeAmt > 0) gemToken.transferFrom(
            tx.origin,
            address(this),
            typeInfo[typeId].gemExchangeAmt * qty
        );
    ...
    */
    function burn(uint256 typeId, uint256 qty, address burnFrom) external override whenNotPaused {
        require(admins[_msgSender()], "Only admins can call this");
        // If the ERC1155 was swapped from $GEM, transfer the GEM from this contract back to whoever owns this token now.
        if (typeInfo[typeId].gemExchangeAmt > 0) gemToken.transferFrom(
            address(this),
            tx.origin,
            typeInfo[typeId].gemExchangeAmt * qty
    ...
    function setExchangeAmt(uint256 typeId, uint256 exchangeAmt) external onlyOwner {
        ...
        typeInfo[typeId].gemExchangeAmt = exchangeAmt;
    }

    function updateOriginAccess() external override {
        require(admins[_msgSender()], "Only admins can call this");
        lastWrite[tx.origin].blockNum = block.number;
        lastWrite[tx.origin].time = block.timestamp;
    }
    ...
    */
    ...
    function uri(uint256 typeId) public view override returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked(
            '{"name": "',
            img.name,
            '", "description": "Mysterious items spawned from the Temple of the Hamster & Owls Hideout. Fabled to hold magical properties, only Act 1 hideout guardians will know the truth in the following acts. All the metadata and images are generated and stored 100% on-chain. No IPFS. NO API. Just the Fantom blockchain.", "image": "data:image/svg+xml;base64,',
            bytes(drawSVG(typeId)).base64(),
            '", "attributes": []',
            "}"
        ));
    ...
    function uploadImage(uint256 typeId, Image memory image) external onlyOwner {
        traitData[typeId] = image;
    }

    function drawImage(Image memory image) internal pure returns (string memory) {
        return string(abi.encodePacked(
            '<image x="4" y="4" width="512" height="512" image-rendering="pixelated" preserveAspectRatio="xMidYMid" xlink:href="data:image/svg+xml;base64,',
            image.png,
            '"/>'
        ));
    }
    ...
    function drawSVG(uint256 typeId) internal view returns (string memory) {
        ...
        return string(abi.encodePacked(
            '<svg id="temple" width="100%" height="100%" version="1.1" viewBox="0 0 520 520" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
            svgString,
            "</svg>"
        ));
    }
    ...
    function safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) public virtual override(ERC1155, ITemple) {
    ...
    function balanceOf(address account, uint256 id) public view virtual override(ERC1155, ITemple) disallowIfStateIsChanging returns (uint256) {
