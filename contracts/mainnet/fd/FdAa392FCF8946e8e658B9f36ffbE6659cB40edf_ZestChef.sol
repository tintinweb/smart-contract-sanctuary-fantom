...
contract ZestChef is Ownable {
    ...
    IZestStaking public rewardMinter;
    ...
    /// @notice Update reward variables for pools. Be careful of gas spending!
    function updatePools(uint256[] memory pids) public {
    ...
    /// @param _update should update all pools
    function add(
        uint256 allocPoint,
        IERC20 _lpToken,
        IRewarder _rewarder,
    ...
    /// @param _update should update all pools
    function setRewardPerSecond(uint256 _rewardPerSecond, bool _update)
        public
        onlyOwnerOrController
    {
        require(
            _rewardPerSecond <= MAX_REWARD_PER_SECOND,
            "ZestChef::setRewardPerSecond: > MAX_REWARD_PER_SECOND"
        );
        if (_update) {
            massUpdatePools();
        }
    ...
    function setEmissionController(address _controller) public onlyOwner {
        require(_controller != address(0), "ZestChef::setEmissionController: INVALID ADDRESS");
    ...
    function setRewardMinter(address _rewardMinter) external onlyOwner {
        require(_rewardMinter != address(0), "ZestChef::setRewardMinter: Invalid address");
        rewardMinter = IZestStaking(_rewardMinter);
    }
