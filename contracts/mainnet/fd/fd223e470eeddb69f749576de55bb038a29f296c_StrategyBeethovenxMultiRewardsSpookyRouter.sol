...
// File contracts/BIFI/strategies/Beethovenx/StrategyBeethovenxMultiRewardsSpookyRouter.sol

pragma solidity ^0.8.0;

contract StrategyBeethovenxMultiRewardsSpookyRouter is StratFeeManager, GasFeeThrottler {
    ...
    // Routes
    address[][] public rewardTokensToNativeRoute;
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        uint256 rewardTokensBal = IERC20(rewardTokens[0]).balanceOf(address(this));
        if (outputBal > 0 || rewardTokensBal > 0) {
    ...
    // swaps rewards to native
    function _swapRewardsToNative() internal {
        if (rewardTokensToNativeRoute.length != 0) {
            for (uint i; i < rewardTokensToNativeRoute.length; i++) {
                uint256 toNative = IERC20(rewardTokensToNativeRoute[i][0]).balanceOf(address(this));
                if (toNative > 0) {
                    IUniswapRouterETH(spookyRouter).swapExactTokensForTokens(
                        toNative,
                        0,
                        rewardTokensToNativeRoute[i],
                        address(this),
                        block.timestamp
                    );
                }
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (OutputToNative > 0) {
            balancerSwap(nativeSwapPoolId, output, native, OutputToNative);
        }

        if (rewardTokensToNativeRoute.length != 0) {
            for (uint i; i < rewardTokensToNativeRoute.length; i++) {
                uint256 RewardToNative = IERC20(rewardTokensToNativeRoute[i][0]).balanceOf(address(this));
                if (RewardToNative > 0) {
                    IUniswapRouterETH(spookyRouter).swapExactTokensForTokens(
                        RewardToNative,
                        0,
    ...
    function _giveAllowances() internal {
        ...
        if (rewardTokensToNativeRoute.length != 0) {
            for (uint i; i < rewardTokensToNativeRoute.length; i++) {
                IERC20(rewardTokensToNativeRoute[i][0]).safeApprove(spookyRouter, 0);
                IERC20(rewardTokensToNativeRoute[i][0]).safeApprove(spookyRouter, type(uint256).max);
            }
        }
    }
    ...
    function addRewardRoute(address[] memory _rewardTokensToNativeRoute) external onlyOwner {
        IERC20(_rewardTokensToNativeRoute[0]).safeApprove(spookyRouter, 0);
        IERC20(_rewardTokensToNativeRoute[0]).safeApprove(spookyRouter, type(uint256).max);
        rewardTokensToNativeRoute.push(_rewardTokensToNativeRoute);
    }

    function removeLastRewardRoute() external onlyManager {
        address reward = rewardTokensToNativeRoute[rewardTokensToNativeRoute.length - 1][0];
        IERC20(reward).safeApprove(spookyRouter, 0);
        rewardTokensToNativeRoute.pop();
    }
    ...
    function rewardTokensToNative() external view returns (address[][] memory) {
        return rewardTokensToNativeRoute;
    }
