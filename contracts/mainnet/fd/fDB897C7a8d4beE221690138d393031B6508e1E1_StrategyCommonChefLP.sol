...
import "../../interfaces/common/IProfitDistributor.sol";
...
contract StrategyCommonChefLP is StratManager, FeeManager, GasThrottler {
    ...
    address[] public outputToProfitRoute;

    event StratHarvest(address indexed harvester, uint256 wantHarvested, uint256 tvl);
    event Deposit(uint256 tvl);
    event Withdraw(uint256 tvl);
    event ChargedFees(uint256 callFees, uint256 beefyFees, uint256 protocolFees, uint256 profitSent);
    ...
    // set Profit
    function setProfit(address _profit, address[] memory _outputToProfitRoute) external onlyManager {
        require(_profit != address(0), "address not valid");
        profit = _profit;
        outputToProfitRoute = _outputToProfitRoute;
    }

    // performance fees
    function chargeFees(address callFeeRecipient) internal {
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(MAX_FEE).div(10000);
        uint256 toProfit = IERC20(output).balanceOf(address(this)).mul(takeProfitBP).div(10000);
        
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toProfit, 0, outputToProfitRoute, address(this), now);
        uint256 nativeBal = IERC20(native).balanceOf(address(this));

        //transfer profit token to the profitDistributor
        uint256 profitBal = IERC20(profit).balanceOf(address(this));
        IERC20(profit).safeTransfer(profitDistributor, profitBal);
        IProfitDistributor(profitDistributor).recordProfitDeposited(profitBal);
        ...
        emit ChargedFees(callFeeAmount, beefyFeeAmount, strategistFeeAmount, profitBal);
    }
    ...
    function setTakeProfit(address _profitDistributor, uint256 _takeProfitBP) external onlyManager {
        require(_profitDistributor != address(0), "not valid profitDistributor");
        require(_takeProfitBP >= 0 || _takeProfitBP <=1500, "not valid takeProfitBP");
        profitDistributor = _profitDistributor;
        takeProfitBP = _takeProfitBP;
    }
...
interface IProfitDistributor {
    function setProfitPerSecond(uint256 _pid) external;    
...
abstract contract FeeManager is StratManager {
    uint constant public STRATEGIST_FEE = 225;
    uint constant public MAX_FEE = 450;
