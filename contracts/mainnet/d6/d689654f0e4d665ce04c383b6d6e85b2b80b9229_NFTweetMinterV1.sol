...
// File: contracts/nftweets/NFTweets_Minter.sol
...
interface INFTWEETS{
    function safeMint(address to, string memory uri, uint256 tweetId) external;
}

contract NFTweetMinterV1 is Pausable, Ownable, AutomationCompatibleInterface {
    ...
    uint256 public OfferDevFee = 200000000000000000 wei;
    uint256 public SaleDevFee = 10; // %
    uint8 public maxEndTime = 30;
    address public DevWallet;
    address public KeeperUpkeep;
    ...
    mapping(uint256 => bool) public mintedTweets; // minted tweets
    ...
    function VerifyTweetData(bytes32 _hashedMessage, uint8 _v, bytes32 _r, bytes32 _s, uint256 _tweetId, string memory uri) internal view returns (bool) {
        require(mintedTweets[_tweetId] != true, "Signature expired");
    ...
    function mintSelfTweet(uint256 _tweetId, string memory uri, bytes32 _hashedMessage, uint8 _v, bytes32 _r, bytes32 _s) public payable{
        ...
        require(msg.value >= MinPrice, "Insufficient funds");
        require(VerifyTweetData(_hashedMessage, _v, _r, _s, _tweetId, uri), "Data is not valid");
        mintedTweets[_tweetId] = true;
        if (orders[_tweetId].offerAmount >= 1) { // if there is an active offer
            AcceptedPayment.safeTransfer(orders[_tweetId].maker, orders[_tweetId].offerAmount); // refund the offer
            cancelOrder(_tweetId);
        }        
        NFTWEETS.safeMint(msg.sender, uri, _tweetId);
    }
    ...
    function acceptMintOffer(uint256 _tweetId, string memory uri, bytes32 _hashedMessage, uint8 _v, bytes32 _r, bytes32 _s) public {
        ...
        require(VerifyTweetData(_hashedMessage, _v, _r, _s, _tweetId, uri), "Data is not valid");
        mintedTweets[_tweetId] = true;
        uint256 SaleDevPercent = (orders[_tweetId].offerAmount * SaleDevFee) / 100;
        uint256 TweetOwnerAmount = orders[_tweetId].offerAmount - SaleDevPercent;
        AcceptedPayment.safeTransfer(msg.sender, TweetOwnerAmount);
        AcceptedPayment.safeTransfer(DevWallet, SaleDevPercent);
        NFTWEETS.safeMint(orders[_tweetId].maker, uri, _tweetId);
        cancelOrder(_tweetId);
    ...
    function getActiveOrders() external view returns (uint _activeOrders) {
         _activeOrders = tweetOrders.length;
    }

    function performUpkeep(bytes calldata) external override {
        require(msg.sender == KeeperUpkeep, "Fulfillment only permitted by Keepers");
    ...
    function setNFTContract(address _NFTweets) public onlyOwner {
    ...
    function setKeeperAddress(address _address) public onlyOwner {
        KeeperUpkeep = address(_address);
    }

    function setAllowedSigner(address _address) public onlyOwner {
        AllowedSigner = address(_address);
    }
    ...
    function setDevFee(uint256 _OfferDevFee, uint256 _SaleDevFee) public onlyOwner {
    ...
    function withdrawDevFees() public payable onlyOwner {
