...
\______   \_____     ______  ____    __| _/   \_   _____/|__|  ____  _____     ____    ____   ____
 ...
 |______  /(____  //____  > \___  >\____ |     \___  /   |__||___|  /(____  /|___|  / \___  >\___  >
        \/      \/      \/      \/      \/         \/             \/      \/      \/      \/     \/
*/
contract InfinitePrinter is ContractGuard, Operator {
    ...
    uint256 public constant PERIOD = 1 hours; //============epoch length TESTING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    address[] public excludedFromTotalSupply = [
        address(0x6dc04fb3094B3739151c4fbaFC893655b299139E), // OBolGenesisRewardPool
        address(0x0A10daD90b9C6FB8B87BFf3857A4B012890C53A5) // Next-Gen-Fund   
    ];
    ...
    modifier checkCondition {
        require(block.timestamp >= startTime, "InfinitePrinter: not started yet");

        _;
    }

    modifier checkEpoch {
        require(block.timestamp >= nextEpochPoint(), "InfinitePrinter: not opened yet");
    ...
    modifier checkOperator {
        require(Operator(boardroom).operator() == address(this), "InfinitePrinter: need more permission" );
        _;
    }

    modifier notInitialized {
        require(!initialized, "InfinitePrinter: already initialized");
    ...
    // // oracle
    // function getOBolPrice() public view returns (uint256 oBolPrice) {
    //     try IOracle(oBolOracle).consult(oBol, 1e18) returns (uint144 price) {
    //         return uint256(price);
    //     } catch {
    //         revert("InfinitePrinter: failed to consult BLIFE price from the oracle");
    //     }
    // }

    // function getOBolUpdatedPrice() public view returns (uint256 _oBolPrice) {
    //     try IOracle(oBolOracle).twap(oBol, 1e18) returns (uint144 price) {
    //         return uint256(price);
    //     } catch {
    //         revert("InfinitePrinter: failed to consult BLIFE price from the oracle");
    //     }
    ...
    // function setOBolOracle(address _oBolOracle) external onlyOperator {
    //     oBolOracle = _oBolOracle;
    // }
    ...
    function setSupplyTiersEntry(uint8 _index, uint256 _value) external onlyOperator returns (bool) {
        require(_index >= 0, "Index has to be higher or equal to 0");
    ...
    // can adjust bootstrap epochs till end of Smelt emissions if flat emission is required 
    ...
    // function _updateOBolPrice() internal {
    //     try IOracle(oBolOracle).update() {} catch {}
    // }

    function getOBolCirculatingSupply() public view returns (uint256) {
        IERC20 oBolErc20 = IERC20(oBol);
        uint256 totalSupply = oBolErc20.totalSupply();
        uint256 balanceExcluded = 0;
        for (uint8 entryId = 0; entryId < excludedFromTotalSupply.length; ++entryId) {
            balanceExcluded = balanceExcluded.add(oBolErc20.balanceOf(excludedFromTotalSupply[entryId]));
        }
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        IBasisAsset(oBol).mint(address(this), _amount);
        ...
        if (protocolFundSharedPercent > 0) {
            ...
            IERC20(oBol).transfer(protocolFund, _protocolFundSharedAmount);
        ...
        if (teamFundSharedPercent > 0) {
            ...
            IERC20(oBol).transfer(teamFund, _teamFundSharedAmount);
        ...
        IERC20(oBol).safeApprove(boardroom, 0);
        IERC20(oBol).safeApprove(boardroom, _amount);
        IBoardroom(boardroom).allocateSeigniorage(_amount);
        emit BoardroomFunded(block.timestamp, _amount);
    }

    function _calculateMaxSupplyExpansionPercent(uint256 _oBolSupply) internal returns (uint256) {
        for (uint8 tierId = 20; tierId >= 0; --tierId) {
            if (_oBolSupply >= supplyTiers[tierId]) {
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        uint256 oBolSupply = getOBolCirculatingSupply();
        if (epoch < bootstrapEpochs) {
            // 30 first epochs with custom expansion
            _sendToBoardroom(oBolSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            uint256 _percentage = _calculateMaxSupplyExpansionPercent(oBolSupply).mul(1e14);
            _sendToBoardroom(oBolSupply.mul(_percentage));
                }
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(oBol), "oBol");
        require(address(_token) != address(smelt), "Smelt");
