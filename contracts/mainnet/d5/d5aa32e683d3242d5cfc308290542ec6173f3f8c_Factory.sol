...
interface ICollateral {
    ...
    function scorpionPriceOracle() external view returns (address);
...
contract Factory is IFactory {
    ...
    IScorpionPriceOracle public scorpionPriceOracle;
    ...
    constructor(
        ...
        IScorpionPriceOracle _scorpionPriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        scorpionPriceOracle = _scorpionPriceOracle;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "Scorpion: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "Scorpion: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "Scorpion: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "Scorpion: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) =
            scorpionPriceOracle.getPair(uniswapV2Pair);
        if (!oracleInitialized) scorpionPriceOracle.initialize(uniswapV2Pair);
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        require(msg.sender == admin, "Scorpion: UNAUTHORIZED");
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "Scorpion: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "Scorpion: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "Scorpion: UNAUTHORIZED");
