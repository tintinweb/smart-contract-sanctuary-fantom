...
contract CETUS is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 0 CETUS-WFTM LP -> CETUS pool
    uint256 public constant INITIAL_CETUS_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the CETUS ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("CETUS", "CETUS") {
        // Mints 1 CETUS to contract creator for initial pool setup
    ...
    function _getCETUSPrice() internal view returns (uint256 _CETUSPrice) {
        try IOracle(CETUSOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("CETUS: failed to fetch CETUS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _CETUSPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_CETUSPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCETUSOracle(address _CETUSOracle) public onlyOperatorOrTaxOffice {
        require(_CETUSOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CETUS to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CETUS to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCETUSPrice = _getCETUSPrice();
            currentTaxRate = _updateTaxRate(currentCETUSPrice);
            if (currentCETUSPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_CETUSPool != address(0), "!_CETUSPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_CETUSPool, INITIAL_CETUS_POOL_DISTRIBUTION);
