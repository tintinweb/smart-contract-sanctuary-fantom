...
░╚════╝░░╚════╝░╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝  ╚═════╝░╚═╝░░╚═╝░╚════╝░
    ...
    http://jokerdao.finance
    http://discord.gg/jokerdao
*/
contract Joker is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the Joker Pool rewards
    uint256 public constant INITIAL_JOKER_POOL_DISTRIBUTION = 70000 ether;
    ...
    /**
     * @notice Constructs the JOKER ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("JOKER", "JOKER") {
        // Mints 2600 JOKER to contract creator for initial pool setup and liquidity
    ...
    function _getJokerPrice() internal view returns (uint256 _jokerPrice) {
        try IOracle(jokerOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Joker: failed to fetch JOKER price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _jokerPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_jokerPrice >= taxTiersTwaps[tierId]) {
    ...
    function setJokerOracle(address _jokerOracle) public onlyOperatorOrTaxOffice {
        require(_jokerOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints JOKER to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of JOKER to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentJokerPrice = _getJokerPrice();
            currentTaxRate = _updateTaxRate(currentJokerPrice);
            if (currentJokerPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_jokerPool != address(0), "!_jokerPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_jokerPool, INITIAL_JOKER_POOL_DISTRIBUTION);
