...
contract HUNGERX is ERC20, ERC20Burnable, Ownable {
    
    address public marketingAddress;
    uint16[3] public marketingFees;

    address public liquidezAddress;
    uint16[3] public liquidezFees;
    ...
    uint256 private _liquidezPending;
    ...
    event liquidezAddressUpdated(address liquidezAddress);
    event liquidezFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event liquidezFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20("HUNGERX", "HUNX") 
    {
        address supplyRecipient = 0x0047B8037249F62db6cbB43cA6cE4569Ef90a974;
        
        marketingAddressSetup(0x0047B8037249F62db6cbB43cA6cE4569Ef90a974);
        marketingFeesSetup(100, 100, 10);

        liquidezAddressSetup(0x0047B8037249F62db6cbB43cA6cE4569Ef90a974);
        liquidezFeesSetup(200, 200, 20);

        autoBurnFeesSetup(100, 100, 30);
        ...
        _transferOwnership(0x0047B8037249F62db6cbB43cA6cE4569Ef90a974);
    }
    ...
    function marketingFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        ...
        totalFees[0] = 0 + marketingFees[0] + liquidezFees[0] + autoBurnFees[0] + liquidityFees[0];
        totalFees[1] = 0 + marketingFees[1] + liquidezFees[1] + autoBurnFees[1] + liquidityFees[1];
        totalFees[2] = 0 + marketingFees[2] + liquidezFees[2] + autoBurnFees[2] + liquidityFees[2];
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        emit marketingFeesUpdated(_buyFee, _sellFee, _transferFee);
    }

    function liquidezAddressSetup(address _newAddress) public onlyOwner {
        liquidezAddress = _newAddress;

        excludeFromFees(_newAddress, true);

        emit liquidezAddressUpdated(_newAddress);
    }

    function liquidezFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        liquidezFees = [_buyFee, _sellFee, _transferFee];
        ...
        emit liquidezFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                if (liquidezFees[txType] > 0) {
                    liquidezPortion = fees * liquidezFees[txType] / totalFees[txType];
                    _sendInTokens(from, liquidezAddress, liquidezPortion);
                    emit liquidezFeeSent(liquidezAddress, liquidezPortion);
                }
                ...
                fees = fees - marketingPortion - liquidezPortion - autoBurnPortion;
            }
