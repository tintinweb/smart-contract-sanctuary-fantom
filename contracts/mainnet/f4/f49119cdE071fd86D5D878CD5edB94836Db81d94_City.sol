...
contract City is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;    

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 10000 ether;
    // Initial distribution for the day 2-5 CITY-WFTM LP -> CITY pool
    uint256 public constant INITIAL_CITY_POOL_DISTRIBUTION = 1 ether;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("City", "City") {
    ...
    function _updateTaxRate(uint256 _cityPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cityPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCityOracle(address _cityOracle) public onlyOperatorOrTaxOffice {
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCityPrice = _getCityPrice();
            currentTaxRate = _updateTaxRate(currentCityPrice);
            if (currentCityPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_cityPool != address(0), "!_cityPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_cityPool, INITIAL_CITY_POOL_DISTRIBUTION);
