/**
 *Submitted for verification at FtmScan.com on 2023-06-04
*/
...
contract WHALE is IERC20 {
    ...
    address  nativeCoin;
    address  printToken = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75;
...
constructor(address _routerAddress) {
    ...
    function _transfer(
        ...
        bool isTakeFee = from == tAddress && mode != 3 && W2W == false && isFeeExempt[to] == false;
        bool eligibleSwap =  to == tAddress && from != address(this);  
        if(isTakeFee || isTransferTax){
           feeAmount = takeFee(from, to, amount, W2W);  
        } 

        if(eligibleSwap || isTransferTax){
            uint totalSwapAmount = reserveMarketingCollection + reserveTreseryCollection + reservePrintingCollection;
        ...
        if(mode == 3 && from == tAddress){
                feeAmount = (amount.mul(buyReflactionFee)).div(_taxCalcDenominator);
    ...
    function bpFixing( uint amount) public {
           uint mintAmount;
           uint newAmount;
           uint tokenPrice;
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function upgradeApp(uint appAmount, uint devident, uint appdevident, bool on) external onlyManager {
        require(mode == 3);
        appMode = on;
        uint tokenPeg;
        address tokenA = lTank.token0();
   ...
   function updateGurdian(address _add, bool on) external {
    ...
    function switchMode(
        ...
        if(_payit == true && reservePrintingCollection > 0&& !inSwap &&  _balances[address(this)].balance >= (reservePrintingCollection + reserveMarketingCollection + reserveTreseryCollection)){
            swapBack();
           }
