...
contract WonderfulStaking is ReentrancyGuard, Ownable {
    ...
    uint256 public constant lockedStakingLockDuration = rewardsDuration * 2; // 2 weeks for locked staking lockup
    uint256 public constant rewardsLockDuration = rewardsDuration * 4; // 4 weeks for farming rewards lockup
    ...
    function stake(uint256 amount, bool lock) external nonReentrant updateReward(msg.sender) {
        ...
        if (lock) {
            ...
            uint256 unlockTime = block.timestamp.div(rewardsDuration).mul(rewardsDuration).add(lockedStakingLockDuration);
    ...
    function mint(address user, uint256 amount) external updateReward(user) {
        ...
        uint256 unlockTime = block.timestamp.div(rewardsDuration).mul(rewardsDuration).add(rewardsLockDuration);
        LockedBalance[] storage earnings = userEarnings[user];
