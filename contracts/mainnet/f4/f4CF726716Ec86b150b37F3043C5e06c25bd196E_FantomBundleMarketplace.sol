...
// File contracts/interfaces/IBouncerKYC.sol
...
interface IBouncerKYC {
    ...
    function setStatusAndLevels(
        address user_,
        Statuses status_,
        VerifiedLevels[] memory levels_
    ) external;
...
// File contracts/Verifiable.sol

contract Verifiable {
    IBouncerKYC public kyc;

    modifier isVerified() {
        require(kyc.getStatus(msg.sender) == IBouncerKYC.Statuses.Verified, 'Verifiable: sender is not KYCed');
        _;
    }
}

// File contracts/interfaces/IFantomAddressRegistry.sol
...
// File contracts/interfaces/IFantomMarketplace.sol
...
// File contracts/interfaces/IFantomTokenRegistry.sol

interface IFantomTokenRegistry {
    function enabled(address) external returns (bool);
}

// File contracts/FantomBundleMarketplace.sol

contract FantomBundleMarketplace is Verifiable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Maximum size of an NFT bundle
    uint256 public constant MAX_BUNDLE_SIZE = 256;
    ...
    constructor(address _kyc) {
        require(_kyc != address(0), 'FantomBundleMarketplace: Zero address provided');
        kyc = IBouncerKYC(_kyc);
    }

    /// @notice Contract initializer
    function initialize(address payable _feeRecipient, uint256 _platformFee) public initializer {
        require(_feeRecipient != address(0), 'FantomBundleMarketplace: Zero address provided');
        require(_platformFee <= 1000, 'Incorrect platform fee');
    ...
    function listItem(
        ...
        require(_nftAddresses.length <= MAX_BUNDLE_SIZE, 'Max bundle size reached');
        require(
            owners[bundleID] == address(0) ||
                (owners[bundleID] == _msgSender() && listings[_msgSender()][bundleID].price == 0),
            'already listed'
        );
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) && IFantomTokenRegistry(tokenRegistry).enabled(_payToken)),
            'invalid pay token'
        );
        ...
        for (uint256 i; i < _nftAddresses.length; i++) {
            require(_nftAddresses[i] != address(0), 'FantomBundleMarketplace: Zero address provided in the list');
            ...
            require(bundleIdsPerItem[_nft][_tokenIds[i]].add(bundleID), 'Fail adding bundle ID');
    ...
    /// @param _bundleID Bundle ID
    function buyItem(string memory _bundleID) external isVerified nonReentrant {
        bytes32 bundleID = _getBundleID(_bundleID);
    ...
    function _buyItem(string memory _bundleID) private {
        bytes32 bundleID = _getBundleID(_bundleID);
        address owner = owners[bundleID];
        ...
        require(block.timestamp >= listing.startingTime, 'not buyable');

        uint256 price = listing.price;
        uint256 feeAmount = (price * platformFee) / 1e3;

        IERC20(listing.payToken).safeTransferFrom(_msgSender(), feeRecipient, feeAmount);
        IERC20(listing.payToken).safeTransferFrom(_msgSender(), owner, price - feeAmount);
        ...
        emit ItemSold(
            ...
            _bundleID,
            listing.payToken,
            IFantomMarketplace(addressRegistry.marketplace()).getPrice(listing.payToken),
            price
        );
    ...
    function createOffer(
        ...
        require(offer.deadline <= block.timestamp, 'offer exists');
    ...
    /// @param _bundleID Bundle ID
    function cancelOffer(string memory _bundleID) external isVerified {
        bytes32 bundleID = _getBundleID(_bundleID);
        Offer memory offer = offers[bundleID][_msgSender()];
        require(offer.deadline > block.timestamp, 'offer not exists or expired');
        delete (offers[bundleID][_msgSender()]);
        emit OfferCanceled(_msgSender(), _bundleID);
    }

    /// @notice Method for accepting the offer
    function acceptOffer(string memory _bundleID, address _creator) external isVerified nonReentrant {
        bytes32 bundleID = _getBundleID(_bundleID);
        ...
        offer.payToken.safeTransferFrom(_creator, _msgSender(), price - feeAmount);
    ...
    function updatePlatformFeeRecipient(address payable _platformFeeRecipient) external onlyOwner {
        require(_platformFeeRecipient != address(0), 'FantomBundleMarketplace: Zero address provided');
    ...
    function updateAddressRegistry(address _registry) external onlyOwner {
        require(_registry != address(0), 'FantomBundleMarketplace: Zero address provided');
    ...
    function validateItemSold(
        ...
        require(_nftAddress != address(0), 'FantomBundleMarketplace: Zero address provided');
