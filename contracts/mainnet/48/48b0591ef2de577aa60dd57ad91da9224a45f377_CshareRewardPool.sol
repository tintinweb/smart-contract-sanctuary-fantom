    ...
    ReentrancyGuard
{
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last block number that CSHARE distribution occurs.
        uint256 accSkyGPerShare; // Accumulated CSHARE per share, times 1e18. See below.
    ...
    function initialize(
        ...
        rewardPerSecond = TOTAL_REWARDS.div(DEFAULT_RUNNING_TIME); // =0.00079274479 CSHARE/sec = 50000 CSHARE / (2 years * 365 days * 24h * 60min * 60s)
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            accSkyGPerShare = accSkyGPerShare.add(
                _cshareReward.mul(1e18).div(lpSupply)
            );
        }
    ...
    // Update the given pool's CSHARE allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // Deposit LP tokens to MasterChef for CSHARE allocation.
    ...
    function _safeSkyGTransfer(address _to, uint256 _amount) internal {
        IERC20 _cshare = IERC20(cshare);
        uint256 _bal = _cshare.balanceOf(address(this));
        if (_amount > _bal) _amount = _bal;
        if (_amount > 0) {
            _cshare.transfer(_to, _amount);
        }
    }
