...
contract Deca711 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public requestToTimestamp;
    ...
    function depositHouse(uint256 _amount) external {
        ...
        for (uint256 i = 0; i < userHouseBalances.length; i++) {
            if (_sender == userHouseBalances[i].userAddress) {
    ...
    function withdrawHouse(uint256 _amount) external nonReentrant {
        ...
        for (uint256 i = 0; i < userHouseBalances.length; i++) {
            if (_sender == userHouseBalances[i].userAddress) {
                uint256 _depositAmount = userHouseBalances[i].depositAmount;
    ...
    //    function refund(uint256 _requestId, uint256 _amount) external onlyOwner nonReentrant {
    function refund(uint256 _requestId, uint256 _amount) external nonReentrant {
        uint256 _requestedTime = requestToTimestamp[_requestId];
        require(_requestedTime != 0, "requestedTime should exist");
        require(block.timestamp >= _requestedTime + 15 minutes, "15minutes should be passed");
        address _sender = msg.sender;
        delete requestToTimestamp[_requestId];
        rewardPool[_sender] += _amount;
    }
    ...
    function bet(uint256 _betAmount, uint256 _faceSide) external {
        ...
        requestToPossibleWin[requestId] = _betAmount * 2;
    ...
    function fulfillRandomWords(
        ...
        require(requestToTimestamp[requestId] != 0, "timestamp should exist");
        ...
        uint256 calculatedFee = playerBetAmount * betFeePercent / 1e18 / 100;
        ...
        delete requestToTimestamp[requestId];
