...
contract GasToken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 10000 ether;
    // Initial distribution for the day 2-5 GAS-WFTM LP -> Gas pool
    uint256 public constant INITIAL_GAS_POOL_DISTRIBUTION = 50000 ether;
    ...
    /**
     * @notice Constructs the Gas ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Gas Station", "GAS") {
        // Mints 1 GAS to contract creator for initial pool setup
    ...
    function _getGasPrice() internal view returns (uint256 _GasPrice) {
        try IOracle(GasOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Gas: failed to fetch Gas price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _GasPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_GasPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGasOracle(address _GasOracle) public onlyOperatorOrTaxOffice {
        require(_GasOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGasPrice = _getGasPrice();
            currentTaxRate = _updateTaxRate(currentGasPrice);
            if (currentGasPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_GasPool != address(0), "!_GasPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_GasPool, INITIAL_GAS_POOL_DISTRIBUTION);
    }
