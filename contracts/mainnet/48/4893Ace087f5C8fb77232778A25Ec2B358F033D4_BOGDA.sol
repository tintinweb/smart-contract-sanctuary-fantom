...
Dev greeting:
Hi folks, we love you <3 -- Good that you are doing research to our contracts to stay safe! We will renounce this contract on the first day of deployment.

*/

contract BOGDA is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;    

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 70000 ether;
    // Initial distribution for the day 2-5 BOGDA-WFTM LP -> BOGDA pool
    uint256 public constant INITIAL_BOGDA_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the BOGDA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("BOGDA", "BOGDA") {
        // Mints 1 BOGDA to contract creator for initial pool setup
    ...
    function _getBOGDAPrice() internal view returns (uint256 _BOGDAPrice) {
        try IOracle(BOGDAOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("BOGDA: failed to fetch BOGDA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _BOGDAPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_BOGDAPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBOGDAOracle(address _BOGDAOracle) public onlyOperatorOrTaxOffice {
        require(_BOGDAOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BOGDA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BOGDA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBOGDAPrice = _getBOGDAPrice();
            currentTaxRate = _updateTaxRate(currentBOGDAPrice);
            if (currentBOGDAPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_BOGDAPool != address(0), "!_BOGDAPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_BOGDAPool, INITIAL_BOGDA_POOL_DISTRIBUTION);
