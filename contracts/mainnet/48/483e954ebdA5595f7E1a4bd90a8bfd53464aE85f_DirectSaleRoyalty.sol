...
contract DirectSaleRoyalty is Ownable, Pausable {
  ...
  //Seller wallet => sale id array
  mapping(address => uint256[]) public marketplaceSellerIds;
  ...
  function setPrice(
    ...
    if (msg.sender != UncappedRoyaltyNFT(_nft).ownerOf(_tokenID)) revert CallerNotNFTOwnerOrTokenInvalid();
  ...
  function purchase(
    ...
    address nftOwner = UncappedRoyaltyNFT(_nft).ownerOf(_tokenID);
  ...
  function _purchase(
    ...
    uint256 royaltyFee = price * UncappedRoyaltyNFT(_nft).royaltiesFeeBP() / 10000;
    IERC20(purchaseToken).safeTransferFrom(_buyer, UncappedRoyaltyNFT(_nft).royaltiesCollector(), royaltyFee);
    IERC20(purchaseToken).safeTransferFrom(_buyer, _tokenOwner, price - royaltyFee);
    UncappedRoyaltyNFT(_nft).safeTransferFrom(_tokenOwner, _buyer, _tokenID);
