...
contract token is Context, IERC20, Ownable {
    ...
    string private _name = "testicletoken";
    string private _symbol = "TST";
    uint8 private _decimals = 9;
    
    uint256 private _taxFeeOnBuy = 5; // 5%
    uint256 private _taxFeeOnSell = 4; // 4%
    ...
    uint256 public _maxWalletSize = 10_000_000 * 10**9; // 1%
    ...
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        require(SwapThresholdAmount < 2_000_000, "Swap Threshold Amount cannot be more than 2 Million");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (contractTokenBalance > swapTokensAtAmount * 10) {
            contractTokenBalance = swapTokensAtAmount * 10; //max 0.1%
        }
        

        
        // //Slight Optimization. Making just 1 swap instead of 2.
        uint256 liquidityRatio = ((_liquidityFeeOnBuy+_liquidityFeeOnSell).mul(10**9)).div(_taxFeeOnBuy+_taxFeeOnSell+_liquidityFeeOnBuy+_liquidityFeeOnSell+_devFeeOnBuy+_devFeeOnSell); //Take approx 20% since 3% (1%+2%) fee of 14% => 3%/14% ~ 21%.
        uint256 devRatio = ((_devFeeOnBuy+_devFeeOnSell).mul(10**9)).div(_taxFeeOnBuy+_taxFeeOnSell+_liquidityFeeOnBuy+_liquidityFeeOnSell+_devFeeOnBuy+_devFeeOnSell); //Ratio should be roughly 10% since 2% (1%, 1%) => 2%/14% ~ 14%. 
        uint256 marketingRatio = 10**9 - liquidityRatio - devRatio;
        uint256 amountForLiquidity = contractTokenBalance.mul(liquidityRatio).div(10**9); 
        ...
        uint256 ethForDevRatio = (devRatio.mul(10**9)).div(marketingRatio + devRatio + halfLiquidityRatio);
        uint256 ethForMarketingRatio = (marketingRatio.mul(10**9)).div(marketingRatio + devRatio + halfLiquidityRatio);
        uint256 ethForDev = newBalance.mul(ethForDevRatio).div(10**9);
        uint256 ethForMarketing = newBalance.mul(ethForMarketingRatio).div(10**9);
