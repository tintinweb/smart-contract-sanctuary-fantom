...
import "./interfaces/ISkeletonV3Pool.sol";
...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    uint256 public latestPeriodRewardTokenPerSecond;
    ...
    /// @notice Record the rewardToken amount belong to MasterChefV3.
    uint256 public rewardTokenAmountBelongToMC;

    error ZeroAddress();
    error NotOwnerOrOperator();
    error NoBalance();
    error NotSkeletonNFT();
    error InvalidNFT();
    error NotOwner();
    error NoLiquidity();
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, ISkeletonV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    constructor(IERC20 _rewardToken, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        rewardToken = _rewardToken;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
    }

    /// @notice Returns the rewardToken per second , period end time.
    /// @param _pid The pool pid.
    /// @return rewardTokenPerSecond RewardToken reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 rewardTokenPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            rewardTokenPerSecond = (latestPeriodRewardTokenPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the rewardToken per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return rewardTokenPerSecond RewardToken reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 rewardTokenPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            rewardTokenPerSecond =
                (latestPeriodRewardTokenPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending rewardToken rewards.
    /// @dev The pending rewardToken amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingRewardToken(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (rewardToken.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    function add(uint256 _allocPoint, ISkeletonV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotSkeletonNFT();
    ...
    /// @notice harvest rewardToken from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward RewardToken reward.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce rewardTokenAmountBelongToMC.
        if (_token == address(rewardToken)) {
            unchecked {
                // In fact balance should always be greater than or equal to rewardTokenAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= rewardTokenAmountBelongToMC) {
                    balance -= rewardTokenAmountBelongToMC;
                } else {
                    // This should never happend.
                    rewardTokenAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        if (token == address(rewardToken)) {
            unchecked {
                // In fact balance should always be greater than or equal to rewardTokenAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= rewardTokenAmountBelongToMC) {
                    balanceToken -= rewardTokenAmountBelongToMC;
                } else {
                    // This should never happend.
                    rewardTokenAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of rewardToken injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer rewardToken token from receiver.
        rewardToken.safeTransferFrom(receiver, address(this), _amount);
        // Update rewardTokenAmountBelongToMC
        unchecked {
            rewardTokenAmountBelongToMC += _amount;
        }
        ...
        uint256 rewardTokenPerSecond;
        uint256 rewardTokenAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingRewardToken = ((latestPeriodEndTime - currentTime) * latestPeriodRewardTokenPerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingRewardToken);
            rewardTokenAmount += remainingRewardToken;
        }
        rewardTokenPerSecond = (rewardTokenAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodRewardTokenPerSecond = rewardTokenPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, rewardTokenPerSecond, rewardTokenAmount);
    }

    /// @notice Update rewardToken reward for all the liquidity mining pool.
    ...
    /// @notice Update rewardToken reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer rewardToken.
    /// @param _to The rewardToken receiver address.
    /// @param _amount Transfer rewardToken amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            // Update rewardTokenAmountBelongToMC
            unchecked {
                if (rewardTokenAmountBelongToMC >= _amount) {
                    rewardTokenAmountBelongToMC -= _amount;
                } else {
                    rewardTokenAmountBelongToMC = balance - _amount;
                }
            }
...
import "./ISkeletonV3Pool.sol";
import "./ILMPool.sol";

interface ILMPoolDeployer {
    function deploy(ISkeletonV3Pool pool) external returns (ILMPool lmPool);
}
