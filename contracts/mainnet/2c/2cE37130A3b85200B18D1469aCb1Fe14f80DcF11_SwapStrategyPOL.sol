...
contract SwapStrategyPOL is ISwapStrategy, Ownable {
    ...
    function execute(uint256 _wethIn, uint256 _yTokenOut) external override {
        ...
        addLiquidity(_yTokenReceived, _wethToSwap, swapSlippage);
    }
    ...
    function swap(uint256 _wethToSwap, uint256 _minYTokenOut) internal returns (uint256) {
        ...
        uint256[] memory _amounts = swapRouter.swapTokensForExactTokens(_wethToSwap, _minYTokenOut, swapPaths, address(this), block.timestamp);
        return _amounts[1];
    }
    ...
    function cleanDust() external onlyOwner {
        uint256 yTokenAmount = yToken.balanceOf(address(this));
        uint256 wethAmount = WethUtils.weth.balanceOf(address(this));
        addLiquidity(yTokenAmount, wethAmount, SLIPPAGE_PRECISION); // don't care about slippage
    }
