...
import "./PegOracle.sol";

contract RuniteOracle is PegOracle {
    constructor(IUniswapV2Pair pair_, address token_, uint256 period_, uint256 startTime_) PegOracle(pair_, token_, period_, startTime_) {}
}
...
contract Epoch is AccessControlEnumerable {
    ...
    constructor(uint256 period_, uint256 startTime_, uint256 startEpoch_) {
        _period = period_;
    ...
    modifier checkStartTime {
        if(block.timestamp < _startTime) revert EpochNotStarted();
        _;
    }
    ...
    modifier checkEpoch {
        ...
        if(block.timestamp < nextEpochPoint_) {
            ...
            _;
            for (;;) {
                _lastEpochTime = nextEpochPoint_;
                _epoch++;
                nextEpochPoint_ = nextEpochPoint();
                if(block.timestamp < nextEpochPoint_) break;
            }
        }
    }
 ...
 */
contract PegOracle is Epoch, IOracle, IUpdate {
    using FixedPoint for *;
    ...
    constructor(IUniswapV2Pair pair_, address token_, uint256 period_, uint256 startTime_) Epoch(period_, startTime_, 0) {
        pair = pair_;
        token = token_;
        token0 = pair.token0();
        token1 = pair.token1();
