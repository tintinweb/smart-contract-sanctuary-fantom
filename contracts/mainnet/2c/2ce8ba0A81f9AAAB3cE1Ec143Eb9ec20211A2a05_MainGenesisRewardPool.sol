...
contract MainGenesisRewardPool {
    ...
    address public parnerCollector;

    uint256 public mainPerSecond = 0.1736 ether; // 30000 MAIN / (24h * 60min * 60s)
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            pool.token.safeTransfer(parnerCollector, parnerfee);
    ...
    function setTexFee(address _address, address _parneraddress, uint _fee) external onlyOperator {
        require(_fee <= 100, "Fee can not be above 1%");
        taxCollector = _address;
        parnerCollector = _parneraddress;
        daoFee = _fee;
    }
}
