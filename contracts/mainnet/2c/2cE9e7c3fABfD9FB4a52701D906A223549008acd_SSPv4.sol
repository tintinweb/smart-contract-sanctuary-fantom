...
contract SSPv4 is AccessControl, Pausable {
    ...
    function buyDei(uint256 amountIn) external whenNotPaused {
        ...
        IDEIStablecoin(dei).pool_mint(msg.sender, deiAmount);
        emit Buy(amountIn);
    }

    function setBound(uint256 topBound_, uint256 lowerBound_) external {
    ...
    function withdrawERC20(address token, uint256 amount, address recv) external {
        require(hasRole(TRUSTY_ROLE, msg.sender), "Caller is not a trusty");
