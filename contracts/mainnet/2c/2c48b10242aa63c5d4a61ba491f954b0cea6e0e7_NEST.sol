...
contract NEST is ERC20Burnable, Operator {
    using SafeMath for uint256;

    // TOTAL MAX SUPPLY = 200,000
    // Team: 10% = 20000
    // DAO: 5% = 10000
    // Sub protocol: 40,000 each
    // Partner: 50,000 not minted yet
    ...
    function mint(address recipient, uint256 amountBN) public onlyOwner returns (bool) {
        uint256 balanceBefore = balanceOf(recipient);
        uint256 amount;
        amount = amountBN.mul(1e18);
     ...
     */
    function distributeReward(uint256 _farmStartTime, address _sharesRewardPool) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_sharesRewardPool != address(0), "!_sharesRewardPool");
        rewardPoolDistributed = true;
        startTime = _farmStartTime;
