...
contract PriceOracle is Governable {
    ...
    function calculateAssetPrice(address _asset) public view returns (uint256) {
        ...
        if(tokenType == AssetType.Token) {
            ...
            // Fetch rate.
            uint256 rate = IBalancerPool(_asset).getRate();
            ...
            uint256 price;
            if(isBoosted) {
                uint256 mainPrice = calculateAssetPrice(IBalancerPool(_asset).getMainToken());
                price = (rate * mainPrice) / 1e18;
            } else {
                bytes32 poolID = IBalancerPool(_asset).getPoolId();
                IBalancerVault vault = IBalancerVault(IBalancerPool(_asset).getVault());

                (IERC20[] memory tokens,,) = vault.getPoolTokens(poolID);
                uint256 mainPrice = calculateAssetPrice(address(tokens[0]));
            ...
            uint256 ratio = IVault(_asset).getPricePerFullShare();
            uint256 underlyingPrice = calculateAssetPrice(IVault(_asset).underlying());
            return (ratio * underlyingPrice) / 1e18;
        }
