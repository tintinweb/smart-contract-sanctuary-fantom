...
contract Operator is Initializable, Context, Ownable {
    ...
    function InitOperator() internal onlyInitializing {
...
contract EVIL is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 Dao-WETH LP -> Daopool
    uint256 public constant INITIAL_DAO_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the DAO ERC-20 contract.
     */

    function InitDaoToken(uint256 _taxRate, address _taxCollectorAddress)
        public
        initializer
    {
        ...
        InitOperator();
        __ERC20_init("Evil Token", "EVIL", 18);
    ...
    function _getDaoPrice() internal view returns (uint256 _DaoPrice) {
        try IOracle(Dao_Oracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("DAO: failed to fetch DAO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _DaoPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (
                ...
                if (_DaoPrice >= taxTiersTwaps[tierId]) {
                    require(
    ...
    function InitDaoOracle(address _DaoOracle) public onlyOperatorOrTaxOffice {
        require(_DaoOracle != address(0), "oracle address cannot be 0 address");
        Dao_Oracle = _DaoOracle;
    }
    ...
    /**
     * @notice Operator mints DAO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DAO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDaoPrice = _getDaoPrice();
            currentTaxRate = _updateTaxRate(currentDaoPrice);
            if (currentDaoPrice < burnThreshold) {
                burnTax = true;
            }
        }
     ...
     */
    function DistributeAirdropRewards(address _airdropRewardsAddress)
        external
        onlyOperator
    {
    ...
    function DistributeRewardsPoolGenesis(address _rewardsPoolAddress)
        external
        onlyOperator
    {
    ...
    function distributeRewardsLPPool(address _rewardsPoolLPAddress)
        external
        onlyOperator
    {
        require(_rewardsPoolLPAddress != address(0), "!_rewardsPoolLPAddress");
        rewardPoolDistributed = true;
        _mint(_rewardsPoolLPAddress, INITIAL_DAO_POOL_DISTRIBUTION);
    }
