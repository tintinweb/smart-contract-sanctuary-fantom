...
/**
 @notice Relies on external Oracles using any price quote methodology. compilethis
 */

contract CompositeOracle is OracleCommon {
    ...
    function amountRequired(address /* token */, uint256 amountUsd) external view override returns(uint256 amountTokens, uint256 volatility) {
        ...
        uint256 adjustedTokens =  tokenToUsd.mul(10**10);
        amountTokens = PRECISION.mul(amountUsd).div(adjustedTokens);
