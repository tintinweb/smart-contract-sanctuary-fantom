...
// File: contracts/interfaces/IPrimeStakeLottery.sol

pragma solidity ^0.8.4;

interface IPrimeStakeLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in STREAK per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in STEAK token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInSteak: price of a ticket in STEAK
...
// File: contracts/PrimeStakeLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title PrimeStake Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PrimeStakeLottery is ReentrancyGuard, IPrimeStakeLottery, Ownable {
    ...
    uint256 public maxPriceTicketInSteak = 50 ether;
    uint256 public minPriceTicketInSteak = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInSteak;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] steakPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInSteak;
        uint32 finalNumber;
    }
     ...
     * @param _steakTokenAddress: address of the STEAK token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _steakTokenAddress, address _randomGeneratorAddress) {
        steakToken = IERC20(_steakTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of STEAK to this contract
        uint256 amountSteakToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInSteak,
            _ticketNumbers.length
        );

        // Transfer STEAK tokens to this contract
        uint256 balanceBefore = steakToken.balanceOf(address(this));
        steakToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountSteakToTransfer
        );
        amountSteakToTransfer = steakToken.balanceOf(address(this)).sub(
            balanceBefore
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInSteak = _lotteries[_lotteryId]
            .amountCollectedInSteak
            .add(amountSteakToTransfer);
    ...
    function claimTickets(
        ...
        // Initializes the rewardInSteakToTransfer
        uint256 rewardInSteakToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInSteakToTransfer = rewardInSteakToTransfer.add(
                rewardForTicketId
            );
        }

        // Transfer money to msg.sender
        steakToken.safeTransfer(msg.sender, rewardInSteakToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInSteakToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in STEAK per group, and make lottery claimable
     * @param _lotteryId: lottery id
     * @param _autoInjection: reinjects funds into next lottery (vs. withdrawing all)
     * @dev Callable by operator
     */
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        // Calculate prizes in STEAK for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].steakPerBracket[j] =
                ...
                // A. No STEAK to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].steakPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury = amountToWithdrawToTreasury
            .add(_lotteries[_lotteryId].amountCollectedInSteak)
            .sub(amountToShareToWinners);

        // Transfer STEAK to treasury address
        if (amountToWithdrawToTreasury > 0) {
            steakToken.safeTransfer(
                treasuryAddress,
        ...
        onlyOwnerOrInjector
    {
        ...
        _amount = steakToken.balanceOf(address(this)).sub(balanceBefore);
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInSteak >= minPriceTicketInSteak) &&
                (_priceTicketInSteak <= maxPriceTicketInSteak),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInSteak: _priceTicketInSteak,
            ...
            amountCollectedInSteak: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(steakToken), "Cannot be STEAK token");
    ...
    /**
     * @notice Set STEAK price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInSteak: minimum price of a ticket in STEAK
     * @param _maxPriceTicketInSteak: maximum price of a ticket in STEAK
     */
    function setMinAndMaxTicketPriceInSteak(
        uint256 _minPriceTicketInSteak,
        uint256 _maxPriceTicketInSteak
    ) external onlyOwner {
        require(
            _minPriceTicketInSteak <= _maxPriceTicketInSteak,
            "minPrice must be < maxPrice"
        );

        minPriceTicketInSteak = _minPriceTicketInSteak;
        maxPriceTicketInSteak = _maxPriceTicketInSteak;
    }
     ...
     * @param _priceTicket price of a ticket (in STEAK)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].steakPerBracket[_bracket];
        } else {
            return 0;
        }
    }
