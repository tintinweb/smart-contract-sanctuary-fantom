 ...
 * out quoting reference 'NyPtvFantomUsdcOathSpookyV3VaultToOath'
 *
...
error NYProfitTakingVault__InvalidRewarder();
...
contract NYProfitTakingVaultBaseV3 is ERC20, Ownable, ReentrancyGuard {
  ...
  event RewarderAdded(address _rewarder);
  event RewarderRemoved();
  ...
  function getPricePerFullShare() public view returns (uint256) {
    return totalSupply() == 0 ? 10 ** UNDERLYING_DECIMALS : (balance() * 1e18) / totalSupply();
  }
  ...
  function deposit(uint256 _amount) public nonReentrant {
    ...
    uint256 newUnderlyingBalance = balance();
    if (newUnderlyingBalance > underlyingCap) {
  ...
  function depositProfitTokenForUsers(uint256 _amount) external nonReentrant {
    ...
    profitToken.safeTransferFrom(strategy, address(this), _amount);
  ...
  function getUserPendingEarnings(address _user) internal view returns (uint256) {
    ...
    uint256 pending = (userAmount * accProfitTokenPerShare) / PROFIT_TOKEN_PER_SHARE_PRECISION - user.rewardDebt;

    return pending;
  }

  function getUserUnderlyingBalance(address _user) public view returns (uint256) {
    uint256 userAmount = balanceOf(_user);
    if (userAmount == 0) {
      return 0;
    }
    uint256 userUnderlyingBalance = (userAmount * getPricePerFullShare()) / (10 ** UNDERLYING_DECIMALS);
  ...
  function setRewarder(IRewarder _rewarder) external onlyOwner {
    if (address(rewarder) != address(0)) {
      revert NYProfitTakingVault__InvalidRewarder();
    }
    if (_rewarder == rewarder) {
      return;
    }

    rewarder = _rewarder;

    emit RewarderAdded(address(_rewarder));
  }

  function removeRewarder() external onlyOwner {
    if (address(rewarder) == address(0)) {
      revert NYProfitTakingVault__InvalidRewarder();
    }
    rewarder = IRewarder(address(0));

    emit RewarderRemoved();
  }
}

contract NyPtvFantomUsdcOathSpookyV3VaultToOath is NYProfitTakingVaultBaseV3 {
  constructor(
    address _token,
    address _profitToken,
    string memory _name,
    string memory _symbol,
    uint256 _underlyingCap
  ) NYProfitTakingVaultBaseV3(_token, _profitToken, _name, _symbol, _underlyingCap) {}
}