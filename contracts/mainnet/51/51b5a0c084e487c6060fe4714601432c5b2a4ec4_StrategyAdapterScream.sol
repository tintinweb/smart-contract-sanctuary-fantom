...
abstract contract AlphaStrategyBase is OwnableUpgradeable {
    ...
    function pendingTokenOfUser(
        address user,
        address token,
        uint256 pending // 46869416048
    ) internal view returns (uint256) {
        uint256 totalSupply = IERC20Upgradeable(vault).totalSupply(); // 3093758
        uint256 userBalance = IERC20Upgradeable(vault).balanceOf(user); // 2718757
        if (totalSupply == 0) return 0;

        if (pending < lastPendingReward[token]) return 0;

        //45994086431 = 46869416048 - 875329617
        uint256 addedReward = pending - lastPendingReward[token];

        //  (15492708757390588212303796390203897628295 + 45994086431*1e36) / 3093758 = 1.4866742e40
        uint256 newAccPerShare = (accRewardPerShare[token] +
            addedReward *
            1e36) / totalSupply;

        // (2718757 * (15492708757390588212303796390203897628295 + 45994086431*1e36) / 3093758  / 1e36) - 49753626325 = 40419058800 - 49753626325
