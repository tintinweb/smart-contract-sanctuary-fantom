...
import "./SpiritV2VaultToken.sol";
import "./interfaces/ISpiritV2VaultTokenFactory.sol";

contract SpiritV2VaultTokenFactory is Ownable, ISpiritV2VaultTokenFactory {
        ...
        returns (address vaultToken)
    {
        ...
        require(!ISpiritV2BaseV1Pair(_underlying).stable(), "VaultTokenFactory: IS_STABLE");
        bytes memory bytecode = type(SpiritV2VaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        SpiritV2VaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            gaugeVaultProxy,
...
import "./interfaces/IGaugeVaultProxyAdmin.sol";
import "./interfaces/IOptiSwap.sol";
import "./interfaces/IBaseV1Router01.sol";
import "./interfaces/ISpiritV2BaseV1Pair.sol";
import "./interfaces/ISpiritV2VaultToken.sol";
...
contract SpiritV2VaultToken is ISpiritV2VaultToken, PoolToken {
    ...
    function _initialize(
        ...
        optiSwap = _optiSwap;
        gaugeVaultProxy = _gaugeVaultProxy;
        address gaugeVaultProxyAdmin = IGaugeVaultProxy(gaugeVaultProxy).admin();
        require(gaugeVaultProxyAdmin != address(0), "VaultToken: NO_GVP_ADMIN");
        require(IGaugeVaultProxyAdmin(gaugeVaultProxyAdmin).getGauge(underlying) != address(0), "VaultToken: NO_GAUGE");
        router = _router;
        pairFactory = _pairFactory;
        WETH = IBaseV1Router01(_router).wftm();
        (token0, token1) = ISpiritV2BaseV1Pair(_underlying).tokens();
        rewardsToken = _rewardsToken;
    }
    ...
    function _update() internal {
        IGaugeVaultProxyAdmin gvpAdmin = IGaugeVaultProxyAdmin(IGaugeVaultProxy(gaugeVaultProxy).admin());
        uint256 _totalBalance = gvpAdmin.gaugeBalanceOf(address(this));
    ...
    function mint(address minter) external nonReentrant update returns (uint256 mintTokens) {
        ...
        uint256 _totalBalanceBefore = gvpAdmin.gaugeBalanceOf(address(this));
        underlying.safeApprove(address(gvpAdmin), 0);
        underlying.safeApprove(address(gvpAdmin), mintAmount);
        gvpAdmin.depositInGauge(mintAmount);
        uint256 _totalBalanceAfter = gvpAdmin.gaugeBalanceOf(address(this));
    ...
    function redeem(address redeemer) external nonReentrant update returns (uint256 redeemAmount) {
        ...
        IGaugeVaultProxyAdmin gvpAdmin = IGaugeVaultProxyAdmin(IGaugeVaultProxy(gaugeVaultProxy).admin());
        gvpAdmin.withdrawFromGauge(redeemAmount);
    ...
    function _optimalDepositA(
        ...
        uint256 pairFee = ISpiritV2BaseV1Pair(underlying).fee();
    ...
    function reinvest() external nonReentrant update {
        ...
        IGaugeVaultProxyAdmin gvpAdmin = IGaugeVaultProxyAdmin(IGaugeVaultProxy(gaugeVaultProxy).admin());
        underlying.safeApprove(address(gvpAdmin), 0);
        underlying.safeApprove(address(gvpAdmin), liquidity);
        gvpAdmin.depositInGauge(liquidity);
    ...
    function observationLength() external view returns (uint) {
        return ISpiritV2BaseV1Pair(underlying).observationLength();
    }
    ...
    {
        return ISpiritV2BaseV1Pair(underlying).observations(index);
    }
    ...
    {
        return ISpiritV2BaseV1Pair(underlying).currentCumulativePrices();
    }
...
interface ISpiritV2VaultTokenFactory {
    ...
    function gaugeVaultProxy() external view returns (address);
...
interface IGaugeVaultProxyAdmin {
    function getGauge(address underlying) external view returns (address);

    function gaugeBalanceOf(address _vaultToken) external view returns (uint256);

    function depositInGauge(uint256 _amount) external;

    function withdrawFromGauge(uint256 _amount) external;

    function claimGaugeReward() external;
}
