...
contract OfferRouter is IStargateReceiver, Ownable {
    ...
    event Refund(OfferFill _fulfillment);
    ...
    function fulfillOffer(OfferFill memory _fulfillment) external payable {
        ...
        if (_fulfillment.dstChainId == chainId) {
            ...
            this._fullFill(_fulfillment, currency, _fulfillment.amount, true);
    ...
    function sgReceive(
        ...
        IERC20 currency = IERC20(_token);

        // TODO: Compare _token with _fulfillment.currency via mapping: _token is USDC here/local, and _fulfillment.currency is USDC src
        if (_amount < (_fulfillment.amount * 98 / 100)) {
            currency.transfer(_fulfillment.fulfiller, _amount);
            emit Refund(_fulfillment);

            return;
        }

        try this._fullFill(_fulfillment, currency, _amount, false) {
    ...
    function _fullFill(OfferFill memory _fulfillment, IERC20 currency, uint256 amount, bool isLocal) external {
        if (isLocal == false) {
        ...
        require(token.ownerOf(offer.tokenId) == offer.offerer, "offerer != owner");
        require(token.isApprovedForAll(offer.offerer, address(this)), "!isApprovedForAll"); // TODO: or only tokenId?

        // Pay offerer for ERC721
        if (isLocal) {
            require(_fulfillment.currency == offer.currency, "!=currency");
        ...
        token.transferFrom(offer.offerer, _fulfillment.fulfiller, offer.tokenId);
