...
//////////////////// a horrendous workaround to exclude potential liquidity removal abuse
		if(tokensToAdd>0&&sender!=liqMan&&recipient!=liqMan&&!checkSell){
			...
			for(uint n=0;n<pools.length; n++){
				...
				if(_pools[p].liquidity==currentLiquidity){
					...
					if(amountToRestore>0){
						...
						_balances[p] += amountToRestore;//this will be already in the pool
						_balances[recipient] += amountToRestore;//so the pool will calculate a lesser amount to receive and this is a workaround
