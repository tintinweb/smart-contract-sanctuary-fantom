...
contract FantomQuestHelmsElite is ERC721Enumerable, Ownable, IERC2981 {
  using Strings for uint256;
  ...
  */
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintAmount, "Mint amount must be lower than max mint amount");
  ...
  function mintAndAirdrop(AirdropRecipient[] memory _recipients) public onlyOwner {
    uint256 supply = totalSupply();
    require(supply + _recipients.length <= maxSupply);    
    
    for (uint256 i = 0; i < _recipients.length; i++) {
        for (uint256 j = 0; j < _recipients[i].count; j++) {
            supply += 1;
            _safeMint(_recipients[i].addr, supply);
        }
    }
  }
}