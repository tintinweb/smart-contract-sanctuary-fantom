...
contract SYRUP is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_TEAM_FUND_DISTRIBUTION = 0 ether;
    ...
    function _getSyrupPrice() internal view returns (uint256 _syrupPrice) {
        try IOracle(syrupOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Error: failed to fetch SYRUP price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _syrupPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_syrupPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSyrupOracle(address _syrupOracle) public onlyOperatorOrTaxOffice {
        require(_syrupOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSyrupPrice = _getSyrupPrice();
            currentTaxRate = _updateTaxRate(currentSyrupPrice);
            if (currentSyrupPrice < burnThreshold) {
                burnTax = true;
            }
        }
