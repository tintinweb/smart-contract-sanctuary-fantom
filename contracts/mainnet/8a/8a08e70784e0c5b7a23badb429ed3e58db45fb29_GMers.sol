    ...
    ClampedRandomizer
{
    ...
    event WithdrawFeesERC(address indexed devAddressERC, uint256 amount);
    ...
    address public royaltyAddress = 0x3e207060Ad54caab4A605B94e2DC8F184921a22d;
    ...
    uint256 public ercPrice = 50000000000000000000000;
    uint256 public price = 20000000000000000000;
    uint256 public burnBase = 10;
    ...
    mapping(address => DevFeeERC) public devFeesERC;
    address[] private devListERC;
    ...
    constructor(
        ...
        address[] memory _devListERC,
        uint256[] memory _feesERC
    ) ERC721("The GMers", "GMER") ClampedRandomizer(maxSupply) {
        ...
        _pause();
        for (uint8 i = 0; i < _devListERC.length; i++) {
        devListERC.push(_devListERC[i]);
        devFeesERC[_devListERC[i]] = DevFeeERC(_feesERC[i], 0);
        totalFee += _feesERC[i];
        }
    }
    ...
    function splitFees(uint256 sentAmount) internal {
        ...
        function splitERC(uint256 sentAmount) internal {
        for (uint8 i = 0; i < devListERC.length; i++) {
            address devAddressERC = devListERC[i];
            uint256 devFeeERC = devFeesERC[devAddressERC].percent;
            uint256 devFeeAmountERC = sentAmount.mul(devFeeERC).div(10000);
            devFeesERC[devAddressERC].amount += devFeeAmountERC;
        }
    }

    function erc20Mint(uint256 amount) public {
        uint256 totalAmount = ercPrice.mul(amount);
        ...
        if (msg.sender != owner()) {
        IERC20(erc20Token).transferFrom(msg.sender, address(this), ercPrice * amount);
        }
        ...
        splitERC(totalAmount);
    }

  function ftmMint(uint256 amount) public payable{
        uint256 supply = totalSupply();
        require(amount > 0 && amount <= maxPerTx,"Error: max par tx limit");
        require(balanceOf(msg.sender) + 1 <= maxPerPerson,"Error: max per address limit");
   ...
   function burnToMint(uint256[] memory tokenIds) public {
        ...
        /// @dev withdraw erc20 fees
    function withdrawERC20() external onlyDevs {
        uint256 amount = devFeesERC[msg.sender].amount;
        require(amount > 0, "Error: no fees :(");
        devFeesERC[msg.sender].amount = 0;

        erc20Token.transfer(msg.sender, amount);
        emit WithdrawFeesERC(msg.sender, amount);
    }
    ...
    function setMaxSupply(uint256 newMaxSupply) external onlyOwner {
        ...
        function setBurnPause(bool _burnPaused) external onlyOwner {
    ...
    function setPrice(uint256 newPrice) external onlyOwner {
        ...
        function setErcPrice(uint256 newErcPrice) external onlyOwner {
        ercPrice = newErcPrice;
    }
