...
contract PiswapToken is ERC20('Pi Swap Token', 'PI') {
    ...
    /// Must only be called by the owner or admins.
    function mintTo(address _to, uint256 _amount) public onlyOwnerOrAdmin {
        _mint(_to, _amount);
    }

    modifier onlyOwnerOrAdmin() {
        require(msg.sender == _owner || isAdmin(msg.sender), "Only owner or admin can call this function");
        _;
    }

    /// @notice Check if the given address is an admin
