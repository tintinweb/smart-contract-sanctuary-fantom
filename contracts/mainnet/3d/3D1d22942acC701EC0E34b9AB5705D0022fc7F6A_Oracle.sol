...
contract Oracle is AccessControl, IOracle {
    ...
    function setPrice(Signature memory signature) external {
        ...
        if (lastPriceUpdate > 0) {
            uint256 diff = price < signature.price
                ? (price * 1e18) / signature.price
                : (signature.price * 1e18) / price;
    ...
    function getPrice() external view returns (uint256) {
        ...
        uint256 dynamicRedeemerDeiPrice = IDynamicRedeemer(dynamicRedeemer)
            .usdRedeemPerDEI();
        if (dynamicRedeemerDeiPrice * 1e12 > price) {
            return dynamicRedeemerDeiPrice * 1e12;
        }
        return price;
    }
}
