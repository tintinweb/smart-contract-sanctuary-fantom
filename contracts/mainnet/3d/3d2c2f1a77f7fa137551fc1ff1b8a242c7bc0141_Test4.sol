...
contract Test4 is ERC20Detailed, Auth {
    ...
    constructor() ERC20Detailed("Test4", "TEST4", uint8(DECIMALS)) Auth(msg.sender) {
    ...
    function manualSync() public {
        for(uint i = 0; i < _markerPairs.length; i++){
            try InterfaceLP(_markerPairs[i]).sync() {

            }catch Error (string memory reason) {
                emit GenericErrorEvent("manualSync(): _markerPairs.sync() Failed");
    ...
    function swapBack() internal swapping {
        ...
        uint256 dynamicLiquidityFee = isLiquidityEnabled ? 0 : liquidityFee;
