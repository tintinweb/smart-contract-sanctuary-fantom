...
contract aFTMb_R5 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20Transparent, ERC165Upgradeable, IAnkrBond_R2 {
    ...
    uint64[9] public getRatioHistory;
    uint40 public lastRatioUpdate;
    ...
    function updateRatio(uint256 newRatio) public override onlyPoolOrOperator {
        ...
        _ratio = newRatio;
        // expected everyday ratio publish
        if (block.timestamp - lastRatioUpdate > 1 days - 15 minutes) {
            uint64 latestOffset = getRatioHistory[0]++;
            getRatioHistory[latestOffset % 8 + 1] = uint64(newRatio);
            lastRatioUpdate = uint40(block.timestamp);
        }
    ...
    returns (int256)
    {
        ...
        int256 oldestRatio = int256(
            // +7 is same thing as -1
            uint256(getRatioHistory[(((latestOffset - day) + 7) % 8) + 1])
        );
        int256 newestRatio = int256(
            uint256(getRatioHistory[(latestOffset + 7) % 8 + 1])
        );
