...
contract TTTTTTT is Ownable, TOKEN, ReentrancyGuard, BlackList {
    ...
    mapping (bytes32 => uint) public totalPoolBet;

    mapping (address => mapping (bytes32 => uint)) public betArray;

    event BuyToken(address indexed buyer, uint256 value);

    event SellToken(address indexed seller, uint256 value);

    event CreateBet(address indexed beter, bytes32 indexed pool, uint256 value);

    event WinBet(address indexed beter, bytes32 indexed pool, uint256 value);

    event NobodyWin(bytes32 indexed pool);
    ...
    function createBet(bytes32 pool_, uint amount_) external onlyIfResetNotInProgress {
    ...
    function winBet(bytes32 pool_) external onlyOwner {
    ...
    function setPool(bytes32 pool_) external onlyOwner{
        pools.push(pool_);
    }
    ...
    function getPools() external view returns (bytes32[] memory){
        return pools;
    }
