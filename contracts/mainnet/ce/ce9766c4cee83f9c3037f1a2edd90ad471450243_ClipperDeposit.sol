...
contract ClipperExchangeInterface is ReentrancyGuard, Ownable {
    ...
    constructor(ApprovalInterface initialApprovalContract, uint256 initialSwapFee, address _WETH) {
    ...
    function getSellQuote(address inputToken, address outputToken, uint256 sellAmount) external view returns (uint256 outputTokenAmount){
        ERC20 _input = ERC20(inputToken==MATCHA_ETH_SIGIL || inputToken == address(WETH) ? CLIPPER_ETH_SIGIL : inputToken);
        ERC20 _output = ERC20(outputToken==MATCHA_ETH_SIGIL || outputToken == address(WETH) ? CLIPPER_ETH_SIGIL : outputToken);
        (outputTokenAmount, ) = calculateSwapAmount(_input, _output, sellAmount+theExchange.lastBalance(_input));
    }
    receive() external payable {
        assert(msg.sender == address (theExchange));
        // only accept ETH via fallback from the theExchange contract
    }
    function sellTokenForToken(address inputToken, address outputToken, address recipient, uint256 minBuyAmount, bytes calldata auxiliaryData) external returns (uint256 boughtAmount) {
        IWETH _weth = WETH;
        // save gas
        if (inputToken == address(_weth)) {
            theExchange.unwrapWETH();
            boughtAmount = sellEthForToken(outputToken, recipient, minBuyAmount, auxiliaryData);
        } else if (outputToken == address(_weth)) {
            boughtAmount = sellTokenForEth(inputToken, payable(this), minBuyAmount, auxiliaryData);
            uint256 amount = address(this).balance;
            _weth.deposit{value: amount}();
            _weth.transfer(recipient, amount);
        } else {
    ...
    function sellEthForToken(address outputToken, address recipient, uint256 minBuyAmount, bytes calldata auxiliaryData) public payable returns (uint256 boughtAmount){
    ...
    function sellTokenForEth(address inputToken, address payable recipient, uint256 minBuyAmount, bytes calldata auxiliaryData) public returns (uint256 boughtAmount){
        ERC20 _input = ERC20(inputToken);
        uint256 inputTokenAmount = _input.balanceOf(address(theExchange));
...
contract ClipperPool is ERC20, ReentrancyGuard, Ownable {
    ...
    */
    ...
    function upsertAsset(ERC20 token, AggregatorV3Interface oracle, uint256 rawMarketShare) external onlyOwner {
        ...
        require(address(token) != address(exchangeInterfaceContract.WETH()), "Clipper: token must not be wrap native token");
    ...
    */
    ...
    function modifyExchangeInterfaceContract(address payable newContract) external onlyOwner {
    ...
    function unwrapWETH() external nonReentrant exchangeContractOnly returns (uint256) {
        IWETH weth = exchangeInterfaceContract.WETH();
