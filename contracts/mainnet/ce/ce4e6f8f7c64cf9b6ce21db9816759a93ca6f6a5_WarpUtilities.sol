...
contract WarpUtilities is IWarpUtilities, WarpBase, ERC1155SupplyUpgradeable {
    //** ===== VARIABLES ==== */
    string public name_;
    ...
    mapping(uint256 => WarpUtility) public utils;
    ...
    function addUtility(
        ...
        WarpUtility storage util = utils[counter];
        util.id = uint64(counter);
        util.metadataUri = _metadataUri;
        util.name = _name;
        util.mintPrice = _mintPrice;
        util.mintable = _mintable;
        if (_maxSupply == 0) _maxSupply = type(uint64).max;
        util.maxSupply = _maxSupply;
        util.transferable = _transferable;
    ...
    function editUtility(
        ...
        require(exists(_idx), 'EditWarpUtility: WarpUtility does not exist');
        utils[_idx].metadataUri = _metadataUri;
        utils[_idx].mintPrice = _mintPrice;
        utils[_idx].name = _name;
        utils[_idx].mintable = _mintable;
        if (_maxSupply == 0) _maxSupply = type(uint64).max;
        utils[_idx].maxSupply = _maxSupply;
        utils[_idx].transferable = _transferable;
    }
    ...
    function setWhitelist(address _whitelist, bool _isWL) external onlyOwner {
        whitelist[_whitelist] = _isWL;
    }

    /**
     * @notice make utility mintable
     */
    function updateMintable(uint256[] calldata _idx, bool[] calldata _mintable) external onlyOwner {
        for (uint256 i = 0; i < _idx.length; i++) {
            utils[_idx[i]].mintable = _mintable[i];
        }
    }

    /**
     * @notice make utility transferable or not
     */
    function updateTransferable(uint256[] calldata _idx, bool[] calldata _transferable)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _idx.length; i++) {
            utils[_idx[i]].transferable = _transferable[i];
        }
    }
    ...
    function mint(
        ...
        uint256 amount,
        address to
    ) external override whenNotPaused onlyCaller {
        require(exists(utilityIdx), 'Mint: WarpUtility does not exist');
        require(
            totalSupply(utilityIdx) + amount <= utils[utilityIdx].maxSupply,
    ...
    /** @dev purchase utils 
        @param utilityIds {uint256[]} id's to mint
        @param amounts {uint256[]} amounts to mint for each id
    */
    function purchase(uint256[] calldata utilityIds, uint256[] calldata amounts)
        external
        payable
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < utilityIds.length; i++) {
            uint256 id = utilityIds[i];
            uint256 amount = amounts[i];

            require(utils[id].mintable, 'WarpUtility is not mintable with ETH');
            require(totalSupply(id) + amount <= utils[id].maxSupply, 'Mint: Max supply reached');

            cost += utils[id].mintPrice * amount;

            _mint(msg.sender, id, amount, '');
        }

        require(msg.value >= cost, "Mint: Insufficient ETH amount, don't cheap out on us!");
    }
    ...
    /** @notice update growth */
    function setGrowth(address _growth) external onlyOwner {
        growth = _growth;
    }

    /**
     * @notice return total supply for all existing utils
     */
    ...
    /**
     *   @notice gets habitatutils
     */
    function getUtilities() external view returns (WarpUtility[] memory) {
        WarpUtility[] memory _utils = new WarpUtility[](counter);
    ...
    /** @notice get utility */
    function getUtility(uint256 _idx) external view override returns (WarpUtility memory) {
        return utils[_idx];
    }

    /** @notice get current idx */
    function utilityCount() external view override returns (uint256) {
        return counter;
    }
    ...
    function safeTransferFrom(
        ...
        require(utils[id].transferable || whitelist[msg.sender], 'Utility is not transferable');
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(utils[ids[i]].transferable, 'Utility is not transferable');
        }
