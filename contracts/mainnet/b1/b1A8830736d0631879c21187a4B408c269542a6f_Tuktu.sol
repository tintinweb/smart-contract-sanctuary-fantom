	...
	Ownable2Step
{
	...
	uint public num;
	uint public constant CFID = 9876543210;
	address public MultiSig;
	IXProgram public XPrograms;
	IMatrix public Matrixes;
	IAPI public APIs;
	mapping(uint => uint) private RT;
	mapping(uint => address) private AID;
	mapping(address => uint[]) private IDA;
	mapping(bytes32 => uint) private AAF;
	mapping(uint => bytes32) private AFA;
	...
	event ChangedAffiliate(
		uint indexed AccountID,
		string FromAffiliate,
		string ToAffiliate
	);
	...
	function Initializer() public virtual notAInitialized {
		XPrograms.Initializer(
			IAccount(address(this)),
			IBalance(address(this)),
			ITuktu(address(this)),
			IAPI(APIs)
		);
	...
	modifier onlyXProgramsOrMatrixes() {
		require(
			msg.sender == address(XPrograms) ||
				msg.sender == address(Matrixes),
	...
	modifier onlyAPI() {
		require(
			msg.sender == address(APIs),
			"caller is not the API"
		);
		_;
	}
	...
	modifier OnlyAccountOwner(uint _AID, address _Owner) {
		require(
			_isExisted(_AID) && _Owner == AddressOfAccount(_AID),
	...
	function isAInitialized()
		private
		returns (bool isInitialized_)
	{
	...
	function _AffiliateCreator(
		uint _AID
	) private view returns (bytes16 affiliate_) {
		while (true) {
	...
	function _ChangeAffiliate(
		...
		require(
			newaff != bytes32(0) &&
				AccountOfAffiliate(_Affiliate) == 0,
			"Affiliate: existed or empty"
		);
		string memory oldAff = AffiliateOfAccount(_AID);
		require(
			newaff != bytes32(bytes(oldAff)),
			"Affiliate: same already exists"
		);
		delete AAF[bytes32(bytes(oldAff))];
		AAF[newaff] = _AID;
		AFA[_AID] = newaff;
		if (_AID.v())
			emit ChangedAffiliate(_AID, oldAff, _Affiliate);
		return true;
	}
	...
	function _ChangeAddress(
		...
		require(
			_NewAddress != address(0) &&
				oldAddress != _NewAddress,
			"same already exists or zero"
		);
		AID[_AID] = _NewAddress;
		IDA[_Owner].RemoveValue(_AID);
		IDA[_NewAddress].AddNoDuplicate(_AID);
		if (_AID.v())
			emit ChangedAddress(_AID, oldAddress, _NewAddress);
		return true;
	}
...
abstract contract Balance is IBalance, Account {
	...
	mapping(uint => mapping(address => uint))
		private Balances;
	mapping(uint => uint[2]) private Locked;
	event ProfitGiven(
		uint indexed AccountID,
		uint Amount,
		uint FromAccountID
	);
	event WithdrawProfit(uint indexed AccountID, uint Amount);
	...
	function SupportedTokens()
		...
		pure
		returns (
			address[NumST] memory SupportedTokens_,
			uint8[NumST] memory Decimals_
		)
	{
		// FANTOM MAINNET
		SupportedTokens_ = [
		...
		Decimals_ = [18, 18, 18, 6, 6];
	}

	function SuportedUNIRouters()
		public
		pure
		returns (
			address[1] memory SuportedUNIRouters_,
			address[1] memory WETHs_
		)
	{
		SuportedUNIRouters_ = [
	...
	function SuportedUNIRouters(
		...
		public
		pure
		returns (address SuportedUNIRouter_, address WETH_)
	{
		(
			address[1] memory SuportedUNIRouters_,
			address[1] memory WrapETH_
		) = SuportedUNIRouters();
		(SuportedUNIRouter_, WETH_) = (
			SuportedUNIRouters_[_Index],
			WrapETH_[_Index]
		);
	}
		...
		returns (address SupportedToken, uint8 Decimal)
	{
		...
		for (uint i; i < NumST; ++i)
			if (SupportedTokens_[i] == _Token)
				return (SupportedTokens_[i], Decimals_[i]);
	}

	function isSupportedToken(
		address _Token
	) public pure returns (bool isSupportedToken_) {
		(
	...
	modifier VerifyBalance(uint _AID, uint _Amount) {
		require(
			_VerifyBalance(_AID, _Amount),
			"VerifyBalance: amount exceeds balance"
		);
		_;
	}
	...
	function _VerifyBalance(
		uint _AID,
		uint _Amount
	) private view returns (bool r_) {
		if (
			_Amount > 0 && AvailableToWithdrawn(_AID) >= _Amount
		) return true;
	}

	modifier VerifyContract(IERC20 _Token, uint _Amount) {
		require(
			_VerifyContract(_Token, _Amount),
			"VerifyContract: Insufficient balance"
		);
		_;
	}
	...
	function _TokenBalanceOf(
		uint _AID,
		address _Token
	) private view returns (uint balanceOf18_) {
		return Balances[_AID][_Token];
	}

	function TokenBalanceOf(
		uint _AID,
		IERC20 _Token
	) public view returns (uint balanceOf_) {
		(
			address SupportedToken,
			uint8 Decimal
		) = SupportedTokens(address(_Token));
	...
	function TotalBalanceOf(
		uint _AID
	) public view returns (uint TotalBalanceOf_) {
		(
			address[NumST] memory SupportedTokens_,

		) = SupportedTokens();
		for (uint i; i < NumST; ++i)
			TotalBalanceOf_ += _TokenBalanceOf(
				_AID,
				SupportedTokens_[i]
			);
	}
	...
	function LockedRecycleOf(
		uint _AID
	) public view returns (uint lockedR_) {
		return Locked[_AID][0];
	}

	function LockedUpgradeOf(
		uint _AID
	) public view returns (uint lockedU_) {
		return Locked[_AID][1];
	}

	function AvailableToWithdrawn(
		uint _AID
	) public view returns (uint availableToWithdrawn_) {
		uint locked = LockedRecycleOf(_AID) +
			LockedUpgradeOf(_AID);
		uint totalbalance = TotalBalanceOf(_AID);
		return
			totalbalance > locked ? totalbalance - locked : 0;
	}

	function AvailableToUpgrade(
		uint _AID
	) public view returns (uint availableToUpgrade_) {
		uint totalbalance = TotalBalanceOf(_AID);
		uint lockedrecycle = LockedRecycleOf(_AID);
		return
			totalbalance > lockedrecycle
				? totalbalance - lockedrecycle
				: 0;
	}
	...
	function _Locking(
		...
		uint _F,
		uint _Amount
	) external virtual onlyXProgramsOrMatrixes {
		Locked[_AID][_F] += _Amount;
	}
	...
	{
		require(
			_Amount > 0 && TotalBalanceOf(_FromAID) >= _Amount,
		...
		returns (bool success_)
	{
		...
		else if (_FromAID.v(_ToAID))
			emit ProfitGiven(_ToAID, _Amount, _FromAID);
	}
		...
		returns (bool success_)
	{
		...
		for (uint i; i < NumST; ++i) {
			...
			if (fBalance > 0) {
				...
				amount = Decimals_[i] < 18
					? amount18 / (10 ** (18 - Decimals_[i]))
					: amount18;
				if (
					_VerifyContract(
						IERC20(SupportedTokens_[i]),
						amount
					) &&
					_WithdrawToken(
						_AID,
					...
					IERC20(SupportedTokens_[i]).transfer(
						msg.sender,
						amount
					)
				)
		...
		returns (bool success_)
	{
		...
		uint amount18 = Decimal < 18
			? _Amount * (10 ** (18 - Decimal))
			: _Amount;
		if (
			_VerifyBalance(_FromAID, amount18) &&
			_TokenBalanceOf(_FromAID, SupportedToken) >= amount18
		)
			success_ = _TransferToken(
				_FromAID,
				_ToAID,
				SupportedToken,
				amount18
		...
		if (!success_)
			revert("Transfer token amount exceeds balance");
		else if (_FromAID.v(_ToAID))
			emit ProfitGiven(_ToAID, amount18, _FromAID);
	}
		...
		returns (bool success_)
	{
		...
		uint amount18 = Decimal < 18
			...
			: _Amount;
		if (
			_VerifyBalance(_AID, amount18) &&
			_TokenBalanceOf(_AID, SupportedToken) >= amount18
		)
			success_ = (_WithdrawToken(
				_AID,
				SupportedToken,
				amount18
			) && _Token.transfer(msg.sender, _Amount));
		if (!success_)
			revert("Withdraw token amount exceeds balance");
		...
		returns (bool success_)
	{
		if (msg.value > 0 && _Amount > 0) {
			(
				address UNIRouter,
				address WETH
			) = SuportedUNIRouters(address(_Token));

			address[] memory path = new address[](2);
			(path[0], path[1]) = (WETH, address(_Token));
			uint deadline = block.timestamp + 15;

			uint[] memory amounts = IUniswapV2Router02(UNIRouter)
	...
	{
		if (
			_Amount > 0 && _Token.balanceOf(msg.sender) >= _Amount
		) {
	...
	function governanceRecoverUnsupported(
		IERC20 _Token
	) public OnlyMultiSig returns (bool success_) {
		// do not allow to drain supported tokens: BUSD, USDT, USDC, DAI
		require(
			!isSupportedToken(address(_Token)),
			"can not drain supported tokens"
		);
		...
		if (address(this).balance > 0)
			(success_, ) = payable(MultiSig).call{
...
contract Tuktu is ITuktu, Balance {
	function PirceOfLevel(
		uint _Level
	) public pure returns (uint PirceOfLevel_) {
	...
	function isContract(
		address _address
	) internal view returns (bool _isContract) {
		if (_address.code.length > 0 || msg.sender != tx.origin)
			return true;
	}

	function PirceOfLevelOn(
		uint _LOn
	) public pure returns (uint PirceOfLevelOn_) {
		for (uint i = 1; i <= _LOn; ++i)
			PirceOfLevelOn_ += i > 2
				? PirceOfLevel(i) * 4
				: PirceOfLevel(i) * 5;
	}
	...
	function Register(
		...
		require(
			!isContract(_NA),
			"Registration: can not contract"
		);
		require(
			_isExisted(_UU) && _isExisted(_UB) && _isExisted(_UT),
		...
		if (!isSupportedToken(address(_Token)))
			_Token = IERC20(DefaultStableToken());
		if (_LOn < 1 || _LOn > 15) _LOn = 1;

		uint amount18 = PirceOfLevelOn(_LOn);
		(
			address SupportedToken,
			uint8 Decimal
		) = SupportedTokens(address(_Token));
		uint amount = Decimal < 18
			? amount18 / (10 ** (18 - Decimal))
			: amount18;
		msg.value > 0
	...
	function UpgradeLevelManually(
		...
		(uint amount18, uint LFrom, uint LTo) = PirceOfLevelUp(
			_AID,
			_XPro,
			_LTo
		);
		require(
			amount18 > 0 && LTo >= LFrom,
			"UpgradeLevelManually: level max"
		);
		if (!isSupportedToken(address(_Token)))
			_Token = IERC20(DefaultStableToken());
		...
		return
			XPrograms._UpgradeLevelManually(
				_AID,
				_XPro,
				LFrom,
				LTo
	...
	{
		if (_XPro == X3)
			if (!XPrograms.isLevelActivated(_AID, X3, 2))
				return (PirceOfLevel(2), 2, 2);
			else return (0, 0, 0);
		for (LFrom_ = 2; LFrom_ <= 15; ++LFrom_)
			if (!XPrograms.isLevelActivated(_AID, _XPro, LFrom_))
				break;
		if (LFrom_ == 15) return (PirceOfLevel(15), 15, 15);
		else if (LFrom_ > 15) return (0, 0, 0);
		(_LTo < 2 || _LTo > 15 || _LTo < LFrom_)
			? LTo_ = LFrom_
			: LTo_ = _LTo;
		if (LTo_ == LFrom_)
			return (PirceOfLevel(LFrom_), LFrom_, LTo_);
		else
			for (uint i = LFrom_; i <= LTo_; ++i)
				PirceOfLevelUp_ += PirceOfLevel(i);
	}
	...
	function AmountETHMin(
		...
		(address UNIRouter, address WETH) = SuportedUNIRouters(
			address(_Token)
		);
		address[] memory path = new address[](2);
		(path[0], path[1]) = (WETH, address(_Token));
		return
			IUniswapV2Router02(UNIRouter).getAmountsIn(
				_amountOut,
				path
			)[0];
	}
