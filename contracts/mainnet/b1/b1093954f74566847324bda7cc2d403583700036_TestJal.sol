...
contract TestJal is FTM20, Ownable {
    ...
    constructor() FTM20("TESTJa", "JALL") {
        creatorWallet = payable(address(0x10c4B4F052a0dD2cB6772554D41066cD8837A3Af));
        marketingWallet = payable(address(0xea7CcBF41E54eD6dfB9D773da622755DdCA2A76a));
        flashWallet = payable(address(0xf49312ce76f7163446A39dE42984693bF271ca21));
    }
    ...
    function sell(uint256 _amount) onlybelievers external {
        ...
        uint256 deductionAmount = sellDeduction(_amount,totalToken,ftmAmount);
        ...
        uint256 _updatedPayouts = (uint256) (profitPerShare_ * _amount + (ftmRemaining * magnitude));
    ...
    function sellDeduction(uint256 amount,uint256 _tokenAmount, uint256 _ftmAmount) internal pure returns(uint256){
        uint256 deductionAmount;
        if(_tokenAmount == 0){
            deductionAmount = 0;
        }
        else if(amount < _tokenAmount.mul(2).div(100)){
            deductionAmount = 0;
        }else if(amount >= _tokenAmount.mul(2).div(100) && amount < _tokenAmount.mul(5).div(100)){
            deductionAmount = _ftmAmount.mul(10).div(100);
        }else if(amount >= _tokenAmount.mul(5).div(100) && amount < _tokenAmount.mul(10).div(100)){
            deductionAmount = _ftmAmount.mul(10).div(100);
        }else if(amount >= _tokenAmount.mul(10).div(100) && amount < _tokenAmount.mul(25).div(100)){
            deductionAmount = _ftmAmount.mul(10).div(100);
        }else if(amount >= _tokenAmount.mul(25).div(100) && amount < _tokenAmount.mul(50).div(100)){
            deductionAmount = _ftmAmount.mul(25).div(100);
        }else if(amount >= _tokenAmount.mul(50).div(100) && amount < _tokenAmount.mul(100).div(100)){
            deductionAmount = _ftmAmount.mul(50).div(100);
        }else{
            deductionAmount = _ftmAmount.mul(50).div(100);
        }
    ...
    function tokenToFtmAfterFee(uint256 _tokens) public view returns(uint256){
        ...
        uint256 deductionAmount = sellDeduction(_tokens,totalToken,ftmAmount);
