...
// File @openzeppelin/contracts/access/Ownable.sol[email protected]
...
// File contracts/EqualFxsFraxvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualFxsFraxvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToFxsPath;
    IEqualizerRouter.Routes[] public fxsToFraxPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToFxsPath,
        IEqualizerRouter.Routes[] memory _fxsToFraxPath
        ...
        for (uint i; i < _equalToFxsPath.length; ++i) {
            equalToFxsPath.push(_equalToFxsPath[i]);
        }

        for (uint i; i < _fxsToFraxPath.length; ++i) {
            fxsToFraxPath.push(_fxsToFraxPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalBal, 0, equalToFxsPath, address(this), block.timestamp);

        uint256 fxsHalf = IERC20(fxs).balanceOf(address(this)) / 2;

        IEqualizerRouter(router).swapExactTokensForTokens(fxsHalf, 0, fxsToFraxPath, address(this), block.timestamp);

        uint256 lp0Bal = IERC20(fxs).balanceOf(address(this));
        uint256 lp1Bal = IERC20(frax).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(fxs, frax, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
