...
contract Thrifty is IERC20Metadata, Ownable {
  ...
  event tEvt(uint256 receiveAmount, uint256 reflectFee, uint256 rewardFee);
  function _transfer(
    address from,
    address to,
    uint256 amount
    ...
    emit tEvt(receiveAmount, reflectFee, rewardFee);
  }

  // event tEvt1(uint256[7] f);
  // function fTransfer1(address to, uint256 amount) public {
  //   address from = address(this);
  //   if (vipAddress[from] == true || vipAddress[to] == true) {
  //     emit tEvt(amount, 0, 0);
  //     return;
  //   }
  ...
  //   uint256[7] memory f = IThriftyRole(roleManager).getFee(totalSupply(), trader, amount, isTrade, isNoLimit);

  //   emit tEvt1(f);
  // }

  event tEvt2(uint256 a, address b, uint256 c, bool d, bool e);
  function fTransfer2(address to, uint256 amount) public {
    address from = address(this);
    bool isBuy = from == pairAddress && msg.sender == uniRouter;
    bool isSell = to == pairAddress && msg.sender == uniRouter;
    bool isTrade = isBuy || isSell;
    ...
    // receiveAmount, marketingFee, treasuryFee, reflectFee, printFee, rewardFee, extraFee
    emit tEvt2(totalSupply(), trader, amount, isTrade, isNoLimit);
  }
  ...
  //       if (reflectFee > 0 && IThriftyRole(roleManager).enableReflect() == true) {
...
interface IThriftyRole {
  ...
  function getFee(uint256 totalSupply, address trader, uint256 amount, bool isTrade, bool noLimitAdress)
