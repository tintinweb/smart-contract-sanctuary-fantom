...
interface ILnToken {
     ...
     */
    function sendToBank(address _from, address _to, uint256 _amount) external;
...
contract Bank is ReentrancyGuard, Ownable {
    ...
    function burnLnToken(uint256 _amount) public onlyOwner {
        lnToken.burn(address(this), _amount);

    }

    /**
     * @dev
     * transfer lnToken to another bank 
     */
    function bankTransfer(address _to, uint256 _amount) public onlyOwner {
        lnToken.sendToBank(address(this), _to, _amount);
    ...
    function borrow(
        ...
        // Charge fees from minting.
        uint256 mintableAmt = chargeFee(_amount);

        vault.transferFrom(msg.sender, address(this), _collateralAmt);
        lnToken.transfer(msg.sender, mintableAmt);
     ...
     */
    function chargeFee(uint256 _borrowAmt) internal returns (uint256 mintableAmt) {
        uint256 fee = _borrowAmt.mul(borrowFee).div(1000);
        lnToken.transfer(treasury, fee);
        mintableAmt = _borrowAmt.sub(fee);
    }
     ...
     * Repay all debt 
     */
    function repayAll() public nonReentrant {
