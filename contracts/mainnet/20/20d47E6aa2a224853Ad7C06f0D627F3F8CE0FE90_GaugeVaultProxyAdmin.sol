...
contract GaugeVaultProxyAdmin is Ownable, ReentrancyGuard {
    ...
    function migrateGauge(
        ...
        if (amount > 0) {
            ...
            gvpSafeTransfer(underlying, address(this), amount);
            underlying.safeApprove(_gaugeTo, 0);
            underlying.safeApprove(_gaugeTo, amount);
            IGauge(_gaugeTo).depositFor(amount, address(gaugeVaultProxy));
        }
    }
    ...
    function depositInGauge(uint256 _amount) external onlyVaultToken nonReentrant {
        ...
        gauge.depositFor(_amount, address(gaugeVaultProxy));
