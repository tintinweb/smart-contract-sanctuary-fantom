...
contract LaunchPool is Ownable {
    ...
    NRT public nrt;
    // Price Quote
    uint256 public priceQuotePrivate = 100000000000000;
    uint256 public priceQuotePublic = 66666666666666;
    // the total amount in stables to be raised
    uint256 public totalraiseCapPrivate;
    uint256 public totalraiseCapPublic;
    // how much was raised
    ...
    // start of the sale
    uint256 public startTimePrivate;
    uint256 public startTimePublic;
    // total duration
    uint256 public duration;
    // end of the sale
    uint256 public endTimePrivate;
    uint256 public endTimePublic;
    // sale has started
    ...
    constructor(
        ...
        uint256 _startTimePrivate,
        uint256 _startTimePublic,
        uint256 _duration,
        address _treasury
    ) {
        investToken = _investToken;
        startTimePrivate = _startTimePrivate;
        startTimePublic = _startTimePublic;
        duration = _duration;
        totalraiseCapPrivate = 650000000000;
        totalraiseCapPublic = 500000000000;
        maxinvestPrivate = 500000000;
        maxinvestPublic = 5000000000;
        treasury = _treasury;
        require(duration < 7 days, "duration too long");
        endTimePrivate = startTimePrivate + duration;
        endTimePublic = startTimePublic + duration;
        nrt = new NRT("aROO", 18);
        redeemEnabled = false;
        saleEnabled = false;
    }
    ...
    // private invest
    function investPrivate(uint256 investAmount) public {
        require(block.timestamp >= startTimePrivate, "not started yet");
        require(endTimePrivate >= block.timestamp, "sale has ended");
        require(saleEnabled, "not enabled yet");
        require(whitelisted[msg.sender] == true, 'msg.sender is not whitelisted');
        require(totalraised + investAmount <= totalraiseCapPrivate, "over total raise");
        require(investAmount >= 100000000, "below min invest");
        require(investAmount <= maxinvestPrivate, "above max invest");
        ...
        uint256 issueAmount = investAmount * priceQuotePrivate;
    ...
    // public invest
    function investPublic(uint256 investAmount) public {
        require(block.timestamp >= startTimePublic, "not started yet");
        require(endTimePublic >= block.timestamp, "sale has ended");
        require(saleEnabled, "not enabled yet");
        require(totalraised + investAmount <= totalraiseCapPrivate + totalraiseCapPublic, "over total raise");
        require(investAmount <= maxinvestPublic, "above max invest");
        ...
        uint256 issueAmount = investAmount * priceQuotePublic;
    ...
    function redeem() public {
        ...
        //require(block.timestamp > endTimePrivate, "not redeemable yet");
    ...
    function setstartTimePrivate(uint256 _startTimePrivate) public onlyOwner {
        require(block.timestamp <= startTimePrivate, "too late, sale has started");
        require(!saleEnabled, "sale has already started");
        startTimePrivate = _startTimePrivate;
        endTimePrivate = _startTimePrivate + duration;
    }

    function setstartTimePublic(uint256 _startTimePublic) public onlyOwner {
        require(block.timestamp <= startTimePublic, "too late, sale has started");
        require(!saleEnabled, "sale has already started");
        startTimePublic = _startTimePublic;
        endTimePublic = _startTimePublic + duration;
    }

    function setMaxPrivateBuyable(uint256 _amount) public onlyOwner {
        require(_amount <= 4000000000, "cannot exceeed $4000");
        maxinvestPrivate = _amount;   
    }
    ...
    function toggleSale() public onlyOwner {
        if(block.timestamp > endTimePrivate ) {
            saleEnabled = false;
