...
contract SwitchCelerDepositReceiver is Switch {
        ...
        if (m.bridgeToken != m.depositToken) {
            ...
            if (m.paraswapUsageStatus == DataTypes.ParaswapUsageStatus.OnDestChain ||
                ...
                ICallDataExecutor(callDataExecutor).execute(
                    ...
                    m.depositInfo.depositContract,
                    m.depositInfo.toApprovalAddress,
                    depositAmount,
                    m.depositInfo.toContractGasLimit,
                    m.depositInfo.depositCalldata
                );
            } else {
                uint256 dstAmount = _executeWithDistribution(m, _amount);
                require(dstAmount >= depositAmount, "failed swapping from dex");
                // deposit token to lending protocal
                _executeCallData(
                    m.depositToken,
                    m.depositInfo.depositContract,
            ...
            require(_amount >= depositAmount, "deposit balance is insufficient");
            _executeCallData(
                _token,
                m.depositInfo.depositContract,
                m.depositInfo.toApprovalAddress,
        ...
        returns (IMessageReceiverApp.ExecutionStatus)
    {
        ...
        switchEvent.emitCrosschainDepositRequest(
            ...
            m.bridge,
            m.recipient,
            m.depositInfo.depositContract, // contract address for deposit
            m.depositInfo.toApprovalAddress, // the approval address for deposit
            m.srcToken,
            m.depositToken,
            m.srcAmount,
    ...
    function _swapInternalForCeler(
        ...
        CelerDepositRequest memory m // callData
    )
        internal
        returns (
        ...
        )
    {
        ...
        if (returnAmount > 0) {
            IERC20(m.depositToken).universalTransfer(m.recipient, returnAmount);
    ...
    function _emitCrosschainDepositDone(
        ...
        internal
    {
        switchEvent.emitCrosschainDepositDone(
            m.id,
            m.bridge,
            m.recipient,
            m.depositInfo.depositContract,
