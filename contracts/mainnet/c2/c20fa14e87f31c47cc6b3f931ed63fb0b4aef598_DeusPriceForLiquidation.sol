...
contract DeusPriceForLiquidation {
    ...
    function deusPriceForLiquidation(address user) external view returns (uint256) {
        IPair pair = IPair(lpAddress);
        uint256 reserveDei;
        uint256 reserveDeus;
        uint256 userLiquidationPrice = ILender(lenderAddress).getLiquidationPrice(user);
        (reserveDei, reserveDeus,) = pair.getReserves();
        uint256 deiAmount = reserveDei - (userLiquidationPrice * pair.totalSupply() / 2);

        uint256 deusAmount = getAmountIn(deiAmount);
        uint256 deusPrice = (reserveDei - deiAmount) / (reserveDeus + deusAmount);
