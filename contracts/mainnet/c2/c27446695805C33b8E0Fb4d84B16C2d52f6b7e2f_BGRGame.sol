...
contract BGRGame is Ownable {
    ...
    function setWinningScore(
        string memory payoutName_,
        int16 aTeamScore_,
        int16 bTeamScore_
    ) external onlyOwner {
        uint8 payoutIndex = findPayoutIndex(payoutName_);
        require(
    ...
    function findPayoutIndex(string memory payoutName_)
        private
        view
        returns (uint8)
    {
        for (uint8 i = 0; i < payouts.length; i++) {
            if (
                keccak256(abi.encodePacked(payoutName_)) ==
                keccak256(abi.encodePacked(payouts[i].name))
            ) return i;
        }
