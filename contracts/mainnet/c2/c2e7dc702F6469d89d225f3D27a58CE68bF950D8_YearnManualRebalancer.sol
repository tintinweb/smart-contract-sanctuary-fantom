...
contract YearnManualRebalancer {
    ...
    function wrap(
        ...
        (IERC20 mainToken, IERC20 wrappedToken, IYearnTokenVault vaultToken) = _getLinearPoolTokens(linearPoolId);
        ...
        _setYearnVaultAllowancesIfNeeded(mainToken, wrappedToken);

        for (uint256 i = 0; i < numLoops; ++i) {
            // Deposit the tokens into the vault
            vaultToken.deposit(mainToken.balanceOf(address(this)), address(this));
    ...
    function unwrap(
        ...
        for (uint256 i = 0; i < numLoops; ++i) {
            ...
            vaultToken.withdraw(vaultToken.balanceOf(address(this)), address(this));            
        }
    ...
    function _getLinearPoolTokens(
        bytes32 linearPoolId
    ) internal view returns (IERC20 mainToken, IERC20 wrappedToken, IYearnTokenVault vaultToken) {
        ...
        vaultToken = IYearnTokenVault(address(wrappedToken));
    }

    function _setYearnVaultAllowancesIfNeeded(
        IERC20 mainToken,
        IERC20 wrappedToken
    ) internal {
