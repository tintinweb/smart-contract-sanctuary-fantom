...
import "../../apis/spookyswap/ISpookyPair.sol";
...
contract SpookySwapRestrictedStrategyAddBaseTokenOnly is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IStrategy {
  ...
  modifier onlyWhitelistedWorkers() {
    require(
      ...
      "SpookySwapRestrictedStrategyAddBaseTokenOnly::onlyWhitelistedWorkers:: bad worker"
    );
    _;
  }
  ...
  function initialize(ISpookyRouter02 _router) external initializer {
  ...
  function execute(
    ...
    ISpookyPair lpToken = ISpookyPair(factory.getPair(farmingToken, baseToken));
    ...
    // Constants come from
    // 2-f = 2-0.002 = 19980
    // 4(1-f) = 4*9980*10000 = 399200000, where f = 0.002 and 10,000 is a way to avoid floating point
    // 19980^2 = 399200400
    // 9980*2 = 19960
    uint256 aIn = KoalaMath.sqrt(rIn.mul(balance.mul(399200000).add(rIn.mul(399200400)))).sub(rIn.mul(19980)) / 19960;
    ...
    require(
      ...
      "SpookySwapRestrictedStrategyAddBaseTokenOnly::execute:: insufficient LP tokens received"
    );
    require(
      lpToken.transfer(msg.sender, lpToken.balanceOf(address(this))),
      "SpookySwapRestrictedStrategyAddBaseTokenOnly::execute:: failed to transfer LP token to msg.sender"
    );
