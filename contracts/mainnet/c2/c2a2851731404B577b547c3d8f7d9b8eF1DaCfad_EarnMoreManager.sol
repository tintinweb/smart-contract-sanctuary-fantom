...
contract EarnMoreManager is Ownable, IEarnMoreManager {
    ...
    modifier isWhitelistedEarnMore() {
    	require(whitelistedEarnMore[msg.sender], "EarnMore contract is not whitelisted");
    	_;
    }
    ...
    constructor(
    	...
    	earnMoreImplementation = _earnMoreImplementation;
    }
	...
	function addEarnMore(EarnMoreParams calldata earnMoreParams) external onlyOwner {
        // console.log("addEarnMore: start");
		require(vaultToEarnMore[earnMoreParams.vault] == address(0), "EarnMore for this address already exists");
        // console.log("addEarnMore: deploy new contract");
		TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(earnMoreImplementation, proxyAdmin, _buildInitCalldata(earnMoreParams)); // Unwrap params

        // console.log("addEarnMore: fill params");
		vaultToEarnMore[earnMoreParams.vault] = address(proxy);
		whitelistedEarnMore[address(proxy)] = true;
	}

	function transferReward(address to, uint256 amount) external isWhitelistedEarnMore returns(bool) {
    ...
    function _buildInitCalldata(EarnMoreParams calldata earnMoreParams) internal returns(bytes memory) {
        return abi.encodeWithSignature("initialize(address,address,address,address,address)", 
