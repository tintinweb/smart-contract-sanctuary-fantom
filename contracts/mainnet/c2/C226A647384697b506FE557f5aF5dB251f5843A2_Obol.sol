...
/*
__________                              / /  
\______   \_____     ______  ____    __| |   
 |    |  _/\__  \   /  ___/_/ __ \  / __ |     
 |    |   \ / __ \_ \___ \ \  ___/ / /_/ |     
 |______  /(____  //____  > \___  >\____ |     
                                                                             
*/
contract Obol is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 240h genesis pools
    // total of obol we pay to users during genesis
    ...
    /**
     * @notice Constructs the OBOL ERC-20 contract.
     */
     // =================================DO WE WANT TO SET DEPLOYER AS MINTOPERATOR IN CONSTRUCTOR RIGHT AWAY??????
    constructor() ERC20("OBTest", "OBTest") {
        // Mints 10000 OBOL to contract creator for initial LP setup and OTC
...
// =============do we need this ? if yes, how we going to use it w Oracle in the future?
    function _getObolPrice() internal view returns (uint256 _obolPrice) {
        try IOracle(obolOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Obol: failed to fetch OBOL price");
        }
    }

// =============do we need this ? if yes, how we going to use it w Oracle in the future?
    function setObolOracle(address _obolOracle) public onlyOperator {
        require(_obolOracle != address(0), "oracle cannot be 0");
        obolOracle = _obolOracle;
    }

    /**
     * @notice Operator mints OBOL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of OBOL to mint to
