...
// File contracts/MmyFtmEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract MmyFtmEqualizerV2  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToMmyPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToMmyPath,
        ...
        for (uint i; i < _equalToMmyPath.length; ++i) {
            equalToMmyPath.push(_equalToMmyPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToMmyPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wftm).balanceOf(address(this));
        uint256 t2Bal = IERC20(mmy).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(wftm, mmy, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(mmy).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(mmy).safeApprove(router, type(uint).max);
    }
