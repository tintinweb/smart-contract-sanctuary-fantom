...
contract RVault {
    ...
    ///@dev sends the request on the destination chain
    function earn(
        uint8 _chainId,
        bytes32 resourceID,
        bytes calldata data,
    ...
    ///@dev Used at the time of smart service
    function depositService(uint256 _amount, uint8 _chainId, address user) public {
        address _reserve = reserves[_chainId];
        uint256 _before = token.balanceOf(_reserve);
        token.safeTransferFrom(msg.sender, _reserve, _amount);
        uint256 _after = token.balanceOf(_reserve);
        _amount = _after.sub(_before); // Additional check for deflationary tokens
        IReserve(_reserve).depositService(_amount, user);
    }
    

    /// @dev Sends withdraw request to corresponding reserve
    function withdraw(
        uint256 _shares,
        uint8 _chainId,
        bytes32 resourceId,
    ...
    /// @dev for use by smart service to operate on user funds
    function withdrawService(
        ...
        address reserve = reserves[_chainId];
        IReserve(reserve).withdrawService(
            _shares,
            resourceId,
            data,
            feeTokenAddress,
...
interface IReserve{
    ...
    function depositService(uint256, address) external;
    function totalSupply() external view returns (uint amount);
    function withdraw(uint256, bytes32, bytes calldata, address) external;
    function withdrawService(uint256, bytes32, bytes calldata, address, address) external;
