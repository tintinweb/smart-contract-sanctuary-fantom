...
import "./interfaces/IRainSpiritStrategy.sol";
import "./interfaces/IRainSpirit.sol";

contract rainSpiritManager {
    ...
    address public rainSpirit;
    uint256 public incentiveSpirit = 0;
    uint256 public unlockTime;

    constructor(address _strategy, address _rainSpirit) public {
        strategy = _strategy;
        rainSpirit = _rainSpirit;
        feeManager = msg.sender;
    }

    function transferFeeManager(address newFeeManager) external {
    ...
    function initialLock() public {
        ...
        IRainSpiritStrategy(strategy).release();
        //create new lock
        uint256 spiritBalanceStrategy = IERC20(spirit).balanceOf(strategy);
        IRainSpiritStrategy(strategy).createLock(spiritBalanceStrategy, unlockAt);
    }

    function getUnlockTime() external view returns (uint256) {
        return IRainSpiritStrategy(strategy).getUnlockTime();
    }
    ...
    function _lockMoreSpirit(uint256 _amount) internal {
        ...
        uint256 _strategyInSpiritBalance = IRainSpiritStrategy(strategy).balanceOfInSpirit();

        if (_strategyInSpiritBalance > 0) {
            //increase amount
            IRainSpiritStrategy(strategy).increaseAmount(_amount);
        } else {
            initialLock();
        }
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        IRainSpirit(rainSpirit).mint(msg.sender, _amount);
    }
