...
contract INSURANCE {
    ...
    function initiate() public {
        ...
        if(balance==0) return;
        iToken.safeTransfer(MAINCONTRACT, balance);
    }
...
contract ALGBAR {
    ...
    uint[] public                   REFERRAL_PERCENTS               = [70, 30, 20, 15, 10]; // 7% 3% 2% 1.5% 1% (14.5%)
    uint constant public            PROJECT_FEE                     = 180;                  // project fee 18% of deposit
    uint constant public            DEV_FEE                         = 20;                   // dev fee 2% of deposit
    ...
    constructor(address payable _walletProject, address payable _walletMarketing, address _erc20Token) {
        INSURANCE_CONTRACT = payable(new INSURANCE(_erc20Token));
        WALLET_PROJECT = _walletProject;
        WALLET_DEV = _walletMarketing;
    ...
    function claim() public {
        ...
        token.safeTransfer(WALLET_PROJECT, reinvestAmount);
        emit ProjectFeePaid(reinvestAmount);
