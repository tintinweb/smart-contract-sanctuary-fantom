...
https://punpalace.finance

*/

contract Punpalace is ERC20Burnable, Operator {
    ...
    address public punpalaceOracle;
    ...
    /**
     * @notice Constructs the PUNPALACE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Punpalace", "Punpalace") {
        // Mints 1 PUNPALACE to contract creator for initial pool setup
    ...
    function _getPunpalacePrice() internal view returns (uint256 _punpalacePrice) {
        try IOracle(punpalaceOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Punpalace: failed to fetch PUNPALACE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _punpalacePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_punpalacePrice >= taxTiersTwaps[tierId]) {
    ...
    function setPunpalaceOracle(address _punpalaceOracle) public onlyOperatorOrTaxOffice {
        require(_punpalaceOracle != address(0), "oracle address cannot be 0 address");
        punpalaceOracle = _punpalaceOracle;
    }
    ...
    /**
     * @notice Operator mints PUNPALACE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PUNPALACE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPunpalacePrice = _getPunpalacePrice();
            currentTaxRate = _updateTaxRate(currentPunpalacePrice);
            if (currentPunpalacePrice < burnThreshold) {
                burnTax = true;
            }
        }
