...
interface IOxLens {
    function userProxyByAccount(address)
        external
        view
        returns (address);
}
...
contract VlOxdSnapshot {
    ...
    address public constant oxdAddress =
    ...
    IERC20 private oxd = IERC20(oxdAddress);
    ...
    function balanceOf(address accountAddress) external view returns (uint256) {
        ...
        return vlOxd.lockedBalanceOf(userProxyAddress) + oxd.balanceOf(accountAddress);
    }
}