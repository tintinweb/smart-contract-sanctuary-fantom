...
// File: Zeus Finance/lib/SafeMath8.sol
...
// File: Zeus Finance/ERC20/ERC20.sol
...
// File: Zeus Finance/ERC20/ERC20Burnable.sol
...
// File: Zeus Finance/ZeusToken.sol
...
contract ZeusToken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first week -> Zeus reward pool
    uint256 public constant INITIAL_ZEUS_POOL_DISTRIBUTION = 24500 ether;
    ...
    /**
     * @notice Constructs the ZEUS ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Zeus Token", "ZEUS") {
        // Mints 1,000 ZEUS to contract creator for initial pool setup & LP
    ...
    function _getZeusPrice() internal view returns (uint256 _zeusPrice) {
        try IOracle(zeusOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Zeus: failed to fetch ZEUS price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _zeusPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_zeusPrice >= taxTiersTwaps[tierId]) {
    ...
    function setZeusOracle(address _zeusOracle) public onlyOperatorOrTaxOffice {
        require(_zeusOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ZEUS to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ZEUS to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentZeusPrice = _getZeusPrice();
            currentTaxRate = _updateTaxRate(currentZeusPrice);
            if (currentZeusPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_zeusPool != address(0), "!_zeusPool");
        rewardPoolDistributed = true;
        _mint(_zeusPool, INITIAL_ZEUS_POOL_DISTRIBUTION);
    }
