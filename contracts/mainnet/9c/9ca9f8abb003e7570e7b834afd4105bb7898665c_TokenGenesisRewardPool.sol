...
contract TokenGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint daoFee; // Withdrawl fee for this token
    ...
    address public daoAddress4;
    address public daoAddress5;

    // TESTNET
    //uint256 public tokenPerSecond = 5.5555555 ether; // 20000 TOKEN / (1h * 60min * 60s)
    //uint256 public runningTime = 1 hours; // 1 hours
    //uint256 public constant TOTAL_REWARDS = 20000 ether;
    // END TESTNET

    // MAINNET
     uint256 public tokenPerSecond = 0.14467592592 ether; // 25000 TOKEN / (2 Days * 24h * 60min * 60s)
    ...
    constructor(
        ...
        address _daoAddress4,
        address _daoAddress5
    ) public {
        ...
        daoAddress4 = _daoAddress4;
        daoAddress5 = _daoAddress5;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            uint _fee = _amount.mul( pool.daoFee ).div( 10000 );
            uint256 _amountSubFees = _amount.sub(_fee);
            user.amount = user.amount.sub(_amount);
            pool.token.safeTransfer(_sender, _amountSubFees);
            pool.token.safeTransfer(daoAddress1, _fee.div(5));
            pool.token.safeTransfer(daoAddress2, _fee.div(5));
            pool.token.safeTransfer(daoAddress3, _fee.div(5));
            pool.token.safeTransfer(daoAddress4, _fee.div(5));
            pool.token.safeTransfer(daoAddress5, _fee.div(5));
        }
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e18);
