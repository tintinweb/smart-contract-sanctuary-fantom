...
contract ZappOut is Ownable, IZapper {
    ...
    function _estimateSwap(address _in, uint256 amount, address out, address routerAddr) public view returns (uint256) {
        return 0;
    }
    function zapInToken(address _in, uint256 amount, address out, address routerAddr, address recipient, uint256 minAmountLp) external {
        // From an ERC20 to an LP token, through specified router, going through base asset if necessary
        IERC20(_in).safeTransferFrom(msg.sender, treasury, amount);
