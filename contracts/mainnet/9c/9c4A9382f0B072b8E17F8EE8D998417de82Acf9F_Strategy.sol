...
contract Strategy is BaseStrategy {
    ...
    address public router = address(0xa38cd27185a464914D3046f0AB9d43356B34829D); 
    address public sellRouter = address(0xa38cd27185a464914D3046f0AB9d43356B34829D); 
   
    uint8 public sellRouterSelected = 1;

    ILpDepositor public masterChef;

    address public wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public solidReward = address(0x888EF71766ca594DED1F0FA3AE64eD2941740A20); 
    address public sexReward = address(0xD31Fcd1f7Ba190dBc75354046F6024A9b86014d7); 

    address public DEI;
    IV1Pair public solidlyLp;
    ISolidexLP public solidexLP;

    uint256 public dust = 1e6;
    uint256 public rewardDust = 1e18;

    bool public simpleAccounting = false;
    bool public collectFeesEnabled = false;

    uint256 public swapSecurityFactor = 9800;
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('Solidex DEI-USDC Strategy'));
    }
    ...
    function balanceOfPairToken() public view returns (uint256) {
        return IERC20(DEI).balanceOf(address(this));
    }
    ...
    function balanceOfLPInMasterChef() public view returns (uint256 _amount) {
        _amount = masterChef.userBalances(address(this), address(solidlyLp));
    }
    ...
    function estimateHarvest() public view returns (uint256 profitInUsdc) {
        (uint256 _solidAmount, uint256 _sexAmount) = pendingRewards();

        (uint256 fromSolid, ) = ISolRouter(router).getAmountOut(_solidAmount, solidReward, wftm);
        uint256 profitInWftm = fromSolid;

        (uint256 fromSex, ) = ISolRouter(router).getAmountOut(_sexAmount, sexReward, wftm);
        profitInWftm += fromSex;

        (profitInUsdc, ) = ISolRouter(router).getAmountOut(profitInWftm, wftm, address(want));
    }

    function balanceOfLpComponents(uint256 liquidity)
        public
        view
        returns (uint256 amountWant, uint256 amountPairToken)
    {
        (amountWant, amountPairToken) = ISolRouter(router).quoteRemoveLiquidity(
            address(want),
            address(DEI),
            true,
            liquidity
    ...
    function balanceOfPooled() public view returns (uint256 _amount) {
        ...
        (uint256 amountWant, uint256 amountPairTokenInLp) = balanceOfLpComponents(lpTokens);
        uint256 totalPairToken = amountPairTokenInLp.add(balanceOfPairToken());
        uint256 pairTokenInWant = lpToken1PerWant(totalPairToken);
        return pairTokenInWant.add(amountWant);
    }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > looseAmount) {
            ...
            _removeLiquidity(wantToLpTokens(toExitAmount));
            _sellAllLpToken1();
            depositLpIntoMasterChef();
    ...
    function lpToken1PerWant(uint256 _amount) internal view returns (uint256) {
        ...
        return solidlyLp.getAmountOut(_amount, DEI);
    }

    function quoteLiquidity(uint256 _amountA) internal view returns (uint256) {
        (uint256 reserveA, uint256 reserveB, ) = solidlyLp.getReserves();
        return _amountA.mul(reserveB).div(reserveA);
    }
    ...
    function getSwapAmount(uint256 investmentA) internal view returns (uint256 _swapAmount) {
        ...
        uint256 denominator = quoteLiquidity(halfInvestment);
    ...
    function _swap(uint256 _fullInvestment) internal returns (uint256[] memory _swapedAmounts) {
        ...
        (, bool isStable) = ISolRouter(router).getAmountOut(swapAmountIn, address(want), DEI);

        route[] memory lpPairPath = new route[](1);
        lpPairPath[0] = route(address(want), DEI, isStable);
    ...
    function getLpTokens(uint256 _amountIn) internal {
        if (_amountIn > 0) {
            ...
            addLiquidity(_amountIn.sub(swapedAmounts[0]), balanceOfPairToken());
        }
    }

    function _sellAllLpToken1() internal {
        uint256 lpToken1Balance = balanceOfPairToken();
        if (lpToken1Balance > 0) {
            if (sellRouterSelected == 1) {
                _sellAllLpToken1_main(lpToken1Balance);
            } else {
                _sellAllLpToken1_Beethoven(lpToken1Balance);
            }
        }
    }

    function _sellAllLpToken1_main(uint256 _toSellAmount) internal {
        (, bool isStable) = ISolRouter(sellRouter).getAmountOut(_toSellAmount, DEI, address(want));

        route[] memory lp1TokenToLp0Route = new route[](1);
        lp1TokenToLp0Route[0] = route(DEI, address(want), isStable);

        ISolRouter(sellRouter).swapExactTokensForTokens(
            _toSellAmount,
            _toSellAmount.mul(swapSecurityFactor).div(basisOne).div(1e12),
            lp1TokenToLp0Route,
            address(this),
            block.timestamp
        );
    ...
    function _sellAllLpToken1_Beethoven(uint256 _toSellAmount) internal {
    ...
    function _sellRewards() internal {
        route memory wftmToWant = route(wftm, address(want), false);

        route[] memory rewardToWantRoutes = new route[](2);
        rewardToWantRoutes[1] = wftmToWant;

        (uint256 _solidRewards, uint256 _sexRewards) = balanceOfReward();

        if (_solidRewards > rewardDust) {
            rewardToWantRoutes[0] = route(solidReward, wftm, false);
        ...
        if (_sexRewards > rewardDust) {
            rewardToWantRoutes[0] = route(sexReward, wftm, false);
            ISolRouter(router).swapExactTokensForTokens(
                _sexRewards,
                0,
                rewardToWantRoutes,
    ...
    function wantToLpTokens(uint256 _amountWant) public view returns (uint256) {
        (uint256 amountWant, uint256 _amountPairToken) = balanceOfLpComponents(1e6);
        uint256 pairTokenAsWant = pairToken1ToWant(_amountPairToken);
        uint256 amountWantPerLp = pairTokenAsWant.add(amountWant);
        return _amountWant.mul(1e6).div(amountWantPerLp);
    }
    ...
    function _removeLiquidity(uint256 _liquidityToExit) internal {
    ...
    function _removeAllLiquidity() internal {
        ...
        if (lpTokenBalance > 0) {
            _removeLiquidity(lpTokenBalance);
        }
    }
    ...
    function _withdrawFromMasterChefAndTransfer(address _to) internal {
        ...
        if (lpTokensBalance > 0) {
            IERC20(address(solidlyLp)).safeTransfer(_to, lpTokensBalance);
        }
        uint256 pairToken1Balance = balanceOfPairToken();
        if (pairToken1Balance > 0) {
            IERC20(address(DEI)).safeTransfer(_to, pairToken1Balance);
        }
    }

    function _transferRewards(address _to) internal {
        (uint256 _solid, uint256 _sex) = balanceOfReward();
        IERC20(solidReward).safeTransfer(_to, _solid);
        IERC20(sexReward).safeTransfer(_to, _sex);
    }
    ...
    function giveRouteSellAllowance() internal {
        IERC20(DEI).safeApprove(sellRouter, 0);
        IERC20(DEI).safeApprove(sellRouter, max);

        IERC20(DEI).safeApprove(router, 0);
        IERC20(DEI).safeApprove(router, max);
    }
    ...
    function sellPairToken() external onlyVaultManagers {
        _sellAllLpToken1();
    }
    ...
    function removeLiquidity(uint256 _amount) external onlyVaultManagers {
        _removeLiquidity(_amount);
    }

    function removeLiquidity() external onlyVaultManagers {
        _withdrawFromMasterChef();
    }
    ...
    function setSimpleAccounting(bool _simpleAccounting) external onlyVaultManagers {
        simpleAccounting = _simpleAccounting;
    }
