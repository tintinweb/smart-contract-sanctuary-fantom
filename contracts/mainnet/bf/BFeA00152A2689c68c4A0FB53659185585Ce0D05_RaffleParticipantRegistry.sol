...
contract RaffleParticipantRegistry is
    RegistryAware,
    IRaffleParticipantRegistry
{
    using SafeMath for uint256;
    using Address for address;

    mapping(address => address[]) userToRafflesRegistry;

    constructor(address _ownerAddress) RegistryAware(_ownerAddress) {}

    function register(address _user) external override onlyIfRegistered {
        userToRafflesRegistry[_user].push(msg.sender);
    }
    ...
    {
        return userToRafflesRegistry[_user];
    }
}
