/**
 *Submitted for verification at FtmScan.com on 2023-03-23
*/

// SPDX-License-Identifier: MIT
// File: BarZapV1/libraries/Babylonian.sol
...
// File: BarZapV1/interfaces/IWETH.sol
...
// File: BarZapV1/interfaces/IBarRouter01.sol
...
// File: BarZapV1/interfaces/IBarRouter02.sol
...
// File: BarZapV1/interfaces/IBarPair.sol
...
// File: BarZapV1/contracts/security/ReentrancyGuard.sol
...
// File: BarZapV1/contracts/utils/Address.sol
...
// File: BarZapV1/contracts/token/ERC20/IERC20.sol
...
// File: BarZapV1/contracts/token/ERC20/utils/SafeERC20.sol
...
// File: BarZapV1/contracts/utils/Context.sol
...
// File: BarZapV1/contracts/access/Ownable.sol
 ...
 * Implemented and modified by BarSwap teams.
 */
contract BarZapV1 is Ownable, ReentrancyGuard {
    ...
    // BarRouter interface
    IBarRouter02 public barRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from BarSwap)
    ...
    // Address BarRouter
    address private barRouterAddress;
     ...
     * @param _barRouter: address of the BarRouter
    ...
    constructor(
        ...
        barRouterAddress = _barRouter;
        barRouter = IBarRouter02(_barRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap FTM in a WFTM pool (e.g. WFTM/token)
     * @param _lpToken: LP token address (e.g. BAR/FTM)
     * @param _tokenAmountOutMin: minimum token amount (e.g. BAR) to receive in the intermediary swap (e.g. FTM --> BAR)
     */
     ...
     * @param _lpToken: LP token address (e.g. BAR/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. BAR) in the intermediary swap (e.g. BUSD --> BAR)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. BAR/WFTM)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in FTM/WFTM) in the intermediary swap (e.g. BAR --> FTM)
     */
    function zapOutFTM(
        address _lpToken,
        ...
        // Call zapOut
        uint256 tokenAmountToTransfer = _zapOut(_lpToken, WFTMAddress, _tokenAmountOutMin, _totalTokenAmountOutMin);
     ...
     * @param _lpToken: LP token address (e.g. BAR/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. BAR or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. BAR) in the intermediary swap (e.g. BUSD --> BAR)
     */
    ...
    {
        address token0 = IBarPair(_lpToken).token0();
        address token1 = IBarPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IBarPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = barRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = barRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IBarPair(_lpToken).token0() || _token0ToZap == IBarPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IBarPair(_lpToken).token0() || _token1ToZap == IBarPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IBarPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IBarPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IBarPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = barRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = barRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IBarPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "BarRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "BarRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        barRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), barRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(barRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(barRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = barRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = barRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = barRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = barRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = barRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
