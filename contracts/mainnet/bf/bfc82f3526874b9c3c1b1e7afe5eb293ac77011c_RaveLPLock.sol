...
contract RaveLPLock is ERC721Enumerable {
    ...
    function calculateAllocatedRAVE(uint256 lpAmount, uint256 time) public view returns(uint256) {
        uint valueOfAmount = lpAmount * getValueOfLPInRAVE(lpAmount);
        uint raveAtMaxTime = valueOfAmount * (reward/uint(100));
        return raveAtMaxTime * (2*time/YEAR);
    }
