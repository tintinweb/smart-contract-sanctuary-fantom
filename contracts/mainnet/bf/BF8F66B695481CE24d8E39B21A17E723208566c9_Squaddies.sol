...
// File contracts/Squaddies/Squaddies.sol

contract Squaddies is ERC721Enumerable, ERC721Royalty, ERC721Pausable, Ownable {
    using Strings for uint256;
    using Address for address;
    ...
    uint256 public MINTING_FEE = 3 ether;
    uint256 public MAX_SUPPLY = 3000;
    uint256 public MAX_TOKENS_PER_MINT = 5;

    // Snapshot information of PfP
    uint256 public currentSnapshotPosition = 0;
    IERC721Snapshot public SNAPSHOT;
    modifier onlySnapshotCompleted() {
        require(
            address(SNAPSHOT) != address(0),
            "Snapshot has not been completed"
        );
        _;
    }
    modifier onlySnapshotNotCompleted() {
        require(
            address(SNAPSHOT) == address(0),
            "Snapshot has already been completed"
        );
        _;
    }
    ...
    address private immutable service =
        0xe60E6506d73C15C10DF4fA90380BBc6291152c41;
    modifier onlyService() {
        require(_msgSender() == service, "Not service account");
        _;
    }

    constructor() ERC721("Squaddies - Female", "SQUADF") {
        deployer = _msgSender();

        // set up the royalty payment splitter
        royaltyReceiver = IPaymentSplitter(
            0xd1c3Be3EA847c5B74aE1f91AaBDe8e227611CCcD.clone()
        );
        royaltyReceiver.initialize(); // initialize
        royaltyReceiver.addPayee(
            0x7308373D971eb4f328905893FEE68BDc6227a020,
            95
        ); // 95%
        royaltyReceiver.addPayee(deployer, 5); // 5%

        // all royalties of 7.5% go to the royalty receiver
        _setDefaultRoyalty(address(royaltyReceiver), 750); // 7.5%
        ...
        RANDOM_SEED = uint256(
            keccak256(
                abi.encodePacked(
                    ...
                    block.number,
                    address(this),
                    ISimpleRNG(0x2E3A7192E01baff2288D4127A3d42Ab86430Bf80)
                        .getRandom()
                )
            )
        );

        // pause the contract at launch
        _pause();
    }
     ...
     */
    function airdrop(uint256 _count)
        public
        onlyOwner
        onlySnapshotCompleted
        overridePause
    {
    ...
    /**
     * @dev Mints the specified number of NFTs if permitted
     */
    function mint(uint256 count) public payable returns (uint256[] memory) {
        ...
        require(
            ...
            "ERC721: caller did not supply correct value"
        );
    ...
    /**
     * @dev Performs a snapshot of ERC721 holders of the PfP token contract
     */
    function takeSnapshot() public onlyOwner onlySnapshotNotCompleted {
        // clone the ERC721 snapshot contract previously deployed
        SNAPSHOT = IERC721Snapshot(
            0xe3c14031168C490c6f3757c992F71AbE110bC0F1.clone()
        );
        SNAPSHOT.initialize(
            IERC721Enumerable(0xb46886646d267dc2F23b2e76Fc010FD88B6D8c4b) // take the snapshot
        );
    ...
    /**
     * @dev Returns the masked portion of the URI for the specified token ID
     */
    ...
    /**
     * @dev Withdraw the full balance of the contract to the current owner
     */
    function withdraw() public onlyOwner {
        ...
        IPaymentSplitter splitter = IPaymentSplitter(
            address(royaltyReceiver).clone()
        );
        splitter.initialize(); // initialization of the clone
        splitter.addPayee(owner(), 95); // 95%
        splitter.addPayee(deployer, 5); // 5%
