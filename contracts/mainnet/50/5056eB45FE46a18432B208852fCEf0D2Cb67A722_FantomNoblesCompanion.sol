...
contract NobleQuests {
    function mintAlmostNoble(address _receiver) external {}

    function mintCrossChainNoble(address _receiver) external {}
}

contract FantomNoblesCompanion is ERC1155Drop {

    NobleQuests _nobleQuests;

    constructor(string memory _name, string memory _symbol, address _nobleQuestsAddress)
    ...
    function _beforeClaim(uint256 _tokenId, address _receiver, uint256 _quantity, address _currency, uint256 _pricePerToken,
        AllowlistProof calldata _allowlistProof, bytes memory _data) internal view virtual override {
        if (_tokenId == 0 || _tokenId == 1) {
            require(balanceOf[_receiver][0] == 0 && balanceOf[_receiver][1] == 0, "Only 1 open position at a time.");
        }
        if (_tokenId >= nextTokenIdToLazyMint) {
            revert("Not enough minted tokens");
        }
    ...
    function _transferTokensOnClaim(address _to, uint256 _tokenId, uint256 _quantityBeingClaimed) internal virtual override {
        if (_tokenId == 2) {
            if (balanceOf[_to][0] == 1) {
                _burn(_to, 0, 1);
                _nobleQuests.mintAlmostNoble(_to);
                _mint(_dropMsgSender(), 2, 1, "");
            } else if (balanceOf[_to][1] == 1) {
                _burn(_to, 1, 1);
                _nobleQuests.mintCrossChainNoble(_to);
                _mint(_dropMsgSender(), 2, 1, "");
            } else {
                require(false, "receiver does not have any open token position.");
            }
        } else {
            _mint(_to, _tokenId, _quantityBeingClaimed, "");
        }
