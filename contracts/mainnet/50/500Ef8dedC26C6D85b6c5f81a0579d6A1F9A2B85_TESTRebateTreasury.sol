...
contract TESTRebateTreasury is Ownable {
    ...
    constructor(address digital, address digitalOracle, address treasury) {
        Digital = IERC20(digital);
        DigitalOracle = IOracle(digitalOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Digital at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 digitalAmount = getDigitalReturn(token, amount);
        require(digitalAmount <= Digital.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient digital balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + digitalAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += digitalAmount;
    }

    // Claim available Digital rewards from bonding
    ...
    function setDigital(address digital) external onlyOwner {
        Digital = IERC20(digital);
    }

    // Set Digital oracle

    function setDigitalOracle(address oracle) external onlyOwner {
        DigitalOracle = IOracle(oracle);
    }

    // Set Digital treasury
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyOwner {
        require(getDigitalPrice() < 1e18, "RebateTreasury: unable to buy back");
    ...
    function _claimVested(address account) internal {
        ...
        Digital.transfer(account, claimable);
    }
    ...
    // Calculate Digital return of bonding amount of token

    function getDigitalReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 digitalPrice = getDigitalPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / digitalPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 digitalPrice = getDigitalPrice();
        if (digitalPrice < 1e18) return 0;

        uint256 digitalPremium = digitalPrice * DENOMINATOR / 1e18 - DENOMINATOR;
        if (digitalPremium < bondThreshold) return 0;
        if (digitalPremium <= secondaryThreshold) {
            return (digitalPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (digitalPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get DIGITAL price from Oracle

    function getDigitalPrice() public view returns (uint256) {
        return DigitalOracle.consult(address(Digital), 1e18);
    }
    ...
    // Get claimable vested Digital for account

    function claimableDigital(address account) external view returns (uint256) {
