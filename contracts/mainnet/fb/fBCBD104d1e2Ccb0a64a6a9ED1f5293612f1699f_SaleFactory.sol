...
contract SaleFactory is ISaleFactory, Ownable, Pausable {
    ...
    function saleByAddress(ISaleModel saleAddress) external view override returns (bool success, uint256 id) {
    ...
    function saleListByIds(uint256[] memory ids) external view override returns (Sale[] memory) {
        Sale[] memory sales = _sales;
        Sale[] memory returning = new Sale[](ids.length);
        for (uint256 i; i < ids.length; i++) {
            returning[i] = sales[ids[i]];
        }
        return returning;
    }
        ...
        returns (address result)
    {
        ...
        emit SaleCreated(msg.sender, _sales.length);
