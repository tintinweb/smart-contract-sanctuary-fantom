...
interface ISaleFactory {
    ...
    function saleByAddr(IInitOwnable addr) external view returns (bool success, uint256 id);
...
error ZeroMarkLength();
error TotalSupplyOverflow();
error SupplyMarkNotGtLast(uint256 lastSupply, uint256 currentSupply);
...
contract BuyoutThresholds is ISaleModel, InitOwnable {
    ...
    ISaleFactory public constant SALE_FACTORY = ISaleFactory(0xC13ddb8fF25E2Fb6846213AEea9bBDe9C70EF146);
    ...
    function init(
        ...
        if (_marks.length < 1) revert ZeroMarkLength();
        ...
        for (uint256 i; i < _marks.length; i++) {
            if (i > 0) {
                if (_marks[i].supplyMark <= _marks[i - 1].supplyMark) {
                    revert SupplyMarkNotGtLast( _marks[i].supplyMark, _marks[i - 1].supplyMark);
                }
            }
            marks.push(_marks[i]);
        }
        ...
        returns (uint256 cost, uint256 bonus, uint256 mark)
    {
        ...
        for (uint256 i; i < amount + bonus; i++) {
            ...
            // Only update cost for amount buying.
            if (i < amount) {
