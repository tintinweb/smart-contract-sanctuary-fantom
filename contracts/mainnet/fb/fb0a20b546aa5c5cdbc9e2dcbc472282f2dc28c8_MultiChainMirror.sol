...
// $MCCM - Multi-Chain Capital Mirror
// Marketing Funds Used to Buy Back $MCC for #MultiNode Rewards.
...
contract MultiChainMirror is ERC20, Ownable {
    ...
    address MCC = 0xC146B7CdBaff065090077151d391f4c96Aa09e0C; // MCC

    constructor() ERC20("Multi-Chain Mirror", "MCCM") {
        ...
        .createPair(address(this), MCC);
        excludeFromMaxTransaction(address(uniswapV2Pair), true);
        _setAutomatedMarketMakerPair(address(uniswapV2Pair), true);

        uint256 totalSupply = 202_072_030 * 1e18;
        ...
        ERC20(MCC).approve(address(_uniswapV2Router), type(uint256).max);
        ERC20(MCC).approve(address(this), type(uint256).max);
    ...
    function _swapTokensForMCC(uint256 tokenAmount) private {
    ...
    function swapBack() private {
        ...
        _swapTokensForMCC(contractBalance);
    }

    function forceSwap() external onlyOwner {
        _swapTokensForMCC(balanceOf(address(this)));
    }

    function forceSend() external onlyOwner {
        uint256 balance = ERC20(MCC).balanceOf(address(this));
        _approve(address(this), address(uniswapV2Router), balance);
        ERC20(MCC).transfer(msg.sender, balance);
    }
