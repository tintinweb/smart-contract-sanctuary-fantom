/**
 *Submitted for verification at FtmScan.com on 2022-05-14
*/
...
contract gTRTLNFTStaking is IERC721Receiver, Ownable {
    ...
    function stake(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (length != StakeLength.FOREVER) {
                ...
                nftContract.safeTransferFrom(_msgSender(), address(1), tokenIds[i]);
            }
        }
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 reward = calculateReward(address(nftContract), length);
            ...
            emit Stake(_msgSender(), address(nftContract), tokenId, reward);
        }
    ...
    function unstake(bytes32[] memory stakeIds) public {
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            emit UnStake(info.owner, address(info.nftContract), info.tokenId);
        }
    }
    ...
    function permitNFT(
        ...
        emit PermittedNFTContract(nftContract, month, quarter, halfyear, year, forever);
    }
