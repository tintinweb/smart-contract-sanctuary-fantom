...
contract RangoV1 is BaseProxyContract {
    ...
    event TestReceived(address from, address to, uint amount, uint msgvalue);
...
//      emit TestReceived(request.fromToken, request.toToken, request.amountIn, msg.value);
        address a = getProxyAddress(BridgeType.Router);
        (uint out, uint value) = onChainSwapsPreBridge(a, request, calls, 0);
        IRangoRouter(a).routerBridge{value : value}(request.fromToken, out, bridgeRequest);
    }
