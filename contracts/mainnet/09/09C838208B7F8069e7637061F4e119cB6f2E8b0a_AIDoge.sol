...
contract AIDoge is ERC20, Ownable {
    ...
    // Buy Fees
    uint256 public burnFeeBuy = 100; // 1% burn
    uint256 public holderFeeBuy = 100; // 1% holder
    uint256 public daoFeeBuy = 0; // 0%
    uint256 public liquidityFeeBuy = 0;
    uint256 public jackpotFeeBuy = 100; // 1% for lucky drop
    uint256 public bonusFeeBuy = 100; // 1% for staking
    uint256 public productFeeBuy = 100;
    uint256 public totalFeeBuy = 500;
    // Sell Fees
    uint256 public burnFeeSell = 200; // 2% burn
    uint256 public holderFeeSell = 300; // 3% holder
    uint256 public daoFeeSell = 300; // 4% buy ARB
    ...
    constructor(IERC20 _backToken, address _factory, address _swapRouter, address _weth) ERC20("AIDOGE", "AIDOGE") {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBackTokenGov1 = (amountBackToken * holderFee) / (backTokenTotalFee);
        uint256 amountBackTokenGov2 = (amountBackToken * daoFee) / (backTokenTotalFee);
        ...
        backToken.transfer(holder1Wallet, amountBackTokenGov1);
        backToken.transfer(marketingWallet, amountBackTokenGov2);
        backToken.transfer(address(jackpotWallet), amountBackTokenJackpot);
        backToken.transfer(productWallet, amountBackTokenDev);
