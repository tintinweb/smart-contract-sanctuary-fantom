...
/***
 *  ____ ___ ____ ___  _    ____    ____ _ _  _ ____ _  _ ____ ____ 
 *  [__   |  |__| |__] |    |___    |___ | |\ | |__| |\ | |    |___ 
 *  ___]  |  |  | |__] |___ |___    |    | | \| |  | | \| |___ |___ 
 *  
 *  Horse around at Stable Finance
 *
 *  twitter.com/theStableFi
 */
// Note that this pool has no minter key of Stable (rewards).
// Instead, the governance will call Stable distributeReward method and send reward to this pool at the beginning.
contract Stable2GenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Stable to distribute.
        uint256 lastRewardTime; // Last time that Stable distribution occurs.
        uint256 accTombPerShare; // Accumulated Stable per share, times 1e18. See below.
    ...
    // The time when Stable mining starts.
    uint256 public poolStartTime;

    // The time when Stable mining ends.
    uint256 public poolEndTime;

    uint256 public tombPerSecond = 0.13888 ether; // 24000 Stable / (48h * 60min * 60s)
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "StableGenesisRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's Stable allocation point. Can only be called by the owner.
    ...
    // View function to see pending Stable on frontend.
    ...
    // Safe Stable transfer function, just in case if rounding error causes pool to not have enough Stables.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (Stable or lps) if less than 90 days after pool ends
            require(_token != tomb, "stable");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
