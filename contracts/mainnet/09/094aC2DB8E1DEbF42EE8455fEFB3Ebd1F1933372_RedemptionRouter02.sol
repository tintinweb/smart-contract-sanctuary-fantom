...
import './libraries/RedemptionLibrary.sol';
import './libraries/SafeMath.sol';
import './libraries/TransferHelper.sol';
import './interfaces/IRedemptionRouter02.sol';
...
contract RedemptionRouter02 is IRedemptionRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = RedemptionLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = RedemptionLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'RedemptionRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = RedemptionLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IRedemptionPair(pair).burn(to);
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, RedemptionLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            uint pairFee = pair.feeAmount().add(pair.controllerFeeShare());
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, RedemptionLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20Redemption(path[path.length - 1]).balanceOf(to);
        ...
        ensure(deadline)
    {
        ...
        uint amountOut = IERC20Redemption(WETH).balanceOf(address(this));
    ...
    {
        return RedemptionLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
...
import '../interfaces/IRedemptionPair.sol';
...
library RedemptionLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'dea23bf94f438092c636b53672c0befc8af8dfa69b5e017985dfea835458284e' // init code hash
            ))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IRedemptionPair(pairFor(factory, tokenA, tokenB)).getReserves();
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint feeAmount) internal pure returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(1000-feeAmount);
...
import './IRedemptionRouter01.sol';
...
interface IRedemptionPair {
    ...
    function controllerFeeShare() external view returns (uint);
