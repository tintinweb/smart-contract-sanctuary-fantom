...
contract Treasury is ContractGuard {
    ...
    function initialize(
        ...
        triggerRebasePriceCeiling = tombPriceOne.mul(80).div(100);
	...
	{
		require(
			_triggerRebasePriceCeiling < tombPriceOne,
		...
		returns (int256)
	{
		...
		int256 supply = (
			IERC20(tomb)
				.totalSupply()
				.sub(IERC20(tomb).balanceOf(address(this)))
				.sub(IMasonry(masonry).balanceOf(masonry))
		).toInt256Safe();

		if (rate < targetRate) {
			supply = IBasisAsset(tomb).rebaseSupply().toInt256Safe();
		}
		return
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        int256 supplyDelta = _computeSupplyDelta(previousEpochTombPrice, tombPriceOne);
        if (epoch < bootstrapEpochs) {
            ...
            } else if (previousEpochTombPrice <= triggerRebasePriceCeiling || epochsUnderOne > triggerRebaseNumEpochFloor) {
			_contractTomb(supplyDelta);
		}
        }
    }
    ...
    function _contractTomb(int256 supplyDelta) private {
		supplyDelta = supplyDelta.mul(contractionIndex).div(10000);
		IBasisAsset(tomb).rebase(epoch, supplyDelta);
	}
