...
\______   \_____     ______  ____    __| _/   \_   _____/|__|  ____  _____     ____    ____   ____
 ...
 |______  /(____  //____  > \___  >\____ |     \___  /   |__||___|  /(____  /|___|  / \___  >\___  >
        \/      \/      \/      \/      \/         \/             \/      \/      \/      \/     \/
*/
contract Coconut is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 48h genesis pools
    // total of coconut we pay to users during genesis
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 27500 ether;

    // DAO FUND INITIAL ALLOCATION IS 1000 COCONUT
    ...
    /**
     * @notice Constructs the COCONUT ERC-20 contract.
     */
    constructor() ERC20("COCONUT", "COCONUT") {
        // Mints 5000 COCONUT to contract creator for initial pool setup

        _mint(msg.sender, 5000 ether);

    }

    function _getCoconutPrice() internal view returns (uint256 _coconutPrice) {
        try IOracle(coconutOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Coconut: failed to fetch COCONUT price from Oracle");
        }
    }

    function setCoconutOracle(address _coconutOracle) public onlyOperator {
        require(_coconutOracle != address(0), "oracle address cannot be 0 address");
        coconutOracle = _coconutOracle;
    }

    /**
     * @notice Operator mints COCONUT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of COCONUT to mint to
