...
contract RebateTreasury is Ownable {
    ...
    IERC20 public Cfingers;
    IOracle public CfingersOracle;
    ...
    constructor(address cfingers, address cfingersOracle, address treasury) {
        Cfingers = IERC20(cfingers);
        CfingersOracle = IOracle(cfingersOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Cfingers at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 cfingersAmount = getCfingersReturn(token, amount);
        require(cfingersAmount <= Cfingers.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient cfingers balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + cfingersAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += cfingersAmount;
    }

    // Claim available Cfingers rewards from bonding
    ...
    function setCfingers(address cfingers) external onlyOwner {
        Cfingers = IERC20(cfingers);
    }

    // Set Cfingers oracle

    function setCfingersOracle(address oracle) external onlyOwner {
        CfingersOracle = IOracle(oracle);
    }

    // Set Cfingers treasury
    ...
    function _claimVested(address account) internal {
        ...
        Cfingers.transfer(account, claimable);
    }
    ...
    // Calculate Cfingers return of bonding amount of token

    function getCfingersReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 cfingersPrice = getCfingersPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / cfingersPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 cfingersPrice = getCfingersPrice();
        if (cfingersPrice < 1e18) return 0;

        uint256 cfingersPremium = cfingersPrice * DENOMINATOR / 1e18 - DENOMINATOR;
        if (cfingersPremium < bondThreshold) return 0;
        if (cfingersPremium <= secondaryThreshold) {
            return (cfingersPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (cfingersPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get REBEL price from Oracle

    function getCfingersPrice() public view returns (uint256) {
        return CfingersOracle.consult(address(Cfingers), 1e18);
    }
    ...
    // Get claimable vested Cfingers for account

    function claimableCfingers(address account) external view returns (uint256) {
