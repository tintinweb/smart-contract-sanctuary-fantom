...
contract ProxyToken is ERC20 {
    ...
    IProxyDepositor public immutable proxyDepositor;

    constructor(IERC20Metadata _underlying, IProxyDepositor _proxyDepositor)
        ERC20(
            // Reaper Farm Proxy Hundred
            string(abi.encodePacked('Reaper Farm Proxy ', _underlying.name())),
            // rfpHND
            string(abi.encodePacked('rfp', _underlying.symbol()))
        )
    {
    ...
    function deposit(uint256 _amount) public {
        // Pull tokens and send to Proxy Depositor
        underlying.safeTransferFrom(msg.sender, address(proxyDepositor), _amount);

        // Tell proxy depositor to lock
        proxyDepositor.lockTokens();

        // Mint proxy token for msg.sender
