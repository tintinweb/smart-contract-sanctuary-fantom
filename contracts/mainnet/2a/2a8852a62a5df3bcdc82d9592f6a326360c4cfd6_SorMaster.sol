...
contract SorMaster is Ownable, Withdrawable, ReentrancyGuard, Pausable {
    ...
    constructor(ISorToken _sor, IERC20 _usdc, IERC20 _luxor, ILuxorRouter _luxorRouter, address _treasury, uint256 _maxStakeAmount, uint256 _maxRedeemAmount, uint256 _maxStakePerSecond) {
        require(
        ...
        treasury = _treasury;
        swapPath = [address(usdc), address(luxor)];
        swapPathReverse = [address(luxor), address(usdc)];
    ...
    function setSwapPath(address[] calldata _swapPath) external onlyOwner {
        require(_swapPath.length > 1 && _swapPath[0] == address(usdc) && _swapPath[_swapPath.length - 1] == address(luxor), "invalid swap path");
        swapPath = _swapPath;
    ...
    function stake(uint256 amount, uint256 luxorAmountOutMin, uint256 sorAmountOutMin) external nonReentrant whenNotPaused {
        ...
        usdc.approve(address(luxorRouter), luxorAmount);
    ...
    function claimUsdc(uint256 luxorAmountOutMin, uint256 usdcAmountOutMin) external nonReentrant whenNotPaused {
        ...
        uint256 usdcTransferAmount = amountWithoutDecimals * (1000 - luxorPermille - treasuryPermille) / 1000;
