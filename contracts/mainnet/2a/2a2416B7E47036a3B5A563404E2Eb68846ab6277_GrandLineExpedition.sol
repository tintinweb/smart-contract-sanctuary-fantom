...
contract GrandLineExpedition is VRFConsumerBaseV2, ReentrancyGuard {
    ...
    function DropCrewMateOverboard() public nonReentrant onlyOwner {
        ...
        deadMate.StartDropCrewMateOverboard();
        deadMate.InspectTreasureHaul();
        deadMate.EndDropCrewMateOverboard();
        ...
        deadMate.transfer(deadAddress, amountToken);
    ...
    function getTokensOut(
        ...
        // Checks if WETH is token0 or token1 in the pair
        ...
        // Calculate token amount proportional to ethAmount, maintaining current price ratio
        // This line needs to handle the possibility of integer division truncation by increasing precision temporarily
        uint256 tokenAmount = ethAmount.mul(tokenReserve).div(ethReserve);

        return tokenAmount;
    }

    function AssignCaptain(address payable _developerAddress) public onlyOwner {
