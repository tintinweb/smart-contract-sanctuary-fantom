...
contract LPTokenProcessorV2 is ILPTokenProcessorV2, KeeperCompatible, AccessControlEnumerable {
    ...
    function removeTokenFromSwapping(address tokenAddress) external onlyRole(LP_ADMIN_ROLE) {
        if (_tokens.remove(tokenAddress)) {
    ...
    function performUpkeep(
        ...
        for (uint256 i = 0; i < tokensLength; i++) {
            address token = _tokens.at(i);
            bool success = _processTokenSwapping(token);
    ...
    function processTokenSwapping(address token) external onlyRole(LP_ADMIN_ROLE) {
        _processTokenSwapping(token);
    }

    function _processTokenSwapping(address token) private returns (bool) {
        address routerFactory = _tokenFactories[token];
        address routerAddress = routerByFactory[routerFactory];
        if (routerAddress == address(0)) return false;
        if ((_lastAdded[token] + _sellDelay) >= block.timestamp) return false;
        ...
        if (_isV2LPToken[token]) {
            ...
            bool success = _swapTokens(token, tokenBalance, USDT, address(this), routerAddress);
        ...
        usdtAmount = _burnFloki(usdtAmount);
        _processFees(token, tokenBalance, usdtAmount);
        _tokens.remove(token);
        emit TokenProcessed(token);
        return true;
    }
    ...
    function _burnFloki(uint256 usdtAmount) private returns (uint256) {
        // Burn FLOKI
        if (FLOKI != address(0)) {
    ...
    function _processFees(
        ...
        for (uint256 i = referrers.length; i > 0; i--) {
            ...
            IERC20Metadata(USDT).safeTransfer(referrers[i - 1].referrer, referrerShare);
            referrers.pop();
        }
