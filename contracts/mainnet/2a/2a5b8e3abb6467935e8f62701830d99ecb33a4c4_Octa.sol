...
contract Octa is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 OCTA-WFTM LP -> OCTA pool
    uint256 public constant INITIAL_OCTA_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the OCTA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("OCTA", "OCTA") {
        // Mints 1 OCTA to contract creator for initial pool setup
    ...
    function _getOctaPrice() internal view returns (uint256 _OctaPrice) {
        try IOracle(octaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Octa: failed to fetch OCTA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _octaPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_octaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setOctaOracle(address _octaOracle) public onlyOperatorOrTaxOffice {
        require(_octaOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints OCTA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of OCTA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentOctaPrice = _getOctaPrice();
            currentTaxRate = _updateTaxRate(currentOctaPrice);
            if (currentOctaPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_octaPool != address(0), "!_octaPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_octaPool, INITIAL_OCTA_POOL_DISTRIBUTION);
