/**
 *Submitted for verification at FtmScan.com on 2023-02-24
*/
...
// File contracts/EqualizerConkBeetsvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualizerConkBeetsvAMM is Ownable, Pausable {
    ...
    address public constant conk = address(0xb715F8DcE2F0E9b894c753711bd55eE3C04dcA4E);
    ...
    address public constant beetsVault = address(0x20dd72Ed959b6147912C2e529F0a0C651c33c9ce);
    bytes32 public constant beetsPoolId = 0xcde5a11a4acb4ee4c805352cec57e236bdbc3837000200000000000000000019;
    ...
    IEqualizerRouter.Routes[] public wrappedToConkPath;
    ...
    uint256 public STRAT_FEE = 200;                          // 20% of Platform fee
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToConkPath
    ) {
        ...
        for (uint i; i < _wrappedToConkPath.length; ++i) {
            wrappedToConkPath.push(_wrappedToConkPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        _beetsSwap(beetsPoolId, wrapped, beets, wrappedHalf);
        IEqualizerRouter(router).swapExactTokensForTokens(wrappedHalf, 0, wrappedToConkPath, address(this), block.timestamp);

        uint256 lp0Bal = IERC20(beets).balanceOf(address(this));
        uint256 lp1Bal = IERC20(conk).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(beets, conk, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);  
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(conk).safeApprove(router, 0);
        IERC20(beets).safeApprove(router, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(conk).safeApprove(router, type(uint).max);
        IERC20(beets).safeApprove(router, type(uint).max);
    }
    ...
    function makeCustomTxn(address [][] memory _tokens, bool[] calldata _stable) external onlyAdmin {
        ...
        IERC20(_tokens[0][0]).safeApprove(_tokens[0][0], bal);
    ...
    function setFees(uint256 newCallFee, uint256 newStratFee, uint256 newWithdrawFee, uint256 newTreasuryFee, uint256 newRecipientFee) external onlyOwner {
    ...
    function _beetsSwap(bytes32 _poolId, address _tokenIn, address _tokenOut, uint256 _amountIn) internal returns (uint256) {
