...
contract If is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the IF ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("IF", "IF") {
        // Mints 1 IF to contract creator for initial pool setup
    ...
    function _getIfPrice() internal view returns (uint256 _ifPrice) {
        try IOracle(ifOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("If: failed to fetch IF price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ifPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ifPrice >= taxTiersTwaps[tierId]) {
    ...
    function setIfOracle(address _ifOracle) public onlyOperatorOrTaxOffice {
        require(_ifOracle != address(0), "oracle address cannot be 0 address");
        ifOracle = _ifOracle;
    }
    ...
    /**
     * @notice Operator mints IF to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of IF to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentIfPrice = _getIfPrice();
            currentTaxRate = _updateTaxRate(currentIfPrice);
            if (currentIfPrice < burnThreshold) {
                burnTax = true;
            }
        }
