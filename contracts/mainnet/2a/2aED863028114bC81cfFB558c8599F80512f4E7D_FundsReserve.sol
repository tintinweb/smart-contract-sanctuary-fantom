...
contract FundsReserve is AccessControlEnumerable, ReentrancyGuard, IUpdate {
    ...
    uint256 public fateDevAlloc = 10_000;
    uint256 public fateDaoAlloc = 25_000;
    uint256 public fateInsuranceAlloc = 14_800;
     ...
     */
    function setup(address runic_, address fate_, address runite_, address devFunds_, address daoFunds_, address insuranceFunds_, address liquidityFunds_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
    ...
    function setFateAlloc(uint256 devAlloc_, uint256 daoAlloc_, uint256 insuranceAlloc_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        update();
        fateDevAlloc = devAlloc_;
        fateDaoAlloc = daoAlloc_;
        fateInsuranceAlloc = insuranceAlloc_;
        fateTotalAlloc = devAlloc_ + daoAlloc_ + insuranceAlloc_;
    }
    ...
    function getFatePerSecond() public view returns (uint256) {
    ...
    function _distributeFate() internal {
        ...
        uint256 insuranceAmount = (block.timestamp - lastClaimTime) * (fateInsuranceAlloc / fateTotalAlloc) * getFatePerSecond();
        if(lastClaimTime < startTime + fateVestingDuration) {
            ...
            IERC20Mintable(fate).mint(insuranceFunds, insuranceAmount);
        }
    }
