...
contract StrategyAlphaHomoraV1 is BaseStrategy {
    ...
    int public pnl;    
    bool public isSpiritPool;
    bool public isSpookyPool;
    ...
    constructor(
        ...
        if(keccak256(abi.encodePacked(poolName)) == keccak256(abi.encodePacked('Spirit LPs'))) {
            isSpiritPool = true;
        }
        if(keccak256(abi.encodePacked(poolName)) == keccak256(abi.encodePacked('Spooky LP'))) {
            isSpookyPool = true;
        }
    ...
    function priceOracleUpdate() public { 
        ...
        require(isSpiritPool == true, 'Oracle-not-enabled');
   ...
   function requireAmmCheckPass() internal view {
    ...
    function _consultRewardOracle() public view returns (uint256){        
        return isSpiritPool==true ? rewardPairOracle.consult(rewards[0], 1*10**ERC20Upgradeable(rewards[0]).decimals()): 0;
    }
    ...
    function _deposit() internal override {
        ...
        if(balanceUSDC > 1 && _isStableNotUSDC() ){
    ...
    function rebalance(uint256 gradient) external {
        ...
        int balanceBefore = int(homoraData.balanceOfPoolInt(positionId));
    ...
    function getPendingRewards() public view returns (uint pendingRewards) {        
        if(isSpiritPool == true) {
             pendingRewards = homoraData.getPendingRewardsSpirit(positionId);             
        }
        if(isSpookyPool == true) {
             pendingRewards = homoraData.getPendingRewardsBoo(positionId);             
        }        
    }
    ...
    function _swapRewards() internal {
        for (uint256 i = 0; i < rewards.length; i++) {
            ...
            uint expectedOut = (balance*price*PRECISION)/uint256(10)**ERC20Upgradeable(rewards[i]).decimals();            
            // uint minOut = expectedOut*(PRECISION-slippage)/PRECISION;
            // Slippage control
            // require(expectedOut>0, "Badd slippage control rewards");            
            uint minOut = expectedOut*(PRECISION-slippage)/(PRECISION**2);
            address[] memory path = new address[](2);
            path[0] = rewards[i];
            path[1] = stableToken;
            uint256 sufficientLiquidity = _routerQuote(balance, path);
            if(sufficientLiquidity > 0){
    ...
    function _swapToUSDC() internal {        
            ...
            // Slippage control
            uint price;
            price = _oraclePriceChainLink(stableToken);           
    ...
    function _swapToStable() internal {        
            ...
            uint minOut = expectedOut*(PRECISION-slippage)/PRECISION;            
            require(minOut>0, "Badd slippage control stable");
...
import "IWMasterChefSpirit.sol";
import "IMasterChefBooV2.sol";
import "IWMasterChefBooV2.sol";
...
function getPendingRewardsSpirit(uint positionId) external view onlyOwner returns (uint pendingRewards) {
        if(positionId == 0){
                return 0;
            }
    ...
    function getPendingRewardsBoo(uint positionId) external view onlyOwner returns (uint pendingRewards) {
       // query position info from position id
            if(positionId == 0){
                return 0;
            }
...
abstract contract BaseStrategy is Pausable, SettAccessControl {
    ...
    address public constant USDC = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75; //MAINNET FANTOM
...
interface IStrategy {
    ...
    function balanceOfPoolUSDC() external view returns (uint256);
