        ...
        for(uint j = 0; j < _amount; j++){
            ...
            if (risk >= 5){
                if (chance < 6) {
                    ...
                    console.log("Unhoused worker count: ", _unhousedWorkerList.length);
...
//Worker Functions
    ...
    function stakeProtectedWorker(uint256 _workerAmount) public {
        uint256[] memory availableWorkerList = worker.getAvailableWorkers();
        console.log("Avail worker count in stake protected should be 8", availableWorkerList.length);    
    ...
    function sendWorkerToBuild(uint256 _amount) public {
        uint256[] memory availableWorkerList = worker.getAvailableWorkers();
        // for (uint i = 0; i < availableWorkerList.length; i++){
        //     console.log("List item ", i, " -> id: ", availableWorkerList[i]);
        //     console.log("Owner of ", worker.ownerOf(availableWorkerList[i]));
        // }
        require(_amount <= availableWorkerList.length);  
        for (uint j = 0; j < _amount; j++){
...
//Soldier Actions
    function sendSoldierToRaid(uint256 _amount) public {
        ...
        for (uint i; i < _amount; i++){
            ...
            console.log("Soldier damage count after raid is ", soldier.idToDamageCount(availableSoldierList[i]));
...
//Building Block Actions
    function houseWorkers(uint256 _index) public {           
        ...
        uint256 _homelessWorkerCount;
        for (uint i = 0; i<_workerList.length; i++){
            if(!worker.idToHousing(_workerList[i])){
            _homelessWorkers[_homelessWorkerCount] = _workerList[i];     
            _homelessWorkerCount++;
            }}
        uint256 _buildingBlockCapacity = buildingblock.idToCapacity(_index);
        uint256 _cumulativeCapacity = buildingblock.idToCumulativeCapacity(_index);
        uint256 _workersHoused = _cumulativeCapacity -_buildingBlockCapacity;
        uint256[] memory _workersHousedList =  new uint256[](_cumulativeCapacity);
            for (uint c = 0 ; c < _buildingBlockCapacity; c++){
                if(!worker.idToHousing(_workerList[c])){
                _workersHousedList[_workersHoused] = _homelessWorkers[c];
                buildingblock.reduceCapacity(_index);
                _workersHoused++;
                worker.setHousing(_workerList[c], true);  
                }}
            buildingblock.houseWorkers(_index, _workersHousedList);    
    }


    function claimUpgradedBuilding (uint256 _indexToEnlarge, uint256 _indexToBurn) public {
        buildingblock.finishConstruction(_indexToEnlarge,_indexToBurn);
    }
...
interface ISoldierANT is IERC721 {
    ...
    function getSoldiersWith1Damage() external view returns(uint256[] memory);
    function getSoldiersWith2Damage() external view returns(uint256[] memory);
    function getSoldiersWith3Damage() external view returns(uint256[] memory);
