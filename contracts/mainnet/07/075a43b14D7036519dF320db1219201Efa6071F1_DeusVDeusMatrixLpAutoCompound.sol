...
/// @title DeusVDeusMatrixLpAutoCompound
contract DeusVDeusMatrixLpAutoCompound is MatrixLpAutoCompound {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;
    ...
    function _setWhitelistedAddresses() internal override {
        ...
        whitelistedAddresses.add(VDEUS);
        whitelistedAddresses.add(VDeusRouterAddress);
    }
    ...
    function _swap(
        ...
        // handling swap VDEUS -> ANY
        if(_fromToken == VDEUS) {
            uint256 _toTokenBefore = IERC20(_toToken).balanceOf(address(this));

            // first swapping to deus
            VDeusRouter(VDeusRouterAddress).swap(0, 1, _amount, 0, block.timestamp);

            // now swapping to _toToken
            _amount = IERC20(DEUS).balanceOf(address(this));
            _swap(DEUS, _toToken, _amount);
            return IERC20(_toToken).balanceOf(address(this)) - _toTokenBefore;
        }
    ...
    function _addLiquidity(uint256 _outputAmount) internal virtual override returns (uint256 _wantHarvested) {
        uint256 _wantBalanceBefore = IERC20(want).balanceOf(address(this));
    ...
    function _giveAllowances() internal override {
        // approving tokens for mastechef
        IERC20(want).safeApprove(masterchef, 0);
        IERC20(want).safeApprove(masterchef, type(uint256).max);

        // approving tokens for router
        IERC20(DEUS).safeApprove(unirouter, 0);
        IERC20(DEUS).safeApprove(unirouter, type(uint256).max);

        IERC20(VDEUS).safeApprove(unirouter, 0);
        IERC20(VDEUS).safeApprove(unirouter, type(uint256).max);

        // approving tokens for VDeusRouter
    ...
    function _removeAllowances() internal override {
        ...
        // removing approve tokens for router
        IERC20(DEUS).safeApprove(unirouter, 0);

        IERC20(VDEUS).safeApprove(unirouter, 0);

        // removing approve tokens for VDeusRouter
        IERC20(DEUS).safeApprove(VDeusRouterAddress, 0);
    ...
    function _beforeWithdraw(uint256 _amout) internal virtual override {
        IMasterChef(masterchef).withdraw(poolId, _amout, address(this));
    }
    ...
    function _beforeHarvest() internal virtual override {
