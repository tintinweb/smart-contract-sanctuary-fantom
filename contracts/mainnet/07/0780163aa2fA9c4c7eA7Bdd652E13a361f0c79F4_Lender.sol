...
contract Lender is AccessControl, ReentrancyGuard {
    ...
    bytes32 public constant FEE_COLLECTOR_ROLE = keccak256("FEE_COLLECTOR_ROLE"); // fee collector role
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE"); // manager role
    bytes32 public constant INTEREST_MANAGER_ROLE = keccak256("INTEREST_MANAGER_ROLE"); // interest manager role
    ...
    function setAllocationPoint(uint256 collateralIndex, uint256 allocationPoint_)
        external
        onlyRole(CAP_MANAGER_ROLE)
    {
        emit SetAllocationPoint(collateralIndex, allocationPoints[collateralIndex], allocationPoint_);
        totalAllocationPoint = totalAllocationPoint.sub(allocationPoints[collateralIndex]).add(allocationPoint_);
        allocationPoints[collateralIndex] = allocationPoint_;
    }

    /// @notice Sets new maxCap for given lending
    /// @param maxCap_ new maxCap
    function setMaxCap(uint256 maxCap_)
        external
        onlyRole(CAP_MANAGER_ROLE)
    {
    ...
    function borrow(
        ...
        require(
            totalBorrows[collateralIndex].elastic <= maxCap.mul(allocationPoints[collateralIndex]) / totalAllocationPoint,
