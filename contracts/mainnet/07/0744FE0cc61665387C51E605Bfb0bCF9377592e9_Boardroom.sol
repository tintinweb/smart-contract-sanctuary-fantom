...
contract Boardroom is ContractGuard, AccessControlEnumerable, IBoardroom, IBoardroomFee {
    ...
    EnumerableSet.AddressSet _claimHooks; // called on when user claim to update any stats related to the account
    ...
    error WithdrawGreaterThanStaked();
    error WithdrawLockup();
    error RewardLockup();
    error NotOperator();
    error Exist();
    error DoesNotExist();
    ...
    function initialize(IERC20 rewardToken_, IERC20 share_, ITreasury treasury_) public notInitialized onlyOperator {
    ...
    function addOperator(address operator_) external override onlyOperator {
        AccessControl.grantRole(AccessControlConstants.OPERATOR_ROLE, operator_);
    }

    function removeOperator(address operator_) external override onlyOperator {
        AccessControl.revokeRole(AccessControlConstants.OPERATOR_ROLE, operator_);
    }

    function setLockUp(uint256 withdrawLockupEpochs_, uint256 rewardLockupEpochs_) external override onlyOperator {
    ...
    function setFeeAddress(address feeAddress_) external override onlyOperator {
    ...
    function earned(address seat) public view returns (uint256) {
        ...
        return ((balanceOf(seat) * (latestRPS - storedRPS)) / 1e18) + seats[seat].rewardEarned;
    }
    ...
    function withdraw(uint256 amount) public onlyOneBlock seatExists updateReward(msg.sender) {
        ...
        _burnReward();
        uint256 share_ = _balances[msg.sender];
        if(share_ < amount) revert WithdrawGreaterThanStaked();
        _totalSupply -= amount;
        _balances[msg.sender] = share_ - amount;
        if(withdrawFee > 0) {
    ...
    function _claimReward() internal {
        ...
        if(reward > 0) {
            ...
            for(uint256 i; i < _claimHooks.length(); i++) {
                try IClaim(_claimHooks.at(i)).claim(msg.sender, reward) {} catch {}
            }
    ...
    function governanceRecoverUnsupported(address token_, uint256 amount_, address to_) external override onlyOperator {
        // do not allow to drain core tokens
        require(token_ != address(rewardToken), "rewardToken");
        require(token_ != address(share), "share");
