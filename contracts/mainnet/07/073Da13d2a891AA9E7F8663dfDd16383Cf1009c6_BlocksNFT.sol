...
import "./interfaces/INFTPowerCenter.sol";
...
contract BlocksNFT is Initializable, UUPSUpgradeable, NFTERC721Upgradeable, INFTStats, IOperatorBurn, IRunicNFT, IBlocksNFT {
    ...
    function addPower(address account_, uint256 tokenId_, uint256 power_) external override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        ...
        _totalPower += power_;
        INFTPowerCenter(_addressMapping[BlocksNFTConstants.ADDRESS_POWER_CENTER]).addUserPower(account_, power_);
    }
    ...
    function removePower(address account_, uint256 tokenId_, uint256 power_) external override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        ...
        _userPower[account_] -= power_;
        _totalPower -= power_;
        INFTPowerCenter(_addressMapping[BlocksNFTConstants.ADDRESS_POWER_CENTER]).removeUserPower(account_, power_);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        INFTPowerCenter(_addressMapping[BlocksNFTConstants.ADDRESS_POWER_CENTER]).transferUserPower(from, to, power);
...
library BlocksNFTConstants {
     ...
     * Address Mapping IDs
     */
    uint256 public constant ADDRESS_POWER_CENTER = 100;
...
interface INFTPowerCenter {
    ...
    function removeUserPower(address user, uint256 power) external;
    function transferUserPower(address from, address to, uint256 power) external;
    function getTotalPower() external view returns (uint256 power);
    function addTotalPower(uint256 power) external;
    function removeTotalPower(uint256 power) external;
    function setTotalPower(uint256 power) external;
    function updateTotalPower() external;
}
...
abstract contract NFTERC721Upgradeable is Initializable, NFTUpgradeable, IERC721BurnableUpgradeable, ERC721BurnableUpgradeable, ERC721EnumerableUpgradeable, IERC2981, INFTMintable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using StringsUpgradeable for uint256;

    error NonExistentToken();

    function __NFTERC721_init(string memory name_, string memory symbol_) internal onlyInitializing {
        __Context_init_unchained();
        __ERC165_init_unchained();
    ...
    function __NFTERC721_init_unchained() internal onlyInitializing {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Upgradeable, ERC721EnumerableUpgradeable, NFTUpgradeable) returns (bool) {
        return super.supportsInterface(interfaceId) ||
        interfaceId == type(IERC2981).interfaceId ||
        interfaceId == type(IERC721BurnableUpgradeable).interfaceId ||
        interfaceId == type(INFTMintable).interfaceId;
    }
    ...
    function burn(uint256 tokenId) public override(IERC721BurnableUpgradeable, ERC721BurnableUpgradeable) {
        super.burn(tokenId);
    }
    ...
    function royaltyInfo(uint256, uint256 salePrice_) external override view returns (address receiver, uint256 royaltyAmount) {
        uint256 royaltyOwed_ = (salePrice_ * _uintMapping[NFTConstants.UINT_MAPPING_ROYALTY_AMOUNT]) / 10000;
