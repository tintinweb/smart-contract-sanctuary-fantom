...
contract MasterOracleEthBase is Ownable {
    ...
    function getYTokenPrice() public view returns (uint256) {
        return oracleYToken.spot(yToken, PRICE_PRECISION) / EthFtmPrice();
    }
    ...
    function getYTokenTWAP() public view returns (uint256) {
        return oracleYToken.twap(yToken, PRICE_PRECISION) / EthFtmPrice();
    }

    function EthFtmPrice() public view returns (uint256) {
        return
            uint256(priceConverter.getDerivedPrice(ethUsdFeed, ftmUsdFeed, 36)) / PRICE_PRECISION;
    }
}