...
contract Cigar is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 CIGAR-WFTM LP -> CIGAR pool
    uint256 public constant INITIAL_CIGAR_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the CIGAR ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("CIGAR", "CIGAR") {
        // Mints 1 CIGAR to contract creator for initial pool setup
    ...
    function _getCigarPrice() internal view returns (uint256 _cigarPrice) {
        try IOracle(cigarOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Cigar: failed to fetch CIGAR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cigarPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cigarPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCigarOracle(address _cigarOracle) public onlyOperatorOrTaxOffice {
        require(_cigarOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CIGAR to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CIGAR to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCigarPrice = _getCigarPrice();
            currentTaxRate = _updateTaxRate(currentCigarPrice);
            if (currentCigarPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_cigarPool != address(0), "!_cigarPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_cigarPool, INITIAL_CIGAR_POOL_DISTRIBUTION);
