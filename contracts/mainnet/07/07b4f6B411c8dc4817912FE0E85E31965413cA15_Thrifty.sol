...
contract Thrifty is IERC20Metadata, Ownable {
  ...
  function _transfer(
    ...
    require (IThriftyRole(roleManager).onlyWhitelist() == false && (IThriftyRole(roleManager).onlyWhitelist() == true && (IThriftyRole(roleManager).whiteListId(from) > 0 || IThriftyRole(roleManager).whiteListId(to) > 0)), "Thrifty: WL");
    ...
    bool reflectOn = IThriftyRole(roleManager).enableReflect();
    unchecked {
      ...
      if (excludedAddress[to] == true) {
        ...
        if (excludedAddress[from] == false && reflectOn == true) {
        ...
        _balances[to] = tBal;
        balanceLogs[to].push(tBal);
        if (excludedAddress[from] == true && reflectOn == true) {
          totalRSupply = totalRSupply.add(receiveAmount).add(reflectFee);
        }

        if (reflectFee > 0 && reflectOn == true) {
