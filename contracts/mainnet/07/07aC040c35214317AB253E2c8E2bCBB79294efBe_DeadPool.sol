...
contract DeadPool is Ownable, ReentrancyGuard {
    ...
    uint256 public rewardPerSecond = 0.00095 ether;
    ...
    constructor(
        ...
        require(RewardToken.decimals() == 6 || RewardToken.decimals() == 18);
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        {
            return (user.amount * (accRewardPerShare) / (1e18) - (user.rewardDebt)) / 1e12;
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant  {
        ...
        uint256 _pending = 0 ;
        if(RewardToken.decimals() == 18)
            _pending = (user.amount * (pool.accRewardPerShare) / (1e18) - (user.rewardDebt));
        else
            _pending = (user.amount * (pool.accRewardPerShare) / (1e18) - (user.rewardDebt)) / 1e12;
...
function setNewRewardToken (address _newToken) external onlyOperator {
    ...
    if(RewardToken.decimals() == 18)
      rewardPerSecond =  RewardToken.balanceOf(address(this)) / (poolEndTime - block.timestamp);
        ...
        rewardPerSecond =  RewardToken.balanceOf(address(this)) * 1e12 / (poolEndTime - block.timestamp);
}
