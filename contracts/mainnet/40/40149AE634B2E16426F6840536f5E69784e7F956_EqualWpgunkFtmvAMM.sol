...
// File: contracts/EqualWpgunkFtmvAMM.sol
...
contract EqualWpgunkFtmvAMM is Ownable, Pausable {
    ...
    address public constant wpgunk = address(0x2B6850bF31874d96A21eD4Dc7C6415B9640BE2A4);
    address public want;
    ...
    IEqualizerRouter.Routes[] public wrappedToWpgunkPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToWpgunkPath
    ) {
        ...
        for (uint i; i < _wrappedToWpgunkPath.length; ++i) {
            wrappedToWpgunkPath.push(_wrappedToWpgunkPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToWpgunkPath, address(this), block.timestamp);
        
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(wpgunk).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, wpgunk, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(wpgunk).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(wpgunk).safeApprove(unirouter, type(uint).max);
    }
