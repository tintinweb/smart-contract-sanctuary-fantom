...
contract Bubbles is IBubbles, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "Bubbles: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "Bubbles: Invalid tier index");
        _;
    }
    ...
    uint256 public numTokensToCollectFTM;
    uint256 public numOfFTMToSwapAndEvolve;
    ...
    event SwapAndEvolve(uint256 FTMSwapped, uint256 tokenReceived, uint256 FTMIntoLiquidity);

    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __Bubbles_v2_init_unchained(_router);
    }

    function __Bubbles_v2_init_unchained(address _router) internal initializer {
        _name = "Bubbles";
        _symbol = "BBL";
        _decimals = 9;
        ...
        WFTM = uniswapV2Router.WETH();
        // Create a uniswap pair for this new token
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), WFTM);

        //exclude owner and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        //
        __Bubbles_tiers_init();

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function __Bubbles_tiers_init() internal initializer {
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(_account, "Bubbles: Selected account is in blacklist")
    {
        require(_account != address(0), "Bubbles: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner {
        require(_account != address(0), "Bubbles: Invalid address");
        require(_accountsTier[_account] > 0, "Bubbles: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "Bubbles: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "Bubbles: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner checkTierIndex(_tierIndex) {
        require(_owner != address(0), "Bubbles: Address Zero is not allowed");
    ...
    //to receive FTM from uniswapV2Router when swapping
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "Bubbles: Owner address is blacklisted")
        preventBlacklisted(spender, "Bubbles: Spender address is blacklisted")
    {
    ...
    function _transfer(
        ...
        private
        preventBlacklisted(_msgSender(), "Bubbles: Address is blacklisted")
        preventBlacklisted(from, "Bubbles: From address is blacklisted")
        preventBlacklisted(to, "Bubbles: To address is blacklisted")
        isRouter(_msgSender())
    {
        ...
        bool overMinTokenBalance = contractTokenBalance >= numTokensToCollectFTM;
        if (overMinTokenBalance && !inSwapAndLiquify && from != uniswapV2Pair && swapAndEvolveEnabled) {
            contractTokenBalance = numTokensToCollectFTM;
            collectFTM(contractTokenBalance);
        }
    ...
    function collectFTM(uint256 contractTokenBalance) private lockTheSwap {
        swapTokensForFTM(contractTokenBalance);
    }
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        require(contractFTMBalance >= numOfFTMToSwapAndEvolve, "FTM balance is not reach for S&E Threshold");

        contractFTMBalance = numOfFTMToSwapAndEvolve;

        uint256 half = contractFTMBalance.div(2);
        uint256 otherHalf = contractFTMBalance.sub(half);
        ...
        uint256 initialBalance = IBubbles(address(this)).balanceOf(msg.sender);
        // swap FTM for Tokens
        swapFTMForTokens(half);

        // how much FTM did we just swap into?
        uint256 newBalance = IBubbles(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        IBubbles(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
        // add liquidity to uniswap
        addLiquidity(swapeedToken, otherHalf);
        emit SwapAndEvolve(half, swapeedToken, otherHalf);
    }

    function swapFTMForTokens(uint256 FTMAmount) private {
        // generate the uniswap pair path of token -> wFTM
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);
        _approve(owner(), address(uniswapV2Router), FTMAmount);
        // make the swap
    ...
    function migrate(address account, uint256 amount)
        external
        override
        preventBlacklisted(account, "Bubbles: Migrated account is blacklisted")
    {
        require(migration != address(0), "Bubbles: Migration is not started");
        require(_msgSender() == migration, "Bubbles: Not Allowed");
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        IBubbles(_token).transfer(msg.sender, _amount);
    }

    function setNumberOfTokenToCollectFTM(uint256 _numToken) public onlyOwner {
        numTokensToCollectFTM = _numToken;
    }

    function setNumOfFTMToSwapAndEvolve(uint256 _numFTM) public onlyOwner {
        numOfFTMToSwapAndEvolve = _numFTM;
    }
    ...
    function getFTMBalance() public view returns (uint256) {
