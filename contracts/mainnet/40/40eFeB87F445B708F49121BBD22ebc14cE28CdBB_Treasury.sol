...
contract Treasury is ITreasury, Operator, ReentrancyGuard {
    ...
    function buyback(uint256 _collateral_amount, uint256 _min_share_amount, bool useSlip) external override onlyOperator notMigrated checkRebalanceCooldown {
        ...
        require(IERC20(collateral).balanceOf(collateralFund) > _collateral_amount, "insufficentPoolBalance");
    ...
    // SWAP tokens
    function _swap(address inputToken, address outputToken, uint256 inputAmount, uint256 outputAmountMin) internal returns (uint256) {
    ...
    function _swap1(address inputToken, address outputToken, uint256 inputAmount, uint256 outputAmountMin) public onlyOperator returns (uint256) {
        uint256 inputTokenBalance = IERC20(inputToken).balanceOf(address(this));
        require(inputTokenBalance > inputAmount, "not enough token balance");
        address[] memory _path = new address[](2);
        _path[0] = inputToken;
