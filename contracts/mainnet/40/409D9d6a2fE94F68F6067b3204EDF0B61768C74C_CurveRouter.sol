...
// MAI 0xfB98B335551a418cD0737375a2ea0ded62Ea213b
  ...
  function deposit(
    ...
    uint256[] calldata initialAmounts) external {
    IERC20(asset).transferFrom(msg.sender, address(this), amount);
    require(pools.length == _froms.length && _froms.length == _tos.length && _froms.length == initialAmounts.length);

    for(uint256 i = 0; i < pools.length; i ++) {
      router(pools[i], _froms[i], _tos[i], initialAmounts[i]);
    }
    ...
    ) internal returns(uint256)
  {
    ...
    for(uint256 i = 0; i < pools.length; i++) {
      ...
      amountReceived = swapCurve(poolToUse, _fromToken, _toToken, amountReceived);
    }
  ...
  function swapCurve(address poolToUse, address _from, address _to, uint256 amount) internal returns(uint256) {
