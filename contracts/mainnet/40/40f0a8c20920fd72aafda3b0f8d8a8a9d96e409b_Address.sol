...
contract SKULLRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last time that SKULLs distribution occurs.
        uint256 accSKULLPerShare; // Accumulated SKULLs per share, times 1e18. See below.
    ...
    uint256 public SKULLPerSecond = 0.00186122 ether; // 59500 SKULL / (370 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SKULLRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "SKULLRewardPool: existing pool?");
        }
    }
    ...
    function safeSKULLTransfer(address _to, uint256 _amount) internal {
        uint256 _SKULLBal = SKULL.balanceOf(address(this));
        if (_SKULLBal > 0) {
            if (_amount > _SKULLBal) {
                SKULL.safeTransfer(_to, _SKULLBal);
            } else {
