...
contract BasedSolidSex {
    ...
    uint public RewardsPerEpoch = 0;

    address[] public stakers;
    mapping(address => uint) public stakingBalance;
    mapping(address => bool) public hasStaked;
    mapping(address => bool) public isStaking;
    mapping(address => uint) public receivedRewards;
    ...
    function UpdateRewardsPerEpoch(uint _amount) public onlyOperator {
    ...
    function issueInterestToken() public onlyOperator{
        for (uint i=0; i<stakers.length; i++) {
            ...
            uint rewards = poolShare*RewardsPerEpoch/(1e18);
    ...
    // if there is a balance transfer the SAME amount of bank tokens to the account that is staking as a reward
            ...
            if(rewards > 0 ) {
                ...
                receivedRewards[recipient]+= rewards;
    ...
    function getReceivedRewards(address staker) external view returns(uint256) {
         return receivedRewards[staker];
    }
