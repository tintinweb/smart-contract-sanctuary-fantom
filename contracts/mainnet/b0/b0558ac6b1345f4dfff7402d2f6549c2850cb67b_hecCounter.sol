...
contract hecCounter {
    ...
    address public hecUsdcLpAddress = 0x0b9589A2C1379138D4cC5043cE551F466193c8dE;

    address public fNFTsFarmsHEC = 0x80993B75e38227f1A3AF6f456Cf64747F0E21612;
    address public fNFTsFarmsHECUSDC = 0xd7faE64DD872616587Cc8914d4848947403078B8;
    ...
    function computeLiquidityShareValue(uint liquidity)
        public
        view
        returns (
            uint256 _LPHecs
    ...
    {
        uint256 _lpSupply = IUniswapV2ERC20(hecUsdcLpAddress).totalSupply();
        uint256 _lpRatio = liquidity / (_lpSupply / 1 gwei);
        (
            uint112 _reserve0,
            uint112 _reserve1,
            uint32 _blockTimestampLast
        ) = ILiquidityValueCalculator(hecUsdcLpAddress).getReserves();
    ...
    function getHecAmount(address _address) public view returns (uint256) {
        ...
        uint256 ownedwsHECtosHec = IwsHEC(wshecAddress).wsHECTosHEC(wshecOwnedAmount);
        ...
        if (ownedFNFTS > 0) {
            for (uint256 i = 0; i < ownedFNFTS; i++) {
                ...
                if (_firstFarm.amount > 0) {
                    ...
                    uint256 _LPHecs = computeLiquidityShareValue(_secondFarm.amount);
                    if (_LPHecs > 0) {
                        totalHEC += _LPHecs;
                    }
                }
