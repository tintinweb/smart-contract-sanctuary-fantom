...
contract StakingFantom is Ownable, ReentrancyGuard {
    ...
    mapping(uint8 => TimeLock) public timeLocks;
    mapping(uint8 => TimeLockStake) private stakeFields;
    IERC20 stakingToken;

    constructor(address _stakingToken) {
        stakingToken = IERC20(_stakingToken);
        timeLocks[0].period = 7 days;     // 1 week
        timeLocks[0].apr = 13;
        timeLocks[1].period = 30 days;    // 1 month
        timeLocks[1].apr = 52;
        timeLocks[2].period = 91 days;    // 3 month
        timeLocks[2].apr = 156;
        timeLocks[3].period = 182 days;   // 6 month
        timeLocks[3].apr = 312;
        timeLocks[4].period = 365 days;   // 1 year
        timeLocks[4].apr = 624;
        timeLocks[5].period = 547 days;   // 1.5 year
        timeLocks[5].apr = 936;
        timeLocks[6].period = 730 days;   // 2 years
        timeLocks[7].apr = 1248;
        timeLocks[8].period = 1095 days;  // 3 years
        timeLocks[8].apr = 1872;
        timeLocks[9].period = 1459 days;  // 4 years
        timeLocks[9].apr = 2496;
        timeLocks[10].period = 1824 days; // 5 years
        timeLocks[10].apr = 3120;
    }
    ...
    function withdrawAll(uint8 timeLockId) public nonReentrant {
        ...
        for(uint256 i = 0; i < stakeUser.itemIds.length(); i ++) {
            ...
            if (block.timestamp - stakeItem.depositAt > timeLocks[timeLockId].period) {
                if (stakeItem.amount > 0) {
                    if (timeLocks[timeLockId].period < block.timestamp - stakeItem.depositAt) {
    ...
    function getRecentReward(uint8 timeLockId, address investor) public view returns (StakeStruct memory recentStakeItem) {
        ...
        if (_timeTmp != 0) {
            recentStakeItem = stakeUser.stakeItems[_itemId];
        }
    }
