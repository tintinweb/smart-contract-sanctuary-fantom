...
contract LarvaLads is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getSVG(LarvaObject memory larvaLad) internal view returns (string memory) {
        ...
        if(larvaLad.skinTone > 15) {
            parts[1] = skinToneLayers[larvaLad.skinTone];
        }
        else {
        ...
        // Eye Color
        parts[4] = '<path d="M8 10h3v3H8zm11 0h3v3h-3z" fill="';
        parts[5] = eyeColorLayers[larvaLad.eyeColor];
        parts[6] = '"/>';
        // Pupil
        parts[7] = '<path fill="#000" d="M9 11h1v1H9zm11 0h1v1h-1z"/>';

        parts[8] = glassesLayers[larvaLad.glasses];
        parts[9] = mouthLayers[larvaLad.mouth];
