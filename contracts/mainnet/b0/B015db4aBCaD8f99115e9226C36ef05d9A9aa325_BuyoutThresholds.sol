...
contract BuyoutThresholds is ISaleModel, InitOwnable {
    ...
    event Buyout(uint256 indexed amount, uint256 indexed bonus);

    ISaleFactory public constant SALE_FACTORY = ISaleFactory(0x117a79eBf039c0F6C0C6070bFa184b2EBeBF569d);
    ...
    struct Marks {
        // When suppy sold reaches `supplyMark` amount, set price per token to `priceMark` amount.
        // -    i.e, supplyMark = 100, priceMark = $200
        //           supplyMark = 200, priceMark = $300
        //           supplyMark = 300, priceMark = $400
        uint256 supplyMark;
        uint256 priceMark;
    }
    mapping(uint256 => Marks) public marks;
    ...
    function init(
        ...
        for (uint256 i; i < _marks.length; i++) {
            _totalSupply += _marks[i].supplyMark;
        }
        ...
        for (uint256 i; i < _marks.length; i++) {
            marks[mSale.totalMarks] = _marks[i];
            mSale.totalMarks++;
        }
        sale.totalMarks = mSale.totalMarks;
    ...
    function buyout(bytes32[] calldata merkleProof, uint256 amount) external onlyInit {
        ...
        (uint256 cost, uint256 newMark) = buyingResults(amount, bonus);
    ...
    function buyingResults(uint256 amount, uint256 bonus)
        public
        view
        returns (uint256 cost, uint256 mark)
    {
        Sale memory mSale = sale;

        // Calculate cost for each NFT being bought.
        for (uint256 i; i < amount; i++) {
            Marks memory mMark = marks[mSale.currentMark];
            ...
            if (mSale.totalSold >= mMark.supplyMark) {
                ...
                if (mSale.currentMark == mSale.totalMarks) {
        ...
        if (bonus > 0) {
            mSale.totalSold += bonus;
            for (uint256 i = mSale.currentMark; i < mSale.totalMarks; i++) {
                // If current mark == highest mark.
                if (mSale.currentMark == mSale.totalMarks) {
                    // Stop looking for next mark.
                    break;
                } else {
                    // Go next mark.
                    mSale.currentMark++;
