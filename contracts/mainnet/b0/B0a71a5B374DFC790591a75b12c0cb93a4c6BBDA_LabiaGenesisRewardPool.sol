...
contract LabiaGenesisRewardPool {
    ...
    address public partnerCollector;

    uint256 public mainPerSecond = 0.22505 ether; // 38888 MAIN / (2*24h * 60min * 60s)
    uint256 public runningTime = 2 days; // 2 days
    uint256 public constant TOTAL_REWARDS = 38888 ether;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            //parner 10%
            uint parnerfee = _fee.div(10);
            pool.token.safeTransfer(partnerCollector, parnerfee);
            pool.token.safeTransfer(taxCollector, _fee - parnerfee);
    ...
    function setTexFee(address _address, address _partneraddress, uint _fee) external onlyOperator {
        require(_fee <= 100, "Fee can not be above 1%");
        taxCollector = _address;
        partnerCollector = _partneraddress;
        daoFee = _fee;
    }
}
