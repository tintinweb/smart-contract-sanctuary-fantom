...
contract StrategyOxdVolatilePgunkBombLp is StrategyOxdFarmLPBase {
    // Token addresses
    address public pgunk_bomb_lp = 0x270b4cdEAebC4E0522AA4371a9BE38c624193cEe;
    address public pgunk = 0xf8Fc059dAfDCe4EF2EdFc72cbBAF410d7531E610;
    address public bomb = 0x8503eb4A136bDBeB323E37Aa6e0FA0C772228378;
    ...
    {
        swapRoutes[pgunk].push(route(wftm, bomb, false));
        swapRoutes[pgunk].push(route(bomb, pgunk, false));
        swapRoutes[bomb].push(route(wftm, bomb, false));
    }

    // **** Views ****

    function getName() external pure override returns (string memory) {
        return "StrategyOxdVolatilePgunkBombLp";
    }
}
...
import "../..//interfaces/oxd-farm.sol";
import "../../interfaces/solid-router.sol";
import {route} from "../../interfaces/solid-router.sol";

abstract contract StrategyOxdFarmLPBase is StrategyBase {
    ...
    address public constant lens = 0xDA00137c79B30bfE06d04733349d98Cf06320e69;
    ...
    constructor(
        ...
        IERC20(oxd).approve(solidRouter, type(uint256).max);
        IERC20(wftm).approve(solidRouter, type(uint256).max);
        IERC20(solid).approve(solidRouter, type(uint256).max);
    ...
    function balanceOfPool() public view override returns (uint256) {
        IUserProxy.PositionStakingPool[] memory positions = IUserProxy(lens)
            .stakingPoolsPositions(address(this));
        
        if (positions.length == 0) return 0;
        return positions[0].balanceOf;
    }
    ...
    function getHarvestable() external view returns (uint256, uint256) {
        ...
        IUserProxy.RewardToken[] memory pendingRewards = positions[0]
            .rewardTokens;

        // (pendingSolid, pending0XD)
        return (pendingRewards[0].earned, pendingRewards[1].earned);
    }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            IOxdFarm(rewards).depositLpAndStake(want, _want);
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IOxdFarm(rewards).unstakeLpAndWithdraw(want, _amount);
        return _amount;
    }

    function harvest() public override {
        // Harvest SOLID and OXD rewards
        IOxdFarm(rewards).claimStakingRewards();
        uint256 _solid = IERC20(solid).balanceOf(address(this));
        uint256 _oxd = IERC20(oxd).balanceOf(address(this));
        if (_solid == 0 && _oxd == 0) return;
        ...
        // Solidly swap OXD to FTM
        if (_oxd > 0) {
            route[] memory oxdRoute = new route[](1);
            oxdRoute[0] = route(oxd, wftm, false);
            ISolidRouter(solidRouter).swapExactTokensForTokens(
                _oxd,
                0,
                oxdRoute,
...
interface IUserProxy {
    ...
    function stakingPoolsPositions(address)
        external
        view
...
interface IOxdFarm {
    function depositLp(address solidPoolAddress, uint256 amount) external;

    function depositLpAndStake(address solidPoolAddress, uint256 amount)
        external;

    function withdrawLp(address solidPoolAddress, uint256 amount) external;
    ...
    function stakeOxLp(address oxPoolAddress, uint256 amount) external;

    function unstakeOxLp(address oxPoolAddress, uint256 amount) external;
