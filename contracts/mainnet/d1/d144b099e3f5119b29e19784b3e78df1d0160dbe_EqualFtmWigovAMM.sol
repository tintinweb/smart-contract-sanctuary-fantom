...
// File contracts/EqualFtmWigovAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualFtmWigovAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public wrappedToWigoPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToWigoPath
    ) {
        ...
        for (uint i; i < _wrappedToWigoPath.length; ++i) {
            wrappedToWigoPath.push(_wrappedToWigoPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToWigoPath, address(this), block.timestamp);

        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(wigo).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, wigo, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
