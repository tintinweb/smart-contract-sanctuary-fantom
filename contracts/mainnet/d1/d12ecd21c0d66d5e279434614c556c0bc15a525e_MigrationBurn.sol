...
contract MigrationBurn {
    ...
    /**
     * @notice Chicken out and return burn tokens
     * @param tokenAddress Address of the token to return
     * @dev Method can only be called during burn period
     */
    function chickenOut(address tokenAddress) external lock onlyBurnPeriod {
        _chickenOut(msg.sender, tokenAddress);
    }

    /**
     * @notice Interanal return burn tokens
     * @param tokenAddress Address of the token to return
     * @dev Used by chickenOut() and refund()
     */
    function _chickenOut(address accountAddress, address tokenAddress)
        internal
    {
    ...
    /**
     * @notice Return burned veNFT
     * @param tokenId Token ID to return
     * @dev Method can only be called during burn period
     */
    function chickenOutVeNft(uint256 tokenId) external lock onlyBurnPeriod {
        _chickenOutVeNft(msg.sender, tokenId);
    }

    /**
     * @notice Internal return burned veNFT
     * @param accountAddress User address to return the veNFT to
     * @param tokenId Token ID to return
     * @dev Used by chickenOutVeNft() and refundVeNft()
     */
    function _chickenOutVeNft(address accountAddress, uint256 tokenId)
        internal
    {
    ...
    function refund(address accountAddress, address tokenAddress)
        ...
        onlyOwner
    {
        _chickenOut(accountAddress, tokenAddress);
    }
    ...
    function refundVeNft(address accountAddress, uint256 tokenId)
        ...
        onlyOwner
    {
        _chickenOutVeNft(accountAddress, tokenId);
    }
