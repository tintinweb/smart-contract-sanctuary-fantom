...
contract BridgeFacet {
    ...
    function bridgeRenew(
        ...
        (uint8 subscriptionId, bool isActive) = manager.getUserSubscriptionStatus(user);

        uint256 price = LibSubscriptionTypes.subscriptions(subscriptionManagerId, subscriptionId).price;
...
library LibPayment {
    ...
    function setUserReferral(uint256 subManagerId, address user, address referrer) internal {
...
library LibSubscriptionTypes {
    bytes32 constant DIAMOND_STORAGE_POSITION = keccak256("com.cicleo.facets.subscriptiontypes");
    ...
    function subscriptions(uint256 subscriptionManagerId, uint8 subscriptionId) internal view returns (SubscriptionStruct memory) {
        return diamondStorage().subscriptions[subscriptionManagerId][subscriptionId];
    }

    function subscriptionNumber(uint256 subscriptionManagerId) internal view returns (uint8) {
