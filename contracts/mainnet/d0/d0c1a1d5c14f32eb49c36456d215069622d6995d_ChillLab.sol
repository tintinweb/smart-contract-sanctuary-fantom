...
// Chill Lab is a an Acelab fork by @lil_esper original contract is here:0x399d73bb7c83a011cd85df2a3cdf997ed3b3439f 
// Changelog: added max TVL limit, Staking Fees, and optimized rarity
...
contract ChillPillRarity is ChillAuth {
    ...
    function rarityOfBatch(uint[] memory tokenIDs) public pure returns (uint[] memory rarities) {
    ...
    function sumOfRarities(uint[] memory tokenIDs) public pure returns (uint sum) {
        uint len = tokenIDs.length;
        sum = 0;
        for(uint i = 0; i < len; i++)
...
contract ChillLab is ChillPillRarity, ReentrancyGuard {
    ...
    uint stakingFeePercent = 1;
    uint stakingFeesAdmin =0;
    ...
    function pendingRewards(uint _pid, address _user) public view returns (uint schillReward, uint chillpillReward) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if(pool.mpStakedAmount != 0) accRewardPerShareChillpill += reward * chillpillBoost / 10000 * precisionOf[pool.TokenPrecision] / pool.mpStakedAmount;
        }
    ...
    function updatePool(uint _pid) public {
        ...
        if(pool.sChillStakedAmount != 0) pool.accRewardPerShare += reward * (10000 - chillpillBoost) / 10000 * precisionOf[pool.TokenPrecision] / pool.sChillStakedAmount;
        if(pool.mpStakedAmount != 0) pool.accRewardPerShareChillpill += reward * chillpillBoost / 10000 * precisionOf[pool.TokenPrecision] / pool.mpStakedAmount;
    ...
    function depositTo(uint _pid, uint _amount, address to) external nonReentrant {
        _deposit(_pid, _amount, to, new uint[](0));
    }

    function booostedDeposit(uint _pid, uint _amount, uint[] memory tokenIDs) external nonReentrant {
        _deposit(_pid, _amount, msg.sender, tokenIDs);
    }

    function boostedDepositTo(uint _pid, uint _amount, address to, uint[] memory tokenIDs) external nonReentrant {
    ...
    function takeFees(uint _amount) internal returns(uint){
        ...
        uint stakingFees = (_amount * stakingFeePercent) /
            BASIS_POINT_DIVISOR;
    ...
    function _deposit(uint _pid, uint _amount_before_fees, address to, uint[] memory tokenIDs) internal {
        ...
        if(poolMaxTVL[_pid]> 0){
            require(_amount+pool.sChillStakedAmount<=poolMaxTVL[_pid],"deposit:pool has hit deposit cap");
        }
