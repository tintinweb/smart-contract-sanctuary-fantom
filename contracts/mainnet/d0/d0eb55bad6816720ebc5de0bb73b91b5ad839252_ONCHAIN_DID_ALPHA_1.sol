...
contract ONCHAIN_DID_ALPHA_1 {
    ...
    function verify(authentication memory AUTHX, uint8 v, bytes32 r, bytes32 s, address user) public  view returns (string memory) {
        ...
        bytes32 digest = keccak256(abi.encodePacked(
            "\x19\x01",
            DOMAIN_SEPARATOR,
            hashAUTH(authentication({
            VERIFY_ONCHAIN: AUTHX.VERIFY_ONCHAIN,
            TOKEN_ADDRESS: AUTHX.TOKEN_ADDRESS,
            CHAIN_ID: AUTHX.CHAIN_ID,
            VERSION: AUTHX.VERSION,
            MESSAGE: AUTHX.MESSAGE,
            WALLET: AUTHX.WALLET
        }))
        ));
