...
// LIF3 Finance - a FATFIRE FINANCE layer on Fantom.  
// Website: lif3.exchange
// Twitter: https://twitter.com/defifatfire (@@defifatfire)
// TG: https://t.me/fatfiredao
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 constant MAXSUPPLY = 85000000000000000000000000 ether;
...
contract Life is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0x390337AeD6D513a02Fc65fc75C5770A15E51f6e4;
    ...
    // Min amount to liquify. (default 10 Lifes)
    ...
    // The operator can call all functions except of minting Life
    address private _operator;
    
    // The lpEarner address set for buyback & burn 
    address constant public lpEarner = 0x390337AeD6D513a02Fc65fc75C5770A15E51f6e4;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "Life::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the LifeSwapToken contract.
     */
    constructor() public BEP20("Life", "LIFE") {
    ...
    /// @dev overrides transfer function to meet tokenomics of Life
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "LIFE::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "LIFE::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the LIFE -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "LIFE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "LIFE::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "LIFE::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "LIFE::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "LIFE::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "LIFE::transferOperator: new operator is the zero address");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Life
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Lifes to distribute per second.
        uint256 lastRewardSecond; // Last second that Lifes distribution occurs.
        uint256 accLifePerShare; // Accumulated Lifes per share, times 1e18. See below.
    ...
    // life tokens created per second.
    ...
    // The block timestamp when Life mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 LifePerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _LifePerSecond,
        uint256 _startTime
    ) public {
        life = _life;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        LifePerSecond = _LifePerSecond;
    ...
    // View function to see pending Life on frontend.
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 lifeReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
                uint256 devReward = lifeReward.div(10);
                uint256 totalRewards = life.totalSupply().add(devReward).add(
                lifeReward
            );
                if (totalRewards > life.maxSupply()) {
                lifeReward = life.maxSupply().sub(life.totalSupply());
            }
            accLifePerShare = accLifePerShare.add(
                lifeReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accLifePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= life.maxSupply()) {
            ...
            // mint the difference only to MC, update lifeReward
            lifeReward = life.maxSupply().sub(life.totalSupply());
            life.mint(address(this), lifeReward);
        }

        if (lifeReward != 0) {
            // only calculate and update if lifeReward is non 0
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accLifePerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accLifePerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeLifeTransfer(msg.sender, pending);
    ...
    function updateEmissionRate(uint256 _LifePerSecond) external onlyOwner {
        require(_LifePerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        LifePerSecond = _LifePerSecond;
        emit UpdateEmissionRate(msg.sender, _LifePerSecond);
    }
