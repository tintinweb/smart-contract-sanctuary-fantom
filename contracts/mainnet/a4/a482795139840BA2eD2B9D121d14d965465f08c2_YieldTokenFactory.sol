...
import {IYieldTokenFactory} from '../interfaces/core/IYieldTokenFactory.sol';
import {IYieldToken, YieldToken} from './YieldToken.sol';
import {RoleNames} from '../library/RoleNames.sol';

contract YieldTokenFactory is IYieldTokenFactory, AccessControl {
  ...
  constructor(address _masterAdmin) {
    ...
    _setRoleAdmin(RoleNames.DEPLOYER, RoleNames.MASTER_ADMIN);
    _setupRole(RoleNames.MASTER_ADMIN, _masterAdmin);
    _setupRole(RoleNames.DEPLOYER, _masterAdmin);
  }
  ...
  function createYieldToken(
  ...
  ) public onlyRole(RoleNames.DEPLOYER) returns (IYieldToken _yieldToken) {
    _yieldToken = IYieldToken(
      new YieldToken{salt: keccak256(abi.encode(_underlyingToken, _symbol))}(
        IYieldToken.YieldTokenParams(_name, _symbol, _underlyingToken, _vault, _masterAdmin)
      )
    );
    ...
    emit FactoryCreatedYieldToken(address(_yieldToken));
  }
...
import {IVaultParameters} from './Vault/IVaultParameters.sol';

interface IYieldTokenFactory is IAccessControl {
  event FactoryCreatedYieldToken(address _yieldToken);
  ...
  function createYieldToken(
    ...
    address _vault,
    address _masterAdmin
  ) external returns (IYieldToken _yieldToken);
