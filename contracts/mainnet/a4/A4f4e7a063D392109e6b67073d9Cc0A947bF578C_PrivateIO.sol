...
contract PrivateIO is IIDO, Ownable {
    ...
    constructor(address reserve_, address finalizer_) {
        require(reserve_ != address(0));
        require(finalizer_ != address(0));

        reserve = reserve_;
        finalizer = finalizer_;
        totalAmount = 27600000 * 10**18;
        amountRemaining = 27600000 * 10**18;
        salePrice = 36231;

        _transferOwnership(finalizer_);
    }
    ...
    function closeSale() external override {
        require(
            msg.sender == finalizer,
            "Can only be called by the approved caller"
        );
    ...
    function purchase(uint256 amount) external returns (bool) {
        ...
        require(amount > 49, "Min allocation is $50");
        require(
            amount.add(usdPurchasedAmounts[msg.sender]) <= MAX_BUY,
            "Amount exceeds max allocation: $1000"
        );
        ...
        usdPurchasedAmounts[msg.sender] = usdPurchasedAmounts[msg.sender].add(
            amount
        );
        totalUsdPurchased = totalUsdPurchased.add(amount);
    ...
    function _calculateSaleQuote(uint256 paymentAmount_)
        ...
        returns (uint256)
    {
        return paymentAmount_.mul(uint256(1e6)).mul(10**18).div(salePrice);
    }

    function closeSaleRequirements() internal view {
        require(
            block.timestamp > (startOfSale + SALE_SECONDS),
