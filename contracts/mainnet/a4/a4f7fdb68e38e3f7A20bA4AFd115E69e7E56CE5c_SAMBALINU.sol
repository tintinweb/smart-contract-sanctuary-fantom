...
contract SAMBALINU is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;    

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 10000 ether;
    // Initial distribution for the day 2-5 SAMBALINU-WFTM LP -> SAMBALINU pool
    uint256 public constant INITIAL_SAMBALINU_POOL_DISTRIBUTION = 1 ether;
    ...
    address public SAMBALINUOracle;
    ...
    /**
     * @notice Constructs the SAMBALINU ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SAMBALINU", "SAMBALINU") {
        // Mints 1 SAMBALINU to contract creator for initial pool setup
    ...
    function _getSAMBALINUPrice() internal view returns (uint256 _SAMBALINUPrice) {
        try IOracle(SAMBALINUOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("SAMBALINU: failed to fetch SAMBALINU price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _SAMBALINUPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_SAMBALINUPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSAMBALINUOracle(address _SAMBALINUOracle) public onlyOperatorOrTaxOffice {
        require(_SAMBALINUOracle != address(0), "oracle address cannot be 0 address");
        SAMBALINUOracle = _SAMBALINUOracle;
    }
    ...
    /**
     * @notice Operator mints SAMBALINU to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SAMBALINU to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSAMBALINUPrice = _getSAMBALINUPrice();
            currentTaxRate = _updateTaxRate(currentSAMBALINUPrice);
            if (currentSAMBALINUPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _SAMBALINUPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_SAMBALINUPool != address(0), "!_SAMBALINUPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_SAMBALINUPool, INITIAL_SAMBALINU_POOL_DISTRIBUTION);
