...
contract CrackVault is ERC20, Ownable, ReentrancyGuard {
    ...
    function sideProfitToken() public view returns (IERC20) {
    ...
    function totalBalanceOfStaked() public view returns (uint) {
        return (stakedToken().balanceOf(address(this))) + (IStrategy(strategy).totalBalanceOfStaked());
    }
    ...
    function depositAll() external {
        deposit(stakedToken().balanceOf(msg.sender));
    }
    ...
    function deposit(uint _amount) public nonReentrant {
        ...
        uint256 _pool = totalBalanceOfStaked();
        stakedToken().safeTransferFrom(msg.sender, address(this), _amount);
        earn();
        uint256 _after = totalBalanceOfStaked();
    ...
    function earn() public {
        uint _bal = available();
        stakedToken().safeTransfer(address(strategy), _bal);
        strategy.deposit();
    }
    ...
    function withdraw(uint256 _shares) public {
        uint256 r = FullMath.mulDiv(totalBalanceOfStaked(), _shares, totalSupply());
        uint256 totalSupplyBeforeBurn = totalSupply();
        _burn(msg.sender, _shares);

        uint b = stakedToken().balanceOf(address(this));
        if (b < r) {
            uint _withdraw = r - b;
            strategy.withdraw(_withdraw);
            uint _after = stakedToken().balanceOf(address(this));
            uint _diff = _after - b;
            if (_diff < _withdraw) {
                r = b + _diff;
            }
        ...
        stakedToken().safeTransfer(msg.sender, r);

        // Transfer share% of profit to the user.
        // r/totalSupplyBeforeBurn should represent their portion of the pool.
        uint256 profitShare = FullMath.mulDiv(sideProfitToken().balanceOf(address(this)), r, totalSupplyBeforeBurn);
        sideProfitToken().safeTransfer(msg.sender, profitShare);
    }
