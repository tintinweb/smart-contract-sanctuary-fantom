...
contract BondReverse is ManagerRole, Ownable, Pausable {
    ...
    function returnPercentPrice(uint256 _newPrice) view public onlyManager returns (uint256 _percentDelta) {
        require(_newPrice != currentPrice, "BondReverse: The price are the same");
        uint256 _percentTotal = (_newPrice * 100000) / currentPrice / 1000;
        if(currentPrice > _newPrice) {
