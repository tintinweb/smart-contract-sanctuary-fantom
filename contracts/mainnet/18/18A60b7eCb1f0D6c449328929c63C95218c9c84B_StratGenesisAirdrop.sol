...
interface IZestStaking {
    ...
    //To notify the staking contract of airdrops and update the total supplies, and user balances
...
// Twitter: @ProtocolZest
// https://zestprotocol.fi
import "../interfaces/IZestStaking.sol";
import "../interfaces/IZestTreasury.sol";
...
contract StratGenesisAirdrop is Ownable {
    using SafeERC20 for IERC20;

    IZestTreasury public immutable treasury;
    IVe public immutable ve;
    IZestStaking public immutable staking;

    uint256 public minimumLock = 86400 * 121; // 4 months
    uint256 public maximumAmount = 100 ether;
    uint256 public lastAirdrop;

    constructor(
        IZestTreasury _treasury,
        IVe _ve,
        IZestStaking _staking
    ) {
        treasury = _treasury;
        ve = _ve;
        staking = _staking;
    ...
    /// @notice Airdrop ZSP to Batasm and Fantasm holders
    /// @param users Array of addresses to airdrop
    /// @param amount Amount of ZSP to lock for each user
    /// @param locktime Locking time of users veZest NFT

    function AirdropVeZest(
        address[] calldata users,
        uint256 amount,
        uint256 locktime
    ) external onlyOwner {
        require(address(ve) != address(0), "StratDistributeFee::AirdropVeZest:ve not set");
        require(
            users.length <= 100 && users.length > 1,
            "StratDistributeFee::AirdropVeZest:maximum or minimum users length"
        );
        require(locktime > minimumLock, "StratDistributeFee::AirdropVeZest:minimum locktime");
        require(amount <= maximumAmount, "StratDistributeFee::AirdropVeZest:maximum amount");
        uint256 total;

        total = amount * users.length;

        treasury.requestFund(ve.token(), total);
        IERC20(ve.token()).safeTransferFrom(address(treasury), address(this), total);
        IERC20(ve.token()).safeIncreaseAllowance(address(ve), total);
        for (uint256 i = 0; i < users.length; i++) {
            ve.create_airdrop_lock_for(amount, locktime, users[i]);
        }
        staking.notifyAirdrop(users, amount, locktime);
        lastAirdrop = block.timestamp;
        emit Airdropped(users.length, amount, total, locktime);
    }

    // EVENTS
    event Airdropped(uint256 users, uint256 amountPerUser, uint256 totalAmount, uint256 locktime);
}