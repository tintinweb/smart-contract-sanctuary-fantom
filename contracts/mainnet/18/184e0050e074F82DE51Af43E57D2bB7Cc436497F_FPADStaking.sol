...
contract FPADStaking is Ownable {
    ...
    function userInfo(address _addr) external view returns(address userAddress, uint256 deposit_time, uint256 deposits, uint256 payouts, uint256 total_ref_bonus, uint256 total_payout) {
        (uint256 _payout) = payoutOf(_addr);
        return (users[_addr].userAddress, users[_addr].deposit_time, users[_addr].deposits, users[_addr].payouts, users[_addr].total_ref_bonus, _payout);
    }
