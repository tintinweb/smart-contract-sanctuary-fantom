...
interface IERC20 {
     ...
     * Returns a boolean value indicating whether the operation succeeded.
     *
     * IMPORTPSHARE: Beware that changing an allowance with this method brings the risk
...
library Address {
     ...
     *
     * IMPORTPSHARE: because control is transferred to `recipient`, care must be
...
library SafeERC20 {
     ...
     * @param data The call data (encoded using abi.encode or one of its varipshares).
     */
...
contract PsharesRewardPoolReserved {
    using SafeMath for uint256;
    ...
    uint256 public PsharePerSecond = 0.0013476 ether; // 42500
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_pending > 0) {
            ...
            if(pool.withdrawFeeBP > 0){
                ...
                safePshareTransfer(feeAddress, withdrawFee);
            }
            userLeft = userLeft.sub(withdrawFee);
            require(userLeft > 0, "withdraw: not good!");
            safePshareTransfer(_sender, userLeft);
            emit RewardPaid(_sender, _pending);
        }
