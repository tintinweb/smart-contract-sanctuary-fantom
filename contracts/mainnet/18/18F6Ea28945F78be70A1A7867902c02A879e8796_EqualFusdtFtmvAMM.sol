...
// File: contracts/EqualFusdtFtmvAMM.sol
...
contract EqualFusdtFtmvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public wrappedToFusdtPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToFusdtPath
    ) {
        ...
        for (uint i; i < _wrappedToFusdtPath.length; ++i) {
            wrappedToFusdtPath.push(_wrappedToFusdtPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToFusdtPath, address(this), block.timestamp);
        
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(fusdt).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, fusdt, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
