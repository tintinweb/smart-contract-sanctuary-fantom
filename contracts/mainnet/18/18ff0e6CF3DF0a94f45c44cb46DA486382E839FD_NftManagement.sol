    ...
    INftGlobal
{
    ...
    function createNft(NftInput memory nftInput) external returns (uint256) {
        if (LISTING_FEE > 0)
            erc20Token.transferFrom(_msgSender(), globalAddress, LISTING_FEE);
        return erc721Token.safeMint(_msgSender(), nftInput);
    }

    function createBatchNft(NftInput[] memory nftInputs)
        external
        returns (uint256[] memory)
    {
        uint256 feeAmount = nftInputs.length * LISTING_FEE;
        if (LISTING_FEE > 0)
...
interface IMeSeaNft is INftGlobal {
