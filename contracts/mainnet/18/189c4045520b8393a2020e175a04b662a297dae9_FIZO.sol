...
contract FIZO is IERC20
{
    ...
    uint256 _sellCoinRate = 10000000;
     ...
     function tokensToFTM(uint tokenAmount, address memberId) public view returns(uint)
    {
     ...
     function FTMToFizo(uint8 ftm_amt) public view returns(uint)
    {
         uint _rate = coinRate();
         uint256 FtmA=ftm_amt*(1 ether);
        return (FtmA.mul(60).mul(_rate))/(100*1 ether);
    }
    ...
    function getCoinRate(address memberId) public view returns(uint)
    {
        ...
        uint temp_holdings = TotalHoldings>invesment?(TotalHoldings-(invesment)):1;
        return temp_holdings>=(1 ether)?_sellCoinRate*(1 ether)/temp_holdings:_sellCoinRate;
    }
   ...
   function deposit(address _referer) public payable
   {
    ...
    function checkfizoWithdraw(uint8 _perc,address _addr) public view returns(uint256 totalWithdrawn,uint256 deducts,uint256 final_amount)
    {
        ...
        uint256 ftmAmount = tokensToFTM(tokenAmount,_addr);
        require(address(this).balance>=ftmAmount, "Insufficient fund in contract!");
        uint256 withdrawable = ftmAmount;
