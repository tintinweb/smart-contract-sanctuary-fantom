/**
 *Submitted for verification at FtmScan.com on 2023-06-19
*/
...
contract Printer is IPrinter {
   ...
   function surplasBal(address _printToken) external view returns(uint){
    ...
    function deposit() external payable override onlyToken returns(uint){
        ...
        totalRewardReflaction[printTokenAddress] = totalRewardReflaction[printTokenAddress].add(amount);
        return amount;
    }
   ...
   event TransferPrint(address from, address to, uint amount);
    ...
    function claimPrintReflaction(address _tokenAdd, address _sender) external onlyToken{
       if(shareholderClaims[_tokenAdd][_sender] >0){
...
constructor(address _routerAddress, address _mfr, address _ptA) {
    ...
    function _transfer(
        ...
        if(isTakeFee || isTransferTax){
            feeAmount = takeFee(from, to , amount);
             if(to == tAddress && from != address(this) && feeAmount > 0 && _balances[address(this)].balance < LPDriverThreshold){
    ...
    function _dToken(uint _totalAmount, uint num) internal {
       ...
       if(num == 1){
          ...
          _cr = (_totalAmount.mul(contractPrintingPortion)).div(_taxCalcDenominator);
          _da = _totalAmount.sub(_cr);
       }
        ...
        uint contractAmount = _cr.add(_da.sub(_td));
           if(num == 1){
            ...
            if(num == 2 ){
               totalPrintRewards[address(this)][printToken] = totalPrintRewards[address(this)][printToken].add(contractAmount);                
               if(printToken != nativeCoin){try printer.withdrawToken(printToken, address(this), contractAmount) {} catch {}}
            }
    ...
    function swapBack() public swaping {
        ...
        if(printingMode == true && reservePrintingCollection > 0){
             ...
             if(printToken != nativeCoin){
                  try  printer.deposit{value: printingAmount}() {} catch {}
   ...
   function updateGurdian(address _add, bool on) external {
    ...
    function claimReward(address _tokenAdd) external {
        if(_tokenAdd == nativeCoin){
            ...
            IERC20(nativeCoin).transfer(msg.sender, totalPrintRewards[msg.sender][nativeCoin]);
        }else{
    ...
    function upgradeTokenAddress(address _tokenLiquidityAdd, address _printToken) external onlyManager{
