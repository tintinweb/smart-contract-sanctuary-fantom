 ...
 * out quoting reference 'NyPtvFantomWftmBooSpookyV2StrategyToWftm'
 *
...
contract NyPtvFantomWftmBooSpookyV2StrategyToWftm is NYBaseProfitTakingStrategyV1 {
  ...
  /**
   * @dev Tokens used
   * {BOO}        Farm emission token sold to {WFTM}
   * {WFTM}       Vault reward token, also used for fees.
   ...
   *      2. Swap from {BOO} to {WFTM}
   *      3. Charge fees.
   *      4. Send remaining {WFTM} to vault.
   */
   ...
   *      Swaps farm emission token to {WFTM}.
   */
  function _swapFarmEmissionTokens() internal {
    ...
    if (booToWftmPath.length < 2 || booBalance == 0) {
      return;
    }
  ...
  /**
   * @dev Core harvest function.
   *      Sends {WFTM} to the vault (with accounting)
   */
