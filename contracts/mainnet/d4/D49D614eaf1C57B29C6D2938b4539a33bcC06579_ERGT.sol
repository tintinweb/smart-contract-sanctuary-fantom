...
contract ERGT is Context, IERC20, Ownable {
    ...
    function _getTValues(uint256 tAmount) private view returns (TValues memory) {
        ...
        tvals_struct.tFee = tAmount.mul(_taxFee).div(100);   // Tax portion (in tokens) of the transaction amount
        tvals_struct.tAdminFee = tAmount.mul(_adminFee).div(100); // Admin portion (in tokens) of the transaction amount
        tvals_struct.tChampFee = tAmount.mul(_champFee).div(100); // Champ portion (in tokens) of the transaction amount
        tvals_struct.tTeamFee = tAmount.mul(_teamFee).div(100); // Team portion (in tokens) of the transaction amount
    ...
    function _setMaxTxLimited(bool is_limited) external onlyAdmin() {
