...
contract equalizerUsdcWftmStrat is StratFeeManager {
    ...
    uint256 public yelPerSecond = 800000000000000; //TODO: DELETE THIS !!!!
    ...
    constructor(
        ...
        address _oneInchRouterv5
    ) StratFeeManager(_commonAddresses) {
        want = _want;
        chef = _chef;
        oneInchRouterv5 = _oneInchRouterv5;
    ...
    function beforeDeposit(bytes calldata _data) external virtual {
        if (harvestOnDeposit) {
    ...
    function _harvest(bytes calldata _data) internal whenNotPaused {
        IMasterChef(chef).getReward();
        (uint rewardsUsdc, uint rewardsNative) = IMasterChef(want).claimFees();
        uint256 rewardsTokenTwoBal = IERC20(outputTwo).balanceOf(address(this));
        if (rewardsTokenTwoBal > 0 || rewardsUsdc > 0 || rewardsNative > 0) {
            chargeFees(rewardsTokenTwoBal, rewardsNative, rewardsUsdc, _data);
            transferToYeller();
        }
    }

    // performance fees
    function chargeFees(uint _rewardsTokenTwoBal, uint _rewardsNative, uint _rewardsUsdc, bytes calldata _data) internal {
        IFeeConfig.FeeCategory memory fees = getFees();
        
        swapRewardsToYel(_rewardsTokenTwoBal, _rewardsNative, _rewardsUsdc, _data);
    ...
    function swapRewardsToYel(uint _rewardsTokenTwoBal, uint _rewardsNative, uint _rewardsUsdc, bytes memory _data) internal {
        if(_rewardsTokenTwoBal > 0) {
            (bool success, ) = oneInchRouterv5.call(_data);
            require(success, "Strat: Rewards swap failed");
        }

        if(_rewardsUsdc > 0) {
            address[] memory tokenOneToYel = new address[](3);
            tokenOneToYel[0] = outputOne;
            tokenOneToYel[1] = native;
            tokenOneToYel[2] = yel;

            IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                _rewardsUsdc, 0, tokenOneToYel, address(this), block.timestamp
            );
        }
        ...
        if(_rewardsNative > 0) {
            ...
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                _rewardsNative, 0, nativeToYel, address(this), block.timestamp
            );
        }
    }
    ...
    function transferToYeller() internal whenNotPaused {
        ...
        if (yelBal > 0) {
            yelRewards = yelBal;
            yelPerSecond = yelRewards / (block.timestamp - lastHarvest);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(outputOne).safeApprove(oneInchRouterv5, type(uint).max);
        IERC20(outputTwo).safeApprove(unirouter, type(uint).max);
        IERC20(native).safeApprove(unirouter, type(uint).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(chef, 0);
        IERC20(outputOne).safeApprove(oneInchRouterv5, 0);
        IERC20(outputTwo).safeApprove(unirouter, 0);
