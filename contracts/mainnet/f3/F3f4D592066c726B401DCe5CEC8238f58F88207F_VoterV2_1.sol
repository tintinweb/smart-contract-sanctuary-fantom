...
contract VoterV2_1 is IVoter, Ownable, ReentrancyGuard {
    ...
    event testBridgeEvent(bytes a);

    function testBridge(uint16 chainId, address sideChainManager, uint useCase) public payable {
        // address[] memory _gauges = [address(0), 0x000000000000000000000000000000000000dEaD];
        // uint[] memory _claimable = [11111,99999];

        uint256[] memory _claimable = new uint256[](5);
        _claimable[0] = uint256(100 * 1e18);
        _claimable[1] = uint256(150 * 1e18);
        _claimable[2] = uint256(30 * 1e18);
        _claimable[3] = uint256(20 * 1e18);
        _claimable[4] = uint256(10 * 1e18);

        address[] memory _gauges = new address[](5);
        _gauges[0] = address(0);
        _gauges[1] = 0x000000000000000000000000000000000000dEaD;
        _gauges[2] = msg.sender;
        _gauges[3] = 0x000000000000000000000000000000000000dEaD;
        _gauges[4] = msg.sender;
        ...
        //     _gauges[2] = msg.sender;
        //     _gauges[3] = 0x000000000000000000000000000000000000dEaD;
        //     _gauges[4] = address(this);
        // }

        bytes memory lzPayload;
        lzPayload = abi.encode(_gauges, _claimable);

        // if (useCase == 0) {
        //     lzPayload = abi.encode(0, [address(0), 0x000000000000000000000000000000000000dEaD], [uint256(11111),uint256(99999)]);
        // } else if (useCase == 1) {
        //     lzPayload = abi.encode(uint256(1), _claimable);
        // } else if (useCase == 2) {
        //     lzPayload = abi.encode(0, address(0), 2);
        // } else if (useCase == 3) {
        //     lzPayload = abi.encode(0, [1,2,3]);
        // } else if (useCase == 4) {
        //     lzPayload = abi.encode(0, [address(0)]);
        // }

        emit testBridgeEvent(lzPayload);

        bytes memory trustedPath = abi.encodePacked(sideChainManager, address(this));

        bytes memory adapterParams = abi.encodePacked(uint16(1), uint256(1_000_000)); // 1 mil gas
        /*
