...
contract C7usdc is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 5000 ether;
    // Initial distribution for the day 2-5 C7USDC-WFTM LP -> C7USDC pool
    // uint256 public constant INITIAL_C7USDC_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the C7USDC ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("C7USDC", "C7USDC") {
        // Mints 1 C7USDC to contract creator for initial pool setup
    ...
    function _getC7usdcPrice() internal view returns (uint256 _c7usdcPrice) {
        try IOracle(c7usdcOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("C7USDC: failed to fetch C7USDC price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _c7usdcPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_c7usdcPrice >= taxTiersTwaps[tierId]) {
    ...
    function setC7usdcOracle(address _c7usdcOracle) public onlyOperatorOrTaxOffice {
        require(_c7usdcOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints C7USDC to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of C7USDC to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentC7usdcPrice = _getC7usdcPrice();
            currentTaxRate = _updateTaxRate(currentC7usdcPrice);
            if (currentC7usdcPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        //require(_c7usdcPool != address(0), "!_c7usdcPool");
        require(_daoFund != address(0), "!_treasury");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        //_mint(_c7usdcPool, INITIAL_C7USDC_POOL_DISTRIBUTION);
