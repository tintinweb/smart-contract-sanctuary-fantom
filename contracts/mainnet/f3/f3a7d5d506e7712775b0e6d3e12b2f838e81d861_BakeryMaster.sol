...
contract BakeryMaster is Ownable {
    ...
    uint256 public maxBakePerBlock;
    ...
    constructor(
        ...
        maxBakePerBlock = _maxBakePerBlock;
    }
    ...
    // (_from,_to]
    function setBakePerBlock(uint256 _newBakePerBlock) public onlyOwner {
    bakePerBlock = _newBakePerBlock;
}

    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
    uint256 bp = bakePerBlock;
    if (_to <= bonusEndBlock) {
        return _to.sub(_from).mul(bp).mul(BONUS_MULTIPLIER);
    } else if (_from >= bonusEndBlock) {
        return _to.sub(_from).mul(bp);
    } else {
        uint256 multiplierBeforeBonus = bonusEndBlock.sub(_from).mul(bp);
        uint256 multiplierAfterBonus = _to.sub(bonusEndBlock).mul(bp);
        return multiplierBeforeBonus.mul(BONUS_MULTIPLIER).add(multiplierAfterBonus);
    }
}
    ...
    // Calculate pending rewards if a new reward is available
    if (currentBlock > pool.lastRewardBlock && lpSupply != 0) {
        uint256 multiplier = currentBlock.sub(pool.lastRewardBlock);
        uint256 bakeReward = multiplier.mul(bakePerBlock).mul(pool.allocPoint).div(totalAllocPoint).mul(10**18);
    ...
    return userInfo.amount.mul(accBakePerShare).div(accBakePerShareMultiple).sub(userInfo.rewardDebt).div(10**18);
}
    ...
    uint256 totalReward = multiplier.mul(bakePerBlock).mul(pool.allocPoint).div(totalAllocPoint).mul(10**18);
    uint256 farmerReward = totalReward.mul(90).div(100); // 90% goes to farmers
    uint256 burnReward = totalReward.mul(10).div(100); // 10% goes to burn
    uint256 devReward = totalReward.mul(30).div(100); // 30% goes to dev
    bake.mintTo(devAddr, devReward);
    bake.mintTo(address(this), totalReward);
    bake.transfer(address(0xdead), burnReward);
    pool.accBakePerShare = pool.accBakePerShare.add(farmerReward.mul(accBakePerShareMultiple).div(lpSupply));
    pool.lastRewardBlock = block.number;
}
