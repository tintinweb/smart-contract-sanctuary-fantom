...
contract MultiAssetTreasury is IMultiAssetTreasury, Operator, ReentrancyGuard {
    ...
    function globalCollateralBalance(uint256 _assetId) public view override returns (uint256) {
        ...
        if (address(investmentController) != address(0)) {
            investedBalance = investmentController.getInvestedCollateral(_assetId);
        }
...
abstract contract IInvestmentController {
    function getInvestedCollateral(uint256 _assetId) external view virtual returns (uint256);

    function getTotalProfit() external view virtual returns (uint256);

    function getAssetInvestmentProfit(uint256 _assetId) external view virtual returns (uint256);
}
