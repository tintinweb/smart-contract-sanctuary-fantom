...
contract TaxOracle is Ownable {
    ...
    constructor(
        ...
        require(_coin != address(0), "coin address cannot be 0");
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(coin), "token needs to be coin");
        uint256 coinBalance = coin.balanceOf(pair);
        uint256 wftmBalance = wftm.balanceOf(pair);
        return uint144(wftmBalance.mul(_amountIn).div(coinBalance));
    }

    function getCoinBalance() external view returns (uint256) {
	return coin.balanceOf(pair);
    }
    ...
    function getPrice() external view returns (uint256) {
        uint256 coinBalance = coin.balanceOf(pair);
        uint256 wftmBalance = wftm.balanceOf(pair);
        return wftmBalance.mul(1e18).div(coinBalance);
    }
