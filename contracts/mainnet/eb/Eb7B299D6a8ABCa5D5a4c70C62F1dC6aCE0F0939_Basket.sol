...
contract Basket is Ownable {
  ...
  function deposit(address account, address beefyzap, address vault, address router, address[] calldata path, uint256 routertype, uint256 iscoin) public payable {
    _deposit(account, beefyzap, vault, router, path, routertype, iscoin, msg.value);
  }

  function depositViaBridge(ILiquidCryptoBridge_v1.SwapVoucher memory voucher, uint256 fee, address account, address beefyzap, address vault, address router, address[] calldata path, uint256 routertype, uint256 iscoin) public {
    uint256 amount = ILiquidCryptoBridge_v1(bridge).withdrawForUser(voucher, fee);
    _deposit(account, beefyzap, vault, router, path, routertype, iscoin, amount);
  }
   ...
   */
  function _deposit(address account, address beefyzap, address vault, address router, address[] calldata path, uint256 routertype, uint256 iscoin, uint256 amount) private {
    if (iscoin == 0) {
      if (routertype == 1) {
  ...
  function withdraw(address account, address beefyzap, address vault, Swaper memory swper, uint256 amount) public {
    uint256 outAmount = _withdraw(beefyzap, vault, swper, amount);

    (bool success1, ) = account.call{value: outAmount}("");
    require(success1, "Failed to withdraw");
  }

  function withdrawViaBridge(address account, address beefyzap, address vault, Swaper memory swper, uint256 amount, uint256 voucherIndex, uint256 inChain, uint256 outChain, uint256 price, uint256 rate, uint256 fee) public {
    uint256 inAmount = _withdraw(beefyzap, vault, swper, amount);
  ...
  function _withdraw(address beefyzap, address vault, Swaper memory swper, uint256 amount) private returns(uint256) {
