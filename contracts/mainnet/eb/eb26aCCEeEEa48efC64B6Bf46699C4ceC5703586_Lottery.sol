...
contract Lottery is Ownable {
    ...
    mapping(uint256 => JackpotInfo) public jackpots;
    ...
    event PrizeWithdrawn(uint256 indexed jackpotId, address indexed user);

    // random number oracle address
    IQrngRandomNumber internal qrngContract;
    ...
    function setQrngAddress(address _qrngAddress) external onlyOwner {
        require(_qrngAddress != address(0), "invalid address");
        qrngContract = IQrngRandomNumber(_qrngAddress);
    }
    ...
    /**
     * @notice Select the winner based on the randomNumber and the amount deposited.
     * randomNumber is any number in uint256 range, so converting to 1 - 100 number
     */
    function selectWinner(uint256 randomNumber) private view returns (address) {
        ...
        uint256 newRandomNumber = randomNumber % 100 + 1;
        uint256 rnd = (newRandomNumber * runningTotal) / 100;
    ...
    function endJackpot(bytes32 qrngRequestId)
        external
        onlyOwner
        returns (address)
    {
        ...
        uint256 randomNumber = qrngContract.getRandomNumber(qrngRequestId);
    ...
    function withdrawPrize(uint256 jackpotId) external {
        ...
        emit PrizeWithdrawn(jackpotId, msg.sender);
    }
