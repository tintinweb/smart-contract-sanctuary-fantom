 ...
 *
 */
contract FantomUSDTYearnStrategy is YearnStrategy {
    ...
    function vaultAddress() override pure public returns(address) {
        ...
        return 0x148c05caf1Bb09B5670f00D511718f733C54bC4c;
    }
    ...
    function uniswapFactoryAddress() public pure virtual override returns (address) {

        return 0x152eE697f2E276fA89E96742e9bB9aB1F2E61bE3;
    }

    function uniswapRouterAddress() public pure virtual override returns (address) {
...
contract YearnStrategy is IStrategy {
    ...
    function farm(address erc20Token_, uint256 amount_) public override returns (uint256) {
        ...
        address vta = vaultTokenAddress();
        uint256 vaultTokenAmount = amount_;
        if (erc20Token_ != vta) {
            uint256 amountBefore = IERC20(vta).balanceOf(address(this));
            _swap(erc20Token_, vta, address(this), amount_);
            vaultTokenAmount = IERC20(vta).balanceOf(address(this)) - amountBefore;
        }
        IERC20(vta).safeApprove(vaultAddress(), vaultTokenAmount);
    ...
    function takeReward(address to_, address expectedToken_, uint256 amount_) public override {
        ...
        if (tokenAddress == expectedToken_) {
            ...
            uint256 amount = VaultInterface(vaultAddress()).withdraw(share, address(this));
            _swap(tokenAddress, expectedToken_, to_, amount);
        }
    }
    ...
    function _swap(address fromToken_, address toToken_, address to_, uint256 amount_) private {
        ...
        require(IUniswapV2Factory(uniswapFactoryAddress()).getPair(fromToken_, toToken_) != address(0x0), "There is no pair");
        IERC20(fromToken_).safeApprove(uniswapRouterAddress(), amount_);
