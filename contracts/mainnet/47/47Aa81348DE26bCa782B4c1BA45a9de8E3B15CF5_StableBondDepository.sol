...
contract StableBondDepository is Ownable {
  ...
  function setBondTerms(PARAMETER _parameter, uint256 _input) external onlyPolicy {
    ...
    } else if (_parameter == PARAMETER.MINIMUN_PRICE) {
      // 4
  ...
  function deposit(
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) IMP
    */
    ...
    if (fee != 0) {
      ...
      IERC20(IMP).safeTransfer(DAO, fee);
    }
  ...
  function redeem(address _recipient, bool _stake) external returns (uint256) {
    ...
    if (percentVested >= 10000) {
      ...
      lastBlock: block.number, pricePaid: info.pricePaid});
   ...
   *  @notice allow anyone to send 
    tokens (excluding principle or IMP) to the DAO
   *  @return bool
   */
