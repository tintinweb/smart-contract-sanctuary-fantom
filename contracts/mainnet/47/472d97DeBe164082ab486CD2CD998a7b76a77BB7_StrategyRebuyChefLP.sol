...
contract StrategyRebuyChefLP is StratManager, FeeManager, GasThrottler {
    ...
    event ChargedFees(uint256 callFees, uint256 rebuyAmount, uint256 burnAmount);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 rebuyAmount = nativeBal.mul(buybackAmount).div(MAX_FEE);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(rebuyAmount, 0, nativeToBuybackRoute, address(this), now);
        uint256 buybackTokenBalance = IERC20(rebuyToken).balanceOf(address(this));
        IERC20(native).safeTransfer(BURN_ADDR, buybackTokenBalance);

        emit ChargedFees(callFeeAmount, rebuyAmount, buybackTokenBalance);
    }
    ...
    function nativeToBuyback() external view returns (address[] memory) {
        return nativeToBuybackRoute;
    }
}
...
abstract contract FeeManager is StratManager {
    ...
    uint constant public BUYBACK_AMOUNT = 111;
    uint constant public BUYBACK_CAP = 50;

    uint public withdrawalFee = 10;

    uint public callFee = 111;
    uint public buybackAmount = 111;
    uint public beefyFee = MAX_FEE - STRATEGIST_FEE - callFee - buybackAmount;

    function setCallFee(uint256 _fee) public onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");
        
        callFee = _fee;
        beefyFee = MAX_FEE - STRATEGIST_FEE - callFee - buybackAmount;
    }
    ...
    function setBuybackAmount(uint256 _fee) public onlyManager {
        require(_fee <= BUYBACK_CAP, "!cap");
