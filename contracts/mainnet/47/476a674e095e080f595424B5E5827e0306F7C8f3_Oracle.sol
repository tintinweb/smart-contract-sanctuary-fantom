...
contract Oracle is AccessControl {
    ...
    event SetThreshold(uint256 oldValue, uint256 newValue);
    ...
    constructor(
        ...
        require(
            address(dei_) != address(0),
            "Oracle: Dei address cannot be zero"
        );
        require(
            address(deus_) != address(0),
        ...
        require(admin != address(0), "Oracle: Admin address cannot be zero");
        require(setter != address(0), "Oracle: Setter address cannot be zero");
    ...
    function setThreshold(uint256 threshold_) external isSetter {
        emit SetThreshold(threshold, threshold_);
        threshold = threshold_;
    }
    ...
    function getOnChainPrice() public view returns (uint256) {
        ...
        uint256 deiBalance = dei.balanceOf(address(pair));
        uint256 deusBalance = deus.balanceOf(address(pair));
        uint256 deusPrice = IBaseV1Pair(address(pair)).getAmountOut(1e18, address(deus));

        return
            (((deiBalance * 1e18) + (deusBalance * deusPrice)) * deiPrice) /
            1e18 /
            pair.totalSupply();
    }
    ...
    function getPrice(
        ...
        require(1e18 - diff < threshold, "ORACLE: PRICE_GAP");
