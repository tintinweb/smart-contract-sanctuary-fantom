...
// Note that this pool has no minter key of darko (rewards).
// Instead, the governance will call darko distributeReward method and send reward to this pool at the beginning.
contract GenesisMultiRewardPool is IMultiRewardPool, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. darkos to distribute in the pool.
        uint256 lastRewardTime; // Last time that darkos distribution occurred.
        uint256 accDarkoPerShare; // Accumulated darkos per share, times 1e18. See below.
    ...
    // The time when darko mining starts.
    ...
    uint256 public constant REWARD_RATE_DBOO = 900; // 1 FTM = 0.09 BOO
    uint256 public constant REWARD_RATE_DUAH = 98000; // 1 FTM = 9.80 UAH
        ...
        returns (address[] memory _rewards)
    {
        ...
        _rewards[0] = address(darko);
        _rewards[1] = address(dboo);
        _rewards[2] = address(duah);
    }
    ...
    // Update the given pool's darko allocation point. Can only be called by the owner.
    ...
    // Update the given pool's darko locked time. Can only be called by the owner.
    ...
    // View function to see pending darkos on frontend.
    function pendingReward(uint256 _pid, address _user)
        public
        view
        override
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _darkoReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accDarkoPerShare = accDarkoPerShare.add(
                _darkoReward.mul(1e18).div(tokenSupply)
            );
        }
    ...
    {
        uint256 _darkoReward = pendingReward(_pid, _user);
        _pendingMultiRewardArr = new uint256[](3);
        _pendingMultiRewardArr[0] = _darkoReward;
        _pendingMultiRewardArr[1] = _darkoReward.mul(REWARD_RATE_DBOO).div(
            10000
        );
        _pendingMultiRewardArr[2] = _darkoReward.mul(REWARD_RATE_DUAH).div(
            10000
        );
    }
    ...
    {
        uint256 _darkoTotalReward = pendingAllRewards(_user);
        _totalMultiRewardArr = new uint256[](3);
        _totalMultiRewardArr[0] = _darkoTotalReward;
        _totalMultiRewardArr[1] = _darkoTotalReward.mul(REWARD_RATE_DBOO).div(
            10000
        );
        _totalMultiRewardArr[2] = _darkoTotalReward.mul(REWARD_RATE_DUAH).div(
            10000
        );
    }
    ...
    function _claimReward(address _account, uint256 _darkoAmt) internal {
        uint256 _dbooAmt = _darkoAmt.mul(REWARD_RATE_DBOO).div(10000);
        uint256 _duahAmt = _darkoAmt.mul(REWARD_RATE_DUAH).div(10000);
        _safeTokenTransfer(darko, _account, _darkoAmt);
        _safeTokenTransfer(dboo, _account, _dbooAmt);
        _safeTokenTransfer(duah, _account, _duahAmt);
        emit RewardPaid(_account, _darkoAmt, _dbooAmt, _duahAmt);
    }

    // Safe darko transfer function, just in case if rounding error causes pool to not have enough darkos.
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < epochEndTimes[3] + 90 days) {
            ...
            require(
                _token != darko && _token != dboo && _token != duah,
                "reward"
            );
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
