...
contract PadiSwap is ERC20("Padi Swap", "PADI"), Ownable {
    uint256 private _cap = 230000000e18;
...
// File: contracts/PadiMasterFarmer.sol
...
interface IMigratorToPadiSwap {
    // Perform LP token migration from legacy UniswapV2 to PadiSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // PadiSwap must mint EXACTLY the same amount of PadiSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// PadiMasterFarmer is the master of LUA. He can make LUA and he is a fair guy.
//
...
contract PadiMasterFarmer is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accPadiPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPadiPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accPadiPerShare; // Accumulated LUAs per share, times 1e12. See below.
    }

    // The LUA TOKEN!
    PadiSwap public padi;
    // Dev address.
    ...
    IMigratorToPadiSwap public migrator;
    ...
    event SendPadiReward(address indexed user, uint256 indexed pid, uint256 amount, uint256 lockAmount);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate) public onlyOwner {
        require(poolId1[address(_lpToken)] == 0, "PadiMasterFarmer::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setMigrator(IMigratorToPadiSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 padiForDev;
        uint256 padiForFarmer;
        (padiForDev, padiForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        
        if (padiForDev > 0) {
            padi.mint(devaddr, padiForDev);
            //For more simple, I lock reward for dev if mint reward in bonus time
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                padi.lock(devaddr, padiForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            }
        }
        padi.mint(address(this), padiForFarmer);
        pool.accPadiPerShare = pool.accPadiPerShare.add(padiForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer) {
        ...
        uint256 padiCanMint = padi.cap().sub(padi.totalSupply());

        if (padiCanMint < amount) {
            forDev = 0;
            forFarmer = padiCanMint;
        }
        else {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accPadiPerShare = pool.accPadiPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            uint256 padiForFarmer;
            (, padiForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
            accPadiPerShare = accPadiPerShare.add(padiForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accPadiPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accPadiPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = padi.balanceOf(address(this));
            ...
            if(pending > 0) {
                padi.transfer(msg.sender, pending);
                uint256 lockAmount = 0;
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    lockAmount = pending.mul(PERCENT_LOCK_BONUS_REWARD).div(100);
                    padi.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendPadiReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accPadiPerShare).div(1e12);
        }
    }

    // Deposit LP tokens to PadiMasterFarmer for LUA allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_amount > 0, "PadiMasterFarmer::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from PadiMasterFarmer.
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "PadiMasterFarmer::withdraw: not good");
    ...
    // Safe padi transfer function, just in case if rounding error causes pool to not have enough LUAs.
    function safePadiTransfer(address _to, uint256 _amount) internal {
        uint256 padiBal = padi.balanceOf(address(this));
        if (_amount > padiBal) {
            padi.transfer(_to, padiBal);
        } else {
            padi.transfer(_to, _amount);
        }
    }
