 ...
 */
// Note that this pool has no minter key of TP (rewards).
// Instead, the governance will call TP distributeReward method and send reward to this pool at the beginning.
contract TPGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. TP to distribute.
        uint256 lastRewardTime; // Last time that TP distribution occurs.
        uint256 accTombPerShare; // Accumulated TP per share, times 1e18. See below.
    ...
    // The time when TP mining starts.
    uint256 public poolStartTime;

    // The time when TP mining ends.
    uint256 public poolEndTime;

    uint256 public tombPerSecond = 0.28935 ether; // 50000 TP / (48h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "TPGenesisPool: caller is not the operator!");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "TPGenesisPool: existing pool?");
        }
    }
    ...
    // Update the given pool's TP allocation point. Can only be called by the owner.
    ...
    // View function to see pending TP on frontend.
    ...
    // Safe TP transfer function, just in case if rounding error causes pool to not have enough TPs.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (TP or lps) if less than 90 days after pool ends
            require(_token != tomb, "tp");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
