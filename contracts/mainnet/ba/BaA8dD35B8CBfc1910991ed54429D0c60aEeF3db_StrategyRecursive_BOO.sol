...
import {RecursiveBooStrategy} from "../../spookyswap/RecursiveBoo.sol";

contract StrategyRecursive_BOO is RecursiveBooStrategy {
    ...
    function initializeStrategy(
        ...
        address aceLab = address(0x2352b745561e7e6FCD03c093cE7220e3e126ace0);
        address boo = address(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE);
        address xboo = address(0xa48d959AE2E88f1dAA7D5F611E01908106dE7598);
        address wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
        address fs = address(0xC758295Cd1A564cdb020a78a681a838CF8e0627D);
        ...
        address bftm_boo_lp = address(0x501e27B8b14C20e9C3D866E67b4fD11A51d55aE0);
        __Strategy_init(
            _store,
            boo,
            _vault,
        ...
        routes[wftm] = [fs, usdc, wftm];
        routes[boo] = [fs, usdc, wftm, boo];
    }
}
...
/// @title Recursive BOO Maximizer.
/// @author Chainvisions
/// @notice A maximizer strategy that farms on Spookyswap's MirrorWorld.

contract RecursiveBooStrategy is BaseUpgradeableStrategy {
    ...
    /// @param _mirrorWorld xBOO/Mirror World contract for staking BOO tokens.
    ...
    /// @notice Rotates into a different xBOO pool.
    /// @param _pid Pool ID to rotate into.
    /// @param _rewardToken Reward token that the pool pays out in.
    function rotatePools(uint256 _pid, address _rewardToken) external onlyGovernance {
        // Withdraw from the current staked pool and liquidate rewards.
        IMasterChef(rewardPool()).withdraw(poolId(), _rewardPoolStake());
        _liquidateReward();
        _notifyMaximizerRewards();

        // Set new pools and rewards.
        _setPoolId(_pid);
        _setRewardToken(_rewardToken);

        // Rotate into the new pool.
        _investAllUnderlying();
    }
    ...
    function withdrawAllToVault() public restricted {
        ...
        // Withdraw BOO tokens from xBOO.
        ITokenBar(mirrorWorld()).leave(IERC20(mirrorWorld()).balanceOf(address(this)));
    ...
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > IERC20(underlying()).balanceOf(address(this))){
            ...
            IMasterChef(rewardPool()).withdraw(poolId(), Math.min(_rewardPoolStake(), amountToBurn));
            ITokenBar(mirrorWorld()).leave(amountToBurn);
        }
    ...
    /// @notice Sets a liquidation route for the strategy.
    /// @param _tokenTo Token to liquidate to.
    /// @param _route Route for liquidating the token. 
    function setRoute(
        address _tokenTo,
    ...
    function unsalvagableTokens(address token) public view returns (bool) {
        return (token == underlying() || token == rewardToken() || token == mirrorWorld());
    }

    /// @notice Spookyswap's xBOO/Mirror World contract.
    function mirrorWorld() public view returns (address) {
        return _getAddress("mirrorWorld");
    }
    ...
    function _investAllUnderlying() internal onlyNotPausedInvesting {
        // Variables written to memory. Used for optimizing costs.
        address boo = underlying();
        address xboo = mirrorWorld();
        address pool = rewardPool();

        // Stake into the MirrorWorld and WFTM reward pool.
        uint256 underlyingBalance = IERC20(boo).balanceOf(address(this));
        if(underlyingBalance > 0) {
            IERC20(boo).safeApprove(xboo, 0);
            IERC20(boo).safeApprove(xboo, underlyingBalance);
            ITokenBar(xboo).enter(underlyingBalance);

            uint256 xbooBalance = IERC20(xboo).balanceOf(address(this));

            IERC20(xboo).safeApprove(pool, 0);
            IERC20(xboo).safeApprove(pool, xbooBalance);
            IMasterChef(pool).deposit(poolId(), xbooBalance);
        }
    }
    ...
    function _liquidateReward() internal {
        ...
        if(isLP()) {
            ...
            address[] memory token0Route = routes[token0];
            address[] memory token1Route = routes[token1];

            if(token0Route.length > 1) {
                uint256[] memory amounts = IUniswapV2Router02(router).swapExactTokensForTokens(toToken0, 0, token0Route, address(this), (block.timestamp + 600));
            ...
            if(token1Route.length > 1) {
                uint256[] memory amounts = IUniswapV2Router02(router).swapExactTokensForTokens(toToken1, 0, token1Route, address(this), (block.timestamp + 600));
    ...
    function _setMirrorWorld(address _value) internal {
        _setAddress("mirrorWorld", _value);
    }

    function _setIsLP(bool _value) internal {
        _setBool("isLP", _value);
    }
    ...
    function _setSwapRouter(address _value) internal {
        _setAddress("swapRouter", _value);
    }
    ...
    function _getRatio() internal view returns (uint256 ratio) {
        ratio = (IERC20(underlying()).balanceOf(mirrorWorld()) * 1e18) / IERC20(mirrorWorld()).totalSupply();
    }
}
