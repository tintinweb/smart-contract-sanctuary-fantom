...
import {ReaperAPI} from "../../interfaces/adapters/yearn/ReaperAPI.sol";
...
contract ReaperWrapper is ERC20, IVaultWrapper, IERC4626 {
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    {
        console.log(assets);
        console.log(yVault.totalSupply());
        console.log(yVault.balance());
        return assets * yVault.totalSupply() / yVault.balance(); 
    }
    // upshift, this one has the precision problem, always needs to add + 1 wei.
    function convertToAssets(uint256 shares)
        ...
        returns (uint assets)
    {        
        return shares * yVault.getPricePerFullShare() / (10 ** 18 - _decimals);
    }
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function maxRedeem(address owner) external view override returns (uint256) {
     ...
     function _deposit(
        ...
        uint allowance = _token.allowance(address(this), address(yVault));
        if (allowance < amount) {
            if (allowance > 0) {
                SafeERC20.safeApprove(_token, address(yVault), 0);
            }
            SafeERC20.safeApprove(_token, address(yVault), type(uint256).max);
        }
        ...
        uint256 beforeShareBal = yVault.balanceOf(address(this));
        yVault.deposit(amount);
        deposited = beforeTokenBal - _token.balanceOf(address(this));
        mintedShares = yVault.balanceOf(address(this)) - beforeShareBal;
        // afterDeposit custom logic
        _mint(receiver, mintedShares);

        // `receiver` now has shares of `yVault` as balance, converted to `token` here
    ...
    function _withdraw(
        ...
        uint256 availableShares = this.balanceOf(sender);

        if (availableShares == 0) revert NoAvailableShares();
        uint256 estimatedMaxShares = previewWithdraw(amount);
        ...
        uint256 beforeShareBal = yVault.balanceOf(address(this));
        uint256 beforeTokenBal = _token.balanceOf(address(this));
        yVault.withdraw(estimatedMaxShares);
        burntShares = beforeShareBal - yVault.balanceOf(address(this));
        withdrawn = _token.balanceOf(address(this)) - beforeTokenBal;
        ...
        if (unusedShares > 0)
            SafeERC20.safeTransfer(IERC20(address(yVault)), sender, unusedShares);
