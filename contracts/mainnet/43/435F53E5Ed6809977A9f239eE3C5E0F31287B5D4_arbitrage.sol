...
contract arbitrage is Withdrawable{
    ...
    function trigger(uint amount, address buy, address sell, address buyLP, address sellLP, address tokenIn, address tokenOut, uint _feeBuy, uint _feeSell) public  returns(bool isComplete){
 //      require(check(buyLP, sellLP, _feeBuy, _feeSell) == true, "call failure!");
        ...
        IERC20(tokenIn).safeApprove(address(this), type(uint256).max);
 //     uint start_amount = _getAmount(buyLP, sellLP, _feeBuy, _feeSell); 
    ...
    function _sell(uint amount, address addr, address token0, address token1) internal returns(uint amountB){
        _swapExactTokensForTokens(amount, 0, _getPath(token0, token1), address(msg.sender), block.timestamp.add(60), addr);
