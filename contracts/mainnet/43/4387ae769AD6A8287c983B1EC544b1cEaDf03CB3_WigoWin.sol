...
contract WigoWin is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant MAX_REFERRER_FEE = 250; // 2.5%
    ...
    event NewReferrerFee(uint256 indexed epoch, uint256 referrerFee);
    ...
    constructor(
        ...
        require(_referrerFee <= MAX_REFERRER_FEE, "Referrer fee too high");
        require(
        ...
        notContract
    {
        require(
            ...
            "WigoGalaxy profile is not active"
        );
        ...
        if (reward > 0) {
            ...
            if (refIsActive && refAddress != address(0)) {
                _safeTransferFTM(
                    address(refAddress),
                    (reward * referrerFee) / 10000
                );
                _safeTransferFTM(
                    address(msg.sender),
                    (reward * (10000 - referrerFee)) / 10000
                );
            } else {
                _safeTransferFTM(
    ...
    function executeRound() external whenNotPaused onlyOperator {
        ...
        if (
            ...
            block.timestamp >
            rounds[currentEpoch - 1].closeTimestamp + bufferSeconds
        ) {
            _pauseAndUnpause();
        } else {
            (
    ...
    /**
     * @notice called by the admin to to pause, triggers stopped state then unpause, returns to normal state
     * Reset genesis state.
     */
    function _pauseAndUnpause() internal {
        _pause();
        ...
        onlyAdmin
    {
        ...
        emit NewReferrerFee(currentEpoch, referrerFee);
    }
        ...
        onlyAdmin
    {
        require(
            _wigoGalaxyAddress != address(0),
            "WigoGalaxy address cannot be zero"
        );
