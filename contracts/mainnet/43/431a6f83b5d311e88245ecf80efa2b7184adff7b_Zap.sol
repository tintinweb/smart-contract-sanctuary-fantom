...
library SafeERC20 {
    ...
    function safeTransfer(IERC20Redemption token, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(IERC20Redemption token, address from, address to, uint256 value) internal {
     ...
     */
    function safeApprove(IERC20Redemption token, address spender, uint256 value) internal {
    ...
    function safeIncreaseAllowance(IERC20Redemption token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender) + value;
        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(IERC20Redemption token, address spender, uint256 value) internal {
        {
     ...
     */
    function _callOptionalReturn(IERC20Redemption token, bytes memory data) private {
...
contract Zap is Ownable {
    ...
    using SafeERC20 for IERC20Redemption;
    ...
    function zapInToken(address _from, uint amount, address _to, address routerAddr, address _recipient) external {
        ...
        IERC20Redemption(_from).safeTransferFrom(msg.sender, address(this), amount);
    ...
    function zapAcross(address _from, uint amount, address _toRouter, address _recipient) external {
        ...
        IERC20Redemption(_from).safeTransfer(_from, amount);
        uint amt0;
        uint amt1;
    ...
    function zapOutToken(address _from, uint amount, address _to, address routerAddr, address _recipient) external {
        ...
        IERC20Redemption(_to).safeTransfer(_recipient, amt0.add(amt1));
    }
    ...
    function _approveTokenIfNeeded(address token, address router) private {
        if (IERC20Redemption(token).allowance(address(this), router) == 0) {
            IERC20Redemption(token).safeApprove(router, type(uint).max);
        }
    }
    ...
    function withdraw(address token) external onlyOwner {
        ...
        IERC20Redemption(token).transfer(owner(), IERC20Redemption(token).balanceOf(address(this)));
    }
