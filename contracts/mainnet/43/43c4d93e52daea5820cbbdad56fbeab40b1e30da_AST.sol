...
contract AST is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_TEAM_FUND_DISTRIBUTION = 7160 ether;
    ...
    function _getAstPrice() internal view returns (uint256 _astPrice) {
        try IOracle(astOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Error: failed to fetch AST price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _astPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_astPrice >= taxTiersTwaps[tierId]) {
    ...
    function setAstOracle(address _astOracle) public onlyOperatorOrTaxOffice {
        require(_astOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentAstPrice = _getAstPrice();
            currentTaxRate = _updateTaxRate(currentAstPrice);
            if (currentAstPrice < burnThreshold) {
                burnTax = true;
            }
        }
