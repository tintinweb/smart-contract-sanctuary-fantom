...
contract FantomPool_R0 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, ERC165Upgradeable, IBeaconUpgradeable, IBeaconDeployer, IStakingPool {
    ...
    function stake() override external nonReentrant payable {
        require(msg.value >= minimumStake, "Value must be greater than min amount");
        IAnkrBond(bondContract).mintBondsTo(msg.sender, msg.value);
    ...
    function getBurnFee(uint256 amount) override public view returns (uint256) {
        uint256 ratio = 0;
        if (amount >= feeIncreaseStop) {
            ratio = maxFeeRatio;
        } else if (amount > feeIncreaseStart) {
            // note that in this branch feeIncreaseStop > feeIncreaseStart holds
            ratio = (minFeeRatio * (feeIncreaseStop - amount) + maxFeeRatio * (amount - feeIncreaseStart)) / (feeIncreaseStop - feeIncreaseStart);
        } else {
            ratio = minFeeRatio;
        }
    ...
    function burn(uint256 amount) override external nonReentrant {
        IAnkrBond(bondContract).burnBondsFrom(msg.sender, amount);

        uint256 shares = IAnkrBond(bondContract).balanceToShares(amount);
        ...
        emit TokensBurned(msg.sender, amount, shares, fee, lastWrId);

        lastWrId++;
    }

    function updateFeeParameters(uint256 _minFeeRatio, uint256 _maxFeeRatio, uint256 _feeIncreaseStart, uint256 _feeIncreaseStop) override external onlyOperator nonReentrant {
        ...
        emit FeeRuleUpdated(minFeeRatio, maxFeeRatio, feeIncreaseStart, feeIncreaseStop);
    }
    ...
    function withdrawLast() override external onlyOperator nonReentrant {
        ...
        (bool sent, bytes memory data) = staker.call{value: amount, gas: 40000}("");
    ...
    function sendToStub(uint256 amount, uint256 stubId) public onlyOperator {
        ...
        (bool sent, bytes memory data) = _registry[stubId].call{value: amount}("");
...
interface IStakingPool {
    ...
    event FeeRuleUpdated(uint256 minFeeRatio, uint256 maxFeeRatio, uint256 feeIncreaseStart, uint256 feeIncreaseStop);
    ...
    function updateFeeParameters(uint256 minFeeRatio, uint256 maxFeeRatio, uint256 feeIncreaseStart, uint256 feeIncreaseStop) external;
