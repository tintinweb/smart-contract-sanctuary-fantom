 ...
 *
 *
 *	GaugeEquivalentFarmland.sol
 *
 *	Inherits:
 *	- Equalizer V2 GaugeEquivalent
 *	- Elite Farmlands
 *	- TvlGuru v8
 *
 *
...
contract GaugeEquivalentFarmland is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public protocolFeesPerMillion;
    ...
    uint256 public splitRatio = 700e3;	//per million
    uint256 public splitLocktime = 26 weeks;
    ...
    constructor(
        ...
        TvlGuru[_stake] = _s_tg;
        TvlPriceFeed[_stake] = _s_tgpf;
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        for (uint i; i < rewardTokens.length; i++) {
            ...
            if (_reward > 0) {
                ...
                if(_rewardsToken == ve.token()) {
                    uint256 _toLock;
                    if(splitRatio > 0) {
                        _toLock = ( _reward.mul(splitRatio) ).div(1e6);
                        ve.create_lock_for(_toLock, splitLocktime, msg.sender);
                        IERC20(_rewardsToken).safeTransfer(msg.sender, _reward.sub(_toLock));
                    }
                }
                else {
        ...
        for (uint i; i < _tokens.length; i++) {
            ...
            if (_reward > 0) {
                ...
                if(_rewardsToken == ve.token()) {
                    ...
                    if(splitRatio > 0) {
                        _toLock = ( _reward.mul(splitRatio) ).div(1e6);
                        ve.create_lock_for(_toLock, splitLocktime, account);
                        IERC20(_rewardsToken).safeTransfer(account, _reward.sub(_toLock));
                    }
                }
                else {
    ...
    function _claimFees() internal returns (uint claimed0, uint claimed1)  {
        uint _pfpm = protocolFeesPerMillion;
        address _pft = _pfpm > 0 ? protocolFeesTaker : address(0);
        ...
        //`vault` is a peripheral contract that aggregates fees from all sources and transfers them here
        try vault.claimFees() {
    ...
    function addReward(
        ...
        TvlGuru[_rewardsToken] = _TvlGuru;
    ...
    function setBribe(address _b) external onlyOwner {
    ...
    function setSplitParameters(uint256 _sr, uint256 _st) external onlyOwner {
        require(_sr<=1e6, "SR: Must be under a million!");
        splitRatio = _sr;
        splitLocktime = _st;
    }
