...
contract StrategyOxdVolatileBifiMaiLp is StrategyOxdFarmLPBase {
    // Token addresses
    address public bifi_mai_lp = 0x8aeB0503E13F7bea02F80986a8FDb2AccE5C6b6C;
    ...
    {
        swapRoutes[bifi].push(route(wftm, usdc, false));
        swapRoutes[bifi].push(route(usdc, mai, true));
        swapRoutes[bifi].push(route(mai, bifi, false));
        swapRoutes[mai].push(route(wftm, usdc, false));
        swapRoutes[mai].push(route(usdc, mai, true));
    }

    // **** Views ****

    function getName() external pure override returns (string memory) {
        return "StrategyOxdVolatileBifiMaiLp";
    }
}
