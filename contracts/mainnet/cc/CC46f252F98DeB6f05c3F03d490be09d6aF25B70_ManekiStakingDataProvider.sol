...
contract ManekiStakingDataProvider {
     ...
     * @return uint256 Value in USD in 8 decimals precision
     * **/
    ...
    function getClaimableRewardsUsdBalance(
        ...
        for (uint256 i = 0; i < claimableRewards.length; i++) {
            if (claimableRewards[i].token == protocolToken) {
                totalUsdValue += (
                    uint256(protocolTokenOracle.latestAnswer())
                        .mul(claimableRewards[i].amount)
                        .div(10 ** ERC20(claimableRewards[i].token).decimals())
                );
            } else {
                ...
                totalUsdValue += (
                    aaveOracle
                        .getAssetPrice(underlyingAsset)
                        .mul(claimableRewards[i].amount)
                        .div(10 ** ERC20(claimableRewards[i].token).decimals())
                        .div(1e10) //AaveOracle returns price feed in 18 decimals point, divide to make it compatible with 8 decimals point
                );
            }
        }
    ...
    /**
     * @notice Get total Paw locked
     * @return uint256 Return total Paw locked in 18 decimals precision
     * **/

    function getAggregateTotalPawLocked() public view returns (uint256) {
        return multiFeeDistribution.lockedSupply();
    }

    /**
     * @notice Get total Paw staked
     * @return uint256 Return total Paw staked in 18 decimals precision
     * **/

    function getAggregateTotalPawStaked() public view returns (uint256) {
        return multiFeeDistribution.totalSupply();
    }

    /**
     * @notice Get the APR of Staking Paw
     * @return uint256 Return APR in 8 decimals precision
     * **/

    function getStakingAPR() public view returns (uint256) {
        /* Total Paw Staked */
        uint256 totalPawSupply = multiFeeDistribution.totalSupply();

        /* Total Paw Staked Value In Usd with 8 decimals precision */
        uint256 totalPawSupplyValue = totalPawSupply
            .mul(uint256(protocolTokenOracle.latestAnswer()))
            .div(1e18);

        /* Get all rewards tokens */
        MultiFeeDistribution.RewardData[]
            memory claimableRewards = getClaimableRewards(address(0));
        address[] memory rewardTokens = new address[](claimableRewards.length);
        for (uint256 i = 0; i < claimableRewards.length; i++) {
            rewardTokens[i] = claimableRewards[i].token;
        }

        /* Extract Usd Value of Reward Tokens */

        uint256 allRewardsAnnualizedUsdValue = 0;

        for (uint256 i = 0; i < rewardTokens.length; i++) {
            if (
                rewardTokens[i] != protocolToken &&
                !isConvertedReward(rewardTokens[i])
            ) {
                (, uint256 ratePerSecond, , , ) = multiFeeDistribution
                    .rewardData(rewardTokens[i]);
                uint256 annualizedRate = ratePerSecond.mul(86400).mul(365);
                uint256 pricePerToken = aaveOracle
                    .getAssetPrice(
                        AToken(rewardTokens[i]).UNDERLYING_ASSET_ADDRESS()
                    )
                    .div(1e10);
                uint256 annualizedRewardUsdValue = annualizedRate
                    .mul(pricePerToken)
                    .div(10 ** AToken(rewardTokens[i]).decimals());
                allRewardsAnnualizedUsdValue += annualizedRewardUsdValue;
            }
        }

        return allRewardsAnnualizedUsdValue.mul(1e4).div(totalPawSupplyValue);
    }

    /**
     * @notice Get the APR of Locking Paw
     * @return uint256 Return APR in 8 decimals precision
     * **/

    function getLockingAPR() public view returns (uint256) {
        uint256 lockedTokens = getAggregateTotalPawLocked();
        uint256 lockedTokensValue = lockedTokens
            .mul(uint256(protocolTokenOracle.latestAnswer()))
            .div(1e18);
        (, uint256 protocolTokenRewardRate, , , ) = multiFeeDistribution
            .rewardData(protocolToken);
        uint256 annualizedRate = protocolTokenRewardRate.mul(86400).mul(365);
        uint256 pricePerToken = uint256(protocolTokenOracle.latestAnswer());
        uint256 additionalLockedApr = pricePerToken
            .mul(annualizedRate)
            .div(1e18)
            .mul(1e4)
            .div(lockedTokensValue);
        return additionalLockedApr.add(getStakingAPR());
    }

    /****** Newly Updated Code ******/

    /**
     * @notice Return the total daily aggregated penalty fee that will be distributed in protocol native token
     * @return uint256 Daily aggregated penalty fee in 18 decimals precision of PAW
     * **/

    function getProtocolAggregatedDailyPenaltyFeeDistribution()
        public
        view
        returns (uint256)
    {
        return multiFeeDistribution.getRewardForDuration(protocolToken).div(7);
    }

    /**
     * @notice Return the total daily aggregated penalty fee in Usd that will be distributed in protocol native token
     * @return uint256 Daily aggregated penalty fee in Usd in precision of 8 decimals
     * **/

    function getProtocolAggregatedDailyPenaltyFeeDistributionInUsd()
        public
        view
        returns (uint256)
    {
        return
            getProtocolAggregatedDailyPenaltyFeeDistribution()
                .mul(uint256(protocolTokenOracle.latestAnswer()))
                .div(1e18);
    }

    /**
     * @notice Return the user daily rewarded penalty fee in Usd
     * @return uint256 Daily user rewarded penalty fee in Usd in precision of 8 decimals
     * **/

    function getUserDailyPenaltyFeeDistributionInUsd(
        address _user
    ) public view returns (uint256) {
        (uint256 userLockedAmount, , , ) = multiFeeDistribution.lockedBalances(
            _user
        ); // 18 decimals
        uint256 totalLockedAmount = getAggregateTotalPawLocked(); //18 decimals
        uint256 userLockedShare = userLockedAmount.mul(1e18).div(
            totalLockedAmount
        ); //user locked share percentage in 18 decimals precision
        return
            getProtocolAggregatedDailyPenaltyFeeDistributionInUsd()
                .mul(userLockedShare)
                .div(1e18);
    }

    /**
     * @notice Get the total daily aggregated protocol distributed fee in Usd
     * @return uint256 Daily aggregated protocol distributed fee in Usd in precision of 8 decimals
     * **/

    function getProtocolAggregatedDailyPlatformFeeDistributionInUsd()
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < claimableRewards.length; i++) {
            if (
                claimableRewards[i].token != protocolToken &&
                !isConvertedReward(claimableRewards[i].token)
            ) {
                address underlyingToken = AToken(claimableRewards[i].token)
                    .UNDERLYING_ASSET_ADDRESS();
                uint256 pricePerToken = aaveOracle.getAssetPrice(
                    underlyingToken
                );
                uint256 totalReward = multiFeeDistribution
                    .getRewardForDuration(claimableRewards[i].token)
                    .div(7);
                uint256 totalRewardUsdValue = totalReward
                    .mul(pricePerToken)
                    .div(10 ** AToken(claimableRewards[i].token).decimals())
                    .div(1e10); //Convert to 8 decimals precision
                totalUsdValue += totalRewardUsdValue;
            }
        }
        return totalUsdValue;
    }
    ...
    /**
     * @notice Get user's daily protocol distributed fee in Usd
     * @return uint256 User's daily protocol distributed fee in Usd in precision of 8 decimals
     * **/

    function getUserDailyPlatformFeeDistributionInUsd(
        address _user
    ) public view returns (uint256) {
        uint256 totalPawStaked = getAggregateTotalPawStaked();
        uint256 userPawStaked = getUnlockedPaw(_user) +
            getTotalPawLocked(_user);
        uint256 userPawStakedShare = userPawStaked.mul(1e18).div(
            totalPawStaked
        ); //user locked share percentage in 18 decimals precision
        return
            getProtocolAggregatedDailyPlatformFeeDistributionInUsd()
                .mul(userPawStakedShare)
                .div(1e18);
    }

    function getUserLockedAndStakedPawInUsd(
        address _user
    ) public view returns (uint256) {
        uint256 userPawStaked = getUnlockedPaw(_user) +
            getTotalPawLocked(_user);
        return
            userPawStaked.mul(uint256(protocolTokenOracle.latestAnswer())).div(
                1e18
            );
    }
    ...
    /**
     * @notice Get the amount of LP staked in MasterChef
     * @return Amount of LP Staked in MasterChef in 18 decimals precision
     **/
    function getLpStaked() public view returns (uint256) {
        return lpPair.balanceOf(address(masterChef));
    }

    /**
     * @notice Get the amount of LP user staked
     * @return Amount of LP Staked by user in MasterChef in 18 decimals precision
     **/

    function getUserLpStaked(address _user) public view returns (uint256) {
        (uint256 userLpStaked, ) = masterChef.userInfo(address(lpPair), _user);
        return userLpStaked;
    }

    /**
     * @notice Total daily rewards of Paw for LP Stakers
     * @return Amount of Paw rewards in 18 decimals precision
     **/

    function getTotalDailyRewards() public view returns (uint256) {
        return masterChef.rewardsPerSecond().mul(86400);
    }

    /**
     * @notice User's share of total staked LP
     * @return % of LP staked in 8 decimals precision
     **/

    function getUserSharePercentage(
        address _user
    ) public view returns (uint256) {
        uint256 totalLpStaked = getLpStaked();
        if (totalLpStaked == 0) {
            return 0;
        }
        uint256 userLpStaked = getUserLpStaked(_user);
        return userLpStaked.mul(1e8).div(totalLpStaked);
    }

    /**
     * @notice Get user's daily Paw Reward
     * @return Paw in 18 decimals precision
     **/

    function getUserDailyRewardsInPaw(
        address _user
    ) public view returns (uint256) {
        return
            getUserSharePercentage(_user).mul(getTotalDailyRewards()).div(1e8);
    }

    /**
     * @notice Get user's daily Paw Reward in Usd
     * @return Usd in 8 decimals precision
     **/

    function getUserDailyRewardsInUsd(
        address _user
    ) public view returns (uint256) {
        return
            getUserDailyRewardsInPaw(_user)
                .mul(uint256(protocolTokenOracle.latestAnswer()))
                .div(1e18);
    }

    /**
     * @notice Get Lp Staking Apr
     * @return % in 2 decimals precision
     **/

    function getLpStakingApr() public view returns (uint256) {
        uint256 totalDailyRewards = getTotalDailyRewards();
        uint256 annualizedRewards = totalDailyRewards.mul(365);
        uint256 annualizedRewardsInUsd = annualizedRewards
            .mul(uint256(protocolTokenOracle.latestAnswer()))
            .div(1e18);
        uint256 totalLpStakedInUsd = getLpStaked().mul((getLpPrice())).div(
            1e18
        );
        return annualizedRewardsInUsd.mul(1e4).div(totalLpStakedInUsd);
    }

    /**  C. Misc Functions **/
    function isConvertedReward(address _reward) private pure returns (bool) {
        if (
            _reward == 0xe578C856933D8e1082740bf7661e379Aa2A30b26 ||
            _reward == 0x940F41F0ec9ba1A34CF001cc03347ac092F5F6B5 ||
            _reward == 0x38aCa5484B8603373Acc6961Ecd57a6a594510A3
        ) return true;
        else return false;
    }
}
