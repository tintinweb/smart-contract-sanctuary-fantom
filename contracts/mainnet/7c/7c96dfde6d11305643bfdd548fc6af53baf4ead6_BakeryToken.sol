...
contract BakeryMaster is Ownable {
    ...
    uint256 accBakePerShare;
    bool exists;
    uint256 apy; // New variable to store current APY
}
    ...
    function setPoolAPY(address _pair, uint256 _newAPY) public onlyOwner {
    PoolInfo storage pool = poolInfoMap[_pair];
    ...
    bake.transfer(devAddr, devReward);
    bake.transfer(address(0xdead), burnReward);
    uint256 apyReward = farmerReward.mul(pool.apy).div(10000); // Calculate APY reward
    pool.accBakePerShare = pool.accBakePerShare.add(apyReward.mul(accBakePerShareMultiple).div(lpSupply));
    pool.lastRewardBlock = block.number;
}
