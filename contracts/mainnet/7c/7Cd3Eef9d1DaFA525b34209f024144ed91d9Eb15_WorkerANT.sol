...
contract WorkerANT is ERC721, Ownable {
    uint256 public counter = 1;
    IBuildingBlock public buildingblock;
    uint256 public immutable STAKE_DURATION = 1 minutes;
    uint256 public immutable BUILD_DURATION = 7 minutes;
    constructor(address _buildingblockaddress) ERC721("WorkerANT","WorkerANT") {
    ...
    function getAvailableWorkers(address _user) public view returns(uint256[] memory) {
        ...
        uint256[] memory availableWorkerList = new uint256[](_workerList.length - _stakedWorkerCount - _builderCount);
        // console.log("Available worker count in worker ant", availableWorkerList.length);
    ...
    function getUnHousedWorkers(address _user) public view returns(uint256[] memory) {
        uint256[] memory _workersList = getWorkers(_user);

        uint256 _homelessCount = getHomelessCount(_workersList);
        // console.log("Homeless count: ", _homelessCount);
        uint256 _workersHoused = 0;
        uint256[] memory _unhousedWorkerList = new uint256[](_homelessCount);
        // console.log("Unhoused worker list length : ", _unhousedWorkerList.length);
        for (uint i; i < _workersList.length; i++){
            if(!idToHousing[i+1]){
                _unhousedWorkerList[_workersHoused] = _workersList[i]; 
                _workersHoused++;
                // console.log("Homeless in worker: ", _workersList[i]);
            }
        }
        return _unhousedWorkerList;
    }

    function getHomelessCount(uint256[] memory _workersList) public view returns(uint256 _homeless) {
        _homeless = 0;
        
        for (uint i; i < _workersList.length; i++){
            // console.log("id ", _workersList[i],"idToHousing status : ", idToHousing[i]);
            if (!idToHousing[i+1]){
                _homeless +=1;
            }     
        }
    ...
    function reduceHP (address _user, uint256 _index) public {
        if (idToHealthPoints[_index] >= 1){
            ...
            console.log("id to be burned:", _index);
            playerToWorkers[_user][_index-1] =  playerToWorkers[_user][ playerToWorkers[_user].length - 1];
            console.log("id to be popped:", playerToWorkers[_user][playerToWorkers[_user].length-1]);
            playerToWorkers[_user].pop();
            console.log("last player id after pop: ", playerToWorkers[_user][playerToWorkers[_user].length-1]);
            _burn(_index);

//            uint256[] memory _bbList = IBuildingBlock(buildingblock).getBuildingBlocks(_user);
//            for (uint i=0; i < _bbList.length; i++){
//                uint256[] memory _workersHoused = IBuildingBlock(buildingblock).getWorkersHoused(i);
//                for (uint j; j < _workersHoused.length; j++) {
//                    if (_workersHoused[j] == _index) {
//                        delete _workersHoused[j];
//                        buildingblock.increaseCapacity(i,1);
//                    } 
//                }
            }
        }
    ...
    function burn(address _user, uint256 _index) public {
        require(playerToWorkers[_user].length > 0 , "index out of bound");
        for (uint i = 0; i < playerToWorkers[_user].length - 1; i++) {
