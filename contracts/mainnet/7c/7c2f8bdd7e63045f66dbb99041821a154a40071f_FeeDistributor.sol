...
contract FeeDistributor is BaseDistributor {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public platformShareBP = 0;
    ...
    event UpdateDevWallet(address indexed user, address devWallet);
    ...
    function distributeFees(address outputToken) external onlyAdmins {
        ...
        if(platformShareBP != 0 && address(devWallet) != address(0)){
            safeTokenTransfer(IERC20(outputToken), devWallet, platformAmount);
        }
    }
    ...
    function updateDevWallet(address _devWallet) external onlyOwner {
        ...
        emit UpdateDevWallet(msg.sender, _devWallet);
    }
}
