...
import "./interfaces/IXinSpiritStrategy.sol";
import "./interfaces/IXinSpirit.sol";

contract xinSpiritManager {
    ...
    function initialLock() public {
        ...
        IXinSpiritStrategy(strategy).release();
        //create new lock
        uint256 spiritBalanceStrategy = IERC20(spirit).balanceOf(strategy);
        IXinSpiritStrategy(strategy).createLock(spiritBalanceStrategy, unlockAt);
    ...
    function _lockMoreSpirit(uint256 _amount) internal {
        ...
        uint256 _strategyInSpiritBalance = IXinSpiritStrategy(strategy).balanceOfInSpirit();

        if (_strategyInSpiritBalance > 0) {
            //increase amount
            IXinSpiritStrategy(strategy).increaseAmount(_amount);
        } else {
            initialLock();
        }
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        IXinSpirit(linSpirit).mint(msg.sender, _amount);
    }
