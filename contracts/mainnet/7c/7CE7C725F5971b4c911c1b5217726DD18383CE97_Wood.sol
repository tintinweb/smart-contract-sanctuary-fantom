...
contract Wood is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    uint256 public constant INITIAL_LAUNCH_DISTRIBUTION = 400 ether;
    uint256 public constant INITIAL_DEAD_DISTRIBUTION = 20000 ether;
    ...
    /**
     * @notice Constructs the WOOD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, address _blacklist) public ERC20("WOOD", "WOOD") {
        // Mints 1 WOOD to contract creator for initial pool setup
    ...
    function updateBlacklist(address _blacklist) public onlyOperator returns (bool) {
        require(_blacklist != address(0), "Blacklist cannot be 0");
        blacklist = _blacklist;
    }
    ...
    function _getWoodPrice() internal view returns (uint256 _woodPrice) {
        try IOracle(woodOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Wood: failed to fetch WOOD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _woodPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_woodPrice >= taxTiersTwaps[tierId]) {
    ...
    function setWoodOracle(address _woodOracle) public onlyOperatorOrTaxOffice {
        require(_woodOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints WOOD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of WOOD to mint to
    ...
    function transferFrom(
        ...
        require(!IBlacklist(blacklist).verifyBacklistedUser(_msgSender()), "Address is blacklisted");
        require(!IBlacklist(blacklist).verifyBacklistedUser(sender) && !IBlacklist(blacklist).verifyBacklistedUser(recipient), "Address is blacklisted");
        uint256 currentTaxRate = 0;
        bool burnTax = false;

        if (autoCalculateTax) {
            uint256 currentWoodPrice = _getWoodPrice();
            currentTaxRate = _updateTaxRate(currentWoodPrice);
            if (currentWoodPrice < burnThreshold) {
                burnTax = true;
            }
        }
     ...
     */
    function distributeReward(address _launcher) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_launcher != address(0), "!_launcher");
        rewardPoolDistributed = true;
        _mint(_launcher, INITIAL_LAUNCH_DISTRIBUTION);
        _mint(address(0x000000000000000000000000000000000000dEaD), INITIAL_DEAD_DISTRIBUTION);
    }
