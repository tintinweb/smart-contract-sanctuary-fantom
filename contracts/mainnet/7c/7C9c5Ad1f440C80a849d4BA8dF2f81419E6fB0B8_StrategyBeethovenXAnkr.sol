...
contract StrategyBeethovenXAnkr is StratFeeManagerInitializable {
    ...
    ISolidlyRouter.Routes[] public nativeToInputRoute;
    ...
    function initialize(
        ...
        ISolidlyRouter.Routes[] calldata _nativeToInputRoute
    ) public initializer {
        __StratFeeManager_init(_commonAddresses);

        for (uint i; i < _secondOutputToankrFTMRoute.length; ++i) {
        ...
        solidlyRouter = address(0x1A05EB736873485655F29a37DEf8a0AA87F5a447);
        ...
        ankrFTMSwapPoolId = _balancerPoolIds[2];
        chefPoolId = _chefPoolId;
        chef = _chef;
        input = _input;
        secondOutput = _secondOutputToankrFTMRoute[0].from;
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        if (outputBal > 1 ether || secondOutputBal > 0) {
    ...
    function _giveAllowances() internal {
        ...
        IERC20(ankrFTM).safeApprove(unirouter, type(uint256).max);
        IERC20(secondOutput).safeApprove(solidlyRouter, type(uint256).max);
        IERC20(native).safeApprove(solidlyRouter, type(uint256).max);
    ...
    function nativetoInput() external view returns (address[] memory) {
        ISolidlyRouter.Routes[] memory _route = nativeToInputRoute;
        return _solidlyToRoute(_route);
    }
}
