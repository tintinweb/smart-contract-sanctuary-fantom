...
contract SpiralBondDepositoryRewardBased is Ownable {
    ...
    address public immutable SPR; // token given as payment for bond
    address public immutable principal; // token used to create bond
    address public immutable treasury; // mints SPR when receives principal
    ...
    struct Bond {
        uint payout; // SPR remaining to be paid
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        if ( _parameter == PARAMETER.VESTING ) { // 0
            require( _input >= 130000, "Vesting must be longer than 36 hours" );
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( SPR ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( SPR ).approve( stakingHelper, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( SPR ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to SPR supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( SPR ).totalSupply();
    ...
    /**
     *  @notice calculate amount of SPR available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principal or SPR) to the DAO
     *  @return bool
     */
