...
contract SwapCenterFantom is ISwap, ReentrancyGuard, Ownable {
  ...
  uint256 constant WFTM_ID = type(uint256).max;
  ...
  uint256 constant SPOOKYSWAP_ID = 0;
  ...
  uint256 constant SPOOKYSWAP_ZAP_IN = 9;

  uint256 constant SUSHISWAP_ZAP_IN = 10;

  uint256 constant SPOOKYSWAP_RM_LIQ = 11;
  ...
  function swapExactTokenIn(address tokenIn, address tokenOut, uint256 amountIn, uint256 minAmountOut) external override payable nonReentrant returns (uint256) {
    ...
    for(uint256 i = 0; i < exchangeLen; i++) {
      ...
      if(exchangeId == SPOOKYSWAP_ID) {
        _swapExactTokenIn_uniswapType(SPOOKYSWAP_ROUTER, curSellPath);
      ...
      } else if (exchangeId == SPOOKYSWAP_ZAP_IN) {
        _swapZapIn_uniswapType(SPOOKYSWAP_ROUTER, curSellPath);
      } else if (exchangeId == SUSHISWAP_ZAP_IN) {
        _swapZapIn_uniswapType(SUSHI_ROUTER, curSellPath);
      } else if(exchangeId == SPOOKYSWAP_RM_LIQ) {
        _swapRmLiq_uniswapType(SPOOKYSWAP_ROUTER, curSellPath);
      } else if(exchangeId == SUSHISWAP_RM_LIQ) {
        _swapRmLiq_uniswapType(SUSHI_ROUTER, curSellPath);
        _swapDepositVault(curSellPath);
      } else if (exchangeId == WFTM_ID) {
        _swapWFTM(curSellPath);
      } else {
  ...
  function _swapWFTM(address[] memory path) internal{
    require(path.length == 2, "Unsupported path");

    address curInputToken = path[0];
    address curOutputToken = path[1];

    address wftm_address = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;

    if (curInputToken == ETH_ADDRESS && curOutputToken == wftm_address) {
      uint256 amountIn = address(this).balance;
      IWETH(wftm_address).deposit{value:amountIn}();
    } else if (curInputToken == wftm_address && curOutputToken == ETH_ADDRESS) {
      uint256 amountIn = IERC20(curInputToken).balanceOf(address(this));
      IERC20(curInputToken).safeApprove(wftm_address, 0);
      IERC20(curInputToken).safeApprove(wftm_address, amountIn);
      IWETH(wftm_address).withdraw(amountIn);
    } else {
