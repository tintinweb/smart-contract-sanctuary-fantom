...
contract LAL is FTM20, Ownable {
    ...
    address payable public flashWallet;
    ...
    uint256 constant public TIME_STEP = 1440 minutes; // 1 Day
    uint256 public INVEST_MIN_AMOUNT = 1 ether; // 1 FTM
    ...
    uint256 public flashAmountCollected;

    uint256[] public REFERRAL_PERCENTS = [90, 30, 20, 10, 10, 10, 10, 10, 10, 10, 20]; // 100 = 10%, 90 = 9%
    uint256[] public LEVEL_MINIMUM = [0, 2500 ether, 4000 ether, 6000 ether, 8000 ether, 10000 ether, 12000 ether, 14000 ether, 16000 ether, 18000 ether, 21000 ether]; 
    ...
    constructor() FTM20("LAL", "LAL") {
        creatorWallet = payable(address(0xbFf9AB7043842de043f193257c81315187AB9afE));
        marketingWallet = payable(address(0x19858B1Bb487005fcb09FE74E0249FEa00Ede87f));
        flashWallet = payable(address(0x360554215802498972bf2e0501431e046E706F2C));
        // New Owner Assigned
        // _transferOwnership(payable(address(0x84f4F2C877C96f58cCF24Bf14BB1178953fc9195)));
    }
    ...
    function buy(address referrer) external payable {
        ...
        creatorFeeCollected = creatorFeeCollected.add(feeCreator);
        ...
        marketingFeeCollected = marketingFeeCollected.add(feeMarketing);
        ...
        if (user.referrer == address(0) && _msgSender()!=creatorWallet) {
            ...
            }else if(referrer == creatorWallet){
        ...
        if (user.referrer != address(0)) {
            address upline = user.referrer;
            uint256 totalInvest = users[upline].totalBusiness[0];
            uint256 flashAmount;

            for (uint256 i = 0; i < REFERRAL_PERCENTS.length; i++) {
                if (upline != address(0)) {
                    users[upline].totalBusiness[i] = users[upline].totalBusiness[i].add(msg.value);
                    if(totalInvest >= LEVEL_MINIMUM[i]){
                        ...
                        uint256 amount = msg.value.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
                        flashAmount = flashAmount.add(amount);
                        flashAmountCollected = flashAmountCollected.add(amount);
                    }
                    upline = users[upline].referrer;
                    totalInvest = users[upline].totalBusiness[i+1];
                } else {
                    uint256 amount = msg.value.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
                    flashAmount = flashAmount.add(amount);
                    flashAmountCollected = flashAmountCollected.add(amount);
            ...
            if(flashAmount>0){
                flashWallet.transfer(flashAmount);
            }
        }
        ...
        if(totalSupply() > 0){
            ...
            if(_fee - (_fee-(tokenBrought * (passiveAmount * magnitude / (totalSupply())))) >= 0){
        ...
        user.deposits.push(Deposit( msg.value, block.timestamp, tokenBrought));

        emit NewDeposit(msg.sender, msg.value);
    }

    function sell(uint256 _amount) onlybelievers external {
        User storage user = users[msg.sender];
        require(block.timestamp > user.passiveWithdrawTime.add(TIME_STEP), "Wait Time not Reached");
        ...
        uint256 deductionAmount = sellDeduction(totalToken,ftmAmount);

        uint256 ftmRemaining = ftmAmount.sub(deductionAmount);
        
        uint256 perPassive = (deductionAmount*((100/20)*5))/100;
        uint256 adminFee = deductionAmount-perPassive;

        // update dividends tracker
        uint256 _updatedPayouts = (uint256) (profitPerShare_ * _amount + (perPassive * magnitude));
        ...
        if (totalSupply() > 0) {
            ...
            profitPerShare_ = SafeMath.add(profitPerShare_, (perPassive * magnitude) / totalSupply());
        }

        totalSellCount += 1;
        user.passiveWithdrawTime = block.timestamp;

        (creatorWallet).transfer(adminFee);
        payable(msg.sender).transfer(ftmRemaining);
        user.withdraws.push(Withdraw( _amount, block.timestamp, ftmRemaining));
        // burn the sold tokens
        _burn(msg.sender, _amount);
    }

    function sellDeduction(uint256 _tokenAmount, uint256 _ftmAmount) internal pure returns(uint256){
        uint256 deductionAmount;
        if(_tokenAmount == 0){
            deductionAmount = 0;
        }else{
            deductionAmount = _ftmAmount.mul(20).div(100);
        }
        ...
        public
    {
        ...
        uint256 collectedFee = _dividends.mul(10).div(100);
        _dividends = _dividends.sub(collectedFee);
        uint256 tokenBrought = ftmToTokens_(_dividends);
        
        uint256 _fee = collectedFee * magnitude;
        if(totalSupply() > 0){
            // take the amount of dividends gained through this transaction, and allocates them evenly to each shareholder
            profitPerShare_ += (collectedFee * magnitude / (totalSupply()));

            // calculate the amount of tokens the customer receives over his purchase 
            if(_fee - (_fee-(tokenBrought * (collectedFee * magnitude / (totalSupply())))) >= 0){
                _fee = _fee - (_fee-(tokenBrought * (collectedFee * magnitude / (totalSupply()))));
            }
    ...
    function bonusAfterFee() public view returns(uint256){
        address _customerAddress = msg.sender;
    ...
    function FtmToTokenAfterFee(uint256 _ftm) public view returns(uint256){
        uint256 tokenAfterTax = _ftm.mul(tokensToGet).div(PERCENTS_DIVIDER);
        // 60% FTM Used to Buy Token
        uint256 tokenBrought = ftmToTokens_(tokenAfterTax);
        return tokenBrought;
    }

    function tokenToFtmAfterFee(uint256 _tokens) public view returns(uint256){
        uint256 totalToken = balanceOf(msg.sender);
        uint256 ftmAmount = tokensToFtm_(_tokens);
        uint256 deductionAmount = sellDeduction(totalToken,ftmAmount);

        uint256 ftmReceivable = ftmAmount.sub(deductionAmount);
        return ftmReceivable;
    }

    function tokenToFtmAfterFeePercent(uint256 _percent) public view returns(uint256){
        uint256 totalToken = balanceOf(msg.sender);
        uint256 _tokens = totalToken.mul(_percent).div(100);
        uint256 ftmAmount = tokensToFtm_(_tokens);
        uint256 deductionAmount = sellDeduction(totalToken,ftmAmount);
    ...
    function tokenToFtmBeforeFeePercent(uint256 _percent) public view returns(uint256){
        uint256 totalToken = balanceOf(msg.sender);
        uint256 _tokens = totalToken.mul(_percent).div(100);
        uint256 ftmAmount = tokensToFtm_(_tokens);
        uint256 ftmReceivable = ftmAmount;
        return ftmReceivable;
    }

    function tokenPercent(uint256 _percent) public view returns(uint256){
        uint256 totalToken = balanceOf(msg.sender);
        uint256 _tokens = totalToken.mul(_percent).div(100);
        return _tokens;
    }
    ...
    function getUserDownlineCount(address userAddress) public view returns (uint256[11] memory referrals) {
        return (users[userAddress].levels);
    }

    function getUserBusinessCount(address userAddress) public view returns (uint256[11] memory business) {
        return (users[userAddress].totalBusiness);
    }

    function getUserBuyInfo(address userAddress, uint256 index) public view returns (uint256 amount, uint256 checkpoint, uint256 recieved) {
        User storage user = users[userAddress];

        amount = user.deposits[index].amount;
        checkpoint = user.deposits[index].checkpoint;
        recieved = user.deposits[index].recieved;
    }

    function getUserSellInfo(address userAddress, uint256 index) public view returns (uint256 amount, uint256 checkpoint, uint256 recieved) {
        User storage user = users[userAddress];
    ...
    function getDepositLength(address userAddress) public view returns(uint256){
        User storage user = users[userAddress];
        return user.deposits.length;
    }

    function getWithdrawLength(address userAddress) public view returns(uint256){
        User storage user = users[userAddress];
        return user.withdraws.length;
    }

    function getUserTotalBusiness(address userAddress) public view returns (uint256 amount) {
    ...
    function getUserTotalLevels(address userAddress) public view returns (uint256 amount) {
    ...
    function getUserDirectBusiness(address userAddress) public view returns (uint256 amount) {
        amount = (users[userAddress].totalBusiness[0]);
        return amount;
    }

    function getUserDirectLevels(address userAddress) public view returns (uint256 amount) {
