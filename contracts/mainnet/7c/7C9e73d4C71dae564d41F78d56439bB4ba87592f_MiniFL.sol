...
contract MiniFL is IMiniFL, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  event LogApproveStakeDebtToken(
  ...
  function initialize(address _alpaca, uint256 _maxAlpacaPerSecond)
    external
    initializer
  {
  ...
  /// @param _withUpdate If true, do mass update pools
  function setAlpacaPerSecond(uint256 _alpacaPerSecond, bool _withUpdate)
    external
    onlyOwner
  {
    ...
    returns (uint256)
  {
    ...
    if (block.timestamp > pool.lastRewardTime && stakedBalance != 0) {
      ...
      uint256 alpacaReward = (timePast * alpacaPerSecond * pool.allocPoint) /
        totalAllocPoint;
      accAlpacaPerShare =
        accAlpacaPerShare +
        ((alpacaReward * ACC_ALPACA_PRECISION) / stakedBalance);
    }

    return
      (((user.amount * accAlpacaPerShare) / ACC_ALPACA_PRECISION).toInt256() -
  ...
  function deposit(
    ...
    user.rewardDebt =
      user.rewardDebt +
      ((_amount * pool.accAlpacaPerShare) / ACC_ALPACA_PRECISION).toInt256();
  ...
  function harvest(uint256 _pid) external nonReentrant {
    ...
    int256 accumulatedAlpaca = ((user.amount * pool.accAlpacaPerShare) /
      ACC_ALPACA_PRECISION).toInt256();
  ...
  function setMaxAlpacaPerSecond(uint256 _maxAlpacaPerSecond)
    external
    onlyOwner
  {
