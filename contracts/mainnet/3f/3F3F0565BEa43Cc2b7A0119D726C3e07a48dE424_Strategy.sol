...
contract Strategy is BaseStrategy {
	...
	constructor(
		...
		rewardTokenToLp0Route = [address(rewardToken), wftm, lpToken0];
		rewardTokenToLp1Route = [address(rewardToken), wftm, lpToken1];
	...
	function balanceOfPooled() public view returns (uint256 _amount) {
		...
		uint256 lpToken1InWant =
			ICurveFi(curvePoolSwap).get_dy_underlying(outTokenIndex, inTokenIndex, lpToken1Amount);
	...
	function sellLpToken1() internal {
		uint256 swapAmountIn = balanceOfLpToken1();
		ICurveFi(curvePoolSwap).exchange_underlying(outTokenIndex, inTokenIndex, swapAmountIn, 1);
	}
	...
	function removeLiquidity(uint256 toExitAmount) internal {
		...
		uint256 percentageOfLpToSell = toExitAmount.mul(1_000_000).div(lpWorth);
		uint256 liquidityToExit = lpTokenBalance.mul(percentageOfLpToSell).div(1_000_000);
