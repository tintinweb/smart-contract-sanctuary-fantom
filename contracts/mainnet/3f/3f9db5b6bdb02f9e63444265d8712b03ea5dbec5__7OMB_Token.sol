...
contract _7OMB_Token is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 7OMB-WETH LP -> 7OMB pool
    // uint256 public constant INITIAL_7OMB_POOL_DISTRIBUTION = 140000 ether;
    ...
    address public seven_ombOracle;
    ...
    /**
     * @notice Constructs the 7OMB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("7omb Token", "7OMB") {
        // Mints 1 7OMB to contract creator for initial pool setup
    ...
    function _get7ombPrice() internal view returns (uint256 _7ombPrice) {
        try IOracle(seven_ombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("7omb: failed to fetch 7OMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _7ombPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_7ombPrice >= taxTiersTwaps[tierId]) {
    ...
    function set7ombOracle(address _7ombOracle) public onlyOperatorOrTaxOffice {
        require(_7ombOracle != address(0), "oracle address cannot be 0 address");
        seven_ombOracle = _7ombOracle;
    }
    ...
    /**
     * @notice Operator mints 7OMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of 7OMB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 current7ombPrice = _get7ombPrice();
            currentTaxRate = _updateTaxRate(current7ombPrice);
            if (current7ombPrice < burnThreshold) {
                burnTax = true;
            }
        }
