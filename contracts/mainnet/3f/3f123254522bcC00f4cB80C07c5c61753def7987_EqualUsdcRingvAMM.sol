...
// File: contracts/EqualUsdcRingvAMM.sol
...
contract EqualUsdcRingvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public usdcToRingPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _usdcToRingPath
    ) {
        ...
        for (uint i; i < _usdcToRingPath.length; ++i) {
            usdcToRingPath.push(_usdcToRingPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedBal, 0, wrappedToUsdcPath, address(this), block.timestamp);

        uint usdcHalf = IERC20(usdc).balanceOf(address(this)) / 2;
        IEqualizerRouter(unirouter).swapExactTokensForTokens(usdcHalf, 0, usdcToRingPath, address(this), block.timestamp);

        
        uint256 lp0Bal = IERC20(usdc).balanceOf(address(this));
        uint256 lp1Bal = IERC20(ring).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(usdc, ring, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(ring).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(ring).safeApprove(unirouter, type(uint).max);
    }
