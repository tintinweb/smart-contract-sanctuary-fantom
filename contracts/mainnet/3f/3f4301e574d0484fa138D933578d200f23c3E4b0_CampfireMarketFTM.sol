...
contract CampfireMarketFTM is Ownable, ReentrancyGuard {
  using ECDSA for bytes32;

  mapping (bytes32 => bool) private _saltUsed;
  mapping (bytes32 => bool) private _saltCancelled;

  address private _g2mPartner;
  address private _wftmAddress;
  ...
  constructor() {
    _active = true;
    _g2mPartner = 0x653F6219A35B79df888301F2b05224DeC02a6361;
  }
  ...
  function purchase(address _seller, address _contractAddress, uint256 _tokenId, uint256 _price, uint256 _expiration, bytes32 _salt, bytes memory _signature) public nonReentrant payable {
    ...
    require(msg.value >= _price, "Campfire Market: Not enough FTM sent.");
    ...
    if (nft.supportsInterface(_INTERFACE_ID_ERC2981)) {
      ...
      payable(owner()).transfer((125 * msg.value) / 10000);
      payable(_g2mPartner).transfer((75 * msg.value) / 10000);

      uint256 remainder = msg.value - (royaltyAmount + ((msg.value * 200) / 10000));
      payable(currentNFTOwner).transfer(remainder);
    } else {
      payable(currentNFTOwner).transfer((msg.value * 9800) / 10000);
      payable(owner()).transfer((125 * msg.value) / 10000);
      payable(_g2mPartner).transfer((75 * msg.value) / 10000);
    }
  ...
  function acceptOffer(address _buyer, address _contractAddress, uint256 _tokenId, uint256 _price, uint256 _expiration, bytes32 _salt, bytes memory _signature) public nonReentrant {
    ...
    IERC20 wftm = IERC20(_wftmAddress);
    require(wftm.balanceOf(_buyer) >= _price, "Campfire Market: Buyer doesn't have enough WFTM.");
    ...
    if (nft.supportsInterface(_INTERFACE_ID_ERC2981)) {
      (address receiver, uint256 royaltyAmount) = nft.royaltyInfo(_tokenId, _price);
      wftm.transferFrom(_buyer, receiver, royaltyAmount);

      wftm.transferFrom(_buyer, owner(), ((_price * 125) / 10000));
      wftm.transferFrom(_buyer, _g2mPartner, ((_price * 75) / 10000));

      uint256 remainder = _price - (royaltyAmount + ((_price * 200) / 10000));
      wftm.transferFrom(_buyer, msg.sender, remainder);
    } else {
      wftm.transferFrom(_buyer, msg.sender, ((_price * 9800) / 10000));
      wftm.transferFrom(_buyer, owner(), ((_price * 125) / 10000));
      wftm.transferFrom(_buyer, _g2mPartner, ((_price * 75) / 10000));
    }
  ...
  function setG2mPartner(address g2mPartner_) public onlyOwner {
    _g2mPartner = g2mPartner_;
  }

  function setWFTM(address wftmAddress_) public onlyOwner {
    _wftmAddress = wftmAddress_;
  }
