...
contract SingleAssetRainMakerStorage is RainMakerAdminStorage {
    ...
    */
    ...
    struct CompMarketState {
        ...
        /// @notice With timestamp we would have until Feb 7th 2106
        uint32 timestamp;
    }
    ...
    /// @notice The amount of incentive tokens each side (supply and borrow) receives per second.
...
contract SingleAssetDynamicRainMakerStorage is RainMakerAdminStorage {
    ...
    */
    ...
    /// @notice The amount of incentive tokens the whole supply side receives per second.
    mapping(address => uint) public compSupplySpeeds;

    /// @notice The amount of incentive tokens the whole borrow side receives per second.
...
contract MultiAssetDynamicRainMakerStorage is RainMakerAdminStorage {
    /**
    * @notice If true - stops updating supply and borrow indexes
    */
    bool public isRetired;
    ...
    /// @notice A list of all markets
    address[] public allIncentiveTokens;
    ...
    /// @notice The amount of incentive tokens accrued for each incentive token for each user as of the last time they accrued
    mapping(address => mapping(address => uint)) public incentivesAccrued;

    /// @notice OToken => incentive token address for supply side
    mapping(address => address) public supplyIncentiveTokens;

    /// @notice OToken => incentive token address for borrow side
    mapping(address => address) public borrowIncentiveTokens;
}
...
contract SingleAssetDynamicRainMaker is SingleAssetDynamicRainMakerStorage, SingleAssetDynamicRainMakerInterface, ExponentialNoError, IComptrollerPeripheral {
    ...
    /**
     * @notice Claim all the rain accrued by holder in all markets
     * @dev used as a compatability hook
     * @param holder The address to claim rain for
     */
    function claimComp(address holder ) public {
        return claimRainByMarkets(holder, allMarkets);
    }
     ...
     * @param holder The address to claim rain for
     */
    function claimRain(address holder) public {
        return claimRainByMarkets(holder, allMarkets);
    }

    /**
     * @notice Claim all the rain accrued by holder in the specified markets
     * @param holder The address to claim rain for
     * @param cTokens The list of markets to claim rain in
     */
    function claimRainByMarkets(address holder, CToken[] memory cTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimRainByMarketsForSides(holders, cTokens, true, true);
    }

    /**
     * @notice Claim all rain accrued by the holders
     * @param holders The addresses to claim rain for
     * @param cTokens The list of markets to claim rain in
     * @param borrowers Whether or not to claim rain earned by borrowing
     * @param suppliers Whether or not to claim rain earned by supplying
     */
    function claimRainByMarketsForSides(address[] memory holders, CToken[] memory cTokens, bool borrowers, bool suppliers) nonReentrant public {
    ...
    function distributeBorrowerCompInternal(address cToken, address borrower, Exp memory marketBorrowIndex) internal {
        // Note : Unborrowable markets will not have a borrowIndex
    ...
    /**
     * @notice Set speeds to zero for a multiple markets, skips index updating.
     * @param _cTokens The markets to zero the speeds for.
     */
    function _emergencyZeroSpeeds(CToken[] calldata _cTokens) external {
        require(isAdmin(), "only admin can set emergency speed");
        ...
        for (uint i = 0; i < _cTokens.length; i++) {
            ...
            emit CompSupplySpeedUpdated(cToken, 0);

            compBorrowSpeeds[address(cToken)] = 0;
            emit CompBorrowSpeedUpdated(cToken, 0);
        }
    }
    ...
    function retire(bytes calldata params) external {
        ...
        // Allows to skip the retire internal logic -- To be used for emergencies
        bool skipIndexUpdate = params.length != 0;

        retireInternal(skipIndexUpdate);
    }


    function retireInternal(bool _skipIndexUpdate) internal {
        require(isComptroller(), "Not comptroller");
    ...
    function sweepToken(EIP20NonStandardInterface token) external {
        ...
        EIP20NonStandardInterface eip20 = EIP20NonStandardInterface(token);

        uint256 balance = eip20.balanceOf(address(this));
        eip20.transfer(admin, balance);
    }
    ...
    function _beforeNonReentrant() private {
        require(_notEntered, "SingleAssetDynamicRainmaker:re-entered");
        _notEntered = false;
    }
