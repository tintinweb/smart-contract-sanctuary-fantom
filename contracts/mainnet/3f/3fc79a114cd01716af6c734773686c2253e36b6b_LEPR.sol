...
contract LEPR is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 0 LEPR-WFTM LP -> LEPR pool
    uint256 public constant INITIAL_LEPR_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the LEPR ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("LEPR", "LEPR") {
        // Mints 1 LEPR to contract creator for initial pool setup
    ...
    function _getLEPRPrice() internal view returns (uint256 _LEPRPrice) {
        try IOracle(LEPROracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("LEPR: failed to fetch LEPR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _LEPRPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_LEPRPrice >= taxTiersTwaps[tierId]) {
    ...
    function setLEPROracle(address _LEPROracle) public onlyOperatorOrTaxOffice {
        require(_LEPROracle != address(0), "oracle address cannot be 0 address");
        LEPROracle = _LEPROracle;
    }
    ...
    /**
     * @notice Operator mints LEPR to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of LEPR to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentLEPRPrice = _getLEPRPrice();
            currentTaxRate = _updateTaxRate(currentLEPRPrice);
            if (currentLEPRPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_LEPRPool != address(0), "!_LEPRPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_LEPRPool, INITIAL_LEPR_POOL_DISTRIBUTION);
