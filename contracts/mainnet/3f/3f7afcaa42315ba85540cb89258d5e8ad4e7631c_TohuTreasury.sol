...
contract TohuTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STOHU }
    ...
    address public sTOHU;
    uint public sTOHUQueue; // Delays change to sTOHU address
    ...
    /**
        @notice allow approved address to deposit an asset for TOHU
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint TOHU needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TOHU ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TOHU for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITOHUERC20( TOHU ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTOHU ).balanceOf( msg.sender ); // Can only borrow against sTOHU held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TOHU
        @param _amount uint
     */
    function repayDebtWithTOHU( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITOHUERC20( TOHU ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TOHU, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TOHU ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TOHU ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TOHU valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TOHU decimals
            value_ = _amount.mul( 10 ** IERC20( TOHU ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STOHU ) { // 9
            sTOHUQueue = block.number.add( blocksNeededForQueue );
        } else return false;
