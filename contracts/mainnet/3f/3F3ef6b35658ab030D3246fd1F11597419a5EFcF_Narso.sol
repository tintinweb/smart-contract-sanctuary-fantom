...
contract Narso is ERC20Burnable, Operator, Orchestrator {
    ...
    // Initial distribution for the day 2-5 SPARTAN-WFTM LP -> SPARTAN pool
    ...
    /**
     * @notice Constructs the SPARTAN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("NARSO", "NARSO") {
        // Mints 1 SPARTAN to contract creator for initial pool setup
    ...
    function _getTombPrice() internal view returns (uint256 _tombPrice) {
        try IOracle(tombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            ...
            revert("Spartan: failed to fetch SPARTAN price from Oracle");
        }
    }
		...
		returns (bool)
	{
        ...
        uint256 tombPriceCeiling = currentTombPrice.mul(101).div(100);
        ...
        if (currentTaxRate == 0 || excludedAddresses[msg.sender] || currentTombPrice > tombPriceCeiling) {
    ...
    function transferFrom(
        ...
        if (currentTaxRate == 0 || excludedAddresses[sender] || currentTombPrice > tombPriceCeiling) {
            _transfer(sender, recipient, amount);
        } else {
