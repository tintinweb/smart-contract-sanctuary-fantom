...
contract Oracle is AccessControl {
    ...
    function getOnChainPrice() public view returns (uint256) {
        (uint256 usdcReserve, uint256 deiReserve, ) = IBaseV1Pair(address(pair))
            .getReserves();
        uint256 deiPrice = IBaseV1Pair(address(pair)).getAmountOut(1e18, address(dei)) * 1e12;
        return
            ((deiReserve * deiPrice  / 1e18) + (usdcReserve * 1e12)) * 1e18 / pair.totalSupply();
    }
