...
contract MTRS is Pausable, BalanceManagement {
    ...
    address public farm; // SF
    address public mediator;
    // tokens used as rewards
    address public USDC;
    address public USDT;
    address public MT;
    // token used as locked and reward
    ...
    constructor(address _MTToken, address _farm, address _mediator, address _USDC, address _USDT) {
        farm = _farm;
        mediator = _mediator;
        USDC = _USDC;
        USDT = _USDT;
    ...
    /*
     * @dev lock MT tokens to receive rewards in USDC and USDT
    ...
    /**
     * @dev Withdraw locked MT tokens after locked time
     */
    ...
    /**
     * @dev Transfer USDC, USDT, MT tokens to the user. Have to be
    ...
    /**
     * @dev Receives MT tokens from farm
     * @param _amount is an amount of MT token (lockToken)
     */
    function rewardsFromPenalties(uint256 _amount) public onlyFarm {
        require(_amount > 0, 'Amount can not be zero');
        IERC20(MT).safeTransferFrom(farm, address(this), _amount);
        _notifyReward(MT, _amount);
    }
