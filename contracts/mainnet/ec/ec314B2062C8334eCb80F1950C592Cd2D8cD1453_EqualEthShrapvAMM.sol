...
// File: contracts/EqualEthShrapvAMM.sol
...
contract EqualEthShrapvAMM is Ownable, Pausable {
    ...
    address public constant eth = address(0x74b23882a30290451A17c44f4F05243b6b58C76d);
    ...
    IEqualizerRouter.Routes[] public wrappedToEthPath;
    IEqualizerRouter.Routes[] public ethToShrapPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToEthPath,
        IEqualizerRouter.Routes[] memory _ethToShrapPath
    ) {
        ...
        for (uint i; i < _wrappedToEthPath.length; ++i) {
            wrappedToEthPath.push(_wrappedToEthPath[i]);
        }

        for (uint i; i < _ethToShrapPath.length; ++i) {
            ethToShrapPath.push(_ethToShrapPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedBal, 0, wrappedToEthPath, address(this), block.timestamp);

        uint256 ethHalf = IERC20(eth).balanceOf(address(this)) / 2;
        IEqualizerRouter(unirouter).swapExactTokensForTokens(ethHalf, 0, ethToShrapPath, address(this), block.timestamp);
        
        uint256 lp0Bal = IERC20(eth).balanceOf(address(this));
        uint256 lp1Bal = IERC20(shrap).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(eth, shrap, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
