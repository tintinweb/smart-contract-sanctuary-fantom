...
contract SoulAura is Ownable {
    ...
    function swapBondPower(address member) public view returns (uint raw, uint formatted) {
        // uint TWO = bond.pendingSoul(2, member); // SEANCE-FTM
        uint THREE = bond.pendingSoul(3, member); // FTM-USDC
        uint FOUR = bond.pendingSoul(4, member); // FTM-DAI
        uint FIVE = bond.pendingSoul(5, member); // FTM-BNB
        uint SIX = bond.pendingSoul(6, member); // FTM-ETH
        uint SEVEN = bond.pendingSoul(7, member); // FTM-BTC
        // uint EIGHT = bond.pendingSoul(8, member); // BTC-ETH
        uint NINE = bond.pendingSoul(9, member); // USDC-DAI

        uint power = THREE + FOUR + FIVE + SIX + SEVEN + NINE;

        return (power, fromWei(power));
    }

    function lendBondPower(address member) public view returns (uint raw, uint formatted) {
        uint TEN = bond.pendingSoul(10, member); // FTM-LEND
        uint ELEVEN = bond.pendingSoul(11, member); // DAI-LEND
        uint TWELVE = bond.pendingSoul(12, member); // ETH-LEND
        uint THIRTEEN = bond.pendingSoul(13, member); // BNB-LEND

        uint power = TEN + ELEVEN + TWELVE + THIRTEEN;
        return (power, fromWei(power));
    }
    ...
    function bondPower(address member) public view returns (uint raw, uint formatted) {
        ...
        (uint LendBondPower, ) = lendBondPower(member);

        uint power = SoulBondPower + SwapBondPower + LendBondPower;
