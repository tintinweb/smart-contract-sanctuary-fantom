...
contract BaseV1Router01 is Ownable{
    ...
    function _addLiquidity(
        ...
        (address vTokenA, address vTokenB) = (underlyingToYv(tokenA), underlyingToYv(tokenB));
        (uint ppsA, uint ppsB) = (tokenHasVault(tokenA) ? normalizedPricePerShare(vTokenA) : 1e18, tokenHasVault(tokenB) ? normalizedPricePerShare(vTokenB) : 1e18);
    ...
    function addLiquidityFTM(
        ...
        erc20(address(wftm)).approve(yvwftm, amountFTM);
    ...
    returns (uint[] memory amounts)
    {
        ...
        address yvwftm = underlyingToYv(address(wftm));
        erc20(address(wftm)).approve(yvwftm, amountsCore[0]);
