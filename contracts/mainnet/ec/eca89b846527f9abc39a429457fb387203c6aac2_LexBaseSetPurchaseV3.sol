...
interface IAffliate {
    ...
    function packBuyRew(address user, uint256 amt) external;
...
contract LexBaseSetPurchaseV3 is MjolnirRBAC {
    ...
    function buyWithFTM(uint256 amount) external payable {
        ...
        aff.packBuyRew(msg.sender, amount);
    }
    ...
    function buyWithRNDMAFF(uint256 amount, address refer) external {
        require(aff.getPartnerStatus(refer) == true);
        require(powerOff == false);
        require(rndmOff == false);
        require(packsLeft - amount >= 0);
        rn.transferFrom(msg.sender, devW, amount * rndmPrice * (1e18) * (aff.getPartnerPercent() / 100));
        rn.transferFrom(msg.sender, devW, amount * rndmPrice * (1e18) * ((100 - aff.getPartnerPercent()) / 100));
        bs.mint(msg.sender,1,amount);
        packsLeft = packsLeft - amount;
        aff.packBuyRew(msg.sender, amount);
    }

    function buyWithSAGAAFF(uint256 amount, address refer) external {
        require(aff.getPartnerStatus(refer) == true);
        require(powerOff == false);
        require(sagaOff == false);
        require(packsLeft - amount >= 0);
        sg.transferFrom(msg.sender, devW, amount * sagaPrice * (1e18) * (aff.getPartnerPercent() / 100));
        sg.transferFrom(msg.sender, devW, amount * sagaPrice * (1e18) * ((100 - aff.getPartnerPercent()) / 100));
        bs.mint(msg.sender,1,amount);
        packsLeft = packsLeft - amount;
        aff.packBuyRew(msg.sender, amount);
    }
