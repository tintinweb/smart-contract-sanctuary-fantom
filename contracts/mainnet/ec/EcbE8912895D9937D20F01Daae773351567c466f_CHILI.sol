...
import "../contracts-library/SafeMath8.sol";
import "../contracts-library/Operator.sol";
import "../contracts-library/IOracle.sol";

contract CHILI is ERC20BurnableUpgradeable, Operator {
    ...
    // Initial distribution for the day 2-5 LP -> Lp Pool
    ...
    function __Initialize__() public initializer {
        __initializeOperator__();
        __ERC20_init("CHILI", "ChiliTomb");
    ...
    function _getChiliPrice() internal view returns (uint256 _StablePrice) {
        try IOracle(_chiliOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("Stable: failed to fetch Stable price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _StablePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (
                ...
                if (_StablePrice >= taxTiersTwaps[tierId]) {
                    require(
    ...
    /**
     * @notice Operator mints Stable to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Stable to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentStablePrice = _getChiliPrice();
            currentTaxRate = _updateTaxRate(currentStablePrice);
            if (currentStablePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeGenesisReward(address _rewardsPoolAddress)
        external
        onlyOperator
    {
    ...
    function distributeLpPoolReward(address _rewardsPoolLPAddress)
        external
        onlyOperator
    {
