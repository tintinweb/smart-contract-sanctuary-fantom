...
import {IMonastery} from "../interfaces/IMonastery.sol";

/// @title Marketplace adapter for ERC4626 vaults.
contract AdapterERC4626 is Ownable, Multicall {
    using SafeERC20 for IERC20;

    address public immutable monastery;
    address public immutable reliquary;

    error NotOwner();

    constructor(address _monastery) {
        monastery = _monastery;
        reliquary = IMonastery(_monastery).reliquary();
    }

    /// @notice List Relic `relicId` with premium `premium` converted to vault shares.
    /// @dev May be implemented more efficiently off-chain.
    function listRelic(uint relicId, uint premium) external {
        if (msg.sender != IReliquary(reliquary).ownerOf(relicId)) revert NotOwner();

        uint premiumInShares = _getPremiumInShares(relicId, premium);
        IMonastery(monastery).listRelic(relicId, premiumInShares);
    }

    /// @notice Modify the premium of your listing for Relic `relicId` with `newPremium` converted to vault shares.
    /// @dev May be implemented more efficiently off-chain.
    function changePremium(uint relicId, uint newPremium) external {
        IMonastery _monastery = IMonastery(monastery);
        if (msg.sender != _monastery.getOwner(relicId)) revert NotOwner();

        uint premiumInShares = _getPremiumInShares(relicId, newPremium);
        _monastery.changePremium(relicId, premiumInShares);
    }

    /// @notice Buy Relic `relicId` with the vault's underlying asset converted to shares equal to those in the Relic
    /// plus the premium.
    function buyRelic(uint relicId) external {
        IReliquary _reliquary = IReliquary(reliquary);
        PositionInfo memory position = _reliquary.getPositionForId(relicId);
        IERC4626 vault = IERC4626(_reliquary.poolToken(position.poolId));

        IMonastery _monastery = IMonastery(monastery);
        uint assets = vault.previewMint(position.amount + _monastery.getPremium(relicId));

        IERC20 asset = IERC20(vault.asset());
        asset.safeTransferFrom(msg.sender, address(this), assets);
        if (asset.allowance(address(this), address(vault)) == 0) {
            asset.approve(address(vault), type(uint).max);
        }
        vault.deposit(assets, address(this));

        if (vault.allowance(address(this), address(_monastery)) == 0) {
            vault.approve(address(_monastery), type(uint).max);
        }
        _monastery.buyRelic(relicId, msg.sender);
    }
    ...
    function _getPremiumInShares(uint relicId, uint premium) internal view returns (uint) {
        IReliquary _reliquary = IReliquary(reliquary);
        PositionInfo memory position = _reliquary.getPositionForId(relicId);
        IERC4626 vault = IERC4626(_reliquary.poolToken(position.poolId));
        return vault.convertToShares(premium);
    }
}
