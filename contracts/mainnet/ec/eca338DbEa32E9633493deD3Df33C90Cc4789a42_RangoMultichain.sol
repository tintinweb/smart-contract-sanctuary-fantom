...
contract RangoMultichain is IRangoMultichain, BaseContract {
    ...
    function multichainBridge(
        ...
        if (_actionType == RangoMultichainModels.MultichainBridgeType.OUT) {
            router.anySwapOut(_fromToken, _receiverAddress, _inputAmount, _receiverChainID);
        } else if (_actionType == RangoMultichainModels.MultichainBridgeType.OUT_UNDERLYING) {
            router.anySwapOutUnderlying(_fromToken, _receiverAddress, _inputAmount, _receiverChainID);
        } else if (_actionType == RangoMultichainModels.MultichainBridgeType.OUT_NATIVE) {
            router.anySwapOutNative{value: msg.value}(_fromToken, _receiverAddress, _receiverChainID);
        } else {
            revert();
        }

        emit MultichainBridge(_actionType, _fromToken, _inputAmount, _multichainRouter, _receiverAddress, _receiverChainID);
    }
}
