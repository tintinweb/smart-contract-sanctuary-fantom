...
contract AToken is ERC20,Ownable {
    ...
    address private BORN_ADDRESS = 0x000000000000000000000000000000000000000C;
    address private TEAM_ADDRESS = 0x000000000000000000000000000000000000000A;
    address private BACK_ADDRESS = 0x000000000000000000000000000000000000000b;
...
//    address private ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    bool public lpShareAutoFlag = false;
    uint256 public rewardPoolA;
    uint256 public rewardPoolB;
    uint256 public rewardPoolC;
    uint256 public rewardPoolD;
    uint256 public currentIndex;
    uint256 public holderRewardCondition = 5 * 1e18;
    uint256 public progressRewardBlock;
    TokenDistributor public tokenDistributor;


    constructor() public ERC20('V5 Token', 'V5'){
        ...
        _mint(msg.sender, MAX.sub(1000*1e18));
        _mint(address(0xcCc006da90F89DB6251E4FCA80171E29a2f0a647), 1000*1e18);
    }
    ...
    function transferBase(address from,address to,uint value) internal returns (uint){
        ...
        if (swapPairList[from] || swapPairList[to]) {
            if (!feeWhiteList[from] && !feeWhiteList[to]) {
                ...
                if(swapPairList[to]){
                    ...
                    lpShareTotal = lpShareTotal.add(bornAmount);

                    uint teamAmount = value.mul(3).div(100);
                    rewardPoolA = teamAmount.mul(50).div(100);
                    rewardPoolB = teamAmount.mul(25).div(100);
                    rewardPoolC = teamAmount.mul(15).div(100);
                    rewardPoolD = teamAmount.mul(10).div(100);

                    super._transfer(from, address(this), bornAmount.add(teamAmount));
                    super._transfer(from, TEAM_ADDRESS, teamAmount);
                    transferAmount  = transferAmount.sub((bornAmount.add(teamAmount)).mul(2));
                }
                //BUY
                if(swapPairList[from]){
                    uint teamAmount = value.mul(1).div(100);
                    super._transfer(from, BORN_ADDRESS, teamAmount);
                    super._transfer(from, TEAM_ADDRESS, teamAmount);
                    super._transfer(from, BACK_ADDRESS, teamAmount);
                    super._transfer(from, address(this), teamAmount);
                    super._transfer(from, address(tokenDistributor), teamAmount);
                    transferAmount  = transferAmount.sub(teamAmount.mul(5));
                }
            }
        }
        if (from != address(this)) {
            ...
            if (lpShareAutoFlag && lpShareTotal >= holderRewardCondition && block.number > progressRewardBlock + 20) {
                swapTokenToUsdt(lpShareTotal);
                processHoldLpReward(500000);
            }
        }
    ...
    function swapTokenToUsdt(uint256 tokenAmount) private lockTheSwap {
        ...
        path[0] = address(this);
        path[1] = USDT;
        ISwapRouter(ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenAmount, 0, path, address(this), block.timestamp);
    }
    ...
    function processHoldLpReward(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 0 && !excludeHolder[shareHolder]) {
                ...
                if (amount > 0) {
                    usdtToken.transferFrom(address(this), shareHolder, amount);
                }
            }
    ...
    function adminProcessHoldLpReward2() public onlyOwner {
        processHoldLpReward(500000);
    }
...
//        address(0xF491e7B69E4244ad4002BC14e878a34207E38c29),//Router Address
//        address(0x049d68029688eAbF473097a2fC38ef61633A3C7A)//USDT Address
