...
contract PopPussiesPoppedOff is ERC721Enumerable, Ownable, ERC2981 {
  ...
  uint256[39] public idsToMint;
  uint256 private idsToMintCounter = 0;
  uint256 private anotherCounter = 0;
  ...
  /*function _pickRandomUniqueId(uint256 _random) private returns (uint256 id) {
    ...
    function mint(address token, uint amount) external payable {
        ...
        if (token == address(0)) {
            ...
            //require(IERC20(token).transferFrom(msg.sender, address(this), amount * acceptedCurrencies[address(wftm)]), "Payment not successful");
    ...
    function _mintInternal(address _token, uint _amount) internal {
        for (uint256 i = 1; i <= _amount; ++i) {
            ...
            _safeMint(msg.sender, idsToMint[idsToMintCounter]);
            idsToMintCounter++;
        }
    }
  ...
  //Will have to call this function 30 times. Not sure how to make it more gas efficient
  function mintFirstHalf_N89(address[] calldata _addresses, uint[] calldata _ids) external onlyOwner {
    ...
    for(uint256 i = 1; i <= 25; i++) {
      if(_addresses[i-1] == address(0)) {
        idsToMint[anotherCounter] = _ids[i-1];
        anotherCounter++;
      }
      else { 
        _safeMint(_addresses[i-1], _ids[i-1]);
      }
    }
  }
  ...
  function withdraw(address token) external onlyOwner {
    ...
    IERC20(token).transfer(team[0], amount * 10 / 100);
    IERC20(token).transfer(team[1], amount * 10 / 100);
    IERC20(token).transfer(team[2], amount * 80 / 100);
  }
}
