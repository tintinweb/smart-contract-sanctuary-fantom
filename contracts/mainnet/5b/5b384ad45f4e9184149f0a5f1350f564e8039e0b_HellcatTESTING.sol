...
Overview of features
    ...
    The largest portion of tax is converted to FTM and distributed on the wallet to seperate 'pots' or reserves.
...
Buy tax
Tax: 10% (slippage 11%)
4% Payouts in $BTCB, $ETH, $BUSD, $MATIC, $ADA, $FTM & $HELLCAT, more added later
3% Auto buyback 
1% Gas Refunds 
1% Marketing in $FTM
0.6% admin 
...
Payouts
→ all the sellers tax is given back in multiple tokens → no $HELLCAT price dump from selling the sell tax
Out of the seling tax:
50% holders in $BTCB, $ETH, $ADA, $MATIC, $FTM, $BUSD & $HELLCAT
...
// Hellcat Finance contract

contract HellcatTESTING is ERC20, ReentrancyGuard {
    string public constant symbol = "HELLCAT TESTING"; //HELLCAT
    string public constant name = "Hellcat Finance TESTING"; //Hellcat Finance
    uint8 public constant decimals = 18;

    //1,000,000+18 zeros
    uint256 constant initialSupply = 1000000000000000000000000000000000; //1 million tokens
   ...
   uint256 public claimableFTMearnings;
 ...
 //Tax reserves and pots kept public for maximum transparency  
    uint256 public LQPotFTM;
    uint256 public LQPotToken;
    uint256 public MarketingFTM;
    
    uint256 public BuybackPotFTM;
    uint256 public GasPotFTM;
    uint256 public HolderPotFTM;
    uint256 public AdminPotFTM;
    uint256 public ModPotFTM;
    uint256 public AirdropPot;
    uint256 public RafflePot;
    uint256 public LotteryPot;
    uint256 public NextBuybackMemberCount;
    
    uint256 private HolderPotFTMcalc;
    uint256 private AdminPotFTMcalc;
    uint256 private ModPotFTMcalc;
   ...
   uint256 public SellTaxFTMPot;

   
   uint256 public BuyTaxTokenPot;
   uint256 public BuyTaxFTMPot;
    ...
    modifier LockedafterLaunch() {
      ...
      doWork(NextTask);
       ...
       modifier RefundGas() {
        ...
        if(GasUsed<=GasPotFTM)
        {
        GasPotFTM -= GasUsed;
        if(SendFTMfromContract(GasUsed, msg.sender)==false)
        GasPotFTM += GasUsed;
        }
        else  // in case of emergency, pump gas from buyback pot to fuel work
        if((GasUsed<=BuybackPotFTM)&&(msg.sender==WorkHelperWallet))
        {
        BuybackPotFTM -= GasUsed;
        if(SendFTMfromContract(GasUsed, msg.sender)==false)
        BuybackPotFTM += GasUsed;
        }
...
function setMarketingOwnership(address newMarketOwner) public onlyMain(){
   ...
   // address constant routerAddress=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    //MainNet
    address constant routerAddress=0xF491e7B69E4244ad4002BC14e878a34207E38c29;
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(_pancakeRouter.WETH(), "FTM"); //FTM added as default
   AddTokentoPayoutList(address(this), "HELLCAT"); //this contract token
  ...
  function UpdateRegister(address recipient, bool ExcludedfromTax) private
   {
        ...
        _AutoPayoutEnabled[recipient] = true; // only human holders get payed auto rewards as default
         ...
         _RewardTokens[recipient].Slice1 = 75; //default 75% FTM
         _RewardTokens[recipient].Slice1 = 12; //12% HELLCAT
         _RewardTokens[recipient].Token1 = 13; 
         _RewardTokens[recipient].Token2 = 0; 
   ...
   function addtoAdminList(address addr) public onlyMain() LockedafterLaunch()
    ...
    function _PayTeam() private {
        ...
        if(SendFTMfromContract(MarketingFTM, MarketingWallet)==true) 
        MarketingFTM = 0;
        {//admin
        
        uint256 L = AdminList.length;
         if((L>0)&&(AdminPotFTM > 0))
           { uint256 Adminshare = AdminPotFTM / N_Admin;
              AdminPotFTM = 0;
            for(uint32 pos = 0 ; pos < L; pos++)
             if(_isAdmin[AdminList[pos]])
                SendFTMfromContract(Adminshare, AdminList[pos]);
          }
        }
        //mod
        {
         ...
         if((L>0)&&(ModPotFTM > 0))
           { uint256 Modshare = ModPotFTM / N_Mods;
              ModPotFTM = 0;
            for(uint32 pos = 0 ; pos < L; pos++)
             if(_isMod[ModList[pos]])
                SendFTMfromContract(Modshare, ModList[pos]);
        }
        }
}
       ...
       require(amount<=_maxTxAmount);
         if((recipient==_pancakePairAddress))                   //sell
           { SellSlippage = getSellSlippage();
             if(SellSlippage>15) //sell tax portion higher than 15% is sent seperately 
             {
                 TaxTokens = amount*15/100;
                 AdditionalTax = amount*(SellSlippage-15)/100; //
             }
             else
...
{   //payouts
        ...
        //4% admin in FTM
        //2% mod in FTM
    if(SellTaxFTMPot>10000)  //skip if pot is empty
    {
          //since the remaining 79.5% is converted to FTM, the calculation percentages need to be normalized.
      //50% total tax * 100/79.5    = 62.89 % FTM tax portion
      //17.5% total tax * 100/79.5  = 22.01 % FTM tax portion
      //6% total tax * 100/79.5     = 7.55 % FTM tax portion
      //4% total tax * 100/79.5     = 5.03 % FTM tax portion
      //2% total tax * 100/79.5     = 2.52 % FTM tax portion
                            //      +=> 100% FTM tax portion  
                            
                     //FTMfromTax = BalanceFTM - FTMfromTax; 
         //50% holder FTM
            HolderPotFTM += SellTaxFTMPot * 6289 / 10000;  //adds to holder reward pot
     
       //6% promo FTM   
         MarketingFTM += SellTaxFTMPot * 755 / 10000;
             
         //4% admin in FTM      
            AdminPotFTM += SellTaxFTMPot * 503 / 10000;
        
        //2% mod in FTM     
            ModPotFTM += SellTaxFTMPot * 252 / 10000;
        
            //35% lq to pancake lq 
     {       
          //  netamount -= LQTax;
     LQPotFTM += SellTaxFTMPot * 2200 / 10000; // half of 35% liquidity tax converted to FTM, rest is tokens on contract
     SellTaxFTMPot = 0; //all tax distributed
     }
      }        
   
}

function addLiquidityFromContract(uint256 TKN, uint256 FTM, bool fromLQpool) public onlyMain()
{ 
    if(fromLQpool)
    sendLiquidity();
    else{
        require(!_isLaunched);
        _addLiquidityFromContract(TKN, FTM);
    }
}

function sendLiquidity() private returns (bool)
{
            if((LQPotToken > 1000)&&(LQPotFTM>=1000000000000000000)) // transfer at least 1 FTM
            {
            (uint TKN, uint FTM) = _addLiquidityFromContract(LQPotToken, LQPotFTM); //sends tokens from contract along with LQ tax
...
function DistributeBuyTax() internal
{
     ...
     {
        //40% buybackpot
        BuybackPotFTM += BuyTaxFTMPot * 30 / 100;
        GasPotFTM += BuyTaxFTMPot * 10 / 100; //1% goes to gas fee refunds
        
        //40% holders in FTM 
        HolderPotFTM += BuyTaxFTMPot * 40 / 100;
        
        //10% marketing FTM
        
         MarketingFTM += BuyTaxFTMPot * 10 / 100;

        //6% admin in FTM
        
        AdminPotFTM += BuyTaxFTMPot * 6 / 100;
        
        //4% mods in FTM
        
        ModPotFTM += BuyTaxFTMPot * 4 / 100;
        BuyTaxFTMPot = 0;
     }    
}
...
{ //work split between transfers and tasks
    if(task == 0)
    {
       //convert tax to FTM and distribute to tax pots
       _swapTaxTokenForFTM(_maxTxAmount,_maxTxAmount); //swap accumilated tax tokens to FTM up to _maxTxAmount   
       DistributeBuyTax;
       DistributeSellTax;
     ...
     if(_CalculatingRewards == true)
       ...
       else
       {
        if(block.timestamp - LastRewardCalculationTime > 7200) //recalculated every 2 hour 7200
            if(HolderPotFTM >= 500000000000000000) //wait until at least 0.5 FTM in rewards have been accumilated 
...
function checkRewards(address addr) public view returns (uint256)
{
   return _earnings[addr];
    ...
    } // fallback to receive FTM
    
function ReservesFTM() public view returns (uint256)
{
...
function ReservesToken() public view returns (uint256)
{
    ...
    function _addLiquidityFromContract(uint256 tokenamount, uint256 FTMamount) private returns (uint, uint){
     ...
     _approveMore(Contract, address(_pancakeRouter), tokenamount); //test if works for third parties also
       (uint amountToken, uint amountETH, ) = _pancakeRouter.addLiquidityETH{value: FTMamount}(
            Contract,
            tokenamount,
            0,
            0,
...
function SendFTMfromContract(uint256 amountFTM, address receiver) private returns (bool) 
{
  (bool success, ) = receiver.call{ value: amountFTM }(new bytes(0));
...
function swapTaxTokenForFTM(uint256 selltaxamount,uint256 buytaxamount) public onlyMain(){
    _swapTaxTokenForFTM(selltaxamount,buytaxamount);
}


function _swapTaxTokenForFTM(uint256 selltaxamount,uint256 buytaxamount) private returns (bool){
  ...
  if(total > 0)
  {
      ...
      //split sell portion into token and FTM portions
   ...
   {uint256 tokenportionofselltax = Sell * 205/1000;
       ...
       //since 20.5% of the tax is not converted to FTM, the calculation percentages need to be normalized. 
    ...
    uint256 additionalLiquidity = Sell * 1/100; //0.5% FTM 0.5% token added additionally to liquidity from contract balance
 
   
   if(__balanceOf[Contract] > total + additionalLiquidity + getReservedTokens()) //enough available tokens for additional liquidity
   {
   FTMs = _swapTokenForFTM(total+additionalLiquidity/2-tokenportionofselltax); //tax + 0.5%
   uint256 extraLQFTM = FTMs * 1/202; //0.5%
   FTMs -= extraLQFTM;
   LQPotFTM += extraLQFTM;
   if(FTMs>0) //swap successful
   LQPotToken += additionalLiquidity/2;
   }
   }
   else
   {
    FTMs = _swapTokenForFTM(total);   
   }

if(FTMs>0)
{
   uint256 buyFTMs = FTMs * Buy / total;
   BuyTaxFTMPot += buyFTMs;
   SellTaxFTMPot += FTMs - buyFTMs;
}
else
...
function _swapTokenForFTM(uint256 tokenamount) private returns (uint256){ 
  if(tokenamount>0)
  {
...
//works for receiver != address(this) 
 function _swapFTMforChosenTokenandPayout(uint256 amountFTM,address PayoutTokenContract, address receiver) private returns (bool){

        address[] memory path = new address[](2);
        path[0] = _pancakeRouter.WETH();
        path[1] = PayoutTokenContract;

    try _pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountFTM}(
            0,
            path,
            address(receiver),
            block.timestamp
...
/*function quotepriceToken(uint256 FTMs) public view returns (uint256)
{
    address[] memory path = new address[](2);
        path[0] = _pancakeRouter.WETH();
        path[1] = address(this);
        uint[] memory amounts = _pancakeRouter.getAmountsOut(FTMs, path); //returns (uint[] memory amounts)
        return amounts[1];
}*/

/*function quotepriceFTM(uint256 Tokens) public view returns (uint256)
{
...
function getFreeFTMs() public view returns (uint256)
{
  uint256 reservedFTMs = LQPotFTM + BuybackPotFTM + HolderPotFTM + AdminPotFTM + ModPotFTM;
  reservedFTMs += MarketingFTM + SellTaxFTMPot + BuyTaxFTMPot + GasPotFTM + claimableFTMearnings;
  return address(this).balance - reservedFTMs;
}

//extra funds can also be sent and distributed
//use with caution. To recover FTM sent to contract without allocation 
function addFTMtoHolderPot(uint256 amountFTM) public onlyMain()
{ 
  require(amountFTM <= getFreeFTMs());
   HolderPotFTM += amountFTM;
}

function addFTMtoGasPot(uint256 amountFTM) public onlyMain()
{ 
  require(amountFTM <= getFreeFTMs());
   GasPotFTM += amountFTM;
}
...
function _buyBackTokens(uint256 amountFTM) private returns (uint256){  
     ...
     uint[] memory amounts = _pancakeRouter.swapExactETHForTokens{value: amountFTM}(
            0,
            path,
...
function _StartRewardCalculation() private {
   ...
   {
   HolderPotFTMcalc = HolderPotFTM; //used as reference for calculation
   AdminPotFTMcalc =  AdminPotFTM;
   ModPotFTMcalc = ModPotFTM;
...
function CalculateRewards(uint32 counts) private returns (bool){
   ...
   {   
    ...
    uint32 N_calculated;
        ...
        if((CircSupplycalc>0)&&(HolderPotFTMcalc>0))//safety check
        {
        ...
        // HolderPotFTM += Pot; //earnings distributed
         return true;
    }
   }
     return false;
...
{      
         ...
         {  _LastRewardCycle[holder] = Rewardcycle; 
            ...
            uint256 NewEarnings = HolderPotFTMcalc * bal / CircSupplycalc; 
            if(NewEarnings <= HolderPotFTM) //safety check, should always be true
            {
            HolderPotFTM -= NewEarnings;  
            claimableFTMearnings += NewEarnings;
...
function ClaimRewards(address receiver) private returns(bool){
  ...
  {  _LastPayout[receiver] = block.timestamp;
      uint256 amountFTM = _earnings[receiver];
     uint256 dist = amountFTM;
     claimableFTMearnings -= amountFTM;
     _earnings[receiver] = 0; //reentrancy guard
    if(PromoPerc > 0)//promoted token of the week
    {
      uint256 promotokenFTM = amountFTM * PromoPerc / 100;
     if( _swapFTMforChosenTokenandPayout(promotokenFTM,PayoutTokenList[RewardTokenoftheWeek],receiver))
     {
       amountFTM -= promotokenFTM;
       //distributed += promotokenFTM;
     }
    }
    ...
    address PayoutTokenContract = PayoutTokenList[_RewardTokens[receiver].Token1];
     if(PayoutTokenContract == _pancakeRouter.WETH()) //payout FTM
     {
         
       if(SendFTMfromContract(amountFTM, receiver))
        {   
          // distributed += amountFTM;
           amountFTM = 0;
        }
     }
       else
     { 
         if(PayoutTokenContract == address(this)) //native token
          SingleUntaxedTransfer = true;
        if(_swapFTMforChosenTokenandPayout(amountFTM,PayoutTokenContract,receiver))
    { 
     amountFTM = 0;
    }
     }
    ...
    slices[5] =   _RewardTokens[msg.sender].Slice6;
      ...
      uint256 slicevalue = amountFTM / 100; //1%
      ...
      {uint256 FTMslice;
       if((slices[i] > 0)&&(pie <= 100))
       {
        ...
        if(pie+percslice>100)// check for sneaky percentages
         percslice = 100-pie;
         pie += percslice;
         FTMslice = slicevalue * percslice;
         if(FTMslice>amountFTM)
          FTMslice = amountFTM; //safety check
      PayoutTokenContract = PayoutTokenList[tokennumbers[i]];
      
      if(PayoutTokenContract == _pancakeRouter.WETH()) //payout FTM
    {
       if(SendFTMfromContract(FTMslice, receiver))
          { amountFTM -= FTMslice; 
    ...
    {   if(PayoutTokenContract == address(this)) //native token
         SingleUntaxedTransfer = true;
      if(_swapFTMforChosenTokenandPayout(FTMslice,PayoutTokenContract,receiver)){ 
       amountFTM -= FTMslice;
   ...
   _earnings[receiver] = amountFTM; //if anything is still left, it gets refunded to the holder earnings pot
  distributedRewards += dist - amountFTM;
  claimableFTMearnings += amountFTM;  
...
function doBuyback() public returns (bool) //set to private
{
   ...
   {
     uint256 amountFTM = BuybackPotFTM * 40 / 100; //max 40% of pot per transaction
      
      uint256 tokens = __balanceOf[MainWallet];

    //burn tokens
    SingleUntaxedTransfer = true;
    BuybackPotFTM -= amountFTM;
    if(_swapFTMforChosenTokenandPayout(amountFTM,address(this),MainWallet)==false) //transaction did not revert
    BuybackPotFTM += amountFTM; //failed put back into pot
    else
    {//success
