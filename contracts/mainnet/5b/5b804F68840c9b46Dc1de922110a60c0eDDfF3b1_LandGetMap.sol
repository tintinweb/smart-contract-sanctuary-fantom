...
// File: contracts/LandGetMap.sol

pragma solidity ^0.8.13;

contract LandGetMap is _LandStorage {
    using EnumerableSet for EnumerableSet.UintSet;

    _Delegator private _land;
    uint256 _delegateMethod;

    constructor(address land, uint256 delegateMethod) {
        _land = _Delegator(land);
        _delegateMethod = delegateMethod;
    }

    function getMapDelegate(uint8 x, uint8 y)
        external
        view
        ...
        return (
            ...
            farmId == EMPTY_FARM
                ...
                : (ls.ownedFarms[farmId] != ZERO_ADDRESS ||
                    ls.vacantFarms.contains(farmId))
                ? farmId
                : EMPTY_FARM
        );
    }

    function getMap(uint8 x, uint8 y) external returns (uint8, uint16) {
        return
            abi.decode(
                _land.delegateCall(
                    address(this),
