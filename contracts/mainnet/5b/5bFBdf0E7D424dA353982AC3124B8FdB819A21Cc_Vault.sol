...
/// USD in the contract means plain US Dollar, it is equivalent to take a
/// token amount and divide it by 10^token.decimals().
...
contract Vault is ERC20, Ownable {
  ...
  uint256 public minDepositInUSD;
  uint256 public maxDepositInUSD;

  // Users cannot redeem more than this amount of oneUSD tokens per transaction.
  uint256 public maxAmountOfOneUSDToWithdraw;
  uint256 public minAmountOfOneUSDToWithdraw;
  ...
  event ChangedMaxDepositUSD(uint256 oldMaxDepositInUSD, uint256 newMaxDepositAmountInUSD);
  event ChangedMinDepositUSD(uint256 oldMinDepositAmountInUSD, uint256 newMinDepositAmountInUSD);
  event ChangedMaxAmountOneUSDToWithdraw(uint256 oldMaxAmountOneUSD, uint256 newMaxAmount);
  event ChangedMinAmountOneUSDToWithdraw(uint256 oldMinAmountOneUSD, uint256 newMinAmount);
  ...
  error Vault_Invalid_New_Max_Deposit_USD();
  error Vault_Invalid_New_Min_Deposit_USD();
  error Vault_Incorrect_Strategy_Manager();
  error Vault_Incorrect_Slippage();
  error Vault_Invalid_Max_OneUSD_Amount_To_Withdraw();
  error Vault_Invalid_Min_OneUSD_Amount_To_Withdraw();
  ...
  constructor() ERC20("testOneUSD","oUSD") {
    slippage = 995;
    maxAmountOfOneUSDToWithdraw = 10000 * (10**decimals());
    minAmountOfOneUSDToWithdraw = 10 * (10**decimals());
    maxDepositInUSD = 10000;
    minDepositInUSD = 10;
  }
  ...
  function deposit(address token, uint256 amountToDeposit) 
    external 
    vaultIsOpen 
    ...
    returns(uint256) 
  {
    ...
    if(amountToDepositInUSD < minDepositInUSD) {
    ...
    uint256 amountMinted = _deposit(msg.sender, token, amountToDeposit,tokenDecimals);

    // Ideally we should mint 1:1 oneUSD for each USD deposited
    uint256 expectedOneUSDTokens = (amountToDeposit*(10**decimals()))/(10**tokenDecimals);

    // The minimum oneUSD tokens we are willing to accept
    uint256 minimumOneUSDTokens = (expectedOneUSDTokens*slippage)/1000;

    if(amountMinted < minimumOneUSDTokens) {
      revert Vault_Too_Much_Slippage();
    }

    lastBlockDepositWasCalled = block.number;

    emit Deposit(msg.sender, token, amountToDeposit, amountMinted);

    return amountMinted;
  }

  function _deposit(address beneficiary, address token, uint256 amountToDeposit, uint256 tokenDecimals) 
    internal 
    returns(uint256) 
  {

    // We send all the tokens to the strategy manager
    IERC20(token).safeTransferFrom(beneficiary, strategyManager, amountToDeposit);
    ...
    uint256 amountToMint = _newLiquidityInUSD*(10**decimals());

    _mint(beneficiary, amountToMint);

    /// INVARIANT
    /// The TVL of the contract must be greater or equal than the circulating supply of oneUSD
    /// So we can ensure liquidity for all the oneUSD
    assert(totalBalanceInUSD() >= totalSupply()/(10**decimals()));
  ...
  function withdraw(uint256 amount) external validBlockNumberForWithdraw vaultIsOpen returns(uint256) {
    ...
    if(amount < minAmountOfOneUSDToWithdraw) {
      revert Vault_Cannot_Withdraw_Less_Than_The_Minimum();
    }

    if(amount > maxAmountOfOneUSDToWithdraw)  {
      revert Vault_Cant_Withdraw_That_Amount();
    }
    ...
    uint256 usdcToWithdraw = (amount * 10**6)/(10**decimals());
  ...
  function _withdraw(address beneficiary, uint256 usdcToWithdraw) internal returns (uint256) {
    ...
    IERC20(USDC_CONTRACT).safeTransfer(beneficiary,usdcToSentToUser);
  ...
  function depositUSDCToExitPool(uint256 amount) external onlyOwner {
    ...
    IERC20(USDC_CONTRACT).safeTransferFrom(msg.sender, address(this), amount);
  ...
  function changeMaxDepositUSD(uint256 newMaxDepositAmountInUSD) external onlyOwner {
    if(newMaxDepositAmountInUSD <= minDepositInUSD) {
      revert Vault_Invalid_New_Max_Deposit_USD();
    }

    uint256 oldMaxDepositInUSD = maxDepositInUSD;
    maxDepositInUSD = newMaxDepositAmountInUSD;

    emit ChangedMaxDepositUSD(oldMaxDepositInUSD, newMaxDepositAmountInUSD);
  }

  function changeMinimumDepositUSD(uint256 newMinDepositAmountInUSD) external onlyOwner {
    if(newMinDepositAmountInUSD >= maxDepositInUSD) {
      revert Vault_Invalid_New_Min_Deposit_USD();
    }
    
    uint256 oldMinDepositAmountInUSD = minDepositInUSD;

    minDepositInUSD = newMinDepositAmountInUSD;

    emit ChangedMinDepositUSD(oldMinDepositAmountInUSD, newMinDepositAmountInUSD);
  }
  ...
  function changeMaxAmountOneUSDToWithdraw(uint256 newMaxAmount) external onlyOwner {
    if(newMaxAmount <= minAmountOfOneUSDToWithdraw) {
      revert Vault_Invalid_Max_OneUSD_Amount_To_Withdraw();
    }
    
    uint256 oldMaxAmountOneUSD = maxAmountOfOneUSDToWithdraw;
    maxAmountOfOneUSDToWithdraw = newMaxAmount;

    emit ChangedMaxAmountOneUSDToWithdraw(oldMaxAmountOneUSD, newMaxAmount);
  }

  function changeMinAmountOneUSDToWithdraw(uint256 newMinAmount) external onlyOwner {
    if(newMinAmount >= maxAmountOfOneUSDToWithdraw) {
      revert Vault_Invalid_Min_OneUSD_Amount_To_Withdraw();
    }

    uint256 oldMinAmountOneUSD = minAmountOfOneUSDToWithdraw;
    minAmountOfOneUSDToWithdraw = newMinAmount;

    emit ChangedMinAmountOneUSDToWithdraw(oldMinAmountOneUSD, newMinAmount);
  }
