...
contract PaymentSplitter is IPaymentSplitter, Cloneable, Ownable {
    ...
    mapping(IERC20 => SwapSettings) public tokenSwapSettings;
    mapping(address => bool) public payeeSwapsTokens;

    uint256 public constant VERSION = 2022052501;
    ...
    function authorizeTokenSwapRouter(
        IERC20 token,
        IUniswapV2Pair pairToken,
        IUniswapV2Router01 swapRouter
    ) public onlyOwner whenInitialized {
        require(pairToken.token0() == address(token) || pairToken.token1() == address(token), "Invalid pair for token");
        tokenSwapSettings[token] = SwapSettings({ pairToken: pairToken, swapRouter: swapRouter });
    }

    function setPayeeSwapsTokens(address payee_, bool state) public onlyOwner whenInitialized {
        payeeSwapsTokens[payee_] = state;
    }
    ...
    function release(address token, address account) public {
        ...
        if (
            payeeSwapsTokens[account] &&
            address(tokenSwapSettings[IERC20(token)].pairToken) != address(0) &&
            address(tokenSwapSettings[IERC20(token)].swapRouter) != address(0)
        ) {
            (uint256 amountOut, bool isETH, IERC20 outputToken) = _swapToken(IERC20(token), payment);
            if (isETH) {
                (bool sent, ) = account.call{ value: amountOut }("");
                require(sent, "could not disperse funds");
            } else {
                outputToken.safeTransfer(account, amountOut);
            }
        } else {
    ...
    function _swapToken(IERC20 token, uint256 amountIn)
        internal
        returns (
            uint256 amountOut,
            bool isEth,
    ...
    {
        require(
            address(tokenSwapSettings[token].pairToken) != address(0) &&
                address(tokenSwapSettings[token].swapRouter) != address(0),
            "Invalid swap token"
        );

        SwapSettings memory settings = tokenSwapSettings[token];

        address token0 = settings.pairToken.token0();
        address token1 = settings.pairToken.token1();
        ...
        token.approve(address(settings.swapRouter), amountIn);

        if (path[1] == settings.swapRouter.WETH()) {
            uint256 startBalance = address(this).balance;
            settings.swapRouter.swapExactTokensForETH(
                amountIn, // amountIn
                0, // amountOutMin: take whatever we can get
                path, // path
                address(this), // to
                block.timestamp // deadline
            );
            amountOut = address(this).balance - startBalance;
            isEth = true;
            outputToken = IERC20(path[1]);
        } else {
            uint256 startBalance = IERC20(path[1]).balanceOf(address(this));
            settings.swapRouter.swapExactTokensForTokens(
                amountIn, // amountIn
                0, // amountOutMin: take whatever we can get
                path, // path
                address(this), // to
            ...
            amountOut = IERC20(path[1]).balanceOf(address(this)) - startBalance;
            isEth = false;
            outputToken = IERC20(path[1]);
        }
    }
...
// File contracts/ERC721.Tiered/ERC721.Tiered.sol
...
contract ERC721Tiered is ERC721Enumerable, ERC721Royalty, ERC721Burnable, Ownable, Pausable, Cloneable {
    ...
    uint256 public constant VERSION = 202202501;
    ...
    function freeMint(uint256 count) public onlyOwner returns (uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](count);
        for (uint256 i = 0; i < count; i++) {
            tokenIds[i] = _mintNFT(_msgSender());
        }
    ...
    function setProceedsRecipient(address[] memory recipients, uint256[] memory shares) public onlyOwner {
        ...
        PROCEEDS_RECIPIENT.transferOwnership(_msgSender());
    ...
    function setRoyalty(
        ...
        ROYALTY_RECEIVER.transferOwnership(_msgSender());
