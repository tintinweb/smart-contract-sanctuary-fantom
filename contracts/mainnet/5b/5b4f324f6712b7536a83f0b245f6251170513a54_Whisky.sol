...
contract Whisky is ERC20Burnable, Operator, Orchestrator {
    ...
    /**
     * @notice Constructs the SPINACH ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("WHISKY", "WHISKY") {
        // Mints 1 SPINACH to contract creator for initial pool setup
...
function  totalSupply() public view override returns (uint256) {
    ...
    function _getTombPrice() internal view returns (uint256 _tombPrice) {
        try IOracle(tombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            ...
            revert("Spinach: failed to fetch SPINACH price from Oracle");
        }
    }
	...
	function transfer(address recipient, uint256 amount)
		...
		returns (bool)
	{
        require(!_includeToBlackList[msg.sender] || !_includeToBlackList[recipient], "This address is blacklisted");
        uint256 currentTaxRate = 0;
        bool burnTax = false;

        if (autoCalculateTax) {
            currentTaxRate = _updateTaxRate(_getTombPrice());
            if (_getTombPrice() < burnThreshold) {
                burnTax = true;
            }
        }


        if (currentTaxRate == 0 || excludedAddresses[msg.sender] || _getTombPrice() > _getTombPrice().mul(101).div(100)) {
            _transfer(msg.sender, recipient, amount);
        } else {
            _transferWithTax(msg.sender, recipient, amount, burnTax);
        }

        _approve(msg.sender, _msgSender(), allowance(msg.sender, _msgSender()).sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
	}
    ...
    function transferFrom(
        ...
        if (currentTaxRate == 0 || excludedAddresses[sender] || _getTombPrice() > _getTombPrice().mul(101).div(100)) {
            _transfer(sender, recipient, amount);
        } else {
    ...
    function distributeReward(
        ...
        mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
