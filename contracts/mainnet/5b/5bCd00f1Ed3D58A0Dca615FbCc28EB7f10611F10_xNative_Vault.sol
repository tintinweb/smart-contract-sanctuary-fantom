...
interface INativeFarm {
    ...
    function withdraw(uint256 _pid, uint256 _amount, uint256 _vaultDuration) external;
...
contract xNative_Vault is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public constant performanceFeeUL = 500; // 5%

    uint256 public withdrawFee = 10; // 0.1%
    uint256 public constant withdrawFeeUL = 100; // 1%

    uint256 public earlyWithdrawFee = 700;
    uint256 public immutable earlyWithdrawDuration;
    uint256 public constant earlyWithdrawFeeUL = 1000;
     ...
     * @param _pid: pid of token strategy
    ...
    function deposit(address _userAddress, uint256 _amount) external whenNotPaused nonReentrant returns (uint256) {
        ...
        user.tokenAtLastUserAction = totalUserBalance(_userAddress);

        _farm();

        emit Deposit(_userAddress, _amount, newShares, block.timestamp);

        return user.tokenAtLastUserAction;
    }
    ...
    function withdraw(uint256 _wantAmt) public {
        ...
        if (bal < withdrawAmt) {
            ...
            nativeFarm.withdraw(pid, balWithdraw, earlyWithdrawDuration);
        ...
        uint256 earlyFee = earlyWithdrawalFee(withdrawAmt, user.lastDepositTime);
        withdrawAmt = withdrawAmt.sub(earlyFee);

        uint256 currentWithdrawFee = withdrawAmt.mul(withdrawFee).div(maxFee);
        withdrawAmt = withdrawAmt.sub(currentWithdrawFee);

        token.safeTransfer(treasury, currentWithdrawFee + earlyFee);
        token.safeTransfer(msg.sender, withdrawAmt);

        if (user.shares > 0) {
            user.tokenAtLastUserAction = totalUserBalance(msg.sender);
        } else {
    ...
    function harvest() external notContract whenNotPaused {
        nativeFarm.withdraw(pid, 0, earlyWithdrawDuration);

        uint256 bal = balanceOf();
        uint256 currentPerformanceFee = bal.mul(performanceFee).div(maxFee);
    ...
    function claimReward(address _userAddress) public {
        ...
        if (block.timestamp > user.lastUserActionTime && user.shares > 0) {
            uint256 rewardAmtCoef = block.timestamp.sub(user.lastUserActionTime).mul(user.shares).div(totalShares);
            uint256 rewardBalance = rewardToken.balanceOf(address(this));
            uint256 rewardAmt = rewardAmtCoef.mul(rewardPerSecond);
            if (rewardBalance > 0) {
    ...
    function setPerformanceFee(uint256 _performanceFee) external onlyAdmin {
        require(_performanceFee <= performanceFeeUL, "performanceFee cannot be more than MAX_PERFORMANCE_FEE");
    ...
    function setWithdrawFee(uint256 _withdrawFee) external onlyAdmin {
        require(_withdrawFee <= withdrawFeeUL, "withdrawFee cannot be more than MAX_WITHDRAW_FEE");
        withdrawFee = _withdrawFee;
    }

    function setEarlyWithdrawFeeFactor(uint256 _earlyWithdrawFee) external onlyAdmin {
        require(_earlyWithdrawFee <= earlyWithdrawFeeUL, "too high");
    ...
    function emergencyWithdraw() external onlyAdmin {
        nativeFarm.emergencyWithdraw(pid);
    }
    ...
    function pendingNATIVERewards() public view returns (uint256) {
        return nativeFarm.pendingNATIVE(pid, address(this));
    }
    ...
    function earlyWithdrawalFee(uint amount, uint depositAt) internal view returns (uint256) {
        if (earlyWithdrawFee > 0 && depositAt.add(earlyWithdrawDuration) > block.timestamp)
            return amount.mul(earlyWithdrawFee).div(maxFee);
        return 0;
    }
    ...
    function totalBalance() public view returns (uint256) {
        uint256 stakedAmount = nativeFarm.stakedWantTokens(pid, address(this));
