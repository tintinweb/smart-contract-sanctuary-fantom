    ...
    ClampedRandomizer
{
    ...
    function cloneDeterministic(bytes32) public override returns (address) {
        revert("disabled");
    }
     ...
     *
     * Note: paymentToken should be set to 0x0000000000000000000000000000000000000000
     *       to use the native L1 currency (ie. FTM, ETH, AVAX, etc.)
     */
    function create(
        string memory name,
        string memory symbol,
    ...
    /**
     * @dev allows the owner to mint NFTs to a specific recipient for free
     */
    function freeMint(address recipient, uint256 count) public onlyOwner overridePause returns (uint256[] memory) {
        return _mintCount(recipient, count);
    }
    ...
    /**
     * @dev enables the automatic distribution of mint fees to the recipients provided weighted
     *      by the shares specified
     */
    function setProceedsRecipient(address[] memory recipients, uint256[] memory shares) public onlyOwner {
        address old = address(PROCEEDS_RECEIVER);
        PROCEEDS_RECEIVER = IPaymentSplitter(BASE_PAYMENT_SPLITTER.clone());
        PROCEEDS_RECEIVER.initialize(recipients, shares);
        emit UpdateProceedsRecipient(old, address(PROCEEDS_RECEIVER));
    }
    ...
    /**
     * @dev sets whether token URIs are unique (true) or not (false)
     */
     ...
     */
    function snapshot(uint256 maxIterationCount) public onlyOwner whenPaused {
    ...
    function withdraw() public onlyOwner {
        if (address(PROCEEDS_RECEIVER) == address(0)) {
            if (PAYMENT_TOKEN == address(0)) {
                ...
                IERC20(PAYMENT_TOKEN).safeTransfer(owner(), IERC20(PAYMENT_TOKEN).balanceOf(address(this)));
            }
        } else {
            if (PAYMENT_TOKEN == address(0)) {
                PROCEEDS_RECEIVER.releaseAll();
