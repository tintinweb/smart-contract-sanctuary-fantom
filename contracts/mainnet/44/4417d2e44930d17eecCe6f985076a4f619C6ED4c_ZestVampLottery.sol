...
contract ZestVampLottery is Ownable, Pausable, AccessControl {
    ...
    mapping(address => totalAmounts) public lotteryStats;
    mapping(address => Pricing) public ticketPrices;
    ...
    bytes32 public DRAW_OPERATOR = keccak256("DRAW_OPERATOR");
    ...
    constructor(
        ...
        _pause();
        _grantRole(DRAW_OPERATOR, msg.sender);
        ticketPrices[_tokenA] = Pricing(_tokenAPrice);
        ticketPrices[_tokenB] = Pricing(_tokenBPrice);
        entryTokens.push(_tokenA);
        entryTokens.push(_tokenB);
    }
    ...
    function enter(uint256 tickets) external whenNotPaused {
        ...
        for (uint256 i = 0; i < entryTokens.length; i++) {
            IERC20(entryTokens[i]).safeTransferFrom(
                msg.sender,
                address(this),
                ticketPrices[entryTokens[i]].ticketPrice * tickets
            );
        }
    ...
    function start(uint256 _endTime) external onlyOps {
        require(_endTime > block.timestamp, "End time must be in the future");
        require(paused(), "Lottery is already active");
        endTimes[lotteryID] = _endTime;
        _unpause();
    }

    function closeLottery() external onlyOps {
    ...
    function setVrf(address _vrf) external onlyOps {
    ...
    function getRngAndDrawLottery() public onlyOps {
    ...
    function drawLottery(uint256 reqestId, uint256[] memory _randomWords) public {
        ...
        amounts[] memory amountData = new amounts[](entryTokens.length);

        if (players.length > 0) {
            //Calclate the total, won, and burn amount per token
            for (uint256 i = 0; i < entryTokens.length; i++) {
                amountData[i].totalAmount =
                    players.length *
                    ticketPrices[entryTokens[i]].ticketPrice;
                amountData[i].winnerAmount =
                    (amountData[i].totalAmount * winnerShare) /
                    shareDenominator /
                    numWords;
                amountData[i].burnAmount =
                    amountData[i].totalAmount -
                    amountData[i].winnerAmount *
                    numWords;
            }
            ...
            uint256 winnerIndex;
            uint256[] memory winner = new uint256[](numWords);

            //Determine the index of the winner
            for (uint256 index; index < numWords; index++) {
                //Find the winner
                winnerIndex = _randomWords[index] % players.length;
                winner[index] = winnerIndex;
                lotteryWinners[lotteryID].push(players[winnerIndex]);
            }

            //Loop through tokens
            for (uint256 idx = 0; idx < entryTokens.length; idx++) {
                //Loop through winners
                for (uint256 idz = 0; idz < winner.length; idz++) {
                    //Loop through tokens
                    IERC20(entryTokens[idx]).transfer(
                        players[winner[idz]],
                        amountData[idx].winnerAmount
                    );
                }
                IYToken(entryTokens[idx]).burn(amountData[idx].burnAmount);
                //Update the totals
                lotteryStats[entryTokens[idx]].totalWon += amountData[idx].winnerAmount;
                lotteryStats[entryTokens[idx]].totalBurned += amountData[idx].burnAmount;
            }
        }
    ...
    function setTicketPrice(address _token, uint256 _ticketPrice) public onlyOps whenPaused {
        require(ticketPrices[_token].ticketPrice != 0, "Must already have a set price");
        require(_ticketPrice > 0, "Ticket price must be greater than 0");
        ticketPrices[_token].ticketPrice = _ticketPrice;
    }

    function setNumWords(uint32 _words) public onlyOps whenPaused {
        require(
            _words >= numWords,
            "Number of words must be greater than or equal to current number of words"
        );
        numWords = _words;
    }

    modifier onlyOps() {
        require(hasRole(DRAW_OPERATOR, msg.sender), "Not allowed.");
        _;
    }
}
