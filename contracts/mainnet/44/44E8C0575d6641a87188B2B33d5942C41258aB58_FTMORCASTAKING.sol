...
contract FTMORCASTAKING is Ownable {
    constructor() {
        stakeholders.push();
    ...
    address public constant FTMORCA_TOKEN = 0xfb78b0a92381371483bB433f35EAfe751cF8FBe8;
    address public devWallet = 0x3ADE60A2c94F992b329a7641379dE5f0261784b8;
    ...
    function stake(uint256 _amount, uint256 _days) public {
        require(IERC20(FTMORCA_TOKEN).balanceOf(msg.sender) >= _amount,"Cannot stake more than you own");
        _stake(_amount, _days);
        IERC20(FTMORCA_TOKEN).transferFrom(msg.sender, address(this), _amount);
    }
    ...
    function withdrawStake(uint256 amount, uint256 stake_index) public {
        ...
        IERC20(FTMORCA_TOKEN).transfer(msg.sender, totalWithdrawalAmount);
    }


    function stakeAllFromTokenContract(address staker, uint256 _days) external {
        require(msg.sender == FTMORCA_TOKEN, "Only the tokencontract can use this");
        uint256 amount = IERC20(FTMORCA_TOKEN).balanceOf(staker);
        require(amount > 0);
        IERC20(FTMORCA_TOKEN).transferFrom(staker, address(this), amount);
    ...
    function stakeFromTokenContract(address staker, uint256 _amount, uint256 _days) external {
        require(msg.sender == FTMORCA_TOKEN, "Only the tokencontract can use this");
        IERC20(FTMORCA_TOKEN).transferFrom(staker, address(this), _amount);
    ...
    function unstakeAllFromTokenContract(address staker) external {
        ...
        IERC20(FTMORCA_TOKEN).transfer(staker, totalWithdrawalAmount);
    }
