...
contract OneBoardroom is ShareWrapper, ContractGuard {
    ...
    function setDarkCollateralDiscountPercent(
        uint256 _darkCollateralDiscountPercent
    ) external onlyOperator {
        require(_darkCollateralDiscountPercent <= 5000, "too high"); // <= 50%
        darkCollateralDiscountPercent = _darkCollateralDiscountPercent;
    }

    function setDarkoTokenSupplyTarget(uint256 _darkoTokenSupplyTarget)
        external
        onlyOperator
    {
        ...
        returns (uint256 _collateralPercent)
    {
        ...
        if (_darkoSupply >= _darkoTokenSupplyTarget) {
            ...
            _collateralPercent = _darkoSupply.mul(10000).div(
                _darkoTokenSupplyTarget
            );
    ...
    function getDarkOverWftmPrice() public view returns (uint256) {
        return
            (darkOracle == address(0))
                ? 1e18
                : uint256(IEpoch(darkOracle).getPegPriceUpdated());
    }
        ...
        )
    {
        ...
        _totalDarkCollateral = _totalCollateral.mul(getDarkOverWftmPrice()).div(
                1e18
            );
        _totalDarkCollateral = _totalDarkCollateral.sub(
            _totalDarkCollateral.mul(darkCollateralDiscountPercent).div(10000)
        ); // sub discount
    }
    ...
    function getTotalDarkCollateralForClaimAllPegTokens(address _member)
        public
        view
        returns (uint256 _totalDarkCollateral)
    {
        uint256 _totalCollateral = getTotalCollateralForClaimAllPegTokens(
            _member
        );
        _totalDarkCollateral = _totalCollateral.mul(1e18).div(
            getDarkOverWftmPrice()
        );
        _totalDarkCollateral = _totalDarkCollateral.sub(
            _totalDarkCollateral.mul(darkCollateralDiscountPercent).div(10000)
        ); // sub discount
...
//        require(canClaimReward(), "contraction");
    ...
    function _claimReward(bool _useDarkCollateral)
        internal
        updateReward(msg.sender)
    {
        uint256 _reward = members[address(darko)][msg.sender].rewardEarned;
        if (_reward > 0) {
            if (_useDarkCollateral) {
                dark.safeTransferFrom(
                    msg.sender,
                    insuranceFund,
                    getTotalDarkCollateralForClaimAllPegTokens(msg.sender)
                );
            } else {
