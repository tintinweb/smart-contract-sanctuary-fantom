...
import "../interfaces/IFantomStub.sol";
import "../interfaces/IStakingPool_R2.sol";
import "./aFTMb_R0.sol";

contract FantomPool_R2 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, ERC165Upgradeable, IBeaconUpgradeable, IBeaconDeployer, IStakingPool_R2 {
    ...
    address public bondContract; // aFTMb

    address private _implementation;
    mapping(uint256 => address payable) private _registry;
    uint256 public stubCount;
    mapping(address => bool) public canDeployMap;
    ...
    uint256 public firstWrId;
    uint256 public lastWrId;
    uint256 public stashedForManualWithdrawals;
    mapping(uint256 => bool) public markedForManualWithdrawal;

    // ratios are multiplied by 10**18
    uint256 public minFeeRatio;
    uint256 public maxFeeRatio;
    // amounts in wei
    uint256 public feeIncreaseStart;
    uint256 public feeIncreaseStop;
    uint256 public feeShift; // how many already withdrawn

    address public certContract; // aFTMc
    ...
    modifier onlyOperator() {
        require(msg.sender == owner() || msg.sender == operator, "Only Owner or Operator");
        _;
    }

    modifier canDeploy() {
        require(msg.sender == owner() || msg.sender == operator || canDeployMap[msg.sender], "canDeploy: not allowed");
        _;
    }
    ...
    function initialize(address _operator, address _sfc) public initializer {
        __Pausable_init();
    ...
    function stakeAndClaimBonds() override external nonReentrant payable {
        _stake(msg.sender, msg.value, true);
    }

    function stakeAndClaimCerts() override external nonReentrant payable {
        _stake(msg.sender, msg.value, false);
        IAnkrBond_R1(bondContract).unlockSharesFor(msg.sender, msg.value);
    }

    function _stake(address staker, uint256 amount, bool isRebasing) internal {
        require(amount >= minimumStake, "Value must be greater than min amount");
        IAnkrBond_R1(bondContract).mintBondsTo(staker, amount);
        emit StakeReceived2(staker, amount, isRebasing);
    }

    function getBurnFee(uint256 amount) override public view returns (uint256) {
        uint256 ratio = 0;
        uint256 point = amount + feeShift; // point to calculate burn fee at
        if (point >= feeIncreaseStop) {
            ratio = maxFeeRatio;
        } else if (point > feeIncreaseStart) {
            // note that in this branch feeIncreaseStop > feeIncreaseStart holds
            ratio = (minFeeRatio * (feeIncreaseStop - point) + maxFeeRatio * (point - feeIncreaseStart)) / (feeIncreaseStop - feeIncreaseStart);
        } else {
            ratio = minFeeRatio;
        }
        return Math.multiplyAndDivideCeil(amount, ratio, Math.unit());
    }

    function burn(uint256 amount) override external nonReentrant {
        uint256 shares = IAnkrBond_R1(bondContract).balanceToShares(amount);
        _burn(msg.sender, amount, shares, true);
    }

    function burnBonds(uint256 amount) override external nonReentrant {
        uint256 shares = IAnkrBond_R1(bondContract).balanceToShares(amount);
        _burn(msg.sender, amount, shares, true);
    }

    function burnCerts(uint256 shares) override external nonReentrant {
        uint256 amount = IAnkrBond_R1(bondContract).sharesToBalance(shares);
        IAnkrBond_R1(bondContract).lockSharesFor(msg.sender, shares);
        _burn(msg.sender, amount, shares, false);
    }

    function _burn(address staker, uint256 amount, uint256 shares, bool isRebasing) internal {
        IAnkrBond_R1(bondContract).burnSharesFrom(staker, shares);

        uint256 fee = getBurnFee(amount);
        if (fee > amount) {
            fee = amount;
        }

        withdrawals[lastWrId] = WithdrawalRequest(payable(msg.sender), amount - fee);
        feeShift += amount;

        emit TokensBurned2(msg.sender, amount, shares, fee, lastWrId, isRebasing);

        lastWrId++;
    }

    function updateFeeParameters(uint256 _minFeeRatio, uint256 _maxFeeRatio, uint256 _feeIncreaseStart, uint256 _feeIncreaseStop, uint256 _feeShift) override external onlyOperator nonReentrant {
        require(_minFeeRatio <= Math.unit() / 100 * 15, "Fee ratio must be not greater than 15%");
        require(_maxFeeRatio <= Math.unit() / 100 * 15, "Fee ratio must be not greater than 15%");
        require(_minFeeRatio <= _maxFeeRatio, "Ratio can not decrease");
        require(_feeIncreaseStart <= _feeIncreaseStop, "Start must be not greater than stop");
        minFeeRatio = _minFeeRatio;
        maxFeeRatio = _maxFeeRatio;
        feeIncreaseStart = _feeIncreaseStart;
        feeIncreaseStop = _feeIncreaseStop;
        feeShift = _feeShift;
        emit FeeRuleUpdated(minFeeRatio, maxFeeRatio, feeIncreaseStart, feeIncreaseStop, feeShift);
    }

    function withdrawLast() override external onlyOperator nonReentrant {
        require(firstWrId < lastWrId, "No active withdrawal requests");

        uint256 amount = withdrawals[firstWrId].amount;
        address payable staker = withdrawals[firstWrId].staker;

        require(address(this).balance >= amount + stashedForManualWithdrawals, "Insufficient pool balance");
        (bool sent, ) = staker.call{value: amount, gas: 40000}("");

        if (sent) {
            delete withdrawals[firstWrId];
            emit Withdrawn(staker, amount, firstWrId);
        } else {
            markedForManualWithdrawal[firstWrId] = true;
            stashedForManualWithdrawals += amount;
            emit ManualWithdrawalExpected(staker, amount, firstWrId);
        }
        firstWrId++;
    }

    function withdrawManual(uint256 wrId) override external nonReentrant {
        require(markedForManualWithdrawal[wrId], "Not marked for manual withdrawal");

        uint256 amount = withdrawals[wrId].amount;
        address payable staker = withdrawals[wrId].staker;

        markedForManualWithdrawal[wrId] = false;
        stashedForManualWithdrawals -= amount;

        require(address(this).balance >= amount + stashedForManualWithdrawals, "Insufficient pool balance");
        (bool sent, ) = staker.call{value: amount}("");

        require(sent, "Failed to send funds to staker");

        delete withdrawals[wrId];
        emit Withdrawn(staker, amount, wrId);
    }
    ...
    function changeCertContract(address newCertContract) external onlyOwner {
        address oldCerContract = certContract;
        certContract = newCertContract;
        emit CertContractChanged(oldCerContract, newCertContract);
    }
    ...
    function deployNewStub() external override canDeploy returns (address) {
        require(_implementation != address(0x00), "implementation is not set");
        // deploy new beacon proxy and do init call
        bytes memory data = abi.encodeWithSignature("initialize(address,address,address)", sfc, address(this), operator);
        BeaconProxy beaconProxy = new BeaconProxy(address(this), data);
        address deployedAddress = address(beaconProxy);
        _registry[stubCount] = payable(deployedAddress);
        emit Deployed(stubCount, deployedAddress);
    ...
    function getContractById(uint256 stubId) external view override returns (address) {
        return _registry[stubId];
    }

    function sendToStub(uint256 amount, uint256 stubId) public onlyOperator {
        require(stubId < stubCount, "stubId is too large");
        require(stashedForManualWithdrawals + amount <= address(this).balance, "Insufficient funds or attempting to send stashed funds for manual withdrawals");
        (bool sent, ) = _registry[stubId].call{value: amount}("");
        require(sent, "Failed to send funds to stub");
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        ...
        || interfaceId == type(IStakingPool_R2).interfaceId;
    }
...
contract aFTMb_R0 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20Transparent, ERC165Upgradeable, IAnkrBond {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return super.supportsInterface(interfaceId)
            ...
            || interfaceId == type(IAnkrBond).interfaceId;
    }
...
interface IStakingPool_R2 {
    event StakeReceived(address indexed staker, uint256 amount);

    event StakeReceived2(address indexed staker, uint256 amount, bool indexed isRebasing);

    event TokensBurned(address indexed staker, uint256 amount, uint256 shares, uint256 fee, uint256 indexed wrId);

    event TokensBurned2(address indexed staker, uint256 amount, uint256 shares, uint256 fee, uint256 indexed wrId, bool indexed isRebasing);

    event ManualWithdrawalExpected(address indexed staker, uint256 amount, uint256 indexed wrId);

    event Withdrawn(address indexed staker, uint256 amount, uint256 indexed wrId);

    event FeeRuleUpdated(uint256 minFeeRatio, uint256 maxFeeRatio, uint256 feeIncreaseStart, uint256 feeIncreaseStop, uint256 feeShift);

    event OperatorChanged(address oldOperator, address newOperator);

    event MinimumStakeChanged(uint256 newMinimalStake);
    ...
    event DeployAllowed(address deployer);

    event DeployDisallowed(address deployer);
    ...
    function burnCerts(uint256 shares) external;

    function sendToStub(uint256 amount, uint256 stubId) external;

    function getBurnFee(uint256 amount) external view returns (uint256);

    function withdrawLast() external;

    function withdrawManual(uint256 wrId) external;

    function updateFeeParameters(uint256 minFeeRatio, uint256 maxFeeRatio, uint256 feeIncreaseStart, uint256 feeIncreaseStop, uint256 feeShift) external;
...
interface IFantomStub {
    function delegate(uint256 toValidatorId, uint256 amount) external;

    function undelegate(uint256 toValidatorID, uint256 amount) external;

    function withdraw(uint256 toValidatorID) external;
    ...
    function setWithdrawalBounds(uint256 validatorId, uint256 _firstWrID, uint256 _lastWrId) external;
}
...
interface IBeaconDeployer {
    ...
    event Deployed(uint256 id, address indexed proxy);

    event Upgraded(address implementation);

    function deployNewStub() external returns (address);

    function upgradeTo(address newImplementation) external;

    function getContractById(uint256 id) external view returns (address);
}
