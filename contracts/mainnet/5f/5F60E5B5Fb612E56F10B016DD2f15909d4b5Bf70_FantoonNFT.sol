...
contract FantoonNFT is ERC721Enumerable, Ownable{
    ...
    uint256 private reservePrice = 0.2 ether;
    uint256 private reservedPeriod = 30 minutes; //24 hours;                 // 24 hours
    uint256 private mintReservedTokenPeriod = 30 minutes; //24 hours;        // 24 hours
    ...
    // Info of each reward.
    uint256[] public _distributeRewards;

    mapping (address=> mapping(uint256=>uint256)) public _nextWithdrawRewards;
    ...
    */
    ...
    function buyKittens() public payable lock {
        ...
        if (enableOGSharedistribute) {
            uint256 amount = previousPrice * 4500 / 10000;     // send 45% = 15% * 3 to 3 NFT holders
    ...
    function sendDividendsforNFTHolders(uint256 _amount) public {
     ...
     */
    function sendDividendsforDiamondHolders(uint256 _amount) public {
        Diamond(diamond).sendDividends(_amount, true);
    }

    function getRewardsOfTokenId(address _nftAddress, uint _tokenId) public view returns(uint256) {
    ...
    function getTotalRewardsOfNFT(address _nftAddress, address _account) public view returns(uint256) {
    ...
    function getPendingRewardsFromNFTs(address _account) public view returns(uint256) {
        uint totalRewards;
    ...
    function withdrawableFTM(address _account) public view returns (uint256){
        ...
        uint256 withdrawableAmountFromDiamond = Diamond(diamond).withdrawableDividendOf(_account, true);
        uint256 withdrawableAmountFromNFTs = getPendingRewardsFromNFTs(_account);
    ...
    function claimFTM() external lock {
        ...
        uint256 withdrawableAmountFromDiamond = Diamond(diamond).withdrawableDividendOf(msg.sender, true);
        uint256 withdrawableAmountFromNFTs = getPendingRewardsFromNFTs(msg.sender);
        uint256 totalRewards = withdrawableAmountFromDiamond + withdrawableAmountFromNFTs;
    ...
    function updateAllNFTs(address _account) private {
