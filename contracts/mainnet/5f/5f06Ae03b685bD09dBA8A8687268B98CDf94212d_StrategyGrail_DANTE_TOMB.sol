...
contract StrategyGrail_DANTE_TOMB is BaseStrategyMCMaximizer {

    address dante_tomb_diff;

    function initializeStrategy(
        address _store,
        address _vault
    ) public initializer {
        address masterchef = address(0xB2922E886AB9dff160479431F831A43C983A9Ef3);
        address dante_tomb_lp = address(0xac32e07c25cb18266841eD7035390744Cd3b1155);
        address grail = address(0x255861B569D44Df3E113b6cA090a1122046E6F89);
        ...
        address bgrail_ftm_lp = address(0x14a8c8EA5CB3145f0F460f77bFFC927D70FFCe96);
        __Strategy_init(
            _store,
            dante_tomb_lp,
            _vault,
        ...
        routes[grail] = [grail];
        routes[wftm] = [grail, wftm];
    }

    function reset() external {
        _setTargetVault(0x14a8c8EA5CB3145f0F460f77bFFC927D70FFCe96);
    }
}
...
contract BaseStrategyMCMaximizer is BaseUpgradeableStrategy {
    ...
    function withdrawToVault(uint256 _amount) public restricted {
        ...
        if(_amount > IERC20(underlying()).balanceOf(address(this))){
            // While we have the check above, we still using SafeMath below
            // for the peace of mind (in case something gets changed in between)
            uint256 needToWithdraw = (_amount - IERC20(underlying()).balanceOf(address(this)));
    ...
    function unsalvagableTokens(address _token) public view returns (bool) {
        return (_token == underlying() || _token == rewardToken());
    }
    ...
    function _investAllUnderlying() internal onlyNotPausedInvesting {
        ...
        IMasterChef _rewardPool = IMasterChef(rewardPool());

        uint256 underlyingBalance = _underlying.balanceOf(address(this));
        if(underlyingBalance > 0) {
            _underlying.safeApprove(address(_rewardPool), 0);
            _underlying.safeApprove(address(_rewardPool), underlyingBalance);
            _rewardPool.deposit(poolId(), underlyingBalance);
        }
    }
    ...
    function _notifyMaximizerRewards() internal {
        ...
        if(targetUnderlyingBalance > 0) {
            ...
            IERC20(vTarget).safeTransfer(vaultToInject, targetVaultBalance);
            IVault(vaultToInject).notifyRewardAmount(vTarget, targetVaultBalance);
        }
    }
