 ...
 */
contract ReaperStrategyGranary is ReaperBaseStrategyv4, ILeverageable, VeloSolidMixin, UniV2Mixin, BalMixin {
    ...
    function _harvestCore(uint256 _debt) internal virtual override returns (int256 roi, uint256 repayment) {
        ...
        for (uint256 i = 0; i < numSteps; i = i.uncheckedInc()) {
            ...
            if (step.dex == Exchange.Velodrome) {
                ...
                _swapUniV2(step.start, step.end, amount, 0, exchangeSettings.uniV2Router);
            }
        }
    ...
    function updateUniV2SwapPath(address _tokenIn, address _tokenOut, address[] calldata _path) external override {
        _atLeastRole(STRATEGIST);
        _updateUniV2SwapPath(_tokenIn, _tokenOut, _path);
    }
