...
contract Maximillion {
    /**
     * @notice The default tFtm market to repay in
     */
    TWrappedNative public tFtm;

    /**
     * @notice Construct a Maximillion to repay max in a TWrappedNative market
     */
    constructor(TWrappedNative tFtm_) public {
        tFtm = tFtm_;
    }

    /**
     * @notice msg.sender sends Ether to repay an account's borrow in the tFtm market
     * @dev The provided Ether is applied towards the borrow balance, any excess is refunded
     * @param borrower The address of the borrower account to repay on behalf of
     */
    function repayBehalf(address borrower) public payable {
        repayBehalfExplicit(borrower, tFtm);
    }

    /**
     * @notice msg.sender sends Ether to repay an account's borrow in a tFtm market
     * @dev The provided Ether is applied towards the borrow balance, any excess is refunded
     * @param borrower The address of the borrower account to repay on behalf of
     * @param tFtm_ The address of the tFtm contract to repay in
     */
    function repayBehalfExplicit(address borrower, TWrappedNative tFtm_) public payable {
        uint256 received = msg.value;
        uint256 borrows = tFtm_.borrowBalanceCurrent(borrower);
        if (received > borrows) {
            tFtm_.repayBorrowBehalfNative.value(borrows)(borrower);
            msg.sender.transfer(received - borrows);
        } else {
            tFtm_.repayBorrowBehalfNative.value(received)(borrower);
        }
    }
}
...
/**
 * @title Cream's TWrappedNative Contract
 * @notice TTokens which wrap the native token
 * @author Cream
 */
contract TWrappedNative is TToken, TWrappedNativeInterface, TProtocolSeizeShareStorage {
    /**
     ...
     *  Keep return in the function signature for backward tombatibility
    ...
    function liquidateBorrowNative(address borrower, TTokenInterface tTokenCollateral)
        ...
        returns (uint256)
    {
        (uint256 err, ) = liquidateBorrowInternal(borrower, msg.value, tTokenCollateral, true);
