...
interface ITreasury {
    ...
    function buyCBonds(uint256 amount, uint256 targetPrice) external;

    function redeemCBonds(uint256 amount, uint256 targetPrice) external;
}
...
contract CShareWrapper {
    ...
    function stake(uint256 amount) public virtual {
        ...
        CShare.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public virtual {
        uint256 memberCShare = _balances[msg.sender];
        require(memberCShare >= amount, "Boardroom: withdraw request greater than staked amount");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = memberCShare.sub(amount);
        CShare.safeTransfer(msg.sender, amount);
    }
}


contract Boardroom is CShareWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        BoardroomSnapshot memory genesisSnapshot = BoardroomSnapshot({time: block.number, rewardReceived: 0, rewardPerCShare: 0});
    ...
    function getCronjetombPrice() external view returns (uint256) {
        return treasury.getCronjetombPrice();
    }

    // =========== Member getters

    function rewardPerCShare() public view returns (uint256) {
        return getLatestSnapshot().rewardPerCShare;
    }

    function earned(address member) public view returns (uint256) {
        uint256 latestRPS = getLatestSnapshot().rewardPerCShare;
        uint256 storedRPS = getLastSnapshotOf(member).rewardPerCShare;
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            Cronjetomb.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        uint256 prevRPS = getLatestSnapshot().rewardPerCShare;
        uint256 nextRPS = prevRPS.add(amount.mul(1e18).div(totalSupply()));

        BoardroomSnapshot memory newSnapshot = BoardroomSnapshot({time: block.number, rewardReceived: amount, rewardPerCShare: nextRPS});
        boardroomHistory.push(newSnapshot);

        Cronjetomb.safeTransferFrom(msg.sender, address(this), amount);
