...
contract FusionLiquidStaking is ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    address feeReceiver;
    uint256 serviceFee = 100;   // 1% service fee 100 / 10000 * 100
    ...
    function withdraw(uint256 tokenId) public nonReentrant nonReentrant {
        address owner;
        TokenData memory _tokenData;
    ...
    function withdrawAll(uint8 timeLockId) public nonReentrant {
        ...
        if (tokenCount != 0) {
            ...
            for (index = 0; index < balanceOf(msg.sender); ) {
                uint256 _tokenId = tokenOfOwnerByIndex(msg.sender, index);
                if (tokenData[_tokenId].timelockId == timeLockId) {
                    if (block.timestamp - tokenData[_tokenId].depositAt > timeLocks[timeLockId].period &&
                        tokenData[_tokenId].depositAmount > 0
                        )
                    {
                        uint256 claimableReward = _getRewardAmount(tokenData[_tokenId].depositAmount, timeLockId);
                        totalAmount += tokenData[_tokenId].depositAmount;
                        totalAmount += claimableReward;
                        stakeFields[tokenData[_tokenId].timelockId].amount = stakeFields[tokenData[_tokenId].timelockId].amount.sub(tokenData[_tokenId].depositAmount);
                        stakeFields[tokenData[_tokenId].timelockId].stakeIds.remove(_tokenId);
                        burn(_tokenId);
                        continue;
                    }
                } 
            ...
            if (totalAmount > balance) totalAmount = balance;
            ...
            SNTToken.safeTransfer(msg.sender, totalAmount);
        }
    }
    ...
    function getStakingItemsOfTimeLock(uint8 timeLockId, address _owner) external view returns (TokenData[] memory) {
        ...
        if (tokenCount == 0) {
            ...
            TokenData[] memory result = new TokenData[](ownedTokenCount);
            uint256 j;
            for (uint index = 0; index < tokenCount; index++) {
                uint256 tokenId = stakeFields[timeLockId].stakeIds.at(index);
                if (ownerOf(tokenId) == _owner) {
                    result[j] = tokenData[tokenId];
                    j++;
                }
            }
            return result;
