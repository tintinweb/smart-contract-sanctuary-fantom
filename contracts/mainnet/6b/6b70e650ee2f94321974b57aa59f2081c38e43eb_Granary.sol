...
contract Granary
{
	...
	constructor ()
	{
		...
		//B-x to Sell or Join USDC we buy
		...
		treasury = dao;
        Eliteness.push(Elites({ELITE:0xf43Cc235E686d7BC513F53Fbffb61F760c3a1882,ELITES:10000000000000000}));
	}
	...
	function id() public pure returns(string memory){return("4804");}
	function name() public pure returns(string memory){return("ftm.guru/GRAIN/4804");}//string(abi.encodePacked("ftm.guru/GRAIN/", id)));}
	function symbol() public pure returns(string memory){return("GRAIN#4804");}//string(abi.encodePacked("GRAIN#", id)));}
	...
	*/
	...
	bytes32 public BPT_BECH = 0x4ba42b43b1316167abf0c39ee6ccb486420fede00002000000000000000004e4;
	...
	address[9] public alladdr = [
		...
		0xb9799De71100e20aC1cdbCc63C69ddA2D0D81710,//2	dex			.router
		0xF24Bcf4d1e507740041C9cFd2DddB29585aDCe1e,//3	BEETS
		0xb868B46BcEC6595A953e351e6d8459bedEF6805E,//4	bECH
		0x04068DA6C83AFCFA0e13ba15A6696662335D5B75,//5	USDC
		0x4ba42b43b1316167ABF0c39EE6ccb486420FEde0,//6	BPT-4echusd	.bech-usdc
		0x03c6B3f09D2504606936b1A4DeCeFaD204687890,//7	BPT-opus	.beets-usdc
	...
	function work(address ben) internal
	{
        ...
        if(beetsEarned > allnums[4])
        {
			...
			//Buy bECH with 80% of USDC at Beethoven-x
			FundManagement memory FMU = FundManagement({
				sender:             address(this),
				fromInternalBalance:false,
				recipient:          payable(address(this)),
				toInternalBalance:  false
			});
			SingleSwap memory U2E = SingleSwap({
				poolid: 	BPT_BECH,
				kind:	    SwapKind.GIVEN_IN,
				assetIn:    alladdr[5],
				assetOut:   alladdr[4],
				amount:     (IERC20(alladdr[5]).balanceOf(address(this))*4)/5,
				userData:   abi.encode(0)
			});
			IBX(alladdr[1]).swap(U2E, FMU, 1, block.timestamp);

		}


		//JoinPool: BPT-4ECHUSD
		...
		IBX(alladdr[1]).joinPool(BPT_BECH, address(this), address(this), R);
	...
	function tvl() public view returns(uint256)
	{
		...
		(,uint256[] memory a,) = IBX(alladdr[1]).getPoolTokens(BPT_BECH);
