/**
 *Submitted for verification at FtmScan.com on 2023-07-18
*/
...
contract FizoDapp is IERC20
{
    ...
    uint256  TotalHoldings;
    uint256  public _investorscount=500;
    address [] vipinvestors;
    ...
    address vipwallet=0xc4abe51fa59BE02e0B782ff19d58BD03DD5803cB;
    address marketingwallet=0xd266c213B3D77dF4594CbF446EdbcC2aff961d2a;
    uint8 lock;
   ...
   function coinRate() public view returns(uint)
    ...
    function _distributePOI(address depositor, uint256 _poi) internal{
       uint256 poiShare;
       uint256 totalinvestor=investors.length;
       uint256 investlimit=totalinvestor;
       if(totalinvestor>_investorscount)
       {
           investlimit=_investorscount;
       }
       uint256 i = investlimit;
       while(i>0){
            User storage user = users[investors[totalinvestor-1]];
    ...
    function redeposit() public payable{
		...
		if(user.totalInvestment>=13000*1e18 && user.isvip==0)
        {
            user.isvip=1;
            vipinvestors.push(msg.sender);
        }
    ...
    function fizoWithdraw(uint8 _perc) public{
        ...
        payouts[msg.sender].push(Withdraw(
         ...
         if(deduct==10)
         {
             aggregator.transfer(withdrawable);
         }
         else if(deduct==20)
         {                     
            _distributePOI(msg.sender,withdrawable.mul(14).div(100));
            aggregator.transfer(withdrawable.mul(3).div(100));
            payable(vipwallet).transfer(withdrawable.mul(3).div(100));
         } 
         else if(deduct==40)
         {
            _distributePOI(msg.sender,withdrawable.mul(14).div(100));
            aggregator.transfer(withdrawable.mul(20).div(100));
            payable(vipwallet).transfer(withdrawable.mul(6).div(100));
         }         
   ...
   function deposit(address _referer) public payable
   {
    ...
    function getVIPLength() external view returns(uint256) {
        return vipinvestors.length;
    }
    function getVIPAddress(uint index) external view returns(address) {
        return vipinvestors[index];
    }
    ...
    function setPOILimit(uint256 _pilimit) external onlyInitiator{       
        _investorscount=_pilimit;
    }
