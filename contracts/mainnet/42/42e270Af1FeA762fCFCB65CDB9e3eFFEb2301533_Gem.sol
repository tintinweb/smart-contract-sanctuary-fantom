...
contract Gem is IGem, ERC20, Ownable {
  ...
  function mint(address to, uint256 amount) external override {
    require(admins[_msgSender()], "Only admins can mint");
    _mint(to, amount);
  }
  ...
  function burn(address from, uint256 amount) external override {
    require(admins[_msgSender()], "Only admins can burn");
    _burn(from, amount);
  }
  ...
  function transferFrom(
      ...
      address recipient,
      uint256 amount
  ) public virtual override(ERC20, IGem) disallowIfStateIsChanging returns (bool) {
    // NICE TRY MOUSE OWL
    ...
    // If it's not an admin entity (game contract, hideout, etc)
    // The entity will need to be given permission to transfer these funds
    // For instance, someone can't just make a contract and siphon $GEM from every account
