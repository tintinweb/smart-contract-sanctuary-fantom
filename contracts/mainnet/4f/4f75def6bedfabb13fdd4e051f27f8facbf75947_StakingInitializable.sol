...
contract StakingInitializable is Ownable, ReentrancyGuard {
    ...
    // Amount user must have staked in order to be eligible for raffle.
    uint256 public requiredHoldings; 

    //Upon draw, was the winner valid (by having the correct or more amount of tokens held in staking)?
    bool public validWinner = true;

    // Raffle end time in UNIX timestamp
    uint raffleEndTime;

    // Raffle Prize
    uint public stakingPrize;
    ...
    //Staking raffle participants
    address[] participants;

    //Total Burned
    uint256 public totalBurned;

    function setPayoutToken(IBEP20 newToken) public onlyOwner {payoutToken = newToken;}

    bool public raffleOver = false;
    bool public isWinnerPicked = false;

    function setNewStakingPrize(uint256 newStakingPrize) public onlyOwner {stakingPrize = newStakingPrize;}
    function setRequiredHoldings(uint256 newRequiredHoldings) public onlyOwner {requiredHoldings = newRequiredHoldings;}
    ...
    function resetBurnedCount() public onlyOwner {
        totalBurned = 0;
    }
    ...
    struct UserInfo {
        uint256 amount; // How many staked tokens the user has in pool
        uint256 rewardDebt; // Reward debt
        uint256 nextWithdrawalUntil; // When can the user withdraw.
    }    
      ...
      function drawWinner() public onlyOwner {
        ...
        UserInfo storage raffleUser = userInfo[winner];

        if(raffleUser.amount < requiredHoldings){
    ...
    function payout() public onlyOwner {
        ...
        isWinnerPicked = false;
        raffleOver = false;
        payoutToken.transfer(winner, stakingPrize);
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if(hasLimit) {
            require(_amount.add(totalStaked) <= poolLimit, "Staked amount above limit");
        }
        ...
        if (_amount > 0) {
            ...
            if (stakedTokenTransferFee > 0) {
             ...
             if (depositFee > 0) {
                ...
                totalBurned += _amount.mul(depositFee).div(10000);
            } 
     ...
     * @dev Only callable by owner.
     * @param _hasLimit: whether the limit remains forced
     * @param _poolLimit: new pool limit 
     */
    function updatePoolLimit(bool _hasLimit, uint256 _poolLimit) external onlyOwner {
        require(hasLimit, "Must be set");
        if (_hasLimit) {
...
contract StakingFactory is Ownable {
    ...
    function deployPool(
        ...
        assembly {
            stakingContractAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        StakingInitializable(stakingContractAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewStakingContract(stakingContractAddress);
    }
}