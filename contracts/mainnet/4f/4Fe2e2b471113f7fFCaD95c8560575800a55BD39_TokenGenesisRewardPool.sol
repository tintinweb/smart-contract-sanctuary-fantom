...
contract TokenGenesisRewardPool {
    ...
    // MAINNET
     uint256 public tokenPerSecond = 0.04032986111 ether; // 6969 TOKEN / (2 Days * 24h * 60min * 60s)
     uint256 public runningTime = 2 days; // 2 days
     uint256 public constant TOTAL_REWARDS = 6969 ether;
    // END MAINNET
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            pool.token.safeTransfer(daoAddress1, _fee.div(4));
            pool.token.safeTransfer(daoAddress2, _fee.div(4));
            pool.token.safeTransfer(daoAddress3, _fee.div(4));
            pool.token.safeTransfer(daoAddress4, _fee.div(4));
