...
contract MillionaireCircle is Ownable, ReentrancyGuard {
    ...
    function activateAll(uint256 refId, address walletAddress) external {
    ...
    function addGlobalAccount(
        ...
        require(usdc.transferFrom(msg.sender, address(this), GLOBAL_FEE));
        ...
        platformToken.transfer(walletAddress, PLATFORM_TOKEN_EARNING);
    ...
    function activatePrivateAccount(uint256 id) public {
        ...
        require(usdc.transferFrom(msg.sender, address(this), PRIVATE_FEE));
        user.privateLevel = 1;
        ...
        if (currentPrivateSlot < 10) {
            ...
            if (currentPrivateSlot == 10) {
                moveToNextGlobalLevel(user.refId);
            }
            return;
        }
    ...
    function endNetworkingPhase(address listingAddress) external onlyOwner {
        require(
            lastId >= 100000 || startDate.add(180 days) < block.timestamp,
    ...
    uint256 public constant PLATFORM_TOKEN_EARNING = 100 * 10 ** 18;
    ...
    function moveToNextGlobalLevel(uint256 id) private {
        Member storage user = accounts[id];
        Member storage upline = accounts[user.refId];
