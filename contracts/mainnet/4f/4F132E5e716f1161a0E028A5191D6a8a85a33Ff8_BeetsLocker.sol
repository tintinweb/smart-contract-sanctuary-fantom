...
import {IMasterChefV2} from "./interfaces/protocols/IMasterChefV2.sol";
...
contract BeetsLocker is ERC20Upgradeable, IUpgradeSource, ControllableInit, EternalStorage {
    ...
    function deposit(uint256 _amount) external defense {
        ...
        IERC20 _fBeets = fBeets(); // Save SLOADs with memory.
        uint256 fBeetsPre = _fBeets.balanceOf(address(this));
        _mint(msg.sender, _amount);
        _fBeets.safeTransferFrom(msg.sender, address(this), _amount);
        _lock((_fBeets.balanceOf(address(this)) - fBeetsPre));
    ...
    function doHardWork() external {
        ...
        if(balanceToLiquidate > 0) {
            ...
            // Swap for fBEETS
            uint256 freshPre = _fBeets.balanceOf(address(this));
            _swapToFreshBeets(BEETS_TOKEN, balanceToLiquidate);
            uint256 freshPost = _fBeets.balanceOf(address(this)) - freshPre;

            // Deposit fBEETS into bfBEETS and inject.
            _fBeets.safeApprove(address(BFBEETS), 0);
            _fBeets.safeApprove(address(BFBEETS), freshPost);
            
            uint256 bfBeetsBefore = IERC20(address(BFBEETS)).balanceOf(address(this));
            BFBEETS.deposit(freshPost);
            uint256 bfBeetsAfter = IERC20(address(BFBEETS)).balanceOf(address(this)) - bfBeetsBefore;

            _notifyRewardAmount(IERC20(address(BFBEETS)), bfBeetsAfter);
        }
    }
