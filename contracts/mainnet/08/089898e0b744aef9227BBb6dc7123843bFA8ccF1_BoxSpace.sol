...
// ("USDT", 0xc2132D05D31c914a87C6611C10748AEb04B58e8F, 0x0A6513e40db6EB1b165753AD52E80663aeA50545)
   ...
   modifier onlyOwner {
    ...
    function buy(uint24 boxId) external payable  returns(uint256 boxTokenMinted){
        ...
        for(uint i = 0 ; i < tokensInBox ; i++){
            ...
            if(keccak256(abi.encodePacked(token.name)) == keccak256(abi.encodePacked('ftm'))){
                ...
                uint tokenAmount = swapTokens(swapAmount, tokenAddress["Wftm"], tokenAddress[token.name]);
                boxBalance[boxId][tokenAddress[token.name]] += tokenAmount;
            }
        }
    ...
    function sell(uint24 boxId, uint256 tokenSellAmount) external  returns(uint) {
        ...
        for(uint i = 0 ; i < tokensInBox ; i++){
            ...
            if(keccak256(abi.encodePacked(token.name)) == keccak256(abi.encodePacked('ftm'))){
                ...
                boxBalance[boxId][tokenAddress[token.name]] -= sellAmount;
                uint wftmAmount = swapTokens(sellAmount, tokenAddress[token.name], tokenAddress["Wftm"]);
    ...
    function getNumberOfTokensInBox(uint24 boxId) public view  returns(uint){
        return(boxDistribution[boxId].length);
    }

    function getBoxDistribution(uint24 boxId, uint tokenNumber) public view  returns(Token memory){
        return (boxDistribution[boxId][tokenNumber]);
    }

    function getBoxTVL(uint24 boxId) public view  returns(uint) {
    ...
    function getBoxTokenPrice(uint24 boxId) public view  returns(uint)  {  
    ...
    function _getBoxTokenMintAmount(uint24 boxId, uint amountInftm) internal view  returns(uint) {
        int256 ftmPrice = getLatestPrice(ftmPriceFeed);
    ...
    function swapTokens(uint256 amountIn, address tokenIn, address tokenOut) internal returns (uint256 amountOut) {
