...
import "./interfaces/IZestChef.sol";
import "./libs/Babylonian.sol";

/*
    ZestZap is a ZapperFi's simplified version of zapper contract which will:
    1. use ETH to swap to target token
    2. make LP between ETH and target token
    3. add into farm
*/
contract ZestZapV2 is Ownable, ReentrancyGuard {
    ...
    function zap(
        ...
        address _lp = address(zestChef.lpToken(_info.pid));
        ...
        approveToken(_lp, address(zestChef), _liquidity);
        zestChef.deposit(_info.pid, _liquidity, msg.sender);
    ...
    /// @notice Zap ETH into 2 tokens, for a non-ETH pair farm.
    /// add liquidity, and deposit LP to farm
    /// @param _zapId: Id of the zap configuration
    /// @param _minLiquidity: Minimum amount of LP received
    /// @param _transferResidual: Flag to determine to transfer dust back to user
    ...
    function zapToken(
        ...
        // STEP 1: Swap into token0
        (uint256 _ethAmtToSwap, uint256 _token0AmtToAddLP) = swapToken(_lp, _ethIn, _token0);

        // STEP 2: Swap into token1
        (uint256 _remaining, uint256 _token1AmtToAddLP) = swapToken(_lp, _ethAmtToSwap, _token1);

        // STEP 3: Approve & add Liquidity

        approveToken(_token0, address(uniRouter), _token0AmtToAddLP);
        approveToken(_token1, address(uniRouter), _token1AmtToAddLP);

        (uint256 _tokenA, uint256 _tokenB, uint256 _liquidity) = uniRouter.addLiquidity(
            _token0,
            _token1,
            _token0AmtToAddLP,
            _token1AmtToAddLP,
        ...
        // STEP 4: Clean up dust
        if (_transferResidual) {
            if (_token0AmtToAddLP > _tokenA)
                IERC20(_token0).safeTransfer(msg.sender, _token0AmtToAddLP - _tokenA);
            if (_token1AmtToAddLP > _tokenB)
                IERC20(_token1).safeTransfer(msg.sender, _token1AmtToAddLP - _tokenB);
            if (_remaining > 0) Address.sendValue(payable(msg.sender), _remaining);
        }

        emit Zapped(_zapId, _ethIn, _liquidity);
    }

    function zapFromOneSide(
        uint256 _zapId,
        uint256 _minLiquidity,
        address _token,
        ...
        approveToken(_token, address(uniRouter), _tokenInAmount);

        uint256 _ethIn = _tokenInAmount;

        address _lp = address(zestChef.lpToken(_info.pid));

        address _token0 = _isToken0 ? IUniswapV2Pair(_lp).token1() : IUniswapV2Pair(_lp).token0();
        address _token1 = _isToken0 ? IUniswapV2Pair(_lp).token0() : IUniswapV2Pair(_lp).token1();

        require(_lp != address(0), "Zap::zap: Invalid LP");

        // STEP 1: Swap into token0
        (uint256 _tokenRemaining, uint256 _targetReceived) = swapTokenForToken(
            _ethIn,
            _token0,
            _token1,
            _lp
        ...
        approveToken(_token0, address(uniRouter), _tokenRemaining);
        approveToken(_token1, address(uniRouter), _targetReceived);
        ...
        uint256 token0BalanceAfter = IERC20(_token0).balanceOf(address(this));
        uint256 token1BalanceAfter = IERC20(_token1).balanceOf(address(this));

        // STEP 4: Clean up dust
        if (_transferResidual) {
            if (token0BalanceAfter > 0) IERC20(_token0).transfer(msg.sender, token0BalanceAfter);
            if (token1BalanceAfter > 0) IERC20(_token1).transfer(msg.sender, token1BalanceAfter);
        }
    }
    ...
    function swapTokenForToken(
        ...
        address _tokenOut,
        address _lp
    ) internal returns (uint256 _tokenInAmt, uint256 _tokenAmtReceived) {
        address _token0 = IUniswapV2Pair(_lp).token0();
        (uint256 _res0, uint256 _res1, ) = IUniswapV2Pair(_lp).getReserves();
        if (_tokenIn == _token0) {
            _tokenInAmt = calculateSwapInAmount(_res1, _tokenInAmount);
        } else {
            _tokenInAmt = calculateSwapInAmount(_res0, _tokenInAmount);
        }
        if (_tokenInAmt <= 0) _tokenInAmt = _tokenInAmount / 2;
        _tokenAmtReceived = doSwapToken(_tokenOut, _tokenIn, _tokenInAmt);
    }
    ...
    function doSwapToken(
        address _tokenOut,
        address _tokenIn,
        uint256 _tokenAmount
    ) internal returns (uint256 _tokenReceived) {
        IERC20(_tokenIn).safeIncreaseAllowance(address(uniRouter), _tokenAmount);
        address[] memory _path = new address[](2);
        _path[0] = _tokenIn;
        _path[1] = _tokenOut;

        _tokenReceived = uniRouter.swapExactTokensForTokens(
            _tokenAmount,
            1,
            _path,
            address(this),
    ...
    function recoverDust(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(0), "Zap::recoverDust: Invalid address");
        require(_amount > 0, "Zap::recoverDust: Invalid amount");
