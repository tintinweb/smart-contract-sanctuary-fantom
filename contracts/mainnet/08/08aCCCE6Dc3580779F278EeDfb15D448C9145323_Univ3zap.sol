...
contract Univ3zap is Ownable {
  ...
  event Removal(address token, address src, address rec, uint256 amount);
  ...
  function withdraw(address poolAddr, uint256 liquidity, bool isToken0, bool isCoin) public {
    ...
    if (liquidity > 0) {
      ...
      if (isToken0) {
        ...
        if (isCoin) {
          emit Removal(token1, address(this), msg.sender, amount);
        }
        else {
  ...
  function trasf(address token, address src, address des, uint256 amount, bool isCoin) public {
    _transferTokens(token, src, des, amount, isCoin);
  }
    ...
    if (isCoin) {
      if (src == address(this)) {
        if (des != address(this)) {
          ...
          (bool success, ) = des.call{value: amount}('');
