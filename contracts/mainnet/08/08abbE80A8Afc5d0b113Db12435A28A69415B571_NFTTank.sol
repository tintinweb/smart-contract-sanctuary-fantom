...
contract NFTTank is ERC721Enumerable, Ownable {
    ...
    function config(address _tankToken, address _treasuryAddress)
        external
        onlyOwner
    {
    ...
    function addNewClasses(string[] memory descriptions, uint[] memory prices)
        external
        onlyOwner
    {
    ...
    //Air drop
    function airdrop(address[] memory players, uint[] memory classTypes)
        external
        onlyOwner
    {
        uint supply = totalSupply();
    ...
    function getMessageHash(uint tokenId, uint level)
        public
        pure
        returns (bytes32)
    {
