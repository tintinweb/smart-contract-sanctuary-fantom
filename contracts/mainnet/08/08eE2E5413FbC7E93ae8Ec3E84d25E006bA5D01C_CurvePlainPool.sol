...
import "../BaseFarm.sol";
import "../../../../interfaces/curve/ICurveLP.sol";
import "../../../../interfaces/curve/IDeposit.sol";
import "../../../../interfaces/curve/IPool.sol";

contract CurveBase {
    uint256 public immutable poolSize;
    constructor(uint256 poolSizeInput) {
        poolSize = poolSizeInput;
    }

    function _addLiq(address p, uint256 amount, uint256 index) internal {
        if (poolSize < 2 || poolSize > 5) revert("CB1");

        if (poolSize == 2) {
            uint256[2] memory a;
            a[index] = amount;
            return IBiPool(p).add_liquidity(a, 0);
        }
        if (poolSize == 3) {
            uint256[3] memory a;
            a[index] = amount;
            return ITriPool(p).add_liquidity(a, 0);
        }
        if (poolSize == 4) {
            uint256[4] memory a;
            a[index] = amount;
            return IQuadPool(p).add_liquidity(a, 0);
        }
        uint256[5] memory b;
        b[index] = amount;
        return IQuintPool(p).add_liquidity(b, 0);
    }

    function _removeLiq(IPool p, uint256 amount, uint256 index) internal {
        IPoolBase(p).remove_liquidity_one_coin(amount, int128(int256(index)), 0);
    }

    function _calculateLpAmount(IPool p, uint256 amount, uint256 index, bool isDeposit) internal view returns (uint256) {
        if (poolSize < 2 || poolSize > 5) revert("CB1");

        if (poolSize == 2) {
            uint256[2] memory a;
            a[index] = amount;
            return IBiPool(p).calc_token_amount(a, isDeposit);
        }
        if (poolSize == 3) {
            uint256[3] memory a;
            a[index] = amount;
            return ITriPool(p).calc_token_amount(a, isDeposit);
        }
        if (poolSize == 4) {
            uint256[4] memory a;
            a[index] = amount;
            return IQuadPool(p).calc_token_amount(a, isDeposit);
        }

        uint256[5] memory b;
        b[index] = amount;
        return IQuintPool(p).calc_token_amount(b, isDeposit);
...
contract CurvePlainPool is BaseFarm, CurveBase {
    using SafeERC20 for IERC20Metadata;
    ...
    constructor(
        IConfig c,
        IERC20Metadata[] memory underlying,
        address positionTokenInput,
        uint256 poolSizeInput,
        IPool poolInput
    ) BaseFarm(c, underlying, positionTokenInput) CurveBase(poolSizeInput) {
        pool = poolInput;
    }
    ...
    function _deployImpl(IERC20Metadata u, uint256 amount) internal override {
        ...
        for (uint256 i; i < poolSize; i++) {
            if (p.coins(i) == address(u)) {
                u.safeIncreaseAllowance(address(p), amount);
                _addLiq(address(p), amount, i);
                return;
            }
        }
        revert("CRV1");
    ...
    function _withdrawAllImpl(IERC20Metadata u)
        ...
        returns (uint256)
    {
        ICurveLP lp = ICurveLP(positionToken);
        uint256 amount = lp.balanceOf(address(this));
        // use pool to remove liquidity
        IPool p = pool;
        for (uint256 i; i < poolSize; i++) {
            if (p.coins(i) == address(u)) {
                IERC20Metadata(lp).safeIncreaseAllowance(address(p), amount);
                uint256 balanceBefore = u.balanceOf(address(this));
                _removeLiq(p, amount, i);
                uint256 balanceAfter = u.balanceOf(address(this));
                require(balanceAfter >= balanceBefore, "CRV2");
    ...
    function _balanceOfUnderlying(address user, IERC20Metadata u)
        ...
        returns (uint256)
    {
        uint256 amount = ICurveLP(positionToken).balanceOf(user);
        if (amount == 0) return 0;
        IPool p = pool;
        for (uint256 i; i < poolSize; i++) {
            if (p.coins(i) == address(u)) {
                return p.calc_withdraw_one_coin(amount, int128(int256(i)));
            }
        }
        revert("CRV1");
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i; i < poolSize; i++) {
            if (p.coins(i) == address(u)) {
                uint256 amountToBurn = _calculateLpAmount(p, amount, i, false);
                IERC20Metadata(lp).safeIncreaseAllowance(address(p), amountToBurn);
                uint256 balanceBefore = u.balanceOf(address(this));
                _removeLiq(p, amountToBurn, i);
                uint256 balanceAfter = u.balanceOf(address(this));
                require(balanceAfter >= balanceBefore, "CRV2");
                return balanceAfter - balanceBefore;
            }
...
interface IDeposit is IBiPool, ITriPool, IQuadPool, IQuintPool {
    function coins(int128 i) external view returns (address);
    function remove_liquidity_one_coin(uint256 _amount, int128 i, uint256 min_uamounts) external;
...
interface IBiPool is IPoolBase {
    function add_liquidity(uint256[2] calldata uamounts, uint256 min_mint_amount) external;
    function calc_token_amount(uint256[2] calldata uamounts, bool deposit) external view returns (uint256);
}
interface ITriPool is IPoolBase {
    function add_liquidity(uint256[3] calldata uamounts, uint256 min_mint_amount) external;
    function calc_token_amount(uint256[3] calldata uamounts, bool deposit) external view returns (uint256);
}
interface IQuadPool is IPoolBase {
    function add_liquidity(uint256[4] calldata uamounts, uint256 min_mint_amount) external;
    function calc_token_amount(uint256[4] calldata uamounts, bool deposit) external view returns (uint256);
}
interface IQuintPool is IPoolBase {
    function add_liquidity(uint256[5] calldata uamounts, uint256 min_mint_amount) external;
    function calc_token_amount(uint256[5] calldata uamounts, bool deposit) external view returns (uint256);
}

interface IBiLendingPool is IPoolBase {
    function add_liquidity(uint256[2] calldata uamounts, uint256 min_mint_amount, bool use_underlying) external;
    function calc_token_amount(uint256[2] calldata uamounts, bool deposit) external view returns (uint256);
}
interface ITriLendingPool is IPoolBase {
    function add_liquidity(uint256[3] calldata uamounts, uint256 min_mint_amount, bool use_underlying) external;
    function calc_token_amount(uint256[3] calldata uamounts, bool deposit) external view returns (uint256);
}
interface IQuadLendingPool is IPoolBase {
    function add_liquidity(uint256[4] calldata uamounts, uint256 min_mint_amount, bool use_underlying) external;
    function calc_token_amount(uint256[4] calldata uamounts, bool deposit) external view returns (uint256);
}
interface IQuintLendingPool is IPoolBase {
    function add_liquidity(uint256[5] calldata uamounts, uint256 min_mint_amount, bool use_underlying) external;
    function calc_token_amount(uint256[5] calldata uamounts, bool deposit) external view returns (uint256);
}

interface IPool is IBiPool, ITriPool, IQuadPool, IQuintPool {}

interface ILendingPool is IBiLendingPool, ITriLendingPool, IQuadLendingPool, IQuintLendingPool {}
