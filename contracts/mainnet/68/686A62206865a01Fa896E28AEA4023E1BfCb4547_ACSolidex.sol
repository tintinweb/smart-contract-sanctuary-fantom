...
contract ACSolidex is AutoCompoundStrategy {
    ...
    IERC20 public boo = IERC20(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE);
    IBooMirrorWorld public xboo = IBooMirrorWorld(0xa48d959AE2E88f1dAA7D5F611E01908106dE7598);
    ...
    function earn(address _bountyHunter) external override onlyOwner returns (uint256 bountyReward) {
        ...
        _safeSwap(wnativeAmount, swapPath[WNATIVE][address(boo)], address(this), false);

        uint256 amountXBoo = boo.balanceOf(address(this)) / 2;
        if (amountXBoo > 0) {
            boo.approve(address(xboo), amountXBoo);
            xboo.enter(amountXBoo / 2);
        }
