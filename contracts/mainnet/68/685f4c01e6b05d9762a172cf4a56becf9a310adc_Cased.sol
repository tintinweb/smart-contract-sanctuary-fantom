...
contract Cased is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 72h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 27000 ether;
    // Distribution for DAO Fund
    uint256 public constant INITIAL_DAO_WALLET_DISTRIBUTION = 3000 ether;
     ...
     * @notice Constructs
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Cased Fund", "CASED") {
    ...
    function _getCasedPrice() internal view returns (uint256 _casedPrice) {
        try IOracle(casedOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Error: failed to fetch CASED price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _casedPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_casedPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCasedOracle(address _casedOracle) public onlyOperatorOrTaxOffice {
        require(_casedOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCasedPrice = _getCasedPrice();
            currentTaxRate = _updateTaxRate(currentCasedPrice);
            if (currentCasedPrice < burnThreshold) {
                burnTax = true;
            }
        }
