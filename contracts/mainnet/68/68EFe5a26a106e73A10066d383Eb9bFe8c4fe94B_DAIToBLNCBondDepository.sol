...
contract DAIToBLNCBondDepository is Ownable, ReentrancyGuard {
  ...
  constructor(address _DAI, address _BLNC, address _DAO) {
  ...
  function deposit(
    ...
    uint256 payoutInDAI = _amount;

    IERC20(BLNC).safeTransferFrom(DAO, address(this), payout);

    // transfer DAI to DAO
    IERC20(DAI).safeTransferFrom(msg.sender, DAO, payoutInDAI);
  ...
  function getBLNCPrice() public view returns (uint256 _marketPrice) {
    return 100e18 / uint256(210); // 1 BLNC = 2.10 DAI, in 18 decimals
  }
   ...
   *  @param _daiValue uint256 DAI value
   *  @return uint256 usdb value
   */
  function payoutFor(uint256 _daiValue) public view returns (uint256) {
    return _daiValue.mul(bondPrice()).div(1e18);
  }
  ...
  function bondPriceInBLNC() public view returns (uint256 price_) {
    price_ = bondPrice().mul(10 ** IERC20(BLNC).decimals()).div(1e18);
  }

  /**
   *  @notice calculate current ratio of debt to DAI supply
  ...
  /**
   *  @notice calculate amount of DAI available for claim by depositor
   *  @param _depositor address
