...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant fbomb = address(0x74ccbe53F77b08632ce0CB91D3A545bF6B8E0979);
    ...
    constructor(
        ...
        uint8 _slippage,
        address _strategist,
        IEqualizerRouter.Routes[] memory _wftmTofBombPath
        )
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR fBOMB-FTM EQUALIZER")),
            string(abi.encodePacked("XpE-fBOMB-FTM"))
        )
        {
        gauge = _gauge;
        router = _router;
        ...
        for (uint i; i < _wftmTofBombPath.length; ++i) {
            wftmTofBombPath.push(_wftmTofBombPath[i]);
        }

        slippageLPs = [address(0x7547d05dFf1DA6B4A2eBB3f0833aFE3C62ABD9a1), address(0x3d6c56f6855b7Cc746fb80848755B0a9c3770122)];
    ...
    function _addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(wftmHalf, minAmt, wftmTofBombPath, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(wftm, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(fbomb, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(wftm, fbomb, false, t1Bal, t2Bal);  
        IEqualizerRouter(router).addLiquidity(wftm, fbomb, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyOwner {   
        if(_token == equal || _token == wftm || _token == fbomb){revert XpandrErrors.InvalidTokenOrPath();}        
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(fbomb, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(fbomb, router, type(uint).max);
    }
