...
/*    function sqrt(uint256 x) public pure returns (uint256) {
    ...
    function getNewReserve(uint256 _pricePublic, uint256 _kPrivate, uint8 _decimalBase, uint8 _decimalAsset) external view returns (uint256) {
        uint256 dBase = uint256(_decimalBase);
        uint256 dAsset = uint256(_decimalAsset);

        if (dAsset >= dBase) {
            return this.sqrt(_kPrivate.mul(_pricePublic).mul(10 ** dAsset)).div(10 ** (dAsset.add(dAsset).sub(dBase)));
        } else {
            return 0;
        }
    }
