...
contract OTClp is Ownable, ContractGuard {    
    ...
    address private tokenForSale;  // address of obol, smelt, etc...
    address private NATIVE;  // native gas token of blockchain
    address private otcFund;
    uint256 private totalSmeltSwapped = 0;

    address private BSHARE_FTM_LP = 0x6F607443DC307DCBe570D0ecFf79d65838630B56;
    address private BBOND = 0xC078285F16665B3F4bCe74AbDCF0f4C877de3E9f;
    ...
    mapping(address => bool) public assetLp;
    mapping(address => mapping(address => address)) private tokenBridgeForRouter;
    mapping(address => address) public priceSourceToken;
    ...
    constructor(address _tokenForSale, address _native, address _otcFund) Ownable() {
    ...
    function initialize() public onlyOwner {
        addAsset(BSHARE_FTM_LP, spookyRouter);
        addAsset(tokenForSale, spookyRouter);
        addAsset(NATIVE, spookyRouter);
        addAsset(BBOND,spookyRouter);
        setTokenBridgeForRouter(BASED, spookyRouter, TOMB);
    }
    ...
    function TotalSmeltSwapped() public view returns (uint256) {
        return totalSmeltSwapped;
    }
    // @in - address of Bshare- FTM LP
    // @amount - amount of our token
    // @_router - router that token uses
    // MAIN ENTRY TO THIS CONTRACT
    function swapToken(address _in, uint256 amount, address _router, uint256 minimumAmount) external onlyOneBlock assetWhitelist(_in, _router) {
        ...
        uint256 tokenForSaleAmount = _swapOnOTC(_in, amount,_router,msg.sender,minimumAmount);
        // used by UI to calculated total amount saved for buyers
        totalSmeltSwapped = totalSmeltSwapped.add(tokenForSaleAmount);
    }

    // @_in - token we want to throw in
    // @amount - amount of our _in
    // @routerAddr - tombswap
    function _estimateOTCSwap(address _in,uint256 _amount, address _routerAddr)
                    public view assetWhitelist(_in, _routerAddr) returns (uint256) {

        address[] memory path;

        if(assetLp[_in] == true)
        {
            // we get Price of Bshare-ftm LPs in FTM
           _amount = _priceInNative(_in,_amount);
            // we receive path for SMELT-FTM
           path = _getPath(NATIVE, tokenForSale , _routerAddr);
        }
        else
        {
            address token = _in;

            if(priceSourceToken[_in] != address(0))
                token = priceSourceToken[_in];
            path = _getPath(token, tokenForSale,_routerAddr);
        }
      ...
      // bshare - ftm LP price getter
    function _priceInNative(address _in,uint256 _amount) public view returns (uint256)   
    {
        IUniswapV2Pair pair = IUniswapV2Pair(_in);

        require(pair.token0() == NATIVE || pair.token1() == NATIVE,"Invalid Pair");  
        ...
        if(pair.token0() == NATIVE)
            reserve = reserve0;
        else
            reserve = reserve1;

        uint256 priceInNative = ((reserve.mul(2)).div(totalSupply)).mul(_amount);  
    ...
    returns (uint256)
    {
        ...
        require (_minimumAmount < estimateOTC, "no fee < dex fee");
        ...
        IERC20(tokenForSale).safeTransfer(_recipient, estimateOTC);

        return estimateOTC;
    }

    function _getPath(address _in, address _out,address _routerAddress) private view returns(address[] memory)
    {
        address fromBridge = tokenBridgeForRouter[_in][_routerAddress];
        address toBridge = tokenBridgeForRouter[_out][_routerAddress];
        address[] memory path;
    ...
    // withdraw tokens for sale from contract in case of emergency to stop swapping ability
    ...
    function addAssetLp(address _assetLp,address _router)public onlyOwner {
        assetLp[_assetLp] = true;
        addAsset(_assetLp,_router);
    }
    ...
    function removeAsset(address _asset, address _router) external onlyOwner
                                            assetWhitelist(_asset, _router){
        asset[_asset][_router] = false;
    }
    function setPriceSourceToken(address _asset, address _token) external onlyOwner{
        priceSourceToken[_asset] = _token;
    }
