...
contract Oracle is AccessControl, IOracle {
    ...
    address public dynamicRedeemer; // Dynamic redeem contract address
    ...
    bytes32 public constant PRICE_SETTER_ROLE = keccak256("PRICE_SETTER_ROLE"); // price setter role

    constructor(
        address muon_,
        address dynamicRedeemer_,
        uint32 appId_,
    ...
    /// @notice sets dynamic redeem contract address
    /// @param dynamicRedeemer_ address of the Dynamic Redeem contract
    function setDynamicRedeemer(address dynamicRedeemer_)
        external
        onlyRole(SETTER_ROLE)
    {
        emit SetDynamicRedeemer(dynamicRedeemer, dynamicRedeemer_);
        dynamicRedeemer = dynamicRedeemer_;
    }
    ...
    function getPrice() external view returns (uint256) {
        ...
        uint256 dynamicRedeemerDeiPrice = IDynamicRedeemer(dynamicRedeemer)
            .usdRedeemPerDEI() * 1e12;
        return
            dynamicRedeemerDeiPrice > price ? dynamicRedeemerDeiPrice : price;
    }
}
...
interface IOracle {
    ...
    event SetDynamicRedeemer(address oldValue, address newValue);
