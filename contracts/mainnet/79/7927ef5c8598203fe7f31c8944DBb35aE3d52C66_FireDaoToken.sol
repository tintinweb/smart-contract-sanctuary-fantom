...
contract FireDaoToken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 FIRE-WETH LP -> FIRE pool
    uint256 public constant INITIAL_FIRE_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the FIRE ERC-20 contract.
     */

     function createFireToken(uint256 _taxRate, address _taxCollectorAddress) public initializer {
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");
        __ERC20_init("FireDao Token", "FIRE", 18);
    ...
    function _getFIREPrice() internal view returns (uint256 _FIREPrice) {
        try IOracle(FIRE_Oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("FIRE: failed to fetch FIRE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _FIREPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_FIREPrice >= taxTiersTwaps[tierId]) {
    ...
    function setupFIREOracle(address _FIREOracle) public onlyOperatorOrTaxOffice {
        require(_FIREOracle != address(0), "oracle address cannot be 0 address");
        FIRE_Oracle = _FIREOracle;
    }
    ...
    /**
     * @notice Operator mints FIRE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FIRE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFIREPrice = _getFIREPrice();
            currentTaxRate = _updateTaxRate(currentFIREPrice);
            if (currentFIREPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeRewardFIREPool(address _FIREPool) external onlyOperator {
        require(_FIREPool != address(0), "!_FIREPool");
        rewardPoolDistributed = true;
        _mint(_FIREPool, INITIAL_FIRE_POOL_DISTRIBUTION);
    }
