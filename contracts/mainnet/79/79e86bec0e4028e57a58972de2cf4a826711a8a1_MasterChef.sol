...
// WokToken.
contract WokToken is BEP20('Wok Finance Token', 'WOK') {
...
// MasterChef is the master of Wok. He can make Wok and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once WOK is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accWokPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accWokPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accWokPerShare;  // Accumulated KRs per share, times 1e18. See below.
        uint16 depositFeeBP;        // Deposit fee in basis points
        uint256 harvestInterval;    // Harvest interval in seconds
    }

    // The WOK TOKEN!
    WokToken public immutable wok;
    // Dev address.
    address public devAddress;
    // WOK tokens created per second.
    uint256 public wokPerSecond;
    // Deposit Fee address
    ...
    // Maximum wokPerSecond
    uint256 public constant MAX_EMISSION_RATE = 0.1 ether;
    // Initial wokyPerSecond
    uint256 public constant INITIAL_EMISSION_RATE = 0.05 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed wokPerSecond);
    ...
    constructor(
        ...
        wokPerSecond = INITIAL_EMISSION_RATE;
    ...
    function pendingWok(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accWokPerShare = pool.accWokPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 wokReward = multiplier.mul(wokPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accWokPerShare = accWokPerShare.add(wokReward.mul(1e18).div(lpSupply));
        }

        uint256 pending = user.amount.mul(accWokPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }


    // View function to see if user can harvest Wok's.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (wok.totalSupply() >= MAX_SUPPLY) {
            wokReward = 0;
        } else if (wok.totalSupply().add(wokReward.mul(11).div(10)) >= MAX_SUPPLY) {
            wokReward = (MAX_SUPPLY.sub(wok.totalSupply()).mul(10).div(11));
        }

        if (wokReward > 0) {
            wok.mint(devAddress, wokReward.div(10));
            wok.mint(address(this), wokReward);
            pool.accWokPerShare = pool.accWokPerShare.add(wokReward.mul(1e18).div(lpSupply)); 
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accWokPerShare).div(1e18);
    ...
    // Pay or lockup pending Wok.
    function payOrLockupPendingWok(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accWokPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.nextHarvestUntil = getPoolHarvestInterval(_pid);
                // send rewards
                safeWokTransfer(msg.sender, rewardsToDistribute);
            }
        } else if (pending > 0) {
    ...
    // Safe Wok transfer function, just in case if rounding error causes pool to not have enough KRs.
    function safeWokTransfer(address _to, uint256 _amount) internal {
        uint256 wokBal = wok.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > wokBal) {
            transferSuccess = wok.transfer(_to, wokBal);
        } else {
            transferSuccess = wok.transfer(_to, _amount);
        }
        require(transferSuccess, "safeWokTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _wokPerSecond) external onlyOwner {
        require (_wokPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: value higher than maximum");
        massUpdatePools();
        wokPerSecond = _wokPerSecond;
        emit UpdateEmissionRate(msg.sender, _wokPerSecond);
    }
