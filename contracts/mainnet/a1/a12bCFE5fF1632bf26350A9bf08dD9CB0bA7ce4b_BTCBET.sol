...
contract BTCBET is Ownable, TOKEN, ReentrancyGuard, BlackList {
    ...
    function createBet(string memory pool_, uint amount_) external onlyIfResetNotInProgress nonReentrant {
    ...
    fallback() external {
        revert("You can't do that with this token. Your action has been canceled");
    }
}