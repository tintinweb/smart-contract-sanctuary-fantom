...
contract EGG is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_TEAM_FUND_DISTRIBUTION = 4000 ether;
    ...
    function _getEggPrice() internal view returns (uint256 _eggPrice) {
        try IOracle(eggOracle).consult(address(this), 1e18) returns (uint144 _price) {
            ...
            revert("Error: failed to fetch EGG price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _eggPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_eggPrice >= taxTiersTwaps[tierId]) {
    ...
    function setEggOracle(address _eggOracle) public onlyOperatorOrTaxOffice {
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEggPrice = _getEggPrice();
            currentTaxRate = _updateTaxRate(currentEggPrice);
            if (currentEggPrice < burnThreshold) {
                burnTax = true;
            }
        }
