...
// File @openzeppelin/contracts-upgradeable/utils/introspection/IERC1820RegistryUpgradeable[email protected]
...
// File @openzeppelin/contracts-upgradeable/access/[email protected]v4.5.2
    ...
    IERC777SenderUpgradeable
{
    ...
    string private constant TOKEN_NAME = "Avalanche Boxed Cat";
    string private constant TOKEN_SYMBOL = "abCAT";
    uint8 private constant TOKEN_DECIMALS = 18;

    bytes32 private constant TAX_MANAGER = keccak256("TAX_MANAGER");
    ...
    function initialize() external virtual initializer {
        ...
        __ERC777_init(TOKEN_NAME, TOKEN_SYMBOL, defOps);
        __AccessControlEnumerable_init();

        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(TAX_MANAGER, _msgSender());

        taxRate = 0;
        taxCollector = _msgSender();

        bridgeRoles.add(_msgSender());
        chainIds[0] = true;
    }
    ...
    // ############## enable IERC20 burn calls to work ################
    ...
    function setTaxRate(uint256 newTaxRate) external virtual onlyRole(TAX_MANAGER) {
        require(newTaxRate <= 5000, "Tax rate must be <= 50%");
        taxRate = newTaxRate;
    }

    function setTaxCollector(address newTaxCollector) external virtual onlyRole(TAX_MANAGER) {
        require(newTaxCollector != address(0), "Tax collector cannot be 0x0");
    ...
    function _send(
        ...
        address operator = _msgSender();
        uint256 toDeduce = amount.mul(taxRate).div(10000);
        require(toDeduce > 0, "Amount too small");

        // deduce tax
        super._send(operator, address(this), amount, userData, operatorData, requireReceptionAck);

        amount = amount.sub(toDeduce);

        // send tokens as intended
        super._send(operator, to, amount, userData, operatorData, requireReceptionAck);
    }

    // ################## IERC777 Recipient + Sender #################
    ...
    // ################ functions requrid by cBridge ##################

    /**
     * @dev Mint function used by bridge. Optional FeeAddress and FeeAmount parameters used to mint 
     * small percentage of transfered assets directly to bridge.
    ...
    function mint(
        ...
        require(bridgeRoles.has(_msgSender()), "Unauthorized.");
        _mint(to, amount, "", "");
        if (feeAmount > 0) {
    ...
    function addSupportedChainId(uint256 chainId) public {
        require(bridgeRoles.has(_msgSender()), 
                "Unauthorized.");
    ...
    function unwrap(uint256 amount, uint256 chainId) public {
        require(tx.origin == (_msgSender()), "Contract calls not supported.");
