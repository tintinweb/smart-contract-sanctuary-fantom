...
import "./ISecretPool.sol";
import "../whilelist/Whitelist.sol";

contract SecretPool is ISecretPool, Whitelist {
...
/*
 *  @title add a whitelist system with an variable staking limit on initial staking contract
 */
abstract contract Whitelist is Ownable {
    mapping(address => uint256) public holdersToStakingLimit;

    event StakingLimitAdded(address holder, uint256 stakingLimit);
    event StakingLimitUpdated(address holder, uint256 stakingLimit);

    function addStakingLimit(address _holder, uint256 _stakingLimit)
        external
        onlyOwner
    {
        require(
            holdersToStakingLimit[_holder] == 0,
            "User has already a staking limit"
        );
        require(_stakingLimit > 0, "Staking limit can not be 0");
        _editStakingLimit(_holder, _stakingLimit);
        emit StakingLimitAdded(_holder, _stakingLimit);
    }

    function updateStakingLimit(address _holder, uint256 _newStakingLimit)
        external
        onlyOwner
    {
        _editStakingLimit(_holder, _newStakingLimit);
        emit StakingLimitUpdated(_holder, _newStakingLimit);
    }

    function _editStakingLimit(address _holder, uint256 _stakingLimit) private {
        holdersToStakingLimit[_holder] = _stakingLimit;
    }

    modifier onlyUnderStakingLimit(uint256 _amountToStake) {
        require(
            holdersToStakingLimit[_msgSender()] >= _amountToStake,
            "Deposit excess staking limit"
        );
        _;
        _editStakingLimit(
            _msgSender(),
