...
import {IUniswapV2Factory} from "@solarprotocol/solar-diamond/contracts/modules/uniswap/interfaces/IUniswapV2Factory.sol";
import {IUniswapV2Router02} from "@solarprotocol/solar-diamond/contracts/modules/uniswap/interfaces/IUniswapV2Router02.sol";

// solhint-disable-next-line contract-name-camelcase
contract setupInitialToken_0002 {
    function migrate(
        IUniswapV2Factory factory,
        IUniswapV2Router02 router,
        address treasury,
        ...
        require(address(router) != address(0), "router is zero address");
        ...
        LibSoloToken.init("VOID", "VOID", defaultOperators, true);

        LibSoloToken.mint(treasury, 1e9 * 1e18, "", "", false);

        require(
            LibSoloToken.totalSupply() == 1e9 * 1e18,
            "TotalSupply doesn't match"
        );

        LibUniswap.setFactory(factory);
        LibUniswap.setRouter(router);
        LibUniswap.setTokenB(stableCoin);
        LibUniswap.createPair(factory, stableCoin);
        LibUniswap.setIntermediateWallet(intermediateWallet);
        LibSoloToken.approve(
            intermediateWallet,
            address(this),
            type(uint256).max
        );
