...
// File: contracts/token/ERC721/CustomFAFz.sol
...
contract stonecold2 is ERC721Enumerable, Ownable {
    using Strings for uint256;

    string private baseURI;

    uint256 public tokenCount = totalSupply() + 1;
    ...
    constructor(string memory _initBaseURI) ERC721("stoneCold2", "STNS2") {
    ...
    function batchAddFreeMints(address[] memory addresses, uint256[] memory freeMints)public onlyOwner{
    ...
    function updateFreeMints(address _address, uint256 _amount)private{
        // First, delete the current value of the mapping
        delete freeMintCount[_address];
        // Then, assign the new value to the key
        freeMintCount[_address] = _amount;
    }

    function getFreeMintCount(address _address)public view returns(uint256){
        return freeMintCount[_address];
    }
    ...
    function overwriteMappingValue(uint256 tokenId, string memory newValue) public onlyOwner{
        // First, delete the current value of the mapping
    ...
    function mint(uint256 _id, string memory _ipfsHash)public {
        require(isfreeMinted(msg.sender), "User not in snapshot");
        require(totalSupply() + 1 == _id, "Incorrect token ID");
        address _user = msg.sender;
        uint256 fmints = getFreeMintCount(_user);
        require(fmints > 0, "No more mints avaiable");
        _safeMint(msg.sender, _id);
        addTokenMapping(_id, _ipfsHash);
        ...
        returns (string memory)
    {
        ...
        string memory hash = getTokenHash(tokenId);
