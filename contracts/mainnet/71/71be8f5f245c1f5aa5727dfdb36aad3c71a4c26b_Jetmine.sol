...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 constant MAXSUPPLY = 3500 ether;
...
contract Jetmine is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0x672D078cc1593c3950b8076422E2bcAFED34C09C;
    ...
    // Min amount to liquify. (default 10 Jetmines)
    ...
    // The operator can call all functions except of minting Jetmine
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "JETMINE::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    modifier blackList(address sender, address recipent) {
        if (maxHolding() > 0) {
            if (_includeToBlackList[sender] == true || _includeToBlackList[recipent] == true) {
                require(_includeToBlackList[sender] == false,"JETMINE::blackList: You have been blacklisted as a bot (SENDER)");
                require(_includeToBlackList[recipent] == false,"JETMINE::blackList: You have been blacklisted as a bot (RECIPENT)");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the JetmineSwapToken contract.
     */
    constructor() public BEP20("Jetmine DAO", "JETMINE") {
    ...
    /// @dev overrides transfer function to meet tokenomics of JETMINE
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "JETMINE::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "JETMINE::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the JETMINE -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "JETMINE::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "JETMINE::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "JETMINE::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "JETMINE::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "JETMINE::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "JETMINE::transferOperator: new operator is the zero address");
...
// MasterChef is the master of JETMINE. He can make JETMINE and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once JETMINE is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of JETMINE
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accJetminePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accJetminePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. JETMINEs to distribute per second.
        uint256 lastRewardSecond; // Last second that JETMINEs distribution occurs.
        uint256 accJetminePerShare; // Accumulated JETMINEs per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The JETMINE TOKEN!
    Jetmine public jetmine;
    // Dev address.
    address public devaddr;
    // JETMINE tokens created per second.
    uint256 public JetminePerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when Jetmine mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 JetminePerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _JetminePerSecond,
        uint256 _startTime
    ) public {
        jetmine = _jetmine;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        JetminePerSecond = _JetminePerSecond;
    ...
    // Update the given pool's Jetmine allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending jetmine on frontend.
    function pendingJetmine(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accJetminePerShare = pool.accJetminePerShare;
        if (
            ...
            uint256 jetmineReward = multiplier
                .mul(JetminePerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
                uint256 devReward = jetmineReward.div(10);
                uint256 totalRewards = jetmine.totalSupply().add(devReward).add(
                jetmineReward
            );
                if (totalRewards > jetmine.maxSupply()) {
                jetmineReward = jetmine.maxSupply().sub(jetmine.totalSupply());
            }
            accJetminePerShare = accJetminePerShare.add(
                jetmineReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accJetminePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= jetmine.maxSupply()) {
            ...
            jetmine.mint(devaddr, jetmineReward.div(10));
            jetmine.mint(address(this), jetmineReward);
        } else {
            // mint the difference only to MC, update jetmineReward
            jetmineReward = jetmine.maxSupply().sub(jetmine.totalSupply());
            jetmine.mint(address(this), jetmineReward);
        }

        if (jetmineReward != 0) {
            // only calculate and update if jetmineReward is non 0
            pool.accJetminePerShare = pool.accJetminePerShare.add(
                jetmineReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    // Deposit LP tokens to MasterChef for jetmine allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accJetminePerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                safeJetmineTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accJetminePerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accJetminePerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeJetmineTransfer(msg.sender, pending);
    ...
    // Safe Jetmine transfer function, just in case if rounding error causes pool to not have enough Jetmines.
    function safeJetmineTransfer(address _to, uint256 _amount) internal {
        uint256 jetmineBal = jetmine.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > jetmineBal) {
            transferSuccess = jetmine.transfer(_to, jetmineBal);
        } else {
            transferSuccess = jetmine.transfer(_to, _amount);
        }
        require(transferSuccess, "safeJetmineTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _JetminePerSecond) external onlyOwner {
        require(_JetminePerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        JetminePerSecond = _JetminePerSecond;
        emit UpdateEmissionRate(msg.sender, _JetminePerSecond);
    }
