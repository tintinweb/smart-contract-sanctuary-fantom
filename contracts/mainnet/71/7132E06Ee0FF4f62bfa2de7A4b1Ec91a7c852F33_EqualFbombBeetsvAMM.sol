...
// File: contracts/EqualFbombBeetsvAMM.sol
...
contract EqualFbombBeetsvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public fbombToBeetsPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _fbombToBeetsPath
        ...
        for (uint i; i < _fbombToBeetsPath.length; ++i) {
            fbombToBeetsPath.push(_fbombToBeetsPath[i]);
        }
    ...
    function chargeFees(address caller) internal {                   
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(equalBal, 0, equalToFbombPath, address(this), block.timestamp);                                                
    }

    /** @dev Converts WMFTM to both sides of the LP token and builds the liquidity pair */
    function addLiquidity() internal {
        uint256 wrappedHalf = IERC20(fbomb).balanceOf(address(this)) / 2;

        
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, fbombToBeetsPath, address(this), block.timestamp);
        
        uint256 lp0Bal = IERC20(fbomb).balanceOf(address(this));
        uint256 lp1Bal = IERC20(beets).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(fbomb, beets, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(fbomb).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(fbomb).safeApprove(unirouter, type(uint).max);
    }
