...
contract LiquidityManager is Operator {
    ...
    address public dollar = address(0xecB47d5cC8f095D62FCEe7A6eF62e3E2b32207A5);
    ...
    {
        require(amtDollar != 0 && amtToken != 0, "amounts can't be 0");

        IERC20(dollar).transferFrom(msg.sender, address(this), amtDollar);
        IERC20(token).transferFrom(msg.sender, address(this), amtToken);
        _approveTokenIfNeeded(dollar, uniRouter);
        _approveTokenIfNeeded(token, uniRouter);

        uint256 resultAmtDollar;
        uint256 resultAmtToken;
        uint256 liquidity;
        (resultAmtDollar, resultAmtToken, liquidity) = IUniswapV2Router(uniRouter).addLiquidity(
            dollar,
            token,
            amtDollar,
            amtToken,
        ...
        if(amtDollar.sub(resultAmtDollar) > 0) {
            IERC20(dollar).transfer(msg.sender, amtDollar.sub(resultAmtDollar));
        }
        if(amtToken.sub(resultAmtToken) > 0) {
            IERC20(token).transfer(msg.sender, amtToken.sub(resultAmtToken));
        }
        return (resultAmtDollar, resultAmtToken, liquidity);
    }
