 ...
 * Those destinations are: Dividends contract, dev address, SAFU funds address and DAO treasury address
 */
contract FeeManager is Ownable, ReentrancyGuard {
  ...
  uint256 public treasuryShare = 5; // = 5%
  uint256 public safundsShare = 5; // = 5%
  uint256 public devShare = 20; // = 20%

  uint256 public constant MIN_DIVIDENDS_SHARE = 50; // 50%
  uint256 public constant MIN_TREASURY_SHARE = 5; // 5%
  uint256 public constant MAX_TREASURY_SHARE = 25; // 25%
  ...
  mapping(address => bool) public tokensToBurn;
  ...
  constructor(
    ...
    // add EXC to autoburn
    tokensToBurn[address(excToken_)] = true;
  }
  ...
  event SharesUpdated(uint256 dividendsShare, uint256 treasuryShare, uint256 safundsShare, uint256 devShare);
  event RouterInitialized(address routerAddress);
  event AutoburnedTokensUpdated(address token, bool oldStatus, bool newStatus);
  ...
  /**
   * @dev Burn collected token
   */
  function burnToken(address token) external onlyOwner {
    if(tokensToBurn[token]) {
      uint256 balance = IERC20(token).balanceOf(address(this));
      if(balance > 0) {
        if(token == address(excToken)) {
          excToken.burn(balance);
        } else {
          IERC20(token).safeTransfer(BURN_ADDRESS, balance);
        }
        emit TokenBurned(token, balance);
      }
    }
  }

  /**
   * @dev Burn collected tokens list
   */
  function burnTokens(address[] calldata tokens) external onlyOwner {
  ...
  function updateShares(
    ...
    require(
      treasuryShare_ >= MIN_TREASURY_SHARE,
      "FeeManager: treasuryShare mustn't exceed minimum"
    );
    require(
      treasuryShare_ <= MAX_TREASURY_SHARE,
      "FeeManager: treasuryShare mustn't exceed maximum"
    );
    ...
    require(
      dividendsShare_.add(treasuryShare_).add(safundsShare_).add(devShare_) == 100,
    ...
    devShare = devShare_;
    emit SharesUpdated(dividendsShare, treasuryShare, safundsShare, devShare);
  }
   ...
   */
  function removeLiquidityToToken(IExcaliburV2Pair pair) external onlyOwner {
    _removeLiquidityToToken(pair);
  }

  /**
   * @dev Unbind list of LP tokens
   */
  function removeAllLiquidityToToken(IExcaliburV2Pair[] calldata pairs) external onlyOwner {
    uint256 pairsLength = pairs.length;
    for (uint256 i = 0; i < pairsLength; i++) {
      _removeLiquidityToToken(pairs[i]);
    }
  }
  ...
  function _swapAmountToToken(address token0, address token1, uint256 token0Amount, uint256 minAmountToReceived) internal onlyOwner {
    ...
    require(!tokensToBurn[token0], "FeeManager: cannot swap tokensToBurn token");
  ...
  /**
   * @dev Updates token autoburn status
   */
  function updateAutoburnedToken(address token, bool newStatus) external onlyOwner {
    emit AutoburnedTokensUpdated(token, tokensToBurn[token], newStatus);
    tokensToBurn[token] = newStatus;
  }
  ...
  function _distributeToken(IERC20 token) internal {
    ...
    uint256 treasuryAmount = tokenBalance.mul(treasuryShare).div(100);
    uint256 devAmount = tokenBalance.sub(dividendsAmount).sub(safundsAmount).sub(treasuryAmount);
   ...
   */
  function _removeLiquidityToToken(IExcaliburV2Pair pair) internal {
