...
contract XShareLocker is Operator, IXShareLocker {
    ...
    bool public unlockAll = false; //Allow to unlock all without waiting lock end, use in emergency cases
    ...
    modifier validDuration(uint256 _duration) {
        require((_duration % MIN_LOCK_DURATION) == 0 && _duration >= MIN_LOCK_DURATION && _duration <= MAX_LOCK_DURATION, "Invalid Lock duration");
        _;
    }

    modifier isUnlockAll() {
        require(unlockAll, "Not unlock all");
        _;
    }
    ...
    function lockXShare(uint256 _amount, uint256 _lockDuration) public override {
        require(_amount > 0, "Invalid amount");
        require((_lockDuration % MIN_LOCK_DURATION) == 0, "Invalid duration");
        require(_lockDuration >= MIN_LOCK_DURATION && _lockDuration <= MAX_LOCK_DURATION, "Min Lock 1 week and max lock 208 week");
        UserInfo storage user = userInfo[msg.sender];
        require(user.lockedAmount == 0, "Please use add more function");
        uint256 yShare_minting_amount = calculateYShareMintAmount(_amount, _lockDuration);

        IYShare(yShare).lockerMintFrom(msg.sender, yShare_minting_amount);
        IERC20(xShare).safeTransferFrom(msg.sender, address(this), _amount);

        user.lockStartTime = block.timestamp;
        user.lockEndTime = block.timestamp.add(_lockDuration);
        ...
        updateAverageLockDuration(_amount, _lockDuration);
        totalLockedAmount = totalLockedAmount.add(_amount);
        //TODO emit lock event
    }
    ...
    function addMoreXShare(uint256 _amount) public override {
        require(_amount > 0, "Invalid Amount");
//        require(userInfo[msg.sender].lockEndTime < block.timestamp && userInfo[msg.sender].lockedAmount > 0, "Lock ended / Not locked any xShare");
        UserInfo storage user = userInfo[msg.sender];
        if (user.lockEndTime < block.timestamp && user.lockedAmount > 0) {
            uint256 yShare_minting_amount = calculateYShareMintAmount(_amount, user.lockDuration);
            IERC20(xShare).safeTransferFrom(msg.sender, address(this), _amount);
            IYShare(yShare).lockerMintFrom(msg.sender, yShare_minting_amount);

            user.lockedAmount = user.lockedAmount.add(_amount);
            user.yShareMinted = user.yShareMinted.add(yShare_minting_amount);
            updateAverageLockDuration(_amount, user.lockDuration);
            totalLockedAmount = totalLockedAmount.add(_amount);
            //TODO add event addMore
        }
    }

    //If User has locked XShare before, user can extend their lock duration to receive more yShare
    function extendLockDuration(uint256 _extendLockDuration) public override {
        UserInfo storage user = userInfo[msg.sender];
        require((_extendLockDuration % MIN_LOCK_DURATION) == 0, "Invalid duration");
        require(_extendLockDuration >= MIN_LOCK_DURATION && _extendLockDuration <= MAX_LOCK_DURATION, "Min Lock 1 week and max lock 208 week");
//        require(userInfo[msg.sender].lockEndTime < block.timestamp, "Lock ended / Not locked any xShare");
        if (user.lockEndTime < block.timestamp && user.lockedAmount > 0) {
            ...
            //TODO add extend event
        }
    }

    function unlockXShare(uint256 _amount) public override {
        UserInfo storage user = userInfo[msg.sender];
        require(_amount > 0, "Invalid amount");
        require(userInfo[msg.sender].lockEndTime > block.timestamp, "Still In Lock");
        require(userInfo[msg.sender].lockedAmount >= _amount, "Exceed locked amount");
        ...
        //TODO Add unlock event event
    }

    //In emergency cases, admin will allow user to unlock their xShare immediately
    function emergencyUnlockAll() public override isUnlockAll {
        UserInfo storage user = userInfo[msg.sender];
//        require(user.lockedAmount > 0, "Not locked any xShare");
        require(IERC20(yShare).balanceOf(msg.sender) >= user.yShareMinted, "Unlock exceed balance");

        IYShare(yShare).lockerBurnFrom(msg.sender, user.yShareMinted);
        IERC20(xShare).safeTransfer(msg.sender, user.lockedAmount);

        totalLockedAmount = totalLockedAmount.sub(user.lockedAmount);
        user.lockedAmount = 0;
        user.yShareMinted = 0;
        user.lockDuration = 0;
        user.lockStartTime = 0;
        ...
        //TODO add emergencyUnlockAll event
    }

    function toggleUnlockAll() public onlyOperator {
        unlockAll = !unlockAll;
        //TODO add event toggle unlock all
    }



    function updateAverageLockDuration(uint256 _amount, uint256 _lockDuration) internal {
        uint256 numerator = totalLockedAmount.mul(averageLockDuration).add(_amount.mul(_lockDuration));
        uint256 denominator = totalLockedAmount.add(_amount);

        averageLockDuration = numerator.div(denominator);
    }

    function calculateYShareMintAmount(uint256 _amount, uint256 _lockDuration) internal returns (uint256){
        uint256 boost_amount_factor = _lockDuration.div(MIN_LOCK_DURATION);
        uint256 extra_yShare_per_xShare = EXTRA_YSHARE_PER_WEEK.mul(boost_amount_factor);
        uint256 actual_extra_yShare = _amount.mul(extra_yShare_per_xShare).div(1e18);
        //To calculate factor for minting yShare
        uint256 yShare_minting_amount = _amount.add(actual_extra_yShare);
