    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
    ...
    mapping(uint256 => PlanetEntity) private _darkForest;
    ...
    modifier onlyPlanetOwner() {
        ...
        require(
            ...
            "DarkForest: No Planet owned by this account"
        );
        require(
    ...
    modifier checkPermissions(uint256 _planetId) {
        address sender = _msgSender();
        require(planetExists(_planetId), "DarkForest: This planet doesn't exist");
        require(
            isOwnerOfPlanet(sender, _planetId),
            "DarkForest: You do not control this Planet"
        );
        _;
    }
    ...
    modifier checkPermissionsMultiple(uint256[] memory _planetIds) {
        ...
        for (uint256 i = 0; i < _planetIds.length; i++) {
            require(
                ...
                "DarkForest: This planet doesn't exist"
            );
            require(
        ...
        forestSet
    {
        ...
        PlanetEntity storage planet = _darkForest[_planetId];
    ...
    function mergeDarkForest(uint256[] memory _planetIds, string memory planetName)
        external
        nonReentrant
        onlyPlanetOwner
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < _planetIds.length; i++) {
            PlanetEntity storage planetFromIds = _darkForest[_planetIds[i]];
            require(
        ...
        forest.accountReward(account, newPlanetValue);
        ...
        PlanetEntity storage planet = _darkForest[currentPlanetId];
    ...
    function _createPlanetWithTokens(
        ...
        require(
            ...
            "DarkForest: Planet value set below minimum"
        );
        require(
        ...
        forest.accountBurn(sender, planetValue);
        ...
        _darkForest[newPlanetId] = PlanetEntity({
            id: newPlanetId,
            name: planetName,
        ...
        forestSet
    {
        ...
        uint256[] memory darkForestOwned = getPlanetIdsOf(account);
        for (uint256 i = 0; i < darkForestOwned.length; i++) {
            rewardsTotal += _getPlanetCashoutRewards(darkForestOwned[i]);
        }
        ...
        forestSet
    {
        ...
        for (uint256 i = 0; i < darkForestOwned.length; i++) {
            (
                uint256 amountToCompound,
                uint256 feeAmount
            ) = _getPlanetCompoundRewards(darkForestOwned[i]);
    ...
    function planetExists(uint256 _planetId) private view returns (bool) {
        require(_planetId > 0, "DarkForest: Id must be higher than zero");
        PlanetEntity memory planet = _darkForest[_planetId];
        if (planet.exists) {
            return true;
        }
        return false;
        ...
        returns (bool)
    {
        uint256[] memory darkForestOwned = getPlanetIdsOf(account);
        for (uint256 i = 0; i < darkForestOwned.length; i++) {
            PlanetEntity memory planet = _darkForest[darkForestOwned[i]];
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory planetIds = new uint256[](numberOfDarkForest);
    ...
    function getDarkForestByIds(uint256[] memory _planetIds)
        external
        view
        returns (PlanetInfoEntity[] memory)
    {
        PlanetInfoEntity[] memory darkForestInfo = new PlanetInfoEntity[](
            _planetIds.length
        );
        for (uint256 i = 0; i < _planetIds.length; i++) {
            uint256 planetId = _planetIds[i];
            PlanetEntity memory planet = _darkForest[planetId];
            darkForestInfo[i] = PlanetInfoEntity(
                planet,
                planetId,
    ...
    function changeNodeMinPrice(uint256 _creationMinPrice) public onlyOwner {
        require(
            _creationMinPrice > 0,
            "DarkForest: Minimum price to create a Planet must be above 0"
        );
    ...
    function _burn(uint256 tokenId)
        ...
        override(ERC721URIStorageUpgradeable, ERC721Upgradeable)
    {
        PlanetEntity storage planet = _darkForest[tokenId];
        planet.exists = false;
        logTier(planet.rewardMult, -int256(planet.planetValue));
