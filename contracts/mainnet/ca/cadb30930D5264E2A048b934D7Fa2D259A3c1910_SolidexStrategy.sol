...
interface IMasterChef {
    ...
    function withdraw(address pool, uint256 _amount) external;
    function getReward(address[] calldata pool) external;
    function userBalances(address pool, address _user) external view returns (uint256);
    function pendingRewards(address _user, address[] calldata pool) external view returns (uint256);
}

pragma solidity 0.6.2;

contract SolidexStrategy is Ownable, Pausable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens
    address public constant wrapped = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public constant sex = address(0xD31Fcd1f7Ba190dBc75354046F6024A9b86014d7);
    ...
    constructor(
        ...
        wrappedToLp0Router = _homeRouter;
        wrappedToLp1Router = _homeRouter;
    ...
    function _deposit() internal whenNotPaused {
        ...
        IMasterChef(masterchef).deposit(want, wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMasterChef(masterchef).withdraw(want, _amount.sub(wantBal));
    ...
    function harvestMC() internal {
        address[] memory pool = new address[](1);
        pool[0] = want;
        IMasterChef(masterchef).getReward(pool);
    }

    function sellSEX() internal {
        uint256 sexBal = IERC20(sex).balanceOf(address(this));
        approveTxnIfNeeded(sex, unirouter, sexBal);
        IUniSwapRouter(outputToWrappedRouter).swapExactTokensForTokensSimple(sexBal, 0, sex, wrapped, false, address(this), now);
    }
    ...
    function chargeFees(address caller) internal {                   
        ...
        IUniSwapRouter(outputToWrappedRouter).swapExactTokensForTokensSimple(toWrapped, 0, output, wrapped, false, address(this), now);                                              
    ...
    function addLiquidity() internal {
        ...
        IUniSwapRouter(unirouter).addLiquidity(lpToken0, lpToken1, stable, lp0Bal, lp1Bal, 1, 1, address(this), now);
    }
    ...
    function callReward() public view returns (uint256) {
        address[] memory pool = new address[](1);
        pool[0] = want;
        uint256 outputBal = IMasterChef(masterchef).pendingRewards(address(this), pool);
        uint256 nativeOut;

        if (outputBal > 0) {
            uint256[] memory amountsOut = IUniSwapRouter(homeRouter).getAmountsOut(outputBal, outputToWrappedRoute);
    ...
    function balanceOfPool() public view returns (uint256) {
        uint256 _amount = IMasterChef(masterchef).userBalances(address(this), want);
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IMasterChef(masterchef).withdraw(want, balanceOfPool());
        uint256 wantBal = IERC20(want).balanceOf(address(this));
