...
contract STAKE1 is ERC20Permit, Ownable, SafeMath {

    constructor() ERC20("STAKE1", "STAKE1") ERC20Permit("STAKE1")
    {
    }

    uint256 constant private _maxTotalSupply = 1000000000e18; // 1,000,000,000 MAX
    uint256 constant private emission_rate = 40e16; // per block per alloc_emission (which add up to 100, so 40e18 per block)
    address[] private vested_addresses;
    uint256[] private alloc_emissions;
    uint256 private last_vesting_block_no;
    ...
    function set_vest(address[] memory _vested_addresses, uint256[] memory _alloc_emission) public onlyOwner {
        ...
        last_vesting_block_no = block.number;
    }
    ...
    function execute_vesting() external {
        ...
        blocks_elapsed = block.number - last_vesting_block_no;
        for(uint i=0;i<vested_addresses.length;i++){
            //calculate total emissions based on last emission
            //update block height of last emission
            _mint(vested_addresses[i], alloc_emissions[i]*blocks_elapsed*emission_rate);
        }
        last_vesting_block_no = block.number;
    }
}