...
contract ExchangeGuruPair is IExchangeGuruPair, ExchangeGuruERC20 {
    ...
    function name() public view returns(string memory) {
        return string(abi.encodePacked("ExchangeGuru Liquidity Position: ", IERC20(token0).name(), " paired with ", IERC20(token1).name()));
    }

    function symbol() public view returns(string memory) {
        return string(abi.encodePacked("exchangeGuruLP-", IERC20(token0).symbol(), "/", IERC20(token1).symbol()));
    }
