...
contract Thrifty is IERC20Metadata, Ownable {
  ...
  mapping(uint256 => uint256[8]) public feeLogs;
  uint256 public feeIndex = 0;
  uint256 public processedFeeIndex = 0;
  uint256 public feeDebt = 0;
  uint256 public totalSwappedReward = 0;
  ...
  function getAutoLpThriftyInAmount() public view returns (uint256) {
    ...
    if (balThis > totalSupply().div(2).add(feeDebt)) {
      uint256 amountD = balThis.sub(totalSupply().div(2)).sub(feeDebt);
      (uint112 reserve0, uint112 reserve1,) = IUniswapV2Pair(pairAddress).getReserves();
  ...
  function _addLiquidity(uint256 amountT, uint256 amountP) internal {
    ...
    (address token0, address token1) = (address(this) < peggedToken) ?
      (address(this), peggedToken) : (peggedToken, address(this));
   ...
   */
  function pegTFUSD(bool autoLp, bool daiFromBuffer) public {
    require(IThriftyRole(roleManager).guardianMode() == true, "Thrifty: disable guardian");
    require(IThriftyRole(roleManager).hasTFUSDLpRole(msg.sender), "Unauthorized");
    if (autoLp == false && daiFromBuffer == false) {
      uint256 daiAmount = ITFUSD(tFUSD).getSwapInAmount(false);
      address token = ITFUSD(tFUSD).peggedToken();
      IERC20(token).safeTransferFrom(msg.sender, address(this), daiAmount);
      _approveTokenIfNeeded(token, tFUSD, daiAmount);
    }
    ITFUSD(tFUSD).peg(autoLp, daiFromBuffer);
    _sendGuadianceFee();
  }
  ...
  function getRewardAmount(address account) public view returns(uint256) {
    ...
    for (; x<len; x++) {
      ...
      for (; reflections[y].reflectedAt < nextTs && y < len; y++) {
        ...
        daiAmount = daiAmount.add(reflections[y].daiFee.mul(uAmount).div(reflections[y].totalUserVolum));
      }
    }
    uint256 totalDai = 0;
    len = reflections.length;
    for (x=y; y<len; y++) {
      totalDai = totalDai.add(reflections[y].daiFee);
    }

    return daiAmount.mul(totalSwappedReward).div(totalDai);
  }
  ...
  function _transfer(
    ...
    require(IThriftyRole(roleManager).validateTransfer(from, to, fromBalance, amount), "Thrift: !role");
    unchecked {
  ...
  function _afterTokenTransfer(
    ...
    if (IThriftyRole(roleManager).enableTFUSDAutoLp()) {
      ITFUSD(tFUSD).autoPegAndAddFeeLiquidity();
      _sendGuadianceFee();
    }
  }

  function _reflect(address from, address to, uint256 amount) internal {
    (uint256 receiveAmount, uint256 totalFee, uint256 reflectFee, uint256 rewardFee) = _getFee(from, to, amount);
    uint256 toBalance = balanceOf(to);
    ...
    uint256 thBal = balanceOf(th);
    thBal = onReflect == true ? thBal.add(totalFee).sub(reflectFee) : thBal.add(totalFee);
    feeDebt = onReflect == true ? feeDebt.add(totalFee).sub(reflectFee) : feeDebt.add(totalFee);
    balanceInfo memory thBalO = balanceInfo({
      balance: thBal,
      updatedAt: block.timestamp
    });
    _balances[th] = thBalO;
    balanceLogs[th].push(thBalO);
    uint256 pairBal = balanceOf(pairAddress);

    if (reflectFee > 0 && onReflect == true) {
      reflectionInfo memory info = reflectionInfo({
        totalRAmount: totalRSupply,
        totalUserVolum: totalSupply().sub(thBal).sub(pairBal),
        rFee: reflectFee,
        daiFee: rewardFee,
  ...
  function _getFee(address from, address to, uint256 amount) internal
    returns(uint256 receiveAmount, uint256 toalFee, uint256 reflectFee, uint256 rewardFee)
  {
    uint256[8] memory f = IThriftyRole(roleManager).getFee(from, to, amount);
    feeLogs[feeIndex] = f;
    feeIndex += 1;
    return (f[0], amount.sub(f[0]), f[3], f[5]);
  }

  function _processFee() internal {
    uint256 totalFee = 0;
    uint256 totalMarketing = 0;
    uint256 totalDai = 0;
    uint256 totalPrint = 0;
    uint256 tmpDecimal = 1000000000;
    uint256 len = feeIndex - processedFeeIndex;
    for(uint256 x = processedFeeIndex; x < feeIndex; x++) {
      totalFee += feeLogs[x][7];
      totalDai += feeLogs[x][7] > 0 ? (feeLogs[x][4] + feeLogs[x][5]) * tmpDecimal / feeLogs[x][7] : 0;
      totalMarketing += feeLogs[x][7] > 0 ? feeLogs[x][1] * tmpDecimal / feeLogs[x][7] : 0;
      totalPrint += (feeLogs[x][4] + feeLogs[x][5]) > 0 ? tmpDecimal * feeLogs[x][4] / (feeLogs[x][4] + feeLogs[x][5]) : 0;
    }
    processedFeeIndex = feeIndex;
    totalDai = len > 0 ? totalDai / len : 0;
    totalMarketing = len > 0 ? totalMarketing / len : 0;
    totalPrint = len > 0 ? totalPrint / len : 0;

    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = peggedToken;
    uint256[] memory amounts = IUniswapV2Router(uniRouter).getAmountsOut(feeDebt, path);
    if (totalFee > 0 && amounts[1] > 0) {
      _approveTokenIfNeeded(address(this), uniRouter, feeDebt);
      amounts = IUniswapV2Router(uniRouter).swapExactTokensForTokens(feeDebt, 0, path, tokenBuffer, block.timestamp);
      ITokenBuffer(tokenBuffer).moveBalance(path[1], amounts[1]);

      uint256 marketingF = amounts[1].mul(totalMarketing).div(tmpDecimal);
      uint256 daiAmount = amounts[1].mul(totalDai).div(tmpDecimal);
      uint256 wethamount = amounts[1].sub(daiAmount);

      path[0] = peggedToken;
      path[1] = ITFUSD(tFUSD).peggedToken();
      uint256[] memory daiAmounts = IUniswapV2Router(uniRouter).getAmountsOut(daiAmount, path);
      if (daiAmount > 0 && daiAmounts[1] > 0) {
        ...
        uint256 addLpAmount = amounts[1].mul(totalPrint).div(tmpDecimal);
        totalSwappedReward += amounts[1].sub(addLpAmount);
        if (addLpAmount > 0) {
...
interface IThriftyRole {
  ...
  function vipAddress(address wallet) external returns(bool);
  function noTradeLimitAddress(address wallet) external returns(bool);
  ...
  function validateTransfer(address from, address to, uint256 userBalance, uint256 amount) external returns(bool);
  function getFee(address from, address to, uint256 amount) external returns(uint256[8] memory fees);
  ...
  function walletMaxLimit() external returns(uint256);
  function walletMaxTx() external returns(uint256);
...
interface ITFUSD {
  ...
  function peg(bool autoLp, bool daiFromBuffer) external;
