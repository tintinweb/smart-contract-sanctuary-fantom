...
contract StrategyLinSpiritFarm is StrategyGeneralMasterChefBase, Ownable {
    using SafeERC20 for IERC20;

    // Token addresses for SpiritSwap
    ...
    address public constant beethovenxRouter =
    ...
    address public linSpiritChef = 0x1CC765cD7baDf46A215bD142846595594AD4ffe3;
    ...
    function getHarvestable() external view override returns (uint256) {
        uint256 _pendingReward = ISpiritMasterChef(masterchef).pendingSpirit(
            poolId,
            address(this)
        );
    ...
    function harvestAndDepositLinSpirit() internal override {
        ...
        uint256 linSpiritBalance = IERC20(linSpirit).balanceOf(address(this));

        // stake in masterchef
        _approveToken(linSpiritChef, linSpirit, linSpiritBalance);
        ILinSpiritChef(linSpiritChef).deposit(
            0,
            linSpiritBalance,
            address(this)
        );
        ...
        // update vault epoch
        IVault(depositor).updateEpoch(linSpiritBalance, strategyVaultPid);
    }

    function claim(uint256 pendingLinSpirit) external onlyBenevolent {
        ILinSpiritChef(linSpiritChef).withdraw(
            0,
            pendingLinSpirit,
            address(this)
        );
        IERC20(linSpirit).transfer(msg.sender, pendingLinSpirit);
    }
    ...
    function _swapLinSpiritSpirit(
        ...
        _approveToken(beethovenxRouter, assetIn, _amount);
        swapAmount = IBalancer(beethovenxRouter).swap(
            singleSwap,
            funds,
            limit,
            deadline
    ...
    function _approveToken(
        ...
        require(
            _token != address(0) && _spender != address(0),
            "_approveToken address(0)"
        );
        uint256 allowances = IERC20(_token).allowance(address(this), _spender);
    ...
    function getBalance() public view returns (uint256) {
        ...
        (uint256 amount, ) = IMasterChef(linSpiritChef).userInfo(
            0,
            address(this)
        );
        return amount;
...
abstract contract StrategyGeneralMasterChefBase is StrategyBase {
    ...
    /*
        Harvest masterchef SPIRIT rewards
        Calls  harvestAndDepositLinSpirit
    */
    function harvest() public override onlyBenevolent {
        IMasterChef(masterchef).withdraw(poolId, 0);
        harvestAndDepositLinSpirit();
    }

    function harvestAndDepositLinSpirit() internal virtual {}
}
...
abstract contract StrategySpiritBase is OwnableUpgradeable {
    ...
    // Controller only function for creating additional rewards from dust
    function withdraw(IERC20 _asset) external onlyOwner returns (uint256 balance) {
        require(want != address(_asset), "want");
        balance = _asset.balanceOf(address(this));
        _asset.safeTransfer(depositor, balance);
    }
...
contract StrategyBeetsFarm is StrategyGeneralMasterChefBase, Ownable {
    // Token addresses
    ...
    address public fBeets = 0xfcef8a994209d6916EB2C86cDD2AFD60Aa6F54b1;
    address public beethovenxVault = 0x20dd72Ed959b6147912C2e529F0a0C651c33c9ce;
    ...
    function getHarvestable() external view override returns (uint256) {
        uint256 _pendingReward = IBeetsMasterChef(masterchef).pendingBeets(
            poolId,
            address(this)
        );
        return _pendingReward;
...
abstract contract StrategyGeneralMasterChefBase is StrategyBase {
    ...
    {
        //IMasterChef(masterchef).withdraw(poolId, _amount);
        IBeetsMasterChef(masterchef).withdrawAndHarvest(
            poolId,
            _amount,
            address(this)
        );
    ...
    function stakeFBeets(uint256 _lpAmount) private returns (uint256 _want) {
        ...
        if (_want > 0) {
            ...
            IBeetsMasterChef(masterchef).deposit(
                pidFreshBeets,
                _want,
                address(this)
            );
...
contract StrategyLinSpirit is StrategySpiritBase {
    ...
    function harvest(address _harvester) public onlyBenevolent returns (uint256) {
        ...
        IERC20(linSpirit).safeTransfer(feeHandler, _feePerformance);
        IERC20(linSpirit).safeTransfer(_harvester, _feeHarvest);
    ...
    function claim(uint256 pendingLinSpirit) external onlyBenevolent {
        ...
        IERC20(linSpirit).safeTransfer(msg.sender, pendingLinSpirit);
    }
