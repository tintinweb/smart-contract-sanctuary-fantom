...
contract Zap is OwnableUpgradeable, TransferHelper {
    ...
    address private constant YFI = 0x29b0Da86e484E1C0029B56e817912d778aC0EC69;
    address private constant BIFI = 0xd6070ae98b8069de6B494332d1A1a81B6179D960;

    IPancakeRouter02 private constant ROUTER_V2 = IPancakeRouter02(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
    IPancakeFactory private constant FACTORY = IPancakeFactory(0x152eE697f2E276fA89E96742e9bB9aB1F2E61bE3);
    ...
    function getBUSDValue(address _token, uint _amount) public view returns (uint) {
        return _getBUSDValue(_token, _amount).mul(1e12); // usdc is 6 decimals, we return
    }
    ...
    function zapInTokenToBNB(address _from, uint amount) external {
        ...
        if (!isFlip(_from)) {
            ...
            if (token0 == WFTM || token1 == WFTM) {
                address other = token0 == WFTM ? token1 : token0;
    ...
    function _getBUSDValue(address _token, uint _amount) internal view returns (uint) {
        if (isFlip(_token)) {
            IPancakePair pair = IPancakePair(_token);
            address token0 = pair.token0();
            address token1 = pair.token1();
            if (token0 == USDC || token1 == USDC) {
                return IBEP20(USDC).balanceOf(_token).mul(_amount).mul(2).div(IBEP20(_token).totalSupply());
            } else if (token0 == WFTM || token1 == WFTM) {
                uint wftmAmount = IBEP20(WFTM).balanceOf(_token).mul(_amount).div(IBEP20(_token).totalSupply());
                address busdWbnbPair = FACTORY.getPair(USDC, WFTM);
                return IBEP20(USDC).balanceOf(busdWbnbPair).mul(wftmAmount).mul(2).div(IBEP20(WFTM).balanceOf(busdWbnbPair));
            } else {
            ...
            if (_token == WFTM) {
                address pair = FACTORY.getPair(USDC, WFTM);
                return IBEP20(USDC).balanceOf(pair).mul(_amount).div(IBEP20(WFTM).balanceOf(pair));
            } else if (routePairAddresses[_token] == address(0)) {
                address pair = FACTORY.getPair(_token, WFTM);
                require(pair != address(0), "No pair");
                uint wftmAmount = IBEP20(WFTM).balanceOf(pair).mul(_amount).div(IBEP20(_token).balanceOf(pair));
                address busdBnbPair = FACTORY.getPair(USDC, WFTM);
                return IBEP20(USDC).balanceOf(busdBnbPair).mul(wftmAmount).div(IBEP20(WFTM).balanceOf(busdBnbPair));
            } else if (routePairAddresses[_token] == USDC) {
                address pair = FACTORY.getPair(_token, USDC);
                require(pair != address(0), "No pair");
                return IBEP20(USDC).balanceOf(pair).mul(_amount).div(IBEP20(_token).balanceOf(pair));
            } else {
