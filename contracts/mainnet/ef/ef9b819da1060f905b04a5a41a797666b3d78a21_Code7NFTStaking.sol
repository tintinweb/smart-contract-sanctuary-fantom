...
contract Code7NFTStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    // gets x %
   function calcTax(uint256 _amount) internal pure returns (uint256){
        uint256 taxedAmount;
        taxedAmount = _amount;
        return taxedAmount;
    }
...
/* STAKING MECHANICS */
    ...
    function calculateRewards(address account, uint256[] memory tokenIds) public view returns (uint256[] memory rewards) {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        rewards[i] = calcTax(rate * (_deposits[account].contains(tokenId) ? 1 : 0) * (Math.min(block.number, expiration) - _depositBlocks[account][tokenId]));
      }
      return rewards;
    }
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused {
      ...
      if(reward > 0) {
        IERC20(erc20Address).safeTransfer(msg.sender,  calcTax(reward));
    }   
   }
    ...
    function resetBalances(uint256 startindex, uint256 endIndex) external onlyOwner(){
      ...
      for (uint256 i = startindex; i <= endIndex; i++){
        ...
        emissions[depositAdd[i]] += calcTax(calculateReward(depositAdd[i],depositTok[i]));
