...
contract Manifestation is IManifestation, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    address public creatorAddress;
    Manifester public manifester;
    address public dao;
    ...
    modifier onlySoul() {
        require(soulDAO == _msgSender(), "onlySoul: caller is not the soulDAO address");
        _;
    }
    ...
    // sets the creatorAddress address (at creation).
    constructor() { creatorAddress = msg.sender; }
    ...
    function manifest(
        ...
        dao = creatorAddress;
        manifester = Manifester(_manifester);
        rewardToken = IERC20(_rewardToken);
        depositToken = IERC20(_depositToken);
        duraDays = _duraDays;
        feeDays = toWei(_feeDays);
        dailyReward = _dailyReward;
        totalRewards = getTotalRewards(_duraDays, _dailyReward);
    ...
    /*/ GETTER FUNCTIONS /*/
    function getTotalRewards(uint _duraDays, uint _dailyReward) public pure returns (uint _totalRewards) {
        _totalRewards = _duraDays * toWei(_dailyReward);    
    }
    ...
    function getNativeSymbol() public view returns (string memory) {
        return manifester.nativeSymbol();
    }
    ...
    // updates: LogoURI (onlyOwner, whileSettable)
    function setLogoURI(string memory _logoURI) external onlyOwner whileSettable {
        logoURI = _logoURI;
    }
    ...
    function toggleLogoSettable(bool enabled) external onlySoul {
        isSettable = enabled;
    }

    // enable soulDAO address update (onlySoul)
    function updateDao(address _soulDAO) external {
...
contract Manifester is Ownable {
    using SafeERC20 for IERC20;
    ...
    constructor() {
        Factory = ISoulSwapFactory(0x1120e150dA9def6Fe930f4fEDeD18ef57c0CA7eF); // FTM: 0x1120e150dA9def6Fe930f4fEDeD18ef57c0CA7eF
        soulDAO = msg.sender;
        bloodSacrifice = toWei(1);
        WNATIVE = IERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83); // WFTM: 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83
        nativeSymbol = IERC20Metadata(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83).symbol();
    }

    function createFarm(address rewardToken, uint duraDays, uint feeDays, uint dailyReward) external whileActive returns (address farm) {
        address depositToken = Factory.getPair(address(WNATIVE), rewardToken);

        // [if] pair does not exist
        if (depositToken == address(0)) {
            // [then] create pair and store as the depositToken.
            createDepositToken(rewardToken);
            depositToken = Factory.getPair(address(WNATIVE), rewardToken);
        }
        ...
        require(ERC20(rewardToken).decimals() == 18, 'reward token must be 18 decimals'); // neccessary for rewards distribution calculation.
        ...
        require(getFarm[depositToken][msg.sender] == address(0), 'reward already exists'); // single check is sufficient
        ...
        if (!testMode) { IERC20(rewardToken).safeTransferFrom(msg.sender, soulDAO, sacrifice); }
        
        // transfers: `totalRewards` to the farm contract.
        if (!testMode) { IERC20(rewardToken).safeTransferFrom(msg.sender, farm, rewards); }

        // creates: new farm based off of the inputs, then stores as an array.
        Manifestation(farm).manifest(rewardToken, depositToken, address(this), duraDays, feeDays, dailyReward);
        
        // populates: the getFarm mapping (also in reverse direction).
        getFarm[depositToken][msg.sender] = farm;
        getFarm[msg.sender][depositToken] = farm; 
        ...
        emit SummonedManifestation(msg.sender, depositToken, rewardToken, farm, duraDays, feeDays, totalFarms);
    }
    ...
    function getInfo(uint id) public view returns (string memory name, string memory symbol) {
        address mAddress = address(allFarms[id]);
    ...
    function toggleTestMode(bool enabled) external onlyOwner {
        testMode = enabled;
