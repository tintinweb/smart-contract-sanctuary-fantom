...
contract Pool is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_feeReserve != address(0), "Pool::initialize: invalidAddress");
        xftm = IXftm(_xftm);
    ...
    function usableFtmBalance() public view returns (uint256) {
        ...
        return _ftmBalance > unclaimedFtm ? (_ftmBalance - unclaimedFantasm) : 0;
    }
    ...
    function collect() external nonReentrant {
        ...
        if (_sendFantasm) {
            safeTransferFantasm(_sender, _fantasmAmount);
        }
        ...
        )
    {
        ...
        if (collateralRatio == COLLATERAL_RATIO_MAX || (collateralRatio > 0 && _ftmIn > 0)) {
            _totalValue = (_ftmIn * COLLATERAL_RATIO_MAX) / collateralRatio;
            _minFtmIn = _ftmIn;
            ...
            _totalValue = (_fantasmValue * COLLATERAL_RATIO_MAX) / (COLLATERAL_RATIO_MAX - collateralRatio);
            _minFantasmIn = _fantasmIn;
            _minFtmIn = _totalValue - _fantasmValue;
        }
        _xftmOut = _totalValue - ((_totalValue * mintingFee) / PRECISION);
        ...
        )
    {
        ...
        if (collateralRatio < COLLATERAL_RATIO_MAX) {
            uint256 _fantasmValueOut = (_xftmIn - ((_xftmIn * collateralRatio) / PRECISION));
            uint256 _fantasmOutPostFee = (_fantasmValueOut * PRICE_PRECISION) / _fantasmPrice; // before fee
            _fantasmOut = _fantasmOutPostFee - ((_fantasmOutPostFee * redemptionFee) / PRECISION); // after fee
        }

        if (collateralRatio > 0) {
            uint256 _ftmOutPostFee = ((_xftmIn * collateralRatio) / PRECISION);
            _ftmFee = (_ftmOutPostFee * redemptionFee) / PRECISION; // redemption fee in FTM
            _ftmOut = _ftmOutPostFee - _ftmFee; // after fee
        }
    }

    /// @notice Transfer and/or Mint Fantasm to receiver based on the current reserve
    /// @param _receiver : the address of receiver
    /// @param _amount : the amount Fantasm to receive
    function safeTransferFantasm(address _receiver, uint256 _amount) internal {
        uint256 _fsmBalance = fantasm.balanceOf(address(this));
        uint256 _transferAmount;
        uint256 _mintAmount;
        if (_amount <= _fsmBalance) {
            _transferAmount = _amount;
        } else {
            _transferAmount = _fsmBalance;
            _mintAmount = _amount - _transferAmount;
        }

        if (_transferAmount > 0) {
            fantasm.safeTransfer(_receiver, _transferAmount);
        }

        if (_mintAmount > 0) {
            fantasm.mint(_receiver, _transferAmount);
        }
    }
    ...
    function swap(uint256 _ftmIn, uint256 _fantasmOut) internal {
