...
interface IFarm {
    function depositLpAndStake(address _pool, uint256 _amount) external;

    function unstakeLpAndWithdraw(address _pool, uint256 _amount) external;

    function claimStakingRewards() external;
}

/**
 * @title AutoCompound 0xDAO
 * @notice strategy for auto-compounding on pools using 0xDAO contracts
 * @author YieldWolf
 */
contract AC0xDAO is AutoCompoundStrategy {
    ...
    function _farmDeposit(uint256 amount) internal override {
        ...
        IFarm(masterChef).depositLpAndStake(address(stakeToken), amount);
        deposited += amount;
    }

    function _farmWithdraw(uint256 amount) internal override {
        IFarm(masterChef).unstakeLpAndWithdraw(address(stakeToken), amount);
        if (deposited > amount) {
            deposited -= amount;
        } else {
            deposited = 0;
    ...
    function _farmEmergencyWithdraw() internal override {
        IFarm(masterChef).unstakeLpAndWithdraw(address(stakeToken), _totalStaked());
        deposited = 0;
    }
    ...
    function _farmHarvest() internal override {
        IFarm(masterChef).claimStakingRewards();
    }
    ...
    function earn(address _bountyHunter) external override onlyOwner returns (uint256 bountyReward) {
        ...
        if (token0Amt > 0 && token1Amt > 0) {
            ...
            ISolidlyRouter(address(liquidityRouter)).addLiquidity(
                address(token0),
                address(token1),
                stable,
                token0Amt,
    ...
    function _distributeFees(uint256 _amount, address _bountyHunter) internal override returns (uint256 bountyReward) {
    ...
    function earnToNative(IERC20 _token, uint256 _amount) internal {
        ...
        ISolidlyRouter(address(liquidityRouter)).swapExactTokensForTokensSimple(
            _amount,
            0,
            address(_token),
            WNATIVE,
    ...
    function tokenToEarn(address _token) public override nonReentrant whenNotPaused {
        require(_token != WNATIVE && _token != address(solid), 'tokenToEarn: NOT_ALLOWED');
