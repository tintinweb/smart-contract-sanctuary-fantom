...
import {LibSoloToken} from "@solarprotocol/solar-diamond/contracts/modules/solo-token/LibSoloToken.sol";
import {LibDiamond} from "contracts-starter/contracts/libraries/LibDiamond.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

// solhint-disable-next-line contract-name-camelcase
contract setupTokenTaxes_0005 {
    function migrate(
        address treasury,
        address growthFund,
        ...
        address pair = LibUniswap.getFactory().getPair(
            address(this),
            stableCoin
        );

        address router = address(LibUniswap.getRouter());

        IERC20(stableCoin).approve(router, type(uint256).max);

        LibSoloToken.approve(address(this), router, type(uint256).max);

        LibTokenTaxes.addTax(
            ITokenTaxes.Tax({
                name: "TRADE_TAX",
                tokenDitributorStrategyId: keccak256("TRADE_TAX"),
                buy: 1700,
                sell: 1700
            })
        );
        LibTokenTaxes.addAddressTax(pair, keccak256("TRADE_TAX"));

        LibTokenTaxes.addTax(
            ITokenTaxes.Tax({
                name: "LAUNCH_TAX",
                tokenDitributorStrategyId: keccak256("LAUNCH_TAX"),
                buy: 0,
                sell: 3000
            })
        );
        LibTokenTaxes.addAddressTax(pair, keccak256("LAUNCH_TAX"));

        LibTokenTaxes.setDistributionPeriod(10 minutes);
        LibTokenTaxes.setDistributeOnSell(true);

        LibTokenTaxes.setExemptFromTaxes(address(this), true);
        LibTokenTaxes.setExemptFromTaxes(LibDiamond.contractOwner(), true);
        LibTokenTaxes.setExemptFromTaxes(treasury, true);
        LibTokenTaxes.setExemptFromTaxes(growthFund, true);
        LibTokenTaxes.setExemptFromTaxes(stakingRewardReceiver, true);
        LibTokenTaxes.setExemptFromTaxes(presaleContract, true);
        LibTokenTaxes.setExemptFromTaxes(intermediateWallet, true);
    }
}