...
import "./FTMBondDepository.sol";

contract BPTBondDepository is FTMBondDepository {
    constructor(
        address _OHM,
        address _principle,
        address _treasury,
        address _DAO,
        address _feed
    ) FTMBondDepository(_OHM, _principle, _treasury, _DAO, _feed){}
...
contract FTMBondDepository is Ownable {
  ...
  address public immutable IMP; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints IMP when receives principle
  ...
  struct Bond {
    uint256 payout; // IMP remaining to be paid
  ...
  function deposit(
    ...
    uint256 value = _amount.mul(10 ** IERC20(IMP).decimals()).div(10 ** IERC20(principle).decimals());
    uint256 payout = payoutFor(value); // payout to bonder is computed
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 IMP ( underflow protection )
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      IERC20(IMP).transfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        IERC20(IMP).approve(stakingHelper, _amount);
        IStakingHelper(stakingHelper).stake(_amount, _recipient);
      } else {
        IERC20(IMP).approve(staking, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(IMP).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  function assetPrice() public view returns (int256) {
    (, int256 price, , , ) = priceFeed.latestRoundData(); // RETRIEVE 8 DECIMALS PRICE
    return price;
  }
  ...
  function bondPriceInUSD() public view returns (uint256 price_) {
    ...
    price_ = bondPrice().mul(uint256(assetPrice())).mul(1e6); // 16 DECIMALS

  
  }

  /**
   *  @notice calculate current ratio of debt to IMP supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(IMP).totalSupply();
  ...
  /**
   *  @notice calculate amount of IMP available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or IMP) to the DAO
   *  @return bool
   */
