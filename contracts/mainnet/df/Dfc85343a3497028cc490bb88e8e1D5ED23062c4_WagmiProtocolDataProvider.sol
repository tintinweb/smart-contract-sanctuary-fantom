...
contract WagmiProtocolDataProvider {
  ...
  function getAllWTokens() external view returns (TokenData[] memory) {
    ILendingPool pool = ILendingPool(ADDRESSES_PROVIDER.getLendingPool());
    address[] memory reserves = pool.getReservesList();
    TokenData[] memory wTokens = new TokenData[](reserves.length);
    for (uint256 i = 0; i < reserves.length; i++) {
      DataTypes.ReserveData memory reserveData = pool.getReserveData(reserves[i]);
      wTokens[i] = TokenData({
        symbol: IERC20Metadata(reserveData.wTokenAddress).symbol(),
        tokenAddress: reserveData.wTokenAddress
      });
    }
    return wTokens;
  }
    ...
    )
  {
    ...
    return (
      IERC20(asset).balanceOf(reserve.wTokenAddress),
    ...
    )
  {
    ...
    currentWTokenBalance = IERC20(reserve.wTokenAddress).balanceOf(user);
    currentVariableDebt = IERC20(reserve.variableDebtTokenAddress).balanceOf(user);
    currentStableDebt = IERC20(reserve.stableDebtTokenAddress).balanceOf(user);
  ...
  function getReserveTokensAddresses(
    address asset
  ) external view returns (address wTokenAddress, address stableDebtTokenAddress, address variableDebtTokenAddress) {
    DataTypes.ReserveData memory reserve = ILendingPool(ADDRESSES_PROVIDER.getLendingPool()).getReserveData(asset);

    return (reserve.wTokenAddress, reserve.stableDebtTokenAddress, reserve.variableDebtTokenAddress);
  }
}
