...
contract FantoonNFT is ERC721Enumerable, Ownable{
    ...
    uint256 private initialReservePrice = 0.1 ether;
    ...
    address private addressOfDiamond;
    ...
    constructor(address _diamond) ERC721
        ("testkittens", "tbk") 
    {
        addressOfDiamond = _diamond;
...
//          currentPrice = previousPrice + addPriceIncrease;
        ...
        {
            uint amount = previousPrice * 1500 / 10000;
            sendDividendsforNFTHolders(amount);

            amount = previousPrice * 500 / 10000;
            sendDividendsforDiamondHolders(amount);                                                                      // Send dividends to diamond
        }
    ...
    function sendDividendsforDiamondHolders(uint256 _amount) public {
        Diamond(addressOfDiamond).sendDividends(_amount, true);
    }
    ...
    function withdrawableFTM(address _account) public view returns (uint256){
        ...
        uint256 withdrawableAmount = Diamond(addressOfDiamond).withdrawableDividendOf(_account, true);
    ...
    function withdrawnFTM(address _account) public view returns (uint256){
        ...
        uint256 withdrawnAmount = Diamond(addressOfDiamond).withdrawnDividendOf(_account, true);

        return _claimedFTM[_account] + withdrawnAmount;
    }
    
    function claimFTM() external lock {
        require(_excludedHolders[msg.sender] == false, 'You are excluded.');

        uint256 withdrawableAmount = Diamond(addressOfDiamond).withdrawableDividendOf(msg.sender, true);
        ...
        _claimedFTM[msg.sender] += mapRewardsForAccount[msg.sender];  // don't add withdrawableAmount, it will be considered in diamond
        mapRewardsForAccount[msg.sender] = 0;

        Diamond(addressOfDiamond).claimFromFantoonNFT(msg.sender);
    }
