...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0xAB07049991AD63982F6Fb639b5CD47F764fe6F98) // HermesGenesisPool
    ];

    // core components
    address public hermes;
    address public pluto;
    address public ares;
    ...
    modifier checkOperator {
        require(
            IBasisAsset(hermes).operator() == address(this) &&
                IBasisAsset(pluto).operator() == address(this) &&
                IBasisAsset(ares).operator() == address(this) &&
    ...
    function getBurnableHermesLeft() public view returns (uint256 _burnableHermesLeft) {
        ...
        if (_hermesPrice <= hermesPriceOne) {
            ...
            uint256 _bondSupply = IERC20(pluto).totalSupply();
    ...
    function initialize(
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn HERMES and mint cBOND)
    ...
    function getHermesCirculatingSupply() public view returns (uint256) {
        ...
        for (uint8 entryId = 0; entryId < excludedFromTotalSupply.length; ++entryId) {
            balanceExcluded = balanceExcluded.add(hermesErc20.balanceOf(excludedFromTotalSupply[entryId]));
        }
    ...
    function buyBonds(uint256 _hermesAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 newBondSupply = IERC20(pluto).totalSupply().add(_bondAmount);
        ...
        IBasisAsset(pluto).mint(msg.sender, _bondAmount);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        IBasisAsset(pluto).burnFrom(msg.sender, _bondAmount);
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        ...
        IERC20(hermes).safeApprove(boardroom, 0);
        IERC20(hermes).safeApprove(boardroom, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToBoardroom(hermesSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochHermesPrice > hermesPriceCeiling) {
                // Expansion ($HERMES Price > 1 $FTM): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(pluto).totalSupply();
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForBoardroom = hermesSupply.mul(_percentage).div(1e18);
                } else {
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(pluto), "bond");
        require(address(_token) != address(ares), "ares");
