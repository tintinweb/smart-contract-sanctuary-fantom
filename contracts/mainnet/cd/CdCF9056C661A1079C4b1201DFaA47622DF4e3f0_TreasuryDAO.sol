 ...
 * ERR8: setSandManSwapRouter: new _router is the zero address
...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
    ...
    address public sandManSwapRouter;

    // default to two weeks
    uint256 public distributionTimeFrame = 3600 * 24; // 3600 * 24 * 14
    ...
    function setSandManSwapRouter(address _sandManSwapRouter) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_sandManSwapRouter != address(0), "ERR8");
        require(address(sandManSwapRouter) == address(0), "ERR9");

        sandManSwapRouter = _sandManSwapRouter;
        emit SetSandManSwapRouter(msg.sender, sandManSwapRouter);
    }
    ...
    function autoLiquidity(address lpAddress) external nonReentrant onlyRole(OPERATOR_ROLE) {
        ...
        if (swapAmount > 0) {
            address WETH = IUniswapV2Router02(sandManSwapRouter).WETH();
            ...
            require(IERC20(swapFrom).approve(sandManSwapRouter, swapAmount), "ERR1");

            IUniswapV2Router02(sandManSwapRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                swapAmount,
        ...
        // (res0, res1, ) = IUniswapV2Pair(lpAddress).getReserves();
        // token1Needed = (res1 * token0Balance) / res0;
        // if (token1Balance < token1Needed) {
        //     token0Balance = (res0 * token1Balance) / res1;
        // } else if (token1Balance > token1Needed) {
        //     token1Balance = token1Needed;
        // }

        require(IERC20(token0).approve(sandManSwapRouter, token0Balance), "ERR1");
        require(IERC20(token1).approve(sandManSwapRouter, token1Balance), "ERR1");

        IUniswapV2Router02(sandManSwapRouter).addLiquidity(
            token0,
            token1,
            token0Balance,
            token1Balance,
            0,
            0,
            address(this), // FIXME: BURN_ADDRESS
            block.timestamp
        );
    }
