...
import "./ILotteryV2.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";
import "@openzeppelin/contracts/utils/Create2.sol";

/**
 * @title LotteriesFactory
 * @notice A contract for creating and managing lottery contracts.
 * @dev This contract uses Create2 to deploy new lottery contracts with unique addresses based on the same bytecode.
 * @author BlueBlur
 */
contract LotteriesFactory is AccessControl {
    bytes32 public constant FACTORY_MANAGER = keccak256("FACTORY_MANAGER"); 

    ILotteryV2[] public contracts;
    ...
    /**
    * @notice Creates a new lottery contract with a unique address based on the given bytecode.
    * @param _bytecode The bytecode of the new lottery contract.
    * @param _salt A unique salt used to derive the address of the new contract.
    * @param _idGenerator The address of the contract used to generate unique lottery IDs.
    * @param _comissionsHub The address of the contract used to distribute commissions to the platform owners.
    ...
    * @param _keyHash The key hash used to verify the authenticity of the VRF request.
    */
    function createLotteryContract(
                bytes calldata _bytecode,
                bytes32 _salt,
                address _idGenerator,
                address _comissionsHub,
                address _vrfCoordinator,
                uint64 subscriptionId,
                bytes32 _keyHash) public onlyRole(FACTORY_MANAGER) {
        require(_vrfCoordinator != address(0) && subscriptionId != 0, "incorrect params");
        bytes memory bytecode = abi.encodePacked(
            _bytecode, abi.encode(_idGenerator, _comissionsHub, _vrfCoordinator, subscriptionId, _keyHash, msg.sender)
        );

        ILotteryV2 lotteryContract = ILotteryV2(Create2.deploy(0, _salt, bytecode));
        contracts.push(lotteryContract);
    }

    /**
    * @notice Adds an existing lottery contract to the contracts array.
    * @param _contractAddress The address of the lottery contract to be added.
    */
    function addLotteryContract(
                address _contractAddress
    ) public onlyRole(FACTORY_MANAGER) {
        require(_contractAddress != address(0), "zero address");
        for(uint256 i = 0; i < contracts.length; i++){
            require(_contractAddress != address(contracts[i]), "address already in array");
        }
        contracts.push(ILotteryV2(_contractAddress));
    }

    /**
    * @notice Deletes an existing lottery contract from the contracts array.
    * @param _contractAddress The address of the lottery contract to be deleted.
    */
    function deleteLotteryContract(
                address _contractAddress
    ) public onlyRole(FACTORY_MANAGER) {
        require(_contractAddress != address(0), "zero address");
        uint256 elIndex;
        for(uint256 i = 0; i < contracts.length; i++){
            if (_contractAddress == address(contracts[i])){
                elIndex = i;
                break;
            }      
        }
        if (elIndex != 0){
            for(uint256 i = elIndex; i < contracts.length-1; i++){
    ...
    /**
     * @notice Retrieves the array of lottery contracts created by the factory.
     * @return An array of ILotteryV2 contracts created by this factory.
     */
    function getContracts() public view returns(ILotteryV2[] memory){
        return contracts;
    }
}
