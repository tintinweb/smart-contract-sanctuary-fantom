...
contract PriceGetterLp is Ownable, IPriceGetter {

    using SafeMath for uint256;
    address private NATIVE;
    address private ROUTER;
    uint private MULT = 100000;
    ...
    mapping(address => address) private lpNative;
    ...
    function _priceNativeInLp(address _in,uint256 _amount) public view returns (uint256)
    {
        IUniswapV2Pair pair = IUniswapV2Pair(_in);

        require(pair.token0() == lpNative[_in] || pair.token1() == lpNative[_in],"Invalid Pair");
        ...
        if(pair.token0() == lpNative[_in])
            reserve = reserve0;
        else
            reserve = reserve1;

        uint256 priceInLp = ((_amount * totalSupply) * MULT) / 2 / reserve;

        return priceInLp / MULT;
    }

    function getPrice2(address _in,address _lp) external view tokensWhitelist(_in, _lp) returns (uint256) {

        address[] memory path = _getPath(_in,lpNative[_lp]);
    ...
    function getPrice(address _in,address _lp) external view tokensWhitelist(_in, _lp) returns (uint256) {
       ...
       uint256 nativeInLps =  _priceNativeInLp(_lp,amounts[amounts.length - 1]);
    ...
    function setLpNative(address _lp, address _lpNative) external onlyOwner{
        lpNative[_lp] = _lpNative;
    }
