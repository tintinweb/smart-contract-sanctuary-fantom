...
interface ITreasury is IEpoch {
    function getCyberPrice() external view returns (uint256);

    function getCyberUpdatedPrice() external view returns (uint256);
    ...
    function getCyberLockedBalance() external view returns (uint256);

    function getCyberCirculatingSupply() external view returns (uint256);
    ...
    function previousEpochCyberPrice() external view returns (uint256);
...
contract Treasury is ITreasury, ContractGuard, ReentrancyGuard {
    ...
    // core components
    address public cyber;
    address public ghost;

    address public override boardroom;
    address public cyberOracle;

    // price
    uint256 public cyberPriceOne;
    uint256 public cyberPriceCeiling;
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of cyber price
    uint256 public bootstrapEpochs;
    uint256 public bootstrapSupplyExpansionPercent;

    uint256 public override previousEpochCyberPrice;
    uint256 public allocateSeigniorageSalary;
    uint256 public maxDiscountRate; // when purchasing bond
    uint256 public maxPremiumRate; // when redeeming bond
    uint256 public discountPercent;
    uint256 public premiumPercent;
    uint256 public mintingFactorForPayingDebt; // print extra cyber during debt phase
    ...
    address[] public cyberLockedAccounts;
    ...
    modifier checkEpoch() {
        ...
        epoch_ = epoch_.add(1);
        epochSupplyContractionLeft = (getCyberPrice() > cyberPriceCeiling)
            ? 0
            : IERC20(cyber).totalSupply().mul(maxSupplyContractionPercent).div(
                10000
            );
    }
    ...
    modifier checkOperator() {
        require(
            IBasisAsset(cyber).operator() == address(this) &&
    ...
    function getPegPrice() external view override returns (int256) {
        return IOracle(cyberOracle).getPegPrice();
    }

    function getPegPriceUpdated() external view override returns (int256) {
        return IOracle(cyberOracle).getPegPriceUpdated();
    }

    // oracle
    function getCyberPrice() public view override returns (uint256 cyberPrice) {
        try IOracle(cyberOracle).consult(cyber, 1e18) returns (uint144 price) {
    ...
    function getCyberUpdatedPrice()
        public
        view
        override
        returns (uint256 _cyberPrice)
    {
        try IOracle(cyberOracle).twap(cyber, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("oracle failed");
        }
    ...
    function getBurnablecyberLeft()
        external
        view
        returns (uint256 _burnablecyberLeft)
    {
        uint256 _cyberPrice = getCyberPrice();
        if (_cyberPrice <= cyberPriceOne) {
            uint256 _bondMaxSupply = IERC20(cyber)
                .totalSupply()
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnablecyber = _maxMintableBond
                    .mul(getBondDiscountRate())
                    .div(1e18);
                _burnablecyberLeft = Math.min(
    ...
    {
        uint256 _cyberPrice = getCyberPrice();
        if (_cyberPrice > cyberPriceCeiling) {
            uint256 _totalcyber = IERC20(cyber).balanceOf(address(this));
            uint256 _rate = getBondPremiumRate();
            if (_rate > 0) {
                _redeemableBonds = _totalcyber.mul(1e18).div(_rate);
            }
        }
    }
        ...
        returns (uint256 _rate)
    {
        ...
        if (_cyberPrice <= cyberPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = cyberPriceOne.mul(1e18).div(_cyberPrice); // to burn 1 cyber
                uint256 _discountAmount = _bondAmount
                    .sub(cyberPriceOne)
                    .mul(discountPercent)
                    .div(10000);
                _rate = cyberPriceOne.add(_discountAmount);
    ...
    function getBondPremiumRate() public view override returns (uint256 _rate) {
        ...
        if (_cyberPrice > cyberPriceCeiling) {
            if (premiumPercent == 0) {
                ...
                uint256 _premiumAmount = _cyberPrice
                    .sub(cyberPriceOne)
                    .mul(premiumPercent)
                    .div(10000);
                _rate = cyberPriceOne.add(_premiumAmount);
                if (maxDiscountRate > 0 && _rate > maxDiscountRate) {
                    _rate = maxDiscountRate;
                }
            }
    ...
    function getCyberCirculatingSupply()
        public
        view
        override
        returns (uint256)
    {
        return IERC20(cyber).totalSupply().sub(getCyberLockedBalance());
    }

    function getCyberLockedBalance()
        public
        view
        override
        returns (uint256 _lockedBalance)
    {
        uint256 _length = cyberLockedAccounts.length;
        IERC20 _cyber = IERC20(cyber);
        for (uint256 i = 0; i < _length; i++) {
            _lockedBalance = _lockedBalance.add(
                _cyber.balanceOf(cyberLockedAccounts[i])
            );
        }
    }

    function getNextExpansionRate() public view override returns (uint256) {
        return getPegTokenExpansionRate(cyber);
    }

    function getNextExpansionAmount() external view override returns (uint256) {
        return getCyberCirculatingSupply().mul(getNextExpansionRate()).div(1e6);
    }
        ...
        returns (uint256 _rate)
    {
        ...
        if (_twap > cyberPriceCeiling) {
            uint256 _percentage = _twap.sub(cyberPriceOne); // 1% = 1e16
            uint256 _mse = (_pegToken == cyber)
    ...
    function initialize(
        address _cyber,
        address _ghost,
        address _cyberOracle,
        address _boardroom,
        uint256 _startTime
    ) public notInitialized {
        cyber = _cyber;
        ghost = _ghost;
        cyberOracle = _cyberOracle;
        ...
        cyberPriceOne = 10**18; // This is to allow a PEG of 1 cyber per VVS
        cyberPriceCeiling = cyberPriceOne.mul(1001).div(1000);

        maxSupplyExpansionPercent = 300; // Upto 3.0% supply for expansion

        bondDepletionFloorPercent = 10000; // 100% of Bond supply for depletion floor
        seigniorageExpansionFloorPercent = 3500; // At least 35% of expansion reserved for boardroom
        maxSupplyContractionPercent = 450; // Upto 4.5% supply for contraction (to burn cyber and mint ghost)
        ...
        seigniorageSaved = IERC20(cyber).balanceOf(address(this));

        nextSupplyTarget = 2000000000 ether; // 2B supply is the next target to reduce expansion rate
        allocateSeigniorageSalary = 1 ether; // 1 CYBER for allocateSeigniorage() calling
    ...
    function setCyberOracle(address _cyberOracle) external onlyOperator {
        cyberOracle = _cyberOracle;
    }

    function setCyberPriceCeiling(uint256 _cyberPriceCeiling)
        external
        onlyOperator
    {
        require(
            _cyberPriceCeiling >= cyberPriceOne &&
                _cyberPriceCeiling <= cyberPriceOne.mul(120).div(100),
            "out of range"
        ); // [$1.0, $1.2]
        cyberPriceCeiling = _cyberPriceCeiling;
    }
    ...
    function setNextSupplyTarget(uint256 _target) external onlyOperator {
        require(_target > IERC20(cyber).totalSupply(), "too small");
        nextSupplyTarget = _target;
    }

    function setCyberLockedAccounts(address[] memory _cyberLockedAccounts)
        external
        onlyOperator
    {
        delete cyberLockedAccounts;
        uint256 _length = _cyberLockedAccounts.length;
        for (uint256 i = 0; i < _length; i++) {
            cyberLockedAccounts.push(_cyberLockedAccounts[i]);
        }
    }
    ...
    function _updatecyberPrice() internal {
        try IOracle(cyberOracle).update() {} catch {}
    }
    ...
    function buyBonds(uint256 _cyberAmount, uint256 targetPrice)
        external
        override
        onlyOneBlock
        checkOperator
        nonReentrant
    {
        require(_cyberAmount > 0, "zero amount");

        uint256 cyberPrice = getCyberPrice();
        require(cyberPrice == targetPrice, "price moved");
        require(
            cyberPrice < cyberPriceOne, // price < $1
            "cyberPrice not eligible for bond purchase"
        );

        require(
            _cyberAmount <= epochSupplyContractionLeft,
        ...
        address _cyber = cyber;
        uint256 _bondAmount = _cyberAmount.mul(_rate).div(1e18);
        uint256 _cyberSupply = IERC20(cyber).totalSupply();
        uint256 newBondSupply = IERC20(ghost).totalSupply().add(_bondAmount);
        require(
            newBondSupply <= _cyberSupply.mul(maxDebtRatioPercent).div(10000),
            "over max debt ratio"
        );

        IBasisAsset(_cyber).burnFrom(msg.sender, _cyberAmount);
        ...
        emit BoughtBonds(msg.sender, _cyberAmount, _bondAmount);
    }
        ...
        nonReentrant
    {
        ...
        require(cyberPrice == targetPrice, "price moved");
        require(
            cyberPrice > cyberPriceCeiling, // price > $1.01
            "cyberPrice not eligible for bond purchase"
        );
        ...
        uint256 _cyberAmount = _bondAmount.mul(_rate).div(1e18);
        require(
            IERC20(cyber).balanceOf(address(this)) >= _cyberAmount,
            "treasury has no more budget"
        );

        seigniorageSaved = seigniorageSaved.sub(
            Math.min(seigniorageSaved, _cyberAmount)
        );
        allocateSeigniorageSalary = 1000 ether; // 1000 cyber salary for calling allocateSeigniorage()

        IBasisAsset(ghost).burnFrom(msg.sender, _bondAmount);
        IERC20(cyber).safeTransfer(msg.sender, _cyberAmount);

        _updatecyberPrice();
        if (regulationStats != address(0))
            IRegulationStats(regulationStats).addRedeemed(epoch_, _cyberAmount);

        emit RedeemedBonds(msg.sender, _cyberAmount, _bondAmount);
    }

    function _sendToBoardroom(uint256 _amount, uint256 _expanded) internal {
        address _cyber = cyber;
        IBasisAsset(_cyber).mint(address(this), _amount);

        uint256 _daoFundSharedAmount = 0;
        if (daoFundSharedPercent > 0) {
            _daoFundSharedAmount = _amount.mul(daoFundSharedPercent).div(10000);
            IERC20(_cyber).transfer(daoFund, _daoFundSharedAmount);
        }
        ...
        if (marketingFundSharedPercent > 0) {
            _marketingFundSharedAmount = _amount
                ...
                .div(10000);
            IERC20(_cyber).transfer(marketingFund, _marketingFundSharedAmount);
        }
        ...
        if (insuranceFundSharedPercent > 0) {
            _insuranceFundSharedAmount = _amount
                ...
                .div(10000);
            IERC20(_cyber).transfer(insuranceFund, _insuranceFundSharedAmount);
        }
        ...
        IERC20(_cyber).safeIncreaseAllowance(boardroom, _amount);
        IBoardroom(boardroom).allocateSeigniorage(_cyber, _amount);

        if (regulationStats != address(0))
            IRegulationStats(regulationStats).addEpochInfo(
                epoch_.add(1),
                previousEpochCyberPrice,
                _expanded,
                _amount,
    ...
    {
        _updatecyberPrice();
        previousEpochCyberPrice = getCyberPrice();
        address _cyber = cyber;
        uint256 _supply = getCyberCirculatingSupply();
        ...
        if (epoch_ < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            if (epoch_ == 0) _supply = IERC20(_cyber).totalSupply();
            _seigniorage = _supply.mul(bootstrapSupplyExpansionPercent).div(
                10000
            );
            _sendToBoardroom(_seigniorage, _seigniorage);
        } else {
            if (previousEpochCyberPrice > cyberPriceCeiling) {
                // Expansion ($cyber Price > 1 $ETH): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(ghost).totalSupply();
                uint256 _percentage = previousEpochCyberPrice.sub(
                    cyberPriceOne
                );
                ...
                if (_savedForBond > 0) {
                    ...
                    IBasisAsset(_cyber).mint(address(this), _savedForBond);
                    emit TreasuryFunded(block.timestamp, _savedForBond);
                }
            } else if (previousEpochCyberPrice < cyberPriceOne) {
                if (regulationStats != address(0))
                    IRegulationStats(regulationStats).addEpochInfo(
                        epoch_.add(1),
                        previousEpochCyberPrice,
        ...
        internal
    {
        ...
        if (_pegTokenTwap > cyberPriceCeiling) {
            uint256 _percentage = _pegTokenTwap.sub(cyberPriceOne); // 1% = 1e16
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(cyber), "cyber");
