...
contract PerpetualEscrowTokenReceiver is Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	uint256 constant DEFAULT_BURNING_RATE = 875e15; // 87,5%
	uint256 constant DEFAULT_REWARD_CAP = 50e18; // 50 cLQDR
	...
	uint256 public rewardCap = DEFAULT_REWARD_CAP;
	...
	{
		require(_newExcessRecipient != address(0), "invalid address");
		address _oldExcessRecipient = excessRecipient;
		excessRecipient = _newExcessRecipient;
		emit UpdateExcessRecipient(_oldExcessRecipient, _newExcessRecipient);
	}
	...
	function setRewardCap(uint256 _newRewardCap) external onlyOwner delayed
	{
		uint256 _oldRewardCap = rewardCap;
		rewardCap = _newRewardCap;
		emit UpdateRewardCap(_oldRewardCap, _newRewardCap);
	}
	...
	{
		uint256 _count = tokens.length;
		for (uint256 _i = 1; _i <= _count; _i++) {
			address _token = tokens[_count - _i];
			SwapInfo storage _swapInfo = swapInfo[_token];
			uint256 _balance = IERC20(_token).balanceOf(address(this));
		...
		}
		{
			...
			if (_balance > 0) {
				...
				if (_amount > rewardCap) {
					uint256 _excess = _amount - rewardCap;
					IERC20(escrowToken).safeTransfer(excessRecipient, _excess);
					_amount = rewardCap;
				}
	...
	event UpdateExcessRecipient(address _oldExcessRecipient, address _newExcessRecipient);
	...
	event UpdateRewardCap(uint256 _oldRewardCap, uint256 _newRewardCap);
}