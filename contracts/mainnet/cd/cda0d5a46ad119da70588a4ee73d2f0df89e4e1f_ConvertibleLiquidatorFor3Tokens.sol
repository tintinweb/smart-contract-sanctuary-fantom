...
import "../USDV.sol";
import "../interfaces/IStableSwapInterface.sol";
import "../interfaces/IWrappedNative.sol";
import "./LiquidatorEnhanced.sol";
import "../libraries/WrappedAsset.sol";

/**
 * @title ConvertibleLiquidatorFor3Tokens
 * @dev Designed for liquidations and simplified conversion of the collateral asset back into USDV.
 *
 */
contract ConvertibleLiquidatorFor3Tokens is Initializable, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using WrappedAsset for IWrappedNative;

    bytes32 public constant WHITELISTED_LIQUIDATOR = keccak256("WHITELISTED_LIQUIDATOR");

    IERC20Upgradeable internal usdv;

    IWrappedNative public wrappedNative;
    IStableSwapInterface public curveSwapPool;
    LiquidatorEnhanced public liquidatorEnhanced;

    address[3] internal poolTokens;
    mapping(address => int128) internal tokensIDXS;

    event ConvertedToUSDV(address indexed stablecoin, uint256 boughtAmount, uint256 usdvReceived);
    ...
    function initialize(
        address[3] memory _stablecoins, //stablecoins` indices must be in the same order as in curvPool
        uint256 _usdvIDX, //index of usdv
        address _wrappedNative,
        address _curveSwapPool,
        address payable _liquidatorEnhanced
    ) public initializer {
        __ReentrancyGuard_init();
        __AccessControl_init();

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(WHITELISTED_LIQUIDATOR, msg.sender);

        poolTokens = _stablecoins;
        curveSwapPool = IStableSwapInterface(_curveSwapPool);
        wrappedNative = IWrappedNative(_wrappedNative);

        liquidatorEnhanced = LiquidatorEnhanced(_liquidatorEnhanced);

        int128 counterIDX = 0;
        for (uint i = 0; i < _stablecoins.length; i++) {
            tokensIDXS[_stablecoins[i]] = counterIDX++;
            if(i == _usdvIDX){
                usdv = IERC20Upgradeable(_stablecoins[i]);
                IERC20Upgradeable(_stablecoins[_usdvIDX]).safeApprove(
                    address(_liquidatorEnhanced),
                    type(uint256).max
                );
            } else {
                IERC20Upgradeable(_stablecoins[i]).safeApprove(
    ...
    function liquidateVault(address _stable, uint256 _vaultId)
        public
        nonReentrant
        onlyRole(WHITELISTED_LIQUIDATOR)
    {
        liquidatorEnhanced.liquidateVault(_stable, _vaultId);
    }

    // Transfer native token held by this contrat to the sender/owner.
        ...
        onlyRole(WHITELISTED_LIQUIDATOR)
    {
        ...
        if(wrappedBalance > 0){
            wrappedNative.safeWithdraw(wrappedBalance);
        }
    ...
    function withdrawToken(IERC20Upgradeable token)
        external
        onlyRole(WHITELISTED_LIQUIDATOR)
    {
    ...
    (
        ...
        bytes calldata swapCallData // Must attach native token equal to the `value` field from the API response.
    ) 
    public
    payable 
    nonReentrant
    onlyRole(WHITELISTED_LIQUIDATOR) {
        require(existsInCurvePool(address(buyToken)), "buyToken must exist in curveSwapPool");
        require(buyToken != usdv, "buyToken cannot be converted to itself");

        if(address(sellToken) == address(wrappedNative)){
            uint256 depositValue = payable(address(this)).balance - msg.value;
            wrappedNative.safeDeposit(depositValue);
        }
        ...
        // passing along any native token attached to this function call to cover protocol fees.
        (bool success, ) = swapTarget.call{value: msg.value}(swapCallData);
        require(success, "SWAP_CALL_FAILED");
        // Disapprove to avoid potential vulnerability
        sellToken.safeApprove(spender, 0);

        // Use our current buyToken balance to determine how much we've bought.
        boughtAmount = buyToken.balanceOf(address(this)) - boughtAmount;

        uint256 usdvBalanceBefore = IERC20Upgradeable(usdv).balanceOf(address(this));
        curveSwapPool.exchange_underlying(
            tokensIDXS[address(buyToken)],
            tokensIDXS[address(usdv)], 
            boughtAmount,
            0,
            address(this)
        );
        uint256 boughtUSDV = IERC20Upgradeable(usdv).balanceOf(address(this)) - usdvBalanceBefore;
        emit ConvertedToUSDV(address(buyToken), boughtAmount, boughtUSDV);
    }

    function liquidateAndConvert(
        address stable,
        uint256 vaultId,
        IERC20Upgradeable sellToken,
        IERC20Upgradeable buyToken,
    ...
    function get_dy(
        ...
        require(
            existsInCurvePool(_sellToken) && existsInCurvePool(_buyToken),
            "use only tokens in exchange pool"
        );

        return
            curveSwapPool.get_dy_underlying(
                tokensIDXS[_sellToken],
                tokensIDXS[_buyToken],
                _amount
            );
    }

    function existsInCurvePool(address _token) internal view returns (bool) {
        for (uint i = 0; i < poolTokens.length; i++) {
            if (poolTokens[i] == _token) return true;
        }
        return false;
    }
...
/**
 * @title LiquidatorEnhanced
 * @dev Proxy-liquidator with timelock-operations support.
 * Provides access for liquidation through contracts StablecoinEnhanced and ERC20StablecoinEnhanced.
 * Contract inherits implementation of AccessControl.
 *
 * NOTE: Vaults can undergo partial liquidation multiple times
 * if they continue to become risky.
 */
contract LiquidatorEnhanced is ReentrancyGuard, AccessControl {
    using SafeERC20 for IERC20;

    bytes32 public constant WHITELISTED_LIQUIDATOR =
        keccak256("WHITELISTED_LIQUIDATOR");
    ...
    // stablecoin => info.
    mapping(address => LendingInfo) public lendingMap;
    ...
    event RemoveLiquidator(address indexed liquidator);
    /**
     * @dev Emitted when the minimum delay for future operations is modified.
     */
    event MinDelayChange(uint256 oldDuration, uint256 newDuration);
    event MaxDelayChange(uint256 oldDuration, uint256 newDuration);
    ...
    constructor(
        ...
        require(_minDelay > 0, "minDelay must be greater than 0");
        require(
            _maxDelay > _minDelay,
        ...
        emit MaxDelayChange(0, maxDelay);
    }
    ...
    function addLiquidator(
        ...
        require(
            !lendingMap[_liquidator].added,
            "liquidator has already been added"
        );
        require(_liquidator != address(0), "liquidator is not specified");

        if (_collateralType == CollateralType.NATIVE) {
            _collateral = address(0);
        }

        lendingMap[_liquidator] = LendingInfo({
            collateral: IERC20(_collateral),
            colType: _collateralType,
            added: true
        });

        emit AddLiquidator(_liquidator, _collateral, _collateralType);
    }

    function removeLiquidator(address _liquidator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        delete lendingMap[_liquidator];
        emit RemoveLiquidator(_liquidator);
    }

    function liquidateVault(address _liquidator, uint256 _vaultId)
        external
        nonReentrant
        onlyRole(WHITELISTED_LIQUIDATOR)
    {
        _liquidateVault(_liquidator, _vaultId);
    }

    function _liquidateVault(address _liquidator, uint256 _vaultId) internal {
        LendingInfo storage lendingInfo = lendingMap[_liquidator];
        require(lendingInfo.added, "liquidator doesn't exist");
        ILiquidator liquidator = ILiquidator(_liquidator);

        uint256 extract = liquidator.checkExtract(_vaultId);
        uint256 debtToPay = liquidator.checkCost(_vaultId);

        usdv.safeTransferFrom(msg.sender, address(this), debtToPay);
        liquidator.liquidateVault(_vaultId);
        liquidator.getPaid();

        if (lendingInfo.colType == CollateralType.NATIVE) {
            payable(msg.sender).transfer(extract);
        } else {
            lendingInfo.collateral.safeTransfer(msg.sender, extract);
        }

        // liquidationProfits[address(lendingInfo.collateral)][msg.sender] += extract;

        emit Liquidation(_liquidator, msg.sender, _vaultId, extract);
    }

    function scheduleLiquidation(address _liquidator, uint256 _vaultId)
        external
    {
        ILiquidator liquidator = ILiquidator(_liquidator);

        bool liquidationCheck = liquidator.checkLiquidation(_vaultId);
        require(
            liquidationCheck,
            "vault is not below minimum collateral percentage"
        );

        uint256 allowed = usdv.allowance(msg.sender, address(this));
        uint256 balance = usdv.balanceOf(msg.sender);
        uint256 debtToPay = liquidator.checkCost(_vaultId);
        require(balance >= debtToPay, "not enough balance for liquidation");
        require(allowed >= debtToPay, "not enough allowance for liquidation");

        bytes32 opID = hashOperation(_liquidator, msg.sender, _vaultId);
        _schedule(opID);
    ...
    /**
     * @dev Schedule an operation that is to becomes valid after a given min delay.
     */
    function _schedule(bytes32 id) internal {
        uint256 timestamp = getTimestamp(id);
        uint256 opMaxDelay = timestamp + maxDelay;

        bool opNotRegistered = timestamp == 0;
        bool opFinished = timestamp == _DONE_TIMESTAMP;
        bool opOverdue = opMaxDelay < block.timestamp;
        require(
            opNotRegistered || opFinished || opOverdue,
     ...
     */
    function executeLiquidation(address _liquidator, uint256 _vaultId)
        public
        nonReentrant
    {
        bytes32 id = hashOperation(_liquidator, msg.sender, _vaultId);
        require(
            isOperationReady(id),
            "the operation does not fit into the allowable time range"
        );
    ...
    /**
     * @dev Payable fallback to allow this contract to receive liquidation reward.
     */
    receive() external payable {}
     ...
     */
    function isOperationReady(bytes32 _id) public view returns (bool ready) {
        uint256 timestamp = getTimestamp(_id);
        uint256 opMaxDelay = timestamp + maxDelay;
        uint256 opMinDelay = timestamp + minDelay;

        return
            timestamp > _DONE_TIMESTAMP &&
            opMinDelay <= block.timestamp &&
            opMaxDelay >= block.timestamp;
    }

    /**
     * @dev Returns whether an operation is ready or not.
     */
    function getValidInterval(bytes32 _id)
        external
        view
        returns (uint256, uint256)
    {
        uint256 timestamp = getTimestamp(_id);
        require(timestamp > _DONE_TIMESTAMP, "operation is not scheduled yet");

        uint256 opMinDelay = timestamp + minDelay;
        uint256 opMaxDelay = timestamp + maxDelay;
        return (opMinDelay, opMaxDelay);
    }
     ...
     */
    function getTimestamp(bytes32 _id)
        public
        view
        virtual
        returns (uint256 timestamp)
    {
        return _timestamps[_id];
    }
    ...
    function hashOperation(
        ...
        return keccak256(abi.encode(_liquidator, _sender, _vaultId));
    }
     ...
     */
    function updateMinDelay(uint256 _newDelay)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(minDelay > 0, "minDelay must be greater than 0");
        emit MinDelayChange(minDelay, _newDelay);
        minDelay = _newDelay;
    }

    /**
     * @dev Changes the max timelock duration for future operations.
     *
     * Emits a {MaxDelayChange} event.
     */
    function updateMaxDelay(uint256 _newDelay)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(maxDelay > minDelay, "maxDelay must be greater than minDelay");
        emit MaxDelayChange(maxDelay, _newDelay);
        maxDelay = _newDelay;
    }
}
 ...
 */
contract Stablecoin is Initializable, ReentrancyGuardUpgradeable, VaultNFTv3, ILiquidator {
     ...
     * @param _minimumCollateralPercentage Collateral percentage 
     */
    function __Stablecoin_init(
    ...
    /**
     * @dev Returns the maximum amount of token to be minted.
    ...
    function getTVL() external view returns (uint256){
        return usdv.supplyMap(address(this)) * minimumCollateralPercentage / 100;
    }
    ...
    function destroyVault(uint256 vaultID) external onlyVaultOwner(vaultID) nonReentrant {
        ...
        if(collat != 0) {
            payable(msg.sender).transfer(collat);
        }
    ...
    function depositCollateral(uint256 vaultID) external payable onlyVaultOwner(vaultID) {
        vaultCollateral[vaultID] += msg.value;
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) {
        ...
        uint256 _closingFee = amount * paybackFee * getTokenPriceSource() / (getEthPriceSource() * 10000); 
     ...
     */
    function getPaid() public override nonReentrant {
     ...
     */
    function checkCost(uint256 _vaultId) public view override returns(uint256) {
        (, uint256 debtValue) = calculateCollateralProperties(vaultCollateral[_vaultId], vaultDebt[_vaultId] );

        debtValue = debtValue / tokenPeg;

        return debtValue * debtRatio / 10000;
    }
     ...
     * @return uint256 Collateral extraction
     */
    function checkExtract(uint256 _vaultId) public view override returns(uint256) {
        (, uint256 debtValue) = calculateCollateralProperties(vaultCollateral[_vaultId], vaultDebt[_vaultId] );

        uint256 halfDebt = debtValue * debtRatio / 10000;

        uint256 collateralExtract = halfDebt * gainRatio / (1000 * getEthPriceSource());
    ...
    function checkValid( uint256 _vaultId ) public view returns(bool, uint256, uint256, uint256) {
        ...
        (, uint256 ogDebtValue) = calculateCollateralProperties(vaultCollateral[_vaultId], vaultDebt[_vaultId] );

        uint256 halfDebt = ogDebtValue * debtRatio / 10000;

        uint256 extract = halfDebt * gainRatio / (1000 * getEthPriceSource());

        uint256 newCollateral = vaultCollateral[_vaultId] - extract;
    ...
    function checkCollat(uint256 _vaultId) public view returns(uint256, uint256) {
        return calculateCollateralProperties(vaultCollateral[_vaultId], vaultDebt[_vaultId] );
    }
    ...
    /**
     * @dev Partially liquidates the vault. Gives some profit to the caller which depends on 'gainRatio'. 
     * The liquidation percentage depends on 'debtRatio'.
     *
     * NOTE: Caller must give approval to trasfer his tokens of stablecoin
     *
     * Requirements:
     * 
     ...
     */
    function liquidateVault(uint256 _vaultId) public override nonReentrant {
        require(_exists(_vaultId), "Vault does not exist");
        require(stabilityPool == address(0) || msg.sender == stabilityPool, "liquidation is disabled for public");

        uint256 collateralPercentage = checkCollateralPercentage(_vaultId);
        ...
        vaultDebt[_vaultId] = vaultDebt[_vaultId] - halfDebt; // we paid back half of its debt.

        uint256 _closingFee = halfDebt * closingFee * getTokenPriceSource() / (getEthPriceSource() * 10000);

        vaultCollateral[_vaultId] = vaultCollateral[_vaultId] - _closingFee;
        vaultCollateral[treasury] = vaultCollateral[treasury] + _closingFee;

        // deduct the amount from the vault's collateral
        vaultCollateral[_vaultId] = vaultCollateral[_vaultId] - collateralExtract;
        totalCollateral = totalCollateral - collateralExtract - _closingFee;

        // let liquidator take the collateral
        nativeTokenDebt[msg.sender] = nativeTokenDebt[msg.sender] + collateralExtract;

        emit LiquidateVault(_vaultId, ownerOf(_vaultId), msg.sender, halfDebt, collateralExtract, _closingFee);
    }
     ...
     */
    function _correctDecimals(uint256 amount, uint8 decimalsA, uint8 decimalsB) private pure returns(uint256) {
