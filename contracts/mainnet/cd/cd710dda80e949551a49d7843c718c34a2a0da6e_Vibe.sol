...
contract Vibe is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the VIBE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("VibeZone", "VIBE") {
        require(_taxRate < 1000, "tax equal or bigger to 10%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        excludeAddress(address(this));
        // Mints 1 VIBE to contract creator for initial pool setup
    ...
    function _getVibePrice() internal view returns (uint256 _vibePrice) {
        try IOracle(vibeOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("VIBE: failed to fetch VIBE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _vibePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_vibePrice >= taxTiersTwaps[tierId]) {
    ...
    function setVibeOracle(address _vibeOracle) public onlyOperatorOrTaxOffice {
        require(_vibeOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints VIBE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of VIBE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentVibePrice = _getVibePrice();
            currentTaxRate = _updateTaxRate(currentVibePrice);
            if (currentVibePrice < burnThreshold) {
                burnTax = true;
            }
        }
