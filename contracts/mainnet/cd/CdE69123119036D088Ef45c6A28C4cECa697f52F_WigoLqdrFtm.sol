...
// File: contracts/WigoLqdrFtm.sol
...
contract WigoLqdrFtm is Ownable, Pausable {
    ...
    address[] public wrappedToLqdrPath;
    ...
    event SetWrappedToGeistPath(address[] indexed path);
    ...
    constructor(
        ...
        wrappedToLqdrPath = [wrapped, lqdr];
    ...
    function addLiquidity() internal {
        uint256 wigoHalf = IERC20(wrapped).balanceOf(address(this)) / 2;

        IUniSwapRouter(unirouter).swapExactTokensForTokens(wigoHalf, 0, wrappedToLqdrPath, address(this), block.timestamp);
    
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lqdr).balanceOf(address(this));

        IUniSwapRouter(unirouter).addLiquidity(wrapped, lqdr, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(lqdr).safeApprove(unirouter, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(lqdr).safeApprove(unirouter, type(uint).max);
    }
    ...
    function setWrappedToGeistPath(address[] calldata _path) external onlyAdmin {
        require (_path[0] == wrapped && _path[_path.length - 1] == lqdr, "Bad path || !auth");

        wrappedToLqdrPath = _path;

        emit SetWrappedToGeistPath(_path);
    }
