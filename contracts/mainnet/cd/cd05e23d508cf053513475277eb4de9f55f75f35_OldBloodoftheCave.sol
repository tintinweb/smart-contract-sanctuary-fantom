    ...
    ERC721Burnable
{
    ...
    IERC721Enumerable public immutable platinumPass;
    ...
    constructor(address[] memory _devList,  uint256[] memory _fees) ERC721("Old Blood of the Cave", "OLDBLOOD") {     
        ...
        platinumPass = IERC721Enumerable(0x46923d2bF75a42F4F88E79BCa51a8f7d533D9B7D);
    ...
    function whiteList(address[] memory _addressList, uint256[] memory _valueList) external onlyOwner {
        require(_addressList.length > 0,"Error: list is empty");
        require(_addressList.length == _valueList.length, "Error: invalid lists");

        for(uint256 i = 0;i < _addressList.length; i++) {
             require(_addressList[i] != address(0), "Address cannot be 0.");
             whiteListed[_addressList[i]] = _valueList[i];
        }
        
    }

    function whiteListCount(address wallet) external view returns(uint256) {
       return whiteListed[wallet];
    }

    function takeSnapshot() external onlyOwner {

        for(uint256 i = 0;i < platinumPass.totalSupply(); i++) {
             address ownerOfToken = platinumPass.ownerOf(i);
             if(ownerOfToken != address(0) || ownerOfToken != 0x000000000000000000000000000000000000dEaD) {
                whiteListed[ownerOfToken]++;
             }
        }
