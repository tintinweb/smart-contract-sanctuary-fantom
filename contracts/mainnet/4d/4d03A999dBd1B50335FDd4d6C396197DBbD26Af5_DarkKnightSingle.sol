/**
 *Submitted for verification at FtmScan.com on 2022-10-24
*/

// File: contracts/interfaces/ISingleChef.sol
...
// File: contracts/DarkKnightSingle.sol
...
contract DarkKnightSingle is Ownable, Pausable {
    ...
    address[] public wrappedToWantPath;
    ...
    constructor(
        ...
        wrappedToWantPath = [wrapped, want];
    ...
    function _deposit() internal whenNotPaused {
        ...
        ISingleChef(masterchef).deposit(wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ISingleChef(masterchef).withdraw(_amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        ISingleChef(masterchef).deposit(0);
    ...
    function chargeFees(address caller) internal {                   
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWrapped, 0, outputToWrappedPath, address(this), block.timestamp);                                                
    ...
    function addLiquidity() internal {
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedBal, 0, wrappedToWantPath, address(this), block.timestamp);
    }

    /** @dev Returns rewards unharvested */
    function rewardBalance() public view returns (uint256) {
        return ISingleChef(masterchef).pendingReward(address(this));
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        if (outputBal > 0) {
            uint256[] memory amountsOut = IUniSwapRouter(unirouter).getAmountsOut(outputBal, outputToWrappedPath);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = ISingleChef(masterchef).userInfo(address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        ISingleChef(masterchef).emergencyWithdraw();
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external onlyAdmin {
        require (_path[0] != output && _path[_path.length - 1] == want, "!auth");
