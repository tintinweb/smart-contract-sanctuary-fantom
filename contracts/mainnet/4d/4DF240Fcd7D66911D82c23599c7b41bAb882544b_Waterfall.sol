...
contract Waterfall is BEP20 {
    ...
    // Min amount to liquify. (default 1 Waterfalls)
    ...
    modifier blackList(address sender, address recipent) {
        if (maxHolding() > 0) {
            if (_includeToBlackList[sender] == true || _includeToBlackList[recipent] == true) {
                require(_includeToBlackList[sender] == false,"WATERFALL::blackList: You have been blacklisted as a bot (SENDER)");
                require(_includeToBlackList[recipent] == false,"WATERFALL::blackList: You have been blacklisted as a bot (RECIPENT)");
            }
        }
        _;
    }
