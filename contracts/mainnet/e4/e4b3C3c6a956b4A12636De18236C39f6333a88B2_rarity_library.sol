...
contract rarity_library {
    ...
    rarity_daycare public        _daycare = rarity_daycare(0x641DA1A3656DFd731786E52f1C04cF60b3A36Cd0);
    rarity_daycare_planet public        _daycare_planet = rarity_daycare_planet(0x447b2569f97729Db49a6b8F1Abc480De02587B20);
    ...
    function misc(uint _s) public view returns (rl._misc memory m) {
        uint daycare_days_paid = _daycare.daysPaid(_s);
        uint daycare_days_paid_planet = _daycare_planet.daysPaid(_s);
        m = rl._misc(
            daycare_days_paid,
            daycare_days_paid_planet
        );
    }
...
interface rarity_daycare_planet {
    function daysPaid(uint _summoner) external view returns (uint days_paid);
}
