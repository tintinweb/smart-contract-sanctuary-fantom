...
contract NativeFarm is Ownable, ReentrancyGuard {
    ...
    // Token address
    address public constant NATIVE = 0x2fa0C0BD92F8ef100D6F5C7d72b4457a5Ef5A95c; //DEVIL address
    ...
    // Approx 15/7/2021
    uint256 public constant startTime = 34793404;
    ...
    uint256[] public nativeHarvestVaultTimes;
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt, uint256 _vaultTime) public nonReentrant {
        updatePool(_pid);
        ...
        _harvestNativeTokens(msg.sender, _pid, _vaultTime);
    ...
    function _harvestNativeTokens(address _userAddress, uint256 _pid, uint256 _vaultTime) internal {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (pending > 0) {
            if (address(pool.want) == NATIVE) {
                ...
                IERC20(NATIVE).safeIncreaseAllowance(nativeHarvestVaults[_vaultTime], pending);
                Vault(nativeHarvestVaults[_vaultTime]).deposit(_userAddress, pending);
            }
        }
    }

    function withdrawAll(uint256 _pid, uint256 _vaultTime) external nonReentrant {
        withdraw(_pid, uint256(-1), _vaultTime);
    }
    ...
    function addNativeHarvestVault(uint256 _time, address _vaultAddress) external onlyOwner{
        nativeHarvestVaults[_time] = _vaultAddress;
    }
