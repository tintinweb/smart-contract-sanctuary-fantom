...
// File: contracts/facades/LandFacade.sol



pragma solidity ^0.8.15;

contract LandFacade is _LandStorage {
    using EnumerableSet for EnumerableSet.UintSet;
    ...
    function getMapDelegate(uint8[2][] calldata xy)
        external
        view
        returns (uint16[2][] memory acres)
    {
        LandStorage storage ls = landStorage();
        acres = new uint16[2][](xy.length);

        for (uint256 i = 0; i < xy.length; i++) {
            Acre memory acre = ls.map[xy[i][0]][xy[i][1]];
            acres[i][0] = acre.biome;
            acres[i][1] = acre.farmId == EMPTY_FARM
                ? acre.farmId
                : (ls.ownedFarms[acre.farmId] != ZERO_ADDRESS ||
                    ls.vacantFarms.contains(acre.farmId))
                ? acre.farmId
                : EMPTY_FARM;
        }
    }

    function getBiomesDelegate()
        external
        view
        returns (uint96[BIOMES_ATTR_NUMBER][BIOMES_NUMBER] memory biomes)
    {
        LandStorage storage ls = landStorage();
        return ls.biomes;
    }

    function getVacantFarmsDelegate()
        external
        view
        returns (uint256[] memory)
    {
        LandStorage storage ls = landStorage();
        return ls.vacantFarms.values();
    }

    function getFarmDelegate(uint16 farmId)
        external
        view
        returns (uint8[ACRE_ATTR_NUMBER][] memory acres, uint256 hq, address owner)
    {
        LandStorage storage ls = landStorage();
        Farm memory farm = ls.farms[farmId];
        return (farm.acres, farm.hq, ls.ownedFarms[farmId]);
    }

    function getMap(uint8[2][] calldata xy)
        external
        returns (uint16[2][] memory acres)
    {
        return
    ...
    function getFarm(uint16 farmId)
        external
        returns (uint8[ACRE_ATTR_NUMBER][] memory acres, uint256 hq, address owner)
    {
        return
            abi.decode(
                ...
                (uint8[3][], uint256, address)
            );
    }

    function getFarmOf(address owner) external view returns (uint256[] memory farmId) {
        uint256 balanceOf = _land.balanceOf(owner);
        farmId = new uint256[](balanceOf);
        for (uint256 i = 0; i < balanceOf; i++) {
            farmId[i] = _land.getFarmId(_land.tokenOfOwnerByIndex(owner, i));
        }
    }
}