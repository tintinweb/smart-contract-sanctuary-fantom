...
contract KAOracle is IKAOracle, ReentrancyGuard
{
  ...
  function _canUpdate (address account) internal view returns (bool)
  {
    ...
    return mgr.isDiscountedForDebts(account) && uint256(stake.amount) >= uint256(mgr.getConfig().minForDebts).mul(2) && block.timestamp > stake.unstakableTimestamp && block.timestamp.sub(stake.unstakableTimestamp) > 60 days;
  }
