...
contract SecondStage is ERC721, ERC721Enumerable, Pausable, ERC721Burnable, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        baseDelta = block.timestamp + _baseDelta;
        stageOneContract = IERC721(stageOneContractAddress);
    }
    ...
    function buyToken(uint8 tokenId) external payable nonReentrant whenNotPaused callerIsUser {
        console.log("Balance of User %s", stageOneContract.balanceOf(msg.sender));
        console.log("Timestmap is %s", block.timestamp);
        console.log("Whitelisted until: %s", getWhitelistPeriod(tokenId));
        require(stageOneContract.balanceOf(msg.sender) > 0, "Error: you dont own whitelist nft");
        if (whiteListedOnly) require(getWhitelistPeriod(tokenId) >= block.timestamp, "Error: token is not whitelisted");
    ...
    function getTokenPrice(uint8 tokenId) public view returns (uint256) {
        if (tokenId >= 0 && tokenId < MAX_SUPPLY) return prices[tokenId];
        else revert("Error: invalid tokenID");
    }
    ...
    function getWhitelistPeriod(uint8 tokenId) public view returns (uint256) {
        if (tokenId >= 0 && tokenId < MAX_SUPPLY) return baseDelta + extraWhitelistPeriod[tokenId];
        else revert("Error: invalid tokenId");
    }
