...
interface IStrategyCommonChefLP {
    ...
    function beforeDeposit(bytes calldata _data) external;
    ...
    function yelPerSecond() external view returns (uint);
}
...
contract equalizerUsdcWftmYeller is Ownable {
    ...
    address public zap;
    address wftmEqualLPPool = 0x3d6c56f6855b7Cc746fb80848755B0a9c3770122;
    ...
    function getPendingRewards() internal view returns(uint256, uint256){
        ...
        yelToNative[0] = outputToNativeToYelRoute[3];
        yelToNative[1] = outputToNativeToYelRoute[2];

        uint yelPerSec = strategy.yelPerSecond();
        uint lastHarvest = poolInfo[0].lastRewardTime;
        uint pendingYelNow = yelPerSec * (block.timestamp - lastHarvest);
        uint[] memory wholeRewardsNative = AggregatorV3Interface(unirouter).getAmountsOut(pendingYelNow, yelToNative);

        return (wholeRewardsNative[1], pendingYelNow);
    }
    ...
    function getYelApr(uint stakedAmount, uint lpTokenPriceUsdt, uint cleanApr, address _unirouter, uint _nativeUsdtPrice) private view returns (uint) {
        ...
        address[] memory yelToNative = new address[](2);
        yelToNative[0] = rewardsToNativeToYelRoute[3];
        yelToNative[1] = rewardsToNativeToYelRoute[2];
        ...
        nativeToYel[0] = rewardsToNativeToYelRoute[2];
        nativeToYel[1] = rewardsToNativeToYelRoute[3];
    ...
    function withdraw(uint256 _pid, uint256 _amount, bytes calldata _data) external {
        require(msg.sender == zap, 'Yeller: access only by zap');
