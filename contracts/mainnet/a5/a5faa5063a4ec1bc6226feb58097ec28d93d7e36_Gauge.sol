...
contract Gauge is ReentrancyGuard {
    ...
    function derivedBalance(address account) public view returns (uint256) {
        ...
        uint256 _adjusted = (((_totalSupply * inSPIRIT.balanceOf(account)) /
            inSPIRIT.totalSupply()) * 60) / 100;
...
contract GaugeProxy is ProtocolGovernance, ReentrancyGuard {
    ...
    // Add new token gauge
    function addGauge(address _tokenLP, address _protocolAddress)
        external
        returns (address)
    {
        require(
    ...
    function preDistribute() external nonReentrant hasDistribute {
        ...
        if (ve) {
            ...
            uint256 _inSpiritRewards = (_balance * _lockedSpirit) /
                _spiritSupply;
