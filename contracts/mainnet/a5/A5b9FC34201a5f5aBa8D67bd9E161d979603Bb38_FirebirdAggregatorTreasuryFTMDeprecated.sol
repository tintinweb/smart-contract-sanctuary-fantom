...
contract FirebirdAggregatorTreasuryFTMDeprecated is OwnableUpgradeable {
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */

    address public constant wrapNative = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83); // WFTM
    ...
    event ForwardToCollector(address indexed token, address collector, uint256 amount);
    event Log(string message);
    event LogBytes(bytes data);

    function multiForwardToCollector(address[] calldata _tokens) external onlyOwner {
        uint256 _length = _tokens.length;
        for (uint256 i = 0; i < _length; i++) {
            _forwardToCollector(_tokens[i]);
        }
    }
    ...
    function _forwardToCollector(address _token) internal {
        ...
        if (_amount > 0) {
            ...
            try IERC20(_token).transfer(_collector, _amount) returns (bool) {
