...
// File contracts/distribution/RoyalGenesisRewardPool.sol

pragma solidity 0.6.12;

// Note that this pool has no minter key of ROYAL (rewards).
// Instead, the governance will call ROYAL distributeReward method and send reward to this pool at the beginning.
contract RoyalGenesisRewardPool {
    using SafeMath for uint256;
    ...
    // MAINNET
    uint256 public tombPerSecond = 0.115740741 ether; // 20000 tomb / (1h * 60min * 60s * 2)
    uint256 public runningTime = 28 hours; // 1 days
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "RoyalGenesisPool: caller is not the operator"
        );
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(
                ...
                "RoyalGenesisPool: existing pool?"
            );
        }
    }
