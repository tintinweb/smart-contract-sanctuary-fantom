...
// File contracts/Frax/LambdaMSIGHelper.sol
...
contract LambdaMSIGHelper is Owned {
    ...
    // Balance tracking
    uint256 public lambda_lent;
    uint256 public shareToken_lent;
    ...
    function getTokenType(address token_address) public view returns (uint256) {
        // 0 = lambda, 1 = shareToken, 2 = Collateral
        if (token_address == address(LAMBDACONTROLLER.lambdaToken())) return 0;
        else if (token_address == address(LAMBDACONTROLLER.shareToken())) return 1;
    ...
    function showTokenBalances() public view returns (uint256[3] memory tkn_bals) {
        tkn_bals[0] = ERC20(LAMBDACONTROLLER.lambdaToken()).balanceOf(address(this)); // lambda
        tkn_bals[1] = ERC20(LAMBDACONTROLLER.shareToken()).balanceOf(address(this)); // shareToken
    ...
    function showAllocations() public view returns (uint256[10] memory allocations) {
        // All numbers given are in lambda unless otherwise stated

        // Get some token balances
        uint256[3] memory tkn_bals = showTokenBalances();

        // lambda
        allocations[0] = tkn_bals[0]; // Free lambda
        allocations[1] = lambda_lent; // Lent lambda
        allocations[2] = allocations[0] + allocations[1]; // Total lambda

        // shareToken
        allocations[3] = tkn_bals[1]; // Free shareToken
        allocations[4] = shareToken_lent; // Lent shareToken
        allocations[5] = allocations[3] + allocations[4]; // Total shareToken
    ...
    // Needed for the lambda contract to function 
    ...
    function dollarBalances() public view returns (uint256 lambda_val_e18, uint256 collat_val_e18) {
        // NOTE: The token tracker will track the actual lambda, shareToken, and Collat on the msig
        // Were it to be included here too, it would be a double count

        // Get the allocations
        uint256[10] memory allocations = showAllocations();

        // lambda portion is lambda * CR
        uint256 lambda_portion_with_cr = (allocations[0] * LAMBDACONTROLLER.global_collateral_ratio()) / PRICE_PRECISION;
        

        // Collateral portion
        uint256 collat_portion = allocations[6];

        // Total value, not including CR, ignoring shareToken
        lambda_val_e18 = allocations[0] + allocations[6];

        // Collat value, accounting for CR on the lambda portion
        collat_val_e18 = collat_portion + lambda_portion_with_cr;
    }
    ...
    function lend(address token_address, uint256 token_amount) external onlyByOwnGov {
        ...
        // Can be overridden
        if (token_type == 0){
            TransferHelper.safeTransfer(LAMBDACONTROLLER.lambdaToken(), msig_address, token_amount);
            lambda_lent += token_amount;
        }
        else if (token_type == 1) {
            TransferHelper.safeTransfer(LAMBDACONTROLLER.shareToken(), msig_address, token_amount);
            shareToken_lent += token_amount;
        }
        else {
    ...
    function burnLambda(uint256 lambda_amount) public onlyByOwnGov {
        ERC20 lambda = ERC20(LAMBDACONTROLLER.lambdaToken());
        lambda.approve(amo_minter_address, lambda_amount);
        amo_minter.burnLambdaFromAMO(lambda_amount);

        // Update the balance after the transfer goes through
        if (lambda_amount >= lambda_lent) lambda_lent = 0;
        else {
            lambda_lent -= lambda_amount;
        }
    }

    // Burn unneeded or excess shareToken. Goes through the minter
    function burnShareToken(uint256 shareToken_amount) public onlyByOwnGov {
        ERC20 shareToken = ERC20(LAMBDACONTROLLER.shareToken());
        shareToken.approve(amo_minter_address, shareToken_amount);
        amo_minter.burnShareTokenFromAMO(shareToken_amount);

        // Update the balance after the transfer goes through
        if (shareToken_amount >= shareToken_lent) shareToken_lent = 0;
        else {
            shareToken_lent -= shareToken_amount;
        }
    }
