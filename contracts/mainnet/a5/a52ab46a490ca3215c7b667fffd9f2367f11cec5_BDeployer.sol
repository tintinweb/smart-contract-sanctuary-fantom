...
contract PoolToken is IPoolToken, TombERC20 {
        ...
        returns (uint256 mintTokens)
    {
        ...
        mintTokens = mintTokens.sub(mintFeeDAO); // value to mint to end user
        if (totalSupply == 0) {
        ...
        _mint(IFactory(factory).admin(), mintFeeDAO); //mint DAO share
...
contract BAllowance is PoolToken, BStorage {
    ...
    function _checkBorrowAllowance(
        ...
        if (spender != owner && _borrowAllowance != uint256(-1)) {
            require(_borrowAllowance >= value, "TOMB: BORROW_NOT_ALLOWED");
...
contract Borrowable is
    ...
    BAllowance
{
    uint256 public BORROW_FEE = 0.001e18; //0.1%
    ...
    function changeBorrowFee(uint256 newFee) public {
        require(msg.sender == IFactory(factory).admin(), "Not permitted");
        BORROW_FEE = newFee;
    }
    ...
    function liquidate(address borrower, address liquidator)
        ...
        returns (uint256 seizeTokens)
    {
        require(IFactory(factory).canLiquidate(msg.sender), "Cannot liquidate");
        uint256 balance = IERC20(underlying).balanceOf(address(this));
