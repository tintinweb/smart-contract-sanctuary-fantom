...
import "./interfaces/IFeeProcessorV2.sol";
...
contract FeeProcessorV2 is Ownable, ReentrancyGuard, IFeeProcessorV2 {
    ...
    address public override optiSwap;
    ...
    function getClaimable() external view override returns (IClaimable) {
        return m_claimable;
    }

    function getFeeDistributor() external view override returns (IFeeDistributor) {
        return m_feeDistributor;
    }
    ...
    function _toTAROT(IERC20 token, uint256 amount) private returns (uint256 tarotOut) {
        tarotOut = optiSwapExactTokensForTokens(address(token), address(TAROT), amount);
    }
...
interface IFeeProcessorV2 {
    function optiSwap() external view returns (address);

    function getClaimable() external view returns (IClaimable);
