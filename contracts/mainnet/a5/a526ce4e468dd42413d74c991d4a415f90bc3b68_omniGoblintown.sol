...
contract omniGoblintown is ERC721, NonblockingReceiver {
    ...
    function mint(uint256 _amount) external payable {
        ...
        require(!paused, "omniGoblintown: Paused");
        require(nextTokenId + _amount <= MAX_MINT, "omniGoblintown: Mint exceeds supply");
        require(MAX_PER_TX >= _amount , "omniGoblintown: Excess max per tx");
        require(MAX_PER_WALLET >= minted[_caller] + _amount, "omniGoblintown: Excess max per wallet");
    ...
    function white_list_mint(uint256 _amount) external payable {
        ...
        require(!paused, "omniGoblintown: Paused");
        require(nextTokenId + _amount <= WHITELIST_MINT, "omniGoblintown: Mint exceeds supply");
        require(MAX_PER_TX_2 >= _amount , "omniGoblintown: Excess max per tx");
        minted[_caller] += _amount;
