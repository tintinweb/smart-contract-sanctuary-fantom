...
contract GIMPToonz is AccessControlEnumerable, ERC721Enumerable, ClampedRandomizer {
    ...
    string public deadBaseURI;
    string public normalBaseURI;
    bool public _openMint = false;
    bool public _deadMint = true;
    address payable private _wallet;
    mapping(uint256 => bool) public deadTokenIds;

    constructor (
        string memory name,
        string memory symbol, 
        string memory deadURI,
    ...
    function maxSupply(uint maxSupply) external onlyAdmin {
        max_supply = maxSupply;
    }
    ...
    function deadSwitch() external onlyAdmin {
        _deadMint = !_deadMint;
    }
    ...
    function deadMint(uint _mintAmount) public payable {
        require(_openMint == true, "Minting is currently not open");
        require(_deadMint == true, "The Dead Mint is currently not open");
        require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= max_supply, "max NFT limit exceeded");
        require(msg.value >= publicPrice * _mintAmount, "Amount sent is incorrect");
        ...
        for (uint i = 0; i < _mintAmount; i++) {
            uint tokenId = internalMint(msg.sender);
            deadTokenIds[tokenId] = true;
        }
        _wallet.transfer(msg.value);
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        string storage baseURI = deadTokenIds[tokenId] ? deadBaseURI : normalBaseURI;
