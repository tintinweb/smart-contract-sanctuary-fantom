...
interface IWsHecForWsHecPriceOracle {
    function wsHECTosHEC( uint _amount ) external view returns ( uint );
}

interface IMinistryForWsHecPriceOracle {
...
interface IPriceOracleWsHecPriceOracle {
    function getAssetPriceUpdateTimestamp(address asset) external view returns (uint);
}

/**
 * @title Ola's price oracle for wsHec.
 * @author Ola
 */
contract WsHecPriceOracle is BasePriceOracle {
    // Scaled by 1e9 (1 wsHEC == 11.083660303 HEC)
    uint constant public fixedWsHecToHec = 11083660303;

    uint constant public  singleWsHecUnit = 10**18;

    IWsHecForWsHecPriceOracle constant public wsHEC = IWsHecForWsHecPriceOracle(0x94CcF60f700146BeA8eF7832820800E2dFa92EdA);
    address constant public sHEC = 0x75bdeF24285013387A47775828bEC90b91Ca9a5F;
    address constant public HEC = 0x5C4FDfc5233f935f20D2aDbA572F770c2E377Ab0;
    IMinistryForWsHecPriceOracle immutable public ministry;

    constructor(address _ministry) {
        ministry = IMinistryForWsHecPriceOracle(_ministry);

        // BasePriceOracle
        _setDecimalsForAsset(address(wsHEC));
        _setDecimalsForAsset(sHEC);
    }

    /**
     * Returns the price of HEC (scaled to 1e18)
     * @dev NOT IN SYSTEM SCALE -- to be used for views only
     */
    function hecPrice() external view returns (uint) {
        uint hecPrice = ministry.getPriceForAsset(HEC);

        return hecPrice / 1e9;
    }

    /**
     * Returns the price of wsHEC (scaled to 1e18)
     * @dev NOT IN SYSTEM SCALE -- to be used for views only
     */
    function wsHecPrice() external view returns (uint) {
        return getPriceFromSourceInternal(address(wsHEC));
    }
    ...
    function getPriceFromSourceInternal(address asset) internal view override returns (uint) {
        // The HEC price will be scaled by 27 (36 - 9)
        uint hecPrice = ministry.getPriceForAsset(HEC);

        // The price of wsHec is = (the price of HEC) * (the amount of HEC in 1 wsHEC)
        // note : the result of this multiplication will be scaled to 36 (27 + 9)
        uint wsHecPrice = mul(hecPrice,fixedWsHecToHec);

        // The scale of the price returned for wsHEC should be 18 (36 - 18)
        wsHecPrice = wsHecPrice / 1e18;

        return wsHecPrice;
    }

    function getPriceUpdateTimestampFromSourceInternal(address asset) internal view override returns (uint) {
        address oracle = ministry.getOracleForAsset(asset);
        return IPriceOracleWsHecPriceOracle(oracle).getAssetPriceUpdateTimestamp(asset);
    }
