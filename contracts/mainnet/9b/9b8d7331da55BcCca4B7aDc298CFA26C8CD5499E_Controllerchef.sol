...
contract Draco is ERC20('Draco TESTING', 'DRACO') {
    ...
    constructor() {
        ...
        _mint(address(0xDD5114D75b4e7aCB344c47298b21dCA28ba87dfb), uint256(20000000000000000000)); // 20 tokens for treasury
    }
...
contract Controllerchef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accDracoPerShare;    
        uint16 depositFeeBP;
        uint256 lpSupply;
    }
    
    Draco public immutable draco;
    ...
    uint256 public dracoPerBlock = 0.004 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 dracoPerBlock);

    constructor(Draco _draco, uint256 _startBlock, address _feeAdd, address _treasuryAdd) {
        draco = _draco;
        startBlock = _startBlock;
        feeAdd = _feeAdd;
        treasuryAdd = _treasuryAdd;
        MAXSUPPLY = _draco.maxSupply();
    }
    ...
    function safeDracoTransfer(address _to, uint256 _amount) internal {
        uint256 dracoBalance = draco.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > dracoBalance) {
            transferSuccess = draco.transfer(_to, dracoBalance);
        } else {
            transferSuccess = draco.transfer(_to, _amount);
        }
        require(transferSuccess, "safeDracoTransfer: Transfer failed");
    }
    
    function updateEmissionRate(uint256 _dracoPerBlock) external onlyOwner {
        require(_dracoPerBlock >= MIN_PERBLOCK && _dracoPerBlock <= MAX_PERBLOCK, "Bad Emissions per second");
        massUpdatePools();
        dracoPerBlock = _dracoPerBlock;
        emit UpdateEmissionRate(msg.sender, _dracoPerBlock);
    }
    ...
    function pendingDraco(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardperBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 dracoReward = (multiplier * dracoPerBlock * pool.allocPoint) / totalAllocPoint;
            uint256 distribution = dracoReward / 110;
            if (dracoReward + distribution + TOTALSUPPLY() > MAXSUPPLY) {
                dracoReward = MAXSUPPLY - TOTALSUPPLY();
            }
            accDracoPerShare += dracoReward * 1e18 / pool.lpSupply;
        }
        return ((user.amount * accDracoPerShare) / 1e18) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 dracoReward = (multiplier * dracoPerBlock * pool.allocPoint) / totalAllocPoint;
        uint256 toTreasury = dracoReward / 100;
        uint256 toBurn = dracoReward / 10;
        
        if (TOTALSUPPLY() + dracoReward + toTreasury + toBurn <= MAXSUPPLY) {
            draco.mint(treasuryAdd, toTreasury);
            draco.mint(disasterAdd(), toBurn);
            draco.mint(address(this), dracoReward);
        } else {
            dracoReward = MAXSUPPLY - TOTALSUPPLY();
            draco.mint(address(this), dracoReward);
        }

        pool.accDracoPerShare += dracoReward * 1e18 / pool.lpSupply;
        pool.lastRewardperBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = ((user.amount * pool.accDracoPerShare) / 1e18) - user.rewardDebt;
            
            if (pending > 0) {
                safeDracoTransfer(msg.sender, pending);
            }
        }
        ...
        user.rewardDebt = (user.amount * pool.accDracoPerShare) / 1e18;
    ...
    function TOTALSUPPLY() private view returns (uint256) {
        return draco.totalSupply();
    }

    function disasterAdd() public view returns (address) {
        return draco.disaster();
    }
