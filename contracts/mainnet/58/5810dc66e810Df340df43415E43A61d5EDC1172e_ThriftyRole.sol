...
contract ThriftyRole is Ownable {
  ...
  uint256 public feeDai = 60000;
  uint256 public feePrint = 500000;
  uint256 public feeDaiReward = 500000;
  ...
  uint256 public feeAntiBot = 500000;
  ...
  event ValidateTrasfer(bool a, string b);
  ...
  constructor() {
    ...
    guardianId[msg.sender] = guardians.length;
  }
  ...
  function validateTransfer(address from, address to, uint256 userBalance, uint256 amount) public returns(bool) {
    if (blackListId[from] > 0 || blackListId[to] > 0) {
      emit ValidateTrasfer(false, "BL");
      return false;
    }
    if (onlyWhitelist == true && (whiteListId[from] == 0 || whiteListId[to] == 0) && enableAntiBot == false) {
      emit ValidateTrasfer(false, "WL");
      return false;
    }
    bool isBuy = from == IThrifty(thriftyAddress).pairAddress();
    bool isSell = to == IThrifty(thriftyAddress).pairAddress();
    if ((isBuy || isSell) && (noTradeLimitAddress[from] == false || noTradeLimitAddress[to] == false)) {
      if (amount >= IThrifty(thriftyAddress).totalSupply().mul(walletMaxTx).div(10**manageDecimal)) {
        emit ValidateTrasfer(false, "MaxTx");
        return false;
      }
      if (isBuy && IThrifty(thriftyAddress).balanceOf(to).add(amount) > IThrifty(thriftyAddress).totalSupply().mul(walletMaxLimit).div(10**manageDecimal)) {
        emit ValidateTrasfer(false, "MaxWalletLimit");
        return false;
      }
    }
    bool canSell = (isSell == true && enableVesting == true && from != thriftyAddress) ? _ableToSell(from, userBalance, amount) : true;
    if (isSell == true && enableVesting == true && canSell == false) {
      emit ValidateTrasfer(false, "VestingLimit");
      return false;
    }
    ...
    emit ValidateTrasfer(true, "Pass");
    return true;
  }
    ...
    returns(uint256[8] memory fees)
  {
    ...
    // printFee & rewardFee
    uint256 dai = (isTrade && (enablePrint || enableDaiReward)) ? amount.mul(feeDai).div(10 ** manageDecimal) : 0;
    if (isTrade == true) {
      if (enablePrint && enableDaiReward) {
        fees[4] = dai.mul(feePrint).div(10 ** manageDecimal);
        fees[5] = dai.mul(feeDaiReward).div(10 ** manageDecimal);
      }
      else if (enablePrint) {
        fees[4] = dai;
        fees[5] = 0;
      }
      else if (enableDaiReward) {
        fees[4] = 0;
        fees[5] = dai;
      }
    }
    ...
    // extraFee
    if (enableAntiBot == true && onlyWhitelist == true && (whiteListId[from] == 0 || whiteListId[to] == 0)) {
      fees[6] = amount.mul(feeAntiBot).div(10 ** manageDecimal);
    }
    else {
      fees[6] = (noLimitAdress==true || isTrade==false) ? 0 : _getExtraFee(amount, isSell, trader);
    }
    // receive amount
    uint256 tmp = fees[1] + fees[2] + fees[3];
    tmp = tmp + fees[4] + fees[5] + fees[6];
    fees[0] = amount - tmp;
    // total fee
    fees[7] = tmp;
  ...
  function setThriftyAddress(address _thriftyAddress) public onlyManager {
    ...
    noTradeLimitAddress[thriftyAddress] = true;
    noTradeLimitAddress[IThrifty(thriftyAddress).pairAddress()] = true;
    vipAddress[thriftyAddress] = true;
    vipAddress[IThrifty(thriftyAddress).pairAddress()] = true;
  }
  ...
  function setFee(
    ...
    uint256 _fDai, uint256 _fPrint, uint256 _fDaiReward,
    ...
    totalFee += _fDai + _fLargeSell;
    totalFee += _fMediumSell + _fFreqTrade + _fQuickSell;
    require(totalFee <= feeMax, "ThriftyRole: Overflow fee");
    require(_fPrint+_fDaiReward <= 10**manageDecimal, "ThriftyRole: Overflow dai fee");
  ...
  function setWalletMaxLimit(uint256 _limit) public onlyManager {
    require(_limit <= 10**manageDecimal, "ThriftyRole: overflow max limit");
  ...
  function setAntiBotMode(bool _mode) public onlyManager {
    enableAntiBot = _mode;
  }

  function setFeeAntiBot(uint256 _fee) public onlyManager {
    require(_fee <= 10**manageDecimal, "ThriftyRole: overflow max limit");
    feeAntiBot = _fee;
  }
