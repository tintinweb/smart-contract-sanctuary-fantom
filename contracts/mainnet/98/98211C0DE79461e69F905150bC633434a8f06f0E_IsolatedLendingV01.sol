...
contract IsolatedLendingV01 is ERC4626{
    ...
    uint256 private constant LIQUIDATION_MULTIPLIER = 7500; // 7.5%
    ...
    constructor(address _asset, address _collateral, string memory _name, string memory _symbol)ERC4626(ERC20(_asset), _name, _symbol){
        ...
        priceFeedCollateral = AggregatorV3Interface(0x9Da678cE7f28aAeC8A578A1e414219049509a552);
        ...
        exchangeRate = yvToken.pricePerShare()*uint256(collateralPrice)/uint256(assetPrice);
        feeTo = msg.sender;
        admin = msg.sender;
    }
    ...
    function liquidate(address _user, uint256 _amount) public {
        ...
        if(!isSolvent(_user)){
            ...
            uint256 collateralLiquidated = _amount*1e10/exchangeRate; //1e6*1e20/1e8 = 1e18 (moobeftm decimal) //1e6*1e10/1e8 = 1e8 (wbtc decimals)
