...
interface IStrat {
    function calcDebtRatio() external view returns (uint256 _debtRatio0, uint256 _debtRatio1);
...
contract HysteriaAlt is Initializable {
    ...
    function debtTrigger(address strategy) public view returns (bool _canExec, bytes memory _execPayload) {
        ...
        if (!isInactive(strategy)) {
            (uint256 debtRatio0, uint256 debtRatio1) = IStrat(strategy).calcDebtRatio();
            _canExec = (debtRatio0 > (IStrat(strategy).debtUpper().add(hysteriaDebt)) || debtRatio1 > IStrat(strategy).debtUpper().add(hysteriaDebt));           
        }
    }
