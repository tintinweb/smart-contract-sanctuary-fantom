...
contract Oracle is Initializable, Operator, Epoch {
    ...
    function setupOracle(IUniswapV2Pair _pair, uint256 _period, uint256 _startTime) external initializer onlyOperator {
        setupEpoch(_period, _startTime, 0);
        pair = _pair;
        token0 = pair.token0();
        token1 = pair.token1();
...
contract Epoch is Initializable,Operator {
    ...
    function setupEpoch( uint256 _period,uint256 _startTime,uint256 _startEpoch) internal onlyInitializing {
        period = _period;
