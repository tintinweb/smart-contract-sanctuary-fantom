...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    function _safeTokenTransfer(address token, address _to, uint256 _amount) internal {
        ...
        IERC20(token).safeTransfer(_to, _amount > tokenBal ? tokenBal : _amount);
    }
    ...
    function _getNFTPowerStaking(uint256 _nftID) internal view returns (uint256) {
        ...
        (
            ...
            magic,
            wisdom
        ) = ImergeAPI(iMergeAPI).getSkillCard(_nftID); // support for merged cards
        ...
        if (strength == 0 && agility == 0 ) {
            (
                ...
                wisdom,
                magic
            ) = IFactoryNFT(iFactoryNFT).getCharacterStats(_nftID);
        }
    ...
    function _getNFTExperience(uint256 _nftID) internal returns (uint256) {
        (,uint256 experience,) = IFactoryNFT(iFactoryNFT).getCharacterOverView(_nftID);
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            _amount = IERC20(pool.lpToken).balanceOf(address(this)) - balanceBefore;
            ...
            if (pool.depositFeeBP > 0) {
                ...
                treasuryDepositFee = (totalDepositFee * 2500) / 10000;
        ...
        emit Deposit(msg.sender, _pid, _amount, treasuryDepositFee);
    }
    ...
    function addNFT(uint256 _pid, uint256 _nftID) external nonReentrant {
        ...
        require(IFactoryNFT(iFactoryNFT).ownerOf(_nftID) == msg.sender, "E12");

        updatePool(_pid);
        _payPendingTheEndlessSandMan(_pid);

        IFactoryNFT(iFactoryNFT).safeTransferFrom(msg.sender, address(this), _nftID);
    ...
    function add(uint256 _tokenType,
        ...
        emit AddPool(poolInfo.length - 1, _tokenType, _allocPoint, _lpToken, _depositFeeBP);
    }
    ...
    function set(uint256 _pid, 
        ...
        emit SetPool(_pid, poolInfo[_pid].lpToken, _allocPoint, _depositFeeBP);
    }
    ...
    function withDrawNFTByIndex(uint256 _nftID, address _userAddress) external onlyOwner {
        require(IFactoryNFT(iFactoryNFT).ownerOf(_nftID) == address(this));

        IFactoryNFT(iFactoryNFT).safeTransferFrom(address(this), _userAddress, _nftID);
...
contract SandManToken is ERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiBot(sender, recipient, amount) {
        //transfer fee
        bool toFromtreasuryDAO = (sender == treasuryDAO || recipient == treasuryDAO);
        ...
        if (toFromtreasuryDAO ||
            ...
            super._transfer(sender, treasuryDAO, liquidityAmount);
    ...
    function addBotAddressToBlackList(address _botAddress, uint256 _expirationTime) external onlyOwner {
    ...
    function removeBotAddressToBlackList(address _botAddress) external onlyOperator {
...
/*
 * Errors table:
 * ERR1: one of the pairs must be the targetAddress
 * ERR2: invalid token type provided
 * ERR3: insufficient matic provided!
 * ERR4: wanted token address can't be the zero address!
 * ERR5: _sandManAddress is the zero address
 * ERR6: sandManAddress already set!
 * ERR7: matic pair !exist
 * ERR8: updateSandManSwapRouter: new _router is the zero address
 * ERR9: router already set!
 * ERR10: transfer failed!
 * ERR11:
 * ERR12:
 * ERR13:
 * ERR14:
...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
     ...
     */
    constructor(address _usdCurrency, address _sandManSwapRouter, uint256 startTime) {
        sandManSwapRouter = IUniswapV2Router02(_sandManSwapRouter);
    ...
    function convertToTargetValueFromPair(IUniswapV2Pair pair, uint256 sourceTokenAmount, address targetAddress) public view returns (uint256) {
        require(pair.token0() == targetAddress || pair.token1() == targetAddress, "ERR1");
    ...
    function getTokenUSDValue(uint256 tokenBalance, address token, uint256 tokenType, bool viaMaticUSD, address usdAddress) public view returns (uint256) {
        require(tokenType == 0 || tokenType == 1, "ERR2");
        ...
        if (tokenType == 1)
            token = IUniswapV2Pair(token).token0() == WETH ? WETH : (IUniswapV2Pair(token).token1() == WETH ? WETH : IUniswapV2Pair(token).token0());
        ...
        if (viaMaticUSD) {
            ...
            if (token == WETH)
                ...
                IUniswapV2Pair maticPair = IUniswapV2Pair(IUniswapV2Factory(sandManSwapRouter.factory()).getPair(WETH, token));

                if (address(maticPair) == address(0))
                    return 0;

                maticAmount = convertToTargetValueFromPair(maticPair, tokenAmount, WETH);
            }

            // As we arent working with usd at this point (early return), this is okay.
            IUniswapV2Pair usdmaticPair = IUniswapV2Pair(IUniswapV2Factory(sandManSwapRouter.factory()).getPair(WETH, address(usdAddress)));
    ...
    function autoLiquidity() external nonReentrant onlyRole(OPERATOR_ROLE) {
        ...
        if (res0 != 0 && res1 != 0) {
            ...
            // convert wnative to native. wftm to ftm.
    ...
    function _swapETHForTokens(uint256 ethAmount, address wantedTokenAddress) internal {
        require(address(this).balance >= ethAmount, "ERR3");
        require(wantedTokenAddress != address(0), "ERR4");
    ...
    function setSandManAddress(address _sandManAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_sandManAddress != address(0), "ERR5");
        require(sandManAddress == address(0), "ERR6");

        sandManAddress = _sandManAddress;

        sandManSwapPair = IUniswapV2Factory(sandManSwapRouter.factory()).getPair(sandManAddress, WETH);

        require(address(sandManSwapPair) != address(0), "ERR7");

        emit SetSandManAddresses(sandManAddress, sandManSwapPair);
    }

    function getUSDDripRate() external view returns (uint256) {
        return IERC20(usdCurrency).balanceOf(address(this)) / distributionTimeFrame;
    }
    ...
    function updateSandManSwapRouter(address _router) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_router != address(0), "ERR8");
        require(address(sandManSwapRouter) == address(0), "ERR9");
    ...
    function transferUSDToOwner(address ownerAddress, uint256 amount) external onlyRole(OPERATOR_ROLE) {
        ...
        require(IERC20(usdCurrency).transfer(ownerAddress, amount), "ERR10");
    ...
    function emergencyWithDrawETH() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (balanceEther > 0)
            payable(msg.sender).transfer(balanceEther);
    }
}
