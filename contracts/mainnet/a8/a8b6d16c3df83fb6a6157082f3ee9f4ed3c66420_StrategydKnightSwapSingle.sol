...
contract StrategydKnightSwapSingle is StratManager, FeeManager {
     ...
     */
    event StratHarvest(address indexed harvester,address indexed signer);
    ...
    function _harvest() internal {
        ...
        IMasterChef(chef).deposit(poolId, 0);   //.deposit(poolId, 0);
        uint256 wantBal = balanceOfWant();
        if (wantBal > 0) {
            chargeFees();
            deposit();
            emit StratHarvest(msg.sender,tx.origin);
        }
    }

    // performance fees
    function chargeFees() internal {
        uint256 toNative = balanceOfWant().mul(compoundingFee).div(1000);
