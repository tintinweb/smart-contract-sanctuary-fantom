...
contract AceLabFetchHelper {
  ...
  function fetchMagicatData (uint256 catId) public view returns (
    CatData memory cat
  ) {
    cat = CatData({
      id: catId,
  ...
  function fetchPoolMagicats (address user, uint256 pid, uint256 version) public view returns (
    CatData[] memory cats
  ) {
    ...
    for (uint256 i = 0; i < catIds.length; i++) {
      cats[i] = fetchMagicatData(catIds[i]);
    }
  }
  ...
  function fetchUserMagicatData (address user) public view returns (
    ...
    for (uint256 pid = 0; pid < v2PoolsCount; pid++) {
      stakedCats[pid] = PoolCats({
        pid: pid,
        version: 2,
        cats: fetchPoolMagicats(user, pid, 2)
      });
    }
    ...
    for (uint256 pid = 0; pid < v3PoolsCount; pid++) {
      stakedCats[pid + v2PoolsCount] = PoolCats({
        pid: pid,
        version: 3,
        cats: fetchPoolMagicats(user, pid, 3)
      });
    }
  }
}
