...
contract EarnMore is Gauge, Ownable {
    ...
    function earnMoreDeposit(uint amount, uint tokenId) public {
        earnMoreDepositTo(msg.sender, amount, tokenId);
    }

    function earnMoreDepositTo(address recepient, uint amount, uint tokenId) public earnMoreEnabled {
    	require(earnMore[recepient] || balanceOf[recepient] == 0, "You already have non-EarnMore deposit");

        earnMore[recepient] = true;
    	baseTokenBalance[recepient] += calculateTokenBalance(amount);
    	_depositTo(recepient, amount, tokenId);
    }

    function calcExcludedProfit(address account, uint256 amount) public view returns(uint256) {
    ...
    function calcExpectedReturn(address account, uint256 amount) public view returns(uint256) {
        return amount - calcExcludedProfit(account, amount);
    }

    /// @dev exclude % of income at the exit from earnMore
    /// @param account - user who exits
    /// @param amount - amount of tokens to be withdrawn
    function _excludeProfit(address account, uint256 amount) internal returns(uint256) {
    	uint256 valueToExclude = calcExcludedProfit(account, amount);
    ...
    function earnMoreWithdraw(uint256 amount) public {
        earnMoreWithdrawFrom(msg.sender, msg.sender, amount);
    }    

    function earnMoreWithdrawFrom(address from, address to, uint256 amount) public {
        // #Check that not approved can do nothing
        require(earnMore[from], "You don't have EarnMore deposit");

        uint256 portion = EM_PRECISION * amount / balanceOf[from];
        uint256 excludedValue = _excludeProfit(from, amount);

        baseTokenBalance[from] -= baseTokenBalance[from] * portion / EM_PRECISION;
        if (baseTokenBalance[from] == 0) {
            earnMore[from] = false;
        }

        _withdrawToken(from, to, amount - excludedValue, tokenIds[msg.sender]);
    }

    function depositTo(address recipient, uint amount, uint tokenId) public override {
        require(!earnMore[recipient], "You already have EarnMore deposit");
        _depositTo(recipient, amount, tokenId);
    }

    /// @dev withdraw tokens from Earn More
    /// @param amount - amount of tokens to withdraw
    /// @param tokenId - NFT token id
    function withdrawToken(address from, address to, uint amount, uint tokenId) public override {
        require(!earnMore[from], "You should withdraw through earnMoreWithdraw");
        _withdrawToken(from, to, amount, tokenId);
    }

    function transferFrom(address from, address to, uint256 amount) public override {
        if(earnMore[from]) {
            earnMoreWithdrawFrom(from, to, amount);
        } else {
...
// Gauges are used to incentivize pools, they emit reward tokens over 28 days for staked LP tokens
...
contract Gauge {
    ...
    uint internal constant DURATION = 28 days; // rewards are released over 28 days
    ...
    event Withdraw(address indexed from, address indexed to, uint tokenId, uint amount);
    ...
    */
    ...
    function getReward(address account, address[] memory tokens) external lock {
        require(msg.sender == account || msg.sender == voter || tx.origin == account);
        _unlocked = 1;
    ...
    function depositAll(uint tokenId) external {
        depositTo(msg.sender, erc20(stake).balanceOf(msg.sender), tokenId);
    }

    function deposit(uint amount, uint tokenId) external {
        depositTo(msg.sender, amount, tokenId);
    }

    function depositTo(address recipient, uint amount, uint tokenId) public virtual {
        _depositTo(recipient, amount, tokenId);
    }

    function _depositTo(address recipient, uint amount, uint tokenId) internal lock {
        require(amount > 0);
        ...
        if (tokenId > 0) {
            require(ve(_ve).ownerOf(tokenId) == recipient);
            if (tokenIds[recipient] == 0) {
                tokenIds[recipient] = tokenId;
                Voter(voter).attachTokenToGauge(tokenId, recipient);
            }
            require(tokenIds[recipient] == tokenId);
        } else {
            tokenId = tokenIds[recipient];
        }

        uint _derivedBalance = derivedBalances[recipient];
        derivedSupply -= _derivedBalance;
        _derivedBalance = derivedBalance(recipient);
        derivedBalances[recipient] = _derivedBalance;
        derivedSupply += _derivedBalance;

        _writeCheckpoint(recipient, _derivedBalance);
        _writeSupplyCheckpoint();

        Voter(voter).emitDeposit(tokenId, recipient, amount);
        emit Deposit(recipient, tokenId, amount);
    }
    ...
    function withdraw(uint amount) public {
        ...
        withdrawToken(msg.sender, msg.sender, amount, tokenId);
    }

    function withdrawToken(address from, address to, uint amount, uint tokenId) public virtual {
         _withdrawToken(from, to, amount, tokenId);
    }

    function _withdrawToken(address from, address to, uint amount, uint tokenId) internal lock {
        if (from != msg.sender) {
        ...
        if (tokenId > 0) {
            require(tokenId == tokenIds[from]);
            tokenIds[from] = 0;
            Voter(voter).detachTokenFromGauge(tokenId, from); // Check functions
        } else {
            tokenId = tokenIds[from];
        }

        uint _derivedBalance = derivedBalances[from];
        derivedSupply -= _derivedBalance;
        _derivedBalance = derivedBalance(from);
        derivedBalances[from] = _derivedBalance;
        derivedSupply += _derivedBalance;

        _writeCheckpoint(from, derivedBalances[from]);
        _writeSupplyCheckpoint();

        Voter(voter).emitWithdraw(tokenId, from, amount);
        emit Withdraw(from, to, tokenId, amount);
    }
