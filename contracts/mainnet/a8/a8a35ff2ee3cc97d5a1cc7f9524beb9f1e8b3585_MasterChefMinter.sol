...
contract MasterChefMinter is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // COMB tokens created per block.
    uint256 public combPerBlock;
    // Bonus muliplier for early comb makers.
    ...
    event UpdateEmissionRate(address indexed user, uint256 combPerBlock);

    function initialize(
        COMB _comb,
        address _devaddr,
        address _feeAddress,
        uint256 _combPerBlock,
        uint256 _startBlock
    ) public initializer {
    ...
    // Update the given pool's COMB allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 combReward = multiplier
                .mul(combPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accCombPerShare = accCombPerShare.add(
                combReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 combReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        comb.mint(devaddr, combReward.div(12));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accCombPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeCombTransfer(msg.sender, pending);
    ...
    function safeCombTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > combBal) {
            transferSuccess = comb.transfer(_to, combBal);
        } else {
            transferSuccess = comb.transfer(_to, _amount);
        }
        require(transferSuccess, "safeCombTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _combPerBlock) public onlyOwner {
        massUpdatePools();
        combPerBlock = _combPerBlock;
        emit UpdateEmissionRate(msg.sender, _combPerBlock);
    }
}
