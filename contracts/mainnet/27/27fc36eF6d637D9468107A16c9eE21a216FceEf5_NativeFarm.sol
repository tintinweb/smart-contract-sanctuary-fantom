/**
 *Submitted for verification at FtmScan.com on 2022-08-23
*/
...
contract NativeFarm is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlockTime; // Last block timestamp that NATIVE distribution occurs.
        uint256 accNATIVEPerShare; // Accumulated NATIVE per share, times 1e12. See below.
        address strat; // Strategy address that will auto compound want tokens
    }

    // Token address
    address public constant NATIVE = 0x14bf0e443380c197dd7019c455F7d5d92c230e41; // TokenX
    uint256 public NATIVEPerSecond; // Amount set on first crosschain sync
    uint256 public startTime; // Timestamp set on first crosschain sync   

    PoolInfo[] public poolInfo; // Info of each pool.
    mapping(uint256 => mapping(address => UserInfo)) public userInfo; // Info of each user that stakes LP tokens.
    uint256 public totalAllocPoint = 0; // Total allocation points. Must be the sum of all allocation points in all pools.

    address public anyCallAddress = 0xC10Ef9F491C9B59f936957026020C321651ac078;
    ...
    function updatePool(uint256 _pid) public {
        ...
        // not minted, instead just assigned from bridged reserve
    ...
    function setAnycallProxy(address _proxy)
        external
        onlyOwner
    {
        anyCallAddress = _proxy;
    }
        ...
        returns (bool success, bytes memory result)
    {
        ...
        } else if (_nativePerSecond < NATIVEPerSecond) {
            // Later allow only lower
            NATIVEPerSecond = _nativePerSecond;
        }
    }
}