...
contract BIGBondDepository2 is Ownable {
    ...
    uint256 public multiplier = 1e16;
    uint256 public multiplier2 = 1e8;
    uint256 public multiplier3 = 10;
    ...
    function setMul1(uint256 _new) public onlyOwner() {
        multiplier = _new;
    }

    function setMul2(uint256 _new) public onlyOwner() {
        multiplier2 = _new;
    }

    function setMul3(uint256 _new) public onlyOwner() {
        multiplier3 = _new;
    }
    ...
    function payoutFor( uint _value ) public view returns ( uint ) { // bondPayout
        ...
        // 10 * 10^9 / 100 = 10 * 10^7
        return FixedPoint.fraction(_value * multiplier3, bondPrice()).decode112with18() / multiplier;
    ...
    function bondPrice() public view returns ( uint price_ ) { 
        price_ = terms.controlVariable.mul(debtRatio()).add(1000000000) / multiplier2;
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = bondPrice().mul( bondCalculator.markdown( address(principle) ) ) / 1000 ;
        } else {
            price_ = bondPrice().mul( 10 ** principle.decimals() ) / 1000;
        }
    }
