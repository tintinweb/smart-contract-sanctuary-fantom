...
contract PlatformVoter is IPlatformVoter, Ownable {
    event UpdateVoteLock(address user, bool lock);
    ...
    function verifyVotes(
        ...
        uint256 i;
        int256 totalUserWeight = 0;
        for (i = 0; i < weights_.length; i++) {
            totalUserWeight += weights_[i] > 0 ? weights_[i] : -weights_[i];
        }

        int256 totalPlatformWeight = 0;
        for (i = 0; i < weights.length; i++) {
            totalPlatformWeight += weights[i] > 0 ? weights[i] : -weights[i];
        }

        for (i = 0; i < poolVote.length; i++) {
            require(
                poolVote[0] == poolVote_[0],
                "PlatformVoter: BAD_POOL_ADDRESS"
            );

            require(
                weights_[i] >=
                    (((totalUserWeight * voteShare) / 100) * weights[i]) /
                        totalPlatformWeight,
                "PlatformVoter: INSUFFICIENT_WEIGHT"
            );
        }
    }
