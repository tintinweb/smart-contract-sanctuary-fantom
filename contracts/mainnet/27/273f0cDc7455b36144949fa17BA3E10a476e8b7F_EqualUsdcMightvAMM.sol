...
// File contracts/EqualUsdcMightvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualUsdcMightvAMM is Ownable, Pausable {
    ...
    address public constant might = address(0x9be0C5b42FB3540675A8d3E6b540f06734dFFbA6);
    address public want;
    ...
    IEqualizerRouter.Routes[] public usdcToMightPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _usdcToMightPath
    ) {
        ...
        for (uint i; i < _usdcToMightPath.length; ++i) {
            usdcToMightPath.push(_usdcToMightPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        uint256 usdcHalf = IERC20(usdc).balanceOf(address(this)) / 2;
        IEqualizerRouter(router).swapExactTokensForTokens(usdcHalf, 0, usdcToMightPath, address(this), block.timestamp);

        uint256 lp0Bal = IERC20(usdc).balanceOf(address(this));
        uint256 lp1Bal = IERC20(might).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(usdc, might, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(might).safeApprove(router, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(might).safeApprove(router, type(uint).max);
    }
