...
// 
// MaoToken with Governance.
contract MaoToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 Mao)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to Mao's router when our own AMM release
    IUniswapV2Router02 public MaoRouter;
    // The trading pair
    address public MaoPair;
    // In swap and liquify
    ...
    event MaoRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (!_excludedFromAntiWhale[sender] && !_excludedFromAntiWhale[recipient]) {
                require(amount <= maxTransferAmount(), "Mao::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the MaoToken contract.
     */
    constructor() public BEP20("MaoToken", "MAO") {
    ...
    /// @dev overrides transfer function to meet tokenomics of Mao
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(MaoRouter) != address(0)
            && MaoPair != address(0)
            && sender != MaoPair
            && sender != owner()
        ) {
        ...
        if (recipient == address(0)) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "Mao::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "Mao::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the Mao pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = MaoRouter.WETH();

        _approve(address(this), address(MaoRouter), tokenAmount);

        // make the swap
        MaoRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        MaoRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from MaoRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "Mao::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "Mao::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "Mao::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateMaoRouter(address _router) public onlyOperator {
        MaoRouter = IUniswapV2Router02(_router);
        MaoPair = IUniswapV2Factory(MaoRouter.factory()).getPair(address(this), MaoRouter.WETH());
        require(MaoPair != address(0), "Mao::updateMaoRouter: Invalid pair address.");
        emit MaoRouterUpdated(msg.sender, address(MaoRouter), MaoPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "Mao::transferOperator: new operator is the zero address");
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "Mao::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Mao::delegateBySig: invalid nonce");
        require(now <= expiry, "Mao::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "Mao::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Maos (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "Mao::_writeCheckpoint: block number exceeds 32 bits");
...
contract Referral is Ownable {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i; i < levelRate.length; i++) {
            ...
            if (
                ...
                mao.safeTransfer(accountAddress, c);
    ...
    /**
     * @dev Transfer Maos away from Referral contract if added by accident (not supposed to be called)
     * @param _amount Amount to transfer away
     */
    function transferFunds(uint256 _amount) external onlyOwner {
        mao.safeTransfer(owner(), _amount);
    }
...
// 
// MasterChef is the master of Maos. He can make Maos and he is a fair guy.
//
...
contract MasterChef is Referral, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MAOs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMaoPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMaoPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. MAOs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that MAOs distribution occurs.
        uint256 accMaoPerShare;   // Accumulated MAOs per share, times 1e12. See below.
    ...
    // Dev address.
    address public devAddress = 0xB50E0B4bdD373F1CD56c11D1784bF8C522dE5309;
    // Deposit Fee address
    address public feeAddress = 0xB50E0B4bdD373F1CD56c11D1784bF8C522dE5309;
    // MAO tokens created per block.
    uint256 public maoPerBlock;
    ...
    // Bonus muliplier for early mao makers.
    ...
    // The block number when MAO mining starts.
    ...
    // Update the given pool's MAO allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending MAOs on frontend.
    function pendingMao(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accMaoPerShare = pool.accMaoPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 maoReward = multiplier.mul(maoPerBlock).mul(pool.allocPoint).div(totalAllocPoint).mul(stakingPercent).div(percentDec);
            accMaoPerShare = accMaoPerShare.add(maoReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accMaoPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function withdrawDev() public{	
        ...
        uint256 maoReward = multiplier.mul(maoPerBlock);	
        mao.mint(devAddress, maoReward.mul(devPercent).div(percentDec));	
        lastBlockDevWithdraw = block.number;	
    }

    // View function to see if user can harvest MAOs.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 maoReward = multiplier.mul(maoPerBlock).mul(pool.allocPoint).div(totalAllocPoint).mul(stakingPercent).div(percentDec);
        mao.mint(devAddress, maoReward.div(10));
        mao.mint(address(this), maoReward);
        pool.accMaoPerShare = pool.accMaoPerShare.add(maoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for MAO allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(mao)) {
                ...
                uint256 transferTax = _amount.mul(mao.transferTaxRate()).div(10000);
        ...
        user.rewardDebt = user.amount.mul(pool.accMaoPerShare).div(1e12);
    ...
    // Pay or lockup pending MAOs.
    function payOrLockupPendingMao(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accMaoPerShare).div(1e12).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            ...
            safeMaoTransfer(msg.sender, actualReward);
            // Pay referral
            ...
            if(isReferralsEnabled() && hasReferrer(msg.sender)) {
                ...
                safeMaoTransfer(feeAddress, rest.add(fee));
                
            } else if(fee > 0 || referralCut > 0){
                // Transfer to fee address
                safeMaoTransfer(feeAddress, referralCut.add(fee));
            }
        }
        else if (pending > 0) {
    ...
    // Safe mao transfer function, just in case if rounding error causes pool to not have enough MAOs.
    function safeMaoTransfer(address _to, uint256 _amount) internal {
        uint256 maoBal = mao.balanceOf(address(this));
        if (_amount > maoBal) {
            mao.transfer(_to, maoBal);
        } else {
            mao.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _maoPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, maoPerBlock, _maoPerBlock);
        maoPerBlock = _maoPerBlock;
    }
