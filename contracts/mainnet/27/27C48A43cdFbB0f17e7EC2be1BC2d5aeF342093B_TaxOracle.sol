...
contract TaxOracle is Ownable {
    ...
    constructor(
        ...
        require(_peg != address(0), "peg address cannot be 0");
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(peg), "token needs to be peg");
        uint256 pegBalance = peg.balanceOf(pair);
        uint256 wftmBalance = wftm.balanceOf(pair);
        return uint144(wftmBalance.mul(_amountIn).div(pegBalance));
    }

    function getTombBalance() external view returns (uint256) {
	return peg.balanceOf(pair);
    }
    ...
    function getPrice() external view returns (uint256) {
        uint256 pegBalance = peg.balanceOf(pair);
        uint256 wftmBalance = wftm.balanceOf(pair);
        return wftmBalance.mul(1e18).div(pegBalance);
    }


    function setTomb(address _peg) external onlyOwner {
        require(_peg != address(0), "peg address cannot be 0");
        peg = IERC20(_peg);
    }
