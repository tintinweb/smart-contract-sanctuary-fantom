...
contract NeoPool is Ownable {
    ...
    IERC20 pills;           // Address of LP token contract.
    ...
    //     rewardSegments[block.timestamp] = 0;
    // }
    
    // function transferFrom(address _sender, address _recipient, uint256 amount, bool transferRewards) 
    // private virtual
    // {
    //     UserInfo storage sender = userInfo[_sender];
    //     UserInfo storage recipient = userInfo[_recipient];

    //     require(sender.amount >= amount, "Not enough to send!");
    //     sender.amount -= amount;
    //     recipient.amount += amount;

    //     updatePool();
    //     uint256 pending = user.amount.mul(accMorphPerShare).div(1e12).sub(sender.rewardDebt);
    //     if(pending > 0) {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > lastRewardTimestamp && pillsSupply != 0) {
            ...
            for (uint j = 0; j < rewardUpdateTimestamps.length; j++) {
                if(rewardUpdateTimestamps[j] <= lastRewardTimestamp)
                    continue;

                uint256 multiplier = 0;
                if (j == rewardUpdateTimestamps.length - 1) {
                    multiplier = block.timestamp - rewardUpdateTimestamps[j];
                } else {
                    multiplier = rewardUpdateTimestamps[j + 1] - rewardUpdateTimestamps[j];
                }
                rewardPerSec.add(multiplier.mul(rewardSegments[rewardUpdateTimestamps[j]]));
            }

            _accMorphPerShare = accMorphPerShare.add(rewardPerSec.mul(1e12).div(pillsSupply));
        }
    ...
    function updateRewardPerSec() public {
        ...
        uint256 rewardPerSec = rewardSupply.div(633312);
        ...
        rewardUpdateTimestamps.push(block.timestamp + 633312);
        rewardSegments[block.timestamp] = rewardPerSec;

        // in case rewardPerSec doesnt update in time
        rewardSegments[block.timestamp + 633312] = 0;
    ...
    function updatePool() public {
        ...
        for (uint j = 0; j < rewardUpdateTimestamps.length; j++) {
            ...
            rewardPerSec = rewardPerSec.add(multiplier.mul(rewardSegments[rewardUpdateTimestamps[j]]));
        }
