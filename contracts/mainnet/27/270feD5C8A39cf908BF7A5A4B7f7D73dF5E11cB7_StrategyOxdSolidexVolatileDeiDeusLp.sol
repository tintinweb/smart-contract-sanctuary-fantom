...
contract StrategyOxdSolidexVolatileDeiDeusLp is StrategyOxdFarmLPBase {
    // Token addresses
    address public dei_deus_lp = 0xF42dBcf004a93ae6D5922282B304E2aEFDd50058;
    ...
    {
        swapRoutes[dei].push(route(wftm, usdc, false));
        swapRoutes[dei].push(route(usdc, dei, true));
        swapRoutes[deus].push(route(wftm, usdc, false));
        swapRoutes[deus].push(route(usdc, dei, true));
        swapRoutes[deus].push(route(dei, deus, false));
    }

    // **** Views ****

    function getName() external pure override returns (string memory) {
        return "StrategyOxdSolidexVolatileDeiDeusLp";
    }
}
