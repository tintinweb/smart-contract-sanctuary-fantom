...
import {FantomFeeRewardForwarder} from "./FantomFeeReward.sol";

/// @title Bounty Simulator
/// @author Chainvisions
/// @notice Contract for simulating the doHardWork bounty of a vault.

contract BountySim is Controllable {

    constructor(address _store) Controllable(_store) {}

    /// @notice Simulates rewards from a doHardWork bounty.
    /// @param _vault Vault to simulate the bounty of.
    /// @return The bounty rewarded for the doHardWork.
    function simulateBounty(address _vault) external returns (uint256) {
        Controller control = Controller(controller());
        FantomFeeRewardForwarder forwarder = FantomFeeRewardForwarder(control.feeRewardForwarder());

        IERC20 reserve = IERC20(forwarder.reserveToken());
        uint256 balanceBefore = reserve.balanceOf(tx.origin);
        control.doHardWork(_vault);
        uint256 balanceAfter = reserve.balanceOf(tx.origin);
...
contract FantomFeeRewardForwarder is Governable {
    using SafeERC20 for IERC20;

    /// @notice Percentage to be sent as a gas rebate for doHardWork calls.
    uint256 public rebateNumerator;

    /// @notice Precision for calculating gas rebates.
    uint256 public rebateDenominator = 10000; // 1e4 precision, allowing for percentages down to 0.01%.
    ...
    function performTokenConversion(address _tokenFrom, uint256 _fee) external {
        ...
        if(targetRouteToReward.length > 1) {
            ...
            address targetRouter = tokenConversionRouter[_tokenFrom][reserve];
            if(targetRouter != address(0)) {
                ...
                uint256 rebateAmount = (endAmount * rebateNumerator) / rebateDenominator;
                uint256 remainingAmount = (endAmount - rebateAmount);
                IERC20(reserve).safeTransfer(tx.origin, rebateAmount);
                IERC20(reserve).safeTransfer(belugaMultisig, remainingAmount);
            } else {
                // Else, we need to perform a cross-dex liquidation.
                address[] memory targetRouters = tokenConversionRouters[_tokenFrom][reserve];
    ...
    function setRebateNumerator(uint256 _rebateNumerator) public onlyGovernanceOrMultisig {
        require(_rebateNumerator <= rebateDenominator, "FeeRewardForwarder: New numerator is higher than the denominator");
        rebateNumerator = _rebateNumerator;
    }

    /// @notice Sets the precision for gas rebates.
    /// @param _rebateDenominator Precision used for buyback calculations.
    function setrebateDenominator(uint256 _rebateDenominator) public onlyGovernanceOrMultisig {
        require(_rebateDenominator >= rebateNumerator, "FeeRewardForwarder: New denominator is lower than the numerator");
        rebateDenominator = _rebateDenominator;
    }
