...
contract StrategyBeethovenxDual is StratManager, FeeManager {
    ...
    bytes32 public outputToNativePoolId;
    bytes32 public rewardToNativePoolId;
    bytes32 public outputToInputPoolId;
    bytes32 public rewardToInputPoolId;
    ...
    constructor(
        ...
        outputToNativePoolId = _balancerPoolIds[1];
        outputToInputRoute = _outputToInputRoute;
        rewardToNativePoolId = _balancerPoolIds[2];
        rewardToInputPoolId = _balancerPoolIds[3];
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (outputBal > 0) {
            balancerSwap(outputToNativePoolId, output, native, outputBal);
        }

        uint256 rewardBal = IERC20(reward).balanceOf(address(this)).mul(45).div(1000);
        if (rewardBal > 0) {
            balancerSwap(rewardToNativePoolId, reward, native, rewardBal);
        }
    ...
    function addLiquidity() internal {
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        if (outputBal > 0 && output != input) {
            IUniswapRouterETH(spiritRouter).swapExactTokensForTokens(outputBal, 0, outputToInputRoute, address(this), now);
        }

        uint256 rewardBal = IERC20(reward).balanceOf(address(this));
        if (rewardBal > 0 && reward != input) {
            balancerSwap(rewardToInputPoolId, reward, input, rewardBal);
        }
    ...
    function callReward() public returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            nativeOut = balancerSwap(outputToNativePoolId, output, native, outputBal);
        }
        if (rewardBal > 0) {
            nativeOut += balancerSwap(rewardToNativePoolId, reward, native, rewardBal);
        }
