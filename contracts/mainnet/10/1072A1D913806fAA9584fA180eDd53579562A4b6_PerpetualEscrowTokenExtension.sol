...
library $
{
	...
	address constant XLQDR_FEE_DISTRIBUTION = 0x095010A79B28c99B2906A8dc217FC33AEfb7Db93;

	// testnet
	//address constant LQDR = 0x78d8B8fc82271fdC927564BB4615C5C6A8C324C6;
	//address constant XLQDR = 0x9EA3DC7D521668aF962a3Cd50daA6Abc30CE8572;
	//address constant XLQDR_FEE_DISTRIBUTION = 0x6DdFa3d8d666ba6Fb15b63604E5ECBC22240b99C;
}
...
contract PerpetualEscrowTokenExtension is Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	constructor(address _escrowToken, address _recipient)
	{
	...
	{
		PerpetualEscrowToken(escrowToken)._call($.XLQDR_FEE_DISTRIBUTION, abi.encodeWithSelector(FeeDistribution.claim.selector));
		for (uint256 _i = 0; _i < 7; _i++) {
			address _token = FeeDistribution($.XLQDR_FEE_DISTRIBUTION).tokens(_i);
...
contract PerpetualEscrowTokenReceiver is Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	function addToken(address _token, address _router1, address[] calldata _path1, address _router2, address[] calldata _path2, uint256 _minAmount) external onlyOwner
	{
		SwapInfo storage _swapInfo = swapInfo[_token];
		require(!_swapInfo.exists, "duplicate token");
		uint256 _index = tokens.length;
		tokens.push(_token);
		_swapInfo.exists = true;
		_swapInfo.index = _index;
		_swapInfo.router1 = _router1;
		_swapInfo.path1 = _path1;
		_swapInfo.router2 = _router2;
		_swapInfo.path2 = _path2;
	...
	{
		SwapInfo storage _swapInfo = swapInfo[_token];
		require(_swapInfo.exists, "unknown token");
		uint256 _index = _swapInfo.index;
		_swapInfo.exists = false;
		_swapInfo.index = 0;
		_swapInfo.router1 = address(0);
		_swapInfo.path1 = new address[](0);
		_swapInfo.router2 = address(0);
		_swapInfo.path2 = new address[](0);
	...
	{
		SwapInfo storage _swapInfo = swapInfo[_token];
		require(!_swapInfo.exists, "invalid token");
		uint256 _balance = IERC20(_token).balanceOf(address(this));
		IERC20(_token).safeTransfer(treasury, _balance);
	}

	function estimate() external view returns (uint256 _shares)
	{
		_shares = 0;
		for (uint256 _i = 0; _i < tokens.length; _i++) {
			...
			address _router1 = _swapInfo.router1;
			address[] memory _path1 = _swapInfo.path1;
			uint256 _shares1 = Router02(_router1).getAmountsOut(_balance, _path1)[_path1.length - 1];
			address _router2 = _swapInfo.router2;
			address[] memory _path2 = _swapInfo.path2;
			uint256 _amount2 = Router02(_router2).getAmountsOut(_balance, _path2)[_path2.length - 1];
			uint256 _shares2 = PerpetualEscrowToken(escrowToken).calcSharesFromAmount(_amount2);
			_shares += _shares1 >= _shares2 ? _shares1 : _shares2;
		}
		return _shares;
	}
	...
	function gulp() external nonReentrant
	{
		for (uint256 _i = 0; _i < tokens.length; _i++) {
			...
			uint256 _amount2 = Router02(_router2).getAmountsOut(_balance, _path2)[_path2.length - 1];
			uint256 _shares2 = PerpetualEscrowToken(escrowToken).calcSharesFromAmount(_amount2);
			if (_shares1 >= _shares2) {
				uint256 _factor = _oracleAveragePriceFactorFromInput(_router1, _path1, _balance);
				if (_factor < minimalGulpFactor) continue;
				IERC20(_token).safeApprove(_router1, _balance);
				Router02(_router1).swapExactTokensForTokens(_balance, _shares1, _path1, address(this), block.timestamp);
			} else {
				uint256 _factor = _oracleAveragePriceFactorFromInput(_router2, _path2, _balance);
				if (_factor < minimalGulpFactor) continue;
				IERC20(_token).safeApprove(_router2, _balance);
				Router02(_router2).swapExactTokensForTokens(_balance, _amount2, _path2, address(this), block.timestamp);
				IERC20(_path2[_path2.length - 1]).safeApprove(escrowToken, _amount2);
				PerpetualEscrowToken(escrowToken).deposit(_amount2, _shares2);
			}
		}
		{
