...
contract BeefyHarvesterGelato is BeefyHarvesterBase, IResolver {
    /*             */
    /* Initializer */
    /*             */

    function initialize(address vaultRegistry_) external initializer {
        __Manageable_init();
    ...
    function checker() external view override returns (bool canExec_, bytes memory execPayload_) {
        (
            ...
            bool isDailyHarvest
        ) = checkUpkeep(tx.gasprice);

        if (!isAnyVaultHarvestable) {
            execPayload_ = bytes("BeefyAutoHarvester: No vaults to harvest");
        } else {
            canExec_ = true;
    ...
    function performUpkeep(
        ...
        _performUpkeep(checkerGasprice_, vault_, newStartIndex_, estimatedTxCost_, estimatedCallRewards_, estimatedProfit_, isDailyHarvest_);
    }

    function getUpkeepTxPremiumFactor() public pure override returns (uint256 upkeepTxPremiumFactor_) {
        // 10% tx fee buffer. Gelato doesn't charge a premium, but this is to account for gas calc errors.
        upkeepTxPremiumFactor_ = .1 gwei;
    }
}
...
abstract contract BeefyHarvesterBase is ManageableUpgradeable, PausableUpgradeable {
    ...
    uint256 public constant BEEFY_HARVESTER_OVERHEAD = 47_000; // Estimated from GasExperiments tests.
    ...
    function checkUpkeep(uint256 gasPrice_)
        public
        view
        returns (
        ...
        )
    {
        ...
        (isAnyVaultHarvestable_, vault_, newStartIndex_, estimatedTxCost_, estimatedCallRewards_, isDailyHarvest_) = _findVaultToHarvest(
            gasPrice_,
            vaults
        );
    ...
    function _findVaultToHarvest(uint256 gasPrice_, address[] memory vaults_)
        internal
        view
        returns (
            bool isAnyVaultHarvestable_,
        ...
        )
    {
        ...
        for (uint256 offset; offset < vaults_.length; ++offset) {
            ...
            (bool willHarvest, uint256 estimatedTxCost, uint256 callRewardsAmount, bool isDailyHarvest) = _willHarvestVault(gasPrice_, vaultAddress);
    ...
    function _willHarvestVault(uint256 gasPrice_, address vaultAddress_)
        internal
        view
        returns (
    ...
    {
        (bool shouldHarvestVault, uint256 estimatedTxCost, uint256 callRewardAmount, bool isDailyHarvest) = _shouldHarvestVault(
            gasPrice_,
            vaultAddress_
        );
        ...
        estimatedTxCost_ = estimatedTxCost;
        callRewardAmount_ = callRewardAmount;
        isDailyHarvest_ = isDailyHarvest;
    }
    ...
    function _shouldHarvestVault(uint256 gasPrice, address vaultAddress_)
        internal
        view
        returns (
        ...
        )
    {
        ...
        txCostWithPremium_ = _calculateTxCostWithPremium(vaultHarvestGasOverhead, gasPrice);
        ...
        isDailyHarvest_ = !hasBeenHarvestedToday && callRewardAmount_ > 0;
    ...
    function _getThirdPartyUpstreamContractGasOverhead() internal view returns (uint256 gasOverhead_) {
