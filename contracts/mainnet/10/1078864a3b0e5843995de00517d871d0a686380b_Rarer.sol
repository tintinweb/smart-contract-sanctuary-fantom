...
// File: Rare Finance/Rarer.sol

pragma solidity 0.6.12;

contract Rarer is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 48h genesis pools 
    uint256 public constant INITIAL_RARER_POOL_DISTRIBUTION = 27500 ether;

    // DAO FUND INITIAL ALLOCATION IS 1000 RARER
    ...
    /**
     * @notice Constructs the RARER ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("RARER", "RARER") {
        // Mints 3,000 RARER to contract creator for initial pool setup & LP
    ...
    function _getRarerPrice() internal view returns (uint256 _rarerPrice) {
        try IOracle(rarerOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Rarer: failed to fetch RARER price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _rarerPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_rarerPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRarerOracle(address _rarerOracle) public onlyOperatorOrTaxOffice {
        require(_rarerOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints RARER to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of RARER to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRarerPrice = _getRarerPrice();
            currentTaxRate = _updateTaxRate(currentRarerPrice);
            if (currentRarerPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_rarerPool != address(0), "!_rarerPool");
         require(_daoFund != address(0), "!_treasury");
        rewardPoolDistributed = true;
        _mint(_rarerPool, INITIAL_RARER_POOL_DISTRIBUTION);
