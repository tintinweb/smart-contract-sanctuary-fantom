...
contract TokenManager {
  ...
  error OnlyDeployer(address fraxlendPairCore);

  modifier onlyDeployer(address fraxlenPairCore) {
    if (msg.sender != IFraxlendPairCore(fraxlenPairCore).DEPLOYER_ADDRESS()) {
      revert OnlyDeployer(fraxlenPairCore);
    }
    _;
  }

  function defineCollaterals(
    address fraxlendPairCore,
    address[] memory tokens,
    address[] memory oracles,
    uint256[] memory ltvs
  ) external onlyDeployer(fraxlendPairCore) {
    ...
    IFraxlendPairCore(fraxlendPairCore).defineCollaterals(collaterals);
  }
  ...
  function defineAssets(
    ...
    for (uint256 i; i < tokens.length; i++) {
      ...
      string memory fTokenName = string.concat('Fujin ', IFraxlendPairCore(fraxlendPairCore).name(), ' ', IERC20Metadata(token).name());
      string memory fTokenSymbol = string.concat(IFraxlendPairCore(fraxlendPairCore).name(), ' f', IERC20Metadata(token).symbol());

      FToken fToken = new FToken(fTokenName, fTokenSymbol);
      fToken.transferOwnership(fraxlendPairCore);
