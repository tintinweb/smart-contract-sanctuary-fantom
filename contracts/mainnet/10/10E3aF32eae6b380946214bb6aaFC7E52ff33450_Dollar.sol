...
contract Dollar is ERC20Burnable, Operator {
    ...
    // total of dollar we pay to users during genesis
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 100000 ether;

    // DAO FUND INITIAL ALLOCATION IS 5000 DOLLAR
    uint256 public constant INITIAL_DAOFUND_DISTRIBUTION = 5000 ether;
    ...
    /**
     * @notice Constructs the DOLLAR ERC-20 contract.
     */
    constructor() ERC20("DOLLAR", "DOLLAR") {
        // Mints 5000 DOLLAR to contract creator for initial pool setup
        dollarPriceOne = 10**18;
        _mint(msg.sender, 5000 ether);

    }

    function _getDollarPrice() internal view returns (uint256 _dollarPrice) {
        try IOracle(dollarOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Dollar: failed to fetch DOLLAR price from Oracle");
        }
    }

    function setChanged() external onlyOperator {
        require(!changed, "Cant change again");
        changed = true; 
    }

    function setDollarOracle(address _dollarOracle) public onlyOperator {
        require(_dollarOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        if (changed) {
            uint256 dollarPrice = _getDollarPrice();
            if (dollarPrice < dollarPriceOne) {
                bool isSell = poolList[recipient];
                bool whitelisted = whitelist[msg.sender];
                require(!isSell || whitelisted, "Can't sell if below peg");
            }
        }

        _transfer(_msgSender(), recipient, amount);
        _approve(_msgSender(), _msgSender(), allowance(_msgSender(), _msgSender()).sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        if (changed) {
            ...
            if (dollarPrice < dollarPriceOne) {
                bool isSell = poolList[recipient];
                bool whitelisted = whitelist[sender];
                require(!isSell || whitelisted, "Can't sell if below peg");
            }
        }
    ...
    function setWhitelist(
        ...
        whitelist[_addy] = _isWhitelist;
    }

    function setPoolList(
        address _addy,
        bool _isSell
    ) external onlyOwner {
        poolList[_addy] = _isSell;
    }
}
