...
contract Treasury is ContractGuard {
    ...
    event BoughpBonds(address indexed from, uint256 tokenAmount, uint256 bondAmount);
    ...
    modifier checkEpoch {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getTokenPrice() > tokenPriceCeiling) ? 0 : IERC20(token).totalSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    modifier checkOperator {
        require(
            IBasisAsset(token).operator() == address(this) &&
                ...
                IBasisAsset(stoken).operator() == address(this) &&
    ...
    function getBurnableTokenLeft() public view returns (uint256 _burnableTokenLeft) {
        ...
        if (_tokenPrice <= tokenPriceOne) {
            uint256 _tokenSupply = IERC20(token).totalSupply();
    ...
    function initialize(
        ...
        maxSupplyExpansionPercent = 450; // Upto 4.0% supply for expansion

        bondDepletionFloorPercent = 10000; // 100% of Bond supply for depletion floor
        seigniorageExpansionFloorPercent = 3500; // At least 35% of expansion reserved for masonry
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn TOKEN and mint pBOND)
    ...
    function setExtraFunds(
        ...
        require(_devFund5 != address(0), "zero");
    ...
    function buyBonds(uint256 _tokenAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        emit BoughpBonds(msg.sender, _tokenAmount, _bondAmount);
    }
    ...
    function _sendToMasonry(uint256 _amount) internal {
        ...
        if (devFundSharedPercent > 0) {
            ...
            IERC20(token).transfer(devFund1, _devFundSharedAmount.div(5));
            IERC20(token).transfer(devFund2, _devFundSharedAmount.div(5));
            IERC20(token).transfer(devFund3, _devFundSharedAmount.div(5));
            IERC20(token).transfer(devFund4, _devFundSharedAmount.div(5));
            IERC20(token).transfer(devFund5, _devFundSharedAmount.div(5));
            emit DevFundFunded(now, _devFundSharedAmount);
        }

        _amount = _amount.sub(_daoFundSharedAmount).sub(_devFundSharedAmount);

        IERC20(token).safeApprove(masonry, 0);
        IERC20(token).safeApprove(masonry, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        uint256 tokenSupply = IERC20(token).totalSupply().sub(seigniorageSaved);
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _sendToMasonry(tokenSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochTokenPrice > tokenPriceCeiling) {
                // Expansion ($TOKEN Price > 1 $FTM): there is some seigniorage to be allocated
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForMasonry = tokenSupply.mul(_percentage).div(1e18);
                } else {
                ...
                if (_savedFopBond > 0) {
                    ...
                    IBasisAsset(token).mint(address(this), _savedFopBond);
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(stoken), "stoken");
        _token.safeTransfer(_to, _amount);
    }

    function tokenSetOperator(address _operator) external onlyOperator {
        IBasisAsset(token).transferOperator(_operator);
    }

    function stokenSetOperator(address _operator) external onlyOperator {
        IBasisAsset(stoken).transferOperator(_operator);
    }
