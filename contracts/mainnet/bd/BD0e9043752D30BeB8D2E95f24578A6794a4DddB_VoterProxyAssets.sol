    ...
    ProxyImplementation
{
    ...
    modifier onlyGovernanceOrVoterProxyOrOperator() {
        require(
            msg.sender == governanceAddress() ||
                operator[msg.sender] ||
                msg.sender == voterProxyAddress,
            "Only governance, voter proxy, or operator"
        );
        _;
    }
        ...
        onlyVoterProxy
    {
         ...
         * This method will only be called by voterProxy after claimSolid is successful
         */

        uint256 amount = ISolidPool(solidPoolAddress).balanceOf(address(this));

        if (amount > 0) {
            // Deposit Solidly LP into gauge
            ISolidPool(solidPoolAddress).approve(gaugeAddress, amount);
            ISolidGauge(gaugeAddress).deposit(amount, primaryTokenId);
        }
    }

    /**
     * @notice Withdraw Solidly LP from a gauge and approves the amount to voterProxy
    ...
    function withdrawFromGauge(
        ...
        if (
            ISolidPool(solidPoolAddress).allowance(
                address(this),
                voterProxyAddress
            ) == 0
        ) {
            ISolidPool(solidPoolAddress).approve(
                voterProxyAddress,
                uint256(2**256 - 1)
            );
        }
        ...
        returns (uint256 amountClaimed)
    {
        ...
        uint256 _balanceBefore = IERC20(solidAddress).balanceOf(address(this));

        IGauge(gaugeAddress).getReward(address(this), solidAddressInArray);

        amountClaimed = IERC20(solidAddress).balanceOf(address(this)).sub(
            _balanceBefore
        );
    }
     ...
     * @param _stakingAddress Address of oxPool multiRewards
    ...
    function getRewardFromBribe(
        ...
        if (primaryTokenId > 0) {
            ...
            uint256 solidBefore = IERC20(solidAddress).balanceOf(address(this));
     ...
     * @param stakingAddress Address of oxPool multiRewards
    ...
    function getRewardFromGauge(
        ...
        ISolidGauge(gaugeAddress).getReward(address(this), claimableAddresses);
        for (
            uint256 tokenIndex;
            tokenIndex < claimableAddresses.length;
            tokenIndex++
    ...
    function approveSolidToVoterProxy() external {
        ISolid(solidAddress).approve(voterProxyAddress, uint256(2**256 - 1));
    }

    // used during migration
    function approveLPToVoterProxy(address solidPoolAddress, uint256 amount)
        external
        onlyVoterProxy
    {
        ISolidPool(solidPoolAddress).approve(voterProxyAddress, amount);
    }
     ...
     */
    function lockSolid(uint256 amount) external onlyVoterProxy {
        ISolid solid = ISolid(solidAddress);
        // Solid will have been transfered from voterProxy
    ...
    function setPrimaryTokenId() external onlyGovernanceOrVoterProxyOrOperator {
        //Sunsetting, set primaryTokenId = 0
        primaryTokenId = 0;
        return;
    ...
    function onERC721Received(
        ...
        require(false, "Sunsetting on Fantom"); // Sunsetting on Fantom, reverts the tx
    ...
    ****************************************/
    ...
    function detachNFT(address[] memory gaugeAddresses)
        external
        onlyGovernanceOrVoterProxy
    {
        ...
        for (uint256 i = 0; i < gaugeAddresses.length; i++) {
            ...
            ISolidGauge gauge = ISolidGauge(gaugeAddresses[i]);
    ...
    function execute(
        ...
        bytes calldata data,
        Operation operation
    ) external onlyGovernanceOrVoterProxyOrOperator returns (bool success) {
