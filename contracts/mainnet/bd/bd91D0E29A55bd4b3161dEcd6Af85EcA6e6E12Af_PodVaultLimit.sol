 ...
 */
contract PodVaultLimit is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public depositLimit = 1000000;
    ...
    function getPricePerFullShare() public view returns (uint256) {
        uint256 one = 10**tokenDecimal;
        return totalSupply() == 0 ? one : balance().mul(one).div(totalSupply());
    }
    ...
    function depositAll() external {
        ...
        if (balance().add(amount) > depositLimit) {
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        require(balance().add(_amount) <= depositLimit, "amount gt limit");
