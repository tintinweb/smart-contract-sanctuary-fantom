...
contract GUARDIAN {
    ...
    constructor() public{
        ...
        ceoAddress1=address(0x5Ddf1b081957E6FaCf52120FADD6992514631abD);
        ceoAddress2=address(0x5Ddf1b081957E6FaCf52120FADD6992514631abD);
    }
    ...
    function catchFishes() public {
        ...
        ERC20(blc).transfer(ceoAddress, fee2);
        ERC20(blc).transfer(ceoAddress1, fee2);
        ERC20(blc).transfer(ceoAddress2, fee2);
        ERC20(blc).transfer(address(msg.sender), SafeMath.sub(fishValue,fee));
    }
    function buyFisherman(address ref, uint256 amount) public {
        require(initialized);
    
        ERC20(blc).transferFrom(address(msg.sender), address(this), amount);
        
        uint256 balance = ERC20(blc).balanceOf(address(this));
    ...
    function calculateMoneyClaim(uint256 printers) public view returns(uint256) {
        return calculateTrade(printers,marketFishes,ERC20(blc).balanceOf(address(this)));
    }
    ...
    function calculatePrinterBuySimple(uint256 eth) public view returns(uint256){
        return calculatePrinterBuy(eth,ERC20(blc).balanceOf(address(this)));
    }
    ...
    function getBalance() public view returns(uint256) {
        return ERC20(blc).balanceOf(address(this));
    }
    ...
    function getEstimateFishes(uint256 amount) public view returns(uint256) {
        ...
        fishermanWillBuy = SafeMath.sub(fishermanWillBuy, devFee(fishermanWillBuy));
        uint256 fPrinterUsed = SafeMath.add(fishermanWillBuy, getMyFish());
        uint256 fNewPrinters= SafeMath.div(fPrinterUsed, FISH_TO_CATCH_1);
        return SafeMath.add(catchFishes[msg.sender], (fNewPrinters));
    }
