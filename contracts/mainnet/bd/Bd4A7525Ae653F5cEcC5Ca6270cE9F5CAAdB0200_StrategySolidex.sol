 ...
 */
contract StrategySolidex is StratManager, FeeManager {
    ...
    function _chargeFees(address callFeeRecipient) internal {
        ...
        IERC20(wftm).safeTransfer(callFeeRecipient, callFeeToUser);

        uint256 treasuryFeeToVault = totalFeeInWftm.mul(treasuryFee).div(
            PERCENT_DIVISOR
        );
        for (uint256 i = 0; i < revenueReceivers.length; i++) {
            uint256 shareRevenue = treasuryFeeToVault
                .mul(revenuePercentage[i])
                .div(PERCENT_DIVISOR);
            IERC20(wftm).safeTransfer(revenueReceivers[i], shareRevenue);
        }
    }
    ...
    function _earnToWFTM(address _earnedToken) internal {
        ...
        ISolidlyRouter(unirouter).swapExactTokensForTokensSimple(
            earnedBal,
            0,
            _earnedToken,
            wftm,
    ...
    function _addLiquidity() internal {
        ...
        uint256 wftmTokenHalf = wftmBal.div(2);
    ...
    function _giveAllowances() internal {
        ...
        IERC20(sex).safeApprove(unirouter, 0);
        IERC20(sex).safeApprove(unirouter, max);

        IERC20(solid).safeApprove(unirouter, 0);
        IERC20(solid).safeApprove(unirouter, max);
        ...
        IERC20(lpToken1).safeApprove(unirouter, max);
    }
