...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x791DB65f7A7772D0808d56b76F267126C8ab6612) // EndgameGenesisPool
    ];
    ...
    function getBurnableEndgameLeft() public view returns (uint256 _burnableEndgameLeft) {
        ...
        if (_endgamePrice <= endgamePriceOne) {
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableEndgame = _maxMintableBond.mul(_endgamePrice).div(1e18);
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_endgamePrice > endgamePriceCeiling) {
            ...
            if (_rate > 0) {
                _redeemableBonds = _totalEndgame.mul(1e18).div(_rate);
            }
        }
    }
    ...
    function initialize(
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn ENDGAME and mint eBOND)
    ...
    function buyBonds(uint256 _endgameAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bondAmount = _endgameAmount.mul(_rate).div(1e18);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _endgameAmount = _bondAmount.mul(_rate).div(1e18);
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        ...
        IERC20(endgame).safeApprove(boardroom, 0);
        IERC20(endgame).safeApprove(boardroom, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToBoardroom(endgameSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochEndgamePrice > endgamePriceCeiling) {
                // Expansion ($ENDGAME Price > 1 $FTM): there is some seigniorage to be allocated
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForBoardroom = endgameSupply.mul(_percentage).div(1e18);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = endgameSupply.mul(_percentage).div(1e18);
