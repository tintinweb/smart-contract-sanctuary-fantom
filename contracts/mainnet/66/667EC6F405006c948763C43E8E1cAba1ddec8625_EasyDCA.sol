...
contract EasyDCA is Ownable {
    ...
    function executeDCA(uint256 _dcaIndex) internal {
        ...
        } catch { // If purchase can't happen disable the DCA order
            dcaList[_dcaIndex].isActive = false;
        }
    }
    ...
    function checkUpkeep(
        ...
        upkeepNeeded = (dcaList[dcaHeap[0]].lastPurchase + dcaList[dcaHeap[0]].frequency) < block.timestamp;
    }

    function performUpkeep(bytes calldata) public {
        if ((dcaList[dcaHeap[0]].lastPurchase + dcaList[dcaHeap[0]].frequency) < block.timestamp) {
