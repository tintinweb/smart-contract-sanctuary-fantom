...
// Original source tomb-finance/contracts MIT license
...
import "../interfaces/ISeigniorage.sol";
...
\     \___\  ___/|  |_\  ___/ \___ \  |  | |  |/ __ \|  |__
 \______  /\___  >____/\___  >____  > |__| |__(____  /____/
  ...
  The Masonry is based on the original BasisCash/Tomb Masonry contract. 
  Its role is to provide the Celestial token staking and reward mechanism:
    - to permenently print HALO to xHALO
    - to print HALO to stakers when above peg
*/

contract Masonry is ShareWrapper, ContractGuard, Ownable {
  ...
  modifier masonExists() {
    require(balanceOf(msg.sender) > 0, "The mason does not exist");
    _;
  }
  ...
  function nextEpochTimestamp() external view returns (uint256) {
    return treasury.nextEpochTimestamp();
  }

  function getHaloPrice() external view returns (uint256) {
    return treasury.getHaloPrice();
  }
  ...
  function withdraw(uint256 amount) public override onlyOneBlock masonExists updateReward(msg.sender) {
    ...
    require(masons[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <= treasury.epoch(), "Masonry: in withdraw lockup");
    claimReward();
  ...
  function claimReward() public updateReward(msg.sender) {
    ...
    if (reward > 0) {
      ...
      halo.safeTransfer(msg.sender, reward);
      emit RewardPaid(msg.sender, reward);
    }
  }

  /**
   * This works the same as the original function but the tokens are now pushed into the Masonry
   * via Seigniorage.distributeHalo() instead of being transferred in via halo.safeTransferFrom
   */
  function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOwner {
    require(amount > 0, "Masonry: Cannot allocate 0");
    require(totalSupply() > 0, "Masonry: Must have supply");
    ...
    // the amount has already been trasferred in!
    // halo.safeTransferFrom(msg.sender, address(this), amount);
  ...
  function governanceRecoverUnsupported(
    ...
    require(address(_token) != address(halo), "halo");
    require(address(_token) != address(celestial), "celestial");
...
\     \___\  ___/|  |_\  ___/ \___ \  |  | |  |/ __ \|  |__
 \______  /\___  >____/\___  >____  > |__| |__(____  /____/
  ...
  The ContractGuard is based on the original BasisCash/Tomb ContractGuard contract. 
  Its role is to guard against multiple actions per block
*/
...
\     \___\  ___/|  |_\  ___/ \___ \  |  | |  |/ __ \|  |__
 \______  /\___  >____/\___  >____  > |__| |__(____  /____/
  ...
  The ShareWrapper is based on the original BasisCash/Tomb ShareWrapper contract. 
  Its role is to provide masonry safe functions when performing token based functionality.
*/
...
contract ShareWrapper {
  ...
  function stake(uint256 amount) public virtual {
    ...
    celestial.safeTransferFrom(msg.sender, address(this), amount);
  }
  ...
  function withdraw(uint256 amount) public virtual {
    ...
    celestial.safeTransfer(msg.sender, amount);
  }
}
