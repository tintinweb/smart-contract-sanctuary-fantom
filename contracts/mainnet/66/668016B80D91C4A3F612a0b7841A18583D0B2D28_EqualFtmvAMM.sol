...
// File: contracts/EqualFtmvAMM.sol
...
contract EqualFtmvAMM is Ownable, Pausable {
    ...
    function addLiquidity() internal {
        uint256 wrappedHalf = IERC20(equal).balanceOf(address(this)) / 2;

        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, equalToWrappedPath, address(this), block.timestamp);
        
        uint256 lp0Bal = IERC20(equal).balanceOf(address(this));
        uint256 lp1Bal = IERC20(wrapped).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(equal, wrapped, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
