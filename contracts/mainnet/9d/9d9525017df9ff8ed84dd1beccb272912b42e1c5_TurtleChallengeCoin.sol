...
// File contracts/gTRTLHolder/TurtleChallengeCoin.sol

contract TurtleChallengeCoin is ERC721Omnichain {
    enum CoinType {
        NORMAL,
        SUPERTURTLE
    }

    mapping(uint256 => CoinType) private coinTypeMapping;
    string private normalTurtle = "ipfs://QmaMxff8mk6NcuuWctsgUEpFBKqC74uqVyRbxxDRa7A2gQ";
    string private superTurtle = "ipfs://QmWoGYxoMAfzEtR8Ac8Zve8QokxELDFvTYhoqgRMeEca43";

    constructor()
        ERC721("TurtleCoin Challenge Coin", "TRTLCoin")
        ClampedRandomizer(0)
        LzApp(0xb6319cC6c8c27A8F5dAF0dD3DF91EA35C4720dd7)
    {
        _setDefaultRoyalty(0x7E6CaC527cc907f70dA4Ef514DA285046e75ba28, 1000); // 10% royalty
    }

    function mint(address[] memory owners, CoinType _type) public onlyOwner {
        for (uint256 i = 0; i < owners.length; i++) {
            uint256 next = totalSupply() + 1;
            coinTypeMapping[next] = _type;
            _safeMint(owners[i], next);
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (coinTypeMapping[tokenId] == CoinType.SUPERTURTLE) {
            return superTurtle;
        }
