...
contract AggregationTransection_FTM_TOMB_BASED is Withdrawable {
	...
	bytes4 private constant FUNC_SELECTOR_REMOVE_LIQUIDITY = bytes4(keccak256("removeLiquidity(address,address,uint,uint,uint,address,uint)"));
...
/*		bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR_SWAP_EXACT_TOKENS_FOR_TOKENS, amountIn, amountOutMin, path, to, deadline);
...
/**			bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR_ADD_LIQUIDITY,
			...
			return returnData;
*/
            IUniswapV2Router01(Spookyswap_ROUTER).addLiquidity(tokenA, 
            tokenB,
			amountADesired,
			amountBDesired,
			amountAMin,
...
/*			bytes memory data = abi.encodeWithSelector(FUNC_SELECTOR_REMOVE_LIQUIDITY,
			...
			return returnData;
*/
            IUniswapV2Router01(Spookyswap_ROUTER).removeLiquidity(tokenA, 
            tokenB,
			liquidityAmount,
			amountAMin,
			amountBMin,
