...
import "./interface/IBorrowable.sol";
import "./interface/IRouter02.sol";

/**
    ImpermaxV1Base is the IV implementation that targets Impermax lending.
*/
contract ImpermaxV1Base is InvestmentVehicleSingleAssetBaseV1Upgradeable {

    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    address public impermaxRouter;
    address public borrowableAddress;

    /// initialize the iv with impermax router and its respective base asset
    /// @param _store the address of system storage
    /// @param _baseAsset the address of base asset
    /// @param _impermaxRouter the address of the impermax router 02
    /// @param _impermaxPool the address of the impermax pool
    function initialize(
        address _store,
        address _baseAsset,
        address _impermaxRouter,
        address _impermaxPool
    ) public initializer {
        super.initialize(_store, _baseAsset);
        impermaxRouter = _impermaxRouter;
        address _borrowableAddress = IRouter02(impermaxRouter).getBorrowable(_impermaxPool, 0);
        
        if(IBorrowable(_borrowableAddress).underlying() != _baseAsset ) {
            _borrowableAddress = IRouter02(impermaxRouter).getBorrowable(_impermaxPool, 1);
            require(IBorrowable(_borrowableAddress).underlying() == _baseAsset );
        }
        borrowableAddress = _borrowableAddress;       
    }
    ...
    function _investAll() internal override {
        ...
        if(baseAssetAmountInVehicle > 0){
            // Approve router
            IERC20Upgradeable(baseAsset).safeApprove(impermaxRouter, 0);
            IERC20Upgradeable(baseAsset).safeApprove(impermaxRouter, baseAssetAmountInVehicle);
            // deposit
            IRouter02(impermaxRouter).mint(
                borrowableAddress,
    ...
    function _pullFundsFromInvestment(uint256 _baseAmount) internal override{
        uint256 owned = IBorrowable(borrowableAddress).balanceOf(address(this));
        uint256 withdrawing = MathUpgradeable.min(owned, _baseAmount);
        uint256 withdrawingBorrowable = baseAssetToBorrowable(_baseAmount);
        
        IBorrowable(borrowableAddress).approve(impermaxRouter, withdrawingBorrowable);

        bytes memory _permit;

        IRouter02(impermaxRouter).redeem(
            borrowableAddress,
            baseAssetToBorrowable(_baseAmount),
            address(this),
            block.timestamp,
            _permit
        );
    ...
    function updateProfit() public {
        IBorrowable(borrowableAddress).exchangeRate();
    }
    ...
    function profitsPending() public view override returns (uint256) {
        ...
        uint256 impermaxBalance = invested();
        uint256 totalBalance = ivBalance.add(impermaxBalance);
    ...
    /** View functions */
    function borrowableToBaseAsset(uint256 _borrowableAmount) public view returns (uint256) {
        uint256 exchangeRate = IBorrowable(borrowableAddress).exchangeRateLast();
        return _borrowableAmount.mul(exchangeRate).div(1e18);
    }

    function baseAssetToBorrowable(uint256 _baseAmount) public view returns (uint256) {
        uint256 exchangeRate = IBorrowable(borrowableAddress).exchangeRateLast();
        return _baseAmount.mul(1e18).div(exchangeRate);
    }

    function invested() public view override returns (uint256){
        return borrowableToBaseAsset(
            IBorrowable(borrowableAddress).balanceOf(address(this))
        );
    }
