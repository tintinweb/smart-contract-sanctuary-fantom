...
// File: contracts/Unreality Froyd Originals.sol
...
contract UnrealityFroydOrigins is ERC721Enumerable {
  /**
  @dev events
 */
 ...
 */
  ...
  function contractURI() public view returns (string memory) {
    return
      string(
        abi.encodePacked(baseURI, 'QmfRKZTFK9H3zMMCy43Zfju1L71aJNeXtZWxj6EoprceaV')
      );
  }
 ...
 */
  ...
  function mintFroyd(string memory _uri) public onlyAdmin returns (uint256 _tokenId) {
    return mintFroydTo(msg.sender, _uri);
  }

  function mintFroydTo(address _to, string memory _uri) public onlyAdmin returns (uint256 _tokenId) {
  ...
  function redeemFroyd(address _to) public returns (uint256 _tokenId) {
    require(nextTokenId <= maxTokenId, 'Must not exceed maximum mint on Fantom');
    require(!isMintPaused || msg.sender == admin, 'Purchases must not be paused');
    require(
