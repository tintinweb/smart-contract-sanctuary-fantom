...
contract StrategyMasterChefLP is StratManager, FeeManager, GasThrottler {
    ...
    address constant public bsm = address(0xB214d491a58a250A99f11cE01C361E7fAd4d3E69);
    address public want;
    ...
    address[] public outputToBsmRoute = [wrapped, bsm];
    ...
    function chargeFees() internal {
        uint256 toWrapped = IERC20(output).balanceOf(address(this)).mul(30).div(1000);
        ...
        // Buy BSM and burn BSM
        uint256 wrappedBuyBsm = wrappedBal.div(2);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(wrappedBuyBsm, 0, outputToBsmRoute, address(this), now);
        burn();

        uint256 wrappedFee = wrappedBal.sub(wrappedBuyBsm);

        uint256 callFeeAmount = wrappedFee.mul(callFee).div(MAX_FEE);
        IERC20(wrapped).safeTransfer(msg.sender, callFeeAmount);

        uint256 batasmFeeAmount = wrappedFee.mul(batasmFee).div(MAX_FEE);
        IERC20(wrapped).safeTransfer(batasmFeeRecipient, batasmFeeAmount);

        uint256 strategistFee = wrappedFee.mul(STRATEGIST_FEE).div(MAX_FEE);
        IERC20(wrapped).safeTransfer(strategist, strategistFee);
    }

    function burn() internal {
        uint256 toBurn = IERC20(bsm).balanceOf(address(this));
