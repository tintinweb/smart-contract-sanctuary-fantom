...
contract MultiAssetPool is ReentrancyGuard, IMultiAssetPool {
    ...
    function calcMint(uint256 _collateralAmount, uint256 _share_amount, uint256 _assetId, uint256 _missing_decimals) public view returns (
        ...
        //            uint256 _collateral_value = calcCollateralValue(_collateralAmount, _assetId, isStable);
        //
        ...
        //        } else {
        //            _actual_asset_amount = _collateralAmount.mul(COLLATERAL_RATIO_PRECISION).div(_target_collateral_ratio);
        //        }
        //
        //        _fee_collect = _collateralAmount.mul(_minting_fee).div(PRICE_PRECISION);
    }
    ...
    function calcRedeem(uint256 _asset_amount, uint256 _assetId) public view returns (
        ...
        uint256 _asset_amount_post_fee = _asset_amount.sub(_fee);
        _collateral_output_amount = _asset_amount_post_fee.mul(_ecr).div(10 ** _missing_decimals).div(PRICE_PRECISION);

        uint256 _collateral_price = IAssetController(assetController).getCollateralPriceInDollar(_assetId);
        uint256 _share_output_value = (_asset_amount_post_fee.sub(_collateral_output_amount)).mul(_collateral_price).div(PRICE_PRECISION);
        ...
        //            uint256 _share_output_value = _asset_amount_post_fee - (_asset_amount_post_fee * _ecr / PRICE_PRECISION);
        //            _share_output_amount = _share_output_value.mul(PRICE_PRECISION).div(_share_price);
        //        }
        //
        //        if (_ecr > 0) {
        //            if (isStable) {
        //                _collateral_output_value = _asset_amount_post_fee * _ecr / (10 ** _missing_decimals) / PRICE_PRECISION;
        //                _collateral_output_amount = _collateral_output_value;
        //            } else {
        //                _collateral_output_value = _asset_amount_post_fee * _ecr / PRICE_PRECISION;
        //                uint256 _collateral_price = IAssetController(assetController).getCollateralPriceInDollar(_assetId);
        //                _collateral_output_amount = _collateral_output_value * PRICE_PRECISION / _collateral_price;
        //            }
        //        }
        //
        //        _fee_collect = _asset_fee.mul(_ecr).div(10 ** _missing_decimals).div(PRICE_PRECISION);
    }
