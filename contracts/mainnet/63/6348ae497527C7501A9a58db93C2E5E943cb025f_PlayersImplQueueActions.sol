...
contract PlayersImplRewards is PlayersImplBase, PlayersBase, IPlayersRewardsDelegateView {
    ...
    )
  {
    ...
    for (U256 iter; iter < pendingRandomRewardsLength; iter = iter.inc()) {
      ...
      (ActionRewards memory actionRewards, Skill actionSkill, uint numSpawnedPerHour,) = world
...
contract World is VRFConsumerBaseV2Upgradeable, UUPSUpgradeable, OwnableUpgradeable {
  ...
  function getRewardsHelper(uint16 _actionId) external view returns (ActionRewards memory, Skill skill, uint numSpanwed, uint8 worldLocation) {
    return (actionRewards[_actionId], actions[_actionId].skill, actions[_actionId].numSpawned, actions[_actionId].worldLocation);
  }
