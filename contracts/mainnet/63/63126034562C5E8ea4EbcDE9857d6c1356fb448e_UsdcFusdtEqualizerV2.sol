...
// File contracts/UsdcFusdtEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract UsdcFusdtEqualizerV2 is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToFusdtPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToFusdtPath,
        ...
        for (uint i; i < _equalToFusdtPath.length; ++i) {
            equalToFusdtPath.push(_equalToFusdtPath[i]);
        }
    ...
    function chargeFees(address caller) internal {
       uint256 toFee = IERC20(equal).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        ...
        uint256 fusdtDecimals = 10**IERC20Extended(fusdt).decimals();
        uint256 usdcDecimals = 10**IERC20Extended(usdc).decimals();
        uint256 fusdtOut = IEqualizerRouter(router).getAmountsOut(equalHalf, equalToFusdtPath)[equalToFusdtPath.length] * 1e18 / fusdtDecimals;
        uint256 usdcOut = IEqualizerRouter(router).getAmountsOut(equalHalf, equalToUsdcPath)[equalToUsdcPath.length] * 1e18 / usdcDecimals;
        (uint256 amount0, uint256 amount1,) = IEqualizerRouter(router).quoteAddLiquidity(fusdt, usdc, stable, fusdtOut, usdcOut);
        amount0 = amount0 * 1e18 / fusdtDecimals;
        amount1 = amount1 * 1e18 / usdcDecimals;
        uint256 ratio = fusdtOut * 1e18 / usdcOut * amount0 / amount1;
        uint256 fusdtRatio = equalHalf * 1e18 / (ratio + 1e18);
        uint256 usdcRatio = equalHalf - fusdtRatio;

        IEqualizerRouter(router).swapExactTokensForTokens(fusdtRatio, 0, equalToFusdtPath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokens(usdcRatio, 0, equalToUsdcPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(fusdt).balanceOf(address(this));
        uint256 t2Bal = IERC20(usdc).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(fusdt, usdc, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(fusdt).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(fusdt).safeApprove(router, type(uint).max);
    }
