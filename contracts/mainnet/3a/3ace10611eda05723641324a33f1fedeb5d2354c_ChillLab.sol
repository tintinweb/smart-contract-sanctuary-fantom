...
contract ChillLab is ChillPillRarity, ReentrancyGuard {
    ...
    uint public mpPerSchill = 300 * 1000;
    ...
    uint32 public chillpillBoost = 1000;
    ...
    constructor(IERC20 _schill, IERC721 _chillpill) {
        schill = _schill;
    ...
    function accRewardPerShareChillpills(uint _rewardsPerShare) internal view returns (uint){
        return _rewardsPerShare/(10000-chillpillBoost);
    }
    ...
    function pendingRewards(uint _pid, address _user) public view returns (uint schillReward, uint chillpillReward) {
        ...
        uint accRewardPerShareChillpill = accRewardPerShareChillpills(accRewardPerShare);

        if (block.timestamp > pool.lastRewardTime) {
            uint reward = pool.RewardPerSecond * getMultiplier(pool.lastRewardTime, block.timestamp, pool.startTime, pool.endTime);
            if(pool.sChillStakedAmount != 0) accRewardPerShare += reward * (10000 - chillpillBoost) / 10000 * precisionOf[pool.TokenPrecision] / pool.sChillStakedAmount;
        }
        schillReward = (user.amount * accRewardPerShare / precisionOf[pool.TokenPrecision]) - user.rewardDebt;
        chillpillReward = (effectiveMP(user.amount, user.mp) * accRewardPerShareChillpill / precisionOf[pool.TokenPrecision]) - user.pillDebt;
    }
    ...
    function _deposit(uint _pid, uint _amount_before_fees, address to, uint[] memory tokenIDs) internal {
        ...
        uint pendingPill = effectiveMP(amount, user.mp) * accRewardPerShareChillpills(pool.accRewardPerShare) / precision - user.pillDebt;
        ...
        if(len == 0) {
            user.pillDebt = effectiveMP(amount, user.mp) * accRewardPerShareChillpills(pool.accRewardPerShare) / precision;
            return;
        }
