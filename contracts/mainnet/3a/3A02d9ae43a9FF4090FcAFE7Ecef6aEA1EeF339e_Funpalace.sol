...
https://funpalace.finance

*/

contract Funpalace is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;    

      // Supply used to launch in LP
    uint256 public constant INITIAL_LAUNCH_DISTRIBUTION = 350 ether;
    ...
    address public funpalaceOracle;
    ...
    /**
     * @notice Constructs the FUNPALACE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Funpalace", "Funpalace") {
        // Mints 1 FUNPALACE to contract creator for initial pool setup
    ...
    function _getFunpalacePrice() internal view returns (uint256 _funpalacePrice) {
        try IOracle(funpalaceOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Funpalace: failed to fetch FUNPALACE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _funpalacePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_funpalacePrice >= taxTiersTwaps[tierId]) {
    ...
    function setFunpalaceOracle(address _funpalaceOracle) public onlyOperatorOrTaxOffice {
        require(_funpalaceOracle != address(0), "oracle address cannot be 0 address");
        funpalaceOracle = _funpalaceOracle;
    }
    ...
    /**
     * @notice Operator mints FUNPALACE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FUNPALACE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFunpalacePrice = _getFunpalacePrice();
            currentTaxRate = _updateTaxRate(currentFunpalacePrice);
            if (currentFunpalacePrice < burnThreshold) {
                burnTax = true;
            }
        }
