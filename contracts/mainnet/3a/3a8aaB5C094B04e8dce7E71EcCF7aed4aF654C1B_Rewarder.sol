...
contract Rewarder is ReentrancyGuard {
    ...
    function balanceOfAt(address _owner, uint256 _timestamp) public view returns (uint256) {
        return IMirroredVotingEscrow(ve).balanceOf(_owner, _timestamp);
    }
    ...
    function earned(address _owner) public view returns(uint256[] memory){
        ...
        for(i; i < rewardTokens.length; i++){
            ...
            for(k; k < 50; k++){
                reward[i] += _getReward(_owner, _rewardToken, _userLastTime);
    ...
    function earned2(address _owner, address _rewardToken) public view returns(uint256){
        uint k = 0;
        uint reward = 0;
        uint256 _lastStartTimestamp = lastUpdate[_rewardToken] / DURATION * DURATION;
        uint256 _userLastTime = userTimestamp[_owner][_rewardToken];
    ...
    function _getReward(address _owner, address _rewardToken, uint256 _timestamp) internal view returns (uint256) {
        uint256 _balance = balanceOfAt(_owner, _timestamp);
        if(_balance == 0){
            return 0;
        } else {
    ...
    function claim2(address to) external nonReentrant {
        require(!locked[msg.sender]);
       _claim(msg.sender, to);
    }

    function _claim(address _owner, address _to) internal {
    ...
    function _checkpoint(address _token) private {
        if(isRewardToken[_token]){
            ...
            if(block.timestamp >= lastPeriodFinish) {
                ...
                uint reward = IERC20(_token).balanceOf(address(this)) - lastBalance[_token];
                if(reward > 0){
                    ...
                    lastBalance[_token] = reward + _lastBalance;
