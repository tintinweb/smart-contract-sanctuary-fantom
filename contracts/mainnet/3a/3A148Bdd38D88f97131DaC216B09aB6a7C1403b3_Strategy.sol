...
contract Strategy is BaseStrategy {
    ...
    IUniPair public lpToken; // SpiritSwap: fUSDT-USDC LP
    address public lpToken0; // Want
    address public lpToken1; // Pair
    ...
    IERC20 public rewardToken = IERC20(0x5Cc61A78F164885776AA610fb0FE1257df78E59B);
    ...
    constructor(
        ...
        wantDust = uint256(1).mul(10**uint256(ERC20(address(want)).decimals())); // 1 USDC
        rewardsDust = uint256(1).mul(10**uint256(ERC20(address(rewardToken)).decimals())); // 1 SPIRIT
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('ss_Spirit_USDC-fUSDT_Curve_strategy'));
    }
    ...
    function pendingRewardsScaled() public view returns (uint256 _shares) {
        _shares = masterChef.pendingSpirit(masterChefPoolId, address(this)).div(1e18);
    }

    function pendingRewards() public view returns (uint256 _shares) {
        _shares = masterChef.pendingSpirit(masterChefPoolId, address(this));
    }
    ...
    function balanceOfPooled() public view returns (uint256 _amount) {
        ...
        uint256 lpToken1InWant =
            lpToken1Amount == 0
                ? 0
                : ICurveFi(curvePoolSwap).get_dy_underlying(
                    outTokenIndex,
                    inTokenIndex,
                    lpToken1Amount
                );
        ...
        returns (uint256 swapAmountIn, uint256 swapAmountOut)
    {
        ...
        uint256 swapOutSecurityRatio = swapAmountIn.mul(swapSecurityRatio).div(basisOne);
        swapAmountOut = ICurveFi(curvePoolSwap).exchange_underlying(
            inTokenIndex, 
            outTokenIndex, 
            swapAmountIn, 
    ...
    function sellLpToken1() internal {
        ...
        uint256 swapOutSecurityRatio = swapInAmount.mul(swapSecurityRatio).div(basisOne);
        ICurveFi(curvePoolSwap).exchange_underlying(
            outTokenIndex,
            inTokenIndex,
            swapInAmount,
            swapOutSecurityRatio
    ...
    function removeLiquidity(uint256 toExitAmount) internal {
        uint256 lpTokenBalance = balanceOfLPInMasterChef().add(balanceOfLpTokens());
        uint256 lpWorth = balanceOfPooled();
        uint256 percentageOfLpToSell = toExitAmount.mul(1e6).div(lpWorth);
        uint256 liquidityToExit = lpTokenBalance.mul(percentageOfLpToSell).div(1e6);
        uint256 toExit = Math.min(liquidityToExit, lpTokenBalance);
    ...
    function depositLpIntoMasterChef() internal {
        uint256 lpTokenBal = balanceOfLpTokens();
        if (lpTokenBal > 0) {
            masterChef.deposit(masterChefPoolId, lpTokenBal);
        }
    }
    ...
    function getSwapAmount(uint256 _fullInvestment) internal view returns (uint256 _toSwapAmount) {
        ...
        uint256 nominator =
            ICurveFi(curvePoolSwap).get_dy_underlying(inTokenIndex, outTokenIndex, halfInvestment);
    ...
    function setSwapSecurityRatio(uint256 _swapSecurityRatio) public onlyVaultManagers {
        require(_swapSecurityRatio <= basisOne);
