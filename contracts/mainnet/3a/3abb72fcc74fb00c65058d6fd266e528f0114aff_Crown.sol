...
contract Crown is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the CROWN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Crown Token", "CROWN") {
        // Mints 1 CROWN to contract creator for initial pool setup
    ...
    function _getCrownPrice() internal view returns (uint256 _crownPrice) {
        try IOracle(crownOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Crown: failed to fetch CROWN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _crownPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_crownPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCrownOracle(address _crownOracle) public onlyOperatorOrTaxOffice {
        require(_crownOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CROWN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CROWN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCrownPrice = _getCrownPrice();
            currentTaxRate = _updateTaxRate(currentCrownPrice);
            if (currentCrownPrice < burnThreshold) {
                burnTax = true;
            }
        }
