...
contract SPEARToken is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    uint256 public constant INITIAL_SPEAR_POOL_DISTRIBUTION = 30000 ether;
    ...
    /**
     * @notice Constructs the SPEAR ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SPEAR Token", "SPEAR") {
        // Mints 750 SPEAR to contract creator for initial pool setup & LP
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        excludeAddress(address(this));

        _mint(msg.sender, 700 ether);
        taxRate = _taxRate;
    ...
    function _getSPEARPrice() internal view returns (uint256 _SPEARPrice) {
        try IOracle(SPEAROracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("SPEAR: failed to fetch SPEAR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _SPEARPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_SPEARPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSPEAROracle(address _SPEAROracle) public onlyOperatorOrTaxOffice {
        require(_SPEAROracle != address(0), "oracle address cannot be 0 address");
        SPEAROracle = _SPEAROracle;
    }
    ...
    /**
     * @notice Operator mints SPEAR to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SPEAR to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSPEARPrice = _getSPEARPrice();
            currentTaxRate = _updateTaxRate(currentSPEARPrice);
            if (currentSPEARPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_SPEARPool != address(0), "!_SPEARPool");
        rewardPoolDistributed = true;
        _mint(_SPEARPool, INITIAL_SPEAR_POOL_DISTRIBUTION);
    }
