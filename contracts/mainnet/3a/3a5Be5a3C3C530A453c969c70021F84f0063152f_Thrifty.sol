...
contract Thrifty is IERC20Metadata, Ownable {
  ...
  function _processFee() internal {
    ...
    for(uint256 x = processedFeeIndex; x < feeIndex; x++) {
      ...
      totalDai += (feeLogs[x][4] + feeLogs[x][5]) * tmpDecimal / feeLogs[x][7];
      totalMarketing += feeLogs[x][1] * tmpDecimal / feeLogs[x][7];
      totalPrint += tmpDecimal * feeLogs[x][4] / (feeLogs[x][4] + feeLogs[x][5]);
    }
