...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant shrap = address(0xbf4906762C38F50bC7Be0A11BB452C944f6C72E1);
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR EQUAL-SHRAP EQUALIZER")),
            string(abi.encodePacked("XpE-EQUAL-SHRAP"))
        )
        {
        gauge = _gauge;
        router = _router;
        ...
        slippageLPs = [address(0x7547d05dFf1DA6B4A2eBB3f0833aFE3C62ABD9a1), address(0x76fa7935a5AFEf7fefF1C88bA858808133058908)];
    ...
    function _chargeFees(address caller) internal {                   
        ...
        uint usdProfit = IEqualizerPair(slippageLPs[1]).sample(equal, toProfit, 1, 1)[0];
    ...
    function _addLiquidity() internal {
        ...
        (uint minAmt) = getSlippage(equalHalf, address(asset), equal);
        IEqualizerRouter(router).swapExactTokensForTokensSimple(equalHalf, minAmt, equal, shrap, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(equal, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(shrap, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(equal, shrap, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(equal, shrap, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyOwner {
        if(_token == equal || _token == wftm || _token == shrap){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(shrap, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(shrap, router, type(uint).max);
    }
