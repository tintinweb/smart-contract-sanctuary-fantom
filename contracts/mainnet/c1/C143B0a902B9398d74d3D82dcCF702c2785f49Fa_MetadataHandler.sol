...
contract MetadataHandler is IMetadataHandler, Ownable {
    ...
    string public constant OR_METADATA = "https://revest.mypinata.cloud/ipfs/QmSSfUenHEyU6y7HHMoKKz6sgXa43ptjri95tzRRGUTA7d";
    string public constant AL_METADATA = "https://revest.mypinata.cloud/ipfs/QmZ24zZKKHY1dbqHUqcRoDH6sRyxKn1RcN7d6Rzow5zo8x";
    ...
    /// May be called when new LP tokens are added to improve QoL and UX
    ...
    function getOutputReceiverBytes(uint fnftId) external view override returns (bytes memory output) {
        ...
        uint interestId = principalId + 1;
        
        output = abi.encode(
            accruedInterest,    // 0
            description,        // 1
            poolId,             // 2
            farmString,         // 3 // CAN COMBINE
            vault,              // 4
            vaultAdapter,       // 5
            wallet,             // 6
            rate,               // 7
    ...
    // TODO: We likely need a better way to 
    // 1. Test this function
    // 2. Communicate what tokens are inside of LPs
    function _formatAsset(address asset) private view returns (string memory formatted) {
        formatted = string(abi.encodePacked(_getName(asset)," - ","[",IERC20Detailed(asset).symbol(),"] - ", asset));
    } 
