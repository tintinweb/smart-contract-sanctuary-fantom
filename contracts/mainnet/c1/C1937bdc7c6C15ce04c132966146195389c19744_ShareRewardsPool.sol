...
import "../ShareRewardsPool/Address.sol";
import "../ShareRewardsPool/Initializable.sol";
import "../ShareRewardsPool/IERC20.sol";
import "../ShareRewardsPool/SafeMath.sol";
import "../ShareRewardsPool/SafeERC20.sol";
...
contract ShareRewardsPool is Initializable {
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt; // Reward debt. See explanation below. SHARE
    }
    ...
    uint256 public sharePerSecond = 0.00186122 ether; // 59500 SHARE / (370 days * 24h * 60min * 60s)
    ...
    function InitShareRewards(address _shareRewardsAddress) public initializer {
        operator = msg.sender;
        if (_shareRewardsAddress != address(0))
            _share = IERC20(_shareRewardsAddress);
    }

    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "ShareRewardsPool: caller is not the operator"
        );
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(
                ...
                "ShareRewardsPool: existing pool?"
            );
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _shareRewards = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint
            );
            accSharePerShare = accSharePerShare.add(
                _shareRewards.mul(1e18).div(tokenSupply)
            );
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 _pending = user.amount.mul(pool.accSharePerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (_pending > 0) {
            safeShareTransfer(_sender, _pending);
    ...
    function safeShareTransfer(address _to, uint256 _amount) internal {
        uint256 _shareBalace = _share.balanceOf(address(this));
        if (_shareBalace > 0) {
            if (_amount > _shareBalace) {
                _share.safeTransfer(_to, _shareBalace);
            } else {
                _share.safeTransfer(_to, _amount);
            }
        }
    }
