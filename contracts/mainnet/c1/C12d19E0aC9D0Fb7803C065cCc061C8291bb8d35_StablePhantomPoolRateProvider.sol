...
interface IRateProviderX
{
	function getRate() external returns (uint256 _rate);
}

contract StablePhantomPoolRateProvider is IRateProviderX
{
	...
	event LogRate(uint256 _rate);
}

contract StablePhantomPoolLpRateProvider is IRateProviderX
{
	address public immutable vault;
	bytes32 public immutable poolId;
	address public immutable pool;
	...
	{
		return IRateProviderX(rateProvider).getRate();
	}
...
*/
	...
	{
		return IRateProviderX(lpRateProvider).getRate();
	}
	...
	function _getTotalSupply(address _token) internal view returns (uint256 _supply)
	{
	...
	{
		Vat(vat).suck(address(this), address(this), _amount * 1e27); // must be whitelisted
		DaiJoin(daiJoin).exit(address(this), _amount);
	}

	function _burn(uint256 _amount) internal
	{
		DaiJoin(daiJoin).join(address(this), _amount);
		Vat(vat).heal(_amount * 1e27);
	}
	...
	function _joinPool(uint256 _amount0, uint256 _amount1, uint256 _minAmount) internal
	{
		...
		//_safeApprove(_token0, vault, _amount0);
		//_safeApprove(_token1, vault, _amount1);
		//IVault.JoinPoolRequest memory _request;
		//_request.assets = new address[](2);
		//_request.assets[0] = token0;
		//_request.assets[1] = token1;
		//_request.maxAmountsIn = new uint256[](2);
		//_request.maxAmountsIn[0] = _amount0;
		//_request.maxAmountsIn[1] = _amount1;
		//_request.userData = abi.encode(JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT, _request.maxAmountsIn, _minAmount);
		//_request.fromInternalBalance = false;
		//IVault(vault).joinPool(poolId, address(this), address(this), _request);
	}
	...
	function _exitPool(uint256 _amount, uint256 _minAmount0, uint256 _minAmount1) internal
	{
		...
		//_request.minAmountsOut[0] = _minAmount0;
		//_request.minAmountsOut[1] = _minAmount1;
		//_request.userData = abi.encode(ExitKind.EXACT_BPT_IN_FOR_TOKENS_OUT, _amount);
		//_request.toInternalBalance = false;
		//IVault(vault).exitPool(poolId, address(this), payable(address(this)), _request);
	}
