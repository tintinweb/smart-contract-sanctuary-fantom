...
// File: pDollar/lib/SafeMath8.sol
...
// File: pDollar/ERC20/ERC20Burnable.sol
...
contract pDollar is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first week -> PDO reward pool
    uint256 public constant INITIAL_PDO_POOL_DISTRIBUTION = 40000 ether;
    ...
    /**
     * @notice Constructs the PDO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("pDollar", "PDO") {
        // Mints 8,000 PDO to contract creator for initial pool setup & LP
    ...
    function _getPdoPrice() internal view returns (uint256 _pdoPrice) {
        try IOracle(pdoOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Pdo: failed to fetch PDO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _pdoPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_pdoPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPdoOracle(address _pdoOracle) public onlyOperatorOrTaxOffice {
        require(_pdoOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints PDO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PDO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPdoPrice = _getPdoPrice();
            currentTaxRate = _updateTaxRate(currentPdoPrice);
            if (currentPdoPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_pdoPool != address(0), "!_pdoPool");
        rewardPoolDistributed = true;
        _mint(_pdoPool, INITIAL_PDO_POOL_DISTRIBUTION);
    }
