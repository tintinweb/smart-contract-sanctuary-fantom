...
contract FactionRankingV2 is Ownable{
    ...
    function getSortedScoresAndRanks(uint256 _round) public view returns(int256[4] memory, uint256[4] memory){
        uint256 factionIndex = 0;
        int256[4] memory scores = getScores(_round);
        uint256[4] memory factions = [factionIndex, factionIndex + 1, factionIndex + 2, factionIndex +3];
        (int256[4] memory sortedScores, uint256[4] memory sortedFactions) = sortFactionScores(scores, factions);
        return(sortedScores, sortedFactions);
    }

    function sortFactionScores(int256[4] memory _scores, uint256[4] memory _factions) public pure returns (int256[4] memory, uint256[4] memory) {
    uint256 l = _scores.length;
    for(uint i = 0; i < l; i++) {
        for(uint j = i+1; j < l ;j++) {
            if(_scores[i] < _scores[j]) {
                int256 temp = _scores[i];
