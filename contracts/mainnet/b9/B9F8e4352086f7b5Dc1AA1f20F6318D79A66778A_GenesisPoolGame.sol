...
contract ERC1155Minter is ERC1155, ERC1155MintBurn, ERC1155Metadata, Ownable, MinterRole, WhitelistAdminRole {
    ...
    function getParticipantById(uint _id) public view returns (uint128, uint64, uint64, address) {
        Participant memory participant = _participants[_id];
        return (participant.nftId, participant.x, participant.y, ownerOf(participant.nftId));
    }
...
contract GenesisPoolGame is TokenWrapper, Ownable {
    ...
    event GameStarted(uint256 gameStartTime, uint256 ticketPrice);
    ...
    event TokensRecovered(address token, address to, uint256 value);

    constructor(ERC1155Minter _gameMinter, IERC20 _erc20Address, IVENFT _veNft) public TokenWrapper(_erc20Address) {
        gameMinter = _gameMinter;
        veNft = _veNft;
    }
    
    function start(uint256 _gameStartTime, uint256 _amount, uint256 _imageWidth, string calldata _url) external onlyOwner {
    ...
    function finish(uint256 _id, uint16 _duration) external onlyOwner {
        require(gameStartTime != 0, "Game not started yet");
        require(!finished, "Game finished");
        address winner = gameMinter.ownerOf(_id);
        require(winner != address(0), "Not existing id provided");

        uint128 amount = uint128(token.balanceOf(address(this)));
        token.safeTransfer(address(veNft), amount);
        veNft.mint(winner, amount, _duration);
        finished = true;
        emit GameFinished(winner, amount);
    }

    function play(uint256 _xPoint, uint256 _yPoint) external {
        require(gameStartTime != 0,  "Not started yet");
        require(token.balanceOf(msg.sender) >= ticketPrice,  "User does not have enough tokens to play");
        require(!finished, "Game finished");
    ...
    function _generateId(uint256 _xPoint, uint256 _yPoint, uint256 _width) internal view returns(uint256) {
        ...
        require(gameMinter.ownerOf(id) == address(0), "Id already used");
        return id;
    }
    ...
    function recoverTokens(IERC20 _token, address _destination) external onlyOwner {
        ...
        require(address(_token) != address(token) || finished, 'SGV recover not allowed');
