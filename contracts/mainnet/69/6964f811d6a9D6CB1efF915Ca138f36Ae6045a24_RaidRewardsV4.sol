...
interface IRaidRewardsV3{
    function addressClaimedRewards(uint256, address) external view returns (uint256);
}

contract RaidRewardsV4 is Ownable, ReentrancyGuard {
    ERC20Burnable public raidToken;
	IRaidersNFT public raidersNFT;
	FactionRanking public factionRanking;
	IRaidRewardsV3 public raidRewardsV3;
    ...
    address public rankingAddress = 0xB9d5792e1e10120830f0B50cFC4191E0765613f9;
    address public raidRewardsV3Address = 0x5dbfA959Ba4201440114Fa9bd1e666b9525022D1;
	...
	event ClaimedByPlayerAddress(uint256 gameRound, uint256 claimID, address indexed playerAddress, uint256 amount);

	constructor() {
		raidToken = ERC20Burnable(tokenAddress);
		raidersNFT = IRaidersNFT(nftAddress);
        factionRanking = FactionRanking(rankingAddress);
        raidRewardsV3 = IRaidRewardsV3(raidRewardsV3Address);
        setGameAddress(4, 0xD3A47D06B69BCE036cE18E5E2821d32d252dF8FE);
        setClaimingStatus(1);
	}
	...
	function claimUsingPlayerAddress(uint256 _gameRound) public returns (uint256){
		...
		uint256 grossRewards = getAddressGrossRewards(_gameRound, msg.sender);
		uint256 netClaimableRewards;
        if(_gameRound == 4){
            netClaimableRewards = safeMinus(grossRewards, (addressClaimedRewards[_gameRound][msg.sender] + raidRewardsV3.addressClaimedRewards(4, msg.sender)));
        }else{
            netClaimableRewards = safeMinus(grossRewards, addressClaimedRewards[_gameRound][msg.sender]);
        }
		...
		emit ClaimedByPlayerAddress(_gameRound, claimID, msg.sender, netClaimableRewards);
		return netClaimableRewards;
	}	

	//GETTERS BASED ON PLAYER ADDRESS

	function getAddressNetRewards(uint256 _gameRound, address _playerAddress) public view returns (uint256) {
		uint256 grossRewards = getAddressGrossRewards(_gameRound, _playerAddress);
		uint256 netClaimableRewards = grossRewards - addressClaimedRewards[_gameRound][msg.sender];
		return netClaimableRewards;
	}

	function getAddressGrossRewards(uint256 _gameRound, address _playerAddress) public view returns (uint256) {
