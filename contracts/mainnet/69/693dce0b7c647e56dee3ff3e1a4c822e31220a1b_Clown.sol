...
contract Clown is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the CLOWN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("CLOWN", "CLOWN") {
        // Mints 1 CLOWN to contract creator for initial pool setup
    ...
    function _getClownPrice() internal view returns (uint256 _clownPrice) {
        try IOracle(clownOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Clown: failed to fetch CLOWN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _clownPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_clownPrice >= taxTiersTwaps[tierId]) {
    ...
    function setClownOracle(address _clownOracle) public onlyOperatorOrTaxOffice {
        require(_clownOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CLOWN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CLOWN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentClownPrice = _getClownPrice();
            currentTaxRate = _updateTaxRate(currentClownPrice);
            if (currentClownPrice < burnThreshold) {
                burnTax = true;
            }
        }
