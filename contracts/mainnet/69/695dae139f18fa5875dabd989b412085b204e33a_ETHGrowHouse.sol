...
contract ETHGrowHouse {
    ...
    constructor() {
        ...
        PercsKey[10] = DivPercs(864000, 30000);
        PercsKey[20] = DivPercs(1728000, 35000);
        PercsKey[30] = DivPercs(2592000, 40000);
        PercsKey[40] = DivPercs(3456000, 5);
        PercsKey[50] = DivPercs(4320000, 5);
        PercsKey[60] = DivPercs(5184000, 5);
        PercsKey[70] = DivPercs(6048000, 5);
        PercsKey[80] = DivPercs(6912000, 5);
        PercsKey[90] = DivPercs(7776000, 5);
        PercsKey[100] = DivPercs(8640000, 5);
        ...
        // FTM_ETH_LP = IERC20(0xA7B8510fe299Bc69f8aF42406e1795319a9226d8);
        // ETH = IERC20(0x39968964A55901043368F18Ff14DbE2eE3B648ce);
        ...
        FTM_ETH_LP = IERC20(0xf0702249F4D3A25cD3DED7859a165693685Ab577);
    ...
    function stakeStableCoins(uint256 amtx, address ref) public {
        ...
        WETH.transferFrom(msg.sender, address(this), amtx);
        ...
        uint256 initialLPBalance = FTM_ETH_LP.balanceOf(address(this));

        // add liquidity to spookeyswap
        addLiquidity(otherHalf, newBalance);

        // how much LP should we stake
        uint256 newLPBalance = FTM_ETH_LP.balanceOf(address(this)).sub(initialLPBalance);
    ...
    function stakeNativeCurrencies(address ref) public payable {
        ...
        // capture the contract's current ETH balance.
        uint256 initialETHBalance = WETH.balanceOf(address(this));

        // swap FTM for ETH
        swapEthForTokens(half, address(this));
      
        // how much ETH should we just swap into?
        uint256 increasedETHBalance = WETH.balanceOf(address(this)).sub(initialETHBalance);

        // capture the contract's current LP balance
        uint256 initialLPBalance = FTM_ETH_LP.balanceOf(address(this));

        // add liquidity to spookeyswap
        addLiquidity(increasedETHBalance, otherHalf);
    ...
    function swapEthForTokens(uint256 ethAmount, address _to) internal {
        // generate the uniswap pair path of FTM -> ETH
    ...
    function stake(uint256 amtx, address ref) internal {
        ...
        FTM_ETH_LP.safeTransfer(owner, stakeFee);
    }
    ...
    function withdrawDivs() external returns (uint256 withdrawAmount) {
        ...
        FTM_ETH_LP.safeTransfer(msg.sender, adjustedAmt);
        FTM_ETH_LP.safeTransfer(owner, withdrawFee);
    ...
    function withdrawInitial(uint256 key) external {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ // LESS THAN 10 DAYS STAKED
            ...
            FTM_ETH_LP.safeTransfer(msg.sender, transferAmt);
            FTM_ETH_LP.safeTransfer(owner, minusAmt);
    ...
    function withdrawRefBonus() external {
        ...
        FTM_ETH_LP.safeTransfer(msg.sender, amtz);
    }
