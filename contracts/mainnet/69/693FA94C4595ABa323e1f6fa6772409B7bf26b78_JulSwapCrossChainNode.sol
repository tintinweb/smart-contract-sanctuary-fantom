    ...
    RemoteExecutionNodeRegistry
{
    ...
    bytes32 constant constructorInputHash = keccak256("");
    ...
    function deployPocket(
        ...
        emit PocketFundClaimed(id, _dstReceiver, erc20Amount, address(pocket), nativeAmount);
    }
    /**
    ...
    function _computeId(address _srcSender, address _dstReceiver, uint64 _nonce) public pure returns (bytes32) {
    ...
    function _processNextStep(
        ...
        if (_execs.length > 0) {
            ...
            bridgeOutReceiver = _getPocketAddr(_id, remote, exec.bridge.toChainId);
    ...
    // function _getPocketAddr(bytes32 _salt, address _deployer, uint64 _chainId) private pure returns (address) {
    function _getPocketAddr(bytes32 _salt, address _deployer, uint64 _chainId) public pure returns (address) {
        ...
        bytes32 codeHash;
        bytes32 hash;
        if (_chainId == 324 || _chainId == 280) {
            //zksync
            codeHash = 0xab234985dc5dfb0e7c37f0f84b93b28661fb5725ef87524a62c4a8ee991d7329;

            hash = keccak256(
                bytes.concat(
                    CREATE2_PREFIX,
                    bytes32(uint256(uint160(_deployer))),
                    _salt,
                    codeHash,
            ...
            codeHash = 0x82b6de7042f2cd8322bf2897323b63a909f2a8f74f74c0933efafa7752618a84;
            hash = keccak256(
                // abi.encodePacked(bytes1(0xff), _deployer, _salt, keccak256(type(Pocket).creationCode))
                abi.encodePacked(bytes1(0xff), _deployer, _salt, codeHash)
            );
        }
