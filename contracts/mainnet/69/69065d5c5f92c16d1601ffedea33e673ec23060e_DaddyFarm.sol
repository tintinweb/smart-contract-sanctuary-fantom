...
contract DaddyFarm {
    ...
    // ACTIVE
        function zapETH() public payable open(_daddyLP) goodRobot {
            ...
            uint256 zapRec = _swapAndLiquify(zapValue);
        ...
        function zapToken(uint256 value) public open(_daddyLP) goodRobot {
            ...
            uint256 zapRec = _swapAndLiquifyToken(zapValue);
            if(!_pool[_daddyLP].isUser[msg.sender]){ _modUser(msg.sender, _daddyLP, true); }
                _pool[_daddyLP].uDeposit[msg.sender] += zapRec;
                _pool[_daddyLP].userTotal += zapRec;
            _updateEmission();
        ...
        function claimPending(address pool, bool compound) public goodRobot {
            ...
            if(!compound){ IERC20(_daddy).transfer(msg.sender, value); }
                ...
                if(pool == _daddy){
                    ...
                    if(!_pool[_daddyLP].isUser[msg.sender]){ _modUser(msg.sender, _daddyLP, true); }
                    uint256 pValue = _swapAndLiquifyToken(value);
    ...
    // INTERNAL
        ...
        function _processFee(address token, uint256 value) internal {
            ...
            if(token == address(0)){
                dFee = value - mFee;
                payable(_feeReceiver).transfer(mFee);
                payable(_devWallet).transfer(dFee);
            } else {
                dFee = value - (mFee + bFee);
                IERC20(token).transfer(_feeReceiver, mFee);
                IERC20(token).transfer(_devWallet, dFee);
                IERC20(token).transfer(address(0xdead), bFee);
            }
        }
        ...
        function _swapAndLiquify(uint256 value) internal returns(uint256){
            uint256 halfLiq = value / 2;
            address[] memory path = new address[](2);
                path[0] = IDexRouter(_router).WETH();
                path[1] = _daddy;
            uint256 spotDaddy = IERC20(_daddy).balanceOf(address(this));
            IDexRouter(_router).swapExactETHForTokensSupportingFeeOnTransferTokens{ value: halfLiq }
                (0, path, address(this), block.timestamp);
            uint256 recDaddy = IERC20(_daddy).balanceOf(address(this)) - spotDaddy;
            require(recDaddy != 0, "Swap Failed");
            uint256 spotPair = IERC20(_daddyLP).balanceOf(address(this));
            IERC20(_daddy).approve(_router, recDaddy);
            IDexRouter(_router).addLiquidityETH{ value: halfLiq }
                (_daddy, recDaddy, 0, 0, address(this), block.timestamp);
            uint256 recPair = IERC20(_daddyLP).balanceOf(address(this)) - spotPair;
            require(recPair != 0, "LP Creation Failed");
            return recPair;
        }

        function _swapAndLiquifyToken(uint256 value) internal returns(uint256){
            uint256 halfLiq = value / 2;
            address[] memory path = new address[](2);
                path[0] = _daddy;
            ...
            IDexRouter(_router).swapExactTokensForETHSupportingFeeOnTransferTokens
                (halfLiq, 0, path, address(this), block.timestamp);
            uint256 recETH = address(this).balance - spotETH;
            require(recETH != 0, "Swap Failed");
            uint256 spotPair = IERC20(_daddyLP).balanceOf(address(this));
            IERC20(IDexRouter(_router).WETH()).approve(_router, recETH);
            IDexRouter(_router).addLiquidityETH{ value: recETH }
                (_daddy, halfLiq, 0, 0, address(this), block.timestamp);
            uint256 recPair = IERC20(_daddyLP).balanceOf(address(this)) - spotPair;
            require(recPair != 0, "LP Creation Failed");
            return recPair;
        }
