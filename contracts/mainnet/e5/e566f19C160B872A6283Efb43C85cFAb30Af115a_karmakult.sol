    ...
    ClampedRandomizer
{
    ...
    uint256 public karmaPrice;
    uint256 private _maxMintable = 666;
    ...
    address private karmaReceiver;
    uint256 public wlTime = 0;
    uint256 public publicTime = 1667174400;
    bool public paused;
    IERC20 karma;

    constructor()
        ERC721("Karma Kult", "KULT")
        ClampedRandomizer(_maxMintable)
    {
        _customBaseURI = "ipfs://QmNswr81CPiWLqMLDkFkjbX2a8yVvZFcp75YW4Doq41zrH/";
        price = 10 ether; //price in milli
        wlPrice = 100000 ether; //price in milli
        karmaPrice = 1000 * 1 ether;
        _royaltyAmount = 666;
        _maxMintPerTx = 66;
        maxPerWallet = 66;
        paused = true;
        karma = IERC20(0x2cBB10cD1e3F6f675ce9E506aCD3Fc70778a4490);
        karmaReceiver = 0x9Ffb9D48c748c8224b0D4631D14D84bCae4eF433;
        payoutAddress = 0x9Ffb9D48c748c8224b0D4631D14D84bCae4eF433;
        royaltyAddress = 0x9Ffb9D48c748c8224b0D4631D14D84bCae4eF433;
    }
    ...
    mapping(address => uint256) karmaMints;
    ...
    function setPrice(uint256 priceInMilli, uint256 wlPriceInMilli)
        ...
        onlyOwner
    {
        price = priceInMilli * 1000000000000000;
        wlPrice = wlPriceInMilli * 1000000000000000;
    }
    function setReceivers(address payoutAddress1, address royaltyReceiver) public onlyOwner{
    ...
    function mint(uint256 quantity) public payable {
        ...
        if (privateSale()) {
            ...
            require(msg.value >= (wlPrice * quantity), "Not enough FTM");
            whitelistMints[msg.sender] -= quantity;
        } else if (publicSale()) {
            require(publicMints[msg.sender] <= maxPerWallet, "minted max");
            require(msg.value >= (price * quantity), "Not enough FTM sent");
            publicMints[msg.sender] += quantity;
        }
        require(totalSupply() + quantity <= _maxMintable, "can't mint more than max supply");
    ...
    function karmaMint(uint256 quantity) public {
        require(publicSale() || privateSale(), "sale isn't active");
        require(karmaMints[msg.sender] <= 5, "Cant mint more than 5 with karma");
        require(totalSupply() + quantity <= _maxMintable, "minting has ended");
        require(!paused, "not active.");
        karma.transferFrom(msg.sender, karmaReceiver, karmaPrice * quantity);
        for (uint256 i = 0; i < quantity; i++) {
            internalMint(msg.sender);
        }
    }
