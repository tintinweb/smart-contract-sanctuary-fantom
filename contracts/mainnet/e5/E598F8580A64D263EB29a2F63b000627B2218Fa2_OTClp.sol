...
contract OTClp is Ownable, ContractGuard {
    ...
    address private BSHARELP = 0x6F607443DC307DCBe570D0ecFf79d65838630B56;
    address private SMELT = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75;//USDC
    ...
    function initialize() public onlyOwner {
        addAsset(BSHARELP, spookyRouter);
        addAsset(SMELT, spookyRouter);
    ...
    function swapToken(address _in, uint256 amount, address _router, uint256 minimumAmount) external onlyOneBlock assetWhitelist(_in, _router) {
        ...
        uint256 amountTokenFromOtcRecieved = _swapOnOTC(_in, amount,_router,msg.sender,minimumAmount);
        // used by UI to calculated total amount saved for buyers
        totalSmeltSwapped = totalSmeltSwapped.add(amountTokenFromOtcRecieved);
    }
    ...
    function _estimateOTCSwap(address _in,uint256 _amount, address _routerAddr) public view assetWhitelist(_in, _routerAddr) returns (uint256) {
        ...
        uint256 amountInNative = _priceInNative(_in,_amount);
        address[] memory path = _getPath(NATIVE, tokenForSale , _routerAddr);
        uint256[] memory amounts =_getAmountsOutWihoutFee(_routerAddr, amountInNative, path);
