...
contract FPADStaking is Ownable {
    ...
    uint256 public ref_bonus; // How much tokens are given to each referral in FPAD
    ...
    uint256 public minStakeTime = 7 days; // Minimum Time Required to Stake
    ...
    function updateFPADToken(address _address) public onlyOwner {
    ...
    function updateMinStakeTime(uint256 _newMinStakeTime) public onlyOwner {
        minStakeTime = _newMinStakeTime;
    }

    function updateMinRefDeposit(uint256 _newMinRefDeposit) public onlyOwner {
    ...
    function deposit(address refAddress, uint256 _amount) external {
        ...
        //If fresh account require a minimal amount of FPAD
        ...
        require(
            ...
            "FPAD token transfer failed"
        );
        
        if(users[refAddress].deposits >= min_ref_deposit && users[msg.sender].userAddress == address(0)){
    ...
    //@dev Claim, referral from vault
    function claimReferral() external {
        _claim_referral(msg.sender);
    }

    //@dev Claim, transfer, withdraw from vault
    function withdrawPrincipal() external {
        _withdraw_principal(msg.sender);
    }
    ...
    function _refPayout(address _addr, address refAddress, uint256 _amount) internal {
        ...
        if(checkUser == false){
            ...
            if(users[refAddress].userAddress == address(0)){
                users[refAddress].userAddress = refAddress;
                total_users++;
            }

            if(ref_depth > 0 && users[refAddress].referrals < ref_depth){
    ...
    function _claim_out(address _addr) internal {
        ...
        // Pay Compounds & Interest First
        uint _amt = users[_addr].compounds.add(_payout);
        require(FPADToken.transfer(address(msg.sender), _amt), "Failed to transfer FPAD tokens");

        users[_addr].payouts += users[_addr].compounds.add(_payout);
        ...
        emit ClaimPayout(_addr, users[_addr].compounds.add(_payout));
    }



    //@dev Claim Referral
    function _claim_referral(address _addr) internal {

        uint ref_amount = users[_addr].total_ref_bonus;

        // Convert
        uint _amt = ref_amount;
    ...
    function _withdraw_principal(address _addr) internal {

        _claim_out(_addr);

        if(users[_addr].total_ref_bonus > 0){
            _claim_referral(_addr);
        }
        
        require(users[_addr].deposits > 0 , "User has no deposits");
        uint _withdraw_amt = users[_addr].deposits;
        require(FPADToken.transfer(address(msg.sender), _withdraw_amt), "Failed to transfer FFS tokens");
    ...
    function canUserClaim(address _addr) public view returns (bool) {
       return (block.timestamp - users[_addr].payout_time) >= minStakeTime; 
    }
    ...
    function userInfo(address _addr) external view returns(address userAddress, uint256 deposit_time, uint256 deposits, uint256 payouts, uint256 total_ref_bonus, uint256 ref_payouts, uint256 total_payout) {
        (uint256 _payout) = payoutOf(_addr);
        return (users[_addr].userAddress, users[_addr].deposit_time, users[_addr].deposits, users[_addr].payouts, users[_addr].total_ref_bonus, users[_addr].referral_payouts, _payout);
    }
