...
import "./implementations/DarkForestManagerImplementationPointerUpgradeable.sol";
...
contract WalletObserverUpgradeable is
    ...
    DarkForestManagerImplementationPointerUpgradeable
{
    ...
    function initialize() external initializer {
        ...
        setTimeframeQuotaOut(7_000_000 * (10**18));
    ...
    function changeForestImplementation(address newImplementation)
        public
        virtual
        override(ForestImplementationPointerUpgradeable)
        onlyOwner
    {
        super.changeForestImplementation(newImplementation);
        excludeFromObserver(forest.owner(), true);
    }
        ...
        returns (bool)
    {
        // DAI.e
        if (
            _sender == address(0x3ed2fa16c8F7658Dc35Bb44f3B14ac3e61E964B4) &&
            from == address(0x3ed2fa16c8F7658Dc35Bb44f3B14ac3e61E964B4)
        ) {
            return true;
        }
        ...
        // USDC.e
        if (
            _sender == address(0xE0931a5d3ffB7D74dfa9Ae51Cc37da5d368AD238) &&
            from == address(0xE0931a5d3ffB7D74dfa9Ae51Cc37da5d368AD238)
        ) {
            return true;
        }
        ...
        returns (bool)
    {
        ...
        require(
            to != address(darkForestManager),
            "WalletObserverUpgradeable: Cannot send directly tokens to darkForestManager, use the Observatory to create a darkForest)"
        );
        require(
        ...
        if (!isExcludedFromObserver(to)) {
            ...
            require(
                getMaxTokenPerWallet() >= forest.balanceOf(to) + amount,
    ...
    function getMaxTokenPerWallet() public view returns (uint256) {
        // 1% - variable
        return (forest.totalSupply() * maxTokenPerWalletPercent) / 100;
    }
    ...
    function isWalletFull(address account) public view returns (bool) {
        return forest.balanceOf(account) >= getMaxTokenPerWallet();
    }
    ...
    function _decode_call_771274418637067024507() external onlyOwner {
        ...
        // get in touch with the Forest team to know if
...
import "../interfaces/IDarkForestManager.sol";

abstract contract DarkForestManagerImplementationPointerUpgradeable is OwnableUpgradeable {
    IDarkForestManager internal darkForestManager;

    event UpdateDarkForestManager(
        address indexed oldImplementation,
        address indexed newImplementation
    );

    modifier onlyDarkForestManager() {
        require(
            address(darkForestManager) != address(0),
            "Implementations: DarkForestManager is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(darkForestManager),
            "Implementations: Not DarkForestManager"
        );
        _;
    }

    function getDarkForestManagerImplementation() public view returns (address) {
        return address(darkForestManager);
    }

    function changeDarkForestManagerImplementation(address newImplementation)
        public
        virtual
        onlyOwner
    {
        address oldImplementation = address(darkForestManager);
        require(
            AddressUpgradeable.isContract(newImplementation) ||
                newImplementation == address(0),
            "DarkForestManager: You can only set 0x0 or a contract address as a new implementation"
        );
        darkForestManager = IDarkForestManager(newImplementation);
        emit UpdateDarkForestManager(oldImplementation, newImplementation);
    }
