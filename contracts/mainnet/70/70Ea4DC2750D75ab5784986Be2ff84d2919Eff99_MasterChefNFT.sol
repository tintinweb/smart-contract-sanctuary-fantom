...
contract SandManToken is ERC20, Ownable {
    ...
    /// Transfer tax Liquidity Rate 3.33%
    uint16 public transferTaxLiquidityRate = 333;

    /// Transfer tax BurnRate 3.33%
    uint16 public transferTaxBurnRate = 333;
        ...
        ERC20('SANDMAN V2', 'SANDMAN')
    {
        ...
        _excludedOperators[_treasuryDAO] = true;
    ...
    function setTransferTaxLiquidityRate(uint16 newTransferTaxLiquidityRate) external onlyOwner {
        require((newTransferTaxLiquidityRate + transferTaxBurnRate) <= MAXIMUM_TRANSFER_TAX_RATE, "E12");
    ...
    function setTransferTaxBurnRate(uint16 newTransferTaxBurnRate) external onlyOwner {
        require((newTransferTaxBurnRate + transferTaxLiquidityRate) <= MAXIMUM_TRANSFER_TAX_RATE, "E12");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiBot(sender, recipient, amount) {
        if (isExcludedOperator(sender) || isExcludedOperator(recipient)) {
            ...
            require(sendAmount > (burnAmount + liquidityAmount), "E13");

            sendAmount = sendAmount - burnAmount - liquidityAmount;
...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
    ...
    constructor(address _usdCurrency, address _routerAddress, address _liquidityLockerAddress, uint256 startTime) {
    ...
    function transferUSDToOwner(address ownerAddress, uint256 amount) external onlyRole(OPERATOR_ROLE) {
        ...
        require(IERC20(usdCurrency).transfer(ownerAddress, amount), "E1");
