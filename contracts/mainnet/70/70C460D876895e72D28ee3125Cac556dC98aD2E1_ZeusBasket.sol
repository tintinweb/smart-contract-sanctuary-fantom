...
contract ZeusToken is ERC20, ERC20Burnable, Ownable, Pausable {
...
/////////////////// CONSTRUCTOR /////////////////
    ...
    constructor (uint256 _tokenSoftCap, uint256 _initialSupply) ERC20("Zeus", "ZEUS") {
...
////////////// PRIVILEGED FUNCTIONS //////////////
    ...
    // This function mints ZEUS tokens. Can only be called by the Masterchef. 
...
// MasterChef is the master of ZEUSs. He can make ZEUSs and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once ZEUS is sufficiently
// distributed and the community can show to govern itself.

contract ZeusBasket is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ZEUS
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accZeusPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accZeusPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. ZEUSs to distribute per second.
        uint256 lastRewardTime;  // Last timestamp that ZEUSs distribution occurs.
    ...
    // The ZEUS TOKEN!
    ZeusToken public immutable zeus;
    // Dev address.
    address public devaddr;
    // ZEUS tokens created per timestamp.
    uint256 public zeusPerSecond;
    // Deposit Fee address
    ...
    // The timestamp when ZEUS distribution starts.
    uint256 public startTime;
    // Maximum ZEUS Per Second
    ...
    event UpdateEmissionRate(address indexed user, uint256 zeusPerSecond);
    ...
    event zeusMintError(bytes reason);

    constructor(
        ZeusToken _tokenAddress,
        address _devaddr,
        address _feeAddress,
        uint256 _zeusPerSecond,
        uint256 _startTime
    ) {
        ...
        require(_zeusPerSecond <= MAX_EMISSION_RATE, "Emission rate cannot be more than the maximum.");

        zeus = _tokenAddress;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        zeusPerSecond = _zeusPerSecond;
    ...
    function pendingZeus(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 zeusReward = multiplier * zeusPerSecond * pool.allocPoint / totalAllocPoint;
            accZeusPerShare = accZeusPerShare + zeusReward * 1e18 / pool.lpSupply;
        }
        return user.amount * accZeusPerShare / 1e18 - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        try zeus.mint(devaddr, zeusReward / 10) {
        } catch (bytes memory reason) {
            zeusReward = 0;
            emit zeusMintError(reason);
        }
        
        try zeus.mint(address(this), zeusReward) {
        } catch (bytes memory reason) {
            zeusReward = 0;
            emit zeusMintError(reason);
        }
        
        pool.accZeusPerShare = pool.accZeusPerShare + zeusReward * 1e18 / pool.lpSupply;
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount * pool.accZeusPerShare / 1e18 - user.rewardDebt;
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount * pool.accZeusPerShare / 1e18;
    ...
    function updateEmissionRate(uint256 _zeusPerSecond) external onlyOwner {
        require(_zeusPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: INCORRECT INPUT");
        massUpdatePools();
        
        zeusPerSecond = _zeusPerSecond;
       
        emit UpdateEmissionRate(msg.sender, _zeusPerSecond);
    }
