...
contract PoolService {
    ...
    function allVaultsInfo(IStablecoin stablecoin) external view returns (VaultInfo[] memory) {
        uint256 vaultCount = stablecoin.vaultCount();
        VaultInfo[] memory vaultInfo = new VaultInfo[](vaultCount);

        for (uint256 i = 0; i < vaultCount; ++i) {
            address owner = _ownerOf(stablecoin, i);
            uint256 collateral = owner == address(0) ? 0 : stablecoin.vaultCollateral(i);
            uint256 debt = owner == address(0) ? 0 : stablecoin.vaultDebt(i);
    ...
    function getVaultInfo(IStablecoin stablecoin, uint256 vaultId) external view returns (VaultInfo memory) {
        address owner = VaultNFTv3(address(stablecoin)).ownerOf(vaultId);
        uint256 collateral = owner == address(0) ? 0 : stablecoin.vaultCollateral(vaultId);
        uint256 debt = owner == address(0) ? 0 : stablecoin.vaultDebt(vaultId);
    ...
    function toLiquidation(IStablecoin stablecoin) external view returns(uint[] memory) {
        uint256 liquidateableCount = _liquidateableCount(stablecoin);
        uint256 vaultCount = stablecoin.vaultCount();

        uint256[] memory liquidateableVaults = new uint256[](liquidateableCount);

        for (uint256 vaultId = 0; vaultId < vaultCount; ++vaultId) {
            if(!stablecoin.exists(vaultId)) { continue; }

            bool liquidateable = stablecoin.checkLiquidation(vaultId);
            if (liquidateable) {
                liquidateableVaults[liquidateableVaults.length] = vaultId;
            }
        }

        return liquidateableVaults;
    }

    function _ownerOf(IStablecoin stablecoin, uint256 vaultId) private view returns (address) {
        bool exist = stablecoin.exists(vaultId);

        if (exist) {
            return VaultNFTv3(address(stablecoin)).ownerOf(vaultId);
        }

        return address(0);
    }

    function _liquidateableCount(IStablecoin stablecoin) private view returns(uint256) {
        uint256 vaultCount = stablecoin.vaultCount();
        uint256 count = 0;
