...
contract Presale is STO,Ownable  {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address public DAO;

    mapping(IERC20=>uint256) public payTokens;
    ...
    bool public openIco = false;
    uint256 public saleStartTime;
    uint256 public salePeriod;
    mapping(address => uint256) public purchasedAmount;

    constructor(ISecurityToken _securityToken, address _payToken) STO(address(_securityToken), _payToken){
    }
    function getTypes() external override view returns(uint8[] memory) {
        //Will be overloaded.
        //TODO NOW
        uint8[] memory temp = new uint8[](1);
        temp[0] = 3;
        return temp;
    }
    // first
    ...
    //fourth
    function purchase(uint256 _purchaseAmount, IERC20 payToken) external returns (bool) {
        require(openIco == true, "ICO is closed");
        require(saleStartTime+salePeriod > block.timestamp, "ICO is finished");
        require(sellAmount + _purchaseAmount <= hardCap, "Hardcap is reached.");
        require(payTokens[payToken] > 0, "Not allowed token.");
        uint256 payAmount = _calculateSaleQuote(_purchaseAmount, payToken);
        payToken.transferFrom(msg.sender, address(this), payAmount);
        securityToken.issue(msg.sender, _purchaseAmount, "");
        sellAmount += _purchaseAmount;
        purchasedAmount[msg.sender] += _purchaseAmount;
        return true;
    }
    //second
    function setPayToken(IERC20 token, uint256 price) external onlyOwner {
        payTokens[token] = price;
    }
    //fifth
    function withdraw(IERC20 token) external onlyOwner {
        require(saleStartTime+salePeriod < block.timestamp, "ICO is not finished yet.");
    ...
    function _calculateSaleQuote(uint256 purchaseAmount, IERC20 payToken)
    internal
    view
    returns (uint256)
    {
        return purchaseAmount.mul(payTokens[payToken]).div(uint256(1e18));
    }
}
...
contract SecurityToken is SecurityTokenStorage, ERC20("DirectPrivateOffers", "DPO"), ReentrancyGuard, IERC1594, IERC1643, IERC1644, IERC1410 {
