...
contract BoozeRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BOOZEs to distribute in the pool.
        uint256 lastRewardTime; // Last time that BOOZEs distribution occurred.
        uint256 accBoozePerShare; // Accumulated BOOZEs per share, times 1e18. See below.
    ...
    uint256[3] public epochboozePerSecond;
    ...
    constructor(address _booze, uint256 _poolStartTime) public {
        ...
        epochboozePerSecond[0] = epochTotalRewards[0].div(4 days);
        epochboozePerSecond[1] = epochTotalRewards[1].div(5 days);

        epochboozePerSecond[2] = 0;
        operator = msg.sender;
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "BoozeRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "BoozeRewardPool: existing pool?");
        }
    }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accBoozePerShare: 0, isStarted: _isStarted}));
        if (_isStarted) {
        ...
        for (uint8 epochId = 2; epochId >= 1; --epochId) {
            if (_toTime >= epochEndTimes[epochId - 1]) {
                if (_fromTime >= epochEndTimes[epochId - 1]) {
                    return _toTime.sub(_fromTime).mul(epochboozePerSecond[epochId]);
                }

                uint256 _generatedReward = _toTime.sub(epochEndTimes[epochId - 1]).mul(epochboozePerSecond[epochId]);
                if (epochId == 1) {
                    return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochboozePerSecond[0]));
                }
                for (epochId = epochId - 1; epochId >= 1; --epochId) {
                    if (_fromTime >= epochEndTimes[epochId - 1]) {
                        return _generatedReward.add(epochEndTimes[epochId].sub(_fromTime).mul(epochboozePerSecond[epochId]));
                    }
                    _generatedReward = _generatedReward.add(epochEndTimes[epochId].sub(epochEndTimes[epochId - 1]).mul(epochboozePerSecond[epochId]));
                }
                return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochboozePerSecond[0]));
            }
        }
        return _toTime.sub(_fromTime).mul(epochboozePerSecond[0]);
    }
    ...
    function safeBoozeTransfer(address _to, uint256 _amount) internal {
        uint256 _boozeBal = booze.balanceOf(address(this));
        if (_boozeBal > 0) {
            if (_amount > _boozeBal) {
                booze.safeTransfer(_to, _boozeBal);
            } else {
