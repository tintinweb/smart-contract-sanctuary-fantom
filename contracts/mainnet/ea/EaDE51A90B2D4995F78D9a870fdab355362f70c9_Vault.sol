...
contract Vault is ERC20, Ownable {
  ...
  uint256 public maxAmountOneUSDToWithdraw;
  ...
  constructor() ERC20("testOneUSD","testoUSD") {
    ...
    unitOneUSD = 10**18;
    maxAmountOneUSDToWithdraw = 50000 * unitOneUSD;
    minimumPercentage = 99;
    maxDepositInUSD = 50000;
  }
  ...
  function deposit(address token, uint256 amountToDeposit) external vaultIsOpen validBlockNumberForDeposit returns(uint256) {
    ...
    uint256 amountToDepositInUSD = (amountToDeposit)/(10**tokenDecimals);
    ...
    uint256 expectedOneUSDTokens = (amountToDeposit*unitOneUSD)/(10**tokenDecimals);
  ...
  function _deposit(address token, uint256 amountToDeposit, uint256 minOneUSDAmount, address sender) 
    ...
    if(_newLiquidityInUSD*(10**tokenDecimals) > amountToDeposit) {
      _newLiquidityInUSD = amountToDeposit;
    }
  ...
  function withdraw(uint256 amount) external validBlockNumberForWithdraw vaultIsOpen returns(uint256){
    ...
    if(amount > maxAmountOneUSDToWithdraw)  {
      revert Vault_Cant_Withdraw_That_Amount();
    }
  ...
  function _withdraw(address sender, uint256 usdcToWithdraw, uint256 minAmountUSDC) internal returns (uint256) {
    ...
    // This is to avoid any problem of usdcToSentToUser to be a lot and sending them by error
    uint256 maxUSDCAllowed = (usdcToWithdraw*10005)/10000;

    if(usdcToSentToUser > maxUSDCAllowed) {
      usdcToSentToUser = maxUSDCAllowed;
    }
  ...
  function changeMaxAmountOneUSDToWithdraw(uint256 newMaxAmount) external onlyOwner {
    require(newMaxAmount > 10000 * unitOneUSD);

    maxAmountOneUSDToWithdraw = newMaxAmount;
  }
