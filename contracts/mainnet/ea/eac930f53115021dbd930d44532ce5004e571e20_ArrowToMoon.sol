...
contract ArrowToMoon is Context, IERC20, Ownable {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= MAX_TAX_FEE,"Tax fee cant be more than defined max value");
        _taxFee = taxFee;
    }

    function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <= MAX_MARKETING_FEE,"Marketing fee cant be more than defined max value");
        _marketingFee = marketingFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= MAX_LIQUIDITY_FEE,"Liquidity fee cant be more than defined max value");
