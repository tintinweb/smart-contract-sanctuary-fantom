...
contract BGRGame is Ownable {
    ...
    function refundAll() private {
        uint256 totalRefund = numSquaresSold * squarePriceInWei;
        require(
            address(this).balance >= totalRefund,
            "Unable to refund, insufficient balance"
        );

        emit GameCancelled(numSquaresSold, squarePriceInWei);
    ...
    function enterGame() external payable {
        enterGameChoice(numSquaresSold);
    }

    function enterGameChoice(uint256 squareId_) public payable {
        require(
        ...
        _enterOneChoice(squareId_);
    }

    function enterGameMany(uint256 many_) external payable {
        require(
            getGameState() == GameState.DepositsOpen,
            "Deposits are not currently open"
        );

        _checkEnterGame(many_);

        for (uint8 i = 0; i < many_; i++) {
            _enterOne();
        }
    }

    function enterGameChoiceMany(uint256[] memory squareIds_) external payable {
        require(
            getGameState() == GameState.DepositsOpen,
            "Deposits are not currently open"
        );

        _checkEnterGame(squareIds_.length);

        for (uint256 i = 0; i < squareIds_.length; i++) {
            _enterOneChoice(squareIds_[i]);
        }
    }

    function enterGameNft(address contractAddress_, uint256 tokenId_)
        external
        payable
    {
        enterGameChoiceNft(contractAddress_, tokenId_, numSquaresSold);
    }

    function enterGameChoiceNft(
        ...
        // Interactions
        nfts.useNft(contractAddress_, tokenId_, _msgSender());
    }

    function _enterOne() private {
        _enterOneChoice(numSquaresSold);
    }

    function _enterOneChoice(uint256 squareId_) private {
        require(allSquares[squareId_] == address(0), "Square is already sold");
        allSquares[squareId_] = _msgSender();
        emit EnterGame(_msgSender(), squareId_);
        numSquaresSold += 1;
    }

    function _checkEnterGame(uint256 numSquares_) private view {
        require(
        ...
        require(
            msg.value == (squarePriceInWei * numSquares_),
