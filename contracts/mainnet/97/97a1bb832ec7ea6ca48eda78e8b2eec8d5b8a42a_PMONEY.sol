...
contract PMONEY is ERC20Burnable, Operator {
    ...
    function _getPMONEYPrice() internal view returns (uint256 _PMONEYPrice) {
        try IOracle(PMONEYOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Error: failed to fetch PMONEY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _PMONEYPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_PMONEYPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPMONEYOracle(address _PMONEYOracle) public onlyOperatorOrTaxOffice {
        require(_PMONEYOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPMONEYPrice = _getPMONEYPrice();
            currentTaxRate = _updateTaxRate(currentPMONEYPrice);
            if (currentPMONEYPrice < burnThreshold) {
                burnTax = true;
            }
        }
