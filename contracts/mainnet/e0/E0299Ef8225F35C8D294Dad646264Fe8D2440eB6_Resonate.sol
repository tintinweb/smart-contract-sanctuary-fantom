...
contract Resonate is IResonate, ReentrancyGuard {
    ...
    /// TODO: Consider changing this to variable to allow for contract-based dev wallet
    /// The address to which fees are paid
    address private constant DEV_ADDRESS = 0xaF84f7D4061df1AaFbbec39DE7726D4F80beb652;
    ...
    function submitConsumer(bytes32 poolId, uint amount) external nonReentrant {
        ...
        address vaultAsset;
        address adapter = vaultAdapters[pool.vault];
        {
        ...
        // return; // 42406
        if(hasCounterparty) {
            IPoolWallet wallet = _getWalletForPool(poolId); // This is a 40,000 gas tx
                ...
                if(consumerOrder.owner.toAddress() == address(0)) {
                    ...
                    _activateCapital(
                        ParamPacker(
                            ...
                            digestAmt, 
                            producerOrder.depositedShares,  //TODO: I believe this isn't used
                            pool, 
                            adapter, 
                            poolId
                        )
    ...
    function modifyExistingOrder(bytes32 poolId, uint112 amount, uint64 position, bool isProvider) external nonReentrant {
        ...
        if(!isProvider || (order.depositedShares > 0 && !isCrossAsset)) {
            ...
            if(isCrossAsset) {
                ...
                address vaultAsset = IERC4626(vaultAdapters[pool.vault]).asset();

                /// TODO: Refactor with oracles
                uint producerPacket = pool.rate * pool.packetSize / PRECISION * order.depositedShares / PRECISION;

                _getWalletForPool(poolId).withdraw(producerPacket * amount, pool.asset, msg.sender);
    ...
    function batchClaimInterest(uint[][] calldata fnftIds, address recipient) external nonReentrant {
        ...
        for(uint i; i < numberPools; ++i) {
            ...
            // Precision loss from this should be negligible TODO
            ...
            for(uint j; j < fnftsByPool.length; ++j) {
                {
                    ...
                    require(prinPackets > 0, 'ER016');
                    {
                        ...
                        uint sharesRedeemed = prinPackets * (active.sharesPerPacket - newSharesPerPacket); // recover extra precision
    ...
    function claimInterest(uint fnftId, address recipient) public override nonReentrant {
        ...
        // NB: Potential violation of checks-effects-interaction. Likely acceptable within context of ERC-20 transfer to this vault
    ...
    function receiveRevestOutput(
        ...
        if(fnftId == active.principalId) {
            ...
            // NB: This also technically violates checks-effects-interactions
            ...
            // TODO: Are we onboard with tracking residuals as interest?
            emit InterestClaimed(active.poolId, fnftId, tokenHolder, residual);
        } else {
            // This FNFT represents the interest
            require(quantity == 1, 'ER013');
            
            //(uint interest, uint sharesRedeemed) = _getWalletForFNFT(active.poolId).DoSOMETHING here
            ...
            
            {
                ...
                // NB: Technically may constitute a checks-effects-interactions violation. Have reviewed
        ...
        if(packer.isProducerNew) {
            {
                ...
                /// TODO: We aren't charging fees on this???
                /// NB I DON'T THINK WE CHARGE A FEE HERE LIKE WE SHOULD
                /// COME BACK AND FIX THIS CODE IT IS IMPORTANT
            ...
            if(packer.producerOrder.depositedShares > 0 && !packer.isCrossAsset) {
                ...
                IPoolWallet(_getAddressForPool(packer.poolId)).withdraw(amountToConsumer, packer.pool.asset, msg.sender);
            }
        } 
        emit CapitalActivated(packer.poolId, packer.quantityPackets, principalId);  //TODO: deprecate sharesPerPacket???
    }
    ...
    function _initPool(
        ...
        poolId = _getPoolId(asset, vault, rate, _additional_rate, lockupPeriod, packetSize);
        require(pools[poolId].lockupPeriod == 0 && pools[poolId].addInterestRate == 0, 'ER002');
        queueMarkers[poolId] = PoolQueue(1, 1, 1, 1);
        pools[poolId] = PoolConfig(asset, vault, rate, _additional_rate, lockupPeriod, packetSize);
    }
    ...
    function _getPoolId(
        ...
        poolId = keccak256(abi.encodePacked(asset, vault, rate, _additional_rate, lockupPeriod, packetSize));
    }
    ...
    /**
     * @notice View methods used to batch-deliver information about a position in Resonate
     * @param fnftId The ID of the FNFT to share information on
     * @return rate The upfront payout rate associated with the pool the FNFT is associated with
     * @return addInterestRate The additional rate associated with the pool the FNFT is associated with. Will be zero for fixed-term
     * @return packetSize The packet size associated with the pool the FNFT is associated with
     * @return startingInterestAccumulator The number of shares per packet that were present in the FNFT at creation – will allow us to derive total accumulated interest
     * @return vault The vault associated with the pool the FNFT is associated with
     * @return sharesPerPacket the number of shares per packet that are currently associated with the FNFT
     * @return residual The residual number of shares associated with the interest-bearing FNFT. Will return zero if fnftId is associated with a principal-bearing FNFT
     * @dev This method is primarily included to reduce the number of calls needed from external contracts and thereby reduce gas costs
     */
    function getPositionInfo(uint fnftId) external view override returns (uint80 rate, uint80 addInterestRate, uint packetSize, uint256 startingInterestAccumulator, address vault, uint256 sharesPerPacket, uint residual) {
        Active memory active = activated[fnftIdToIndex[fnftId]];
        PoolConfig memory pool = pools[active.poolId];
        rate = pool.rate;
        addInterestRate = pool.addInterestRate;
        packetSize = pool.packetSize;
        vault = vaultAdapters[pool.vault];
        startingInterestAccumulator = active.startingSharesPerPacket;
        sharesPerPacket = active.sharesPerPacket;
        residual = residuals[fnftId];
    }
...
interface IResonate {
    ...
    struct PoolConfig {
        ...
        address vault;
        uint80  rate;
        uint80  addInterestRate; //Amount additional (10% on top of the 30%) - If not a bond then just zero
        uint32  lockupPeriod;
        uint256 packetSize;
    }
    ...
    event PoolCreated(bytes32 indexed poolId, address indexed asset, address indexed vault, address payoutAsset, uint80 rate, uint80 addInterestRate, uint32 lockupPeriod, uint256 packetSize, bool isFixedTerm, string poolName, address creator);
    ...
    /// TODO: Some of these may be extraneous – we will discover this as we develop further

    // TODO: Does this need more args? 
    ...
    function getPositionInfo(uint fnftId) external view returns (uint80 rate, uint80 addInterestRate, uint packetSize, uint256 startingInterestAccumulator, address vault, uint256 sharesPerPacket, uint residual);
...
interface ISmartWallet {
    ...
    function proxyCall(address token, address vault, address[] memory targets, uint256[] memory values, bytes[] memory calldatas) external;

    function withdrawOrDeposit(address vaultToken, address vaultAdapter, uint amount, bool isWithdrawal) external;

    /// TODO: How to safely implement governance execution functions? Need to ensure call functions cannot spend tokens
    /// TODO: They likely cannot just because there's no way to call approve() from this contract using arbitrary execution
    ///       Given that it is "call" and not "delegatecall"
