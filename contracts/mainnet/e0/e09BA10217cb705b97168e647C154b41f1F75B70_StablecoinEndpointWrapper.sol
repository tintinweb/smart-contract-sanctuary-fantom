...
interface IYamaRouter {
  ...
  function getEndpointToken(uint256 endpointId) external view returns (
    address token);

  function getEndpointLPToken(uint256 endpointId) external view returns (
    address lpToken);
  ...
  // 10 ** decimals() scales depositorRatio().
  // If 1 LP token = 2 underlying tokens, then depositorRatio() is
  // 2 * (10 ** decimals())
  ...
  function deposit(uint256 id, uint256 amount) external returns (
  ...
  function withdraw(uint256 id, uint256 lpTokenAmount) external returns (
