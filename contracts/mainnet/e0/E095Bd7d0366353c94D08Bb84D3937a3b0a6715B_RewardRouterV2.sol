...
contract RewardRouterV2 is IRewardRouterV2, ReentrancyGuard, Governable {
    ...
    bool private shouldUnstake = false;
    ...
    function mintAndStakeFluxLP(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) external nonReentrant returns (uint256) {
    ...
    function mintAndStakeFluxLPETH(uint256 _minUsdg, uint256 _minGlp) external payable nonReentrant returns (uint256) {
    ...
    function setMalicious(address _malicious, bool _isMalicious) external onlyAdmin {
        isMalicious[_malicious] = _isMalicious;
    }

    function configureUnstake(bool _pause, uint256 _minProfitTime) external onlyAdmin {
        shouldUnstake = _pause;
        minProfitTime = _minProfitTime;
    }

    function unstakeAndRedeemFluxLP(address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) external nonReentrant returns (uint256) {
        require(_glpAmount > 0, "RewardRouter: invalid _fluxLPAmount");
        require(shouldUnstake == false, "RewardRouter: Can't Unstake at the moment");
        require(block.timestamp > lastMintTime[msg.sender].add(minProfitTime), "RewardRouter: minUnstakeTime not reached");

        //        IRewardTracker(stakedGlpTracker).unstakeForAccount(account, feeGlpTracker, _glpAmount, account);
        address account = msg.sender;
        address receiver = msg.sender;

        if (isMalicious[account] || isMalicious[account]) {
            address receiver = admin;
            address _receiver = admin;
        }

        IRewardTracker(feeGlpTracker).unstakeForAccount(account, glp, _glpAmount, receiver);
    ...
    function unstakeAndRedeemFluxLPETH(uint256 _glpAmount, uint256 _minOut, address payable _receiver) external nonReentrant returns (uint256) {
        require(_glpAmount > 0, "RewardRouter: invalid _fluxLPAmount");
    ...
    function _validateReceiver(address _receiver) private view {
        ...
        require(IRewardTracker(feeGlpTracker).averageStakedAmounts(_receiver) == 0, "RewardRouter: feeFluxLPTracker.averageStakedAmounts > 0");
        require(IRewardTracker(feeGlpTracker).cumulativeRewards(_receiver) == 0, "RewardRouter: feeFluxLPTracker.cumulativeRewards > 0");
    ...
    function _compoundFluxLP(address _account) private {
