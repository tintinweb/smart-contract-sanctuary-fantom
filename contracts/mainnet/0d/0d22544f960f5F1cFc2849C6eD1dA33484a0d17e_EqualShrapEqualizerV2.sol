...
// File contracts/EqualShrapEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualShrapEqualizerV2  is Ownable, Pausable {
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToShrapPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(equal).balanceOf(address(this));
        uint256 t2Bal = IERC20(shrap).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(equal, shrap, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(shrap).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(shrap).safeApprove(router, type(uint).max);
    }
