...
/**
 * @title scUnderlyingTransferHelper
 * @author beepidibop.eth
 * @notice Helper contract that multisig can call to queue/execute multiple txs
 *			on the timelock at once
 * @dev Flow:
 *		1. getReduceBy with array of scToken addresses, save the list of amounts
 *      2. queueReduceReserves()
 *      3. wait for timelock
 *      4. execReduceReserves()
 */
...
contract ScUnderlyingTransferHelper {
    /**
     * @notice Queue txs for _reduceReserves() and transfering the token to destination
     * @param timelock address of timelock contract
     * @param scTokens list of scTokens
     * @param reduceAmounts amount to reduce the totalReserve by
     * @param destination address of where to send the tokens to
     * @param eta time when multisig can execute the tx in timelock
     */
    function queueReduceReserves(
        ITimelock timelock,
        address[] calldata scTokens,
        uint256[] calldata reduceAmounts,
        address destination,
        uint256 eta
    ) external {
        require(scTokens.length == reduceAmounts.length, "length mismatch");
        for (uint256 i; i < scTokens.length; i++) {
            // populate tx data for _reduceReservesData() and transfer()
            (
                bytes memory _reduceReservesData,
                bytes memory transferData
            ) = populateTxData(reduceAmounts[i], destination);

            // queuing the _reduceReservesData() txs
            ...
            // queuing the transfer() txs
            address underlying = CErc20Interface(scTokens[i]).underlying();
            timelock.queueTransaction(underlying, 0, "", transferData, eta);
        }
    }

    /**
     * @notice Executes txs for _reduceReserves() and transfering the token to destination
     * @param timelock address of timelock contract
     * @param scTokens list of scTokens
     * @param reduceAmounts amount to reduce the totalReserve by
     * @param destination address of where to send the tokens to
    ...
    function execReduceReserves(
        ...
        for (uint256 i; i < scTokens.length; i++) {
            ...
            // queuing the transfer() txs
            address underlying = CErc20Interface(scTokens[i]).underlying();
            timelock.executeTransaction(underlying, 0, "", transferData, eta);
        }
    }
    ...
    /**
     * @notice Get reduce amount for a list of scTokens
     * @param scTokens list of scTokens
     * @param reduceRatio ratio to reduce the totalReserve by, out of 10000
     * @dev Save the output somewhere as we'll need it to execute the queued tx after timelock expires
     */
    function getReduceBy(address[] calldata scTokens, uint256 reduceRatio)
        external
        view
        returns (uint256[] memory reduceAmounts)
    {
        reduceAmounts = new uint256[](scTokens.length);
        for (uint256 i; i < scTokens.length; i++) {
            CTokenInterface scToken = CTokenInterface(scTokens[i]);
            uint256 totalReserves = scToken.totalReserves();
            reduceAmounts[i] = (totalReserves * reduceRatio) / 10000;
        }
    }

    /**
     * @notice Populate tx data for _reduceReserves() and transfering the token to destination
     * @param reduceAmount amount to reduce the totalReserve by
     * @param destination address of where to send the tokens to
     * @param _reduceReservesData tx data for _reduceReserves()
     * @param transferData tx data for transfer()
     */
    function populateTxData(uint256 reduceAmount, address destination)
        public
        pure
        returns (bytes memory _reduceReservesData, bytes memory transferData)
    {
        _reduceReservesData = abi.encodeWithSignature(
