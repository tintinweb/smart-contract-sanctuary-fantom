...
contract EsBfxBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esBfx;

    constructor(address _esBfx) public {
        admin = msg.sender;
        esBfx = _esBfx;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsBfxBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esBfx).transferFrom(msg.sender, _accounts[i], _amounts[i]);
