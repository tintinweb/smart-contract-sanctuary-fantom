...
contract SupplyVaultAPRHelper is Ownable {
    ...
    function setFeeDistributor(address _supplyVault, address _feeDistributor) external onlyOwner {
        // NOTE: We allow address(0) here to disable
        feeDistributors[_supplyVault] = _feeDistributor;
    }

    function getAPREstimate(address _supplyVault) external returns (uint256 apr) {
        ISupplyVault supplyVault = ISupplyVault(_supplyVault);
        apr = supplyVault.getSupplyRate();
        address feeDistributorAddress = feeDistributors[_supplyVault];
        if (feeDistributorAddress != address(0)) {
            ...
            uint256 feeDistributorBalance = underlying.balanceOf(feeDistributorAddress);
            uint256 supplyVaultTotalUnderlying = supplyVault.getTotalUnderlying();
            apr = apr.add(
                feeDistributorBalance.mul(1e18).div(supplyVaultTotalUnderlying).div(feeDistributor.periodLength())
            );
        }
        if (supplyVault.feeTo() != address(0)) {
            apr = apr.mul(MAX_BPS.sub(supplyVault.feeBps())).div(MAX_BPS);
        }
        apr = apr.mul(SECONDS_PER_YEAR);
    }
}
