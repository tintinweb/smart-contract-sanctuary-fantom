    ...
    ProxyImplementation
{
        ...
        voterProxyAssetsSet
    {
        ...
        if (solidClaimed) {
            ...
            if (
                ISolidGauge(gaugeAddress).balanceOf(voterProxyAssetsAddress) > 0
            ) {
                voterProxyAssets.withdrawFromGauge(
                    solidPoolAddress,
                    gaugeAddress,
        ...
        returns (bool _claimSolid)
    {
        ...
        if (
            ...
            claimDisabledByOxPoolAddress[oxPoolAddress]
        ) {
            return false;
        }
        IOxPool oxPool = IOxPool(oxPoolAddress);
     ...
     * @param tokenAddress Reward token address
     */
    function oogSyncLimit(address gaugeAddress, address tokenAddress)
        public
        view
        returns (bool _getReward)
    {
        ...
        uint256 lastUpdateTime = Math.max(
            gauge.lastEarn(tokenAddress, address(this)),
        ...
        uint256 numCheckpoints = gauge.numCheckpoints(address(this));
        uint256 lag;
