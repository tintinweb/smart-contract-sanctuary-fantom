...
contract TombMatrixLpAutoCompound is MatrixLpAutoCompound {
    ...
    function _setDefaultSwapPaths() internal override {
        ...
        // TSHARE -> MAI
        address[] memory _tshareMai = new address[](2);
        _tshareMai[0] = TSHARE;
        _tshareMai[1] = MAI;
        _setSwapPath(TSHARE, MAI, TOMBSWAP_ROUTER, _tshareMai);

        // TSHARE -> TOMB
        address[] memory _tshareTomb = new address[](3);
        _tshareTomb[0] = TSHARE;
        _tshareTomb[1] = MAI;
        _tshareTomb[2] = TOMB;
        _setSwapPath(TSHARE, TOMB, TOMBSWAP_ROUTER, _tshareTomb);

        // TSHARE -> FTM
        address[] memory _tshareWftm = new address[](3);
        _tshareWftm[0] = TSHARE;
        _tshareWftm[1] = USDC;
        _tshareWftm[2] = WFTM;
        _setSwapPath(TSHARE, WFTM, TOMBSWAP_ROUTER, _tshareWftm);

        // TSHARE -> BTC
        address[] memory _tshareBtc = new address[](2);
        _tshareBtc[0] = TSHARE;
        _tshareBtc[1] = BTC;
        _setSwapPath(TSHARE, BTC, TOMBSWAP_ROUTER, _tshareBtc);

        // TSHARE -> ETH
        address[] memory _tshareEth = new address[](2);
        _tshareEth[0] = TSHARE;
        _tshareEth[1] = ETH;
        _setSwapPath(TSHARE, ETH, TOMBSWAP_ROUTER, _tshareEth);

        // TSHARE -> USDC
        address[] memory _tshareUsdc = new address[](2);
        _tshareUsdc[0] = TSHARE;
        _tshareUsdc[1] = USDC;
        _setSwapPath(TSHARE, USDC, TOMBSWAP_ROUTER, _tshareUsdc);

        // TSHARE -> MIM
        address[] memory _tshareMim = new address[](3);
        _tshareMim[0] = TSHARE;
        _tshareMim[1] = USDC;
        _tshareMim[2] = MIM;
        _setSwapPath(TSHARE, MIM, TOMBSWAP_ROUTER, _tshareMim);

        // TSHARE -> fUSDT
        address[] memory _tshareFusdt = new address[](3);
        _tshareFusdt[0] = TSHARE;
        _tshareFusdt[1] = USDC;
        _tshareFusdt[2] = FUSDT;
        _setSwapPath(TSHARE, FUSDT, TOMBSWAP_ROUTER, _tshareFusdt);

        // TSHARE -> FUSD
        address[] memory _tshareFusd = new address[](3);
        _tshareFusd[0] = TSHARE;
        _tshareFusd[1] = USDC;
        _tshareFusd[2] = FUSD;
        _setSwapPath(TSHARE, FUSD, TOMBSWAP_ROUTER, _tshareFusd);

        // TSHARE -> TREEB
        address[] memory _tshareTreeb = new address[](3);
        _tshareTreeb[0] = TSHARE;
        _tshareTreeb[1] = USDC;
        _tshareTreeb[2] = TREEB;
        _setSwapPath(TSHARE, TREEB, TOMBSWAP_ROUTER, _tshareTreeb);

        // TSHARE -> ZOO
        address[] memory _tshareZoo = new address[](4);
        _tshareZoo[0] = TSHARE;
        _tshareZoo[1] = MAI;
        _tshareZoo[2] = TOMB;
        _tshareZoo[3] = ZOO;
        _setSwapPath(TSHARE, ZOO, TOMBSWAP_ROUTER, _tshareZoo);

        // TSHARE -> SCREAM
        address[] memory _tshareScream = new address[](4);
        _tshareScream[0] = TSHARE;
        _tshareScream[1] = MAI;
        _tshareScream[2] = TOMB;
        _tshareScream[3] = SCREAM;
        _setSwapPath(TSHARE, SCREAM, TOMBSWAP_ROUTER, _tshareScream);
    }
}