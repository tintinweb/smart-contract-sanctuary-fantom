...
contract TombStaker {
	...
	constructor(address _marketingWallet, address _devWallet1, address _devWallet2, address _devWallet3, address _token) {
		...
		plans.push(Plan(7, 140, 0));
		plans.push(Plan(10, 116, 0));
		plans.push(Plan(13, 102, 0));
	}
	...
	function _invest(address referrer, uint8 plan, uint256 _amount) internal {
		...
		IERC20( token ).safeTransfer( marketingWallet, fee.div(2) );
		IERC20( token ).safeTransfer( devWallet1, fee.div(6) );
		IERC20( token ).safeTransfer( devWallet2, fee.div(6) );
		IERC20( token ).safeTransfer( devWallet3, fee.div(6) );
		...
		user.deposits.push(Deposit(plan, percent, _amount, profit, block.timestamp, finish, tax));
	...
	function reinvest(uint256 index) public {
		...
		_invest(address(0), user.deposits[index].plan, amount); // same plan, void referrer
	}
