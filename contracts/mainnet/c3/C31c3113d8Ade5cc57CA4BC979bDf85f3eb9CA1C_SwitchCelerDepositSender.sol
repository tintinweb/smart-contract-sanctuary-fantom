...
contract SwitchCelerDepositSender is Switch {
    ...
    function getSgnFeeForDeposit(
        CelerDepositRequest calldata request
    )
        external
        view
    ...
    {
        require(depositArgs.depositInfo.toApprovalAddress != address(0), "The approval address shouldn't be zero");
        ...
        if ((IERC20(depositArgs.srcSwap.srcToken).isETH() && IERC20(depositArgs.srcSwap.dstToken).isETH()) ||
            (IERC20(depositArgs.srcSwap.srcToken).isETH() && (depositArgs.srcSwap.dstToken == nativeWrap))
        ) {
        ...
        bytes memory message = abi.encode(
            CelerDepositRequest({
                ...
                dstDistribution: depositArgs.dstDistribution,
                depositInfo: depositArgs.depositInfo
            })
        );
        ...
        if (depositArgs.srcSwap.srcToken == depositArgs.srcSwap.dstToken) {
            ...
            if ((depositArgs.paraswapUsageStatus == DataTypes.ParaswapUsageStatus.OnSrcChain) || (depositArgs.paraswapUsageStatus == DataTypes.ParaswapUsageStatus.Both)) {
                returnAmount = _swapFromParaswap(depositArgs, amountAfterFee);
            } else {
                (returnAmount, ) = _swapBeforeCeler(depositArgs, amountAfterFee);
            }
        }
        ...
        transferId = MessageSenderLib.sendMessageWithTransfer(
            depositArgs.callTo,
            bridgeTokenIsNative ? nativeWrap : depositArgs.srcSwap.dstToken,
            returnAmount,
            depositArgs.dstChainId,
            depositArgs.nonce,
            depositArgs.bridgeSlippage,
            message,
            depositArgs.bridgeTransferType,
            celerMessageBus,
            sgnFee
        );
    ...
    function _swapBeforeCeler(DepositArgsCeler calldata depositArgs, uint256 amount) private returns (uint256 returnAmount, uint256 parts) {
        parts = 0;
        uint256 lastNonZeroIndex = 0;
        for (uint i = 0; i < depositArgs.srcDistribution.length; i++) {
            if (depositArgs.srcDistribution[i] > 0) {
                parts += depositArgs.srcDistribution[i];
                lastNonZeroIndex = i;
            }
        }
        ...
        // break function to avoid stack too deep error
        returnAmount = _swapInternalForSingleSwap(depositArgs.srcDistribution, amount, parts, lastNonZeroIndex, IERC20(depositArgs.srcSwap.srcToken), IERC20(depositArgs.srcSwap.dstToken));
        require(returnAmount > 0, "Swap failed from dex");

        switchEvent.emitSwapped(msg.sender, address(this), IERC20(depositArgs.srcSwap.srcToken), IERC20(depositArgs.srcSwap.dstToken), amount, returnAmount, 0);
    }
    ...
    function _emitCrossChainDepositRequest(DepositArgsCeler calldata depositArgs, bytes32 transferId, uint256 returnAmount, address sender, DataTypes.DepositStatus status) internal {
        switchEvent.emitCrosschainDepositRequest(
            ...
            depositArgs.bridge,
            sender,
            depositArgs.depositInfo.depositContract,
            depositArgs.depositInfo.toApprovalAddress,
