...
contract Controller is IController, Governable {
    ...
    // We pack strategy data into one struct for less gas.
    struct StrategyData {
    ...
    /// @notice Data for each strategy contract.
    ...
    function notifyFee(address _token, uint256 _fee) external override {
        ...
        // Record strategy profits.
        StrategyData storage data = strategyData[msg.sender];
        data.lastHarvestTimestamp = uint32(block.timestamp);
        if((data.epoch + 30 days) < block.timestamp) {
            data.epoch = uint32(block.timestamp);
            data.profits = 0;
        }
        data.profits += feeAfter;
