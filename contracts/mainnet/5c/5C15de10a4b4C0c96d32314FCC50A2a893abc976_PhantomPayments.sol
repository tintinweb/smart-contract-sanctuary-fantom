...
contract PhantomPayments is PhantomStorageMixin, IPhantomPayments {
    ...
    function claimSalary() external override {
        ...
        require(PhantomStorage().getUint(
                keccak256(abi.encodePacked(phantom.treasury.balances, paymentToken[msg.sender], daoKey()))
        ) >= payment, "PhantomPayments: Not enough balance in treasury");
