...
// No permissions granted before Saturday, 10 Aug 2024, then GPL-3.0 after this date.
...
error NyPtvFantomUsdcFusdtEqualizerStrategyToUsdc__MaxSlippageOutOfBounds(uint256 maxSlippage);

contract NyPtvFantomUsdcFusdtEqualizerStrategyToUsdc is NYBaseProfitTakingStrategyV3 {
  ...
  function _swapFarmEmissionTokens() internal {
    ...
    if (equalToUsdcPath.length >= 2 && equalBalance > 0) {
      ...
      IEqualizerRouter.Route[] memory routes = new IEqualizerRouter.Route[](2);
      routes[0] = IEqualizerRouter.Route({from: EQUAL, to: WFTM, stable: useStable});
      routes[1] = IEqualizerRouter.Route({from: WFTM, to: USDC, stable: useStable});
      uint[] memory amounts = router.getAmountsOut(equalBalance, routes);
      uint percentage = (amounts[2] * maxSlippage) / 10000;
  ...
  function updateMaxSlippage(uint256 _maxSlippage) external {
    if (_maxSlippage > 10000) {
      revert NyPtvFantomUsdcFusdtEqualizerStrategyToUsdc__MaxSlippageOutOfBounds(_maxSlippage);
    }
    _atLeastRole(ADMIN);
