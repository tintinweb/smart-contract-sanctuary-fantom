...
interface IMasterChef {
    ...
    function userBalances(address pool, address _user) external view returns (uint256, uint256);
    function pendingRewards(address _user, address pool) external view returns (uint256);
}
...
contract SolidexStrategy is Ownable, Pausable {
    ...
    function callReward() public view returns (uint256) {
        uint256 outputBal = IMasterChef(masterchef).pendingRewards(address(this), want);
        uint256 nativeOut;
