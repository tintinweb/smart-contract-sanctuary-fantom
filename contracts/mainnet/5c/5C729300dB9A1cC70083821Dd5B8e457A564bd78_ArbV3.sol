...
contract ArbV3 is Operator {
    ...
    function setRewarPool(address _rewardPool) public onlyOperator {
    ...
    function createConfig(
        ...
        configs.push(
            Config(_privatePool, _publicPool, _asset, _base, _threshold)
        );
    }
    ...
    function estimateDualDexTradeV2(uint256 _index, uint256 _amount)
        external
        view
        returns (uint256)
    {
        (
            ...
            address _token1,
            address _token2
        ) = this.comparePrice(_index, _amount);
        uint256 amtBack1 = getAmountOutMin(_router1, _token1, _token2, _amount);
        uint256 amtBack2 = getAmountOutMin(
            _router2,
            _token2,
    ...
    function dualDexTradeV2(uint256 _index, uint256 _amount)
        external
        onlyOperator
    {
        (
    ...
    function comparePrice(uint256 _index, uint256 _amount)
        external
        view
        returns (
            bool,
        ...
        )
    {
        ...
        uint256 privateAmountRequired = IUniswapV2Router(config.privatePool)
            .getAmountsOut(_amount, pairs)[1];
        uint256 publicAmountRequired = IUniswapV2Router(config.publicPool)
            .getAmountsOut(_amount, pairs)[1];
        if (privateAmountRequired >= publicAmountRequired) {
            ...
            uint256 priceDifPercentage = priceDif.mul(10000).div(
                publicAmountRequired
            );
