...
contract Treasury is ITreasury, Operator, ReentrancyGuard {
    ...
    // Strategies for using collateral to earn profit in Lending or yield aggregator protocol
    mapping(address => bool) public strategists;
    address[] public strategists_array;
    ...
    //re-balance function
    uint256 public rebalance_cooldown = 3 hours;
    ...
    modifier onlyStrategists {
        require(strategists[msg.sender], "Only strategist can use this function");
        _;
    }
    ...
    constructor(address _collateral, address _router) public {
        ratio_step = 2500;
        // = 0.25% at 6 decimals of precision
        target_collateral_ratio = 980000;
        // = 99% - fully collateralized at start
        effective_collateral_ratio = 980000;
        // = 99% - fully collateralized at start
    ...
    function initializing(address _dollar, address _share, address _oracleDollar, address _oracleShare, address _collateralFund, address _daoFund) external onlyOperator {
    ...
    function dollarPrice() public view returns (uint256) {
        return IOracle(oracleDollar).consult(dollar, 1e18);
    }

    function sharePrice() public view returns (uint256) {
        return IOracle(oracleShare).consult(share, 1e18);
    }
    ...
    function hasStrategist(address _strategist) external view override returns (bool) {
        return strategists[_strategist] == true;
    }
    ...
    function globalCollateralBalance() public view override returns (uint256) {
        uint256 _collateralReserveBalance = IERC20(collateral).balanceOf(collateralFund);
    ...
    function globalCollateralValue() public view override returns (uint256) {
        return
        (globalCollateralBalance() * PRICE_PRECISION * (10 ** missing_decimals)) /
        PRICE_PRECISION;
    }
    ...
    function extractProfit(uint256 _amount) external onlyProfitController {
        ...
        _maxExcess -
        ((_maxExcess * excess_collateral_safety_margin) / RATIO_PRECISION);
        require(_amount <= _maxAllowableAmount, "Excess allowable amount");
        ICollateralFund(collateralFund).transferTo(collateral, profitSharingFund, _amount);
        emit ProfitExtracted(_amount);
    }

    //TODO fix buyback function
    //1s fix attemp: failed
    //2nd fix attemp:

    // Use excess collateral to buy back Alpha, then using Alpha to buy back xShare and burn
    function buyback(uint256 _collateral_amount, uint256 _min_share_amount, bool useSlip) external override onlyOperator notMigrated checkRebalanceCooldown {
        ...
        uint256 _collateral_value = _collateral_amount * PRICE_PRECISION * (10 ** missing_decimals) / PRICE_PRECISION;
        require(_collateral_amount > 0 && _collateral_value < _excess_collateral_value, "Invalid collateral amount");
        ICollateralFund(collateralFund).transferTo(collateral, address(this), _collateral_amount);
        //        uint256 collateralBalance = globalCollateralBalance();
        //        require(_exceeded && _excess_collateral_value > 0, "!exceeded");
        //        require(_collateral_amount > 0 && _collateral_amount < _excess_collateral_value, "invalidCollateralAmount");
        //        require(collateralBalance > _collateral_amount, "insufficentPoolBalance");
        //        // Transfer collateral from pool to treasury to execute _swap
        //        ICollateralFund(collateralFund).transferTo(collateral, address(this), _collateral_amount);
        //        uint256 min_dollar_out = 0;
        //        if (useSlip) {
        //            min_dollar_out = _collateral_amount.mul(dollarPrice()).div(PRICE_PRECISION).mul(99000).div(100000);
        //        }
        //        // Buy back dollar
        //        uint256 out_dollar_amount = _swap(collateral, dollar, _collateral_amount, min_dollar_out);
        //        // Buy back share with dollar
        //        uint256 out_xShare_amount = _swap(dollar, share, out_dollar_amount, _min_share_amount);
        //
        //        ERC20Burnable(share).burnFrom(address(this), out_xShare_amount);
        emit BoughtBackAndBurned(_collateral_amount, _collateral_amount, 0);
    }

    function buyback1(uint256 _collateral_amount, uint256 _min_share_amount, bool useSlip) external onlyOperator notMigrated checkRebalanceCooldown {
        (uint256 _excess_collateral_value, bool _exceeded) = calcCollateralBalance();
        require(_exceeded && _excess_collateral_value > 0, "!exceeded");
        uint256 _collateral_value = _collateral_amount * PRICE_PRECISION * (10 ** missing_decimals) / PRICE_PRECISION;
        require(_collateral_amount > 0 && _collateral_value < _excess_collateral_value, "Invalid collateral amount");
        ...
        uint256 out_dollar_amount = _swap(collateral, dollar, _collateral_amount, min_dollar_out);
        emit BoughtBackAndBurned(_collateral_amount, _collateral_amount, out_dollar_amount);
    }

    function buyback2(uint256 _collateral_amount, uint256 _min_share_amount, bool useSlip) external onlyOperator notMigrated checkRebalanceCooldown {
        (uint256 _excess_collateral_value, bool _exceeded) = calcCollateralBalance();
        require(_exceeded && _excess_collateral_value > 0, "!exceeded");
        uint256 _collateral_value = _collateral_amount * PRICE_PRECISION * (10 ** missing_decimals) / PRICE_PRECISION;
        require(_collateral_amount > 0 && _collateral_value < _excess_collateral_value, "Invalid collateral amount");
    ...
    //TODO added transfer share from daoFund to treasury, need to recheck on next deploy
    //Ready for next deploy testing

    // Transfer xShare from Dao Fund to sell and recollateraliza
    function recollateralize(uint256 _share_amount, uint256 _min_collateral_amount) external override onlyOperator notMigrated checkRebalanceCooldown {
        ...
        IDaoFund(daoFund).transferTo(share, address(this), _share_amount);
        uint256 out_collateral_amount = _swap(share, collateral, _share_amount, _min_collateral_amount);
        uint256 _collateral_balance = IERC20(collateral).balanceOf(collateralFund);
        if (_collateral_balance > 0) {
            IERC20(collateral).safeTransfer(collateralFund, _collateral_balance);
    ...
    // Add new Strategist
    function addStrategist(address strategist_address) public onlyOperator notMigrated {
        require(strategists[strategist_address] == false, "strategist existed");
        strategists[strategist_address] = true;
        strategists_array.push(strategist_address);
    }
    ...
    // Remove a strategist
    function removeStrategist(address strategist_address) public onlyOperator notMigrated {
        require(strategists[strategist_address] == false, "Strategist not existed");
        // Delete from mapping
        delete strategists[strategist_address];
        // Remove from array by replace with null address
        for (uint256 i = 0; i < strategists_array.length; i++) {
            if (strategists_array[i] == strategist_address) {
                strategists_array[i] = address(0);
                break;
            }
        }
    }
    ...
    //TODO changed from internal to public and onlyOperator to test swap function
    // 1st attempt to fix swap function
    // Function _swap worked!
    function _swap(address inputToken, address outputToken, uint256 inputAmount, uint256 outputAmountMin) public onlyOperator returns (uint256) {
        ...
        require(inputTokenBalance > inputAmount, "not enough token balance");
    ...
    function setCollateralAddress(address _collateral) public onlyOperator {
