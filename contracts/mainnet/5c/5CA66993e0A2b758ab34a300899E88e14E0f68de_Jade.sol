...
import "./interfaces/IShareBond.sol";

/**
 * @title Jade
 * Minter must be RewardPool of ClaimToken
 */
contract Jade is ERC20Burnable, AccessControlEnumerable, ReentrancyGuard, IERC20Burnable, IERC20Mintable, IShareBond {
    using SafeERC20 for IERC20;
    address public claimToken;
    uint256 public claimTokenPriceCeiling;
    ...
    constructor(address claimToken_) ERC20("JADE", "JADE") {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        ...
        interfaceId == type(IShareBond).interfaceId;
    }

    /**
     * Can only claim when above price target
     */
    function claim(uint256 amount_) external override nonReentrant {
        uint256 price = IOracleToken(claimToken).getPriceUpdated();
        if(price < claimTokenPriceCeiling) revert PriceNotEligible();
    ...
    function setClaimTokenPriceCeiling(uint256 price_) external override onlyOperator {
        claimTokenPriceCeiling = price_;
    }
...
interface IShareBond {
    function claim(uint256 amount_) external;
    function setClaimTokenPriceCeiling(uint256 price_) external;
}
