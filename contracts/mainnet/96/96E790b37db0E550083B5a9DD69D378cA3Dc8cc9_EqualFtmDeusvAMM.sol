...
// File: contracts/EqualFtmDeusvAMM.sol
...
contract EqualFtmDeusvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public wrappedToDeusPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToDeusPath
    ) {
        ...
        for (uint i; i < _wrappedToDeusPath.length; ++i) {
            wrappedToDeusPath.push(_wrappedToDeusPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToDeusPath, address(this), block.timestamp);
        
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(deus).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, deus, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
