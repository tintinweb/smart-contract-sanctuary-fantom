...
contract Treasury is Initializable, ContractGuard {
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of Chili price
    uint256 public bootstrapEpochs;
    uint256 public bootstrapSupplyExpansionPercent;

    /* =================== Added variables =================== */
    uint256 public previousEpochChiliPrice;
    uint256 public maxDiscountRate; // when purchasing bond
    uint256 public maxPremiumRate; // when redeeming bond
    uint256 public discountPercent;
    uint256 public premiumThreshold;
    uint256 public premiumPercent;
    uint256 public mintingFactorForPayingDebt; // print extra Chili during debt phase

    address public _chiliFund;
    ...
    event ChiliFundFunded(uint256 timestamp, uint256 seigniorage);
    ...
    function __initializeOperator__() internal initializer {
        operator = msg.sender;
    }
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getChiliPrice() > _chiliPriceCeiling)
            ? 0
            : getChiliCirculatingSupply().mul(maxSupplyContractionPercent).div(
                10000
            );
    }
    ...
    // oracle
    function getChiliPrice() public view returns (uint256 _chiliPrice) {
        try IOracle(_chiliOracle).consult(_chili, 1e18) returns (
            uint144 price
        ) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult Chili price from the oracle");
        }
    }

    function getChiliUpdatedPrice() public view returns (uint256 _chiliPrice) {
    ...
    function getBurnableChiliLeft()
        public
        view
        returns (uint256 _burnableChiliLeft)
    {
        uint256 _chiliPrice = getChiliPrice();
        if (_chiliPrice <= _chiliPriceOne) {
            uint256 _chiliSupply = getChiliCirculatingSupply();
            ...
            if (_cbondMaxSupply > _cbondSupply) {
                ...
                uint256 _maxBurnableChili = _maxMintableBond
                    .mul(_chiliPrice)
                    .div(1e18);
                _burnableChiliLeft = MathUpgradeable.min(
        ...
        returns (uint256 _redeemableBonds)
    {
        ...
        if (_chiliPrice > _chiliPriceCeiling) {
            uint256 _totalChili = IERC20Upgradeable(_chili).balanceOf(
                address(this)
            );
            uint256 _rate = getBondPremiumRate();
            if (_rate > 0) {
                _redeemableBonds = _totalChili.mul(1e18).div(_rate);
            }
        }
    }
    ...
    function setTime(uint256 _startTime) external onlyOperator {
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn Chili and mint BOND)
    ...
    function _updateChiliPrice() internal {
        try IOracle(_chiliOracle).update() {} catch {}
    }

    function getChiliCirculatingSupply() public view returns (uint256) {
        ...
        checkOperator
    {
        ...
        uint256 ChiliPrice = getChiliPrice();
        require(ChiliPrice == targetPrice, "Treasury: _chili price moved");
        require(
            ChiliPrice < _chiliPriceOne, // price < $1
            "Treasury: ChiliPrice not eligible for bond purchase"
        );
        ...
        checkOperator
    {
        ...
        uint256 ChiliPrice = getChiliPrice();
        require(ChiliPrice == targetPrice, "Treasury: Chili price moved");
        require(
            ChiliPrice > _chiliPriceCeiling, // price > $1.01
            "Treasury: ChiliPrice not eligible for bond purchase"
        );
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        ...
        if (_chiliFundSharedPercent > 0) {
            ...
            emit ChiliFundFunded(block.timestamp, _chiliFundSharedAmount);
        }
    ...
    {
        _updateChiliPrice();
        previousEpochChiliPrice = getChiliPrice();
        uint256 _chiliSupply = getChiliCirculatingSupply().sub(
            seigniorageSaved
        );
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochChiliPrice > _chiliPriceCeiling) {
                // Expansion ($Chili Price > 1 $FTM): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20Upgradeable(_cbond).totalSupply();
                uint256 _percentage = previousEpochChiliPrice.sub(
                    _chiliPriceOne
                );
                ...
                if (
                    ...
                    // have not saved Chili enough to pay debt, mint more Chili
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(_chili), "Chili");
...
interface IBoardroom {
    ...
    function getChiliPrice() external view returns (uint256);
