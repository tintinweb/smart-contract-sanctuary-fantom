...
import "./Error.sol";
import "./UncappedRoyaltyNFT.sol";

contract DirectSaleRoyalty is Ownable, Pausable {
  ...
  function openForSale(
    ...
    if (msg.sender != UncappedRoyaltyNFT(_nft).ownerOf(_tokenID)) revert CallerNotNFTOwner(); //replace royaltnft here
    // should be a way to check if owner has approved
    // something like UncappedRoyaltyNFT(_nft).getApproved(_tokenID) -
    // this may need simply happen on the UI end since it's hard to get the deployed contract address into this contract code
  ...
  function setPrice(
    ...
    if (msg.sender != UncappedRoyaltyNFT(nftSale.nftContractAddress).ownerOf(nftSale.tokenid)) revert CallerNotNFTOwnerOrTokenInvalid();
    nftSale.price = _price;

    emit LogPriceSet(nftSale.tokenid, _price);
  }

  /**
    * @dev Close `_saleid` for sale
    * Accessible by only nft owner
    * `_saleid` must exist
    */
  function closeForSale(
    ...
    emit LogCloseForSale(nftSale.tokenid);
  }
  ...
  function purchase(
    ...
    if (!nftSale.isOpenForSale) revert NFTClosedForSale();
    if (nftSale.price == 0) revert InvalidPrice();
    address nftOwner = UncappedRoyaltyNFT(nftSale.nftContractAddress).ownerOf(_tokenID);
    ...
    uint256 royaltyFee = nftSale.price * UncappedRoyaltyNFT(nftSale.nftContractAddress).royaltiesFeeBP() / 10000;
    IERC20(purchaseToken).safeTransferFrom(msg.sender, UncappedRoyaltyNFT(nftSale.nftContractAddress).royaltiesCollector(), royaltyFee);
    IERC20(purchaseToken).safeTransferFrom(msg.sender, nftOwner, nftSale.price - royaltyFee);
    UncappedRoyaltyNFT(nftSale.nftContractAddress).safeTransferFrom(nftOwner, msg.sender, _tokenID);
    nftSale.isOpenForSale = false;
    nftSale.price = 0;

    emit LogPurchase(_tokenID, nftOwner, msg.sender);
  }
...
import "./Error.sol";
import "./UncappedBaseRoyaltyNFT.sol";

contract UncappedRoyaltyNFT is UncappedBaseRoyaltyNFT {
  ...
  ) UncappedBaseRoyaltyNFT(_name, _symbol, _baseTokenURI, _royaltiesCollector) {
...
contract UncappedBaseRoyaltyNFT is ERC721URIStorage, Operatorable {
  // Base token URI
