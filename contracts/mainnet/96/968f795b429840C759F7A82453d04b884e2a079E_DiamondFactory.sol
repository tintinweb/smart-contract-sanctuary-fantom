...
contract DiamondFactory is Ownable, Initializable, Controllable {
    ...
    function getFacets() external view returns  (IDiamondCut.FacetCut[] memory) {
        return contractData.facetsToAdd;
    }

    function setFacet(uint256 idx, IDiamondCut.FacetCut memory facetAddress) external onlyOwner {
        contractData.facetsToAdd[idx] = facetAddress;        
    }
    ...
    function create(
        ...
        require(tokenAddress != address(0), "create_failed");
...
interface IBitgemDiamond {
    ...
    function setFeeNamed(string memory feeLabel, Fee memory _value) external;
...
library ClaimLibrary {
    ...
    function createClaim(
        ...
        // refund oversend
        if (msg.value > claimPrice + matureRefundBonus && msg.value > 1 ether / 100) {
