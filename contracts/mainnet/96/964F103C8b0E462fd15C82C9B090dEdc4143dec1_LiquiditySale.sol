...
contract LiquiditySale is AccessControlEnumerable, Pausable, ReentrancyGuard {
    ...
    error ZeroAmount();
    error PayableAmount();
    error LowBalance(string);
    error TooLate();
    error TooSoon();
    error MaxAmount(string);

    constructor(address router_, address runic_, address fate_, address runite_, address usdc_, uint256 maxRunicAmountFTM_, uint256 maxFateAmountFTM_, uint256 maxRuniteAmountUSDC_, uint256 startTime_, uint256 claimTime_) {
        router = router_;
        runic = runic_;
        fate = fate_;
        runite = runite_;
    ...
    /**
     * @notice Must be called after construction because the construction of the locked lp tokens need this address
     */
    ...
    function setStartTime(uint256 startTime_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        startTime = startTime_;
    }
    ...
    // Allow 30 days after claimTime to recover any tokens
