...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
    ...
    function getTokenUSDValue(uint256 tokenBalance, address token, uint256 tokenType, bool viaMaticUSD, address usdAddress) public view returns (uint256) {
        ...
        if (tokenType == 1)
            token = IUniswapV2Pair(token).token0() == _WETH ? _WETH : (IUniswapV2Pair(token).token1() == _WETH ? _WETH : IUniswapV2Pair(token).token0());
        ...
        if (viaMaticUSD) {
            ...
            if (token == _WETH)
                ...
                IUniswapV2Pair maticPair = IUniswapV2Pair(IUniswapV2Factory(sandManSwapRouter.factory()).getPair(_WETH, token));

                if (address(maticPair) == address(0))
                    return 0;

                maticAmount = convertToTargetValueFromPair(maticPair, tokenAmount, _WETH);
            }

            // As we arent working with usd at this point (early return), this is okay.
            IUniswapV2Pair usdmaticPair = IUniswapV2Pair(IUniswapV2Factory(sandManSwapRouter.factory()).getPair(_WETH, address(usdAddress)));
    ...
    function setSandManAddress(address _sandManAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        sandManSwapPair = IUniswapV2Factory(sandManSwapRouter.factory()).getPair(sandManAddress, _WETH);
     ...
     */
    function convertDepositFeesToUSD(address token, uint256 tokenType, uint256 amount) public onlyRole(OPERATOR_ROLE) returns(uint256) {
        //tokenType = 0 from here
        if (amount == 0)
            return 0;

        if (tokenType == 1) {
            // Get reserves and totalSupply before removeLiquidity
            (uint256 res0, uint256 res1, ) = IUniswapV2Pair(token).getReserves();
            (address _token0, address _token1, uint256 _totalSupply) = _removeLiquidity(token);
            convertDepositFeesToUSD(_token0, 0, (((amount * 1e24) / _totalSupply) * res0) / 1e24);
            convertDepositFeesToUSD(_token1, 0, (((amount * 1e24) / _totalSupply) * res1) / 1e24);
            return 0;
        }
        ...
        uint256 diff = 0;
        uint256 bo = IERC20(token).balanceOf(address(this));

        if (holdingLiquidity > bo) {
            diff = holdingLiquidity - bo;
            holdingLiquidity = bo;
        }
    ...
    function _removeLiquidity(address token) internal returns(address, address, uint256) {
        ...
        uint256 prevTotalSupply = lpToken.totalSupply();
        ...
        return (token0, token1, prevTotalSupply);
    }
