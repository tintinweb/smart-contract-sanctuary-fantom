...
contract ReceiptToken is IReceiptToken, ERC721URIStorage, Operatorable {
     ...
     */
    function getReceiptIds(address account)
        public
        view
        override
        returns (uint256[] memory)
    {
        require(
            account != address(0),
            "ReceiptToken#getReceiptIds: ZERO_ADDRESS"
        );
        ...
        returns (Receipt memory)
    {
        require(
            _exists(receiptId),
            "ReceiptToken#getReceiptInfo: RECEIPT_NOT_FOUND"
        );
     ...
     */
    function useReceipt(uint256 receiptId, string memory hashOfUse)
        external
        override
        onlyOperator
    {
...
interface IPayments is IReceiptToken {
    function payProduct(string memory productId) external;
