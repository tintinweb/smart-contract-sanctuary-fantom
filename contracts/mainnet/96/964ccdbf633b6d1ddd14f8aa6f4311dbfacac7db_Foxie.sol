...
// File: Foxie Finance/math/Math.sol
...
// File: Foxie Finance/math/SafeMath.sol
...
// File: Foxie Finance/interfaces/IERC20.sol
...
// File: Foxie Finance/utils/Context.sol
...
// File: Foxie Finance/owner/Ownable.sol
...
// File: Foxie Finance/GSN/Context.sol



pragma solidity >=0.6.0 <0.8.0;
// File: Foxie Finance/owner/Operator.sol
...
// File: Foxie Finance/ERC20/ERC20.sol
...
// File: Foxie Finance/ERC20/ERC20Burnable.sol
...
// File: Foxie Finance/FoxieToken.sol
...
contract Foxie is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 48 hours -> Foxie reward pool
    uint256 public constant INITIAL_FOXIE_POOL_DISTRIBUTION = 20000 ether;
    ...
    /**
     * @notice Constructs the FOXIE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("FOXIE TOKEN", "FOXIE") {
        // Mints 1 FOXIE to contract creator for initial pool setup        
    ...
    function _getFoxiePrice() internal view returns (uint256 _foxiePrice) {
        try IOracle(foxieOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Foxie: failed to fetch FOXIE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _foxiePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_foxiePrice >= taxTiersTwaps[tierId]) {
    ...
    function setFoxieOracle(address _foxieOracle) public onlyOperatorOrTaxOffice {
        require(_foxieOracle != address(0), "oracle address cannot be 0 address");
        foxieOracle = _foxieOracle;
    }
    ...
    /**
     * @notice Operator mints FOXIE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FOXIE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFoxiePrice = _getFoxiePrice();
            currentTaxRate = _updateTaxRate(currentFoxiePrice);
            if (currentFoxiePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_foxiePool != address(0), "!_foxiePool");
        rewardPoolDistributed = true;
        _mint(_foxiePool, INITIAL_FOXIE_POOL_DISTRIBUTION);
    }
