...
/*
    TRIAD FINANCE - The king of Passive Income You were Waiting For ;)
*/
 
contract Triad is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 TRIAD-WFTM LP -> TRIAD pool
    uint256 public constant INITIAL_TRIAD_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the TRIAD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("TRIAD", "TRIAD") {
        // Mints 1 TRIAD to contract creator for initial pool setup
    ...
    function _getTriadPrice() internal view returns (uint256 _triadPrice) {
        try IOracle(triadOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Triad: failed to fetch TRIAD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _triadPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_triadPrice >= taxTiersTwaps[tierId]) {
    ...
    function setTriadOracle(address _triadOracle) public onlyOperatorOrTaxOffice {
        require(_triadOracle != address(0), "oracle address cannot be 0 address");
        triadOracle = _triadOracle;
    }
    ...
    /**
     * @notice Operator mints TRIAD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of TRIAD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentTriadPrice = _getTriadPrice();
            currentTaxRate = _updateTaxRate(currentTriadPrice);
            if (currentTriadPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_triadPool != address(0), "!_triadPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_triadPool, INITIAL_TRIAD_POOL_DISTRIBUTION);
