...
contract Strategy is BaseStrategy {
    ...
    IERC20 public rewardToken =
        IERC20(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE); // SPOOKY (BOO) Reward Token
    ...
    constructor(
        ...
        rewardsDust = uint256(1).mul(
            10**uint256(ERC20(address(rewardToken)).decimals())
        );
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked("ss_Spooky_USDC-MAI_Curve_strategy"));
    }
    ...
    function pendingRewardsScaled() public view returns (uint256 _shares) {
        _shares = masterChef.pendingBOO(masterChefPoolId, address(this)).div(
            1e18
        );
    }
    ...
    function balanceOfPooled() public view returns (uint256 _amount) {
        uint256 totalLpTokens =
            balanceOfLpTokens().add(balanceOfLPInMasterChef());
        ...
        )
    {
        ...
        collectTradingFees();
        // Claim Spooky rewards
        claimRewards();
        // Sell Rewards for want (USDC) token;
        sellAllRewards();
        ...
        returns (uint256 swapAmountIn, uint256 swapAmountOut)
    {
        ...
        uint256 swapOutSecurityRatio =
            swapAmountIn.mul(swapSecurityRatio).div(basisOne);
        swapAmountOut = ICurveFi(curvePoolSwap).exchange(
            inTokenIndex, // Index value for the underlying coin to send
            outTokenIndex, // Index value of the underlying coin to receive
            swapAmountIn, // Amount of i being exchanged
    ...
    function sellLpToken1() internal {
        ...
        uint256 swapOutSecurityRatio =
            swapInAmount.mul(swapSecurityRatio).div(basisOne).div(1e12);
    ...
    function removeLiquidity(uint256 toExitAmount) internal {
        ...
        uint256 liquidityToExit =
            lpTokenBalance.mul(percentageOfLpToSell).div(1e18);
    ...
    function getSwapAmount(uint256 _fullInvestment)
        internal
        view
        returns (uint256 _toSwapAmount)
    {
        (uint256 reserveA, uint256 reserveB, ) = lpToken.getReserves();
        ...
        uint256 denominator =
            IUni(router).quote(halfInvestment, reserveA, reserveB);
    ...
    function setDust(uint256 _rewardsDust, uint256 _wantDust)
        public
        onlyVaultManagers
    {
        wantDust = _wantDust;
    ...
    function setSwapSecurityRatio(uint256 _swapSecurityRatio)
        public
        onlyVaultManagers
    {
