...
/// @title Callback for ISkeletonPoolActions#flash
/// @notice Any contract that calls ISkeletonPoolActions#flash must implement this interface
interface ISkeletonFlashCallback {
    /// @notice Called to `msg.sender` after transferring to the recipient from ISkeletonPool#flash.
    /// @dev In the implementation you must repay the pool the tokens sent by flash plus the computed fee amounts.
    /// The caller of this method must be checked to be a SkeletonPool deployed by the canonical SkeletonFactory.
    /// @param fee0 The fee amount in token0 due to the pool by the end of the flash
    /// @param fee1 The fee amount in token1 due to the pool by the end of the flash
    /// @param data Any data passed through by the caller via the ISkeletonPoolActions#flash call
...
/// @title Callback for ISkeletonPoolActions#mint
/// @notice Any contract that calls ISkeletonPoolActions#mint must implement this interface
interface ISkeletonMintCallback {
    /// @notice Called to `msg.sender` after minting liquidity to a position from ISkeletonPool#mint.
    /// @dev In the implementation you must pay the pool tokens owed for the minted liquidity.
    /// The caller of this method must be checked to be a SkeletonPool deployed by the canonical SkeletonFactory.
    /// @param amount0Owed The amount of token0 due to the pool for the minted liquidity
    /// @param amount1Owed The amount of token1 due to the pool for the minted liquidity
    /// @param data Any data passed through by the caller via the ISkeletonPoolActions#mint call
...
/// @title Callback for ISkeletonPoolActions#swap
/// @notice Any contract that calls ISkeletonPoolActions#swap must implement this interface
interface ISkeletonSwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via ISkeletonPool#swap.
    ...
    /// @param data Any data passed through by the caller via the ISkeletonPoolActions#swap call
...
import './interfaces/callback/ISkeletonMintCallback.sol';
import './interfaces/callback/ISkeletonSwapCallback.sol';
import './interfaces/callback/ISkeletonFlashCallback.sol';

import './interfaces/ISkeletonLmPool.sol';

contract SkeletonPool is ISkeletonPool {
    using LowGasSafeMath for uint256;
    using LowGasSafeMath for int256;
    using SafeCast for uint256;
    using SafeCast for int256;
    ...
    /// @inheritdoc ISkeletonPoolImmutables
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(msg.sender == factory || msg.sender == ISkeletonFactory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = ISkeletonPoolDeployer(msg.sender).parameters();
    ...
    /// @inheritdoc ISkeletonPoolDerivedState
    function snapshotCumulativesInside(int24 tickLower, int24 tickUpper)
    external
    view
    override
    ...
    /// @inheritdoc ISkeletonPoolActions
    ...
    function mint(
        ...
        ISkeletonMintCallback(msg.sender).uniswapV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            ISkeletonSwapCallback(msg.sender).uniswapV3SwapCallback(amount0, amount1, data);
    ...
    function flash(
        ...
        ISkeletonFlashCallback(msg.sender).uniswapV3FlashCallback(fee0, fee1, data);
    ...
    /// @inheritdoc ISkeletonPoolOwnerActions
    ...
    function setLmPool(address _lmPool) external override onlyFactoryOrFactoryOwner {
        lmPool = ISkeletonLmPool(_lmPool);
