...
contract StrategyCommonChefLP is StratManager, FeeManager {
    ...
    address[] public outputToSereneRoute;
    ...
    constructor(
        ...
        address[] memory _outputToSereneRoute,
        ...
        serene = _outputToSereneRoute[_outputToSereneRoute.length - 1];
        outputToNativeRoute = _outputToNativeRoute;
        outputToSereneRoute = _outputToSereneRoute;
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 toSerene = IERC20(output).balanceOf(address(this)).mul(fee).div(1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toSerene, 0, outputToSereneRoute, address(this), now);
        uint256 sereneBal = IERC20(serene).balanceOf(address(this));
        IERC20(serene).transfer(callFeeRecipient, sereneBal.div(10));
        sereneBal = IERC20(serene).balanceOf(address(this));
        IERC20Burnable(serene).burn(sereneBal);
    }
...
pragma solidity ^0.6.0;
contract StrategyCommonChefLPSerene is StrategyCommonChefLP, GasThrottler {
