...
contract KingColdCover is ERC721Enumerable, Ownable, ERC2981 {
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  address[1] private team = [
    0xC87bf1972Dd048404CBd3FbA300b69277552C472 // Funeral wallet
  ];
  uint256 public cost = 5 ether;
  uint256 public maxSupply = 555;
  uint256 public maxMintAmount = 1;
  bool public publicPaused = true;
  address private giveawayAddr = 0xC87bf1972Dd048404CBd3FbA300b69277552C472;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(!publicPaused, "Mint is paused");
    require(_mintAmount > 0, "Cannot mint 0");
    require(_mintAmount <= maxMintAmount, "Cannot mint more than 1");
    require(supply + _mintAmount <= maxSupply, "Not enough left to mint");
    require(balanceOf(msg.sender) < 1, "You already own a King Cold Cover");

    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Not enough monies");
    }
    ...
    payable(team[0]).transfer(totalPayout);
  }

  function mintGiveaways() public onlyOwner {
    uint256 supply = totalSupply();
    require(supply <= 95, "Cannot mint more giveaways");
    ...
    returns (string memory)
  {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, "king_cold_cover", baseExtension))
        : "";
  }
