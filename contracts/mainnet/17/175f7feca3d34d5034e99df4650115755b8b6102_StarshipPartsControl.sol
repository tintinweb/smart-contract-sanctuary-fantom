...
contract StarshipPartsControl is IStarshipPartsControl, WarpBase {
    ...
    /** ===== Initialize ===== */
    function initialize(uint256 _maxParts) public initializer {
        __WarpBase_init(); // also inits ownable

        multiplier = 1;
        maxIntegrity = 5000;
        maxParts = _maxParts; // 3000 spaceships on FTM
    ...
    ) external override whenNotPaused onlyCallers returns (uint256[] memory) {
        uint256[] memory ids = new uint256[](_to.length);

        for (uint256 i = 0; i < _to.length; i++) {
            uint256 id = buildSpecificPart(_to[i], _paid[i], _part[i]);
            ids[i] = id;
        }
    ...
    function buildSpecificPart(
        ...
        uint256 _paid,
        PartType _part
    ) public override whenNotPaused onlyCallers returns (uint256) {
    ...
    function getPartType() internal returns (PartType) {
        ...
        if (totals[PartType.BRIDGE] > totals[PartType.HULL]) {
            nextPart = PartType.HULL;
        }

        if (totals[PartType.HULL] > totals[PartType.ENGINE]) {
            nextPart = PartType.ENGINE;
        }

        if (totals[PartType.ENGINE] > totals[PartType.BRIDGE]) {
            nextPart = PartType.BRIDGE;
        }
