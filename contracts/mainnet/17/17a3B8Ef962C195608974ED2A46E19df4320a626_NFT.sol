...
// File contracts/Radiant.Tier1/Radiant.Tier1.sol
...
contract NFT is ERC721Enumerable, ERC721Royalty, ERC721Pausable, IERC721Burnable, Ownable, Cloneable {
    using Address for address;
    using SafeERC20 for IERC20;
    using Strings for uint256;
    ...
    uint96 private _WHITELIST_DISCOUNT_BASIS = 0; // expressed in basis points, 2000 = 20%
    ...
    function mint(uint256 count) public whenNotPaused returns (uint256[] memory) {
        ...
        if (TOKEN_TYPE == TokenType.ERC20) {
            ...
            require(mintCost <= IERC20(TOKEN).balanceOf(_msgSender()), "ERC20: Mint price exceeds balance");
            require(mintCost <= allowance, "ERC20: Mint price exceeds allowance");

            IERC20(TOKEN).safeTransferFrom(_msgSender(), address(this), mintCost);
        } else {
            require(
                IERC721Burnable(TOKEN).isApprovedForAll(_msgSender(), address(this)),
                "ERC721: Contract not approved for spending"
            );
            require(mintCost <= IERC721Burnable(TOKEN).balanceOf(_msgSender()), "ERC721: Mint price exceeds balance");

            // loop through and burn the required tokens
            for (uint256 i = 0; i < mintCost; i++) {
                uint256 tokenId = IERC721Burnable(TOKEN).tokenOfOwnerByIndex(_msgSender(), 0); // burn from the top of the pile
    ...
    function setRoyaltyReceiver(address receiver, uint96 basisPoints) public onlyOwner {
    ...
    function withdraw(address[] memory payees, uint256[] memory shares) public onlyOwner {
        if (TOKEN_TYPE == TokenType.ERC20) {
            require(IERC20(TOKEN).balanceOf(address(this)) > 0, "Contract has no balance of token");
            IPaymentSplitter splitter = IPaymentSplitter(BASE_PAYMENT_SPLITTER.clone());
            splitter.initialize(payees, shares);
            IERC20(TOKEN).safeTransfer(address(splitter), IERC20(TOKEN).balanceOf(address(this)));
    ...
    function _mintNFT(address to) internal returns (uint256) {
        ...
        require(supply + 1 <= MAX_SUPPLY, "ERC721: Mint exceeds MAX_SUPPLY");
    ...
    function _beforeApprove(address to, uint256 tokenId) internal override {
        tokenId; // silences warning for unused variable
        if (to.isContract()) {
    ...
    function _beforeApprovalForAll(
        ...
        owner; // silences warning for unused variable
