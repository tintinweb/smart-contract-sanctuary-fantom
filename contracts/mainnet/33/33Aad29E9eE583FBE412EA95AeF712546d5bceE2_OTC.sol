...
contract OTC is Ownable, ContractGuard {
    ...
    modifier routerWhitelist(address route) {
        require(router[route], "route not whitelisted");
        _;
    }
    // insures only whilelisted assets are allowed to be swapped for NATIVE
    ...
    // @_NATIVE - token treasury is selling
    // @_otcFund - wallet were buyers transfer their stables, ftm , etc.
    // @_slippage - bonus % buyers get in based when purchasing through OTC

    constructor(address _NATIVE, address _otcFund, uint256 _discount) Ownable() {
        NATIVE = _NATIVE;
        otcFund = _otcFund;
        discount = _discount;
    }
    ...
    // slippage is a multiplier(discount), needs to be in a format of 4 digits
    ...
    // @_router - depends on the _in token (mai, usdc - tombswap, ftm - spookyswap)
    function swapToken(address _in, uint256 amount, address _router, uint256 estimateAPI) external onlyOneBlock assetWhitelist(_in, _router) {
      
        IERC20(_in).safeTransferFrom(msg.sender, otcFund, amount);
        _approveTokenIfNeeded(_in, address(this));
        // get an estimate from spooky or tombswap in our case
       uint256 estimateDex = _estimateSwap(_in, amount, _router);
       // used by UI to calculated total amount saved for buyers
       totalBasedSwapped += estimateDex;
       
       require (estimateAPI < estimateDex.mul(discount).div(1000), "no fee < dex fee");
       
       // make sure OTC swapper has enough for swap
       require (IERC20(NATIVE).balanceOf(address(this)) >= estimateDex, "not enough asset to sell"); 
       IERC20(NATIVE).safeTransfer(msg.sender, estimateDex.mul(discount).div(1000));
        
    }

    // @_in - token we want to throw in
    // @amount - amount of our _in
    // @routerAddr - spookyswap or tombswap
    function _estimateSwap(address _in, uint256 amount, address routerAddr) public view assetWhitelist(_in, routerAddr) returns (uint256) {
        IUniswapV2Router _router = IUniswapV2Router(routerAddr);
        address fromBridge = tokenBridgeForRouter[_in][routerAddr];

        address[] memory path;
        
         if (fromBridge != address(0)) {
    ...
    function setSlippage(uint256 _discount) external onlyOwner {
        discount = _discount;
    }

    function setTokenBridgeForRouter(address token, address _router, address bridgeToken) external onlyOwner {
    ...
    function withdraw(address token) external onlyOwner {
        ...
        // first need to add whitelisted routers, then add assets
    function addRouter(address _router) external onlyOwner {
        router[_router] = true;
    }
    function addAsset(address _asset, address _router) external onlyOwner routerWhitelist(_router) {
    ...
    // before removing router , need to remove assets associated with this router.
    function removeRouter(address _router) external onlyOwner routerWhitelist(_router) {
