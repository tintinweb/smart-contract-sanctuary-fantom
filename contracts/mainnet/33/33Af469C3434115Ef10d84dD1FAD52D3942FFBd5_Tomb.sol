...
contract Tomb is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, address _router, uint256 _taxFee, uint256 _liquidityFee) public ERC20("TEST", "TEST") {
        // Mints 1 TOMB to contract creator for initial pool setup
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        //require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");
        tomb = IERC20(address(this));
        taxFee = 0;
        liquidityFee = 0;
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 contractTokenBalance = tomb.balanceOf(address(this));
        uint256 initialBalance = tomb.balanceOf(address(this));
    ...
    function setTaxFee(uint256 _taxFee) public onlyOperator {
        taxFee = _taxFee;
    }
    function setLiqFee(uint256 _liquidFee) public onlyOperator {
        taxFee = _liquidFee;
    }
    ...
    function transferFrom(
        ...
        if (taxFee == 0 || excludedAddresses[sender]) {
    ...
    function _transferWithTax(
        ...
        uint256 amountAfterTax = amount.sub(taxAmount).sub(liquidityAmount);
