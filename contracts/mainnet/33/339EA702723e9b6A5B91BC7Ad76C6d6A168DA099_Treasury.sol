...
contract Treasury is ITreasury, Operator, ReentrancyGuard {
    ...
    function _swap(
        ...
        _path[0] = _input_token;
        _path[1] = _output_token;
        IERC20(_input_token).safeApprove(router, 0);
        IERC20(_input_token).safeApprove(router, _input_amount);
        uint256[] memory out_amounts = IUniswapV2Router01(router).swapExactTokensForTokens(_input_amount, _min_output_amount, _path, address(this), now.add(1800));
