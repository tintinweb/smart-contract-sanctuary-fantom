...
contract CoconutGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accBasedPerShare; // Accumulated COCONUT per share, times 1e18. See below.
    ...
    uint256 public basedPerSecond = 0.159143 ether; // 27500 COCONUT / (48h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "CoconutGenesisRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "CoconutGenesisRewardPool: existing pool?");
        }
    }
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 3 days) {
            // do not allow to drain core token (COCONUT or lps) if less than 3 days after pool ends
            require(_token != based, "coconut");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
