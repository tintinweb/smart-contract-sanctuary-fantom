...
contract ZtwoToken is ERC20 {
    ...
    // The operator can call all functions except of minting ZtwoToken
    ...
    /**
     * @notice Constructs the Defi Ztwo Token contract.
     */
    constructor(address _to, uint256 _initialSupply, uint256 _maxSupply) ERC20("DeFi Zone Finance", "ZTWO") {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ZTWOs to distribute per second.
        uint256 lastRewardSecond; // Last second that ZTWOs distribution occurs.
        uint256 accZtwoPerShare; // Accumulated ZTWOs per share, times 1e18. See below.
    ...
    // The COIN!
    ZtwoToken public immutable ztwo;
    // Dev address.
    address public devaddr;
    // COIN tokens created per second.
    uint256 public ZtwoPerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when ztwo mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 ZtwoPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _ZtwoPerSecond,
        uint256 _startTime
    ) {
        ztwo = _ztwo;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        ZtwoPerSecond = _ZtwoPerSecond;
    ...
    // Update the given pool's ztwo allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function initializePools(address _nativeLP) external onlyOwner {
        ...
        add(200, IERC20(_nativeLP), 0, true, false);                                        // ZTWO LP
        add(150, IERC20(ztwo), 0, true, false);                                             // ZTWO
    ...
    // View function to see pending ztwo on frontend.
    function pendingZtwo (uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accZtwoPerShare = pool.accZtwoPerShare;
        if (
            ...
            uint256 coinReward = multiplier
                ...
                uint256 totalRewards = ztwo.totalSupply().add(devReward).add(
                coinReward
            );
                if (totalRewards > ztwo.maxSupply()) {
                coinReward = ztwo.maxSupply().sub(ztwo.totalSupply());
            }
            accZtwoPerShare = accZtwoPerShare.add(
                coinReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accZtwoPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= ztwo.maxSupply()) {
            ...
            ztwo.mint(devaddr, coinReward.div(10));
            ztwo.mint(address(this), coinReward);
        } else {
        ...
        if (coinReward != 0) {
            ...
            pool.accZtwoPerShare = pool.accZtwoPerShare.add(
                coinReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    // Deposit LP tokens to MasterChef for ztwo allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accZtwoPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                safeZtwoTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accZtwoPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accZtwoPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeZtwoTransfer(msg.sender, pending);
    ...
    // Safe ztwo transfer function, just in case if rounding error causes pool to not have enough coins.
    function safeZtwoTransfer(address _to, uint256 _amount) internal {
        uint256 ztwoBal = ztwo.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > ztwoBal) {
            transferSuccess = ztwo.transfer(_to, ztwoBal);
        } else {
            transferSuccess = ztwo.transfer(_to, _amount);
        }
        require(transferSuccess, "safeZtwoTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _ZtwoPerSecond) external onlyOwner {
        require(_ZtwoPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        ZtwoPerSecond = _ZtwoPerSecond;
        emit UpdateEmissionRate(msg.sender, _ZtwoPerSecond);
    }
