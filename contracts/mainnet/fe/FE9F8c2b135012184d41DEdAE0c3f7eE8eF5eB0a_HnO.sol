...
contract HnO is IHnO, ERC721Enumerable, Ownable, Pausable {
    ...
    struct StolenEntities {
        uint256 hamstersCount;
        uint256 owlsCount;
        uint256 chestsCount;
    }

    event HamsterMinted(uint256 indexed tokenId, bool savedByChest);
    event OwlMinted(uint256 indexed tokenId, bool savedByChest);
    event HamsterStolen(uint256 indexed tokenId);
    event OwlStolen(uint256 indexed tokenId);
    event HamsterBurned(uint256 indexed tokenId);
    event OwlBurned(uint256 indexed tokenId);

    // max number of tokens that can be minted: 36,999 in production
    uint256 public maxTokens;
    // number of tokens that can be claimed for a fee: 3,500
    uint256 public PAID_TOKENS;
    // number of tokens have been minted so far
    uint256 public override minted;

    // mapping from tokenId to a struct containing the token's traits
    mapping(uint256 => HamsterOwl) private tokenTraits;
    ...
    mapping(uint256 => StolenEntities) public stolenEntitiesCount;
    // list of probabilities for each trait type
    // 0 - 9 are associated with Hamster, 10 - 18 are associated with Owls
    uint256[][18] public rarities;
    // list of aliases for Walker's Alias algorithm
    // 0 - 9 are associated with Hamster, 10 - 18 are associated with Owls
    uint256[][18] public aliases;

    // reference to the Hideout contract to allow transfers to it without approval
    ...
    uint256 public hamstersCount;
    uint256 public owlsCount;

    // address => allowedToCallFunctions
    mapping(address => bool) private admins;

    constructor() ERC721("Hamsters & Owls Game", "HnO") {
        maxTokens = 36999;
        PAID_TOKENS = 4000;
        _pause();
    }

    function initRaritiesAndAliases() external onlyOwner {
    ...
    function setContracts(address _traits, address _hideout) external onlyOwner {
        address zero = address(0);
    ...
    function getTokenWriteBlock(uint256 tokenId) external view override returns(uint256) {
        require(admins[_msgSender()], "Only admins can call this");
        return lastWriteToken[tokenId].blockNum;
    }

    function initialMint(uint256 qty, uint256 seed, address to) external whenNotPaused onlyOwner {
        address owner_ = owner();
        require(minted + qty <= 500, "Initial mint finished");
        require(to != address(0), "To is zero address");
        uint256 newMinted = minted + qty;
        for (uint256 i = minted + 1; i <= newMinted; i++) {
            generate(i, seed, lastWriteAddress[owner_], false);
            _safeMint(to, i);
    ...
    */
    function mint(
        ...
        minted++;
        generate(minted, seed, lastWriteAddress[tx.origin], savedByChest);
        if (tx.origin != recipient && recipient != address(hideout)) {
            // Stolen!
            if (tokenTraits[minted].isHamster) {
                if (stolenById > 0) stolenEntitiesCount[stolenById].hamstersCount += 1;
                emit HamsterStolen(minted);
            } else {
                if (stolenById > 0) stolenEntitiesCount[stolenById].owlsCount += 1;
                emit OwlStolen(minted);
            }
        } else if (savedByChest) if (stolenById > 0) stolenEntitiesCount[stolenById].chestsCount += 1;
    ...
    */
    function burn(uint256 tokenId) external override whenNotPaused {
        ...
        if (tokenTraits[tokenId].isHamster) {
            hamstersCount -= 1;
            emit HamsterBurned(tokenId);
        }
        else {
            owlsCount -= 1;
            emit OwlBurned(tokenId);
        }
        _burn(tokenId);
    }
    ...
    function updateOriginAccess(uint256[] memory tokenIds) external override {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) lastWriteToken[tokenIds[i]] = LastWrite(time, blockNum);
    }
    ...
    */
    function generate(
        ...
        LastWrite memory lw,
        bool savedByChest
    ) internal returns (HamsterOwl memory t) {
        t = selectTraits(seed);
        if (existingCombinations[structToHash(t)] == 0) {
            ...
            if (t.isHamster) {
                hamstersCount += 1;
                emit HamsterMinted(tokenId, savedByChest);
            }
            else {
                owlsCount += 1;
                emit OwlMinted(tokenId, savedByChest);
            }
            return t;
        }
        return generate(
    ...
    */
    function selectTrait(uint256 seed, uint256 traitType) internal view returns (uint256) {
        uint256 trait = seed % rarities[traitType].length;
    ...
    */
    function selectTraits(uint256 seed) internal view returns (HamsterOwl memory t) {    
        t.isHamster = (seed & 0xFFFF) % 10 != 0;
        uint256 shift = t.isHamster ? 0 : 9;
        seed >>= 16;
    ...
    */
    function structToHash(HamsterOwl memory s) internal pure returns (uint256) {
    ...
    /**
    * checks if a token is a Hamsters
    * @param tokenId the ID of the token to check
    * @return hamster - whether or not a token is a Hamsters
    */
    function isHamster(uint256 tokenId) external view override blockIfChangingToken(tokenId) returns (bool) {
        // Sneaky owls will be slain if they try to peep this after mint. Nice try.
        IHnO.HamsterOwl memory s = tokenTraits[tokenId];
        return s.isHamster;
    }
    ...
    */
    ...
    function getTokenTraits(
        uint256 tokenId
    ) external view override blockIfChangingAddress blockIfChangingToken(tokenId) returns (HamsterOwl memory) {
        return tokenTraits[tokenId];
    }

    function tokenURI(
        uint256 tokenId
    ) public view override blockIfChangingAddress blockIfChangingToken(tokenId) returns (string memory) {
    ...
    function tokenOfOwnerByIndex(
        address owner,
        uint256 index
    ) public view virtual override(ERC721Enumerable, IERC721Enumerable) blockIfChangingAddress returns (uint256) {
    ...
    function balanceOf(
        address owner
    ) public view virtual override(ERC721, IERC721) blockIfChangingAddress returns (uint256) {
        // Y U checking on this address in the same block it's being modified... hmmmm
        require(admins[_msgSender()] || lastWriteAddress[owner].blockNum < block.number, "hmmmm what doing?");
    ...
    function ownerOf(
        ...
        ERC721,
        IERC721
    ) blockIfChangingAddress blockIfChangingToken(tokenId) returns (address) {
    ...
    function tokenByIndex(
        uint256 index
    ) public view virtual override(ERC721Enumerable, IERC721Enumerable) returns (uint256) {
    ...
    function getApproved(
        uint256 tokenId
    ) public view virtual override(ERC721, IERC721) blockIfChangingToken(tokenId) returns (address) {
    ...
    function setApprovalForAll(
        address operator,
        bool approved
    ) public virtual override(ERC721, IERC721) blockIfChangingAddress {
    ...
    function isApprovedForAll(
        address owner,
        address operator
    ) public view virtual override(ERC721, IERC721) blockIfChangingAddress returns (bool) {
