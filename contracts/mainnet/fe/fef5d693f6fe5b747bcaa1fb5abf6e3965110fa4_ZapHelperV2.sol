...
contract ZapHelperV2 {
    ...
    function zapETH(address _LP) public payable returns (uint256 liquidity) {
        ...
        address tokenA = IUniswapV2Pair(_LP).token0();
        address tokenB = IUniswapV2Pair(_LP).token1();
    ...
    function zapETHquote(address _LP, uint256 amount) public view returns (address tokenA, address tokenB, uint256 tokenAfinal, uint256 tokenBfinal) {

        tokenA = IUniswapV2Pair(_LP).token0();
        tokenB = IUniswapV2Pair(_LP).token1();
        uint256 totalAmountA;
        
        try this.optiSwapExactTokensForTokensReadOnly(WETH, tokenA, amount) returns (uint256 amountOut) {            
            totalAmountA = amountOut;
        } catch {
            totalAmountA = this.optiSwapExactTokensForTokensReadOnly(WETH, tokenB, amount);
            address intermed = tokenA;
            tokenA = tokenB;
            tokenB = intermed;
        }
        ...
        uint256 balanceBeforeB = tokenB.myBalance();
        uint256 balanceAfterB = tokenB.myBalance() + router.getAmountOut(swapAmount, r0, r1);
        return (tokenA, tokenB, totalAmountA.sub(swapAmount), balanceAfterB.sub(balanceBeforeB));
    }

    function zapquote(address _LP, address _inputToken, uint256 _inputAmount) public view returns (address tokenA, address tokenB, uint256 tokenAfinal, uint256 tokenBfinal) {
