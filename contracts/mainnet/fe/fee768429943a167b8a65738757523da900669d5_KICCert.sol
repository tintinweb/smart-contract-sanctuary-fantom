...
contract KICCert is NFTokenMetadata, Ownable {
...
modifier safe() {
  ...
  constructor() {
    nftName = "Witness Certified Identity";
    nftSymbol = "WIC-CERT";
    owner = msg.sender;
    is_auth[owner] = true;
  }
...
function mintNFT(string memory identity) payable public safe {
    
    if (!is_auth[msg.sender] && !isFree[msg.sender]) {
        require(mintEnabled, "Minting not enabled");
        require(msg.value>=100000000000000000000); // 100 FTM
    }
    uint256 _tokenId = _lastTokenId;
    super._mint(msg.sender, _tokenId);
    string memory full_url = string(abi.encodePacked("https://witness.mypinata.cloud/ipfs/", identity));
    _setTokenUri(_tokenId, full_url);
...
function setTokenPrice(uint _price) public onlyAuth {
  tokenPrice = _price;
}

function setWitness(address addy) public onlyAuth {
  Witness = addy;
}

function mintNFTWithToken(string memory identity)  public safe {
    IERC20 WitnessToken = IERC20(Witness);
    if (!is_auth[msg.sender] && !isFree[msg.sender]) {
      require(WitnessToken.allowance(msg.sender, address(this)) >= tokenPrice, "Allowance");
      require(WitnessToken.balanceOf(msg.sender) >= tokenPrice, "No tokens");
      bool sent = WitnessToken.transferFrom(msg.sender, address(this), tokenPrice);
