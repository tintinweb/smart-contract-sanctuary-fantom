/**
 *Submitted for verification at FtmScan.com on 2023-04-12
*/
...
// File contracts/MclbFbombEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract MclbFbombEqualizerV2 is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public wftmToFbombPath;
    ...
    event MakeCustomTxn(address indexed token, uint256 indexed amount);
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToFbombPath
    ) {
        ...
        for (uint i; i < _equalToFbombPath.length; ++i) {
            wftmToFbombPath.push(_equalToFbombPath[i]);
        }
    ...
    function _deposit() internal whenNotPaused {
        ...
        IEqualizerGauge(gauge).deposit(wantBal);
    }
    ...
    function chargeFees(address caller) internal {
      uint256 toWftm = IERC20(equal).balanceOf(address(this));

        IEqualizerRouter(router).swapExactTokensForTokens(toWftm, 0, equalToWftmPath, address(this), block.timestamp);

        uint256 wftmBal = IERC20(wftm).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        uint256 callFee = wftmBal * CALL_FEE / FEE_DIVISOR;
        IERC20(wftm).safeTransfer(caller, callFee);

        uint256 recipientFee = wftmBal * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(wftm).safeTransfer(grimFeeRecipient, recipientFee);

        uint256 treasuryFee = wftmBal * TREASURY_FEE / FEE_DIVISOR;
        IERC20(wftm).safeTransfer(treasury, treasuryFee);

        uint256 stratFee = wftmBal * STRAT_FEE / FEE_DIVISOR;
        IERC20(wftm).safeTransfer(strategist, stratFee);

    }

    /** @dev Converts WMFTM to both sides of the LP token and builds the liquidity pair */
    function addLiquidity() internal {
        uint256 wftmHalf = IERC20(wftm).balanceOf(address(this)) / 2;
        IEqualizerRouter(router).swapExactTokensForTokens(wftmHalf, 0, wftmToFbombPath, address(this), block.timestamp);

        if(toPathToken != address(0) && pathToken != address(0)){
        _beethovenSwap(toPathId, wftm, toPathToken, wftmHalf);
        ...
        _beethovenSwap(beetsPoolId, pathToken, mclb, pathTokenBal);}
            ...
            _beethovenSwap(pathTokenId, wftm, pathToken, wftmHalf);
            uint256 pathTokenBal =  IERC20(pathToken).balanceOf(address(this));
            _beethovenSwap(beetsPoolId, pathToken, mclb, pathTokenBal);}
        else{_beethovenSwap(beetsPoolId, wftm, mclb, wftmHalf);}
    ...
    function _addAllowance() internal {
        ...
        IERC20(wftm).safeApprove(beetsVault, type(uint).max);
    ...
    function makeCustomTxn(IEqualizerRouter.Routes[] memory _path, address _token) external onlyAdmin {
        delete customPath;
    ...
    for (uint i; i < _path.length; ++i) {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(tokenBal, 0, customPath, address(this), block.timestamp);

        emit MakeCustomTxn(_token, tokenBal);
    }
