...
contract AssentVIP is IAssentVIP, Ownable {
    ...
    UserData internal mutltiUserUpdateDatas;
    ...
    // Owner must be recorded as operator before
    function multiSimpleVIPUser(address[] calldata _user) external onlyOwner {
        for (uint i; i < _user.length; i++) {
            simpleVIPUser(_user[i]);
        }
    }    

    // Owner must be recorded as operator before
    function multiRemoveVIPUser(address[] calldata _user) external onlyOwner {
        for (uint i; i < _user.length; i++) {
            removeVIPUser(_user[i]);
        }
    }

    // Need to be executed before multiFullVIPUser / Needed to avoid stack too deep
    function mutltiUserPrepareDatas(uint _dexFeeReduction,
                            uint _aUSDMarketFeeReduction,
                            uint _farmsDepFeeReduction,
                            uint _bondsReduction,
                            uint _dexFeeReducDuration,
                            ...
                            uint _bondsReducDuration,
                            bool _lifeTimeReduction,
                            bool _NFTHold) external onlyOwner {
            mutltiUserUpdateDatas.dexFeeReduction = _dexFeeReduction;
            mutltiUserUpdateDatas.aUSDMarketFeeReduction = _aUSDMarketFeeReduction;
            mutltiUserUpdateDatas.farmsDepFeeReduction = _farmsDepFeeReduction;
            mutltiUserUpdateDatas.bondsReduction = _bondsReduction;
            mutltiUserUpdateDatas.dexFeeReduceUntil = block.timestamp + _dexFeeReducDuration;
            mutltiUserUpdateDatas.aUSDMarketFeeReduceUntil = block.timestamp + _aUSDMarketFeeReducDuration;
            mutltiUserUpdateDatas.farmsDepFeeReduceUntil = block.timestamp + _farmsDepFeeReducDuration;
            mutltiUserUpdateDatas.bondsReduceUntil = block.timestamp + _bondsReducDuration;
            mutltiUserUpdateDatas.lifeTimeReduction = _lifeTimeReduction;
            mutltiUserUpdateDatas.NFTHold = _NFTHold;
    } 

    // Need mutltiUserUpdateDatas prepared with data first
    function multiFullVIPUser(address[] calldata _user) external onlyOwner {
        for (uint i; i < _user.length; i++) {
            if (_user[i] != address(0) && userData[_user[i]].VIPlisted == false) {
                // first time add the user into the list
                // set datas with params value
                userData[_user[i]].VIPlisted = true;
                userData[_user[i]].user = _user[i];
                require (mutltiUserUpdateDatas.dexFeeReduction <= MAXFEEREDUCTION 
                        && mutltiUserUpdateDatas.aUSDMarketFeeReduction <= MAXFEEREDUCTION 
                        && mutltiUserUpdateDatas.farmsDepFeeReduction <= MAXFEEREDUCTION
                        && mutltiUserUpdateDatas.bondsReduction <= MAXFEEREDUCTION, "Fee too high");
                userData[_user[i]].dexFeeReduction = mutltiUserUpdateDatas.dexFeeReduction;
                userData[_user[i]].aUSDMarketFeeReduction = mutltiUserUpdateDatas.aUSDMarketFeeReduction;
                userData[_user[i]].farmsDepFeeReduction = mutltiUserUpdateDatas.farmsDepFeeReduction;
                userData[_user[i]].bondsReduction = mutltiUserUpdateDatas.bondsReduction;
                userData[_user[i]].dexFeeReduceUntil = mutltiUserUpdateDatas.dexFeeReduceUntil;
                userData[_user[i]].aUSDMarketFeeReduceUntil = mutltiUserUpdateDatas.aUSDMarketFeeReduceUntil;
                userData[_user[i]].farmsDepFeeReduceUntil = mutltiUserUpdateDatas.farmsDepFeeReduceUntil;
                userData[_user[i]].bondsReduceUntil = mutltiUserUpdateDatas.bondsReduceUntil;
                userData[_user[i]].lifeTimeReduction = mutltiUserUpdateDatas.lifeTimeReduction;
                userData[_user[i]].NFTHold = mutltiUserUpdateDatas.NFTHold;
                addUserList(_user[i]);               
            }
        }
    }
