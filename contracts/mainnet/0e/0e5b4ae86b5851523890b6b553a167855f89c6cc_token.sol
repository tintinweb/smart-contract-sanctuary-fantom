...
contract token is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 100000 * 10**9; //0.1%
    ...
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        ...
        swapTokensAtAmount = SwapThresholdAmount * 10**9;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (contractTokenBalance > swapTokensAtAmount * 10) {
            contractTokenBalance = swapTokensAtAmount * 10; //max 1%
        }
        
        // I dont get the eth for dev and marketing ratio, why is the eth for it coming from the eth for liquidity?

        uint256 totalFee = _taxFeeOnBuy+_taxFeeOnSell+_liquidityFeeOnBuy+_liquidityFeeOnSell+_devFeeOnBuy+_devFeeOnSell;
        // I'm assuming that contractTokenBalance is already == fee, if not need to modify.
        //uint256 contractTokenBalance = contractTokenBalance * (totalFee / basis);
        uint256 liquidityRatio = (_liquidityFeeOnBuy+_liquidityFeeOnSell) / totalFee; //Take approx 20% since 3% (1%+2%) fee of 14% => 3%/14% ~ 21%.
        uint256 devRatio = (_devFeeOnBuy+_devFeeOnSell) / totalFee; //Ratio should be roughly 10% since 2% (1%, 1%) => 2%/14% ~ 14%. 
        uint256 marketingRatio = liquidityRatio - devRatio;
        uint256 amountForLiquidity = contractTokenBalance * liquidityRatio;
        uint256 halfForLiquidity = amountForLiquidity/2;
        uint256 forDev = contractTokenBalance * devRatio;
        uint256 forMarketing = contractTokenBalance * marketingRatio;

        uint256 toSwap = halfForLiquidity + forDev + forMarketing;
        ...
        uint256 ethForDev = newBalance * devRatio;
        uint256 ethForMarketing = newBalance * marketingRatio;
        uint256 ethForLiquidity = newBalance * liquidityRatio;
        ...
        emit SwapAndLiquify(halfForLiquidity, ethForLiquidity, halfForLiquidity);
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        // Sanity check, so it doesn't try to transfer more than what it has.
