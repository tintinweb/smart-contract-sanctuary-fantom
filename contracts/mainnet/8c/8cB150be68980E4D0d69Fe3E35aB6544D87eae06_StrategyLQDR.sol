...
contract StrategyLQDR is StratManager, FeeManager {
     ...
     * {masterChef} - masterChefV2 contract
    ...
    function deposit() public whenNotPaused {
        ...
        if (pairBal > 0) {
            ILQDRMasterChef(masterChef).deposit(poolId, pairBal, msg.sender);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {
            ILQDRMasterChef(masterChef).withdraw(poolId, _amount, msg.sender);
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ILQDRMasterChef(masterChef).harvest(poolId, address(this));
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = ILQDRMasterChef(masterChef).userInfo(
            poolId,
            vault
        );
        return _amount;
    ...
    function rewardsAvailable() public view returns (uint256) {
        return
            ILQDRMasterChef(masterChef).pendingLqdr(poolId, address(this));
    }
    ...
    function retireStrat() external {
        ...
        ILQDRMasterChef(masterChef).emergencyWithdraw(poolId, msg.sender);
    ...
    function panic() public onlyOwner {
        pause();
        ILQDRMasterChef(masterChef).withdraw(poolId, balanceOfPool(), msg.sender);
    }
