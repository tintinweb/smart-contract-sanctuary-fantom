...
contract Zapper is Ownable {
    ...
    // @WNATIVE - WNATIVE token that is not a part of our zap-in LP
    ...
    // from WNATIVE to an LP token through the specified router
    ...
    function nativeZapIn(uint256 amount, address out, address routerAddr, address recipient, uint256 minAmountOfLp) external payable whitelist (routerAddr) {
        //  IERC20(WNATIVE).safeTransferFrom(msg.sender, address(this), msg.value);
        //  _approveTokenIfNeeded(WNATIVE, routerAddr);
        uint256 amountOfLp = _swapNativeToLP(amount, out, recipient, routerAddr);
    ...
    function zapOutToNative(address _in, uint256 amount, address routerAddr, address recipient, uint256 minAmountNative) external whitelist(routerAddr) {
        ...
        _approveTokenIfNeeded(pair._token1, routerAddr);
         ...
         if (pair._token0 == WNATIVE || pair._token1 == WNATIVE) {
           ...
           uint256 halfOfEth =  _swapTokenForNative(args._token, pair._amountToken0, args._recipient, args._routerAddr);
            require(halfOfEth.add(pair._amtETH) >= minAmountNative, "native amt < minAmountNative");
            // send other half of Native
            TransferHelper.safeTransferETH(args._recipient, pair._amtETH);
        } else {
        ...
        require (pair._amountToken0.add(pair._amountToken1) >= minAmountNative, "native amt < minAmountNative");
    ...
    function _swapTokenToLP(address _in, uint256 amount, address out, address recipient, address routerAddr) private returns (uint256) {
        ...
        if (args._in == IUniswapV2Pair(args._out).token0() || args._in == IUniswapV2Pair(args._out).token1()) { 
         ...
         _dustDistribution( args._amount.sub(args._swapAmt), 
                            args._otherAmt, 
                            pair._amountToken0, 
                            pair._amountToken1, 
                            args._in, 
            ...
            return _swapNativeToLP(nativeAmount, args._out, args._recipient, args._routerAddr);
        }
    }
    
    // @amount - amount of our native token
    // @out - LP we want to get
    function _swapNativeToLP(uint256 amount, address out,  address recipient, address routerAddress) private returns (uint256) {
    ...
    function _swapNativeToEqualTokensAndProvide(address token0, address token1, uint256 amount, address routerAddress, address recipient) private returns (uint256) {
        ...
        uint256 token0Amt = _swapNativeForToken(pair._token0, args._swapAmt, address(this), routerAddress);
        uint256 token1Amt = _swapNativeForToken(pair._token1, args._amount.sub(args._swapAmt), address(this), routerAddress);
    ...
    function _swapNativeForToken(address token, uint256 amount, address recipient, address routerAddr) private returns (uint256) {
        ...
        uint256[] memory amounts = router.swapExactETHForTokens{value: amount} (0, path, recipient, block.timestamp);
        return amounts[amounts.length - 1];
    }
    ...
    // @_lp - lp address we want to move to another dex
    // amount - amount of lp we moving
    // @_toRouter - new router address
    // ===========will have to change to external and whitelisted modifier before testing!!!!
    function moveLpAcross(address _lp, uint256 amount, address _toRouter, address _recipient) private {
        ...
        IERC20(_lp).safeTransfer(_lp, amount);
        uint256 amt0;
        uint256 amt1;
