...
// File: contracts/Fantomized/flickets.sol
...
abstract contract ERC721Tradable is ERC721Enumerable, Ownable {
    ...
    function mintNTo(address _to, uint256 number) public onlyOwner {
        require(number <= 50, "50 max at once");  // Limit or likely to fail anyway
        while (number != 0) {
 ...
 * Flickets
 */
contract Flickets is ERC721Tradable {

    string private _baseUrl = "https://fantomized.art/flickets/json/";
    string private _imagesBaseUrl;  // For on-chain data
    mapping(uint256 => bytes32) private _admindata; // Custom field set by the admin
    mapping(uint256 => bytes32) private _userdata; // Custom field set by the nft owner.

    constructor()
        ERC721Tradable("Flickets", "FLICKET")
    {}
    ...
    function updateAdmindata(uint256 tokenId, bytes32 newData) public onlyOwner {
        _admindata[tokenId] = newData;
    }

    function getAdmindata(uint256 tokenId) public view returns (bytes32) {
        require(_exists(tokenId), "ERC721: operator query for nonexistent token");
        return _admindata[tokenId];
    }
