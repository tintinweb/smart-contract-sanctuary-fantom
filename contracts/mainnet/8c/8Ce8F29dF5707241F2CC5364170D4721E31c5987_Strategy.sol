...
contract Strategy is BaseStrategy {
    ...
    IERC20 public rewardToken = IERC20(0x2F6F07CDcf3588944Bf4C42aC74ff24bF56e7590);
    ILpPool public lpToken = ILpPool(0x12edeA9cd262006cC3C4E77c90d2CD2DD4b1eb97);
    ...
    function wantToLPToken(uint256 _wantAmount) public view returns (uint256) {
        return _wantAmount.mul(lpToken.totalSupply()).div(lpToken.totalLiquidity()); // NOTE ibTkn
    }
    ...
    function estimatedHarvest() public view returns (uint256 _profitInUSDC) {
        uint256 _stargateBalance = pendingRewards();

        address[] memory rewardToWant = new address[](3);
        rewardToWant[0] = address(rewardToken);
        rewardToWant[1] = address(wftm);
        rewardToWant[2] = address(want);

        if (_stargateBalance > 0) {
            _profitInUSDC = router.getAmountsOut(_stargateBalance, rewardToWant)[
                rewardToWant.length - 1
            ];
        }
    }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > looseAmount) {
            ...
            removeLiquidity(wantToLPToken(toExitAmount));
            depositLpIntoMasterChef(); 
    ...
    function addLiquidity(uint256 _amountToInvest) internal {
        stargateRouter.addLiquidity(liquidityPoolId, _amountToInvest, address(this));
    }

    function removeLiquidity(uint256 _lpToExit) internal {
    ...
    function giveAllowanceRouter() internal {
        rewardToken.approve(address(router), 0);
        rewardToken.approve(address(router), MAX);
    }

    function removeAllowanceRouter() internal {
        rewardToken.approve(address(router), 0);
    }
    ...
    function giveAllowances() internal {
        ...
        IERC20(address(lpToken)).approve(address(masterChef), MAX);
    ...
    function setRouter(address _newRouter) external onlyVaultManagers {
        require(_newRouter != address(router));
        removeAllowanceRouter();
        router = IUni(_newRouter);
        giveAllowanceRouter();
    }
