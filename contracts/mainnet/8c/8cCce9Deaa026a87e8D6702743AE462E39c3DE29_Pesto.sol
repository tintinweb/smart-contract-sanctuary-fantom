...
contract Pesto is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the PESTO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("PESTO", "PESTO") {
        // Mints 10 PESTO to contract creator for initial pool setup
    ...
    function _getPestoPrice() internal view returns (uint256 _pestoPrice) {
        try IOracle(pestoOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Pesto: failed to fetch PESTO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _pestoPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_pestoPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPestoOracle(address _pestoOracle) public onlyOperatorOrTaxOffice {
        require(_pestoOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints PESTO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PESTO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPestoPrice = _getPestoPrice();
            currentTaxRate = _updateTaxRate(currentPestoPrice);
            if (currentPestoPrice < burnThreshold) {
                burnTax = true;
            }
        }
