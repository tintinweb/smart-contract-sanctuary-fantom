...
contract DeusPriceForLiquidation {
    ...
    function deusPriceForLiquidation(address user, uint256 eps) external view returns (uint256 deusPrice) {
        IPair pair = IPair(lpAddress);
        uint256 reserveDei;
        uint256 reserveDeus;
        uint256 userLiquidationPrice = ILender(lenderAddress).getLiquidationPrice(user);
        (reserveDei, reserveDeus,) = pair.getReserves();
        uint256 deiAmount = reserveDei - (userLiquidationPrice * pair.totalSupply() / (2 * 1e18));
