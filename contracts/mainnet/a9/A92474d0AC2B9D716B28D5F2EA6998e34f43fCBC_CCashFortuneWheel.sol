...
contract CCashFortuneWheel is GovernableUpgradable {
    ...
    uint256 private constant defaultMultiplicand = 250_00;  /// @notice default win CCASH amount for first day; _00 - decimals
    ...
    function getMultiplicand(uint16 roundNumber, uint32 roundDay) public view returns(uint256) {
        if(roundDay == 1) {
            return defaultMultiplicand;
        } else {
            return (crimeCashGamePools.getPoolStaticStaked(roundNumber, 0) * crimeCashGamePools.getCCashApyRateByDay() / 100 
                + crimeCashGamePools.getPoolsStaticStakedExceptCCash(roundNumber)) * percentValueFormula / maxPercentFormula;
        }
    }
    ...
    function rollRandom() external {
        ...
        uint256 multiplicand = getMultiplicand(roundNumber, roundDay);
...
contract CrimeCashGamePools is GovernableUpgradable {
  ...
  function initialize(Storage store, CrimeCashGame crimeCashGame) public initializer {
    ...
    _defaultLpPoolApyRates = [
      150_000_00, 180_000_00, 200_000_00, 230_000_00,   250_000_00, 
      280_000_00, 300_000_00, 330_000_00, 350_000_00,   380_000_00, 
      400_000_00, 430_000_00, 450_000_00, 480_000_00,   500_000_00, 
      530_000_00, 550_000_00, 580_000_00, 600_000_00,   630_000_00, 
      650_000_00, 680_000_00, 700_000_00, 730_000_00,   750_000_00, 
      780_000_00, 800_000_00, 850_000_00, 900_000_00, 1_000_000_00 
    ];
  }
  ...
  function getPoolsStaticStakedExceptCCash(uint16 roundNumber) external view returns (uint256 result) {
    for (uint256 i; i < _pools[roundNumber].length; i ++) {
      if (_pools[roundNumber][i].infoData.exists && !_pools[roundNumber][i].infoData.isCrimeCash) {
        result += getPoolStaticStaked(roundNumber, i);
      }
    } 
  }
  ...
  function _addPool(bool isCrimeCash, address lpToken, uint8 openDay) private { 
    ...
    newPool.infoData.lpToken = IERC20(lpToken);
...
contract CrimerInfoStorage { 
  ...
  modifier onlyFromCrimerInfo {
    require(msg.sender == crimerInfo || msg.sender == crimeBankInteraction || msg.sender == nftAssetManager
      || msg.sender == _crimeCashGamePools || msg.sender == _cCashFortuneWheel, "Sender not a CrimerInfo");
    _;
  }
...
contract CrimerInfo is Governable {
  ...
  function buyAsset(uint8 assetType, uint256 assetIndex, uint256 count) external {
    ...
    require (count <= _getMaxCountOfAssetThatAvailableToBuyForCrimer(msg.sender, assetType, assetIndex, assetCost, assetHasBuyLimit, crimer.cash), "Count > MaxCountOfAssetThatAvailableToBuy");
...
contract CrimeCashGame is Governable {
  ...
  function startRound(address newCrimeCash) external {
    ...
    address(_nftAssetManager), address(_crimeGoldStaking), address(_crimeCashGamePools), address(_cCashFortuneWheel));
...
contract AssetManager is Governable {
    ...
    function addDefaultAssets(uint256 decimals) external {
        ...
        addAssetToMap(2, 1, 250, precisionToken * 300, true);
        addAssetToMap(2, 2, 550, precisionToken * 600, true);
        addAssetToMap(2, 3, 1155, precisionToken * 1200, true);
        addAssetToMap(2, 4, 2541, precisionToken * 2400, true);
        addAssetToMap(2, 5, 5336, precisionToken * 4800, true);
        addAssetToMap(2, 6, 11739, precisionToken * 19200, true);
        addAssetToMap(2, 7, 24653, precisionToken * 38400, true);
        addAssetToMap(2, 8, 54236, precisionToken * 76800, true);
        addAssetToMap(2, 9, 113896, precisionToken * 223600, true);
        addAssetToMap(2, 10, 250571, precisionToken * 621600, true);
        addAssetToMap(2, 11, 551256, precisionToken * 1843200, true);
        addAssetToMap(2, 12, 1157637, precisionToken * 7686400, true);
        addAssetToMap(2, 13, 2546802, precisionToken * 19372800, true);
        addAssetToMap(2, 14, 5348285, precisionToken * 51745600, true);
        addAssetToMap(2, 15, 11766227, precisionToken * 100000000, false);
...
contract NFTAssetManager is Governable, ERC1155Receiver {
    ...
    CrimeMarketplace public immutable _crimeMarketplace;
    CrimeCashGame public immutable _crimeCashGame;
    ...
    constructor(
        ...
        Storage _store,
        CrimeCashGame criemeCashGame
    ) Governable(_store) {
        _crimeMarketplace = crimeMarketplace;
        _crimeCashGame = criemeCashGame;
    }
...
contract CrimerInfoStorageFactory { 
  ...
  external returns (address) {
    return address(new CrimerInfoStorage(_crimerInfo, _crimeBankInteraction, _nftAssetManager, crimeGoldStaking, crimeCashGamePools, cCashFortuneWheel));
  }
}
...
contract CrimeMarketplace is Governable {
    using SafeMath for uint256;
    ...
    CrimeNFTItem immutable public nftItem;
...
contract CrimeNFTItem is ERC1155, Governable {
    ...
    constructor(
        ...
        Storage _store
    ) ERC1155(baseUrl) Governable(_store){
        minters[_store.owner()] = true;
    }
    
    function mintItem(address mintTo, uint256 nftId, uint256 amount) external { 
        require(minters[msg.sender] || msg.sender == marketplace, "CrimeNFTItem: not minter");
        _mint(mintTo, nftId, amount, "");
        emit MintItem(mintTo, nftId, amount);
    }

    function burnItem(address from, uint256 nftId, uint256 amount) external onlyMarketplace {
