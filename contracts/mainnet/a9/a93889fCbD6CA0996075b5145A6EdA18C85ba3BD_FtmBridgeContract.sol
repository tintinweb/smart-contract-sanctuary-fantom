...
contract FtmBridgeContract is Ownable {
    ...
    /// @notice Transfers `amount` FTM initializes a bridging transaction to the target chain.
    /// @param _targetChain The target chain to which the wrapped asset will be minted
    /// @param _amount The amount of FTM to bridge
    function bridge(
        ...
        require(
            msg.value >= processFee + _amount,
            "Insufficient Amount to bridge"
        );
        _nonces[_targetChain] = _nonces[_targetChain] + 1;
        _sendFTM(system, processFee);
    ...
    function bridgeBack(
        ...
        if(serviceFee > 0) {
            _sendFTM(bridgeFeesAddress, serviceFee);
        }
        _sendFTM(_receiver, transferAmount);
    ...
    function _sendFTM(address _receipient, uint256 _amount) internal {
