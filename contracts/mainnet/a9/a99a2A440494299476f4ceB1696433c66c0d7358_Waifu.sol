...
contract Waifu is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0xd32636Ccd5D49FC0Bfdb677f02Aa195bfca90F05;
    ...
    // Min amount to liquify. (default 10 Waifu)
    ...
    // The operator can call all functions except of minting Waifu
    address private _operator;
    
    // The lpEarner address set to burn address to lock liquidity forever
    address constant public lpEarner = 0xd32636Ccd5D49FC0Bfdb677f02Aa195bfca90F05;
    ...
    /**
     * @notice Constructs the WaifuSwapToken contract.
     */
    constructor() public BEP20("WaifuSwapDefi", "WAIFU") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "WAIFU::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "WAIFU::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the WAIFU -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "WAIFU::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "WAIFU::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "WAIFU::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "WAIFU::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "WAIFU::updateSwapRouter: Invalid pair address.");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. WAIFU to distribute per second.
        uint256 lastRewardSecond; // Last second that WAIFU distribution occurs.
        uint256 accWaifuPerShare; // Accumulated WAIFU per share, times 1e18. See below.
    ...
    // Waifu tokens created per second.
    uint256 public WaifuPerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when Waifu mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 WaifuPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _WaifuPerSecond,
        uint256 _startTime
    ) public {
        waifu = _waifu;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        WaifuPerSecond = _WaifuPerSecond;
    ...
    // View function to see pending waifu on frontend.
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 waifuReward = multiplier
                .mul(WaifuPerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
                uint256 devReward = waifuReward.div(10);
                uint256 totalRewards = waifu.totalSupply().add(devReward).add(
                waifuReward
            );
                if (totalRewards > waifu.maxSupply()) {
                waifuReward = waifu.maxSupply().sub(waifu.totalSupply());
            }
            accWaifuPerShare = accWaifuPerShare.add(
                waifuReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accWaifuPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= waifu.maxSupply()) {
            ...
            waifu.mint(devaddr, waifuReward.div(10));
            waifu.mint(address(this), waifuReward);
        } else {
            // mint the difference only to MC, update waifuReward
            waifuReward = waifu.maxSupply().sub(waifu.totalSupply());
            waifu.mint(address(this), waifuReward);
        }

        if (waifuReward != 0) {
            // only calculate and update if waifuReward is non 0
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accWaifuPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accWaifuPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeWaifuTransfer(msg.sender, pending);
    ...
    // Safe Waifu transfer function, just in case if rounding error causes pool to not have enough Waifu.
    ...
    function updateEmissionRate(uint256 _WaifuPerSecond) external onlyOwner {
        require(_WaifuPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        WaifuPerSecond = _WaifuPerSecond;
        emit UpdateEmissionRate(msg.sender, _WaifuPerSecond);
    }
