...
/*
    https://gundamfinance.xyz/
*/
contract Gundam is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("GUNDAM", "GUNDAM") {
        // Mints 500 TOMB to contract creator for initial pool setup
        ...
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_tombPrice >= taxTiersTwaps[tierId]) {
                    require(taxTiersRates[tierId] < 1000, "tax equal or bigger to 100%");
