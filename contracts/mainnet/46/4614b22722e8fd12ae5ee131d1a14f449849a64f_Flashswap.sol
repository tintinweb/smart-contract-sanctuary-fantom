...
contract Flashswap {
    ...
    function start(
        ...
        (uint profit, uint _tokenBorrowAmount) = check(_tokenBorrow, _amountTokenPay, _tokenPay, _sourceRouter, _targetRouter);
    ...
    function check(
        ...
        return (
            uint(amountRepay - _amountTokenPay), // our profit or loss; example output: BNB amount
    ...
    function execute(address _sender, uint _amount0, uint _amount1, bytes calldata _data) internal {
        ...
        path[0] = path1[1] = forward; // c&p
        path[1] = path1[0] = backward; // c&p
        ...
        uint amountRequired = IUniswapV2Router(sourceRouter).getAmountsIn(amountToken, path1)[0];
    ...
    function soulswapCall(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
        execute(_sender, _amount0, _amount1, _data);
    }

    function waultSwapCall(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
        execute(_sender, _amount0, _amount1, _data);
    }
    ...
    // pantherswap
    function pantherCall(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
        execute(_sender, _amount0, _amount1, _data);
    }

    // jetswap
    function jetswapCall(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
        execute(_sender, _amount0, _amount1, _data);
    }

    // cafeswap
    function cafeCall(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
        execute(_sender, _amount0, _amount1, _data);
    }
    ...
    // @TODO: pending release
    function wardenCall(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external {
        execute(_sender, _amount0, _amount1, _data);
    }
}