...
interface IMIBOAddressRegistry {
    function mibo() external view returns (address);
...
interface IMIBOBundleMarketplace {
...
interface IMIBONFTFactory {
    function exists(address) external view returns (bool);
}

interface IMIBOTokenRegistry {
    function enabled(address) external view returns (bool);
}

interface IMIBOPriceFeed {
    function wMIBO() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract MIBOMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IMIBOAddressRegistry public addressRegistry;
    ...
    function _buyItem(
        ...
        IMIBOBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IMIBOAuction auction = IMIBOAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isMIBONFT(_nftAddress), "invalid nft address");
    ...
    function _isMIBONFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.mibo() == _nftAddress ||
            IMIBONFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IMIBONFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IMIBONFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IMIBONFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IMIBOPriceFeed priceFeed = IMIBOPriceFeed(
            addressRegistry.priceFeed()
        );

        if (_payToken == address(0)) {
            (unitPrice, decimals) = priceFeed.getPrice(priceFeed.wMIBO());
        } else {
    ...
    /**
     @notice Update MIBOAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IMIBOAddressRegistry(_registry);
    }
    ...
    function _validPayToken(address _payToken) internal {
        require(_payToken == address(0) || (addressRegistry.tokenRegistry() != address(0) && IMIBOTokenRegistry(addressRegistry.tokenRegistry()).enabled(_payToken)),"invalid pay token");
        // require(_payToken == addressRegistry.tokenRegistry() && IMIBOTokenRegistry(addressRegistry.tokenRegistry()).enabled(_payToken),"invalid pay token");
    }
