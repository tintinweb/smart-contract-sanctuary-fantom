...
contract EqualizerMatrixLpAutoCompound is MatrixLpAutoCompound {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    address internal constant EQUAL = 0x3Fd3A0c85B70754eFc07aC9Ac0cbBDCe664865A6;
    ...
    function _setWhitelistedAddresses() internal override {
        ...
        whitelistedAddresses.add(EQUAL);
    }
    ...
    function _setDefaultSwapPaths() internal override {
        ...
        // EQUAL -> FRAX
        address[] memory _equalFrax = new address[](4);
        _equalFrax[0] = EQUAL;
        _equalFrax[1] = wrapped;
        _equalFrax[2] = USDC;
        _equalFrax[3] = FRAX;
        _setSwapPath(EQUAL, FRAX, unirouter, _equalFrax);
    }
    ...
    function _swap(
        ...
        for (uint256 i; i < _swapPath.path.length - 1; i++) {
            (uint256 _amountBack, bool _stable) = IEqualizerRouter(_swapPath.unirouter).getAmountOut(_lastAmountBack, _swapPath.path[i], _swapPath.path[i + 1]);
            _lastAmountBack = _amountBack;
            _routes[i] = route({ from: _swapPath.path[i], to: _swapPath.path[i + 1], stable: _stable });
        }
        ...
        uint256 _toTokenBefore = IERC20(_toToken).balanceOf(address(this));
        IEqualizerRouter(_swapPath.unirouter).swapExactTokensForTokens(_amount, 0, _routes, address(this), block.timestamp);
    ...
    function _addLiquidity(uint256 _outputAmount) internal override returns (uint256 _wantHarvested) {
        ...
        IEqualizerRouter(unirouter).addLiquidity(lpToken0, lpToken1, isStable, _lp0Balance, _lp1Balance, 1, 1, address(this), block.timestamp);
        return IERC20(want).balanceOf(address(this)) - _wantBalanceBefore;
    }
}