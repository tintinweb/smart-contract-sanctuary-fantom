...
contract ScorpionPriceOracle is IScorpionPriceOracle {
    ...
    function toUint224(uint256 input) internal pure returns (uint224) {
        require(input <= uint224(-1), "ScorpionPriceOracle: UINT224_OVERFLOW");
    ...
    function initialize(address uniswapV2Pair) external {
        ...
        require(
            ...
            "ScorpionPriceOracle: ALREADY_INITIALIZED"
        );
        ...
        returns (uint224 price, uint32 T)
    {
        ...
        require(pair.initialized, "ScorpionPriceOracle: NOT_INITIALIZED");
        Pair storage pairStorage = getPair[uniswapV2Pair];
        ...
        require(T >= MIN_T, "ScorpionPriceOracle: NOT_READY"); //reverts only if the pair has just been initialized
