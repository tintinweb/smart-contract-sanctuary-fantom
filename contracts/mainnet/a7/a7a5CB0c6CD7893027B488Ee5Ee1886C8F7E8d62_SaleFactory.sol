...
contract SaleFactory is ISaleFactory, Ownable, Pausable {
    ...
    function initFactory(
        ...
        emit Initiated(models, treasury, treasuryCut, commission);
    }

    /// @dev Deploy a new model to be used.
    /// @param ref    Address of the sale contract being referenced.
    function addModel(ISaleModel ref) external onlyInit onlyOwner {
    ...
    {
        require(_models.length >= modelId, "MODEL_NOT_FOUND");
        ...
        ISaleModel saleClone = ISaleModel(cloneContract);
        ...
        emit SaleCreated(msg.sender, saleClone);
    ...
    function adjustHost(
        ...
        require(commissionPerc <= 10000, "REACHED_MAX_COMMISSION_PERC");
        Host storage sHost = _hosts[hostId];
        Host memory mHost = sHost;
        require(mHost.owner == msg.sender, "NOT_HOST_OWNER");

        sHost.treasury = treasury;
        sHost.commissionPerc = commissionPerc;
