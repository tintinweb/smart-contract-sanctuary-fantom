...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0xc4D54FF1F67BC4f1F30B6fF84F5A838E3A17FC2f) // PegGenesisPool
    ];

    // core components
    address public peg;
    address public pegbond;
    address public pegshare;
    ...
    modifier checkOperator {
        require(
            IBasisAsset(peg).operator() == address(this) &&
                IBasisAsset(pegbond).operator() == address(this) &&
                IBasisAsset(pegshare).operator() == address(this) &&
    ...
    function getBurnablePegLeft() public view returns (uint256 _burnablePegLeft) {
        ...
        if (_pegPrice <= pegPriceOne) {
            ...
            uint256 _bondSupply = IERC20(pegbond).totalSupply();
    ...
    function initialize(
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn PEG and mint PEGBOND)
        maxDebtRatioPercent = 3500; // Upto 35% supply of PEGBOND to purchase
    ...
    function buyBonds(uint256 _pegAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 newBondSupply = IERC20(pegbond).totalSupply().add(_bondAmount);
        ...
        IBasisAsset(pegbond).mint(msg.sender, _bondAmount);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        IBasisAsset(pegbond).burnFrom(msg.sender, _bondAmount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochPegPrice > pegPriceCeiling) {
                // Expansion ($PEG Price > 1 $FTM): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(pegbond).totalSupply();
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(pegbond), "bond");
        require(address(_token) != address(pegshare), "pegshare");
