...
// File contracts/wTRTL.Calculator/wTRTL.Calculator.sol

interface IgTRTLCalculator {
    function getValue(address lpToken, uint256 amount) external view returns (uint256);
}

contract wTRTLgTRTLCalculator is IgTRTLCalculator, Ownable {
    uint256 public basis = 3750;

    event UpdateBasis(uint256 indexed _old, uint256 indexed _new);

    constructor() {
        _transferOwnership(0x7E6CaC527cc907f70dA4Ef514DA285046e75ba28);
    }

    function getValue(address token, uint256 amount) public view returns (uint256) {
        token;
        return (amount * basis) / 10000;
    }

    function updateBasis(uint256 _basis) public onlyOwner {
        uint256 _old = basis;
        basis = _basis;
        emit UpdateBasis(_old, _basis);
    }
}