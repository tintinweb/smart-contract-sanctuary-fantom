...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        require(
            memberShare >= amount,
            "Bedroom: withdraw request greater than staked amount"
        );
...
contract Bedroom is ShareWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    ...
    BedroomSnapshot[] public bedroomHistory;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "Bedroom: caller is not the operator");
        _;
    }
    ...
    modifier memberExists() {
        require(
            ...
            "Bedroom: The member does not exist"
        );
        _;
    }
    ...
    modifier notInitialized() {
        require(!initialized, "Bedroom: already initialized");
        _;
    }
    ...
    function initialize(
        ...
        BedroomSnapshot memory genesisSnapshot = BedroomSnapshot({
            time: block.number,
            rewardReceived: 0,
            rewardPerShare: 0
        });
        bedroomHistory.push(genesisSnapshot);
    ...
    function latestSnapshotIndex() public view returns (uint256) {
        return bedroomHistory.length.sub(1);
    }

    function getLatestSnapshot()
        internal
        view
        returns (BedroomSnapshot memory)
    {
        return bedroomHistory[latestSnapshotIndex()];
    }
    ...
    function getLastSnapshotOf(address member)
        ...
        returns (BedroomSnapshot memory)
    {
        return bedroomHistory[getLastSnapshotIndexOf(member)];
    }
    ...
    {
        require(amount > 0, "Bedroom: Cannot stake 0");
        super.stake(amount);
    ...
    function withdraw(uint256 amount)
        ...
        updateReward(msg.sender)
    {
        require(amount > 0, "Bedroom: Cannot withdraw 0");
        require(
            members[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <=
                treasury.epoch(),
            "Bedroom: still in withdraw lockup"
        );
        claimReward();
        ...
        if (reward > 0) {
            require(
                members[msg.sender].epochTimerStart.add(rewardLockupEpochs) <=
                    treasury.epoch(),
                "Bedroom: still in reward lockup"
            );
            members[msg.sender].epochTimerStart = treasury.epoch(); // reset timer
            members[msg.sender].rewardEarned = 0;
            baep.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount)
        ...
        onlyOperator
    {
        require(amount > 0, "Bedroom: Cannot allocate 0");
        require(
            totalSupply() > 0,
            "Bedroom: Cannot allocate when totalSupply is 0"
        );
        ...
        BedroomSnapshot memory newSnapshot = BedroomSnapshot({
            time: block.number,
            rewardReceived: amount,
            rewardPerShare: nextRPS
        });
        bedroomHistory.push(newSnapshot);

        baep.safeTransferFrom(msg.sender, address(this), amount);
