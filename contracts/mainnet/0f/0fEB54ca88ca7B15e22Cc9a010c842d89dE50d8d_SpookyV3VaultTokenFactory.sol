...
interface ISpookyV3VaultTokenFactory {
...
import "./interfaces/ISpookyV3VaultToken.sol";
...
contract SpookyV3VaultToken is ISpookyV3VaultToken, IUniswapV2Pair, PoolToken {
    ...
    event ReinvestRewardToken(address indexed caller, address rewardToken, uint256 reward, uint256 bounty, uint256 fee);
    ...
    function _initialize(
        ...
        pid = _pid;
        underlying = MasterChefV2(address(masterChef)).lpToken(_pid);
    ...
    function getRewardTokenInfo()
        ...
        returns (
            ...
            MasterChefV2 masterChefV2 = MasterChefV2(address(masterChef));
            address rewarder = masterChefV2.rewarder(pid);
    ...
    function reinvestOne(address _rewardToken) internal returns (uint256 liquidity) {
        ...
        uint256 deltaB;
        {
            ...
            deltaB = balanceAfterB.sub(balanceBeforeB);
        }
        ...
        emit ReinvestRewardToken(msg.sender, _rewardToken, reward, bounty, fee);
    }
...
import "@openzeppelin/contracts/ownership/Ownable.sol";
import "./SpookyV3VaultToken.sol";
import "./interfaces/IMasterChef.sol";
import "./interfaces/ISpookyV3VaultTokenFactory.sol";
import "./interfaces/IUniswapV2Pair.sol";
import "./interfaces/IUniswapV2Router01.sol";

contract SpookyV3VaultTokenFactory is Ownable, ISpookyV3VaultTokenFactory {
    address public optiSwap;
    address public rewarderHelper;
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(SpookyV3VaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), pid)
        }
        SpookyV3VaultToken(vaultToken)._initialize(
            optiSwap,
            rewarderHelper,
