...
contract Univ3zap is Ownable {
  ...
  event Removal(uint256 amount0, uint256 amount1);
  ...
  function deposit(address poolAddr, bool isToken0, uint256 amount, bool isCoin) public payable {
    ...
    _addLiquidity(poolAddr, amount0, amount1);
  }
  ...
  function _addLiquidity(address poolAddr, uint256 amount0, uint256 amount1) private {
    address token0 = address(IPool(poolAddr).token0());
    address token1 = address(IPool(poolAddr).token1());
  ...
  function withdraw(address poolAddr, uint256 liquidity) public {
    ...
    if (liquidity > 0) {
      ...
      emit Removal(amnt0, amnt1);
    }
  }

  function withdraw2(address poolAddr, uint256 amnt0, uint256 amnt1, bool isToken0, bool isCoin) public {

      address token0 = address(IPool(poolAddr).token0());
      address token1 = address(IPool(poolAddr).token1());
      if (isToken0) {
        _approveTokenIfNeeded(token0, address(this));
        (int256 qty0, int256 qty1) = swap(poolAddr, address(this), address(this), int256(amnt1), ! isToken0);
        uint256 amount = amnt0 + uint256(-qty0);
        if (amnt1 > uint256(qty1)) {
          IERC20(token1).transfer(msg.sender, amnt1 - uint256(qty1));
        }
        _transferTokens(token0, address(this), msg.sender, amount, isCoin);
      }
      else {
        _approveTokenIfNeeded(token1, address(this));
        (int256 qty0, int256 qty1) = swap(poolAddr, address(this), address(this), int256(amnt0), ! isToken0);
        uint256 amount = amnt1 + uint256(-qty1);
        if (amnt0 > uint256(qty0)) {
          IERC20(token0).transfer(msg.sender, amnt0 - uint256(qty0));
        }
...
interface IUniv3LpLedger {
  ...
  function withdraw(address _account, address _pool, uint256 _amount) external view returns (uint256);
}
