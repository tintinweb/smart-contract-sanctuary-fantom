...
contract Rahu is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 RAHU-WFTM LP -> RAHU pool
    uint256 public constant INITIAL_RAHU_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the RAHU ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("RAHU", "RAHU") {
        // Mints 1 RAHU to contract creator for initial pool setup
    ...
    function _getRahuPrice() internal view returns (uint256 _rahuPrice) {
        try IOracle(rahuOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Rahu: failed to fetch RAHU price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _rahuPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_rahuPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRahuOracle(address _rahuOracle) public onlyOperatorOrTaxOffice {
        require(_rahuOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints RAHU to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of RAHU to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRahuPrice = _getRahuPrice();
            currentTaxRate = _updateTaxRate(currentRahuPrice);
            if (currentRahuPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_rahuPool != address(0), "!_rahuPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_rahuPool, INITIAL_RAHU_POOL_DISTRIBUTION);
