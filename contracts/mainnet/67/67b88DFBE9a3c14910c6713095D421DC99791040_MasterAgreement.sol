...
/*
    ...
    - Convert taker field to white list of takers
    - Add leverage
    - Add FVA
    - How to handle fees
...
contract MasterAgreement is AccessControl {
    ...
    function sendQuote(
        ...
        _balances[msg.sender] -= makerCVA + makerIM;
        _lockedCVA[msg.sender] += makerCVA;
        _lockedIM[msg.sender] += makerIM;
    ...
    function openPosition(
        ...
        uint256 qid,
        SchnorrSign[] memory solvencySignatures
    ) external onlyRole(TAKER_ROLE) notLiquidatedUser(maker) {
    ...
    function _closePosition(
        ...
        uint256 qid,
        uint256 closedPrice
    ) internal notLiquidatedUser(maker) {
        Quote storage quote = _quotes[maker][qid];
        require(
            quote.quoteState == QuoteState.OPEN ||
        ...
        _balances[quote.taker] += (quote.takerCVA + quote.takerIM);
        _lockedCVA[maker] -= quote.makerCVA;
        _lockedCVA[quote.taker] -= quote.takerCVA;
        _lockedIM[maker] -= quote.makerIM;
        _lockedIM[quote.taker] -= quote.takerIM;

        (int256 makerPnL, int256 takerPnL) = calculatePnL(
            quote.positionType,
            quote.limitPrice,
            quote.closedPrice,
            quote.quantity
        );
        _balances[maker] = (makerPnL < 0)
            ? (_balances[maker] - uint256(-makerPnL))
            : (_balances[maker] + uint256(makerPnL));
        _balances[quote.taker] = (takerPnL < 0)
            ? (_balances[quote.taker] - uint256(-takerPnL))
            : (_balances[quote.taker] + uint256(takerPnL));

        _openPositionsLength[maker]--;
        _openPositionsLength[quote.taker]--;
    ...
    function closePosition(
        ...
        _closePosition(maker, qid, closedPrice);
    ...
    function performClosePosition(
        ...
        SchnorrSign[] memory priceSignatures,
        SchnorrSign[] memory solvencySignatures
    ) external notLiquidatedUser(maker) {
        // todo: check signatures
        Quote storage quote = _quotes[maker][qid];
        require(
            quote.quoteState == QuoteState.CLOSE_PENDING,
        ...
        _closePosition(maker, qid, quote.limitPrice);
    ...
    function calculatePnL(
        ...
        int256 priceDiff = int256(currentPrice) - int256(limitPrice);
    ...
    function liquidatePositions(
        ...
        for (uint256 i = 0; i < quoteIds.length; i++) {
            Quote storage quote = _quotes[maker][quoteIds[i]];
            require(
                quote.quoteState == QuoteState.OPEN ||
                    quote.quoteState == QuoteState.CLOSE_PENDING ||
                    quote.quoteState == QuoteState.CLOSE_CANCELING,
            ...
            _updateQuoteState(maker, quoteIds[i], QuoteState.LIQUIDATED);
            _openPositionsLength[maker]--;
            _openPositionsLength[quote.taker]--;
        ...
        if (_openPositionsLength[maker] == 0) {
