...
contract BeQiCrossChainDepositor is Ownable, Pausable {
    ...
    constructor (
        ...
        IERC20(qi).safeApprove(address(anyswapRouter), type(uint256).max);
    }
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        uint256 oldBalance = IERC20(qi).balanceOf(address(this));

        IERC20(qi).safeTransferFrom(msg.sender, address(this), _amount);

        uint256 balance = IERC20(qi).balanceOf(address(this)) - oldBalance;

        anyswapRouter.anySwapOutUnderlying(anyQi, polygonQiClient, balance, 137);
