...
contract OTC is Ownable, ContractGuard {
    ...
    // % of tokens that will be bought on market
    uint16 private marketSwapPercent;
    ...
    function swapToken(address _in, uint256 amount, address _router, uint256 minimumAmount) external onlyOneBlock assetWhitelist(_in, _router) {
        ...
        //Do we need EMIT EVENT HERE?

    }

    // @_in - token we want to throw in
    // @_out - token that is being sold by treasury
    ...
    function _estimateSwap(address _in,uint256 amount, address routerAddr) public view assetWhitelist(_in, routerAddr) returns (uint256) {
        ...
        address[] memory path = _getPath(_in,routerAddr);

        uint256[] memory amounts =_getAmountsOutWihoutFee(routerAddr,amount, path);
                                ...
                                returns (uint256)
    {
        ...
        uint256 estimateDex = _estimateSwap(_in,_amount, _routerAddr);
        ...
        require (_minimumAmount < estimateDex.mul(discount).div(1000), "no fee < dex fee");

        // make sure OTC swapper has enough for swap
        require (IERC20(tokenForSale).balanceOf(address(this)) >= estimateDex, "not enough asset to sell");

        IERC20(tokenForSale).safeTransfer(_recipient, estimateDex.mul(discount).div(1000));
                                                ...
                                                returns (uint256[] memory amounts)
    {
        ...
        for (uint i; i < _path.length - 1; i++) {
            IUniswapV2Pair pair = IUniswapV2Pair(IUniswapV2Factory(IUniswapV2Router(_router).factory()).getPair( _path[i], _path[i + 1]));
    ...
    function settokenForSaleToken(address _tokenForSale) external onlyOwner {
        tokenForSale = _tokenForSale;
    }
    ...
    function setTokenBridgeForRouter(address token, address _router, address bridgeToken) external onlyOwner assetWhitelist(token, _router){
    ...
    function addAsset(address _asset, address _router) external onlyOwner {
