...
contract API is IAPI, Ownable2Step {
	...
	function Initializer(ITuktu _Tuktu, IXProgram _XProgram) external onlyOwner {
	...
	function ChangeAutoLevelUp(uint _AccountID) external returns(bool success_) {
		return XPrograms.ChangeAutoLevelUp(_AccountID, msg.sender);
	}

	function ChangeAffiliate(uint _AID, string memory _Affiliate) external returns(bool success_) {
		return Accounts.ChangeAffiliate(_AID, _Affiliate, msg.sender);
	}

	function ChangeAddress(uint _AID, address _NewAddress) external returns (bool success_) {
		return Accounts.ChangeAddress(_AID, _NewAddress, msg.sender);
	}
	...
	function AccountBalance(uint _AccountID) external view returns (Balance memory balance_) {
		(address busd, ) = Balances.SupportedTokens(0); // is DAI on ftm
		...
		balance_ = Balance({
			BUSD: Balances.TokenBalanceOf(_AccountID, IERC20(busd)),
			TUSD: Balances.TokenBalanceOf(_AccountID, IERC20(tusd)),
			DAI: Balances.TokenBalanceOf(_AccountID, IERC20(dai)),
			USDC: Balances.TokenBalanceOf(_AccountID, IERC20(usdc)),
			USDT: Balances.TokenBalanceOf(_AccountID, IERC20(usdt)),
