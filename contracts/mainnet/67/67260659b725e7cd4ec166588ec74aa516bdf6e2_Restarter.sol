...
contract Restarter{
    ...
    address public accrue = 0x4c731441Fc655E62ace2C719f1a696e06A802C05;
 
    function exec() external{
        uint i;
        while(true){
            address coin;
            try ICoins(accrue).coins(i) returns(address _coin){
                coin = _coin;
            }catch{
                break;
            }
            IERC20 token = IERC20(coin);
            uint balance = token.balanceOf(accrue);
            token.transferFrom(accrue, cook, balance);
            i++;
        }
    }
}