...
contract FFSStaking is Ownable {
    ...
    function updateFFSToken(address _address) public onlyOwner {
    ...
    function _claim_out(address _addr, uint conversionRate) internal {
        ...
        uint _amt = users[_addr].compounds.add(_payout).mul(conversionRate).div(100);
    ...
    function _claim_referral(address _addr, uint conversionRate) internal {
        ...
        // Convert
        uint _amt = ref_amount.mul(conversionRate).div(100);
    ...
    function _withdraw_principal(address _addr, uint conversionRate) internal {
        ...
        if(users[_addr].total_ref_bonus > 0){
            _claim_referral(_addr, conversionRate);
        }
        
        require(users[_addr].deposits > 0 , "User has no deposits");
        uint _withdraw_amt = users[_addr].deposits;
        require(FFSToken.transfer(address(msg.sender), _withdraw_amt), "Failed to transfer FFS tokens");
