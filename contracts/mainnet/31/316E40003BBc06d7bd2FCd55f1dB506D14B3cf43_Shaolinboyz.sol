...
contract Shaolinboyz is ERC721 {
    ...
    constructor()
        ERC721("Shaolinboyz", "SHBZ")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 9998) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeidvi2azcqsk7c6u2itwb3b5ns5slnj2zz7nu4gcllqp3w2ndytvcy/", Strings.toString(tokenId), ".json")
            );
    }
