...
*____________Ԫ����ͷ��MEME���ţ���Ǯ�����ǿ����Ϯ��_______________
*____________��ȫ������¡�صĽ��գ�ũ�����������ڼ䡣______________
*_��Ǯ�������Դ���й��������Ҫ������Ϊг�����Ǯ���������ⱬ�����硣_
*______________ȫ���վ���������ڣ�MEME�����������ǡ�_______________
*_________________�ݹ�ȫ������Σ�ѣ����ڷ籩���ԡ�__________________
*______________��Ǯ���������ũ������һƬ��͸���֮����______________
*__________��ȫ�������ͳ�һ�����õ�ף����������֣���Ǯ������_________
*/
...
"Mint" fee is 1.8 FTM.
Mint new Leopard will require a seedsId, you automatically get 1 FTM each time when anyone fills with your seedsId.


Postscript:

1200 NFTs were airdroped to early contributor, who worked hard in pushing the PardDAO to a well-known project,
and the remaining 8800 NFTs will "mint" with fees 1.8 FTM for anyone.

If there is any FTM income, 1 FTM for the seedsToken owner,
and the remaining FTM will automatically make liqudity and lock LP to address(0).

You must deposit the NFT in this contract and then you will own the seedsId to rewarding #PAO tax and FTM.

I will renounce the ownership to address(0) to transfer #Leopard and #PAO to the community, make sure it's 100% safe.
...
contract Leopard is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    constructor (address _pao, address paoMigr) ERC721("Leopard Pro", "PARD") {
        pao = IERC20(_pao);
        paoMigrate = paoMigr;
        baseURI = "ipfs://QmUESZAPg6ZJo5NxwjurCAqu1SBrUFzYdwSuSkDRqMziXe/";
    }

    uint256 public constant PASS = 1.8 ether; // 1.8 ht
    uint256 private latestId = 1200;
    address private paoMigrate;
    
    receive() external payable {
        if(msg.value >= PASS){
        _mint(_msgSender(), latestId);
        latestId++;
    ...
    function mint(uint256 _seedsId) public payable nonReentrant {
        ...
        require(msg.value >= PASS, "Insufficient funds to mint, at least 1.8 FTM.");
        _mint(_msgSender(), latestId);
        latestId++;
        tokenInfo[_seedsId]++;
        require(totalSupply() <= 10000, "Max mint amount");
        uint256 seedsETH = 1 ether;
    ...
    //migrate from old Leopard, require less than 1200 for early contributor
    function migrateNFT(uint256 amount) public onlyOwner {
        require(
            totalSupply() <= 10000,
            "Max supply exceeded"
        );
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '",  "description": "The ID card for contributor sharing rewards of DAOPAO and FTM. Congrats, #DAOPAO to the moon!!!", "image": "',
                        output,
                        '"}'
                    )
                )
    ...
    uint256 private  multiplier = 5319148900000000;
    // Info of each pool.
        ...
        returns (uint256)
    {
        ...
        uint256 rewards = paoSupply.mul(percent).mul(multiplier).div(10000).div(10000000000000000);
    ...
    function getRewards() public {
        ...
        rewards = pendingPao(pao, _user)*(tokenInfo[user.tokenId] + 1);
    ...
    function getUserInfo(address _user) public view returns(uint256 userId,uint256 amount,uint256 totalAmount,uint256 tokenId,uint256 startTime, uint256 nowTime, uint256 enjoyTime, uint256 pending, uint256 coe){
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[_user];
        userId = user.id;
        amount = user.amount;
    ...
    function getTokenInfo(uint256 _tokenId) public view returns(uint256 tokenId, uint256 coe, address owner, bool isDeposit){
        tokenId = _tokenId;
        coe = tokenInfo[_tokenId] + 1;
        if(ownerOf(_tokenId) == address(this)){
            owner = seedsInfo[_tokenId];
            isDeposit = true;
        }else{
