...
contract Metal is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the METAL ERC-20 contract.
     */
    constructor() public ERC20("METAL", "METAL") {
        // Mints 1 METAL to contract creator for initial pool setup
    ...
    function _getMetalPrice() internal view returns (uint256 _metalPrice) {
        try IOracle(metalOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Metal: failed to fetch METAL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _metalPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_metalPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMetalOracle(address _metalOracle) public onlyOperatorOrTaxOffice {
        require(_metalOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints METAL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of METAL to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMetalPrice = _getMetalPrice();
            currentTaxRate = _updateTaxRate(currentMetalPrice);
            if (currentMetalPrice < burnThreshold) {
                burnTax = true;
            }
        }
