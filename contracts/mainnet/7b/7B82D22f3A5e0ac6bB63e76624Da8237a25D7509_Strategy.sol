...
contract Strategy is BaseStrategy {
	...
	function name() external view override returns (string memory) {
		return string(abi.encodePacked('sLending USDC Alpaca Strategy'));
	}
	...
	function ibPrice() public view returns (uint256) {
		return alpacaVault.totalToken().mul(1e18).div(alpacaVault.totalSupply());
	}

	function wantToIbToken(uint256 _amount) public view returns (uint256) {
		return _amount.mul(1e18).div(ibPrice());
	}
