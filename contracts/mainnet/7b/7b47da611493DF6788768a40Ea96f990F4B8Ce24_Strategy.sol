...
contract Strategy is BaseStrategy {
    ...
    IUniPair public lpToken = IUniPair(0xfca12A13ac324C09e9F43B5e5cfC9262f3Ab3223); // SpookySwap: TOMB-WFTM LP
    address public tomb = address(0x6c021Ae822BEa943b2E66552bDe1D2696a53fbB7); // TOMB

    uint256 public dust = 1e18;
    uint256 public rewardDust = 1e15;

    bool public collectFeesEnabled;
    uint256 public swapSecurityFactor = 9800;
    uint256 public sellSecurityFactor = 9800;
    uint256 public profitLiquidationFactor = 10000;

    address public router = address(0x6D0176C5ea1e44b08D3dd001b0784cE42F47a3A7); // TOMB SWAP
    address public swapRouter = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29); // TOMB SWAP
    ...
    function balanceOfPooled() public view returns (uint256 _amount) {
        ...
        uint256 ftmPerTOMB = IUni(swapRouter).getAmountOut(1e18, reserveB, reserveA); // NEW
        uint256 totalReservesInFTM = ftmPerTOMB.mul(reserveB).div(1e18).add(reserveA); // NEW
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (amountIn > dust) {
            ...
            uint256 investedWant = amountIn.sub(balanceOfWant()); // NEW

            depositLpIntoMasterChef();

            enforceSlippageIn(investedWant, pooledBefore); // NEW
    ...
    function removeLiquidity(uint256 toExitAmount) internal {
        ...
        uint256 totalReservesInFTM = TombPerFTM.mul(reserveB).div(1e18).add(reserveA); // NEW 1e18

        uint256 ftmPerLP = totalReservesInFTM.mul(1e18).div(totalSupply);
        uint256 liquidityToExit = toExitAmount.mul(1e18).div(ftmPerLP);

        uint256 toExit = Math.min(liquidityToExit, balanceOfLpTokens()); // NEW
    ...
    function collectTradingFees() internal {
        ...
        if (total > debt) {
            ...
            uint256 profitToLiquidate = profit.mul(profitLiquidationFactor).div(basisOne); // NEW
