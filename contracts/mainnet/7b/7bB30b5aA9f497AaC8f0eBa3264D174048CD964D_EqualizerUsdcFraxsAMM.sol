...
// File contracts/EqualizerUsdcFraxsAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualizerUsdcFraxsAMM is Ownable, Pausable {
    ...
    //Routes
    IEqualizerRouter.Routes[] public equalToFraxPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToFraxPath,
        ...
        for (uint i; i < _equalToFraxPath.length; ++i) {
           equalToFraxPath.push(_equalToFraxPath[i]);
        }
   ...
   //Withdraws funds from masterchef
    ...
    function chargeFees(address caller) internal {
        uint256 equalFee = IERC20(equal).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        uint256 callFee = equalFee * CALL_FEE / FEE_DIVISOR;
        IERC20(equal).safeTransfer(caller, callFee);

        uint256 recipientFee = equalFee * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(equal).safeTransfer(grimFeeRecipient, recipientFee);

        uint256 treasuryFee = equalFee * TREASURY_FEE / FEE_DIVISOR;
        IERC20(equal).safeTransfer(treasury, treasuryFee);

        uint256 stratFee = equalFee * STRAT_FEE / FEE_DIVISOR;
        IERC20(equal).safeTransfer(strategist, stratFee);
    }
    ...
    function addLiquidity() internal {
        ...
        uint256 fraxDecimals = 10**IERC20Extended(frax).decimals();
        uint256 usdcDecimals = 10**IERC20Extended(usdc).decimals();
        uint256 fraxOut = IEqualizerRouter(router).getAmountsOut(equalHalf, equalToFraxPath)[equalToFraxPath.length] * 1e18 / fraxDecimals;
        uint256 usdcOut = IEqualizerRouter(router).getAmountsOut(equalHalf, equalToUsdcPath)[equalToUsdcPath.length] * 1e18 / usdcDecimals;
        (uint256 amount0, uint256 amount1,) = IEqualizerRouter(router).quoteAddLiquidity(frax, usdc, stable, fraxOut, usdcOut);
        amount0 = amount0 * 1e18 / fraxDecimals;
        amount1 = amount1 * 1e18 / usdcDecimals;
        uint256 ratio = fraxOut * 1e18 / usdcOut * amount0 / amount1;
        uint256 fraxRatio = equalHalf * 1e18 / (ratio + 1e18);
        uint256 usdcRatio = equalHalf - fraxRatio;

        IEqualizerRouter(router).swapExactTokensForTokens(fraxRatio, 0, equalToFraxPath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokens(usdcRatio, 0, equalToUsdcPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(frax).balanceOf(address(this));
        uint256 t2Bal = IERC20(usdc).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(frax, usdc, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(frax).safeApprove(router, 0);
    ...
    function _addAllowance() internal {
        ...
        IERC20(frax).safeApprove(router, type(uint).max);
