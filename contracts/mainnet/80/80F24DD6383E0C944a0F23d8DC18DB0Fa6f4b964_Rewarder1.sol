  ...
  ReentrancyGuardUpgradeable
{
  ...
  function onDeposit(
    ...
    user.rewardDebt =
      user.rewardDebt +
      ((_amount * pool.accRewardPerShare) / ACC_REWARD_PRECISION).toInt256();
  ...
  function onWithdraw(
    ...
    if (user.amount >= _newAmount) {
      ...
      user.rewardDebt =
        user.rewardDebt -
        (((_amount * pool.accRewardPerShare) / ACC_REWARD_PRECISION))
          .toInt256();
      ...
      user.rewardDebt =
        user.rewardDebt +
        ((_newAmount * pool.accRewardPerShare) / ACC_REWARD_PRECISION)
          .toInt256();
  ...
  function onHarvest(
    ...
    int256 _accumulatedRewards = ((user.amount * pool.accRewardPerShare) /
    ...
    returns (uint256)
  {
    ...
    uint256 _stakedBalance = IMiniFL(miniFL).stakingToken(_pid).balanceOf(
      miniFL
    );
    if (block.timestamp > _poolInfo.lastRewardTime && _stakedBalance != 0) {
      uint256 _timePast = block.timestamp - _poolInfo.lastRewardTime;
      uint256 _rewards = (_timePast * rewardPerSecond * _poolInfo.allocPoint) /
        totalAllocPoint;
      _accRewardPerShare =
        _accRewardPerShare +
        ((_rewards * ACC_REWARD_PRECISION) / _stakedBalance);
    }
    return
      (((_userInfo.amount * _accRewardPerShare) / ACC_REWARD_PRECISION)
        .toInt256() - _userInfo.rewardDebt).toUint256();
  }
  ...
  function _updatePool(uint256 _pid) internal returns (PoolInfo memory) {
    ...
    if (block.timestamp > _poolInfo.lastRewardTime) {
      ...
      if (_stakedBalance > 0) {
        uint256 _timePast = block.timestamp - _poolInfo.lastRewardTime;
        uint256 _rewards = (_timePast *
          rewardPerSecond *
          _poolInfo.allocPoint) / totalAllocPoint;
  ...
  function setMaxRewardPerSecond(uint256 _maxRewardPerSecond)
    external
    onlyOwner
  {
