...
import "./interfaces/IPoolTreasury.sol";
...
contract Pool is ReentrancyGuard, IPool {
    ...
    uint256 xShareDailyMinted;
    uint256 xShareDailyMintLimitation;
    uint256 xShareDailyMintLimitPercentage = 1000;
    ...
    modifier checkDayStart() {
        if (block.timestamp > currentDayStartTs + 86400) {
            ...
            xShareDailyMintLimitation = xShareCirSupply.mul(xShareDailyMintLimitPercentage).div(10000); //Set max minted daily based on xShare circulating supply
            xShareDailyMinted = 0;
        }
        _;
    }

    modifier notExceedXShareDailyMinted() {
        require(xShareDailyMinted < xShareDailyMintLimitation, "Exceeds xShare daily mint limit");
        _;
    }
    ...
    // Returns alpha value of collateral held in this pool
    function collateralalphaBalance() external view override returns (uint256) {
    ...
    function mint(
        ...
        (uint256 _alphaPrice, uint256 _share_price, , uint256 _target_collateral_ratio, , , uint256 _minting_fee, ) = IPoolTreasury(treasury).info();
    ...
    function redeem(
    ...
    ) external notMigrated checkDayStart notExceedXShareDailyMinted{
        require(redeem_paused == false, "Redeeming is paused");

        (, uint256 _share_price, , , uint256 _effective_collateral_ratio, , , uint256 _redemption_fee) = IPoolTreasury(treasury).info();
        ...
        if (_share_output_amount > 0) {
            XShareToken(_xShare).poolMint(address(this), _share_output_amount);
    ...
    function collectRedemption() external {
        ...
        if (_send_share) {
            ERC20(_xShare).transfer(msg.sender, _share_amount);
        }
...
interface IPool {
    function collateralalphaBalance() external view returns (uint256);
