...
contract ChillLab is ChillPillRarity, ReentrancyGuard {
    ...
    function pendingRewards(uint _pid, address _user) public view returns (uint schillReward, uint pillReward) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if(pool.sChillStakedAmount != 0) accRewardPerShare += reward * (10000 - pillBoost) / 10000 * precisionOf[pool.TokenPrecision] / pool.sChillStakedAmount;
            if(pool.mpStakedAmount != 0) accRewardPerShareChillPill += reward * pillBoost / 10000 * precisionOf[pool.TokenPrecision] / pool.mpStakedAmount;
        }
    ...
    function updatePool(uint _pid) public {
        ...
        uint reward = pool.RewardPerSecond * getMultiplier(pool.lastRewardTime, block.timestamp, pool.startTime, pool.endTime);
        if(pool.sChillStakedAmount != 0) pool.accRewardPerShare += reward * (10000 - pillBoost) / 10000 * precisionOf[pool.TokenPrecision] / pool.sChillStakedAmount;
        if(pool.mpStakedAmount != 0) pool.accRewardPerShareChillPill += reward * pillBoost / 10000 * precisionOf[pool.TokenPrecision] / pool.mpStakedAmount;
    ...
    function setPillBoost(uint32 _boost) external onlyAdmin {
        pillBoost=_boost;
    }
