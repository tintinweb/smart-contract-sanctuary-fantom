...
// File contracts/EqualizerFtmLinkvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualizerFtmLinkvAMM is Ownable, Pausable {
    ...
    address public constant link = address(0xb3654dc3D10Ea7645f8319668E8F54d2574FBdC8);
    address public want;
    ...
    IEqualizerRouter.Routes[] public wrappedToLinkPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToLinkPath
    ) {
        ...
        for (uint i; i < _wrappedToLinkPath.length; ++i) {
           wrappedToLinkPath.push(_wrappedToLinkPath[i]);
        }
   ...
   //Withdraws funds from masterchef
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(wrappedHalf, 0, wrappedToLinkPath, address(this), block.timestamp); 

        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(link).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(wrapped, link, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);   
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(link).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(link).safeApprove(router, type(uint).max);
    }
