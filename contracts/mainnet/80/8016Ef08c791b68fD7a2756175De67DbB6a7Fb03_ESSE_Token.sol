...
contract ESSE_Token is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for additional exclusve ESSE-FTM LP -> ESSE pool
    uint256 public constant INITIAL_ESSE_POOL_DISTRIBUTION = 10000 ether;
    ...
    /**
     * @notice Constructs the ESSE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Esse Token", "ESSE") {
        // Mints 1 ESSE to contract creator for initial pool setup
    ...
    function _getEssePrice() internal view returns (uint256 _EssePrice) {
        try IOracle(EsseOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Esse: failed to fetch ESSE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _EssePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_EssePrice >= taxTiersTwaps[tierId]) {
    ...
    function setEsseOracle(address _EsseOracle) public onlyOperatorOrTaxOffice {
        require(_EsseOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ESSE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ESSE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEssePrice = _getEssePrice();
            currentTaxRate = _updateTaxRate(currentEssePrice);
            if (currentEssePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_EssePool != address(0), "!_EssePool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_EssePool, INITIAL_ESSE_POOL_DISTRIBUTION);
    }
