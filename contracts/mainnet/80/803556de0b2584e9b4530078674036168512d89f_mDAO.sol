...
contract mDAO is ERC20Burnable, Operator {
    ...
    function _getmDAOPrice() internal view returns (uint256 _mDAOPrice) {
        try IOracle(mDAOOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Error: failed to fetch mDAO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _mDAOPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_mDAOPrice >= taxTiersTwaps[tierId]) {
    ...
    function setmDAOOracle(address _mDAOOracle) public onlyOperatorOrTaxOffice {
        require(_mDAOOracle != address(0), "oracle address cannot be 0 address");
        mDAOOracle = _mDAOOracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentmDAOPrice = _getmDAOPrice();
            currentTaxRate = _updateTaxRate(currentmDAOPrice);
            if (currentmDAOPrice < burnThreshold) {
                burnTax = true;
            }
        }
