...
*/
contract SwapStrategyToken is ISwapStrategy, Ownable {
    ...
    uint256 public swapSlippage = 250000; // 25%
    ...
    function execute(uint256 _wethIn, uint256 _yTokenOut) external override {
        collateralToken.safeTransferFrom(msg.sender, address(this), _wethIn);

        // 1. Swap all WETH to WFTM
        uint256 _wethReceived = swapToken(_wethIn, 1, address(collateralToken));
        // 2. Determine WFTM amount to swap to YToken
        ...
        if (_token0 == address(yToken)) {
            _wethToSwap = calculateSwapInAmount(_res1, _wethReceived);
        } else {
            _wethToSwap = calculateSwapInAmount(_res0, _wethReceived);
        }
        if (_wethToSwap <= 0) _wethToSwap = _wethReceived / 2;

        uint256 _wethToAdd = _wethReceived - _wethToSwap;
        // 3. Swap WFTM to YToken
        uint256 _yTokenReceived = swap(_wethToSwap, 1);

        // 4. add liquidity for YToken/WETH LP
    ...
    function swapToken(
        ...
        IERC20(_token).safeIncreaseAllowance(address(swapRouter), _wethToSwap);
        uint256[] memory _amounts = swapRouter.swapExactTokensForTokens(
            _wethToSwap,
            _minYTokenOut,
            path,
