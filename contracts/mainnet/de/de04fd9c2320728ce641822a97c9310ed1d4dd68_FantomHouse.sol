...
contract FantomHouse is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    // Info of each House Depositor
    struct DepositorInfo {
        uint256 amount;
        uint256 accRewardsSnapshot;
    }

    // Info of each pool.
    struct HouseInfo {
        uint256 balance;    // total House balance
        uint256 accRewards; // Accumulated rewards
    }

    HouseInfo public houseInfo;

    // Info of each depositor mapped to address
    ...
    // distribute rewards to House Depositors
    // called anytime the House receives rewards
    function distributeRewards(uint256 _rewards) public {
        HouseInfo storage house = houseInfo;
        if (house.balance > 0) {
            house.accRewards = house.accRewards + _rewards / house.balance;
        }
    }

    function minusRewards(uint256 _rewards) public {
        HouseInfo storage house = houseInfo;
        if (house.balance > 0 && house.accRewards >= _rewards) {
            house.accRewards = house.accRewards - _rewards / house.balance;
        }
    }

    // Deposit into House
    function deposit(uint256 _amount, address _referral) public {
        // TODO: NEEDS referral / treasury fee calculations
        address _sender = msg.sender;
        HouseInfo storage house = houseInfo;
        DepositorInfo storage depositor = depositorInfo[_sender];
        if (depositor.amount > 0) {
            // claim rewards for the user
            // that way we can reset the accRewardsSnapshot value for them
            uint256 _pending = depositor.amount * (house.accRewards - depositor.accRewardsSnapshot);
            if (_pending > 0) {
                // TRANSFER OUT REWARDS TO USER
                // requires to validate balance and stuff first
                usdcToken.safeTransfer(_sender, _pending);

                emit RewardPaid(_sender, _pending);
            }
        }
        if (_amount > 0) {
            // HANDLE FEE STUFF !!!! skipped for the example
            ...
            // update total House balance
            house.balance += amountAfterFee;
            // update depositor amount
            depositor.amount += amountAfterFee;
            // tranfer form user to House
            usdcToken.safeTransferFrom(_sender, address(this), amountAfterFee);
        }

        depositor.accRewardsSnapshot = house.accRewards;
        // snapshot accumulated rewards at the time of latest deposit

        emit Deposit(_sender, _amount);
    }

    // Withdraw deposit from House
    function withdraw(uint256 _amount) public {
        address _sender = msg.sender;
        HouseInfo storage house = houseInfo;
        DepositorInfo storage depositor = depositorInfo[_sender];
        // rewards are always claimed on deposit / withdraw so they are handled separately
        // so only allow withdrawal of deposited amount
        require(depositor.amount >= _amount, "withdraw: not good");
        ...
        if (_amount > 0) {
            // update total House balance
            house.balance -= _amount;
            // update depositor amount
            depositor.amount -= _amount;
            // transfer out to user
            //            pool.token.safeTransfer(_sender, _amount);
            usdcToken.safeTransfer(_sender, _amount);
        }
        // we don't modify depositor.accRewardsSnapshot since that is updated on Deposit
        // they may have only withdraw partial amount
    ...
    function getUserHouseBalance(address _address) public view returns (uint256) {
        ...
        return depositorInfo[_address].amount;
    }
    ...
    function bet(uint256 _betAmount, uint256 _faceSide, address _referral) external nonReentrant {
        ...
        require(_betAmount <= maxBetPercent * houseInfo.balance / 1e18 / 100, "Larger than maximum bet amount");
    ...
    function fulfillRandomWords(
        ...
        if (userWon == 1) {
            ...
            uint256 _reward = payoutAppliedAmount + calculatedFee;
            minusRewards(_reward);
            //            for (uint256 i = 0; i < depositors.length; i++) {
            ...
            uint256 _reward = playerBetAmount - calculatedFee;
            distributeRewards(_reward);
    ...
    function returnDepositAndRemoveDepositor(uint256 _length, bool _shouldRemoveDepositor) internal {
        for (uint i = 0; i < _length; i++) {
            address _depositor = depositors[i];
            uint256 _depositAmount = depositorInfo[_depositor].amount;
            usdcToken.safeTransfer(_depositor, _depositAmount);
            houseInfo.balance -= _depositAmount;
            delete depositorInfo[_depositor];
