...
// File contracts/CrossSwapHandler.sol

pragma solidity 0.8.4;

contract CrossSwapHandler is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeERC20 for IWETH;
    ...
    function getAmountOut(
        ...
        uint256 denominator = reserveIn * feePrecision + amountInWithFee;
        amountOut = numerator / denominator;
    }

    function getAmountsOut(uint256 amountIn, Step[] memory steps)
        external
        view
        returns (uint256[] memory amounts)
    {
        amounts = _getAmountsOut(amountIn, steps);
    }

    function _getAmountsOut(uint256 amountIn, Step[] memory steps)
        internal
        view
        returns (uint256[] memory amounts)
    {
        require(steps.length >= 2, "UniswapV2Library: INVALID_PATH");
        amounts = new uint256[](steps.length + 1);
        amounts[0] = amountIn;
    ...
    function execute(
        ...
        uint256[] memory amountOuts = _getAmountsOut(_amountIn, _steps);
        require(
            amountOuts[amountOuts.length - 1] > _minAmountOut,
            "CrossSwapHandler::execute: Amount out not enough"
        );

        uint256 preAmount = Utils.balanceOf(_steps[0].tokenIn, address(this));
        Utils.transfer(_steps[0].tokenIn, _steps[0].pair, _amountIn);
        for (uint256 i = 0; i < _steps.length; i++) {
            Step memory step = _steps[i];
            console.log(step.tokenIn);
            console.log(step.pair);
            console.log(step.fee);
            console.log(step.feePrecision);
            address to = i < _steps.length - 1 ? _steps[i + 1].pair : address(this);
            IUniswapV2Pair pair = IUniswapV2Pair(step.pair);
        ...
        uint256 postAmount = Utils.balanceOf(_steps[0].tokenIn, address(this));
        require(postAmount > preAmount, "UniswapV2Handler::execute: amount out not enough");

        emit Swaped(_amountIn, amountOuts[amountOuts.length - 1]);
    }
