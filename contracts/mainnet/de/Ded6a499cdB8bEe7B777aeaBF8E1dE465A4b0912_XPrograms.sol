...
abstract contract Matrix is IMatrix, Ownable2Step {
	...
	mapping(uint => uint) private sl;
	mapping(uint => mapping(uint => uint[])) private l1;
	mapping(uint => mapping(uint => uint)) private uid;
	uint public CFStatus = TRUE;
	uint public CFRatio = 5; // %

	function MatrixInitializer()
		internal
		notMatrixInitialized
	{
	...
	function UpdateCFStatusAndCFRatio(
		uint _CFStatus,
		uint _CFRatio
	) external onlyOwner {
		require(
			_CFRatio <= 10 &&
				(_CFStatus == FALSE || _CFStatus == TRUE)
		);
		CFStatus = _CFStatus;
		CFRatio = _CFRatio;
	}
	...
	function UplineOfNode(
		uint _AID
	) public view returns (uint UU_, uint UB_, uint UT_) {
		return (
			_UplineOnMatrix(_AID, UNILEVEL),
			_UplineOnMatrix(_AID, BINARY),
			_UplineOnMatrix(_AID, TERNARY)
		);
	}

	function _UplineOnMatrix(
		uint _AID,
		uint _MATRIX
	) internal view returns (uint UID_) {
		return uid[_AID][_MATRIX];
	}

	function _SetUplineOnMatrix(
		uint _AID,
		uint _MATRIX,
		uint _UID
	) private {
	...
	function _InitNode(
		...
		if (
			F1OfNode(_SID, UNILEVEL).length == 3 &&
			SponsorLevel(_SID) < 15
		) _UpdateSponsorLevelForUpline(_AID);
	...
	function _VerifyUplineID(
		...
		uint _UID,
		uint _MATRIX
	) private view returns (bool Success_) {
		if (F1OfNode(_UID, _MATRIX).length >= _MATRIX)
			return (false);
		while (_UID != 0) {
			if (_UID == _SID) {
				return (true);
	...
	function _UpdateSponsorLevelForUpline(uint _AID) private {
		...
		if (s2sl >= 2 && s2sl <= 4) {
			...
			if (s2sl == 5) {
				s2sl += (
					SLTrack[s2].F2SL2 >= 9 ? 9 : SLTrack[s2].F2SL2
				);
				s2sl5 = true;
			}
			_SetSponsorLevel(s2, s2sl);
		}
		...
		if (s2sl5 && ++SLTrack[s3].F1SL5 >= 10 && s3sl < 15)
			_SetSponsorLevel(s3, 15);
		if (s3sl >= 5 && s3sl < 14) ++sl[s3];
		uint s4 = _UplineOnMatrix(s3, UNILEVEL);
		if (s4 == 0) return;
		if (++SLTrack[s4].F3SL2 >= 27 && SponsorLevel(s4) < 15)
			_SetSponsorLevel(s4, 15);
	}
}
...
abstract contract XProgram is IXProgram, Matrix {
	...
	mapping(uint => mapping(uint => mapping(uint => uint)))
		private L4U;
	mapping(uint => uint) private ALU;
	bool public Initialized;
	modifier notXProgramInitialized() {
		MatrixInitializer();
		require(
			!(Initialized.v(Cycles, LA) && Initialized),
			"Already initialized"
		);
		_;
	}
	...
	function isLevelActivated(
		...
		uint _XPro,
		uint _Level
	) public view returns (bool isLA_) {
		return LA[_AID][_XPro][_Level] == TRUE ? true : false;
	}

	function _SetLevelActivity(
		uint _AID,
		uint _XPro,
		uint _Level,
		uint _Status
	...
	function _isLocked4Upgrade(
		uint _AID,
		uint _XPro,
		uint _Level
	) internal view returns (bool isL4U_) {
		return L4U[_AID][_XPro][_Level] == TRUE ? true : false;
	}

	function _SetLock4Upgrade(
		uint _AID,
		uint _XPro,
		uint _Level,
		uint _Status
	...
	function isAutoLevelUp(
		uint _AID
	) public view returns (bool isALU_) {
		return ALU[_AID] == TRUE ? true : false;
	}

	function _SetAutoLevelUp(
		uint _AID,
		uint _Status
	) internal {
		ALU[_AID] = _Status;
	...
	function GetCurrentCycle(
		...
		uint _XPro,
		uint _Level
	) internal view returns (Cycle storage currentCycle_) {
		return
			Cycles[_AID][_XPro][_Level][
				GetCycleCount(_AID, _XPro, _Level)
			];
	}
	...
	function _Recycling(
		...
		uint _XPro,
		uint _Level
	) internal virtual returns (uint cycleCount_) {
		return ++CycleCounts[_AID][_XPro][_Level];
	}
...
contract XPrograms is XProgram {
	...
	modifier OnlyAccountOwner(uint _AID, address _Owner) {
		require(
			Accounts.RegistrationTime(_AID) != 0 &&
				_Owner == Accounts.AddressOfAccount(_AID),
	...
	modifier OnlyAPI() {
		require(
			...
			"XProgram: caller is not API"
		);
		_;
	}

	function _InitXPrograms(
		uint _AID,
		uint _LOn
	) external onlyTuktu {
		// X3
	...
	function _UpgradeLevel(
		uint _AID,
		uint _XPro,
		uint _LTo
	) private {
		if (
			Balances.AvailableToUpgrade(_AID) >=
			Tuktus.PirceOfLevel(_LTo)
		) {
	...
	function _Recycling(
		...
		uint _XPro,
		uint _Level
	) internal override returns (uint cycleCount_) {
		cycleCount_ = super._Recycling(_AID, _XPro, _Level);
		if (_AID.v())
			emit Recycled(_AID, _XPro, _Level, cycleCount_);
		...
		if (isLevelActivated(_AID, _XPro, _Level + 1))
			return cycleCount_;
		if (isAutoLevelUp(_AID))
			_UpgradeLevel(_AID, _XPro, _Level + 1);
		else if (cycleCount_ > 0)
			_LockedToUpgrade(_AID, _XPro, _Level + 1);
	}
	...
	function _ShareReward(
		...
		uint cu1 = GetCurrentCycle(_AID, _XPro, _Level)
			.CycleUplineID;
		if (cu1 == 0) return true;

		Cycle storage ccu1 = GetCurrentCycle(
			cu1,
			_XPro,
			_Level
		);
		if (_XPro == X3) {
			if (ccu1.XY[1][3] == _AID)
				Balances._Locking(cu1, 0, lc);
			if (cu1.v())
				emit RewardGiven(cu1, lc, _AID, _XPro, _Level);
			return Balances._TransferReward(_AID, cu1, lc);
		}
		uint cu2 = ccu1.CycleUplineID;
		if (cu2 == 0)
			return Balances._TransferReward(_AID, cu1, lc);
		Cycle storage ccu2 = GetCurrentCycle(
			cu2,
			_XPro,
			_Level
		);
		...
		if (_XPro == X6 || _XPro == X7) {
			acu2 = lc;
			if (_XPro == X6 && ccu2.XY[2][3] == _AID)
				Balances._Locking(cu2, 0, acu2);
			else if (_XPro == X7 && ccu2.XY[2][7] == _AID)
				Balances._Locking(cu2, 0, acu2);
			if (CFStatus == TRUE) {
				acu2 = (acu2 * (100 - CFRatio)) / 100;
				Balances._TransferReward(
					_AID,
					Accounts.CFID(),
					lc - acu2
				);
			...
			if (cu2.v())
				emit RewardGiven(cu2, acu2, _AID, _XPro, _Level);
			return Balances._TransferReward(_AID, cu2, acu2);
		}
		uint cu3 = ccu2.CycleUplineID;
		if (cu3 == 0)
			return Balances._TransferReward(_AID, cu2, lc);
		Cycle storage ccu3 = GetCurrentCycle(
			cu3,
			_XPro,
			_Level
		);
		...
		if (_XPro == X8 || _XPro == X9) {
			...
			if (_XPro == X8) {
				if (ccu2.XY[2][4] == _AID)
					Balances._Locking(cu2, 0, acu2);
				if (ccu3.XY[3][7] == _AID)
					Balances._Locking(cu3, 0, acu3);
			} else {
				if (ccu2.XY[2][9] == _AID)
					Balances._Locking(cu2, 0, acu2);
				if (ccu3.XY[3][25] == _AID)
					Balances._Locking(cu3, 0, acu3);
			}
			...
			if (cu2.v())
				emit RewardGiven(cu2, acu2, _AID, _XPro, _Level);
			if (cu3.v())
				emit RewardGiven(cu3, acu3, _AID, _XPro, _Level);
			return (Balances._TransferReward(_AID, cu2, acu2) &&
				Balances._TransferReward(_AID, cu3, acu3));
		}
	}

	function _FindCurrentCycleUpline(
		uint _AID,
		uint _XPro,
		uint _Level
	) private {
		uint pendingToRecycle = _InitX(_AID, _XPro, _Level);
		_ShareReward(_AID, _XPro, _Level);
		if (pendingToRecycle != 0)
			_Recycling(pendingToRecycle, _XPro, _Level);
	}

	function _FindCycleUpline(
		uint _AID,
		uint _XPro,
		uint _Level
	) private returns (uint cu_) {
		uint aSL = SponsorLevel(_AID);
		uint matrix = _XPro == X3
			? UNILEVEL
			: (_XPro == X7 || _XPro == X9)
			? TERNARY
			: BINARY;
		...
		if (cu_ != 0)
			while (_UplineOnMatrix(cu_, matrix) != 0) {
				if (
					isLevelActivated(cu_, _XPro, _Level) &&
					SponsorLevel(cu_) >= aSL
				) return cu_;
	...
	function _InitX(
		...
		uint _XPro,
		uint _Level
	) private returns (uint PendingToRecycle_) {
		uint C = _FindCycleUpline(_AID, _XPro, _Level);
		if (C == 0) return 0;

		uint spill = C != _UplineOnMatrix(_AID, UNILEVEL)
			? BELOW
			: DIRECT;
		Cycle storage CurrentCycle = GetCurrentCycle(
			C,
			_XPro,
			_Level
		);
		...
		if (_XPro == X3) {
			...
			if (ay_L1C == 3) PendingToRecycle_ = C;
			if (C.v())
				emit NewPartner(C, _XPro, _Level, _AID, spill);
			...
			if (CurrentCycle.XCount[1] < 2) {
				...
				if (B != 0) {
					...
					uint ay_L2B = (
						CurrentCycle.XY[1][1] == C ? 1 : 2
					) == 1
						? ay_L1C
						: ay_L1C + 2;
					CurrentCycle.XY[2][ay_L2B] = _AID;
					if (B.v())
						emit NewPartner(B, _XPro, _Level, _AID, BELOW);
					if (++CurrentCycle.XCount[2] == 4)
						PendingToRecycle_ = B;
				}
				GetCurrentCycle(_AID, _XPro, _Level)
					.CycleUplineID = C;
				...
				CurrentCycle.XY[1][
					(ay_L2C > 2 ? ay_L2C - 2 : ay_L2C)
				] = _AID;
				if (D.v())
					emit NewPartner(D, _XPro, _Level, _AID, ABOVE);
			...
			if (CurrentCycle.XCount[1] < 2) {
				...
				CurrentCycle.XY[3][ay_L3C] = _AID;
				if (C.v())
					emit NewPartner(C, _XPro, _Level, _AID, spill);
				if (ay_L3C == 8) PendingToRecycle_ = C;
				uint D = ay_L3C > 4
					? CurrentCycle.XY[1][2]
					: CurrentCycle.XY[1][1];
				CurrentCycle = GetCurrentCycle(D, _XPro, _Level);
				uint ay_L2D = ay_L3C > 4 ? ay_L3C - 4 : ay_L3C;
				CurrentCycle.XY[2][ay_L2D] = _AID;
				if (D.v())
					emit NewPartner(D, _XPro, _Level, _AID, ABOVE);
				++CurrentCycle.XCount[2];
				...
				CurrentCycle.XY[1][ay_L1E] = _AID;
				if (E.v())
					emit NewPartner(E, _XPro, _Level, _AID, ABOVE);
				++CurrentCycle.XCount[1];
				GetCurrentCycle(_AID, _XPro, _Level)
					.CycleUplineID = E;
			}
			...
			if (CurrentCycle.XCount[1] < 3) {
				...
				if (B != 0) {
					CurrentCycle = GetCurrentCycle(B, _XPro, _Level);
					uint cy_L1B = CurrentCycle.XY[1][1] == C
						? 1
						: CurrentCycle.XY[1][2] == C
						? 2
						: 3;
					CurrentCycle.XY[2][
						(((cy_L1B - 1) * 3) + ay_L1C)
			...
			if (CurrentCycle.XCount[1] < 3) {
				...
				if (B != 0) {
					...
					if (A != 0) {
						...
						uint by_L1A = CurrentCycle.XY[1][1] == B
							? 1
							: CurrentCycle.XY[1][2] == B
							? 2
							: 3;
						CurrentCycle.XY[3][
							by_L1A == 1 ? ay_L2B : by_L1A == 2
								? ay_L2B + 9
								: ay_L2B + 18
						] = _AID;
						if (A.v())
				...
				if (C.v())
					emit NewPartner(C, _XPro, _Level, _AID, spill);
				if (ay_L3C == 27) PendingToRecycle_ = C;
				uint D = ay_L3C > 18
					? CurrentCycle.XY[1][3]
					: ay_L3C > 9
					? CurrentCycle.XY[1][2]
					: CurrentCycle.XY[1][1];
				CurrentCycle = GetCurrentCycle(D, _XPro, _Level);
				uint ay_L2D = ay_L3C > 18 ? ay_L3C - 18 : ay_L3C > 9
					? ay_L3C - 9
					: ay_L3C;
				CurrentCycle.XY[2][ay_L2D] = _AID;
				if (D.v())
	...
	function _LockedToUpgrade(
		...
		if (!_isLocked4Upgrade(_AID, _XPro, _Level)) {
			...
			Balances._Locking(
				_AID,
				1,
				Tuktus.PirceOfLevel(_Level)
			);
		}
	}

	function _UnlockWhenUpgraded(
		uint _AID,
		uint _XPro,
		uint _Level
	) private {
	...
	function _ChangeStatusALU(
		...
		if (isAutoLevelUp(_AID)) {
			...
			if (
				!isLevelActivated(_AID, X3, 2) &&
				GetCycleCount(_AID, X3, 1) == 0
			) _UnlockWhenUpgraded(_AID, X3, 2);

			for (uint xp = X6; xp <= X9; ++xp)
				for (uint lv = 2; lv <= 15; ++lv)
					if (!isLevelActivated(_AID, xp, lv)) {
						if (GetCycleCount(_AID, xp, lv - 1) == 0)
							_UnlockWhenUpgraded(_AID, xp, lv);
						break;
					}
			...
			if (!isLevelActivated(_AID, X3, 2))
				_LockedToUpgrade(_AID, X3, 2);
