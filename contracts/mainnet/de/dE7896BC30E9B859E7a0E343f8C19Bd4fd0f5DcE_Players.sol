...
contract Donation is UUPSUpgradeable, OwnableUpgradeable, IOracleRewardCB {
  ...
  event ClanDonationThreshold(uint startThreshold, uint thresholdIncrement, uint16 rewardItemTokenId);
  ...
  mapping(uint clanId => ClanInfo clanInfo) private clanDonationInfo;
  Clans private clans;
  uint40 private startClanThreshold;
  uint40 private clanThresholdIncrement;
  ...
  function initialize(
    ...
    nextRewardAmount = 1;
    lastLotteryId = 1;
    startClanThreshold = uint40(_startClanThreshold / 1 ether);
    clanThresholdIncrement = uint40(_startClanThreshold / 1 ether);

    emit SetRaffleEntryCost(_raffleEntryCost);
    emit NextDonationThreshold(_startGlobalThreshold, PRAY_TO_THE_BEARDIE);
    emit ClanDonationThreshold(_startClanThreshold, _startClanThreshold, CLAN_BOOST);
    ...
    if (_playerId != 0) {
      ...
      if (hasEnoughForRaffle) {
        ...
        if (clanId != 0) {
          ...
          if (clanDonationInfo[clanId].totalDonated >= clanDonationInfo[clanId].lastThreshold) {
            // Give them a clan reward
            clanItemTokenId = donationRewardItemTokenId;
  ...
  function setStartClanDonationThreshold(uint _startClanThreshold, uint _clanThresholdIncrement) external onlyOwner {
    startClanThreshold = uint40(_startClanThreshold / 1 ether);
    clanThresholdIncrement = uint40(_clanThresholdIncrement / 1 ether);
    emit ClanDonationThreshold(_startClanThreshold, _clanThresholdIncrement, CLAN_BOOST);
  }
