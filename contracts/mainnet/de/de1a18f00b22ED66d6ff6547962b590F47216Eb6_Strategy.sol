...
import {ISolidlyRouter, route} from '../interfaces/ISolidlyRouter.sol';
...
contract Strategy is BaseStrategy {
    ...
    IV1Pair public solidlyLpToken; //  StableV1 AMM - USDC/MIM
    ...
    uint256 public swapSecurityFactor = 9990;
    ...
    constructor(
        ...
        require(solidlyLpToken.token0() == address(want), 'Not correctPool');
        ...
        rewardDust = uint256(1).mul(10**uint256(ERC20(address(solidReward)).decimals()));
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('Solidly_USDC-MIM_strategy'));
    }
    ...
    function pendingRewards() public view returns (uint256 _shares1, uint256 _shares2) {
        address[] memory pools = new address[](1);
        pools[0] = address(solidlyLpToken);
        Amounts[] memory amounts = masterChef.pendingRewards(address(this), pools);
        _shares1 = amounts[0].solid;
        _shares2 = amounts[0].sex;
    }

    function balanceOfPooled() public view returns (uint256 _amount) {
        (uint256 reserveA, uint256 reserveB, ) = solidlyLpToken.getReserves();
        uint256 totalSupply = solidlyLpToken.totalSupply();

        uint256 totalLpTokens = balanceOfLPInMasterChef().add(balanceOfLpTokens());
        ...
        (uint256 lpToken1InWant, ) =
            ISolidlyRouter(router).getAmountOut(lpToken1Amount, lpPairToken1, address(want));
    ...
    function sellMIMTokens() internal {
    ...
    function sellAllRewards() internal {
        ...
        route[] memory sexToWantRoutes = new route[](2);
        route memory sexToWant = route(sexReward, wftm, false);
        sexToWantRoutes[0] = sexToWant;
        sexToWantRoutes[1] = wftmToWant;

        route[] memory solidToWantRoutes = new route[](2);
        route memory solidToWant = route(solidReward, wftm, false);
        solidToWantRoutes[0] = solidToWant;
        solidToWantRoutes[1] = wftmToWant;
    ...
    function removeLiquidity(uint256 toExitAmount) internal {
        ...
        uint256 wantPerMim = solidlyLpToken.getAmountOut(1e18, lpPairToken1);
        uint256 totalReservesInWant = wantPerMim.mul(reserveB).div(1e18).add(reserveA);

        uint256 wantPerLP = totalReservesInWant.mul(1e18).div(totalSupply);
        uint256 liquidityToExit = toExitAmount.mul(1e18).div(wantPerLP);
    ...
    function _withdrawFromMasterChefAndTransfer(address _to) internal {
        ...
        if (lpTokensBalance > 0) {
            solidlyLpToken.approve(_to, lpTokensBalance);
...
contract CommonHealthCheck {
    ...
    function setDisabledCheck(address _strategy, bool disabled)
        external
        onlyAuthorized
    {
    ...
    function _runChecks(
        ...
        return
            ICustomHealthCheck(customCheck).check(
                strategy,
                profit,
                loss,
                debtPayment,
