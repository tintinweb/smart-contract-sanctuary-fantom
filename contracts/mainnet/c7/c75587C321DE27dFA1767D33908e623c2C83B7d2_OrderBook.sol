...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    uint256 public constant USDE_PRECISION = 1e18;
    ...
    function initialize(
        ...
        emit Initialize(_router, _vault, _weth, _usde, _minExecutionFee, _minPurchaseTokenAmountUsd);
    }
    ...
    function getUsdeMinPrice(address _otherToken) public view returns (uint256) {
        // USDE_PRECISION is the same as 1 USDE
        uint256 redemptionAmount = IVault(vault).getRedemptionAmount(_otherToken, USDE_PRECISION);
    ...
    function validateSwapOrderPriceWithTriggerAboveThreshold(
        ...
        // 1. USDE doesn't have a price feed so we need to calculate it based on redepmtion amount of a specific token
        // That's why USDE price in USD can vary depending on the redepmtion token
        // 2. In complex scenarios with path=[USDE, BNB, BTC] we need to know how much BNB we'll get for provided USDE
        // to know how much BTC will be received
        // That's why in such scenario BNB should be used to determine price of USDE
        if (tokenA == usde) {
            // with both _path.length == 2 or 3 we need usde price against _path[1]
            tokenAPrice = getUsdeMinPrice(_path[1]);
        } else {
    ...
    function _vaultSwap(address _tokenIn, address _tokenOut, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (_tokenOut == usde) { // buyUSDE
            amountOut = IVault(vault).buyUSDE(_tokenIn, _receiver);
        } else if (_tokenIn == usde) { // sellUSDE
            amountOut = IVault(vault).sellUSDE(_tokenOut, _receiver);
        } else { // swap
