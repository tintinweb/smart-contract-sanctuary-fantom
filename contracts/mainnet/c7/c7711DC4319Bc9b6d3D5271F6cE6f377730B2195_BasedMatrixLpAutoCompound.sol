...
contract BasedMatrixLpAutoCompound is MatrixLpAutoCompound {
    ...
    function _setDefaultSwapPaths() internal override {
        // Force swap to go trou SPOOKYSWAP instead of TOMBSWAP
        // Even if we provide liquidity to TOMBSWAP
        // Cause we get best routes and liquidity for these pairs
        ...
        if (lpToken0 == wrapped) {
            ...
            _path[0] = output;
            _path[1] = wrapped;
            _setSwapPath(output, lpToken0, SPOOKYSWAP_ROUTER, _path);
        } else if (lpToken0 != output) {
            address[] memory _path = new address[](3);
            _path[0] = output;
            _path[1] = wrapped;
        ...
        if (lpToken1 == wrapped) {
            ...
            _path[0] = output;
            _path[1] = wrapped;
            _setSwapPath(output, lpToken1, SPOOKYSWAP_ROUTER, _path);
        } else if (lpToken1 != output) {
            address[] memory _path = new address[](3);
            _path[0] = output;
            _path[1] = wrapped;
        ...
        if (output != wrapped) {
            ...
            _path[0] = output;
            _path[1] = wrapped;
            _setSwapPath(output, wrapped, SPOOKYSWAP_ROUTER, _path);
        }
    }
}