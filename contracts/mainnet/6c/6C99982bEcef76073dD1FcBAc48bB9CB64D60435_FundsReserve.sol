 ...
 * This contract is excluded from tax for each token
 * Total funds
 * Runic: 1_250_000 ether. 50_000 ether (initial dev) + 250_000 ether (liquidity) + 950_000 ether (vested dev)
 * Fate: 49_900 ether. 100 ether (initial dev) + 100 (liquidity) + 9900 ether (vested dev) + 25_000 ether (dao) + 14_800 ether (insurance)
 * Runite: 600_000 ether. 25_000 ether (initial dev) + 100_000 ether (liquidity) + 475_000 ether (vested dev)
 */
contract FundsReserve is AccessControlEnumerable, ReentrancyGuard, IUpdate {
    ...
    uint256 public constant runicInitialDevFunds = 50_000 ether;
    uint256 public constant fateInitialDevFunds = 100 ether;
    uint256 public constant runiteInitialDevFunds = 25_000 ether;
    uint256 public constant runicInitialLiquidity = 250_000 ether;
    uint256 public constant fateInitialLiquidity = 100 ether;
    uint256 public constant runiteInitialLiquidity = 100_000 ether;
    uint256 public constant runicTotalDevFunds = 950_000 ether;
    uint256 public constant runicVestingDuration = 730 days;
    uint256 public constant fateTotalDevFunds = 9_900 ether;
    uint256 public constant fateVestingDuration = 730 days;
    uint256 public constant runiteTotalDevFunds = 475_000 ether;
    uint256 public constant runiteVestingDuration = 365 days;
    uint256 public constant fateTotalDaoFunds = 25_000 ether;
    uint256 public constant fateTotalInsuranceFunds = 14_800 ether;
    uint256 public startTime;
    uint256 public lastClaimTime;
    bool public initialized;
    bool public initialSent;
    ...
    /**
     * @notice Initializes addresses
     * Must exclude each address
     * Afterwards send funds to contract
     */
    function initialize(address runic_, address fate_, address runite_, address devFunds_, address daoFunds_, address insuranceFunds_, address liquidityFunds_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        ...
        devFunds = devFunds_;
        daoFunds = daoFunds_;
        insuranceFunds = insuranceFunds_;
        liquidityFunds = liquidityFunds_;
        initialized = true;
    }

    /**
     * @dev send initial funds once received
     */
    function initialFunds() external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        require(!initialSent && initialized, "Initial sent");
        // initial dev funds
        _safeTransferERC20(runic, devFunds, runicInitialDevFunds);
        _safeTransferERC20(fate, devFunds, fateInitialDevFunds);
        _safeTransferERC20(runite, devFunds, runiteInitialDevFunds);
        // initial liquidity
        _safeTransferERC20(runic, liquidityFunds, runicInitialLiquidity);
        _safeTransferERC20(fate, liquidityFunds, fateInitialLiquidity);
        _safeTransferERC20(runite, liquidityFunds, runiteInitialLiquidity);
        initialSent = true;
    ...
    function setInsuranceFunds(address insuranceFunds_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        insuranceFunds = insuranceFunds_;
    }

    function setLiquidityFunds(address liquidityFunds_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        liquidityFunds = liquidityFunds_;
    }

    function _distributeRunic() internal {
        uint256 amount = (block.timestamp - lastClaimTime) * (runicTotalDevFunds / runicVestingDuration);
        if(lastClaimTime < startTime + runicVestingDuration) {
            _safeTransferERC20(runic, devFunds, amount);
        }
    }

    function _distributeFate() internal {
        uint256 devAmount = (block.timestamp - lastClaimTime) * (fateTotalDevFunds / fateVestingDuration);
        uint256 daoAmount = (block.timestamp - lastClaimTime) * (fateTotalDaoFunds / fateVestingDuration);
        uint256 insuranceAmount = (block.timestamp - lastClaimTime) * (fateTotalInsuranceFunds / fateVestingDuration);
        if(lastClaimTime < startTime + fateVestingDuration) {
            _safeTransferERC20(fate, devFunds, devAmount);
            _safeTransferERC20(fate, daoFunds, daoAmount);
            _safeTransferERC20(fate, insuranceFunds, insuranceAmount);
        }
    }

    function _distributeRunite() internal {
        uint256 amount = (block.timestamp - lastClaimTime) * (runiteTotalDevFunds / runiteVestingDuration);
        if(lastClaimTime < startTime + runiteVestingDuration) {
            _safeTransferERC20(runite, devFunds, amount);
        }
    }

    /**
     * @dev allows this contract to be automatically updated by hooking onto another contract
     */
     ...
     */
    function withdraw(uint256 amount_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        _safeTransfer(msg.sender, amount_);
    }

    /**
     * @dev withdraw any other tokens
     */
    function withdrawERC20(address token_, uint256 amount_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        if(token_ == runic) {
            require(block.timestamp >= startTime + runicVestingDuration, "Not Time");
        } else if(token_ == fate) {
            require(block.timestamp >= startTime + fateVestingDuration, "Not Time");
        } else if(token_ == runite) {
            require(block.timestamp >= startTime + runiteVestingDuration, "Not Time");
        }
        _safeTransferERC20(token_, msg.sender, amount_);
    }

    function _safeTransfer(address to_, uint256 amount_) internal {
    ...
    function _safeTransferERC20(address token_, address to_, uint256 amount_) internal {
        if(amount_ > IERC20(token_).balanceOf(address(this))) {
            amount_ = IERC20(token_).balanceOf(address(this));
        }
        if(amount_ > 0) IERC20(token_).safeTransfer(to_, amount_);
    }
}
