...
contract Factory is IFactory {
    ...
    address public liquidationsAdmin;
    mapping(address => bool) public allowedToLiquidate;
    bool public isLiquidationPermissioned;

    function canLiquidate(address who) external view returns(bool){
        if(isLiquidationPermissioned){
            return allowedToLiquidate[who];
        }else{
            return true;
        }
    }
	
    function isLiquidationPermissionedToggle(bool value) external {
        require(msg.sender == liquidationsAdmin, "Not permitted");
        isLiquidationPermissioned = value;
     }
    
    function changeLiquidationAdmin(address newAdmin) external {
        require(msg.sender == liquidationsAdmin, "Not permitted");
        liquidationsAdmin = newAdmin;
    }

    function allowLiquidation(address who, bool value) external {
        require(msg.sender == liquidationsAdmin, "Not permitted");
        allowedToLiquidate[who] = value;
    }
    ...
    ITombPriceOracle public tombPriceOracle;
    ...
    constructor(
        ...
        ITombPriceOracle _tombPriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        tombPriceOracle = _tombPriceOracle;
        liquidationsAdmin = msg.sender;
        allowedToLiquidate[msg.sender] = true;
        isLiquidationPermissioned = true;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "MORTUARY: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "MORTUARY: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "MORTUARY: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "MORTUARY: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) =
            tombPriceOracle.getPair(uniswapV2Pair);
        if (!oracleInitialized) tombPriceOracle.initialize(uniswapV2Pair);
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "MORTUARY: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "MORTUARY: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "MORTUARY: UNAUTHORIZED");
