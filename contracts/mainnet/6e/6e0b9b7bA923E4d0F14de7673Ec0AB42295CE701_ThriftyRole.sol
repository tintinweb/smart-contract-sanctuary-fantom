...
contract ThriftyRole is Ownable {
  ...
  bool public enableFreeTransfer = false;
  bool public enableVesting = false;
  address[] public whiteList;
  address[] public blackList;
  mapping (address => uint256) private whiteListId;
  mapping (address => uint256) private blackListId;
  ...
  function addSellLog(address account, uint256 amount, uint256 sellAt) public onlyThrifty {
    sellHistory[account].push(sellInfo({
      amount: amount,
      sellAt: sellAt
    }));
  }

  function ableToSell(address account, uint256 userBalance, uint256 amount) internal view returns (bool) {
  ...
  /******************                  Calculate Fee                ****************************/
  /*********************************************************************************************/
  function getFee(uint256 totalSupply, address trader, uint256 amount, bool isTrade, bool isSell, bool noLimitAdress) public
    returns(uint256[7] memory fees)
  {
    ...
    // extraFee
    fees[6] = (noLimitAdress==true || isTrade==false) ? 0 : _getExtraFee(totalSupply, amount, isSell, trader);
    // receive amount
  ...
  function _getExtraFee(uint256 totalSupply, uint256 amount, bool isSell, address trader) internal returns(uint256 extraFee) {
