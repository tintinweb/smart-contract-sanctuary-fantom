...
contract HONEY is ERC20Burnable, Operator {
    ...
    function _getHoneyPrice() internal view returns (uint256 _honeyPrice) {
        try IOracle(honeyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            ...
            revert("Error: failed to fetch HONEY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _honeyPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_honeyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHoneyOracle(address _honeyOracle) public onlyOperatorOrTaxOffice {
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHoneyPrice = _getHoneyPrice();
            currentTaxRate = _updateTaxRate(currentHoneyPrice);
            if (currentHoneyPrice < burnThreshold) {
                burnTax = true;
            }
        }
