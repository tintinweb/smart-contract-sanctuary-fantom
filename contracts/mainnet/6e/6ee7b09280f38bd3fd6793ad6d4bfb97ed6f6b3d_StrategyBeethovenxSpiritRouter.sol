...
// File contracts/BIFI/strategies/Beethovenx/StrategyBeethovenxSpiritRouter.sol
...
contract StrategyBeethovenxSpiritRouter is StratFeeManager, GasFeeThrottler {
    ...
    address public spiritRouter = address(0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52); //new
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (toNative > 0) {
            IUniswapRouterETH(spiritRouter).swapExactTokensForTokens(
                toNative,
                0,
    ...
    function _giveAllowances() internal {
        ...
        IERC20(secondOutput).safeApprove(spiritRouter, type(uint256).max);
        if (secondOutput != native) {
            IERC20(native).safeApprove(spiritRouter, type(uint256).max);
        }
