...
contract sTwinkleERC20 is ERC20("sTWINKLE", "sTWINKLE"), ERC20Permit("sTWINKLE"), IxToken, Ownable {
    ...
    function getShareValue() external view override returns (uint256) {
        return totalSupply() > 0 ? (1e18 * token.balanceOf(address(this))) / totalSupply() : 1e18;
    }
    ...
    function depositWithPermit(uint256 _amount, Permit calldata permit) external override {
        IERC20Permit(address(token)).permit(permit.owner, permit.spender, permit.amount, permit.deadline, permit.v, permit.r, permit.s);
        deposit(_amount);
    }
