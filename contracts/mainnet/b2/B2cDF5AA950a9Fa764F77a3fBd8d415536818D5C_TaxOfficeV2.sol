...
contract TaxOfficeV2 is Operator {
    ...
    address public rabbit = address(0xe665B8185F697F215cD6cAc8c19FE9E1d849D6de);
    ...
    function _excludeAddressFromTax(address _address) private returns (bool) {
        if (!ITaxable(rabbit).isAddressExcluded(_address)) {
    ...
    function taxRate() external view returns (uint256) {
        return ITaxable(rabbit).taxRate();
    }
    ...
    {
        require(amtRabbit != 0 && amtToken != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);

        IERC20(rabbit).transferFrom(msg.sender, address(this), amtRabbit);
        IERC20(token).transferFrom(msg.sender, address(this), amtToken);
        _approveTokenIfNeeded(rabbit, uniRouter);
        _approveTokenIfNeeded(token, uniRouter);

        _includeAddressInTax(msg.sender);

        uint256 resultAmtRabbit;
        uint256 resultAmtToken;
        uint256 liquidity;
        (resultAmtRabbit, resultAmtToken, liquidity) = IUniswapV2Router(uniRouter).addLiquidity(
            rabbit,
            token,
            amtRabbit,
            amtToken,
        ...
        if (amtRabbit.sub(resultAmtRabbit) > 0) {
            IERC20(rabbit).transfer(msg.sender, amtRabbit.sub(resultAmtRabbit));
        }
        if (amtToken.sub(resultAmtToken) > 0) {
            IERC20(token).transfer(msg.sender, amtToken.sub(resultAmtToken));
        }
        return (resultAmtRabbit, resultAmtToken, liquidity);
    }
    ...
    {
        require(amtRabbit != 0 && msg.value != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);
        ...
        uint256 liquidity;
        (resultAmtRabbit, resultAmtFtm, liquidity) = IUniswapV2Router(uniRouter).addLiquidityETH{value: msg.value}(
            rabbit,
            amtRabbit,
            amtRabbitMin,
            amtFtmMin,
        ...
        return (resultAmtRabbit, resultAmtFtm, liquidity);
    }
    ...
    function taxFreeTransferFrom(
        ...
        IERC20(rabbit).transferFrom(_sender, _recipient, _amt);
