 ...
 */
contract TwoShareMasonry is IStrategy, Ownable, Pausable {
    ...
    address public want = address(0xc54A1684fD1bef1f077a336E6be4Bd9a3096a6Ca);
    ...
    address public masonry =
        address(0x627A83B6f8743c89d58F17F994D3F7f69c32F461);

    /**
     * @dev Controllers:
     * {locked} - Check if the fund in masonry still beeing locked
     * {initiateTimeLock} - ...
     */
    bool public locked;
     ...
     * @dev Routes:
     */
    address[] tombToWFTMRoute;
    address[] tombToTshareRoute;
    address[] wftmToTshareRoute;
    address[] customPath;
    ...
    constructor(address _vault, address _treasury) {
        ...
        tombToWFTMRoute = [tomb, wftm];
        tombToTshareRoute = [tomb, wftm, want];
        wftmToTshareRoute = [wftm, want];
     ...
     * 1. It claims reward
     * 2. It swap the reward to tshare
     * 4. It deposits {tshare} in the masonry to farm {tomb}
     * 5. Lock deposit
     */
     ...
     * It withdraw {tshare} from the masonry.
     * The available {tshare} minus fees is returned to the vault.
     * If the stratey is paused or called from owner, then withdraw to vault without fee
     */
    function withdraw(uint256 _amount) external override {
        require(msg.sender == vault, "!vault");
        require(!locked, "Withdraw locked");

        // Get the want token balance of this contract
        uint256 wantBal = IERC20(want).balanceOf((address(this)));
     ...
     * 3. It swaps the {rewardToken} token for {want}
     * 5. It deposits the new tshare.
     */
    ...
    /**
     * @dev Check to ensure that the correct time has elapsed and withdrawals are possible
     */
    function _checkEpoch() internal {
        if (
            IMasonry(masonry).canWithdraw(address(this)) &&
    ...
    /**
     * @dev Unlock the vault, allow user to withdraw
     * 1. Withdraw tshare and claim reward by calling withdraw function of masonry
     * 2. Charge fees
     * 3. Swap the left reward to tshare
     * 4. Unlock the vault
     */
    function _startUnlockWindow() internal {
        // withdraw all tshare and claim reward in the masonry
        if (IMasonry(masonry).balanceOf(address(this)) > 0) {
            IMasonry(masonry).withdraw(
                IMasonry(masonry).balanceOf(address(this))
            );
        }
        _chargeFees();
    ...
    /**
     * @dev Swaps {rewardToken} for {tshare} using SpookySwap.
     */
    ...
    /**
     * @dev It calculates how much {want} the strategy has allocated in the masonry.
     */
    ...
    function timeLockActionNeeded() public view returns (bool) {
        if (
            (unlockWindowRemaining() == 0 && !locked) ||
            (lockWindowRemaining() == 0 && locked)
        ) {
            return true;
        } else {
            return false;
    ...
    function _giveAllowances() internal {
        ...
        IERC20(want).safeApprove(masonry, max);
        IERC20(tomb).safeApprove(uniRouter, max);
        IERC20(want).safeApprove(uniRouter, 0);
        IERC20(want).safeApprove(uniRouter, max);
    }
