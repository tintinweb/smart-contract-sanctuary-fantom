...
contract RangoCBridge is IRangoCBridge, MessageSenderApp, MessageReceiverApp, BaseContract {
    ...
    function executeMessageWithTransferRefund(
        ...
        if (m.bridgeNativeOut) {
            _sendNative(m.originalSender, _amount);
        } else {
    ...
    function executeMessageWithTransfer(
        ...
        if (m.path.length > 1) {
            ...
            address sourceToken = m.bridgeNativeOut ? m.path[0] : NULL_ADDRESS;
            bool withdraw = m.bridgeNativeOut ? false : true;
            _sendToken(sourceToken, _amount, m.recipient, m.nativeOut, withdraw);
            dstAmount = _amount;
            status = OperationStatus.Succeeded;
