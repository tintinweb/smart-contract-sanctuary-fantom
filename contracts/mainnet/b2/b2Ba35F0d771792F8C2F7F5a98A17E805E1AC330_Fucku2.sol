...
contract Fucku2 is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the FUCKU2 ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Fucku2", "Fucku2") {
        // Mints 1 FUCKU2 to contract creator for initial pool setup
    ...
    function _getFucku2Price() internal view returns (uint256 _fucku2Price) {
        try IOracle(fucku2Oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Fucku2: failed to fetch FUCKU2 price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _fucku2Price) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_fucku2Price >= taxTiersTwaps[tierId]) {
    ...
    function setFucku2Oracle(address _fucku2Oracle) public onlyOperatorOrTaxOffice {
        require(_fucku2Oracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints FUCKU2 to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FUCKU2 to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFucku2Price = _getFucku2Price();
            currentTaxRate = _updateTaxRate(currentFucku2Price);
            if (currentFucku2Price < burnThreshold) {
                burnTax = true;
            }
        }
