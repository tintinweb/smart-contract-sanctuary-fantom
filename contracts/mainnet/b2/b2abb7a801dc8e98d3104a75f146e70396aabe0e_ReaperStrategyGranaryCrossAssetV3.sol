...
import "./interfaces/IFTMStaking.sol";
...
// Supply asset A, borrow asset B, compound rewards back into asset A
contract ReaperStrategyGranaryCrossAssetV3 is ReaperBaseStrategyv3 {
    ...
    IRewarder public constant REWARDER = IRewarder(0x7780E1A8321BD58BBc76594Db494c7Bfe8e87040);

    IERC20Upgradeable public supplyAsset;
    IERC20Upgradeable public borrowAsset;

    // Array containing supply aToken and borrow vToken for claiming rewards
    address[] public rewardClaimingTokens;

    uint256 public targetLtv; // in basis points
    uint256 public maxLtv; // in basis points
    uint256 public withdrawSlippageTolerance; // in basis points
    ...
    // V2 upgrade constants and variables
    IFTMStaking public constant STAKING = IFTMStaking(0xB458BfC855ab504a8a327720FcEF98886065529b);
    IWFTM public constant WFTM = IWFTM(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    bool public v2UpgradeCompleted;
    uint256 public unused_borrowAssetSupplied; // raw amount of borrowAsset that's been supplied back to the pool

    // V3 upgrade constants and variables
    IBeetVault public constant BEET_VAULT = IBeetVault(0x20dd72Ed959b6147912C2e529F0a0C651c33c9ce);
    bytes32 public constant sFTMX_POOL = 0xc0064b291bd3d4ba0e44ccfc81bf8e7f7a579cd200000000000000000000042c;
    bytes32 public constant WFTM_LINEAR_POOL = 0xc3bf643799237588b7a6b407b3fc028dd4e037d200000000000000000000022d;
    address public constant WFTM_LINEAR_BPT = 0xC3BF643799237588b7a6B407B3fc028Dd4e037d2;
    uint256 public constant MAX_SWAP_BUFFER_BPS = 200;
    uint256 public minSwapAmount;
    uint256 public swapBufferBPS;
    ...
    function initialize(
        ...
        IERC20Upgradeable _supplyAsset,
        IERC20Upgradeable _borrowAsset,
        uint256 _targetLtv,
        uint256 _maxLtv
    ) public initializer {
        __ReaperBaseStrategy_init(_vault, _feeRemitters, _strategists, _multisigRoles);
        supplyAsset = _supplyAsset;
        borrowAsset = _borrowAsset;

        (address aToken, , ) = DATA_PROVIDER.getReserveTokensAddresses(address(supplyAsset));
        (, , address vToken) = DATA_PROVIDER.getReserveTokensAddresses(address(borrowAsset));
        rewardClaimingTokens = [aToken, vToken];

        safeUpdateTargetLtv(_targetLtv, _maxLtv);
    ...
    function completeV2Upgrade() external {
        ...
        (address borrowAToken, , ) = DATA_PROVIDER.getReserveTokensAddresses(address(borrowAsset));
        rewardClaimingTokens.push(borrowAToken);
        unused_borrowAssetSupplied = 0;
        _borrowFromPool(0);
    }

    // Setter for params used to liquidate supplyAsset to borrowAsset for repaying
    function setSwapParams(uint256 _minSwapAmount, uint256 _swapBufferBPS) external {
        _atLeastRole(STRATEGIST);
        require(_swapBufferBPS <= MAX_SWAP_BUFFER_BPS);
        minSwapAmount = _minSwapAmount;
        swapBufferBPS = _swapBufferBPS;
    }
    ...
    function _swap(
        ...
        uint256 currentAllowance = IERC20Upgradeable(_from).allowance(address(this), address(BEET_VAULT));
        if (_amount > currentAllowance) {
            IERC20Upgradeable(_from).safeIncreaseAllowance(address(BEET_VAULT), _amount - currentAllowance);
        }
    ...
    function _withdraw(uint256 _amount) internal override {
        uint256 supplyAssetBal = supplyAsset.balanceOf(address(this));
        if (_amount > supplyAssetBal) {
            uint256 remaining = _amount - supplyAssetBal;
            _rebalance(remaining);
            _withdrawFromPool(remaining);
            supplyAssetBal = supplyAsset.balanceOf(address(this));
            require(
                supplyAssetBal >= (_amount * (PERCENT_DIVISOR - withdrawSlippageTolerance)) / PERCENT_DIVISOR,
                "Withdraw slippage exceeded!"
            );
            _amount = MathUpgradeable.min(supplyAssetBal, _amount);
        }

        supplyAsset.safeTransfer(vault, _amount);
    }
    ...
    function _addLiquidity() internal override {
        // harvest steps should have produced borrowAsset
        (, uint256 borrowAssetBorrowed, uint256 borrowAssetSupplied) = getSupplyAndBorrow();
        if (borrowAssetBorrowed > borrowAssetSupplied) {
            uint256 borrowAssetDeficit = borrowAssetBorrowed - borrowAssetSupplied;
            uint256 stratBorrowAssetBal = borrowAsset.balanceOf(address(this));
            _repayToPool(MathUpgradeable.min(borrowAssetDeficit, stratBorrowAssetBal));

            if (borrowAssetDeficit > stratBorrowAssetBal) {
                uint256 extraBorrowAssetNeeded = borrowAssetDeficit - stratBorrowAssetBal;
                (uint256 supplyAssetPrice, uint256 borrowAssetPrice) = getAssetPrices();
                uint256 extraBorrowAssetNeededInUSD = extraBorrowAssetNeeded * borrowAssetPrice;
                uint256 supplyAssetToLiquidate = extraBorrowAssetNeededInUSD / supplyAssetPrice;
                supplyAssetToLiquidate += (supplyAssetToLiquidate * swapBufferBPS) / PERCENT_DIVISOR;

                if (supplyAssetToLiquidate >= minSwapAmount) {
                    _withdrawFromPool(supplyAssetToLiquidate);
                    _swap(address(supplyAsset), WFTM_LINEAR_BPT, supplyAsset.balanceOf(address(this)), sFTMX_POOL);
                    _swap(
                        WFTM_LINEAR_BPT,
                        address(borrowAsset),
                        IERC20Upgradeable(WFTM_LINEAR_BPT).balanceOf(address(this)),
                        WFTM_LINEAR_POOL
                    );
                    stratBorrowAssetBal = borrowAsset.balanceOf(address(this));
                    _repayToPool(MathUpgradeable.min(extraBorrowAssetNeeded, stratBorrowAssetBal));
                    _borrowFromPool(0);
                }
        ...
        // make sFTMX with whatever wFTM is left
        _swap(address(borrowAsset), WFTM_LINEAR_BPT, borrowAsset.balanceOf(address(this)), WFTM_LINEAR_POOL);
        _swap(
            WFTM_LINEAR_BPT,
            address(supplyAsset),
            IERC20Upgradeable(WFTM_LINEAR_BPT).balanceOf(address(this)),
    ...
    /**
     * @dev Returns the total supplyAsset present in the strat and the lending pool.
     */
    function balanceOf() public view override returns (uint256) {
        (uint256 supplyAssetSupplied, , ) = getSupplyAndBorrow();
        return supplyAsset.balanceOf(address(this)) + supplyAssetSupplied;
    }

    /**
     * @dev Internal helper function to modify borrowed amount so that current LTV
     *      is in the desired range between targetLtv and maxLtv. Deducts
     *      _supplyAssetToFree from the actual supply so that this amount
     *      can be safely withdrawn once this function returns.
     */
    function _rebalance(uint256 _supplyAssetToFree) internal {
        (uint256 supplyAssetSupplied, uint256 borrowAssetBorrowed, ) = getSupplyAndBorrow();
        _supplyAssetToFree = MathUpgradeable.min(_supplyAssetToFree, supplyAssetSupplied);
        supplyAssetSupplied -= _supplyAssetToFree;

        (uint256 supplyAssetPrice, uint256 borrowAssetPrice) = getAssetPrices();
        uint256 supplyInUSD = supplyAssetSupplied * supplyAssetPrice;
        uint256 borrowInUSD = borrowAssetBorrowed * borrowAssetPrice;

        uint256 ltv;
        if (borrowInUSD == 0) ltv = 0;
        else if (supplyInUSD == 0) ltv = type(uint256).max;
        else ltv = (borrowInUSD * PERCENT_DIVISOR) / supplyInUSD;

        uint256 desiredBorrowInUSD = (targetLtv * supplyInUSD) / PERCENT_DIVISOR;
        uint256 desiredBorrow = desiredBorrowInUSD / borrowAssetPrice;
        if (ltv > maxLtv) {
            _repayToPool(borrowAssetBorrowed - desiredBorrow);
        } else if (ltv < targetLtv) {
            _borrowFromPool(desiredBorrow - borrowAssetBorrowed);
        }
    }
     ...
     */
    function _supplyToPool() internal {
        uint256 supplyAssetBal = supplyAsset.balanceOf(address(this));
        if (supplyAssetBal != 0) {
            ILendingPool pool = ILendingPool(ADDRESSES_PROVIDER.getLendingPool());
            supplyAsset.safeIncreaseAllowance(address(pool), supplyAssetBal);
            pool.deposit(address(supplyAsset), supplyAssetBal, address(this), LENDER_REFERRAL_CODE_NONE);
        }
    }

    /**
     * @dev Internal helper function to withdraw specified amount of supplyAsset from pool.
     */
    function _withdrawFromPool(uint256 _supplyAssetToWithdraw) internal {
        (uint256 supplyAssetSupplied, uint256 borrowAssetBorrowed, ) = getSupplyAndBorrow();
        (uint256 supplyAssetPrice, uint256 borrowAssetPrice) = getAssetPrices();
        uint256 borrowInUSD = borrowAssetBorrowed * borrowAssetPrice;

        uint256 necessarySupplyInUSD = maxLtv != 0 ? (borrowInUSD * PERCENT_DIVISOR) / maxLtv : 0;
        uint256 necessarySupply = necessarySupplyInUSD / supplyAssetPrice;
        require(supplyAssetSupplied >= necessarySupply, "Can't withdraw anything");

        uint256 withdrawable = supplyAssetSupplied - necessarySupply;
        _supplyAssetToWithdraw = MathUpgradeable.min(withdrawable, _supplyAssetToWithdraw);
        if (_supplyAssetToWithdraw != 0) {
            ILendingPool pool = ILendingPool(ADDRESSES_PROVIDER.getLendingPool());
            pool.withdraw(address(supplyAsset), _supplyAssetToWithdraw, address(this));
        }
    }

    /**
     * @dev Internal helper function to repay specified amount of borrowAsset.
     */
    function _repayToPool(uint256 _borrowAssetToRepay) internal {
        uint256 stratBorrowAssetBal = borrowAsset.balanceOf(address(this));
        (, , uint256 borrowAssetSupplied) = getSupplyAndBorrow();
        uint256 totalBorrowAssetAvailable = stratBorrowAssetBal + borrowAssetSupplied;
        _borrowAssetToRepay = MathUpgradeable.min(_borrowAssetToRepay, totalBorrowAssetAvailable);

        ILendingPool pool = ILendingPool(ADDRESSES_PROVIDER.getLendingPool());
        if (_borrowAssetToRepay > stratBorrowAssetBal) {
            uint256 remainingBorrowAssetToWithdraw = _borrowAssetToRepay - stratBorrowAssetBal;
            pool.withdraw(address(borrowAsset), remainingBorrowAssetToWithdraw, address(this));
            stratBorrowAssetBal = borrowAsset.balanceOf(address(this));
            _borrowAssetToRepay = MathUpgradeable.min(_borrowAssetToRepay, stratBorrowAssetBal);
        }

        if (_borrowAssetToRepay != 0) {
            borrowAsset.safeIncreaseAllowance(address(pool), _borrowAssetToRepay);
            pool.repay(address(borrowAsset), _borrowAssetToRepay, INTEREST_RATE_MODE_VARIABLE, address(this));
        }
    }

    /**
     * @dev Internal helper function to borrow specified amount of borrowAsset.
     */
    function _borrowFromPool(uint256 _borrowAssetToBorrow) internal {
        ILendingPool pool = ILendingPool(ADDRESSES_PROVIDER.getLendingPool());
        if (_borrowAssetToBorrow != 0) {
            pool.borrow(
                address(borrowAsset),
                _borrowAssetToBorrow,
        ...
        // re-supply whatever we may have just borrowed
        uint256 borrowAssetBal = borrowAsset.balanceOf(address(this));
        if (borrowAssetBal != 0) {
            borrowAsset.safeIncreaseAllowance(address(pool), borrowAssetBal);
            pool.deposit(address(borrowAsset), borrowAssetBal, address(this), LENDER_REFERRAL_CODE_NONE);
        }
    }

    /**
     * @notice Returns the current supply and borrow state of this contract as
     *         reported by the lending pool. Note that supply and borrow are denominated
    ...
    function getSupplyAndBorrow()
        ...
        view
        returns (
            uint256 supplyAssetSupplied,
            uint256 borrowAssetBorrowed,
            uint256 borrowAssetSupplied
        )
    {
        (supplyAssetSupplied, , , , , , , , ) = DATA_PROVIDER.getUserReserveData(address(supplyAsset), address(this));
        (borrowAssetSupplied, , borrowAssetBorrowed, , , , , , ) = DATA_PROVIDER.getUserReserveData(
            address(borrowAsset),
            address(this)
        );
    }
    ...
    /**
     * @notice Returns the current asset prices for supplyAsset and borrowAsset
     *         as reported by Granary's oracle.
     */
    function getAssetPrices() public view returns (uint256 supplyAssetPrice, uint256 borrowAssetPrice) {
        IAaveOracle oracle = IAaveOracle(ADDRESSES_PROVIDER.getPriceOracle());
        address[] memory assets = new address[](2);
        assets[0] = address(supplyAsset);
        assets[1] = address(borrowAsset);
     ...
     */
    function safeUpdateTargetLtv(uint256 _targetLtv, uint256 _maxLtv) public {
        _atLeastRole(KEEPER);
        (, uint256 ltv, , , , , , , , ) = DATA_PROVIDER.getReserveConfigurationData(address(supplyAsset));
        require(_maxLtv <= (ltv * LTV_SAFETY_ZONE) / PERCENT_DIVISOR, "maxLtv not safe");
        require(_targetLtv <= _maxLtv, "targetLtv must <= maxLtv");
        targetLtv = _targetLtv;
        maxLtv = _maxLtv;
    }

    /**
     * @dev Updates slippage tolerance (when withdrawing).
     *      Can only be called by strategist or higher roles.
     */
    function setWithdrawSlippageTolerance(uint256 _newWithdrawSlippageTolerance) external {
        _atLeastRole(STRATEGIST);
    ...
    /**
     * @dev Frees up specified amount of supplyAsset by manipulating borrow.
     */
    function authorizedRebalance(uint256 _supplyAssetToFree) external {
        _atLeastRole(STRATEGIST);
        _rebalance(_supplyAssetToFree);
    }

    /**
     * @dev Attempts to safely withdraw specified amount of supplyAsset from the pool.
     */
    function authorizedWithdrawFromPool(uint256 _supplyAssetToWithdraw) external {
        _atLeastRole(STRATEGIST);
        _withdrawFromPool(_supplyAssetToWithdraw);
    }

    /**
     * @dev Attempts to send specified amount of supplyAsset that's sitting in the strategy
     *      back to the vault.
     */
    ...
    function _reclaimWant() internal override {
        _rebalance(type(uint256).max);
        _withdrawFromPool(type(uint256).max);
    }

    /**
     * Receive function to be able to receive native FTM from the WFTM contract.
     * Verifies that sender must be the WFTM contract.
     */
    ...
    PausableUpgradeable
{
    ...
    function _swap(uint256 amount, address[] storage path) internal {
        if (amount != 0) {
            IERC20Upgradeable(path[0]).safeIncreaseAllowance(address(SPOOKY_ROUTER), amount);
...
interface IRewarder {
    ...
    function setRewardsVault(address vault, address reward) external;
