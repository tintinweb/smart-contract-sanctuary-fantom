...
contract RainSpiritChef is BoringOwnable, BoringBatchable {
    using SafeMath for uint256;
    using BoringMath128 for uint128;
    using BoringERC20 for IERC20Permit;
    ...
    IERC20Permit public SPIRIT;

    /// @notice Info of each MCV2 pool.
    PoolInfo[] public poolInfo;
    /// @notice Address of the LP token for each MCV2 pool.
    IERC20Permit[] public lpToken;
    ...
    event LogPoolAddition(uint256 indexed pid, uint256 allocPoint, IERC20Permit indexed lpToken);
    ...
    constructor(IERC20Permit _spirit) public {
        SPIRIT = _spirit;
    ...
    function add(uint256 allocPoint, IERC20Permit _lpToken) public onlyOwner {
...
interface IMasterChef {
    ...
    struct PoolInfo {
        IERC20Permit lpToken;           // Address of LP token contract.
...
library BoringERC20 {
    function safeSymbol(IERC20Permit token) internal view returns(string memory) {
        (bool success, bytes memory data) = address(token).staticcall(abi.encodeWithSelector(0x95d89b41));
        return success && data.length > 0 ? abi.decode(data, (string)) : "???";
    }

    function safeName(IERC20Permit token) internal view returns(string memory) {
        (bool success, bytes memory data) = address(token).staticcall(abi.encodeWithSelector(0x06fdde03));
        return success && data.length > 0 ? abi.decode(data, (string)) : "???";
    }

    function safeDecimals(IERC20Permit token) internal view returns (uint8) {
        (bool success, bytes memory data) = address(token).staticcall(abi.encodeWithSelector(0x313ce567));
        return success && data.length == 32 ? abi.decode(data, (uint8)) : 18;
    }

    function safeTransfer(IERC20Permit token, address to, uint256 amount) internal {
        (bool success, bytes memory data) = address(token).call(abi.encodeWithSelector(0xa9059cbb, to, amount));
        require(success && (data.length == 0 || abi.decode(data, (bool))), "BoringERC20: Transfer failed");
    }

    function safeTransferFrom(IERC20Permit token, address from, address to, uint256 amount) internal {
