...
contract StrategyBeethovenxDualTUSD is StratManager, FeeManager {
    ...
    bytes32 public usdcToNativePoolId = 0xcf9d4940fe4c194c83d4d3b1de4c2dff4233f612000200000000000000000253;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 rewardBal = IERC20(reward).balanceOf(address(this));
        if (rewardBal > 0) {
            //No TUSD -> FTM pool available so have to swap TUSD into USDC
            balancerSwap(rewardSwapPoolId, reward, native, rewardBal);

            // Then swap USDC into FTM
            uint256 usdcBal = IERC20(usdc).balanceOf(address(this));
            balancerSwap(usdcToNativePoolId, usdc, native, usdcBal);
