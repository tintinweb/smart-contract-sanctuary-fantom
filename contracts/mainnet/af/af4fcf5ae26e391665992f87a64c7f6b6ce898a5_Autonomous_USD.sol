...
function _buyFixing(uint amount) private {
    ...
    function periodInflation(uint _inflationRate, uint _periodThreshold, uint _upgradingPeriod, uint _update) external onlyManager{
    ...
    function _upgradeAppreciation(uint _upgrade) internal onlyManager{
   ...
   function upgradeAppreciation(uint _update, bool on) external onlyManager{
        if(on == true){
           _upgradeAppreciation(_update);
        }else{
            appreciationMode = false;
