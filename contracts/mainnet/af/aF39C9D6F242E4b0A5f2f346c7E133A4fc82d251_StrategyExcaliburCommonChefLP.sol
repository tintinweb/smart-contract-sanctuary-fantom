...
contract StrategyExcaliburCommonChefLP is StratManager, FeeManager, GasThrottler {
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        IExcaliburRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toNative, 0, outputToNativeRoute, address(this), beefyFeeRecipient, now);
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IExcaliburRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(outputHalf, 0, outputToLp0Route, address(this), beefyFeeRecipient, now);
        }

        if (lpToken1 != output) {
            IExcaliburRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(outputHalf, 0, outputToLp1Route, address(this), beefyFeeRecipient, now);
        }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            uint256[] memory amountOut = IExcaliburRouter(unirouter).getAmountsOut(IExcaliburRouter(unirouter).factory(), outputBal, outputToNativeRoute);
...
interface IUniswapV2Router01 {
  ...
  function getAmountsOut(address _factory, uint amountIn, address[] memory path) external view returns (uint[] memory amounts);
}
