...
contract IceFusdtEqualizerV2 is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToIcePath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToIcePath,
        ...
        for (uint i; i < _equalToIcePath.length; ++i) {
            equalToIcePath.push(_equalToIcePath[i]);
        }
    ...
    function addLiquidity() internal {
        uint256 equalBal = IERC20(equal).balanceOf(address(this)) / 2;
        IEqualizerRouter(router).swapExactTokensForTokens(equalBal, 0, equalToIcePath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokens(equalBal, 0, equalToFusdtPath, address(this), block.timestamp);
