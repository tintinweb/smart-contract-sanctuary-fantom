...
interface IMasterChef {
    ...
    function claimableRewards(address _user) external view returns (address[] memory, uint256);
    function withdrawAll() external;
}

pragma solidity 0.6.2;

contract ShadeSingleStake is Ownable, Pausable {
    ...
    function callReward() public view returns (uint256) {
        ( ,uint256 outputBal) = IMasterChef(masterchef).claimableRewards(address(this));
        uint256 reward;
        if (output != wrapped) {
    ...
    function addLiquidity() internal {
        ...
        if (want != wrapped) {
            IUniswapRouter(liquidityRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedBal, 0, wrappedToWantRoute, address(this), now.add(600));
        }
    }

    /** @dev Sets the swap route of the strategy contract after deployment in the event of liquidity issues */
    function setWrappedToWantRoute(address[] calldata _route, address router) external onlyOwner {
    require(_route[0] == wrapped && _route[_route.length - 1] == want, "!auth || bad path");
    ...
    function setOutputToWrappedRoute(address[] calldata _route, address router) external onlyOwner {
        require(_route[0] == output && _route[_route.length - 1] == wrapped, "!auth || bad path");
    ...
    function balanceOfPool() public view returns (uint256) {
        uint256 _amount = IMasterChef(masterchef).balances(address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IMasterChef(masterchef).withdrawAll();
    ...
    function setFees(uint256 newCallFee, uint256 newStratFee, uint256 newWithdrawFee, uint256 newBuyBackFee, uint256 newInsuranceFeeAmount, uint256 newTreasuryFee) external onlyOwner {
