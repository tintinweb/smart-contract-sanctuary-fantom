...
interface IGaugeForBoostedFarmLens {
    ...
    function rewardRate() external view returns (uint256); // reward per second
...
contract BoostedFarmLens {
    ...
    function viewBoosterAPR(IBoosterForBoostedFarmLens booster) external view returns (BoostedFarmAPRView memory) {
        return viewBoosterAPRInternal(IGaugeForBoostedFarmLens(booster.GAUGE()));
    }
    ...
    function viewBoosterAPRInternal(IGaugeForBoostedFarmLens gauge) internal view returns (BoostedFarmAPRView memory) {
        ...
        boosterAPR.lpSupplyInGauge = gauge.totalSupply();
