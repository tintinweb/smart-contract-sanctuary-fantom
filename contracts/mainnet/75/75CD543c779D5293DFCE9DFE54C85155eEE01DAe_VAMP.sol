...
contract VAMP is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 VAMP-WETH LP -> VAMP pool
    // uint256 public constant INITIAL_VAMP_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the VAMP ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("VAMP", "VAMP") {
        // Mints 1 VAMP to contract creator for initial pool setup
    ...
    function _getVAMPPrice() internal view returns (uint256 _VAMPPrice) {
        try IOracle(VAMPOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("VAMP: failed to fetch VAMP price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _VAMPPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_VAMPPrice >= taxTiersTwaps[tierId]) {
    ...
    function setVAMPOracle(address _VAMPOracle) public onlyOperatorOrTaxOffice {
        require(_VAMPOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints VAMP to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of VAMP to mint to
    ...
    function _transfer(
        ...
        if (autoCalculateTax) {
            uint256 currentVAMPPrice = _getVAMPPrice();
            currentTaxRate = _updateTaxRate(currentVAMPPrice);
            if (currentVAMPPrice < burnThreshold) {
                burnTax = true;
            }
        }  
