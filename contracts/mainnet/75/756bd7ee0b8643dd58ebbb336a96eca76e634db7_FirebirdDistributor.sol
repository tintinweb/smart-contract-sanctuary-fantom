...
contract FirebirdDistributor is OwnableUpgradeable, ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => mapping(IERC20 => uint256)))
    ...
    event PhaseCreated(uint256 indexed id, bytes32 root);
    ...
    function claim(
        ...
        // verify if can claim
        require(
            isValidClaim(id, index, user, tokens, amounts, merkleProof),
            "invalid claim data"
        );
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            uint256 claimable = amounts[i] -
                claimedAmounts[user][id][tokens[i]];
    ...
    function proposeDistribution(
        ...
        distributionInfo[_phaseId] = Distribution(root);
        emit PhaseCreated(_phaseId, root);
    }
    ...
    function encodeClaim(
        ...
        external
        pure
        returns (bytes memory encodedData, bytes32 encodedDataHash)
    {
        require(
