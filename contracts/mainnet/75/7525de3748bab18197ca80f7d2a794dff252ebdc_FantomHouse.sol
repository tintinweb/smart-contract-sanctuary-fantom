...
contract FantomHouse is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    function addDepositorsForTesting(uint256 _number, address _address) public {
        for (uint256 i = 0; i < _number; i++) {
//            address _testAddress = address(keccak256(abi.encodePacked(block.timestamp)));
            address _testAddress = address(uint160(uint(keccak256(abi.encodePacked(nonce, blockhash(block.number))))));
            nonce++;
            depositors.push(_testAddress);
            userHouseBalances[_testAddress].depositAmount = 100;
        }
    }

    function removeDepositorsForTesting(uint256 _number) public {
        for (uint256 i = 0; i < _number; i++) {
            depositors.pop();
        }
    }
