...
contract FantomonFeeding is Ownable {
    ...
    function feed(uint256 _tokenId, ERC20 _food, uint8 _mode, uint48 _amountNoDecimals, bool _force) external {
        ...
        if (foodBurnable_[_food]) {
            ERC20Burnable(address(_food)).burnFrom(msg.sender, _amountNoDecimals * _food.decimals());
        } else {
            _food.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, _amountNoDecimals * _food.decimals());
        }
    }
