...
// File: contracts/facades/BabyCreatureFacade.sol



pragma solidity ^0.8.14;

contract BabyCreatureFacade is _BabyCreatureStorage {
    ...
    function getAllApproval(address owner, address operator)
        external
        view
        returns (bool approvedAllOld, bool approvedAllNew)
    {
        approvedAllOld = _babyCreatureOld.isApprovedForAll(
            owner,
            address(operator)
        );
        approvedAllNew = _babyCreatureNew.isApprovedForAll(
            owner,
            address(operator)
        );
    }

    function getBabiesOf(
        address owner,
        uint256 from,
        uint256 to
    )
        ...
        view
        returns (
            string[] memory babies,
            uint256 last,
            uint256 tokenIdShift
        )
    {
        uint256 balanceOfOld = _babyCreatureOld.balanceOf(owner);
        uint256 balanceOfNew = _babyCreatureNew.balanceOf(owner);
        last = balanceOfOld + balanceOfNew;
        tokenIdShift = _tokenIdShift;

        if (from < last) {
            babies = new string[](to < last ? to - from : last - from);
            uint256 index = 0;
            for (uint256 i = from; i < (to < last ? to : last); i++) {
                if (i < balanceOfOld) {
                    babies[index] = _babyCreatureOld.tokenURI(
                        _babyCreatureOld.tokenOfOwnerByIndex(owner, i)
                    );
                } else {
                    babies[index] = _babyCreatureNew.tokenURI(
                        _babyCreatureNew.tokenOfOwnerByIndex(
                            owner,
                            i - balanceOfOld
                        )
                    );
    ...
    function getLastBabiesOf(address owner, uint256 last)
        external
        view
        returns (string[] memory babies, uint256 balance)
    {
        uint256 balanceOfOld = _babyCreatureOld.balanceOf(owner);
        uint256 balanceOfNew = _babyCreatureNew.balanceOf(owner);
        balance = balanceOfOld + balanceOfNew;

        babies = new string[](balance < last ? balance : last);
        uint256 index = 0;
        uint256 from = balance < last ? 0 : balance - last;
    ...
    function getApprovalOf(uint256[] calldata tokenId, address operator)
        external
        view
        returns (bool[] memory approved)
    {
        approved = new bool[](tokenId.length);
        for (uint256 i = 0; i < tokenId.length; i++) {
            if (tokenId[i] < _tokenIdShift) {
                approved[i] =
                    address(operator) ==
                    _babyCreatureOld.getApproved(tokenId[i]);
            } else {
                approved[i] =
                    address(operator) ==
                    _babyCreatureNew.getApproved(tokenId[i]);
            }
        }
    }
}