...
import "../../nft/NFTERC1155Upgradeable.sol";
import "./interfaces/ICurrenciesNFT.sol";

/**
 * @title CurrenciesNFT
 * @notice Fungible currency (item) such as materials, key items, etc
 */
contract CurrenciesNFT is Initializable, UUPSUpgradeable, NFTERC1155Upgradeable, ICurrenciesNFT {
    mapping(uint256 => CurrenciesNFTLib.CurrencyAttributes) private _currenciesAttributes;
    ...
    function initialize() public initializer {
        __CurrenciesNFT_init();
    }

    function __CurrenciesNFT_init() internal onlyInitializing {
        ...
        __NFTERC1155_init_unchained("Runic Currencies", "RUNIC CURRENCIES");
        __CurrenciesNFT_init_unchained();
    }

    function __CurrenciesNFT_init_unchained() internal onlyInitializing {}
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        ...
        interfaceId == type(ICurrenciesNFT).interfaceId;
    }
    ...
    function getCurrencyAttributes(uint256 tokenId) external view override returns (CurrenciesNFTLib.CurrencyAttributes memory) {
        return _currenciesAttributes[tokenId];
    }

    /**
     * @notice Sets all attributes of a currency
     */
    function setCurrencyAttributes(uint256 tokenId, CurrenciesNFTLib.CurrencyAttributes calldata attributes_) external override onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        _currenciesAttributes[tokenId] = attributes_;
    }

    /**
     * @notice Easy add function for new currency with max supply of type(uint256).max
     */
    function setCurrencyAttributes(uint256 tokenId, uint32 categoryId, uint32 rarityId, uint32 elementId, uint32 value, uint32 valuePc, uint256 valueId, string calldata name, string calldata description) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        _currenciesAttributes[tokenId].categoryId = categoryId;
        _currenciesAttributes[tokenId].rarityId = rarityId;
        _currenciesAttributes[tokenId].elementId = elementId;
        _currenciesAttributes[tokenId].value = value;
        _currenciesAttributes[tokenId].valuePc = valuePc;
        _currenciesAttributes[tokenId].valueId = valueId;
        _currenciesAttributes[tokenId].name = name;
        _currenciesAttributes[tokenId].description = description;
        _currenciesAttributes[tokenId].maxSupply = type(uint256).max;
    }

    /**
     * @notice Operator burns without approval
     */
    function operatorBurn(address account, uint256 tokenId, uint256 amount) external override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
     ...
     * @notice Operator burns without approval
     */
    function operatorBurnBatch(address account, uint256[] memory ids, uint256[] memory values) external onlyRole(AccessControlConstants.OPERATOR_ROLE) {
...
abstract contract ERC1155EnumerableUpgradeable is ERC1155Upgradeable, IERC1155Enumerable {
     ...
     */
    function _updateAccountsTokens(address from, address to, uint256[] memory ids) internal virtual {
     ...
     */
    function getAccountTokensCount(address account) public view virtual override returns (uint256) {
     ...
     */
    function getAccountTokensByIndex(address account, uint256 index) public view virtual override returns (uint256) {
     ...
     */
    function getAccountTokensPaginated(address account, uint256 cursor, uint256 limit) public view virtual override returns (uint256[] memory tokenIds, uint256[] memory amounts, uint256 nextCursor) {
...
library NFTConstants {
    ...
    uint256 public constant UINT_VERSION = 1;
     ...
     * Uint Mapping IDs
     */
    uint256 public constant UINT_MAPPING_ROYALTY_AMOUNT = 10;   // royalties amount / 10000. ex: 1 = 0.01%
}
...
interface INFTTotalSupplyERC1155 {
...
abstract contract NFTUpgradeable is Initializable, AccessControlEnumerableUpgradeable, INFTAttributes {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using StringsUpgradeable for uint256;
    CountersUpgradeable.Counter internal _tokenIdCounter;                        // id
    ...
    function __NFT_init_unchained() internal onlyInitializing {
        ...
        AccessControlUpgradeable._grantRole(AccessControlConstants.ADMIN_ROLE, msg.sender);
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  super.supportsInterface(interfaceId) ||
        interfaceId == type(INFTAttributes).interfaceId;
    }

    function setBaseURI(string calldata uri_) external virtual onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        baseURI = uri_;
    }

    /**
     * @notice Set specific attribute
     */
    function setAttribute(uint256 id_, uint256 attributeId_, uint256 attributeValue_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
     ...
     * @notice Set specific attribute
     */
    function setAttributeString(uint256 id_, uint256 attributeId_, string calldata attributeValue_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _attributesString[id_][attributeId_] = attributeValue_;
    }

    /**
     * @notice Set specific attributeAddress
     */
    function setAttributeAddress(uint256 id_, uint256 attributeId_, address attributeAddressValue_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _attributesAddress[id_][attributeId_] = attributeAddressValue_;
    }

    /**
     * @notice Set specific attributeArray
     */
    function setAttributeArray(uint256 id_, uint256 attributeId_, uint256[] calldata attributeArrayValue_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _attributesArray[id_][attributeId_] = attributeArrayValue_;
    }

    /**
     * @notice Set specific attributeBytes
     */
    function setAttributeBytes(uint256 id_, uint256 attributeId_, bytes32[] calldata attributeBytesValue_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _attributesBytes[id_][attributeId_] = attributeBytesValue_;
    }

    /**
     * @notice Set specific data for addresses
     */
    function setTokenAddressData(uint256 id_, address address_, uint256 data_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _tokenAddressData[id_][address_] = data_;
    }

    /**
     * @notice Set specific data for addresses of id
     */
    function setAddressData(uint256 id_, address address_, uint256 data_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _addressData[id_][address_] = data_;
    }

    /**
     * @notice Set specific addresses of id such as contract addresses
     */
    function setAddressMapping(uint256 id_, address address_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _addressMapping[id_] = address_;
    }

    /**
     * @notice Set specific uint of id such as royalty amount or chainId
     */
    function setUintMapping(uint256 id_, uint256 value_) external virtual override onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        _uintMapping[id_] = value_;
    }

    function getAttribute(uint256 id_, uint256 attributeId_) external virtual override view returns(uint256) {
        return _attributes[id_][attributeId_];
    }

    function getAttributeString(uint256 id_, uint256 attributeId_) external virtual override view returns(string memory) {
        return _attributesString[id_][attributeId_];
    }

    function getAttributeAddress(uint256 id_, uint256 attributeId_) external virtual override view returns(address) {
        return _attributesAddress[id_][attributeId_];
    }

    function getAttributeArray(uint256 id_, uint256 attributeId_) external virtual override view returns(uint256[] memory) {
        return _attributesArray[id_][attributeId_];
    }

    function getAttributeBytes(uint256 id_, uint256 attributeId_) external virtual override view returns(bytes32[] memory) {
        return _attributesBytes[id_][attributeId_];
    }

    function getTokenAddressData(uint256 id_, address address_) external virtual override view returns(uint256) {
        return _tokenAddressData[id_][address_];
    }

    function getAddressData(uint256 id_, address address_) external virtual override view returns(uint256) {
        return _addressData[id_][address_];
    }

    function getAddressMapping(uint256 id_) external virtual override view returns(address) {
        return _addressMapping[id_];
    }

    function getUintMapping(uint256 id_) external virtual override view returns(uint256) {
...
import "../token/ERC1155/extensions/IERC1155Burnable.sol";
import "../token/ERC1155/extensions/IERC1155Supply.sol";
import "../token/ERC1155/extensions/ERC1155EnumerableUpgradeable.sol";
import "../token/ERC2981/IERC2981.sol";
import "./interfaces/INFTMintableERC1155.sol";
import "./interfaces/INFTTotalSupplyERC1155.sol";
import "./lib/NFTConstants.sol";
import "./NFTUpgradeable.sol";

abstract contract NFTERC1155Upgradeable is Initializable, NFTUpgradeable, IERC1155Burnable, ERC1155BurnableUpgradeable, IERC1155Supply, ERC1155SupplyUpgradeable, ERC1155EnumerableUpgradeable, IERC2981, INFTMintableERC1155, INFTTotalSupplyERC1155 {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using StringsUpgradeable for uint256;
    string internal _name;
    string internal _symbol;

    function __NFTERC1155_init(string memory name_, string memory symbol_) internal onlyInitializing {
        __Context_init_unchained();
        __ERC165_init_unchained();
        __ERC1155_init_unchained("");
        __ERC1155Burnable_init_unchained();
        ...
        __AccessControlEnumerable_init_unchained();
        __NFT_init_unchained();
        __NFTERC1155_init_unchained(name_, symbol_);
    }

    function __NFTERC1155_init_unchained(string memory name_, string memory symbol_) internal onlyInitializing {
        _uintMapping[NFTConstants.UINT_MAPPING_ROYALTY_AMOUNT] = 500;
        _addressMapping[NFTConstants.ADDR_MAPPING_ROYALTY_RECEIVER] = msg.sender;
        _name = name_;
        _symbol = symbol_;
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(NFTUpgradeable, ERC1155Upgradeable, ERC1155EnumerableUpgradeable) returns (bool) {
        return super.supportsInterface(interfaceId) ||
        interfaceId == type(IERC1155Burnable).interfaceId ||
        interfaceId == type(IERC1155Supply).interfaceId ||
        interfaceId == type(INFTTotalSupplyERC1155).interfaceId ||
        interfaceId == type(IERC2981).interfaceId ||
        interfaceId == type(INFTMintableERC1155).interfaceId;
    }
    ...
    function setName(string calldata name_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        _name = name_;
    }

    function setSymbol(string calldata symbol_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        _symbol = symbol_;
    }
    ...
    function setTokenURI(uint256 tokenId_, string calldata tokenURI_) external onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        _tokenURIs[tokenId_] = tokenURI_;
        emit URI(tokenURI_, tokenId_);
    }

    function totalSupply(uint256 id) public view virtual override(IERC1155Supply, ERC1155SupplyUpgradeable) returns(uint256) {
        return super.totalSupply(id);
    }

    function exists(uint256 id) public view virtual override(IERC1155Supply, ERC1155SupplyUpgradeable) returns(bool) {
    ...
    function _afterTokenTransfer(address operator, address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal virtual override(ERC1155Upgradeable, ERC1155EnumerableUpgradeable) {
        super._afterTokenTransfer(operator, from, to, ids, amounts, data);
    }

    function burn(address account, uint256 tokenId, uint256 amount) public override(IERC1155Burnable, ERC1155BurnableUpgradeable) {
        super.burn(account, tokenId, amount);
    }

    function burnBatch(address account, uint256[] memory ids, uint256[] memory values) public override(IERC1155Burnable, ERC1155BurnableUpgradeable) {
        super.burnBatch(account, ids, values);
    }

    function setBaseURI(string calldata uri_) external override onlyRole(AccessControlConstants.DEFAULT_ADMIN_ROLE) {
        baseURI = uri_;
        _setURI(baseURI);
    }

    /**
     * @notice Mint Fungible token
     */
    ...
    /**
     * @notice Mint Non-fungible token. Shouldn't be used if everything is currency
     */
    function mintNFT(address to_) external override onlyRole(AccessControlConstants.MINTER_ROLE) returns(uint256) {
        addToken();
        uint256 id_ = _tokenIdCounter.current() - 1; // addToken incremented it already
        _mint(to_, id_, 1, "");
        return id_;
    }
    ...
    function royaltyInfo(uint256, uint256 salePrice_) external override view returns (address receiver, uint256 royaltyAmount) {
        uint256 royaltyOwed_ = (salePrice_ * _uintMapping[NFTConstants.UINT_MAPPING_ROYALTY_AMOUNT]) / 1000;
        return(_addressMapping[NFTConstants.ADDR_MAPPING_ROYALTY_RECEIVER], royaltyOwed_);
    }

    /**
     * @notice Add token without minting
     */
    function addToken() public virtual onlyRole(AccessControlConstants.MINTER_ROLE) {
        uint256 id_ = _tokenIdCounter.current();
        _tokenIdCounter.increment();

        // set dbId to same as id
        _attributes[id_][NFTConstants.UINT_DBID] = id_;
    }

    /**
     * @notice Set token. Doesn't increment tokenId
     */
    function setToken(uint256 id_) external virtual onlyRole(AccessControlConstants.OPERATOR_ROLE) {
        // set dbId to same as id
        _attributes[id_][NFTConstants.UINT_DBID] = id_;
    }
