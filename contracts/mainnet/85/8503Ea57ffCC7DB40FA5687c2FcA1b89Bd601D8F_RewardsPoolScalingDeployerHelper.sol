...
import "../rock/interfaces/IOxLens.sol";
import "../rock/interfaces/IRewardsDistributor.sol";
import "../rock/interfaces/IPartnersRewards.sol";
...
/**
 * @title Helper contract meant to be delegate-called by deployer to link and initiate upgrades atomically
 * @author 0xDAO
 */

contract RewardsPoolScalingDeployerHelper is OxDAODeployer {
    function updateProtocol() external {
        IOxLens oxLens = IOxLens(
            address(0xDA00137c79B30bfE06d04733349d98Cf06320e69)
        );
        address rewardsDistributorAddress = oxLens.rewardsDistributorAddress();

        address tokensAllowlistAddress = oxLens.tokensAllowlistAddress();

        address partnersRewardsAddress = oxLens.partnersRewardsPoolAddress();
        address oxSolidRewardsAddress = oxLens.oxSolidRewardsPoolAddress();
        address oxdV1RewardsAddress = oxLens.oxdV1RewardsAddress();

        address oxdAddress = oxLens.oxdAddress();
        address oxSolidAddress = oxLens.oxSolidAddress();
        address solidAddress = oxLens.solidAddress();
        address wftmAddress = address(
            0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83
        );

        //link proxy to new implementation
        IProxy(tokensAllowlistAddress).updateImplementationAddress(
            address(0xa80b1ceaA252e3E1BCaEba9d6B537FE01b226Fd2)
        );

        IProxy(rewardsDistributorAddress).updateImplementationAddress(
            address(0x54D712cFacaeAb1Be6b8D3Fb7548d6AC346C3c5e)
        );

        IProxy(partnersRewardsAddress).updateImplementationAddress(
            address(0x60fa02b28bf7f71031B0F0adcd0CB00e643D543C)
        );
        IProxy(oxSolidRewardsAddress).updateImplementationAddress(
            address(0xB600C9be12C2487140d603C63506a462a6feaC17)
        );
        IProxy(oxdV1RewardsAddress).updateImplementationAddress(
            address(0xF71aD7Bee5798500395E550dBe3EbFe2BB6A9215)
        );

        // transfer owner for oxSolidPool and v1Rewards
        IRewardsDistributor(rewardsDistributorAddress).setRewardPoolOwner(
            oxSolidRewardsAddress,
            address(this)
        );
        IMultiRewards(oxSolidRewardsAddress).acceptOwnership();
        IPartnersRewards(oxSolidRewardsAddress).setOperator(
            rewardsDistributorAddress,
            true
        );
        IPartnersRewards(oxdV1RewardsAddress).setOperator(
            rewardsDistributorAddress,
            true
        );

        address[] memory _tokensAllowedInPools = new address[](4);
        _tokensAllowedInPools[0] = oxdAddress;
        _tokensAllowedInPools[1] = oxSolidAddress;
        _tokensAllowedInPools[2] = solidAddress;
        _tokensAllowedInPools[3] = wftmAddress;

        ITokensAllowlist(tokensAllowlistAddress).setTokenIsAllowedInPools(
            _tokensAllowedInPools,
            true
        );

        for (uint256 i; i < _tokensAllowedInPools.length; i++) {
            IPartnersRewards(partnersRewardsAddress).addReward(
                _tokensAllowedInPools[i],
                rewardsDistributorAddress,
                604800
            );
            IPartnersRewards(oxSolidRewardsAddress).addReward(
                _tokensAllowedInPools[i],
                rewardsDistributorAddress,
                604800
            );
            IPartnersRewards(oxdV1RewardsAddress).addReward(
                _tokensAllowedInPools[i],
                rewardsDistributorAddress,
                604800
            );
