...
// EternityToken.
contract EternityToken is BEP20('Eternity Token', 'ETERNITY') {
...
// MasterChef is the master of Eternity. He can make Eternity and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once ETERNITY is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accEternityPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accEternityPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accEternityPerShare;  // Accumulated KRs per share, times 1e18. See below.
        uint16 depositFeeBP;        // Deposit fee in basis points
        uint256 harvestInterval;    // Harvest interval in seconds
    }

    // The ETERNITY TOKEN!
    EternityToken public immutable eternity;
    // Dev address.
    address public devAddress;
    // ETERNITY tokens created per second.
    uint256 public eternityPerSecond;
    // Deposit Fee address
    ...
    event UpdateEmissionRate(address indexed user, uint256 indexed eternityPerSecond);
    event RewardLockedUp(address indexed user, uint256 indexed pid, uint256 amountLockedUp);
    event StartTimeChanged(uint256 oldStartTime, uint256 newStartTime);

    constructor(
        EternityToken _eternity,
        address _devAddress,
        address _feeAddress,
        uint256 _startTime
    ) public {
        ...
        eternityPerSecond = INITIAL_EMISSION_RATE;
    ...
    function pendingEternity(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accEternityPerShare = pool.accEternityPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0 && totalAllocPoint > 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 eternityReward = multiplier.mul(eternityPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accEternityPerShare = accEternityPerShare.add(eternityReward.mul(1e18).div(lpSupply));
        }

        uint256 pending = user.amount.mul(accEternityPerShare).div(1e18).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }


    // View function to see if user can harvest Eternity's.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (eternity.totalSupply() >= MAX_SUPPLY) {
            eternityReward = 0;
        } else if (eternity.totalSupply().add(eternityReward.mul(11).div(10)) >= MAX_SUPPLY) {
            eternityReward = (MAX_SUPPLY.sub(eternity.totalSupply()).mul(10).div(11));
        }

        if (eternityReward > 0) {
            eternity.mint(devAddress, eternityReward.div(10));
            eternity.mint(address(this), eternityReward);
            pool.accEternityPerShare = pool.accEternityPerShare.add(eternityReward.mul(1e18).div(lpSupply)); 
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        payOrLockupPendingEternity(_pid);
        ...
        user.rewardDebt = user.amount.mul(pool.accEternityPerShare).div(1e18);
    ...
    // Pay or lockup pending Eternity.
    function payOrLockupPendingEternity(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accEternityPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.nextHarvestUntil = getPoolHarvestInterval(_pid);
                // send rewards
                safeEternityTransfer(msg.sender, rewardsToDistribute);
            }
        } else if (pending > 0) {
    ...
    // Safe Eternity transfer function, just in case if rounding error causes pool to not have enough KRs.
    function safeEternityTransfer(address _to, uint256 _amount) internal {
        uint256 eternityBal = eternity.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > eternityBal) {
            transferSuccess = eternity.transfer(_to, eternityBal);
        } else {
            transferSuccess = eternity.transfer(_to, _amount);
        }
        require(transferSuccess, "safeEternityTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _eternityPerSecond) external onlyOwner {
        require (_eternityPerSecond <= MAX_EMISSION_RATE, "updateEmissionRate: value higher than maximum");
        massUpdatePools();
        eternityPerSecond = _eternityPerSecond;
        emit UpdateEmissionRate(msg.sender, _eternityPerSecond);
    }
