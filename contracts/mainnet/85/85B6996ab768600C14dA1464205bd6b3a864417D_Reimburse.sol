...
import {SolidexHolderReimburse as Holder} from "./SolidexHolderReimburse.sol";

/// @title Savior of DeiLenderSolidex
/// @author DEUS Finance
/// @notice to refund people affected in the attack of 15.03.2022
contract Reimburse is AccessControl {
    IERC20 public dei;
    IERC20 public collateral;
    IERC20 public solid;
    IERC20 public solidex;

    mapping(address => uint256) public userDei;
    ...
    modifier isSetter {
        require(hasRole(SETTER, msg.sender), "REIMBURSE: Caller is not setter");
        _;
    }
    ...
    function _setCollateral(address to, uint256 amount) internal {
        ...
        collateral.transferFrom(msg.sender, userHolder[to], amount);
    }

    /// @notice receives users dei and sends him collateral
    /// @dev user should have dei and collateral balance in the contract
    /// @param to collateral receiver
    function deiToCollateral(address to) public returns (bool) {
        uint256 deiAmount = userDei[msg.sender];
        uint256 collateralAmount = userCollateral[msg.sender];

        dei.transferFrom(msg.sender, address(this), deiAmount);
        ...
        emit RefundCollateral(to, deiAmount, collateralAmount);

        userDei[msg.sender] = 0;
        userCollateral[msg.sender] = 0;

        return true;
    }

    /// @notice sets users dei and collateral amount
    function setUserDeiAndCollateral(
        address[] memory users,
        uint256[] memory deiAmounts,
        uint256[] memory collateralAmounts
    ) external isSetter {
        for (uint256 i = 0; i < users.length; i += 1) {
            _setCollateral(users[i], collateralAmounts[i]);
            userDei[users[i]] = deiAmounts[i];
        }
    }
    ...
    event RefundCollateral(
        address receiver,
        uint256 deiAmount,
        uint256 collaterlAmount
    );
}
...
contract SolidexHolderReimburse {
