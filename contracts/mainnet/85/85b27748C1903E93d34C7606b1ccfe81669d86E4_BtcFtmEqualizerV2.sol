...
// File contracts/BtcFtmEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract BtcFtmEqualizerV2  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToBtcPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToBtcPath,
        ...
        for (uint i; i < _equalToBtcPath.length; ++i) {
            equalToBtcPath.push(_equalToBtcPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToBtcPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wftm).balanceOf(address(this));
        uint256 t2Bal = IERC20(btc).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(wftm, btc, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(btc).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(btc).safeApprove(router, type(uint).max);
    }
