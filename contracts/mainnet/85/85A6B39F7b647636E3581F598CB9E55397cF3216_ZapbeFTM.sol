...
contract ZapbeFTM {
    ...
    // accepted slippage
    uint256 public withSlippage  = 9000; 
    ...
    function depositNative(bool testMint) external payable {
        ...
        uint256 swappedAmount = IBaseRouter(router).getAmountOut(nativeBal, wftm, beFTM);
        uint256 swappedAmountWithSlippage = swappedAmount.mul(withSlippage).div(max);

        if (swappedAmountWithSlippage > nativeBal && testMint == false) {
            IBaseRouter(router).swapExactTokensForTokensSimple(nativeBal, swappedAmountWithSlippage, wftm, beFTM, true, msg.sender, block.timestamp);
        } else {
