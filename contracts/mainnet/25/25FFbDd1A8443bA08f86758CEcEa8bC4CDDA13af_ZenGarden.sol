...
contract ZenGarden is Ownable, ReentrancyGuard, IZenGarden {
  ...
  function updatePool(uint256 _pid) public {
    (uint256 accumulatedHNR, uint256 maxEndBlock) = pendingPoolShare(_pid);
    ...
    PoolInfo storage pool = poolInfo[_pid];
    pool.accumulatedHNRPerShare = accumulatedHNR;
    pool.lastRewardBlock = maxEndBlock;
  }

  function pendingPoolShare(uint256 _pid)
    public
    view
    returns (uint256, uint256)
  {
    ...
    return (
      pool.accumulatedHNRPerShare.add(hnrReward.mul(1e18).div(erc20Supply)),
      maxEndBlock
    );
  }
