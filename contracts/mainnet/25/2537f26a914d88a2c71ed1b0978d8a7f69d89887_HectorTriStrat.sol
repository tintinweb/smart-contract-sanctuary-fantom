...
interface ICurveRouter {
    function add_liquidity(address _pool, uint256[3] memory _deposit_amounts, uint256 min_mint_amount) external;
    function calc_token_amount(address _pool, uint256[3] memory _amounts, bool is_deposit) external view returns (uint256);   
}
...
// File: contracts/HectorTriStrat.sol
...
contract HectorTriStrat is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wrapped = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public want;
    address public constant input = address(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
    ...
    address[] public wrappedToInputPath;
    ...
    constructor(
        ...
        wrappedToInputPath = [wrapped, input];
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IStakingRewards(masterchef).withdraw(_amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        uint256 outputBal = IERC20(wrapped).balanceOf(address(this));
    ...
    function addLiquidity() internal {
        uint256 wrappedBal = IERC20(wrapped).balanceOf(address(this));
            ...
            IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedBal, 0, wrappedToInputPath, address(this), block.timestamp);

        uint256 inputBal = IERC20(input).balanceOf(address(this));

        ICurveRouter(liquidityRouter).add_liquidity(want, [0, 0, inputBal], 1);
    }

     /** @dev Returns rewards unharvested */
    function rewardBalance() public view returns (uint256) {
        return IStakingRewards(masterchef).earned(address(this));
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        return rewardBalance() * PLATFORM_FEE / FEE_DIVISOR * CALL_FEE / FEE_DIVISOR;
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(input).safeApprove(liquidityRouter, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(input).safeApprove(liquidityRouter, type(uint).max);
    }
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external onlyAdmin {
        require (_path[0] != wrapped && _path[_path.length - 1] == wrapped, "Bad path");

        IERC20(_path[0]).safeApprove(unirouter, type(uint).max);
         IUniSwapRouter(unirouter).swapExactTokensForTokens(_amount, 0, _path, address(this), block.timestamp);
