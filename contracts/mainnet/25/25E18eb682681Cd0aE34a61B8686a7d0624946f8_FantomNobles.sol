...
contract NobleQuests {
    function burn(address owner, uint256 tokenId, uint256 amount) external {}
}

contract FantomNobles is ERC1155Drop {

    Noble _nobleToken;
    NobleQuests _nobleQuests;
    address _teamWallet;

    event Knighted(address indexed _from);
    event Bishoped(address indexed _from);
    event Rooked(address indexed _from);
    event Queened(address indexed _from);
    event Kinged(address indexed _from);
    event Nobled(address indexed _from);

    constructor(string memory _name, string memory _symbol, address _nobleTokenAddress, address _nobleQuestsAddress, address _team)
    ERC1155Drop(_name, _symbol, msg.sender, 0, _teamWallet) {
        _nobleToken = Noble(_nobleTokenAddress);
        _nobleQuests = NobleQuests(_nobleQuestsAddress);
        _teamWallet = _team;
    }

    function setTeamWallet(address _newTeam) external onlyOwner {
        _teamWallet = _newTeam;
    }

    function setNobleQuests(address _newNobleQuest) external onlyOwner {
        _nobleQuests = NobleQuests(_newNobleQuest);
    }

    function upgradeToKnight() public {
        _burn(msg.sender, 0, 3);
        _mint(msg.sender, 1, 1, "");
        emit Knighted(msg.sender);
    }

    function upgradeToBishop() public {
        _burn(msg.sender, 1, 1);
        // tournament participation
        _nobleQuests.burn(msg.sender, 0, 1);
        _mint(msg.sender, 2, 1, "");
        emit Bishoped(msg.sender);
    }

    function upgradeToRook() public {
        _burn(msg.sender, 2, 2);
        // almost noble
        _nobleQuests.burn(msg.sender, 1, 1);
        _mint(msg.sender, 3, 1, "");
        emit Rooked(msg.sender);
    }

    function upgradeToQueen() public {
        _burn(msg.sender, 3, 2);
        _mint(msg.sender, 4, 1, "");
        emit Queened(msg.sender);
    }

    function skipAndPayToQueen() public {
    ...
    function upgradeToKing() public {
        _burn(msg.sender, 4, 1);
        // cross chain noble
        _nobleQuests.burn(msg.sender, 2, 1);

        _mint(msg.sender, 5, 1, "");
        emit Kinged(msg.sender);
        _nobleToken.mintTo(_teamWallet, 2000000000000000000);
        _nobleToken.mintTo(owner(), 8000000000000000000);
    }

    function claimNoble() public {
        _burn(msg.sender, 5, 1);
        _nobleToken.mintTo(msg.sender, 10000000000000000000);
        emit Nobled(msg.sender);
    }
}