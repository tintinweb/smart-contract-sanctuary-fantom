...
function pendingToken(address _pair, address _user) external view override returns (uint256) {
    ...
    uint256 stakeBalance = ERC20(_pair).balanceOf(address(this)).mul(10**uint256(PI.decimals())).div(
...
function PI_mintTo(address account, uint256 amount) internal {
    // Thực hiện logic của hàm PI_mintTo tại đây
    // ...
}
// Update reward variables of the given pool to be up-to-date.
function updatePool(address _pair) public override {
    ...
    uint256 totalReward = getTotalReward(pool.lastRewardBlock, block.number);
    uint256 tokenReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
    PI_mintTo(devAddr, tokenReward.div(100));
    PI_mintTo(address(this), tokenReward);
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = PI.balanceOf(address(this));
        if (_amount > tokenBal) {
            PI.transfer(_to, tokenBal);
        } else {
            PI.transfer(_to, _amount);
        }
    }
