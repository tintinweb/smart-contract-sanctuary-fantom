...
interface INFTToken {
    ...
    function setNFTPowerCenter(address contract_) external;
}
...
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import "../nft/interfaces/INFTPowerCenter.sol";
...
abstract contract NFTToken is ERC20, OracleToken, INFTToken {
    ...
    address public nftPowerCenter; // stores all nft power
    ...
    /**
     * @notice Sets the NFT power center which contains all power information
     */
    function setNFTPowerCenter(address nftPowerCenter_) external override onlyNFTConfigOperator {
        nftPowerCenter = nftPowerCenter_;
    }
    ...
    function _beforeTokenTransfer(address from_, address to_, uint256 amount_) internal virtual override {
        ...
        if(currentNFTPowerRequired != 0 && !excludedFromNFTAddresses[from_] && !excludedToNFTAddresses[to_]) {
            uint256 power;
            try INFTPowerCenter(nftPowerCenter).getUserPower(from_) returns (uint256 val) {
                power = val;
