...
contract fixedVault is ReentrancyGuard, VaultNFTv4 {
    ...
    modifier frontExists(uint256 vaultID) {
        ...
        require(promoter[vaultID] <= 10000 && promoter[vaultID] > 0, "Front end not added");
        _;
    }
    ...
    function checkExtract(uint256 vaultID) public view returns (uint256) {
        ...
        if ((halfDebt) / (10**priceSourceDecimals) <= minDebt) {
            ...
            return (debtValue * ( gainRatio)) / (1000) / (getEthPriceSource()) / decimalDifferenceRaisedToTen;
        } else {
            return (halfDebt * (gainRatio)) / 1000 / (getEthPriceSource()) / decimalDifferenceRaisedToTen;
        }
...
contract stableQiVault is fixedVault, Ownable {
    ...
    function updateOracleName(string memory _oracle) public onlyOwner {
        oracleType = _oracle;
    ...
    function setOpeningFee(uint256 _openingFee) external onlyOwner {
        require(_openingFee > 10 && _openingFee <= 500, "setOpeningFee: cannot be more than 5%");
    ...
    function setClosingFee(uint256 _closingFee) external onlyOwner {
        require(_closingFee > 10 && _closingFee <= 500, "setClosingFee: cannot be more than 5%");
    ...
    function withdrawInterest() public onlyOperators {
    ...
    function burn(uint256 amountToken) public onlyAdmin {
        // Burn
