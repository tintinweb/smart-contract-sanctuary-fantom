...
contract ShareRewardsPool is Initializable, Operator {
    ...
    IERC20Upgradeable public _share;
    ...
    function __initialize__(address _shareAddress) public initializer {
        __initializeOperator__();
        if (_shareAddress != address(0))
            _share = IERC20Upgradeable(_shareAddress);
    }
    ...
    function setTime(uint256 _time) external onlyOperator {
        require(block.timestamp < _time, "late");
        require(!poolStarted, "Set");
        poolStartTime = _time;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 _pending = user
                ...
                .sub(user.rewardDebt);
            if (_pending > 0) {
                safeStableTransfer(_sender, _pending);
    ...
    function safeStableTransfer(address _to, uint256 _amount) internal {
