...
// File: contracts/SpookyV2ORBS.sol
...
contract SpookyV2ORBS is Ownable, Pausable {
    ...
    constructor(
        ...
        wrappedToLp1Route = [wrapped, lpToken0, lpToken1];
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != wrapped) {
            IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedHalf, 0, wrappedToLp0Route, address(this), block.timestamp);
        }
        if (lpToken1 != wrapped) {
            IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedHalf, 0, wrappedToLp1Route, address(this), block.timestamp);
        }
    ...
    function rewardBalance() public view returns (uint256) {
        return ISpookyChefV2(masterchef).pendingBOO(poolId, address(this));
    }
