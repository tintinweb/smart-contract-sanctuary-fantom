...
contract DAIBondDepositoryV1 is Ownable {
    ...
    function deposit(
        ...
        // ITreasury(treasury).deposit(_amount, principle, payout.add(fee));
        ITreasury(treasury).deposit(_amount, principle, payout.add(fee));
        totalPrinciple = totalPrinciple.add(_amount);

        if (fee != 0) {
            // IERC20(COFFIN).safeTransfer(DAO, fee);
            IERC20(COFFIN).safeTransfer(staking, fee);
        }
        ...
        // swap
        uint256 b = IERC20(cousd).balanceOf(treasury);
        ITreasury(treasury).swapDAI2COUSD(_amount.div(3), 0);
        uint256 a = IERC20(cousd).balanceOf(treasury);
        if (a.sub(b) > 0) {
            // create LP
            ITreasury(treasury).createCoUSDLiquidity(dai, a.sub(b), _amount.div(3), 0, 0);
        }
