...
contract Gold is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the GOLD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("GOLD FINANCE", "GOLD") {
        // Mints 1 GOLD to contract creator for initial pool setup
    ...
    function _getGoldPrice() internal view returns (uint256 _goldPrice) {
        try IOracle(goldOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Gold: failed to fetch GOLD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _goldPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_goldPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGoldOracle(address _goldOracle) public onlyOperatorOrTaxOffice {
        require(_goldOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints GOLD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GOLD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGoldPrice = _getGoldPrice();
            currentTaxRate = _updateTaxRate(currentGoldPrice);
            if (currentGoldPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_goldPool != address(0), "!_goldPool");
