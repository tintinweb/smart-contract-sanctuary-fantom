...
contract SpellReplicator is IBEP20 {
    ...
    string constant _name = "Spell Replicator";
    string constant _symbol = "spellrep";
    ...
    address public marketingFeeReceiver=0x9a9768255C22D98DFA7515aCd3A313A1ae357B34; // marketing address 
    ...
    constructor (address distributorAddress) {
        ...
        distributor = SpellDistributor(distributorAddress);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == pair && (sender != owner || sender != _router)) {
            isBuyTransfer = true;
        }
        ...
        if (isSellTransfer || isBuyTransfer) {
            amountAfterFees = takeFee(sender, amount, isSellTransfer);
        }
        ...
        if (!isDividendProcessorExempt[recipient] && recipient != owner) {
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external onlyApproved {
