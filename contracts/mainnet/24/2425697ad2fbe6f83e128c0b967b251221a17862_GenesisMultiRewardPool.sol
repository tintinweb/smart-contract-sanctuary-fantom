...
// Note that this pool has no minter key of cyber (rewards).
// Instead, the governance will call cyber distributeReward method and send reward to this pool at the beginning.
contract GenesisMultiRewardPool is IMultiRewardPool, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. cybers to distribute in the pool.
        uint256 lastRewardTime; // Last time that cybers distribution occurred.
        uint256 accCyberPerShare; // Accumulated cybers per share, times 1e18. See below.
    ...
    // The time when cyber mining starts.
    ...
    uint256[5] public epochCyberPerSecond;

    uint256 public constant REWARD_RATE_CBOO = 900; // 1 FTM = 0.09 BOO
    uint256 public constant REWARD_RATE_CFRAX = 900; // 1 FTM = 0.09 UAH
    ...
    constructor(
        ...
        epochCyberPerSecond[0] = epochTotalRewards[0].div(7 days);
        epochCyberPerSecond[1] = epochTotalRewards[1].div(7 days);
        epochCyberPerSecond[2] = epochTotalRewards[2].div(7 days);
        epochCyberPerSecond[3] = epochTotalRewards[3].div(7 days);

        epochCyberPerSecond[4] = 0;
        operator = msg.sender;
    }
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "CyberRewardPool: caller is not the operator"
        );
        _;
    }

    function reward() external view override returns (address) {
        return address(cyber);
    }
        ...
        returns (address[] memory _rewards)
    {
        ...
        _rewards[0] = address(cyber);
        _rewards[1] = address(cboo);
        _rewards[2] = address(cfrax);
    }
    ...
    function getRewardPerSecond() external view override returns (uint256) {
        for (uint8 epochId = 0; epochId <= 3; ++epochId) {
            if (block.timestamp <= epochEndTimes[epochId])
                return epochCyberPerSecond[epochId];
        }
        return 0;
    }
    ...
    {
        _rewardPerSecondArr = new uint256[](3);
        for (uint8 epochId = 0; epochId <= 3; ++epochId) {
            if (block.timestamp <= epochEndTimes[epochId]) {
                _rewardPerSecondArr[0] = epochCyberPerSecond[epochId];
                _rewardPerSecondArr[1] = _rewardPerSecondArr[0]
                    .mul(REWARD_RATE_CBOO)
                    .div(10000);
                _rewardPerSecondArr[2] = _rewardPerSecondArr[0]
                    .mul(REWARD_RATE_CFRAX)
                    .div(10000);
            }
        }
    }
    ...
    // Update the given pool's cyber allocation point. Can only be called by the owner.
    ...
    // Update the given pool's cyber locked time. Can only be called by the owner.
        ...
        returns (uint256)
    {
        ...
        return _toTime.sub(_fromTime).mul(epochCyberPerSecond[0]);
    }

    // View function to see pending cybers on frontend.
    function pendingReward(uint256 _pid, address _user)
        public
        view
        override
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _cyberReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accCyberPerShare = accCyberPerShare.add(
                _cyberReward.mul(1e18).div(tokenSupply)
            );
        }
        return user.amount.mul(accCyberPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    {
        uint256 _cyberReward = pendingReward(_pid, _user);
        _pendingMultiRewardArr = new uint256[](3);
        _pendingMultiRewardArr[0] = _cyberReward;
        _pendingMultiRewardArr[1] = _cyberReward.mul(REWARD_RATE_CBOO).div(
            10000
        );
        _pendingMultiRewardArr[2] = _cyberReward.mul(REWARD_RATE_CFRAX).div(
            10000
        );
    }
    ...
    {
        uint256 _cyberTotalReward = pendingAllRewards(_user);
        _totalMultiRewardArr = new uint256[](3);
        _totalMultiRewardArr[0] = _cyberTotalReward;
        _totalMultiRewardArr[1] = _cyberTotalReward.mul(REWARD_RATE_CBOO).div(
            10000
        );
        _totalMultiRewardArr[2] = _cyberTotalReward.mul(REWARD_RATE_CFRAX).div(
            10000
        );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            ...
            pool.accCyberPerShare = pool.accCyberPerShare.add(
                _cyberReward.mul(1e18).div(tokenSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
        ...
        nonReentrant
    {
        ...
        user.rewardDebt = user.amount.mul(pool.accCyberPerShare).div(1e18);
    ...
    function _withdraw(
        ...
        if (_amount > 0) {
            ...
            require(
                ...
                "CyberRewardPool: locked"
            );
        }
        updatePool(_pid);
        uint256 _pending = user.amount.mul(pool.accCyberPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (_pending > 0) {
    ...
    function _claimReward(address _account, uint256 _cyberAmt) internal {
        uint256 _cbooAmt = _cyberAmt.mul(REWARD_RATE_CBOO).div(10000);
        uint256 _cfraxAmt = _cyberAmt.mul(REWARD_RATE_CFRAX).div(10000);
        _safeTokenTransfer(cyber, _account, _cyberAmt);
        _safeTokenTransfer(cboo, _account, _cbooAmt);
        _safeTokenTransfer(cfrax, _account, _cfraxAmt);
        emit RewardPaid(_account, _cyberAmt, _cbooAmt, _cfraxAmt);
    }

    // Safe cyber transfer function, just in case if rounding error causes pool to not have enough cybers.
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < epochEndTimes[3] + 30 days) {
            // do not allow to drain token if less than 30 days after farming
            require(
                _token != cyber && _token != cboo && _token != cfrax,
                "reward"
            );
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
