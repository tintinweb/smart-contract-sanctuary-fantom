...
contract OpenBox is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_TEAM_FUND_DISTRIBUTION = 2500 ether;
    ...
    function _getOpenBoxPrice() internal view returns (uint256 _OpenBoxPrice) {
        try IOracle(OpenBoxOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Error: failed to fetch OpenBox price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _OpenBoxPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_OpenBoxPrice >= taxTiersTwaps[tierId]) {
    ...
    function setOpenBoxOracle(address _OpenBoxOracle) public onlyOperatorOrTaxOffice {
        require(_OpenBoxOracle != address(0), "oracle address cannot be 0 address");
        OpenBoxOracle = _OpenBoxOracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentOpenBoxPrice = _getOpenBoxPrice();
            currentTaxRate = _updateTaxRate(currentOpenBoxPrice);
            if (currentOpenBoxPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_teamFund != address(0), "!_teamFund");
        rewardPoolDistributed = true;
        // _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_teamFund, INITIAL_TEAM_FUND_DISTRIBUTION);
    }
