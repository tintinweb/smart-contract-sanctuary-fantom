...
contract StablecoinFarm is Pausable, BalanceManagement {
    ...
    constructor(address _rewardToken, uint256 _rewardTokenPerSecond, uint32 _startTime) {
    ...
    /*
     * @dev Sets a new vesting duration period
     * @param _vestingDuration is a vesting duration period in seconds
     */
     ...
     * @param _amount is an amount of deposit tokens
     */
     ...
     * @param _pid is a pool id
     * @param _amount is an amount of withdraw tokens
     */
     ...
     * @param _addSeconds is an additional seconds
     */
    ...
    /*
     * @dev Update the given pool's reward token allocation point. Can only be called by the owner.
     * @param _pid is a pool id that exist in the list
     * @param _allocPoint is an allocation point
     * @param _withUpdate if set in true all pools will be updated,
    ...
    /*
     * @dev user can get his rewards for staked iUSDC/iUSDT if locked time is already occurred
     * @param _pid is a pool id
     */
    ...
    function exitEarly(uint256 _pid) public {
        ...
        (uint256 amountTotal, , , ) = checkVestingBalances(_pid, msg.sender);
        require(amountTotal > 0, 'Total vesting tokens can not be zero');
        ...
        require(
            amountUser + amountITP + amountLP <= amountTotal,
            'Calculated amount is more then total vesting amount'
        );
    ...
    /*
     * @dev View function to see pending reward token on frontend.
     * @param _pid is a pool id
    ...
    function pendingRewardToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        (uint256 _totalVesting, , , ) = checkVestingBalances(_pid, _user);
        ...
        if (pending >= _totalVesting) {
            pending = pending - _totalVesting;
        }
    }
     ...
     * @param _pid is a pool id
     * @param _user is a user address to check rewards
     * @return total is the amount of vested and vesting tokens
     * @return vestedTotal is the amount of vested tokens (that are avalaible to withdraw)
     * @return vestingTotal is the amount of vesting tokens (that are not avalaible to withdraw yet)
     * @return vestData is the list with amount of tokens and its unlock time
     */
    function checkVestingBalances(
        ...
        view
        returns (
            uint256 total, // vestedTotal + vestingTotal
            uint256 vestedTotal, // avalaible to withdraw
        ...
        )
    {
        ...
        for (uint256 i = 0; i < vests.length; i++) {
            if (vests[i].unlockTime > block.timestamp) {
                ...
                index++;
                vestingTotal = vestingTotal + vests[i].amount;
                total += vestingTotal;
            } else {
     ...
     * Just in case if the pool does not have enough reward token.
     * @param _to is an address to transfer rewards
     * @param _amount is an amount of reward tokens that will be transfer to the user
     */
