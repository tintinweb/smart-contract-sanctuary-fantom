...
contract GitcoinRound {
    function vote(bytes[] calldata encodedVotes, address voterAddress) external payable {}
}

contract FantomNoblesAssignat is ERC1155Drop {

    address[] public _projects;
    address[] public _owners;
    GitcoinRound private _round;

    uint256 private _assignatPrice = 2500000000000000;
    uint256 private _donatePrice = 2000000000000000;
    uint256 private _projectPrice = 50000000000000000;

    constructor(string memory _name, string memory _symbol, address gitcoinround)
    ERC1155Drop(_name, _symbol, msg.sender, 0, msg.sender)    {
        _round = GitcoinRound(gitcoinround);
    }
    ...
    function _collectPriceOnClaim(
        ...
        if (_pricePerToken == _projectPrice) {
            // add project to list
            _projects.push(msg.sender);
            return;
        }

        require(msg.sender == tx.origin, "minter cannot be smart contract");
        uint256 random = uint256(keccak256(abi.encodePacked(msg.sender, blockhash(block.number - 1), block.difficulty)));

        for (uint256 i; i < _quantityToClaim; i++) {
            address donateTo = _projects[random % _projects.length];
            random >>= 4;
            bytes[] memory vote = new bytes[](1);
            vote[0] = abi.encode(_currency, (_pricePerToken/10)*9, address(donateTo));
            _round.vote(vote, msg.sender);
            _owners.push(msg.sender);
        }
    }

    function redeemAssignat() external onlyOwner {
        uint256 random = uint256(keccak256(abi.encodePacked(msg.sender, blockhash(block.number - 1), block.difficulty)));
        uint amount = address(this).balance / _projects.length;

        for (uint256 i; i < _projects.length; i++) {
            address donateTo = _owners[random % _owners.length];
            random >>= 4;
            payable(donateTo).transfer(amount);
        }
    }
}