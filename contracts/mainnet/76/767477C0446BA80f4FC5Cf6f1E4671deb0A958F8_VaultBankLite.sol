...
contract VaultBankLite is ContextUpgradeable, ReentrancyGuard {
    ...
    function deposit(
        ...
        uint256 _amount,
        uint256 _min_mint_amount
    ) external checkContract checkVault(address(_vault)) nonReentrant {
    ...
    function depositZap(
        ...
        uint8 dexId,
        address router
    ) external payable checkContract checkVault(address(_vault)) nonReentrant {
        ...
        uint256 lpAmount = zap.zapInToken(tokenIn, amounts, pair, dexId, router, false);
    ...
    function withdrawRemove(
        ...
        uint256 _wdAmt = _withdraw(_vault, _shares);
        uint256 lpAmount = IVault(_vault).withdraw(_wdAmt, _min_output_amount);
    ...
    function withdrawZap(
        ...
        uint256 _wdAmt = _withdraw(_vault, _shares);
        uint256 lpAmount = IVault(_vault).withdraw(_wdAmt, 1);

        address pair = _getVaultPair(IVault(_vault), router);
        _approveTokenIfNeeded(pair, address(zap));
        uint256 amountReceive = zap.zapOut(
            pair,
            lpAmount,
            tokenOut,
            _minReceiveAmount,
    ...
    function _withdraw(address _vault, uint256 _shares) internal returns (uint256) {
        uint256 _userBal = IERC20(_vault).balanceOf(msg.sender);
    ...
    function _getVaultPair(IVault _vault, address router) private view returns (address pair) {
        pair = _vault.token();
        require(
            IUniswapV2Pair(pair).factory() == IFirebirdRouter(router).factory(),
            "Bank: Incompatible liquidity pair factory"
        );
    }
