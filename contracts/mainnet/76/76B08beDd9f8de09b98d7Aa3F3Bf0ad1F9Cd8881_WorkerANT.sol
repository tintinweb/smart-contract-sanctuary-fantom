...
contract WorkerANT is ERC721, Ownable {
    ...
    function getUnHousedWorkers() public view returns(uint256[] memory) {
        ...
        uint256 _housedWorkerCount;
        for (uint i = 0; i < _workersList.length; i++){
            if (idToHousing[i]){
                _housedWorkerCount +=1;
            }     
        }
        uint256 _workersHoused = 0;
        uint256[] memory _unhousedWorkerList = new uint256[](_workersList.length - _housedWorkerCount);
...
interface IBuildingBlock {
    ...
    function finishConstruction (uint256 _indexToEnlarge, uint256 _indexToBurn) external;
