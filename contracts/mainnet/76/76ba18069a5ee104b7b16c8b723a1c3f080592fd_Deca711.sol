...
contract Deca711 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    function depositHouse(uint256 _amount) external {
        ...
        for (uint256 i = 0; i < userHouseBalances.length; i++) {
            ...
            if (_sender == balance.userAddress) {
                balance.depositAmount += amountAfterFee;
    ...
    function refundByIndex(uint256 _index) external nonReentrant {
        BetRequest memory _betRequest = betRequest[_index];
        if (_betRequest.timestamp != 0 && block.timestamp >= _betRequest.timestamp + 15 minutes) {
            delete betRequest[_index].timestamp;
