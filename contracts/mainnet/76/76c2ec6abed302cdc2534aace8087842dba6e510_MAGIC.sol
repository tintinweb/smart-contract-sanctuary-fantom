...
contract MAGIC is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 275000 ether;
    // Initial distribution for the day 2-5 MAGIC-WETH LP -> MAGIC pool
    uint256 public constant INITIAL_MAGIC_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the MAGIC ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MAGIC", "MAGIC") {
        // Mints 1 MAGIC to contract creator for initial pool setup
    ...
    function _getMAGICPrice() internal view returns (uint256 _MAGICPrice) {
        try IOracle(MAGICOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("MAGIC: failed to fetch MAGIC price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _MAGICPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_MAGICPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMAGICOracle(address _MAGICOracle) public onlyOperatorOrTaxOffice {
        require(_MAGICOracle != address(0), "oracle address cannot be 0 address");
        MAGICOracle = _MAGICOracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMAGICPrice = _getMAGICPrice();
            currentTaxRate = _updateTaxRate(currentMAGICPrice);
            if (currentMAGICPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_MAGICPool != address(0), "!_MAGICPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_MAGICPool, INITIAL_MAGIC_POOL_DISTRIBUTION);
