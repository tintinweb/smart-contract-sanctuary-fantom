...
contract SunsettingRewardsHelper is OxDAODeployer {
    function upgradeProcotol(
        address oxSolidRewardsImpAddress,
        address partnersRewardsImpAddress
    ) external {
        ...
        IProxy(oxSolidRewardsAddress).updateImplementationAddress(
            oxSolidRewardsImpAddress
        );

        IProxy(partnersRewardsAddress).updateImplementationAddress(
            partnersRewardsImpAddress
        );

        // set operators for VoterProxyAssets
        IOperator oxSolidRewards = IOperator(oxSolidRewardsAddress);
        if (!oxSolidRewards.operator(beepAddress)) {
            oxSolidRewards.setOperator(beepAddress, true);
        }
        if (!oxSolidRewards.operator(c30Address)) {
            oxSolidRewards.setOperator(c30Address, true);
        }

        // set operators for oxdV1Redeem
        IOperator partnersRewards = IOperator(partnersRewardsAddress);
        if (!partnersRewards.operator(beepAddress)) {
            partnersRewards.setOperator(beepAddress, true);
        }
        if (!partnersRewards.operator(c30Address)) {
            partnersRewards.setOperator(c30Address, true);
        }
    }
}
