...
contract Grim is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the GRIM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("iReaper", "GRIM") {
        require(_taxRate < 1000, "tax equal or bigger to 10%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        excludeAddress(address(this));
        // Mints 1 GRIM to contract creator for initial pool setup
    ...
    function _getGrimPrice() internal view returns (uint256 _grimPrice) {
        try IOracle(grimOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("GRIM: failed to fetch GRIM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _grimPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_grimPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGrimOracle(address _grimOracle) public onlyOperatorOrTaxOffice {
        require(_grimOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints GRIM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GRIM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGrimPrice = _getGrimPrice();
            currentTaxRate = _updateTaxRate(currentGrimPrice);
            if (currentGrimPrice < burnThreshold) {
                burnTax = true;
            }
        }
