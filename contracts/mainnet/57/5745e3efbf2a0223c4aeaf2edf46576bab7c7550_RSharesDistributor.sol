...
contract RSharesDistributor is Administrable {
    ...
    constructor(address[] memory accounts, uint256[] memory points) {
        RShare = IERC20(0xB835631824ED68D84c5f41C231f3C2b6C2D709d5);
    ...
    function distribute() external onlyAdmin {
        uint256 balance = RShare.balanceOf(address(this));
        for (uint256 a = 0; a < allocations.length; a ++) {
            RShare.transfer(allocations[a].account, balance * allocations[a].points / totalPoints);
        }
    }

    function setRShare(address share) external onlyOwner {
        RShare = IERC20(share);
    }
