...
contract Pool is Ownable, ReentrancyGuard {
    ...
    uint256 public devRenumeration = 10_000; // 10%
    ...
    function swap(uint256 _minXTokenOut) external payable nonReentrant {
        require(!mintPaused, "Pool::mint: Minting has not been enabled yet.");
        ...
        require(_xTokenOut + xToken.totalSupply() <= IController(controller).maxXTokenSupply(), "Pool::mint: > Max mintable total supply");
        ...
        if (devWallet != address(0)) {
            ...
            _fee = _fee - _devFee;
            //10% of the fees are taken for buybacks & burns, and marketing expenses (as required)
    ...
    function redeem(
        ...
        require(!redeemPaused, "Pool::redeem: Redeeming has not been enabled yet.");
    ...
    function enableSwapAndRedeem(bool _mintPaused, bool _redeemPaused) public onlyOwner {
        if (!mintPaused || !redeemPaused) {
            require(!_mintPaused, "Pool::enableSwapAndRedeem: Minting may not be disabled");
            require(!_redeemPaused, "Pool::enableSwapAndRedeem: Redemption may not be disabled");
        }
        mintPaused = _mintPaused;
        redeemPaused = _redeemPaused;
        emit enabledSwappingAndRedemption(_mintPaused, _redeemPaused);
    }
    ...
    function setYTokenSlippage(uint256 _slippage) external onlyOwner {
        require(_slippage <= 150000, "Pool::setYTokenSlippage: yTokenSlippage cannot be more than 15%");
    ...
    function transferToDeveloper(uint256 _amount) internal {
        ...
        if (_amount > 0) {
            WethUtils.weth.safeTransfer(devWallet, _amount);
        }
    }
