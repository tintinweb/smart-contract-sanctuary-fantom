...
contract FluxLPManager is ReentrancyGuard, Governable, IGlpManager {
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external override onlyGov {
        require(shortsTrackerAveragePriceWeight <= BASIS_POINTS_DIVISOR, "FluxLPManager: invalid weight");
    ...
    function setCooldownDuration(uint256 _cooldownDuration) external override onlyGov {
        require(_cooldownDuration <= MAX_COOLDOWN_DURATION, "FluxLPManager: invalid _cooldownDuration");
        cooldownDuration = _cooldownDuration;
    }
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("FluxLPManager: action not enabled"); }
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) private returns (uint256) {
        require(_amount > 0, "FluxLPManager: invalid _amount");

        // calculate aum before buyUSDG
        uint256 aumInUsdg = getAumInUsdg(true); // 2.1 usd
        uint256 glpSupply = IERC20(glp).totalSupply(); // 0

        IERC20(_token).safeTransferFrom(_fundingAccount, address(vault), _amount);
        uint256 usdgAmount = vault.buyUSDG(_token, address(this));
        require(usdgAmount >= _minUsdg, "FluxLPManager: insufficient USDG output");

        uint256 mintAmount = aumInUsdg == 0 ? usdgAmount : usdgAmount.mul(glpSupply).div(aumInUsdg);  // (30 * 0)/ 2.1 = 0
        // mintAmount = 0  _minGlp = 30
        require(mintAmount >= _minGlp, "FluxLPManager: insufficient FluxLP output");
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        require(_glpAmount > 0, "FluxLPManager: invalid _glpAmount");
        require(lastAddedAt[_account].add(cooldownDuration) <= block.timestamp, "FluxLPManager: cooldown duration not yet passed");
        ...
        require(amountOut >= _minOut, "FluxLPManager: insufficient output");
    ...
    function _validateHandler() private view {
        require(isHandler[msg.sender], "FluxLPManager: forbidden");
    }
}
