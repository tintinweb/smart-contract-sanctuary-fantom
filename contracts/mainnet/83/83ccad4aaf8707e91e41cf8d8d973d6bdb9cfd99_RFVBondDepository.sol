...
contract RFVBondDepository is Ownable, ReentrancyGuard {
  ...
  function deposit(
    ...
    require(
      whitelistedAmount[msg.sender] >= bondedAmount[msg.sender].add(_amount),
      "Exceed your whitelisted amount"
    );
    bondedAmount[msg.sender] = bondedAmount[msg.sender].add(_amount);
    ...
    IERC20(DAI).safeTransferFrom(DAO, address(this), payout);
  ...
  function redeem(address _recipient, bool _stake) external returns (uint256) {
    ...
    IERC20(DAI).transfer(_recipient, info.payout); // pay user everything due
  ...
  function modifyWhitelist(
    ...
    int256[] memory addAmounts
  ) external onlyPolicy {
    require(users.length == addAmounts.length, "Invalid length");
    for (uint256 i = 0; i < users.length; i++) {
      address user = users[i];
      int256 addAmount = addAmounts[i];
      if (addAmount > 0) {
        whitelistedAmount[user] = whitelistedAmount[user].add(
          uint256(addAmount)
        );
      } else {
        whitelistedAmount[user] = whitelistedAmount[user].sub(
          uint256(-addAmount)
        );
      }
    }
  ...
  function getRFVPrice() public view returns (uint256 _marketPrice) {
    return 210; // $2.1
  }
  ...
  function maxPayout() public view returns (uint256) {
    return
      IFHMCirculatingSupply(fhmCirculatingSupply)
  ...
  function bondPriceInUSD() public view returns (uint256 price_) {
    price_ = bondPrice().mul(10 ** IERC20(DAI).decimals()).div(100);
  }
  ...
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IFHMCirculatingSupply(fhmCirculatingSupply)
      .OHMCirculatingSupply()
      .mul(bondPrice())
      .mul(10000000);
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding DAI or FHM) to the DAO
   *  @return bool
   */
