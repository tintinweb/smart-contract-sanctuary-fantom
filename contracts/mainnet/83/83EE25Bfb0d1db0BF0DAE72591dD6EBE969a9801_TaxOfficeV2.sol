...
contract TaxOfficeV2 is Operator {
    ...
    constructor(address _beta, address _usdc, address _router) public {
        beta = _beta;
        usdc = _usdc;
        uniRouter = _router;
    }
    ...
    function setTaxTiersTwap(uint8 _index, uint256 _value) public onlyOperator returns (bool) {
        return ITaxable(beta).setTaxTiersTwap(_index, _value);
    }

    function setTaxTiersRate(uint8 _index, uint256 _value) public onlyOperator returns (bool) {
        return ITaxable(beta).setTaxTiersRate(_index, _value);
    }

    function enableAutoCalculateTax() public onlyOperator {
        ITaxable(beta).enableAutoCalculateTax();
    }

    function disableAutoCalculateTax() public onlyOperator {
        ITaxable(beta).disableAutoCalculateTax();
    }

    function setTaxRate(uint256 _taxRate) public onlyOperator {
        ITaxable(beta).setTaxRate(_taxRate);
    }

    function setBurnThreshold(uint256 _burnThreshold) public onlyOperator {
        ITaxable(beta).setBurnThreshold(_burnThreshold);
    }

    function setTaxCollectorAddress(address _taxCollectorAddress) public onlyOperator {
        ITaxable(beta).setTaxCollectorAddress(_taxCollectorAddress);
    }
    ...
    function _excludeAddressFromTax(address _address) private returns (bool) {
        if (!ITaxable(beta).isAddressExcluded(_address)) {
            return ITaxable(beta).excludeAddress(_address);
        }
    }
    ...
    function _includeAddressInTax(address _address) private returns (bool) {
        if (ITaxable(beta).isAddressExcluded(_address)) {
            return ITaxable(beta).includeAddress(_address);
        }
    }

    function taxRate() external view returns (uint256) {
        return ITaxable(beta).taxRate();
    }
        ...
        )
    {
        ...
        IERC20(beta).transferFrom(msg.sender, address(this), amtTomb);
        IERC20(token).transferFrom(msg.sender, address(this), amtToken);
        _approveTokenIfNeeded(beta, uniRouter);
        ...
        uint256 liquidity;
        (resultAmtTomb, resultAmtToken, liquidity) = IUniswapV2Router01(uniRouter).addLiquidity(
            beta,
            token,
            amtTomb,
            amtToken,
        ...
        if(amtTomb.sub(resultAmtTomb) > 0) {
            IERC20(beta).transfer(msg.sender, amtTomb.sub(resultAmtTomb));
        }
        ...
        )
    {
        ...
        uint256 liquidity;
        (resultAmtTomb, resultAmtFtm, liquidity) = IUniswapV2Router01(uniRouter).addLiquidityETH{value: msg.value}(
            beta,
            amtTomb,
            amtTombMin,
            amtFtmMin,
    ...
    function setTaxableTombOracle(address _tombOracle) external onlyOperator {
        ITaxable(beta).setTombOracle(_tombOracle);
    }

    function transferTaxOffice(address _newTaxOffice) external onlyOperator {
        ITaxable(beta).setTaxOffice(_newTaxOffice);
    }
    ...
    function taxFreeTransferFrom(
        ...
        IERC20(beta).transferFrom(_sender, _recipient, _amt);
