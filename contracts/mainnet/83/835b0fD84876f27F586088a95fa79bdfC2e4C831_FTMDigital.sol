...
contract FTMDigital {
    ...
    uint8[BonusLinesCount] public referralBonus = [90, 40, 30, 20,10];
    uint40 public TIME_STEP =86400;
    uint constant public WITHDRAW_COOLDOWN =21600;			// claim 4 times per day
    ...
    event FeePayed(address indexed user, uint256 totalAmount); 
        ...
        uint40 percentage = 150;
        for (uint8 daysInvested = 6; daysInvested <= 25; daysInvested++) {
    ...
    function deposit(uint8 _lockUp, address _upline) external payable{ 
        ...
        require(WPR[_lockUp].life_days >= 6, "Tarif not found");
        require(amount >= 1 ether, "Minimum deposit amount is 0.05 BNB");
        ...
        {
            giveaway = amount.mul(5).div(100);
        }
        else if(WPR[_lockUp].life_days>=18)
        {
            giveaway = amount.mul(4).div(100);
        }
        else if(WPR[_lockUp].life_days>=12)
        {
        ...
        {
            investor.lastPayout = uint40(block.timestamp);
        }
    ...
    function _canClaim(address _user) internal view returns(bool) {
		return (block.timestamp-investorsMap[_user].lastPayout >= WITHDRAW_COOLDOWN);
	}
    ...
    function depositHalf(uint8 _lockUp, address _upline, uint256 amount) internal {
        ...
        require(amount >= 0.5 ether, "Minimum deposit amount is 0.025 BNB");
