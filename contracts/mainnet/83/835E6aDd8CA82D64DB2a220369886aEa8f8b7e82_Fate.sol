...
contract Fate is TaxableToken, NFTToken {
    ...
    uint256 public constant FUND_POOL_ALLOCATION = 50000 ether;
    bool public rewardPoolDistributed = false;

    error RewardDistributed();

    constructor(uint256 taxRate_, address taxCollectorAddress_, address funds_) TaxableToken(taxRate_, taxCollectorAddress_) ERC20("FATE", "FATE") {
        ...
        AccessControl._grantRole(AccessControlConstants.OPERATOR_ROLE, funds_);
        if(funds_ == address(0)) revert ZeroAddress();
        ...
        TaxableToken.excludedFromTaxAddresses[funds_] = true;
        TaxableToken.excludedToTaxAddresses[funds_] = true;
        NFTToken.excludedFromNFTAddresses[msg.sender] = true;
        NFTToken.excludedFromNFTAddresses[funds_] = true;
        NFTToken.excludedToNFTAddresses[funds_] = true;

        _mint(msg.sender, 100 ether); // deployer
        _mint(funds_, 49_900 ether); // funds
    }
     ...
     */
    function distributeReward(address farmingIncentiveFund_) external onlyOperator {
        if(rewardPoolDistributed) revert RewardDistributed();
        if(farmingIncentiveFund_ == address(0)) revert ZeroAddress();
        TaxableToken.excludedFromTaxAddresses[farmingIncentiveFund_] = true;
        TaxableToken.excludedToTaxAddresses[farmingIncentiveFund_] = true;
        NFTToken.excludedFromNFTAddresses[farmingIncentiveFund_] = true;
        NFTToken.excludedToNFTAddresses[farmingIncentiveFund_] = true;
        _mint(farmingIncentiveFund_, FARMING_POOL_REWARD_ALLOCATION);
