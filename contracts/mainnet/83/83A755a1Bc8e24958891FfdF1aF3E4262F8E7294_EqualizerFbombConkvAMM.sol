...
// File contracts/EqualizerFbombConkvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualizerFbombConkvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToConkPath;
    IEqualizerRouter.Routes[] public equalToFbombPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToConkPath,
        ...
        for (uint i; i < _equalToConkPath.length; ++i) {
           equalToConkPath.push(_equalToConkPath[i]);
        }

        for (uint i; i < _equalToFbombPath.length; ++i) {
           equalToFbombPath.push(_equalToFbombPath[i]);
        }
   ...
   //Withdraws funds from masterchef
    ...
    // This function converts all funds to WFTM, charges fees and sends fees to respective accounts
    function chargeFees(address caller) internal {
        uint256 toWrapped = IERC20(equal).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;
    ...
    // Converts Equal to both sides of the LP token and builds the liquidity pair
    function addLiquidity() internal {
        uint256 equalHalf = IERC20(equal).balanceOf(address(this)) / 2;
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToConkPath, address(this), block.timestamp);  
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToFbombPath, address(this), block.timestamp); 


        uint256 t1Bal = IERC20(conk).balanceOf(address(this));
        uint256 t2Bal = IERC20(fbomb).balanceOf(address(this));
        IEqualizerRouter(router).addLiquidity(conk, fbomb, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);    
    }


    // Determines the amount of reward in WFTM upon calling the harvest function
