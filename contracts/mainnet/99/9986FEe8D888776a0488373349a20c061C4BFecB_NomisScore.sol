...
contract NomisScore is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, Pausable, EIP712 {
    ...
    */
    ...
    /**
     * @dev A mapping of token ids to scores.
     */
    mapping (uint256 => Score) tokenScores;

    /**
     * @dev A mapping of token ids to ca;culation models.
     */
    mapping (uint256 => uint16) tokenCalcModels;

    /**
     * @dev A mapping of token ids to chain ids.
     */
    mapping (uint256 => uint256) tokenChainIds;
    ...
    */
    ...
    function setScore(
        ...
        tokenScores[scoreStruct.tokenId] = scoreStruct;
        tokenCalcModels[scoreStruct.tokenId] = calculationModel;
        tokenChainIds[scoreStruct.tokenId] = chainId;
    ...
    */
    function getScore(address addr, uint256 blockchainId, uint16 calcModel) external view returns(uint16 score, uint256 updated, uint256 tokenId, uint16 calculationModel, uint256 chainId) {
        require(scores[addr][blockchainId][calculationModel].updated != 0, "getScore: No score found for the specified address, blockchain and calculation model");
    ...
    */
    function getScore(uint256 id) external view returns(uint16 score, uint256 updated, uint256 tokenId, uint16 calculationModel, uint256 chainId) {
        require(tokenScores[id].updated != 0, "getScore: No score found for the specified address, blockchain and calculation model");

        Score storage _score = tokenScores[id];

        score = _score.value;
        updated = _score.updated;
        tokenId = _score.tokenId;
        calculationModel = tokenCalcModels[id];
        chainId = tokenChainIds[id];
    }
    ...
    */
    function _afterTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override (ERC721) {
        if (to == address(0))
        {
            ...
            delete tokenScores[tokenId];
            delete tokenCalcModels[tokenId];
            delete tokenChainIds[tokenId];
        }
