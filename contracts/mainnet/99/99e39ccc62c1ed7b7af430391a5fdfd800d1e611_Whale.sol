...
contract Whale is ERC20Burnable, Operator {
    ...
    // Initial distribution for the first 168h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 135914 ether;
    ...
    // Buy and Sell Tax System
    uint256 public StorehouseFundFee = 23298;
    uint256 public DevelopmentFee = 3882;

    uint256 constant public FEE_PERCENTS_DIVIDER = 1000000;

    address public StorehouseFundAddr;
    ...
    function isPairAddress(address _address) public view returns (bool) {
        return PairAddresses[_address];
    }
    ...
    function _getWhalePrice() internal view returns (uint256 _whalePrice) {
        try IOracle(whaleOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Whale: failed to fetch WHALE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _whalePrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_whalePrice >= taxTiersTwaps[tierId]) {
    ...
    function setWhaleOracle(address _whaleOracle) public onlyOperatorOrTaxOffice {
        require(_whaleOracle != address(0), "oracle address cannot be 0 address");
    ...
    function setStorehouseFundAddr(address _StorehouseFundAddr) public {
        require(msg.sender == StorehouseFundAddr, "setStorehouseFundAddr: FORBIDDEN");
        require(_StorehouseFundAddr != address(0), "StorehouseFund address must be non-zero address");
        StorehouseFundAddr = _StorehouseFundAddr;
    }

    function setStorehouseFundFee(uint256 _StorehouseFundFee) public {
        require(msg.sender == StorehouseFundAddr, "setStorehouseFundFee: FORBIDDEN");
        require(_StorehouseFundFee <= 27180, "tax bigger than 2.718%");
        StorehouseFundFee = _StorehouseFundFee;
    }

    function setDevelopmentAddr(address _DevelopmentAddr) public {
        require(msg.sender == DevelopmentAddr, "setDevelopmentAddr: FORBIDDEN");
        require(_DevelopmentAddr != address(0), "Development address must be non-zero address");
        DevelopmentAddr = _DevelopmentAddr;
    }

    function setDevelopmentFee(uint256 _DevelopmentFee) public {
        require(msg.sender == DevelopmentAddr, "setDevelopmentFee: FORBIDDEN");
        require(_DevelopmentFee <= 27180, "tax bigger than 2.718%");
    ...
    function addPairAddress(address _address) public returns (bool) {
        require(msg.sender == StorehouseFundAddr || msg.sender == DevelopmentAddr, "excludeAddress: FORBIDDEN");
        require(!PairAddresses[_address], "address can't be added");
        PairAddresses[_address] = true;
        return true;
    }

    function removePairAddress(address _address) public returns (bool) {
        require(msg.sender == StorehouseFundAddr || msg.sender == DevelopmentAddr, "excludeAddress: FORBIDDEN");
        require(PairAddresses[_address], "address can't be removed");
        PairAddresses[_address] = false;
        return true;
    }

    /**
     * @notice Operator mints WHALE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of WHALE to mint to
    ...
    function _transfer(address from, address to, uint256 amount) internal override 
    {
        ...
        // if this is buy transaction, take buy fee.
        if(PairAddresses[from] && (!PairAddresses[to])) {
            takeFee = true;
        }
        // if this is sell transaction, take sell fee.
        if((!PairAddresses[from]) && PairAddresses[to]) {
            takeFee = true;
        }

        if(takeFee) {
            uint256 amountStorehouseFundFee = amount.mul(StorehouseFundFee).div(FEE_PERCENTS_DIVIDER);
            uint256 amountDevelopmentFee = amount.mul(DevelopmentFee).div(FEE_PERCENTS_DIVIDER);
            amount = amount.sub(amountStorehouseFundFee);
            amount = amount.sub(amountDevelopmentFee);

            super._transfer(from, StorehouseFundAddr, amountStorehouseFundFee);
            super._transfer(from, DevelopmentAddr, amountDevelopmentFee);
        }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentWhalePrice = _getWhalePrice();
            currentTaxRate = _updateTaxRate(currentWhalePrice);
            if (currentWhalePrice < burnThreshold) {
                burnTax = true;
            }
        }
