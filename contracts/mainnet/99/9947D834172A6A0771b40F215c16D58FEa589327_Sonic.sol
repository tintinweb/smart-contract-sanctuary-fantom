...
contract Sonic is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 SONIC-WETH LP -> SONIC pool
    // uint256 public constant INITIAL_SONIC_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the SONIC ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SONIC", "SONIC") {
        // Mints 1 SONIC to contract creator for initial pool setup
    ...
    function _getSonicPrice() internal view returns (uint256 _sonicPrice) {
        try IOracle(sonicOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Sonic: failed to fetch SONIC price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _sonicPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_sonicPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSonicOracle(address _sonicOracle) public onlyOperatorOrTaxOffice {
        require(_sonicOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SONIC to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SONIC to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSonicPrice = _getSonicPrice();
            currentTaxRate = _updateTaxRate(currentSonicPrice);
            if (currentSonicPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_sonicPool != address(0), "!_sonicPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_sonicPool, INITIAL_SONIC_POOL_DISTRIBUTION);
