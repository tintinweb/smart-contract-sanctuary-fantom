...
// ENJOY THE UNENDING REWARDS: $ETH, $BTC, $BOO
...
contract TEST5050 is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "TEST5050";
    string constant _symbol = "TEST5050";
...
//        emit Transfer(sender, recipient, amount);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountFTMReflection = amountFTM.mul(reflectionFee).div(totalFTMFee).div(3);
        uint256 amountFTMMarketing = amountFTM.mul(marketingFee).div(totalFTMFee).div(4);

        try distributor.depositEP{value: amountFTMReflection}() {} catch {}
        payable(marketingFeeReceiver).transfer(amountFTMMarketing);

        try distributor.depositTP{value: amountFTMReflection}() {} catch {}
        payable(marketingFeeReceiver).transfer(amountFTMMarketing);

        try distributor.depositBP{value: amountFTMReflection}() {} catch {}
        payable(marketingFeeReceiver).transfer(amountFTMMarketing);
