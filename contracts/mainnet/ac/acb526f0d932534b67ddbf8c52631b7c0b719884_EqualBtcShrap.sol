...
// File: contracts/EqualBtcShrap.sol
...
contract EqualBtcShrap is Ownable, Pausable {
    ...
    address public constant shrap = address(0xbf4906762C38F50bC7Be0A11BB452C944f6C72E1);
    address public constant btc = address(0x321162Cd933E2Be498Cd2267a90534A804051b11);
    address public want;
    ...
    IEqualizerRouter.Routes[] public equalToShrapPath;
    IEqualizerRouter.Routes[] public wrappedToBtcPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToShrapPath,
        IEqualizerRouter.Routes[] memory _wrappedToBtcPath
        ...
        for (uint i; i < _equalToShrapPath.length; ++i) {
            equalToShrapPath.push(_equalToShrapPath[i]);
        }

        for (uint i; i < _wrappedToBtcPath.length; ++i) {
            wrappedToBtcPath.push(_wrappedToBtcPath[i]);
        }
    ...
    function chargeFees(address caller) internal {                   
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(toWrapped, 0, equalToWrappedPath, address(this), block.timestamp);                                                
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(equalHalf, 0, equalToWrappedPath, address(this), block.timestamp);
        IEqualizerRouter(unirouter).swapExactTokensForTokens(equalHalf, 0, equalToShrapPath, address(this), block.timestamp);

        uint256 wrappedBal = IERC20(wrapped).balanceOf(address(this));
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedBal, 0, wrappedToBtcPath, address(this), block.timestamp);

        
        uint256 lp0Bal = IERC20(shrap).balanceOf(address(this));
        uint256 lp1Bal = IERC20(btc).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(shrap, btc, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 wrappedOut;
        if (outputBal > 0) {
            (wrappedOut,) = IEqualizerRouter(unirouter).getAmountOut(outputBal, equal, wrapped);
        } 
    ...
    function _removeAllowances() internal {
        ...
        IERC20(equal).safeApprove(unirouter, 0);
        IERC20(wrapped).safeApprove(unirouter, 0);
        IERC20(shrap).safeApprove(unirouter, 0);
        IERC20(btc).safeApprove(unirouter, 0);
    }

    function _giveAllowances() internal {
        IERC20(want).safeApprove(gauge, type(uint).max);
        IERC20(equal).safeApprove(unirouter, type(uint).max);
        IERC20(wrapped).safeApprove(unirouter, type(uint).max);
        IERC20(shrap).safeApprove(unirouter, type(uint).max);
        IERC20(btc).safeApprove(unirouter, type(uint).max);
    }
    ...
    function makeCustomTxn(address [][] memory _tokens, bool[] calldata _stable) external onlyAdmin {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(bal, 0, customPath, address(this), block.timestamp + 600);
   
        emit MakeCustomTxn(_tokens[0][0], _tokens[0][_tokens.length - 1], bal);
    }

    /** @dev Sets the fee amounts */
    function setFees(uint256 newCallFee, uint256 newStratFee, uint256 newWithdrawFee, uint256 newTreasuryFee) external onlyOwner {
