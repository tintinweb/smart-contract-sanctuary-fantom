...
// File contracts/abstracts/ReaperBaseStrategyv2.sol
...
// File contracts/interfaces/IProxyDepositor.sol
...
// File contracts/ReaperStrategyHnd.sol



pragma solidity ^0.8.0;
error ReaperStrategyHnd__RedeemError();

/**
 * @dev Deposit and stake want in Hundred Gauges via a ProxyDepositor. Harvest HND rewards and recompound.
 */
contract ReaperStrategyHnd is ReaperBaseStrategyv2 {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // 3rd-party contract addresses
    address public constant SPOOKY_ROUTER = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
    address public proxyDepositor;

    /**
     * @dev Tokens Used:
     * {WFTM} - Required for liquidity routing when doing swaps.
     * {HND} - Reward token for staking in gauges.
     * {want} - Address of token staked via proxyDepositor.
     */
     ...
     * {hndToWftmPath} - to swap {HND} to {WFTM} (using SPOOKY_ROUTER)
     * {wftmToWantPath} - to swap {WFTM} to {want} (using SPOOKY_ROUTER)
     */
    address[] public hndToWftmPath;
    ...
    function initialize(
        ...
        address[] memory _feeRemitters,
        address[] memory _strategists,
        address _proxyDepositor,
        address _cWant
    ) public initializer {
        __ReaperBaseStrategy_init(_vault, _feeRemitters, _strategists);
        cWant = _cWant;
        want = CErc20I(cWant).underlying();
        proxyDepositor = _proxyDepositor;
        hndToWftmPath = [HND, WFTM];
        wftmToWantPath = [WFTM, want];
    }
    ...
    function _deposit() internal override {
        ...
        if (wantBalance != 0) {
            IERC20Upgradeable(want).safeIncreaseAllowance(cWant, wantBalance);
            CErc20I(cWant).mint(wantBalance);

            uint256 cWantBalance = IERC20Upgradeable(cWant).balanceOf(address(this));
            IERC20Upgradeable(cWant).safeTransfer(proxyDepositor, cWantBalance);
            IProxyDepositor(proxyDepositor).depositToStake();
        }
    }
    ...
    function _withdraw(uint256 _amount) internal override {
        ...
        if (wantBal < _amount) {
            ...
            uint256 exchangeRate = CErc20I(cWant).exchangeRateCurrent();
            uint256 toWithdraw = remaining * 1e18 / exchangeRate;

            IProxyDepositor(proxyDepositor).withdrawFromStake(toWithdraw);

            if (CErc20I(cWant).redeemUnderlying(remaining) != 0) {
                revert ReaperStrategyHnd__RedeemError();
            }
        }
        IERC20Upgradeable(want).safeTransfer(vault, _amount - 1);
    }

    /**
     * @dev Core function of the strat, in charge of collecting and re-investing rewards.
     *      1. Claims {HND} from the {proxyDepositor}.
     *      2. Swaps {HND} to {WFTM} using {SPOOKY_ROUTER}.
     *      3. Claims fees for the harvest caller and treasury.
     *      4. Swaps the {WFTM} token for {want} using {SPOOKY_ROUTER}.
     *      5. Deposits and stakes via {proxyDepositor}.
     */
    function _harvestCore() internal override {
        // 1
        IProxyDepositor(proxyDepositor).retrieveRewards();

        // 2
        uint256 hndBal = IERC20Upgradeable(HND).balanceOf(address(this));
        _swap(hndBal, hndToWftmPath, SPOOKY_ROUTER);

        // Also should swap extra rewards to WFTM
        ...
        _swap(wftmBal, wftmToWantPath, SPOOKY_ROUTER);
    ...
    /**
     * @dev Function to calculate the total {want} held by the strat.
     *      It takes into account both the funds in hand, plus the funds in the ProxyDepositor and Hundred.
     */
    function balanceOf() public view override returns (uint256) {
        return balanceInProxyDepositor() + IERC20Upgradeable(want).balanceOf(address(this));
    }

    function balanceInProxyDepositor() public view returns (uint256) {
        uint256 amount = IProxyDepositor(proxyDepositor).balanceOfStrategy();
        uint256 exchangeRate = CErc20I(cWant).exchangeRateStored();
     ...
     */
    function estimateHarvest() external returns (uint256 profit, uint256 callFeeToUser) {
        uint256 pendingReward = IProxyDepositor(proxyDepositor).claimableRewardOfStrategy();
        uint256 totalRewards = pendingReward + IERC20Upgradeable(HND).balanceOf(address(this));

        if (totalRewards != 0) {
             profit += IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(totalRewards, hndToWftmPath)[1];
        }
    ...
    function _retireStrat() internal override {
        ...
        // Withdraw and recover want
        uint256 toWithdraw = balanceInProxyDepositor();
        if (toWithdraw != 0) { 
            IProxyDepositor(proxyDepositor).withdrawFromStake(IProxyDepositor(proxyDepositor).balanceOfStrategy());
            if (CErc20I(cWant).redeemUnderlying(toWithdraw) != 0) {
                revert ReaperStrategyHnd__RedeemError();
            }
        }
    ...
    function _reclaimWant() internal override {
        IProxyDepositor(proxyDepositor).reclaimTokensOfStrategy(address(this));
        uint256 cWantBal = IERC20Upgradeable(cWant).balanceOf(address(this));
        uint256 exchangeRate = CErc20I(cWant).exchangeRateCurrent();
        uint256 wantAmount = cWantBal * exchangeRate / 1e18;
        if (CErc20I(cWant).redeemUnderlying(wantAmount) != 0) {
            revert ReaperStrategyHnd__RedeemError();
        }
    }
}