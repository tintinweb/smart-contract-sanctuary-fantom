...
import "./TCollateralCapErc20.sol";

/**
 * @title Cream's TCollateralCapErc20Delegate Contract
 * @notice TTokens which wrap an EIP-20 underlying and are delegated to
 * @author Cream
 */
contract TCollateralCapErc20Delegate is TCollateralCapErc20 {
    /**
    ...
    function _becomeImplementation(bytes memory data) public {
        ...
        // Set TToken version in tombtroller
        TombtrollerInterfaceExtension(address(tombtroller)).updateTTokenVersion(
            address(this),
            TombtrollerV1Storage.Version.COLLATERALCAP
        );
    }
...
/**
 * @title Cream's TCollateralCapErc20 Contract
 * @notice TTokens which wrap an EIP-20 underlying with collateral cap
 * @author Cream
 */
contract TCollateralCapErc20 is TToken, TCollateralCapErc20Interface, TProtocolSeizeShareStorage {
    /**
    ...
    function maxFlashLoan() external view returns (uint256) {
        uint256 amount = 0;
        if (TombtrollerInterfaceExtension(address(tombtroller)).flashloanAllowed(address(this), address(0), amount, "")) {
            amount = getCashPrior();
        }
        return amount;
    }
    ...
    function flashFee(uint256 amount) external view returns (uint256) {
        require(
            TombtrollerInterfaceExtension(address(tombtroller)).flashloanAllowed(address(this), address(0), amount, ""),
            "flashloan is paused"
        );
    ...
    function flashLoan(
        ...
        require(
            TombtrollerInterfaceExtension(address(tombtroller)).flashloanAllowed(
                address(this),
                address(receiver),
                amount,
                data
     ...
     * @dev This function could only be called by tombtroller.
    ...
    function registerCollateral(address account) external returns (uint256) {
        ...
        require(msg.sender == address(tombtroller), "only tombtroller may register collateral for user");
    ...
    function unregisterCollateral(address account) external {
        ...
        require(msg.sender == address(tombtroller), "only tombtroller may unregister collateral for user");
    ...
    function initializeAccountCollateralTokens(address account) internal {
        ...
        if (!isCollateralTokenInit[account]) {
            if (TombtrollerInterfaceExtension(address(tombtroller)).checkMembership(account, TToken(this))) {
    ...
    function doTransferIn(
        ...
        assembly {
            ...
            case 32 {
                // This is a tombliant ERC-20
                returndatacopy(0, 0, 32)
                success := mload(0) // Set `success = returndata` of external call
            }
            default {
                // This is an excessively non-tombliant ERC-20, revert.
                revert(0, 0)
            }
        }
    ...
    function transferTokens(
        ...
        /**
         * Since bufferTokens are not collateralized and can be transferred freely, we only check with tombtroller
         * whether collateralized tokens can be transferred.
         */
        uint256 allowed = tombtroller.transferAllowed(address(this), src, dst, collateralTokens);
        if (allowed != 0) {
    ...
    function decreaseUserCollateralInternal(address account, uint256 amount) internal {
        require(tombtroller.redeemAllowed(address(this), account, amount) == 0, "tombtroller rejection");
    ...
    function mintFresh(
         ...
         */
        if (TombtrollerInterfaceExtension(address(tombtroller)).checkMembership(minter, TToken(this))) {
    ...
    function redeemFresh(
        ...
        uint256 allowed = tombtroller.redeemAllowed(address(this), redeemer, collateralTokens);
        if (allowed != 0) {
