...
contract GenericMinerV2 is IGenericMinerV2 {
  ...
  constructor(IGovernanceAddressProvider _addresses, BoostConfig memory boostConfig) public {
    ...
    BoostConfig memory b = BoostConfig({
      a: boostConfig.a,
      b: boostConfig.b,
      c: boostConfig.c,
      d: boostConfig.d,
      e: boostConfig.e,
      maxBoost: boostConfig.maxBoost
    });
    _boostConfig = b;
   ...
   */
  function setBoostConfig(BoostConfig calldata newBoostConfig) external onlyManager {
    BoostConfig memory b = BoostConfig({
      a: newBoostConfig.a,
      b: newBoostConfig.b,
      c: newBoostConfig.c,
      d: newBoostConfig.d,
      e: newBoostConfig.e,
      maxBoost: newBoostConfig.maxBoost
    });
    _boostConfig = b;
  ...
  */
  function releaseRewards(address _user) public virtual override {
    ...
    uint256 pendingMIMO = _pendingMIMO(_user);
    uint256 pendingPAR = _pendingPAR(_user);
    _refresh();
    _mimoBalanceTracker = _mimoBalanceTracker.sub(pendingMIMO);
    _parBalanceTracker = _parBalanceTracker.sub(pendingPAR);
    userInfo.accAmountPerShare = _accMimoAmountPerShare;
  ...
  */
  function _increaseStake(address user, uint256 value) internal {
    ...
    uint256 pendingMIMO = _pendingMIMO(user);
    uint256 pendingPAR = _pendingPAR(user);

    _refresh();

    if (userInfo.stakeWithBoost > 0) {
      _mimoBalanceTracker = _mimoBalanceTracker.sub(pendingMIMO);
      _parBalanceTracker = _parBalanceTracker.sub(pendingPAR);
    }
    ...
    if (pendingPAR > 0) {
      require(_par.transfer(user, pendingPAR), "LM100");
    }
    if (pendingMIMO > 0) {
      require(_a.mimo().transfer(user, pendingMIMO), "LM100");
    }
  ...
  */
  function _decreaseStake(address user, uint256 value) internal {
    require(value > 0, "LM101");
    UserInfo storage userInfo = _users[user];
    require(userInfo.stake >= value, "LM102");
  ...
  /**
    ...
    @param _user the address of the user
   */
  function _updateBoost(address _user) internal {
    UserInfo storage userInfo = _users[_user];

    // if user had a boost already, first remove it from the totalStakeWithBoost
    if (userInfo.stakeWithBoost > 0) {
      _totalStakeWithBoost = _totalStakeWithBoost.sub(userInfo.stakeWithBoost);
    }
    uint256 multiplier = _getBoostMultiplier(_user);
    userInfo.stakeWithBoost = userInfo.stake.wadMul(multiplier);
    _totalStakeWithBoost = _totalStakeWithBoost.add(userInfo.stakeWithBoost);
  }
  ...
  */
  ...
  /**
    ...
    @return number of MIMO tokens that the user can claim
  */
  function _pendingMIMO(address _user) internal view returns (uint256) {
    ...
    return _users[_user].stakeWithBoost.rayMul(accMimoAmountPerShare.sub(_users[_user].accAmountPerShare));
  }
  ...
  /**
    ...
    @return number of PAR tokens that will be sent automatically when staking/unstaking
  */
  function _pendingPAR(address _user) internal view returns (uint256) {
    if (_totalStakeWithBoost == 0) {
      return 0;
    }
    ...
    return _users[_user].stakeWithBoost.rayMul(accParAmountPerShare.sub(_users[_user].accParAmountPerShare));
  }
...
contract VotingMinerV2 is IVotingMinerV2, GenericMinerV2 {
  ...
  /**
    ...
    @dev this method is for upgradability purposes
    @param user address of the user
  */
  function syncStake(address user) external override {
    UserInfo storage userInfo = _users[user];
    uint256 votingPower = _a.votingEscrow().balanceOf(user);
    _totalStake = _totalStake.add(votingPower).sub(userInfo.stake);
  ...
  */
  function releaseMIMO(address _user) external {
    IVotingEscrow votingEscrow = _a.votingEscrow();
    UserInfo storage userInfo = _users[_user];
    _refresh();
...
contract PARMinerV2 is IGenericMinerV2 {
  ...
  */
  function _increaseStake(address user, uint256 value) internal {
    ...
    uint256 newTotalStake = _totalStake.add(value);
    uint256 pendingMIMO = _pendingMIMO(user);
  ...
  */
  function _decreaseStake(address user, uint256 value) internal {
    ...
    uint256 newTotalStake = _totalStake.sub(value);
    uint256 pendingMIMO = _pendingMIMO(user);
...
contract DexAddressProvider is IDexAddressProvider {
  ...
  mapping(uint256 => Dex) public override dexMapping;
  ...
  */
  function setDexMapping(
    ...
    dexMapping[_index] = Dex({ proxy: _proxy, router: _router });
  }
...
contract DemandMinerV2 is IDemandMinerV2, GenericMinerV2 {
  ...
  constructor(
    ...
    FeeConfig memory f = FeeConfig({ depositFee: feeConfig.depositFee, withdrawFee: feeConfig.withdrawFee });
    _feeConfig = f;
    emit FeeConfigSet(f);
  }
   ...
   */
  function setFeeConfig(FeeConfig calldata newFeeConfig) external onlyManager {
    FeeConfig memory f = FeeConfig({ depositFee: newFeeConfig.depositFee, withdrawFee: newFeeConfig.withdrawFee });
    _feeConfig = f;
    emit FeeConfigSet(f);
  }
