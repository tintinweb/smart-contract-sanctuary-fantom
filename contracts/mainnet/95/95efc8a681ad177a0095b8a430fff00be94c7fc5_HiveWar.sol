...
contract HiveWar is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    **/
    ...
    uint256 public totalCost = 0 ether;
    uint256 public maxSupply = 5000;
    uint256 public maxMintAmount = 10;
    uint256 public presaleLimit = 10;
    ...
    function preMint(uint256 _mintAmount) public payable {
        ...
        require(presaleMintActive && !publicMintActive, "the preMint has not started");
        uint256 supply = totalSupply();
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
        totalCost = cost.mul(_mintAmount);
        ...
        if (msg.sender != owner()) {
            ...
            if (isOG(msg.sender)) {
                totalCost = totalCost.mul(80).div(100);
            } else if (isWhitelisted(msg.sender)) {
                totalCost = totalCost.mul(90).div(100);
            }
    ...
    function mint(uint256 _mintAmount) public payable {
        require(!paused, "the contract is paused");
        require(publicMintActive && !presaleMintActive, "public mint has not started");
        uint256 supply = totalSupply();
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
        require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    ...
    **/
    ...
    function addOGUsers(address[] calldata _users) public onlyOwner {
        delete OGAddresses;
        OGAddresses = _users;
    }
    ...
    function setPresaleMintStartTime(uint256 _presaleMintStartTime) public onlyOwner {
        presaleMintStartTime = _presaleMintStartTime;
    }
    ...
    function setMintActive(bool _presaleMintActive, bool _publicMintActive) public onlyOwner {
