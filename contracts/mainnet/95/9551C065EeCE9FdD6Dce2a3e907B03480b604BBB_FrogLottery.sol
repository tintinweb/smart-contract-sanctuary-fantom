...
contract FrogLottery is ReentrancyGuard {
    struct EpochInfo {
        uint32 goldfishJackpot;
        uint32 whalesJackpot;
    ...
    /** Packed */
    // We will assume it's not in wei (10 ** 0) but in eth (10 ** 18)
    // We save gas by packing 8 uint32 in 1 slot
    uint32 public goldfishJackpot;
    uint32 public whalesJackpot;
    uint32 public totalGoldfishBidders;
    uint32 public totalWhalesBidders;
    uint32 public minGoldfishBid;
    uint32 public minWhalesBid;
    ...
    mapping(address => uint32) public goldFishBids;
    mapping(address => uint32) public whalesBids;
    ...
    constructor(
        ...
        uint32 _minGoldfishBid,
        uint32 _minWhalesBid
    ) {
        owner = msg.sender;
        DAO = _DAO;
    ...
    event Bid(address indexed bidder, uint16 indexedepoch, uint32 totalBidAmount);
    ...
    function bid(uint8 _mode) external payable canBid {
        ...
        uint32 prevBid = _mode == 0 ? goldFishBids[msg.sender] : whalesBids[msg.sender];
        // Can not realistically overflow, type(uint32).max = 4,294,967,295
        // FTM = $2 => 4,294,967,295 * 2 = $91,91,230,011 bid
        uint32 newBid = prevBid + uint32(msg.value / (10**18));
    ...
    function _processGoldfishBid(uint32 _prevBid, uint32 _newBid) internal {
    ...
    /// @param _prevBid The previous bid of the bidder
    function _processWhaleBid(uint32 _prevBid, uint32 _newBid) internal {
        // We update the bidder's bid
        // We increase the total amount of bidders only if it's a new bidder
