...
/// @title EctoplasmFTMRoyaltyRouter
/// @author out.eth
/// @notice This contract is used to swap NFTs and pay royalties.
contract EctoplasmFTMRoyaltyRouter is Owned, ERC721TokenReceiver {
    ...
    function nftBuy(
        ...
        // make the swap
        inputAmount = Pair(pair).nftBuy{value: maxInputAmount}(
            tokenIds,
            maxInputAmount,
            deadline
        );
    ...
    function nftSell(
        ...
        // make the swap
        outputAmount = Pair(pair).nftSell(
            tokenIds,
            minOutputAmount,
            deadline,
            proofs
    ...
    function getRoyaltyRate(
        ...
        address lookupAddress = royaltyRegistry.getRoyaltyLookupAddress(
            tokenAddress
        );
    ...
    function _payRoyalties(
        ...
        uint256[] calldata tokenIds,
        uint256 salePrice
    ) internal returns (uint256 totalRoyaltyAmount) {
        address lookupAddress = royaltyRegistry.getRoyaltyLookupAddress(
            tokenAddress
        );
        ...
        totalRoyaltyAmount;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            (address _recipient, uint256 royaltyAmount) = _getRoyalty(
                lookupAddress,
                tokenIds[i],
                salePrice
            );
    ...
    function _getRoyalty(
        ...
        if (
            ...
            (recipient, royaltyAmount) = IERC2981(lookupAddress).royaltyInfo(
                tokenId,
                salePrice
            );
        }
...
contract Pair is ERC20, ERC721TokenReceiver {
    ...
    address public immutable baseToken; // address(0) for FTM
    bytes32 public immutable merkleRoot;
    LpToken public immutable lpToken;
    Ectoplasm public immutable ectoplasm;
    ...
    constructor(
        ...
        ectoplasm = Ectoplasm(msg.sender);
    }
    ...
    function add(
        ...
        if (lpTokenSupply == 0) {
            lpToken.mint(ectoplasm.owner(), MINIMUM_LIQUIDITY);
        }
    ...
    /// @dev Can only be called by the ectoplasm owner. This is used as an emergency exit in case
    ///      the ectoplasm owner suspects that the pair has been compromised.
    function close() public {
        // check that the sender is the ectoplasm owner
        require(ectoplasm.owner() == msg.sender, "Close: not owner");

        // set the close timestamp with a grace period
        closeTimestamp = block.timestamp + CLOSE_GRACE_PERIOD;

        // remove the pair from the Ectoplasm contract
        ectoplasm.destroy(nft, baseToken, merkleRoot);

        emit Close(closeTimestamp);
    }

    /// @notice Withdraws a particular NFT from the pair.
    /// @dev Can only be called by the ectoplasm owner after the close grace period has passed. This
    ...
    function withdraw(uint256 tokenId) public {
        // check that the sender is the ectoplasm owner
        require(ectoplasm.owner() == msg.sender, "Withdraw: not owner");
        ...
        // transfer the nft to the ectoplasm owner
...
/// @title ectoplasm.sh
/// @author out.eth (@outdoteth)
/// @author modified by 0xesper.eth (@lil_esper)
/// @notice An AMM for creating and trading fractionalized NFTs.
contract Ectoplasm is Owned {
