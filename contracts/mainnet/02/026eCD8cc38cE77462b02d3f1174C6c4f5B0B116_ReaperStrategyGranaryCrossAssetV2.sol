...
contract ReaperStrategyGranaryCrossAssetV2 is ReaperBaseStrategyv3 {
    ...
    uint256 public borrowAssetSupplied; // raw amount of borrowAsset that's been supplied back to the pool
    ...
    function _addLiquidity() internal override {
        (uint256 suppliedBorrowAssetBal, , , , , , , , ) = DATA_PROVIDER.getUserReserveData(
            address(borrowAsset),
            address(this)
        );
        if (suppliedBorrowAssetBal > borrowAssetSupplied) {
            uint256 withdrawableProfit = suppliedBorrowAssetBal - borrowAssetSupplied;
            ILendingPool pool = ILendingPool(ADDRESSES_PROVIDER.getLendingPool());
            withdrawableProfit = pool.withdraw(address(borrowAsset), withdrawableProfit, address(this));
            if (withdrawableProfit >= STAKING.minDeposit()) {
                withdrawableProfit = MathUpgradeable.min(withdrawableProfit, STAKING.maxDeposit());
                require(WFTM.withdraw(withdrawableProfit) == WFTM.ERR_NO_ERROR(), "Unwrapping failed.");
                STAKING.deposit{value: withdrawableProfit}();
            }
        }
    }
    ...
    function balanceOf() public view override returns (uint256) {
        (uint256 supply, ) = getSupplyAndBorrow();
        return supplyAsset.balanceOf(address(this)) + supply;
    }
    ...
    function _rebalance(uint256 _supplyAssetToFree) internal {
        ...
        _supplyAssetToFree = MathUpgradeable.min(_supplyAssetToFree, supply);
        supply -= _supplyAssetToFree;

        (uint256 supplyAssetPrice, uint256 borrowAssetPrice) = getAssetPrices();
        uint256 supplyInUSD = supply * supplyAssetPrice;
        uint256 borrowInUSD = borrow * borrowAssetPrice;
        ...
        if (ltv > maxLtv) {
            _repayToPool(borrow - desiredBorrow);
        } else if (ltv < targetLtv) {
            _borrowFromPool(desiredBorrow - borrow);
        }
    }
    ...
    function _repayToPool(uint256 _borrowAssetToRepay) internal {
        ...
        if (_borrowAssetToRepay > stratBorrowAssetBal) {
            ...
            borrowAssetSupplied -= pool.withdraw(address(borrowAsset), remainingBorrowAssetToWithdraw, address(this));
    ...
    function _borrowFromPool(uint256 _borrowAssetToBorrow) internal {
        ...
        if (borrowAssetBal != 0) {
            ...
            borrowAssetSupplied += borrowAssetBal;
        }
    }
    ...
    function getSupplyAndBorrow() public view returns (uint256 supply, uint256 borrow) {
        (supply, , , , , , , , ) = DATA_PROVIDER.getUserReserveData(address(supplyAsset), address(this));
        (, , borrow, , , , , , ) = DATA_PROVIDER.getUserReserveData(address(borrowAsset), address(this));
