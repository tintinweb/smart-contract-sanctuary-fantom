...
/**
 * @title ScreamDistributor
 * @author beepidibop.eth
 * @notice Contract for distributing weekly SCREAM rewards to the unitroller, callable by Gelato or owner
 */

contract ScreamDistributor is OpsReady, Ownable {
    using SafeERC20 for IERC20;

    IERC20 public constant scream =
        IERC20(0xe0654C8e6fd4D733349ac7E09f6f23DA256bF475);
    address public constant unitroller =
        address(0x3d3094Aec3b63C744b9fe56397D36bE568faEBdF); //v2
    uint256 public distributeAmount;
    uint256 public cooldown;
    uint256 public lastDistTimestamp;
    bool public paused;

    uint256 public maxGasPriceInGwei; //for Gelato tasks
    bytes32 public taskId; //Gelato taskId

    event rewardDistributed(uint256 distTimestamp, uint256 amount);

    constructor(address _ops) OpsReady(_ops) {
        distributeAmount = 20000 ether; //default 20,000 SCREAM per week
        cooldown = 86400 * 7; //default weekly
    ...
    function gelatoDistribute() external onlyOps notPaused {
    ...
    function forceDistribute(uint256 amount) external onlyOwner {
        lastDistTimestamp = block.timestamp;

        scream.transfer(unitroller, amount);

        emit rewardDistributed(lastDistTimestamp, amount);
    }

    function _distribute() internal {
        require(block.timestamp > lastDistTimestamp + cooldown, "Cooldown");
        lastDistTimestamp = block.timestamp;

        scream.transfer(unitroller, distributeAmount);

        emit rewardDistributed(lastDistTimestamp, distributeAmount);
    }

    //Settings
    function setDistributionAmount(uint256 _distributeAmount)
        external
        onlyOwner
    {
    ...
    function setLastDistTimestamp(uint256 _lastDistTimestamp)
        external
        onlyOwner
    {
        lastDistTimestamp = _lastDistTimestamp;
    }
    ...
    function createGelatoTask() external onlyOwner {
        ...
        bytes4 _execSelector = bytes4(
            abi.encodeWithSignature("gelatoDistribute()")
        );
    ...
    function recoverFTM(uint256 amount) external onlyOwner {
