...
// File contracts/UsdcMimEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract UsdcMimEqualizerV2 is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToMimPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToMimPath,
        ...
        for (uint i; i < _equalToMimPath.length; ++i) {
            equalToMimPath.push(_equalToMimPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        uint256 mimDecimals = 10**IERC20Extended(mim).decimals();
        uint256 usdcDecimals = 10**IERC20Extended(usdc).decimals();
        uint256 mimOut = IEqualizerRouter(router).getAmountsOut(equalHalf, equalToMimPath)[equalToMimPath.length] * 1e18 / mimDecimals;
        uint256 usdcOut = IEqualizerRouter(router).getAmountsOut(equalHalf, equalToUsdcPath)[equalToUsdcPath.length] * 1e18 / usdcDecimals;
        (uint256 amount0, uint256 amount1,) = IEqualizerRouter(router).quoteAddLiquidity(mim, usdc, stable, mimOut, usdcOut);
        amount0 = amount0 * 1e18 / mimDecimals;
        amount1 = amount1 * 1e18 / usdcDecimals;
        uint256 ratio = mimOut * 1e18 / usdcOut * amount0 / amount1;
        uint256 mimRatio = equalHalf * 1e18 / (ratio + 1e18);
        uint256 usdcRatio = equalHalf - mimRatio;

        IEqualizerRouter(router).swapExactTokensForTokens(mimRatio, 0, equalToMimPath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokens(usdcRatio, 0, equalToUsdcPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(mim).balanceOf(address(this));
        uint256 t2Bal = IERC20(usdc).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(mim, usdc, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(mim).safeApprove(router, type(uint).max);
    }
