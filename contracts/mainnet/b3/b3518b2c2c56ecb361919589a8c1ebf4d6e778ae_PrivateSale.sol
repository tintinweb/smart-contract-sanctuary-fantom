...
contract PrivateSale is Ownable {
  ...
  mapping(address => bool) public publicSaleAllocations;
  ...
  function addAllocation(address[] memory users, uint8 tier) external onlyOwner {
    ...
    for (uint256 i = 0; i < users.length; i++) {
        ...
        if (tier == 3) {
            // Assign public sale allocation
            publicSaleAllocations[users[i]] = true;
        } else {
            // Whitelist only for tier 1 and tier 2
            if (tierOf[users[i]] != 3 && !whitelisted[users[i]]) {
  ...
  function allocationFor(address user) public view returns (uint256) {
    if (publicSaleAllocations[user]) {
        uint256 available = hardcapOfTier[3].sub(purchaseOf[user]);
        return available;
    } else {
        if (!whitelisted[user]) return 0;
        uint8 tier = tierOf[user];
        if (tier < 1 || tier > 2) return 0;
        uint256 available = hardcapOfTier[tier].sub(purchaseOf[user]);
        return available;
    }
}
