...
// No permissions granted before Tuesday, 23 Jul 2024, then GPL-3.0 after this date.
 ...
 * out quoting reference 'NyPtvFantomWftmSftmxWigoSwapStrategyToUsdc'
 *
...
interface IMasterChef {
  ...
  function pendingWigo(uint256 _pid, address _to) external view returns (uint256 pendingReward);
}
...
contract NyPtvFantomWftmSftmxWigoSwapStrategyToUsdc is NYBaseProfitTakingStrategyV3 {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  uint256 public constant POOL_ID = 23;
  address public constant WIGO_SWAP = address(0xA1a938855735C0651A6CfE2E93a32A28A236d0E9);
  address public constant WIGO_SWAP_ROUTER = address(0x5023882f4D1EC10544FCB2066abE9C1645E95AA0);
  ...
  address public constant LP_TOKEN = address(0x43E1059c05D3153B5D74303DD6474a43BC87E73e);
  ...
  function _deposit() internal override {
    ...
    if (underlyingBalance != 0) {
      IERC20Upgradeable(LP_TOKEN).safeIncreaseAllowance(WIGO_SWAP, underlyingBalance);
      IMasterChef(WIGO_SWAP).deposit(POOL_ID, underlyingBalance);
    }
  }
  ...
  function _withdraw(uint256 _amount) internal override {
    ...
    if (underlyingBalance < _amount) {
      IMasterChef(WIGO_SWAP).withdraw(POOL_ID, _amount - underlyingBalance);
    }
  ...
  function _harvestCore()
    ...
    returns (uint256 callerFee, uint256 underlyingTokenCount, uint256 sentToVault)
  {
    IMasterChef(WIGO_SWAP).deposit(POOL_ID, 0);
  ...
  function _swapFarmEmissionTokens() internal {
    IERC20Upgradeable wigo = IERC20Upgradeable(WIGO);
    uint256 wigoBalance = wigo.balanceOf(address(this));
    if (wigoToUsdcPath.length >= 2 && wigoBalance > 0) {
      wigo.safeIncreaseAllowance(WIGO_SWAP_ROUTER, wigoBalance);

      uint256[] memory amounts = IUniswapV2Router02(WIGO_SWAP_ROUTER).getAmountsOut(wigoBalance, wigoToUsdcPath);
      uint256 amountOutMin = (amounts[amounts.length - 1] * maxSlippage) / PERCENT_DIVISOR;

      IUniswapV2Router02(WIGO_SWAP_ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(
        wigoBalance,
        amountOutMin,
        wigoToUsdcPath,
        address(this),
  ...
  function balanceOf() public view override returns (uint256) {
    (uint256 amount, ) = IMasterChef(WIGO_SWAP).userInfo(POOL_ID, address(this));
    return amount + IERC20Upgradeable(LP_TOKEN).balanceOf(address(this));
  }

  function _reclaimUnderlying() internal override {
    IMasterChef(WIGO_SWAP).emergencyWithdraw(POOL_ID);
  }

  function estimateHarvest() external view override returns (uint256 profit, uint256 callFeeToUser) {
    uint256 pendingReward = IMasterChef(WIGO_SWAP).pendingWigo(POOL_ID, address(this));
    uint256 totalRewards = pendingReward + IERC20Upgradeable(WIGO).balanceOf(address(this));
    if (totalRewards != 0) {
      uint256[] memory harvestAmounts = IUniswapV2Router02(WIGO_SWAP_ROUTER).getAmountsOut(
        totalRewards,
        wigoToUsdcPath
      );
  ...
  function _retireStrategy() internal override {
    ...
    (uint256 poolBal, ) = IMasterChef(WIGO_SWAP).userInfo(POOL_ID, address(this));
    if (poolBal != 0) {
      IMasterChef(WIGO_SWAP).withdraw(POOL_ID, poolBal);
    }
