...
/*
______ _     _____  ________  _________ _____ _   _   ___   _   _ _____  _____ _____ _____ 
| ___ \ |   |  _  ||  _  |  \/  ||  ___|_   _| \ | | / _ \ | \ | /  __ \|  ___|_   _|  _  |
| |_/ / |   | | | || | | | .  . || |_    | | |  \| |/ /_\ \|  \| | /  \/| |__   | | | | | |
| ___ \ |   | | | || | | | |\/| ||  _|   | | | . ` ||  _  || . ` | |    |  __|  | | | | | |
| |_/ / |___\ \_/ /\ \_/ / |  | || |    _| |_| |\  || | | || |\  | \__/\| |_____| |_\ \_/ /
\____/\_____/\___/  \___/\_|  |_/\_|    \___/\_| \_/\_| |_/\_| \_/\____/\____(_)___/ \___/ 
    https://bloomfinance.io
*/
...
contract Bloom is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 BLOOM-WFTM LP -> BLOOM pool
    //uint256 public constant INITIAL_BLOOM_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the BLOOM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress)
        public
        ERC20("BLOOM", "BLOOM Token")
    {
        // Mints 1 BLOOM to contract creator for initial pool setup
    ...
    function _getBloomPrice() internal view returns (uint256 _bloomPrice) {
        try IOracle(bloomOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("Bloom: failed to fetch BLOOM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _bloomPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (
                ...
                if (_bloomPrice >= taxTiersTwaps[tierId]) {
                    require(
    ...
    function setBloomOracle(address _bloomOracle)
        public
        onlyOperatorOrTaxOffice
    {
        require(
            _bloomOracle != address(0),
            "oracle address cannot be 0 address"
        );
        bloomOracle = _bloomOracle;
    }
    ...
    /**
     * @notice Operator mint BLOOM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BLOOM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBloomPrice = _getBloomPrice();
            currentTaxRate = _updateTaxRate(currentBloomPrice);
            if (currentBloomPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_bloomPool != address(0), "!_bloomPool");
