...
contract ClampedRandomizer {
    ...
    function _genClampedNonce() virtual public returns(uint256) {
 ...
 */
contract FDDealerz is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint256[] emptyTables = [151,1565,1617,1843,1929,1941,2249,2257,2437,275,39,774];

    address public tritWallet = 0x3e05c7FFfEfe9030523c1eb14E50ace5B0da9Cf7;
    
    ClampedRandomizer P1 = new ClampedRandomizer(1000);
    ClampedRandomizer P2 = new ClampedRandomizer(1000);
    ClampedRandomizer P3 = new ClampedRandomizer(1000);

    ClampedRandomizer[] phaseRandomizers = [P1, P2, P3];

    ERC721 public fantomDrugs = ERC721(0x10476aFCb828fc2320c5B4af83c56e068f4D0286);

    uint256 public phaseIndex = 0;
    uint256[] public phaseCaps = [1000,2000,3000];
    ...
    uint256 public highifyFee;

    mapping(uint256 => uint256) public _balances;
    uint256 _percentageScope = 100;
    ...
    mapping(uint256 => uint256[]) _effects; //Composition index for the custom NFTs

    mapping(uint256 => address) private _payee;
    mapping(uint256 => uint256) private _percentage;

    uint256[] giveawayMints = [8,7,7];
    ...
    constructor(
        ...
        _payee[0] = address(0x7f8fd0e91010c579440D65e9b5D69054cCe67E96);
        _payee[1] = address(0x57d91B54e67FE58dB2A90033615C51a554F82105);
        _payee[2] = address(0x4002587449bDBbAB2D60037a338602bAa9d4d0fc);
        _payee[3] = address(0x78d0a0a7bCeD90877B064C2B9ddf1E58B3a4Cd93);
    ...
    function setTritWallet(address newAddy) public {
        require(msg.sender == tritWallet);
        tritWallet = newAddy;
    }
    ...
    function setHighifyFee(uint256 val) public onlyOwner {
        highifyFee = val;
    }
    ...
    function mint(uint256 mintAmount) public payable {
        ...
        require(totalSupply() + mintAmount <= phaseCaps[phaseIndex], "Mint exceeds current allocation");
        ...
        for (uint i = 0; i < mintAmount; i++) {
            uint256 tokenId = phaseRandomizers[phaseIndex]._genClampedNonce() + 1 + (1000*phaseIndex);
            _safeMint(msg.sender, tokenId);
        }

        for (uint i = 0; i < 4; i++){
            uint256 payout = ((msg.value*_percentage[i])/_percentageScope)/10;
            _balances[i] = _balances[i] + payout;
        }
    }

    function highifyPayment() public payable {
        require(msg.value == highifyFee);
    }

    function setTokenURI(uint256 tokenId, string calldata URI, uint256[] calldata effects) public {
        require(msg.sender == _nftBuilder || msg.sender == owner(), "only the authorized builder or contract owner may set URI");
        _effects[tokenId] = effects;
        super._setTokenURI(tokenId, URI);
    }

    function setTokenURIBurn(address origin, uint256 tokenId, uint256 burnId, string calldata URI, uint256[] calldata effects) public {
        require(msg.sender == _nftBuilder || msg.sender == owner(), "only the authorized builder or contract owner may set URI");
        require(fantomDrugs.ownerOf(burnId) == origin);
        fantomDrugs.safeTransferFrom(origin, address(1), burnId);
        _effects[tokenId] = effects;
        super._setTokenURI(tokenId, URI);
    }
    ...
    function mintCredit() public {
        require(hasSaleStarted == true, "Minting is not currently enabled");
        require(totalSupply() < phaseCaps[phaseIndex], "Exceeds current allocation");
    ...
    function advancePhase() public onlyOwner {
        if (hasSaleStarted == false && phaseIndex == 0) {
            hasSaleStarted = true;
            mintToEntitled();
        } else if (phaseIndex < 2) {
            require(totalSupply() >= phaseCaps[phaseIndex], "phase not yet sold out");
            phaseIndex++;
            mintToEntitled();
        }
    }

    function mintToEntitled() private {
        for(uint256 i = 0; i < emptyTables.length; i++) {
            try fantomDrugs.ownerOf(emptyTables[i]) {
                address to = fantomDrugs.ownerOf(emptyTables[i]);
                uint256 tokenId = phaseRandomizers[phaseIndex]._genClampedNonce() + 1 + (1000*phaseIndex);
                _safeMint(to, tokenId);
            } catch(bytes memory) {}
        }
        for(uint i = 0; i < giveawayMints[phaseIndex]; i++) {
            uint256 tokenId = phaseRandomizers[phaseIndex]._genClampedNonce() + 1 + (1000*phaseIndex);
            _safeMint(owner(), tokenId);
        }

        address[3] memory devs = [tritWallet, _payee[1], _payee[2]];
        for(uint i = 0; i < 3; i++) {
            for(uint256 j = 0; j < 3; j++) {
                uint256 tokenId = phaseRandomizers[phaseIndex]._genClampedNonce() + 1 + (1000*phaseIndex);
                _safeMint(devs[i], tokenId);
            }
        }
    }
    ...
    function withdrawFTM() public {
        for (uint i = 0; i < 4; i++){
            payable(_payee[i]).transfer(_balances[i]);
            _balances[i] = 0;
        }
    }
    ...
    function changePayeeAddress(uint256 index, address newAddress) public {
        if (index == 0){
            require(msg.sender == tritWallet, "only _payeeZeroAdmin can change _payee[0]");
            _payee[0] = newAddress;
        } else {
            require(msg.sender == _payee[index], "only current payees can change their reception address");
            _payee[index] = newAddress;
        }
    }
