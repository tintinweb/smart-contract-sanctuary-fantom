...
contract YieldOptimizer is Ownable {
    ...
    struct Pool {
        ...
        address[] tokens;
        bytes32[] swapRoutes;
        uint256[] tokensWeights;
        bool isActive;
    }

    struct SwapBeforeInvest {
        address[] tokens;
        uint256[] amounts;
    }
    ...
    mapping(string => mapping(uint256 => uint256)) public userBptBalance;
    ...
    function addPool(
        ...
        bytes32[] calldata _swapRoutes
    ) external onlyOwner {
        (address[] memory poolTokens, , ) = IVault(vault).getPoolTokens(
            _poolId
        );
        poolInfo[poolsCounter] = Pool({
            tokens: poolTokens,
            poolId: _poolId,
            poolName: IWeightedPool(_poolAddress).name(),
            tokensWeights: IWeightedPool(_poolAddress).getNormalizedWeights(),
            bptToken: _poolAddress,
            swapRoutes: _swapRoutes,
            isActive: true
        });
    ...
    function updatePoolSwapRoutes(
        ...
        Pool storage pool = poolInfo[investedPoolId];
        pool.swapRoutes = newSwapRoutes;
    }

    function turnOffPool(uint256 investedPoolId) external onlyOwner {
        Pool storage pool = poolInfo[investedPoolId];
        pool.isActive = false;
    }

    function turnOnPool(uint256 investedPoolId) external onlyOwner {
        Pool storage pool = poolInfo[investedPoolId];
        pool.isActive = true;
    }
    ...
    function invest(
        ...
        balancerJoin(pool.poolId, pool.tokens, giveAmounts);
        uint256 bptBalance = IERC20(pool.bptToken).balanceOf(address(this)) -
            bptBalanceBefore;
        userBptBalance[userId][investedPoolId] += bptBalance;
        emit Invest(
            pool.poolId,
            pool.bptToken,
            amount,
    ...
    function withdrawFromPool(
        ...
        require(amount <= userBptBalance[userId][investedPoolId]);
        Pool storage pool = poolInfo[investedPoolId];
        uint256[] memory outAmounts = new uint256[](pool.tokens.length);
        ...
        balancesBefore = checkBalances(pool.tokens);
        balancerExit(pool.poolId, pool.tokens, outAmounts, amount);
        balancesAfter = checkBalances(pool.tokens);
        ...
        userBptBalance[userId][investedPoolId] -= amount;
    ...
    function balancerExit(
        ...
        // bytes memory userData = abi.encode(0, bptAmount, 0);
    ...
    function checkBalances(address[] memory tokens)
        public
        view
