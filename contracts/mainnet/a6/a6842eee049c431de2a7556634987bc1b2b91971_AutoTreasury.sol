 ...
 * Optimizing Gains.
 */
contract AutoTreasury is ReentrancyGuard, AccessControl {
    ...
    event HarvestToSLVR(uint256 amount);
    ...
    constructor(address _treasuryDAOAddress, address _stabilityPool, address _LQTYPool, address _reward, address _underlying) {
        treasuryDAOAddress = _treasuryDAOAddress;
        treasuryLiquidityAddress = _treasuryDAOAddress;
        usdCurrency =  _underlying;
    ...
    function CycleOne() public onlyRole(OPERATOR_ROLE) {
        unInvest();
        HarvestRewards();
        Invest();
    }

    function CycleTwo() public onlyRole(OPERATOR_ROLE) {
        unInvest();
        unStake();
        Invest();
        Stake();
    ...
    function removeSLVR() external onlyRole(DEFAULT_ADMIN_ROLE) {
        address token = capital;
        uint256 balanceToken = IERC20(address(token)).balanceOf(address(this));
        if (balanceToken > 0)
            IERC20(address(token)).transfer(msg.sender, balanceToken);
    }

    function removeGOLD() external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function Stake() public onlyRole(OPERATOR_ROLE) {
        uint256 balance = lqty.balanceOf(address(this));
        if (balance == 0) return;
   ...
   function unInvest() public onlyRole(OPERATOR_ROLE) {
        if (recentInvestment == 0) return;

        // claims LQTY & ETH rewards if there are any
        stabilityPool.withdrawFromSP(recentInvestment);
   ...
   function unStake() public onlyRole(OPERATOR_ROLE) {
    ...
    /// Public functions ///
    ///@dev sell all of a current type of token for SLVR.
