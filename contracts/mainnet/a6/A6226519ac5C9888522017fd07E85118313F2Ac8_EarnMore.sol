...
contract EarnMore is OwnableUpgradeable {
    ...
    uint256 constant PRESICION = 1000;
    ...
    uint32 public targetRegion;

    address constant choAddress = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75; // In fantom cho - usdc ...
    ...
    mapping(address => bool) public whitelistedBalanceCalculator;

    mapping(address => mapping(address => DepositInfo)) public depositInfo; // Amount

    mapping(address => address) public vaultToBalanceCalculator;

    mapping(address => address) public unipool;

    mapping(address => mapping(address => mapping(address => uint256))) public allowance; // vault => user => user => amount 
    ...
    event Approve(address indexed vault, address indexed owner, address indexed recepient, uint256 amount);
    event VaultAdded(address indexed vault);
    event UnipoolSet(address indexed token, address indexed pool);
    event BalanceCalculatorWhitleisted(address indexed balanceCaclulator);
    ...
    event NewExcludePercent(uint256 value);
    event NewEarnMorePercent(uint256 value);
    event NewMaxVeMultiplier(uint256 newValue);
    event NewMaxVePortion(uint256 newValue);

    modifier vaultWhitelisted(address vault) {
        require(
            vaultToBalanceCalculator[vault] != address(0),
    ...
    function initialize(
        ...
        uint32 _targetRegion,
        address _choUniPool,
        // uint256 _maxVeMultiplier, = 100 (10 %)
        // uint256 _maxVePortion, = 50 (5 %)
        address[] calldata vaults,
        address[] calldata underlyingUnipools,
        address[] calldata balanceCalculators
    ) external initializer {
        uint256 _maxVeMultiplier = 100;
        uint256 _maxVePortion = 50;
        ...
        maxVeMultiplier = _maxVeMultiplier;
        maxVePortion = _maxVePortion;

        unipool[choAddress] = _choUniPool;

        require(
            vaults.length == underlyingUnipools.length &&
                vaults.length == balanceCalculators.length,
            "Unequal lengths"
        );
        for (uint256 i = 0; i < vaults.length; i++) {
            _whitelistBalanceCalculator(balanceCalculators[i], true);
            _addVault(vaults[i], underlyingUnipools[i], balanceCalculators[i]);
        }
    }
    ...
    function deposit(
        ...
        address recepient,
        uint256 tokenId
    ) external vaultWhitelisted(vault) {
        uint256 veMultiplier = 0;

        if (tokenId != 0) {
            require(IVe(ve).ownerOf(tokenId) == recepient, "recepient are not owner of lock");
            uint256 maxVeValue = IVe(ve).totalSupply() * maxVePortion / PRESICION;
            uint256 calculatedMultiplier = maxVeMultiplier * IVe(ve).balanceOfNFT(tokenId) / maxVeValue;

            veMultiplier = calculatedMultiplier > maxVeMultiplier ? maxVeMultiplier : calculatedMultiplier;
        }

        IERC20(vault).safeTransferFrom(msg.sender, address(this), amount);
        DepositInfo storage info = depositInfo[vault][recepient];
        info.amountDeposited += amount;
        info.depositUnderlyingValue += _calcUnderlying(vault, amount);
        info.veMultiplier = veMultiplier;
    }

    function withdraw(address vault, address from, uint256 amount) external {
        withdraw2(vault, from, amount, false);
    }
    ...
    function withdraw2(
        ...
        if (from != msg.sender) {
            _decreaseAllowence(vault, from, msg.sender, amount);
        }

        // Think about naming
        DepositInfo storage info = depositInfo[vault][from];
        require(amount <= info.amountDeposited, "Wrong amount");

        uint256 portion = (PRESICION * amount) / info.amountDeposited;

        (
            uint256 excludedValue,
            uint256 finalAmount,
            uint256 choRewardAmount
        ) = _earnMoreCalc(vault, from, portion);

        info.amountDeposited -= (portion * info.amountDeposited) / PRESICION; // Do we need it?
        info.depositUnderlyingValue -=
            (portion * info.depositUnderlyingValue) /
            PRESICION;

        if (IERC20(choAddress).balanceOf(address(this)) >= choRewardAmount) {
            if (excludedValue != 0) {
                IERC20(vault).safeTransfer(treasury, excludedValue);
                IERC20(choAddress).safeTransfer(msg.sender, choRewardAmount);
            }
    ...
    function approve(address vault, address recepient, uint256 amount) external {
        allowance[vault][msg.sender][recepient] = amount;

        emit Approve(vault, msg.sender, recepient, amount);
    }

    function _decreaseAllowence(address vault, address owner, address recepient, uint256 amount) internal {
        allowance[vault][owner][recepient] -= amount;
    }

    function earnMoreCalc(address vault, address account, uint256 portion) external view returns(uint256 excludedValue, uint256 finalAmount, uint256 choRewardAmount) {
        return _earnMoreCalc(vault, account, portion);
    }
    ...
    {
        // Exclude profit
        DepositInfo storage info = depositInfo[vault][account];

        uint256 amount = (portion * info.amountDeposited) / PRESICION;
        uint256 prevUnderlyingValue = (portion * info.depositUnderlyingValue) /
            PRESICION;
        uint256 currentUnderlyingValue = _calcUnderlying(vault, amount);

        // No profit
        return (0, amount, 0);

        uint256 profit = currentUnderlyingValue - prevUnderlyingValue;
        excludedValue =
            (profit * amount * excludePercent) /
            PRESICION /
            currentUnderlyingValue;
        uint256 excludedBaseValue = (profit * excludePercent) / PRESICION;
        finalAmount = amount - excludedValue;

        // Get price in USDT
        uint256 excludedPrice = _getTokenPrice(
            _getUnderlyingToken(vault),
            excludedBaseValue
        );

        uint256 rewardsPrice = (excludedPrice * (earnMorePercent + PRESICION) * (info.veMultiplier + PRESICION)) / PRESICION / PRESICION;
    ...
    function _getUnderlyingToken(
        ...
        return
            IBalanceCalculator(vaultToBalanceCalculator[vault]).getUnderlying(
                vault
            );
    }

    function _calcUnderlying(
        address vault,
        uint256 amount
    ) internal view returns (uint256) {
        return
            IBalanceCalculator(vaultToBalanceCalculator[vault]).calcValue(
                vault,
                amount
            );
    }
    ...
    function _getChoReward(uint256 price) internal view returns (uint256) {
        ...
        return (price * choBase) / choInUSDT; // check for overflow
    }
    ...
    function balanceOf(
        ...
        return depositInfo[vault][account].amountDeposited;
    }
    ...
    function whitelistBalanceCalculator(
        address _balanceCalculator,
        bool _value
    ) external onlyOwner {
        _whitelistBalanceCalculator(_balanceCalculator, _value);
    }
    ...
    function _whitelistBalanceCalculator(
        address _balanceCalculator,
        bool _value
    ) internal {
        require(
            _balanceCalculator != address(0),
            "Zero address for balanceCalculator"
        );

        whitelistedBalanceCalculator[_balanceCalculator] = _value;

        emit BalanceCalculatorWhitleisted(_balanceCalculator);
    }
    ...
    function _addVault(
        ...
        require(
            whitelistedBalanceCalculator[balanceCalculator],
            "Unknown balanceCalculator"
        );
        vaultToBalanceCalculator[vault] = balanceCalculator;
        _setUnipool(_getUnderlyingToken(vault), underlyingUnipool);
    ...
    function _setMaxVeMultiplier(uint256 newValue) internal onlyOwner {
        maxVeMultiplier = newValue;
        emit NewMaxVeMultiplier(newValue);
    }

    function _setMaxVePortion(uint256 newValue) internal onlyOwner {
