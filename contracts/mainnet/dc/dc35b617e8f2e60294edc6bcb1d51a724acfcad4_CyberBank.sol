...
contract CyberBank is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public startTime = 1655589581;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "CyberBank: caller is not the operator");
        _;
    }

    modifier onlyTimelock() {
        require(timelock == msg.sender, "CyberBank: caller is not timelock");
        _;
    }

    modifier notInitialized() {
        require(!initialized, "CyberBank: already initialized");
        _;
    }
    ...
    function addRewardPool(address _rewardToken, uint256 _rewardPerSecond) public nonReentrant onlyOperator {
        require(rewardPoolInfo.length <= 16, "CyberBank: Reward pool length > 16");
        massUpdatePools();
    ...
    function setUnstakingFrozenTime(uint256 _unstakingFrozenTime) external nonReentrant onlyOperator {
        require(_unstakingFrozenTime <= 7 days, "CyberBank: !safe - dont lock for too long");
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant notContract {
        ...
        if (_wantAmt > 0) {
            ...
            user.lastStakedBalance = stakedWantTokens(_pid, msg.sender);
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant notContract {
        ...
        require(user.shares > 0, "CyberBank: user.shares is 0");
        require(sharesTotal > 0, "CyberBank: sharesTotal is 0");
        ...
        if (_wantAmt > 0) {
            ...
            if (_wantAmt > 0) {
                require(whitelisted[msg.sender] || block.timestamp >= unfrozenStakeTime(_pid, msg.sender), "CyberBank: frozen");
    ...
    function setTimelock(address _timelock) external {
        require(msg.sender == timelock || (timelock == address(0) && msg.sender == operator), "CyberBank: !authorised");
        timelock = _timelock;
    }
    ...
    function executeTransaction(address target, uint256 value, string memory signature, bytes memory data) external onlyTimelock returns (bytes memory) {
        ...
        require(success, "CyberBank::executeTransaction: Transaction execution reverted.");
