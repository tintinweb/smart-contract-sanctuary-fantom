...
contract FantomHouse is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    function depositHouse(uint256 _amount, address _referral) external {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            Balance memory balance = userHouseBalances[_user];
            userHouseBalances[_user].depositAmount += amountAfterFee;
    ...
    function withdrawHouse(uint256 _amount) external nonReentrant {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            Balance memory _userHouseBalance = userHouseBalances[users[i]];
            if (_sender == users[i]) {
                uint256 _depositAmount = _userHouseBalance.depositAmount;
