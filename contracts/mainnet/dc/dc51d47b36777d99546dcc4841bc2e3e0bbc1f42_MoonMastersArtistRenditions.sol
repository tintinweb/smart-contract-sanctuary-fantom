...
contract MoonDaoNFT is ClampedRandomizer, Context, AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage {
  using Counters for Counters.Counter;
  Counters.Counter public _tokenIdTrackerWL;
  ...
  uint private _max;
  /// @notice Max number of token mintable via the whitelist free mint function
  uint private _maxwl;
  ...
  uint _maxPerWallet = 3;

  mapping(address => uint) private whitelist;

  constructor(string memory name, string memory symbol, string memory baseTokenURI, uint mintPrice, uint max, uint maxwl, address wallet, address admin) ERC721(name, symbol) ClampedRandomizer(max - maxwl, maxwl) {
      ...
      _tokenIdTrackerWL.increment(); // Increment the whitelist tracker once to start at 1
  ...
  function getOpenMintStatus() public view returns (bool) {
    return _openMint;
  }

  function getOpenFreeMintStatus() public view returns (bool) {
    return _openWhitelistMint;
  }
  ...
  function mint(uint amount) public payable {
    ...
    require(amount <= 1, "Max of 1 NFT per mint");
    require(ERC721.balanceOf(msg.sender) < _maxPerWallet, "Max per wallet reached");
  ...
  function mintWhitelist(uint amount) public {
    ...
    require(ERC721.balanceOf(msg.sender) < _maxPerWallet, "Max per wallet reached");
    require(whitelist[msg.sender] > 0, "No freemint spot");
    require(amount <= whitelist[msg.sender], "Not enough freemint spot");
    require(supply < _max, "All NFTs have been minted");
    //require(_tokenIdTrackerWL.current() < _maxwl, "max whitelist mint reached");
    
    for(uint i = 0; i < amount; i++) {
      require(_tokenIdTrackerWL.current() <= _maxwl, "max whitelist mint reached");
      _mint(msg.sender, _tokenIdTrackerWL.current());
      _tokenIdTrackerWL.increment();
      //internalMint(msg.sender);
  ...
  function whitelistUser(address user, uint nbspot) public {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Need to be admin");
...
contract ClampedRandomizer {
    ...
    constructor(uint256 scopeCap, uint256 scopeOffset) {
        _scopeCap = scopeCap;
...
contract MoonMastersArtistRenditions is Context,  AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage{
  using Counters for Counters.Counter;
  ...
  function mint(uint amount) public payable {
    ...
    require(_openMint == true, "Minting is closed");
    require(msg.value == _price*amount, "Must send correct price");
    require(_tokenIdTracker.current() + amount <= _max + 1, "not enough 1_1 left to be minted");
    for(uint i = 0; i < amount; i++) {
      _mint(msg.sender, _tokenIdTracker.current());
      _tokenIdTracker.increment();
    }
  ...
  function mintWhitelist(uint amount) public {
    require(_openWhitelistMint == true, "Minting is closed");
    require(whitelist[msg.sender] > 0, "user must be whitelisted to mint");
    require(amount <= whitelist[msg.sender], "user must have enough whitelist spots left to mint");
    require(_tokenIdTracker.current() + amount <= _max + 1, "not enough 1_1 left to be minted");
