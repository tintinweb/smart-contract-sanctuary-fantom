...
//    address private ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    constructor() public ERC20('LPB Token', 'LPB'){
        ...
        _mint(msg.sender, MAX.sub(30000*1e18));
        _mint(address(0x115C81DD01104B2e7fb25Ee7339e2e93208aa59E), 30000*1e18);
    }
    ...
    function sendReadrawAbcdBatch(uint256 poolIndex, address[] memory userList, uint256[] memory amountList) public onlyOwner payable
    {
        ...
        if(poolIndex == 1){
            totalPoolBalance = rewardPoolA;
        }else if(poolIndex == 2){
            totalPoolBalance = rewardPoolB;
        }else if(poolIndex == 3){
            totalPoolBalance = rewardPoolC;
        }else{
            totalPoolBalance = rewardPoolD;
        }
        ...
        for(uint256 i = 0; i < userList.length; i++) {
            IERC20(this).transfer(userList[i], amountList[i]);
        }
        if(poolIndex == 1){
            rewardPoolA = rewardPoolA.mul(totalToSend);
        }else if(poolIndex == 2){
            rewardPoolB = rewardPoolB.mul(totalToSend);
        }else if(poolIndex == 3){
            rewardPoolC = rewardPoolC.mul(totalToSend);
        }else{
            rewardPoolD = rewardPoolD.mul(totalToSend);
        }
    }
