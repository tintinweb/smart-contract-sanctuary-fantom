...
contract Strategy is BaseStrategy {
	...
	uint256 public dust;
	bool public isExact = false;

	bytes32 public pool1Id =
		bytes32(0x3b998ba87b11a1c5bc1770de9793b17a0da61561000000000000000000000185);
	bytes32 public pool2Id =
		bytes32(0x5ddb92a5340fd0ead3987d3661afcd6104c3b757000000000000000000000187);

	address pool1 = address(0x3B998BA87b11a1c5BC1770dE9793B17A0dA61561);
	address pool2 = address(0x5ddb92A5340FD0eaD3987D3661AfcD6104c3b757);
	...
	function name() external view override returns (string memory) {
		return
			string(
				abi.encodePacked('Beethoven_USDC_Boosted_Pool')
			);
	}
	...
	function investRewards() internal {
		sellRewards();
	...
	function balanceOfPooled() public view returns (uint256) {
		uint256 bptUsdPrice = IBalancerPool(pool2).getRate();
		uint256 bptAmount = balanceOfBpt().add(balanceOfBptInMasterChef());
		if (isExact) {
			uint256 linearPoolBptPrice = IBalancerPool(pool1).getRate();
			return bptAmount.mul(bptUsdPrice).mul(linearPoolBptPrice).div(1e36).div(1e12);
		} else {
	...
	function exitPoolExactBpt(uint256 _bpts) internal {
		if (_bpts > 0) {
			IBalancerVault.BatchSwapStep[] memory steps = new IBalancerVault.BatchSwapStep[](2);
			steps[0] = IBalancerVault.BatchSwapStep(pool2Id, 0, 1, _bpts, abi.encode(0));
			steps[1] = IBalancerVault.BatchSwapStep(pool1Id, 1, 2, 0, abi.encode(0));
			IAsset[] memory setAssets = new IAsset[](3);
			setAssets[0] = IAsset(address(pool2));
			setAssets[1] = IAsset(address(pool1));
			setAssets[2] = IAsset(address(want));
	...
	function exitPoolExactToken(uint256 _amountTokenOut) internal {
		...
		IBalancerVault.BatchSwapStep[] memory steps = new IBalancerVault.BatchSwapStep[](2);
		steps[0] = IBalancerVault.BatchSwapStep(pool1Id, 1, 0, _amountTokenOut, abi.encode(0));
		steps[1] = IBalancerVault.BatchSwapStep(pool2Id, 2, 1, 0, abi.encode(0));
		...
		int256 totalBPT = int256(balanceOfBptInMasterChef().add(balanceOfBpt()));
		int256[] memory limits = new int256[](3);
		limits[2] = totalBPT;
		balancerVault.batchSwap(
	...
	function joinPool(uint256 _amountIn) internal returns (bool _joined) {
		if (_amountIn > dust) {
			...
			IBalancerVault.BatchSwapStep[] memory batchSwapStep = new IBalancerVault.BatchSwapStep[](2);
	...
	function setIsBalanceOfPooledExact(bool _isExact) external onlyVaultManagers {
		isExact = _isExact;
	}
