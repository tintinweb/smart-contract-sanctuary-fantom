...
contract DepositDistributor is BaseDistributor {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public devShareBP = 1000;
    uint256 public buybackShareBP = 1000;
    ...
    event UpdateFeeSettings(address indexed user, uint256 devShareBP, uint256 buybackShareBP);
    event UpdateFeeReceiver(address indexed user, address feeReceiver);
    event UpdateDevWallet(address indexed user, address devWallet, address buybackWallet);

    constructor(address _operatorAddress) BaseDistributor(_operatorAddress) {
    }

    function distributeFees(address outputToken) external onlyAdmins {
        uint256 balance = IERC20(outputToken).balanceOf(address(this));

        uint256 devAmount = balance.mul(devShareBP).div(10000);
        uint256 buybackAmount = balance.mul(buybackShareBP).div(10000);
        uint256 receiverAmount = balance.sub(devAmount).sub(buybackAmount);

        if(address(feeReceiver) != address(0)){
            IERC20(outputToken).safeIncreaseAllowance(address(feeReceiver), receiverAmount);
            feeReceiver.income(receiverAmount);
        }

        if(devAmount != 0 && address(devWallet) != address(0)){
            safeTokenTransfer(IERC20(outputToken), devWallet, devAmount);
        }

        if(buybackAmount != 0 && address(buybackWallet) != address(0)){
            safeTokenTransfer(IERC20(outputToken), buybackWallet, buybackAmount);
        }
    }

    function updateFeeSettings(uint256 _devShareBP, uint256 _buybackShareBP) external onlyOwner {
        devShareBP = _devShareBP;
        buybackShareBP = _buybackShareBP;
        emit UpdateFeeSettings(msg.sender, devShareBP, buybackShareBP);
    }

    function updateFeeReceiver(address _feeReceiver) external onlyOwner {
        feeReceiver = IFeeReceiver(_feeReceiver);
        emit UpdateFeeReceiver(msg.sender, _feeReceiver);
    }

    function updateDevWallet(address _devWallet, address _buybackWallet) external onlyOwner {
        devWallet = _devWallet;
        buybackWallet = _buybackWallet;
        emit UpdateDevWallet(msg.sender, _devWallet, _buybackWallet);
    }
}
...
abstract contract BaseDistributor is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    event SetOperatorAddress(address indexed user, address newAddr);

    struct SellTokenData {
        address router;
        address inputToken;
        address outputToken;
        address[] path;
    ...
    modifier onlyAdmins(){
        require(msg.sender == operatorAddress || msg.sender == owner(), "onlyAdmins: FORBIDDEN");
        _;
    }

    function _removeLiquidity(address router, address lpToken) internal {
    ...
    function removeLiquidity(address router, address lpToken) external onlyAdmins {
        _removeLiquidity(router, lpToken);
    }

    function BulkRemoveLiquidity(address router, address[] memory lpTokens) external onlyAdmins {
    ...
    function _sellTokens(address router, address inputToken, address outputToken, address[] memory path) internal {
    ...
    function sellTokens(address router, address inputToken, address outputToken, address[] memory path) external onlyAdmins {
        _sellTokens(router, inputToken, outputToken, path);
    }

    function bulkSellTokens(SellTokenData[] calldata data) external onlyAdmins {
        uint256 length = data.length;
        for (uint256 index = 0; index < length; ++index) {
            SellTokenData calldata d = data[index];
            _sellTokens(d.router, d.inputToken, d.outputToken, d.path);
        }
    }
    ...
    function setOperatorAddress(address newAddr) external onlyAdmins {
        operatorAddress = newAddr;
        emit SetOperatorAddress(msg.sender, newAddr);
    }
}
