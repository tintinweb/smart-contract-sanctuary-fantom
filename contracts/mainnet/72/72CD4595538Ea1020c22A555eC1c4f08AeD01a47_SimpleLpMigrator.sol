...
import "../lplocker/CryptExLpTokenLocker.sol";
import "@openzeppelin/contracts/math/SafeMath.sol";
import "./UniswapV2Library.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract SimpleLpMigrator is IMigrator, Ownable {
    using SafeMath for uint256;

    CryptExLpTokenLockerV3 public locker;
	address public lpReceiver;

    constructor (CryptExLpTokenLockerV3 _locker, address _lpReceiver) {
		require(address(_locker) != address(0), "zero locker address");
	...
	function changeReceiverAddress(address _newAddress) external onlyOwner {
		...
		lpReceiver = _newAddress;
	}

    function migrate(address lpToken, uint256 amount, uint256 unlockTime, address owner) override external {
        require(msg.sender == address(locker), "Migrator: only Locker can call it");
        require(IERC20(lpToken).transferFrom(address(locker), lpReceiver, amount));
    }
