 ...
 * @author Wagmi, inspired by the Openzeppelin ERC20 implementation
 **/
abstract contract IncentivizedERC20 is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
...
/**
 * @title Wagmi ERC20 WToken
 * @dev Implementation of the interest bearing token for the Wagmi protocol
 * @author Wagmi
 */
contract WToken is IncentivizedERC20, IWToken, IERC20PermitUpgradeable, EIP712Upgradeable {
  using CountersUpgradeable for CountersUpgradeable.Counter;
   ...
   * @dev Initializes the wToken
   * @param pool The address of the lending pool where this wToken will be used
   * @param treasury The address of the treasury, receiving the fees on this wToken
   * @param underlyingAsset The address of the underlying asset of this wToken (E.g. WETH for aWETH)
   * @param wTokenDecimals The decimals of the wToken, same as the underlying asset"s
   * @param wTokenName The name of the wToken
   * @param wTokenSymbol The symbol of the wToken
  ...
  function initialize(
    ...
    IncentivizedERC20.initialize(pool, underlyingAsset, wTokenDecimals, wTokenName, wTokenSymbol);
    __EIP712_init(wTokenName, "1");
    _treasury = treasury;
  }
   ...
   * @param index The new liquidity index of the reserve
   **/
  function burn(address user, address receiverOfUnderlying, uint256 amount, uint256 index) external onlyLendingPool {
   ...
   **/
  function balanceOf(address user) public view override(IncentivizedERC20, IERC20Upgradeable) returns (uint256) {
  ...
  /**
   * @dev Returns the address of the Wagmi treasury, receiving the fees on this wToken
   **/
  ...
  /**
   * @dev Returns the address of the lending pool where this wToken is used
   **/
   ...
   * @dev Transfers the underlying asset to `target`. Used by the LendingPool to transfer
   * assets in borrow(), withdraw() and flashLoan()
   * @param target The recipient of the wTokens
   * @param amount The amount getting transferred
   * @return The amount transferred
   **/
  function transferUnderlyingTo(address target, uint256 amount) external onlyLendingPool returns (uint256) {
   ...
   * @param amount The amount getting repaid
   **/
  function handleRepayment(address user, uint256 amount) external onlyLendingPool {}

  /**
   * @dev Transfers the wTokens between two users. Validates the transfer
