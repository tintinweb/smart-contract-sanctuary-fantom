...
contract fantom2022win {
	using SafeMath for uint256;
    using SafeMath for uint8;

	uint256 constant public INVEST_MIN_AMOUNT = 2 ether; // 2 Fantom
    ...
    uint256 constant public NFT_BOXES_STEP = 1000 ether; //1000 Fantom
    ...
    function getBoxesByDepositAmount(uint8 plan, uint256 depositedAmount) public pure returns(uint256) {
        if(depositedAmount >= NFT_BOXES_STEP) {
            ...
            if(plan == 3) {
                uint256 boxes =  depositedAmount.div(NFT_BOXES_STEP); // 1 box per 1000 Fantom
            ...
            if(plan == 4) {
                uint256 boxes = depositedAmount.div(NFT_BOXES_STEP).mul(2); // 2 boxes per 1000 Fantom
            ...
            if(plan == 5) {
                uint256 boxes = depositedAmount.div(NFT_BOXES_STEP).mul(3); // 3 boxes per 1000 Fantom
