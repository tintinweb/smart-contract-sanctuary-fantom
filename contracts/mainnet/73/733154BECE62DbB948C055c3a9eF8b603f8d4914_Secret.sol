...
contract Secret is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 23500 ether;
    ...
    /**
     * @notice Constructs the SECRET ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SECRET", "SECRET") {
        // Mints 500 SECRET to contract creator for initial pool setup
    ...
    function _getSecretPrice() internal view returns (uint256 _secretPrice) {
        try IOracle(secretOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Secret: failed to fetch SECRET price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _secretPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_secretPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSecretOracle(address _secretOracle) public onlyOperatorOrTaxOffice {
        require(_secretOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SECRET to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SECRET to mint to
    ...
    function transferFrom(
        ...
        if (recipient == pair && takeFee) if (recipient == pair && taxRate > 0) {
        ...
        uint256 currentTaxRate = 200; //2% tax initial;
        bool burnTax = false;

        if (autoCalculateTax) {
            uint256 currentSecretPrice = _getSecretPrice();
            currentTaxRate = _updateTaxRate(currentSecretPrice);
            if (currentSecretPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_secretPool != address(0), "!_secretPool");
