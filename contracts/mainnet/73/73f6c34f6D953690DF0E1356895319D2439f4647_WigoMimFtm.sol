...
// File: contracts/interfaces/IWigoChef.sol
...
contract WigoMimFtm is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wrapped = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public constant mim = address(0x82f0B8B456c1A451378467398982d4834b6829c1);
    ...
    // Routes
    address[] public wigoToWrappedPath;
    address[] public wrappedToMimPath;
    ...
    event SetWigoToWrappedPath(address[] indexed path);
    event SetWrappedToMimPath(address[] indexed path);
    ...
    constructor(
        ...
        wigoToWrappedPath = [wigo, wrapped];
        wrappedToMimPath = [wrapped, mim];
    ...
    function _deposit() internal whenNotPaused {
        ...
        IWigoChef(masterchef).deposit(poolId, wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IWigoChef(masterchef).withdraw(poolId, _amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        if (caller != vault){
         ...
         IWigoChef(masterchef).deposit(poolId, 0);
        uint256 outputBal = IERC20(wigo).balanceOf(address(this));
    ...
    function chargeFees(address caller) internal {                   
        uint256 toWrapped = IERC20(wigo).balanceOf(address(this));

        IUniSwapRouter(unirouter).swapExactTokensForTokens(toWrapped, 0, wigoToWrappedPath, address(this), block.timestamp);                                                
    ...
    function addLiquidity() internal {
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToMimPath, address(this), block.timestamp);
    
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(mim).balanceOf(address(this));

        IUniSwapRouter(unirouter).addLiquidity(wrapped, mim, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }

     /** @dev Returns rewards unharvested */
    function rewardBalance() public view returns (uint256) {
        return IWigoChef(masterchef).pendingWigo(poolId, address(this));
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        if (outputBal > 0) {
            uint256[] memory amountsOut = IUniSwapRouter(unirouter).getAmountsOut(outputBal, wigoToWrappedPath);
            wrappedOut = amountsOut[amountsOut.length -1];
        }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IWigoChef(masterchef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IWigoChef(masterchef).emergencyWithdraw(poolId, address(this));
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external onlyAdmin {
        require (_path[0] != wrapped && _path[_path.length - 1] == wigo, "Bad path");

        IERC20(_path[0]).safeApprove(unirouter, type(uint).max);
    ...
    function setOutputToWrappedPath(address[] calldata _path) external onlyAdmin {
        require(_path[0] == wigo && _path[_path.length - 1] == wrapped, "Bad path || !auth");

        wigoToWrappedPath = _path;

        emit SetWigoToWrappedPath(_path);
    }

    /** @dev Modular function to set the transaction route of LP token 0 */
    function setWrappedToMimPath(address[] calldata _path) external onlyAdmin {
        require (_path[0] == wrapped && _path[_path.length - 1] == mim, "Bad path || !auth");

        wrappedToMimPath = _path;

        emit SetWrappedToMimPath(_path);
    }
