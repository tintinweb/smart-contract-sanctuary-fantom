...
contract MultiAssetTreasury is IMultiAssetTreasury, Operator, ReentrancyGuard {
    ...
    //TODO create struct, mapping with assetId
    ...
    function initializing(address _share, address _oracleShare, address _collateralFund, address _daoFund, address _assetController) external onlyOperator {
    ...
    //TODO get from assetController => Done
    ...
    )
    {
        ...
        return (assetPrice(_assetId), sharePrice(), assetTotalSupply, assetTcr(_assetId), assetEcr(_assetId), globalCollateralValue(_assetId), minting_fee, redemption_fee);
    }
    ...
    function globalCollateralBalance(uint256 _assetId) public view override returns (uint256) {
        ...
        uint256 _collateralReserveBalance = IERC20(_collateral).balanceOf(collateralFund);
        return _collateralReserveBalance - totalUnclaimedBalance(_assetId);
    }

    //TODO add assetId => done
    function globalCollateralValue(uint256 _assetId) public view override returns (uint256) {
    ...
    function calcEffectiveCollateralRatio(uint256 _assetId) public view returns (uint256) {
        ...
        uint256 total_collateral_value = globalCollateralValue(_assetId);
        uint256 total_supply_dollar = IAssetController(assetController).getAssetTotalSupply(_assetId);
    ...
    //TODO add for loop to refresh all => done
