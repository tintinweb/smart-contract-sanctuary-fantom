...
interface IUnifiAutoController {
    ...
    function feeOn(address _pool) external view returns(bool);
    function owner() external returns(address);
    
}

contract AutoRouter is IUnifiRouter02 {
    using SafeMath for uint;

    address public owner ;
    address public immutable override factory;
    address public immutable override WETH;
    IUnifiAutoController public iuc ;
    modifier ensure(uint deadline) {
       ...
       constructor(address _factory,address _weth , address _iuc) public{
        factory = _factory;
        WETH = _weth;
        iuc = IUnifiAutoController(_iuc);
        owner = msg.sender;
    }

    function setUC(address _uc) public {
        if(msg.sender == owner){
            iuc = IUnifiAutoController(_uc);
        }
    }
