...
contract CrimeGoldV5 is Initializable, ERC20Upgradeable, PausableUpgradeable {
  ...
  address constant fortunewheel = 0x92f70e35c6b07dE0fF184C829407294B0859Ba5b;

  uint256 constant public _taxPercentageForDevs = 50; // 50 == 5%
  uint256 constant public _taxPercentageForLiquidity = 25; // 25 == 2.5%
  uint256 constant public _taxPercentageForBurn = 25; // 25 == 2.5%
  address constant public _devAddress = 0x73509341dc0DeBbBCb65C0AC8B1EDe7133932A8A; // dev address
  ...
  function burn(uint256 amount) external whenNotPausedExceptGovernance {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal override {
    ...
    uint256 amountForDevs = 0;
    uint256 amountForLiquidity = 0;
    uint256 amountForBurn = 0;

    if(!_inSwapAndLiquify && (pancakePair == recipient || _isIncludeAddressPairBurn[recipient]) && !_isExcludedFromBurn[sender]) {
      amountForDevs = amount.mul(_taxPercentageForDevs).div(10**3); // 5% - devs
      amountForLiquidity = amount.mul(_taxPercentageForLiquidity).div(10**3); // 2.5% - liquidity
      amountForBurn = amount.mul(_taxPercentageForBurn).div(10**3); // 2.5% - burn

      _swapAndSendToDevAddress(sender, amountForDevs); // transfer to dev address
      _swapAndLiquify(amountForLiquidity, sender); // swap and liquify
      _burn(sender, amountForBurn); // burn
    }

    super._transfer(sender, recipient, amount.sub(amountForDevs).sub(amountForLiquidity).sub(amountForBurn));
  }
  ...
  function _swapAndSendToDevAddress(address from, uint256 amount) private lockTheSwap {
    super._transfer(from, address(this), amount);

    uint256 initialBalance = IERC20(tokenB).balanceOf(address(this));

    _swapper.swapTokensAForTokensB(amount);

    IERC20(tokenB).transfer(_devAddress, IERC20(tokenB).balanceOf(address(this)) - initialBalance);
  }
