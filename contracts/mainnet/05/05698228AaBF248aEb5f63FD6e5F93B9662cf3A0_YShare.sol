...
import "./Operator.sol";
import "./ERC20/ERC20WhitelistTransferable.sol";
import "./interfaces/IYShare.sol";

contract YShare is ERC20WhitelistTransferable, IYShare, Operator {
    ...
    function lockerBurnFrom(address _address, uint256 _amount) external override onlyLocker {
        burnFrom(_address, _amount);
        emit Burned(_address, msg.sender, _amount);
    }

    // Mint DOLLAR. Can be used by Pool only
    function lockerMintFrom(address _address, uint256 _amount) external override onlyLocker {
        super._mint(_address, _amount);
        emit Minted(msg.sender, _address, _amount);
    }

    function addWhitelistTransferableAddress(address _address) external onlyOperator {
        super.addWhitelistAddress(_address);
 ...
 * allowances. See {IERC20-approve}.
 */
contract ERC20WhitelistTransferable is Context, IERC20 {
	using SafeMath for uint256;
	...
	function addWhitelistAddress(address _address) public virtual {
