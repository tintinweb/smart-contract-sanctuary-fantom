...
contract StrainNFTStakingContract is ERC721Holder {
    ...
    function incS(uint256 x) private pure returns (uint256) {
        unchecked {
            return x + 1;
        }
    }
    ...
    function claimReward() public {
        uint256 stakingReward = userRewardHistory[msg.sender];

        require(stakingReward > 0, "No Reward");

        userRewardHistory[msg.sender] = 0;
        emit RewardClaimed(msg.sender, stakingReward, block.timestamp);
