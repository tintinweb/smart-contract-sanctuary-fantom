...
contract Referral {
    ...
    uint256 public referralPercentage=20;//percentage of the borrow fee that goes to the affiliate
    uint256 public numLevels=2;//how many tiers to calculate referral fee allocations
    ...
    mapping (address => bool) public hasReferee;
    /// @notice Only ParanormalSafes can established affiliates
    mapping (address => bool) public canEstablishedAffiliate;
    ...
    function set_promo_code_ownership(string calldata new_promo_code) public{
        require(bytes(new_promo_code).length>0, "Invalid code");
    ...
    /// @notice PERMANENTLY establishes a referrer connection
    /// @param user The user that is signing up
    /// @param promo_code_used The promo code that is used when signing up (if any)
    function establishPromoLink(address user, string calldata promo_code_used) public{
        //Needs to fail silently
        if(canEstablishedAffiliate[msg.sender]==false){
            return;
        }
        if(bytes(promo_code_used).length==0){//No referral code
            return;
        }
        if(hasReferee[user]==true){//Can't sign up with someone else's referral code if already has a referee of his own
            return;
        }
        ...
        hasReferee[affliate_owner]=true;
...
/////////////////////////////////////////////////////////////////////////////Utility Functions////////////////////////////////////////////////////
    function setCanEstablishedAffiliate(address vaultAddress, bool yes_or_no) external{
        require(owner==msg.sender,"Only owner");
        canEstablishedAffiliate[vaultAddress]=yes_or_no;
    }
    function getNumLevels() public view returns(uint256){
        return numLevels;
    }
    function setNumLevels(uint256 newNumLevels) external{
        require(owner==msg.sender,"Only owner");
        require(newNumLevels>=1,"At least 1 referral level");
    ...
    /// @notice Utility function used to fetch the address that created a specific promo code
    /// @param user The user
    function findReferrerPermanentLink(address user)public view returns(address){
