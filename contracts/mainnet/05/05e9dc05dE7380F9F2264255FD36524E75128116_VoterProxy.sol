    ...
    ProxyImplementation
{
    ...
    mapping(address => bool) internal claimDisabledByOxPoolAddress;

    // Records solid stored in voterProxy for an oxPool
    mapping(address => uint256) public solidStoredForOxPool;

    // Migration
    address public voterProxyAssetsAddress;
    IVoterProxyAssets internal voterProxyAssets;
    address public voterProxyTargetAddress;
    mapping(address => bool) public solidPoolMigrated;
    ...
    {
        // Susetting
        require(false, "Sunsetting on Fantom"); // Sunsetting on Fantom, only withdrawals are allowed

        // Cannot deposit nothing
        require(amount > 0, "Nothing to deposit");

        // oxPool has transferred LP to VoterProxy...

        // Find gauge address
        address gaugeAddress = voter.gauges(solidPoolAddress);
        ISolidPool solidPool = ISolidPool(solidPoolAddress);
        solidPool.transfer(voterProxyAssetsAddress, amount);

        // Claim solid on every interaction if possible
        bool solidClaimed = claimSolid(msg.sender);

        // Deposit all LP if solid is claimed, withdraw all if not
        if (solidClaimed) {
            // Deposit Solidly LP into gauge
            voterProxyAssets.depositInGauge(solidPoolAddress, gaugeAddress);
        } else {
            uint256 gaugeBalance = ISolidGauge(gaugeAddress).balanceOf(
            ...
            if (gaugeBalance > 0) {
                voterProxyAssets.withdrawFromGauge(
                    solidPoolAddress,
                    gaugeAddress,
                    ISolidGauge(gaugeAddress).balanceOf(voterProxyAssetsAddress)
                );
            }
        }
    }
        ...
        voterProxyAssetsSet
    {
        ...
        /**
         * Claim solid on every interaction if possible
         * In some cases it's not possible to claim due to an unbounded for loop in Solidy gauge.getReward
         */
        bool solidClaimed = claimSolid(msg.sender);

        if (!solidPoolMigrated[solidPoolAddress]) {
            migrateLp(solidPoolAddress);
        }

        // Only withdraw from gauge if solidClaimed, otherwise enter caching mode
        if (solidClaimed) {
            // If there's LP in voterProxyAssets, it means we just got back from caching mode
            // deposit all except the withdrawal amount

            if (solidPool.balanceOf(voterProxyAssetsAddress) > 0) {
                solidPool.transferFrom(
                    voterProxyAssetsAddress,
                    msg.sender,
                    amount
                );
                // voterProxyAssets.depositInGauge(solidPoolAddress, gaugeAddress); // Disabled because sunsetting
            } else {
                voterProxyAssets.withdrawFromGauge(
                    solidPoolAddress,
                    gaugeAddress,
            ...
            solidPool.transferFrom(voterProxyAssetsAddress, msg.sender, amount);
        }
    }

    /**
     * @notice Pokes LP into gauge if gauge is sync'd
     * @param oxPoolAddress Address of oxPool to poke
     * @param batchAmount Amount of checkpoints to sync
     */
    function pokeGauge(address oxPoolAddress, uint256 batchAmount)
        external
        voterProxyAssetsSet
    {
        // Find addresses
        ...
        // Determine lag
        uint256 lag = bribeSupplyLag(gaugeAddress, solidAddress);
        ...
        // Batch checkpoints
        batchCheckPointOrGetReward(gaugeAddress, solidAddress, batchAmount);

        // Claim solid on every interaction if possible
        bool solidClaimed = claimSolid(oxPoolAddress);

        // // Deposit all LP if solid is claimed
        // if (solidClaimed) {
        //     // Deposit Solidly LP into gauge
        //     voterProxyAssets.depositInGauge(solidPoolAddress, gaugeAddress); // Disabled because sunsetting
        // }
        notifySolidRewards(oxPoolAddress);
    }
        ...
        returns (bool allClaimed)
    {
        ...
        if (allClaimed) {
            ...
            // doing this because tax-on-transfer tokens brick solidly's fee contracts
        ...
        returns (bool _claimSolid)
    {
        ...
        _claimSolid = (distributed &&
            _batchCheckPointOrGetReward(gaugeAddress, solidAddress));

        if (_claimSolid) {
            // Claim SOLID via voterProxyAssets
            uint256 amountClaimed = voterProxyAssets.claimSolid(gaugeAddress);
            // Record SOLID claimed
            solidStoredForOxPool[oxPoolAddress] = solidStoredForOxPool[
                oxPoolAddress
            ].add(amountClaimed);

            if (
                solidStoredForOxPool[oxPoolAddress] >
                tokensAllowlist.notifySolidThreshold()
            ) {
                notifySolidRewards(oxPoolAddress);
            }
        }
    ...
    /**
     * @notice Notify solid rewards for an oxPool
     * @param oxPoolAddress the oxPool to nottify rewards for
     */
    function notifySolidRewards(address oxPoolAddress) public {
        // auth to prevent legacy pools from claiming but without reverting
        require(
            IOxPoolFactory(oxPoolFactoryAddress).isOxPool(oxPoolAddress),
            "Not an oxPool"
        );
        ...
        uint256 _solidEarned = solidStoredForOxPool[oxPoolAddress];

        solidStoredForOxPool[oxPoolAddress] = 0;

        IERC20(solidAddress).safeTransferFrom(
            voterProxyAssetsAddress,
            rewardsDistributorAddress,
            _solidEarned
        );
    ...
    function getRewardFromBribe(
        ...
        (allClaimed, claimed) = _getRewardFromSolidly(
            oxPoolAddress,
            _tokensAddresses,
            true
        );
    ...
    /**
     * @notice Fetch reward from Solidly bribe or gauge
     * @param oxPoolAddress Address of oxPool contract
     * @param _tokensAddresses Tokens to fetch rewards for
     * @param fromBribe getting from bribe rather than gauge
     */
    function _getRewardFromSolidly(
        address oxPoolAddress,
        address[] memory _tokensAddresses,
        bool fromBribe
    )
        ...
        returns (bool allClaimed, bool[] memory claimed)
    {
        ...
        if (fromBribe) {
            ISolidlyLens.Pool memory solidPoolInfo = oxPool.solidPoolInfo();
            _gaugeOrBribeAddress = solidPoolInfo.bribeAddress;
        } else {
            _gaugeOrBribeAddress = oxPool.gaugeAddress();
        }
     ...
     * @dev This method is important because if we don't do this Solidly claiming can be bricked due to gas costs
     */
    ...
    /**
     * @notice checks whether claiming solid can be done within block gas limit, returns false if it will run out-of-gas
        ...
        returns (uint256 lag)
    {
        ...
        ISolidGauge gauge = ISolidGauge(gaugeAddress);
    ...
    /**
     * @notice Whitelist a token on Solidly
    ...
    function detachNFT(uint256 startingIndex, uint256 range)
        ...
        onlyGovernance
    {
        IOxPoolFactory oxPoolFactory = IOxPoolFactory(oxPoolFactoryAddress);

        // calc endIndex, compare to existing oxPoolsLength
        uint256 endIndex = startingIndex.add(range);
        uint256 oxPoolsLength = oxPoolFactory.oxPoolsLength();
        if (endIndex > oxPoolsLength) {
            endIndex = oxPoolsLength;
        }

        // operation for each oxPool
        for (uint256 i = startingIndex; i < endIndex; i++) {
            // get gauge via oxPool
            ISolidGauge gauge = ISolidGauge(
                IOxPool(oxPoolFactory.oxPools(i)).gaugeAddress()
            );
        ...
        onlyGovernance
    {
        ...
        IERC20 solid = IERC20(solidAddress);
        ...
        solid.transfer(
            ...
            solid.balanceOf(address(this))
        );
    }

    function migrateLp(address solidPoolAddress) public voterProxyAssetsSet {
        ...
        ISolidGauge(gaugeAddress).withdraw(gaugeBalance);

        // TODO: make sure balance increased

        // Find total LP amount in voter proxy
        ISolidPool solidPool = ISolidPool(solidPoolAddress);
        uint256 totalBalance = solidPool.balanceOf(address(this));

        // Transfer LP tokens to voterProxyAssets
        solidPool.transfer(voterProxyAssetsAddress, totalBalance);
         ...
         */
        solidPoolMigrated[solidPoolAddress] = true;
    }
    ...
    function setClaimDisabledOxByPoolAddress(address poolAddress, bool disabled)
        public
        onlyGovernance
    {
        claimDisabledByOxPoolAddress[poolAddress] = disabled;
    }
    ...
    function lockSolid(uint256 amount) external {
        ...
        solid.transferFrom(msg.sender, voterProxyAssetsAddress, amount);
        voterProxyAssets.lockSolid(amount);
    ...
    function solidEarned(address gaugeAddress) public view returns (uint256) {
        return
            IGauge(gaugeAddress).earned(solidAddress, voterProxyAssetsAddress);
    }
}
...
interface IVoterProxyAssets {
    ...
    function depositInGauge(address solidPoolAddress, address gaugeAddress)
        external;
    ...
    function claimSolid(address gaugeAddress)
        external
    ...
    function approveSolidToVoterProxy() external;
...
interface IVoterProxy {
    ...
    function voterProxyAssetsAddress()
        external
        view
        returns (address voterProxyAssetsAddress);
}
