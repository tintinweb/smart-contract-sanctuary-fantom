...
contract UnoswapRouter is EthReceiver, Permitable {
    ...
    uint256 private constant _WETH = 0x00000000000000000000000021be370D5312f44cB42ce377BC9b8a0cEF1A4C83;
...
contract UnoswapV3Router is EthReceiver, Permitable, IUniswapV3SwapCallback {
    ...
    function uniswapV3SwapCallback(
        ...
        assembly {  // solhint-disable-line no-inline-assembly
            ...
            let emptyPtr := mload(0x40)
            let resultPtr := add(emptyPtr, 21)

            mstore(emptyPtr, _SELECTORS)
            if iszero(staticcall(gas(), caller(), emptyPtr, 0x4, resultPtr, 0x20)) {
                reRevert()
            }
            if iszero(staticcall(gas(), caller(), add(emptyPtr, 0x4), 0x4, add(resultPtr, 32), 0x20)) {
                reRevert()
            }
            if iszero(staticcall(gas(), caller(), add(emptyPtr, 0x8), 0x4, add(resultPtr, 64), 0x20)) {
                reRevert()
            }
            ...
            default {
                token := mload(add(resultPtr, 32))
                amount := amount1Delta
            }

            mstore(emptyPtr, _FF_FACTORY)
            mstore(resultPtr, keccak256(resultPtr, 96)) // Compute the inner hash in-place
            ...
            case 1 {
                ...
                mstore(add(emptyPtr, 16), caller())
                mstore(add(emptyPtr, 48), amount)
                if iszero(call(gas(), token, 0, add(emptyPtr, 12), 68, 0, 0)) {
                    reRevert()
                }
            }
            default {
                // token.safeTransferFrom(payer, msg.sender, amount);
                mstore(add(emptyPtr, 20), payer)
                mstore(add(emptyPtr, 52), caller())
                mstore(add(emptyPtr, 84), amount)
                if iszero(call(gas(), token, 0, add(emptyPtr, 16), 100, 0, 0)) {
                    reRevert()
                }
            }
        }
