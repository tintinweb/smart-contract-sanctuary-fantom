...
contract FusionIncomeStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public serviceFee = 100;   // 1% service fee 100 / 10000 * 100
    ...
    constructor(address _stakingToken) {
        ...
        TimeLock memory timeLock1 = TimeLock(182.625 days, 250);
        timeLocks.push(timeLock1);
        TimeLock memory timeLock2 = TimeLock(365.25 days, 499);
        timeLocks.push(timeLock2);
        TimeLock memory timeLock3 = TimeLock(730.5 days, 998);
        timeLocks.push(timeLock3);
        TimeLock memory timeLock4 = TimeLock(1095.75 days, 1498);
        timeLocks.push(timeLock4);
        TimeLock memory timeLock5 = TimeLock(1461 days, 1997);
        timeLocks.push(timeLock5);
        TimeLock memory timeLock6 = TimeLock(1826.25 days, 2496);
    ...
    function setServiceFee (uint256 _serviceFee) external onlyOwner nonReentrant {
        require(_serviceFee < 1000, "Too high");
    ...
    function _getRewardAmount(uint256 amount, uint256 period, uint256 timeLockId) internal view returns (uint256) {
        return amount.mul(period).mul(timeLocks[timeLockId].apr).div(ONE_YEAR).div(percentRate);
    }

    // claim reward by deposit id
    function claimReward(uint8 timeLockId, uint256 id) external nonReentrant {
    ...
    function claimAllRewardOfTimeLock(uint8 timeLockId) external nonReentrant {
        ...
        for(uint256 i = 0; i < stakeUser.itemIds.length(); i ++) {
            ...
            uint256 stakedAmount = stakeUser.stakeItems[itemId].amount;
            if (stakedAmount > 0) {
                allClaimableReward += _getRewardAmount(
                    stakedAmount,
                    block.timestamp - stakeUser.stakeItems[itemId].depositAt,
                    timeLockId
                );
    ...
    function _getClaimableReward(uint8 timeLockId, uint256 id, address investor) internal view returns (uint256) {
        ...
        return _getRewardAmount(stakeItem.amount, stakingTime, timeLockId);
    }
    ...
    // withdraw capital by timelock id
    function withdrawAll(uint8 timeLockId) public nonReentrant {
        ...
        while (loopValue < stakeUser.itemIds.length()) {
            ...
            if (block.timestamp - stakeItem.depositAt > timeLocks[timeLockId].period && stakeItem.amount > 0) {
                claimableReward = _getRewardAmount(stakeItem.amount, block.timestamp - stakeItem.depositAt, timeLockId);
    ...
    function withdraw(uint8 timeLockId, uint256 id) public nonReentrant {
        ...
        require (block.timestamp - stakeItem.depositAt > timeLocks[timeLockId].period, "withdraw lock time is not finished yet");

        uint256 claimableReward = _getRewardAmount(stakeItem.amount, block.timestamp - stakeItem.depositAt, timeLockId);
        ...
        // transfer capital to the user
        uint256 totalAmount = stakeItem.amount + claimableReward;

        uint256 feeAmount = totalAmount * serviceFee / percentRate;
        
        if (feeAmount > 0) {
            totalAmount = totalAmount - feeAmount;
            stakingToken.safeTransfer(feeReceiver, feeAmount);
        }
        stakingToken.safeTransfer(msg.sender, totalAmount);
    ...
    function getRecentReward(uint8 timeLockId, address investor) public view returns (uint256 amount, uint256 reward, uint256 remainedTime, uint256 itemId) {
        ...
        if (_timeTmp != 0) {
            ...
            reward = _getClaimableReward(timeLockId, _itemId, investor);
    ...
    function getStakingData(uint8 timeLockId, address investor) public view returns (uint256 invests, uint256 availableInvests, uint256 rewards) {
        ...
        for (uint256 i = 0; i < stakeUser.itemIds.length(); i++) {
            ...
            rewards += _getRewardAmount(stakeItem.amount, block.timestamp - stakeItem.depositAt, timeLockId);
            if (block.timestamp - stakeItem.depositAt > timeLocks[timeLockId].period) {
