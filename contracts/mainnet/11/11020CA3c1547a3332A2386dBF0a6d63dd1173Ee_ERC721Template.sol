...
// File contracts/ERC721.MetaSharks/ERC721.MetaSharks.sol

contract ERC721Template is ERC721Enumerable, ERC721Royalty, Ownable, IERC1155Receiver {
    using Strings for uint256;
    using Clones for address;

    event ReceivedERC1155(address operator, address from, uint256 id, uint256 value, bytes _calldata, uint256 gas);
    ...
    // URI settings
    string private BASE_URI = "https://metashark-cdn.pages.dev/schema/";
    ...
    IERC1155 public SHARKS = IERC1155(0x40ded8808A968E1067ABB91E13C888C9A46ba099);

    constructor() ERC721("Meta Sharks FTM", "SHARK") {
        ...
        royaltyReceiver.addPayee(0xb1a9a840E7ED5b97e559aFCA416C566A6a0ad06F, 80); // 80%
        royaltyReceiver.addPayee(_msgSender(), 20); // 20%

        _setDefaultRoyalty(address(royaltyReceiver), 500); // 5.0%
    }
    ...
    function isApproved(address account) public view returns (bool) {
        return SHARKS.isApprovedForAll(account, address(this));
    }
    ...
    function migrate(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 balance = SHARKS.balanceOf(_msgSender(), tokenIds[i]);
            require(balance != 0, string(abi.encodePacked("Missing balance of token ID ", tokenIds[i].toString())));

            SHARKS.safeTransferFrom(_msgSender(), address(this), tokenIds[i], balance, "");
    ...
    function onERC1155Received(
        ...
        emit ReceivedERC1155(operator, from, id, value, data, gasleft());
    ...
    function onERC1155BatchReceived(
        ...
        emit ReceivedERC1155Batch(operator, from, ids, values, data, gasleft());
