...
contract FTMGrowHouse {
    ...
    constructor() {
        ...
        PercsKey[10] = DivPercs(864000, 3000);
        PercsKey[20] = DivPercs(1728000, 3500);
        PercsKey[30] = DivPercs(2592000, 4000);
        PercsKey[40] = DivPercs(3456000, 4500);
        PercsKey[50] = DivPercs(4320000, 5000);
        PercsKey[60] = DivPercs(5184000, 5500);
        PercsKey[70] = DivPercs(6048000, 6000);
        PercsKey[80] = DivPercs(6912000, 6500);
        PercsKey[90] = DivPercs(7776000, 7000);
        PercsKey[100] = DivPercs(8640000, 10000);

        FeesKey[10] = FeesPercs(864000, 20);
        FeesKey[20] = FeesPercs(1728000, 19);
        FeesKey[30] = FeesPercs(2592000, 18);
        FeesKey[40] = FeesPercs(3456000, 17);
        FeesKey[50] = FeesPercs(4320000, 16);
        FeesKey[60] = FeesPercs(5184000, 15);
        FeesKey[70] = FeesPercs(6048000, 14);
        FeesKey[80] = FeesPercs(6912000, 13);
        FeesKey[90] = FeesPercs(7776000, 12);
        FeesKey[100] = FeesPercs(8640000, 10);
        
        // addresses and instanxes in Fantom testnet
        // FTM_BTC_LP = IERC20(0xA7B8510fe299Bc69f8aF42406e1795319a9226d8);
        // BTC = IERC20(0x39968964A55901043368F18Ff14DbE2eE3B648ce);
        ...
        FTM_BTC_LP = IERC20(0xFdb9Ab8B9513Ad9E419Cf19530feE49d412C3Ee3);
        WBTC = IERC20(0x321162Cd933E2Be498Cd2267a90534A804051b11);
    ...
    function stakeStableCoins(uint256 amtx, address ref) public {
        ...
        WBTC.transferFrom(msg.sender, address(this), amtx);
        ...
        uint256 initialLPBalance = FTM_BTC_LP.balanceOf(address(this));

        // add liquidity to spookeyswap
        addLiquidity(otherHalf, newBalance);

        // how much LP should we stake
        uint256 newLPBalance = FTM_BTC_LP.balanceOf(address(this)).sub(initialLPBalance);
    ...
    function stakeNativeCurrencies(address ref) public payable {
        ...
        uint256 initialBTCBalance = WBTC.balanceOf(address(this));

        // swap FTM for BTC
        swapEthForTokens(half, address(this));
      
        // how much BTC should we just swap into?
        uint256 increasedBTCBalance = WBTC.balanceOf(address(this)).sub(initialBTCBalance);

        // capture the contract's current LP balance
        uint256 initialLPBalance = FTM_BTC_LP.balanceOf(address(this));

        // add liquidity to spookeyswap
        addLiquidity(increasedBTCBalance, otherHalf);
    ...
    function swapEthForTokens(uint256 ethAmount, address _to) internal {
        // generate the uniswap pair path of FTM -> BTC
    ...
    function swapTokensForEth(uint256 tokenAmount, address _to) internal {
        ...
        WBTC.approve(address(uniswapV2Router), tokenAmount);    
    ...
    function stake(uint256 amtx, address ref) internal {
        ...
        FTM_BTC_LP.safeTransfer(owner, stakeFee);
    }
    ...
    function withdrawDivs() external returns (uint256 withdrawAmount) {
        ...
        FTM_BTC_LP.safeTransfer(msg.sender, adjustedAmt);
        FTM_BTC_LP.safeTransfer(owner, withdrawFee);
    ...
    function withdrawInitial(uint256 key) external {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ // LESS THAN 10 DAYS STAKED
            ...
            FTM_BTC_LP.safeTransfer(msg.sender, transferAmt);
            FTM_BTC_LP.safeTransfer(owner, minusAmt);
    ...
    function withdrawRefBonus() external {
        ...
        FTM_BTC_LP.safeTransfer(msg.sender, amtz);
    }
