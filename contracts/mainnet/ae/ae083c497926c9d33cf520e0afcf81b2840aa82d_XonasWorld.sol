...
contract XonasWorld is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("XonasWorld", "XC") {
    ...
    function whitelistMembers(address[] memory addresses) public onlyOwner {
    ...
    function whitelistMint() public {
        uint8 isWhitelisted = whitelisted[msg.sender];
        uint8 didMint = whitelistMinted[msg.sender];
        require(isWhitelisted != 0,"ur not on the list homie, check ur address!");
        require(didMint == 0,"u already got a whitelist mint!!");
        require(_tokenIdCounter.current() < maximumSupply);
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
