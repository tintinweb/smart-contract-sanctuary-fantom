/**
 *Submitted for verification at FtmScan.com on 2023-05-16
*/
...
// File: contracts/BIFI/strategies/degens/StrategyJetswapLP.sol
...
contract StrategydKnightswapswapLP is StratManager, FeeManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
 
    // Tokens used
    address constant public wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address constant public usd = address(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
    ...
    uint public compoundingFee = 100;
    ...
    constructor(
        ...
        } else if (lpToken0 == usd) {
            outputToLp0Route = [output, usd];
        ...
        } else if (lpToken1 == usd) {
            outputToLp1Route = [output, usd];
    ...
    function chargeFees() internal {
        uint256 toWbnb = IERC20(output).balanceOf(address(this)).mul(compoundingFee).div(1000);
        IUniswapRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, outputToWftmRoute, address(this), now);

        uint256 wbnbBal = IERC20(wftm).balanceOf(address(this));
    ...
    function changeCompoundingFee(uint _newFee) external onlyManager {
        require(_newFee <= 500, "fee too high");
        compoundingFee = _newFee;
    }
