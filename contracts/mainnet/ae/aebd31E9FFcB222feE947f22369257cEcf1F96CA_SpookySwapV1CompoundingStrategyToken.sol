 ...
 *         reward token (BOO). The compounding happens by calling the gulp function;
 ...
 */
contract SpookySwapV1CompoundingStrategyToken is ERC20, Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	uint256 constant DEFAULT_PERFORMANCE_FEE = 25e16; // 25%
	...
	{
		uint256 _poolLength = MasterChefBooV1(_masterChef).poolLength();
		require(_pid < _poolLength, "invalid pid");
		(_reserveToken,,,) = MasterChefBooV1(_masterChef).poolInfo(_pid);
		_rewardToken = MasterChefBooV1(_masterChef).boo();
	...
	{
		return MasterChefBooV1(masterChef).pendingBOO(pid, address(this));
	}
	...
	{
		(_reserveAmount,) = MasterChefBooV1(masterChef).userInfo(pid, address(this));
