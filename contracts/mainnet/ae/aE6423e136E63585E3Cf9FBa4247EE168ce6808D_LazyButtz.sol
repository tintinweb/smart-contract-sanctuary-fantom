...
contract LazyButtz is ERC721Enumerable, Ownable {

    uint256 public saleOpens = 1644775200;
    uint256 public presaleOpens = 1644775200;
    uint256 public TOKEN_PRICE = 22 ether;
    uint256 public TOKEN_PRICE_PRESALE = 11 ether;
    uint256 public MAX_SUPPLY = 2222;
    mapping(address => bool) public presaleWhitelist;    
    string private BASE_URI = "https://enefte.info/lazybuttz/?token_id=";

    constructor() ERC721("LazyButtz", "BUTT") {}
    
    function mint(uint256 numberOfTokens) external payable  {
        uint256 totalTokens = totalSupply();
        require(block.timestamp >= saleOpens, "Sale not open");
        require(totalTokens + numberOfTokens <= MAX_SUPPLY, "Not enough left");
    ...
    function mintPresale() external payable  {
        uint256 totalTokens = totalSupply();
        require(block.timestamp >= presaleOpens, "Sale not open");
        require(totalTokens < MAX_SUPPLY, "Not enough left");
        require(presaleWhitelist[msg.sender], "not on whitelist");
        require(TOKEN_PRICE_PRESALE <= msg.value, 'missing eth');
        _safeMint(msg.sender, totalTokens+1);
    ...
    function setPreSaleStart(uint256 _time) external onlyOwner {
        presaleOpens = _time;
    }
    ...
    function withdrawBalance(address _wallet) external onlyOwner {
        ...
        payable(_wallet).call{value: balance}("");
        delete balance;
    }
