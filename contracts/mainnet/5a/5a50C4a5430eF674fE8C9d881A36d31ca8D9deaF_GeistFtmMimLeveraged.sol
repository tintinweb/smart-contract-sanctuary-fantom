...
contract GeistFtmMimLeveraged is DefiiWithCustomEnter {
    ...
    IERC20 constant gMIM = IERC20(0xc664Fc7b8487a3E10824Cda768c1d239F2403bBe);
    IERC20 constant variableDebtMIM = IERC20(0xe6f5b2d4DE014d8fa4c45b744921fFDf13f15D4a);

    IGeistPool constant geistPool = IGeistPool(0x9FAD24f572045c7869117160A571B2e50b10d068);
    uint256 constant VARIABLE_INTEREST_RATE_MODE = 2;
    IGeistDataProvider constant dataProvider = IGeistDataProvider(0xf3B0611e2E4D2cd6aB4bb3e01aDe211c3f42A8C3);
    ILendingPoolAddressesProvider constant addressesProvider = ILendingPoolAddressesProvider(
        0x6c793c628Fe2b480c5e6FB7957dDa4b9291F9c9b
    );
    IGeistChefIncentivesController constant incentiveController = IGeistChefIncentivesController(
        0x297FddC5c33Ef988dd03bd13e162aE084ea1fE57
    );

    IGeistChefMultiFeeDistribution constant multiFeeDistributor = IGeistChefMultiFeeDistribution(
        0x49c93a95dbcc9A6A4D8f77E59c038ce5020e82f8
    );

    function hasAllocation() external view returns (bool) {
        return variableDebtMIM.balanceOf(address(this)) > 0;
    }

    function logAccountData() internal view returns(uint8){
        ...
        (
            ...
            ltv,
            healthFactor
        ) = geistPool.getUserAccountData(address(this));
        console.log('totalCollateralETH: %s', totalCollateralETH);
        console.log('totalDebtETH: %s', totalDebtETH);
        console.log('availableBorrowsETH: %s', availableBorrowsETH);
    ...
    function _enterWithParams(bytes memory params) internal override {
        ...
        console.log('Enter with loopCount: %s; ltv: %s', loopCount, ltv);
        require(ltv < 100, "Ltv must be less then 1%");
        IPriceOracle oracle = IPriceOracle(addressesProvider.getPriceOracle());
        MIM.approve(address(geistPool), type(uint256).max);
        uint256 amount = MIM.balanceOf(address(this));
        uint256 price = oracle.getAssetPrice(address(MIM));
        console.log('Price: %s', price);
        logAccountData();
        for (uint8 i = 0; i < loopCount; i++) {
            console.log('Price: %s', price);
            console.log('Deposit: %s', amount);
            logAccountData();
            geistPool.deposit(address(MIM), amount, address(this), 0);
            amount = (((((amount * price) / 1e18) * 1e18) / price) * ltv) / 100;
            console.log('Borrow: %s', amount);
            logAccountData();
            geistPool.borrow(address(MIM), amount, VARIABLE_INTEREST_RATE_MODE, 0, address(this));
        }

        geistPool.deposit(address(MIM), amount, address(this), 0);
        MIM.approve(address(this), 0);
    }
    ...
    function _exit() internal override {
        ...
        assets[0] = address(MIM);
        uint256[] memory amounts = new uint256[](1);
        amounts[0] = variableDebtMIM.balanceOf(address(this));
    ...
    function _claim(bool claimWithPenalty) internal {
        TokenData[] memory gtokens = dataProvider.getAllATokens();
        address[] memory assets = new address[](gtokens.length + 1);
        for (uint8 gIdx = 0; gIdx < gtokens.length; gIdx++) {
            assets[gIdx] = gtokens[gIdx].tokenAddress;
        }
        assets[gtokens.length] = address(variableDebtMIM);

        incentiveController.claim(address(this), assets);

        // for (uint8 i = 0; i < assets.length; i++) {
        //     uint256 gBalance = IERC20(assets[i]).balanceOf(address(this));
        //     if (gBalance > 0 && assets[i] != address(gMIM) && assets[i] != address(variableDebtMIM)) {
        //         _claimIncentive(IERC20(assets[i]));
        //     }
        // }
        ...
        if (claimWithPenalty) {
            ...
            uint256 unlockedBalance = multiFeeDistributor.unlockedBalance(address(this));
            if (unlockedBalance > 0) {
                multiFeeDistributor.withdraw(unlockedBalance);
            }
        }
        for (uint8 i = 0; i < assets.length; i++) {
            if (assets[i] != address(gMIM) && assets[i] != address(variableDebtMIM)) {
                _claimIncentive(IERC20(assets[i]));
            }
        }
        _claimIncentive(GEIST);
    }
    ...
    function executeOperation(
        ...
        require(address(this) == initiator, "This contract must be initiator");
        require(msg.sender == address(geistPool), "Caller must be Geist Pool");

        MIM.approve(address(geistPool), amounts[0]);
        geistPool.repay(
            address(MIM),
            amounts[0],
        ...
        geistPool.withdraw(assets[0], type(uint256).max, address(this));
        MIM.approve(address(geistPool), amounts[0] + premiums[0]);
        return true;
    }
}
...
interface IGeistChefIncentivesController {
    function claim(address _user, address[] calldata _tokens) external;
}

interface IGeistChefMultiFeeDistribution {
