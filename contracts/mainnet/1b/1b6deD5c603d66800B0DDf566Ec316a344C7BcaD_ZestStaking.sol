...
contract ZestStaking is ReentrancyGuard, Ownable {
    ...
    uint256 public constant maximumLockDuration = 86400 * 365 * 2; // 2 years
    ...
    //array of current reward
    mapping(address => Distributor[]) public rewardDistributorsArray;
    ...
    constructor(
        ...
        //Approve the veToken contract
        stakingToken.approve(address(_veZest), 2**256 - 1);
    }
    ...
    function addReward(address _rewardsToken, address _distributor) public onlyOwner {
        ...
        rewardDistributorsArray[_rewardsToken].push(
            Distributor({rewardToken: _rewardsToken, distributor: _distributor})
        );
    ...
    function _getLockedZestBalanceFromNFT(address account) internal view returns (uint256) {
        uint256 numTokens = ve.balanceOf(account);
        if (numTokens == 0) {
            return 0;
        }
        ...
        for (uint256 index = 0; index < numTokens; index++) {
            uint256 tokenId = ve.tokenOfOwnerByIndex(account, index);
            total += ve.locked__amount(tokenId);
        }

        return total;
    }

    function getLockedZestBalanceFromNFT(address account) external view returns (uint256) {
        return _getLockedZestBalanceFromNFT(account);
    }
        ...
        returns (RewardData[] memory _rewards)
    {
        ...
        for (uint256 i = 0; i < _rewards.length; i++) {
            ...
            uint256 balance = i == 0
                ? _getLockedZestBalanceFromNFT(account)
        ...
        )
    {
        ...
        return (_getLockedZestBalanceFromNFT(user), unlockable, locked, lockData);
    }
    ...
    function currentMinters(uint256 index) public view returns (address) {
        return mintersArray[index];
    }

    function currentRewardDistributors(address _rewardToken, uint256 index)
        external
        view
        returns (address _distributor)
    {
        Distributor[] storage dist = rewardDistributorsArray[_rewardToken];
        for (uint256 i = 0; i < dist.length; i++) {
            _distributor = rewardDistributorsArray[_rewardToken][index].distributor;
        }
        return _distributor;
    }
    ...
    function stake(
        uint256 amount,
        bool lock,
        uint256 userDefinedLockDuration
        ...
        if (lock) {
            uint256 duration = userDefinedLockDuration > 0 ? userDefinedLockDuration : lockDuration;
            require(
                duration >= lockDuration && duration <= maximumLockDuration,
                "stake: Lock duration must be more than 56 days and less than 2 years."
            );
            lockedSupply = lockedSupply.add(amount);
            bal.locked = _getLockedZestBalanceFromNFT(msg.sender).add(amount);
            uint256 unlockTime = block.timestamp.div(rewardsDuration).mul(rewardsDuration).add(
                duration
            );
            uint256 idx = userLocks[msg.sender].length;
            //If the lock is the users first
            //or the lock has expired
            //or the lock duration is greather than the default lock duration
            //create a new veZest NFT, so users can create locks with multiple unlock times
            if (
                idx == 0 ||
                userLocks[msg.sender][idx - 1].unlockTime < unlockTime ||
                duration > lockDuration
            ) {
                //User has no active locks
                stakingToken.safeTransferFrom(msg.sender, address(this), amount);
                uint256 newTokenId = ve.create_staking_lock_for(amount, duration, msg.sender);
                userLocks[msg.sender].push(
                    LockedBalance({amount: amount, unlockTime: unlockTime, tokenId: newTokenId})
                );
                emit veZestMinted(msg.sender, amount, unlockTime, newTokenId);
            } else {
                //User already has an active lock within this group and is locking for the minimum duration, so we increase their locked amount
                stakingToken.safeTransferFrom(msg.sender, address(this), amount);
                uint256 tokenId = userLocks[msg.sender][idx - 1].tokenId;
                ve.increase_amount_staking(tokenId, amount);
                userLocks[msg.sender][idx - 1].amount = userLocks[msg.sender][idx - 1].amount.add(
                    amount
                );
            }
        } else {
            //if the user is locking, safeTransfer handles the balance otherwise for veZest, ve handles it.
            bal.unlocked = bal.unlocked.add(amount);
            stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        }
        //ve contract takes the length of the lock in seconds, whereas here we keep the unlock epoch, not duration.
    ...
    function mint(address user, uint256 amount) external updateReward(user) {
        require(minters[msg.sender], "mint: Only minters allowed");
        totalSupply = totalSupply.add(amount);
        Balances storage bal = balances[user];
        bal.total = bal.total.add(amount);
        ...
        //When rewards are locked following a user claiming rewards from the chef, we do not mint a veZest NFT.
        if (idx == 0 || earnings[idx - 1].unlockTime < unlockTime) {
            uint256 nullTokenId = 0;
            earnings.push(
                LockedBalance({amount: amount, unlockTime: unlockTime, tokenId: nullTokenId})
            );
        } else {
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        if (amount <= bal.unlocked) {
            ...
            require(bal.earned >= remaining, "withdraw: Insufficient unlocked balance");
            bal.unlocked = 0;
            ...
            for (uint256 i = 0; ; i++) {
                ...
                if (
                    ...
                    require(
                        ...
                        "withdraw: Insufficient balance after penalty"
                    );
                    bal.earned = bal.earned.sub(remaining);
                    if (bal.earned == 0) {
    ...
    // Withdraw full unlocked balance, without caring about pending rewards
    function emergencyWithdrawWithoutRewards() external updateReward(msg.sender) {
        (uint256 amount, uint256 penaltyAmount) = withdrawableBalance(msg.sender);
        delete userEarnings[msg.sender];
        Balances storage bal = balances[msg.sender];
        bal.total = bal.total.sub(bal.unlocked).sub(bal.earned);
    ...
    function withdrawExpiredLocks() external {
        ...
        uint256 amount;
        uint256 _tokenId;
        uint256 numTokens = ve.balanceOf(msg.sender);
        uint256 length = locks.length;

        uint256[] memory unlockedVe = new uint256[](numTokens);

        // First check if the user has any NFTs that can be unlocked
        //... the user will want to withdraw it, even if the tokenID is not in storage
        if (numTokens >= 1) {
            //User has more than 1 NFT that could be withdrawable
            uint256 total;
            if (numTokens > 1) {
                //Loop through NFTs, find the withdrawable tokenIds
                for (uint256 index = 0; index < numTokens; index++) {
                    _tokenId = ve.tokenOfOwnerByIndex(msg.sender, index);
                    if (ve.locked__end(_tokenId) > block.timestamp || _tokenId == 0) continue;
                    total += ve.locked__amount(_tokenId);
                    unlockedVe[index] = _tokenId;
                }
                //Withdraw the unlocked tokens
                for (uint256 i = 0; i < unlockedVe.length; i++) {
                    if (unlockedVe[i] == 0) continue;
                    ve.withdraw_staking(unlockedVe[i]);
                }
            } else {
                //User only has 1 NFT
                _tokenId = ve.tokenOfOwnerByIndex(msg.sender, 0);
                total = ve.locked__amount(_tokenId);
                ve.withdraw_staking(_tokenId);
            }

            bal.locked = bal.locked.sub(total);
            bal.total = bal.total.sub(total);
            totalSupply = totalSupply.sub(total);
            lockedSupply = lockedSupply.sub(total);
            // Send to the user
            stakingToken.safeTransfer(msg.sender, total);
            return;
        }
        //The tokenId is in storage and we can update their balances held in the contract
        if (locks[length - 1].unlockTime <= block.timestamp) {
            _tokenId = locks[length - 1].tokenId;
            if (numTokens > 1) {
                for (uint256 i = 0; i < numTokens; i++) {
                    //check if the token is unlocked, since we handle transfers only token owners will have lockData
                    if (
                        locks[i].unlockTime > block.timestamp ||
                        locks[i].tokenId == 0 ||
                        ve.locked__end(locks[i].tokenId) > block.timestamp
                    ) continue;
                    unlockedVe[i] = locks[i].tokenId;
                    amount += ve.locked__amount(unlockedVe[i]);
                    delete locks[i];
                }
            } else {
                amount = ve.locked__amount(_tokenId);
            }
        ...
        updateReward(address(0))
    {
        require(
            ...
            "notifyRewardAmount: Only reward distributors allowed"
        );
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(
            ...
            "recoverERC20: Cannot withdraw staking token"
        );
        require(
            rewardData[tokenAddress].lastUpdateTime == 0,
            "recoverERC20: Cannot withdraw reward token"
        );
    ...
    function addMinter(address _newMinter) external onlyOwner {
        require(_newMinter != address(0), "addMinter: Invalid address");
        require(!minters[_newMinter], "addMinter: Minter existed");
    ...
    function removeMinter(address _minter) external onlyOwner {
        require(minters[_minter], "addMinter: Minter not found");
    ...
    function notifyAirdrop(
        address[] calldata users,
        uint256 _amountPerUser,
        uint256 _duration
    ) external {
        require(
            msg.sender == address(airdrop),
            "notifyAirdrop: Only the airdropper can perform an airdrop."
        );
        require(
            _amountPerUser <= 100 ether,
            "notifyAirdrop: Airdrops are only allowed to be 100 ZSP per user."
        );
        require(users.length <= 100, "notifyAirdrop: Maximum airdrop length is 100 users.");
        require(
            _duration <= maximumLockDuration && _duration >= lockDuration,
            "notifyAirdrop: Lock duration must be between the maximum and minimum lock duration."
        );
        totalSupply = totalSupply.add(_amountPerUser * users.length);
        lockedSupply = lockedSupply.add(_amountPerUser * users.length);
        //push the locked balance for each recepient, users can then withdraw as normal from the staking contract
        for (uint256 i = 0; i < users.length; i++) {
            Balances storage bal = balances[users[i]];
            bal.total = bal.total.add(_amountPerUser);
            bal.locked = bal.locked.add(_amountPerUser);
        }
    }
    ...
    function notifyTransfer(
        ...
        require(msg.sender == address(ve), "notifyTransfer: Only the staking contract can notify.");
        require(
            ve.ownerOf(tokenId) == address(_to),
            "notifyTransfer: Recipient must be the owner of the token."
        );
        Balances storage recipientBal = balances[_to];

        LockedBalance[] storage senderLocks = userLocks[_from];
        Balances storage senderBal = balances[_from];

        uint256 lockedAmount = ve.locked__amount(tokenId);
        uint256 unlockTime = ve.locked__end(tokenId);

        for (uint256 i = 0; i < senderLocks.length; i++) {
            if (senderLocks[i].tokenId == tokenId) {
                delete senderLocks[i];
                break;
            }
        }

        //Push the lock info for the recipient
        userLocks[_to].push(
            LockedBalance({amount: lockedAmount, unlockTime: unlockTime, tokenId: tokenId})
        );

        //Decrease the senders balances
        senderBal.locked = senderBal.locked.sub(lockedAmount);
        senderBal.total = senderBal.total.sub(lockedAmount);

        //Increase the recipients balances
        recipientBal.locked = recipientBal.locked.add(lockedAmount);
        recipientBal.total = recipientBal.total.add(lockedAmount);
    }

    function setAirdrop(address _contract) external onlyOwner {
        require(address(airdrop) == address(0), "setAirdrop: Address may only be set once.");
        airdrop = _contract;
    }
    ...
    event veZestMinted(address indexed user, uint256 amount, uint256 unlockTime, uint256 tokenId);
}
...
library WethUtils {
    ...
    // IWETH public constant weth = IWETH(0xf1277d1Ed8AD466beddF92ef448A132661956621); //WFTM testnet

    IWETH public constant weth = IWETH(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83); //WFTM Mainnet
