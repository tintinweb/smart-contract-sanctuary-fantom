 ...
 * @notice Merkle tree fun, plenty of others for this, but added a few touches myself
 */
...
import {MaxRoles} from "./MaxRoles.sol";

contract MerkleDistributor is MaxRoles {
  ...
  constructor(address token_, bytes32 merkleRoot_) {
    ...
    lastCall = block.timestamp + 45 days;
  }
  ...
  function withdraw(
    ...
    onlyDev() {
    if (_token == token && lastCall >= block.timestamp) {
      revert MaxSplaining({
        reason: "Time locked"
      });
    }
    uint256 quant = IERC20(_token).balanceOf(address(this));
    IERC20(_token).safeTransfer(this.owner(), quant);
  }

  function withdraw()
    external
    onlyDev() {
    uint256 quant = address(this).balance;
    Address.sendValue(payable(this.owner()), quant);
  }
 ...
 * -#%[email protected]#-  :@#@@+%[email protected]*@*:=%+..%%#=      *@  *@++##.    =%@%@%%#-  =#%[email protected]#-   :*+**+=: %%++%*
 *
 * @title: [Not an EIP]: MaxFlow's 173/Dev/Roles NON UUPS
 * @author: Max Flow O2 -> @MaxFlowO2 on bird app/GitHub
 */
...
abstract contract MaxRoles is MaxAccess {
