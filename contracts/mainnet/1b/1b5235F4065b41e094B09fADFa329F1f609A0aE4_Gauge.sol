...
contract Gauge is ReentrancyGuard {
    ...
    function derivedBalance(address account) public view returns (uint256) {
        ...
        uint256 _adjusted = (
            _totalSupply.mul(inSPIRIT.balanceOf(account)).div(
                inSPIRIT.totalSupply()
            )
        ).mul(60).div(100);
...
contract GaugeProxy is ProtocolGovernance {
    ...
    uint256 public pid = uint256(-1); // -1 means 0xFFF....F and hasn't been set yet
    ...
    function addGauge(
        ...
        gauges[_tokenLP] = address(
            new Gauge(address(SPIRIT), address(inSPIRIT), _tokenLP)
        );
    ...
    function deposit() public {
        require(pid != uint256(-1), "pid not initialized");
        IERC20 _token = TOKEN;
    ...
    function preDistribute() external hasDistribute {
        ...
        if (ve) {
            ...
            uint256 _inSpiritRewards = _balance.mul(_lockedSpirit).div(
                _spiritSupply
            );
        ...
        if (lockedBalance > 0 && lockedTotalWeight > 0) {
            for (uint256 i = _start; i < _end; i++) {
                ...
                if (!hasDistributed[_token] && gaugeStatus[_token]) {
                    address _gauge = gauges[_token];
                    uint256 _reward = lockedBalance
                        .mul(lockedWeights[_token])
                        .div(lockedTotalWeight);
                    if (_reward > 0) {
    ...
    function addBribeToken(address _gauge, address _rewardToken)
        external
        payable
    {
        uint256 _inSpiritTotal = inSPIRIT.totalSupply();
        require(
            (msg.value >= minFee &&
                inSPIRIT.balanceOf(msg.sender) >=
                _inSpiritTotal.mul(10).div(1000)) ||
                msg.sender == admin ||
                msg.sender == governance,
            "not enough fee paid or 1% supply needed"
        );
        IBribe(bribes[_gauge]).addReward(_rewardToken);
    }

    function withdrawFee() external {
        uint256 feeBal = address(this).balance;
        payable(governance).transfer(feeBal);
    }

    function setBribeFactory(address _newBribeFactory) external {
        require(
            (msg.sender == governance || msg.sender == admin),
            "setBribeFactory: permission is denied!"
        );
        bribeFactory = _newBribeFactory;
    }
    ...
    function setNewBribe(
        ...
        require(
            (msg.sender == governance || msg.sender == admin),
            "setNewBribe: permission is denied!"
        );
        // deplopy bribe contract for gauge
        address _bribe = IBaseV1BribeFactory(bribeFactory).createBribe(
            governance,
