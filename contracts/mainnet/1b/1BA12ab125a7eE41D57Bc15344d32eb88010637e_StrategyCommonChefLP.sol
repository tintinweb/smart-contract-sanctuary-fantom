...
contract StrategyCommonChefLP is StratManager, FeeManager, GasThrottler {
    ...
    event ChargedFees(uint256 callFees, uint256 beefyFees, uint256 protocolFees);
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 protocolAmount = IERC20(output).balanceOf(address(this)).mul(STRATEGIST_FEE).div(10000);
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(MAX_FEE.add(takeProfitBP).sub(STRATEGIST_FEE)).div(10000);
        
        //transfer output token, which is the protocol rewardtoken to burn/protocol owner
        IERC20(output).safeTransfer(strategist, protocolAmount);

        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);
        uint256 nativeBal = IERC20(native).balanceOf(address(this));

        //transfer native token to the profitDistributor
        uint256 profitAmount = nativeBal.mul(takeProfitBP).div(MAX_FEE.add(takeProfitBP).sub(STRATEGIST_FEE));
        IERC20(native).safeTransfer(profitDistributor, profitAmount);

        uint256 callFeeAmount = nativeBal.mul(callFee).div(MAX_FEE.add(takeProfitBP).sub(STRATEGIST_FEE));
        IERC20(native).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 beefyFeeAmount = nativeBal.mul(beefyFee).div(MAX_FEE.add(takeProfitBP).sub(STRATEGIST_FEE));
        IERC20(native).safeTransfer(beefyFeeRecipient, beefyFeeAmount);

        //uint256 strategistFeeAmount = nativeBal.mul(STRATEGIST_FEE).div(MAX_FEE);
        //IERC20(native).safeTransfer(strategist, strategistFeeAmount);

        emit ChargedFees(callFeeAmount, beefyFeeAmount, protocolAmount);
    }
