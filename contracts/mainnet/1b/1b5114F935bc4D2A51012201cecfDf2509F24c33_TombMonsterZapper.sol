...
contract TombMonsterZapper is Ownable {
    ...
    // @WNATIVE - base token, is used as a native for swaps when there is no liquidity pair to swap 
   ...
   // @in - LP we want to throw in
        ...
        if(_to == WNATIVE) {
            if(token0 == WNATIVE) {
                ...
                return _estimateSwap(token0, minAmountToken0, _to, _router).add(minAmountToken1);
                 ...
                 return _estimateSwap(token0, minAmountToken0, _to, _router).add(_estimateSwap(token1, minAmountToken1, _to, _router));
            }
        }
    ...
    function addIntermediateTokenForRouter(address token, address router, address intermediateToken) external onlyOwner {
