...
contract WsHecPriceOracle is BasePriceOracle {
    ...
    address constant public wsHEC = 0x94CcF60f700146BeA8eF7832820800E2dFa92EdA;
    ...
    function wsHecPrice() external view returns (uint) {
        return getPriceFromSourceInternal(wsHEC);
    }
    ...
    function getPriceUpdateTimestampFromSourceInternal(address asset) internal view override returns (uint) {
        if (asset == HEC || asset == wsHEC) {
            address hecOracle = ministry.getOracleForAsset(HEC);
            return IPriceOracleWsHecPriceOracle(hecOracle).getAssetPriceUpdateTimestamp(asset);
        } else {
            return 0;
        }
    }
