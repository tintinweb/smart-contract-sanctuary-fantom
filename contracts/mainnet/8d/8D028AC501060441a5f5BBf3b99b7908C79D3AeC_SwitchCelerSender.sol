...
contract SwitchCelerSender is Switch {
        ...
        returns (bytes32 transferId)
    {
        ...
        if ((IERC20(transferArgs.srcSwap.srcToken).isETH() && IERC20(transferArgs.srcSwap.dstToken).isETH()) ||
            (IERC20(transferArgs.srcSwap.srcToken).isETH() && (transferArgs.srcSwap.dstToken == nativeWrap))
        ) {
        ...
        transferId = MessageSenderLib.sendMessageWithTransfer(
            transferArgs.callTo,
            bridgeTokenIsNative ? nativeWrap : transferArgs.srcSwap.dstToken,
            returnAmount,
            transferArgs.dstChainId,
            transferArgs.nonce,
            transferArgs.bridgeSlippage,
            message,
            transferArgs.bridgeTransferType,
            celerMessageBus,
            sgnFee
        );
