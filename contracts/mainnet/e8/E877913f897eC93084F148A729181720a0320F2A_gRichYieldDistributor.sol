...
contract gRichYieldDistributor is Owned, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Yield tracking
    uint256 public yieldPergRICHStored = 0;
    mapping(address => uint256) public userYieldPerTokenPaid;
    mapping(address => uint256) public yields;

    // gRICH tracking
    uint256 public totalgRICHParticipating = 0;
    uint256 public totalgRICHSupplyStored = 0;
    mapping(address => bool) public userIsInitialized;
    mapping(address => uint256) public usergRICHCheckpointed;
    mapping(address => uint256) public usergRICHEndpointCheckpointed;
    ...
    function fractionParticipating() external view returns (uint256) {
        return totalgRICHParticipating.mul(PRICE_PRECISION).div(totalgRICHSupplyStored);
    }

    // Only positions with locked gRICH can accrue yield. Otherwise, expired-locked gRICH
    // is de-facto rewards for RICH.
    function eligibleCurrentgRICH(address account) public view returns (uint256 eligible_gRICH_bal, uint256 stored_ending_timestamp) {
        uint256 curr_gRICH_bal = gRICH.balanceOf(account);

        // Stored is used to prevent abuse
        stored_ending_timestamp = usergRICHEndpointCheckpointed[account];

        // Only unexpired gRICH should be eligible
        if (stored_ending_timestamp != 0 && (block.timestamp >= stored_ending_timestamp)){
            eligible_gRICH_bal = 0;
        }
        else if (block.timestamp >= stored_ending_timestamp){
            eligible_gRICH_bal = 0;
        }
        else {
            eligible_gRICH_bal = curr_gRICH_bal;
        }
    }
    ...
    function yieldPergRICH() public view returns (uint256) {
        if (totalgRICHSupplyStored == 0) {
            return yieldPergRICHStored;
        } else {
            return (
            yieldPergRICHStored.add(
                lastTimeYieldApplicable()
                .sub(lastUpdateTime)
                .mul(yieldRate)
                .mul(1e9)
                .div(totalgRICHSupplyStored)
            )
            );
        }
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        // Get eligible gRICH balances
        (uint256 eligible_current_gRICH, uint256 ending_timestamp) = eligibleCurrentgRICH(account);

        // If your gRICH is unlocked
        uint256 eligible_time_fraction = PRICE_PRECISION;
        if (eligible_current_gRICH == 0){
        ...
        // If the amount of gRICH increased, only pay off based on the old balance
        // Otherwise, take the midpoint
        uint256 gRICH_balance_to_use;
        {
            uint256 old_gRICH_balance = usergRICHCheckpointed[account];
            if (eligible_current_gRICH > old_gRICH_balance){
                gRICH_balance_to_use = old_gRICH_balance;
            }
            else {
                gRICH_balance_to_use = ((eligible_current_gRICH).add(old_gRICH_balance)).div(2);
            }
        }

        return (
        gRICH_balance_to_use
        .mul(yieldPergRICH().sub(userYieldPerTokenPaid[account]))
    ...
    function _checkpointUser(address account) internal {
        ...
        // Get the old and the new gRICH balances
        uint256 old_gRICH_balance = usergRICHCheckpointed[account];
        uint256 new_gRICH_balance = gRICH.balanceOf(account);

        // Update the user's stored gRICH balance
        usergRICHCheckpointed[account] = new_gRICH_balance;

        // Update the user's stored ending timestamp
        IgRICH.LockedBalance memory curr_locked_bal_pack = gRICH.locked(account);
        usergRICHEndpointCheckpointed[account] = curr_locked_bal_pack.end;

        // Update the total amount participating
        if (new_gRICH_balance >= old_gRICH_balance) {
            uint256 weight_diff = new_gRICH_balance.sub(old_gRICH_balance);
            totalgRICHParticipating = totalgRICHParticipating.add(weight_diff);
        } else {
            uint256 weight_diff = old_gRICH_balance.sub(new_gRICH_balance);
            totalgRICHParticipating = totalgRICHParticipating.sub(weight_diff);
        }
    ...
    function _syncEarned(address account) internal {
        if (account != address(0)) {
            ...
            userYieldPerTokenPaid[account] = yieldPergRICHStored;
        }
    }
    ...
    function sync() public {
        // Update the total gRICH supply
        yieldPergRICHStored = yieldPergRICH();
        totalgRICHSupplyStored = gRICH.totalSupply().add(gRICH.supply());
