...
contract token is Context, IERC20, Ownable {
    ...
    string private _name = "testicletoken2";
    string private _symbol = "TST2";
    uint8 private _decimals = 18;
    
    uint256 private _taxFeeOnBuy = 500; // 5%
    ...
    uint256 private basis = 10000;

    uint256 private _devFeeOnBuy = 1; // 1%
    uint256 private _devFeeOnSell = 1; // 1%
    ...
    uint256 private _liquidityFeeOnBuy = 1; //1%
    uint256 private _liquidityFeeOnSell = 2; //2%
    ...
    uint256 public _maxWalletSize = 10000000 * 10**18; // 1%
    uint256 public swapTokensAtAmount = 100000 * 10**18; //0.1%
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x31F63A33141fFee63D4B26755430a390ACdD8a4d);
        marketingWallet = payable(0xCF2C2fdc9A5A6fa2fc237DC3f5D14dd9b49F66A3);
        devTaxWallet = payable(0xCF2C2fdc9A5A6fa2fc237DC3f5D14dd9b49F66A3);
    ...
    function setMaxWalletSize(uint256 maxWalletSize) public onlyOwner {
        require(maxWalletSize > 10000000, "Max Tx Amount cannot be less than 10 Million");
        _maxWalletSize = maxWalletSize*10**18;
    }

    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        require(SwapThresholdAmount < 2000000, "Swap Threshold Amount cannot be more than 2 Million");
        swapTokensAtAmount = SwapThresholdAmount * 10**18;
    }
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_taxFee).div(basis);
    }

    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee).div(basis);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // I don't get this if statement. and the *10
        ...
        // //Slight Optimization. Making just 1 swap instead of 2.
        uint256 liquidityRatio = ((_liquidityFeeOnBuy+_liquidityFeeOnSell).mul(10**18)).div(_taxFeeOnBuy+_taxFeeOnSell+_liquidityFeeOnBuy+_liquidityFeeOnSell+_devFeeOnBuy+_devFeeOnSell); //Take approx 20% since 3% (1%+2%) fee of 14% => 3%/14% ~ 21%.
        uint256 devRatio = ((_devFeeOnBuy+_devFeeOnSell).mul(10**18)).div(_taxFeeOnBuy+_taxFeeOnSell+_liquidityFeeOnBuy+_liquidityFeeOnSell+_devFeeOnBuy+_devFeeOnSell); //Ratio should be roughly 10% since 2% (1%, 1%) => 2%/14% ~ 14%. 
        uint256 marketingRatio = 10**18 - liquidityRatio - devRatio;
        uint256 amountForLiquidity = contractTokenBalance.mul(liquidityRatio).div(10**18); 
        uint256 halfForLiquidity = amountForLiquidity.div(2);
        uint256 halfToSwapToETH = amountForLiquidity.div(2);
        ...
        uint256 halfLiquidityRatio = liquidityRatio.div(2);
        uint256 ethForDevRatio = (devRatio.mul(10**18)).div(marketingRatio + devRatio + halfLiquidityRatio);
        uint256 ethForMarketingRatio = (marketingRatio.mul(10**18)).div(marketingRatio + devRatio + halfLiquidityRatio);
        uint256 ethForDev = newBalance.mul(ethForDevRatio).div(10**18);
        uint256 ethForMarketing = newBalance.mul(ethForMarketingRatio).div(10**18);
