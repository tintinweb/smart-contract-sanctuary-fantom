...
contract KillSeigniorageAdmin is
	Pausable,
	ReentrancyGuard,
	AccessControlEnumerable
{
	...
	uint256 public executionEpoch;

	uint256 public burnProjectWalletAmount;
	...
	struct FarmInfo {
		...
		address farmAddress; // Farm address
	}

	event KillExecuted(address indexed executor);

	modifier whenNotExecuted() {
	...
	constructor(
		...
		IBoardroom[] memory _boardrooms,
		uint256 _executionEpoch,
		address _projectWallet,
		uint256 _burnProjectWalletAmount
	) {
		_setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

		treasury = _treasury;
		chargeToken = _chargeToken;
		boardrooms = _boardrooms;
		executionEpoch = _executionEpoch;
		projectWallet = _projectWallet;
		burnProjectWalletAmount = _burnProjectWalletAmount;
	}
	...
	{
		require(
			treasury.epoch() >= executionEpoch,
			'Execution epoch has not passed'
		);

		// migrate treasury so seigniorage can't be called
		treasury.migrate(projectWallet);

		for (uint256 i = 0; i < boardrooms.length; i++) {
			boardrooms[i].setLockUp(0, 0);
		}
		// burn project wallet charge
		if (burnProjectWalletAmount > 0) {
			chargeToken.burnFrom(projectWallet, burnProjectWalletAmount);
		}
	...
	function updateExecutionEpoch(uint256 _executionEpoch)
		external
		whenNotExecuted
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
	...
	function updateBurnProjectWalletAmount(uint256 _burnProjectWalletAmount)
		external
		whenNotExecuted
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
