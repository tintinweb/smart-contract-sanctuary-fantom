...
contract RShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accRSharePerShare; // Accumulated bSHAREs per share, times 1e18. See below.
    ...
    //Mainnet
    //RAVEN-FTM = 50.000 alloc points // RSHARE-FTM = 35.000 alloc points // RAVEN-RSHARE = 15.000 alloc points
    uint256 public rSharePerSecond = 0.0064300 ether; // 100000 rshare / (180 days * 24h * 60min * 60s)
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accRSharePerShare: 0, isStarted: _isStarted}));
        if (_isStarted) {
    ...
    // Safe rshare transfer function, just in case if rounding error causes pool to not have enough bSHAREs.
