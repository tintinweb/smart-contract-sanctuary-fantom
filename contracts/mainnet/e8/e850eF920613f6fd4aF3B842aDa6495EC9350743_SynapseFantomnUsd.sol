...
contract SynapseFantomnUsd is Defii, DefiiWithCustomExit {
    IERC20 constant USDC = IERC20(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
    IERC20 constant fUSDT = IERC20(0x049d68029688eAbF473097a2fC38ef61633A3C7A);

    IERC20 constant nUSDLP = IERC20(0x2DC777ff99058a12844A33D9B1AE6c8AB4701F66);
    IERC20 constant SYN = IERC20(0xE55e19Fb4F2D85af758950957714292DAC1e25B2);

    ISwapFlashLoan constant swapFlashLoan =
        ISwapFlashLoan(0x85662fd123280827e11C59973Ac9fcBE838dC3B4);
    IMiniChefV2 constant miniChef =
        IMiniChefV2(0xaeD5b25BE1c3163c907a471082640450F928DDFE);
    ...
    function exitParams(uint256 slippage) external view returns (bytes memory) {
        ...
        uint256 fUsdtPerLp = swapFlashLoan.calculateRemoveLiquidityOneToken(
            1e18,
            2
        );
        ...
        if (usdcPerLp > fUsdtPerLp) {
            returnTokenIndex = 1;
            tokenAmounts = usdcPerLp;
        } else {
            returnTokenIndex = 2;
            tokenAmounts = fUsdtPerLp;
        }
