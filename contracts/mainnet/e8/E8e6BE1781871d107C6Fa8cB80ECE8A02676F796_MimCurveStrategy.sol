...
interface IFarm {
    function userInfo(uint256 _pid, address _user) external returns (uint256 amount, uint256 rewardDebt, uint256 remainingIceTokenReward);
...
interface CurvePool {
    ...
    function add_liquidity(uint256[3] memory amounts, uint256 _min_mint_amount, address recipient) external;
...
contract MimCurveStrategy is BaseStrategy {
    using SafeERC20 for IERC20;

    event LpMinted(uint256 total, uint256 strategyAmount, uint256 feeAmount);
    IFarm private immutable farm;

    uint256 public constant CurveMimLPPid = 0;
    IDexRouter public constant spookyRouter = IDexRouter(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
    ERC public constant MIM = ERC(0x82f0B8B456c1A451378467398982d4834b6829c1);
    IERC20 public constant WFTM = IERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    ERC public constant SPELL = ERC(0x468003B688943977e6130F4F68F23aad939a1040);
    CurvePool constant public mimCurvePool = CurvePool(0x2dd7C9371965472E5A5fD28fbE165007c61439E1);
    ...
    */
    constructor(
        ...
        IERC20(_strategyToken).safeApprove(address(_farm), type(uint256).max);
        SPELL.approve(address(spookyRouter), type(uint256).max);
        MIM.approve(address(mimCurvePool), type(uint256).max);
    }

    function _skim(uint256 amount) internal override {
        totalDepositAmount += amount;
        farm.deposit( CurveMimLPPid , amount);
    }

    function _harvest(uint256) internal override returns (int256) {

        /// Withdraw everything and deposit back the original amount.
        (uint256 total,,) = farm.userInfo(CurveMimLPPid, address(this));
        farm.withdraw(CurveMimLPPid, total);
        ...
        farm.deposit(CurveMimLPPid, totalDepositAmount);
        convertSpellToCollateral();
        return int256(0);
    }

    function _withdraw(uint256 amount) internal override {
        totalDepositAmount -= amount;
        farm.withdraw(CurveMimLPPid, amount);
        convertSpellToCollateral();
    }

    function _exit() internal override {
        totalDepositAmount = 0;
        (uint256 totalDeposited,,) = farm.userInfo(CurveMimLPPid, address(this));
        farm.withdraw(CurveMimLPPid, totalDeposited);
        convertSpellToCollateral();
    }
    ...
    function convertSpellToCollateral() internal{
        ...
        if(spellBalance > 0){
            ...
            uint[] memory amounts = spookyRouter.swapExactTokensForTokens(spellBalance, 0, path, address(this), block.timestamp);
            uint mimAmount=amounts[2];

            uint256[3] memory amountsAdded = [mimAmount, 0, 0];
            mimCurvePool.add_liquidity(amountsAdded, 0, address(this));
        }
...
//To-do: each withdrawal and deposit of collateral results in withdrawal of SPELL tokens from the farm