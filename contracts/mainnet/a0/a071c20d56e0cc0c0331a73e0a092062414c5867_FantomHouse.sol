...
contract FantomHouse is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    event ReceivedBetResult(bool userWon, uint256 indexed requestId, address indexed requestUser, uint256 response, uint256 sortedUserFace, uint256 predictedUserFace, uint256 betAmount, uint256 winMoney);
    ...
    /* ========== VIEW FUNCTIONS ========== */
    function isHouseDepositPaused() public view returns (bool){
        return pauseHouseDeposit;
    }

    function isBetPaused() public view returns (bool){
        return pauseBet;
    }

    function isHouseWithdrawPaused() public view returns (bool){
    ...
    function depositHouse(uint256 _amount, address _referral) external {
        require(pauseHouseDeposit == false, "House Deposit is not open now");
    ...
    function withdrawHouse(uint256 _amount) external nonReentrant {
        require(pauseHouseWithdraw == false, "House Withdraw is not open now");
        address _sender = msg.sender;
    ...
    function bet(uint256 _betAmount, uint256 _faceSide, address _referral) external nonReentrant {
        ...
        if (_referral == msg.sender) revert("Referral shouldn't be same as sender");
