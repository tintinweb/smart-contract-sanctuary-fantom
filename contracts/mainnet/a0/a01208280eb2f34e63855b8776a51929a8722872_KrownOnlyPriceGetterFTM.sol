...
// File: polycrystal-on-chain-stats/contracts/interfaces/IKrownPair.sol
...
// File: polycrystal-on-chain-stats/contracts/libraries/KrownOnlyPriceGetterBSC.sol

pragma solidity ^0.8.6;


// This library provides simple price calculations for KrownSwap tokens, accounting
...
// FTM KrownSwap only version

//import "./AMMData.sol";

library KrownOnlyPriceGetterFTM {

//    using AMMData for AmmData;

    address public constant FACTORY = 0x152eE697f2E276fA89E96742e9bB9aB1F2E61bE3; //KrownFactory

    bytes32 public constant INITCODEHASH = hex'cdf2deca40a0bd56de8e3ce5c7df6727e5b1bf2ac96f283fa9c4b3e6b42ea9d2'; // for pairs created by KrownFactory
    ...
    uint private constant USD_RAW_PRICE = 1e18;

    //Krown LP addresses

    address constant USDC_WFTM_PAIR = 0x2b4C76d0dc16BE1C31D4C1DC53bF9B45987Fc75c; 

    address constant DAI_WFTM_PAIR = 0xe120ffBDA0d14f3Bb6d6053E90E63c572A66a428;  

    address constant USDT_WFTM_PAIR = 0x5965E53aa80a0bcF1CD6dbDd72e6A9b2AA047410; 
    ...
    function getRawPrice(address token) public view returns (uint) {
        ...
        return getRawPrice(token, getftmPrice());
    }
    ...
    function getRawPrices(address[] calldata tokens) public view returns (uint[] memory prices) {
        ...
        uint ftmPrice = getftmPrice();

        for (uint i; i < prices.length; i++) {
            address token = tokens[i];
            uint pegPrice = pegTokenPrice(token, ftmPrice);
            if (pegPrice != 0) prices[i] = pegPrice;
            else prices[i] = getRawPrice(token, ftmPrice);
        }
    }
    ...
    function getRawLPPrice(address token) internal view returns (uint) {
        uint pegPrice = pegTokenPrice(token);
        if (pegPrice != 0) return pegPrice;
        return getRawLPPrice(token, getftmPrice());
    }
    ...
    function getRawLPPrices(address[] calldata tokens) internal view returns (uint[] memory prices) {
        ...
        for (uint i; i < prices.length; i++) {
            ...
            uint pegPrice = pegTokenPrice(token, ftmPrice);
            if (pegPrice != 0) prices[i] = pegPrice;
            else prices[i] = getRawLPPrice(token, ftmPrice);
        }
    }

    //returns the current USD price of ftm based on primary stablecoin pairs

    function getftmPrice() public view returns (uint) {
        (uint daiReserve, uint wftmReserve0,) = IKrownPair(DAI_WFTM_PAIR).getReserves();
        (uint wftmReserve1, uint usdcReserve,) = IKrownPair(USDC_WFTM_PAIR).getReserves();
        (uint usdtReserve, uint wftmReserve2,) = IKrownPair(USDT_WFTM_PAIR).getReserves();
    ...
    //tokens using Krown prices. If the provided token is not a LP, it will attempt to price the token as a
    //standard token. This is useful for MasterChef farms which stake both single tokens and pairs

    function getRawLPPrice(address lp, uint ftmPrice) internal view returns (uint) {

        //if not a LP, handle as a standard token
        try IKrownPair(lp).getReserves() returns (uint112 reserve0, uint112 reserve1, uint32) {
            address token0 = IKrownPair(lp).token0();
            address token1 = IKrownPair(lp).token1();
            uint totalSupply = IKrownPair(lp).totalSupply();
            //price0*reserve0+price1*reserve1
            uint totalValue = getRawPrice(token0, ftmPrice) * reserve0 
                + getRawPrice(token1, ftmPrice) * reserve1;
            return totalValue / totalSupply;
        } catch {
            return getRawPrice(lp, ftmPrice);
        }
    }

    // checks for primary tokens and returns the correct predetermined price if possible, otherwise calculates price

    function getRawPrice(address token, uint ftmPrice) internal view returns (uint rawPrice) {
        uint pegPrice = pegTokenPrice(token, ftmPrice);
        if (pegPrice != 0) return pegPrice;
        uint numTokens;
        uint pairedValue;
    ...
    function pegTokenPrice(address token, uint ftmPrice) private pure returns (uint) {
        if (token == USDT || token == USDC || token == DAI) return PRECISION;
        if (token == WFTM) return ftmPrice;
        return 0;
    }

    function pegTokenPrice(address token) private view returns (uint) {
        if (token == USDT || token == USDC || token == DAI) return PRECISION;
        if (token == WFTM) return getftmPrice();
        return 0;
    }
    ...
    function pairTokensAndValue(address token, address peg) private view returns (uint tokenNum, uint pegValue) {
        ...
        try IKrownPair(tokenPegPair).getReserves() returns (uint112 reserve0, uint112 reserve1, uint32) {
            uint reservePeg;
