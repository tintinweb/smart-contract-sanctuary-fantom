...
contract CommonMaster is ICommonMaster, Pausable, Ownable {
   ...
   function updatePool(address _pair) public override {
    ...
    uint256 lpSupply = ERC20(_pair).balanceOf(address(this));
    if (lpSupply == 0) {
    ...
    require(tokenReward <= rewardTokenRemaining, "INSUFFICIENT REWARDS");
    pool.accTokenPerShare = pool.accTokenPerShare.add(
        tokenReward.mul(10**accTokenPerShareMultiple).div(lpSupply)
    );
    rewardTokenRemaining = rewardTokenRemaining.sub(tokenReward);
    pool.lastRewardBlock = block.number;
}

    // Stake LP tokens to TokenMaster for TOKEN allocation.
uint256 public rewardPerBlock; // số lượng token được mint cho mỗi block
uint256 public rewardTokenRemaining; // số lượng token còn lại có thể được mint
...
function stake(address _pair, uint256 _amount) external override whenNotPaused {
    ...
    if (_amount != 0) {
        ...
        uint256 rewardAmount = _amount.mul(rewardPerBlock).div(accTokenPerShareMultiple);
        require(rewardAmount <= rewardTokenRemaining, "INSUFFICIENT REWARDS");
        rewardTokenRemaining = rewardTokenRemaining.sub(rewardAmount);
