  ...
  ReentrancyGuardUpgradeable
{
  ...
  event Airdop(uint256 amountOfNodes, address[] recipients);
  ...
  function airdrop(
    ...
    require(tokenIds.length <= nodeCap, "Node cap exceeded on Airdrop");
    require(recipients.length == amounts.length, "Arrays length mismatch");

    uint256 pointer = 0;
    for (uint256 i = 0; i < recipients.length; i++) {
      uint256 selectedAmount = amounts[i];
      uint256[] memory selectedTokenIds = new uint256[](selectedAmount);
      for (uint256 j = 0; j < selectedAmount; j++) {
        selectedTokenIds[j] = tokenIds[pointer];
        pointer++;
      }
      _batchTransfer(recipients[i], selectedTokenIds);
    }

    emit Airdop(tokenIds.length, recipients);
  }
  ...
  function claimRewards(uint256 tokenId) external nonReentrant {
    ...
    require(
      sender != address(0),
      "HonourNode: Cannot claim from the zero address"
    );
    uint256 rewardAmount = syncNodeReward(tokenId);
    require(rewardAmount > 0, "HonourNode: Not enough reward to claim");

    uint256 feeAmount;
    if (claimFee > 0) {
      feeAmount = rewardAmount.mul(claimFee).div(100);
      if (swapLiquify) {
