...
import "../../apis/spookyswap/ISpookyFactory.sol";
import "../../apis/spookyswap/ISpookyRouter02.sol";
import "../../apis/spookyswap/ISpookyMasterChef.sol";
...
/// @title SpookySwapWorker is a SpookyswapWorker with with reinvest-optimized and beneficial vault buyback functionalities
contract SpookySwapWorker is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker02 {
  /// @notice Libraries
  ...
  ISpookyFactory public factory;
  ISpookyRouter02 public router;
  ...
  /// @notice Configuration varaibles for Spookyswap
  uint256 public fee;
  uint256 public feeDenom;

  /// @notice Upgraded State Variables for SpookySwapWorker
  ...
  function initialize(
    ...
    router = _router;
    factory = ISpookyFactory(_router.factory());
    ...
    // 6. Set Spookyswap swap fees
    fee = 9980; // 0.2%
    feeDenom = 10000;

    // 7. Check if critical parameters are config properly
    require(baseToken != boo, "SpookySwapWorker::initialize:: base token cannot be a reward token");
    require(
    ...
    require(
      reinvestPath[0] == boo && reinvestPath[reinvestPath.length - 1] == baseToken,
      "SpookySwapWorker::initialize:: reinvestPath must start with CHE, end with BTOKEN"
    );
  }
  ...
  function _reinvest(
    ...
    require(_treasuryAccount != address(0), "SpookySwapWorker::_reinvest:: bad treasury account");
  ...
  function _addShare(uint256 id) internal {
    ...
    if (balance > 0) {
      ...
      // 3. Deposit balance to SpookyMasterChef
  ...
  function setReinvestConfig(
    ...
    require(
      ...
      "SpookySwapWorker::setReinvestConfig:: _reinvestBountyBps exceeded maxReinvestBountyBps"
    );
    require(_reinvestPath.length >= 2, "SpookySwapWorker::setReinvestConfig:: _reinvestPath length must >= 2");
    require(
      _reinvestPath[0] == boo && _reinvestPath[_reinvestPath.length - 1] == baseToken,
      "SpookySwapWorker::setReinvestConfig:: _reinvestPath must start with CHE, end with BTOKEN"
    );
  ...
  function setMaxReinvestBountyBps(uint256 _maxReinvestBountyBps) external onlyOwner {
    ...
    require(
      ...
      "SpookySwapWorker::setMaxReinvestBountyBps:: _maxReinvestBountyBps exceeded 30%"
    );
  ...
  function setRewardPath(address[] calldata _rewardPath) external onlyOwner {
    require(rewardPath.length >= 2, "SpookySwapWorker::setRewardPath:: rewardPath length must be >= 2");
    require(
      rewardPath[0] == boo && rewardPath[rewardPath.length - 1] == beneficialVault.token(),
      "SpookySwapWorker::setRewardPath:: rewardPath must start with CHE and end with beneficialVault token"
    );
  ...
  function setTreasuryConfig(address _treasuryAccount, uint256 _treasuryBountyBps) external onlyOwner {
    require(
      ...
      "SpookySwapWorker::setTreasuryConfig:: _treasuryBountyBps exceeded maxReinvestBountyBps"
    );
  ...
  function setBeneficialVaultConfig(
    ...
    require(
      ...
      "SpookySwapWorker::setBeneficialVaultConfig:: _beneficialVaultBountyBps exceeds 100%"
    );
    require(_rewardPath.length >= 2, "SpookySwapWorker::setBeneficialVaultConfig:: rewardPath length must >= 2");
    require(
      _rewardPath[0] == boo && _rewardPath[_rewardPath.length - 1] == _beneficialVault.token(),
      "SpookySwapWorker::setBeneficialVaultConfig:: rewardPath must start with CHE, end with beneficialVault token"
    );
...
contract ISpookyMasterChef {
  ...
  struct PoolInfo {
    ...
    uint256 accBOOPerShare; // Accumulated SUSHIs per share, times 1e12. See below.
  }
