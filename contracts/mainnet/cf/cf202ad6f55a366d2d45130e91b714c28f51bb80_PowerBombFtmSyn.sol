...
interface ICurve {
    function exchange(uint i, uint j, uint dx, uint min_dy) external;
...
interface IPool {
    function swapStorage() external view returns (uint, uint, uint, uint, uint, uint, address lpToken);
    function calculateTokenAmount(uint[] memory, bool) external view returns (uint);
    function addLiquidity(uint[] memory, uint, uint) external returns (uint);
    function removeLiquidityOneToken(
        uint tokenAmount, uint8 tokenIndex, uint minAmount, uint deadline
    ) external returns (uint);
    function getVirtualPrice() external view returns (uint);
}

interface IStake {
    function userInfo(uint pid, address account) external view returns (uint);
    function deposit(uint pid, uint amount, address to) external;
    function withdraw(uint pid, uint amount, address to) external;
    function harvest(uint pid, address account) external;
    function pendingSynapse(uint pid, address account) external view returns (uint);
}
...
interface IChefIncentivesController {
    function claimableReward(address user, address[] memory tokens) external view returns (uint[] memory);
    function claim(address user, address[] memory tokens) external;
}

contract PowerBombFtmSyn is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable constant USDT = IERC20Upgradeable(0x049d68029688eAbF473097a2fC38ef61633A3C7A);
    IERC20Upgradeable constant USDC = IERC20Upgradeable(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
    IERC20Upgradeable constant nUSD = IERC20Upgradeable(0xED2a7edd7413021d440b09D654f3b87712abAB66);
    IERC20Upgradeable constant SYN = IERC20Upgradeable(0xE55e19Fb4F2D85af758950957714292DAC1e25B2);
    ...
    ISolidly constant solidly = ISolidly(0xa38cd27185a464914D3046f0AB9d43356B34829D);
    ICurve constant geist = ICurve(0x0fa949783947Bf6c1b171DB13AEACBB488845B3f);
    ICurve constant tricrypto = ICurve(0x3a1659Ddcf2339Be3aeA159cA010979FB49155FF);
    ISpooky constant spooky = ISpooky(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
    IPool public pool;
    IStake public stake;
    uint public pid;
    ...
    IChefIncentivesController constant chefIncentivesController = IChefIncentivesController(0x297FddC5c33Ef988dd03bd13e162aE084ea1fE57); // Claim reward
    IERC20Upgradeable public ibRewardToken; // gToken
    ...
    function initialize(
        IPool _pool, IStake _stake, uint _pid,
        address _treasury,
        ...
        stake = _stake;
        pid = _pid;
        (,,,,,,address lpTokenAddr) = pool.swapStorage();
        lpToken = IERC20Upgradeable(lpTokenAddr);
        ...
        nUSD.safeApprove(address(pool), type(uint).max);
        lpToken.safeApprove(address(pool), type(uint).max);
        lpToken.safeApprove(address(stake), type(uint).max);
        SYN.safeApprove(address(solidly), type(uint).max);
        GEIST.safeApprove(address(spooky), type(uint).max);
        USDC.safeApprove(address(geist), type(uint).max);
        USDT.safeApprove(address(tricrypto), type(uint).max);
        rewardToken.safeApprove(address(lendingPool), type(uint).max);
    }

    function deposit(IERC20Upgradeable token, uint amount, uint slippage) external virtual nonReentrant whenNotPaused {
        require(token == USDT || token == USDC || token == nUSD || token == lpToken, "Invalid token");
        ...
        if (token != lpToken) {
            ...
            else if (token == nUSD) amounts[0] = amount;

            uint estimatedMintAmt = pool.calculateTokenAmount(amounts, true);
            uint minMintAmt = estimatedMintAmt - (estimatedMintAmt * slippage / 10000);

            lpTokenAmt = pool.addLiquidity(amounts, minMintAmt, block.timestamp);
        } else {
            lpTokenAmt = amount;
        }

        stake.deposit(pid, lpTokenAmt, address(this));
    ...
    function withdraw(IERC20Upgradeable token, uint amountOutLpToken, uint) external virtual nonReentrant {
        require(token == USDT || token == USDC || token == nUSD || token == lpToken, "Invalid token");
        User storage user = userInfo[msg.sender];
        ...
        stake.withdraw(pid, amountOutLpToken, address(this));

        uint amountOutToken;
        if (token != lpToken) {
            uint8 tokenIndex;
            if (token == USDT) tokenIndex = 2;
            else if (token == USDC) tokenIndex = 1;
            else tokenIndex = 0; // nUSD
            // getPricePerFullShareInUSD() won't drop under 1, amount out will be larger than LP token amount
            // So, LP token amount be the minimum amount out
            uint minAmount = token == nUSD ? amountOutLpToken : amountOutLpToken / 1e12;
            amountOutToken = pool.removeLiquidityOneToken(amountOutLpToken, tokenIndex, minAmount, block.timestamp);
        } else {
    ...
    function harvest() public virtual {
        ...
        // Collect SYN from Synapse staking contract
        stake.harvest(pid, address(this));

        uint SYNAmt = SYN.balanceOf(address(this));
        uint estimatedUSDCAmt = solidly.getAmountsOut(SYNAmt, getRoute(address(SYN), address(USDC), false))[1];
        if (estimatedUSDCAmt > 25e6) { // 25 USDC, 6 decimals
            // Swap SYN to USDC with Solidly
            uint USDCAmt = solidly.swapExactTokensForTokens(
                SYNAmt, 0, getRoute(address(SYN), address(USDC), false), address(this), block.timestamp
            )[1];

            // Swap USDC to USDT with Curve geist pool
            geist.exchange_underlying(1, 2, USDCAmt, 0);

            // Swap USDT to reward token with Curve tricrypto pool
            uint rewardTokenIndex = address(rewardToken) == BTC ? 1 : 2;
            tricrypto.exchange(0, rewardTokenIndex, USDT.balanceOf(address(this)), 0);
            ...
            // Collect GEIST reward from Geist
            address[] memory tokens = new address[](1);
            tokens[0] = address(ibRewardToken);
            uint unclaimedRewardsAmt = chefIncentivesController.claimableReward(address(this), tokens)[0];
            if (unclaimedRewardsAmt > 100e18) { // Approximately $10.8 @ 3 Apr 2022
                // Will get a 50% haircut for no waiting
                // https://docs.geist.finance/geist-and-fantom-rewards 
                chefIncentivesController.claim(address(this), tokens);
                uint GEISTAmt = GEIST.balanceOf(address(this));

                // Swap GEIST to rewardToken
                address[] memory path = new address[](3);
                path[0] = address(GEIST);
                path[1] = WFTM;
                path[2] = address(rewardToken);
                uint _rewardTokenAmt = spooky.swapExactTokensForTokens(
                    GEISTAmt, 0, path, address(this), block.timestamp
                )[2];
            ...
            emit Harvest(SYNAmt, rewardTokenAmt, fee);
        }
    }
    ...
    /// @dev Solidly
    function getRoute(address tokenIn, address tokenOut, bool _stable) internal virtual pure returns (ISolidly.route[] memory routes) {
        routes = new ISolidly.route[](1);
        routes[0] = ISolidly.route(tokenIn, tokenOut, _stable);
    }

    function getAllPool() public virtual view returns (uint) {
        return stake.userInfo(pid, address(this));
    }

    /// @return Price per full share in USD (6 decimals)
    function getPricePerFullShareInUSD() public virtual view returns (uint) {
        return pool.getVirtualPrice() / 1e12;
    }
    ...
    /// @return poolPendingReward in USD (18 decimals)
    function getPoolPendingReward(IERC20Upgradeable) external virtual view returns (uint poolPendingReward) {
        uint pendingSYN = stake.pendingSynapse(pid, address(this));
        poolPendingReward = solidly.getAmountsOut(
            pendingSYN, getRoute(address(SYN), address(USDC), false)
        )[1] * 1e12;
    }
