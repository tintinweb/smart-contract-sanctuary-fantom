...
contract DfyMarketingMultisig is Ownable
{
    ...
    function setDfyTransaction (address _receiver , uint256 _amount) external onlyDev returns(uint256)
    {
    ...
    function setFTMTransaction (address _receiver , uint256 _amount) external onlyDev returns(uint256)
    {
    ...
    function executeTransation(uint256 tx_index) public payable onlyDev{
    ...
    {
        Transaction storage txn = transactions[tx_index] ;
        return (txn.token , txn.amount, txn.receiver, txn.dev1_sign, txn.dev2_sign, txn.isftm);
    }
