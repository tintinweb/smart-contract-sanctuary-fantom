...
// File contracts/GrimEvoFtmEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract GrimEvoFtmEqualizerV2 is Ownable, Pausable {
    ...
    //Routes
    IEqualizerRouter.Routes[] public equalToFtmPath;
    IEqualizerRouter.Routes[] public equalToEvoPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToFtmPath,
        IEqualizerRouter.Routes[] memory _equalToEvoPath
    ) {
        ...
        for (uint i; i < _equalToFtmPath.length; ++i) {
            equalToFtmPath.push(_equalToFtmPath[i]);
        }

        for (uint i; i < _equalToEvoPath.length; ++i) {
           equalToEvoPath.push(_equalToEvoPath[i]);
        }
   ...
   //Withdraws funds from masterchef
    ...
    function chargeFees(address caller) internal {
        uint256 toWftm = IERC20(equal).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        IEqualizerRouter(router).swapExactTokensForTokens(toWftm, 0, equalToFtmPath, address(this), block.timestamp);

        uint256 wftmFee = IERC20(wftm).balanceOf(address(this));

        uint256 callFee = wftmFee * CALL_FEE / FEE_DIVISOR;
        IERC20(wftm).transfer(caller, callFee);

        uint256 recipientFee = wftmFee * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(wftm).transfer(grimFeeRecipient, recipientFee);

        uint256 treasuryFee = wftmFee * TREASURY_FEE / FEE_DIVISOR;
        IERC20(wftm).transfer(treasury, treasuryFee);

        uint256 stratFee = wftmFee * STRAT_FEE / FEE_DIVISOR;
        IERC20(wftm).transfer(strategist, stratFee);
    }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToEvoPath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToFtmPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(grimEvo).balanceOf(address(this));
        uint256 t2Bal = IERC20(wftm).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(grimEvo, wftm, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(grimEvo).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(grimEvo).safeApprove(router, type(uint).max);
    }
