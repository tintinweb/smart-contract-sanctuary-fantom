 ...
 * @notice Interface for the Wagmi borrow rate oracle. Provides the average market borrow rate to be used as a base for the stable borrow rate calculations
 **/
...
/**
 * @title IWagmiOracle interface
 * @notice Interface for the Wagmi oracle.
 **/

interface IWagmiOracle {
...
import {IWagmiOracle} from "./interfaces/IWagmiOracle.sol";
...
contract UiPoolDataProviderV2V3 is IUiPoolDataProviderV3 {
  ...
  function getReservesData(
    ...
    IWagmiOracle oracle = IWagmiOracle(provider.getPriceOracle());
    ILendingPool lendingPool = ILendingPool(provider.getLendingPool());
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      reserveData.wTokenAddress = baseData.wTokenAddress;
      ...
      reserveData.availableLiquidity = IERC20(reserveData.underlyingAsset).balanceOf(reserveData.wTokenAddress);
      (
  ...
  function getUserReservesData(
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      userReservesData[i].scaledWTokenBalance = IWToken(baseData.wTokenAddress).scaledBalanceOf(user);
 ...
 * - An instance of this same contract, can"t be used across different Wagmi markets, due to the caching
...
contract DefaultReserveInterestRateStrategy is IReserveInterestRateStrategy {
  ...
  function calculateInterestRates(
    ...
    uint256 availableLiquidity = IERC20(reserve).balanceOf(wToken);
   ...
   * @param reserve The address of the reserve
   * @param availableLiquidity The liquidity available in the corresponding wToken
   * @param totalStableDebt The total borrowed from the reserve a stable rate
   * @param totalVariableDebt The total borrowed from the reserve at a variable rate
   * @param averageStableBorrowRate The weighted average of all the stable rate loans
   * @param reserveFactor The reserve portion of the interest that goes to the treasury of the market
