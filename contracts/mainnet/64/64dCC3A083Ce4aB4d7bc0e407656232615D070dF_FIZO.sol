...
contract FIZO is IERC20
{
    ...
    function balanceOf(address account) public view virtual override returns (uint256) 
    {
     ...
     function tokensToMatic(uint tokenAmount, address memberId) public view returns(uint)
    {
    ...
    function fizoWithdraw(uint8 _perc) public{
        ...
        if(_perc == 10 || _perc == 50 || _perc == 100)
		{
         ...
         uint256 calcWithdrawable = user.token.mul(perc).div(100).mul(getCoinRate(msg.sender));
        contractBalance-=calcWithdrawable;
        uint256 withdrawable = user.token.mul(perc).div(100).mul(getCoinRate(msg.sender));
        ...
        TotalHoldings-=withdrawable.mul(getCoinRate(msg.sender));
