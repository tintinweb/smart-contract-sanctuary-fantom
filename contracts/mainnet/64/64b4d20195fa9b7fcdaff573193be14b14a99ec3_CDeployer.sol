...
contract ScorpionERC20 {
    ...
    function _checkSignature(
        ...
        require(deadline >= block.timestamp, "Scorpion: EXPIRED");
        bytes32 digest =
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
...
interface IScorpionPriceOracle {
    event PriceUpdate(
...
contract PoolToken is IPoolToken, ScorpionERC20 {
    ...
    function _setFactory() external {
        require(factory == address(0), "Scorpion: FACTORY_ALREADY_SET");
        factory = msg.sender;
    }
        ...
        returns (uint256 mintTokens)
    {
        ...
        require(mintTokens > 0, "Scorpion: MINT_AMOUNT_ZERO");
        ...
        returns (uint256 redeemAmount)
    {
        ...
        require(redeemAmount > 0, "Scorpion: REDEEM_AMOUNT_ZERO");
        require(redeemAmount <= totalBalance, "Scorpion: INSUFFICIENT_CASH");
    ...
    modifier nonReentrant() {
        require(_notEntered, "Scorpion: REENTERED");
        _notEntered = false;
        _;
        _notEntered = true;
    }
...
contract CSetter is PoolToken, CStorage {
    ...
    function _initialize(
        ...
        require(msg.sender == factory, "Scorpion: UNAUTHORIZED"); // sufficient check
    ...
    function _checkSetting(
        ...
        _checkAdmin();
        require(parameter >= min, "Scorpion: INVALID_SETTING");
        require(parameter <= max, "Scorpion: INVALID_SETTING");
    }

    function _checkAdmin() internal view {
        require(msg.sender == IFactory(factory).admin(), "Scorpion: UNAUTHORIZED");
    }
}
...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) =
            IScorpionPriceOracle(tarotPriceOracle).getResult(underlying);
         ...
         */
        require(price0 > 100, "Scorpion: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Scorpion: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Scorpion: INSUFFICIENT_LIQUIDITY");
    ...
    function seize(
        ...
        require(
            ...
            "Scorpion: UNAUTHORIZED"
        );

        (, uint256 shortfall) = accountLiquidity(borrower);
        require(shortfall > 0, "Scorpion: INSUFFICIENT_SHORTFALL");
    ...
    function flashRedeem(
        ...
        _safeTransfer(redeemer, redeemAmount);
        if (data.length > 0)
            IScorpionCallee(redeemer).tarotRedeem(msg.sender, redeemAmount, data);
        ...
        require(
            ...
            "Scorpion: INSUFFICIENT_REDEEM_TOKENS"
        );
