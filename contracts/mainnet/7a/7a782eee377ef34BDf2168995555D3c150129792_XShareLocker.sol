...
contract XShareLocker is Operator, IXShareLocker {
    ...
    function lockXShare(uint256 amount, uint256 lockDuration) public override {
        ...
        updateAverageLockDuration(amount, lockDuration);
    ...
    function addMoreXShare(uint256 amount) public override {
        ...
        require(amount > 0, "Invalid Amount");
//        if (user.lockEndTime > block.timestamp) revert("Lock ended");
//        if (user.lockedAmount <= 0) revert("Not locked any xShare");
        require(isLocked(_sender), "Lock ended"); //=>> Cause Bug
//        require(user.lockedAmount > 0, "Not Locked Any XShare"); //=>> Cause Bug
        ...
        user.lockedAmount = user.lockedAmount.add(amount);
        user.yShareMinted = user.yShareMinted.add(yShare_minting_amount);
        updateAverageLockDuration(amount, user.lockDuration);
        totalLockedAmount = totalLockedAmount.add(amount);
        //TODO add event addMore
    }

    //If User has locked XShare before, user can extend their lock duration to receive more yShare
    function extendLockDuration(uint256 extendLockDuration) public override {
        address _sender = msg.sender;
        UserInfo storage user = userInfo[_sender];
...
//        if (user.lockedAmount <= 0) revert("Not Locked any xShare");
        require(isLocked(_sender), "Lock ended"); //=>> Cause Bug
        //        require(user.lockEndTime < block.timestamp, "Lock ended"); =>> Cause Bug
//        require(user.lockedAmount > 0, " Not locked any xShare"); =>> Cause Bug
...
//        if (user.lockEndTime < block.timestamp) revert("Still in lock");
        if (user.lockedAmount < amount) revert("Exceed locked amount");
//        require(user.lockEndTime > block.timestamp, "Still In Lock");
//        require(user.lockedAmount >= _amount, "Exceed locked amount");
        uint256 require_yShare_balance = calculateYShareMintAmount(amount, user.lockDuration);
        ...
        IERC20(xShare).safeTransfer(msg.sender, amount);
