...
contract StabilityGenesisRewardPool {
    ...
    uint256 public tokenPerSecond = 0.07716 ether; // 20000 TOKEN /  (72h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "TokenGenesisPool: caller is not the operator"
        );
        _;
    }
    ...
    // @ _allocPoint - amount of token this pool will emit
    ...
    function add(
        ...
        require(_fee >= 0 && _fee <= 500, "_fee: out of range"); // [0%, 5%]
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _tokenReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint
            );
            accTokenPerShare = accTokenPerShare.add(
                _tokenReward.mul(1e18).div(tokenSupply)
            );
        }
    ...
    // Safe TOKEN transfer function, in case if rounding error causes pool to not have enough TOKENs.
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 30 days) {
            // do not allow to drain core token (TOKEN or lps) if less than 30 days after pool ends
