...
contract FantomHouse is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    function depositHouse(uint256 _amount, address _referral) external {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            userHouseBalances[_user].userContributionPortion = calculateUserContributionPortion(_user);
        }
    ...
    function fulfillRandomWords(
        ...
        if (userWon == 1) {
            ...
            for (uint256 i = 0; i < users.length; i++) {
                userHouseBalances[users[i]].depositAmount += (playerBetAmount - calculatedFee) * userHouseBalances[users[i]].userContributionPortion / 1e18;
            }
        }
