...
import "../interfaces/IGodFootPrint.sol";
...
contract SmeltRewardPool is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        address receiptToken; // receipt token this pool might emit
    ...
    uint256 constant runningTime = 700 days;
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant  {
        ...
        if (_amount > 0) {
            ...
            if (pool.givesReceipts == true) {
                IBasedReceipt(pool.receiptToken).mint(_sender, _amount);
            }
        }
    ...
    function StakeNft(uint256 _pid, uint256[] calldata _tokenIds) external nonReentrant {
        ...
        if (user.amount > 0) {
         ...
         if (_tokenIds.length > 0) {
            ...
            if (pool.givesReceipts == true) {
              IGodFootPrint(pool.receiptToken).mint(_sender, _tokenIds.length*10**18);
            }
    ...
    function UnstakeNft(uint256 _pid, uint256[] calldata _tokenIds) external nonReentrant  {
        ...
        if (_tokenIds.length > 0) {
            ...
            if (pool.givesReceipts == true) {
                IERC20 godsRcpt = IERC20(pool.receiptToken);
                require(godsRcpt.balanceOf(_sender)>= _tokenIds.length, "rcpt token amt < withdraw amt");
                ERC20Burnable(pool.receiptToken).burnFrom(_sender, _tokenIds.length*10**18);
            }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant  {
        ...
        if (_amount > 0) {
            ...
            if (pool.givesReceipts == true) {
                IERC20 bsdRcpt = IERC20(pool.receiptToken);
                require(bsdRcpt.balanceOf(msg.sender)>= _amount, "rcpt token amt < withdraw amt");
                IBasedReceipt(pool.receiptToken).burnFrom(msg.sender, _amount);
            }
   ...
   for (uint256 i = 0; i < userIds.length; ++i) {
            ...
            if (pool.givesReceipts == true) {
                ...
                ERC20Burnable(pool.receiptToken).burnFrom(_sender, userIds.length*10**18);
                }
...
// future updates ability through multisig owner function calls
  ...
  function governanceRecoverUnsupportedNft(
        IERC721 _token,
        ...
        address _to
           ...
           if (block.timestamp < poolEndTime + 30 days) {
            ...
            for (uint256 pid = 0; pid < length; ++pid) {
                PoolInfo storage pool = poolInfo[pid];
                require(_token != pool.nft, "pool.nft");
            }
        }
         if (_tokenIds.length > 0) {
            for (uint256 i = 0; i < _tokenIds.length; ++i) {
            PoolInfo storage pool = poolInfo[i];
            pool.nft.safeTransferFrom(address(this), _to, _tokenIds[i]);
            }
         }
    }
}
