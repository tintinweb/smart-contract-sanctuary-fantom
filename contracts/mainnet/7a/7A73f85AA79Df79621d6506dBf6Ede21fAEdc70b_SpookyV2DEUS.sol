...
// File: contracts/interfaces/ISpookyRewarder.sol
...
// File: contracts/interfaces/ISpookyChefV2.sol
...
// File: contracts/SpookyV2DEUS.sol
...
contract SpookyV2DEUS is Ownable, Pausable {
    ...
    address public lpToken1ToWrappedRouter;
    ...
    // Routes
    address[] public lpToken1ToWrappedRoute;
    ...
    event SetLpToken1ToWrappedRoute(address[] indexed route, address indexed router);
    ...
    constructor(
        ...
        lpToken1ToWrappedRoute = [lpToken1, wrapped];
        lpToken1ToWrappedRouter = unirouter;
    ...
    function _deposit() internal whenNotPaused {
        ...
        ISpookyChefV2(masterchef).deposit(poolId, wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ISpookyChefV2(masterchef).withdraw(poolId, _amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        if (caller != vault){
         ...
         ISpookyChefV2(masterchef).deposit(poolId, 0);
        uint256 outputBal = IERC20(lpToken1).balanceOf(address(this));
    ...
    function chargeFees(address caller) internal {                   
        uint256 toWrapped = IERC20(lpToken1).balanceOf(address(this));

        IUniSwapRouter(lpToken1ToWrappedRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWrapped, 0, lpToken1ToWrappedRoute, address(this), block.timestamp);                                                
    ...
    function callReward() public view returns (uint256) {
        ...
        address rewarder = ISpookyChefV2(masterchef).rewarder(poolId);
        if (rewarder != address(0)) {
            pendingReward = ISpookyRewarder(rewarder).pendingToken(poolId, address(this));
        } 

        return pendingReward * PLATFORM_FEE / FEE_DIVISOR * CALL_FEE / FEE_DIVISOR;
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = ISpookyChefV2(masterchef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        ISpookyChefV2(masterchef).emergencyWithdraw(poolId, address(this));
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external onlyOwner {
        require (_path[0] != lpToken1 && _path[_path.length - 1] == lpToken1, "Bad path");
    ...
    /** @dev Modular function to set the lpToken1 to wrapped route */
    function setLpToken1ToWrappedRoute(address[] calldata _route, address _router) external onlyOwner {
        require(_route[0] == lpToken1 && _route[_route.length - 1] == wrapped, "Bad path");

        lpToken1ToWrappedRoute = _route;
        lpToken1ToWrappedRouter = _router;

        emit SetLpToken1ToWrappedRoute(_route, _router);
    }
    ...
    function setWrappedToLp0Route(address[] calldata _route, address _router) external onlyOwner {
        require (_route[0] == wrapped && _route[_route.length - 1] == lpToken0, "Bad path");
    ...
    function setWrappedToLp1Route(address[] calldata _route, address _router) external onlyOwner {
        require (_route[0] == wrapped && _route[_route.length - 1] == lpToken1, "Bad path");
