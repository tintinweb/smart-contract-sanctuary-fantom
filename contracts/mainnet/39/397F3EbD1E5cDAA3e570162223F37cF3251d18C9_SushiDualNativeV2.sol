...
// File: contracts/SushiDualNativeV2.sol
...
contract SushiDualNativeV2 is Ownable, Pausable {
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMiniChefV2(masterchef).withdraw(poolId, _amount - wantBal, address(this));
    ...
    function rewardBalance() public view returns (uint256) {
        return IMiniChefV2(masterchef).pendingSushi(poolId, address(this));
    }

    /** @dev Determines the amount of reward in WFTM upon calling the harvest function */
    function callReward() public view returns (uint256) {
        uint256 pendingReward;
        address rewarder = IMiniChefV2(masterchef).rewarder(poolId);
        ...
        uint toWrapped = wrappedOut + pendingReward;

        return toWrapped * PLATFORM_FEE / FEE_DIVISOR * CALL_FEE / FEE_DIVISOR;
    }
