...
contract StakeHelper is Ownable {
    ...
    IgRich public gRich;
    IRichDistributor public distributor;

    constructor (IgRich _gRich, IRichDistributor _distributor) {
        gRich = _gRich;
        distributor = _distributor;
    }

    function create_lock_helper(uint256 _value, uint256 _unlock_time) external{
        gRich.create_lock(msg.sender,_value,_unlock_time);
        distributor.checkpointOtherUser(msg.sender);
    }

    function increase_amount_helper(uint256 _value, uint256 _unlock_time) external{
        gRich.increase_amount(msg.sender,_value);
        distributor.checkpointOtherUser(msg.sender);
    }

    function increase_unlock_time_helper(uint256 _value, uint256 _unlock_time) external{
        gRich.increase_unlock_time(msg.sender,_unlock_time);
        distributor.checkpointOtherUser(msg.sender);
    }
