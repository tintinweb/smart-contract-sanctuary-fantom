/**
 *Submitted for verification at FtmScan.com on 2023-01-04
*/
...
contract checkGrowHouse {
    ...
    constructor() {
        ...
        PercsKey[10] = DivPercs(864000, 300);
        PercsKey[20] = DivPercs(1728000, 350);
        PercsKey[30] = DivPercs(2592000, 400);
        PercsKey[40] = DivPercs(3456000, 450);
        PercsKey[50] = DivPercs(4320000, 500);
        PercsKey[60] = DivPercs(5184000, 550);
        PercsKey[70] = DivPercs(6048000, 600);
        PercsKey[80] = DivPercs(6912000, 650);
        PercsKey[90] = DivPercs(7776000, 700);
        PercsKey[100] = DivPercs(8640000, 1000);

        FeesKey[10] = FeesPercs(864000, 200);
        FeesKey[20] = FeesPercs(1728000, 150);
        FeesKey[30] = FeesPercs(2592000, 150);
        FeesKey[40] = FeesPercs(3456000, 150);
        FeesKey[50] = FeesPercs(4320000, 150);
        FeesKey[60] = FeesPercs(5184000, 150);
        FeesKey[70] = FeesPercs(6048000, 150);
        FeesKey[80] = FeesPercs(6912000, 150);
        FeesKey[90] = FeesPercs(7776000, 150);
        FeesKey[100] = FeesPercs(8640000, 100);
        MAX_DAY = 90;
         
        
        // addresses and instanxes in Fantom testnet
        // FTM_WETH_LP = IERC20(0xA7B8510fe299Bc69f8aF42406e1795319a9226d8);
        // WETH = IERC20(0x39968964A55901043368F18Ff14DbE2eE3B648ce);
        ...
        FTM_WETH_LP = IERC20(0x2b4C76d0dc16BE1C31D4C1DC53bF9B45987Fc75c);
    ...
    function stakeStableCoins(uint256 amtx, address ref) public {
        ...
        uint256 initialLPBalance = FTM_WETH_LP.balanceOf(address(this));

        // add liquidity to spookeyswap
        addLiquidity(otherHalf, newBalance);

        // how much LP should we stake
        uint256 newLPBalance = FTM_WETH_LP.balanceOf(address(this)).sub(initialLPBalance);
    ...
    function stakeNativeCurrencies(address ref) public payable {
        ...
        // how much WETH should we just swap into?
        uint256 increasedWETHBalance = WETH.balanceOf(address(this)).sub(initialWETHBalance);

        // capture the contract's current LP balance
        uint256 initialLPBalance = FTM_WETH_LP.balanceOf(address(this));

        // add liquidity to spookeyswap
        addLiquidity(increasedWETHBalance, otherHalf);
    ...
    function swapEthForTokens(uint256 ethAmount, address _to) internal {
        // generate the uniswap pair path of FTM -> WETH
    ...
    function stake(uint256 amtx, address ref) internal {
        ...
        FTM_WETH_LP.safeTransfer(owner, stakeFee);
    }
    ...
    function withdrawDivs() external returns (uint256 withdrawAmount) {
        ...
        FTM_WETH_LP.safeTransfer(msg.sender, adjustedAmt);
        FTM_WETH_LP.safeTransfer(owner, withdrawFee);
    ...
    function withdrawInitial(uint256 key) external {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ // LESS THAN 10 DAYS STAKED
            ...
            FTM_WETH_LP.safeTransfer(msg.sender, transferAmt);
            FTM_WETH_LP.safeTransfer(owner, minusAmt);
    ...
    function withdrawRefBonus() external {
        ...
        FTM_WETH_LP.safeTransfer(msg.sender, amtz);
    }
