...
contract ZtestToken is ERC20 {
    ...
    // The operator can call all functions except of minting ZtestToken
    ...
    /**
     * @notice Constructs the Defi Ztest Token contract.
     */
    constructor(address _to, uint256 _initialSupply, uint256 _maxSupply) ERC20("Ztest Token", "ZTEST") {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ZTESTs to distribute per second.
        uint256 lastRewardSecond; // Last second that ZTESTs distribution occurs.
        uint256 accZtestPerShare; // Accumulated ZTESTs per share, times 1e18. See below.
    ...
    // The COIN!
    ZtestToken public immutable ztest;
    // Dev address.
    address public devaddr;
    // COIN tokens created per second.
    uint256 public ZtestPerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when ztest mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 ZtestPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _ZtestPerSecond,
        uint256 _startTime
    ) {
        ztest = _ztest;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        ZtestPerSecond = _ZtestPerSecond;
    ...
    // Update the given pool's ztest allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function initializePools(address _nativeLP) external onlyOwner {
        ...
        add(200, IERC20(_nativeLP), 0, true, false);                                        // ZTEST LP
        add(150, IERC20(ztest), 0, true, false);                                             // ZTEST
    ...
    // View function to see pending ztest on frontend.
    function pendingZtest (uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accZtestPerShare = pool.accZtestPerShare;
        if (
            ...
            uint256 coinReward = multiplier
                ...
                uint256 totalRewards = ztest.totalSupply().add(devReward).add(
                coinReward
            );
                if (totalRewards > ztest.maxSupply()) {
                coinReward = ztest.maxSupply().sub(ztest.totalSupply());
            }
            accZtestPerShare = accZtestPerShare.add(
                coinReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accZtestPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= ztest.maxSupply()) {
            ...
            ztest.mint(devaddr, coinReward.div(10));
            ztest.mint(address(this), coinReward);
        } else {
        ...
        if (coinReward != 0) {
            ...
            pool.accZtestPerShare = pool.accZtestPerShare.add(
                coinReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    // Deposit LP tokens to MasterChef for ztest allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accZtestPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                safeZtestTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accZtestPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accZtestPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeZtestTransfer(msg.sender, pending);
    ...
    // Safe ztest transfer function, just in case if rounding error causes pool to not have enough coins.
    function safeZtestTransfer(address _to, uint256 _amount) internal {
        uint256 ztestBal = ztest.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > ztestBal) {
            transferSuccess = ztest.transfer(_to, ztestBal);
        } else {
            transferSuccess = ztest.transfer(_to, _amount);
        }
        require(transferSuccess, "safeZtestTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _ZtestPerSecond) external onlyOwner {
        require(_ZtestPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        ZtestPerSecond = _ZtestPerSecond;
        emit UpdateEmissionRate(msg.sender, _ZtestPerSecond);
    }
