...
// File: contracts/AdventMowse.sol
...
contract AdventMowse is ERC721, ERC721Enumerable, Ownable {
  ...
  modifier canMint (address to) {
    require(bytes(adventItems[to][dayIndex].tokenURI).length == 0, "Cannot mint more than one per day");
    _;
  }
  ...
  function safeMint(address to) canMint(to) public {
    require(!paused, "Contract is paused");
    uint256 tokenId = _tokenIdCounter.current();
    _tokenIdCounter.increment();
    adventItems[to][dayIndex].tokenId = tokenId;
    adventItems[to][dayIndex].tokenURI = tokenURIOfTheDay;
    mapTokenIdToURI[tokenId] = tokenURIOfTheDay;
    _safeMint(to, tokenId);
  ...
  returns(string memory)
  {
    ...
    return mapTokenIdToURI[tokenId];
  }
