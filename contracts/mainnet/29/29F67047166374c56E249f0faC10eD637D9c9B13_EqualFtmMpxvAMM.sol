...
// File contracts/EqualFtmMpxvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualFtmMpxvAMM  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public wrappedToMpxPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToMpxPath
    ) {
        ...
        for (uint i; i < _wrappedToMpxPath.length; ++i) {
            wrappedToMpxPath.push(_wrappedToMpxPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToMpxPath, address(this), block.timestamp);

        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(mpx).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, mpx, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(mpx).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(mpx).safeApprove(unirouter, type(uint).max);
    }
