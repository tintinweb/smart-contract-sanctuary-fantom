...
// File contracts/LqdrFtmEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract LqdrFtmEqualizerV2  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToLqdrPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToLqdrPath,
        ...
        for (uint i; i < _equalToLqdrPath.length; ++i) {
            equalToLqdrPath.push(_equalToLqdrPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToLqdrPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wftm).balanceOf(address(this));
        uint256 t2Bal = IERC20(lqdr).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(wftm, lqdr, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
