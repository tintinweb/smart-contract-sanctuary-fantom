...
contract GoldCoin is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 240h genesis pools
    // total of GoldCoin we pay to users during genesis
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 90000 ether;
    ...
    mapping (address => bool) private mintOperators;        // Contracts that will be able to mint GoldCoin
    ...
    /**
     * @notice Constructs the GoldCoin ERC-20 contract.
     */
     // =================================DO WE WANT TO SET DEPLOYER AS MINTOPERATOR IN CONSTRUCTOR RIGHT AWAY??????
    constructor() ERC20("GoldCoin", "GC") {
        // Mints 5600000 GoldCoin to contract creator for initial LP setup and OTC
        _mint(msg.sender, 5600000 ether);
        //SpookyRouter
    ...
    // =============do we need this ? if yes, how we going to use it w Oracle in the future?
    function _getGoldCoinPrice() internal view returns (uint256 _GoldCoinPrice) {
        try IOracle(GCOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("GoldCoin: failed to fetch GoldCoin price");
        }
    }

    // =============do we need this ? if yes, how we going to use it w Oracle in the future?
    function setGCOracle(address _GCOracle) public onlyOperator {
        require(_GCOracle != address(0), "oracle cannot be 0");
        GCOracle = _GCOracle;
    }

    /**
     * @notice Operator mints GoldCoin to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GoldCoin to mint to
     * @return whether the mint is successful
     */
    ...
    /**
     * @notice distribute to reward pool (only once) , has mint allowance to certain contracts
     */
