...
// File: contracts/token/onft/extension/MagePunkProxy.sol
...
contract MagePunkProxy is ONFT721Core, IERC721Receiver {
    ...
    constructor(uint256 _minGasToTransfer, address _lzEndpoint, address _proxyToken) ONFT721Core(_minGasToTransfer, _lzEndpoint) {
        require(_proxyToken.supportsInterface(type(IERC721).interfaceId), "MagePunkProxy: invalid ERC721 token");
    ...
    function _debitFrom(address _from, uint16, bytes memory, uint _tokenId) internal virtual override {
        require(_from == _msgSender(), "MagePunkProxy: owner is not send caller");
