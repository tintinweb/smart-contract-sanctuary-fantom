...
// SPDX-License-Identifier: MIT 
 ...
 /*   FantomStaker - An investment platform based on Multimatic with innovative features
 
 *
 *   [USAGE INSTRUCTION]
 *
 *   1) Connect browser extension Metamask (see help: https://medium.com/stakingbits/setting-up-metamask-for-fantom-network-32cde917e3bf )
 *   2) Choose one of the investment packages, enter the FANTOM amount (1 FANTOM minimum) using our website "Deposit" button
 *   3) Wait for your earnings
 *   4) Withdraw earnings any time using our website "Withdraw" button
 *
 *   [INVESTMENT CONDITIONS]
 *
 *   - Minimal deposit: 1 FANTOM, no maximal limit
 *   - Total income: Your investment package rate + base interest rate
 *   - Locked plans with snooze option for flexibility
 *
 *   [AFFILIATE PROGRAM]
 *
 *   - 3-level referral commission: 7,5% - 2% - 0.5%
 *
 *   [FUNDS DISTRIBUTION]
 *
...
contract FantomStaker {
	using SafeMath for uint256;

    string public name = "FantomStaker";
	...
	constructor(address _devWallet, address _operationalAccount) {
        ...
        plans.push(Plan(14, 70, 150)); 
        plans.push(Plan(21, 77, 250));
        plans.push(Plan(28, 87, 350));
	}
	...
	function getFeeAllocation(uint256 fees) public view returns (uint256 devFee, uint256 operatingFee) {
		if(totalStaked < 400000 ether) {
			...
			devFee       = fees.mul(700).div(PERCENTS_DIVIDER);
			operatingFee = fees.mul(300).div(PERCENTS_DIVIDER);
		}
	}
