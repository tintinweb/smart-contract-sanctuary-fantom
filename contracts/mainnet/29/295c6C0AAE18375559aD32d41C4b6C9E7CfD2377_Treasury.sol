...
contract Treasury is ContractGuard {
    ...
    function airdropSwapperTomb(uint256 _amount) external onlyOperator {
        require(_amount >= 0, "Amount equal or smaller to 0");
        IBasisAsset(tomb).mint(airdropSwapper, _amount);
    }

    function airdropSwapperShare(uint256 _amount) external onlyOperator {
        require(_amount >= 0, "Amount equal or smaller to 0");
        IBasisAsset(tshare).mint(airdropSwapper, _amount);
    }
