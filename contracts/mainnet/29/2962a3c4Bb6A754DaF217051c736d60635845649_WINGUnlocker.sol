...
contract WINGUnlocker is Ownable {
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    if (block.number < startReleaseBlock) {
      ...
      return wing.lockOf(_account).sub(_unlocks[_account]).mul(releasedBlock).div(blockLeft);
    }
  }

  function unlock() external {
    require(wing.lockOf(msg.sender).sub(_unlocks[msg.sender]) > 0, "WING::unlock::no locked WING");
