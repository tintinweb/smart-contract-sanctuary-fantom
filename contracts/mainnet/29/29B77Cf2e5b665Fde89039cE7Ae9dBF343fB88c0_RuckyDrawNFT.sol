...
// File contracts/interfaces/IRuckyDrawNFT.sol

interface IRuckyDrawNFT is IERC721Enumerable {
    function initialize(
        string memory name_,
    ...
    function setWinner(uint256 tokenId) external;
...
// File contracts/RuckyDrawNFT/RuckyDrawNFT.sol

contract RuckyDrawNFT is
    IRuckyDrawNFT,
    ERC721Enumerable,
    ERC721Royalty,
    Ownable,
    Initializable
{
    ...
    // URI settings
    string private BASE_URI = "https://schema.ruckydraw.com/";
    string private WINNER_URI = "https://schemaw.ruckydraw.com/";
    string private URI_EXTENSION = ".json";

    uint256 public COLLECTION = 0;
    uint256 public WINNER = 0;

    modifier NoWinnerSet() {
        require(WINNER == 0, "Winner has already been set");
        require(
            totalSupply() == MAX_SUPPLY,
            "Winner cannot be set until all mints are completed"
        );
        _;
    }
    ...
    function initialize(
        ...
        require(_collection >= 1, "Collection must be at least 1");
        _name = name_;
        _symbol = symbol_;
    ...
    function setWinner(uint256 tokenId)
        public
        whenInitialized
        NoWinnerSet
        onlyOwner
