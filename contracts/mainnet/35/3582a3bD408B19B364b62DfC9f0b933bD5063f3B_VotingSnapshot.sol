    ...
    ProxyImplementation
{
    ...
    // Timestamps
    uint256 public solidlyVotesLastCleared;
        ...
        returns (address[] memory, int256[] memory)
    {
        ...
        for (uint256 voteIndex; voteIndex < _topVotes.length; voteIndex++) {
            ...
            // Set pool votes
            uint256 poolWeight = Math.abs(vote.weight);
    ...
    function submitVote() external {
        ...
        require(!ve.voted(tokenId), "Clear votes before submitting");
        (
    ...
    /**
     * @notice Clears votes on Solidly for 0xDAO, only once per epoch
     * @dev if gov wants to clear votes, they can do so with voterProxy directly
     */
    function clearVotesOnSolidly() external {
        require(
            block.timestamp >= nextVoteSubmission() &&
                block.timestamp < nextEpoch(),
        ...
        require(
            solidlyVotesLastCleared + window < nextEpoch(),
            "Epoch vote already cleared"
        );

        // TODO: need to check if bribes are claimed

        solidlyVotesLastCleared = block.timestamp;

        // Voting with empty weights will reset 0xDAO's vote on Solidly
