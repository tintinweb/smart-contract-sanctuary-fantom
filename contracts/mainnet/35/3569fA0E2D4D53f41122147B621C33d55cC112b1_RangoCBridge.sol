...
contract RangoCBridge is IRangoCBridge, MessageSenderApp, MessageReceiverApp, BaseContract {
    ...
    function executeMessageWithTransfer(
        ...
        if (m.path.length > 1) {
            ...
            _sendToken(m.path[0], _amount, m.recipient, m.nativeOut, true);
            dstAmount = _amount;
            status = OperationStatus.Succeeded;
...
contract BaseContract is Pausable, Initializable, Ownable, ReentrancyGuard {
    ...
    function onChainSwapsInternal(SwapRequest memory request, Call[] calldata calls) internal returns (bytes[] memory, uint) {
        ...
        uint balanceBefore = request.fromToken == NULL_ADDRESS
            ? address(this).balance
            : ercToken.balanceOf(address(this));

        bytes[] memory result = callSwapsAndFees(request, calls);

        uint balanceAfter = request.fromToken == NULL_ADDRESS
            ? address(this).balance
            : ercToken.balanceOf(address(this));
