...
contract Wi2ard is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the WI2ARD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Wi2ard Finance", "WI2ARD") {
        // Mints 1 WI2ARD to contract creator for initial pool setup
    ...
    function _getWi2ardPrice() internal view returns (uint256 _wi2ardPrice) {
        try IOracle(wi2ardOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Wi2ard: failed to fetch WI2ARD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _wi2ardPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_wi2ardPrice >= taxTiersTwaps[tierId]) {
    ...
    function setWi2ardOracle(address _wi2ardOracle) public onlyOperatorOrTaxOffice {
        require(_wi2ardOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints WI2ARD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of WI2ARD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentWi2ardPrice = _getWi2ardPrice();
            currentTaxRate = _updateTaxRate(currentWi2ardPrice);
            if (currentWi2ardPrice < burnThreshold) {
                burnTax = true;
            }
        }
