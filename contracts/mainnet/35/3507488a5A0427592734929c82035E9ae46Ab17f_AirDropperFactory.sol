...
// --------------------------------------------------------------------------------------
//
// (c) AirDropFactory 01/02/2022 | SPDX-License-Identifier: AGPL-3.0-only
//  Designed by, DeGatchi (https://github.com/DeGatchi).
//
// --------------------------------------------------------------------------------------

contract AirDropperFactory is Ownable, Pausable {
    event Initiated(address[] indexed models);
    event ModelAdded(address indexed ref);
    event ModelRemoved(uint256 indexed ref);
    event Created(address indexed creator, address indexed clone);
    ...
    function createdLength() external view returns (uint256) {
        return created.length;
    }

    function createdList() external view returns (address[] memory) {
        return created;
    }
    ...
    /**
        @dev Adds a new model available for use.
    ...
    function addModel(address ref) external onlyInit onlyOwner {
        ...
        emit ModelAdded(address(ref));
    }

    /**
        @dev Removes model from being available to use. 
    ...
    function cloneModel(uint16 _model) external onlyInit whenNotPaused returns (address result) {
        ...
        created.push(cloneContract);
        models[_model].totalCreated++;
        
        emit Created(cloneContract, msg.sender);
