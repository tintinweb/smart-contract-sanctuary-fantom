...
contract BShareDistributor is Ownable {
    using SafeERC20 for IERC20;

    string public name = "BShare Distributor";
    ...
    event BshareDistributed(uint256 _amount);

    constructor(address[] memory _teamWallets, address[] memory _devWallets) {
        bshare = IERC20(0x49C290Ff692149A4E16611c694fdED42C954ab7a);
        totalDistributed = 0;
        totalAllocation = 0;

        for (uint256 i = 0; i < _teamWallets.length; i ++) {
            wallets.push(_teamWallets[i]);
            allocation[_teamWallets[i]] = 1500000000000000000;
            totalAllocation += 1500000000000000000;
        }
        for (uint256 i = 0; i < _devWallets.length; i ++) {
            wallets.push(_devWallets[i]);
            allocation[_devWallets[i]] = 4500000000000000000;
            totalAllocation += 4500000000000000000;
        }
    }
    ...
    function distributeBshare() external onlyOwner {
        uint256 balance = bshare.balanceOf(address(this));
        ...
        for (uint256 i = 0; i < wallets.length; i ++) {
            ...
            uint256 amount = balance * ((allocation[wallets[i]] * 1e18) / totalAllocation);
            totalDistributed += amount;
            bshare.safeTransfer(wallets[i], amount);
        }

        emit BshareDistributed(totalDistributed - oldTotalDistributed);
    }

    function deposit(uint256 _amount) external onlyOwner {
        bshare.safeTransferFrom(msg.sender, address(this), _amount);
    }
