...
abstract contract BaseFarm is IFarm {
    ...
    function _position(address user) internal view returns (Position memory p) {
        ...
        p.positionToken = positionToken;
        p.amount = _balanceOfUnderlying(user);
    }
    ...
    function _balanceOfUnderlying(address user)
        internal
        view
        virtual
        returns (uint256);

    function _updateBalanceOfUnderlying(address user)
        internal
        virtual
        returns (uint256);
}
...
contract Geist is BaseFarm {
    ...
    function _deployImpl(IERC20Metadata u, uint256 amount) internal override {
        require(address(u) != address(0), "GS1");
        if (amount == 0) return;
        ...
        returns (uint256)
    {
        require(address(u) != address(0), "GS1");
        ILendingPool lendingPool = ILendingPool(addrProvider.getLendingPool());
        uint256 amount = _balanceOfUnderlying(address(this));
    ...
    function _updateBalanceOfUnderlying(address user)
        ...
        returns (uint256)
    {
        return _balanceOfUnderlying(user);
    }
