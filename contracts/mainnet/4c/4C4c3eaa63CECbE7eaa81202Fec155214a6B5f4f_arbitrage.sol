...
//		IERC20(tokenIn).safeIncreaseAllowance(sell, amount);
        ...
        uint amountA = swap(start_amount, buy, tokenIn, tokenOut);
        uint last_amount = swap(amountA, sell, tokenOut, tokenIn);
        assert(last_amount > start_amount / 2);
        isComplete = true;
        return isComplete;
    }

 /*   function triggerETH(address buy, address sell, address buyLP, address sellLP, address tokenOut, uint _feeBuy, uint _feeSell) public payable {
        uint amount = payable(address(this)).transfer(msg.value);
//      uint amount = 1000000000000000000;
        _trigger(amount, buy, sell, buyLP, sellLP, tokenIn, tokenOut, _feeBuy, _feeSell);
    }
*/

    function swap(uint amountIn, address addr, address token0, address token1) internal returns(uint amountOut){
        _swapExactTokensForTokens(amountIn, 0, _getPath(token0, token1), address(this), block.timestamp.add(60), addr);
