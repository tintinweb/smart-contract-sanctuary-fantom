...
contract Treasury is ContractGuard {
    ...
    // core components
    address public tpiggy;
    address public tpbond;
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of TPIGGY price
    ...
    uint256 public mintingFactorForPayingDebt; // print extra TPIGGY during debt phase
    ...
    modifier checkOperator {
        require(
            IBasisAsset(tpiggy).operator() == address(this) &&
                IBasisAsset(tpbond).operator() == address(this) &&
    ...
    function getPiggyPrice() public view returns (uint256 piggyPrice) {
        try IOracle(piggyOracle).consult(tpiggy, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult TPIGGY price from the oracle");
        }
    }

    function getPiggyUpdatedPrice() public view returns (uint256 _piggyPrice) {
        try IOracle(piggyOracle).twap(tpiggy, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("Treasury: failed to consult TPIGGY price from the oracle");
        }
    ...
    function getBurnablePiggyLeft() public view returns (uint256 _burnablePiggyLeft) {
        ...
        if (_piggyPrice <= piggyPriceOne) {
            ...
            uint256 _bondSupply = IERC20(tpbond).totalSupply();
            if (_bondMaxSupply > _bondSupply) {
                uint256 _maxMintableBond = _bondMaxSupply.sub(_bondSupply);
                uint256 _maxBurnablePiggy = _maxMintableBond.mul(_piggyPrice).div(1e15);
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_piggyPrice > piggyPriceCeiling) {
            uint256 _totalPiggy = IERC20(tpiggy).balanceOf(address(this));
            uint256 _rate = getBondPremiumRate();
            if (_rate > 0) {
                _redeemableBonds = _totalPiggy.mul(1e15).div(_rate);
            }
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_piggyPrice <= piggyPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = piggyPriceOne.mul(1e18).div(_piggyPrice); // to burn 1 TPIGGY
    ...
    function initialize(
        ...
        piggyPriceOne = 10**15; // This is to allow a PEG of 1000 TPIGGY per TSHARE
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn TPIGGY and mint TPBOND)
        maxDebtRatioPercent = 3500; // Upto 35% supply of TPBOND to purchase
        ...
        seigniorageSaved = IERC20(tpiggy).balanceOf(address(this));
    ...
    function getPiggyCirculatingSupply() public view returns (uint256) {
        IERC20 piggyErc20 = IERC20(tpiggy);
    ...
    function buyBonds(uint256 _piggyAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(piggyPrice == targetPrice, "Treasury: TPIGGY price moved");
        require(
        ...
        uint256 _bondAmount = _piggyAmount.mul(_rate).div(1e15);
        uint256 piggySupply = getPiggyCirculatingSupply();
        uint256 newBondSupply = IERC20(tpbond).totalSupply().add(_bondAmount);
        require(newBondSupply <= piggySupply.mul(maxDebtRatioPercent).div(10000), "over max debt ratio");

        IBasisAsset(tpiggy).burnFrom(msg.sender, _piggyAmount);
        IBasisAsset(tpbond).mint(msg.sender, _bondAmount);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _piggyAmount = _bondAmount.mul(_rate).div(1e15);
        require(IERC20(tpiggy).balanceOf(address(this)) >= _piggyAmount, "Treasury: treasury has no more budget");

        seigniorageSaved = seigniorageSaved.sub(Math.min(seigniorageSaved, _piggyAmount));

        IBasisAsset(tpbond).burnFrom(msg.sender, _bondAmount);
        IERC20(tpiggy).safeTransfer(msg.sender, _piggyAmount);
    ...
    function _sendToMasonry(uint256 _amount) internal {
        IBasisAsset(tpiggy).mint(address(this), _amount);

        uint256 _daoFundSharedAmount = 0;
        if (daoFundSharedPercent > 0) {
            _daoFundSharedAmount = _amount.mul(daoFundSharedPercent).div(10000);
            IERC20(tpiggy).transfer(daoFund, _daoFundSharedAmount);
        ...
        if (communityFundSharedPercent > 0) {
            ...
            IERC20(tpiggy).transfer(communityFund, _communityFundSharedAmount);
        ...
        if (devFundSharedPercent > 0) {
            ...
            IERC20(tpiggy).transfer(devFund, _devFundSharedAmount);
            emit DevFundFunded(now, _devFundSharedAmount);
        }

        _amount = _amount.sub(_daoFundSharedAmount).sub(_communityFundSharedAmount).sub(_devFundSharedAmount);

        IERC20(tpiggy).safeApprove(masonry, 0);
        IERC20(tpiggy).safeApprove(masonry, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochPiggyPrice > piggyPriceCeiling) {
                // Expansion ($TPIGGY Price > 0.001 $TSHARE): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(tpbond).totalSupply();
                ...
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    ...
                    _savedForMasonry = piggySupply.mul(_percentage).div(1e15);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = piggySupply.mul(_percentage).div(1e15);
                ...
                if (_savedForBond > 0) {
                    ...
                    IBasisAsset(tpiggy).mint(address(this), _savedForBond);
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(tpiggy), "tpiggy");
        require(address(_token) != address(tpbond), "bond");
