...
contract ERC721Snapshot is IERC721Snapshot, Cloneable {
    ...
    function initialize(IERC721Enumerable ERC721Contract) public initializer {
        uint256 tokenId;
        do {
            try ERC721Contract.tokenByIndex(tokenId) returns (uint256 id) {
                tokenId = id;
                holders.push(ERC721Contract.ownerOf(tokenId));
            } catch {
                break;
            }
        } while (true);
