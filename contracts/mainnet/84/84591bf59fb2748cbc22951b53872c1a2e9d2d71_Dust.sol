...
contract Dust is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the DUST ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("DigiFi", "DUST") {
        require(_taxRate < 1000, "tax equal or bigger to 10%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        excludeAddress(address(this));
        // Mints 2000 DUST to contract creator for initial pool setup
    ...
    function _getDustPrice() internal view returns (uint256 _dustPrice) {
        try IOracle(dustOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("DUST: failed to fetch DUST price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _dustPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_dustPrice >= taxTiersTwaps[tierId]) {
    ...
    function setDustOracle(address _dustOracle) public onlyOperatorOrTaxOffice {
        require(_dustOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints DUST to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DUST to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDustPrice = _getDustPrice();
            currentTaxRate = _updateTaxRate(currentDustPrice);
            if (currentDustPrice < burnThreshold) {
                burnTax = true;
            }
        }
