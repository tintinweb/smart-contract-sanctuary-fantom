 ...
 */
contract BooXBooAutoCompoundSolidex is IStrategy, Ownable, Pausable {
     ...
     * {solid, sex} - Earned reward from solidex.
     * {lpPair} - Solidly Lp token.
     * {lpToken0, lpToken1} - Tokens that the strategy maximizes. IUniswapV2Pair tokens.
     * {rewardTokens} - Tokens generated by staking our funds.
     */
    address public wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public solid = address(0x4cdF39285D7Ca8eB3f090fDA0C069ba5F4145B37);
    address public sex = address(0x4cdF39285D7Ca8eB3f090fDA0C069ba5F4145B37);
    address public lpPair;
    address public lpToken0;
    address public lpToken1;
    address[] public rewardTokens = [solid, sex];

    /**
     * @dev Third Party Contracts:
     * {uniRouter} - the uniRouter for target DEX
     * {lpDepositor} - solidex lp depositor contract
     */
    ...
    /**
     * @dev solidly pool type (stable or variable)
     */
    ...
    function deposit() public override whenNotPaused {
        ...
        if (pairBal > 0) {
            ILpDepositor(lpDepositor).deposit(lpPair, pairBal);
        }
    ...
    /**
     * @dev Withdraw funds and sends them back to the vault
     * It withdraw {lpPair} from the masterChef.
     * The available {lpPair} minus fees is returned to the vault.
     */
    function withdraw(uint256 _amount) external override {
        require(msg.sender == vault, "!vault");

        // Get the lp balance of this contract
        uint256 pairBal = IERC20(lpPair).balanceOf((address(this)));

        // If withdraw amount less than current lp balance, then withdraw from masterchef
        if (pairBal < _amount) {
            ILpDepositor(lpDepositor).withdraw(lpPair, _amount);
     ...
     * 1. It claims rewards (solid & sex) from the solidex.
     * 2. It swaps rewards for {wftm}
     * 2. It charges the system fees to simplify the split.
     * 3. It swaps the {wftm} token for {boo} & {xboo}
     * 4. Adds more liquidity to the pool.
     * 5. It deposits the new LP tokens to solidex.
     */
    ...
    function rewardPerSec(address _token) external view returns (uint256) {
        ILpDepositor.Amounts memory amounts = ILpDepositor(lpDepositor)
            .rewardIntegralFor(address(this), lpPair);
        if (_token == solid) {
            return amounts.solid;
        }
        return amounts.sex;
    }
     ...
     * as is treasuretyFeeVault
     */
    function _chargeFees() internal {
        uint256 totalFeeInWftm = IERC20(wftm)
        ...
        uint256 callFeeToUser = totalFeeInWftm.mul(callFee).div(
            PERCENT_DIVISOR
        );
        IERC20(wftm).safeTransfer(msg.sender, callFeeToUser);

        uint256 treasuryFeeToVault = totalFeeInWftm.mul(treasuryFee).div(
            PERCENT_DIVISOR
        );
    ...
    /**
     * @dev Use solidly exchange to swap earned token (sex/solid) to wftm
     */
    function _earnToWFTM(address _earnedToken) internal {
        uint256 earnedBal = IERC20(solid).balanceOf(address(this));
        if (earnedBal == 0) {
            return;
        }

        address[] memory earnedTokenToWftmRoute = new address[](2);
        earnedTokenToWftmRoute[0] = _earnedToken;
        earnedTokenToWftmRoute[1] = wftm;

        IUniswapRouterETH(liquidityRouter)
            .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                earnedBal,
                0,
                earnedTokenToWftmRoute,
                address(this),
    ...
    /**
     * @dev Swaps {wftm} for {boo}, stake half {boo} for {xboo} using SpookySwap.
     */
    function _addLiquidity() internal {
        ...
        address[] memory wftmToLpToken0Route = new address[](2);
        wftmToLpToken0Route[0] = wftm;
        wftmToLpToken0Route[1] = lpToken0;
        ...
        uint256 amountXBoo = IERC20(lpToken0).balanceOf(address(this)) / 2;

        if (amountXBoo > 0) {
            IERC20(lpToken0).safeApprove(lpToken1, amountXBoo);
            IBooMirrorWorld(lpToken1).enter(amountXBoo.mul(98).div(100)); // slippage 2%
        }

        uint256 lp0Bal = IERC20(lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lpToken1).balanceOf(address(this));

        ISolidlyRouter(liquidityRouter).addLiquidity(
            lpToken0,
            lpToken1,
            stable,
            lp0Bal,
    ...
    /**
     * @dev It calculates how much {lpPair} the strategy has allocated in the solidex.
     */
    function balanceOfPool() public view returns (uint256) {
        uint256 amount = ILpDepositor(lpDepositor).userBalances(
            address(this),
            lpPair
        );
        return amount;
    ...
    /**
     * @dev Function that has to be called as part of strat migration. It sends all the available funcs back to the
     * vault, redy to be migrated to the new strat.
     */
    function retireStrat() external override {
        require(msg.sender == vault, "!vault");

        ILpDepositor(lpDepositor).withdraw(lpPair, balanceOfPool());
    ...
    /**
     * @dev Pauses deposits. Withdraws all funds from the solidex, leaving rewards behind
     */
    ...
    /**
     * @dev approve to use all the tokens
     */
    function _giveAllowances() internal {
        uint256 max = type(uint256).max;
        IERC20(lpPair).safeApprove(lpDepositor, 0);
        IERC20(lpPair).safeApprove(lpDepositor, max);

        IERC20(sex).safeApprove(lpDepositor, 0);
        IERC20(sex).safeApprove(lpDepositor, max);

        IERC20(solid).safeApprove(lpDepositor, 0);
        IERC20(solid).safeApprove(lpDepositor, max);

        IERC20(wftm).safeApprove(lpDepositor, 0);
        IERC20(wftm).safeApprove(lpDepositor, max);

        IERC20(lpToken0).safeApprove(liquidityRouter, 0);
        IERC20(lpToken0).safeApprove(liquidityRouter, max);

        IERC20(lpToken1).safeApprove(liquidityRouter, 0);
        IERC20(lpToken1).safeApprove(liquidityRouter, max);

        IERC20(wftm).safeApprove(uniRouter, 0);
        IERC20(wftm).safeApprove(uniRouter, max);

        IERC20(lpToken0).safeApprove(uniRouter, 0);
        IERC20(lpToken0).safeApprove(uniRouter, max);
    }

    /**
     * @dev remove the permission of using the tokens
     */
     ...
     */
    function updateTotalFee(uint256 _totalFee) external override onlyOwner {
     ...
     */
    function updateCallFee(uint256 _callFee) external override onlyOwner {
        callFee = _callFee;
...
interface ILpDepositor {
    ...
    function rewardIntegralFor(address user, address pool)
        external
        view
        returns (Amounts memory);
}
