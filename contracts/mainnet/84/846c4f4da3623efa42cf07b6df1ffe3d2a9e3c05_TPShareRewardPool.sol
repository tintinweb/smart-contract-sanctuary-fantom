 ...
 *             (_)        (_)   (__)                           (_)      
 *
 *  Pop a squat at toiletdao.com
 *
 *  twitter.com/ToiletDao
 */
// Note that this pool has no minter key of tpSHARE (rewards).
// Instead, the governance will call tpSHARE distributeReward method and send reward to this pool at the beginning.
contract TPShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. tpSHAREs to distribute per block.
        uint256 lastRewardTime; // Last time that tpSHAREs distribution occurs.
        uint256 accTSharePerShare; // Accumulated tpSHAREs per share, times 1e18. See below.
    ...
    // The time when tpSHARE mining starts.
    uint256 public poolStartTime;

    // The time when tpSHARE mining ends.
    uint256 public poolEndTime;

    uint256 public tSharePerSecond = 0.00382587 ether; // 59500 tpSHARE / (180 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "TPShareRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "TPShareRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's tpSHARE allocation point. Can only be called by the owner.
    ...
    // View function to see pending tpSHAREs on frontend.
    ...
    // Safe tpSHARE transfer function, just in case if rounding error causes pool to not have enough tSHAREs.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (tpSHARE or lps) if less than 90 days after pool ends
            require(_token != tshare, "tpshare");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
