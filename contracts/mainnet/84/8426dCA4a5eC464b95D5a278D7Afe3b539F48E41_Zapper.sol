...
contract Zapper is Ownable, IZapper {
    ...
    function zapInTokens(address _token0, address _token1,uint256 _token0Amount,
        uint256 _token1Amount,address _routerAddr, address _recipient, uint256 _minAmountLp ) override external whitelist(_routerAddr)
    {
    ...
    function zapInToken(address _in, uint256 amount, address out, address routerAddr, address recipient, uint256 minAmountLp) override external whitelist(routerAddr) {
    ...
    function zapOutToTokens(address _in, uint256 _amount,
        ...
        IERC20(_token0).safeTransfer(msg.sender, _amountToken0);
        IERC20(_token1).safeTransfer(msg.sender, _amountToken1);
    }
    ...
    // @amount - amount of our _in
    // @out - token we want to get out
    function _estimateSwap(address _in, uint256 amount, address out, address routerAddr) override public view whitelist (routerAddr) returns (uint256) {
        IUniswapV2Router router = IUniswapV2Router(routerAddr);
...
interface IZapper {
    ...
    function zapInTokens(address _token0, address _token1,uint256 _token0Amount,
        uint256 _token1Amount,address _routerAddr, address _recipient, uint256 _minAmountLp ) external;
