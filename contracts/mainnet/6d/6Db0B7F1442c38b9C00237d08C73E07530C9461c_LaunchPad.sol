...
contract LaunchPad is Operator, ILaunchPad {
    ...
    uint256 public sTokenAmount; // Amount of Total sToken to be sell

    ERC20 public rToken; // Fund raising Token
    uint256 public fundToRaise;

    uint256 public missing_decimal;

    uint256 public priceDenominator = 10000;
    uint256 public priceNumerator; // 1 sToken = {price/priceDenominator} rToken. ex: price = 5000, 1 sToken = 5000/10000 = 0.5 rToken
    ...
    uint256 public totalCommitted; // Total pToken committed

    uint256 public sTokenLeft; // Amount of sToken left over;
    ...
    function stakedBalanceOf(address investor) public view returns (uint256) {
        return stakedBalances[investor];
    }

    function allocationOf(address investor) public view returns (uint256) {
        return getAllocation(investor);
    }

    function isUserClaimed(address investor) public view returns (bool) {
        return isClaimed[investor];
    }

    function getRemainToken(address investor) public view returns (uint256) {
        return investorInfos[investor].rTokenLeft;
    }

    /* ========== GOVERNANCE ========== */

    function initialize(address _sToken, address _rToken, uint256 _startTime, uint256 _priceNumerator, uint256 _sTokenAmount, uint256 _fundToRaise, uint256 _duration) public notInitialized onlyOperator {
        ...
        isInitialized = true;
        allocationPerCommit = sTokenAmount.mul(1e18).div(fundToRaise);
    ...
    function claimToken() public override checkTimeEnd initialized {
        ...
        require(investorInfo.rTokenLeft > 0, "No Token left to be claimed");

        rToken.safeTransfer(msg.sender, investorInfo.rTokenLeft);
        sToken.safeTransfer(msg.sender, investorInfo.sTokenAllocation);

        isClaimed[msg.sender] = true;

        emit ClaimedToken(msg.sender, investorInfo.sTokenAllocation, investorInfo.rTokenLeft);
    }
    ...
    function updateInvestorInfo(address investor) internal {
        ...
        investorInfo.rTokenToBeSpent = priceNumerator.div(priceDenominator).mul(investorInfo.sTokenAllocation);
        investorInfo.rTokenLeft = stakedBalances[investor].sub(investorInfo.rTokenToBeSpent);
    ...
    function getAllocation(address investor) internal view returns (uint256) {
        ...
        uint256 allocation = investorCommitted.mul(allocationPerCommit).div(1e18);
        return allocation;
    }
