...
contract VotingData is Ownable {
	...
	function convertToHEC(address _stakingToken, uint256 _amount) internal view returns (uint256) {
		uint256 hecWeight = 0;
		// Check LP token
		SpookySwapPair _lpToken = SpookySwapPair(_stakingToken);
        ERC20 _token = ERC20(_stakingToken);
		if (compareStringsbyBytes(_token.symbol(), 'spLP') && _lpToken.factory() == address(spookySwapFactory)) {
			// HEC-USDC
		...
		if (address(_stakingToken) == address(HEC) || address(_stakingToken) == address(sHEC)) {
			// HEC, sHEC
			hecWeight = _amount;
		}

		if (address(_stakingToken) == address(wsHec)) {
			// wsHEC
	...
	// Get all voting datas
	function getVotingData(FNFT _fnft, TokenVault _tokenVault) external view returns (uint256[] memory _fnftIds, uint256[] memory _powers) {
		uint256 totalSupply = _fnft.totalSupply();
		uint256[] memory _rIds = new uint256[](totalSupply);
		uint256[] memory _rPowers = new uint256[](totalSupply);
		for(uint256 i = 0; i < totalSupply; i++) {
			uint _fnftId = _fnft.tokenByIndex(i);
			IERC20 _erc20Token = IERC20(_tokenVault.getFNFT(_fnftId).asset);
			uint256 _lockedAmount = _tokenVault.getFNFT(_fnftId).depositAmount;
			uint256 power = convertToHEC(address(_erc20Token), _lockedAmount);
			_rIds[i] = _fnftId;
			_rPowers[i] = power;
		}
		return (_rIds, _rPowers);
	}

	// Get all voting count
	function getVotingDataCount(FNFT _fnft) external view returns (uint256 totalVotes) {
		uint256 totalSupply = _fnft.totalSupply();
		return totalSupply;
	}

	// Get voting datas by limit
	function getVotingData(FNFT _fnft, TokenVault _tokenVault, uint offset, uint limit) external view returns (uint256[] memory _fnftIds, uint256[] memory _powers) {
		uint256 totalSupply = _fnft.totalSupply();
		uint256[] memory _rIds = new uint256[](totalSupply);
		uint256[] memory _rPowers = new uint256[](totalSupply);
