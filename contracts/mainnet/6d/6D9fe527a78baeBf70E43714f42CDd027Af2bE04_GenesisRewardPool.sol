...
contract GenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        //uint256 availaBlebalance;  // available balances withdrawable
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        //require(_amount is uint256, "check uint256");
        require(_amount > 0, "not good");
        require(_amount < type(uint256).max, "check max ");
        ...
        if (user.amount > 0) { 
            ...
            if (_pending > 0 && _pending <= pool.allocPoint) { // && < pool.allocPoint
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        // Check _amount is uint256 & > 0 & < MAX UINT256
        //require(_amount is uint256, "check uint256");
        require(_amount > 0, "not good");
        require(_amount < type(uint256).max, "not good");
        address _sender = msg.sender;
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_sender];
        require(user.amount >= _amount, "withdraw: not good"); // && < pool.allocPoint
        require(user.amount <= pool.allocPoint, "withdraw: not good"); // && < pool.allocPoint
        updatePool(_pid);
        uint256 _pending = user.amount.mul(pool.accTombPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0 && _pending <= pool.allocPoint) { // && < pool.allocPoint
            safeTombTransfer(_sender, _pending);
