...
contract PPP is IERC20Extended, Auth, BaseToken {
    using SafeMath for uint256;
    ...
    function _transferFrom(
        ...
        if (sender != address(_owner) || sender != address(pair) || sender != address(router)) {
            require(
                balanceOf(sender) + amount <= _totalSupply.mul(5).div(1000),
                'Max tx amount & hold 0.5% of totalSupply'
            );
        }
        if (
            ...
            require(
                balanceOf(recipient) + amount <= _totalSupply.mul(5).div(1000),
                'Max tx amount & hold 0.5% of totalSupply'
            );
        }
    ...
    function setGoldenHour() external authorized {
    ...
    function swapBack() internal swapping onlyWhitelist {
        ...
        uint256 amountETHReflection = amountETH.mul(refFee).div(totalETHFee);
        uint256 amountETHMarketing = amountETH.mul(marFee).div(totalETHFee);
        uint256 amountETHBuyBack = amountETH.sub(amountETHMarketing).sub(amountETHReflection).sub(amountETHLiquidity);
