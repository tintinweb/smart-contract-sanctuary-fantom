...
abstract contract StrategyLPBase is StratManager, FeeManager {
    ...
    function chargeFees() internal {
        ...
        if (toProfit > 0)
        {
            ...
            uint256 sideProfitAmount = FullMath.mulDiv(sideProfitBalance, sideProfitFee, totalFee);
            if (sideProfitAmount > 0)
            {
                sideProfitToken.safeTransfer(vault, sideProfitAmount);
                ICrackVault(vault).addProfit(sideProfitAmount);
            }
        }
    }
    ...
    function totalBalanceOfStaked() public view returns (uint256) {
        return balanceOfStaked() + balanceOfPool();
    }
