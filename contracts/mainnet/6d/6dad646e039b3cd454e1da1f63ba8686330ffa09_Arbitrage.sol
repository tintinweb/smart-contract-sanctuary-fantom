...
contract Arbitrage {
  ...
  function actualToken(address _address) public view returns(address){
      return tokenToVault[_address] == address(0)? _address : tokenToVault[_address];
  }
  ...
  function hook(
    ...
    require(
      msg.sender == IUniswapV2Factory(factory).getPair(token0, token1, true), 
      'Unauthorized'
    ); 
    ...
    address vault = actualToken(address(token));

    if(vault != address(token)){
        token.approve(vault, amountToken);
        IYfi(vault).deposit(amountToken);
    }
    
    uint vBal = IERC20(token).balanceOf(address(this));
    IERC20(token).approve(address(sushiRouter), vBal);
    ...
    /*
    {
        ...
        //amountRequired = IUniswapV2Pair(msg.sender).getAmountOut(amountToken, path[1]);
    }
    */
    uint vAm = IYfi(vault).balanceOf(address(this));
    sushiRouter.swapExactTokensForTokensSimple(
      vAm, 
      0, 
      vault, 
      actualToken(path[1]),
