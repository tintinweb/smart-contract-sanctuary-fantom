...
contract LPChainlinkOracleV1 is IAggregator {
    ...
    function latestAnswer() external view override returns (int256) {
        ...
        (, int256 priceFeed, , , ) = tokenOracle.latestRoundData();//Both tokens of LP need to be Chainlink supported (i.e. MIM/FTM); tokenOracle.latestRoundData() then retrieves MIM/FTM price ratio

        uint256 normalizedPriceFeed = uint256(priceFeed) * (10**(WAD - oracleDecimals));
        
        //Then you can calculate the entire LP pair value b/c the other token is the equivalent dollar value
