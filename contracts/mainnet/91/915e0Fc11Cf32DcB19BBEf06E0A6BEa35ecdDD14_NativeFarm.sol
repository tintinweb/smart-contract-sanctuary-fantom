...
contract NativeFarm is Ownable, ReentrancyGuard {
    ...
    // Token address
    address public NATIVE = 0x65b536DB30d1ed57ff905F4D0e6f4c1Fdc67C06C;
    // Owner reward per block: 10% ==> 11.11%
    uint256 public ownerNATIVEReward = 1111;
    // Native total supply: 2 mil = 2000000e18
    uint256 public NATIVEMaxSupply = 20000000000000000000000000e18;
    ...
    // Approx 8/3/2021
    uint256 public startBlock = 65150722; // fantom mainnet // https://bscscan.com/block/countdown/5496000
    ...
    function deposit(uint256 _pid, uint256 _wantAmt, address _user) public nonReentrant {
        updatePool(_pid);
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        if (user.shares > 0) {
            ...
            if (pending > 0) {
                safeNATIVETransfer(_user, pending);
            }
        }
        if (_wantAmt > 0) { 
            ...
            uint256 sharesAdded =
                IStrategy(poolInfo[_pid].strat).deposit(_user, _wantAmt);
