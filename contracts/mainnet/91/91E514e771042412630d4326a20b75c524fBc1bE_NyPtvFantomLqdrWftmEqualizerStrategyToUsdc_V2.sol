...
// No permissions granted before Wednesday, 4 Sept 2024, then GPL-3.0 after this date.
 ...
 * out quoting reference 'NyPtvFantomLqdrWftmEqualizerStrategyToUsdc'
 *
...
contract NyPtvFantomLqdrWftmEqualizerStrategyToUsdc_V2 is NYBaseProfitTakingStrategyV3 {
  ...
  address public constant GAUGE = address(0xB0f9069BABb65F104acDBf9e1FC521F3f1870d5a);
  address public constant LP_TOKEN = address(0x0272A69b9f7D12489E0E0C756cD645E538A00DDE);

  address[] public rewards;

  address[] public equalToUsdcPath;
  ...
  function _makeRoutes() internal view returns (IEqualizerRouter.Route[] memory) {
    IERC20Upgradeable equal = IERC20Upgradeable(EQUAL);
    uint256 equalBalance = equal.balanceOf(address(this));
    IEqualizerRouter.Route[] memory routes = new IEqualizerRouter.Route[](equalToUsdcPath.length - 1);

    if (equalBalance == 0) {
      return routes;
    }

    if (equalToUsdcPath.length >= 2) {
      IEqualizerRouter router = IEqualizerRouter(EQUALIZER_ROUTER);

      uint256 amountToRoute = equalBalance;
      for (uint256 i = 0; i < equalToUsdcPath.length; i++) {
        if (i == equalToUsdcPath.length - 1) {
          break;
        }

        address left = equalToUsdcPath[i];
        address right = equalToUsdcPath[i + 1];
        (uint256 amount, bool useStable) = router.getAmountOut(amountToRoute, left, right);
        routes[i] = IEqualizerRouter.Route({from: left, to: right, stable: useStable});

        amountToRoute = amount;
      }
    }
  ...
  function _swapFarmEmissionTokens() internal {
    ...
    IEqualizerRouter.Route[] memory routes = _makeRoutes();
    equal.safeIncreaseAllowance(EQUALIZER_ROUTER, equalBalance);
    IEqualizerRouter router = IEqualizerRouter(EQUALIZER_ROUTER);
    uint256[] memory amounts = router.getAmountsOut(equalBalance, routes);
    router.swapExactTokensForTokens(equalBalance, amounts[amounts.length - 1], routes, address(this), block.timestamp);
  }
  ...
  function setEqualToUsdcPath(address[] memory _path) external {
    _atLeastRole(STRATEGIST);
    require(_path[0] == EQUAL && _path[_path.length - 1] == USDC, "INVALID INPUT");
    equalToUsdcPath = _path;
  }
