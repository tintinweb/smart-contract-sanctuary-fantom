...
contract Strategy is BaseStrategy {
	...
	ICurveFi public curve = ICurveFi(0x4FC8D635c3cB1d0aa123859e2B2587d0FF2707b1); // This is our curve pool.
	IGauge public gauge = IGauge(0xDee85272EAe1aB4afBc6433F4d819BaBC9c7045A);
	IERC20 public curveLpToken = IERC20(0xDf38ec60c0eC001142a33eAa039e49E9b84E64ED); // Curve Pool Token
	...
	constructor(
		...
		// Approvals and path specific to this contract
		want.approve(address(curve), MAX);
		// Approve deposit LP to masterChef
		curveLpToken.approve(address(gauge), MAX);
	}
	...
	function balanceInMasterChef() public view returns (uint256) {
		return gauge.balanceOf(address(this));
	}

	function balanceOfPooled() public view returns (uint256) {
		return lpToWantAmount(balanceInMasterChef().add(balanceOfLpTokens()));
	}

	function lpToWantAmount(uint256 _LpAmount) public view returns (uint256) {
		// lp decimals multiplication + lp decimals vs want token
		return _LpAmount.mul(curve.get_virtual_price()).div(10**18).div(10**12);
	}

	function wantToLPAmount(uint256 _wantAmount) public view returns (uint256) {
		return _wantAmount.mul(10**12).mul(10**18).div(curve.get_virtual_price());
	}
		...
		)
	{
		...
		// Serious loss should never happen.
		// But if it does (for instance, if Curve is hacked), let's record it accurately
	 ...
	 * 		1- Invest want token into Curve by adding liquidity
	 * 		2- Deposit Curve-IB-LP into Curve Gauge
	 */
	function adjustPosition(uint256 _debtOutstanding) internal override {
		...
		if (_toInvest > 0) {
			// 1- deposit on Curve Pool and get LP in return
			curve.add_liquidity([0, _toInvest, 0], 0, true);

			// 2- deposit on Curve Gauge
			depositAllLPOnGauge();
		...
		returns (uint256 _liquidatedAmount, uint256 _loss)
	{
		...
		if (_amountNeeded > _looseWant) {
			uint256 toExitAmount = _amountNeeded.sub(_looseWant);
			uint256 lpToWithdraw = wantToLPAmount(toExitAmount);

			withdrawSomeWant(lpToWithdraw);
	...
	function liquidateAllPositions() internal override returns (uint256 liquidated) {
		...
		// Withdraw LP from from masterChef
		gauge.withdraw(balanceInMasterChef());
		// Withdraw want token from the POOL
		withdrawFromPool(balanceOfLpTokens());
	...
	function collectTradingFees() internal {
		...
		if (total > debt) {
			...
			uint256 lpToWithdraw = wantToLPAmount(profit);
			// Convert a specific number of lp to want token
			withdrawSomeWant(lpToWithdraw);
		}
	}
	...
	function depositAllLPOnGauge() internal {
		uint256 lpBalance = balanceOfLpTokens();
		if (lpBalance > 0) {
			gauge.deposit(lpBalance);
		}
	}

	function withdrawSomeWant(uint256 _lpAmount) internal {
		// Withdraw LP from from masterChef
		gauge.withdraw(_lpAmount);
		// Withdraw lp tokens from pool and ge Want token
		withdrawFromPool(_lpAmount);
		// Put remaining of LP back to the Gauge if any are free
		depositAllLPOnGauge();
	}

	function withdrawFromPool(uint256 _amount) internal returns (uint256) {
		if (_amount > 0) {
			curve.remove_liquidity_one_coin(_amount, 1, 0, true);
		}
	}
	...
	// TODO: send lp tokens to next strategy
	function prepareMigration(address _newStrategy) internal override {
		uint256 _stakedBal = balanceInMasterChef();
		if (_stakedBal > 0) {
			gauge.withdraw(_stakedBal);
			curveLpToken.safeTransfer(_newStrategy, balanceOfLpTokens());
		}
	}
