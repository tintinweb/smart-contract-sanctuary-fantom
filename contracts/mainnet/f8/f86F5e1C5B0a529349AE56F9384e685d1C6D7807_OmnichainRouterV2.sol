    ...
    */
    ...
    function _route(RouteCall memory params, bool isRedirected) internal {
        ...
        if (keccak256(bytes(route.provider)) == keccak256(bytes('lz'))) {
            _lzProcess(route.chainId, params, isRedirect, isRedirected, params.adapterParams);
            return;
        }
     ...
     */
    function _lzProcess(uint16 chainId, RouteCall memory params, bool isRedirect, bool isRedirected, bytes memory _adapterParams) internal {
        require(trustedRemoteLookup[chainId].length != 0, "LzSend: destination chain is not a trusted source.");

        if (isRedirected) {
            (uint messageFee,) = lzEndpoint.estimateFees(chainId, address(this), params.payload, false, _adapterParams);
            lzEndpoint.send{value : messageFee}(chainId, this.getTrustedRemote(chainId), params.payload, payable(params.user), lzConfig.zroPaymentAddress, _adapterParams);
            return;
        }
        lzEndpoint.send{value : (msg.value - params.redirectFee)}(chainId, this.getTrustedRemote(chainId), params.payload, payable(params.user), lzConfig.zroPaymentAddress, _adapterParams);
    }
