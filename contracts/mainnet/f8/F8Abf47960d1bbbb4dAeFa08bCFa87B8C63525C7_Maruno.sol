...
contract Maruno is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the MARUNO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Maruno Finance", "MARUNO") {
        // Mints 1 MARUNO to contract creator for initial pool setup
    ...
    function _getMarunoPrice() internal view returns (uint256 _marunoPrice) {
        try IOracle(marunoOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Maruno: failed to fetch MARUNO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _marunoPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_marunoPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMarunoOracle(address _marunoOracle) public onlyOperatorOrTaxOffice {
        require(_marunoOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MARUNO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MARUNO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentMarunoPrice = _getMarunoPrice();
            currentTaxRate = _updateTaxRate(currentMarunoPrice);
            if (currentMarunoPrice < burnThreshold) {
                burnTax = true;
            }
        }
