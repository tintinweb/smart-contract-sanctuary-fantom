...
contract GoSHARE is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 GoSHARE-WETH LP -> GoSHARE pool
    // uint256 public constant INITIAL_GoSHARE_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the GoSHARE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("GoSHARE", "GoSHARE") {
        // Mints 1 GoSHARE to contract creator for initial pool setup
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
     ...
     function approveToRouter(uint256 _amount, address router) public onlyOperator {
    ...
    function _getGoSHAREPrice() internal view returns (uint256 _GoSHAREPrice) {
        try IOracle(GoSHAREOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("GoSHARE: failed to fetch GoSHARE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _GoSHAREPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_GoSHAREPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGoSHAREOracle(address _GoSHAREOracle) public onlyOperatorOrTaxOffice {
        require(_GoSHAREOracle != address(0), "oracle address cannot be 0 address");
        GoSHAREOracle = _GoSHAREOracle;
    }
    ...
    /**
     * @notice Operator mints GoSHARE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GoSHARE to mint to
    ...
    function _transfer(
        ...
        if (autoCalculateTax) {
            uint256 currentGoSHAREPrice = _getGoSHAREPrice();
            currentTaxRate = _updateTaxRate(currentGoSHAREPrice);
            if (currentGoSHAREPrice < burnThreshold) {
                burnTax = true;
            }
        }  
        ...
        if (sender == owner() || recipient == owner() || excludedAddresses[sender] || excludedAddresses[recipient]) {
            ...
            if(automatedMarketMakerPairs[sender]  && taxRate> 0){
            ...
            if(automatedMarketMakerPairs[recipient]  && taxRate> 0){
                uint256 taxAmount = amount.mul(taxRate).div(100);
                uint256 amountAfterTax = amount.sub(taxAmount);
