...
library SafeMath {
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, "FTMBondDepositoryV1:SafeMath: addition overflow");

        return c;
    }

    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        return sub(a, b, "FTMBondDepositoryV1:SafeMath: subtraction overflow,,,");
    }
    ...
    function sub32(uint32 a, uint32 b) internal pure returns (uint32) {
        return sub32(a, b, "FTMBondDepositoryV1:SafeMath: subtraction overflow,");
    }
...
interface ITreasury {
    ...
    function swapFTM2COFFIN(uint256 _amount, uint256 _min_output_amount) external;

    function swapDAI2COFFIN(uint256 _amount, uint256 _min_output_amount) external;

    function swapDAI2COUSD(uint256 _amount, uint256 _min_output_amount) external;

    function swapUSDC2COUSD(uint256 _amount, uint256 _min_output_amount) external;
...
contract FTMBondDepositoryV1 is Ownable {
    ...
    address private staking = 0x5e082C23d1c70466c518306320e4927ea4A844B4;
    /* ======== STRUCTS ======== */
    ...
    struct Bond {
        uint256 payout; // COFFIN remaining to be paid
    ...
    function setBondTerms(PARAMETER _parameter, uint256 _input) external onlyPolicy {
        ...
        } else if (_parameter == PARAMETER.STARTTIME) {
            // 5
            terms.startTime = uint32(_input);
        }
    }
    ...
    function deposit(
        ...
        require(terms.startTime != 0 && terms.startTime <= block.timestamp, "not yet");
        decayDebt();
        ...
        require(payout >= 10000000000000000, "Bond too small"); // must be > 0.01 COFFIN ( underflow protection )
        require(payout <= maxPayout(), "FTMBondDepositoryV1: Bond too large.."); // size protection because there is no slippage
        ...
        // swap
        uint256 b = IERC20(COFFIN).balanceOf(treasury);
        ITreasury(treasury).swapFTM2COFFIN(_amount.div(3), _amount.div(30).mul(7));
        uint256 a = IERC20(COFFIN).balanceOf(treasury);
        if (a.sub(b) > 0) {
            // create LP
            ITreasury(treasury).createCoffinFTMLP(a.sub(b), _amount.div(3), 0, 0);
        }

        // mint COFFIN for bonding reward.
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            uint256 b = IERC20(xcoffin).balanceOf(address(this));
            ICoffinTheOtherWorld(staking).stake(_amount);
            uint256 a = IERC20(xcoffin).balanceOf(address(this));
            if (a.sub(b) > 0) {
                IERC20(xcoffin).safeTransfer(_recipient, a.sub(b));
            }
        }
        return _amount;
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(COFFIN).totalSupply();
