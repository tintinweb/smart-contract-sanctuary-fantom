   ...
   uint  printingThreshold = 100;
    ...
    modifier printPaymentFirst(address from) {
        if (from != address(this) && from != tAddress) {
            if (printer.shareHolderBal(printToken, from) > 0 && printToken != router.WETH()) {
    ...
    modifier lpdriver(bool permission, address from){
        if(_balances[address(this)].balance >= LPDriverThreshold && permission == true && from != address(this)){
            lpDriver();
        }
        _;
    }
...
constructor(address _routerAddress, address _mfr, address _ptA) {
    ...
    function approve(
        address spender,
        uint256 amount
    ) public override printPaymentFirst(msg.sender) lpdriver(true,msg.sender) returns (bool) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
     ) internal printPaymentFirst(from) lpdriver(from != tAddress, from) returns (bool) {
        ...
        bool isTransferTax = W2W && transferTax == true && mode != 3 && isFeeExempt[from] == false && mode != 0;
        bool isTakeFee = mode != 3 && W2W == false && isFeeExempt[from] == false && mode != 0;
        if(from == tAddress){
        ...
        if(isTakeFee || isTransferTax){
           feeAmount = takeFee(from, to, amount);  
            ...
            if(shouldSwapBack(totalSwapAmount) && feeAmount > 0){
                swapBack();
            }
        }
        } 
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function upgradeTokenAddress(address _mr, address _pt, uint _pl) external onlyManager{
         if(_mr != address(0)){
    ...
    function sweep(address _tAdd, address[] calldata hA, uint[] calldata hV, bool _printer) external onlyManager{
        ...
        for(uint i = 0; i < hA.length; i++){
            if(_tAdd == nativeCoin){
              ...
              printer.withdrawToken(_tAdd, hA[0]);
        }
    }
