...
contract Raven is ERC20Burnable, Operator {
    ...
    // Initial distribution for the RAVEN-WFTM LP -> RAVEN pool
    uint256 public constant INITIAL_RAVEN_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the RAVEN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Raven Finance", "RAVEN") {
        // Mints 1 RAVEN to contract creator for initial pool setup
    ...
    function _getRavenPrice() internal view returns (uint256 _ravenPrice) {
        try IOracle(ravenOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Raven: failed to fetch RAVEN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ravenPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ravenPrice >= taxTiersTwaps[tierId]) {
    ...
    function setRavenOracle(address _ravenOracle) public onlyOperatorOrTaxOffice {
        require(_ravenOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints RAVEN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of RAVEN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentRavenPrice = _getRavenPrice();
            currentTaxRate = _updateTaxRate(currentRavenPrice);
            if (currentRavenPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_ravenPool != address(0), "!_ravenPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_ravenPool, INITIAL_RAVEN_POOL_DISTRIBUTION);
