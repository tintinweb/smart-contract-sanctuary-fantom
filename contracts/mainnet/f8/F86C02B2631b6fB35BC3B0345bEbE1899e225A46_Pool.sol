...
contract Pool is ReentrancyGuard, IPool {
    ...
    address public _alpha;
    address public treasury;
    address public _xShare;
    ...
    uint256 public xShareDailyMintLimitPercentage = 1000;
    uint256 currentDayStartTs;

    uint256 public netMinted;
    uint256 public netRedeemed;
    uint256 public uncollectedFee;
    ...
    modifier checkDayStart() {
        if (block.timestamp > currentDayStartTs + 86400) {
            ...
            uint256 xShareCirSupply = XShareToken(_xShare).circulatingSupply();
            xShareDailyMintLimitation = xShareCirSupply.mul(xShareDailyMintLimitPercentage).div(10000);
    ...
    function collateralBalance() external view override returns (uint256) {
        return (ERC20(collateral).balanceOf(collateralFund()).sub(unclaimed_pool_collateral)).mul(10 ** missing_decimals);
        //.mul(collateral_usd_price).div(PRICE_PRECISION);
    }
    ...
    )
    {
        ...
        ERC20(collateral).balanceOf(collateralFund()), // amount of COLLATERAL locked in this contract
    ...
    function collateralFund() public view returns (address) {
        return ITreasury(treasury).collateralFund();
    }
    ...
    //TODO check mint and redeem function with origin contracts

    function mint(
        uint256 _collateral_amount,
        uint256 _share_amount,
        uint256 _alpha_out_min
    ) external notMigrated checkDayStart {
        require(mint_paused == false, "Minting is paused");

        (uint256 _alphaPrice, uint256 _share_price, , uint256 _target_collateral_ratio, , , uint256 _minting_fee,) = ITreasury(treasury).info();
        require(ERC20(collateral).balanceOf(collateralFund()).sub(unclaimed_pool_collateral).add(_collateral_amount) <= pool_ceiling, ">poolCeiling");
        uint256 _total_alpha_value = 0;
        ...
        if (_target_collateral_ratio > 0) {
            ...
            //.mul(_price_collateral).div(PRICE_PRECISION);
            _total_alpha_value = _collateral_value.mul(COLLATERAL_RATIO_PRECISION).div(_target_collateral_ratio);
            if (_target_collateral_ratio < COLLATERAL_RATIO_MAX) {
                _required_share_amount = _total_alpha_value.sub(_collateral_value).mul(PRICE_PRECISION).div(_share_price);
            }
        } else {
            _total_alpha_value = _share_amount.mul(_share_price).div(PRICE_PRECISION);
            _required_share_amount = _share_amount;
        }

        uint256 _actual_alpha_amount = _total_alpha_value.sub((_total_alpha_value.mul(_minting_fee)).div(PRICE_PRECISION));

        require(_alpha_out_min <= _actual_alpha_amount, ">slippage");

        if (_required_share_amount > 0) {
            require(_required_share_amount <= _share_amount, "<shareBalance");
            XShareToken(_xShare).poolBurnFrom(msg.sender, _required_share_amount);
        }

        if (_collateral_amount > 0) {
            _transferCollateralToReserve(msg.sender, _collateral_amount);

            uncollectedFee.add(_collateral_amount.mul(_minting_fee).div(PRICE_PRECISION));
        }

        netMinted = netMinted.add(_actual_alpha_amount);

        AlphaToken(_alpha).poolMint(msg.sender, _actual_alpha_amount);

        emit Minted(msg.sender, _collateral_amount, _required_share_amount, _actual_alpha_amount);
    }

    function redeem(
        uint256 _alpha_amount,
        ...
        uint256 _alpha_amount_post_fee = _alpha_amount.sub((_alpha_amount.mul(_redemption_fee)).div(PRICE_PRECISION));
        uint256 _collateral_output_amount = 0;
        uint256 _share_output_amount = 0;

        if (_effective_collateral_ratio < COLLATERAL_RATIO_MAX) {
            uint256 _share_output_value = _alpha_amount_post_fee.sub(_alpha_amount_post_fee.mul(_effective_collateral_ratio).div(PRICE_PRECISION));
            _share_output_amount = _share_output_value.mul(PRICE_PRECISION).div(_share_price);
        }

        if (_effective_collateral_ratio > 0) {
            uint256 _collateral_output_value = _alpha_amount_post_fee.mul(_effective_collateral_ratio).div(10 ** missing_decimals).div(PRICE_PRECISION);
            _collateral_output_amount = _collateral_output_value;
            //.mul(PRICE_PRECISION).div(PRICE_PRECISION);
        }

        //Add To Fee
        uint256 _alpha_fee = _alpha_amount.sub(_alpha_amount_post_fee);
        uncollectedFee.add(_alpha_fee.mul(_effective_collateral_ratio).div(10 ** missing_decimals).div(PRICE_PRECISION));

        // Check if collateral balance meets and meet output expectation
        require(_collateral_output_amount <= ERC20(collateral).balanceOf(collateralFund()).sub(unclaimed_pool_collateral), "<collateralBlanace");
        ...
        netRedeemed = netRedeemed.add(_alpha_amount);

        // Move all external functions to the end
        AlphaToken(_alpha).poolBurnFrom(msg.sender, _alpha_amount);
        if (_share_output_amount > 0) {
        ...
        emit Redeemed(msg.sender, _alpha_amount, _collateral_output_amount, _share_output_amount);
    }
    ...
    function collectFee() external onlyFeeCollector {
        require(uncollectedFee > 0, "No Mint Fee To Collect");

        _requestTransferCollateral(_feeCollector, uncollectedFee);

        uncollectedFee = 0;

        emit CollectFee(uncollectedFee);
    }
    ...
    function _mintShareToCollateralReserve(uint256 _amount) internal {
        address _reserve = collateralFund();
        require(_reserve != address(0), "Invalid reserve address");
        XShareToken(_xShare).poolMint(_reserve, _amount);
    }
    ...
    function _requestTransferShare(address _receiver, uint256 _amount) internal {
        ITreasury(treasury).requestTransfer(_xShare, _receiver, _amount);
    }
    ...
    event RedeemCollected(address indexed user, uint256 usdtAmountOut, uint256 _xShareAmountOut);
    event CollectFee(uint256 mintFee);
}
...
interface ITreasury {
    ...
    function buyback(uint256 _collateral_amount, uint256 _min_share_amount) external;
