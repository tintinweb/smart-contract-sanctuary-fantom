...
contract LpMaker is Ownable, ILpMaker {
    ...
    function addLiquidity(address _token0, address _token1,uint256 _token0Amount,
        uint256 _token1Amount,address _routerAddr, address _recipient, uint256 _minAmountLp ) external whitelist(_routerAddr)
    {
        IERC20(_token0).safeTransferFrom(msg.sender, address(this), _token0Amount);
        IERC20(_token1).safeTransferFrom(msg.sender, address(this), _token1Amount);

        _approveTokenIfNeeded(_token0, _routerAddr);
        _approveTokenIfNeeded(_token1, _routerAddr);

        (uint256 amount0 , uint256 amount1  , uint256 liqTokenAmt) = IUniswapV2Router(_routerAddr).addLiquidity(_token0, _token1,
           _token0Amount, _token1Amount, 0 , 0, _recipient, block.timestamp);

        require(liqTokenAmt >= _minAmountLp);

        if(_token0Amount > amount0){
            IERC20(_token0).safeTransfer(msg.sender, _token0Amount.sub(amount0));
        }
        if(_token1Amount > amount1){
            IERC20(_token1).safeTransfer(msg.sender, _token1Amount.sub(amount1));
        }
    }

    function addLiquidityETH(address _token1,uint256 _token1Amount,address _routerAddr,
        address _recipient, uint256 _minAmountLp ) payable external override whitelist(_routerAddr)
    {
        ...
        (uint amountToken, uint amountETH, uint liqTokenAmt) = IUniswapV2Router(_routerAddr).addLiquidityETH {
        value: msg.value
        } (
           _token1,
            _token1Amount,
        ...
        if(_token1Amount > amountToken){
            IERC20(_token1).safeTransfer(msg.sender, _token1Amount.sub(amountToken));
        }
    ...
    function removeLiquidityEth(address _in, uint256 _amount,
        address _routerAddr, address _recipient) external whitelist(_routerAddr)  {

        require(_recipient != address(0));

        require(IUniswapV2Pair(_in).token0() == NATIVE ||
            IUniswapV2Pair(_in).token1() == NATIVE, "Native pair required");

        IERC20(_in).safeTransferFrom(msg.sender, address(this), _amount);
        _approveTokenIfNeeded(_in, _routerAddr);
        address _token0 = IUniswapV2Pair(_in).token0();
        address _token1 = IUniswapV2Pair(_in).token1();
        (uint256 amountToken,uint256 amountETH) =
        IUniswapV2Router(_routerAddr).removeLiquidity(_token0, _token1, _amount,
            0, 0, address(this), block.timestamp);

        IWETH(_token1).withdraw(amountETH);

        IERC20(IUniswapV2Pair(_in).token0()).safeTransfer(_recipient, amountToken);
        TransferHelper.safeTransferETH(_recipient, amountETH);

    }

    function removeLiquidity(address _in, uint256 _amount,
        address _routerAddr, address _recipient) external whitelist(_routerAddr)  {
        ...
        address _token0 = IUniswapV2Pair(_in).token0();
        address _token1 = IUniswapV2Pair(_in).token1();
        (uint256 _amountToken0,uint256 _amountToken1) =
        IUniswapV2Router(_routerAddr).removeLiquidity(_token0,
            _token1, _amount, 0, 0, address(this), block.timestamp);

        IERC20(_token0).safeTransfer(_recipient, _amountToken0);
        IERC20(_token1).safeTransfer(_recipient, _amountToken1);
    }
