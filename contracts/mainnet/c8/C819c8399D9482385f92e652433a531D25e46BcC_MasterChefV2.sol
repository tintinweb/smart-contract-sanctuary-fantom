...
contract MasterChefV2 is Ownable {
    ...
    function getFarmData(uint pid) external view returns (FarmData memory) {
        return FarmData(poolInfo[pid], totalAllocPoint, rewarder[pid]);
    }

    function getMultipleFarmData(uint startPid, uint endPid) external view returns (FarmData[] memory data) {
        require(endPid > startPid, "endPid > startPid");
        data = new FarmData[](endPid - startPid);
    }
