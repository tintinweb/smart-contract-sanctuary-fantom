...
import "./interfaces/IFateVault.sol";

/**
 * @title FateVault
 * Allows exchange of Jade to Fate when Fate is above claimPrice
 */
contract FateVault is AccessControlEnumerable, ReentrancyGuard, IFateVault {
    ...
    constructor(address claimToken_, address exchangeToken_) {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        ...
        interfaceId == type(IFateVault).interfaceId;
    }

    /**
     * @notice start the timer for timelocked functions
     */
     ...
     */
    function claim(uint256 amount_) external override nonReentrant {
        uint256 price = IOracleToken(claimToken).getPriceUpdated();
        if(price < claimPrice) revert PriceNotEligible();
        IERC20 claimToken_ = IERC20(claimToken);
        if(claimToken_.balanceOf(address(this)) < amount_) revert LowBalance();
        IERC20Burnable(exchangeToken).burnFrom(msg.sender, amount_);
        claimToken_.safeTransfer(msg.sender, amount_);
    }
    ...
    /**
     * @notice At some point, Jade will no longer be minted and supply of Fate may be in excess
     * The amount to be burned will be decided then. Function is timelocked for 730 days from startTime
     */
    function burnClaimToken(uint256 amount_) external onlyOperator {
        if(block.timestamp < startTime + 730 days) revert TooEarly();
        IERC20Burnable(claimToken).burn(amount_);
    }

    /**
     * @notice Timelocked function of 90 days from startTime if there is a need to upgrade the vault
     */
    function migrateClaimToken(address newVault_, uint256 amount_) external onlyOperator {
        if(block.timestamp < startTime + 90 days) revert TooEarly();
        IERC20(claimToken).safeTransfer(newVault_, amount_);
    }

    function governanceRecoverUnsupported(IERC20 token_, uint256 amount_, address to_) external onlyOperator {
        if(address(token_) == claimToken) revert Token();
