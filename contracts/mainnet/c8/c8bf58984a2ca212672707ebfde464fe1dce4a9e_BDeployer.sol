...
contract BStorage {
    ...
    function safe112(uint256 n) internal pure returns (uint112) {
        require(n < 2**112, "Scorpion: SAFE112");
        return uint112(n);
    }
}
...
contract BAllowance is PoolToken, BStorage {
    ...
    function _checkBorrowAllowance(
        ...
        if (spender != owner && _borrowAllowance != uint256(-1)) {
            require(_borrowAllowance >= value, "Scorpion: BORROW_NOT_ALLOWED");
    ...
    function borrow(
        ...
        if (adjustedBorrowAmount > repayAmount)
            require(
                ...
                "Scorpion: INSUFFICIENT_LIQUIDITY"
            );
