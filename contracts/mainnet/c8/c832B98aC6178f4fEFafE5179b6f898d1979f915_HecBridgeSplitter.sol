    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @param _bridgeDatas the core information needed for bridging
    /// @param _nxtpDatas data needed to complete an NXTP cross-chain transaction
    function startBridgeTokensViaNXTP(
        ILiFi.BridgeData[] memory _bridgeDatas,
        IHecBridgeSplitterInterface.NXTPData[] calldata _nxtpDatas,
        uint256[] memory fees
    ) external payable {
        require(
            _bridgeDatas.length > 0 &&
                _bridgeDatas.length <= CountDest &&
                _bridgeDatas.length == _nxtpDatas.length,
            "Splitter: bridge or stargate data is invalid"
        );
        for (uint256 i = 0; i < _bridgeDatas.length; i++) {
            if (_bridgeDatas[i].sendingAssetId != address(0)) {
            ...
            bytes memory callData = abi.encodeWithSelector(
                selectors["startBridgeTokensViaNXTP"],
                _bridgeDatas[i],
                _nxtpDatas[i]
            );
    ...
    function swapAndStartBridgeTokensViaNXTP(
        ...
        for (uint256 i = 0; i < _bridgeDatas.length; i++) {
            ...
            bytes memory callData = abi.encodeWithSelector(
                selectors["swapAndStartBridgeTokensViaNXTP"],
                _bridgeDatas[i],
                _swapDatas[i],
                _nxtpDatas[i]
            );
