...
// File: contracts/misc/UiDataCalcutor.sol
...
contract UiDataCalcutor {
    ...
    function calculateApy(ILendingPoolAddressesProvider provider, address asset)
      public
      view
    ...
    function calculateApyIncentive(ILendingPoolAddressesProvider provider, address incentiveController, address asset, address pair)
      public
      view
      returns (uint256, uint256)
    {
      ...
      uint256 apyVarDebtToken =
        (rewardPerSec*SECONDS_PER_YEAR*price*(allocVar*1e18)/incentive.totalAllocPoint()).div(varSupply*fetchPriceAsset);
      uint256 apyAToken =
        (rewardPerSec*SECONDS_PER_YEAR*price*(allocAToken*1e18)/incentive.totalAllocPoint()).div(aTokenSupply*fetchPriceAsset);
