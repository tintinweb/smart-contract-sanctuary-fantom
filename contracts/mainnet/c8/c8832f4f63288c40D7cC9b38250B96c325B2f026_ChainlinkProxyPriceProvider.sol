...
contract ChainlinkProxyPriceProvider is IPriceOracleGetter, Ownable {
    ...
    mapping(address => PriceGetter) private priceGetters;
    ...
    function setPriceGetter(address[] memory assets,address[] memory _priceGetters) external onlyOwner {
        require(assets.length == _priceGetters.length, 'INCONSISTENT_PARAMS_LENGTH');
        for (uint256 i = 0; i < assets.length; i++) {
            priceGetters[assets[i]] = PriceGetter(_priceGetters[i]);
        }
    }

    /// @dev returns assets price from router for tests without chainlink
    function getAssetsPrice(address _assetA, address _assetB) external view returns(uint256){
        //require(priceGetters[_assetA] != 0, 'Price getter not inited');
        return priceGetters[_assetA].getPrice(_assetA,_assetB);
    }
    ...
    function getAssetPrice(address asset) public view override returns (uint256) {
        ...
        // If we have not asset source, we will get price token in native
        // and then return tokenPrive * nativeInDollars

        if(address(source) == address(0))
        {
            PriceGetter priceGetter = priceGetters[asset];
            require(address(priceGetter) != address(0),'NO_PRICE_GETTER' );
            int256 priceAssetInNative = int256(priceGetter.getPrice(asset,priceGetter.getNative()));
            require(priceAssetInNative > 0, 'INVALID_PRICE_IN_NATIVE');
            AggregatorV3Interface sourceNative = _assetsSources[priceGetter.getNative()];
            require(address(sourceNative) != address(0), 'SOURCE_IS_MISSING');
            (, int256 priceNative, , , ) = sourceNative.latestRoundData();
            require(uint256(priceNative * priceAssetInNative) / 100000000 > 0, 'INVALID_PRICE');
            uint256 testPrice = uint256(priceNative * priceAssetInNative);
            return uint256(priceNative * priceAssetInNative) / 100000000;
        }
...
contract PriceGetter is Ownable  {
    ...
    function getReserves(
        ...
        address pai = IUniswapV2Factory(factory).getPair( tokenA, tokenB);
    ...
    function normalizePrice(uint256 _price, uint8 _decimalA,uint8 _decimalB) internal
                                                                view returns(uint256)
    {
        if(_decimalB > NORM_DECIMAL){
            _price = _price / (10**(_decimalB - NORM_DECIMAL));
        }
        else if(_decimalB == NORM_DECIMAL)
        {
            return _price;
        }
        else
        {
            _price = _price * (10**(NORM_DECIMAL-_decimalB));
        }

        return _price;
    }

    function getPrice(address _in,address _out) public view tokensWhitelist(_in, _out) returns (uint256) {
        ...
        return normalizePrice(amounts[amounts.length - 1],ERC20(_in).decimals(),
                                                    ERC20(_out).decimals()) ;
    }
