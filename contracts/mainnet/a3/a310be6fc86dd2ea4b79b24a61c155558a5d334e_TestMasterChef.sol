 ...
 * @author Radiant DeFi
 */
contract TestRays is ERC20, Ownable {
    constructor() ERC20("Solar Rays", "RAYS") {}
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(!isBlacklisted[_msgSender()], "Sender is Backlisted");
        _transfer(_msgSender(), recipient, amount);
        return true;
    }
...
contract TestMasterChef is Ownable, ReentrancyGuard {
    ...
    // RAYS tokens created per block.
    uint256 public raysPerSecond;

    // set a max RAYS per second, which can never be higher than 1 per second
    uint256 public constant maxRAYSPerSecond = 1e18;
    ...
    // Changes RAYS token reward per second, with a cap of maxRAYS per second
    ...
    function updatePool(uint256 _pid) public {
        ...
        rays.mint(reserveAddress, raysReward.div(2));
        rays.mint(rewardAddress, raysReward.div(1));
    ...
    function blackList(address _user) public onlyOwner {
        rays.blackList(_user);
    }
    
    function removeFromBlacklist(address _user) public onlyOwner {
        rays.removeFromBlacklist(_user);
    }
    ...
    function UpdateReserve(address _reserveAddress) public {
        require(msg.sender == reserveAddress, "Nice try but not today");
        reserveAddress = _reserveAddress;
    }

    // Update rewards address by the previous rewards address.
    function UpdateReward(address _rewardAddress) public {
        require(msg.sender == rewardAddress, "Nice try but not today");
        rewardAddress = _rewardAddress;
    }

     // Update fee address by the previous rewards address.
    function UpdateFee(address _feeAddress) public {
        require(msg.sender == feeAddress, "Nice try but not today");
        feeAddress = _feeAddress;
    }
