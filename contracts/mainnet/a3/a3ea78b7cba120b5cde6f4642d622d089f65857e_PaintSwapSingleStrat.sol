...
// File: contracts/interfaces/IPaintSwapDeco.sol

pragma solidity 0.8.4;

interface IPaintSwapDeco {
...
// File: contracts/PaintSwapSingleStrat.sol
...
//PaintSwap BRUSH Staking

contract PaintSwapSingleStrat is Ownable, Pausable {
    ...
    constructor(
        ...
        outputToWrappedRoute = [want, wrapped];
    ...
    function _deposit() internal whenNotPaused {
        ...
        IPaintSwapDeco(masterchef).enterStaking(wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IPaintSwapDeco(masterchef).leaveStaking(_amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        IPaintSwapDeco(masterchef).leaveStaking(0);
    ...
    function chargeFees(address caller) internal {                   
        uint256 toWrapped = IERC20(want).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        uint256 inputBal = IERC20(want).balanceOf(address(this));
        IPaintSwapDeco(masterchef).enterStaking(inputBal);
    }

     /** @dev Returns rewards unharvested */
    function rewardBalance() public view returns (uint256) {
        return IPaintSwapDeco(masterchef).pendingBrush(0, address(this));
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IPaintSwapDeco(masterchef).userInfo(0, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IPaintSwapDeco(masterchef).emergencyWithdraw(0);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external onlyAdmin {
        require (_path[0] != want && _path[_path.length - 1] == want, "Bad path");
