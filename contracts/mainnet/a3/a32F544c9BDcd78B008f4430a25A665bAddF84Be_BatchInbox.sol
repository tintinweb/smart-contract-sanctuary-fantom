...
contract BatchInbox is DecompressorExtension {
  ...
  constructor(address _forwarder) {
    ...
    (, , , minGasUsed) = IForwarder(forwarderAdderss).getFeeSetting();
  }
  ...
  fallback() external {
    ...
    // executeBatch
    if (selector == 0x01) {
      (IForwarder.ForwardRequest[] memory reqs, bytes[] memory sigs) = abi.decode(data, (IForwarder.ForwardRequest[], bytes[]));
      executeBatch(reqs, sigs);
    }
  }

  function executeBatch(
    IForwarder.ForwardRequest[] memory reqs,
    bytes[] memory sigs
  ) public payable {
    ...
    for (uint i = 0; i < count; i++) {
      try IForwarder(forwarderAdderss).execute{gas: reqs[i].gas + minGasUsed, value: reqs[i].value}(reqs[i], sigs[i]) returns (bool success, bytes memory returnData) {
