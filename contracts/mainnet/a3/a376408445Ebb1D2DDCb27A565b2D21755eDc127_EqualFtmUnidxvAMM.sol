...
// File contracts/EqualFtmUnidxvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualFtmUnidxvAMM is Ownable, Pausable {
    ...
    address public constant unidx = address(0x2130d2a1e51112D349cCF78D2a1EE65843ba36e0);
    address public want;
    ...
    IEqualizerRouter.Routes[] public wrappedToUnidxPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToUnidxPath
    ) {
        ...
        sentinel = _sentinel;
        _grimFeeRecipient = _grimFeeRecipient;

         for (uint i; i < _equalToWrappedPath.length; ++i) {
            equalToWrappedPath.push(_equalToWrappedPath[i]);
        }

        for (uint i; i < _wrappedToUnidxPath.length; ++i) {
            wrappedToUnidxPath.push(_wrappedToUnidxPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToUnidxPath, address(this), block.timestamp);        

        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(unidx).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, unidx, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(unidx).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(unidx).safeApprove(unirouter, type(uint).max);
    }
