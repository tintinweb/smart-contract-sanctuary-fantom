...
contract DeadPool is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accRewardPerShare; // Accumulated REWARD per share, times 1e6. See below.
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare + (_reward * (1e6) / (tokenSupply));
        }
        return user.amount * (accRewardPerShare) / (1e6) - (user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.timestamp <= pool.lastRewardTime) {
            ...
            pool.accRewardPerShare = pool.accRewardPerShare + (_reward * (1e6) / (tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant  {
        ...
        if (user.amount > 0) {
            ...
            uint256 _pending = user.amount * (pool.accRewardPerShare) / (1e6) - (user.rewardDebt);
            if (_pending > 0) {
        ...
        user.rewardDebt = user.amount * (pool.accRewardPerShare) / (1e6);
