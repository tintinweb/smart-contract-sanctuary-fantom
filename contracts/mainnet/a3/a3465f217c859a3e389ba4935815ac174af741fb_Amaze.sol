...
contract Amaze is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 48h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 35000 ether;
    ...
    /**
     * @notice Constructs the AMAZE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Amaze Finance", "AMAZE") {
        // Mints 1 AMAZE to contract creator for initial pool setup
    ...
    function _getAmazePrice() internal view returns (uint256 _amazePrice) {
        try IOracle(amazeOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Amaze: failed to fetch AMAZE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _amazePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_amazePrice >= taxTiersTwaps[tierId]) {
    ...
    function setAmazeOracle(address _amazeOracle) public onlyOperatorOrTaxOffice {
        require(_amazeOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints AMAZE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of AMAZE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentAmazePrice = _getAmazePrice();
            currentTaxRate = _updateTaxRate(currentAmazePrice);
            if (currentAmazePrice < burnThreshold) {
                burnTax = true;
            }
        }
