...
contract Fucku is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the FUCKU ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Fucku", "Fucku") {
        // Mints 1 FUCKU to contract creator for initial pool setup
    ...
    function _getFuckuPrice() internal view returns (uint256 _fuckuPrice) {
        try IOracle(fuckuOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Fucku: failed to fetch FUCKU price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _fuckuPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_fuckuPrice >= taxTiersTwaps[tierId]) {
    ...
    function setFuckuOracle(address _fuckuOracle) public onlyOperatorOrTaxOffice {
        require(_fuckuOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints FUCKU to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FUCKU to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFuckuPrice = _getFuckuPrice();
            currentTaxRate = _updateTaxRate(currentFuckuPrice);
            if (currentFuckuPrice < burnThreshold) {
                burnTax = true;
            }
        }
