...
contract FGenesisRewardPool {
    ...
    // The tax when user Deposit to Genesis Pool.
    uint256 public taxGenesisPool = 100; // 1%
    address public taxCollectorAddress = address(0x479ecB27BD40676D29c2f9760464a1256b408a09); // _taxCollectorAddress FTOM 
    ...
    function setPoolStartTime(uint256 _poolStartTime) external onlyOperator {
        require(_poolStartTime >= block.timestamp, "time in the past");
        poolStartTime = _poolStartTime;
        //Update Pool
        poolEndTime = poolStartTime + runningTime;
    }
    ...
    function setRunningTime(uint256 _runningTime) external onlyOperator {
        require(_runningTime >= 1 hours, "time is too short");
    ...
    function setTaxGenesisPool(uint256 _taxGenesisPool) external onlyOperator {
        require(_taxGenesisPool >= 10 && _taxGenesisPool <= 1000, "out of range"); // [0.1%, 10%]
        taxGenesisPool = _taxGenesisPool;
    }

    function setTaxCollectorAddress(address _taxCollectorAddress) external onlyOperator {
        require(_taxCollectorAddress != address(0), "not address");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            pool.token.safeTransferFrom(_sender, address(this), _amount); // Send all _amount to contract
            user.amount = user.amount.add(_amount);
            // if (taxGenesisPool != 0) {
            //     uint256 _taxAmount = _amount.mul(taxGenesisPool).div(10000);
            //     uint256 _userAmount = _amount.sub(_taxAmount);
            //     user.amount = user.amount.add(_userAmount);
            //     //pool.token.safeTransferFrom(address(this), taxCollectorAddress, _taxAmount); // Send _taxAmount form contract
            // }else{ // No Tax
            //     user.amount = user.amount.add(_amount);
            // }
        }
