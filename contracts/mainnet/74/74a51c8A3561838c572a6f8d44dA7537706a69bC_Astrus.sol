  ...
  function checkUpkeep(bytes calldata checkData) external view returns (bool upkeepNeeded, bytes memory performData) {
    for(uint j=0; j<subscribers.length; j++) {
      for(uint i=0; i<channels.length; i++) {
        bool _upkeepNeeded = isPaymentNeeded(subscribers[i], channels[j]);
        if(_upkeepNeeded) {
          // needs to send address of subscriber and address of channel
          upkeepNeeded = true;
          performData = abi.encode(subscribers[i], channels[j]); // encode [subscriber, channel]
        }
      }
    }
  }
