...
contract EarnMore is OwnableUpgradeable {
    ...
    address public underlyingUniPool;
    ...
    // mapping(address => address) public vaultToBalanceCalculator; // Move it as object params
    ...
    event Approve(address indexed owner, address indexed recepient, uint256 amount);
    ...
    function initialize(
        ...
        unipool[_getUnderlying()] = _underlyingUnipool;
        earnMoreManager = msg.sender; // EarnMoreManager deploys proxy contract

        targetRegion = 60 * 60 * 24; // #TODO change to param
    ...
    function deposit(
        ...
        if (tokenId != 0) {
            (uint256 maxVePortion, uint256 maxVeMultiplier) = IEarnMoreManager(earnMoreManager).getVeInfo();
        ...
        console.log(vault);
        console.log(amount);
        console.log(IERC20(vault).balanceOf(msg.sender));
    ...
    function withdraw2(
        ...
        if (IEarnMoreManager(earnMoreManager).transferReward(msg.sender, choRewardAmount)) {
            if (excludedValue != 0) {
    ...
    function _decreaseAllowance(address owner, address recepient, uint256 amount) internal {
        console.log("_decreaseAllowance", allowance[owner][recepient], amount);
        allowance[owner][recepient] -= amount;
    }

    function _decreaseWithdrawAllowance(address owner, address recepient, uint256 amount) internal {
        console.log("_decreaseWithdrawAllowance", withdrawAllowance[owner][recepient], amount);
        withdrawAllowance[owner][recepient] -= amount;
    }


    function earnMoreCalc(address account, uint256 portion) external view returns(uint256 excludedValue, uint256 newTokenUderlying, uint256 finalAmount, uint256 choRewardAmount) {
    ...
    {
        (uint256 excludePercent, uint256 earnMorePercent) = IEarnMoreManager(earnMoreManager).getPercentEarnMoreInfo();
        // Exclude profit
        DepositInfo storage info = depositInfo[account];

        return (0, info.depositUnderlyingValue, (portion * info.amountDeposited) / PRESICION, 0); // Only for fantom
        ...
        if (totalUnderlying <= info.depositUnderlyingValue) {
            newTokenUnderlying = (portion * info.depositUnderlyingValue) / PRESICION;
            return (0, newTokenUnderlying, amount, 0);
        }

        uint256 totalProfit = totalUnderlying - info.depositUnderlyingValue;
        uint256 profit = (portion * totalProfit) / PRESICION;

        newTokenUnderlying = _calcUnderlying(info.amountDeposited - amount) - (totalProfit - profit);

        // uint256 prevUnderlyingValue = (portion * info.depositUnderlyingValue) /
        //     PRESICION;
        // uint256 currentUnderlyingValue = _calcUnderlying(amount);

        // // No profit
        // if (currentUnderlyingValue <= prevUnderlyingValue) {
        //     return (0, amount, 0);
        // }
...
interface IEarnMoreManager {
    function getPercentEarnMoreInfo() external view returns (uint256 exludePercent, uint256 earnMorePercent);

    function getVeInfo() external view returns(uint256 maxVePortion, uint256 maxVeMultiplier);

    function treasury() external view returns(address);

    function transferReward(address account, uint256 amount) external returns(bool);
}
