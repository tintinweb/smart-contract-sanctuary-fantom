...
contract LUXO is ERC721Burnable {
    ...
    bool public isSaleOpened;

    constructor() ERC721("LUXO", "LUXO") {
        MAX_SUPPLY = 10000;
        mintPrice = 1 ether;
    }

    function setSaleStatus(bool _status) external onlyOwner {
        isSaleOpened = _status;
    }
    ...
    function mint(address _account, uint8 _amount) external payable {
        require(tx.origin == msg.sender, "Only EOA");
        require(isSaleOpened, "Not opened sale.");
        require(totalSupply() + _amount <= MAX_SUPPLY, "Max Limit Exceeds");
        require( mintPrice * _amount <= msg.value, "Insufficient value To Mint"
        );
