...
// File contracts/SolidexSuperVault.sol
...
contract SolidexSuperVault {
    ...
    uint256 public scoreStored;
    uint256 public lastUpdateTime;
    mapping(address => uint256) public userScoreStored;
    mapping(address => uint256) public userLastUpdateTime;

    mapping(address => uint256) public totalEarningAmounts;
    ...
    function init(address _stakeToken) public {
        require(owner == address(0), "already init");
        require(_stakeToken != solidSexSolidLp && _stakeToken != wftmSexLp, "invalid stakeToken");
        owner = msg.sender;
        operator = msg.sender;
        stakeToken = _stakeToken;

        IERC20(_stakeToken).approve(solidexLpDepositor, 1e60);
        IERC20(solidSexSolidLp).approve(solidexLpDepositor, 1e60);
        IERC20(wftmSexLp).approve(solidexLpDepositor, 1e60);

        IERC20(wftm).approve(solidlyRouter, 1e60);
        IERC20(sexToken).approve(solidlyRouter, 1e60);
        IERC20(solidToken).approve(solidlyRouter, 1e60);
    }
    ...
    function reinvest() public {
        ...
        uint256 stakeTokenAmount_ = IERC20(stakeToken).balanceOf(address(this));
        ...
        if (stakeTokenAmount_ > 0) {
            ISolidexDepositor(solidexLpDepositor).deposit(stakeToken, stakeTokenAmount_);
        }
        ...
        if (solidTokenAmount_ > 0) {
            ...
            ISolidlyRouter(solidlyRouter).swapExactTokensForTokens(
                solidTokenAmount_.div(2), 1, routes_, address(this), block.timestamp);
            ...
            totalEarningAmounts[solidSexSolidLp] += lpAmount_;
        }
        ...
        if (sexTokenAmount_ > 0) {
            ...
            ISolidlyRouter(solidlyRouter).swapExactTokensForTokens(
                sexTokenAmount_.div(2), 1, routes_, address(this), block.timestamp);
            (uint256 a, uint256 b, uint256 lpAmount_) = ISolidlyRouter(solidlyRouter).addLiquidity(
                sexToken, wftm, false, 
            ...
            totalEarningAmounts[wftmSexLp] += lpAmount_;
        }
       
    }

    modifier updateScore(address user) {
        scoreStored = getTotalScore();
        lastUpdateTime = block.timestamp;
        userScoreStored[user] = getUserScore(user);
    ...
    function getUserScore(address user) public view returns (uint256) {
        return block.timestamp.sub(userLastUpdateTime[user])
            .mul(shares[user]).add(userScoreStored[user]);
    }

    function getTotalScore() public view returns (uint256) {
        return block.timestamp.sub(lastUpdateTime)
            .mul(totalStakeTokenBalance).add(scoreStored);
    }

    function getEarningTokens(address user) public pure returns(address[] memory) {
        address[] memory earnTokens = new address[](2);
        (earnTokens[0], earnTokens[1]) = (solidSexSolidLp, wftmSexLp);
        return earnTokens;
    }

    function getUserEarnings(address user) public view returns(uint256, uint256) {
        uint256 ratio = getUserScore(user).mul(1e18).div(getTotalScore());
        return (totalEarningAmounts[solidSexSolidLp].mul(ratio).div(1e18), 
            totalEarningAmounts[wftmSexLp].mul(ratio).div(1e18));
    }

    function claimEarnings() public {
        (uint256 solidLpAmount, uint256 sexLpAmount) = getUserEarnings(msg.sender);
        if (solidLpAmount > 0) {
            ...
            totalEarningAmounts[solidSexSolidLp] -= solidLpAmount;
        }
        if (sexLpAmount > 0) {
            ...
            totalEarningAmounts[wftmSexLp] -= sexLpAmount;
        }

        userScoreStored[msg.sender] = 0;
        userLastUpdateTime[msg.sender] = block.timestamp;
    }

    function deposit(uint256 amount) public updateScore(msg.sender) {
        require(!stop, "stop");
        IERC20(stakeToken).safeTransferFrom(msg.sender, address(this), amount);
        shares[msg.sender] = shares[msg.sender].add(amount);
        totalStakeTokenBalance = totalStakeTokenBalance.add(amount);
        
    }

    function withdraw(uint256 amount) public updateScore(msg.sender) {
        require(amount <= shares[msg.sender], "invalid");
        
        uint256 stakeTokenBalance = IERC20(stakeToken).balanceOf(address(this));
        if (stakeTokenBalance < amount) {
            // withdraw stake token
            ISolidexDepositor(solidexLpDepositor).withdraw(stakeToken, amount.sub(stakeTokenBalance));
        }

        IERC20(stakeToken).safeTransfer(msg.sender, amount);
        shares[msg.sender] = shares[msg.sender].sub(amount);
        totalStakeTokenBalance = totalStakeTokenBalance.sub(amount);
    }
