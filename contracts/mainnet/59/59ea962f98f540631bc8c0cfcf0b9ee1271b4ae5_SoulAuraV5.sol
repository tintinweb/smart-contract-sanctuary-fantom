...
contract SoulAuraV5 is Ownable {
    ...
    IERC20 chant = IERC20(0x083423C61B9373050e62E2A6Ec170e663F9c7BFa);
    ...
    function soulBondPower(address member) public view returns (uint raw, uint formatted) {
        ...
        uint TWO = bond.pendingSoul(2, member); // FTM-SEANCE
        uint power = ZERO + ONE + TWO;
    ...
    function bondPower(address member) public view returns (uint raw, uint formatted) {
        ...
        uint THREE = bond.pendingSoul(3, member); // USDC
        uint FOUR = bond.pendingSoul(4, member); // DAI
        uint FIVE = bond.pendingSoul(5, member); // BNB
        uint SIX = bond.pendingSoul(6, member); // ETH
        uint SEVEN = bond.pendingSoul(7, member); // BTC
        uint ftmBondPower = THREE + FOUR + FIVE + SIX + SEVEN;

        uint stableBondPower = bond.pendingSoul(9, member); // USDC-DAI
    ...
    function stakedPower(address member) public view returns (uint raw, uint formatted) {
        ...
        uint staked_power = chant.balanceOf(member);
