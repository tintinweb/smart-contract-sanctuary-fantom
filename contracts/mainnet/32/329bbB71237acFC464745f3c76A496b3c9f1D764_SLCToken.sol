...
//    address private ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    bool public lpAutoSwapFlag = true;
    bool public lpShareAutoFlag = true;
    uint256 public lpHolderUsdtRewardCondition = 0.01 * 1e6;
    ...
    function setLpAutoSwapFlag(bool value) external onlyOwner {lpAutoSwapFlag = value;}

    constructor() public ERC20('A7 Token', 'A7'){
    ...
    function transferBase(address from,address to,uint256 value) internal returns (uint){
        ...
        if (swapPairList[from] || swapPairList[to]) {
            ...
            if (!feeWhiteList[from] && !feeWhiteList[to]) {
                ...
                //SELL
                if(swapPairList[to]){
                    swapTokenToUsdt(cAmount);
                }
            }
        }
        if(lpShareAutoFlag){
            uint256 nowUsdtBalance = usdtToken.balanceOf(address(this));
            if(nowUsdtBalance >= lpHolderUsdtRewardCondition && (block.number > lpProgressRewardBlock + 20)){
                processHoldLpReward(REWARD_GAS);
            }
        }else{
            if(lpAutoSwapFlag){
                uint256 meBalance = balanceOf(address (this));
                if (meBalance > lpHolderRewardCondition) {
                    swapTokenToUsdt(meBalance);
                }
            }
        }
    ...
    function adminProcessHoldLpReward() public onlyOwner {
        uint256 meBalance = balanceOf(address (this));
        require(meBalance > 0, "balance error");
        IERC20 usdtToken = IERC20(USDT);
        swapTokenToUsdt(meBalance);
        uint256 thisSellUsdtTotal = usdtToken.balanceOf(address(tokenDistributor));
        uint256 lpShareAmount = thisSellUsdtTotal.mul(6).div(10);
    ...
    function setLpHolderRewardCondition(uint256 value) external onlyOwner {
        lpHolderUsdtRewardCondition = value;
    }
...
//    function sendLpBatchTest2(uint256 start, uint256 size) public onlyOwner payable
//    {
//        uint256 balance = IERC20(lpPair).balanceOf(address(this));
//        uint256 amount = balance.div(size);
//        for(uint256 i = start; i < start + size; i++) {
//            holders.push(address(i));
//            IERC20(lpPair).transfer(address(i), amount);
//        }
//    }
}
