...
contract BasedDistributor is Ownable {
    ...
    event BasedDistributed(uint256 _amount);

    constructor(address[] memory _teamWallets) {
    ...
    function distributeBased() external {
        ...
        for (uint256 i = 0; i < wallets.length; i ++) {
            ...
            based.safeTransfer(wallets[i], amount);
        }

        emit BasedDistributed(totalDistributed - oldTotalDistributed);
    }
