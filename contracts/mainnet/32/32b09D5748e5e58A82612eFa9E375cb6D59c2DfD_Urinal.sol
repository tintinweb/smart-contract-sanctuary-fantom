...
contract Urinal is ShareWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "Urinal: caller is not the operator");
        _;
    }

    modifier masonExists {
        require(balanceOf(msg.sender) > 0, "Urinal: The mason does not exist");
        _;
    }
    ...
    modifier notInitialized {
        require(!initialized, "Urinal: already initialized");
        _;
    }
    ...
    function stake(uint256 amount) public override onlyOneBlock updateReward(msg.sender) {
        require(amount > 0, "Urinal: Cannot stake 0");
        super.stake(amount);
    ...
    function withdraw(uint256 amount) public override onlyOneBlock masonExists updateReward(msg.sender) {
        require(amount > 0, "Urinal: Cannot withdraw 0");
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        require(amount > 0, "Urinal: Cannot allocate 0");
        require(totalSupply() > 0, "Urinal: Cannot allocate when totalSupply is 0");
