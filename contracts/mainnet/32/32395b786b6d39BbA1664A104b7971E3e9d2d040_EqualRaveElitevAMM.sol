...
// File contracts/EqualRaveElitevAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualRaveElitevAMM is Ownable, Pausable {
    ...
    address public constant rave = address(0x88888a335b1F65a79Ec56A610D865b8b25B6060B);
    address public constant elite = address(0xf43Cc235E686d7BC513F53Fbffb61F760c3a1882);
    address public want;
    ...
    IEqualizerRouter.Routes[] public wrappedToRavePath;
    IEqualizerRouter.Routes[] public wrappedToElitePath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToRavePath,
        IEqualizerRouter.Routes[] memory _wrappedToElitePath
        ...
        for (uint i; i < _wrappedToRavePath.length; ++i) {
            wrappedToRavePath.push(_wrappedToRavePath[i]);
        }

        for (uint i; i < _wrappedToElitePath.length; ++i) {
            wrappedToElitePath.push(_wrappedToElitePath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(wrappedHalf, 0, wrappedToRavePath, address(this), block.timestamp);     
        IEqualizerRouter(router).swapExactTokensForTokens(wrappedHalf, 0, wrappedToElitePath, address(this), block.timestamp);    

        uint256 lp0Bal = IERC20(rave).balanceOf(address(this));
        uint256 lp1Bal = IERC20(elite).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(rave, elite, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(rave).safeApprove(router, 0);
        IERC20(elite).safeApprove(router, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(rave).safeApprove(router, type(uint).max);
        IERC20(elite).safeApprove(router, type(uint).max);
    }
    ...
    function setFees(uint256 newCallFee, uint256 newStratFee, uint256 newWithdrawFee, uint256 newTreasuryFee) external onlyAdmin {        require(newWithdrawFee <= 10, "> Max Fee");
