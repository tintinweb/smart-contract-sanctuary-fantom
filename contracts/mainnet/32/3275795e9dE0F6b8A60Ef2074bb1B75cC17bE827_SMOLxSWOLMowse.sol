/**
 *Submitted for verification at FtmScan.com on 2022-08-19
*/
...
// File: contracts/libraries/SlothVerifiableDelay.sol
...
// File: contracts/SMOLxSWOLMowse.sol
...
contract SMOLxSWOLMowse is Context, ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  string public smolBaseURI;
  string public swolBaseURI;
  ...
  // Mapping for SMOL or SWOL type
  // False = SMOL
  // True = SWOL
  mapping(uint256 => bool) public size;
  mapping(uint256 => bool) public revealed;

  // large prime used for VDF
  uint256 public prime = 1130581483564540440614170209683946819683;
  // iterations for the VDF
  uint256 public iterations = 1000;
  // mapping to get seed from tokenId
  mapping(uint256 => uint256) public tokenIdToSeed;

  Randomness.RNG private _rng;
  // Track which IDs have and have not been minted
  uint16[300] public mintedIds;
  uint16 private index;

  address private _trustedForwarder;

  // events
  event MowseMinted(uint256 tokenIds);
  event MowseRevealed(uint256 tokenId);
  event MowseChangeSize(uint256 tokenId);

  constructor(
    string memory _name,
    string memory _symbol,
    string memory _initSmolBaseUri,
    string memory _initSwolBaseUri,
    string memory _initNotRevealedUri,
    address forwarder
  ) ERC721(_name, _symbol) {
    setSmolBaseURI(_initSmolBaseUri);
    setSwolBaseURI(_initSwolBaseUri);
  ...
  function _prove(uint256 proof, uint256 seed) internal view returns (bool) {
    return SlothVerifiableDelay.verify(proof, seed, prime, iterations);
  }
  function _pickRandomUniqueId(uint256 random) internal returns (uint256 id) {
    uint256 len = mintedIds.length - index++;
    require(len > 0, "No more Ids left to choose");
    uint256 randomIndex = random % len;
    id = mintedIds[randomIndex] != 0 ? mintedIds[randomIndex] : randomIndex;
    mintedIds[randomIndex] = uint16(mintedIds[len - 1] == 0 ? len - 1 : mintedIds[len - 1]);
    mintedIds[len - 1] = 0;
  }

  /// Copied from openzeppelin/contracts/metatx/ERC2771Context.sol
  ...
  // external
  // This method creates the seed and accepts the funds to mint
  function mint(uint256 mintAmount) external payable {
    uint256 supply = totalSupply();
    require(!paused, "Contract is paused");
    require(mintAmount > 0, "Must mint at least 1");
    require(mintAmount <= maxMintAmount, "Mint Amount cannot exceed Max Mint Amount");
    require(supply + mintAmount <= maxSupply, "Cannot mint more than max supply");
    
    if (msg.sender != owner()) {
      require(msg.value >= cost * mintAmount, "Insufficient funds");
    }
    for (uint256 i = 0; i < mintAmount; i++) {
      (, uint256 _randomSeed) = _rng.getRandom();
      uint256 randomId = _pickRandomUniqueId(_randomSeed);
      tokenIdToSeed[randomId] = _randomSeed;
      _safeMint(msg.sender, randomId);
      
      emit MowseMinted(randomId);
    }
  }
  function proveById(uint256 tokenId, uint256 proof) external view returns (bool) {
    if (_prove(proof, tokenIdToSeed[tokenId])) {
      return true;
    } else {
      return false;
    }
  ...
  function verifyMessage(string memory message, bytes memory signature) public view returns (address, bool) {
    ...
    if (_msgSender() == signerAddress || isTrustedForwarder(msg.sender)) {
  ...
  //only holder
  function changeSize(uint256 tokenId) public {
    require(_msgSender() == whoOwns(tokenId) || isTrustedForwarder(msg.sender), "Caller does not own this NFT");
    size[tokenId] = !size[tokenId];
    
    emit MowseChangeSize(tokenId);
  }
  function revealMowse(uint256 tokenId) public {
    require(_msgSender() == whoOwns(tokenId) || isTrustedForwarder(msg.sender), "Caller does not own this NFT");
    require(!revealed[tokenId], "Mowse has already been revealed!");
    revealed[tokenId] = true;
    emit MowseRevealed(tokenId);
  }
  ...
  function setSmolBaseURI(string memory _newBaseURI) public onlyOwner {
    smolBaseURI = _newBaseURI;
  }
  function setSwolBaseURI(string memory _newBaseURI) public onlyOwner {
    swolBaseURI = _newBaseURI;
  }
