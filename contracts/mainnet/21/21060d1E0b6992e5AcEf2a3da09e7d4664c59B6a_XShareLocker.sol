...
contract XShareLocker is Operator, IXShareLocker {
    ...
    uint256 public EXTRA_YSHARE_PER_WEEK = MAX_YSHARE_MINTED_PER_XSHARE.div(MAX_LOCK_DURATION).mul(MIN_LOCK_DURATION); //Extra yShare for every 1 week longer lock
    ...
    function calculateYShareMintAmount(uint256 _amount, uint256 _lockDuration) internal returns (uint256){
        ...
        uint256 yShare_minting_amount = _amount.add(boost_amount_factor.mul(EXTRA_YSHARE_PER_WEEK));
    ...
    function addMoreXShare(uint256 _amount) public override {
        require(_amount > 0, "Invalid Amount");
        UserInfo storage user = userInfo[msg.sender];
        require(user.lockEndTime < block.timestamp && user.lockedAmount > 0, "Lock ended / Not locked any xShare");
    ...
    function extendLockDuration(uint256 _extendLockDuration) public override {
        ...
        user.lockEndTime = user.lockedAmount.add(_extendLockDuration);
    ...
    function unlockXShare(uint256 _amount) public override {
        UserInfo storage user = userInfo[msg.sender];
        require(_amount > 0, "Invalid amount");
        require(user.lockedAmount > 0 && block.timestamp >= user.lockEndTime, "Still in lock");
    ...
    function emergencyUnlockAll() public override isUnlockAll {
        ...
        require(IERC20(yShare).balanceOf(msg.sender) >= user.yShareMinted);
