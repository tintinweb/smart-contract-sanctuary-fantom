...
contract AddressLockProxy is Ownable, IAddressLock, ERC165 {
    ...
    function isUnlockable(uint fnftId, uint) external view returns (bool) {
        (uint80 rate, uint80 addInterestRate, uint packetSize, uint sharesAtDeposit, address vaultAdapter,, uint residual)
            = resonate.getPositionInfo(fnftId);

        if(residual > 0) {
            return true;
        }

        uint112 expectedReturn = rate + addInterestRate; //1E18
        uint tokensExpectedPerPacket = packetSize * expectedReturn / PRECISION;
        // TODO: Does precision recovery here cause issues with small share amounts? 
        // Could this ever lead to early unlocks?
        uint tokensAccumulatedPerPacket = IERC4626(vaultAdapter).previewRedeem(sharesAtDeposit / PRECISION) - packetSize;
    ...
    function getDisplayValues(uint fnftId, uint) external view returns (bytes memory output) {
        (uint80 rate, uint80 addInterestRate, uint packetSize, uint sharesAtDeposit, address vaultAdapter,, uint residual)
            = resonate.getPositionInfo(fnftId);
        bool unlocked;
        uint80 expectedReturn = rate + addInterestRate; 
        uint tokensExpectedPerPacket = packetSize * expectedReturn / PRECISION;
