...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant ring = address(0x582423C10c9e83387a96d00A69bA3D11ee47B7b5);  //vaultProfit denominator
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR RING-USDC EQUALIZER")),
            string(abi.encodePacked("XpE-RING-USDC"))
        )
        {
        gauge = _gauge;
        router = _router;
        ...
        slippageLPs = [address(0x7547d05dFf1DA6B4A2eBB3f0833aFE3C62ABD9a1), address(_asset), address(0x76fa7935a5AFEf7fefF1C88bA858808133058908)];
    ...
    function _chargeFees(address caller) internal {                   
        ...
        uint feeBal = SafeTransferLib.balanceOf(equal, address(this)) * platformFee / FEE_DIVISOR;
        uint toProfit = equalBal - feeBal;
    ...
    function _addLiquidity() internal {
        uint equalBal = SafeTransferLib.balanceOf(equal, address(this));
        (uint minAmtUsdc) = getSlippage(equalBal, slippageLPs[2], equal);
        IEqualizerRouter(router).swapExactTokensForTokensSimple(equalBal, minAmtUsdc, equal, usdc, false, address(this), lastHarvest);

        uint usdcHalf = SafeTransferLib.balanceOf(usdc, address(this)) >> 1;
        (uint minAmtRing) = getSlippage(usdcHalf, slippageLPs[1], usdc);
        IEqualizerRouter(router).swapExactTokensForTokensSimple(usdcHalf, minAmtRing, usdc, ring, false, address(this), lastHarvest);
        
        uint t1Bal = SafeTransferLib.balanceOf(usdc, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(ring, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(usdc, ring, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(usdc, ring, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyOwner {
        if(_token == equal || _token == ring || _token == usdc){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(ring, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(ring, router, type(uint).max);
    }
