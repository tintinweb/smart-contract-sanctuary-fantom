...
contract Zapper is Ownable, IZapper {
    ...
    function zapInTokensETH(address _token1,uint256 _token1Amount,address _routerAddr,
        address _recipient, uint256 _minAmountLp ) payable external whitelist(_routerAddr)
    {
    ...
    function zapOutToTokenEth(address _in, uint256 _amount,
        ...
        address token;
        if(IUniswapV2Pair(_in).token0() != NATIVE)
            token = IUniswapV2Pair(_in).token0();
        else
            token = IUniswapV2Pair(_in).token1();


        (uint256 amountToken,uint256 amountETH) =
        IUniswapV2Router(_routerAddr).removeLiquidityETH(token,
            _amount, 0, 0, address(this), block.timestamp);

        IERC20(token).safeTransfer(_recipient, amountToken);
