...
contract Bat is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 BAT-WFTM LP -> BAT pool
    uint256 public constant INITIAL_BAT_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the BAT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("BAT", "BAT") {
        // Mints 1 BAT to contract creator for initial pool setup
    ...
    function _getBatPrice() internal view returns (uint256 _batPrice) {
        try IOracle(batOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Bat: failed to fetch BAT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _batPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_batPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBatOracle(address _batOracle) public onlyOperatorOrTaxOffice {
        require(_batOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BAT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BAT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBatPrice = _getBatPrice();
            currentTaxRate = _updateTaxRate(currentBatPrice);
            if (currentBatPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_batPool != address(0), "!_batPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_batPool, INITIAL_BAT_POOL_DISTRIBUTION);
