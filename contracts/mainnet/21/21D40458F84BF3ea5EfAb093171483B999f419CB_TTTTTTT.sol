...
contract TTTTTTT is Ownable, TOKEN, ReentrancyGuard, BlackList {
    ...
    mapping (bytes => uint) public totalPoolBet;

    mapping (address => mapping (bytes => uint)) public betArray;

    event BuyToken(address indexed buyer, uint256 value);

    event SellToken(address indexed seller, uint256 value);

    event CreateBet(address indexed beter, bytes indexed pool, uint256 value);

    event WinBet(address indexed beter, bytes indexed pool, uint256 value);

    event NobodyWin(bytes indexed pool);
    ...
    function createBet(bytes memory pool_, uint amount_) external onlyIfResetNotInProgress {
    ...
    function winBet(bytes memory pool_) external onlyOwner {
    ...
    function sellToken(uint amount_) external nonReentrant {
        require(balanceOf(_msgSender()) >= amount_, "Insufficient funds");
        _burn(_msgSender(), amount_);
    ...
    function setPool(bytes memory pool_) external onlyOwner{
        pools.push(pool_);
    }
    ...
    function withdrawalsFee() external onlyOwner {
    ...
    function getPools() external view returns (bytes[] memory){
        return pools;
    }
