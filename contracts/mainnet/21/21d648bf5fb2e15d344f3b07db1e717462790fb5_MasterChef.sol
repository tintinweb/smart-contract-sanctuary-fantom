...
contract ZoneToken is ERC20 {
    ...
    // The operator can call all functions except of minting ZoneToken
    ...
    /**
     * @notice Constructs the Defi Zone Token contract.
     */
    constructor(address _to, uint256 _initialSupply, uint256 _maxSupply) ERC20("Zone Token", "ZONE") {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ZONEs to distribute per second.
        uint256 lastRewardSecond; // Last second that ZONEs distribution occurs.
    ...
    // The COIN!
    ZoneToken public immutable zone;
    // Dev address.
    address public devaddr;
    // COIN tokens created per second.
    uint256 public ZonePerSecond;
    // Deposit Fee address
    ...
    // The block timestamp when zone mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 ZonePerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _ZonePerSecond,
        uint256 _startTime
    ) {
        zone = _zone;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        ZonePerSecond = _ZonePerSecond;
    ...
    // Update the given pool's zone allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    function initializePools(address _nativeLP) external onlyOwner {
        ...
        add(200, IERC20(_nativeLP), 0, true, false);                                        // ZONE LP
        add(150, IERC20(zone), 0, true, false);                                             // ZONE
    ...
    // View function to see pending zone on frontend.
    function pendingZone (uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 coinReward = multiplier
                ...
                uint256 totalRewards = zone.totalSupply().add(devReward).add(
                coinReward
            );
                if (totalRewards > zone.maxSupply()) {
                coinReward = zone.maxSupply().sub(zone.totalSupply());
            }
            accZonePerShare = accZonePerShare.add(
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= zone.maxSupply()) {
            ...
            zone.mint(devaddr, coinReward.div(10));
            zone.mint(address(this), coinReward);
        } else {
        ...
        if (coinReward != 0) {
            ...
            pool.accZonePerShare = pool.accZonePerShare.add(
                coinReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    // Deposit LP tokens to MasterChef for zone allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accZonePerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                safeZoneTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accZonePerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeZoneTransfer(msg.sender, pending);
    ...
    // Safe zone transfer function, just in case if rounding error causes pool to not have enough coins.
    function safeZoneTransfer(address _to, uint256 _amount) internal {
        uint256 zoneBal = zone.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > zoneBal) {
            transferSuccess = zone.transfer(_to, zoneBal);
        } else {
            transferSuccess = zone.transfer(_to, _amount);
        }
        require(transferSuccess, "safeZoneTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _ZonePerSecond) external onlyOwner {
        require(_ZonePerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        ZonePerSecond = _ZonePerSecond;
        emit UpdateEmissionRate(msg.sender, _ZonePerSecond);
    }
