...
contract ScarabERC20 {
    ...
    function _checkSignature(
        ...
        require(deadline >= block.timestamp, "Scarab: EXPIRED");
        bytes32 digest =
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
...
contract PoolToken is IPoolToken, ScarabERC20 {
    ...
    function _setFactory() external {
        require(factory == address(0), "Scarab: FACTORY_ALREADY_SET");
        factory = msg.sender;
    }
        ...
        returns (uint256 mintTokens)
    {
        ...
        require(mintTokens > 0, "Scarab: MINT_AMOUNT_ZERO");
        ...
        returns (uint256 redeemAmount)
    {
        ...
        require(redeemAmount > 0, "Scarab: REDEEM_AMOUNT_ZERO");
        require(redeemAmount <= totalBalance, "Scarab: INSUFFICIENT_CASH");
    ...
    modifier nonReentrant() {
        require(_notEntered, "Scarab: REENTERED");
        _notEntered = false;
        _;
        _notEntered = true;
    }
...
contract CSetter is PoolToken, CStorage {
    ...
    function _initialize(
        ...
        require(msg.sender == factory, "Scarab: UNAUTHORIZED"); // sufficient check
    ...
    function _checkSetting(
        ...
        _checkAdmin();
        require(parameter >= min, "Scarab: INVALID_SETTING");
        require(parameter <= max, "Scarab: INVALID_SETTING");
    }

    function _checkAdmin() internal view {
        require(msg.sender == IFactory(factory).admin(), "Scarab: UNAUTHORIZED");
    }
}
...
contract Collateral is ICollateral, PoolToken, CStorage, CSetter {
    ...
    function getPrices() public returns (uint256 price0, uint256 price1) {
        (uint224 twapPrice112x112, ) =
            IScarabPriceOracle(tarotPriceOracle).getResult(underlying);
         ...
         */
        require(price0 > 100, "Scarab: PRICE_CALCULATION_ERROR");
        require(price1 > 100, "Scarab: PRICE_CALCULATION_ERROR");
    }
    ...
    function _transfer(
        ...
        require(tokensUnlocked(from, value), "Scarab: INSUFFICIENT_LIQUIDITY");
    ...
    function seize(
        ...
        require(shortfall > 0, "Scarab: INSUFFICIENT_SHORTFALL");
    ...
    function flashRedeem(
        ...
        _safeTransfer(redeemer, redeemAmount);
        if (data.length > 0)
            IScarabCallee(redeemer).tarotRedeem(msg.sender, redeemAmount, data);
        ...
        require(
            ...
            "Scarab: INSUFFICIENT_REDEEM_TOKENS"
        );
