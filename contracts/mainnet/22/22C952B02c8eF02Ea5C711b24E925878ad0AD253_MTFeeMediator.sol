...
contract MTFeeMediator is BalanceManagement {
    ...
    uint256 public feeDistributionMTLockersPart; // Value in millipercent
    ...
    event SetDistributionParts(
        uint256 buybackPart,
        uint256 feeDistributionMTLockersPart,
        uint256 feeDistributionLPLockersPart
    );
    ...
    constructor(
        ...
        address _feeDistributionMTLockersAddress,
        address _feeDistributionLPLockersAddress,
        address _treasuryAddress,
        uint256 _buybackPart, // Value in millipercent
        uint256 _feeDistributionMTLockersPart, // Value in millipercent
        ...
        feeDistributionMTLockersAddress = _feeDistributionMTLockersAddress;
        feeDistributionLPLockersAddress = _feeDistributionLPLockersAddress;
        treasuryAddress = _treasuryAddress;

        _setDistributionParts(
            _buybackPart,
            _feeDistributionMTLockersPart,
            _feeDistributionLPLockersPart
        );
    ...
    function setFeeDistributionMTLockersAddress(
        address _feeDistributionMTLockersAddress
    ) external onlyManager {
        feeDistributionMTLockersAddress = _feeDistributionMTLockersAddress;

        emit SetFeeDistributionMTLockersAddress(_feeDistributionMTLockersAddress);
    }

    function setFeeDistributionLPLockersAddress(
        address _feeDistributionLPLockersAddress
    ) external onlyManager {
        feeDistributionLPLockersAddress = _feeDistributionLPLockersAddress;

        emit SetFeeDistributionMTLockersAddress(_feeDistributionLPLockersAddress);
    }
    ...
    function setDistributionParts(
        ...
        uint256 _feeDistributionMTLockersPart,
        uint256 _feeDistributionLPLockersPart
    ) external onlyManager {
        _setDistributionParts(
            _buybackPart,
            _feeDistributionMTLockersPart,
    ...
    function process() external onlyManager {
        ...
        for (uint256 assetIndex; assetIndex < assetListLength; assetIndex++) {
            ...
            if (assetBalance > 0) {
                ...
                uint256 feeDistributionMTLockersAmount = (assetBalance *
                    feeDistributionMTLockersPart) / Constants.MILLIPERCENT_FACTOR;
                uint256 feeDistributionLPLockersAmount = (assetBalance *
                    feeDistributionLPLockersPart) / Constants.MILLIPERCENT_FACTOR;
                uint256 treasuryAmount = assetBalance -
                    buybackAmount -
                    feeDistributionMTLockersAmount -
                    feeDistributionLPLockersAmount;

                if (buybackAmount > 0) {
                    if (AddressHelper.isContract(buybackAddress)) {
                        // Transfer to buyback contract
                        TransferHelper.safeApprove(assetToken, buybackAddress, buybackAmount);
                        IAssetReceiver(buybackAddress).receiveAsset(assetToken, buybackAmount);
                        TransferHelper.safeApprove(assetToken, buybackAddress, 0);
                    } else {
                ...
                if (feeDistributionMTLockersAmount > 0) {
                ...
                if (feeDistributionLPLockersAmount > 0) {
    ...
    function _setDistributionParts(
        ...
        if (
            _buybackPart + _feeDistributionMTLockersPart + _feeDistributionLPLockersPart >
            Constants.MILLIPERCENT_FACTOR
        ) {
        ...
        feeDistributionMTLockersPart = _feeDistributionMTLockersPart;
        feeDistributionLPLockersPart = _feeDistributionLPLockersPart;

        emit SetDistributionParts(
            _buybackPart,
            _feeDistributionMTLockersPart,
            _feeDistributionLPLockersPart
        );
