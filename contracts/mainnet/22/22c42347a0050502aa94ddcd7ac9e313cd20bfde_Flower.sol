...
contract Flower is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 0 Flower-WFTM LP -> Flower pool
    uint256 public constant INITIAL_Flower_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the Flower ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Flower", "Flower") {
        // Mints 1 Flower to contract creator for initial pool setup
    ...
    function _getFlowerPrice() internal view returns (uint256 _FlowerPrice) {
        try IOracle(FlowerOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Flower: failed to fetch Flower price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _FlowerPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_FlowerPrice >= taxTiersTwaps[tierId]) {
    ...
    function setFlowerOracle(address _FlowerOracle) public onlyOperatorOrTaxOffice {
        require(_FlowerOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints Flower to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Flower to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFlowerPrice = _getFlowerPrice();
            currentTaxRate = _updateTaxRate(currentFlowerPrice);
            if (currentFlowerPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_FlowerPool != address(0), "!_FlowerPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_FlowerPool, INITIAL_Flower_POOL_DISTRIBUTION);
