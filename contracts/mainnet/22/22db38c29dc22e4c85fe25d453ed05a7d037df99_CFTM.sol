...
contract CFTM is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_TEAM_FUND_DISTRIBUTION = 2000 ether;
    ...
    function _getCftmPrice() internal view returns (uint256 _cftmPrice) {
        try IOracle(cftmOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Error: failed to fetch CFTM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cftmPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cftmPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCftmOracle(address _cftmOracle) public onlyOperatorOrTaxOffice {
        require(_cftmOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCftmPrice = _getCftmPrice();
            currentTaxRate = _updateTaxRate(currentCftmPrice);
            if (currentCftmPrice < burnThreshold) {
                burnTax = true;
            }
        }
