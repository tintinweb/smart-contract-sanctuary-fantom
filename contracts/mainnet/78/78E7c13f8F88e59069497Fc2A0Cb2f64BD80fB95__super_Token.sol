...
// File contracts/super.sol



pragma solidity 0.6.12;

contract _super_Token is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Super Token", "SUPER") {
    ...
    function _getsuperPrice() internal view returns (uint256 _superPrice) {
        try IOracle(superOracle).consult(address(this), 1e18) returns (uint144 _price) {
    ...
    function setsuperOracle(address _superOracle) public onlyOperatorOrTaxOffice {
        require(_superOracle != address(0), "oracle address cannot be 0 address");
