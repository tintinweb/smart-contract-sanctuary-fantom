...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0xF0E09F1C9fCa4d70CC73742D521b80cE04CD0cbd) // DollarGenesisPool
    ];
    ...
    uint256 public paperDepletionFloorPercent;
    ...
    uint256 public maxDiscountRate; // when purchasing paper
    uint256 public maxPremiumRate; // when redeeming paper
    ...
    event BurnedPapers(address indexed from, uint256 paperAmount);
    event RedeemedPapers(address indexed from, uint256 dollarAmount, uint256 paperAmount);
    event BoughtPapers(address indexed from, uint256 dollarAmount, uint256 paperAmount);
    ...
    function getBurnableDollarLeft() public view returns (uint256 _burnableDollarLeft) {
        ...
        if (_dollarPrice <= dollarPriceOne) {
            ...
            uint256 _paperMaxSupply = _dollarSupply.mul(maxDebtRatioPercent).div(10000);
            uint256 _paperSupply = IERC20(paper).totalSupply();
            if (_paperMaxSupply > _paperSupply) {
                uint256 _maxMintablePaper = _paperMaxSupply.sub(_paperSupply);
                uint256 _maxBurnableDollar = _maxMintablePaper.mul(_dollarPrice).div(1e18);
                _burnableDollarLeft = Math.min(epochSupplyContractionLeft, _maxBurnableDollar);
            }
        }
    }

    function getRedeemablePapers() public view returns (uint256 _redeemablePapers) {
        uint256 _dollarPrice = getDollarPrice();
        if (_dollarPrice > dollarPriceCeiling) {
            uint256 _totalDollar = IERC20(dollar).balanceOf(address(this));
            uint256 _rate = getPaperPremiumRate();
            if (_rate > 0) {
                _redeemablePapers = _totalDollar.mul(1e18).div(_rate);
            }
        }
    }

    function getPaperDiscountRate() public view returns (uint256 _rate) {
        uint256 _dollarPrice = getDollarPrice();
        if (_dollarPrice <= dollarPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _paperAmount = dollarPriceOne.mul(1e18).div(_dollarPrice); // to burn 1 SCARAB
                uint256 _discountAmount = _paperAmount.sub(dollarPriceOne).mul(discountPercent).div(10000);
    ...
    function getPaperPremiumRate() public view returns (uint256 _rate) {
        uint256 _dollarPrice = getDollarPrice();
        if (_dollarPrice > dollarPriceCeiling) {
    ...
    function initialize(
        ...
        paperDepletionFloorPercent = 10000; // 100% of Paper supply for depletion floor
        seigniorageExpansionFloorPercent = 3500; // At least 35% of expansion reserved for masonry
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn SCARAB and mint tBOND)
    ...
    function setPaperDepletionFloorPercent(uint256 _paperDepletionFloorPercent) external onlyOperator {
        require(_paperDepletionFloorPercent >= 500 && _paperDepletionFloorPercent <= 10000, "out of range"); // [5%, 100%]
        paperDepletionFloorPercent = _paperDepletionFloorPercent;
    }
    ...
    function _updateDollarPrice() internal {
        bool canUpdate = IOracle(dollarOracle).canUpdate();
        if (canUpdate) {
    ...
    function buyPapers(uint256 _dollarAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        require(_dollarAmount > 0, "Treasury: cannot purchase papers with zero amount");

        uint256 dollarPrice = getDollarPrice();
        require(dollarPrice == targetPrice, "Treasury: SCARAB price moved");
        require(
            dollarPrice < dollarPriceOne, // price < $1
            "Treasury: dollarPrice not eligible for paper purchase"
        );

        require(_dollarAmount <= epochSupplyContractionLeft, "Treasury: not enough paper left to purchase");

        uint256 _rate = getPaperDiscountRate();
        require(_rate > 0, "Treasury: invalid paper rate");

        uint256 _paperAmount = _dollarAmount.mul(_rate).div(1e18);
        uint256 dollarSupply = getDollarCirculatingSupply();
        uint256 newPaperSupply = IERC20(paper).totalSupply().add(_paperAmount);
        require(newPaperSupply <= dollarSupply.mul(maxDebtRatioPercent).div(10000), "over max debt ratio");

        IBasisAsset(dollar).burnFrom(msg.sender, _dollarAmount);
        IBasisAsset(paper).mint(msg.sender, _paperAmount);

        epochSupplyContractionLeft = epochSupplyContractionLeft.sub(_dollarAmount);
        _updateDollarPrice();

        emit BoughtPapers(msg.sender, _dollarAmount, _paperAmount);
    }

    function redeemPapers(uint256 _paperAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        require(_paperAmount > 0, "Treasury: cannot redeem papers with zero amount");
        ...
        uint256 _dollarAmount = _paperAmount.mul(_rate).div(1e18);
        ...
        IBasisAsset(paper).burnFrom(msg.sender, _paperAmount);
        IERC20(dollar).safeTransfer(msg.sender, _dollarAmount);

        _updateDollarPrice();

        emit RedeemedPapers(msg.sender, _dollarAmount, _paperAmount);
    }
    ...
    function _sendToMasonry(uint256 _amount) internal {
        ...
        IERC20(dollar).safeApprove(masonry, 0);
        IERC20(dollar).safeApprove(masonry, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToMasonry(dollarSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochDollarPrice > dollarPriceCeiling) {
                // Expansion ($SCARAB Price > 1 $FTM): there is some seigniorage to be allocated
                uint256 paperSupply = IERC20(paper).totalSupply();
                uint256 _percentage = previousEpochDollarPrice.sub(dollarPriceOne);
                uint256 _savedForPaper;
                uint256 _savedForMasonry;
                uint256 _mse = _calculateMaxSupplyExpansionPercent(dollarSupply).mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if (seigniorageSaved >= paperSupply.mul(paperDepletionFloorPercent).div(10000)) {
                    // saved enough to pay debt, mint as usual rate
                    _savedForMasonry = dollarSupply.mul(_percentage).div(1e18);
                } else {
                    ...
                    _savedForPaper = _seigniorage.sub(_savedForMasonry);
                    if (mintingFactorForPayingDebt > 0) {
                        _savedForPaper = _savedForPaper.mul(mintingFactorForPayingDebt).div(10000);
                    }
                }
                ...
                if (_savedForPaper > 0) {
                    seigniorageSaved = seigniorageSaved.add(_savedForPaper);
                    IBasisAsset(dollar).mint(address(this), _savedForPaper);
                    emit TreasuryFunded(block.timestamp, _savedForPaper);
                }
            }
        }
    }
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(gold), "share");
