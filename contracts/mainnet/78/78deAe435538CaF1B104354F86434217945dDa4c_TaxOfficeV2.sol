...
contract TaxOfficeV2 is Operator {
    ...
    function _excludeAddressFromTax(address _address) private returns(bool) {
        if (!ITaxable(pumpkin).isAddressExcluded(_address))
    ...
    function taxRate() external view returns (uint256) {
        return ITaxable(pumpkin).getTaxRate();
    }
    ...
    {
        require(amtPumpkin != 0 && amtToken != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);

        IERC20(pumpkin).transferFrom(msg.sender, address(this), amtPumpkin);
        IERC20(token).transferFrom(msg.sender, address(this), amtToken);
        _approveTokenIfNeeded(pumpkin, uniRouter);
        _approveTokenIfNeeded(token, uniRouter);

        _includeAddressInTax(msg.sender);

        uint256 resultAmtPumpkin;
        uint256 resultAmtToken;
        uint256 liquidity;
        (resultAmtPumpkin, resultAmtToken, liquidity) = IUniswapV2Router(uniRouter).addLiquidity(
            pumpkin,
            token,
            amtPumpkin,
            amtToken,
        ...
        if(amtPumpkin.sub(resultAmtPumpkin) > 0) {
            IERC20(pumpkin).transfer(msg.sender, amtPumpkin.sub(resultAmtPumpkin));
        }
        if(amtToken.sub(resultAmtToken) > 0) {
            IERC20(token).transfer(msg.sender, amtToken.sub(resultAmtToken));
        }
        return (resultAmtPumpkin, resultAmtToken, liquidity);
    }
    ...
    {
        require(amtPumpkin != 0 && msg.value != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);
        ...
        uint256 liquidity;
        (resultAmtPumpkin, resultAmtFtm, liquidity) = IUniswapV2Router(uniRouter).addLiquidityETH{value: msg.value}(
            pumpkin,
            amtPumpkin,
            amtPumpkinMin,
            amtFtmMin,
        ...
        return (resultAmtPumpkin, resultAmtFtm, liquidity);
    }
    ...
    function taxFreeTransferFrom(
        ...
        IERC20(pumpkin).transferFrom(_sender, _recipient, _amt);
