...
contract GasStation is Ownable {
    ...
    /* Wrapped Fantom Token */
    IWFTM public WFTM_ERC20 =
        IWFTM(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    ...
    function _wrapFTM(uint256 amount) internal returns (uint256) {
        WFTM_ERC20.deposit{value: amount}();
        uint256 amountWrapped = WFTM_ERC20.balanceOf(address(this));
        return amountWrapped;
    }

    function _swapToLINK(uint256 amountIn) internal returns (uint256) {
        WFTM_ERC20.approve(address(router), amountIn);
        address[] memory path;
        path = new address[](2);
        path[0] = address(WFTM_ERC20);
