...
contract PromoDeposit is Ownable {
	...
	mapping(address => mapping(uint256 => uint256)) public aprs;
	...
	constructor(
		...
		/// Should we call setAprs here? Can cause increase in gas
	}
	...
	function setAprs(address[] calldata addrs, uint256[][] calldata durations, uint256[][] calldata _aprs) external onlyOwner {
		for (uint256 i = 0; i < addrs.length; i++) {
			for (uint256 j = 0; j < durations.length; j++) {
				aprs[addrs[i]][durations[i][j]] = _aprs[i][j];
			}
		}
	}
