...
contract ArbV6_1 is Operator {
    ...
    function sqrt_babylonian2(uint y) public pure returns (uint z) {
        if (y > 3) {
            z = y;
            uint x = y / 2 + 1;
            while (x < z) {
    ...
    function sqrt_babylonian1(uint256 x) public pure returns (uint256) {
        if (x == 0) {
            return 0;
        }
        uint256 y = x;
    ...
    function sqrt_binarysearch1(uint256 x) public pure returns (uint256) {
        if (x == 0) {
            return 0;
        }
    ...
    function sqrt_newtonRaphson(uint256 x) public pure returns (uint256) {
        if (x == 0) {
            return 0;
        }
    ...
    function sqrt_binarysearch2(uint256 y) public pure returns (uint256 z) {
        if (y > 3) {
            uint256 x = y / 2 + 1;
            z = y;
            while (x < z) {
    ...
    function calculateY (uint256 _pricePublic, uint256 _kPrivate, uint8 _decimalAsset) public pure returns (uint256) {
        uint256 dAsset = uint256(_decimalAsset);
        uint256 y = _kPrivate.mul(_pricePublic).mul(10 ** dAsset);
        return y;
    }

    function getNumerator (uint256 y) external view returns (
        uint256 numerator, uint256 n1, uint256 n2, uint256 n3, uint256 n4, uint256 n5
        ) {
        n1 = this.sqrt_babylonian1(y);
        n2 = this.sqrt_babylonian2(y);
        n3 = this.sqrt_binarysearch1(y);
        n4 = this.sqrt_binarysearch2(y);
        n5 = this.sqrt_newtonRaphson(y);
        numerator = n1>0 ? n1 : (n2>0 ? n2 : (n3>0 ? n3 : (n4>0 ? n4 : (n5>0 ? n5 : 0))));
    }

    function getNewReserve(uint256 _pricePublic, uint256 _kPrivate, uint8 _decimalBase, uint8 _decimalAsset) 
        external view returns (
            uint256 newReserve
        ) {
        uint256 dBase = uint256(_decimalBase);
        uint256 dAsset = uint256(_decimalAsset);
        uint256 y = this.calculateY(_pricePublic, _kPrivate, _decimalAsset);
        (uint256 num, , , , , ) = this.getNumerator(y);
        uint256 den = 10 ** (dAsset.add(dAsset).sub(dBase));
        newReserve = num.div(den);
    }
    ...
    function getAmount(address _privatePool, address _publicPool, address _baseAddr) external view returns (uint256) {
        ...
        uint256 reserveNew = (token0_addr == _baseAddr) ? 
            this.getNewReserve(pricePublic, kPrivate, decimal0, decimal1) : 
            this.getNewReserve(pricePublic, kPrivate, decimal1, decimal0); 
        uint256 amt = (reserveNew > reserveOld) ? reserveNew - reserveOld : reserveOld - reserveNew;
        return amt;
    }
                                    
    function comparePriceDiff(uint256 _pricePublic, uint256 _pricePrivate, address _privateRouter, address _publicRouter) external pure returns (
            uint256,
            address,
            address
        ) {
        ...
        if (_pricePublic > _pricePrivate) {
            _priceDiffPercentage = (_pricePublic.sub(_pricePrivate)).mul(10000).div(_pricePrivate);
            _addr1 = _privateRouter;
            _addr2 = _publicRouter;
        } else {
            _priceDiffPercentage = (_pricePrivate.sub(_pricePublic)).mul(10000).div(_pricePrivate);
