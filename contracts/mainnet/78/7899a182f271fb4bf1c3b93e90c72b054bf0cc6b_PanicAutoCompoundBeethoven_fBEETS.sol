 ...
 */
contract PanicAutoCompoundBeethoven_fBEETS is Ownable, Pausable {
    ...
    /**
     * @dev Panic Contracts:
     * {treasury} - Address of the Panic treasury
    ...
    uint public treasuryFee = 10000;
    uint public totalFee = 2000;
    uint constant public MAX_FEE = 2000;
    ...
    constructor (
      ...
      address _treasury
        ...
        _giveAllowances();
        harvestLog.push(Harvest({timestamp: block.timestamp, vaultSharePrice: IRFVault(_vault).pricePerShare()}));
    }
    ...
    require(msg.sender == vault, "!vault");
      ...
      IERC20(lpToken).safeTransfer(vault, pairBal);
    }
    ...
    function _harvest() internal whenNotPaused {
        ...
        if (block.timestamp >= harvestLog[harvestLog.length - 1].timestamp + harvestLogCadence) {
            harvestLog.push(
                Harvest({timestamp: block.timestamp, vaultSharePrice: IRFVault(vault).pricePerShare()})
            );
        }
    ...
    function _chargeFees() internal {
        uint256 fBeetsBal = IERC20(lpToken).balanceOf(address(this));

        uint256 fees = fBeetsBal.mul(totalFee).div(PERCENT_DIVISOR);
        uint256 treasuryFeeToVault = fees.mul(treasuryFee).div(PERCENT_DIVISOR);
        uint shares = IRFVault(vault).deposit(treasuryFeeToVault);
        IERC20(vault).transfer(treasury, shares);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(fBEETS).safeApprove(vault, 0);
        ...
        IERC20(fBEETS).safeApprove(vault, type(uint256).max);
    ...
    function swapBribeToken(address _token, bytes32 _route) external onlyOwner {
        _swap(_token, rewardToken, _route, IERC20(_token).balanceOf(address(this)));
        _harvest();
    }
