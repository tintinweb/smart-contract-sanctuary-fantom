...
contract Firefly is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the FIREFLY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("FIREFLY", "FIREFLY") {
        // Mints 1 FIREFLY to contract creator for initial pool setup
    ...
    function _getFireflyPrice() internal view returns (uint256 _fireflyPrice) {
        try IOracle(fireflyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Firefly: failed to fetch FIREFLY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _fireflyPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_fireflyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setFireflyOracle(address _fireflyOracle) public onlyOperatorOrTaxOffice {
        require(_fireflyOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints FIREFLY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FIREFLY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFireflyPrice = _getFireflyPrice();
            currentTaxRate = _updateTaxRate(currentFireflyPrice);
            if (currentFireflyPrice < burnThreshold) {
                burnTax = true;
            }
        }
