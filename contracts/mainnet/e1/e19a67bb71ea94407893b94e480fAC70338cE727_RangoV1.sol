...
import "RangoCBridge.sol";
import "RangoAnyswap.sol";

contract RangoV1 is RangoCBridge, RangoAnyswap {
...
contract RangoCBridge is Base, MessageSenderApp, MessageReceiverApp {
    address cBridgeAddress;

//    event SourceSwapRequestSent(bytes32 id, uint64 dstChainId, uint256 srcAmount, address srcToken, address dstToken);
    event DestinationSwapRequestDone(bytes32 id, uint256 dstAmount, DestinationSwapStatus status);
    event SwapRequestDone(bytes32 id, uint256 dstAmount, DestinationSwapStatus status);

    struct DestinationUniswapV2Swap {
        address dexAddress;
        address fromToken;
        address toToken;
        uint amountOutMin;
    ...
    struct DestinationMessage {
        DestinationUniswapV2Swap swap;
        address sender;
        address receiver;
    }

    enum DestinationSwapStatus {
        Null,
        Succeeded,
        Failed,
        Fallback
    ...
    function cBridgeSend(
        ...
        approve(request.toToken, cBridgeAddress, outputAmount);

        CBridge cbridge = CBridge(cBridgeAddress);
        cbridge.send(_receiver, request.toToken, outputAmount, _dstChainId, _nonce, _maxSlippage);
    }
    ...
    function cBridgeIM(
        ...
        address _receiver, // The receiver app contract address, not recipient
        uint64 _dstChainId,
        uint64 _nonce,
        uint32 _maxSlippage,

        DestinationUniswapV2Swap memory _destinationSwap,
        address _destinationReceiver
    ) external whenNotPaused nonReentrant {
        require(messageBus != NULL_ADDRESS, 'cBridge message-bus address not set');

        (, uint out1) = onChainSwapsInternal(request, calls);
        approve(request.toToken, cBridgeAddress, out1);
        uint64 chainId = uint64(block.chainid);

        bytes memory message = abi.encode(DestinationMessage({
            swap: _destinationSwap,
            sender: msg.sender,
            receiver: _destinationReceiver
        }));
        uint256 sgnFee = IMessageBusSender(messageBus).calcFee(message);
...
//        emit SourceSwapRequestSent(id, _dstChainId, out1, request.fromToken, request.toToken);
    }
    ...
    function executeMessageWithTransferRefund(
        ...
        DestinationMessage memory m = abi.decode((_message), (DestinationMessage));
        TransferHelper.safeTransfer(_token, m.sender, _amount);
    ...
    function executeMessageWithTransfer(
        ...
        require(_token == m.swap.fromToken, "bridged token must be the same as the requested swap token");
        bytes32 id = _computeSwapRequestId(m.receiver, _srcChainId, uint64(block.chainid), _message);
        uint256 dstAmount;
        DestinationSwapStatus status = DestinationSwapStatus.Succeeded;

        if (m.swap.path.length > 1) {
            bool ok = true;
            (ok, dstAmount) = _trySwap(m.swap, _amount);
            if (ok) {
                _sendToken(m.swap.toToken, dstAmount, m.receiver, m.swap.nativeOut);
                status = DestinationSwapStatus.Succeeded;
            } else {
                // handle swap failure, send the received token directly to receiver
                _sendToken(_token, _amount, m.receiver, false);
                dstAmount = _amount;
                status = DestinationSwapStatus.Fallback;
            }
        } else {
            // no need to swap, directly send the bridged token to user
            _sendToken(m.swap.path[0], _amount, m.receiver, m.swap.nativeOut);
            dstAmount = _amount;
            status = DestinationSwapStatus.Succeeded;
        }
    ...
    function executeMessageWithTransferFallback(
        ...
        DestinationMessage memory m = abi.decode((_message), (DestinationMessage));
        bytes32 id = _computeSwapRequestId(m.receiver, _srcChainId, uint64(block.chainid), _message);
        emit SwapRequestDone(id, 0, DestinationSwapStatus.Failed);
    ...
    function _trySwap(DestinationUniswapV2Swap memory _swap, uint256 _amount) private returns (bool ok, uint256 amountOut) {
...
contract Base is Pausable, Initializable, Ownable, ReentrancyGuard {
