...
contract SpookyV2MatrixLpAutoCompound is MatrixLpAutoCompound {
    ...
    function _setDefaultSwapPaths() internal virtual override {
        ...
        if (output2 != address(0)) {
            ...
            // BTC -> WFTM
            address[] memory _btcWFTM = new address[](2);
            _btcWFTM[0] = BTC;
            _btcWFTM[1] = WFTM;
            _setSwapPath(BTC, WFTM, SPOOKYSWAP_ROUTER, _btcWFTM);

            // WFTM -> BTC
            address[] memory _wftmBtc = new address[](2);
            _wftmBtc[0] = WFTM;
            _wftmBtc[1] = BTC;
            _setSwapPath(WFTM, BTC, SPOOKYSWAP_ROUTER, _wftmBtc);

            // BOO -> BTC
            address[] memory _booBtc = new address[](3);
            _booBtc[0] = BOO;
            _booBtc[1] = WFTM;
            _booBtc[2] = BTC;
            _setSwapPath(BOO, BTC, SPOOKYSWAP_ROUTER, _booBtc);
        }
    }
