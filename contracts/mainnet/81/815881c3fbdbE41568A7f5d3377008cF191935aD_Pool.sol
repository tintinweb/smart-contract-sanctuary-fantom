...
contract Pool is Ownable, ReentrancyGuard {
    ...
    function zap(uint256 _minXftmOut) external payable nonReentrant {
        ...
        if (_fantasmOut > 0 && _ftmSwapIn > 0) {
            ...
            helper.zap(_ftmSwapIn, _fantasmOut);
        }
    ...
    function setPoolHelper(IPoolHelper _helper) external onlyOwner {
        require(address(_helper) != address(0), "Pool:setPoolHelper: invalid address");
        helper = _helper;
        emit PoolHelperChanged(address(_helper));
    }
    ...
    event PoolHelperChanged(address indexed _addr);
}
...
interface IPoolHelper {
    function zap(uint256 _ftmIn, uint256 _minFantasmOut) external;
}
