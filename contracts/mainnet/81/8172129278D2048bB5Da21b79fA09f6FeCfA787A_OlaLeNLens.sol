...
//        uint priceScale = 10 ** (36 - underlyingDecimals);
    ...
    struct OMarketBalances {
        ...
        // OToken balance
        uint balanceOf;
        // Borrow balance in underlying units
        uint borrowBalanceCurrent;
        // The oToken balance in terms of underlying units
        uint balanceOfInUnderlying;
    ...
    function oMarketBalances(OTokenForOlaLenLensInterface market, address payable account) public returns (OMarketBalances memory) {
        uint balanceOf = market.balanceOf(account);
        ...
        return OMarketBalances({
            oToken: address(market),
            balanceOf: balanceOf,
            borrowBalanceCurrent: borrowBalanceCurrent,
            balanceOfInUnderlying: balanceOfInUnderlying,
            accountUnderlyingBalance: accountUnderlyingBalance,
            marketAllowance: marketAllowance
        });
    }

    function oMarketBalancesAll(OTokenForOlaLenLensInterface[] calldata oMarkets, address payable account) external returns (OMarketBalances[] memory) {
        ...
        for (uint i = 0; i < oMarketsCount; i++) {
            res[i] = oMarketBalances(oMarkets[i], account);
        }
        return res;
    }

    function oMarketBalancesInLeN(ComptrollerOlaLeNLensInterface unitroller, address payable account) external returns (OMarketBalances[] memory) {
        OTokenForOlaLenLensInterface[] memory allMarkets = unitroller.getAllMarkets();

        uint oMarketsCount = allMarkets.length;
        OMarketBalances[] memory res = new OMarketBalances[](oMarketsCount);
        for (uint i = 0; i < oMarketsCount; i++) {
            res[i] = oMarketBalances(allMarkets[i], account);
        }
        return res;
    }
