...
contract SoulManifester is AccessControl, ReentrancyGuard {
    ...
    constructor(address _team, address _dao, address _soulAddress, address _seanceAddress) {
        // sets: user addresses
        team = _team;
        dao = _dao;
        ...
        // sets: `soul` & `seance`
        setTokens(_soulAddress, _seanceAddress);
    ...
    function addPool(uint _allocPoint, IERC20 _lpToken, bool _withUpdate, uint _feeDays) external obey(isis) { 
            ...
            require(toWei(_feeDays) <= maxFeeDays, 'feeDays may not exceed the maximum of 100 days');
    ...
    function setPool(
        ...
        // gets: pool data (stored for updates).
            ...
            require(pool.allocPoint != _allocPoint || pool.feeDays != toWei(_feeDays), 'no change requested.');
            
            // checks: fee is below maximum
            require(toWei(_feeDays) <= maxFeeDays, 'fee days exceeds max');
    ...
    function updateFeeDays(uint pid, uint _feeDays) external obey(maat) {
        Pools storage pool = poolInfo[pid];
        ...
        require(toWei(_feeDays) <= maxFeeDays, 'exceeds allowable feeDays');
