...
contract AceLabHelper is Ownable {
    ...
    function getUserXBOO(address _user) public view returns(uint256 amount) {
        uint256 _amt;
        for (uint256 i = 31; i < acelab.poolLength(); i++) {
            (_amt,)  = acelab.userInfo(i, _user);
            amount += _amt;
        }
    }
    
    function balanceOf(address _user) public view returns(uint256 amount) {
        uint conversionRate = XBOO.xBOOForBOO(1e18);
        amount = getUserXBOO(_user) * conversionRate / 1e18;
    }
}
