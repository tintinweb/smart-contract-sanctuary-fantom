    ...
    IERC777SenderUpgradeable
{
    ...
    bytes32 private constant BRIDGE = keccak256("BRIDGE");

    uint256 public taxRate;
    address public taxCollector;

    function initialize(address bridger) external virtual initializer {
    ...
    // ################# cBridge required functions ##################

    function mint(address _to, uint256 _amount) external virtual onlyRole(BRIDGE) {
        _mint(_to, _amount, "", "");
    }

    function burn(address _from, uint256 _amount) external virtual onlyRole(BRIDGE) {
        _burn(_from, _amount, "", "");
    }
    ...
    function setTaxCollector(address newTaxCollector) external virtual onlyRole(TAX_MANAGER) {
        require(newTaxCollector != address(0), "Tax collector cannot be zero address");
