...
abstract contract URIMaskable is Context {
    ...
    function RANDOM_SEED() public view onlyMaskService returns (uint256) {
        return _RANDOM_SEED;
    }
...
// File contracts/Buried/Buried.sol

contract BuriedSkellies is ERC721Omnichain {
    using Address for address;

    uint256 public immutable MAX_SUPPLY = 1920;
    uint256 public immutable MAX_PER_WALLET = 50;
    uint256 public TOTAL_MINTED = 0;
    string private URI_PREFIX;

    uint256 private immutable MINT_START_TIME = 1655233200;
    uint256 private BASE_STAKING_REWARD = 10 ether;
    uint256 private BASE_STAKING_TIME = 60 days;
    uint256 public immutable DRIP_RATE = BASE_STAKING_REWARD / BASE_STAKING_TIME;

    mapping(uint256 => uint256) public lastClaimTimestamp;

    constructor()
        ERC721("Buried Skellies", "BS")
        ClampedRandomizer(1920)
        LzApp(0xb6319cC6c8c27A8F5dAF0dD3DF91EA35C4720dd7)
    {
        _enableMasking();
        _setMaskingService(0xe60E6506d73C15C10DF4fA90380BBc6291152c41);
    ...
    function claimable(address owner) public view returns (uint256 amount, uint256[] memory tokenIds) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (lastClaimTimestamp[tokenIds[i]] <= block.timestamp) {
    ...
    function claimable(uint256 tokenId) public view returns (uint256) {
        if (lastClaimTimestamp[tokenId] >= block.timestamp) {
            return 0;
        }
    ...
    function mint(uint256 count) public payable returns (uint256[] memory) {
        ...
        require(count <= 10, "max mint per call exceeded");
        require(msg.value == count * MINT_PRICE(), "caller supplied wrong value");
        (bool sent, ) = payable(0xb1a9a840E7ED5b97e559aFCA416C566A6a0ad06F).call{
            value: msg.value - (count * BASE_STAKING_REWARD)
        }("");
        require(sent);
    ...
    function MINT_PRICE() public view returns (uint256) {
        if (block.timestamp >= MINT_START_TIME && block.timestamp <= MINT_START_TIME + 10 minutes) {
            if (IERC721(0x2F14c76B8793F775D37d45beE5ed6336471805c4).balanceOf(_msgSender()) != 0) {
                // cyber skellies
                return 15 ether;
            } else if (IERC721(0xBf73c78d6df63941d29593916DDf11a62B7B02f3).balanceOf(_msgSender()) != 0) {
                // champion skellies
                return 16 ether;
            } else if (
                IERC721(0x5fFA60EF5790dC4c01113D24449fF445D97f22db).balanceOf(_msgSender()) != 0 || // mutants
                IERC721(0x11020CA3c1547a3332A2386dBF0a6d63dd1173Ee).balanceOf(_msgSender()) != 0 || // sharks
                IERC721(0x0DC238932032dE4ac7de445b533Be2a205892984).balanceOf(_msgSender()) != 0 || // OG skellies
                IERC721(0xf1EdD11dF477d5d3BF335C8b1b1B15177cb405b2).balanceOf(_msgSender()) != 0 || // punks v1
                IERC721(0xE84EbAD6fc852Ba2bE5A6d737CdFf6e4B44C73A7).balanceOf(_msgSender()) != 0 // punks v2
            ) {
                return 17 ether;
            }
        }
    ...
    function stake(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(lastClaimTimestamp[tokenIds[i]] != type(uint256).max, "token already staked");
            lastClaimTimestamp[tokenIds[i]] = block.timestamp;
        }
    }
    ...
    function tokenMask(uint256 tokenId) public view onlyMaskService returns (string memory) {
        return _generateURIMask(tokenId);
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: tokenID does not exist");
        return string(abi.encodePacked(URI_PREFIX, _generateURIMask(tokenId), ".json"));
    }
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(ownerOf(tokenIds[i]) == _msgSender(), "not the owner of token");
            require(lastClaimTimestamp[tokenIds[i]] != type(uint256).max, "token not staked");
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            require(lastClaimTimestamp[tokenId] == type(uint256).max, "token is currently staked");
        }
    ...
    function _mintNFT(address to, uint256 count) internal returns (uint256[] memory tokenIds) {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            lastClaimTimestamp[tokenIds[i]] = (block.timestamp > MINT_START_TIME) ? block.timestamp : MINT_START_TIME;
        }
    }
