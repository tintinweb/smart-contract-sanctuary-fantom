...
contract BabyCreatureBreedingFacade is _BabyCreatureStorage {
        ...
        )
    {
        ...
        wolfCurrencies = _getDataFor(_wolfCurrencies);
        sheepCurrencies = _getDataFor(_sheepCurrencies);
    }
    ...
    function _getDataFor(uint256[] memory currencies)
        private
        view
        returns (uint256[3][] memory data)
    {
        data = new uint256[3][](currencies.length);
        for (uint256 i = 0; i < currencies.length; i++) {
            (, uint256 price, address erc20, ) = _babyCreatureNew.getCurrency(
                currencies[i]
            );
