...
import {BeetsMaximizer} from "../BeetsMaximizer.sol";

contract StrategyBeets_MelodyFromHell is BeetsMaximizer {
    address private constant BEETS = 0xF24Bcf4d1e507740041C9cFd2DddB29585aDCe1e;
    address private constant bfBEETS = 0x3abeC3aEA43808C889C622FeA0064FEEBa66bAe3;
    address private constant MELODY_FROM_HELL = 0x8C3C964C2d08679d3D09866CF62C5b14a5346479;
    ...
    function initializeStrategy(
        ...
        __Strategy_init(
            ...
            rewards,
            54,
            LiquidationConfig(BEETS, 0xcde5a11a4acb4ee4c805352cec57e236bdbc3837000200000000000000000019, 1, 2),
            bfBEETS
        );
    }
}
...
import {ITokenBar} from "../../interfaces/protocols/ITokenBar.sol";
...
import {BaseStrategy, Errors, _require} from "../BaseStrategy.sol";
import {IMasterChef} from "./interfaces/IMasterChef.sol";
import "hardhat/console.sol";

/// @title BeethovenX Maximizer Strategy 
/// @author Chainvisions & jlontele 
/// @notice A maximizer strategy for BeethovenX LP farm pools.

contract BeetsMaximizer is BaseStrategy {
    ...
    /// @notice BeethovenX's fBEETS contract.
    ITokenBar public constant FRESH_BEETS = ITokenBar(0xfcef8a994209d6916EB2C86cDD2AFD60Aa6F54b1);

    /// @notice Bytes32 values stored by the strategy.
    mapping(bytes32 => bytes32) public bytes32Storage;

    /// @notice Assets of the liquidity pool to liquidate into.
    address[] public poolAssets;

    /// @notice Initializes the strategy contract.
    /// @param _storage Beluga storage contract for access control.
    /// @param _underlying Underlying token of the strategy for investing.
    /// @param _vault The vault that owns the strategy contract.
    /// @param _stakingContract BeethovenX's MasterChef contract for farming BEETS.
    /// @param _rewards Reward tokens rewarded by the MasterChef.
    /// @param _pid Pool ID to stake into on the MasterChef.
    /// @param _liquidationConfig Configuration for reward liquidations on the contract.
    /// @param _targetVault Target vault to deposit yield into.
    ...
    public initializer {
        ...
        address bvault = IBalancerPool(underlying()).getVault();
    ...
    function doHardWork() external onlyNotPausedInvesting restricted {
        ...
        console.log(IERC20(_rewardTokens[0]).balanceOf(address(this)));
        _liquidateReward();
        _mintFreshBeets();
    ...
    function withdrawToVault(uint256 amount) public restricted {
        ...
        if(amount > _underlying.balanceOf(address(this))){
            // While we have the check above, we still using SafeMath below
            // for the peace of mind (in case something gets changed in between)
            uint256 needToWithdraw = (amount - _underlying.balanceOf(address(this)));
            IMasterChef(rewardPool()).withdrawAndHarvest(poolId(), Math.min(_rewardPoolStake(), needToWithdraw), address(this));
        }

        _underlying.safeTransfer(vault(), amount);
    }
    ...
    function _swapRewardToDeposit(IERC20 _rewardToken) internal {
        ...
        singleSwap.assetIn = IAsset(address(_rewardToken));
        ...
        _rewardToken.safeApprove(bVault(), 0);
        _rewardToken.safeApprove(bVault(), rewardBalance);
    ...
    */ 
    ...
    function _handleLiquidation(uint256[] memory _balances) internal override {
        ...
        for(uint256 i = 0; i < rewards.length; i++) {
            ...
            if(reward == address(depositToken())) {
                _depositLP();
            } else {
                _swapRewardToDeposit(IERC20(reward));
                _depositLP();
            }
        }
    }
    ...
    function _notifyMaximizerRewards() internal {
        IVault _targetVault = IVault(targetVault());
        ...
        if(targetUnderlyingBalance > 0) {
            ...
            IERC20(targetVaultUnderlying).safeApprove(address(_targetVault), 0);
            IERC20(targetVaultUnderlying).safeApprove(address(_targetVault), targetUnderlyingBalance);
            _targetVault.deposit(targetUnderlyingBalance);

            // Notify the rewards on the vault.
            uint256 targetVaultBalance = IERC20(address(_targetVault)).balanceOf(address(this));
            IERC20(address(_targetVault)).safeTransfer(vault(), targetVaultBalance);
            IVault(vault()).notifyRewardAmount(address(_targetVault), targetVaultBalance);
        }
    }

    function _mintFreshBeets() internal {
        uint256 duettoBalance = DUETTO_POOL.balanceOf(address(this));
        if(duettoBalance > 0) {
            DUETTO_POOL.safeApprove(address(FRESH_BEETS), 0);
            DUETTO_POOL.safeApprove(address(FRESH_BEETS), duettoBalance);
            FRESH_BEETS.enter(duettoBalance);
        }
    }
