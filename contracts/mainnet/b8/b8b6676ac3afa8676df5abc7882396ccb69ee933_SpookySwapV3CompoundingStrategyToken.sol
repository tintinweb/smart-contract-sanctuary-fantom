...
// File @openzeppelin/contracts/utils/[email protected].4.2
...
interface MasterChefBooV1
{
	function boo() external view returns (address _boo);
	function poolLength() external view returns (uint256 _poolLength);
	function poolInfo(uint256 _pid) external view returns (address _lpToken, uint256 _allocPoint, uint256 _lastRewardBlock, uint256 _accBOOPerShare);
...
interface AceLabxBoo
{
	function xboo() external view returns (address _xboo);
	function poolLength() external view returns (uint256 _poolLength);
	function poolInfo(uint256 _pid) external view returns (address _rewardToken, uint256 _rewardPerSecond, uint256 _tokenPrecision, uint256 _xBooStakedAmount, uint256 _lastRewardTime, uint256 _accRewardPerShare, uint256 _endTime, uint256 _startTime, uint256 _userLimitEndTime, address _protocolOwnerAddress);
...
interface BooMirrorWorld
{
	function boo() external view returns (address _boo);
	function BOOBalance(address _account) external view returns (uint256 _amount);
	function BOOForxBOO(uint256 _amount) external view returns (uint256 _shares);
...
/**
 * @notice This contract implements a compounding strategy for SpookSwap AceLab.
 *         It basically deposits and withdraws xBOO from AceLab and collects the
 *         underlying reward token. The compounding happens by calling the gulp function;
 *         it converts the reward into more funds which are further deposited into
 *         AceLab. A performance fee is deducted from the converted funds and sent
 *         to the fee collector contract.
 */
contract SpookySwapV3CompoundingStrategyToken is ERC20, Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	address private immutable aceLab;
	uint256 private pid;

	// strategy token configuration
	address private rewardToken;
	address private routingToken;
	 ...
	 * @param _aceLab The aceLab contract address.
	 * @param _pid The AceLab Pool ID (pid).
	...
	constructor(string memory _name, string memory _symbol, uint8 _decimals,
		address _aceLab, uint256 _pid, address _routingToken,
		address _treasury, address _collector, address _exchange)
		ERC20(_name, _symbol)
	{
		decimals_ = _decimals;
		(address _reserveToken, address _rewardToken) = _getTokens(_aceLab, _pid);
		require(_routingToken == _reserveToken || _routingToken == BooMirrorWorld(_reserveToken).boo(), "invalid token");
		aceLab = _aceLab;
		pid = _pid;
	...
	function _gulp() internal returns (bool _success)
	{
		...
		if (routingToken != reserveToken) {
			...
			BooMirrorWorld(reserveToken).enter(_totalRouting);
		}
	...
	function updatePidAndSwitchPools(uint256 _newPid) external onlyOwner nonReentrant //delayed
	{
		require(!emergencyMode, "not allowed");
		(address _reserveToken, address _rewardToken) = _getTokens(aceLab, _newPid);
		require(_reserveToken == reserveToken, "incompatible pool");
		address _routingToken = _rewardToken == _reserveToken ? _reserveToken : BooMirrorWorld(_reserveToken).boo();
		require(_gulp(), "unavailable");
		uint256 _oldPid = pid;
		{
		...
		emit UpdatePidAndSwitchPools(_oldPid, _newPid);
	}
	...
	/// @dev Lists the reserve and reward tokens of the AceLab pool
	function _getTokens(address _aceLab, uint256 _pid) internal view returns (address _reserveToken, address _rewardToken)
	{
		uint256 _poolLength = AceLabxBoo(_aceLab).poolLength();
		require(_pid < _poolLength, "invalid pid");
		_reserveToken = AceLabxBoo(_aceLab).xboo();
		(_rewardToken,,,,,,,,,) = AceLabxBoo(_aceLab).poolInfo(_pid);
		return (_reserveToken, _rewardToken);
	}

	/// @dev Retrieves the current pending reward for the AceLab pool
	function _getPendingReward() internal view returns (uint256 _pendingReward)
	{
		return AceLabxBoo(aceLab).pendingReward(pid, address(this));
	}

	/// @dev Retrieves the deposited reserve for the AceLab pool
	function _getReserveAmount() internal view returns (uint256 _reserveAmount)
	{
		(_reserveAmount,) = AceLabxBoo(aceLab).userInfo(pid, address(this));
		return _reserveAmount;
	}

	/// @dev Performs a deposit into the AceLab pool
	function _deposit(uint256 _amount) internal
	{
		Transfers._approveFunds(reserveToken, aceLab, _amount);
		AceLabxBoo(aceLab).deposit(pid, _amount);
	}

	/// @dev Harvests the AceLab pool
	function _harvest() internal
	{
		AceLabxBoo(aceLab).withdraw(pid, 0);
	}

	/// @dev Performs an withdrawal from the AceLab pool
	function _withdraw(uint256 _amount) internal
	{
		AceLabxBoo(aceLab).withdraw(pid, _amount);
	}

	/// @dev Performs an emergency withdrawal from the AceLab pool
	function _emergencyWithdraw() internal
	{
		AceLabxBoo(aceLab).emergencyWithdraw(pid);
	}

	// ----- END: underlying contract abstraction

	// events emitted by this contract
	event UpdatePidAndSwitchPools(uint256 _oldPid, uint256 _newPid);
