...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant dai = address(0x8D11eC38a3EB5E956B052f67Da8Bdc9bef8Abf3E);
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR DAI-FTM EQUALIZER")),
            string(abi.encodePacked("XpE-DAI-FTM"))
        )
        {
        gauge = _gauge;
        router = _router;
    ...
    function _addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmt, wftm, dai, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(wftm, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(dai, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(wftm, dai, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(wftm, dai, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyOwner {
        if(_token == equal || _token == wftm || _token == dai){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(dai, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(dai, router, type(uint).max);
    }
