...
// MasterChef is the master of SUN. He can make Sun and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once SUN is sufficiently
...
contract Masterchef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of SUN
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accSunPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSunPerShare` (and `lastRewardSecond`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardSecond;  // Last second that SUNs distribution occurs.
        uint256 accSunPerShare;   // Accumulated SUNs per share, times 1e18. See below.
    ...
    // SUN tokens created per second.
    uint256 public sunPerSecond;
    ...
    // The block timestamp when SUN farming starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 sunPerSecond);
    ...
    event SunMintError(bytes reason);
    ...
    constructor (
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _sunPerSecond,
        uint256 _startTime
    ) public {
        sun = _sun;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        sunPerSecond = _sunPerSecond;
    ...
    function addInitialPools(IBEP20 _SUNwFTMPair, IBEP20 _SUNUSDCPair) external onlyOwner{
        require(poolInfo.length == 0, "addInitialPools: Initial Pools has already been added");
        add(40000, IBEP20(_SUNUSDCPair), 0, 28800, false); // SUN-USDC 0
        add(40000, IBEP20(_SUNwFTMPair), 0, 28800, false); // SUN-wFTM 1    
        add(5000, sun, 0, 28800, false); // SUN solo staking 2
    ...
    // Update the given pool's SUN allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending Suns on frontend.
    function pendingSun(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardSecond && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            uint256 sunReward = multiplier.mul(sunPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            if (sun.totalSupply().add(sunReward) > sun.maxSupply()) {
                sunReward = sun.maxSupply() - sun.totalSupply();
            }
            accSunPerShare = accSunPerShare.add(sunReward.mul(1e18).div(pool.lpSupply));
        }
        uint256 pending = user.amount.mul(accSunPerShare).div(1e18).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardSecond, block.timestamp);
        uint256 sunReward = multiplier.mul(sunPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 devReward = sunReward/10;
        uint256 totalRewards = sun.totalSupply() + devReward + sunReward;

        if (totalRewards <= sun.maxSupply()) {
            // mint dev reward as normal as not at maxSupply
            sun.mint(devaddr, devReward);
        } else {
            // update sunReward to difference
            sunReward = sun.maxSupply() - sun.totalSupply();
        }

        if (sunReward != 0) {
            // only mint to MC and calculate and update accSunPerShare if sunReward is non 0
            sun.mint(address(this), sunReward);
            pool.accSunPerShare = pool.accSunPerShare + (sunReward * 1e18 / pool.lpSupply);
        }
        pool.lastRewardSecond = block.timestamp;
    }
    
    // Deposit LP tokens to MasterChef for SUN allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accSunPerShare).div(1e18);
    ...
    function payOrLockupPending(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accSunPerShare).div(1e18).sub(user.rewardDebt);
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                safeSunTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    // Safe test transfer function, just in case if rounding error causes pool to not have enough SUNs.
    function safeSunTransfer(address _to, uint256 _amount) internal {
        uint256 sunBal = sun.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > sunBal) {
            transferSuccess = sun.transfer(_to, sunBal);
        } else {
            transferSuccess = sun.transfer(_to, _amount);
        }
        require(transferSuccess, "safeSunTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _sunPerSecond) external onlyOwner {
        require(_sunPerSecond <= MAX_EMISSION_RATE, "Emmision Rate must no exceed max emission rate");
        massUpdatePools();
        sunPerSecond = _sunPerSecond;
        emit UpdateEmissionRate(msg.sender, _sunPerSecond);
    }
