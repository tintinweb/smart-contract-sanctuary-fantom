...
contract Granary
{
	...
	//string public id = "4803";
	function id() public pure returns(string memory){return("4803");}
	function name() public pure returns(string memory){return("ftm.guru/GRAIN/4803");}//string(abi.encodePacked("ftm.guru/GRAIN/", id)));}
	function symbol() public pure returns(string memory){return("GRAIN#4803");}//string(abi.encodePacked("GRAIN#", id)));}
	...
	*/
	...
	uint256[7] public allnums = [
		...
		1e8,	//mw		4       config, default 1 (near zero)
	...
	function work(address ben) internal
	{
		...
		//Deposit 50% of WFTM into yvWFTM: $0
		{
            IVault(alladdr[2]).deposit((IERC20(alladdr[5]).balanceOf(address(this))).div(2));
        }


		//Buy bb-yv-FTM with 100% of remaining WFTM (50%) at B-x: $0.125
        {
            FundManagement memory FM_F2bb = FundManagement({
                sender:             address(this),
                fromInternalBalance:false,
                recipient:          payable(address(this)),
                toInternalBalance:  false
            });
            SingleSwap memory SS_F2bb = SingleSwap({
                poolid: 	BPT[2],
                kind:	    SwapKind.GIVEN_IN,
                assetIn:    alladdr[5],
                assetOut:   alladdr[9],
                amount:     IERC20(alladdr[5]).balanceOf(address(this)),
                userData:   abi.encode(0)
            });
            IBX(alladdr[1]).swap(SS_F2bb, FM_F2bb, 1, block.timestamp);
        }


		//Buy bb-yv-FTM with 100% of yvWFTM at B-x: $0.125
        {
            FundManagement memory FM_y2bb = FundManagement({
                sender:             address(this),
                fromInternalBalance:false,
                recipient:          payable(address(this)),
                toInternalBalance:  false
            });
            SingleSwap memory SS_y2bb = SingleSwap({
                poolid: 	BPT[2],
                kind:	    SwapKind.GIVEN_IN,
                assetIn:    alladdr[2],
                assetOut:   alladdr[9],
                amount:     IERC20(alladdr[2]).balanceOf(address(this)),
                userData:   abi.encode(0)
            });
            IBX(alladdr[1]).swap(SS_y2bb, FM_y2bb, 1, block.timestamp);
        }


		//Buy PANIC with 50% of bb-yv-FTM at B-x: $0.125
        {
            FundManagement memory FM_bb2P = FundManagement({
                sender:             address(this),
                fromInternalBalance:false,
                recipient:          payable(address(this)),
                toInternalBalance:  false
            });
            SingleSwap memory SS_bb2P = SingleSwap({
                poolid: 	BPT[0],
                kind:	    SwapKind.GIVEN_IN,
                assetIn:    alladdr[9],
                assetOut:   alladdr[4],
                amount:     (IERC20(alladdr[9]).balanceOf(address(this))).div(2),
                userData:   abi.encode(0)
            });
            IBX(alladdr[1]).swap(SS_bb2P, FM_bb2P, 1, block.timestamp);
        }
        ...
        T_n[0] = IAsset(alladdr[4]);
        T_n[1] = IAsset(alladdr[9]);
		uint256[] memory A_n = new uint256[](2);
        A_n[0] = IERC20(alladdr[4]).balanceOf(address(this));
        A_n[1] = IERC20(alladdr[9]).balanceOf(address(this));
	...
	function tvl() public view returns(uint256)
	{
        ...
        uint256 bpt_price = (p_price * p_poold * 2) / bpt_ts;
