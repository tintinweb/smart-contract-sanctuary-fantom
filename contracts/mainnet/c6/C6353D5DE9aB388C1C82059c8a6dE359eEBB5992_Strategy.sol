...
contract Strategy is BaseStrategy {
     ...
     * lpToken0 (USDC), want token and the deposited one from the vault.
     * lpToken1 (fUSDT) - lp Token pair for the want.
     */
     ...
     * router - the router for SpookySwap DEX
     * rewardToken - Token generated by staking our funds
     * masterChef - Spooky masterChef contract
     * masterChefPoolId - Spooky masterChef pool id
     */
    ...
    /**
     * @dev Routes we take to swap tokens using SpookySwap.
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('ss Spooky USDC-MAI Curve strategy'));
    }
     ...
     *  Calculate how many lpToken1 a lp share represents,
     *	then extrapolate to get how many lpToken1 we have on the pool.
     * 	Do the same for want token.
     *
     * 			totalSupply   -->   reservesB (MAI)
     * 			ourLPShares   -->   x lpToken1 amount (MAI)
     */
     ...
     * 		1- Get USDC-MAI LP Tokens
     *			1.1- Swap 50% of the USDC for MAI tokens
     * 			1.2- Deposit USDC-MAI to Spooky Liquidity Pool
     * 		2- Deposit USDC-MAI-LP into Spooky masterChef
     */
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (amountIn > wantDust) {
            // Get LP tokens by swapping want for Spooky
            getLpTokens(amountIn);
            // Wanted invested on
            uint256 investedWant = amountIn.sub(balanceOfWant());
            // Deposit all LP tokens in Spooky MasterChef
            depositLpIntoMasterChef();
            // Check that slippage is between the set limits
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        if (estimatedTotalAssets() < _amountNeeded) {
            ...
            // TODO: Add check in case of positive slippage
        ...
        if (_amountNeeded > looseAmount) {
            ...
            // Remove liquidity to get USDC + MAI tokens
            removeLiquidity(toExitAmount);
            // Sell all MAI tokens that are on the strategy
            sellLpToken1();
    ...
    function liquidateAllPositions() internal override returns (uint256 _liquidated) {
        ...
        withdrawFromMasterChef();
        // Remove liquidity from SpookySwap to get wants + MAI tokens
        removeAllLiquidity();
        // Sell all lpToken1 (MAI) tokens for want tokens
        sellLpToken1();
    ...
    /**
     * @notice
     *  Swap 50% of the USDC for MAI tokens
     */
    ...
    /**
     * @notice
     *  1- Get USDC-MAI ratio.
     *  2- Get LP price in USDC.
     *  3- Calculate LP to sell to get desired amount USDC out.
     * @dev
     *  Simple logic for the operations:
     *
     *     value in want	 --->   all Lp Tokens
     *      exit Amount    --->   X lpShares
     */
    function removeLiquidity(uint256 toExitAmount) internal {
        // Gather information from the pool and LP tokens
        uint256 lpTokenBalance = balanceOfLpTokens(); // Remove liquidity only after a withdrawFromMasterChef();
        // Calculate the worth of all our LP tokens
        uint256 lpWorth = balanceOfPooled();
        // Calculate the fraction of the want we have to remove from the pool
        uint256 percentageOfLpToSell = toExitAmount.mul(1e18).div(lpWorth);
        // Calculate how much this exit percentage represents in lp Tokens
        uint256 liquidityToExit = lpTokenBalance.mul(percentageOfLpToSell).div(1e18);
        // Make sure we do not remove more Lp than our Lp holdings
        uint256 toExit = Math.min(liquidityToExit, balanceOfLpTokens());
        // TODO: min liquidity to exit.
        // TODO: remove the 1 and add a check to make sure we are removing something.
        if (toExit > 0) {
    ...
    /**
     * @notice
     *  Claim all BOO rewards from Spooky masterChef.
     * @dev
     *  When doing a deposit we claim the rewards even if the deposit is 0.
     */
