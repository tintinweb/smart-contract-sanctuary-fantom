...
contract ChillLab is ChillPillRarity, ReentrancyGuard {
    ...
    struct PoolInfo { //full slot = 32B
        ...
        uint sChillStakedAmount;        //32B # of schill allopilled to this pool
        uint mpStakedAmount;          //32B # of mp allopilled to this pool
    ...
    event StakingFeePercentage(uint newStakinFeePercent);

    constructor(IERC20 _chill,IERC20 _schill, IERC721 _chillpill) {
        chill = _chill;
        schill = _schill;
        chillpill = _chillpill;
        isRewardToken[address(_schill)] = true;
    }
 ...
 function getBoostedMultiplier(
        uint _from,
        uint _to,
        uint startTime,
        uint endTime
    ) public pure returns (uint) {
        uint multiplier = getMultiplier(_from, _to, startTime, endTime);
        if (multiplier > 14 days) {
            return multiplier + ((multiplier * 2) / 5);
        } else if (multiplier > 7 days) {
    ...
    function getMultiplier(uint _from, uint _to, uint startTime, uint endTime) public pure returns (uint) {
    ...
    function pendingRewards(uint _pid, address _user) public view returns (uint schillReward, uint chillpillReward) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint accRewardPerShare = pool.accRewardPerShare;

        if (block.timestamp > pool.lastRewardTime) {
            uint reward = pool.RewardPerSecond * getBoostedMultiplier(pool.lastRewardTime, block.timestamp, pool.startTime, pool.endTime);
            if(pool.sChillStakedAmount != 0) accRewardPerShare += reward * precisionOf[pool.TokenPrecision] / pool.sChillStakedAmount;
        }
        schillReward = (user.amount * accRewardPerShare / precisionOf[pool.TokenPrecision]) - user.rewardDebt;
        chillpillReward = (effectiveMP(user.amount, user.mp) * accRewardPerShare / precisionOf[pool.TokenPrecision]) - user.pillDebt;
    }
    ...
    function updatePool(uint _pid) public {
        ...
        if(pool.sChillStakedAmount != 0) pool.accRewardPerShare += reward * precisionOf[pool.TokenPrecision] / pool.sChillStakedAmount;
        pool.lastRewardTime = uint32(block.timestamp);
    }

    function userCurrentStakeableMP(uint _pid, address _user) public view returns (uint) {
        return effectiveMP(userInfo[_pid][_user].amount,userInfo[_pid][_user].mp);
    }

    function effectiveMP(uint _amount, uint _mp) public pure returns (uint) {
        return (_amount * (_mp))/BASIS_POINT_DIVISOR;
    }
    ...
    function takeFees(uint256 _amount) internal returns (uint256) {
        if (stakingFeePercent == 0||_amount == 0) {
            return _amount;
        }
    ...
    // Deposit tokens.
    function _deposit(uint _pid, uint _amount_before_fees, address to, uint[] memory tokenIDs) internal {
        ...
        uint pendingPill = effectiveMP(amount, user.mp) * pool.accRewardPerShare / precision - user.pillDebt;
        ...
        if(len == 0) {
            user.pillDebt = effectiveMP(amount, user.mp) * pool.accRewardPerShare / precision;
            return;
        }
        require(len+_stakedChillpills[_pid][to].length() < 3,"max 2 pill boost");
    ...
    function collectChillFees() external onlyAdmin {
        uint bal = chill.balanceOf(address(this));
        chill.safeTransfer(address(msg.sender), bal);

        emit AdminTokenRecovery(address(chill), bal);
    }
    ...
    function _add(uint _rewardPerSecond, IERC20Ext _Token, uint32 _startTime, uint32 _endTime, address _protocolOwner) internal {
        require(_startTime >= block.timestamp, "AceLab _add: _startTime needs to be in the furture");
        require(_endTime >= _startTime, "AceLab _add: _endTime needs to be after starttime");
        require(_rewardPerSecond > 9, "AceLab _add: _rewardPerSecond needs to be at least 10 wei");

        checkForToken(_Token); // ensure you cant add duplipille pools
        ...
        poolMaxTVL[poolAmount]=25*10**30;
        poolAmount += 1;
    }
    ...
    function updateStakingFees(uint newStakinFeePercent) external onlyAdmin {
