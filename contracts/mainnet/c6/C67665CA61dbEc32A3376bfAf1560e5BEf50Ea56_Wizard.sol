...
contract Wizard is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the WIZARD ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Wizard Finance", "WIZARD") {
        // Mints 1 WIZARD to contract creator for initial pool setup
    ...
    function _getWizardPrice() internal view returns (uint256 _wizardPrice) {
        try IOracle(wizardOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Wizard: failed to fetch WIZARD price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _wizardPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_wizardPrice >= taxTiersTwaps[tierId]) {
    ...
    function setWizardOracle(address _wizardOracle) public onlyOperatorOrTaxOffice {
        require(_wizardOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints WIZARD to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of WIZARD to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentWizardPrice = _getWizardPrice();
            currentTaxRate = _updateTaxRate(currentWizardPrice);
            if (currentWizardPrice < burnThreshold) {
                burnTax = true;
            }
        }
