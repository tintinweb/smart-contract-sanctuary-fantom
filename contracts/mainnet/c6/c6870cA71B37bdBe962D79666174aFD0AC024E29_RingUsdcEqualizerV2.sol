...
// File contracts/RingUsdcEqualizerV2.sol

// SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract RingUsdcEqualizerV2 is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToRingPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToRingPath,
        ...
        for (uint i; i < _equalToRingPath.length; ++i) {
            equalToRingPath.push(_equalToRingPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalBal, 0, equalToRingPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(usdc).balanceOf(address(this));
        uint256 t2Bal = IERC20(ring).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(usdc, ring, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(ring).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(ring).safeApprove(router, type(uint).max);
    }
