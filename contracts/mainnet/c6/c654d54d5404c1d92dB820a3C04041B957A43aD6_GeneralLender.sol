...
contract GeneralLender is BoringOwnable {
    ...
    //todo: implement totalBorrowCap & totalUserBorrowCap
    ...
    function borrow(
        ...
        require(
            userBorrow[msg.sender] <= manager.getCap(msg.sender),
    ...
    function liquidate(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            // getDebt(user) >
            // userCollateral[user].mul(oracle.getPrice()).mul(LIQUIDATION_RATIO) / (LIQUIDATION_RATIO_PRECISION * 1e18)
            if (!isSolvent(user)) {
