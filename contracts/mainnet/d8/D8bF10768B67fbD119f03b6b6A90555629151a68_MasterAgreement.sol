...
contract MasterAgreement is AccessControl {
    ...
    mapping(address => uint256) private _activePositionsLength;
    mapping(address => uint256) private _quotesLength;
    mapping(address => uint256) private _balances;
    mapping(address => uint256) private _lockedCVA;
    mapping(address => uint256) private _pendingLockedCVA;
    mapping(address => uint256) private _lockedIM;
    mapping(address => uint256) private _pendingLockedIM;
    ...
    function getPendingLockedCVA(address owner)
        external
        view
        returns (uint256)
    {
        return _pendingLockedCVA[owner];
    }
    ...
    function getPendingLockedIM(address owner) external view returns (uint256) {
        return _pendingLockedIM[owner];
    }
    ...
    function getActivePositionsLength(address user)
        external
        view
        returns (uint256)
    {
        return _activePositionsLength[user];
    }
    ...
    function getPositions(
        ...
        uint256 count = 0;
        for (uint256 i = from; i < to; i++) {
            if (_quotes[user][i].isActive == isActive) {
                count++;
            }
        }
    ...
    function sendQuote(
        ...
        _pendingLockedCVA[msg.sender] += partyACVA;
        _pendingLockedIM[msg.sender] += partyAIM;
    ...
    function openPosition(
        ...
        external
        // onlyRole(PARTY_B_ROLE) // todo uncomment this for production
        notLiquidatedUser(partyA)
    {
        ...
        // todo: validate limit price and filled price difference
        ...
        _pendingLockedCVA[partyA] -= quote.partyACVA;
        _pendingLockedIM[partyA] -= quote.partyAIM;
        _lockedCVA[partyA] += quote.partyACVA;
        _lockedIM[partyA] += quote.partyAIM;

        _activePositionsLength[partyA]++;
        _activePositionsLength[msg.sender]++;
    ...
    /// @dev WARNING: USE THIS FUNCTION CAREFULLY
    function _closePosition(
        ...
        _activePositionsLength[partyA]--;
        _activePositionsLength[quote.partyB]--;
    ...
    function performClosePosition(
        ...
        // todo: check solvencySignatures
        // todo: check priceSignatures with limitPrice
        Quote storage quote = _quotes[partyA][qid];
        require(
            quote.quoteState == QuoteState.CLOSE_PENDING,
            "MasterAgreement: INVALID_STATE"
    ...
    function liquidatePositions(
        ...
        if (_activePositionsLength[partyA] == 0) {
