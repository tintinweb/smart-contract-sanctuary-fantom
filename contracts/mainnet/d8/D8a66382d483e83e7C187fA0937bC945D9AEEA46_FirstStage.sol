...
contract FirstStage is ERC721, ERC721Enumerable, Pausable, ERC721Burnable, ReentrancyGuard, Ownable {
    ...
    function buyToken(uint8 tokenId) external payable nonReentrant whenNotPaused callerIsUser {
        if (whiteListedOnly) {
            require(isWhitelisted[msg.sender] || ownWhitelistToken(msg.sender), "Error: you are not whitelisted");
        }
    ...
    function setTokenPrice(uint8 tokenId, uint256 price) public onlyOwner {
    ...
    function addWhitelistedContracts(address[] calldata _contracts) external onlyOwner {
        require(_contracts.length > 0, "Error: invalid contract list");
