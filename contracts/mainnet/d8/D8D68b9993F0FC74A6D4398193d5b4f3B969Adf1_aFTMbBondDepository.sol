...
contract aFTMbBondDepository is Ownable {
    ...
    address public immutable RICH; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints RICH when receives principle
    ...
    struct Bond {
        uint256 payout; // RICH remaining to be paid
    ...
    function deposit(
        ...
        // must be > 0.0001 RICH ( underflow protection )
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(RICH).transfer(_recipient, _amount);
            // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(RICH).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(RICH).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(RICH).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function assetPrice() public view returns (uint256 result) {
        (uint256 reserve0, uint256 reserve1, ) = IPancakePair(lp)
            .getReserves();
        result = usdc == IPancakePair(lp).token0()
    ...
    /**
     *  @notice calculate current ratio of debt to RICH supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(RICH).totalSupply();
    ...
    /**
     *  @notice calculate amount of RICH available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or RICH) to the DAO
     *  @return bool
     */
