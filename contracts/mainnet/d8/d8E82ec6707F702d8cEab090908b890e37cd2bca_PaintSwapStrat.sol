/**
 *Submitted for verification at FtmScan.com on 2022-10-29
*/

// File: contracts/interfaces/IPaintSwapDecorator.sol
...
// File: contracts/PaintSwapStrat.sol
...
contract PaintSwapStrat is Ownable, Pausable {
    ...
    function _deposit() internal whenNotPaused {
        ...
        IPaintSwapDecorator(masterchef).deposit(poolId, wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IPaintSwapDecorator(masterchef).withdraw(poolId, _amount - wantBal);
    ...
    function harvest() external {
        require(msg.sender == owner() || msg.sender == strategist || msg.sender == harvester, "!auth");
    ...
    function _harvest(address caller) internal whenNotPaused {
        if (caller != vault){
         ...
         IPaintSwapDecorator(masterchef).deposit(poolId, 0);
    ...
    function chargeFees(address caller) internal {                   
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWrapped, 0, outputToWrappedRoute, address(this), block.timestamp);                                                
    ...
    /** @dev Converts to both sides of the LP token and builds the liquidity pair */
    function addLiquidity() internal {
        uint256 wrappedHalf = IERC20(output).balanceOf(address(this)) / 2;

        IUniSwapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wrappedHalf, 0, outputToWrappedRoute, address(this), block.timestamp);
   

        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(output).balanceOf(address(this));

        IUniSwapRouter(unirouter).addLiquidity(wrapped, output, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }

     /** @dev Returns rewards unharvested */
    function rewardBalance() public view returns (uint256) {
        return IPaintSwapDecorator(masterchef).pendingBrush(poolId, address(this));
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IPaintSwapDecorator(masterchef).userInfo(poolId, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IPaintSwapDecorator(masterchef).emergencyWithdraw(poolId);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external onlyAdmin {
        require (_path[0] != wrapped && _path[_path.length - 1] == output, "Bad path");
