...
/*
 * Errors table:
 * ERR1: insufficient matic provided!
 * ERR2: wanted token address can't be the zero address!
 * ERR3: approval failed
 * ERR4: updateSandManSwapRouter: new _router is the zero address
 * ERR5: router already set!
 * ERR6:
 */
contract TreasuryUtil is ReentrancyGuard, AccessControl {
    ...
    event SetTreasuryDAOAddress(address treasuryDAO);

    /**
     * @notice Constructs the SandManToken contract.
     */
    constructor(address _treasuryDAO, address _usdCurrency, address _sandManSwapRouter) {
    ...
    function _swapTokensForEth(address saleTokenAddress, uint256 tokenAmount) internal {
        ...
        IERC20(saleTokenAddress).approve(address(sandManSwapRouter), tokenAmount);
    ...
    function _swapETHForTokens(uint256 ethAmount, address wantedTokenAddress) internal {
        require(address(this).balance >= ethAmount, "ERR1");
        require(wantedTokenAddress != address(0), "ERR2");
    ...
    function convertDepositFeesToUSD(address token, uint256 tokenType) public onlyRole(OPERATOR_ROLE) {
        ...
        if (tokenType == 1) {
            ...
            // FIXME: ver si dejamos esto (creo que si)
        ...
        if (token == usdCurrency) {
            ...
            _swapETHForTokens(holdingOwnership, usdCurrency);
        } else {
            ...
            require(IERC20(token).approve(address(sandManSwapRouter), holdingOwnership), "ERR3");
        ...
        uint256 wnativeAmount = IERC20(WETH).balanceOf(address(this));
        uint256 usdAmount = IERC20(usdCurrency).balanceOf(address(this));

        if (wnativeAmount > 0)
            IERC20(WETH).safeTransfer(treasuryDAO, wnativeAmount);

        if (usdAmount > 0)
            IERC20(usdCurrency).safeTransfer(treasuryDAO, usdAmount);
    ...
    function _removeLiquidity(address token, uint256 tokenAmount) internal returns(address, address) {
        require(IERC20(token).approve(address(sandManSwapRouter), tokenAmount), "ERR3");
    ...
    function updateSandManSwapRouter(address _router) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_router != address(0), "ERR4");
        require(address(sandManSwapRouter) == address(0), "ERR5");
    ...
    function setTreasuryDAOAddress(address _treasuryDAO) external onlyRole(DEFAULT_ADMIN_ROLE) {
        treasuryDAO = _treasuryDAO;

        emit SetTreasuryDAOAddress(treasuryDAO);
    }
