...
contract AggregationTransection is Withdrawable {
    ...
    function _swapExactTokensForETH(uint amountIn, 
        ...
        address[] memory path, 
        address to, 
        uint deadline) internal onlyWhitelisted {
	...
	function _addLiquidityETH(
        ...
        uint256 deadline
			...
			(bool success, bytes memory returnData) = TOMB_FINANCE_ROUTER.call{
                value: getLiquidityETHDesired(amountTokenDesired)}(data);
			require(success == true, "call failure");
    ...
    //withdraw
	function _withdraw(uint256 _pid, uint256 _amount) internal onlyWhitelisted returns (bytes memory) {
        require(_amount <= whitelist[current_address].liquidityDeposited, "not enough liquidity! Please check for the account.");
    ...
    */
    function _getAmountIn(uint totalAmountETH) public view returns(uint amount){
        ...
        uint temp1 = totalAmountETH.div(2000).mul(995);
        temp1 = temp1 * (1 << 64);
        uint temp2 = temp1.div(_reserveWFTM);
        temp2 = temp2.add(1 << 64);
        uint temp3 = temp2.mul(1995).div(1000);
        totalAmountETH = totalAmountETH * (1 << 64);
        amount = totalAmountETH.div(temp3).mul(9999999999).div(10000000000);
    }
}