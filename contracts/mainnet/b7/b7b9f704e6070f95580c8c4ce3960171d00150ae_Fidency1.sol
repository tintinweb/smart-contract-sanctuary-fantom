...
contract Fidency1 is IERC20
{
    ...
    event FVMWithdraw(address withdrawer, uint256 amount);
     ...
     function tokensToFTM(uint tokenAmount) public view returns(uint)
    {
     ...
     function FTMToFVM(uint256 bnb_amt) public view returns(uint)
    {
         uint _rate = coinRate();
     ...
     function deposit(address _referer) public payable
   {
    ...
    function userReferral(address _addr) view external returns(uint256[9] memory team, uint256[9] memory referrals, uint256[9] memory income) {
     ...
     function redeposit() public payable{
      ...
      function fvmWithdraw(uint8 _perc) public{
        User storage user = users[msg.sender];
        ...
        if(fund.status == 0)
		{
            ...
            emit  FVMWithdraw(msg.sender,withdrawable2);
         ...
         function checkfvmWithdraw(uint8 _perc,address _addr) public view returns(uint256 totalWithdrawn,uint256 deducts,uint256 final_amount)
    {
         User storage user = users[_addr];
         require(user.totalInvestment>0, "Invalid User!");
         if(_perc == 10 || _perc == 25 || _perc == 50 || _perc == 100)
     ...
     function sellfvm(address payable buyer, uint _amount) external onlyInitiator{
