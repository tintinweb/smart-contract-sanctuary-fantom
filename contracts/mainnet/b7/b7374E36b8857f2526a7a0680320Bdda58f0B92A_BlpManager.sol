...
contract BlpManager is ReentrancyGuard, Governable, IBlpManager {
    ...
    constructor(address _vault, address _usdb, address _blp, address _shortsTracker, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdb = _usdb;
        blp = _blp;
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdb, uint256 _minBlp) external override nonReentrant returns (uint256) {
    ...
    function addLiquidityForAccount(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdb, uint256 _minBlp) external override nonReentrant returns (uint256) {
        _validateHandler();
    ...
    function getAumInUsdb(bool maximise) public override view returns (uint256) {
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdb, uint256 _minBlp) private returns (uint256) {
