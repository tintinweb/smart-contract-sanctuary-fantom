...
interface IMasterNFT {
 ...
 contract MasterNFT is IMasterNFT, Pausable, Ownable, ReentrancyGuard {
     using SafeMath for uint256;
    ...
    uint256 public maxtokenPerSecond;
    ...
    function settokenPerSecond(uint256 _tokenPerSecond) external override onlyOwner {
        require(tokenPerSecond != _tokenPerSecond && _tokenPerSecond <= maxtokenPerSecond, 'FORBIDDEN');
        massUpdatePools();
        emit SettokenPerSecond(msg.sender, _tokenPerSecond);
        tokenPerSecond = _tokenPerSecond;
    }
    ...
    function addTotalMintAmount(uint256 _pendingTotalMintAmount) external override onlyOwner {
        ...
        massUpdatePools();
        token.safeTransferFrom(msg.sender, address(this), _pendingTotalMintAmount);
        TotalMintAmount = TotalMintAmount.add(_pendingTotalMintAmount);
        emit AddTotalMintAmount(msg.sender, _pendingTotalMintAmount, TotalMintAmount);
    }
    ...
    function getTotalReward(uint256 _from, uint256 _to) public view override returns (uint256 totalReward) {
        ...
        return Math.min(TotalMintAmount.sub(mintedAmount), _to.sub(_from).mul(tokenPerSecond));
    }
    ...
    function pendingToken(address _pair, address _user) external view override returns (uint256) {
        ...
        if (block.timestamp  > pool.lastRewardTime && stakeBalance != 0) {
