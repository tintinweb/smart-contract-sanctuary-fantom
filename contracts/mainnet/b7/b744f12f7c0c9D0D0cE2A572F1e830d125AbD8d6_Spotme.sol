...
contract Spotme is Ownable {
    ...
    function changeFee(uint256 _newFee) external onlyOwner {
        require(_newFee <= 10, 'Value exceeds max fee'); 
        swapFee = _newFee;
    }
    ...
    function fulfillRequest(
        ...
        if (_path.length == 1) {
            ...
            require(_path[0] == _tokenProvided, "First token in path must be token provided");
            ...
            require(total <= _amountInMax, "actual amount exceeds max");
            IERC20Metadata(_tokenProvided).safeTransferFrom(
                msg.sender,
                address(this),
                total
    ...
    function getUserDataByRole(address _user, Role _role) external view returns (uint256[] memory) {
