...
contract PoolToken is Ownable, ReentrancyGuard {
    ...
    IYTokenReserve public yTokenReserveStrategy;
    ...
    //Once minting and redeeming are enabled, they cannot be disabled again.

    // Collateral ratio
    uint256 public collateralRatio = 1e6;
    uint256 public lastRefreshCrTimestamp;
    uint256 public refreshCooldown = 3600; // = 1 hour
    uint256 public ratioStepUp = 2500; // = 0.0025 or 0.25% -> ratioStep when CR increase
    uint256 public ratioStepDown = 2500; // = 0.0025 or 0.25% -> ratioStep when CR decrease
    ...
    // fees
    uint256 public redemptionFee = 7500; // 6 decimals of precision
    uint256 public constant REDEMPTION_FEE_MAX = 9000; // 0.9%
    uint256 public mintingFee = 3000; // 6 decimals of precision
    uint256 public constant MINTING_FEE_MAX = 7000; // 0.7%
    uint256 public yTokenSlippage = 1e6; // 100% at genesis, to be reduced after launch to 10%
    uint256 public devRenumeration = 100_000; // 10%
    ...
    constructor(
        ...
        yTokenReserveStrategy = IYTokenReserve(_yTokenReserve);
        collateralToken = ERC20(_baseToken);
        xToken.setMinter(address(this));
        yTokenReserveStrategy.setPool(address(this)); //Strategy address
    }
    ...
    function refreshCollateralRatio() public {
        ...
        require(oracle.getXTokenTWAP() != 0, "Pool::refreshCollateralRatio: Invalid XToken price");
    ...
    function swapFromToken(uint256 _collateralTokenIn, uint256 _minXTokenOut)
        external
        nonReentrant
    {
        require(mintingEnabled, "Pool::swapFromToken: Minting has not been enabled yet.");
        collateralToken.safeTransferFrom(msg.sender, address(this), _collateralTokenIn);
        uint256 _ethIn = _collateralTokenIn;
        ...
        require(_xTokenOut >= _minXTokenOut, "Pool::swapFromToken: > slippage");
        require(
            _xTokenOut + xToken.totalSupply() <= IController(controller).maxXTokenSupply(),
            "Pool::swapFromToken: > Max mintable total supply"
        );

        userInfo[_sender].lastAction = block.number;

        if (_yTokenOutTwap > 0 && _wethSwapIn > 0) {
            collateralToken.safeIncreaseAllowance(address(swapStrategy), _wethSwapIn);
        ...
        if (devWallet != address(0)) {
            uint256 _devFee = (_fee * devRenumeration) / PRECISION;
            _fee = _fee - _devFee;
            /// 10% of the fees are taken for buybacks & burns, marketing expenses (as required)
            /// ... sent to a multi-signature wallet
            transferToDeveloper(_devFee);
        }
    ...
    function redeem(
        ...
        require(redeemEnabled, "Pool::redeem: Redeeming has not been enabled yet.");
    ...
    function collect() external nonReentrant {
        ...
        if (_sendYToken) {
            yTokenReserveStrategy.transfer(_sender, _yTokenAmount);
        }

        if (_sendEth) {
            collateralToken.safeTransfer(_sender, _ethAmount);
        }
    }

    /// @notice Function to recollateralize the pool by receiving ETH
    function recollateralize(uint256 _collateralAmt) external payable {
        collateralToken.safeTransferFrom(msg.sender, address(this), _collateralAmt);
        uint256 _amount = _collateralAmt;
    ...
    /// @notice Turn on minting and redemption - minting and redemption may not be turned off once they are turned on.
    function enableSwapAndRedeem() public onlyOwner {
        mintingEnabled = true;
        redeemEnabled = true;
        emit enabledSwappingAndRedemption(true, true);
    }
    ...
    function setYTokenSlippage(uint256 _slippage) external onlyOwner {
        require(
            _slippage <= 150000,
            "Pool::setYTokenSlippage: yTokenSlippage cannot be more than 15%"
        );
    ...
    /// @notice Set the address of devWallet, can only be set once.
    /// @param _devWallet address of devWallet multisig
    function setDevWallet(address _devWallet) external {
        require(devWallet == address(0), "Pool::setDevWallet: not allowed");
    ...
    function transferToTreasury(uint256 _amount) internal {
        ...
        if (_amount > 0) {
            collateralToken.safeTransfer(treasury, _amount);
        }
    }

    /// @notice Move weth to treasury
    function transferToDeveloper(uint256 _amount) internal {
        require(devWallet != address(0), "Pool::transferToDeveloper: Invalid address");
        if (_amount > 0) {
            collateralToken.safeTransfer(devWallet, _amount);
        }
    }

    /// @notice set Controller address
    function setController(address _controller) external onlyOwner {
        require(_controller != address(0), "Pool::setController: Invalid Address");
    ...
    event enabledSwappingAndRedemption(bool _mintPaused, bool _redeemPaused);
