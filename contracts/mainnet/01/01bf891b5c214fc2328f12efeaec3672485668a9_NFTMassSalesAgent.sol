...
interface INFTKey {
    ...
    function delistToken(address token, uint256 tokenId) external;
}

contract NFTMassSalesAgent is Ownable, IERC721Receiver {
    event ReceivedERC721(address operator, address from, uint256 tokenId, bytes data, uint256 gas);

    INFTKey public NFTKey = INFTKey(0x1A7d6ed890b6C284271AD27E7AbE8Fb5211D0739);
    ...
    function listCollection(
        ...
        IERC721(collection).setApprovalForAll(address(NFTKey), true);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            IERC721(collection).safeTransferFrom(_msgSender(), address(this), tokenIds[i]);
            NFTKey.listToken(collection, tokenIds[i], amount, expiration);
        }
    }

    function delistCollection(address collection, uint256[] memory tokenIds) public onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            NFTKey.delistToken(collection, tokenIds[i]);
            IERC721(collection).safeTransferFrom(address(this), _msgSender(), tokenIds[i]);
        }
    }
