...
interface ISaleModel is IInitOwnable {
    ...
    event MetaUpdated(string indexed twitterHandle, string indexed verificationTweet, string indexed preview);
    event Finalised();
    event ClaimedRaised(uint256 indexed amount);

    function initialised() external view returns(bool);
    function setMeta(string memory twitterHandle, string memory verificationTweet, string memory preview) external;
...
contract BuyoutFixed is ISaleModel, InitOwnable {
    ...
    /// @param twitterHandle            Link to twitter account (e.g., DeGatchi)
    /// @param verificationTweet        Link to tweet that confirms contract + deployer address (e.g., 1477141773516357633)
    /// @param preview                  Link to metadata image/gif, used as preview on FE (e.g., IPFS link).
    function setMeta(
        string memory twitterHandle,
        string memory verificationTweet,
        string memory preview
    ) external override onlyOwner {
        _info.twitter = string(abi.encodePacked("https://twitter.com/", twitterHandle));
        _info.verificationTweet = string(abi.encodePacked("https://twitter.com/status/", verificationTweet));
        _info.preview = preview;

        Info memory mInfo = _info;
        emit MetaUpdated(mInfo.twitter, mInfo.verificationTweet, mInfo.preview);
    }
    ...
    function getSaleDetails() external view returns (bool isInitialised, Info memory info, Sale memory sale, BulkBonus[] memory) {
        return (initialised, _info, _sale, bulkBonuses);
    }
}