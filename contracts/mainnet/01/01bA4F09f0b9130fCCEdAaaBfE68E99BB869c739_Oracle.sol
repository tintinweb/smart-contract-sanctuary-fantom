...
import {OracleTokens} from "./OracleTokens.sol";
...
contract Oracle is IOracle, OracleTokens {
    ...
    function _price(address token_, uint256 amount_) internal view onlySupported(token_) returns (uint256) {
        ...
        if (result <= 0) {
            feed = AggregatorV3Interface(reservePriceFeed(token_));
...
abstract contract OracleTokens is Ownable {
    ...
    function setPriceFeed(
        ...
        address[] memory reservePriceFeed_,
    ...
    function priceFeed(address token_) public view returns (address) {
        return _tokens[token_].priceFeed;
    }

    // Get the reserve price feed for a given asset
    function reservePriceFeed(address token_) public view returns (address) {
