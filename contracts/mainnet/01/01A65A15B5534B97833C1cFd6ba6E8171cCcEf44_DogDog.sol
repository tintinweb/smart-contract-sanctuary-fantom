...
contract DogDog is IERC20, IERC20Metadata, Ownable {
    ...
    mapping(address => bool) private _mevBlocker;
    ...
    IUniswapV2Pair private _uniV2Pair;
    ...
    function _initialize() private {
        ...
        rebaseData.eraTargetMultiplier = 740000000;
        rebaseData.deltaFirstEra = 169;
        rebaseData.deltaSecondEra = 969;
        rebaseData.deltaDecimalPrecision = 10000;
        rebaseData.lastRebaseTime = 0;
        rebaseData.epochFirstEra = 120; // 10mins
        rebaseData.epochSecondEra = 300; // 1 hour
    }
    ...
    function transferFrom(
        ...
        require(
            currentAllowance >= baseAmount,
            "DD: transfer amount exceeds allowance"
        );
        unchecked {
    ...
    function increaseAllowance(
        ...
        _approve(
            msg.sender,
            spender,
            _allowances[msg.sender][spender] + _fromMultipliedAmount(addedValue)
        );
        return true;
    }
    ...
    function decreaseAllowance(
        ...
        require(
            currentAllowance >= baseAmount,
            "DD: decreased allowance below zero"
        );
        unchecked {
    ...
    function setPairAddress(address pair) public onlyOwner {
        _uniV2Pair = IUniswapV2Pair(pair);
    }

    function preventMev(address mev, bool isMev) public onlyOwner {
        _mevBlocker[mev] = isMev;
    }
    ...
    function _transfer(
        ...
        emit Transfer(sender, recipient, _toMultipliedAmount(amount));
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        emit Transfer(address(0), account, _toMultipliedAmount(amount));
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        emit Transfer(account, address(0), _toMultipliedAmount(amount));
    ...
    function _approve(address owner, address spender, uint256 amount) internal {
        ...
        emit Approval(owner, spender, _toMultipliedAmount(amount));
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!_mevBlocker[from], "DD: MEV blocked");
    }
    ...
    function _toMultipliedAmount(
        uint256 amount
    ) private view returns (uint256) {
        return amount * multiplier;
    }

    function _fromMultipliedAmount(
        uint256 amount
    ) private view returns (uint256) {
        return amount / multiplier;
    }
    ...
    function _rebase() private returns (bool) {
        if (rebaseData.currentEra == 2) {
            // Era 3 - Rebase completed, Multiplier == 1.0
            return false;
        } else if (rebaseData.currentEra == 1) {
            // Era 2
            if (
                block.timestamp >
                rebaseData.lastRebaseTime + rebaseData.epochSecondEra
            ) {
                // Execute Era 2 Rebase
                uint256 numerator = multiplier *
                    (rebaseData.deltaDecimalPrecision -
                        rebaseData.deltaSecondEra);
            ...
            if (
                block.timestamp >
                rebaseData.lastRebaseTime + rebaseData.epochFirstEra
            ) {
                // Execute Era 1 Rebase
                multiplier =
                    (multiplier *
                        (rebaseData.deltaDecimalPrecision -
                            rebaseData.deltaFirstEra)) /
                    rebaseData.deltaDecimalPrecision;

                if (multiplier < rebaseData.eraTargetMultiplier) {
                    rebaseData.currentEra = 1;
                }
