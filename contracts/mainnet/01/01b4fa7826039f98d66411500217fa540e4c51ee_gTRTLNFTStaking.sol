...
// File contracts/TiMMYNFTStaking/TiMMYNFTStaking.sol

interface IgTRTL is IERC20 {
    function mint(address account, uint256 amount) external;
}

contract gTRTLNFTStaking is IERC721Receiver, Ownable {
    using SafeERC20 for IgTRTL;
    ...
    event Stake(address indexed owner, address indexed nftContract, uint256 indexed tokenId, uint256 stakingReward);
    event UnStake(address indexed owner, address indexed nftContract, uint256 indexed tokenId);
    event PermittedNFTContract(
    ...
    IgTRTL public constant GTRTL = IgTRTL(0xA064e15e284D5D288281cc24d94DD8ed39d42AA4);

    // holds the list of permitted NFTs
    EnumerableSet.AddressSet private permittedNFTs;
    mapping(address => mapping(StakeLength => uint256)) public collectionRewards;

    struct StakedNFT {
        bytes32 stakeId; // the stake id of the stake
        address owner; // the owner of the NFT
        IERC721 collection; // the ERC721 contract for which the NFT belongs
        uint256 tokenId; // the token ID staked
        uint256 stakedTimestamp; // the time that the NFT was staked
        uint256 lockedUntil; // the time until the NFT unlocks
        StakeLength stakeLength; // the stake length
    }
    ...
    function staked(address account) public view returns (StakedNFT[] memory) {
        ...
        bytes32[] memory ids = userStakeIds(account);
     ...
     */
    function userStakeIds(address account) public view returns (bytes32[] memory) {
    ...
    function calculateReward(address collection, StakeLength length) public view returns (uint256) {
        return collectionRewards[collection][length];
    }
    ...
    function stake(
        ...
        require(permittedNFTs.contains(address(collection)), "NFT is not permitted to be staked");
        require(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (length != StakeLength.FOREVER) {
                collection.safeTransferFrom(_msgSender(), address(this), tokenIds[i]);
            } else {
                collection.safeTransferFrom(_msgSender(), address(1), tokenIds[i]);
            }
        }

        bytes32[] memory stakeIds = new bytes32[](tokenIds.length);

        uint256 gTRTLDistribution = 0;

        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];

            // generate the stake ID
            bytes32 stakeId = _generateStakeId(_msgSender(), address(collection), tokenId);

            uint256 reward = calculateReward(address(collection), length);

            uint256 lockedUntil = block.timestamp;

            if (length == StakeLength.MONTH) {
                lockedUntil += 30 days;
            } else if (length == StakeLength.QUARTER) {
                lockedUntil += 90 days;
            } else if (length == StakeLength.HALFYEAR) {
                lockedUntil += 180 days;
            } else if (length == StakeLength.YEAR) {
                lockedUntil += 365 days;
            } else {
                lockedUntil = type(uint256).max;
            }
            ...
            gTRTLDistribution += reward;

            emit Stake(_msgSender(), address(collection), tokenId, reward);
        }

        if (gTRTLDistribution != 0) {
            // bulk mint it all at once to save gas
            GTRTL.mint(_msgSender(), gTRTLDistribution);
        }
     ...
     */
    function unstake(bytes32[] memory stakeIds) public {
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            require(stakedNFTs[stakeId].lockedUntil <= block.timestamp, "stake is still locked");
            ...
            info.collection.safeTransferFrom(address(this), info.owner, info.tokenId);

            emit UnStake(info.owner, address(info.collection), info.tokenId);
        }
    }

    /****** MANAGEMENT OF PERMITTED NFTs ******/

    function isPermittedNFT(address collection) public view returns (bool) {
        return permittedNFTs.contains(collection);
    }
    ...
    function permitNFT(
        ...
        collectionRewards[collection][StakeLength.MONTH] = month;
        collectionRewards[collection][StakeLength.QUARTER] = quarter;
        collectionRewards[collection][StakeLength.HALFYEAR] = halfyear;
        collectionRewards[collection][StakeLength.YEAR] = year;
        collectionRewards[collection][StakeLength.FOREVER] = forever;

        if (!permittedNFTs.contains(collection)) {
            permittedNFTs.add(collection);
        }

        emit PermittedNFTContract(collection, month, quarter, halfyear, year, forever);
    }

    /**
     * @dev removes the specified nft contract from being an acceptable NFT for staking purposes
     */
    function denyNFT(address collection) public onlyOwner {
        require(permittedNFTs.contains(collection), "NFT is not permitted");

        permittedNFTs.remove(collection);

        emit DeniedNFTContract(collection);
    }
