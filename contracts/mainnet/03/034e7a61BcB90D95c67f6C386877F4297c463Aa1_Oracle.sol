...
contract Oracle is AccessControl, IOracle {
    ...
    uint256 public expireTime; // signatures expiration time in seconds

    mapping(address => address) public priceSources; // collateral -> price source

    mapping(address => address[]) public pairs0; // collateral -> paris0
    mapping(address => address[]) public pairs1; // collateral -> pairs1
    ...
    /// @param expireTime_ signatures expiration time in seconds
    function setExpireTime(uint256 expireTime_) external onlyRole(SETTER_ROLE) {
        emit SetExpireTime(expireTime, expireTime_);
        expireTime = expireTime_;
    }

    /// @notice sets price source for given collateral
    /// @param collateral collateral contract address
    /// @param priceSource collateral price source contract address
    function setPriceSource(address collateral, address priceSource)
        external
        onlyRole(SETTER_ROLE)
    {
        emit SetPriceSource(collateral, priceSource);
        priceSources[collateral] = priceSource;
    }

    /// @notice
    function setPairs(
        address collateral,
        address[] memory pairs0_,
        address[] memory pairs1_
    ) external onlyRole(SETTER_ROLE) {
        emit SetPairs(collateral, pairs0_, pairs1_);

        uint256 i;
        address[] memory localPairs0 = new address[](pairs0_.length);
        for (i = 0; i < pairs0_.length; i++) {
            localPairs0[i] = pairs0_[i];
        }
        pairs0[collateral] = localPairs0;

        address[] memory localPairs1 = new address[](pairs1_.length);
        for (i = 0; i < pairs1_.length; i++) {
            localPairs1[i] = pairs1_[i];
        }
        pairs1[collateral] = localPairs1;
    }

    /// @notice verifies a signature
    /// @param collateral token address
    /// @param signature signature to verify
    /// @return price if signature is valid
        ...
        returns (uint256)
    {
        ...
        require(
            signature.timestamp + expireTime >= block.timestamp,
            "Oracle: SIGNATURE_EXPIRED"
        );
        require(
            priceSources[collateral] != address(0),
            "Oracle: PRICE_SOURCE_NOT_FOUND"
        );

        bytes32 hash = keccak256(
            abi.encodePacked(
                appId,
                priceSources[collateral],
                pairs0[collateral],
                pairs1[collateral],
                signature.price,
                signature.timestamp
    ...
    function getPairs(address collateral)
        external
        view
        returns (address[] memory, address[] memory)
    {
        return (pairs0[collateral], pairs1[collateral]);
    }
}
