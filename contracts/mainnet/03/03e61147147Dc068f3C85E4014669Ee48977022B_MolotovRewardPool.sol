...
// Note that this pool has no minter key of MOOTOV (rewards).
...
contract MolotovRewardPool {
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "MolotovRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "MolotovRewardPool: existing pool?");
        }
    }
    ...
    // View function to see pending MOLOTOVs on frontend.
    ...
    // Safe tomb transfer function, just in case if rounding error causes pool to not have enough MOLOTOVs.
