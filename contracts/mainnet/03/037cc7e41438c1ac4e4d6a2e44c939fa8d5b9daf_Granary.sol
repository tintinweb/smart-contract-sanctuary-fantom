...
contract Granary
{
	...
	constructor ()
	{
		...
		//B-x to join yvftm
		IERC20(alladdr[2]).approve(alladdr[1],uint256(-1));
		//B-x to join bb-yv-ftm
		IERC20(alladdr[9]).approve(alladdr[1],uint256(-1));
		//B-x to join PANIC
		IERC20(alladdr[4]).approve(alladdr[1],uint256(-1));
	...
	//string public id = "4802";
	function id() public pure returns(string memory){return("4802");}
	function name() public pure returns(string memory){return("ftm.guru/GRAIN/4802");}//string(abi.encodePacked("ftm.guru/GRAIN/", id)));}
	function symbol() public pure returns(string memory){return("GRAIN#4802");}//string(abi.encodePacked("GRAIN#", id)));}
	...
	//GRAIN#4802 doesnt use tvlGuru <= v6.3.0, which was the latest version at the time of deployment of this GRAIN.
	//address public utvl;
	/*
	uint8 public pid;
	...
	*/
	...
	bytes32[3] public BPT = 
    [
        bytes32(0x1e2576344d49779bdbb71b1b76193d27e6f996b700020000000000000000032d), //NEVR
        0xcde5a11a4acb4ee4c805352cec57e236bdbc3837000200000000000000000019, //FIDO
        0xc3bf643799237588b7a6b407b3fc028dd4e037d200000000000000000000022d  //yFTM
    ];
	...
	address[10] public alladdr = [
		0x8166994d9ebBe5829EC86Bd81258149B87faCfd3,//0	BMC			.mc
		0x20dd72Ed959b6147912C2e529F0a0C651c33c9ce,//1	B-x
		0x0DEC85e74A92c52b7F708c4B10207D9560CEFaf0,//2	yvFTM
		0xF24Bcf4d1e507740041C9cFd2DddB29585aDCe1e,//3	BEETS
		0xA882CeAC81B22FC2bEF8E1A82e823e3E9603310B,//4	PANIC
		0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83,//5	wftm
		0x1E2576344D49779BdBb71b1B76193d27e6F996b7,//6	BPT-nevr	.
		0xcdE5a11a4ACB4eE4c805352Cec57E236bdBC3837,//7	BPT-fidelio	.
		0x0786c3a78f5133F08C1c70953B8B10376bC6dCad,//8  TvlGuru
		0xC3BF643799237588b7a6B407B3fc028Dd4e037d2 //9	bb-yv-ftm
	];
	...
	{
		//harvest()
		if(	mc.pendingBeets(allnums[0],address(this)) > allnums[4] )
	...
	function work(address ben) internal
	{
        ...
        // : Cumulative Fee paid per $100 of doHardWork()

        
		//Sell 100% BEETS for WFTM at Beethoven-x: $0.25
        {
            FundManagement memory FM_B2F = FundManagement({
                sender:             address(this),
                fromInternalBalance:false,
                recipient:          payable(address(this)),
                toInternalBalance:  false
            });
            SingleSwap memory SS_B2F = SingleSwap({
                poolid: 	BPT[1],
            ...
            IBX(alladdr[1]).swap(SS_B2F, FM_B2F, 1, block.timestamp);
        }
        

		//Buy PANIC with 50% of WFTM at B-x: $0.125
        {
            FundManagement memory FM_F2P = FundManagement({
                sender:             address(this),
                fromInternalBalance:false,
                recipient:          payable(address(this)),
                toInternalBalance:  false
            });
            SingleSwap memory SS_F2P = SingleSwap({
                poolid: 	BPT[0],
                kind:	    SwapKind.GIVEN_IN,
                assetIn:    alladdr[5],
                assetOut:   alladdr[4],
                amount:     (IERC20(alladdr[5]).balanceOf(address(this))).div(2),
                userData:   abi.encode(0)
            });
            IBX(alladdr[1]).swap(SS_F2P, FM_F2P, 1, block.timestamp);
        }
        

		//Deposit 50% of remaining WFTM (25%) into yvWFTM: $0
		{
            IVault(alladdr[2]).deposit((IERC20(alladdr[5]).balanceOf(address(this))).div(2));
        }


		//JoinPool: bb-yv-ftm:	$0
		//Using Code Snippet from Kucino Casino (kcc.guru)
        // generate the eliteDEX pair path of token -> * -> target
        {
            IAsset[] memory T_y = new IAsset[](2);
            T_y[0] = IAsset(alladdr[5]);
            T_y[1] = IAsset(alladdr[2]);
            uint256[] memory A_y = new uint256[](2);
            A_y[0] = IERC20(alladdr[5]).balanceOf(address(this));
            A_y[1] = IERC20(alladdr[2]).balanceOf(address(this));

            bytes memory U_y = abi.encode(1, A_y, 1);
            //EXACT_TOKENS_IN_FOR_BPT_OUT (=enum JoinKind @1) "+" amountsIn (=A) "+" minimumBPT (1 = 10^-18 BPT)

            JoinPoolRequest memory R_y = JoinPoolRequest({	//T,A,U, fib ==> R
                assets:	 			T_y,
                maxAmountsIn:		A_y,	//amounts=amounts since we want to give it ALL, no slippage
                userData:			U_y,
                fromInternalBalance:false
            });

            IBX(alladdr[1]).joinPool(BPT[2], address(this), address(this), R_y);
        }
        ...
        // generate the eliteDEX pair path of token -> * -> target
        {
        IAsset[] memory T_n = new IAsset[](2);
        T_n[0] = IAsset(alladdr[9]);
        T_n[1] = IAsset(alladdr[4]);
		uint256[] memory A_n = new uint256[](2);
        A_n[0] = IERC20(alladdr[9]).balanceOf(address(this));
        A_n[1] = IERC20(alladdr[4]).balanceOf(address(this));

		bytes memory U_n = abi.encode(1, A_n, 1);
		//EXACT_TOKENS_IN_FOR_BPT_OUT (=enum JoinKind @1) "+" amountsIn (=A) "+" minimumBPT (1 = 10^-18 BPT)

		JoinPoolRequest memory R_n = JoinPoolRequest({	//T,A,U, fib ==> R
			assets:	 			T_n,
			maxAmountsIn:		A_n,	//amounts=amounts since we want to give it ALL, no slippage
			userData:			U_n,
			fromInternalBalance:false
		});

		IBX(alladdr[1]).joinPool(BPT[0], address(this), address(this), R_n);
	...
	{
		//Uses tvlGuru v6, to calculate FTMUSD price.
		ITVL tg = ITVL(alladdr[8]);
		uint256 aum = mc.userInfo(allnums[0],address(this)).amount + IERC20(want).balanceOf(address(this));
		//uint256[] memory w = IBPT(address(want)).getNormalizedWeights();
		(,uint256[] memory a,) = IBX(alladdr[1]).getPoolTokens(BPT[0]);
        uint256 p_poold = a[1];
        uint256 p_price = tg.p_t_coin_usd(0xa341D77315e5E130ad386e034B4c9714cB149F4a);
		uint256 bpt_ts =  want.totalSupply();
        uint256 bpt_price = (p_price * p_poold) / bpt_ts; 
		return (bpt_price * aum / 1e18);
	}
