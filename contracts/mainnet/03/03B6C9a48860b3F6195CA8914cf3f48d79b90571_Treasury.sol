...
contract Treasury is ITreasury, Operator, ReentrancyGuard {
    ...
    function buyback(uint256 _collateral_amount, uint256 _min_share_amount) external override onlyOperator notMigrated checkRebalanceCooldown {
        ...
        uint256 min_dollar_out = dollarPrice().mul(9950).div(10000).mul(_collateral_amount);
        // Buy back dollar
    ...
    function recollateralize(uint256 _share_amount, uint256 _min_collateral_amount) external override onlyOperator notMigrated checkRebalanceCooldown {
        ...
        if (_collateral_balance > 0) {
            IERC20(collateral).safeTransfer(collateralFund, _collateral_balance); // Transfer collateral from Treasury to Pool
        }
