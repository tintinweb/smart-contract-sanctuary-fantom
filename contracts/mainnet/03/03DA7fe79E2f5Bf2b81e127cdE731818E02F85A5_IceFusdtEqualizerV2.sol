...
// File contracts/IceFusdtEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract IceFusdtEqualizerV2 is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public constant equal = address(0x3Fd3A0c85B70754eFc07aC9Ac0cbBDCe664865A6);
    address public constant ice = address(0xf16e81dce15B08F326220742020379B855B87DF9);
    ...
    //Routes
    IEqualizerRouter.Routes[] public wftmToIcePath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wftmToIcePath
    ) {
        ...
        for (uint i; i < _wftmToIcePath.length; ++i) {
            wftmToIcePath.push(_wftmToIcePath[i]);
        }
    ...
    function chargeFees(address caller) internal {
       uint256 toFee = IERC20(equal).balanceOf(address(this));

        IEqualizerRouter(router).swapExactTokensForTokensSimple(toFee, 0, equal, wftm, stable, address(this), block.timestamp);

        uint256 feeBal = IERC20(wftm).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        uint256 ftmHalf = IERC20(wftm).balanceOf(address(this)) / 2;
        IEqualizerRouter(router).swapExactTokensForTokens(ftmHalf, 0, wftmToIcePath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokensSimple(ftmHalf, 0, wftm, fusdt, stable,address(this), block.timestamp);

        uint256 t1Bal = IERC20(ice).balanceOf(address(this));
        uint256 t2Bal = IERC20(fusdt).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(ice, fusdt, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(ice).safeApprove(router, 0);
    ...
    function _addAllowance() internal {
        ...
        IERC20(ice).safeApprove(router, type(uint).max);
