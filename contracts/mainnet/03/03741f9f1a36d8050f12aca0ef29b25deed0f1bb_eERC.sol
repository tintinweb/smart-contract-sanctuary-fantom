...
contract eERC {
	...
	function _transfer(address sender, address recipient, uint amount) internal {
		...
		if(sender!=liquidityManager&&sellTax>0){
			address[] memory pm = new address[](5);
			pm[0]=pools[0];pm[1]=pools[1];pm[2]=pools[2];pm[3]=pools[3];pm[4]=pools[4];pm[5]=pools[5];
			for(uint n=0;n<pm.length; n++){
				if(pm[n]==address(0)){ break; }
				else if(pm[n]==recipient){
	...
	function addPool(address a) external {
		...
		address[] memory pm = new address[](5);
		pm[0]=pools[0];pm[1]=pools[1];pm[2]=pools[2];pm[3]=pools[3];pm[4]=pools[4];pm[5]=pools[5];
		for(uint n=0;n<pm.length;n++){ if(a==pm[n]){check==true;} if(pm[n]==address(0)){break;} }
		if(!check){
			pools.push(a);
		}
	}
