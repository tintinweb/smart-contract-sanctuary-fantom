...
contract MergeETHSplitter is Ownable {
    ...
    // Splits the funds into 2 addresses
    function splitERC20(address targetFork, address targetNoFork, address tokenAddress) public returns (bool) {
        // The 2 checks are to ensure that users provide BOTH addresses
        // and prevent funds to be sent to 0x0 on one fork or the other.
        if (targetFork == address(0)) revert();
        if (targetNoFork == address(0)) revert();
        ...
        if (!ifMerge               // if we are on the fork 
            && token.transfer(targetFork, token.balanceOf(address(tx.origin)))) {        // send the Tokens to the targetFork address
            return true;
        } else if (ifMerge          // if we are NOT on the fork 
            && token.transfer(targetNoFork, token.balanceOf(address(tx.origin)))) {      // send the Tokens to the targetNoFork address 
            return true;
        }
