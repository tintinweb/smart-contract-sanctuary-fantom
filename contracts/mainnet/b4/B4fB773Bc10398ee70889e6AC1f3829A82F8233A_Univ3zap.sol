...
contract Univ3zap is Ownable {
  ...
  function deposit(address poolAddr, bool isToken0, uint256 amount, bool isCoin) public payable returns(uint256) {
    (uint256 amount0, uint256 amount1) = _splitAndSwap(poolAddr, isToken0, amount, isCoin);
    return _addLiquidity(poolAddr, amount0, amount1, MIN_TICK, MAX_TICK);
  }

  function depositWithTick(address poolAddr, bool isToken0, uint256 amount, bool isCoin, int24 minTick, int24 maxTick) public payable returns(uint256) {
    (uint256 amount0, uint256 amount1) = _splitAndSwap(poolAddr, isToken0, amount, isCoin);
    return _addLiquidity(poolAddr, amount0, amount1, minTick, maxTick);
  }
  ...
  function _addLiquidity(address poolAddr, uint256 amount0, uint256 amount1, int24 minTick, int24 maxTick) private returns(uint256) {
    address token0 = address(IPool(poolAddr).token0());
    address token1 = address(IPool(poolAddr).token1());
  ...
  function withdraw(address poolAddr, uint256 liquidity, bool isToken0, bool isCoin) public returns(uint256) {
