...
contract Beast is ERC20Burnable, Operator {
    ...
    // Initial distribution for the BEAST-WFTM LP -> BEAST pool
    uint256 public constant INITIAL_BEAST_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the BEAST ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Test Beast Finance", "tBEAST") {
        // Mints 1 BEAST to contract creator for initial pool setup
    ...
    function _getBeastPrice() internal view returns (uint256 _beastPrice) {
        try IOracle(beastOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Beast: failed to fetch BEAST price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _beastPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_beastPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBeastOracle(address _beastOracle) public onlyOperatorOrTaxOffice {
        require(_beastOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BEAST to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BEAST to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBeastPrice = _getBeastPrice();
            currentTaxRate = _updateTaxRate(currentBeastPrice);
            if (currentBeastPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_beastPool != address(0), "!_beastPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_beastPool, INITIAL_BEAST_POOL_DISTRIBUTION);
