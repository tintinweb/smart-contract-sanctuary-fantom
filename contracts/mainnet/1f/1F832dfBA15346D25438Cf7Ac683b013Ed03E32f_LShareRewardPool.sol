...
contract LShareRewardPool is Operator {
    ...
    struct PoolInfo {
        ...
        /// Accumulated LShare per share.
        uint256 accLSharePerShare;
    ...
    /// The time when LShare emissions start.
    uint256 public poolStartTime;

    /// The time when LShare emissions end.
    uint256 public poolEndTime;

    /// Running time of emissions (in seconds).
    uint256 public runningTime = 1460 days;

    /// Total amount of tokens to be emitted.
    uint256 public constant TOTAL_REWARDS = 28000 ether;

    /// Time when each epoch ends.
    /// @dev Epochs are time periods with different token distribution, than the starting one
    uint256[4] public epochEndTimes;

    /// Rewards per second for each of 4 epochs (last item is equal to 0 - for sanity).
    uint256[4] public epochLSharePerSecond;

    /// Amount of tokens allocated to each of the epochs.
    uint256[] public epochTotalRewards = [14000 ether, 7000 ether, 7000 ether];
    ...
    /// Default constructor.
    /// @param _lshare Address of LShare token
    ...
    constructor(
        ...
        epochEndTimes[0] = poolStartTime + 365 days; // 1st year
        epochEndTimes[1] = epochEndTimes[0] + 365 days; // 2nd year
        epochEndTimes[2] = epochEndTimes[1] + 730 days; // 3rd and 4th year

        epochLSharePerSecond[0] = epochTotalRewards[0] / (365 days);
        epochLSharePerSecond[1] = epochTotalRewards[1] / (365 days);
        epochLSharePerSecond[2] = epochTotalRewards[2] / (730 days);

        epochLSharePerSecond[3] = 0;
    }
    ...
    /// Return amount of accumulated rewards over the given time, according to the lshare per second emission.
        ...
        for (uint8 epochId = 3; epochId >= 1; --epochId) {
            if (_toTime >= epochEndTimes[epochId - 1]) {
                if (_fromTime >= epochEndTimes[epochId - 1]) {
                    return (_toTime - _fromTime) * epochLSharePerSecond[epochId];
                }

                uint256 _generatedReward = (_toTime - epochEndTimes[epochId - 1]) * epochLSharePerSecond[epochId];
                if (epochId == 1) {
                    return _generatedReward + ((epochEndTimes[0] - _fromTime) * epochLSharePerSecond[0]);
                }
                for (epochId = epochId - 1; epochId >= 1; --epochId) {
                    if (_fromTime >= epochEndTimes[epochId - 1]) {
                        return _generatedReward + ((epochEndTimes[epochId] - _fromTime) * epochLSharePerSecond[epochId]);
                    }
                    _generatedReward = _generatedReward + ((epochEndTimes[epochId] - epochEndTimes[epochId - 1]) * epochLSharePerSecond[epochId]);
                }
                return _generatedReward + ((epochEndTimes[0] -_fromTime) * epochLSharePerSecond[0]);
            }
        }
        return (_toTime - _fromTime) * epochLSharePerSecond[0];
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _lshareReward = (_generatedReward * pool.allocPoint) / totalAllocPoint;
            accLSharePerShare = accLSharePerShare + ((_lshareReward * 1e18) / tokenSupply);
        }
        return ((user.amount * accLSharePerShare) / 1e18) - user.rewardDebt;
    }

    /// Return current emission per second
    /// @return LSHARE emission per second
    /// @dev To be used in UI
    function emissionPerSecond() external view returns (uint256) {
        uint256 _currentTime = block.timestamp;
        if(_currentTime < poolStartTime){
            return 0;
        }
        for (uint8 epochId = 3; epochId >= 1; --epochId) {
            if(_currentTime > epochEndTimes[epochId - 1]){
                return epochLSharePerSecond[epochId];
            }
        }
        return epochLSharePerSecond[0];
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _lshareReward = (_generatedReward * pool.allocPoint) / totalAllocPoint;
            pool.accLSharePerShare = pool.accLSharePerShare + ((_lshareReward * 1e18) / tokenSupply);
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = ((user.amount * pool.accLSharePerShare) / 1e18) - user.rewardDebt;
            if (_pending > 0) {
        ...
        user.rewardDebt = (user.amount * pool.accLSharePerShare) / 1e18;
    ...
    /// Safe LShare transfer function.
    /// @param _to Recipient address of the transfer
    /// @param _amount Amount of tokens to be transferred
    /// @dev Used just in case if rounding error causes pool to not have enough LShare.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        if (block.timestamp < poolEndTime + 1 days) {
            // do not allow to drain core token (lSHARE or lps) if less than 24 hours after pool ends
