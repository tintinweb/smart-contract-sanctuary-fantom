...
contract EasyDCA is Ownable {
    ...
    function removeMinFromHeap() internal returns (uint256) {
        ...
        while (currentIndex * 2 < dcaHeap.length - 1) {
            ...
            if (
                dcaList[leftChild].treshold >
                dcaList[rightChild].treshold
            ) {
                j += 1;
            }
    ...
    // Chainlink Methods
    function buyTokens(address _buyer, uint256 _amount, address _from, address _to) internal returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = _from;
        path[1] = _to;

        IERC20(_from).approve(dex, _amount);
        return SpookySwap(dex).swapExactTokensForTokens(_amount, 0, path, _buyer, block.timestamp + 1000)[1];
    }
    ...
    function executeDCA(uint256 _dcaIndex) internal {
        ...
        try IERC20(_currentStable).transferFrom(_currentAccount, address(this), _currentAmount) returns(bool) {
            ...
            uint256 _purchaseAmount = buyTokens(_currentAccount, _amountToBuy, _currentStable, _currentTarget);

            // Update the last purchase time and re-add to the heap
            dcaList[_dcaIndex].lastPurchase = block.timestamp;
            dcaList[_dcaIndex].treshold = block.timestamp + dcaList[_dcaIndex].frequency;
        ...
        } catch { // If purchase can't happen disable the DCA order and remove it from heap
            removeMinFromHeap();
            dcaList[_dcaIndex].isActive = false;
        }
    }
    ...
    function checkUpkeep(
        ...
        upkeepNeeded = (dcaHeap.length > 0) && (dcaList[dcaHeap[0]].treshold < block.timestamp);
    }
