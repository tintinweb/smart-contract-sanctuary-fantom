...
contract StrategyTarot is SimpleStratManager {
    ...
    /**
     * @dev
     * To get top 10 best bTarot borrowRate.
     */
    function getBestbTarot() public returns (address[] memory) {
        /**
         Get top 10 highest borrowRate bTarot.
         return array
         */
        BTarot[] memory temp = new BTarot[](bTarots.length);
        for (uint256 i = 0; i < bTarots.length; i++) {
            uint256 borrowRate = IBorrowable(bTarots[i]).borrowRate();
            temp[i] = BTarot(bTarots[i], borrowRate);
        }

        BTarot[] memory best = quickSort(
            temp,
            uint32(0),
            uint32(bTarots.length - 1)
        );
        address[] memory bestAddresses = new address[](best.length);
        for (uint256 i = 0; i < best.length; i++) {
            bestAddresses[i] = best[i].bTarotAddress;
        }
        if (autoset) {
            autoSetBestbTarots(bestAddresses);
        }
    ...
    function quickSort(
        ...
        uint32 left,
        uint32 right
    ) internal pure returns (BTarot[] memory) {
        uint32 i = left;
        uint32 j = right;
        if (i == j) return arr;
        BTarot memory pivot = arr[uint32(left + (right - left) / 2)];
        while (i <= j) {
            while (arr[uint32(i)].borrowRate > pivot.borrowRate) i++;
            while (pivot.borrowRate > arr[uint32(j)].borrowRate) j--;
            if (i <= j) {
    ...
    /**
     * @dev
     * enable auto setting of bestbTarot.
     */
    function setAuto(bool _set) public onlyManager {
        autoset = _set;
    }
    ...
    function withdrawHelper(uint256 _amount) internal {
        ...
        for (int256 i = int256(bestbTarots.length - 1); i >= 0; i--) {
            ...
            if (underlying <= underlyingBalance) {
                uint256 sharesToRedeem = IBorrowable(bestbTarots[uint256(i)])
                    ...
                    )
                {
                    ...
                    isbTarotUsed[bestbTarots[uint256(i)]] = false;
                } catch Error(string memory reason) {
    ...
    /**
     * @dev
     * internal function to automatically set best borrowables.
     */
    function autoSetBestbTarots(address[] memory _bestbTarots) internal {
        bestbTarots = _bestbTarots;
        _givebTarotsAllowances(bestbTarots);
    }
     ...
     * To let contract auto search for bTarot with highest borrowRate.
