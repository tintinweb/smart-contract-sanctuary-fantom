...
contract PiswapToken is ERC20('Pi Swap Token', 'PI') {
    ...
    _moveDelegates(_owner, newOwner, balanceOf(_owner)); // Chuyển quyền bỏ phiếu từ người sở hữu hiện tại sang người được chuyển quyền sở hữu mới
    _owner = newOwner; // Cập nhật người sở hữu mới
}
    ...
    /// Must only be called by the owner (TokenMaster) or admin.
    function mintTo(address _to, uint256 _amount) public onlyOwnerOrAdmin {
        _mint(_to, _amount);
    ...
    modifier onlyOwnerOrAdmin() {
        require(msg.sender == _owner || msg.sender == _admin, "Only owner or admin can call this function");
        _;
    }
...
contract CommonMaster is ICommonMaster, Pausable, Ownable {
    ...
    function pendingToken(address _pair, address _user) external view override returns (uint256) {
        ...
        uint256 stakeBalance = ERC20(_pair).balanceOf(address(this)).mul(10**uint256(bake.decimals())).div(
    ...
    uint256 devReward = tokenReward.mul(5).div(100); // 5% for devAddr
    uint256 burnAmount = tokenReward.mul(10).div(100); // 10% burn
    uint256 remainingReward = tokenReward.sub(devReward).sub(burnAmount); // 90% remaining

    bake.mintTo(devAddr, devReward);
    bake.transfer(address(0xdead), burnAmount); // Burn 10% of tokenReward by transferring to deadAddress
    bake.mintTo(address(this), remainingReward); // Mint the remaining 90% tokens
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBal = bake.balanceOf(address(this));
        if (_amount > tokenBal) {
            bake.transfer(_to, tokenBal);
        } else {
