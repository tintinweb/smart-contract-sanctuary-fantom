...
contract Strategy is BaseStrategy {
    ...
    bool public collectFeesEnabled = true;
    uint256 public swapSecurityFactor;
    ...
    function _swap(uint256 _fullInvestment) internal returns (uint256[] memory _swapedAmounts) {
        ...
        lpPairPath[0] = route(address(want), lpPairToken1, true);

        _swapedAmounts = ISolidlyRouter(router).swapExactTokensForTokens(
            swapAmountIn,
            swapAmountIn.mul(1e12).mul(swapSecurityFactor).div(basisOne),
            lpPairPath,
            address(this),
            block.timestamp
        );
    ...
    function sellMIMTokens() internal {
        ...
        lp1TokenToLp0Route[0] = route(lpPairToken1, address(want), true);
    ...
    function removeLiquidity(uint256 toExitAmount) internal {
        ...
        (uint256 wantPerMim, ) =
            ISolidlyRouter(router).getAmountOut(1e18, lpPairToken1, address(want));
    ...
    function _withdrawFromMasterChefAndTransfer(address _to) internal {
        if (abandonRewards) {
            masterChef.withdraw(address(solidlyLpToken), balanceOfLPInMasterChef());
        } else {
            withdrawFromMasterChef();
            (uint256 _solidRewards, uint256 _sexRewards) = balanceOfReward();
    ...
    function getSwapAmount(uint256 investmentA) internal view returns (uint256 _swapAmount) {
        ...
        (uint256 nominator, ) =
            ISolidlyRouter(router).getAmountOut(halfInvestment, address(want), lpPairToken1);

        uint256 denominator = quoteLiquidity(halfInvestment);

        _swapAmount = investmentA.sub(
            Babylonian.sqrt((halfInvestment.mul(halfInvestment).mul(nominator)) / denominator)
        );
    }
    ...
    function giveAllowances() internal {
        solidlyLpToken.approve(address(masterChef), max);
