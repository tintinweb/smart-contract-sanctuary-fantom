...
contract YearnEndpoint is Endpoint {
  uint256 totalDeposited;
  IVault vault;

  constructor(address _router, IERC20 _token, IVault _vault) Endpoint(_router, _token) {
    vault = _vault;
  }

  function withdrawableAmount() external view override returns (uint256) {
    return (totalDeposited * vault.pricePerShare()) / (10 ** vault.decimals());
  }
  ...
  function deposit(uint256 amount) external override onlyRouter {
    ...
    totalDeposited += vault.deposit(amount);
  }
  ...
  function withdraw(uint256 amount) external override onlyRouter returns (
    ...
    amount = (amount * (10 ** vault.decimals())) / vault.pricePerShare();
    if (amount > totalDeposited) {
      amount = totalDeposited;
    }
    withdrawn = vault.withdraw(amount);
...
abstract contract Endpoint {
  ...
  constructor(address _router, IERC20 _token) {
    token = _token;
    router = _router;
  }
