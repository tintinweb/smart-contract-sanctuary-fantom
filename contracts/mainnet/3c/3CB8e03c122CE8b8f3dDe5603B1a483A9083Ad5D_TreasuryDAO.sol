...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
    ...
    function convertDepositFeesToUSD(address token, uint256 tokenType, uint256 amount) public onlyRole(OPERATOR_ROLE) {
        ...
        if (tokenType == 1) {
            (address token0, address token1, uint256 token0Supply, uint256 token1Supply) = _removeLiquidity(token);
    ...
    function _removeLiquidity(address token) internal returns(address, address, uint256, uint256) {
