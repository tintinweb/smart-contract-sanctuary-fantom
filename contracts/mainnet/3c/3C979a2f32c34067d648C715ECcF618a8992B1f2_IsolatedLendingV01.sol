...
contract IsolatedLendingV01 is ERC4626{
    ...
    uint256 private constant OPEN_COLLATERIZATION_RATE = 80000; // 80%
    ...
    uint256 private constant LIQUIDATION_MULTIPLIER = 6000; // 6%
    ...
    constructor(address _asset, address _collateral, string memory _name, string memory _symbol)ERC4626(ERC20(_asset), _name, _symbol){
        ...
        yvToken = IYearnVault(_collateral);
        priceFeedCollateral = AggregatorV3Interface(0x11DdD3d147E5b83D01cee7070027092397d63658);
        ...
        exchangeRate = yvToken.pricePerShare()*(uint256(collateralPrice)*1e10)/(uint256(assetPrice)*1e20);
        feeTo = msg.sender;
        admin = msg.sender;
    }
    ...
    function liquidate(address _user, uint256 _amount) public {
        ...
        if(!isSolvent(_user)){
            ...
            uint256 collateralLiquidated = _amount*1e20/exchangeRate; //1e6*1e20/1e8 = 1e18 (moobeftm decimal) //1e6*1e10/1e8 = 1e8 (wbtc decimals)
    ...
    function userCollateralValue(address _user) public view returns (uint256){
        return userCollateralAmount[_user]*(exchangeRate*1e10)/1e30; //1e18*1e18/1e30 = 1e6 (usdc decimals)
    }
}