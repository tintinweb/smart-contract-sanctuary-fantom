...
import "./interfaces/IInfinitePrinter.sol";
import "./owner/Operator.sol";

contract SmeltWrapper {
    ...
    function stake(uint256 amount) public virtual {
        ...
        smelt.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public virtual {
        uint256 andrasSmelt = _balances[msg.sender];
        require(andrasSmelt >= amount, "Boardroom: withdraw request greater than staked amount");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = andrasSmelt.sub(amount);
        smelt.safeTransfer(msg.sender, amount);
    }
}
...
\______   \_____     ______  ____    __| _/   \_   _____/|__|  ____  _____     ____    ____   ____
 ...
 |______  /(____  //____  > \___  >\____ |     \___  /   |__||___|  /(____  /|___|  / \___  >\___  >
        \/      \/      \/      \/      \/         \/             \/      \/      \/      \/     \/
*/
contract Boardroom is SmeltWrapper, ContractGuard, Operator {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        BoardroomSnapshot memory genesisSnapshot = BoardroomSnapshot({time : block.number, rewardReceived : 0, rewardPerSmelt : 0});
        boardroomHistory.push(genesisSnapshot);

        withdrawLockupEpochs = 2; // Lock for 3 epochs (72h) before release withdraw
    ...
    function rewardPerSmelt() public view returns (uint256) {
        return getLatestSnapshot().rewardPerSmelt;
    }

    function earned(address andras) public view returns (uint256) {
        uint256 latestRPS = getLatestSnapshot().rewardPerSmelt;
        uint256 storedRPS = getLastSnapshotOf(andras).rewardPerSmelt;
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            oBol.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        uint256 prevRPS = getLatestSnapshot().rewardPerSmelt;
        uint256 nextRPS = prevRPS.add(amount.mul(1e18).div(totalSupply()));

        BoardroomSnapshot memory newSnapshot = BoardroomSnapshot({
            time: block.number,
            rewardReceived: amount,
            rewardPerSmelt: nextRPS
        });
        boardroomHistory.push(newSnapshot);

        oBol.safeTransferFrom(msg.sender, address(this), amount);
