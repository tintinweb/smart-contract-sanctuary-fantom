...
contract Buyback is Ownable {
    ...
    function buyback() public {
        require(!paused, "Buyback not open (yet)");
        uint256[] memory holdings = game.walletOfOwner(msg.sender);
        require(holdings.length > 0, "You don't have any WOA NFT");

        for (uint256 i = 0; i < holdings.length; i++) {
            if (holdings[i] == 0) {
                continue;
            }
            game.transferFrom(_msgSender(), address(this), holdings[i]);
            payable(msg.sender).transfer(price);
        }
    }

    function boughtBack() public view returns (uint256 bought) {
        return game.walletOfOwner(address(this)).length;
    }
