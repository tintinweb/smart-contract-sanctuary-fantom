...
contract Banana is ERC20, Ownable {
  ...
  /**
   * mints $BANANA to a recipient
   * @param to the recipient of the $BANANA
   * @param amount the amount of $BANANA to mint
   */
  ...
  /**
   * burns $BANANA from a holder
   * @param from the holder of the $BANANA
   * @param amount the amount of $BANANA to burn
   */
...
contract Bank is Ownable, IERC721Receiver, Pausable {
    ...
    // reference to the WorldOfApes NFT contract
    WorldOfApes game;
    // reference to the $banana contract for minting $banana earnings
    Banana banana;
    ...
    // amount of $banana due for each alpha point staked
    uint256 public bananaPerAlpha = 0;

    // thief earn 10000 $banana per day
    uint256 public DAILY_BANANA_RATE = 10000 ether;
    // thief must have 2 days worth of $banana to unstake or else it's too cold
    // uint256 public MINIMUM_TO_EXIT = 2 days;
    uint256 public MINIMUM_TO_EXIT = 2 days;
    // wolves take a 20% tax on all $banana claimed
    uint256 public constant BANANA_CLAIM_TAX_PERCENTAGE = 20;
    // there will only ever be (roughly) 2.4 billion $banana earned through staking
    uint256 public constant MAXIMUM_GLOBAL_BANANA = 1000000000 ether;

    uint256 public POACHING_PRICE = 5000 ether;
    uint256 public POACHING_DELAY = 1 days;
    bool public poachesEnabled = true;
    bool disableHunterStakedRequirement = false;
    mapping (address => uint256) public poaches;
    
    RandomGenerator public randomGenerator;

    // amount of $banana earned so far
    uint256 public totalBananaEarned;
    // number of Thief staked in the Bank
    uint256 public totalThiefStaked;
    // the last time $banana was claimed
    uint256 public lastClaimTimestamp;

    // emergency rescue to allow unstaking without any checks but without $banana
    ...
    /**
     * @param _game reference to the WorldOfApes NFT contract
     * @param _banana reference to the $banana token
     */
    constructor(address _game, address _banana, address _randomGenerator) {
        game = WorldOfApes(_game);
    ...
    function _addPoliceToPack(address account, uint256 tokenId) internal {
        ...
        pack[alpha].push(
            Stake({
                owner: account,
                tokenId: uint16(tokenId),
                value: uint80(bananaPerAlpha)
            })
        );
        // Add the police to the Pack
        emit TokenStaked(account, tokenId, bananaPerAlpha);
    }

    /***CLAIMING / UNSTAKING */

    /**
     * realize $banana earnings and optionally unstake tokens from the Bank / Pack
     * to unstake a Thief it will require it has 2 days worth of $banana unclaimed
        ...
        _updateEarnings
    {
        ...
        if (owed == 0) return;
        banana.mint(_msgSender(), owed);
    }

    function estimatedRevenuesOf(uint16[] memory tokenIds)
        public
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (isThief(tokenIds[i])) {
                ...
                uint256 newOwed = 0;
                if (totalBananaEarned < MAXIMUM_GLOBAL_BANANA) {
                    newOwed =
                        ((block.timestamp - stake.value) * DAILY_BANANA_RATE) /
                        1 days;
                ...
                owed[i] = (alpha) * (bananaPerAlpha - stake.value);
            }
        }
        return owed;
    }

    function hasStakedHunters(address sender) public view returns (bool) {
        uint256[] memory stakedTokens = bags[sender];
        for (uint256 i = 0; i < stakedTokens.length; i++) {
            if (!isThief(stakedTokens[i]) && (disableHunterStakedRequirement || bank[stakedTokens[i]].value + POACHING_DELAY <= block.timestamp)) {
                return true;
            }
        }
        return false;
    ...
    function canPoach(address _address) public view returns(bool)  {
      return poaches[_address] + POACHING_DELAY <= block.timestamp;
    }

    function poach() external returns (uint256 stolen) {
      require(poachesEnabled, "Poaches are disabled");
      require(hasStakedHunters(_msgSender()), "You do not have any Hunters that have been staked for long enough!");
      require(banana.balanceOf(_msgSender()) >= POACHING_PRICE, "You do not have enough $BANANAS");
      require(canPoach(_msgSender()), "You must wait before poaching again");
      banana.burn(_msgSender(), POACHING_PRICE);
      uint16 tokenId = randomGenerator.generateRandomApe(_msgSender());
      uint16[] memory tokenIds = new uint16[](1);
      tokenIds[0] = tokenId;
      uint256 amount = estimatedRevenuesOf(tokenIds)[0];
      banana.mint(_msgSender(), amount);
      bank[tokenId] = Stake({
        owner: _msgSender(),
        tokenId: uint16(tokenId),
        value: uint80(block.timestamp)
      });
      poaches[_msgSender()] = block.timestamp;
      return amount;
    }

    /**
     * realize $banana earnings for a single Thief and optionally unstake it
     * if not unstaking, pay a 20% tax to the staked Polices
     * if unstaking, there is a 50% chance all $banana is stolen
     * @param tokenId the ID of the Thief to claim earnings from
     * @param unstake whether or not to unstake the Thief
     * @return owed - the amount of $banana earned
     */
        ...
        returns (uint256 owed)
    {
        ...
        require(
            ...
            "GONNA BE COLD WITHOUT TWO DAY'S BANANAS"
        );
        if (totalBananaEarned < MAXIMUM_GLOBAL_BANANA) {
            owed =
                ((block.timestamp - stake.value) * DAILY_BANANA_RATE) /
            ...
            owed = 0;
            // $banana production stopped already
        } else {
            owed =
                ((lastClaimTimestamp - stake.value) * DAILY_BANANA_RATE) /
                1 days;
            // stop earning additional $banana if it's all been earned
        }
        if (unstake) {
            _remove(_msgSender(), tokenId);
            if (random(tokenId) & 1 == 1) {
                // 50% chance of all $banana stolen
                _payPoliceTax(owed);
                owed = 0;
            }
            ...
            _payPoliceTax((owed * BANANA_CLAIM_TAX_PERCENTAGE) / 100);
            // percentage tax to staked wolves
            owed = (owed * (100 - BANANA_CLAIM_TAX_PERCENTAGE)) / 100;
    ...
    /**
     * realize $banana earnings for a single Police and optionally unstake it
     * Polices earn $banana proportional to their Alpha rank
        ...
        returns (uint256 owed)
    {
        ...
        require(stake.owner == _msgSender(), "SWIPER, NO SWIPING");
        owed = (alpha) * (bananaPerAlpha - stake.value);
    ...
    /**
     * add $banana to claimable pot for the Pack
     * @param amount $banana to add to the pot
     */
    function _payPoliceTax(uint256 amount) internal {
        if (totalAlphaStaked == 0) {
            // if there's no staked wolves
            unaccountedRewards += amount;
            // keep track of $banana due to wolves
            return;
        }
        // makes sure to include any unaccounted $banana
        bananaPerAlpha += (amount + unaccountedRewards) / totalAlphaStaked;
        unaccountedRewards = 0;
    }

    /**
     * tracks $banana earnings to ensure it stops once 2.4 billion is eclipsed
     */
    ...
    function setSettings(uint256 rate, uint256 exit) external onlyOwner {
        ...
        DAILY_BANANA_RATE = rate;
    }

    function setPoachingSettings(uint256 delay, uint256 price) external onlyOwner {
        POACHING_DELAY = delay;
        POACHING_PRICE = price;
    }
    ...
    function getRandomGenerator () view public returns (RandomGenerator) {
      return randomGenerator;
    }

    function togglePoaches (bool _toggle) public onlyOwner {
      poachesEnabled = _toggle;
    }
    function toggleHunterStakedRequirement (bool _disableHunterStakedRequirement) public onlyOwner {
      disableHunterStakedRequirement = _disableHunterStakedRequirement;
    }
}
...
interface IWorldOfApes {
  // struct to store each token's traits
  struct WorldOfApesTraits {
    bool isApe;
    uint8 base;
    uint8 top;
    uint8 pants;
  ...
  function getTokenTraits(uint256 tokenId)
    external
    view
    returns (WorldOfApesTraits memory);
}
...
contract RandomGenerator is Ownable {
  ...
  function generateRandomBase(address _from) internal view returns (uint256 base) {
    uint16 max = game.minted() - 10;
    return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _from))) % max;
  }
  

  function generateRandomId (uint256 base, address owner) internal view returns (uint256 tokenId) {
    require(base <= game.minted(), "Invalid base");
    if (!bank.isThief(base)) {
      return generateRandomId(base + 1, owner);
    } else if (bank.getToken(base).tokenId == 0) {
      return generateRandomId(base + 1, owner);
    } else if (bank.getToken(base).owner == owner) {
      return generateRandomId(base + 1, owner);
    }
    return base;
  }

  function generateRandomApe(address _from) public view returns (uint16 token) {
    uint256 base = generateRandomBase((_from));
    uint256 tokenId = generateRandomId(base, _from);
    return uint16(tokenId);
  }

  function generateDegenMint(address _from) public view returns (bool success) {
    uint256 random = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _from))) % 100;
    return random <= 10;
  }
}
...
contract Traits is Ownable, ITraits {
    ...
    IWorldOfApes public worldOfApes;
    ...
    function setGame(address _worldOfApes) external onlyOwner {
        worldOfApes = IWorldOfApes(_worldOfApes);
    }
    ...
    function drawSVG(uint256 tokenId) public view returns (string memory) {
        IWorldOfApes.WorldOfApesTraits memory s = worldOfApes
            .getTokenTraits(tokenId);
        uint8 shift = s.isApe ? 0 : 10;

        string memory svgString = string(
            abi.encodePacked(
                s.isApe ? draw(thiefBody) : draw(policeBody),
                drawTrait(
        ...
        return
            string(
                abi.encodePacked(
                    '<svg id="WorldOfApes" width="100%" height="100%" version="1.1" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                    svgString,
                    "</svg>"
                )
            );
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    tokenId <= worldOfApes.getPaidTokens()
                        ? '"Gen 0"'
                        : '"Gen 1"',
        ...
        returns (string memory)
    {
        ...
        string memory metadata = string(
            abi.encodePacked(
                ...
                s.isApe ? "Ape #" : "Hunter #",
                tokenId.toString(),
                '", "description": "In a cruel jungle, ruled for years by groups of apes, a ruthless war is brewing for a treasure coveted by all. The $BANANAS. The hunters gather and organize themselves to lead an offensive against the apes. Choose your team and conquer the jungle!", "image": "data:image/svg+xml;base64,',
...
contract WorldOfApes is IWorldOfApes, ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public PAID_PRICE = 50 ether;
    ...
    mapping(uint256 => WorldOfApesTraits) public tokenTraits;
    // mapping from hashed(tokenTrait) to the tokenId it's associated with
    // used to ensure there are no duplicates
    mapping(uint256 => uint256) public existingCombinations;
    // reference to the Bank for choosing random hunter
    Bank public bank;
    // reference to $banana for burning on mint
    ...
    bool public enableDegenMint = true;
    uint16 public degenMintPercent = 11;
    ...
    ) ERC721("World of Apes Token", "APES") {
    ...
    /**
     * mint a token - 90% Apes, 10% Hunters
     * The first 20% are free to claim, the remaining cost $banana
     */
    function mint(uint256 amount, bool stake, bool degen)
        external
        payable
        nonReentrant
        whenNotPaused
    {
        ...
        require(!degen || enableDegenMint, "Degen mint is currently disabled");

        uint256 cost = amount * mintCost(minted);
        bool shouldMint = true;

        if (degen) {
            cost = (cost / 100) * degenMintPercent;
            shouldMint = bank.getRandomGenerator().generateDegenMint(
                msg.sender
            );
        }
        ...
        if (minted < PAID_TOKENS) {
            ...
            require(cost == msg.value, "Invalid payment amount");
        } else {
            require(msg.value == 0);
        }
    ...
    function mintCost(uint256 tokenId) public view returns (uint256) {
        ...
        if (tokenId <= PAID_TOKENS) return PAID_PRICE;
        // $BANANA
    ...
    function generate(uint256 tokenId, uint256 seed)
        internal
        returns (WorldOfApesTraits memory t)
    {
        t = selectTraits(seed);
     ...
     * the remaining 80% have a 10% chance to be given to a random staked hunter
    ...
    function selectRecipient(uint256 seed) internal view returns (address) {
        ...
        address hunter = bank.randomHunterOwner(seed >> 144);
        // 144 bits reserved for trait selection
        if (hunter == address(0x0)) return _msgSender();
        return hunter;
    }
     ...
     */
    function structToHash(WorldOfApesTraits memory s)
        internal
        pure
        returns (uint256)
    {
    ...
    /**
     * called after deployment so that the contract can get random hunter
    ...
    function setPaidPrice(uint256 _paidPrice) external onlyOwner {
        PAID_PRICE = _paidPrice;
    }
    ...
    function toggleDegenMint(bool _toggle) external onlyOwner {
        enableDegenMint = _toggle;
    }
}
