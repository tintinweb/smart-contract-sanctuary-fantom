...
import "./interfaces/deus/VDeusRouter.sol";
import "./interfaces/IWFTM.sol";


contract ZapperVDeus is MatrixSwapHelper, Ownable {
    ...
    address internal constant VDEUS = 0x953Cd009a490176FcEB3a26b9753e6F01645ff28;
    address internal constant VDEUS_DEUS_LP = 0xECd9E18356bb8d72741c539e75CEAdB3C5869ea0;
    address internal constant VDeusRouterAddress = 0x54a5039C403fff8538fC582e0e3f07387B707381;
    address internal constant VDEUS_FACTORY = 0x152eE697f2E276fA89E96742e9bB9aB1F2E61bE3;
    address internal constant SPOOKY_FACTORY = 0x152eE697f2E276fA89E96742e9bB9aB1F2E61bE3;
    ...
    /// @notice SPOOKY ROUTER AS DEFAULT FOR LP TOKENS AND SWAPS
    constructor() MatrixSwapHelper(SPOOKY_ROUTER) {
        /// Default router to put LPs in:
        _setDefaultRouter(SPOOKY_ROUTER);

        // Factory to Router
        factoryToRouter[VDEUS_FACTORY] = VDeusRouterAddress;
        factoryToRouter[SPOOKY_FACTORY] = SPOOKY_ROUTER;

        // Additional allowances
        IERC20(VDEUS).safeApprove(VDeusRouterAddress, 0);
        IERC20(VDEUS).safeApprove(VDeusRouterAddress, type(uint256).max);
        IERC20(DEUS).safeApprove(VDeusRouterAddress, 0);
        IERC20(DEUS).safeApprove(VDeusRouterAddress, type(uint256).max);
    }
    ...
    // ovverride swap for deus -> vdeus and opposite
    function _swap(
        address _fromToken,
        address _toToken,
        uint256 _amount
    ) internal virtual override returns (uint256 _toTokenAmount) {
        if(_fromToken == DEUS && _toToken == VDEUS) {
            uint256 _toTokenBefore = IERC20(_toToken).balanceOf(address(this));

            VDeusRouter(VDeusRouterAddress).swap(1, 0, _amount, 0, block.timestamp);

            _toTokenAmount = IERC20(_toToken).balanceOf(address(this)) - _toTokenBefore;
            return _toTokenAmount;
        }
        if(_fromToken == VDEUS && _toToken == DEUS) {
            uint256 _toTokenBefore = IERC20(_toToken).balanceOf(address(this));

            VDeusRouter(VDeusRouterAddress).swap(0, 1, _amount, 0, block.timestamp);

            _toTokenAmount = IERC20(_toToken).balanceOf(address(this)) - _toTokenBefore;
            return _toTokenAmount;
        }
        if(_toToken == VDEUS) {
            _swap(_fromToken, DEUS, _amount);
            uint256 deusBalance = IERC20(DEUS).balanceOf(address(this));
            uint256 _toTokenBefore = IERC20(_toToken).balanceOf(address(this));

            VDeusRouter(VDeusRouterAddress).swap(1, 0, deusBalance, 0, block.timestamp);

            _toTokenAmount = IERC20(_toToken).balanceOf(address(this)) - _toTokenBefore;
            return _toTokenAmount;
        }

        return super._swap(_fromToken, _toToken, _amount);
    }
        ...
        returns (address)
    {
        ...
        address _factory;
        if(_lpToken == VDEUS_DEUS_LP) {
            _factory = VDEUS_FACTORY;
        } 
        else {
            _factory = IUniswapV2Pair(_lpToken).factory();
        }
    ...
    function _zapToLp(
        ...
        if(_toLpToken == VDEUS_DEUS_LP) {
            _token0 = VDEUS;
            _token1 = DEUS;
        } 
        else {
            _token0 = IUniswapV2Pair(_toLpToken).token0();
            _token1 = IUniswapV2Pair(_toLpToken).token1();
        }
    ...
    function _addLiquidity(
        ...
        if(_token0 == VDEUS && _token1 == DEUS) {
            uint256 lp0Balance = IERC20(VDEUS).balanceOf(address(this));
            uint256 lp1Balance = IERC20(DEUS).balanceOf(address(this));

            uint256[] memory amounts = new uint256[](2);
            amounts[0] = lp0Balance;
            amounts[1] = lp1Balance;

            VDeusRouter(VDeusRouterAddress).addLiquidity(
                amounts,
                0,
                block.timestamp
            );
    ...
    function _removeLiquidity(
        ...
        if(_token0 == VDEUS && _token1 == DEUS) {
            ...
            VDeusRouter(VDeusRouterAddress).removeLiquidity(
                _amountLpIn,
                _minAmounts,
                block.timestamp
            );
    ...
    function _unzapFromLp(
        ...
        if(_fromLpToken == VDEUS_DEUS_LP) {
            _token0 = VDEUS;
            _token1 = DEUS;
        } 
        else {
            _token0 = IUniswapV2Pair(_fromLpToken).token0();
            _token1 = IUniswapV2Pair(_fromLpToken).token1();
        }
...
interface VDeusRouter {
    ...
    function removeLiquidity(uint256 amount, uint256[] calldata minAmounts, uint256 deadline) external;
}
