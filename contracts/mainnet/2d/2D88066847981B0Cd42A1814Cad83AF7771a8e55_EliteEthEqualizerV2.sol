...
// File contracts/EliteEthEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EliteEthEqualizerV2  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToElitePath;
    IEqualizerRouter.Routes[] public equalToEthPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToElitePath,
        IEqualizerRouter.Routes[] memory _equalToEthPath,
        ...
        for (uint i; i < _equalToElitePath.length; ++i) {
            equalToElitePath.push(_equalToElitePath[i]);
        }

        for (uint i; i < _equalToEthPath.length; ++i) {
            equalToEthPath.push(_equalToEthPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToElitePath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalToEthPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(elite).balanceOf(address(this));
        uint256 t2Bal = IERC20(eth).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(elite, eth, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(eth).safeApprove(router, type(uint).max);
    }
