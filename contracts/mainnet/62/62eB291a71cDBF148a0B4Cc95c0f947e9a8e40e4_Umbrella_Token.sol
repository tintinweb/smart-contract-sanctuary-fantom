...
contract Umbrella_Token is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 Umbrella-WETH LP -> 2OMB pool
    uint256 public constant INITIAL_UMBRELLA_POOL_DISTRIBUTION = 140000 ether;
    ...
    address public Umbrella_Oracle;
    ...
    /**
     * @notice Constructs the UMBRELLA ERC-20 contract.
     */

     function initializUmbrellaToken(uint256 _taxRate, address _taxCollectorAddress) public initializer {
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");
        __ERC20_init("Umbrella Token", "UMBRELLA", 18);
    ...
    function _getUmbrellaPrice() internal view returns (uint256 _UmbrellaPrice) {
        try IOracle(Umbrella_Oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Umbrella: failed to fetch 2OMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _UmbrellaPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_UmbrellaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setupUmbrellaOracle(address _UmbrellaOracle) public onlyOperatorOrTaxOffice {
        require(_UmbrellaOracle != address(0), "oracle address cannot be 0 address");
        Umbrella_Oracle = _UmbrellaOracle;
    }
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentUmbrellaPrice = _getUmbrellaPrice();
            currentTaxRate = _updateTaxRate(currentUmbrellaPrice);
            if (currentUmbrellaPrice < burnThreshold) {
                burnTax = true;
            }
        }
     ...
     */
    function distributeReward(address _airdropWallet) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
    }

    function distributeRewardGenesisPool(address _genesisPool) external onlyOperator {
        require(_genesisPool != address(0), "!_genesisPool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
    }

    function distributeRewardUmbrellaPool(address _UmbrellaPool) external onlyOperator {
        require(_UmbrellaPool != address(0), "!_UmbrellaPool");
        rewardPoolDistributed = true;
        _mint(_UmbrellaPool, INITIAL_UMBRELLA_POOL_DISTRIBUTION);
    }
...
contract ERC20 is Context, IERC20 {
     ...
     * construction.
     */
    function __ERC20_init(string memory name_, string memory symbol_ , uint8 _decimal) internal onlyInitializing {
