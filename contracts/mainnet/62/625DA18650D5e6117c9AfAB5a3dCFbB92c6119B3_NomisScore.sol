...
contract NomisScore is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, Pausable, EIP712 {
    ...
    */
     ...
     * @param multichainChainIds Blockchains in which the score is calculated.
     */
    event ScoreChanged(uint256 indexed tokenId, address indexed owner, uint16 score, uint16 calculationModel, uint256 chainId, uint256[] multichainChainIds);
    ...
    */
    constructor(uint256 initialFee, uint16 initialCalcModelsCount, uint16 initialfreeMintCount) ERC721("NomisScore", "NMSS") EIP712("NMSS", "0.8") {
        _tokenIds.reset();
        _mintFee = initialFee;
    ...
    function setScore(
        ...
        uint256 chainId,
        uint256[] memory multichainChainIds
        ...
        bytes32 messageHash = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("SetScoreMessage(uint16 score,uint16 calculationModel,address to,uint256 nonce,uint256 deadline,bytes32 metadataUrl,uint256 chainId,uint256[] multichainChainIds)"),
            score,
            calculationModel,
            msg.sender,
            nonces[msg.sender],
        ...
        scoreStruct.multichainChainIds = multichainChainIds;
        ...
        emit ScoreChanged(scoreStruct.tokenId, msg.sender, score, calculationModel, chainId, multichainChainIds);
    }
    ...
    * @return multichainChainIds Blockchains in which the score is calculated.
    * @return owner The score owner.
    */
    function getScore(address addr, uint256 blockchainId, uint16 calcModel) external view returns(uint16 score, uint256 updated, uint256 tokenId, uint16 calculationModel, uint256 chainId, uint256[] memory multichainChainIds, address owner) {
        ...
        multichainChainIds = _score.multichainChainIds;
        owner = addr;
    }
    ...
    * @return owner The score owner.
    */
    function getScoreByTokenId(uint256 id) external view returns(uint16 score, uint256 updated, uint256 tokenId, uint16 calculationModel, uint256 chainId, uint256[] memory multichainChainIds, address owner) {
