...
import {IVaultParameters} from './Vault/VaultParameters.sol';
import {RoleNames} from '../library/RoleNames.sol';

contract VaultFactory is IVaultFactory, AccessControl {
  ...
  function createVault(IVaultParameters.VaultParams calldata _initParams) public onlyRole(RoleNames.DEPLOYER) returns (IVault _vault) {
    _vault = IVault(
      new Vault{salt: keccak256(abi.encode(_initParams._yieldToken, _yieldTokenVaults[_initParams._yieldToken].length()))}(_initParams)
    );

    _yieldTokens.add(_initParams._yieldToken);
    _yieldTokenVaults[_initParams._yieldToken].add(address(_vault));

    emit FactoryCreatedVault(address(_vault));
  }
...
interface IVaultFactory is IAccessControl {
  event FactoryCreatedVault(address _vault);

  function createVault(IVaultParameters.VaultParams calldata _initParams) external returns (IVault _vault);
