...
contract ZestZapV2 is Ownable, ReentrancyGuard {
    ...
    function zapFromOneSide(
        ...
        // STEP 4: Clean up dust
        if (_transferResidual) {
            if (_targetReceived > _tokenA)
                IERC20(_token1).safeTransfer(msg.sender, _targetReceived - _tokenA);
            if (_tokenRemaining > _tokenB)
                IERC20(_token0).safeTransfer(msg.sender, _tokenRemaining - _tokenB);
        }
    }
