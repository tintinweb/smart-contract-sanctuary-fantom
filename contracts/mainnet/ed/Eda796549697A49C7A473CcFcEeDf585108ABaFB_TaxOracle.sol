...
|_|  |_|\_, |_|_\__,_|_.__/_.__/_|\__(_)_|\__,_|_| |_|_|_|
  ...
  Inspired by http://rabbit.finance
*/
...
contract TaxOracle is Ownable {
    ...
    constructor(
        ...
        rabbit = IERC20(_rabbit);
        wftm = IERC20(_wftm);
        pair = _pair;
    }

    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(rabbit), "token needs to be rabbit");
        uint256 rabbitBalance = rabbit.balanceOf(pair);
        uint256 wftmBalance = wftm.balanceOf(pair);
        return uint144(rabbitBalance.mul(_amountIn).div(wftmBalance));
    }
