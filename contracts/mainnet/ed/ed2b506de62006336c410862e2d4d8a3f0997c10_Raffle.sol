...
contract Raffle is Ownable {
    ...
    uint256 discountPrice = (ticketPrice.mul(9)).div(10);
    uint raffleEndTime;
    ...
    function getUtilityPrizePool() public view returns (uint256) { return prizePool ; }
    ...
    function buyTicket(uint256 amount) public {
        ...
        IERC20(erc20Token).transferFrom(msg.sender, address(this), totalAmount);
            ...
            split = totalAmount.div(4);
            uint256 devFeeSplit = split.div(5);
        ...
        uint256 discountDifference = discountPrice.mul(amount);

        IERC20(utilityToken).transferFrom(msg.sender, address(this), totalDiscounted);

            uint256 finalTotal = (totalDiscounted.mul(9)).div(10);
            uint256 burnAmount = discountDifference.sub(finalTotal);
            erc20burner.burnFrom(msg.sender, burnAmount);
            burnedTokens += burnAmount;

            split = totalDiscounted.div(4);
            uint256 devFeeSplit = split.div(5);
            uint256 protocolFeeSplit = split.sub(devFeeSplit);
            uint256 prizePoolSplit = totalDiscounted.sub(split);
