...
contract Granary
{
	...
	constructor (address _w, address _m, uint8 _p, string memory _id)
	{
		want=IERC20(_w);
		mc=IMasterchef(_m);
		earn=want;
	...
	*/
	...
	function tvl() public view returns(uint256)
	{
		ITVL tc = ITVL(utvl);
		uint256 aum = mc.userInfo(allnums[0],address(this)).amount + IERC20(want).balanceOf(address(this));
		return ((tc.p_t_coin_usd(address(0x8d9221A0cd46ba97DbC303397447BB983925364B))).mul(aum)).div(1e18);
	}
