...
// Forked from: Votium Merkle Stash
...
contract MultiMerkleStash is Ownable {
  ...
  // the current merkleRoot for a token address
  mapping(address => bytes32) public merkleRoot;
  // Tracks the "version" of the merkleRoot for an address, used for tracking claims
  // across different merkle roots for the same token
  mapping(address => uint256) public update;
  // This is a packed array of booleans.
  // Tracks where an address has claimed given some token and "version" of the merkle root
  ...
  function updateMerkleRoot(address token, bytes32 _merkleRoot) public onlyOwner {
    // Increment the update (simulates the clearing of the claimedBitMap by updating the "version" of the merkle root)
    update[token] += 1;
