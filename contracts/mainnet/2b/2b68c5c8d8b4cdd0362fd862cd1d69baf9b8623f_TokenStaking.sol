...
contract TokenStaking {
  ...
  function EditLockingPeriod(uint256 HowManyBlocks) public {
    require(msg.sender == admin, "You aren't the admin so you can't press this button");
  ...
  function EditLockReward(uint256 HowManyTokens) public {
    require(msg.sender == admin, "You aren't the admin so you can't press this button");
  ...
  function SweepToken(ERC20 TokenAddress) public {
    require(msg.sender == admin, "You aren't the admin so you can't press this button");
    require(
      TokenAddress != Token || TokenAddress != RewardToken,
      'You cannot sweep a token that is being used by the contract'
    );
  ...
  function deposit(uint256 amount) public {
    require(
      ...
      'You do not have the right NFT in order to use this contract'
    );
  ...
  function CalculateTime(address YourAddress) public view returns (uint256) {
    uint256 Time = (block.timestamp - LockTimestamp[YourAddress]);
    if (LockTimestamp[YourAddress] == block.timestamp) {
      Time = 0;
    }
  ...
  function CalculateReward(address Who) public view returns (uint256) {
