...
contract RebateTreasury is Ownable {
    ...
    uint256 public buybackAmount = 50 * 1e4;
    ...
    constructor(address degen, address degenOracle, address treasury) {
        Degen = IERC20(degen);
        DegenOracle = IOracle(degenOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Degen at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 degenAmount = getDegenReturn(token, amount);
        require(degenAmount <= Degen.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient degen balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + degenAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += degenAmount;
    }

    // Claim available Degen rewards from bonding
    ...
    function setDegen(address degen) external onlyOwner {
        Degen = IERC20(degen);
    }

    // Set Degen oracle

    function setDegenOracle(address oracle) external onlyOwner {
        DegenOracle = IOracle(oracle);
    }
    ...
    function _claimVested(address account) internal {
        ...
        Degen.transfer(account, claimable);
    }
    ...
    // Calculate Degen return of bonding amount of token

    function getDegenReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 degenPrice = getDegenPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / degenPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 degenPrice = getDegenPrice();
        if (degenPrice < 1e18) return 0;

        uint256 degenPremium = degenPrice * DENOMINATOR / 1e18 - DENOMINATOR;
        if (degenPremium < bondThreshold) return 0;
        if (degenPremium <= secondaryThreshold) {
            return (degenPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (degenPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get DEGEN price from Oracle

    function getDegenPrice() public view returns (uint256) {
        return DegenOracle.consult(address(Degen), 1e18);
    }
    ...
    // Get claimable vested Degen for account

    function claimableDegen(address account) external view returns (uint256) {
