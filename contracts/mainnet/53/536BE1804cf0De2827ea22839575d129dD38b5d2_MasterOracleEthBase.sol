...
contract MasterOracleEthBase is Ownable {
    ...
    function getYTokenPrice() public view returns (uint256) {
        return uint256(int256(oracleYToken.spot(yToken, PRICE_PRECISION)) / EthFtmPrice());
    }
    ...
    function getYTokenTWAP() public view returns (uint256) {
        return uint256(int256(oracleYToken.twap(yToken, PRICE_PRECISION)) / EthFtmPrice());
    }

    function EthFtmPrice() public view returns (int256) {
        return priceConverter.getDerivedPrice(ethUsdFeed, ftmUsdFeed, 18);
    }
}