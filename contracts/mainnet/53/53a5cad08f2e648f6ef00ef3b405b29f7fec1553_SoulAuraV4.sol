...
contract SoulAuraV4 is Ownable {

    ISummoner SummonerContract = ISummoner(SummonerAddress);
    IStake AutoStakeContract = IStake(AutoStakeAddress);
    ...
    address public SummonerAddress;
    address public AutoStakeAddress;
    ...
    constructor() {
        ...
        AutoStakeAddress = 0x083423C61B9373050e62E2A6Ec170e663F9c7BFa;
        DaoAddress = 0x1C63C726926197BD3CB75d86bCFB1DaeBcD87250;
    }
    ...
    function balanceOf(address member) public view returns (uint) {
        ...
        ( uint memberStaked, ) = stakedPower(member);
        ( uint memberSoul, ) = soulPower(member);
        ( uint memberSeance, ) = seancePower(member);

        return memberLiquidity +memberStaked + memberSoul + memberSeance;
    }
...
// gets: member's stakedPower
    function stakedPower(address member) public view returns (uint raw, uint formatted) {
        // staked power is the members' staked balance in the AutoStakeContract.
        uint staked_power = AutoStakeContract.balanceOf(member);
    ...
    function pooledSoul(address member) public view returns (uint raw, uint formatted) {
        ...
        (uint staked_soulFtm, ) = SummonerContract.userInfo(soulFtmPid, member);
        (uint staked_soulUsdc, ) = SummonerContract.userInfo(soulUsdcPid, member);
    ...
    function pooledSeance(address member) public view returns (uint raw, uint formatted) {
        ...
        // member | staked LP balance
        (uint staked_seanceFtm, ) = SummonerContract.userInfo(seanceFtmPid, member);
        (uint staked_seanceUsdc, ) = SummonerContract.userInfo(seanceUsdcPid, member);
    ...
    function updateAutoStake(address _AutoStakeAddress) public onlyOwner {
        AutoStakeAddress = _AutoStakeAddress;
    }

    function updateSummoner(address _SummonerAddress) public onlyOwner {
        SummonerAddress = _SummonerAddress;
    }
