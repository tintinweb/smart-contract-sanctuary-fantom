...
contract DeadPool is Ownable, ReentrancyGuard {
    ...
    // amount of reward token / (365d * 24h * 60min * 60s)
    uint256 public rewardPerSecond = 0.00095 * 10**6;
    
    uint256 constant runningTime = 365 days;
    ...
    event AllocationAdjusted (uint256 indexed pid, uint256 allocPoints);
    event RewardTokenChanged (IERC20 indexed oldReward, IERC20 indexed newReward);
    ...
    constructor(
        ...
        require(_rewardToken != address(0), "cant be uninitialized");
        require(_protocolFund != address(0), "cant be uninitialized");

        RewardToken = ERC20(_rewardToken);
        //rewardPerSecond = 0.00095 *  10**RewardToken.decimals();
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "DeadPool: not operator");
        _;
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _reward = _multiplyHelper / (totalAllocPoint);
            accRewardPerShare = accRewardPerShare + (_reward * (1**RewardToken.decimals()) / (tokenSupply));
        }
        return user.amount * (accRewardPerShare) / (1**RewardToken.decimals()) - (user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.timestamp <= pool.lastRewardTime) {
            ...
            uint256 _reward = multiplyHelper / (totalAllocPoint);
            pool.accRewardPerShare = pool.accRewardPerShare + (_reward * (1**RewardToken.decimals()) / (tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant  {
        ...
        if (user.amount > 0) {
            ...
            uint256 _pending = user.amount * (pool.accRewardPerShare) / (1**RewardToken.decimals()) - (user.rewardDebt);
            if (_pending > 0) {
        ...
        user.rewardDebt = user.amount * (pool.accRewardPerShare) / (1**RewardToken.decimals());
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        require (RewardToken.balanceOf(address(this)) >= _amount, "not enough reward in contract");
        RewardToken.safeTransfer(_to, _amount);
    }
//===================================== ONLY OPERATOR FUNCTIONS =======================================================

 // Add a new pool. Can only be called by operator.
    // @ _allocPoint - amount of rewards this pool will emit
    ...
    function setAllocations(uint256 _pid, uint256 _allocPoint) public onlyOperator {
        massUpdatePools();
        PoolInfo storage pool = poolInfo[_pid];
        ...
        emit AllocationAdjusted(_pid, _allocPoint);
    }
...
// should run this function only after new Rewards are transferred to the contract
function setNewRewardToken (address _newToken) external onlyOperator {
    RewardToken = ERC20(_newToken);
    emit RewardTokenChanged(RewardToken, IERC20(_newToken));
     rewardPerSecond =  RewardToken.balanceOf(address(this)) / (poolEndTime - block.timestamp);
}
  ...
  function moveRewardsToUpdatedContract(
        ...
        require(address(_token) == address(RewardToken), "reward token only");
  ...
  function governanceRecoverUnsupported(
        ...
        address _to
           ...
           if (block.timestamp < poolEndTime + 30 days) {
            require(_token != RewardToken, "reward token");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
