...
interface Diamond {
    ...
    function withdrawableDividendOf(address account, bool nftContract) external view returns(uint256);
    function withdrawnDividendOf(address account, bool nftContract) external view returns(uint256);
...
contract FantoonNFT is ERC721Enumerable, Ownable{
    ...
    uint256 private addPriceIncrease = 0; //0.3 ether;

    uint256 private max_Kittens = 300;
    uint256 private max_TokenCountOfAccount = 10;
    
    address private developerWallet;
    bool private enableMintTax = true;
    uint256 private mintTaxRateForDeveloper = 5000;            // 50%
    ...
    mapping(address => uint256) private _tokenCountOfAccount;
    ...
    uint256 private currentReservedTokenId = 0;
    uint256 private reservePrice = 0.01 ether;
    uint256 private reservedPeriod = 5 minutes; //24 hours;                 // 24 hours
    uint256 private mintReservedTokenPeriod = 5 minutes; //24 hours;        // 24 hours
    uint256 private initialTime;
    uint256 private initialPrice = 0.1 ether;

    uint256 private amountOfOGShareDistribute;
    uint256 private amountOfOGShareDistributePumpkitten;
    uint256 private amountOfOGShareDistributeBitkitten;
    uint256 private amountOfOGShareDistributeSamurai;
...
*/
    ...
    mapping (address => uint256) private mapRewardsForAccount;
    ...
    constructor(address _diamond) ERC721("TexiTails", "TXT") {
        diamond = _diamond;
    ...
    function setMintTaxRateForDeveloper(uint256 _rate) public onlyOwner {
    ...
    function buyKittens() public payable lock {
        ...
        if ((block.timestamp - initialTime) <= mintReservedTokenPeriod) {
            for (uint256 i=0; i<max_TokenCountOfAccount; i++) {
                uint256 tokenId = _reservedInfo[_msgSender()].tokenId[i];
            ...
            if (initialPrice > 0) {
             ...
             for (uint256 i=1; i<= max_Kittens; i++) {
        ...
        if (enableOGSharedistribute) {
            uint256 amount = previousPrice * 1500 / 10000;     // send 15% * 3 to 3 NFT holders
            sendDividendsforNFTHolders(amount);

            amount = previousPrice - mintTaxAmount - amount * 3; // send 5% ( = 100% - 50% - 15% * 3)to Diamond holders
        ...
        if (enableDistributeShare) {
            address account;
            uint256 amount;
            uint256 nlength = _distributeShareInfo.length;
    ...
    /**
     * @notice Send transaction fee to nft holders.
     */
    function sendDividendsforNFTHolders(uint256 _amount) internal {
        uint256 length = ERC721Enumerable(addressOfPumpkitten).totalSupply();
        uint256 share = _amount / length;

        address holder;
        uint256 tokenId;

        for (uint256 i=0; i<length; i++) {
            tokenId = ERC721Enumerable(addressOfPumpkitten).tokenByIndex(i);
            holder = ERC721Enumerable(addressOfPumpkitten).ownerOf(tokenId);
            mapRewardsForAccount[holder] += share;
        }

        length = ERC721Enumerable(addressOfBitkitten).totalSupply();
        share = _amount / length;

        for (uint256 i=0; i<length; i++) {
            tokenId = ERC721Enumerable(addressOfBitkitten).tokenByIndex(i);
            holder = ERC721Enumerable(addressOfBitkitten).ownerOf(tokenId);
            mapRewardsForAccount[holder] += share;
        }
        ...
        for (uint256 i=0; i<length; i++) {
            tokenId = ERC721Enumerable(addressOfSamuraikitten).tokenByIndex(i);
            holder = ERC721Enumerable(addressOfSamuraikitten).ownerOf(tokenId);
            mapRewardsForAccount[holder] += share;
        }
    }
     ...
     */
    function sendDividendsforDiamondHolders(uint256 _amount) internal {
        Diamond(diamond).sendDividends(_amount, true);
    }
    
    function withdrawableFTM(address _account) public view returns (uint256){
        require(_account != address(0), "Wrong address");

        if (_excludedHolders[_account] == true )
            return 0;

        uint256 withdrawableAmount = Diamond(diamond).withdrawableDividendOf(_account, true);

        return mapRewardsForAccount[_account] + withdrawableAmount;
    }

    function withdrawnFTM(address _account) public view returns (uint256){
        require(_account != address(0), "Wrong address");
    ...
    function claimFTM() external lock {
        ...
        uint256 withdrawableAmount = Diamond(diamond).withdrawableDividendOf(msg.sender, true);
        
        require(mapRewardsForAccount[msg.sender] + withdrawableAmount < address(this).balance, "Insufficient balance.");
        
        require(payable(msg.sender).send(mapRewardsForAccount[msg.sender] + withdrawableAmount));

        _claimedFTM[msg.sender] += (mapRewardsForAccount[msg.sender] + withdrawableAmount);
    ...
    function setExcludedTokenListOGShareDistribute(uint256 tokenId, address nft) public onlyOwner {
    ...
    function addReserveToken(address account) public onlyOwner {
        ...
        uint256 currentReservedCount = _reservedInfo[account].tokenId.length;
        require(currentReservedCount < max_TokenCountOfAccount, "Too Many Tokens");

        uint256 addLength = max_TokenCountOfAccount - currentReservedCount;
        uint256 index;
        for (uint256 i=1; i<=max_Kittens; i++) {
            if (_tokenStatus[i] != Status.Pending)
                continue;
    ...
    function addReserveRandomTokens(uint256[] memory tokenIds, address account) public onlyOwner {
    ...
    function deleteReserveToken(address account) public onlyOwner {
        uint256 length = _reservedInfo[account].tokenId.length;
    ...
    function getReservedTokenPrice(address account) public view returns (uint256) {
        uint256 length = _reservedInfo[account].price.length;
    ...
    function setAddPriceIncrease(uint256 _increase) public onlyOwner {
        addPriceIncrease = _increase;
    }
    
    function getAddPriceIncrease() public view returns(uint256) {
    ...
    function setMaxKittens(uint256 amount) public onlyOwner {
        max_Kittens = amount;
    }
    
    function setMaxTokenCountOfAccount(uint256 amount) public onlyOwner {
        max_TokenCountOfAccount = amount;
    }
    
    function getTokenIdsOfOwner(address account) public view 
        returns(uint256[] memory tokenIds, uint256[] memory priceOfTokenIds, string[] memory tokenURIs)
    {
        uint256 balance = balanceOf(account);
        tokenIds = new uint256[](balance);
        priceOfTokenIds = new uint256[](balance);
    ...
    function addListForDistributeShare(address _account, uint256 _share) public onlyOwner {
    ...
    function setReservePeriod(uint256 _period) public onlyOwner {
        reservedPeriod = _period;
    }
    
    function setMintReservedTokenPeriod(uint256 _period) public onlyOwner {
