...
contract MultiFeeDistribution is ReentrancyGuard, Ownable {
    ...
    // 2 years
    uint256 public constant lockDuration = rewardsDuration * 105;
    ...
    function mintUnlocked(address _address, uint _amount) public{
    ...
    function mintLocked(address _address, uint _amount) public{
        require(minters[msg.sender]);
        totalSupply = totalSupply.add(_amount);
        Balances storage bal = balances[_address];
        bal.total = bal.total.add(_amount);
    ...
    function mintVested(address _address, uint _amount) public{
        require(minters[msg.sender]);
        MultiFeeDistribution(address(this)).mint(_address, _amount);
    }
