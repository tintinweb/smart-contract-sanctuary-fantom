...
contract NewVault is INewVault, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  ...
  modifier transferTokenToVault(uint256 value) {
    if (msg.value != 0) {
      require(token == config.getWrappedNativeAddr(), "bToken !wNative");
  ...
  modifier inExec() {
    require(POSITION_ID != _NO_ID, "!within execution scope");
    require(STRATEGY == msg.sender, "!from the strategy");
  ...
  //Addition Varible
  uint256 HEALTH_BEFORE;
  uint256 HEALTH_AFTER;
  ...
  function work(
    ...
    require(meowMiningPoolId != uint256(-1), "pId !set");
    ...
    if (id == 0) {
      ...
      pos = positions[id];
      require(id < nextPositionID, "bad id");
      require(pos.worker == worker, "bad worker");
      require(pos.owner == msg.sender, "!owner");
      require(IWorker(worker).health(id) != 0, "!active position");
    ...
    require(config.isWorker(worker), "!worker");
    require(loan == 0 || config.acceptDebt(worker), "worker !accept more debt");
    ...
    if (debt > 0) {
      ...
      uint256 killFactor = config.killFactor(worker, debt);
      if (HEALTH_BEFORE == 0) {
        require(HEALTH_AFTER.mul(workFactor) >= debt.mul(10000), "bad work factor");
      } else {
        require(HEALTH_AFTER.mul(killFactor) > debt.mul(10000), "debtRatio > killFactor");
      }
      _addDebt(id, debt);
  ...
  function kill(uint256 id) external onlyWhitelistedBots accrue(0) nonReentrant {
    require(meowMiningPoolId != uint256(-1), "poolId !set");
  ...
  function takeProfitORstopLoss(string memory mode, uint256 id) external onlyWhitelistedBots accrue(0) nonReentrant {
    ...
    uint256 debt = _removeDebt(id);
    uint256 health = IWorker(pos.worker).health(id);
    if (_compareStrings(mode, "TP")) require(health.mul(pos.takeProfit) >= debt.mul(10000), "can't TP");
    else require(health.mul(pos.stopLoss) <= debt.mul(10000), "can't SL");
    // 3. Perform liquidation and compute the amount of token received.
    uint256 beforeToken = SafeToken.myBalance(token);
    IWorker(pos.worker).liquidate(id);
    uint256 back = SafeToken.myBalance(token).sub(beforeToken);
  ...
  function setValut2(address valut2) external onlyOwner {
  ...
  function _transferMMeowFeeToVault(uint256 _id) internal {
    ...
    if (mMeowFeeAmt > 0) {
      ...
      require(SafeToken.balanceOf(mMeowToken, msg.sender) > mMeowFeeAmt, "fee !enough");
