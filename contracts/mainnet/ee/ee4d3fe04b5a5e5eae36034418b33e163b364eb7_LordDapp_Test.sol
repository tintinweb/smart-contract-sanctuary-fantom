...
contract LordDapp_Test is IERC20
{
    ...
    mapping(uint8=>uint) HoldingUser;
    ...
    address constant public owner=0x5eC0DB646281020464d7691a4a778bAbcafDE2a8;
    address constant public marketingAddress=0xd8b67fEEc37ecb7C23271b550CC11571E8EFD915;
    ...
    bool IsHoldingDistribution=true;
    ...
    function updateRank(uint _memberId) internal
    {
        ...
        if(map_Users[_memberId].Business>=map_ranks[nextRank].CumBusiness*(1 ether) && currentRank<8){
    ...
    function holdingWithdraw(uint tokenAmount) public
    {
        ...
        uint fmtAmount = tokensToFTM(tokenAmount);
        ...
        if(deduction>0)
        {
            if(deductionPercentage == 80 && IsHoldingDistribution)
            {
                distributeHoldingToMember(deduction/2);
                payable(owner).transfer(deduction/2);
            }
            else 
            {
                payable(owner).transfer(deduction);
    ...
    function distributeHoldingToMember(uint distributionAmount) public 
    {
        uint totalUser100 = 0;
        uint totalUser1000 = 0;
        uint totalUser5000 = 0;
        for(uint i=1; i<=TotalUser; i++)
        {
            if(map_Users[i].Investment>=1*(1 ether) && map_Users[i].Investment<10*(1 ether))
            {
                totalUser100=totalUser100+1;
            }
            else if(map_Users[i].Investment>=10*(1 ether) && map_Users[i].Investment<50*(1 ether))
            {
                totalUser1000=totalUser1000+1;
            }
            else if(map_Users[i].Investment>=50*(1 ether))
            {
                totalUser5000=totalUser5000+1;
            }
        }
        uint disAmount100=0;
        uint disAmount1000=0;
        uint disAmount5000=0;  
        if(totalUser100>0)
        {
            disAmount100=distributionAmount*20/(100*totalUser100);
        }
        else 
        {
            disAmount100=distributionAmount*20/100;
            map_Users[1].HoldingWithdrawIncome+=disAmount100;
        }
        if(totalUser1000>0)
        {
            disAmount1000=distributionAmount*30/(100*totalUser1000);
        }
        else 
        {
            disAmount1000=distributionAmount*30/100;
            map_Users[1].HoldingWithdrawIncome+=disAmount1000;
        }
        if(totalUser5000>0)
        {
            disAmount5000=distributionAmount*50/(100*totalUser5000);            
        }
        else 
        {
            disAmount5000=distributionAmount*50/100;
            map_Users[1].HoldingWithdrawIncome+=disAmount5000;
        }             
        for(uint i=1; i<=TotalUser; i++)
        { 
            if(map_Users[i].Investment>=1*(1 ether) && map_Users[i].Investment<10*(1 ether))
            {
                map_Users[i].HoldingWithdrawIncome+=disAmount100;
            }
            else if(map_Users[i].Investment>=10*(1 ether) && map_Users[i].Investment<50*(1 ether))
            {
                map_Users[i].HoldingWithdrawIncome+=disAmount1000;
            }
            else if(map_Users[i].Investment>=50*(1 ether))
            {
                map_Users[i].HoldingWithdrawIncome+=disAmount5000;
            }
        }
    }
    ...
    function changeLock() external {
        ...
        IsHoldingDistribution = !IsHoldingDistribution;
    }
    ...
    {
        return getMemberTotalSecurity(memberId)+map_Users[memberId].HoldingWithdrawIncome - map_Users[memberId].SecurityWithdrawn;
    }
