...
contract DeltaNeutralVaultGateway is
  ...
  OwnableUpgradeable
{
  ...
  function withdraw(
    ...
    uint256 _stableTokenBack = _getBalance(_stableToken) -
      _stableTokenBalanceBefore;
    uint256 _assetTokenBack = _getBalance(_assetToken) -
      _assetTokenBalanceBefore;
    if (
      _stableTokenBack < _minSwapStableTokenAmount ||
      _assetTokenBack < _minSwapAssetTokenAmount
    ) {
  ...
  function _transferShareToVaultGateway(uint256 _amount) internal {
    IERC20Upgradeable(address(deltaNeutralVault)).safeTransferFrom(
      msg.sender,
      address(this),
      _amount
    );
  ...
  function _getTokenPrice(address _token) internal returns (uint256) {
    (uint256 _price, uint256 _lastUpdated) = deltaNeutralVault
      .priceOracle()
      .getTokenPrice(_token);
    // _lastUpdated > 1 day revert
    if (block.timestamp - _lastUpdated > 86400)
      revert DeltaNeutralVaultGateway_UnTrustedPrice();
    return _price;
  }
  ...
  function _adjustTokenAmount(
    ...
    uint256 _withdrawStableAmountInUSD = ((_withdrawStableAmount * (1e18)) /
      (10**_stableTokenDecimals)).mulWadDown(_stableTokenPrice);
    uint256 _withdrawAssetAmountInUSD = ((_withdrawAssetAmount * (1e18)) /
      (10**_assetTokenDecimals)).mulWadDown(_assetTokenPrice);
    ...
    if (_withdrawStableAmountInUSD > _expectedStableInUSD) {
      uint256 _swapStableAmount = (_withdrawStableAmountInUSD -
        _expectedStableInUSD).divWadDown(_stableTokenPrice);
    ...
    if (_withdrawAssetAmountInUSD > _expectedAssetInUSD) {
      uint256 _swapAssetAmount = (_withdrawAssetAmountInUSD -
        _expectedAssetInUSD).divWadDown(_assetTokenPrice);
  ...
  function _swap(address _token, uint256 _swapAmount) internal {
    IDeltaNeutralVaultConfig config = IDeltaNeutralVaultConfig(
      deltaNeutralVault.config()
    );
    ...
    if (_token0 == _nativeToken || _token1 == _nativeToken) {
      if (_token0 == _nativeToken) {
        _router.swapExactETHForTokens{ value: _swapAmount }(
          0,
          _path,
          address(this),
          block.timestamp
