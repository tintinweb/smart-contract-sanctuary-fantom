...
contract Masonry is AssetsWrapper, ContractGuard {
    ...
    modifier masonExists {
        require(shareBalanceOf(msg.sender) > 0, "Masonry: The mason does not exist");
        _;
    }
    ...
    function initialize(
        ...
        shareHoldersRewardPerc = 4000;
        // 40% Reward goes to share Holders
        alphaHoldersRewardPerc = 6000;
    ...
    function getTombPrice() external view returns (uint256) {
        return betaTreasury.getTombPrice();
    }
    ...
    function stake(uint256 amount, bool isShare) public onlyOneBlock updateReward(msg.sender) {
        ...
        if (isShare) {
            super.stakeShare(amount);
        } else {
            super.stakeAlpha(amount);
        }
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        ...
        require(address(_token) != address(beta), "tomb");
