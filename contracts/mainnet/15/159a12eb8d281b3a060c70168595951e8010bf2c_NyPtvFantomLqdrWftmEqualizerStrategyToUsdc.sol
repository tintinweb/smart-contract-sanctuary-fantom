...
contract NyPtvFantomLqdrWftmEqualizerStrategyToUsdc is NYBaseProfitTakingStrategyV3 {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  address public constant EQUALIZER_ROUTER = address(0xa132DAB612dB5cB9fC9Ac426A0Cc215A3423F9c9);
  ...
  function initialize(
    ...
    equalToUsdcPath = [EQUAL, WFTM, USDC];
    rewards.push(EQUAL);
  }
  ...
  function _deposit() internal override {
    ...
    if (underlyingBalance != 0) {
      uint256 wantId = IVeloGauge(GAUGE).tokenIds(LP_TOKEN);
      IERC20Upgradeable(LP_TOKEN).safeIncreaseAllowance(GAUGE, underlyingBalance);
      IVeloGauge(GAUGE).deposit(underlyingBalance, wantId);
    }
  }
  ...
  function _swapFarmEmissionTokens() internal {
    ...
    if (equalToUsdcPath.length >= 2 && equalBalance > 0) {
      equal.safeIncreaseAllowance(EQUALIZER_ROUTER, equalBalance);
      IVeloRouter router = IVeloRouter(EQUALIZER_ROUTER);

      (, bool useStable) = router.getAmountOut(equalBalance, EQUAL, USDC);
      IVeloRouter.route[] memory routes = new IVeloRouter.route[](1);
      routes[0] = IVeloRouter.route({from: EQUAL, to: USDC, stable: useStable});
      router.swapExactTokensForTokens(equalBalance, 0, routes, address(this), block.timestamp);
    }
  }
