...
contract WigoBtcFtm is Ownable, Pausable {
    ...
    address[] public wrappedToBtcPath;
    ...
    event SetWrappedToBtcPath(address[] indexed path);
    ...
    function addLiquidity() internal {
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToBtcPath, address(this), block.timestamp);
    
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(btc).balanceOf(address(this));

        IUniSwapRouter(unirouter).addLiquidity(wrapped, btc, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function setOutputToWrappedRoute(address[] calldata _path) external onlyAdmin {
    ...
    function setWrappedToLp0Route(address[] calldata _path) external onlyAdmin {
        require (_path[0] == wrapped && _path[_path.length - 1] == btc, "Bad path || !auth");

        wrappedToBtcPath = _path;

        emit SetWrappedToBtcPath(_path);
    }
