...
contract Reaper is ERC20Burnable, Operator {
    using SafeMath for uint256;

    // TOTAL MAX SUPPLY = 100,000 Reapers
    ...
    constructor(address _communityFund, address _devFund1, address _devFund2) public ERC20("iReaper", "Reaper") {
        _mint(msg.sender, 1 ether); // mint 1 Reaper for initial pools deployment
    ...
    function claimRewards() external {
        ...
        if (_pending > 0 && devFund1 != address(0) && devFund2 != address(0)) {
            _mint(devFund1, _pending.div(2));
            _mint(devFund2, _pending.div(2));
     ...
     */
    function distributeReward(uint256 _poolStartTime, address _lpPoolAddress) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_lpPoolAddress != address(0), "!_lpPoolAddress");
        rewardPoolDistributed = true;
        startTime = _poolStartTime;
        endTime = startTime + VESTING_DURATION;

        communityFundLastClaimed = startTime;
        devFundLastClaimed = startTime;
        _mint(_lpPoolAddress, FARMING_POOL_REWARD_ALLOCATION);
    }
