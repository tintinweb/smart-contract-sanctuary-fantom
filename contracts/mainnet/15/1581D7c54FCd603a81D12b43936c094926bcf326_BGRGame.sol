...
contract BGRGame is Ownable {
    ...
    event EarlyAccessEndTimeUpdated(uint256 oldValue, uint256 newValue);
    event DepositsEndTimeUpdated(uint256 oldValue, uint256 newValue);
    event ClaimsEndTimeUpdated(uint256 oldValue, uint256 newValue);
    ...
    function setEarlyAccessEndTime(uint256 earlyAccessEndTime_)
        public
        onlyOwner
    {
        require(
        ...
        emit EarlyAccessEndTimeUpdated(earlyAccessEndTime, earlyAccessEndTime_);
        earlyAccessEndTime = earlyAccessEndTime_;
    }

    function setDepositsEndTime(uint256 depositsEndTime_) public onlyOwner {
        require(
            depositsEndTime_ >= earlyAccessEndTime + MINIMUM_TRANSITION_TIME,
            "Deposit End Time Check Failed"
        );

        emit DepositsEndTimeUpdated(depositsEndTime, depositsEndTime_);
        depositsEndTime = depositsEndTime_;
    }

    function setClaimsEndTime(uint256 claimsEndTime_) public onlyOwner {
        require(
            claimsEndTime_ >= depositsEndTime + MINIMUM_TRANSITION_TIME,
            "Claims End Time Check Failed"
        );

        emit ClaimsEndTimeUpdated(claimsEndTime, claimsEndTime_);
    ...
    function setWinningScore(
        ...
        // TODO else, The house "won" this square, since it was unsold.  Pay the contract instead.
        // Requires the contract to fund unsold squares
        // The other option is to fail if there are unsold squares.
    ...
    function enterGameMany(uint8 many) public payable {
        checkEnterGame(many);
    ...
    function enterGameNft(address contractAddress, uint256 tokenId)
        ...
        payable
    {
        // TODO -- disallow entering a game during early access without a NFT
        require(
            nfts.useNft(contractAddress, tokenId, _msgSender()),
            "NFT invalid or already used"
        );
...
library NFTEarlyAccess {
    ...
    function useNft(
        ...
        if (nftContract.usedTokens[tokenId] == false) {
            nftContract.usedTokens[tokenId] = true;
            // TODO -- replace the below with a call to the contract, to verify the sender is the owner of the token id
        ...
        returns (NFTContract storage)
    {
        ...
        revert("Did not find NFT contract");
    }
}