...
import "@thirdweb-dev/contracts/drop/DropERC1155.sol"; // For my collection of Unknowns
...
contract Mining is ReentrancyGuard, ERC1155Holder {
    ...
    constructor(DropERC1155 unknownContractAddress, TokenERC20 gemsContractAddress) {
        unknownNftCollection = unknownContractAddress;
    ...
    // Mapping of player addresses to their current unknown
    // By default, player has no unknown. They will not be in the mapping.
    // Mapping of address to unknown is not set until they stake a one.
    // In this example, the tokenId of the unknown is the multiplier for the reward.
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        require(unknownNftCollection.balanceOf(msg.sender, _tokenId) >= 1, "Anda harus memiliki setidaknya 1 token yang Anda coba staked");

        // If they have a unknown already, send it back to them.
        ...
        // Transfer the unknown to the contract
        unknownNftCollection.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "Stake token Anda");

        // Update the playerUnknown mapping
    ...
    function withdraw() external nonReentrant {
        // Ensure the player has a unknown
        require(playerUnknown[msg.sender].isData, "Anda tidak memiliki token untuk claim.");
    ...
    // If playerLastUpdate or playerUnknown is not set, then the player has no rewards.
