...
contract Code77 is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 15000 ether;
    // Initial distribution for the day 2-5 CODE77-WFTM LP -> CODE77 pool
    // uint256 public constant INITIAL_CODE77_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the CODE77 ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("CODE77", "CODE77") {
        // Mints 1 CODE77 to contract creator for initial pool setup
    ...
    function _getCode77Price() internal view returns (uint256 _code77Price) {
        try IOracle(code77Oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("CODE77: failed to fetch CODE77 price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _code77Price) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_code77Price >= taxTiersTwaps[tierId]) {
    ...
    function setCode77Oracle(address _code77Oracle) public onlyOperatorOrTaxOffice {
        require(_code77Oracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CODE77 to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CODE77 to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCode77Price = _getCode77Price();
            currentTaxRate = _updateTaxRate(currentCode77Price);
            if (currentCode77Price < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        //require(_code77Pool != address(0), "!_code77Pool");
        require(_daoFund != address(0), "!_treasury");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        //_mint(_code77Pool, INITIAL_CODE77_POOL_DISTRIBUTION);
