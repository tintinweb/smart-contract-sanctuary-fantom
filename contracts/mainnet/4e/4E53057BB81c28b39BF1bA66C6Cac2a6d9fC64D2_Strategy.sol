...
import {IBeFTM} from '../interfaces/IbeFTM.sol';
import {IbeFTMRewardPool} from '../interfaces/IbeFTMRewardPool.sol';
...
contract Strategy is BaseStrategy {
    ...
    address public sellRouter = address(0xa38cd27185a464914D3046f0AB9d43356B34829D); // SolidlySwap router
    uint8 public sellRouterSelected = 1; // SolidlySwap router

    IbeFTMRewardPool public rewardPool;

    address public wftm = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public beFTM = address(0x7381eD41F6dE418DdE5e84B55590422a57917886);

    IV1Pair public solidlyLp; //  StableV1 AMM - beFTM/FTM
    ...
    constructor(
        ...
        require(solidlyLp.token1() == address(beFTM), 'Not correctPool');

        maxSlippageIn = _maxSlippageIn;
        maxSlippageOut = _maxSlippageOut;

        rewardPool = IbeFTMRewardPool(_rewardPool);
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('Beefy beFTM Reward_Pool Strategy'));
    }
    ...
    function balanceOfBeFtmInRewardPool() public view returns (uint256) {
    ...
    function beFtmPrice() public view returns (uint256 _price) {
        _price = solidlyLp.getAmountOut(1e18, beFTM);
    }

    function balanceOfPooled() public view returns (uint256 _amount) {
        uint256 totalBeFtm = balanceOfBeFtmInRewardPool().add(balanceOfBeFTM());
        _amount = solidlyLp.getAmountOut(totalBeFtm, beFTM);
    }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > looseAmount) {
            ...
            sellBeFTM(wantToBeFTM(toExitAmount));
            depositBeFTMinRewardPool();
    ...
    function prepareMigration(address _newStrategy) internal override {
        _withdrawFromRewardPoolAndTransfer(_newStrategy);
    }
    ...
    function wantToBeFTM(uint256 _amount) internal view returns (uint256) {
        if (_amount == 0) return 0;
        return solidlyLp.quote(address(want), _amount, 100);
    }

    function getBeFTM(uint256 _amountIn) internal {
        if (beFtmPrice() >= 1e18) {
            IBeFTM(beFTM).deposit(_amountIn);
        } else {
            buyBeFTM(_amountIn);
        }
    }

    function buyBeFTM(uint256 _toBuyAmount) internal {
        (, bool isStable) = ISolRouter(router).getAmountOut(_toBuyAmount, address(want), beFTM);

        route[] memory wantToBeFTMRoute = new route[](1);
        wantToBeFTMRoute[0] = route(address(want), beFTM, isStable);

        ISolRouter(router).swapExactTokensForTokens(
            _toBuyAmount,
            _toBuyAmount.mul(swapSecurityFactor).div(basisOne),
            wantToBeFTMRoute,
            address(this),
            block.timestamp
        );
    ...
    function sellBeFTM(uint256 _amount) internal {
        uint256 amountToSell = Math.min(_amount, balanceOfBeFTM());
        if (sellRouterSelected == 1) {
            sellAllBeFTM_main(amountToSell);
        } else if (sellRouterSelected == 2) {
            sellAllBeFTM_uni(amountToSell);
        } else {
            sellAllBeFTM_Beethoven(amountToSell);
        }
    }

    function sellAllBeFTM() internal {
        uint256 beFtmBalance = IERC20(beFTM).balanceOf(address(this));
        sellBeFTM(beFtmBalance);
    }

    function sellAllBeFTM_main(uint256 _toSellAmount) internal {
    ...
    function sellAllBeFTM_uni(uint256 _toSellAmount) internal {
    ...
    function sellAllBeFTM_Beethoven(uint256 _toSellAmount) internal {
    ...
    //--------------------------------//
    //   Internal Reward Pool func    //
    //--------------------------------//

    function depositBeFTMinRewardPool() internal {
        if (balanceOfBeFTM() > 0) {
            rewardPool.stake(balanceOfBeFTM());
        }
    }

    function withdrawFromRewardPool() internal {
        uint256 masterChefBal = balanceOfBeFtmInRewardPool();
        if (masterChefBal > 0) {
            rewardPool.withdraw(masterChefBal);
        }
    }

    function _withdrawFromRewardPoolAndTransfer(address _to) internal {
        if (!abandonRewards) claimRewards();
        withdrawFromRewardPool();
        uint256 beFTMBalance = balanceOfBeFTM();
        if (beFTMBalance > 0) {
            IERC20(beFTM).transfer(_to, beFTMBalance);
        }
    }
    ...
    function collectTradingFees() internal {
        ...
        if (total > debt) {
            ...
            uint256 looseAmount = balanceOfWant();
            if (profit > looseAmount) {
                uint256 toExitAmount = profit.sub(looseAmount);
                liquidatePosition(toExitAmount);
            }
        }
    }
    ...
    function giveSellRouterAllowance() internal {
        IERC20(beFTM).safeApprove(sellRouter, 0);
        IERC20(beFTM).safeApprove(sellRouter, MAX);
    }

    function removeRouterSellAllowance() internal {
        IERC20(beFTM).safeApprove(sellRouter, 0);
    }

    function giveAllowances() internal {
        IERC20(beFTM).safeApprove(address(rewardPool), 0);
        IERC20(beFTM).safeApprove(address(rewardPool), MAX);

        IERC20(want).safeApprove(router, 0);
        IERC20(want).safeApprove(router, MAX);

        IERC20(want).safeApprove(beFTM, 0);
        IERC20(want).safeApprove(beFTM, MAX);

        giveSellRouterAllowance();
    }
    ...
    function emergencyWithdrawFromRewardPool() external onlyVaultManagers {
        _withdrawFromRewardPoolAndTransfer(address(this));
    }
