...
contract StrategyCommonChefLP is StratManager, FeeManager {
    ...
    function chargeFees(address callFeeRecipient) internal {
        //CHARGE A 4.5% FEE ON THE REWARDS
        //OBTAIN 2% OF THE OUTPUT (WIGO)
        uint256 toBurn = IERC20(output).balanceOf(address(this)).mul(20).div(1000);
        //OBTAIN 2.5% OF THE OUTPUT (WIGO) TO BE USED FOR REST OF FEE DISTRIBUTION
        uint256 toNative = IERC20(output).balanceOf(address(this)).mul(25).div(1000);
        chef.call(abi.encodeWithSignature("wigoBurn(uint256)", toBurn));
