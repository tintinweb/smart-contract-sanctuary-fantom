...
contract _Pint_Token is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 Pint-WETH LP -> Pint pool
    uint256 public constant INITIAL_Pint_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the Pint ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Pint Token", "Pint") {
        // Mints 1 Pint to contract creator for initial pool setup
    ...
    function _getPintPrice() internal view returns (uint256 _PintPrice) {
        try IOracle(two_ombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Pint: failed to fetch Pint price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _PintPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_PintPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPintOracle(address _PintOracle) public onlyOperatorOrTaxOffice {
        require(_PintOracle != address(0), "oracle address cannot be 0 address");
        two_ombOracle = _PintOracle;
    }
    ...
    /**
     * @notice Operator mints Pint to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Pint to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPintPrice = _getPintPrice();
            currentTaxRate = _updateTaxRate(currentPintPrice);
            if (currentPintPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_PintPool != address(0), "!_PintPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_PintPool, INITIAL_Pint_POOL_DISTRIBUTION);
    }
