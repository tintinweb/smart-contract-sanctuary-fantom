...
contract FaithToken is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 FAITH-WETH LP -> FAITH pool
    // uint256 public constant INITIAL_FAITH_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the FAITH ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("FaithBasedBlockchain", "FAITH") {
        // Mints 1 FAITH to contract creator for initial pool setup
    ...
    function _getFaithPrice() internal view returns (uint256 _faithPrice) {
        try IOracle(faithOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Faith: failed to fetch FAITH price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _faithPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_faithPrice >= taxTiersTwaps[tierId]) {
    ...
    function setFaithOracle(address _faithOracle) public onlyOperatorOrTaxOffice {
        require(_faithOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints FAITH to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FAITH to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFaithPrice = _getFaithPrice();
            currentTaxRate = _updateTaxRate(currentFaithPrice);
            if (currentFaithPrice < burnThreshold) {
                burnTax = true;
            }
        }
