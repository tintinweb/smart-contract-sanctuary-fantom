...
// File contracts/EqualTombLif3vAMM.sol

// SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualTombLif3vAMM is Ownable, Pausable {
    ...
    address public constant tomb = address(0x6c021Ae822BEa943b2E66552bDe1D2696a53fbB7);
    address public constant lif3 = address(0xbf60e7414EF09026733c1E7de72E7393888C64DA);
    address public want;
    ...
    IEqualizerRouter.Routes[] public wrappedToTombPath;
    IEqualizerRouter.Routes[] public tombToLif3Path;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToTombPath,
        IEqualizerRouter.Routes[] memory _tombToLif3Path
        ...
        for (uint i; i < _wrappedToTombPath.length; ++i) {
            wrappedToTombPath.push(_wrappedToTombPath[i]);
        }

        for (uint i; i < _tombToLif3Path.length; ++i) {
            tombToLif3Path.push(_tombToLif3Path[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(solidlyRouter).swapExactTokensForTokens(wrappedBal, 0, wrappedToTombPath, address(this), block.timestamp);

        uint256 tombHalf = IERC20(tomb).balanceOf(address(this)) / 2;

        IEqualizerRouter(router).swapExactTokensForTokens(tombHalf, 0, tombToLif3Path, address(this), block.timestamp);

        uint256 lp0Bal = IERC20(tomb).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lif3).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(tomb, lif3, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(lif3).safeApprove(router, 0);
        IERC20(wrapped).safeApprove(solidlyRouter, 0);
        IERC20(tomb).safeApprove(solidlyRouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(tomb).safeApprove(router, type(uint).max);
        IERC20(lif3).safeApprove(router, type(uint).max);
        IERC20(wrapped).safeApprove(solidlyRouter, type(uint).max);
        IERC20(tomb).safeApprove(solidlyRouter, type(uint).max);
    }
