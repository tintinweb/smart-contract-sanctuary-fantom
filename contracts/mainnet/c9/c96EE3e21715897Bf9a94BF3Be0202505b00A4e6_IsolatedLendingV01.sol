...
contract IsolatedLendingV01 is ERC4626{
    ...
    IsFTMXStaking public immutable sFTMXStaking;
    ...
    uint256 private constant OPEN_COLLATERIZATION_RATE = 70000; // 70%
    ...
    // Fees
    uint256 public PROTOCOL_FEE = 20000; // 20%
    uint256 private constant PROTOCOL_FEE_DIVISOR = 1e5;
    uint256 private constant BORROW_OPENING_FEE = 50; // 0.05%
    uint256 private constant BORROW_OPENING_FEE_PRECISION = 1e5;

    uint256 private constant LIQUIDATION_MULTIPLIER = 15000; // 15%
    ...
    constructor(address _asset, address _collateral, string memory _name, string memory _symbol)ERC4626(ERC20(_asset), _name, _symbol){
        ...
        sFTMXStaking = IsFTMXStaking(0xB458BfC855ab504a8a327720FcEF98886065529b);
        priceFeedCollateral = AggregatorV3Interface(0xf4766552D15AE4d256Ad41B6cf2933482B0680dc);
        priceFeedAsset = AggregatorV3Interface(0x2553f4eeb82d5A26427b8d1106C51499CBa5D99c);
        (,int collateralPrice,,,) = priceFeedCollateral.latestRoundData();
        (,int assetPrice,,,) = priceFeedAsset.latestRoundData();
        exchangeRate = sFTMXStaking.getExchangeRate()*(uint256(collateralPrice)*1e10)/(uint256(assetPrice)*1e20);
        feeTo = msg.sender;
        admin = msg.sender;
    }
    ...
    function setFeeTo(address _feeTo) public onlyAdmin{
        require(_feeTo != address(0), "NenoLend: set feeTo zero address");
        feeTo = _feeTo;
        emit LogFeeTo(_feeTo);
    }

    function setAdmin(address _admin) public onlyAdmin{
        require(_admin != address(0), "NenoLend: set admin to zero address");
        address oldAdmin = admin;
        admin = _admin;
        emit AdminTransferred(oldAdmin, _admin);
    }

    function setProtocolFee(uint256 _feeAmount) public onlyAdmin{
        PROTOCOL_FEE = _feeAmount;
        emit LogFee(_feeAmount);
    } 
    ...
    function liquidate(address _user, uint256 _amount) public {
        ...
        if(!isSolvent(_user)){
            ...
            uint256 collateralLiquidated = _amount*1e20/exchangeRate; 
    ...
    function userCollateralValue(address _user) public view returns (uint256){
        return userCollateralAmount[_user]*(exchangeRate*1e10)/1e30; 
    }
}