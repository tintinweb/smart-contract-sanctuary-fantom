...
contract Flashswap {
    ...
    function start(
        ...
        (uint256 profit, uint256 _tokenBorrowAmount) = check(_tokenBorrow, _amountTokenPay, _tokenPay, _sourceRouter, _targetRouter);
    ...
    function check1(
        ...
        return (
            _amountTokenPay, // our profit or loss; example output: BNB amount
    ...
    function check2(
        address _tokenBorrow, // example: BUSD
        uint256 _amountOut, // example: BNB => 10 * 1e18
        address _tokenPay, // example: BNB
        address _sourceRouter,
        address _targetRouter
    ) public view returns(uint256, uint256) {
        ...
        uint256 amountRepay = IUniswapV2Router(_targetRouter).getAmountsOut(_amountOut, path2)[1];

        return (
            amountRepay, // our profit or loss; example output: BNB amount
            _amountOut // the amount we get from our input "_amountTokenPay"; example: BUSD amount
        );
    }

    function check3(
        uint256 _item1, // example: BNB => 10 * 1e18
        uint256 _item2
    ) public view returns(uint256) {
        return (
            uint256(_item1 - _item2)
        );
    }
    ...
    function check(
        ...
        return (
            uint256(amountRepay - _amountTokenPay), // our profit or loss; example output: BNB amount
            amountOut // the amount we get from our input "_amountTokenPay"; example: BUSD amount
        );
    }
    ...
    function soulswapCall(address _sender, uint256 _amount0, uint256 _amount1, bytes calldata _data) external {
        execute(_sender, _amount0, _amount1, _data);
    }
