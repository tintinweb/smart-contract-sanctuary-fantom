...
contract Kandy is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the KANDY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Kandy Finance", "KANDY") {
        // Mints 1 KANDY to contract creator for initial pool setup
    ...
    function _getKandyPrice() internal view returns (uint256 _kandyPrice) {
        try IOracle(kandyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Kandy: failed to fetch KANDY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _kandyPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_kandyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setKandyOracle(address _kandyOracle) public onlyOperatorOrTaxOffice {
        require(_kandyOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints KANDY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of KANDY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentKandyPrice = _getKandyPrice();
            currentTaxRate = _updateTaxRate(currentKandyPrice);
            if (currentKandyPrice < burnThreshold) {
                burnTax = true;
            }
        }
