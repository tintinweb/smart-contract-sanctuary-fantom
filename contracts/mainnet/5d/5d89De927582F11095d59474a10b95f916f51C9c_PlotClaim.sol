...
/**
 * @title For minting each one of Plots.Zone
 * 
 * @dev Plot mint one by one for pure user and avoid sybil.
 * 
 */
...
contract PlotClaim is  Ownable {
    ...
    address payable _turnWallet = address(0x35C05eAf8b3FC27dF2A381DD809CC4c41Cb973d2);
    address public plotszone = address(0xc9B317F61551E6D17B4Dc375667D293D32b04af4);  
    ...
    function setPlotsZone( address _contractaddr ) external onlyOwner {
        plotszone = _contractaddr;
    }

    function claim_go( address reg_addr,uint256 tokeid) internal returns (uint16) 
    {
        IPlotsZone _plotszone = IPlotsZone(plotszone);
        _plotszone.mint(reg_addr, tokeid);
    }
