...
import {BaseStrategyMC} from "../../base/BaseStrategyMC.sol";

contract StrategyRare_RSHARE_FTM is BaseStrategyMC {

    address rshare_wftm_diff;

    function initializeStrategy(
        address _store,
        address _vault
    ) public initializer {
        address masterchef = address(0x27Cd5a4c9E797931B79C370Bc0BF1Ce819b4f65D);
        address rshare_ftm_lp = address(0x33dC25b73D8f38C612eD9d55B2d6cBe6f78e5f4E);
        address rshare = address(0xB835631824ED68D84c5f41C231f3C2b6C2D709d5);
        ...
        routes[rshare] = [rshare];
        routes[wftm] = [rshare, wftm];
    }

    /// @notice A method for fetching the strategy's current pending yields.
    /// @return The current pending yield of the strategy.
    function pendingYield() external view override returns (uint256[] memory) {
        // Fetch pending yield from the contract.
        uint256[] memory yields = new uint256[](1);
        (,bytes memory pendingInfo) = rewardPool().staticcall(abi.encodeWithSignature("pendingShare(uint256,address)", poolId(), address(this)));
        yields[0] = abi.decode(pendingInfo, (uint256));
...
/// @title Base MasterChef Strategy.
/// @author Chainvisions
/// @notice A base strategy for MasterChef-based reward pools.

contract BaseStrategyMC is BaseUpgradeableStrategy {
    ...
    */
    function _liquidateReward() internal {
        ...
        if(isLP()) {
            ...
            IERC20(rewardToken()).safeApprove(swapRouter(), 0);
            IERC20(rewardToken()).safeApprove(swapRouter(), rewardBalance);

            IUniswapV2Router02(swapRouter()).swapExactTokensForTokens(rewardBalance, 0, routes[underlying()], address(this), (block.timestamp + 600));
        }
    }
