...
contract CrimeLootBot is GovernableUpgradable {
    ...
    function addParamsBot(address bot, uint256 addDefense, uint256 addCash) external {
      onlyGovernaneOrController();

      Crimer memory c = _crimerInfoStorage.getCrimer(bot);

      c.defense += addDefense;
      c.cash += addCash * 10 ** 2;
      _crimerInfoStorage.setCrimer(bot, c);

      /// @dev mint ccah to increase totalSupply
      _crimeCashMainnet.mint(_crimeBankInteraction, addCash * 10 ** 2);

      emit BotSecretEventStart(bot, roundNumber, addCash * 10 ** 2, addDefense);
    }

    function claimDefenseDistribution(address bot) external {
        onlyGovernaneOrController();
        require(bs[bot].isBot, "Bot only");
        uint8 rd = CrimeCashGame(_crimeCashGame).roundDay();
        uint256 rn;
        if (rd * 1000 > bs[bot].lastAssetClaimTime)
            rn = 0;
        else
            rn = bs[bot].lastAssetClaimTime - rd * 1000;
        require(rn < 6, "Claim distribution exceeded");

        uint256 calculateAsset = calculateAssetGeometric(bs[bot].startTime) / 3;
        
        Crimer memory c = _crimerInfoStorage.getCrimer(bot);
        c.cash += calculateAsset;

        uint256 calculateExtraDefense = 0;
        if (rn == 0)
        {
            if (rd <= 5)
                calculateExtraDefense = 500000 * rd; 
            else
                calculateExtraDefense = calculateDefenseGeometric(6);
        ...
        bs[bot].lastAssetClaimTime = rd * 1000 + rn + 1;
    ...
    function calculateAssetGeometric(uint256 startDay)
        public
        view
        returns (uint256)
    {
        uint256 result = initialAmount;
        uint256 n = _calculateNumberOfDays(startDay);
        for (uint256 i = 1; i < n; i++)
            result = result * accelerationK / 1000; 

        return result;
    }

    function calculateDefenseGeometric(uint256 startDay)
        public
        view
        returns (uint256)
    {
        uint256 result = initialDefense;
        uint256 n = _calculateNumberOfDays(startDay);
        for (uint256 i = 1; i < n; i++)
            result = result * accelerationD / 1000; 
    ...
    function _createLootBot(address botAddress) private {
        ...
        bs[botAddress].startTime = CrimeCashGame(_crimeCashGame).roundDay();
        bs[botAddress].lastAssetClaimTime = CrimeCashGame(_crimeCashGame).roundDay() * 1000;
    }

    function _calculateNumberOfDays(uint256 startDay)
        private
        view
        returns (uint256)
    {
        return CrimeCashGame(_crimeCashGame).roundDay() - startDay + 1;
    }

    function SetAssetAcceleration(uint256 k) external
    {
        onlyGovernaneOrController();
    ...
    function SetDefenseAcceleration(uint256 k) external
    {
        onlyGovernaneOrController();
    ...
    function SetInitialAmount(uint256 k) external
    {
        onlyGovernaneOrController();
    ...
    function SetInitialDefense(uint256 k) external
    {
        onlyGovernaneOrController();
    ...
    function ResetBot(address botAddress, uint16 startDay) external
    {
        onlyGovernaneOrController();
        
        bs[botAddress].startTime = startDay;
        bs[botAddress].lastAssetClaimTime = startDay * 1000;
    }
}
...
contract AssetManager is Governable {
    ...
    function addDefaultAssets(uint256 decimals) external {
        ...
        addAssetToMap(2, 1, 300, precisionToken * 500, true);
        addAssetToMap(2, 2, 660, precisionToken * 1000, true);
        addAssetToMap(2, 3, 1386, precisionToken * 2000, true);
        addAssetToMap(2, 4, 3049, precisionToken * 4000, true);
        addAssetToMap(2, 5, 6403, precisionToken * 8000, true);
        addAssetToMap(2, 6, 14087, precisionToken * 16000, true);
        addAssetToMap(2, 7, 29583, precisionToken * 32000, true);
        addAssetToMap(2, 8, 65083, precisionToken * 104000, true);
        addAssetToMap(2, 9, 136675, precisionToken * 328000, true);
        addAssetToMap(2, 10, 300685, precisionToken * 756000, true);
        addAssetToMap(2, 11, 661507, precisionToken * 1912000, true);
        addAssetToMap(2, 12, 1389165, precisionToken * 8024000, true);
        addAssetToMap(2, 13, 3056163, precisionToken * 22048000, true);
        addAssetToMap(2, 14, 6417942, precisionToken * 54096000, true);
        addAssetToMap(2, 15, 14119472, precisionToken * 108192000, false);
