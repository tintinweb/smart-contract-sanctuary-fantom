...
import "../../VelodromPairPriceOracle/VelodromPairPriceOracle.sol";
...
contract UniswapAnchoredView is UniswapConfig, VelodromPairPriceOracle {
...
//    }
    ...
    function fetchAnchorPriceVelodromeLikePair(string memory symbol, TokenConfig memory config, uint nativePrice) internal virtual returns (uint) {
        ...
        (uint T, uint anchorPriceRaw) = getResult(config.uniswapMarket);

        return anchorPriceRaw;
...
//        uint anchorPrice = mul(priceInReferenceUnits, nativePrice) / ethBaseUnit;
//        return anchorPrice;
    }
...
contract VelodromPairPriceOracle is ITarotSolidlyPriceOracle {
    using UQ112x112 for uint224;

//    uint32 public constant MIN_T = 1200;
    uint32 public constant MIN_T = 60;
    ...
    function getResult(address pair) public override returns (uint224 price, uint32 T) {
