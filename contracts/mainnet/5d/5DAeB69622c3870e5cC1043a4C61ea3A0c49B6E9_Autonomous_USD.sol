...
contract Autonomous_USD is IERC20 {
   ...
   bool appreciationMode = true;
    ...
    function _sellFixing(address sender, uint _amount, uint feeAmount) public {
           uint contractAmount;
           uint newAmount;
           uint mintAmount;
           uint rPeg;
...
function _buyFixing(uint amount) private {
    ...
    function periodInflation(uint _inflationRate, uint _periodThreshold, uint _upgradingPeriod, uint _update, uint devident) external onlyManager{
    ...
    function continualMultiplyInflation(uint _inflationRate, uint _targetReachingPeriod, uint _upgradingPeriod, uint _update) public onlyManager{
           uint oneday = 24*60*60;
           ...
           isPriceStable = false;
           _upgradeAppreciation(_update, txVolume);
           mode = 2;
    }
    ...
    function _upgradeAppreciation(uint _upgrade, uint devident) internal onlyManager{
              ...
              require(distance > devident);
              appreciationRate = distance.div(devident);
              txVolume = devident;
              appreciationMode = true;
