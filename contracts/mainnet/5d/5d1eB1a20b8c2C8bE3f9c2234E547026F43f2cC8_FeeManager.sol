...
contract FeeManager is Ownable{
    ...
    IProtofiRouter02 public router;
    ...
    constructor (IERC20 _Proton, address _teamAddr) public{
        Proton = _Proton;
        teamAddr = _teamAddr; /*swap fees team wallet*/
    }

    function getRegisteredToken(uint256 index) internal virtual view returns (address){
        return registeredToken[index];
    }

    function getRegisteredTokenLength() internal virtual view returns (uint256){
        return registeredToken.length;
    }

    function getRegisteredPair(uint256 index) internal virtual view returns (address){
        return registeredPair[index];
    }

    function getRegisteredPairLength() internal virtual view returns (uint256){
        return registeredPair.length;
    }

    function addPair(address _pair) external onlyOwner{
        if (!pairInitialized[_pair]){
            registeredPair.push(_pair);
            pairInitialized[_pair] = true;
        }
    }

    function removePair(address _pair) external onlyOwner{
        if (pairInitialized[_pair]){
            uint256 length = registeredPair.length;
            uint256 indexToRemove = length; // If pair not found web do not try to remove bad index
            for (uint256 index = 0; index < length; ++index) {
                if(registeredPair[index] == _pair){
                    indexToRemove = index;
                    break;
                }
            }
            if(indexToRemove < length){ // Should never be false.. Or something wrong happened
                registeredPair[indexToRemove] = registeredPair[registeredPair.length-1];
                registeredPair.pop();
            }
            pairInitialized[_pair] = false;
            return;
        }
    }
    ...
    */
    ...
    function removeLiquidityToToken(address _token) external onlyOwner{
        IProtofiPair _pair = IProtofiPair(_token);
    ...
    function removeAllLiquidity() external onlyOwner{
        uint256 length = getRegisteredPairLength();

        for (uint256 index = 0; index < length; ++index) {
            IProtofiPair _curPair = IProtofiPair(getRegisteredPair(index));

            uint256 _amount = _curPair.balanceOf(address(this));
            if(_amount == 0){
                continue;
            }
            address token0 = _curPair.token0();
            address token1 = _curPair.token1();

            _curPair.approve(address(router), _amount);
            router.removeLiquidity(token0, token1, _amount, 0, 0, address(this), block.timestamp.add(100));
        }
    }

    function swapBalanceToToken(address _token0, address _token1) external onlyOwner {
        // require(_token0 != address(Proton), "Proton can only be burn");
    ...
    function swapAllToToken() external onlyOwner {
        uint256 length = getRegisteredPairLength();
        for (uint256 index = 0; index < length; ++index) {
            IProtofiPair _curPair = IProtofiPair(getRegisteredPair(index));
            ...
            if(token0 == address(TargetToken)){
                token0 = token1;
                token1 = address(TargetToken);
            }else if(token1 != address(TargetToken)){
                continue;
            }

            uint256 _amount = IERC20(token0).balanceOf(address(this));
    ...
    function setupRouter(address _router) external onlyOwner{
        router = IProtofiRouter02(_router);
    }

    /* distribute fee to dev wallet  */
    function distributeFee() external onlyOwner {
        uint256 length = getRegisteredTokenLength();
        for (uint256 index = 0; index < length; ++index) {
            IERC20 _curToken = IERC20(getRegisteredToken(index));
            uint256 _amount = _curToken.balanceOf(address(this));
            _curToken.safeTransfer(teamAddr, _amount);
        }
        if (Proton.balanceOf(address(this)) > 0){
            Proton.transfer(BURN_ADDRESS, Proton.balanceOf(address(this)));
        }
    }

    function sendUnswappedToken(address _token) external onlyOwner {
        IERC20 _curToken = IERC20(_token);
        uint256 _amount = _curToken.balanceOf(address(this));
        if(_amount != 0) _curToken.safeTransfer(teamAddr, _amount);
    }
}
