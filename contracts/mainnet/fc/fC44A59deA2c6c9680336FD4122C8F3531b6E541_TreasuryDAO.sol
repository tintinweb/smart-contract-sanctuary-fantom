...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
     ...
     */
    constructor(address _wnativeToken, address _usdCurrency, address _sandManSwapRouter, uint256 startTime) {
    ...
    function _swapTokens(address tokenAddress, uint256 tokenAmount) internal {
        address[] memory path = new address[](3);
        path[0] = tokenAddress;
        path[1] = address(wnativeToken);
        path[2] = address(usdCurrency);

        ERC20(tokenAddress).approve(address(sandManSwapRouter), tokenAmount);
    ...
    function convertDepositFeesToUSD(address token, uint256 tokenType, uint256 amount) public onlyRole(OPERATOR_ROLE) {
        ...
        if (tokenType == 1) {
            (address token0, address token1, uint256 token0Supply, uint256 token1Supply) = _removeLiquidity(token, amount);
            convertDepositFeesToUSD(token0, 0, token0Supply);
            convertDepositFeesToUSD(token1, 0, token1Supply);
            return;
        }
        ...
        if (holdingLiquidity > tokenBalance)
            holdingLiquidity = tokenBalance;
    ...
    function _removeLiquidity(address token, uint256 tokenAmount) internal returns(address, address, uint256, uint256) {
        ...
        uint256 token0BeforeLiquidation = IERC20(token0).balanceOf(address(this));
        uint256 token1BeforeLiquidation = IERC20(token1).balanceOf(address(this));
        ...
        uint256 token0FromLiquidation = IERC20(token0).balanceOf(address(this)) - token0BeforeLiquidation;
        uint256 token1FromLiquidation = IERC20(token1).balanceOf(address(this)) - token1BeforeLiquidation;


        return (token0, token1, token0FromLiquidation, token1FromLiquidation);
    }
    ...
    function setDistributionRate(uint256 _distributionRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
