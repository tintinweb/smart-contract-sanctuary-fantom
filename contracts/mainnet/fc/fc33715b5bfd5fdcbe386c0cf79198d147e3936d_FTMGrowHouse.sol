...
contract FTMGrowHouse {
    ...
    uint256 public maxDeposit = 30000000000000000000000; //30,000 FTM Max limit
    uint256 public minDeposit = 200000000000000000000; //200 FTM Min limit
...
// ============================================================
        // daily reward.  3 % to 6% 
// ==============================================================
        PercsKey[10] = DivPercs(864000, 30);   //3%
        PercsKey[20] = DivPercs(1728000, 35); //3.5%
        PercsKey[30] = DivPercs(2592000, 40); //4%
        PercsKey[40] = DivPercs(3456000, 45); //4.5%
        PercsKey[50] = DivPercs(4320000, 50); //5%
        PercsKey[60] = DivPercs(5184000, 55); //5.5%
        PercsKey[70] = DivPercs(6048000, 55); //5.5%
        PercsKey[80] = DivPercs(6912000, 55); //5.5%
        PercsKey[90] = DivPercs(7776000, 70); // 6%.  // 91 days => 425%
       
//====================================================================
//                     1 to  60 days =>  20%
//                     whale tax 61 to 90 days => 30%
 //===================================================================
        FeesKey[10] = FeesPercs(864000, 200);   // 1 to  60 days =>  20%
        FeesKey[20] = FeesPercs(1728000, 200);    // 1 to  60 days =>  20%
        FeesKey[30] = FeesPercs(2592000, 200);     // 1 to  60 days =>  20%
        FeesKey[40] = FeesPercs(3456000, 200);   // 1 to  60 days =>  20%
        FeesKey[50] = FeesPercs(4320000, 200);    // 1 to  60 days =>  20%
        FeesKey[60] = FeesPercs(5184000, 200);    // 1 to  60 days =>  20%


        FeesKey[70] = FeesPercs(6048000, 300);  //whale tax 61 to 90 days => 30%
        FeesKey[80] = FeesPercs(6912000, 300);   //whale tax 61 to 90 days => 30%
        FeesKey[90] = FeesPercs(7776000, 300);   //whale tax 61 to 90 days => 30%
    ...
    function stakeNativeCurrencies(address ref) public payable {
       ...
       User storage user = UsersKey[msg.sender];
        require(msg.value > minDeposit , "you can't deposit less than min limit");
        require(user.totalInits +  msg.value < maxDeposit , "you can't deposit more than max limit");
    ...
    function withdrawInitial(uint256 key) external {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ // LESS THAN 10 DAYS STAKED
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[20].feePercentage).div(percentdiv); //20% fee
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[30].feePercentage).div(percentdiv); //20% fee
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[50].feePercentage).div(percentdiv); //20% fee
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[60].feePercentage).div(percentdiv); //20% fee
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[70].feePercentage).div(percentdiv); // whale 30% fee
            ...
            uint256 minusAmt = initialAmt.mul(FeesKey[80].feePercentage).div(percentdiv);  //whale 30% fee
            ...
            USDC_FTM_LP.safeTransfer(msg.sender, transferAmt);
            USDC_FTM_LP.safeTransfer(owner, minusAmt);
        } else if (currDays >= FeesKey[80].daysInSeconds && currDays < FeesKey[91].daysInSeconds){ // BETWEEN 80 and 90 DAYS
            uint256 minusAmt = initialAmt.mul(FeesKey[90].feePercentage).div(percentdiv);  //whale 30% fee
    ...
    function calcdiv(address dy) public view returns (uint256 totalWithdrawable) {
        ...
        for (uint256 i = 0; i < user.depoList.length; i++){	
            ...
            if (user.depoList[i].initialWithdrawn == false){
                ...
                if (elapsedTime > PercsKey[80].daysInSeconds && elapsedTime <= PercsKey[91].daysInSeconds){
