...
contract ArbV6_2 is Operator {
    ...
    function calculateKP (uint256 _pricePublic, uint256 _kPrivate) public pure returns (uint256) {
        uint256 kp = _kPrivate.mul(_pricePublic);
        return kp;
    }

    function getNumerator (uint256 kp) external view returns (
        uint256 numerator, uint256 n1, uint256 n2, uint256 n3, uint256 n4, uint256 n5
        ) {
        n1 = this.sqrt_babylonian1(kp);
        n2 = this.sqrt_babylonian2(kp);
        n3 = this.sqrt_binarysearch1(kp);
        n4 = this.sqrt_binarysearch2(kp);
        n5 = this.sqrt_newtonRaphson(kp);
    ...
    function getNewReserve(uint256 _pricePublic, uint256 _kPrivate, uint8 _decimalBase, uint8 _decimalAsset) 
        ...
        uint256 kp = this.calculateKP(_pricePublic, _kPrivate);
        (uint256 num, , , , , ) = this.getNumerator(kp);
        uint256 factor = 10 ** (dAsset.div(2).sub(dAsset.add(dAsset).sub(dBase)));
        newReserve = num.mul(factor);
    }
