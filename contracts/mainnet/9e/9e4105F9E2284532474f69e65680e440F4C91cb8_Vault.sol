...
import "../tokens/interfaces/IUSDE.sol";
...
contract Vault is ReentrancyGuard, IVault {
    ...
    uint256 public constant USDE_DECIMALS = 18;
    ...
    // usdeAmounts tracks the amount of USDE debt for each whitelisted token
    mapping(address => uint256) public override usdeAmounts;

    // maxUsdeAmounts allows setting a max amount of USDE debt for a token
    mapping(address => uint256) public override maxUsdeAmounts;
    ...
    // this value is used to calculate the redemption values for selling of USDE
    ...
    event BuyUSDE(address account, address token, uint256 tokenAmount, uint256 usdeAmount, uint256 feeBasisPoints);
    event SellUSDE(address account, address token, uint256 usdeAmount, uint256 tokenAmount, uint256 feeBasisPoints);
    ...
    event IncreaseUsdeAmount(address token, uint256 amount);
    event DecreaseUsdeAmount(address token, uint256 amount);
    ...
    function setTokenConfig(
        ...
        maxUsdeAmounts[_token] = _maxUsdeAmount;
    ...
    function setUsdeAmount(address _token, uint256 _amount) external override {
        _onlyGov();

        uint256 usdeAmount = usdeAmounts[_token];
        if (_amount > usdeAmount) {
            _increaseUsdeAmount(_token, _amount.sub(usdeAmount));
            return;
        }

        _decreaseUsdeAmount(_token, usdeAmount.sub(_amount));
    }
    ...
    // deposit into the pool without minting USDE tokens
    ...
    function buyUSDE(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        ...
        uint256 usdeAmount = tokenAmount.mul(price).div(PRICE_PRECISION);
        usdeAmount = adjustForDecimals(usdeAmount, _token, usde);
        _validate(usdeAmount > 0, 18);

        uint256 feeBasisPoints = vaultUtils.getBuyUsdeFeeBasisPoints(_token, usdeAmount);
        uint256 amountAfterFees = _collectSwapFees(_token, tokenAmount, feeBasisPoints);
        uint256 mintAmount = amountAfterFees.mul(price).div(PRICE_PRECISION);
        mintAmount = adjustForDecimals(mintAmount, _token, usde);

        _increaseUsdeAmount(_token, mintAmount);
        _increasePoolAmount(_token, amountAfterFees);

        IUSDE(usde).mint(_receiver, mintAmount);

        emit BuyUSDE(_receiver, _token, tokenAmount, mintAmount, feeBasisPoints);

        useSwapPricing = false;
        return mintAmount;
    }

    function sellUSDE(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        _validate(whitelistedTokens[_token], 19);
        useSwapPricing = true;

        uint256 usdeAmount = _transferIn(usde);
        _validate(usdeAmount > 0, 20);

        updateCumulativeFundingRate(_token, _token);

        uint256 redemptionAmount = getRedemptionAmount(_token, usdeAmount);
        _validate(redemptionAmount > 0, 21);

        _decreaseUsdeAmount(_token, usdeAmount);
        _decreasePoolAmount(_token, redemptionAmount);

        IUSDE(usde).burn(address(this), usdeAmount);

        // the _transferIn call increased the value of tokenBalances[usde]
        // usually decreases in token balances are synced by calling _transferOut
        // however, for usde, the tokens are burnt, so _updateTokenBalance should
        // be manually called to record the decrease in tokens
        _updateTokenBalance(usde);

        uint256 feeBasisPoints = vaultUtils.getSellUsdeFeeBasisPoints(_token, usdeAmount);
        uint256 amountOut = _collectSwapFees(_token, redemptionAmount, feeBasisPoints);
        _validate(amountOut > 0, 22);

        _transferOut(_token, amountOut, _receiver);

        emit SellUSDE(_receiver, _token, usdeAmount, amountOut, feeBasisPoints);
    ...
    function swap(
        ...
        // adjust usdeAmounts by the same usdeAmount as debt is shifted between the assets
        uint256 usdeAmount = amountIn.mul(priceIn).div(PRICE_PRECISION);
        usdeAmount = adjustForDecimals(usdeAmount, _tokenIn, usde);

        uint256 feeBasisPoints = vaultUtils.getSwapFeeBasisPoints(_tokenIn, _tokenOut, usdeAmount);
        uint256 amountOutAfterFees = _collectSwapFees(_tokenOut, amountOut, feeBasisPoints);

        _increaseUsdeAmount(_tokenIn, usdeAmount);
        _decreaseUsdeAmount(_tokenOut, usdeAmount);
    ...
    function getRedemptionAmount(address _token, uint256 _usdeAmount) public view override returns (uint256) {
        uint256 price = getMaxPrice(_token);
        uint256 redemptionAmount = _usdeAmount.mul(PRICE_PRECISION).div(price);
        return adjustForDecimals(redemptionAmount, usde, _token);
    }
    ...
    function adjustForDecimals(
        ...
        uint256 decimalsDiv = _tokenDiv == usde ? USDE_DECIMALS : tokenDecimals[_tokenDiv];
        uint256 decimalsMul = _tokenMul == usde ? USDE_DECIMALS : tokenDecimals[_tokenMul];
    ...
    function getFeeBasisPoints(
        ...
        return vaultUtils.getFeeBasisPoints(_token, _usdeDelta, _feeBasisPoints, _taxBasisPoints, _increment);
    }

    function getTargetUsdeAmount(address _token) public view override returns (uint256) {
        uint256 supply = IERC20(usde).totalSupply();
        if (supply == 0) {
            return 0;
        }
    ...
    function _increaseUsdeAmount(address _token, uint256 _amount) private {
        usdeAmounts[_token] = usdeAmounts[_token].add(_amount);
        uint256 maxUsdeAmount = maxUsdeAmounts[_token];
        if (maxUsdeAmount != 0) {
            _validate(usdeAmounts[_token] <= maxUsdeAmount, 51);
        }
        emit IncreaseUsdeAmount(_token, _amount);
    }

    function _decreaseUsdeAmount(address _token, uint256 _amount) private {
        uint256 value = usdeAmounts[_token];
        // since USDE can be minted using multiple assets
        // it is possible for the USDE debt for a single asset to be less than zero
        // the USDE debt is capped to zero for this case
        if (value <= _amount) {
            usdeAmounts[_token] = 0;
            emit DecreaseUsdeAmount(_token, value);
            return;
        }
        usdeAmounts[_token] = value.sub(_amount);
        emit DecreaseUsdeAmount(_token, _amount);
    }
