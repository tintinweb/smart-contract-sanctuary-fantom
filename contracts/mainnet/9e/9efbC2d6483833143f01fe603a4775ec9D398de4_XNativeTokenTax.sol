...
contract XNativeTokenTax is ERC20("XNATIVE Token", "xNative") {
    using SafeMath for uint256;
    IERC20 public boo;
    ...
    constructor(IERC20 _boo, address _feeToSetter) public {
        boo = _boo;
    ...
    function leave(uint256 _share) public {
        ...
        uint256 tax = what.mul(TAX_RATE).div(100);
        uint256 payout = what.sub(tax);

        // Burn the xBoo and transfer the payout to the sender
        burn(msg.sender, _share);
        boo.transfer(msg.sender, payout);

        // Transfer the tax to the tax address
        boo.transfer(feeTo, tax);
    }
