...
contract DummyReceiver is NonblockingLzApp {
    ...
    function encodeAddresses(address[] memory addressesToEncode) public pure returns (bytes memory) {
        return abi.encode(addressesToEncode);
    }
    
    function decodeAddresses(bytes memory encodedAddresses) public pure returns (address[] memory) {
        return abi.decode(encodedAddresses, (address[]));
    }
