    ...
    WalletObserverImplementationPointer
{
    ...
    address public immutable darkForestManager;

    address public devWallet;
    address public nftWallet;

    uint256 public devSellFee = 3;
    uint256 public nftSellFee = 3;
    uint256 public treasurySellFee = 4;
    ...
    constructor(address _darkForestManager, address _devWallet, address _nftWallet) ERC20("Dark Forest", "DF") {
        require(
            _darkForestManager != address(0),
            "Implementations: DarkForestManager is not set"
        );

        devWallet = _devWallet;
        nftWallet = _nftWallet;
        darkForestManager = _darkForestManager;
    ...
    function _transfer(
        ...
        if(to == liquidityPoolManager.getPair()) {
            devFee = amount.mul(devSellFee).div(100);
            nftFee = amount.mul(nftSellFee).div(100);
            treasuryFee = amount.mul(treasurySellFee).div(100);
            amount = amount.sub(devFee).sub(nftFee).sub(treasuryFee);
            address treasuryAddress = liquidityPoolManager.getTreasuryAddress();
            super._transfer(from, treasuryAddress, treasuryFee);
    ...
    function liquidityReward(uint256 amount) external onlyDarkForestManager {
        require(
    ...
    function setDevSellFee(uint256 devSellFee_) external onlyOwner {
        devSellFee = devSellFee_;
    }

    function setNftSellFee(uint256 nftSellFee_) external onlyOwner {
        nftSellFee = nftSellFee_;
    }

    function setTreasurySellFee(uint256 treasurySellFee_) external onlyOwner {
        treasurySellFee = treasurySellFee_;
    }
    ...
    function setDevWallet(address _devWallet) public {
        require(devWallet == msg.sender, "Only old dev can set new dev!");
        devWallet = _devWallet;
    }

    function setNftWallet(address _nftWallet) public {
        require(nftWallet == msg.sender, "Only old nft can set new nft!");
        nftWallet = _nftWallet;
    }
}
