...
/_/  \____/_/ /_/ /_/_.___/  /_/   /_/_/ /_/\__,_/_/ /_/\___/\___/
    ...
    http://eagle.finance
*/
contract Eagle is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 14000 ether;
    // Initial distribution for the day 2-5 EAGLE-WFTM LP -> EAGLE pool
    uint256 public constant INITIAL_EAGLE_POOL_DISTRIBUTION = 150000 ether;
    ...
    /**
     * @notice Constructs the EAGLE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("EAGLE", "EAGLE") {
        // Mints 1 EAGLE to contract creator for initial pool setup
    ...
    function _getEaglePrice() internal view returns (uint256 _eaglePrice) {
        try IOracle(eagleOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Eagle: failed to fetch EAGLE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _eaglePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_eaglePrice >= taxTiersTwaps[tierId]) {
    ...
    function seteagleOracle(address _eagleOracle) public onlyOperatorOrTaxOffice {
        require(_eagleOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints EAGLE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of EAGLE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEaglePrice = _getEaglePrice();
            currentTaxRate = _updateTaxRate(currentEaglePrice);
            if (currentEaglePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_eaglePool != address(0), "!_eaglePool");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_eaglePool, INITIAL_EAGLE_POOL_DISTRIBUTION);
    }
