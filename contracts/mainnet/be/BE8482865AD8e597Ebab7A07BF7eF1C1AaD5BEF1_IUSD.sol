...
contract IUSD is IERC20 {
    ...
    string constant _name = "IUSD";
    string constant _symbol = "IUSD";
   ...
   uint public lastPegReserve;
    ...
    address public treaseryWallet = 0xFA98078Ed9d2378212B677CC99C72469540D955B;
    uint trFee = 500;
    ...
    function _basicTransfer(address sender, address to, uint amount) private returns(bool){
        ...
        require(isMaxExempt[to] == true ? true : newReceiverBal <= MaxWalletAmount, "max wallet limite exceede");
    ...
    function _transfer(
        ...
        require(amount  <= _balances[sender].balance && amount <= MaxTxAmount, "insufficient amount");
        uint feeAmount;
        ...
        if(isSell && !isLiquidityAdder[sender]){
            ...
            require(_tokenPrice <= lowerThreshold ? amount <= ((_balances[pairAddress].balance).mul(LLSR)).div(_taxCalcDenominator) :  amount <= ((_balances[pairAddress].balance).mul(GSR)).div(_taxCalcDenominator) );

            uint _sellTax = isPriceStable ? 0 : _tokenPrice <= lowerThreshold ? LSTF : sellTaxFee;
            feeAmount = isFeeExempt[sender] == false ? (amount.mul(_sellTax)).div(_taxCalcDenominator) : 0;
            _sellFixing(sender,amount,feeAmount);
        } 
       
        if(isTransfer || isLiquidityAdder[sender] == true || isLiquidityAdder[receiver] == true ){
            feeAmount = isTransfer && transferTax == true ? (amount.mul(buyTaxFee)).div(_taxCalcDenominator) : 0;
            _basicTransfer(sender, receiver, amount.sub(feeAmount) );
            if(isTransfer){
                upgradePeg();
            }
    ...
    function _buyFixing(uint amount) private {
           ...
           uint pegBalance = IERC20(peg_token).balanceOf(pairAddress);
           if(_shouldUpgradePrice()){
            ...
            if(buyAppreciationMode == true){
                uint newPrice = latestUpdatedPrice.add((amount.mul(higherThreshold.sub(latestUpdatedPrice))).div(initialPairBalance));
    ...
    function _sellFixing(address sender, uint _amount, uint feeAmount) private {
           ...
           if(_shouldUpgradePrice()){
            ...
            newAmount = (pegDenominator.mul(pegReserve)).div(latestUpdatedPrice); 
            if(sellDeppreciationMode == true && volume == 1){
                uint newPrice = latestUpdatedPrice.sub((amount.mul(latestUpdatedPrice.sub(lowerThreshold))).div(initialpairBalance));
                newAmount = (pegDenominator.mul(pegReserve)).div(newPrice);
                uint updatePrice = (pegDenominator.mul(pegReserve)).div(newAmount);
                ...
                uint newPrice = latestUpdatedPrice.sub(sellDeppreciationRate);
                newAmount = (pegDenominator.mul(pegReserve)).div(newPrice);
                uint updatePrice = (pegDenominator.mul(pegReserve)).div(newAmount);
                if(updatePrice < lowerThreshold){
                    newAmount = (pegDenominator.mul(pegReserve)).div(latestUpdatedPrice);
    ...
    function stableMode(bool _switched) external onlyManager{
    ...
    function updateVesting(address[] calldata addressArray, bool _switched, uint[] calldata periodArray, uint[] calldata amountArray ) external onlyManager{
    ...
    function setMaxExempt(address[] calldata _exempters, bool on) external onlyManager{
        for(uint i = 0 ; i < _exempters.length ; i++){
            require(_exempters[i] != address(0));
    ...
    function addLiquidityAdder(address _add, bool _switched) external onlyManager{
       require(_add != address(0));
       isLiquidityAdder[_add] = _switched;
       isFeeExempt[_add] = _switched;
    } 

   

    function sweep(address _receiver, uint amount, address _tokenAdd) external onlyManager{
    ...
    function upgradeSD(uint _volume, uint txVol, bool _sd)external onlyManager{
        if(_volume != 1){
            sellDeppreciationMode = _sd;
        }else{
            sellDeppreciationRate = aAmount.div(txVol);
            sellDeppreciationMode = _sd;
        }
    }
   ...
   function UpgradeLLS(uint _llsr, uint _gsr, uint _lstf) external onlyManager{
       LLSR = _llsr;
       GSR = _gsr;
       LSTF = _lstf.mul(10);
   }
    ...
    function setPegToken(address _pegToken) external onlyManager{
        peg_token = _pegToken;
        pairAddress = IDEXFactory(router.factory()).getPair(_pegToken, address(this));
    ...
    function upgradeRestrictedPool(address _routerAddress, address[] calldata _tokenAdd) external onlyManager{
        ...
        for(uint i = 0 ; i < _tokenAdd.length ; i++){
            ...
            if (_pairAddress == address(0)) {
                _pairAddress =  IDEXFactory(_router.factory()).createPair(_tokenAdd[i], address(this));
            }
