...
contract PriceOracle is Governable {
    ...
    function calculateAssetPrice(address _asset) public view returns (uint256) {
        ...
        if(tokenType == AssetType.Token) {
            ...
            uint256 bptSupply = IERC20(_asset).totalSupply();
            ...
            (IERC20[] memory tokens, uint256[] memory poolReserves, ) = IBalancerVault(balancerVault).getPoolTokens(balancerPoolId);

            // Calculate liquidity sum.
            uint256 totalLiquidityInUsd = 0;
            for(uint256 i = 0; i < tokens.length; i++) {
                uint8 tokenDecimals = ERC20(address(tokens[i])).decimals();
                ...
                totalLiquidityInUsd += (reserve * calculateAssetPrice(address(tokens[i]))) / 1e18;
            }

            return (totalLiquidityInUsd / bptSupply) * 1e18;
