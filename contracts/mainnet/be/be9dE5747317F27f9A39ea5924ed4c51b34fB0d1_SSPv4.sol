...
contract SSPv4 is AccessControl, Pausable {
    address public usdc;
    address public dei;
    ...
    constructor(
        ...
        _setupRole(TRUSTY_ROLE, adminAddress);
        _setupRole(WITHDRAWER_ROLE, adminAddress);

        IERC20(usdc).approve(mintHelper, type(uint256).max);
    }

    function buyDei(uint256 amountIn) external whenNotPaused {
        require(
            amountIn <= topBound && amountIn >= lowerBound,
            "you need to buy in valid range"
        );
        ...
        IMintHelper(mintHelper).mint(
            msg.sender,
            arbitragers[msg.sender] ? deiAmount : deiAmount - mintFee
        );
        emit Buy(amountIn);
    }

    function setBound(uint256 topBound_, uint256 lowerBound_)
        external
    ...
    function setMintFeeRate(uint256 mintFeeRate_)
        external
        onlyRole(TRUSTY_ROLE)
    {
        mintFeeRate = mintFeeRate_;
    ...
    function setArbitrager(address addr, bool isArbitrager_)
        external
        onlyRole(TRUSTY_ROLE)
    {
