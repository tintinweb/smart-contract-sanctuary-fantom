...
contract PropertyVault is Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(address _userAddress, uint256 _amount) external whenNotPaused nonReentrant returns (uint256) {
        ...
        require(vaultCap <= MAX_Cap.mul(1e18), "Exceeded max cap!");
        ...
        emit Deposit(_userAddress, _amount, block.timestamp);
    ...
    function getClaimAmt(address _userAddress) public view returns (uint256) {
        UserInfo storage user = userInfo[_userAddress];
        return (block.timestamp.sub(user.lastUserActionTime)).mul(rewardPerSecond).mul(user.stakedAmt).mul(tokenPrice).div(100000).div(MAX_Cap).div(1e18) + user.rewardDebt;
    }
    ...
    function claimReward(address _userAddress) public {
        ...
        if (block.timestamp > user.lastUserActionTime && user.stakedAmt > 0) {
            ...
            uint256 rewardAmt = block.timestamp.sub(user.lastUserActionTime).mul(rewardPerSecond).mul(user.stakedAmt).mul(tokenPrice).div(100000).div(MAX_Cap).div(1e18);
            if (rewardBalance > 0) {
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(user.stakedAmt >= _amount, "Insufficient to withdraw");
