...
contract TokenRegistry is ITokenRegistry, Ownable
{
  ...
  event Unwhitelist(address token);
  event SetStable(address token);
  event UnsetStable(address token);


  function isStable (address token) external view override returns (bool)
  {
    return _stable[token];
  }
  ...
  {
    return _whitelisted[debtToken] && _whitelisted[collateralToken] && (!_stable[collateralToken] || !_stable[debtToken]);
  }
  ...
  function _whitelist (address token) private
  {
    ...
    require(_whitelistedTokens.add(token), "whitelisted");


    _whitelisted[token] = true;


    emit Whitelist(token);
  }
  ...
  function unwhitelist (address token) external onlyOwner
  {
    require(_whitelistedTokens.remove(token), "!whitelisted");


    _whitelisted[token] = false;


    emit Unwhitelist(token);


    if (_stableTokens.remove(token))
    {
      _stable[token] = false;


      emit UnsetStable(token);
    }
  }
  ...
  function _setAsStable (address token) private
  {
    require(_whitelistedTokens.contains(token), "!whitelisted");
    require(_stableTokens.add(token), "set");


    _stable[token] = true;


    emit SetStable(token);
  }

  function setAsStable (address token) external onlyOwner
  {
    _setAsStable(token);
  }

  function setAsStableTokens (address[] calldata tokens) external onlyOwner
  {
    for (uint256 i = 0; i < tokens.length; i++)
    {
      _setAsStable(tokens[i]);
    }
  }

  function unsetAsStable (address token) external onlyOwner
  {
    require(_stableTokens.remove(token), "!stabletoken");
