...
contract NomisScore is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, Pausable, EIP712 {
    ...
    */
    modifier equalsFee(uint _fee, address _wallet, uint16 _calcModel) {
        require(Whitelist[_wallet][_calcModel] || _fee == _mintFee || _tokenIds.current() < CalculationModelsFreeMintCount[_calcModel], "wrong mint fee value");
        _;
    }
    ...
    */
    constructor(uint256 initialFee, uint16 initialCalcModelsCount, uint16 initialfreeMintCount) ERC721("NomisScore", "NMSS") EIP712("NMSS", "0.8") {
        _tokenIds.reset();
        _mintFee = initialFee;
        require(initialCalcModelsCount > 0, "calc models count should be greater than 0");
    ...
    * @param calcModel The scoring calculation model.
    */
    function WhitelistAddresses(address[] calldata actors, uint16 calcModel) external onlyOwner {
    ...
    * @param calcModel The scoring calculation model.
    */
    function UnWhitelistAddresses(address[] calldata actors, uint16 calcModel) external onlyOwner {
        for (uint256 i = 0; i < actors.length; ++i) {
    ...
    */
    function setCalcModelsCount(uint16 calcModelsCount) external onlyOwner {
        require(calcModelsCount > 0, "calcModelsCount should be greater than 0");
    ...
    */
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override (ERC721) {
        require(from == address(0) || to == address(0), "Nomis score can't be transferred.");
    ...
    */
    function _burn(uint256 /* tokenId */) internal pure override (ERC721, ERC721URIStorage) {
        require(false, "Nomis score can't be burned.");
    }
