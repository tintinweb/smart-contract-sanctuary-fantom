...
import "../interfaces/IPerpetualEscrowToken.sol";
import "../interfaces/IFeeDistribution.sol";
import "../interfaces/IMirrorGate.sol";
import "../interfaces/IxLQDR.sol";

contract PerpetualEscrowTokenExtension is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    // chainId => MirrorGateV2 address
    mapping(uint256 => address) public gates;
    ...
    function mirrorLocks(
        ...
        require(
            gates[currentChainId] != address(0),
            "mirrorGate address is ZERO"
        );
        (int128 lockAmount, uint256 lockEnds) = IxLQDR(xLQDR).locked(
            escrowToken
        );
        for (uint256 i = 0; i < toChainIds.length; i++) {
            require(
                gates[toChainIds[i]] != address(0),
                "mirrorGate address is ZERO"
            );
            require(toChainIds[i] != currentChainId, "invalid chainId");
            IPerpetualEscrowToken(escrowToken)._call(
                gates[currentChainId],
                abi.encodeWithSelector(
                    IMirrorGate.mirrorLocks.selector,
                    toChainIds[i],
                    gates[toChainIds[i]],
                    [currentChainId],
                    [0],
    ...
    function collect() external nonReentrant {
        IPerpetualEscrowToken(escrowToken)._call(
            distributor,
            abi.encodeWithSelector(IFeeDistribution.claim.selector)
        );
        for (uint256 _i = 0; _i < tokenCount; _i++) {
            address _token = IFeeDistribution(distributor).tokens(_i);
            if (_token == address(0)) break;
            uint256 _balance = IERC20Upgradeable(_token).balanceOf(escrowToken);
            if (_balance > 0) {
                IPerpetualEscrowToken(escrowToken)._call(
                    _token,
                    abi.encodeWithSelector(
    ...
    function setMirrorGate(
        uint256 chainId,
        address mirrorGate
    ) external onlyOwner {
        gates[chainId] = mirrorGate;
        emit SetMirrorGate(chainId, mirrorGate);
    }

    function setDistributorAndTokenCount(
        address _newDistributor,
        uint256 _newTokenCount
    ) external onlyOwner {
    ...
    event SetMirrorGate(uint256 chainId, address mirrorGate);
...
interface IPerpetualEscrowToken {
    function _call(
        address _target,
