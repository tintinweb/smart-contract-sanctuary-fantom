...
contract Claim is Ownable {
    ...
    IWeight public immutable weightProvider;
    ...
    mapping (address => bool) public hasMapping;

    constructor(address _grain, address _weightProvider, uint256 _lgeEnd) {
        grain = IERC20(_grain);
        weightProvider = IWeight(_weightProvider);
        lgeEnd = _lgeEnd;
    }

    // FIRST CLAIM
    function firstClaim() external returns (uint256 claimable) {
        if (gracePeriodOver == false) {
            revert GrainLGE__GrainNotSet();
        }
        // if user mapping is defined, revert
        if (hasMapping[msg.sender]) {
            revert GrainLGE__Unauthorized();
        }

        // fetches user total weight, sets mappings
        (uint256 userTotalWeight, uint256 numberOfReleases) = weightProvider.getUserShares(msg.sender);
        userShares[msg.sender].totalWeight = userTotalWeight;
        userShares[msg.sender].numberOfReleases = numberOfReleases;
        hasMapping[msg.sender] = true;
    ...
    function totalOwed(address user) public view returns (uint256 userTotal) {
        uint256 shareOfLge = userShares[user].totalWeight * PERCENT_DIVISOR / totalWeight;
    ...
    function setTotalChainWeight(uint256 _totalWeight) external onlyOwner {
...
interface IWeight {
    function getUserShares(address user) external view returns (uint256 totalWeight, uint256 numberOfReleases);
}
...
contract UserShares is Ownable {
    ...
    function getUserShares(address user) public view returns (uint256 totalWeight, uint256) {
        ...
        uint256 weight = vestingPremium == 0 ? usdcValue : ((usdcValue) * ((PERCENT_DIVISOR * PERCENT_DIVISOR) / (PERCENT_DIVISOR - vestingPremium))) / PERCENT_DIVISOR;
...
import "../GrainLGEMetis.sol";
import "lib/openzeppelin-contracts/contracts/access/Ownable.sol";

contract WeightMetis is Ownable {
    uint256 public constant MAX_KINK_RELEASES = 8;
    uint256 public constant MAX_RELEASES = 20;
    uint256 public constant maxKinkDiscount = 4e26;
    uint256 public constant maxDiscount = 6e26;
    uint256 public constant PERCENT_DIVISOR = 1e27;
    GrainLGEMetis public immutable lge;
    ...
    constructor(address _lge) {
        lge = GrainLGEMetis(_lge);
    }

    function getUserWeights(address user) public view returns (uint256 weight, uint256 bonusWeight, uint256 totalWeight) {
        (uint256 usdcValue, uint256 numberOfReleases,,, address nft,) = lge.userShares(user);
        ...
        weight = vestingPremium == 0 ? usdcValue : ((usdcValue) * ((PERCENT_DIVISOR * PERCENT_DIVISOR) / (PERCENT_DIVISOR - vestingPremium))) / PERCENT_DIVISOR;

        bonusWeight = nft == address(0) ? 0 : weight * whitelistedBonuses / PERCENT_DIVISOR;

        totalWeight = weight + bonusWeight;

        return (weight, bonusWeight, totalWeight);
    }
}
...
import "../GrainLGEUniV2.sol";
import "lib/openzeppelin-contracts/contracts/access/Ownable.sol";

contract WeightUniv2 is Ownable {
    uint256 public constant MAX_KINK_RELEASES = 8;
    uint256 public constant MAX_RELEASES = 20;
    uint256 public constant maxKinkDiscount = 4e26;
    uint256 public constant maxDiscount = 6e26;
    uint256 public constant PERCENT_DIVISOR = 1e27;
    GrainLGEUniV2 public immutable lge;
...
import "../GrainLGEUniV3.sol";
import "lib/openzeppelin-contracts/contracts/access/Ownable.sol";

contract WeightUniv3 is Ownable {
    uint256 public constant MAX_KINK_RELEASES = 8;
    uint256 public constant MAX_RELEASES = 20;
    uint256 public constant maxKinkDiscount = 4e26;
    uint256 public constant maxDiscount = 6e26;
    uint256 public constant PERCENT_DIVISOR = 1e27;
    GrainLGEUniV3 public immutable lge;
