...
contract SecondStage is ERC721, ERC721Enumerable, Pausable, ERC721Burnable, ReentrancyGuard, Ownable {
    ...
    function buyToken(uint8 tokenId) external payable nonReentrant whenNotPaused callerIsUser {
        ...
        require(getTokenPrice(tokenId, msg.sender) == msg.value, "Error: invalid price");
    ...
    function getTokenPrice(uint8 tokenId, address buyer) public view returns (uint256) {
        ...
        if(buyer != address(0) && stageOneContract.exists(tokenId) && stageOneContract.ownerOf(tokenId) == buyer) {
