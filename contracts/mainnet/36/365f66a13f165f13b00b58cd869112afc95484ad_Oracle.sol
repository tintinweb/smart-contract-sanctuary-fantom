...
contract Oracle is Epoch {
    ...
    function update() external checkEpoch {
        ...
        price0Average = FixedPoint.uq112x112(uint224((price0Cumulative.div(1e6).mul(1e18) - price0CumulativeLast.div(1e6).mul(1e18)) / timeElapsed));
        price1Average = FixedPoint.uq112x112(uint224((price1Cumulative - price1CumulativeLast) / timeElapsed));

        price0CumulativeLast = price0Cumulative.div(1e6).mul(1e18);
    ...
    function twap(address _token, uint256 _amountIn) external view returns (uint144 _amountOut) {
        ...
        uint32 timeElapsed = blockTimestamp - blockTimestampLast; // overflow is desired
        if (_token == token0) {
            _amountOut = FixedPoint.uq112x112(uint224((price0Cumulative.div(1e6).mul(1e18) - price0CumulativeLast.div(1e6).mul(1e18)) / timeElapsed)).mul(_amountIn).decode144();
