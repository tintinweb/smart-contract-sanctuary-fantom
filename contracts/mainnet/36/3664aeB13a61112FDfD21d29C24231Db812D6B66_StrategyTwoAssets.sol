...
//    bytes32 public constant swapPool = 0x03c6b3f09d2504606936b1a4decefad204687890000200000000000000000015; // Beets-USDC pool
    ...
    address public constant gOhmAddr = 0x91fa20244Fb509e8289CA630E5db3E9166233FDc; // gOHM
//    address public constant depositTokenAddr = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75; // USDC
    // Pool to deposit
    bytes32 public constant depositpool = 0xf7bf0f161d3240488807ffa23894452246049916000200000000000000000198;
    uint256 public constant depositIndex = 0;
    uint256 public constant nTokens = 2;
    // Deposit pool assets
    IAsset[] public assets = new IAsset[](nTokens);
    // Fee token
    address public constant feeTokenAddr = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;

    address public multiHarvest = 0x0000000000000000000000000000000000000000;

    // Other constants
    uint256 public constant underlyingPoolId = 40;
    ...
    uint256 public harvestCutoffBps = 10**5;
    uint256 public harvestCutoffDepositBps = 10**5;
    uint256 public harvestCutoffgOhmBps = 10**1;
    ...
    function set_gOhmharvestCutoff(uint256 newCutoff) external onlyOwner() {
        harvestCutoffgOhmBps = newCutoff;
        emit SetHarvestCutoff(harvestCutoffgOhmBps);
    }

    function set_depositharvestCutoff(uint256 newCutoff) external onlyOwner() {
        harvestCutoffDepositBps = newCutoff;
        emit SetHarvestCutoff(harvestCutoffDepositBps);
    }
    ...
    function swapgOhmToDeposit() internal {
        uint256 gOhmBalance = IERC20(gOhmAddr).balanceOf(address(this));

        IBalancer.SingleSwap memory singleSwap;
        IBalancer.SwapKind swapKind = IBalancer.SwapKind.GIVEN_IN;

        singleSwap.poolId = depositpool;
        singleSwap.kind = swapKind;
        singleSwap.assetIn = IAsset(gOhmAddr);
        singleSwap.assetOut = IAsset(depositTokenAddr);
        singleSwap.amount = gOhmBalance;
        singleSwap.userData = abi.encode(0);
        ...
        IERC20(gOhmAddr).safeApprove(beethovenVault, 0);
        IERC20(gOhmAddr).safeApprove(beethovenVault, gOhmBalance);
    ...
    function harvest() public override nonReentrant {
        ...
        // Swap gOhm to reward
        uint256 _gOhmBalance = IERC20(gOhmAddr).balanceOf(address(this));
        if (_gOhmBalance > harvestCutoffgOhmBps){
          swapgOhmToDeposit();
        }
        ...
        uint256 _depositBalance = IERC20(depositTokenAddr).balanceOf(address(this));
        if (_depositBalance > harvestCutoffDepositBps){
            if (kogefarmFeeAmountBps > 0) {
                collectFee(_depositBalance);
            }
            // Add liquidity
            addLiquidity();
            // Stake the LP tokens
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    ...
    address public LP_TOKEN = 0xf7bF0f161d3240488807FFa23894452246049916;
    constructor()
        public
