...
contract Joetroller is JoetrollerV1Storage, JoetrollerInterface, JoetrollerErrorReporter, Exponential {
    ...
    function mintAllowed(
        ...
        if(enabledSingleSupplier[jToken]){
            require(canSupply[jToken][minter], "minter is not allowed");
        }
    ...
    function _setSingleSupplierPool(JToken jToken, bool value) public returns (uint256) {
        if (msg.sender != admin) {
            return uint256(Error.UNAUTHORIZED);
        }

        enabledSingleSupplier[address(jToken)] = value;
        return uint256(Error.NO_ERROR);
    }

    function _addSingleSupplierToPool(JToken jToken, address supplier, bool value) public returns (uint256) {
        if (msg.sender != admin) {
            return uint256(Error.UNAUTHORIZED);
        }

        canSupply[address(jToken)][supplier] = value;
        return uint256(Error.NO_ERROR);
    }
