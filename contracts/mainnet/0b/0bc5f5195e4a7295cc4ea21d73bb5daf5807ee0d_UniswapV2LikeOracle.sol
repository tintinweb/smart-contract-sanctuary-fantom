...
abstract contract OracleBase is IOracle {
    ...
    function _getBalances(IERC20 srcToken, IERC20 dstToken) public view virtual returns (uint256 srcBalance, uint256 dstBalance);
}
...
contract UniswapV2LikeOracle is OracleBase {
    ...
    event InitcodeHashCalculated(bytes32 indexed initcodeHash);

    constructor(address _factory) {
        factory = _factory;
        initcodeHash = getInitCodeHash(_factory);

        emit InitcodeHashCalculated(initcodeHash);
    }
    ...
    function _pairFor(IERC20 tokenA, IERC20 tokenB) public view returns (address pair) {
    ...
    function _getBalances(IERC20 srcToken, IERC20 dstToken) public view override returns (uint256 srcBalance, uint256 dstBalance) {
