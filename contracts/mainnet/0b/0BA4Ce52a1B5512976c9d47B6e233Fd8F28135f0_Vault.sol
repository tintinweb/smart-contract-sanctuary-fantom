...
contract Vault is ERC20, Ownable {
  ...
  constructor() ERC20("testOneUSD","oUSD") {
    ...
    maxDepositInUSD = 10000000;
    minDepositInUSD = 10000;
  }
    ...
    returns(uint256) 
  {
    ...
    uint256 amountToDepositInUSD = (amountToDeposit)/(10**(tokenDecimals-3));
    ...
    returns(uint256) 
  {
    ...
    if(_newLiquidityInUSD*(10**(tokenDecimals-3)) > amountToDeposit) {
      _newLiquidityInUSD = amountToDeposit;
    }

    // oneUSD is 1:1 ratio with the final amount of USD deposited
    uint256 amountToMint = _newLiquidityInUSD*(10**(decimals()-3));
    ...
    assert(totalBalanceInUSD() >= totalSupply()/(10**(decimals()-3)));
  ...
  function totalBalanceInUSD() public view returns (uint256) {
    ...
    uint256 reservesUSD = reservesUSDC/(10**(6-3));
