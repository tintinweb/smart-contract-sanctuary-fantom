...
interface CasinoInterface {
    function payWinnings(address _to, uint256 _amount, uint256 _tokenId) external;
    function transferFrom(address _from, uint256 _amount, uint256 _tokenId) external;
}
...
abstract contract CasinoGame is Ownable {
    ...
    function rewardUser(address _user, uint256 _amount, uint256 _tokenId) internal {
        require(_amount >= 0, "Not enough to withdraw.");
        casinoContract.payWinnings(_user, _amount, _tokenId);
        emit RewardPaid(_user, _amount);
    }

    // Allows a user to place a bet by paying the contract the specified amount.
    function payContract(address _address, uint256 _amount, uint256 _tokenId) internal {
        casinoContract.transferFrom(_address, _amount, _tokenId);
...
contract Blackjack is Ownable, CasinoGame {
    ...
    constructor(uint256 _minBet, uint256 _maxBet, uint8 _numDecks, uint256 _randie) CasinoGame(_minBet, _maxBet) {
        numDecks = _numDecks;
        randie = _randie;
    }

    function setRandie(uint256 _randie) public onlyOwner {
        randie = _randie;
    }
    ...
    function playRound(uint256 _betAmount, uint256 _tokenId) external {
        ...
        payContract(msg.sender, _betAmount, _tokenId);
    ...
    function endRound(address _playerAddress, uint256 _tokenId) public {
        ...
        */
        ...
        rewardUser(_playerAddress, totalPayout, _tokenId);
    ...
    function doublePlayerBet(address _address, BlackjackGame storage _game, uint8 _handInd, uint256 _tokenId) private {
        // Double the user's bet using a CasinoGame parent function
        payContract(_address, _game.player.hands[_handInd].bet, _tokenId);
    ...
    function splitPlayerHand(uint8 _handInd, uint256 _tokenId) public {
        ...
        doublePlayerBet(msg.sender, game, _handInd, _tokenId);
    ...
    function doubleDown(uint8 handInd, uint256 _tokenId) public {
        require(roundInProgress[msg.sender] == true, "Not playing round.");
        ...
        // Double the user's bet using a CasinoGame parent function
        doublePlayerBet(msg.sender, game, handInd, _tokenId);

        // Hit a single time, then end the player's turn
        hitPlayerAuto(msg.sender, handInd, _tokenId);
        emit PlayerCardsUpdated(msg.sender, bjGames[msg.sender].player.hands[0], bjGames[msg.sender].player.hands[1],
            bjGames[msg.sender].player.hands[2], bjGames[msg.sender].player.hands[3], bjGames[msg.sender].player.numHands);
        endPlayerTurn(msg.sender, _tokenId);
    }

    function hitPlayerAuto(address _playerAddress, uint8 handInd, uint _tokenId) private {
        require(roundInProgress[_playerAddress] == true, "Not playing round.");
    ...
    function hitPlayer(uint8 handInd, uint _tokenId) public {
        require(roundInProgress[msg.sender] == true, "Not playing round.");
    ...
    function standPlayer(uint _tokenId) public {
        require(roundInProgress[msg.sender] == true, "Not playing round.");
        endPlayerTurn(msg.sender, _tokenId);
    }

    // Handles finishing a player's turn.
    function endPlayerTurn(address _playerAddress, uint _tokenId) public {
        require(roundInProgress[_playerAddress] == true, "Not playing round.");
        ...
        // Begin dealer's turn
        dealerPlay(_playerAddress, _tokenId);
    }

    // Handles logic for the dealer's turn
    function dealerPlay(address _playerAddress, uint _tokenId) private {
        require(roundInProgress[_playerAddress] == true, "Not playing round.");
        BlackjackGame storage game = bjGames[_playerAddress];
        ...
        endRound(_playerAddress, _tokenId);
    }
    ...
    function rand(uint256 _upper) private returns(uint256) {
