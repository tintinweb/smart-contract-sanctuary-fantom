...
contract StrategyTombV2 is StratManager, FeeManager {
    ...
    address public constant LSHARE =
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBalance < _amount) {
            ...
            wantBalance = IERC20(lpPair).balanceOf(address(this));
        }
        ...
        IERC20(lpPair).safeTransfer(vault, wantBalance.sub(withdrawFee));
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        uint256 rewardBal = IERC20(LSHARE).balanceOf(address(this));
        if (rewardBal > 0) {
            _swap(IERC20(LSHARE).balanceOf(address(this)), lshareToUsdcPath);
            _chargeFees(callFeeRecipient);
            _swap(
                IERC20(USDC).balanceOf(address(this)) / 2,
                usdcToLPTokenPath0
            );
            _swap(IERC20(USDC).balanceOf(address(this)), usdcToLPTokenPath1);
            _addLiquidity();
            deposit();
        }
    ...
    function _addLiquidity() internal {
        ...
        if (lp0Bal != 0 && lp1Bal != 0) {
            IERC20(lpToken0).safeIncreaseAllowance(TOMB_ROUTER, lp0Bal);
            IERC20(lpToken1).safeIncreaseAllowance(TOMB_ROUTER, lp1Bal);
            IUniswapRouterETH(TOMB_ROUTER).addLiquidity(
                lpToken0,
                lpToken1,
                lp0Bal,
                lp1Bal,
    ...
    function setPathToLPToken(uint256 _lpTokenNum, address[] memory _path)
        external
        onlyOwner
    {
        require(
            _path[0] == USDC &&
                (_path[_path.length - 1] == lpToken0 ||
                    _path[_path.length - 1] == lpToken1),
            "path error"
        );
        if (_lpTokenNum == 0) {
