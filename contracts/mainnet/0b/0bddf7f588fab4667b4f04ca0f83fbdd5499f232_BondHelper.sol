...
contract BondHelper is Ownable {
    ...
    function deposit(uint amount, uint slippage, address bondAddress) external {
        ...
        IBond( bonds[i] ).deposit(amount, maxPrice, msg.sender);

        emit BondCreated(amount, price, msg.sender);
    }
    ...
    function approve(address bondAddress) external {
        uint i = getBondIndex(bondAddress);
        address principleToken = IBond( bonds[i] ).principle();
        IERC20(principleToken).approve(bondAddress, 10^256);
    }
