...
//    address private ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    uint256 public lpShareUsdtTotal;
    uint256 public lpShareTotal;
    bool public lpShareAutoFlag = true;
    uint256 public lpHolderUsdtRewardCondition = 5 * 1e6;
    ...
    constructor() public ERC20('B7 Token', 'B7'){
    ...
    function transferBase(address from,address to,uint256 value) internal returns (uint){
        ...
        if (swapPairList[from] || swapPairList[to]) {
            if (!feeWhiteList[from] && !feeWhiteList[to]) {
                ...
                //SELL
//                if(swapPairList[to] ){
//                    if (totalNeedSell > 0 && from != address(this)) {
//                        swapTokenToUsdt(totalNeedSell);
...
//            uint256 nowUsdtBalance = usdtToken.balanceOf(address(this));
            if (lpShareAutoFlag && lpShareTotal >= lpHolderUsdtRewardCondition && block.number > lpProgressRewardBlock + 20) {
...
//        uint256 nowUsdtBalance = usdtToken.balanceOf(address(this));
//        if(lpShareAutoFlag && nowUsdtBalance > lpHolderUsdtRewardCondition && block.number > lpProgressRewardBlock + 20){
//            processHoldLpReward(REWARD_GAS);
//        }
