...
contract GrimFeeRecipientPOL is Ownable {
    ...
    event PolRebalance(address indexed fromToken, address indexed toToken, uint256 indexed amount);
    event EvoBribe(uint256 indexed amount);
    event MixedBribe(address[] indexed tokens, uint256[] indexed amounts);
    event Vote(address[] indexed poolsVoted, uint256[] indexed weights);
    ...
    event ExitFromContract(address indexed newFeeRecipient);
   ...
   //POL//
    ...
    function addEvoPOL(uint256 _wftmAmount) external onlyAdmin {
        ...
        if(_wftmAmount == 0){
            wftmBal = IERC20(wftm).balanceOf(address(this)) / 2; }
            else {wftmBal = _wftmAmount / 2;
        }

        approvalCheck(solidlyRouter, wftm, wftmBal);
        (evoBal,) = ISolidlyRouter(solidlyRouter).getAmountOut(wftmBal, wftm, grimEvo);
        ISolidlyRouter(solidlyRouter).swapExactTokensForTokensSimple(wftmBal, 1, wftm, grimEvo, false, address(this), block.timestamp);
    ...
    function addOrRemoveCustomPOL(address[2] calldata _tokens, ISolidlyRouter.Routes[] calldata _path, address _vault, bool _stable, bool addOrRemove) external onlyAdmin{
        ...
        uint256 receiptBal;
        if(addOrRemove){
            approvalCheck(solidlyRouter, _tokens[0], t1Bal);
    ...
    function vote(address[] calldata _pools, uint256[] calldata _weights) external onlyAdmin {
        lastBribes = _pools;
    ...
    function exitFromContract(address _receiver) external onlyOwner {
        ...
        emit ExitFromContract(_receiver);
    }


    /** VIEWS **/
    function tokenBalances() external view returns(uint256 _grimEvo, uint256 _wftm, uint256 _equal, uint256 _stableToken, uint256 _receipt, uint256 
_evoLP){
        uint256 receiptBal = IGrimVaultV2(evoVault).balanceOf(address(this));
        uint256 lpBal = IERC20(evoLP).balanceOf(address(this));
        uint256 evoBal = IERC20(grimEvo).balanceOf(address(this));
