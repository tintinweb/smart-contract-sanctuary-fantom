...
abstract contract RevenueShareBase is Ownable, Pausable {
    ...
    /**
     * @dev User can withdrow his locked funds without lock period if publicExitAreSet set
     */
     ...
     * @return avalaible is the amount of unlocked tokens (that are avalaible to withdraw)
     * @return lockedTotal is the amount of locked tokens (that are not avalaible to withdraw yet)
     * @return lockData is the list with amount of tokens and its unlock time
     */
        ...
        )
    {
        ...
        for (uint256 i = 0; i < locks.length; i++) {
            if (locks[i].unlockTime > block.timestamp) {
                ...
                idx++;
                lockedTotal = lockedTotal.add(locks[i].amount);
            } else {
                avalaible = avalaible.add(locks[i].amount);
            }
        }
        return (balances[_user].locked, avalaible, lockedTotal, lockData);
    }

    /**
     * @dev Withdraw `lockToken` after locked time (withdraw Expired Locks)
     */
     ...
     * @return _rewards is the list with amount and token addresses that can be claimed
     */
    ...
    /**
     * @dev Transfer Revard tokens to the user
     ...
     * @param _lockDuration is a new lock duration
     */
    function setLockDuration(uint256 _lockDuration) external onlyOwner {
        require(_lockDuration <= 6 * 4 weeks, 'No more then 6 months');
    ...
    function _lock(uint256 _amount, address _onBehalfOf) internal {
        ...
        userLocks[_onBehalfOf].push(
            LockedBalance({ amount: _amount, unlockTime: block.timestamp.add(lockDuration) })
        );
    ...
    function _getReward(address[] memory _rewardTokens) internal {
        ...
        for (uint256 i; i < length; i++) {
            ...
            uint256 reward = rewards[msg.sender][token].div(SHARE_PRECISION);
     ...
     * @param _reward is unseen amount of reward token
     */
    function _notifyReward(address _rewardsToken, uint256 _reward) internal {
        Reward storage r = rewardData[_rewardsToken];
        if (block.timestamp >= r.periodFinish) {
            r.rewardRate = _reward.mul(SHARE_PRECISION).div(rewardsDuration);
        } else {
            uint256 remaining = r.periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(r.rewardRate).div(SHARE_PRECISION);
            r.rewardRate = _reward.add(leftover).mul(SHARE_PRECISION).div(rewardsDuration);
        }
     ...
     * @param _token is the token that will be checked useen balance
     * @param _balance is provided current balance for the token
     ...
     * @param _user is the user address
     * @param _rewardsToken is an address of reward token
     * @param _balance is a current locked balance for the user
     * @param _currentRewardPerToken is provided current reward per token
...
contract MTRevenueShare is RevenueShareBase {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    modifier onlyLocker() {
        require(lockers.contains(msg.sender), 'Not a locker');
        _;
    }
    ...
    function setLockers(address[] calldata _lockers) external onlyOwner {
        for (uint256 i = 0; i < lockers.length(); i++) {
            lockers.remove(lockers.at(i));
        }
        for (uint256 i = 0; i < _lockers.length; i++) {
            lockers.add(_lockers[i]);
        }
    }
    ...
    function _unseen(
        ...
        unseen = IERC20(token).balanceOf(address(this)).sub(balance);
        if (token == address(lockToken)) {
            unseen = unseen.sub(lockedSupply);
        }
    }
}