...
// File contracts/EqualDeusvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualDeusvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToDeusPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToDeusPath
    ) {
        ...
        for (uint i; i < _equalToDeusPath.length; ++i) {
           equalToDeusPath.push(_equalToDeusPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(equalHalf, 0, equalToDeusPath, address(this), block.timestamp);      

        uint256 lp0Bal = IERC20(equal).balanceOf(address(this));
        uint256 lp1Bal = IERC20(deus).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(equal, deus, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);      
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(deus).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(deus).safeApprove(unirouter, type(uint).max);
    }
