...
import { IBeethovenxMasterChef, IRewarder } from '../interfaces/MasterChef.sol';
...
contract Strategy is BaseStrategy {
	...
	uint256 public maxSlippageIn = 25; // bips
	uint256 public maxSlippageOut = 25; // bips
	...
	uint256 public rewardDust = 1e16;
	...
	IRewarder rewarder = IRewarder(0x1B7Dc916df9D39878477A9eFdE39563BAF7033f4);

	address rewardRouter = 0xF491e7B69E4244ad4002BC14e878a34207E38c29;

	bytes32 pool1Id = bytes32(0x3b998ba87b11a1c5bc1770de9793b17a0da61561000000000000000000000185); // yv USDC
	bytes32 pool2Id = bytes32(0x5ddb92a5340fd0ead3987d3661afcd6104c3b757000000000000000000000187); // Yearn Boosted USDC-DEI
	bytes32 pool3Id = bytes32(0xa10285f445bcb521f1d623300dc4998b02f11c8f00000000000000000000043b); // Tenacious Dollar

	address pool1 = address(0x3B998BA87b11a1c5BC1770dE9793B17A0dA61561); // yv USDC
	address pool2 = address(0x5ddb92A5340FD0eaD3987D3661AfcD6104c3b757); // Yearn Boosted USDC-DEI
	address pool3 = address(0xa10285f445BCB521F1d623300dc4998B02f11c8F); // Tenacious Dollar
	...
	constructor(
		...
		require(masterChef.lpTokens(masterChefPoolId) == address(bpt), 'Incorrect Masterchef ID');
	...
	function name() external view override returns (string memory) {
		return 'Beethoven Boosted SS Tenacious';
	}
	...
	function estimateHarvest() external view returns (uint256) {
		// {BEETS} reward
		uint256 pendingReward = masterChef.pendingBeets(masterChefPoolId, address(this));
		uint256 profit = 0;
		if (pendingReward != 0) {
			address[] memory beetsToWftmPath = new address[](2);
			beetsToWftmPath[0] = BEETS;
			beetsToWftmPath[1] = WFTM;
			beetsToWftmPath[2] = address(want);
			uint256 beetsProfits =
				IUniswapV2Router02(rewardRouter).getAmountsOut(pendingReward, beetsToWftmPath)[2];
			profit = profit.add(beetsProfits);
		}

		// {gALCX} reward
		pendingReward = rewarder.pendingToken(masterChefPoolId, address(this));
		if (pendingReward != 0) {
			address[] memory gAlcxToWftmPath = new address[](2);
			gAlcxToWftmPath[0] = gALCX;
			gAlcxToWftmPath[1] = WFTM;
			gAlcxToWftmPath[2] = address(want);
			profit = profit.add(
				IUniswapV2Router02(rewardRouter).getAmountsOut(pendingReward, gAlcxToWftmPath)[2]
			);
		}
		return profit;
	}
		...
		)
	{
		...
		if (balance > debt) {
			if (currentWant > initialWantBalance) {
				_profit = currentWant.sub(initialWantBalance);
			}
			_loss = 0;
			if (currentWant < _profit) {
				_profit = currentWant;
				_debtPayment = 0;
			} else if (currentWant > _profit.add(_debtOutstanding)) {
				_debtPayment = _debtOutstanding;
			} else {
				_debtPayment = currentWant.sub(_profit);
			}
		} else {
			_loss = debt.sub(balance);
			_debtPayment = Math.min(currentWant, _debtOutstanding);
		}
	}
	...
	function adjustPosition(uint256 _debtOutstanding) internal override {
		...
		if (amountIn > dust && joinPool(amountIn)) {
	...
	{
		if (
			estimatedTotalAssets() <= _amountNeeded ||
			_amountNeeded == vault.strategies(address(this)).totalDebt
		) {
		...
		if (_amountNeeded > looseAmount) {
			...
			_withdrawFromMasterChefAndSellBpt(tokensToBpts(toExitAmount));
			masterChef.deposit(masterChefPoolId, balanceOfBpt(), address(this));
	...
	function liquidateAllPositions() internal override returns (uint256 liquidated) {
		...
		_sellRewards();
		_exitPoolExactBpt(balanceOfBpt());
	...
	function _sellBeets() internal {
		uint256 amount = balanceOfReward();
		if (amount > rewardDust) {
	...
	function _sellgAlcx() internal {
		uint256 amount = IERC20(gALCX).balanceOf(address(this));
		if (amount > rewardDust) {
			address[] memory gAlcxToUsdcPath = new address[](3);
			gAlcxToUsdcPath[0] = gALCX;
			gAlcxToUsdcPath[1] = WFTM;
			gAlcxToUsdcPath[2] = address(want);
			IUniswapV2Router02(rewardRouter).swapExactTokensForTokens(
				amount,
				0,
				gAlcxToUsdcPath,
				address(this),
	...
	function balanceOfPooled() public view returns (uint256) {
		uint256 bptUsdPrice = IBalancerPool(pool3).getRate();
		uint256 bptAmount = balanceOfBpt().add(balanceOfBptInMasterChef());
		return bptAmount.mul(bptUsdPrice).div(1e18).div(1e12);
	}

	function usdToBpt(uint256 toExit) internal view returns (uint256) {
		uint256 bptUsdPrice = IBalancerPool(pool3).getRate();
		return toExit.mul(bptUsdPrice).div(1e6);
	}
	...
	function _exitPoolExactBpt(uint256 _bpts) internal {
		if (_bpts > 0) {
			...
			// -----------------------
			IBalancerVault.BatchSwapStep[] memory batchSwapStep = new IBalancerVault.BatchSwapStep[](3);
			batchSwapStep[0] = IBalancerVault.BatchSwapStep(pool3Id, 0, 1, _bpts, abi.encode(0));
			batchSwapStep[1] = IBalancerVault.BatchSwapStep(pool2Id, 1, 2, 0, abi.encode(0));
			batchSwapStep[2] = IBalancerVault.BatchSwapStep(pool1Id, 2, 3, 0, abi.encode(0));
			// -----------------------
			// EXIT POOL ASSETS ORDER
			// -----------------------
			IAsset[] memory setAssets = new IAsset[](4);
			setAssets[0] = IAsset(address(pool3));
			setAssets[1] = IAsset(address(pool2));
			setAssets[2] = IAsset(address(pool1));
			...
			uint256 minOut = _bpts.mul(swapSecurityFactor).div(basisOne).div(1e12);
			require(balanceOfWant().sub(wantBefore) > minOut, 'Swap');
		}
	}

	function tokensToBpts(uint256 _amountTokens) public view returns (uint256 _amount) {
	...
	function joinPool(uint256 _amountIn) internal returns (bool _joined) {
		if (_amountIn > dust) {
			// -----------------------
			// JOIN POOL SWAP STEPS
			// -----------------------
			IBalancerVault.BatchSwapStep[] memory batchSwapStep = new IBalancerVault.BatchSwapStep[](3);
			batchSwapStep[0] = IBalancerVault.BatchSwapStep(pool1Id, 0, 1, _amountIn, abi.encode(0));
			batchSwapStep[1] = IBalancerVault.BatchSwapStep(pool2Id, 1, 2, 0, abi.encode(0));
			...
			// -----------------------
			IAsset[] memory setAssets = new IAsset[](4);
			setAssets[0] = IAsset(address(want));
			setAssets[1] = IAsset(address(pool1));
			setAssets[2] = IAsset(address(pool2));
			setAssets[3] = IAsset(address(pool3));
			// -----------------------
			// SWAP LIMITS
			// -----------------------
			int256[] memory limits = new int256[](4);
			...
			uint256 minOut = _amountIn.mul(1e12).mul(swapSecurityFactor).div(basisOne);
			require(balanceOfBpt() > minOut, 'Swap');
			return true;
		}
		return false;
	}

	function _giveRewardRouterAllowance() internal {
		IERC20(gALCX).approve(rewardRouter, max);
	}

	function _removeRewardRouterAllowance() internal {
		IERC20(gALCX).approve(rewardRouter, 0);
	}

	function _giveApproval() internal {
		want.safeApprove(address(balancerVault), max);
		bpt.approve(address(masterChef), max);
		_giveRewardRouterAllowance();
	}
	...
	function setRewardRouter(address _router) external onlyVaultManagers {
		_removeRewardRouterAllowance();
	...
	function setProfitLiquidationFactor(uint256 _profitLiquidationFactor) external onlyVaultManagers {
