...
struct authentication {
        ...
        string private constant AUTHENTICATION_TYPE = "authentication(uint256 VERIFY_ONCHAIN,address TOKEN_ADDRESS, uint256 CHAIN_ID, string VERSION, string MESSAGE, address WALLET)";
    ...
    function _buildDomainSeparator(
        ...
        return keccak256(abi.encode(typeHash, nameHash, versionHash, 250, 0x3e3e660E23DCf096b2f35f78638c63F15a950b6D));
    }
...
function ENTRANCE(authentication memory AUTHENTICATION,
    bytes memory signature, address user) public view returns (string memory) {
    ...
    bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
            ...
            uint256(AUTHENTICATION.TOKEN_ADDRESS),
            AUTHENTICATION.CHAIN_ID,
            keccak256(abi.encodePacked(AUTHENTICATION.VERSION)),
            AUTHENTICATION.MESSAGE,
            uint256(AUTHENTICATION.WALLET)
        )));
      address signer = ECDSA.recover(digest, signature);
