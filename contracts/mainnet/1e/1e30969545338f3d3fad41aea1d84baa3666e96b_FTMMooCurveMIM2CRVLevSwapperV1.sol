...
// File contracts/swappers/Leverage/FTMMooCurveG3CRVLevSwapperV1.sol
...
contract FTMMooCurveMIM2CRVLevSwapperV1 {
    using BoringMath for uint256;
    ...
    CurvePool constant public mimPool = CurvePool(0x2dd7C9371965472E5A5fD28fbE165007c61439E1);
    IVault constant public mooVault = IVault(0x837BEe8567297dd628fb82FFBA193A57A9F6B655); 

    constructor() public {
        MIM.approve(address(MIM3POOL), type(uint256).max);
        USDC.approve(address(mimPool), type(uint256).max);
        mimPool.approve(address(mooVault), type(uint256).max);
        mooVault.approve(address(bentoBox), type(uint256).max);
    }
    ...
    function swap(
...

        {
            ...
            uint256[3] memory amountsAdded = [0, 0, amountIntermediate_usdc];

            uint256 amountsOfLiquidity = mimPool.add_liquidity(amountsAdded, 0);
        ...
        (, shareReturned) = bentoBox.deposit(mooVault, address(this), recipient, amountTo, 0);
