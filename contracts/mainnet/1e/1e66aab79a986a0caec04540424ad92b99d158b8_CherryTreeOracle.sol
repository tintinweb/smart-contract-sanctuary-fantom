...
/**
  ...
  Strongly simplified so that:
    ...
    - we don't use UniswapV2Library
        it does too much for our need
    - we don't care about the price of FTM in CHRT (aka token1 / token0)
...
// Here I'm only interested in the token1 (CHRT) and don't care about token0 (FTM)
...
contract CherryTreeOracle is Ownable {
  ...
  address public constant token  = 0x8Dc673878d325C028D08d73C24cD59E15df62a4c;
  ...
  constructor() {
    ...
    (reserveFTM, reserveToken, blockTimestampLast) = IPool(pool).getReserves();

    // make sure liquidity pool not empty
    require(reserveFTM   > 0, 'Oracle: FTM   reserve empty');
    require(reserveToken > 0, 'Oracle: Token reserve empty');
