...
import "IStargateGauge.sol";
import "IStargatePool.sol";
import "IStargateRouter.sol";
import "IUniswapV2Router02.sol";

contract StargazerFantom is BaseStrategy {
    ...
    constructor(
        ...
        gauge = _gauge;
        pool = IStargatePool(address(want));
        ...
        minStgToSell = 10000000000000;
    }

    // ******** OVERRIDE THESE METHODS FROM BASE CONTRACT ************

    function name() external view override returns (string memory) {
        return "SpookyStargazer";
    }
    ...
    function getGaugeBalanceInWant() internal view returns (uint256) {
        (uint256 _amount, uint256 _rewardDebt) = gauge.userInfo(spid, address(this));
        return _amount;
    }
    ...
    function _disposeOfStg() internal {
        ...
        uint256[] memory amounts = currentRouter.swapExactTokensForTokens(
            _amountIn,
            0,
            getTokenOutPathV2(stg, address(stakingToken)),
            address(this),
            now
        );
        uint256 amountOut = amounts[amounts.length - 1];
        IERC20(stakingToken).approve(address(starRouter), amountOut);
        starRouter.addLiquidity(pid, amountOut, address(this));
    }

    function harvester() public {
        // claim STG
        gauge.deposit(spid, 0);
        _disposeOfStg();
    }
        ...
        )
    {
        ...
        if ((balance == 0 && debt == 0) || balance == debt) {
