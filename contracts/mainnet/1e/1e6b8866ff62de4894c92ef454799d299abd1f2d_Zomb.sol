...
contract Zomb is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, address _blacklist) public ERC20("ZOMB", "ZOMB") {
    ...
    function _getZombPrice() internal view returns (uint256 _zombPrice) {
        try IOracle(zombOracle).consult(address(this), 1e18) returns (uint144 _price) {
    ...
    function _updateTaxRate(uint256 _zombPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_zombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setZombOracle(address _zombOracle) public onlyOperatorOrTaxOffice {
        require(_zombOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentZombPrice = _getZombPrice();
            currentTaxRate = _updateTaxRate(currentZombPrice);
            if (currentZombPrice < burnThreshold) {
                burnTax = true;
            }
        }
