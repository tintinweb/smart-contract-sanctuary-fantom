...
abstract contract DeliAuth {
    ...
    modifier onlyAuth() {
        require(isAuth[msg.sender] || msg.sender == admin, "Deliland: FORBIDDEN (auth)");
        _;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "Deliland: FORBIDDEN (admin)");
        _;
    }
...
// The goal of this farm is to allow a stake pENERg earn anything model
// Each new pool added is a new reward token, each with its own start times
// end times, and rewards per second.
contract EnerGProviderRewards is ReentrancyGuard, DeliAuth {
    ...
    struct PoolInfo { 
        ...
        uint8 TokenPrecision;         //The precision factor used for calculations, equals the tokens decimals
        uint pENERgStakedAmount;      //# of penerg allocated to this pool
        uint RewardPerSecond;         //reward token per second for this pool in wei
        uint accRewardPerShare;       //Accumulated reward per share, times the pools token precision. See below.
        ...
        uint32 lastRewardTime;        //Last block time that reward distribution occurs.
        uint32 endTime;               //end time of pool
        uint32 startTime;             //start time of pool
    }


    IERC20 public immutable penerg;
    ...
    // Total staked amount of penerg in all pools by user
    ...
    constructor(IERC20 _penerg) {
        penerg = _penerg;
        isRewardToken[address(_penerg)] = true;
    }
    ...
    function pendingReward(uint _pid, address _user) external view returns (uint) {
        uint penergReward = pendingRewards(_pid, _user);
        return penergReward;
    }

    function pendingRewards(uint _pid, address _user) public view returns (uint penergReward) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if(pool.pENERgStakedAmount != 0)
                accRewardPerShare += reward * precisionOf[pool.TokenPrecision] / pool.pENERgStakedAmount;
        }
        penergReward = (user.amount * accRewardPerShare / precisionOf[pool.TokenPrecision]) - user.rewardDebt;
    ...
    function updatePool(uint _pid) public {
        ...
        if(pool.pENERgStakedAmount != 0) pool.accRewardPerShare += reward * precisionOf[pool.TokenPrecision] / pool.pENERgStakedAmount;
    ...
    function deposit(uint _pid, uint _amount, address to) external nonReentrant {
        _deposit(_pid, _amount, to);
    }

    // Deposit tokens.
    function _deposit(uint _pid, uint _amount, address to) internal {
        ...
        user.amount += _amount;
        amount += _amount;
        pool.pENERgStakedAmount += _amount;
        balanceOf[to] += _amount;

        user.rewardDebt = amount * pool.accRewardPerShare / precision;

        if(pending > 0)
            safeTransfer(pool.RewardToken, to, pending);
        if(_amount > 0)
            penerg.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function withdraw(uint _pid, uint _amount, address to) external nonReentrant {
        _withdraw(_pid, _amount, to);
    }

    function _withdraw(uint _pid, uint _amount, address to) internal {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if(pending > 0)
            safeTransfer(pool.RewardToken, to, pending);
        if(_amount > 0)
            safeTransfer(penerg, to, _amount);
    ...
    function emergencyWithdraw(uint _pid) external {
        ...
        pool.pENERgStakedAmount -= user.amount;
        user.amount = 0;
        user.rewardDebt = 0;
        balanceOf[msg.sender] -= oldUserAmount;

        penerg.safeTransfer(address(msg.sender), oldUserAmount);
    ...
    function checkForToken(IERC20 _Token) private view {
        require(!isRewardToken[address(_Token)], "checkForToken: reward token or penerg provided");
    }
    ...
    function _add(uint _rewardPerSecond, IERC20Ext _Token, uint32 _startTime, uint32 _endTime, address _protocolOwner) internal {
        require(_rewardPerSecond > 9, "_add: _rewardPerSecond needs to be at least 10 wei");
