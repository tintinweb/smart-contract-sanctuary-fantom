...
contract SlotMachine is Ownable {
    ...
    function PlaySlotMachine(uint betAmount) public {
        ...
        uint[4] memory reelIndex = getRandomNumbers(Jackpot);
    ...
    function getRandomNumbers(uint _jackpot) public view returns (uint256[4] memory) {
    uint256 randomNumber = uint256(keccak256(abi.encodePacked(_jackpot, block.difficulty, block.timestamp, block.number)));
    return [
      randomNumber % reel1.length,
      randomNumber / reel1.length % reel2.length,
      randomNumber / reel1.length / reel2.length % reel3.length,
      randomNumber / reel1.length / reel2.length / reel3.length % 10
    ];
}
