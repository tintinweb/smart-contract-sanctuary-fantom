...
// Note that this pool has no minter key of SPINACH (rewards).
// Instead, the governance will call SPINACH distributeReward method and send reward to this pool at the beginning.
contract SpinachRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SPINACHs to distribute in the pool.
        uint256 lastRewardTime; // Last time that SPINACHs distribution occurred.
        uint256 accTombPerShare; // Accumulated SPINACHs per share, times 1e18. See below.
    ...
    // The time when SPINACH mining starts.
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SpinachRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "SpinachRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's SPINACH allocation point. Can only be called by the owner.
    ...
    // View function to see pending SPINACHs on frontend.
    ...
    // Safe tomb transfer function, just in case if rounding error causes pool to not have enough SPINACHs.
