...
import "../interfaces/odin/IStrategy.sol";
 ...
 */
contract OdinVaultV7 is ERC20, Ownable, ReentrancyGuard {
    ...
    function withdraw(uint256 _shares) public onlyOwner {
        ...
        uint256 wantBal = balance().mul(_shares).div(totalSupply());
        _burn(msg.sender, _shares);

        uint256 vaultBal = want().balanceOf(address(this));
        if (vaultBal < wantBal) {
            strategy.withdraw(wantBal.sub(vaultBal));
            vaultBal = want().balanceOf(address(this));
        }
        ...
        want().safeTransfer(msg.sender, wantBal);
    }
    ...
    function inCaseROSEGetStuck() external onlyOwner {
...
interface IStrategy {
    ...
    function withdrawAndHarvest(uint256) external;
