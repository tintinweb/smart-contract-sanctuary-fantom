/**
 *Submitted for verification at FtmScan.com on 2023-07-17
*/
...
contract Raffle is Ownable {
    address public dev = 0x058ee28E10313C5b3d7a1fbB38eD840e32F57189;
    address public winner;
    address public protocolWallet = 0x058ee28E10313C5b3d7a1fbB38eD840e32F57189;


    IERC20 public erc20Token;
    IERC20 public utilityToken;
    ERC20Burnable public erc20burner;
    ...
    uint256 ticketPrice = 8548200000000000000000000000;
    uint256 discountPrice = 100000000000000000000;
    uint raffleEndTime;

    using SafeMath for uint256;

    uint256 prizePool;
    uint256 utilityPrizePool;
    uint256 burnedTokens;
    uint256 split;
    uint256 devFee;
    uint256 utilityDevFee;
    uint256 protocolFee;
    uint256 utilityProtocolFee;
    uint256 ticketsSold;
    uint256 addedAmount;

    // A check to see if the winner has been found
    bool public raffleOver = false;
    bool public utilityRaffle = false;
    ...
    function setERCBurnable(ERC20Burnable newBurnToken) public onlyOwner {erc20burner = newBurnToken;}

    function setERCUtilityToken(IERC20 newUtilityToken) public onlyOwner {
        utilityToken = newUtilityToken;
        utilityRaffle = true;
    }

    function burnOther(uint256 amount) external {
        erc20burner.burnFrom(msg.sender, amount);
    }

    function setProtocolWallet(address newProtocolWallet) public onlyOwner {protocolWallet = newProtocolWallet;}

    function setNewStacksPrice(uint256 newStacksPrice) public onlyOwner {discountPrice = newStacksPrice;}
    ...
    function getBurnedTokens() public view returns(uint256) { return burnedTokens; }
    ...
    function getUtilityPrizePool() public view returns (uint256) { return utilityPrizePool; }
    ...
    function getTicketDiscountPrice() public view returns (uint) { return discountPrice; }
    ...
    function buyTicket(uint256 amount) public {
        ...
        IERC20(erc20Token).transferFrom(msg.sender, address(this), totalAmount);
            ...
            split = (totalAmount.mul(3)).div(10);
            uint256 devFeeSplit = totalAmount.div(10);
        ...
        function buyDiscount(uint256 amount) public {
        require(raffleOver == false);
        require(block.timestamp <= raffleEndTime, "The raffle has ended.");
        uint256 totalDiscounted = discountPrice.mul(amount);
        uint256 discountDifference = totalDiscounted.div(10);
        uint256 burnAmount = discountDifference;
        uint256 finalDiscountPrice = totalDiscounted.sub(burnAmount);

        IERC20(utilityToken).transferFrom(msg.sender, address(this), finalDiscountPrice);

            erc20burner.burnFrom(msg.sender, burnAmount);
            burnedTokens += burnAmount;

            

            split = (finalDiscountPrice.mul(3)).div(10);
            uint256 devFeeSplit = finalDiscountPrice.div(10);
            uint256 protocolFeeSplit = split.sub(devFeeSplit);
            uint256 prizePoolSplit = finalDiscountPrice.sub(split);

            utilityDevFee += devFeeSplit;
            utilityProtocolFee += protocolFeeSplit;
            utilityPrizePool += prizePoolSplit;
        ...
        //multiply by three then divide by four for 75 per//

            split = (amount.mul(3)).div(10);
            uint256 devFeeSplit = amount.div(10);
            uint256 protocolFeeSplit = split.sub(devFeeSplit);
    ...
    function payout() public onlyOwner {
        ...
        utilityToken.transfer(winner, utilityPrizePool);
        utilityPrizePool = 0;
        utilityToken.transfer(dev, utilityDevFee);
        utilityDevFee = 0;
        utilityToken.transfer(protocolWallet, utilityProtocolFee);
        utilityProtocolFee = 0;}
    }

    // for any incorrect tokens received
    function withdrawIncorrectTokens(address _tokenContract) external onlyOwner {
