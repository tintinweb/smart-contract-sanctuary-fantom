...
interface IRarity {
    function class(uint tokenID) external view returns (uint);
    function level(uint tokenID) external view returns (uint);
}
...
contract NFTMarket {
    ...
    function getClassAndLevel(uint tokenID) external view returns (uint class_, uint level_) {
        class_ = IRarity(nft).class(tokenID);
        level_ = IRarity(nft).level(tokenID);
    }
