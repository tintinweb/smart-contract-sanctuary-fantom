...
contract ArbV5 is Operator {
        ...
        )
    {
        ...
        uint256 _amt = this.getAmount(config.privatePool, config.publicPool, config.base);
        ...
        if (pricePublic > pricePrivate) {
            priceDiffPercentage = (pricePublic - pricePrivate).mul(10000).div(pricePrivate);
            _addr1 = config.privateRouter;
            _addr2 = config.publicRouter;
        } else {
            priceDiffPercentage = (pricePrivate - pricePublic).mul(10000).div(pricePrivate);
