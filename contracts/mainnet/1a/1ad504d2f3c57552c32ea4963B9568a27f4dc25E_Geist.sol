    ...
    IFarm
{
    ...
    UnderlyingAsset[] public override underlyingSupported;
    // the token that represents the position/stake/deposit
    mapping(IERC20MetadataUpgradeable => address) public override positionToken;
    IConfig public override config;

    function __init__(IConfig c, PositionToken[] memory positionTokenInput)
        internal
        onlyInitializing
    {
        require(positionTokenInput.length <= type(uint8).max, "FM1");
        for (uint8 i; i < positionTokenInput.length; i++) {
            underlyingSupported.push(positionTokenInput[i].underlying);
            positionToken[
                positionTokenInput[i].underlying.token
            ] = positionTokenInput[i].positionToken;
        }
        config = c;
    ...
    {
        return underlyingSupported;
    }

    function deployTokenAll(IERC20MetadataUpgradeable underlying)
        external
        override
        nonReentrant
    {
        _deploy(underlying.balanceOf(msg.sender), underlying, msg.sender);
    }

    function deployToken(uint256 amount, IERC20MetadataUpgradeable underlying)
        external
        override
        nonReentrant
    {
        _deploy(amount, underlying, msg.sender);
    }

    function deployNative() external payable override nonReentrant {
        _deployNative(msg.value, msg.sender);
    }

    function withdrawTokenAll(IERC20MetadataUpgradeable underlying)
        external
        override
        nonReentrant
    {
        _withdrawAll(underlying, msg.sender);
    }

    function withdrawToken(IERC20MetadataUpgradeable underlying, uint256 amount)
        external
        override
        nonReentrant
    {
    ...
    function withdrawNative() external payable override nonReentrant {
        _withdrawNative(msg.value, msg.sender);
    }
    ...
    function _deploy(
        ...
        _deployImpl(amount, underlying, onBehalfOf);
        emit TokenDeployed(onBehalfOf, underlying, amount);
    }

    function _deployNative(uint256 amount, address onBehalfOf) internal {
        _deployNativeImpl(amount, onBehalfOf);
        emit NativeDeployed(onBehalfOf, amount);
    }
    ...
    function _withdrawAll(
        ...
        emit TokenWithdrawn(
            onBehalfOf,
            underlying,
            _withdrawAllImpl(underlying, onBehalfOf)
        );
    }
    ...
    function _withdraw(
        ...
        _withdrawImpl(underlying, amount, onBehalfOf);
        emit TokenWithdrawn(onBehalfOf, underlying, amount);
    }

    function _withdrawNative(uint256 amount, address onBehalfOf) internal {
        _withdrawNativeImpl(amount, onBehalfOf);
        emit NativeWithdrawn(onBehalfOf, amount);
    }
    ...
    function _deployNativeImpl(uint256 amount, address onBehalfOf)
        internal
        virtual;

    function _withdrawAllImpl(
        IERC20MetadataUpgradeable underlying,
        address onBehalfOf
    ...
    function _withdrawNativeImpl(uint256 amount, address onBehalfOf)
        internal
        virtual;
    ...
    function _position(address user)
        internal
        view
        returns (Position[] memory positions)
    {
        uint256 len = underlyingSupported.length;
        positions = new Position[](len);
        for (uint8 i; i < len; i++) {
            UnderlyingAsset memory u = underlyingSupported[i];
            positions[i].underlying = u;
            positions[i].amount = _balanceOfUnderlying(
                user,
                positionToken[u.token]
            );
        }
    }

    function _balanceOfUnderlying(address user, address positionToken)
        internal
        view
        virtual
        returns (uint256);

    function _updateBalanceOfUnderlying(address user, address positionToken)
        internal
        virtual
        returns (uint256);
}
...
contract Geist is BaseFarm {
    using SafeERC20Upgradeable for IERC20MetadataUpgradeable;

    ILendingPoolAddressesProvider public addrProvider;

    function _initialize(
        IConfig c,
        PositionToken[] memory positionTokenMapping,
        ILendingPoolAddressesProvider lpap
    ) external initializer {
        BaseFarm.__init__(c, positionTokenMapping);
        addrProvider = lpap;
    }
    ...
    function _deployImpl(
        ...
        require(amount > 0, "GS1");
        require(address(underlying) != address(0), "GS2");
        require(onBehalfOf != address(0), "GS3");
        IAToken gToken = IAToken(positionToken[underlying]);
        require(address(gToken) != address(0), "GS4");
        ILendingPool lendingPool = ILendingPool(addrProvider.getLendingPool());
        underlying.safeTransferFrom(msg.sender, address(this), amount);
        underlying.safeIncreaseAllowance(address(lendingPool), amount);
        lendingPool.deposit(address(underlying), amount, onBehalfOf, 0);
    }

    function _deployNativeImpl(uint256, address) internal pure override {
    ...
    function _withdrawAllImpl(
        ...
        require(address(underlying) != address(0), "GS2");
        require(onBehalfOf != address(0), "GS3");
        address pToken = positionToken[underlying];
        require(pToken != address(0), "GS4");
        ILendingPool lendingPool = ILendingPool(addrProvider.getLendingPool());
        uint256 amount = _balanceOfUnderlying(msg.sender, pToken);
        IERC20MetadataUpgradeable(pToken).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );
        ...
        lendingPool.withdraw(address(underlying), amount, onBehalfOf);
        return amount;
    }

    function _withdrawNativeImpl(uint256, address) internal pure override {
        // native not supported, yet
        revert("VS5");
    }

    function _balanceOfUnderlying(address user, address _positionToken)
        internal
        view
        override
        returns (uint256)
    {
        return IAToken(_positionToken).balanceOf(user);
    }

    function _updateBalanceOfUnderlying(address user, address _positionToken)
        internal
        view
        override
        returns (uint256)
    {
        return _balanceOfUnderlying(user, _positionToken);
    }
