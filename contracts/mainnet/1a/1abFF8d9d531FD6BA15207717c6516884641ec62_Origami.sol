...
contract Origami is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_REWARD_POOL_DISTRIBUTION = 20000 ether;
    ...
    /**
     * @notice Constructs the ORIGAMI ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ORIGAMI", "ORIGAMI") {
        // Mints 1 ORIGAMI to contract creator for initial pool setup
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        excludeAddress(address(this));

        _mint(msg.sender, 500 ether); // mint 500 ORIGAMi for initial liquidity
        taxRate = _taxRate;
    ...
    function _getOrigamiPrice() internal view returns (uint256 _origamiPrice) {
        try IOracle(origamiOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Origami: failed to fetch ORIGAMI price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _origamiPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_origamiPrice >= taxTiersTwaps[tierId]) {
    ...
    function setOrigamiOracle(address _origamiOracle) public onlyOperatorOrTaxOffice {
        require(_origamiOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ORIGAMI to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ORIGAMI to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentOrigamiPrice = _getOrigamiPrice();
            currentTaxRate = _updateTaxRate(currentOrigamiPrice);
            if (currentOrigamiPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(address _genesisPool) external onlyOperator {
        ...
        _mint(_genesisPool, INITIAL_REWARD_POOL_DISTRIBUTION);
    }
