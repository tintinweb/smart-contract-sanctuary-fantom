...
contract Minter is IMinter, Ownable {
    ...
    uint public constant MAX_GROWTH_RATE = 200; // 20%
    ...
    uint public growth = 100; // 10%


    event Mint(address indexed sender, uint weekly, uint total_supply, uint circulating_emission);
    event TeamSet(address indexed account);
    event VoterSet(address indexed account);
    event GrowthSet(uint256 rate);
    event TeamRateSet(uint256 rate);
    ...
    function setGrowth(uint256 _growth) external onlyOwner {
        require(_growth <= MAX_GROWTH_RATE, "rate too high");
        growth = _growth;
        emit GrowthSet(_growth);
    }

    function setTeamRate(uint _teamRate) external onlyOwner {
