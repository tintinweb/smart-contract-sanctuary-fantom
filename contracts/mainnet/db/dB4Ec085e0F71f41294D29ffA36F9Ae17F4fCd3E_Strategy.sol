...
contract Strategy is BaseStrategy {
    ...
    IERC20 public rewardToken = IERC20(0xCbE0CA46399Af916784cADF5bCC3aED2052D6C45); // LSHARE
    address public TSHARE = address(0x4cdF39285D7Ca8eB3f090fDA0C069ba5F4145B37); // TSHARE
    IUniPair public lpToken = IUniPair(0xfca12A13ac324C09e9F43B5e5cfC9262f3Ab3223);
    address public tomb = address(0x6c021Ae822BEa943b2E66552bDe1D2696a53fbB7);

    uint256 public dust = 1e19;
    uint256 public rewardDust = 1e15;

    bool public collectFeesEnabled = false;
    bool public simpleAccounting = false;
    uint256 public swapSecurityFactor = 10000;
    uint256 public sellSecurityFactor = 2500;
    uint256 public profitLiquidationFactor = 10000;

    address public router = address(0x6D0176C5ea1e44b08D3dd001b0784cE42F47a3A7);
    address public swapRouter = address(0x6D0176C5ea1e44b08D3dd001b0784cE42F47a3A7);

    IMasterChef public masterChef; // 0x1F832dfBA15346D25438Cf7Ac683b013Ed03E32f
    uint8 public masterChefPoolId; // 0

    address[] public rewardToWantRoute = [address(rewardToken), TSHARE, wftm];
    ...
    function name() external view override returns (string memory) {
        return 'TombSwap FTM-TOMB Strategy v3';
    }
    ...
    function balanceOfLpToken1() public view returns (uint256) {
        return IERC20(tomb).balanceOf(address(this));
    }
    ...
    function estimatedHarvest() public view returns (uint256) {
        uint256 _rewards = pendingRewards();
        if (_rewards == 0) return 0;
        uint256[] memory amounts = IUni(router).getAmountsOut(_rewards, rewardToWantRoute);
        return amounts[2];
    }
    ...
    function balanceOfPooled() public view returns (uint256 _amount) {
        ...
        uint256 ftmPerTOMB = IUni(router).getAmountOut(1e18, reserveB, reserveA);
        uint256 totalReservesInFTM = ftmPerTOMB.mul(reserveB).div(1e18).add(reserveA);
    ...
    function liquidateAllPositions() internal override returns (uint256 _liquidated) {
        ...
        _withdrawFromMasterChef(balanceOfLPInMasterChef());
    ...
    function _swap(uint256 _fullInvestment) internal returns (uint256[] memory _swapedAmounts) {
        (uint256 reserveA, uint256 reserveB, ) = lpToken.getReserves();
        uint256 swapAmountIn = _getSwapAmount(_fullInvestment, reserveA, reserveB);
        _swapedAmounts = IUni(swapRouter).swapExactTokensForTokens(
            swapAmountIn,
    ...
    function _removeLiquidity(uint256 toExitAmount) internal {
        (uint256 reserveA, uint256 reserveB, ) = lpToken.getReserves();
        uint256 totalSupply = lpToken.totalSupply();

        uint256 TombPerFTM = IUni(router).getAmountOut(1e18, reserveB, reserveA);
        uint256 totalReservesInFTM = TombPerFTM.mul(reserveB).div(1e18).add(reserveA);

        uint256 ftmPerLP = totalReservesInFTM.mul(1e18).div(totalSupply);
        uint256 liquidityToExit = toExitAmount.mul(1e18).div(ftmPerLP);

        uint256 toExit = Math.min(liquidityToExit, balanceOfLPInMasterChef());

        if (toExit > 0) {
            _withdrawFromMasterChef(toExit);
    ...
    function _withdrawFromMasterChef(uint256 amount) internal {
        uint256 toWithdraw = Math.min(balanceOfLPInMasterChef(), amount);
        if (toWithdraw > 0) {
            masterChef.withdraw(masterChefPoolId, toWithdraw);
        }
    }
    ...
    function _withdrawFromMasterChefAndTransfer(address _to) internal {
        ...
        uint256 lpToken1Amount = balanceOfLpToken1();
        if (lpToken1Amount > 0) {
            IERC20(tomb).transfer(_to, lpToken1Amount);
        }
    }
    ...
    function _getLpTokens(uint256 _amountIn) internal {
        if (_amountIn > 0) {
            uint256[] memory swapedAmounts = _swap(_amountIn);
            _addLiquidity(_amountIn.sub(swapedAmounts[0]), swapedAmounts[1]);
        }
    }

    function _giveSwapRouterAllowance() internal {
        want.safeApprove(swapRouter, 0);
        want.safeApprove(swapRouter, max);

        IERC20(tomb).safeApprove(swapRouter, 0);
        IERC20(tomb).safeApprove(swapRouter, max);
    }

    function _removeSwapRouterAllowance() internal {
        want.safeApprove(swapRouter, 0);
        IERC20(tomb).safeApprove(swapRouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        want.safeApprove(router, max);

        IERC20(tomb).safeApprove(router, 0);
        IERC20(tomb).safeApprove(router, max);

        IERC20(rewardToken).safeApprove(router, 0);
        IERC20(rewardToken).safeApprove(router, max);

        _giveSwapRouterAllowance();
    }
    ...
    function enforceSlippageIn(uint256 _amountIn, uint256 _pooledBefore) internal view {
        uint256 balancePooled = balanceOfPooled();
        uint256 pooledDelta = 0;
        uint256 joinSlipped = 0;
        if (balancePooled > _pooledBefore) {
            pooledDelta = balancePooled.sub(_pooledBefore);
            joinSlipped = _amountIn > pooledDelta ? _amountIn.sub(pooledDelta) : 0;
        } else {
            joinSlipped = _pooledBefore.sub(balancePooled);
        }
    ...
    function setDust(uint256 _dust, uint256 _wantDust) external onlyVaultManagers {
        dust = _dust;
        rewardDust = _wantDust;
    }

    function setSwapRouter(address _router) external onlyVaultManagers {
        _removeSwapRouterAllowance();
        swapRouter = _router;
        _giveAllowances();
    }

    function sellTombTokens() external onlyVaultManagers {
        _sellTombTokens();
    }

    function depositLpIntoMasterChef() external onlyVaultManagers {
        _depositLpIntoMasterChef();
    }
    ...
    function emergencyWithdrawFromMasterChef(uint256 _amount) external onlyVaultManagers {
        _amount > 0
            ? _withdrawFromMasterChef(_amount)
            : _withdrawFromMasterChefAndTransfer(address(this));
    }

    function giveAllowances() external onlyVaultManagers {
        _giveAllowances();
    }

    function setRewardToWantRoute(address[] memory _rewardToWantRoute) external onlyVaultManagers {
    ...
    function setMasterChefPoolId(uint8 _masterChefPoolId) external onlyVaultManagers {
        masterChefPoolId = _masterChefPoolId;
    }

    function setSecurityFactors(uint256 _swapSecurityFactor, uint256 _sellSecurityFactor)
        external
        onlyVaultManagers
    {
        swapSecurityFactor = _swapSecurityFactor;
        sellSecurityFactor = _sellSecurityFactor;
    }

    function setCollectFeesAndLiquidationFactor(
        uint256 _profitLiquidationFactor,
        bool _collectFeesEnabled
