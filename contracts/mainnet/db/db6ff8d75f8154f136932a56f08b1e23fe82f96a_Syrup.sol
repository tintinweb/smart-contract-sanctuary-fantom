...
contract Syrup is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 SRP-WETH LP -> SRP pool
    uint256 public constant INITIAL_SRP_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the SRP ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Syrup", "SRP") {
        // Mints 1 SRP to contract creator for initial pool setup
    ...
    function _getSrpPrice() internal view returns (uint256 _srpPrice) {
        try IOracle(srpOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("SRP: failed to fetch SRP price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _srpPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_srpPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSrpOracle(address _srpOracle) public onlyOperatorOrTaxOffice {
        require(_srpOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SRP to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SRP to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSrpPrice = _getSrpPrice();
            currentTaxRate = _updateTaxRate(currentSrpPrice);
            if (currentSrpPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_srpPool != address(0), "!_srpPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_srpPool, INITIAL_SRP_POOL_DISTRIBUTION);
