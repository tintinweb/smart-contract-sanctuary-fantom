...
contract Oracle is IOracle {
    ...
    Roles.Role private _priceSetters;
    ...
    address public deiUsdc;

    uint256 public onChainPriceThreshold;
    ...
    constructor(
        ...
        onChainPriceThreshold = onChainThreshold_;

        minimumRequiredSignatures = minimumRequiredSignatures_;

        _admins.add(admin);
        _setters.add(setter);
        _priceSetters.add(priceSetter);
    }

    modifier onlyRole(Roles.Role storage _role) {
        require(_role.has(msg.sender), "Oracle: ACCESS_CONTROLL");
        _;
    }

    function grantRole(uint256 role, address user) public onlyRole(_admins) {
        if (role == 0) {
            _admins.add(user);
        } else if (role == 1) {
            _setters.add(user);
        } else if (role == 2) {
            _priceSetters.add(user);
        }
    }

    function revokeRole(uint256 role, address user) public onlyRole(_admins) {
        if (role == 0) {
            _admins.remove(user);
        } else if (role == 1) {
            _setters.remove(user);
        } else if (role == 2) {
            _priceSetters.remove(user);
        }
    }

    function setDeiUsdcPair(address deiUsdc_) external onlyRole(_setters) {
        deiUsdc = deiUsdc_;
        emit SetDeiUsdc(deiUsdc);
    }

    function setOnChainThreshold(uint256 onChainThreshold_) external onlyRole(_setters) {
        emit SetOnChainThreshold(onChainPriceThreshold, onChainThreshold_);
        onChainPriceThreshold = onChainThreshold_;
    }

    /// @notice sets muon contract address
    /// @param muon_ address of the Muon contract
    function setMuon(address muon_) external onlyRole(_setters) {
    ...
    function setAppId(uint32 appId_) external onlyRole(_setters) {
    ...
    function setValidEpoch(uint256 validEpoch_) external onlyRole(_setters) {
    ...
    function setPrice(Signature memory signature)
        public
        onlyRole(_priceSetters)
    {
        require(
        ...
        require(
            signature.timestamp > lastPriceUpdate,
    ...
    /// @notice returns on chain dei price
    function getOnChainPrice() public view returns (uint256) {
        (uint256 usdcReserve, uint256 deiReserve, ) = IBaseV1Pair(address(deiUsdc))
            .getReserves();
        uint256 deiPrice = usdcReserve.mul(1e30).div(deiReserve);
        return deiPrice;
    }
    ...
    function getUnderlyingPrice(CToken cToken) external view returns (uint256) {
        ...
        if (price > onChainPrice) {
            require(price - onChainPrice < onChainPriceThreshold, "Oracle: PRICE_GAP");
        } else {
            require(onChainPrice - price < onChainPriceThreshold, "Oracle: PRICE_GAP");
        }
...
interface IOracle {
    ...
    event SetDeiUsdc(address deiUsdc);
