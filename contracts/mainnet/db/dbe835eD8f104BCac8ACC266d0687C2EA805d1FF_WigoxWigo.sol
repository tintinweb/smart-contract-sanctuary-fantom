...
contract WigoxWigo is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wrapped = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    address public constant xwigo = address(0xB9bBe9355828A1aD0e2d605480f969c6903Ad9e4);
    ...
    address[] public wigoToXwigoPath;
    ...
    function chargeFees(address caller) internal {                   
        uint256 toWrapped = IERC20(wigo).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        uint256 wigoHalf = IERC20(wigo).balanceOf(address(this)) / 2;

        IUniSwapRouter(unirouter).swapExactTokensForTokens(wigoHalf, 0, wigoToXwigoPath, address(this), block.timestamp);
    
        uint256 lp0Bal = IERC20(wigo).balanceOf(address(this));
        uint256 lp1Bal = IERC20(xwigo).balanceOf(address(this));

        IUniSwapRouter(unirouter).addLiquidity(wigo, xwigo, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(xwigo).safeApprove(unirouter, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(xwigo).safeApprove(unirouter, type(uint).max);
    }
