...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR axlUSDC-FTM EQUALIZER")),
            string(abi.encodePacked("XpE-axlUSDC-FTM"))
        )
        {
        gauge = _gauge;
        router = _router;
    ...
    function _addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmt, wftm, axlUsdc, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(wftm, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(axlUsdc, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(wftm, axlUsdc, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(wftm, axlUsdc, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyAdmin {
        if(_token == equal || _token == wftm || _token == axlUsdc){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
