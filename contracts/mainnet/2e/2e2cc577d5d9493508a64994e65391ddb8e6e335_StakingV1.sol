...
contract StakingV1 is Initializable, OwnableUpgradeable, IERC721Receiver {
    using SafeMath for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    bool public disableStaking;
    bool public disableUnstaking;
    ...
    // address => number_of_neko_staking
    mapping(address => uint256) public balances;

    // tracking reward token
    mapping(address => bool) public isPrizeToken;

    event Staked(address staker, uint256[] ids);
    event Unstaked(address staker, uint256[] ids);
    ...
    /////////////////////////////////////////////////////
    //  STAKING RELATED FUNCTION ////////////////////////
    /////////////////////////////////////////////////////
    function stakeNekosFor(uint256[] calldata tokenIds, address staker)
        external
    {
        require(!disableStaking, "staking disable");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                NekoContract.ownerOf(tokenIds[i]) == msg.sender,
                "NFTStakingV1: not owner"
            );
            ...
            owners[tokenIds[i]] = staker;
        }
        totalSupply = totalSupply + tokenIds.length;
        balances[staker] = balances[staker] + tokenIds.length;
        emit Staked(staker, tokenIds);
    }

    function unstakeNekos(uint256[] calldata tokenIds) external {
        require(!disableUnstaking, "unstake disable");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(owners[tokenIds[i]] == msg.sender, "not owner");
            NekoContract.safeTransferFrom(
                address(this),
                msg.sender,
                tokenIds[i]
        ...
        totalSupply = totalSupply - tokenIds.length;
        balances[msg.sender] = balances[msg.sender] - tokenIds.length;
        emit Unstaked(msg.sender, tokenIds);
    }

    function setDisable(bool stake, bool unstake) external onlyOwner {
        disableStaking = stake;
        disableUnstaking = unstake;
    }
    ...
    // retuns earned amount based on user and reward id
