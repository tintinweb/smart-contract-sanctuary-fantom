...
interface IImpStaking {
  function stake(uint256 _amount, address _recipient) external returns (bool);

  function claim(address _recipient) external;
}

contract ImpStakingHelper {
  event LogStake(address indexed recipient, uint256 amount);

  IImpStaking public immutable staking;
  IERC20 public immutable IMP;

  constructor(address _staking, address _IMP) {
    require(_staking != address(0));
    staking = IImpStaking(_staking);
    require(_IMP != address(0));
    IMP = IERC20(_IMP);
  }

  function stake(uint256 _amount, address recipient) external {
    IMP.transferFrom(msg.sender, address(this), _amount);
    IMP.approve(address(staking), _amount);
