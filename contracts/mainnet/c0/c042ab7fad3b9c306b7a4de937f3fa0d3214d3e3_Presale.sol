...
contract Presale is ReentrancyGuard {
    ...
    // Price of presale TOKEN in terms of $
    uint256 tokenPrice;
    // Amount of stables received in presale
    uint256 stableReceived = 0;
    // Hard cap in terms of $
    uint256 hardCap;
    // Stablecoin
    IBEP20 STABLE;
    // decimals of stable in uint256
    uint256 decimals;
    ...
    constructor(
        ...
        TOKEN = IBEP20(_TOKEN);
        STABLE = IBEP20(_STABLE);
        decimals = uint256(STABLE.decimals());
        owner = msg.sender;
        startingTimeStamp = _startingTimeStamp;
        endingTimeStamp = _endingTimeStamp;
        hardCap = _hardCap.mul(uint256(10)**decimals);
        tokenPrice = _tokenPrice;
    }

    function buyTokens(uint256 _amount, address _buyer) public nonReentrant {
        require(
            block.timestamp >= startingTimeStamp,
            "Presale has not started"
        );
        require(block.timestamp <= endingTimeStamp, "Presale has ended");

        address buyer = _buyer;
        uint256 tokens = _amount.div(tokenPrice).mul(
            uint256(10)**(uint256(18).sub(decimals))
        );

        require(stableReceived + _amount <= hardCap, "Presale hardcap reached");

        STABLE.safeTransferFrom(buyer, address(this), _amount);

        tokensOwned[buyer] = tokensOwned[buyer].add(tokens);
        tokensUnclaimed[buyer] = tokensUnclaimed[buyer].add(tokens);
        totalTokensSold = totalTokensSold.add(tokens);
        stableReceived = stableReceived.add(_amount);
        emit TokenBuy(buyer, tokens);
    }

    function setStartingTimeStamp(uint256 _startingTimeStamp)
        external
        onlyOwner
    {
        startingTimeStamp = _startingTimeStamp;
    ...
    function setEndingTimeStamp(uint256 _endingTimeStamp) external onlyOwner {
        endingTimeStamp = _endingTimeStamp;
    }
    ...
    function getStableReceived() external view returns (uint256) {
    ...
    function withdrawUnsold() external onlyOwner {
        uint256 amount = TOKEN.balanceOf(address(this)).sub(totalTokensSold);
