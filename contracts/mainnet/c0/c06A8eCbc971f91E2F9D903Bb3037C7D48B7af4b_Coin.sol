...
// File: Mario/openzeppelin/ERC20.sol
/**
...
// File: Mario/openzeppelin/ERC20Burnable.sol
/**
...
contract Coin is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 COIN-WFTM LP -> COIN pool
    // uint256 public constant INITIAL_COIN_POOL_DISTRIBUTION = 140000 ether;
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("COIN", "COIN") {
        // Mints 1 COIN to contract creator for initial pool setup
    ...
    function _getCoinPrice() internal view returns (uint256 _coinPrice) {
        try IOracle(coinOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Coin: failed to fetch COIN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _coinPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_coinPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCoinOracle(address _coinOracle) public onlyOperatorOrTaxOffice {
        require(_coinOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCoinPrice = _getCoinPrice();
            currentTaxRate = _updateTaxRate(currentCoinPrice);
            if (currentCoinPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_coinPool != address(0), "!_coinPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_coinPool, INITIAL_COIN_POOL_DISTRIBUTION);
