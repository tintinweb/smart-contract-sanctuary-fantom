...
// File contracts/wPgunkFtmEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract wPgunkFtmEqualizerV2  is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalTowPgunkPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalTowPgunkPath,
        ...
        for (uint i; i < _equalTowPgunkPath.length; ++i) {
            equalTowPgunkPath.push(_equalTowPgunkPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalHalf, 0, equalTowPgunkPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(wftm).balanceOf(address(this));
        uint256 t2Bal = IERC20(wpgunk).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(wftm, wpgunk, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(wpgunk).safeApprove(router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(wpgunk).safeApprove(router, type(uint).max);
    }
