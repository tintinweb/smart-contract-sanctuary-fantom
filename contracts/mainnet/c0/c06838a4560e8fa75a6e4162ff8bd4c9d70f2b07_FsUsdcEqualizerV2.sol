...
// File contracts/FsUsdcEqualizerV2.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract FsUsdcEqualizerV2 is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public equalToFsPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _equalToFsPath,
        ...
        for (uint i; i < _equalToFsPath.length; ++i) {
            equalToFsPath.push(_equalToFsPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(equalBal, 0, equalToFsPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(usdc).balanceOf(address(this));
        uint256 t2Bal = IERC20(fs).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(usdc, fs, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
