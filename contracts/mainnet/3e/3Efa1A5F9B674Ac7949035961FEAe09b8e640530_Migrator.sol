...
import "./interfaces/IRedemptionRouter01.sol";
...
contract Migrator {
    ...
    IRedemptionRouter01 public router;

    constructor(IUniswapV2Router01 _oldRouter, IRedemptionRouter01 _router) public {
    ...
    function pairForOldRouter(address tokenA, address tokenB) internal view returns (address pair) {
        (address token0, address token1) = RedemptionLibrary.sortTokens(tokenA, tokenB);
        IUniswapV2Factory factory = IUniswapV2Factory(oldRouter.factory());
    ...
    function addLiquidity(
        ...
        address pair = RedemptionLibrary.pairFor(router.factory(), tokenA, tokenB);
        IERC20(tokenA).safeTransfer(pair, amountA);
        IERC20(tokenB).safeTransfer(pair, amountB);
        IRedemptionPair(pair).mint(msg.sender);
    }
    ...
    function _addLiquidity(
        ...
        IRedemptionFactory factory = IRedemptionFactory(router.factory());
        if (factory.getPair(tokenA, tokenB) == address(0)) {
            factory.createPair(tokenA, tokenB);
        }
        (uint256 reserveA, uint256 reserveB) = RedemptionLibrary.getReserves(address(factory), tokenA, tokenB);
