...
contract Zapper is Ownable {
    ...
    function nativeZapIn(address out, address routerAddr, address recipient, uint256 minAmountOfLp) external payable whitelist (routerAddr) {
        //  IERC20(WNATIVE).safeTransferFrom(msg.sender, address(this), msg.value);
        //  _approveTokenIfNeeded(WNATIVE, routerAddr);
        uint256 amountOfLp = _swapNativeToLP(out, msg.value, recipient, routerAddr);
