...
contract WitnessIDO is Protected {
    ...
    mapping(uint => uint) public round_in;
    ...
    function partecipate() public payable safe {
        ...
        require(round_in[current_round] <= round_limit, "Round is full");
        require(!in_round[msg.sender]);
        require(msg.value == round[current_round], "Wrong value");

        round_in[current_round] += 1;
        round_value[current_round] += msg.value;
    ...
    function get_round() public view returns(uint currentround, uint startime, uint endtime, uint partecipating, uint funds, uint rin) {
        return (current_round,
                current_round_started_time,
                current_round_finish_time,
                round[current_round],
                round_value[current_round],
                round_in[current_round]);
    }

    
    function get_specific_round(uint id) public view returns(uint cost, uint funds, uint rin) {
        return (round[id],
                round_value[id],
                round_in[id]);
    }
