...
contract TaxOracle is Ownable {
    ...
    constructor(
        ...
        require(_faith != address(0), "faith address cannot be 0");
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        require(_token == address(faith), "token needs to be faith");
        uint256 faithBalance = faith.balanceOf(pair);
        uint256 wftmBalance = wftm.balanceOf(pair);
        return uint144(wftmBalance.mul(_amountIn).div(faithBalance));
    }

    function getTombBalance() external view returns (uint256) {
	return faith.balanceOf(pair);
    }
    ...
    function getPrice() external view returns (uint256) {
        uint256 faithBalance = faith.balanceOf(pair);
        uint256 wftmBalance = wftm.balanceOf(pair);
        return wftmBalance.mul(1e18).div(faithBalance);
    }


    function setTomb(address _faith) external onlyOwner {
        require(_faith != address(0), "faith address cannot be 0");
        faith = IERC20(_faith);
    }
