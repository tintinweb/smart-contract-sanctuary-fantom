...
contract DeiBonds is AccessControl, Pausable {
    ...
    uint256 public constant CLAIM_INTERVAL = 12 * 3600;
    ...
    function setAPY(address apy_) public onlyRole(TRUSTY_ROLE) {
        apy = apy_;
    }

    /// @notice pause contract
    function pause() external onlyRole(TRUSTY_ROLE) {
        super._pause();
    }
    ...
    function claim(uint256 bondId) external whenNotPaused {
        ...
        require(
            BondNFT(nft).tokensBond(bondId) == address(this),
            "DeiBond: BOND_HAS_NOT_NFT"
        );
        uint256 deusPrice = IOracle(oracle).getPrice();
        uint256 deusAmount = claimableDeus(bondId, deusPrice);
        require(deusAmount > 0, "DeiBond: NO_CLAIM_EXISTS");
        bonds[bondId].lastClaimTimestamp = block.timestamp;
        IERC20(deus).safeTransfer(msg.sender, deusAmount);
    }

    function buyBond(uint256 amount) external whenNotPaused {
        require(amount + soldBond <= capacity, "DeiBond: NO_CAPACITY");
    ...
    function exit(uint256 bondId) external whenNotPaused {
        ...
        require(
            ...
            "DeiBond: BOND_HAS_NOT_EXPIRED"
        );
        uint256 deusPrice = IOracle(oracle).getPrice();
        uint256 deusAmount = claimableDeus(bondId, deusPrice);
        require(deusAmount == 0, "DeiBond: BOND_HAS_CLAIMABLE_TOKEN");
        soldBond -= bonds[bondId].amount;
        IERC20(dei).safeTransfer(msg.sender, bond.amount * 10**12);
    ...
    function bondsOfOwner(address user) external view returns (Bond[] memory) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            bondsOfUser[i] = bonds[tokens[i]];
        }
        return bondsOfUser;
    }

    function getAPY() external view returns (uint256 apyValue, uint256 precision) {
        apyValue = Iapy(apy).getAPY();
        precision = Iapy(apy).APY_PRECISION();
    }
        ...
        returns (uint256 deusAmount)
    {
        ...
        uint256 numberOfClaims = (currentTimestamp - bond.startTime) /
            ...
            CLAIM_INTERVAL;
        uint256 totalClaims = bond.duration / CLAIM_INTERVAL;
        uint256 claimableDeusValue = (bond.amount * numberOfClaims * bond.apy) /
            (totalClaims * Iapy(apy).APY_PRECISION());
        deusAmount = (claimableDeusValue * 1e30) / deusPrice;
    }
}
...
pragma solidity ^0.8.13;
interface Iapy {
    function APY_PRECISION() external view returns (uint256);
...
contract BondNFT is AccessControl, ERC721Enumerable {
    ...
    mapping(uint256 => address) public tokensBond;
    ...
    function mint(address user, address bondContract)
        public
        onlyRole(MINTER_ROLE)
        returns (uint256)
    {
        uint256 supply = totalSupply();
        tokensBond[supply] = bondContract;
        _safeMint(user, supply);
        return supply;
    }

    function changeTokenBond(uint256 tokenId, address bondContract)
        public
        onlyRole(SET_BOUND_ROLE)
    {
        tokensBond[tokenId] = bondContract;
    }
