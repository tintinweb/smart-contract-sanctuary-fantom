...
contract AceLab is MagicatRarity {
    ...
    function setCatMultiplier(uint mul) external onlyOwner {
        mpPerXboo = mul;
    }

    function setMagicatBoost(uint32 boost) external onlyOwner {
    ...
    function changePoolUserLimitEndTime(uint _pid, uint32 _time) external onlyOwner {
    ...
    function changeBaseUserLimitTime(uint32 _time) external onlyOwner {
    ...
    function add(uint _rewardPerSecond, IERC20Ext _Token, uint32 _startTime, uint32 _endTime, address _protocolOwner) external onlyOwner {
    ...
    function setRewardPerSecond(uint _pid, uint _rewardPerSecond) external onlyOwner {
...
contract MagicatRarity is SpookyAuth {
    ...
    function setRarities(uint[] memory rarities, uint256 offset) public onlyOwner {
