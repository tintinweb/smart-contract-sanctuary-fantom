...
// File: contracts/EqualFtmLqdrvAMM.sol
...
contract EqualFtmLqdrvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public wrappedToLqdrPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToLqdrPath
    ) {
        ...
        for (uint i; i < _wrappedToLqdrPath.length; ++i) {
            wrappedToLqdrPath.push(_wrappedToLqdrPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToLqdrPath, address(this), block.timestamp);
        
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lqdr).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(wrapped, lqdr, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
