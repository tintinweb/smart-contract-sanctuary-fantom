...
contract MultiAssetPool is ReentrancyGuard, IMultiAssetPool {
    ...
    //Create struct mapping with 
    //    uint256 public netMinted;
    //    uint256 public netRedeemed;
    //    uint256 public totalUnclaimedCollateral;
    uint256 private missing_decimals;
    //    uint256 public uncollectedFee;
    //    uint256 public pool_ceiling = 0;

    //    address public collateral;// get from assetController
    //    address public asset;// get from assetController
    address public xShare; // Move to asset Controller
    ...
    function calcMint(uint256 _collateralAmount, uint256 _share_amount, uint256 _assetId) internal view returns (
        uint256 _actual_asset_amount,
        uint256 _required_share_amount,
        uint256 _fee_collect
    ) {
        (uint256 _assetPrice, uint256 _share_price, , uint256 _target_collateral_ratio, , , uint256 _minting_fee,) = IMultiAssetTreasury(treasury).info(_assetId);

        uint256 _total_asset_value = 0;
        _required_share_amount = 0;

        if (_target_collateral_ratio > 0) {
            uint256 _collateral_value = calcCollateralValue(_collateralAmount, _assetId);

            _total_asset_value = _collateral_value.mul(COLLATERAL_RATIO_PRECISION).div(_target_collateral_ratio);
            if (_target_collateral_ratio < COLLATERAL_RATIO_MAX) {
                _required_share_amount = _total_asset_value.sub(_collateral_value).mul(PRICE_PRECISION).div(_share_price);
            }
        } else {
            _total_asset_value = _share_amount.mul(_share_price).div(PRICE_PRECISION);
            _required_share_amount = _share_amount;
        }

        _actual_asset_amount = _total_asset_value.sub((_total_asset_value.mul(_minting_fee)).div(PRICE_PRECISION));
    ...
    function mint(
        ...
        (uint256 _actual_asset_amount, uint256 _required_share_amount, uint256 _fee_collect) = calcMint(_collateral_amount, _share_amount, _assetId);
    ...
    function calcRedeem(uint256 _asset_amount, uint256 _assetId) internal view returns (
        uint256 _collateral_output_amount,
        uint256 _share_output_amount,
        uint256 _fee_collect
    ) {
        (, uint256 _share_price, , , uint256 _effective_collateral_ratio, , , uint256 _redemption_fee) = IMultiAssetTreasury(treasury).info(_assetId);
        bool isStable = IAssetController(assetController).isAssetStable(_assetId);
        uint256 _missing_decimals = getMissingDecimals(_assetId);
        uint256 _asset_fee = (_asset_amount.mul(_redemption_fee)).div(PRICE_PRECISION);
        uint256 _asset_amount_post_fee = _asset_amount.sub(_asset_fee);
        _collateral_output_amount = 0;
        _share_output_amount = 0;
        uint256 _collateral_output_value = 0;

        if (_effective_collateral_ratio < COLLATERAL_RATIO_MAX) {
            uint256 _share_output_value = _asset_amount_post_fee - (_asset_amount_post_fee * _effective_collateral_ratio / PRICE_PRECISION);
            _share_output_amount = _share_output_value.mul(PRICE_PRECISION).div(_share_price);
        }

        if (_effective_collateral_ratio > 0) {
            if (isStable) {
                _collateral_output_value = _asset_amount_post_fee * _effective_collateral_ratio / (10 ** _missing_decimals) / PRICE_PRECISION;
                _collateral_output_amount = _collateral_output_value;
            } else {
                _collateral_output_value = _asset_amount_post_fee * _effective_collateral_ratio / PRICE_PRECISION;
        ...
        _fee_collect = _asset_fee.mul(_effective_collateral_ratio).div(10 ** _missing_decimals).div(PRICE_PRECISION);
    }
    ...
    function redeem(
        ...
        AssetStat storage _assetStat = assetStat[_assetId];
        (uint256 _collateral_output_amount, uint256 _share_output_amount, uint256 _fee_collect) = calcRedeem(_asset_amount, _assetId);
    ...
    //TODO add For loop to collect all
    ...
    //TODO add For loop to collect all => done
...
interface IMultiAssetTreasury {
    ...
    function globalCollateralValue(uint256 _assetId) external view returns (uint256);
...
interface IAssetController {
    function assetCount() external returns(uint256);
