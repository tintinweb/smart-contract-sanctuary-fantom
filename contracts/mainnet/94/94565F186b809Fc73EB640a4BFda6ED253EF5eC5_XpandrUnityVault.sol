...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant chill = address(0xe47d957F83F8887063150AaF7187411351643392);  //vaultProfit denominator
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR CHILL-FTM EQUALIZER")),
            string(abi.encodePacked("XpE-CHILL-FTM"))
        )
        {
        gauge = _gauge;
        router = _router;
        ...
        slippageLPs = [address(0x7547d05dFf1DA6B4A2eBB3f0833aFE3C62ABD9a1), address(0x3d6c56f6855b7Cc746fb80848755B0a9c3770122), address(_asset)];
    ...
    function _addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmt, wftm, chill, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(wftm, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(chill, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(wftm, chill, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(wftm, chill, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyAdmin {
        if(_token == equal || _token == wftm || _token == chill){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(chill, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(chill, router, type(uint).max);
    }
