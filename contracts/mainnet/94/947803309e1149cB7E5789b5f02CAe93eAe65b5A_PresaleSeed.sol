...
contract PresaleSeed is Ownable {
    ...
    uint256 public MIN_INVESTMENT = 250;
...
function invest(uint256 amount) external onlySeed {
    if(msg.sender != owner()) {
        require(amount >= MIN_INVESTMENT, "Minimum pledge not met.");
    }
    require(investOpen, "Investing is now closed.");
    ...
    if (vest.totalDue > 0) {
        tokensPurchased += vest.totalDue;
    }
    vest.totalDue = tokensPurchased;
