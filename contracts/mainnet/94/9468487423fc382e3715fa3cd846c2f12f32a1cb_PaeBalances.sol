...
contract PaeBalances is Ownable {
  ...
  constructor (
    ...
    wolfBankPid = _wolfBankPid;

    address pae = _paeRewardPool.pae();
    if (paeLp.token0() == pae) isToken0 = true;
    else if (paeLp.token1() == pae) isToken0 = false;
  ...
  function balanceOf(address account) external view returns (uint256) {
    ...
    lpBalance = lpBalance + poolBalance + balanceBeefyLP(account) + balanceWolfLP(account) + balanceWolfBank(account);
  ...
  function balanceWolfBank(address account) public view returns (uint256) {
    if (address(wolfVault) == address(0)) return 0;
    return wolfVault.stakedTokens(wolfBankPid, account);
  }
  ...
  function setYieldWolfVault(IVaultYieldWolf _wolfVault, uint256 _lpPid, uint256 _bankPid) external onlyOwner {
    wolfVault = _wolfVault;
    wolfLpPid = _lpPid;
    wolfBankPid = _bankPid;
  }
}