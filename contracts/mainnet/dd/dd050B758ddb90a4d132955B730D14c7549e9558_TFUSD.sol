...
contract TFUSD is IERC20Metadata, Ownable {
  ...
  address public thirtyAddress;
  bool public enableThirtyAutoLp;
  ...
  modifier onlyThirty() {
    require(msg.sender == thirtyAddress, "!Thirty");
    _;
  }
  ...
  function _peg(bool autoLp) internal {
  ...
  function autoPegAndAddFeeLiquidity() public onlyThirty {
  ...
  function _autoPegAndAddFeeLiquidity() internal {
    ...
    if (amounts[1] > 0 && tfusdAmount > 0) {
      ...
      _swapTfusdToDaiAddLiquidity(_autoLpDebt);
      _autoLpDebt = 0;
    }
  }
  ...
  function _transfer(
    ...
    unchecked {
      ...
      if (excludedAddress[to] == true) {
        ...
        uint256 aLpFee = (from == pairAddress || to == pairAddress) ? amount.mul(autoLpFee).div(10 ** manageDecimal) : 0;
  ...
  function _afterTokenTransfer(
    ...
    if (enableThirtyAutoLp) {
      amount = IThirty(thirtyAddress).getAutoLpThrityInAmount();
      if (amount > 0) {
        IThirty(thirtyAddress).setAutoLpFromTFUSD(amount);
      }
    }
  }
  ...
  function setThirtyAddress(address _thirtyAddress) public onlyManager {
    require(_thirtyAddress != address(0), "TFUSD: zero address");
    thirtyAddress = _thirtyAddress;
  }

  function setEnableThirtyAutoLp(bool _enableThirtyAutoLp) public onlyManager {
    enableThirtyAutoLp = _enableThirtyAutoLp;
  }
...
interface IThirty {
  function getAutoLpThrityInAmount() external view returns (uint256);
