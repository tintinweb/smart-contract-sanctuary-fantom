...
// File: contracts/WigoGeistFtm.sol
...
contract WigoGeistFTM is Ownable, Pausable {
    ...
    address[] public wrappedToGeistPath;
    ...
    constructor(
        ...
        wrappedToGeistPath = [wrapped, geist];
    ...
    function addLiquidity() internal {
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokens(wigoHalf, 0, wrappedToGeistPath, address(this), block.timestamp);
    
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(geist).balanceOf(address(this));

        IUniSwapRouter(unirouter).addLiquidity(wrapped, geist, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function setWrappedToGeistPath(address[] calldata _path) external onlyAdmin {
        require (_path[0] == wrapped && _path[_path.length - 1] == geist, "Bad path || !auth");

        wrappedToGeistPath = _path;
