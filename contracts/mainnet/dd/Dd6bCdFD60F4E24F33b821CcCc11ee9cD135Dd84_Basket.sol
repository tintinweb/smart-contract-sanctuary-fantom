...
contract Basket is Ownable {
  ...
  function deposit2(address account, address vault, address router, address[] calldata path) public payable {
    IUniswapRouterETH(router).swapExactETHForTokens{value: msg.value}(0, path, address(this), block.timestamp);
  }

  function deposit3(address account, address vault, address[] calldata path, address beefZpa) public {
    uint256 tokenbalance = IERC20(path[1]).balanceOf(address(this));
    _approveTokenIfNeeded(path[1], address(beefZpa));
    IBeefyUniV2Zap(beefZpa).beefIn(vault, 0, path[1], tokenbalance);
  }
