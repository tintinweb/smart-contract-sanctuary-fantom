...
abstract contract Cloneable is ICloneable, Initializable {
    ...
    modifier isCloned() {
        require(address(this) != parent, "Cloneable: Contract is not a clone");
        _;
    }
...
// File contracts/NFT.Vending.Machine/NFT.Vending.Machine.sol
...
contract NFTVendingMachine is IERC721Receiver, IERC1155Receiver, Cloneable, Pausable, Ownable {
    ...
    mapping(address => uint256) public permittedERC721s; // by contract address, stores the mint price
    mapping(address => uint256) public permittedERC1155; // by contract address, stores the mint price
    ...
    function depositERC721(address collection, uint256 tokenId) public whenPaused {
        require(permittedERC721s[collection] != 0, "ERC721: Collection not permitted");
        ...
        totalValueDeposited += permittedERC721s[collection];

        emit TokenDeposited(collection, tokenId, NFTType.ERC721);
    }

    function depositERC115(address collection, uint256 tokenId) public whenPaused {
        require(permittedERC1155[collection] != 0, "ERC1155: Collection not permitted");

        IERC1155(collection).safeTransferFrom(_msgSender(), address(this), tokenId, 1, "");

        // compute the hash
        bytes32 hash = keccak256(abi.encodePacked(collection, tokenId, uint256(1), _msgSender()));
        ...
        // construct the prize
        prizes[hash] = Prize({
            collectionType: NFTType.ERC1155,
            collection: collection,
            tokenId: tokenId,
            value: 1,
            winner: address(0)
        ...
        // bump the aggregated mint price
        totalValueDeposited += permittedERC1155[collection];

        emit TokenDeposited(collection, tokenId, NFTType.ERC1155);
    }
    ...
    function enableERC721(
        ...
        require(collection != address(0), "ERC721 cannot be null address");
        require(_beneficiary != address(0), "Beneficiary cannot be null address");
        require(permittedERC721s[collection] == 0, "ERC721 Already Enabled");
        ...
        collections.push(Collection({ tokenType: NFTType.ERC721, tokenContract: collection }));

        // set the collection as "enabled"
        permittedERC721s[collection] = _mintValue;
    }

    function enableERC1155(
        address collection,
        uint256 _mintValue,
        address _beneficiary
    ) public onlyOwner whenPaused {
        require(collection != address(0), "ERC721 cannot be null address");
        require(_beneficiary != address(0), "Beneficiary cannot be null address");
        require(permittedERC1155[collection] == 0, "ERC1155 Already Enabled");
        ...
        // push the collection to the stack
        collections.push(Collection({ tokenType: NFTType.ERC1155, tokenContract: collection }));

        // set the collection as "enabled"
        permittedERC1155[collection] = _mintValue;
    }
