...
interface IImpERC20Token is IERC20Mintable, IERC20 {
...
contract ImpTreasury is Ownable {
  using LowGasSafeMath for uint256;
  ...
  IImpERC20Token public immutable IMP;
  ...
  IERC20 public sIMP;
  uint256 public sIMPQueue; // Delays change to sIMP address
  ...
  constructor(
    ...
    isReserveToken[_BOO] = true;
    reserveTokens.push(_BOO);
  ...
  /**
        @notice allow approved address to deposit an asset for IMP
        @param _amount uint
  ...
  function deposit(
    ...
    // mint IMP needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    console.log(value);
    limitRequirements(msg.sender, send_);
    IMP.mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn IMP for reserves
        @param _amount uint
        @param _token address
     */
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = sIMP.balanceOf(msg.sender); // Can only borrow against sIMP held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with IMP
        @param _amount uint
     */
  function repayDebtWithIMP(uint256 _amount) external {
    require(isDebtor[msg.sender], 'Not approved as debtor');
    require(isReserveSpender[msg.sender], 'Not approved as spender');

    IMP.burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, address(IMP), _amount, _amount);
  }
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IMP.totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns IMP valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  function valueOfToken(address _token, uint256 _amount) public view returns (uint256 value_) {
    if (isReserveToken[_token]) {
      // convert amount to match IMP decimals
      value_ = _amount.mul(10**IMP.decimals()).div(10**IERC20(_token).decimals());
  ...
  function queue(MANAGING _managing, address _address) external onlyOwner returns (bool) {
    ...
    } else if (_managing == MANAGING.sIMP) {
      // 9
      sIMPQueue = uint32(block.timestamp).add32(secondsNeededForQueue);
    } else return false;
