...
contract Strategy is BaseStrategy {
    ...
    uint256 public swapSecurityRatio;
    uint256 public sellSecurityRatio;
    bool public simpleAccounting = false;
    uint256 public profitLiquidationFactor = 8000;
    ...
    int128 public inTokenIndex;
    int128 public outTokenIndex;
    address public curvePoolSwap;

    address public router = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
    IERC20 public rewardToken = IERC20(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE); // SPOOKY (BOO) Reward Token
    ...
    constructor(
        ...
        wantDust = uint256(1).mul(10**uint256(ERC20(address(want)).decimals()));
        rewardsDust = uint256(1).mul(10**uint256(ERC20(address(rewardToken)).decimals()));
        swapSecurityRatio = 9_950;
        sellSecurityRatio = 9_900;

        lpToken = IUniPair(_lpToken);
        lpToken0 = lpToken.token0();
        require(lpToken0 == address(want), 'Wrong Pool');
        lpToken1 = lpToken.token1();

        rewardToWantRoute = [address(rewardToken), wftm, lpToken0];
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('Spooky USDC-MAI Curve Strategy v2'));
    }
    ...
    function pendingRewardsScaled() public view returns (uint256 _shares) {
        _shares = masterChef.pendingBOO(masterChefPoolId, address(this)).div(1e18);
    }

    function pendingRewards() public view returns (uint256 _shares) {
        _shares = masterChef.pendingBOO(masterChefPoolId, address(this));
    }

    function balanceOfPooled() public view returns (uint256 _amount) {
        uint256 totalLpTokens = balanceOfLpTokens().add(balanceOfLPInMasterChef());
        (uint256 reserveA, uint256 reserveB, ) = lpToken.getReserves();
        uint256 totalSupply = lpToken.totalSupply();

        uint256 lpToken1Amount = totalLpTokens.mul(reserveB).div(totalSupply);
        uint256 lpToken0Amount = totalLpTokens.mul(reserveA).div(totalSupply);

        uint256 lpToken1InWant =
            lpToken1Amount == 0
                ? 0
                : ICurveFi(curvePoolSwap).get_dy(outTokenIndex, inTokenIndex, lpToken1Amount);

        _amount = lpToken1InWant.add(lpToken0Amount);
    }
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        if (_amountNeeded > looseAmount) {
            ...
            _loss = _amountNeeded > _liquidatedAmount ? _amountNeeded.sub(_liquidatedAmount) : 0; // NEW
    ...
    {
        swapAmountIn = getSwapAmount(_fullInvestment);
        uint256 swapOutSecurityRatio = swapAmountIn.mul(1e12).mul(swapSecurityRatio).div(basisOne);
        swapAmountOut = ICurveFi(curvePoolSwap).exchange(
            inTokenIndex, 
            outTokenIndex, 
            swapAmountIn, 
    ...
    function sellLpToken1() internal {
        uint256 swapInAmount = balanceOfLpToken1();
        uint256 swapOutSecurityRatio = swapInAmount.mul(sellSecurityRatio).div(basisOne).div(1e12);
    ...
    function sellAllRewards() internal {
        uint256 toSellRewardsAmount = balanceOfReward();
        if (toSellRewardsAmount > rewardsDust) {
    ...
    function removeLiquidity(uint256 toExitAmount) internal {
        ...
        uint256 lpWorth = balanceOfPooled();
        uint256 percentageOfLpToSell = toExitAmount.mul(1e18).div(lpWorth);
        uint256 liquidityToExit = lpTokenBalance.mul(percentageOfLpToSell).div(1e18);       
    ...
    function _withdrawFromMasterChefAndTransfer(address _to) internal {
        if (abandonRewards) {
            ...
            claimRewards();
            rewardToken.transfer(_to, balanceOfReward());
            withdrawFromMasterChef();
        }
    ...
    function collectTradingFees() internal {
        uint256 pooledBalance = balanceOfPooled();
        uint256 debt = vault.strategies(address(this)).totalDebt;
        uint256 deltaDebt = debt.sub(balanceOfWant());
        if (pooledBalance > deltaDebt) {
            uint256 profit = pooledBalance.sub(deltaDebt);
    ...
    function getSwapAmount(uint256 _fullInvestment) internal view returns (uint256 _toSwapAmount) {
        (uint256 reserveA, uint256 reserveB, ) = lpToken.getReserves();
        uint256 halfInvestment = _fullInvestment.div(2);
        uint256 nominator =
            ICurveFi(curvePoolSwap).get_dy(inTokenIndex, outTokenIndex, halfInvestment);
        uint256 denominator = IUni(router).quote(halfInvestment, reserveA, reserveB);
        _toSwapAmount = _fullInvestment.sub(
    ...
    function getLpTokens(uint256 _amountIn) internal {
        (uint256 swapAmountIn, uint256 swapAmountOut) = _swap(_amountIn);
        addLiquidity(_amountIn.sub(swapAmountIn), swapAmountOut);
    }

    function giveSwapAllowance() internal {
        IERC20(lpToken0).safeApprove(curvePoolSwap, 0);
        IERC20(lpToken0).safeApprove(curvePoolSwap, MAX);

        IERC20(lpToken1).safeApprove(curvePoolSwap, 0);
        IERC20(lpToken1).safeApprove(curvePoolSwap, MAX);
    }

    function giveAllowances() internal {
        lpToken.approve(address(masterChef), MAX);
        lpToken.approve(router, MAX);
    ...
    function setSwapParams(
        int128 _inTokenIndex,
        int128 _outTokenIndex,
        address _curvePoolSwap
    ) public onlyVaultManagers {
        inTokenIndex = _inTokenIndex;
        outTokenIndex = _outTokenIndex;
    ...
    function setSwapSecurityRatio(uint256 _swapSecurityRatio, uint256 _sellSecurityRatio)
        public
        onlyVaultManagers
    {
        swapSecurityRatio = _swapSecurityRatio;
        sellSecurityRatio = _sellSecurityRatio;
    }
