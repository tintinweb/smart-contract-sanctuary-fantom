...
contract FirstStage is ERC721, ERC721Enumerable, Pausable, ERC721Burnable, ReentrancyGuard, Ownable {
    ...
    event WhitelisteUpdated(address indexed admin, address[] users, bool removed);
    ...
    function buyToken(uint8 tokenId) external payable nonReentrant whenNotPaused callerIsUser {
        if (whiteListedOnly) require(isWhitelisted[msg.sender], "Error: you are not whitelisted");
    ...
    function addWhitelistedUsers(address[] calldata _users) external onlyOwner {
        require(_users.length > 0, "Error: invalid user list");
        for (uint256 i = 0; i < _users.length; ++i) {
            isWhitelisted[_users[i]] = true;
        }
        emit WhitelisteUpdated(msg.sender, _users, false);
    }
    ...
    function removeWhitelistedUsers(address[] calldata _users) external onlyOwner {
        ...
        emit WhitelisteUpdated(msg.sender, _users, true);
    }
