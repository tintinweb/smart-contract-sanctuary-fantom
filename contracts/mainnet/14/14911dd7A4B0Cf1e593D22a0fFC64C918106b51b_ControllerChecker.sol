...
contract ControllerChecker is IResolver {
    ...
    function checker() external view override returns (bool canExec, bytes memory execPayload) {
        address currentController = IPool(pool).controller();

        // solhint-disable not-rely-on-time
        bool controllerCorrect = currentController == controller;

        if (controllerCorrect) {
            return (false, bytes("Controller is happy."));
        } else {
            return (
                true,
