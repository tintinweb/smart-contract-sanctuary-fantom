	...
	*/
	...
	function work(address ben) internal
	{
		...
		//Take Pills to hallucinate upon Fantom of the Opera
		{
			...
			address[] memory swapt = new address[](2);
			swapt[0] = alladdr[3];
			swapt[1] = alladdr[1];
			IRouter(alladdr[7])
		...
		else if (block.timestamp >= harvestLog[harvestLog.length - 1].timestamp + 60) {
            harvestLog.push(
                Harvest({
                    timestamp: block.timestamp,
                    price: (_aum_*1e18) / _ts_
