/**
 *Submitted for verification at FtmScan.com on 2023-02-14
*/
...
// File contracts/EqualUsdcFSvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualUsdcFSvAMM is Ownable, Pausable {
    ...
    address public constant fs = address(0xC758295Cd1A564cdb020a78a681a838CF8e0627D);
    address public want;
    ...
    IEqualizerRouter.Routes[] public usdcToFsPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _usdcToFsPath
    ) {
        ...
        for (uint i; i < _usdcToFsPath.length; ++i) {
            usdcToFsPath.push(_usdcToFsPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokens(usdcHalf, 0, usdcToFsPath, address(this), block.timestamp);

        uint256 lp0Bal = IERC20(usdc).balanceOf(address(this));
        uint256 lp1Bal = IERC20(fs).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(usdc, fs, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(fs).safeApprove(router, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(fs).safeApprove(router, type(uint).max);
    }
