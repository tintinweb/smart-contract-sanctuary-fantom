...
import "./interfaces/IEqualizerV2PairFactory.sol";
import "./interfaces/IEqualizerV2Gauge.sol";
import "./interfaces/IVeloRouter.sol";
import "./interfaces/IBaseV1Pair.sol";
import "./interfaces/IEqualizerV2VaultToken.sol";
...
contract EqualizerV2VaultToken is IEqualizerV2VaultToken, PoolToken {
    ...
    function _update() internal {
        uint256 _totalBalance = IEqualizerV2Gauge(gauge).balanceOf(address(this));
    ...
    function mint(address minter) external nonReentrant update returns (uint256 mintTokens) {
        ...
        uint256 _totalBalanceBefore = IEqualizerV2Gauge(gauge).balanceOf(address(this));
        IEqualizerV2Gauge(gauge).deposit(mintAmount);
        uint256 _totalBalanceAfter = IEqualizerV2Gauge(gauge).balanceOf(address(this));
    ...
    function redeem(address redeemer) external nonReentrant update returns (uint256 redeemAmount) {
        ...
        IEqualizerV2Gauge(gauge).withdraw(redeemAmount);
    ...
    function _optimalDepositA(
        ...
        uint256 swapFee = IEqualizerV2PairFactory(pairFactory).getRealFee(underlying);
    ...
    function _getReward() internal returns (uint256 amount) {
        ...
        IEqualizerV2Gauge(gauge).getReward(address(this), tokens);
    ...
    function reinvest() external nonReentrant update {
        ...
        IEqualizerV2Gauge(gauge).deposit(liquidity);
    ...
    function adminClaimRewards(address[] calldata _tokens) external onlyFactoryOwner nonReentrant {
        IEqualizerV2Gauge(gauge).getReward(address(this), _tokens);
    }
...
import "@openzeppelin/contracts/ownership/Ownable.sol";
import "./interfaces/IBaseV1Pair.sol";
import "./EqualizerV2VaultToken.sol";
import "./interfaces/IEqualizerV2VaultTokenFactory.sol";

contract EqualizerV2VaultTokenFactory is Ownable, IEqualizerV2VaultTokenFactory {
    address public optiSwap;
    address public router;
    address public voter;
    address public pairFactory;
        ...
        returns (address vaultToken)
    {
        ...
        bytes memory bytecode = type(EqualizerV2VaultToken).creationCode;
        assembly {
            vaultToken := create2(0, add(bytecode, 32), mload(bytecode), _underlying)
        }
        EqualizerV2VaultToken(vaultToken)._initialize(
            _underlying,
            optiSwap,
            router,
            voter,
...
interface IEqualizerV2PairFactory {
    ...
    function getRealFee(address pair) external view returns (uint);
}

pragma solidity >=0.5.0;

interface IEqualizerV2VaultToken {
    /*** Tarot ERC20 ***/
...
interface IEqualizerV2VaultTokenFactory {
