...
contract Hopium is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 HOPIUM-WFTM LP -> HOPIUM pool
    ...
    /**
     * @notice Constructs the HOPIUM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("HOPIUM", "HOPIUM") {
        // Mints 1 HOPIUM to contract creator for initial pool setup
    ...
    function _getTombPrice() internal view returns (uint256 _tombPrice) {
        try IOracle(tombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            ...
            revert("Hopium: failed to fetch HOPIUM price from Oracle");
        }
    }
    ...
    /**
     * @notice Operator mints HOPIUM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of HOPIUM to mint to
