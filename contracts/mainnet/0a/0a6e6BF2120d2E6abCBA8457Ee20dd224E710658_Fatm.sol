...
/*
   ____     __          _____                       
  / __/__ _/ /___ _    / __(_)__  ___ ____  _______ 
 / _// _ `/ __/  ' \  / _// / _ \/ _ `/ _ \/ __/ -_)
/_/  \_,_/\__/_/_/_/ /_/ /_/_//_/\_,_/_//_/\__/\__/ 
...
contract Fatm is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_FATM_FOR_LP_DISTRIBUTION = 500000 ether;
    ...
    /**
     * @notice Constructs the FATM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("FATM Token", "FATM") {
        // Mints 1 FATM to contract creator for initial pool setup
    ...
    function _getFatmPrice() internal view returns (uint256 _fatmPrice) {
        try IOracle(fatmOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Fatm: failed to fetch FATM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _fatmPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_fatmPrice >= taxTiersTwaps[tierId]) {
    ...
    function setFatmOracle(address _fatmOracle) public onlyOperatorOrTaxOffice {
        require(_fatmOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints FATM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of FATM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentFatmPrice = _getFatmPrice();
            currentTaxRate = _updateTaxRate(currentFatmPrice);
            if (currentFatmPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _fatmReserve
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_fatmReserve != address(0), "!_fatmReserve");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_fatmReserve, INITIAL_FATM_FOR_LP_DISTRIBUTION);
    }
