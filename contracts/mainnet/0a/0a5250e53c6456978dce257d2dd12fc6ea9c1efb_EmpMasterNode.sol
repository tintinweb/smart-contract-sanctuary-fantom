...
// File contracts/distribution/EmpMasterNode.sol
...
contract EmpMasterNode {
    ...
    IERC20 public constant EMP = IERC20(0x4fFbB4F7c335AbD33e6E2C45b6F2d3b19d9C8421);
    ...
    function claimTreasuryRewards() external {
        if (treasury_rewards > 0) {
            EMP.safeTransfer(dev, treasury_rewards);
            treasury_rewards = 0;
        }   
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        require(EMP.balanceOf(_sender) >= tierPrice, "Insufficient balance");
        require(EMP.allowance(_sender, address(this)) >= tierPrice, "Insufficient allowance");
        EMP.safeTransferFrom(_sender, address(this), tierPrice);
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            ...
            IERC20(EMP).safeTransfer(_sender, _rewards);
    ...
    function getRewardDrip() public view returns (uint256) {
        if (lastDripTime < block.timestamp) {
            uint256 poolBalance = getEmpBalancePool();
    ...
    function getEmpBalance() public view returns (uint256) {
        return IERC20(EMP).balanceOf(address(this));
    }

     function getEmpBalancePool() public view returns (uint256) {
        return getEmpBalance().sub(total_rewards).sub(treasury_rewards);
    }
