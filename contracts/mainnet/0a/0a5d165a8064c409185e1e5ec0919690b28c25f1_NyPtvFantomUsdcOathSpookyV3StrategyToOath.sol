...
// No permissions granted before Saturday, 13 Jul 2024, then GPL-3.0 after this date.
 ...
 * out quoting reference 'NyPtvFantomUsdcOathSpookyV3StrategyToOath'
 *
  ...
  PausableUpgradeable
{
  ...
  uint256 public constant UPGRADE_TIMELOCK = 36 hours;
...
error NyPtvFantomWftmBooSpookyV2StrategyToUsdc__MaxSlippageOutOfBounds(uint256 max);

contract NyPtvFantomUsdcOathSpookyV3StrategyToOath is NYBaseProfitTakingStrategyV3 {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  uint256 public constant POOL_ID = 4;
  address public constant SPOOKY_ROUTER = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
  address public constant SPOOKY_SWAP_FARM_V3 = address(0x9C9C920E51778c4ABF727b8Bb223e78132F00aA4);
  ...
  address public constant LP_TOKEN = address(0x9ba499753Ea5407D5578718A8D37453ecbf609f9);
  address public constant LP_TOKEN_0 = address(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
  address public constant LP_TOKEN_1 = address(0x21Ada0D2aC28C3A5Fa3cD2eE30882dA8812279B6);

  address[] public booToOathPath;
  ...
  function initialize(
    ...
    booToOathPath = [BOO, WFTM, USDC, OATH];
    maxSlippage = 9700;
  }
  ...
  function _deposit() internal override {
    ...
    if (underlyingBalance != 0) {
      IERC20Upgradeable(LP_TOKEN).safeIncreaseAllowance(SPOOKY_SWAP_FARM_V3, underlyingBalance);
      IMasterChef(SPOOKY_SWAP_FARM_V3).deposit(POOL_ID, underlyingBalance);
    }
  }
  ...
  function _withdraw(uint256 _amount) internal override {
    ...
    if (underlyingBalance < _amount) {
      IMasterChef(SPOOKY_SWAP_FARM_V3).withdraw(POOL_ID, _amount - underlyingBalance);
    }
  ...
  function _harvestCore()
    ...
    returns (uint256 callerFee, uint256 underlyingTokenCount, uint256 sentToVault)
  {
    IMasterChef(SPOOKY_SWAP_FARM_V3).deposit(POOL_ID, 0);
  ...
  function _swapFarmEmissionTokens() internal {
    ...
    if (booToOathPath.length < 2 || booBalance == 0) {
      return;
    }
    boo.safeIncreaseAllowance(SPOOKY_ROUTER, booBalance);

    uint256[] memory amounts = IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(booBalance, booToOathPath);
  ...
  function _chargeFees() internal returns (uint256 callerFee) {
    IERC20Upgradeable oath = IERC20Upgradeable(OATH);
    uint256 fee = (oath.balanceOf(address(this)) * totalFee) / PERCENT_DIVISOR;
    if (fee != 0) {
      callerFee = (fee * callFee) / PERCENT_DIVISOR;
      uint256 treasuryFeeToVault = (fee * treasuryFee) / PERCENT_DIVISOR;
      uint256 feeToStrategist = (treasuryFeeToVault * strategistFee) / PERCENT_DIVISOR;
      treasuryFeeToVault -= feeToStrategist;
      oath.safeTransfer(msg.sender, callerFee);
      oath.safeTransfer(treasury, treasuryFeeToVault);
      oath.safeTransfer(strategistRemitter, feeToStrategist);
    }
  }

  function _sendYieldToVault() internal returns (uint256 sentToVault) {
    sentToVault = IERC20Upgradeable(OATH).balanceOf(address(this));
    if (sentToVault > 0) {
      IERC20Upgradeable(OATH).approve(vault, sentToVault);
      IVault(vault).depositProfitTokenForUsers(sentToVault);
    }
  }

  function balanceOf() public view override returns (uint256) {
    (uint256 amount, ) = IMasterChef(SPOOKY_SWAP_FARM_V3).userInfo(POOL_ID, address(this));
    return amount + IERC20Upgradeable(LP_TOKEN).balanceOf(address(this));
  }

  function _reclaimUnderlying() internal override {
    IMasterChef(SPOOKY_SWAP_FARM_V3).emergencyWithdraw(POOL_ID, address(this));
  }

  function estimateHarvest() external view override returns (uint256 profit, uint256 callFeeToUser) {
    uint256 pendingReward = IMasterChef(SPOOKY_SWAP_FARM_V3).pendingBOO(POOL_ID, address(this));
    uint256 totalRewards = pendingReward + IERC20Upgradeable(BOO).balanceOf(address(this));
    if (totalRewards != 0) {
      uint256[] memory harvestAmounts = IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(totalRewards, booToOathPath);
      profit += harvestAmounts[harvestAmounts.length - 1];
    }

    profit += IERC20Upgradeable(OATH).balanceOf(address(this));

    uint256 oathFee = (profit * totalFee) / PERCENT_DIVISOR;
    callFeeToUser = (oathFee * callFee) / PERCENT_DIVISOR;
    profit -= oathFee;
  }
  ...
  function _retireStrategy() internal override {
    ...
    (uint256 poolBal, ) = IMasterChef(SPOOKY_SWAP_FARM_V3).userInfo(POOL_ID, address(this));
    if (poolBal != 0) {
      IMasterChef(SPOOKY_SWAP_FARM_V3).withdraw(POOL_ID, poolBal);
    }
  ...
  function updateMaxSlippage(uint256 _maxSlippage) external {
    if (_maxSlippage > 10000) {
      revert NyPtvFantomWftmBooSpookyV2StrategyToUsdc__MaxSlippageOutOfBounds(_maxSlippage);
    }
    _atLeastRole(ADMIN);
