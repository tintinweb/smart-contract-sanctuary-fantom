...
contract FtmStakingImplementation is ReentrancyGuard {
    event Delegate(address indexed staker, uint amount);
    event Undelegate(address indexed spender, uint amount);
    ...
    mapping(uint256 => uint256) internal stake;
    mapping(uint256 => mapping(uint256 => uint256)) internal unstakes;
    ...
    function delegate(uint256 toValidatorID) external payable onlyOwner nonReentrant {
        ...
        emit Delegate(owner, msg.value);
    }

    function withdraw(uint256 toValidatorID, uint256 wrID) external onlyOwner nonReentrant {
