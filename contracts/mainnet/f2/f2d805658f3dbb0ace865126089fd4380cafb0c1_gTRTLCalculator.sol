...
// File contracts/interfaces/IgTRTLCalculator.sol

interface IgTRTLCalculator {
    function getValue(address lpToken, uint256 amount)
        external
        view
        returns (uint256);
}

// File contracts/gTRTLCalculator/gTRTLCalculator.sol

contract gTRTLCalculator is IgTRTLCalculator {
    ...
    function getValue(address lpToken, uint256 amount)
        ...
        returns (uint256)
    {
        return valuation(lpToken, amount) * 1e9;
    }
    ...
    function markdown(address _pair) public view returns (uint256) {
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(_pair)
            .getReserves();
        ...
        if (IUniswapV2Pair(_pair).token0() == address(TOKEN)) {
            ...
            require(
                IUniswapV2Pair(_pair).token1() == address(TOKEN),
                "Invalid pair"
            );
            reserve = reserve0;
        }
        return
            reserve
                .mul(2 * (10**IERC20Metadata(address(TOKEN)).decimals()))
