...
// File contracts/BaseVaultStrategy/common/Context.sol
...
// File contracts/BaseVaultStrategy/StrategyLPBase.sol
...
abstract contract StrategyLPBase is StratManager, FeeManager {
    ...
    // Routes
    address[] public rewardTokenToWrappedRoute;
    ...
    uint256 sideProfitTokenPercent = 0; // in basis points (1/100 of a percent)
    /**
    ...
    constructor(
        address _wrappedNativeToken, // Wrapped Token
        ...
        rewardTokenToWrappedRoute = [address(rewardToken), address(wrappedNativeToken)];
        lpToken = IUniswapV2Pair(_lpToken);
        lpAsset0 = IERC20(lpToken.token0());
        lpAsset1 = IERC20(lpToken.token1());
        sideProfitToken = IERC20(_sideProfitToken);

        if (wrappedNativeToken == sideProfitToken) {
            rewardTokenToProfitRoute = [address(rewardToken), address(sideProfitToken)];
        } else {
            rewardTokenToProfitRoute = [address(rewardToken), address(wrappedNativeToken), address(sideProfitToken)];
        }


        if (lpAsset0 == wrappedNativeToken) {
            rewardTokenToLp0Route = [address(rewardToken), address(wrappedNativeToken)];
        } else if (lpAsset0 != rewardToken) {
            rewardTokenToLp0Route = [address(rewardToken), address(wrappedNativeToken), address(lpAsset0)];
        }

        if (lpAsset1 == wrappedNativeToken) {
            rewardTokenToLp1Route = [address(rewardToken), address(wrappedNativeToken)];
        } else if (lpAsset1 != rewardToken) {
            rewardTokenToLp1Route = [address(rewardToken), address(wrappedNativeToken), address(lpAsset1)];
        }
    }

    function farmAddress() internal virtual returns (address) {}
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (tx.origin == owner() || paused()) {
            lpToken.safeTransfer(vault, lpTokenBal);
        } else {
            uint256 withdrawalFeeAmount = ((lpTokenBal * withdrawalFee) / WITHDRAWAL_MAX);
            lpToken.safeTransfer(vault, lpTokenBal - withdrawalFeeAmount);
        }
    }
    ...
    function chargeFees() internal {
        uint256 toWrapped = ((rewardToken.balanceOf(address(this)) * stratFee) / 1000);
        IUniswapRouterETH(unirouter).swapExactTokensForTokens(toWrapped, 0, rewardTokenToWrappedRoute, address(this), block.timestamp);
        wrappedNativeToken.safeTransfer(strategist, wrappedNativeToken.balanceOf(address(this)));

        if (sideProfitTokenPercent > 0) {
            uint256 toProfit = FullMath.mulDiv(rewardToken.balanceOf(address(this)), sideProfitTokenPercent, 10000); // Take sideProfitTokenPercent% of rewardToken
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(toProfit, 0, rewardTokenToProfitRoute, address(this), block.timestamp); // swap to Profit
    ...
    function setPercentProfits(uint256 _percent) public onlyManager {
      require(_percent < 10000, "percent sideProfitToken must be less than 100%");
      sideProfitTokenPercent = _percent;
    }
    ...
    function retireStrat() external {
        ...
        uint256 lpTokenBal = lpToken.balanceOf(address(this));
        lpToken.transfer(vault, lpTokenBal);
    }
    ...
    function resetValues(address _wrappedNativeToken, address _rewardToken, address _farmAddr, address _lpToken, uint256 _poolId, address _sideProfitToken) public onlyOwner {
        removeAllowances();
        wrappedNativeToken = IERC20(_wrappedNativeToken);
        rewardToken = IERC20(_rewardToken);
...
// File contracts/BaseVaultStrategy/StrategyLPMasterChef.sol
...
contract StrategyLPMasterChef is StrategyLPBase {
    ...
    function farmAddress() internal view override(StrategyLPBase) returns (address) {
        return address(farm);
    }
    ...
    function giveAllowances() internal override(StrategyLPBase) {
        IERC20(lpToken).safeApprove(farmAddress(), MAX_INT);
    ...
    function removeAllowances() internal override(StrategyLPBase) {
        IERC20(lpToken).safeApprove(farmAddress(), 0);
