...
contract NomisScore is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, Pausable, EIP712 {
    ...
    */
    ...
    /**
    * @dev Returns the token ids for a given address.
    * @param addr The address to get the token ids.
    * @return tokenIds The token ids for the specified address.
    */
    function getTokenIds(address addr) external view returns(uint256[] memory tokenIds) {
        uint256 index = 0;
        for (uint16 i = 0; i < _calcModelsCount; i++) {
            for (uint256 j = 0; j < 9999999999999; j++) {
                if (scores[addr][j][i].updated != 0) {
                    tokenIds[index] = scores[addr][j][i].tokenId;
                    index++;
                }
            }
        }
