...
// No permissions granted before Saturday, 11 May 2024, then MIT after this date.
 ...
 * out quoting reference 'NyPtvFantomWftmBooSpookyV2StrategyToFusdt'
 *
...
import "../../../../abstract/NYBaseProfitTakingStrategyV1.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable/utils/math/MathUpgradeable.sol";

// Minimized version of MasterChef interface
interface IMasterChef {
  function deposit(uint256 _pid, uint256 _amount) external;
  function emergencyWithdraw(uint256 _pid, address _to) external;
  function userInfo(uint256, address) external view returns (uint256 amount, uint256 rewardDebt);
  function withdraw(uint256 _pid, uint256 _amount) external;
  function pendingBOO(uint256 _pid, address _to) external view returns (uint256 pendingReward);
}

// Minimized version of UniswapV2Router02 interface
...
// Minimized version of Vault interface
interface IVault {
  function depositOutputTokenForUsers(uint256 _amount) external;
}

contract NyPtvFantomWftmBooSpookyV2StrategyToFusdt is NYBaseProfitTakingStrategyV1 {
  using SafeERC20Upgradeable for IERC20Upgradeable;

  // 3rd-party contracts
  uint256 public constant POOL_ID = 0;
  address public constant SPOOKY_ROUTER = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
  address public constant SPOOKY_SWAP_FARM_V2 = address(0x18b4f774fdC7BF685daeeF66c2990b1dDd9ea6aD);

  /**
   * @dev Tokens used
   * {BOO}        Farm emission token sold to {FUSDT}
   * {FUSDT}      Vault reward token, also used for fees.
   * {LP_TOKEN}   Address of the LP token to farm.
   * {LP_TOKEN_0} WFTM - First token of the LP
   * {LP_TOKEN_1} BOO  - Second token of the LP
   */
  ...
  address public constant LP_TOKEN = address(0xEc7178F4C41f346b2721907F5cF7628E388A7a58);
  address public constant LP_TOKEN_0 = address(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
  address public constant LP_TOKEN_1 = address(0x841FAD6EAe12c286d1Fd18d1d525DFfA75C7EFFE);

  /**
   * @dev Paths used to swap tokens:
   * {booToFusdtPath} - to swap {BOO} to {FUSDT} (using SPOOKY_ROUTER)
   */
  address[] public booToFusdtPath;
  ...
  function _deposit() internal override {
    ...
    if (underlyingBalance != 0) {
      IERC20Upgradeable(LP_TOKEN).safeIncreaseAllowance(SPOOKY_SWAP_FARM_V2, underlyingBalance);
  ...
  function _withdraw(uint256 _amount) internal override {
    ...
    if (underlyingBalance < _amount) {
      IMasterChef(SPOOKY_SWAP_FARM_V2).withdraw(POOL_ID, _amount - underlyingBalance);
    }
    IERC20Upgradeable(LP_TOKEN).safeTransfer(vault, _amount);
  }

  /**
   * @dev Core function of the strategy, in charge of collecting and processing rewards.
   *      1. Claims {BOO} from {SPOOKY_SWAP_FARM_V2}.
   *      2. Swap from {BOO} to {FUSDT}
   *      3. Charge fees.
   *      4. Send remaining {FUSDT} to vault.
   */
  ...
  {
    IMasterChef(SPOOKY_SWAP_FARM_V2).deposit(POOL_ID, 0); // deposit 0 to claim rewards
   ...
   *      Swaps farm emission token to {FUSDT}.
   */
  function _swapFarmEmissionTokens() internal {
    ...
    if (booToFusdtPath.length < 2 || booBalance == 0) {
      return;
    }
  ...
  function _chargeFees() internal returns (uint256 callerFee) {
    IERC20Upgradeable fusdt = IERC20Upgradeable(FUSDT);
    uint256 fee = (fusdt.balanceOf(address(this)) * totalFee) / PERCENT_DIVISOR;
    if (fee != 0) {
      callerFee = (fee * callFee) / PERCENT_DIVISOR;
      uint256 treasuryFeeToVault = (fee * treasuryFee) / PERCENT_DIVISOR;
      uint256 feeToStrategist = (treasuryFeeToVault * strategistFee) / PERCENT_DIVISOR;
      treasuryFeeToVault -= feeToStrategist;
      fusdt.safeTransfer(msg.sender, callerFee);
      fusdt.safeTransfer(treasury, treasuryFeeToVault);
      fusdt.safeTransfer(strategistRemitter, feeToStrategist);
    }
  }

  /**
   * @dev Core harvest function.
   *      Sends {FUSDT} to the vault (with accounting)
   */
  function _sendYieldToVault() internal returns (uint256 sentToVault) {
    sentToVault = IERC20Upgradeable(FUSDT).balanceOf(address(this));
    IERC20Upgradeable(FUSDT).approve(vault, sentToVault);
    IVault(vault).depositOutputTokenForUsers(sentToVault);
  }

  /**
   * @dev Function to calculate the total {LP_TOKEN} held by the strategy.
   *      It takes into account both the funds in hand, plus the funds in {SPOOKY_SWAP_FARM_V2}.
   */
  function balanceOf() public view override returns (uint256) {
    (uint256 amount, ) = IMasterChef(SPOOKY_SWAP_FARM_V2).userInfo(POOL_ID, address(this));
    return amount + IERC20Upgradeable(LP_TOKEN).balanceOf(address(this));
  }

  /**
   * @dev Withdraws all funds from {SPOOKY_SWAP_FARM_V2} leaving rewards behind.
   */
  ...
  /**
   * @dev Estimates the amount of profit that can be earned by harvesting the strategy including 
   *      the callFee which enables externals to decide if it's worth it.
   */
  function estimateHarvest() external view override returns (uint256 profit, uint256 callFeeToUser) {
    ...
    if (totalRewards != 0) {
      profit += IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(totalRewards, booToFusdtPath)[1];
    }

    profit += IERC20Upgradeable(FUSDT).balanceOf(address(this));

    uint256 fusdtFee = (profit * totalFee) / PERCENT_DIVISOR;
    callFeeToUser = (fusdtFee * callFee) / PERCENT_DIVISOR;
    profit -= fusdtFee;
  }
  ...
  function _retireStrategy() internal override {
    ...
    (uint256 poolBal, ) = IMasterChef(SPOOKY_SWAP_FARM_V2).userInfo(POOL_ID, address(this));
    if (poolBal != 0) {
...
/**
 * @notice Based on ReaperBaseStrategyv3.sol MIT contract at
 * https://github.com/Byte-Masons/spooky-farmer/blob/ftm-brush-v2/contracts/abstract/ReaperBaseStrategyv3.sol
 */

abstract contract NYBaseProfitTakingStrategyV1 is
  IStrategy,
  UUPSUpgradeable,
  ...
  PausableUpgradeable
{
  ...
  // @todo set back to 48 hours once through initial testing period, so we can move faster if required.
  uint256 public constant UPGRADE_TIMELOCK = 1 hours; // minimum 1 hours for NY
  ...
  /**
   * Access Roles in increasing order of privilege.
   ...
   * {treasury}             - Address of the Ninja treasury
   ...
   * {MAX_SECURITY_FEE}     - Maximum security fee charged on withdrawal to prevent flash deposit/harvest attacks.
  ...
  /**
   * @dev Normal distribution of fees earned, expressed as % of the profit from each harvest.
   * {totalFee}         - divided by 10,000 to determine the % fee. Set to 5.00% by default and lowered as necessary to
   *                      provide users with the most competitive APY.
   * {callFee}          - Percent of the totalFee reserved for the harvester (500 = 5% of total fee: 0.25% by default)
   * {treasuryFee}      - Percent of the totalFee taken by maintainers of the software (9500 = 95% of total fee: 4.75% by default)
   * {strategistFee}    - Percent of the treasuryFee taken by strategist (1052 = 10% of Treasury fee: 0.5% % by default, leaving 4.25% for the Treasury)
   ...
   * {StrategyHarvest}           - Event that is fired each time the strategy gets harvested.
  ...
  event StrategyHarvest(
    ...
    uint256 underlyingTokenCount, // how many it took
    uint256 harvestSeconds, // how long it took
    uint256 sentToVault // how much it created
  );
  ...
  /**
   * @dev harvest() - Subcontracts should override _harvestCore() and implement their specific logic in it.
   */
  ...
  /**
   * @dev subclasses should add their custom logic in this function.
   */
