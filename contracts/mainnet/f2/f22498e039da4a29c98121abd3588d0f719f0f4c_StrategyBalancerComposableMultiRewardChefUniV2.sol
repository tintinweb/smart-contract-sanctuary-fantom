...
// File contracts/BIFI/strategies/Balancer/StrategyBalancerComposableMultiRewardChefUniV2.sol
...
contract StrategyBalancerComposableMultiRewardChefUniV2 is StratFeeManagerInitializable {
    using SafeERC20 for IERC20;

    // Tokens used
    address public want;
    address public output;
    address public rewardSpooky;
    ...
    address[] public rewardToNativeRouteSpooky;
    ...
    function initialize(
        ...
        address[] calldata _rewardToNativeRouteSpooky,
        ...
        rewardSpooky = _rewardToNativeRouteSpooky[0];
        require(
            _rewardToNativeRouteSpooky[_rewardToNativeRouteSpooky.length - 1] == native,
            "rewardToNativeRouteSpooky[last] != native"
        );
        rewardToNativeRouteSpooky = _rewardToNativeRouteSpooky;
    ...
    function swapRewardsToNative() internal {
        ...
        // second reward
        uint256 rewardSpookyBal = IERC20(rewardSpooky).balanceOf(address(this));
        if (rewardSpookyBal > 0) {
            IUniswapRouterETH(spookyRouter).swapExactTokensForTokens(
                rewardSpookyBal,
                0,
                rewardToNativeRouteSpooky,
                address(this),
                block.timestamp
            );
        }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(rewardSpooky).safeApprove(spookyRouter, 0);
        IERC20(rewardSpooky).safeApprove(spookyRouter, type(uint).max);
        if (rewardSpooky != native) {
            IERC20(native).safeApprove(spookyRouter, type(uint).max);
        }
    }
