...
// File: contracts/WigoFusdtFtm.sol
...
contract WigoFusdtFtm is Ownable, Pausable {
    ...
    address[] public wrappedToFusdtPath;
    ...
    event SetWrappedToFusdtPath(address[] indexed path);
    ...
    constructor(
        ...
        wrappedToFusdtPath = [wrapped, fusdt];
    ...
    function addLiquidity() internal {
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToFusdtPath, address(this), block.timestamp);
    
        uint256 lp0Bal = IERC20(wrapped).balanceOf(address(this));
        uint256 lp1Bal = IERC20(fusdt).balanceOf(address(this));

        IUniSwapRouter(unirouter).addLiquidity(wrapped, fusdt, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(fusdt).safeApprove(unirouter, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        IERC20(fusdt).safeApprove(unirouter, type(uint).max);
    }
    ...
    function setWrappedToFusdtPath(address[] calldata _path) external onlyAdmin {
        require (_path[0] == wrapped && _path[_path.length - 1] == fusdt, "Bad path || !auth");

        wrappedToFusdtPath = _path;

        emit SetWrappedToFusdtPath(_path);
    }
