...
contract RangoV1 is BaseProxyContract {
    ...
    event TestReceived(address from, uint amount, uint msgvalue);
    ...
    function routerBridge(
        ...
        (uint out, uint value) = onChainSwapsPreBridge(a, request, calls, 0);
        emit TestReceived(request.toToken, out, value);
        uint nativeValue = value+msg.value;

//        approve(request.toToken, a, bridgeRequest.feeAmount + bridgeRequest.feeAmount);
//        SafeERC20Upgradeable.safeTransferFrom(IERC20Upgradeable(bridgeRequest.feeTokenAddress), msg.sender, address(this), bridgeRequest.feeAmount);
