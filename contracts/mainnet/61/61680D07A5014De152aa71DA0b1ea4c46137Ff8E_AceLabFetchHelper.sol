...
contract AceLabFetchHelper {
  ...
  function fetchPoolData (uint256 pid, uint256 version) public view returns (
    ...
    if (version == 2) {
      ...
      uint256 magicatBoost = aceLabV2.magicatBoost();
      (,,,xBooStaked,mpStaked,rewardPerSecondRaw,,,,,endTime,startTime) = aceLabV2.poolInfo(pid);
  ...
  function fetchUserPoolData (address user, uint256 pid, uint256 version) public view returns (
    ...
    // Shared
    allowance = xBOO.allowance(user, version == 1 ? address(aceLabV1) : address(aceLabV2));
    ...
    // AceLabV2, includes catstaking
    if (version == 2) {
      magicatsApproved = aceLabV2.magicat().isApprovedForAll(user, address(aceLabV2));

      (staked,,, mpStaked) = aceLabV2.userInfo(pid, user);
      earnings = aceLabV2.pendingReward(pid, user);
      stakedMagicats = aceLabV2.getStakedMagicats(pid, user);
      currentStakeableMP = aceLabV2.stakeableMP(pid, user);
      baseStakeableMP = aceLabV2.stakeableMP(1e18);
    }
  }
  ...
  function fetchUserMagicatData (address user) public view returns (
    uint256[] memory ownedCats,
    PoolCats[] memory stakedCats
  ) {
    ownedCats = aceLabV2.magicat().walletOfOwner(user);

    uint256 v2PoolsCount = aceLabV2.poolLength();
    stakedCats = new PoolCats[](v2PoolsCount);

    for (uint256 pid = 0; pid < v2PoolsCount; pid++) {
      stakedCats[pid] = PoolCats({
        pid: pid,
        cats: aceLabV2.getStakedMagicats(pid, user)
      });
    }
  }
}
