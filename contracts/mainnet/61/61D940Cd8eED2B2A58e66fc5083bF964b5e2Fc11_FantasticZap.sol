...
contract FantasticZap is Ownable, ReentrancyGuard {
    ...
    function zap(uint256 _zapId, uint256 _minLiquidity) external payable nonReentrant {
        ...
        uint256 _ethAmtToAddLP;
        (_ethAmtToAddLP, _tokenAmtToAddLP) = swap(_lp, _ethIn, address(_info.token));

        // STEP 2: Add liquditiy
        approveToken(address(_info.token), address(uniRouter), _tokenAmtToAddLP);
        (uint256 _tokenAmtUsed, uint256 _ethAmtUsed, uint256 _liquidity) = uniRouter
        ...
        if (_tokenAmtToAddLP > _tokenAmtUsed) {
            _info.token.safeTransfer(msg.sender, _tokenAmtToAddLP - _tokenAmtUsed);
        }
        if (_ethAmtToAddLP > _ethAmtUsed) {
            require(
                payable(msg.sender).send(_ethAmtToAddLP - _ethAmtUsed),
                "Failed to send ETH back to sender"
            );
        }
    }
    ...
    function swap(
        ...
        uint256 _ethIn,
        address _token
    ) internal returns (uint256 _ethAmt, uint256 _tokenAmt) {
        address _token0 = IUniswapV2Pair(_lp).token0();
        (uint256 _res0, uint256 _res1, ) = IUniswapV2Pair(_lp).getReserves();

        uint256 _ethToSwap;
        if (_token == _token0) {
            _ethToSwap = calculateSwapInAmount(_res1, _ethIn);
        } else {
            _ethToSwap = calculateSwapInAmount(_res0, _ethIn);
        }

        if (_ethToSwap <= 0) _ethToSwap = _ethIn / 2;
        _tokenAmt = doSwapETH(_token, _ethToSwap);
        _ethAmt = _ethIn - _ethToSwap;
    }

    function doSwapETH(address _toToken, uint256 _amount)
        internal
        returns (uint256 _tokenReceived)
    {
        ...
        _tokenReceived = uniRouter.swapExactETHForTokens{value: _amount}(
            1,
            _path,
            address(this),
            block.timestamp
