...
contract OneBoardroom is ShareWrapper, ContractGuard {
    ...
    IERC20 public cyber;
    IERC20 public wftm;
    IERC20 public spirit; // second collateral
    ITreasury public treasury;
    address public insuranceFund;
    address public spiritOracle;
    ...
    uint256 public spiritCollateralDiscountPercent;
    uint256 public cyberTokenSupplyTarget;
    ...
    function initialize(
        ...
        spiritCollateralDiscountPercent = 2000; // 20% discount if pay by SPIRIT
        cyberTokenSupplyTarget = 200000000 ether;
    ...
    function setSpirit(IERC20 _spirit) external onlyOperator {
        spirit = _spirit;
    }
    ...
    function setSpiritOracle(address _spiritOracle) external onlyOperator {
        spiritOracle = _spiritOracle;
    }
    ...
    function setSpiritCollateralDiscountPercent(
        uint256 _spiritCollateralDiscountPercent
    ) external onlyOperator {
        require(_spiritCollateralDiscountPercent <= 5000, "too high"); // <= 50%
        spiritCollateralDiscountPercent = _spiritCollateralDiscountPercent;
    }

    function setCyberTokenSupplyTarget(uint256 _cyberTokenSupplyTarget)
    external
    onlyOperator
    {
        require(_cyberTokenSupplyTarget >= 100000000 ether, "too low"); // >= 100 million CYBER
        cyberTokenSupplyTarget = _cyberTokenSupplyTarget;
    }
    ...
    function canClaimReward() public pure returns (bool) {
        ...
        // return _treasury.previousEpochCyberPrice() >= 1e18 && _treasury.getNextExpansionRate() > 0; // current epoch and next epoch are both expansion
    }
    ...
    {
        uint256 _cyberSupply = cyber.totalSupply();
        uint256 _cyberTokenSupplyTarget = cyberTokenSupplyTarget;
        if (_cyberSupply >= _cyberTokenSupplyTarget) {
            _collateralPercent = maxCollateralPercent;
        } else {
            _collateralPercent = _cyberSupply.mul(10000).div(
                _cyberTokenSupplyTarget
            );
    ...
    function getSpiritOverWftmPrice() public view returns (uint256) {
        return
        (spiritOracle == address(0))
        ? 1e18
        : uint256(IEpoch(spiritOracle).getPegPriceUpdated());
    }

    function getPegOriginalTokenOverWftmPrice(address _token)
    public
    view
    returns (uint256)
    {
    ...
    returns (
        ...
        uint256 _totalSpiritCollateral
    )
    {
        _numOfPegTokens = numOfPegTokens();
        _pegTokenAddresses = new address[](_numOfPegTokens);
        ...
        _totalSpiritCollateral = _totalCollateral.mul(1e18).div(
            getSpiritOverWftmPrice()
        );
        _totalSpiritCollateral = _totalSpiritCollateral.sub(
            _totalSpiritCollateral.mul(spiritCollateralDiscountPercent).div(10000)
        ); // sub discount
    }
    ...
    function getTotalSpiritCollateralForClaimAllPegTokens(address _member)
    public
    view
    returns (uint256 _totalSpiritCollateral)
    {
    ...
    function claimReward(bool _useSpiritCollateral) external onlyOneBlock {
        //        require(canClaimReward(), "contraction");
        _claimReward(_useSpiritCollateral);
    }

    function _claimReward(bool _useSpiritCollateral)
    internal
    updateReward(msg.sender)
    {
        uint256 _reward = members[address(cyber)][msg.sender].rewardEarned;
        if (_reward > 0) {
            if (_useSpiritCollateral) {
                spirit.safeTransferFrom(
                    msg.sender,
                    insuranceFund,
                    getTotalSpiritCollateralForClaimAllPegTokens(msg.sender)
                );
            } else {
                wftm.safeTransferFrom(
                    msg.sender,
                    insuranceFund,
