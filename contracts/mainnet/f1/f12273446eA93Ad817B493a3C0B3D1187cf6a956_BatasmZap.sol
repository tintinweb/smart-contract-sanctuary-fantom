...
import "./interfaces/IBatChef.sol";
import "./libs/Babylonian.sol";

/*
    ZapperFi's simplified version of zapper contract which will:
    1. use ETH to swap to target token
    2. make LP between ETH and target token
    3. add into farm
*/
contract BatasmZap is Ownable, ReentrancyGuard {
    ...
    ZapInfo[] public zaps;
    IBatChef public batasmChef;
    IUniswapV2Router02 public uniRouter;

    address public constant wethAddress = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83; // WFTM

    constructor(address _batasmChef, address _uniRouter) {
        require(_batasmChef != address(0), "Zap::constructor: Invalid address");
        require(_uniRouter != address(0), "Zap::constructor: Invalid address");
        batasmChef = IBatChef(_batasmChef);
    ...
    function zap(
        ...
        address _lp = address(batasmChef.lpToken(_info.pid));
        ...
        (uint256 _tokenAmtUsedInLp, uint256 _ethAmtUsedInLp, uint256 _liquidity) = uniRouter.addLiquidityETH{value: _ethAmtToAddLP}(
            address(_info.token),
            _tokenAmtToAddLP,
            1,
            1,
        ...
        approveToken(_lp, address(batasmChef), _liquidity);
        batasmChef.deposit(_info.pid, _liquidity, msg.sender);
    ...
    function doSwapETH(address _toToken, uint256 _ethAmt) internal returns (uint256 _tokenReceived) {
        ...
        _tokenReceived = uniRouter.swapExactETHForTokens{value: _ethAmt}(1, _path, address(this), block.timestamp)[_path.length - 1];
    ...
    function calculateSwapInAmount(uint256 _reserveIn, uint256 _tokenIn) internal pure returns (uint256) {
        return (Babylonian.sqrt(_reserveIn * ((_tokenIn * 3992000) + (_reserveIn * 3992004))) - (_reserveIn * 1998)) / 1996;
    }
