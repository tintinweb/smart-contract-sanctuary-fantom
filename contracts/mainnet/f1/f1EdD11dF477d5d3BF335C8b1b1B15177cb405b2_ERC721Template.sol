...
contract ERC721Template is ERC721Enumerable, ERC721Royalty, Ownable {
    ...
    constructor() ERC721("Fin's Play House Punks", "PHP") {
        deployer = _msgSender();
        ...
        royaltyReceiver.addPayee(address(this), 90); // 90%
        royaltyReceiver.addPayee(_msgSender(), 10); // 10%
    ...
    function mint() public payable returns (uint256) {
        require(msg.value == MINT_PRICE, "Wrong amount supplied");

        return _mintNFT(_msgSender());
    }
    ...
    function withdraw() public onlyOwner {
        ...
        IPaymentSplitter splitter = IPaymentSplitter(0xd1c3Be3EA847c5B74aE1f91AaBDe8e227611CCcD.clone());
        splitter.initialize(); // initialization of the clone
        if (owner() != deployer) {
            splitter.addPayee(owner(), 90); // 90%
        }
        splitter.addPayee(deployer, 10); // 10%
