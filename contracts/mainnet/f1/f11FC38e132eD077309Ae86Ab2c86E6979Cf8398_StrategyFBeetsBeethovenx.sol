...
contract StrategyFBeetsBeethovenx is StratManager, FeeManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Tokens used
    address public underlyingLP;
    address public want;
    ...
    constructor(
        ...
        (underlyingLP, ) = IBalancerVault(unirouter).getPool(wantPoolId);
        want = _want;
        output = _output;
        native = _native;
        input = _input;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IBeethovenxChef(masterChef).deposit(
                chefPoolId,
                wantBal,
                address(this)
            );
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IBeethovenxChef(masterChef).withdrawAndHarvest(
                chefPoolId,
                _amount.sub(wantBal),
                address(this)
            );
        ...
        uint256 withdrawFee = wantBal.mul(securityFee).div(PERCENT_DIVISOR);
        IERC20(want).safeTransfer(vault, wantBal.sub(withdrawFee));
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IBeethovenxChef(masterChef).harvest(chefPoolId, address(this));
    ...
    function _chargeFees(address callFeeRecipient) internal {
        ...
        if (input != native) {
            toNative = toNative.mul(totalFee).div(PERCENT_DIVISOR);
        }

        _balancerSwap(nativeSwapPoolId, output, native, toNative);

        uint256 nativeBal = IERC20(native).balanceOf(address(this));
        if (input == native) {
            nativeBal = nativeBal.mul(totalFee).div(PERCENT_DIVISOR);
        }

        uint256 callFeeAmount = nativeBal.mul(callFee).div(PERCENT_DIVISOR);
        IERC20(native).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 treasuryFeeAmount = nativeBal.mul(treasuryFee).div(
            PERCENT_DIVISOR
        );

        for (uint256 i = 0; i < revenueReceivers.length; i++) {
            uint256 shareRevenue = treasuryFeeAmount
                .mul(revenuePercentage[i])
                .div(PERCENT_DIVISOR);
            IERC20(native).safeTransfer(revenueReceivers[i], shareRevenue);
        }
    }
    ...
    function _balancerJoin(
        ...
        IBaseWeightedPool.JoinKind joinKind = IBaseWeightedPool
            .JoinKind
            .EXACT_TOKENS_IN_FOR_BPT_OUT;

        bytes memory userData = abi.encode(joinKind, amounts, 1);
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IBeethovenxChef(masterChef).userInfo(
            chefPoolId,
            address(this)
        );
        return _amount;
    ...
    function rewardsAvailable() public view returns (uint256) {
        return
            IBeethovenxChef(masterChef).pendingBeets(chefPoolId, address(this));
    }
    ...
    function retireStrat() external {
        ...
        IBeethovenxChef(masterChef).emergencyWithdraw(
            chefPoolId,
            address(this)
        );
    ...
    function _giveAllowances() internal {
        ...
        IERC20(want).safeApprove(masterChef, max);
        IERC20(output).safeApprove(unirouter, max);
        IERC20(underlyingLP).safeApprove(want, 0);
        IERC20(underlyingLP).safeApprove(want, max);
        IERC20(input).safeApprove(unirouter, 0);
        IERC20(input).safeApprove(unirouter, max);
    }
    ...
    function anualReward() external view returns (uint256) {
        return
            IBeethovenxChef(masterChef).beetsPerBlock().mul(3600).mul(24).mul(
                365
            );
    }
    ...
    function swapBribeToken(address _token, bytes32 _route) external onlyOwner {
        _balancerSwap(
            _route,
            _token,
            output,
    ...
    function wrapNativeBribe(address _wrappedNative) external onlyOwner {
        ...
        require(
            wrapped.deposit{ value: balance }() == wrapped.ERR_NO_ERROR(),
            "wrapping error"
        );
    }
...
contract StratManager is Ownable, Pausable {
     ...
     */
    constructor(address _unirouter, address _vault) {
...
abstract contract FeeManager is StratManager {
    ...
    uint256[] public revenuePercentage;

    event TotalFeeUpdated(uint256 newFee);
    event CallFeeUpdated(uint256 newCallFee, uint256 newTreasuryFee);

    function setRevenueReceivers(
        address[] memory _receivers,
        uint256[] memory _percentage
    ) external onlyOwner {
        require(_receivers.length == _percentage.length);
        uint256 sumPercentage = 0;
        for (uint256 i = 0; i < _percentage.length; i++) {
            sumPercentage += _percentage[i];
        }
        require(sumPercentage == PERCENT_DIVISOR, "Incorrect percentage");
        revenueReceivers = _receivers;
        revenuePercentage = _percentage;
    }
     ...
     */
    function updateCallFee(uint256 _callFee) external onlyOwner {
        callFee = _callFee;
