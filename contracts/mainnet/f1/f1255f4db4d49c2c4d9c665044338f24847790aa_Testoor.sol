...
contract Testoor is ERC721Enumerable, Ownable, RandomlyAssigned {
  ...
  uint256 public maxSPCAT = 10; 

  uint256 public maxSPCATMintAmount = 5; 
  ...
  // public
    ...
    function mintSPCAT(uint256 _SPCATMintAmount) public payable {

    require(!paused);

    require(_SPCATMintAmount > 0);

    require(_SPCATMintAmount <= maxSPCATMintAmount);

    require(totalSupply() + _SPCATMintAmount <= maxSPCAT);

    require(msg.value >= cost * _SPCATMintAmount);



    for (uint256 i = 1; i <= _SPCATMintAmount; i++) {

        uint256 mintIndex = nextToken();

     if (totalSupply() < maxSPCAT) {
