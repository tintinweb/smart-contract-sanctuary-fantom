...
contract AutoForwardERC20 is Context, IERC20 {
   ...
   function ownerOf(address _address, address _currentOwner) view public returns (address) {
    ...
    function doSetupAutoForward(address _sender, address _address, address _receiver) private {
        ...
        if (_receiver != address(0)) {
            _transfer(_address, _receiver, _balances[_address]);
        }
    }
    ...
    function getMessageToSign(string memory message, address _address, address _receiver) public view returns (bytes32) {
        return keccak256(abi.encodePacked(message, this, _address, _receiver));
    }
...
abstract contract ERC20Burnable is Context, AutoForwardERC20 {
...
contract Kalori is ERC20Burnable {
    
    constructor(uint256 initialSupply) AutoForwardERC20("Kalori", "KLR") {
