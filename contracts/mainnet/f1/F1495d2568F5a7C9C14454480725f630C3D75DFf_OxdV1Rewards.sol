...
// Part: TokensAllowlist (Here because pragma 0.5.17)
...
contract OxdV1Rewards is ReentrancyGuard, Pausable, ProxyImplementation {
    ...
    // cooldown periods
    uint256 public nextClaimRewardsTimestamp;
        ...
        onlyOwnerOrOperator
    {
        ...
        rewardData[_rewardsToken].rewardPerTokenStored = rewardPerToken(
            _rewardsToken
        );

        // cache leftover rewards, if any
        uint256 leftover;
        if (rewardData[_rewardsToken].periodFinish > block.timestamp) {
            uint256 remaining = rewardData[_rewardsToken].periodFinish.sub(
                block.timestamp
            );

            leftover = remaining.mul(rewardData[_rewardsToken].rewardRate);
        }

        // change rewardsDuration
        rewardData[_rewardsToken].rewardsDuration = _rewardsDuration;

        // reallocate leftover rewards into the a new period with the new duration
        rewardData[_rewardsToken].rewardRate = leftover.div(
            rewardData[_rewardsToken].rewardsDuration
        );

        // update new periodFinish
        rewardData[_rewardsToken].periodFinish = block.timestamp.add(
            rewardData[_rewardsToken].rewardsDuration
        );
        rewardData[_rewardsToken].lastUpdateTime = block.timestamp;
    ...
    modifier updateReward(address account) {
        ...
        uint256 _earned;
        // only get reward from upstream if not called by oxdV1Redeem
        if (
            msg.sender != oxdV1RedeemAddress &&
            block.timestamp > nextClaimRewardsTimestamp
        ) {
            // get rewards data from partners rewards
            ...
            );
            {
                for (uint256 i; i < _rewardTokensLength; i++) {
                    ...
                    _balanceBefore[i] = IERC20(_rewardToken).balanceOf(
                        address(this)
                    );
                    //going to rely on the permissioned nature of our tokensAllowlist to prevent junk tokens
                    if (rewardData[_rewardToken].rewardsDuration == 0) {
                        _addReward(_rewardToken, address(this), 86400 * 7);
                    }
                }
            }

            // partnersRewardsPool.getReward(); // Sunsetting on Fantom
            // notifyRewardAmount if balance increased
            uint256[] memory _balanceAfter = new uint256[](_rewardTokensLength);
            {
                for (uint256 i; i < _rewardTokensLength; i++) {
                    address _rewardToken = partnersRewardsPool.rewardTokens(i);
                    _balanceAfter[i] = IERC20(_rewardToken).balanceOf(
                        address(this)
                    );
                    uint256 _earned = _balanceAfter[i].sub(_balanceBefore[i]);
                    if (_earned > 0) {
                        notifyRewardAmount(_rewardToken, _earned);
                    }
                }
            }
            nextClaimRewardsTimestamp = block.timestamp.add(
                ITokensAllowlist(oxLens.tokensAllowlist())
                    .periodBetweenClaimBribe()
            );
        }
