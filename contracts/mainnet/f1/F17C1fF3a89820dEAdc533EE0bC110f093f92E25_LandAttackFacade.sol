...
// File: contracts/storage/_LandAttackStorage.sol



pragma solidity ^0.8.15;

contract _LandAttackStorage {
    bytes32 internal constant LAND_ATTACK_STORAGE_POSITION =
        keccak256("games.wolfland.contracts.landattackstorage");

    struct LandAttackStorage {
        mapping(uint256 => Attacker) attackers;
        mapping(uint256 => Protector) protectors;
        mapping(uint16 => mapping(uint256 => uint256)) acreProtectors;
        mapping(uint16 => uint256[]) farmAttackers;
        address wolfBattle;
        uint256 healBase;
    }
    ...
    function landAttackStorage()
        internal
        pure
        returns (LandAttackStorage storage ls)
    {
        bytes32 position = LAND_ATTACK_STORAGE_POSITION;
        assembly {
            ls.slot := position
        }
    }
...
library Stakes {
    ...
    uint8 private constant DEFENCE = 1;
    ...
    function estimateShear(
        ...
        _LandStakerStorage.Stake storage _stake = ls.stakes[farmId][acre][
            stakableType
        ][stakeId];
        ...
        uint256 fee = 0;
        if (ls.fee > 0) {
            fee = (amount * ls.fee) / DENOMINATOR;
        }
        return amount - fee;
    }

    function getAcreData(uint16 farmId, uint256 acre)
        external
        view
        returns (
            address owner,
    ...
    {
        _LandStorage.LandStorage storage ls = _landStorage();
        owner = ls.ownedFarms[farmId];
        hq = false;

        _LandStorage.Farm memory farm = ls.farms[farmId];
        if (farm.acres.length > 0) {
            uint8[3] memory _acre = farm.acres[acre];
            ...
            defence = ls.biomes[biome][DEFENCE];
        }
    }

    function getFarmData(uint16 farmId)
        external
        view
        returns (address owner, uint256 acres)
    {
        _LandStorage.LandStorage storage ls = _landStorage();
        owner = ls.ownedFarms[farmId];
        acres = ls.farms[farmId].acres.length;
    }
    ...
    function shear(
        ...
        if (block.timestamp > _stake.start && size > 0) {
            ...
            BabyWool(ls.babyWool).mint(
                owner,
                (duration * size * ls.shearBase * _stake.fertility) /
                    DENOMINATOR,
                ls.fee
            );
        }
    ...
    function withdraw(
        ...
        _LandStakerStorage.LandStakerStorage storage ls = _landStakerStorage();
        _LandStakerStorage.Stake[] storage stakes = ls.stakes[farmId][acre][
            stakableType
        ];
        require(stakeId < stakes.length && stakeId >= 0, "Incorrect stakeId.");
        uint256[] memory stakables = ls
        .stakes[farmId][acre][stakableType][stakeId].stakable;
        uint256[] storage stakablesRef = ls
        .stakes[farmId][acre][stakableType][stakeId].stakable;
        ...
        ls.stakeSize[farmId][acre][stakableType] -= indexSize;
        _ERC721 stakable = _ERC721(ls.stakables[stakableType]);
    ...
    _LandStakerStorage
{
    ...
    constructor(
        ...
        LandStakerStorage storage ls = landStakerStorage();
        ls.shearBase = shearBase;
        ls.fee = fee;
        ls.babyWool = babyWool;
    }
    ...
    {
        return Stakes.getAcreData(farmId, acre);
    }
    ...
    {
        return Stakes.getFarmData(farmId);
    }
    ...
    function allSheepDelegate(uint256[] calldata creatureId)
        ...
        returns (bool)
    {
        return Stakes.allSheepDelegate(creatureId);
    }
    ...
    function stake(
        ...
        require(allSheep, "Only sheep is allowed.");

        (address owner, uint256 capacity, uint256 fertility, , ) = abi.decode(
            _land.delegateCall(address(this), 0, abi.encode(farmId, acre)),
            (address, uint256, uint256, uint256, bool)
        );
        require(
            owner == msg.sender && capacity > 0,
        ...
        for (uint256 i = 0; i < ls.stakables.length; i++) {
            staked += ls.stakeSize[farmId][acre][i];
        }
        require(
            capacity > staked && (capacity - staked) >= stakables.length,
            "Not enought staking capacity."
        );

        IERC721 stakable = IERC721(ls.stakables[stakableType]);
        ...
        ls.stakes[farmId][acre][stakableType].push(
            Stake(block.timestamp, fertility, stakables)
        );
        ls.stakeSize[farmId][acre][stakableType] += stakables.length;
    }
    ...
    {
        LandStakerStorage storage ls = landStakerStorage();
        Stake memory _stake = ls.stakes[farmId][acre][stakableType][stakeId];
        return (_stake.start, _stake.fertility, _stake.stakable);
    }
    ...
    function getStakeSize(
        ...
        LandStakerStorage storage ls = landStakerStorage();
        return ls.stakes[farmId][acre][stakableType].length;
    }
    ...
    function estimateShear(
        ...
        return Stakes.estimateShear(farmId, acre, stakableType, stakeId);
    }

    function shearAll(uint16 farmId) external {
        LandStakerStorage storage ls = landStakerStorage();
        (address owner, uint256 acres) = abi.decode(
            _land.delegateCall(address(this), 1, abi.encode(farmId)),
            (address, uint256)
        );
        require(owner == msg.sender, "Incorrect owner.");
        uint256 stakables = ls.stakables.length;
        for (uint256 acre = 0; acre < acres; acre++) {
            for (uint256 stakable = 0; stakable < stakables; stakable++) {
                Stake[] memory stakes = ls.stakes[farmId][acre][stakable];
                for (uint256 _stake = 0; _stake < stakes.length; _stake++) {
                    if (stakes[_stake].stakable.length > 0) {
                        _shear(farmId, acre, stakable, _stake, owner);
                    }
                }
            }
        }
    ...
    function shear(
        ...
        LandStakerStorage storage ls = landStakerStorage();
        Stake storage _stake = ls.stakes[farmId][acre][stakableType][stakeId];
        require(block.timestamp > _stake.start, "Nothing to shear.");
        address owner = _land.getOwnedFarm(farmId);
        require(owner == msg.sender, "Incorrect owner.");
        _shear(farmId, acre, stakableType, stakeId, owner);
    }
    ...
    function withdraw(
        ...
        _withdraw(farmId, acre, stakableType, stakeId, stakableIndex, owner);
    }
        ...
        for (uint256 acre = 0; acre < acres; acre++) {
            for (uint256 stakable = 0; stakable < stakables; stakable++) {
                ...
                for (uint256 _stake = 0; _stake < stakes.length; _stake++) {
                    if (stakes[_stake].stakable.length > 0) {
                        if (andShear) {
                            _shear(farmId, acre, stakable, 0, owner);
                        }
                        _withdraw(
                            farmId,
                            acre,
    ...
    function _shear(
        ...
        Stakes.shear(farmId, acre, stakableType, stakeId, owner);
    }
...
// File: contracts/abstract/_BabyCreatureOperator.sol



pragma solidity ^0.8.15;

contract _BabyCreatureOperator {
    address private constant ADDRESS_ZERO = address(0);
    uint256 private constant INVALID = 2;
    uint256 private constant NOT_APPROVED = 0;
    ...
    constructor(
        ...
        _babyCreature[OLD] = _ERC721(babyCreatureOld);
        _babyCreature[NEW] = _ERC721(babyCreatureNew);
        TOKEN_ID_SHIFT = tokenIdShift;
    }

    function getBabyCreature(bool old)
        external
        view
        returns (address babyCreature)
    {
        babyCreature = old
            ? address(_babyCreature[OLD])
            : address(_babyCreature[NEW]);
    }

    function _transferToThis(uint256 tokenId) internal {
        uint256[] memory _tokenId = new uint256[](1);
        _tokenId[0] = tokenId;
        _transferToThis(_tokenId);
    }

    function _transferFromThis(uint256 tokenId, address to) internal {
        uint256[] memory _tokenId = new uint256[](1);
        _tokenId[0] = tokenId;
        _transferFromThis(_tokenId, to);
    }

    function _transferToThis(uint256[] memory tokenId) internal {
        uint256 currentContract;
        uint256[2] memory allApproved = [INVALID, INVALID];
        for (uint256 i = 0; i < tokenId.length; i++) {
            if (tokenId[i] < TOKEN_ID_SHIFT) {
                currentContract = OLD;
            } else {
                currentContract = NEW;
            }
            ...
            require(
                msg.sender ==
                    _babyCreature[currentContract].ownerOf(tokenId[i]),
                "Incorrect owner."
            );

            if (allApproved[currentContract] == INVALID) {
                allApproved[currentContract] = _babyCreature[currentContract]
                    .isApprovedForAll(msg.sender, address(this))
                    ? APPROVED
                    : NOT_APPROVED;
            }
            if (allApproved[currentContract] == NOT_APPROVED) {
                require(
                    address(this) ==
                        _babyCreature[currentContract].getApproved(tokenId[i]),
                    "Not approved."
                );
            }

            _babyCreature[currentContract].safeTransferFrom(
                msg.sender,
                address(this),
                tokenId[i]
            );
    ...
    function _transferFromThis(uint256[] memory tokenId, address to) internal {
        for (uint256 i = 0; i < tokenId.length; i++) {
            if (tokenId[i] < TOKEN_ID_SHIFT) {
                ADDRESS_ZERO == to
                    ? _babyCreature[OLD].burn(tokenId[i])
                    : _babyCreature[OLD].transferFrom(
                        address(this),
                        to,
                        tokenId[i]
                    );
                ...
                ADDRESS_ZERO == to
                    ? _babyCreature[NEW].burn(tokenId[i])
                    : _babyCreature[NEW].transferFrom(
                        address(this),
                        to,
                        tokenId[i]
                    );
    ...
    function _setBabyCreature(address babyCreature, bool old) internal {
        old
            ? _babyCreature[OLD] = _ERC721(babyCreature)
            : _babyCreature[NEW] = _ERC721(babyCreature);
    }
}
...
contract LandAttack is LandStaker, _BabyCreatureOperator, _LandAttackStorage {
    uint8 private constant MAX_ALPHA = 8;
    uint8 private constant MIN_STAKE = 1;
    uint256 private _healBase;
    address private _wolfBattle;

    event Attack(
        uint16 indexed fromFarmId,
        uint16 indexed toFarmId,
        uint256 indexed wolfId,
        uint256 acre,
        bool won,
        bool shear,
        bool stolen,
    ...
    constructor(
        ...
        LandStaker(land, babyWool, creatureAttributesDelegator, shearBase, fee)
        _BabyCreatureOperator(babyCreatureOld, babyCreatureNew, tokenIdShift)
    {
        _healBase = healBase;
        _wolfBattle = wolfBattle;
    }
    ...
    function getAlphaDelegate(uint256 wolfId)
        ...
        returns (uint64 alpha)
    {
        return Attacks.getAlphaDelegate(wolfId);
    }

    function getSurvivorDelegate(uint256 sheepId) external view returns (bool) {
        return Attacks.getSurvivorDelegate(sheepId);
    }
    ...
    function getDistanceDelegate(
        ...
        return Attacks.getDistanceDelegate(fromFarmId, toFarmId, acre);
    }
    ...
    function attack(
        ...
        (address owner, ) = abi.decode(
            _land.delegateCall(address(this), 1, abi.encode(fromFarmId)),
            (address, uint256)
        );
        require(owner == msg.sender, "Incorrect owner.");

        LandAttackStorage storage _storage = landAttackStorage();
        Attacker storage attackerRef = _storage.attackers[wolfId];

        if (attackerRef.alpha == 0) {
            uint64 alpha = _getAlpha(wolfId);
            require(alpha > 0, "Incorrect wolfId.");

            _transferToThis(wolfId);

            attackerRef.alpha = alpha;
            attackerRef.farmId = fromFarmId;
            _storage.farmAttackers[fromFarmId].push(wolfId);
        } else {
            require(
                attackerRef.farmId == fromFarmId &&
                    attackerRef.healTime <= block.timestamp,
                "Incorrect wolf parameter."
            );
        }

        (uint256 distance, bool hq) = abi.decode(
            _land.delegateCall(
                address(this),
                2,
                abi.encode(fromFarmId, toFarmId, acre)
            ),
            (uint256, bool)
        );
        assert(distance > 0);
        ...
        if (hq) {
            require(attackerRef.alpha > 6, "Incorrect alpha.");
        }

        attackerRef.healTime = block.timestamp + (distance * _healBase);
        _attack(fromFarmId, wolfId, toFarmId, acre, shear, hq);
    }
    ...
    function protect(
        ...
        (address owner, , , uint256 defence, bool hq) = abi.decode(
            _land.delegateCall(address(this), 0, abi.encode(farmId, acre)),
            (address, uint256, uint256, uint256, bool)
        );
        require(owner == msg.sender, "Incorrect owner.");
        ...
        require(
            _storage.acreProtectors[farmId][acre] == 0,
            "Incorrect acre parameter."
        );

        Protector storage protectorRef = _storage.protectors[wolfId];

        require(protectorRef.alpha == 0, "Incorrect wolf parameter.");

        uint64 alpha = _getAlpha(wolfId);
        if (hq) {
            require(alpha > 6, "Incorrect alpha.");
        }

        _transferToThis(wolfId);

        protectorRef.alpha = alpha;
        protectorRef.farmId = farmId;
        protectorRef.defence = defence;

        _storage.acreProtectors[farmId][acre] = wolfId;
    }

    function withdrawProtector(uint16 farmId, uint256 acre) external {
        (address owner, ) = abi.decode(
            _land.delegateCall(address(this), 1, abi.encode(farmId)),
            (address, uint256)
        );
        require(owner == msg.sender, "Incorrect owner.");
        _withdrawProtection(farmId, acre, owner);
    }

    function withdrawAllProtector(uint16 farmId) external {
        (address owner, uint256 acres) = abi.decode(
            _land.delegateCall(address(this), 1, abi.encode(farmId)),
            (address, uint256)
        );
    ...
    function withdrawAttacker(uint16 farmId, uint256 index) external {
        (address owner, ) = abi.decode(
            _land.delegateCall(address(this), 1, abi.encode(farmId)),
            (address, uint256)
        );
        ...
        LandAttackStorage storage _storage = landAttackStorage();
        uint256 wolfId = _storage.farmAttackers[farmId][index];

        require(
            _storage.attackers[wolfId].healTime <= block.timestamp,
            "Healing is in progress."
        );

        _storage.farmAttackers[farmId][index] = _storage.farmAttackers[farmId][
            _storage.farmAttackers[farmId].length - 1
        ];
        _storage.farmAttackers[farmId].pop();
        delete _storage.attackers[wolfId];

        _transferFromThis(wolfId, owner);
    }

    function withdrawAllAttacker(uint16 farmId) external {
        (address owner, ) = abi.decode(
            _land.delegateCall(address(this), 1, abi.encode(farmId)),
            (address, uint256)
        );
        ...
        LandAttackStorage storage _storage = landAttackStorage();
        uint256[] memory farmAttackers = _storage.farmAttackers[farmId];
        delete _storage.farmAttackers[farmId];

        for (
            uint256 attacker = 0;
            attacker < farmAttackers.length;
            attacker++
        ) {
            require(
                _storage.attackers[farmAttackers[attacker]].healTime <=
                    block.timestamp,
                "Healing is in progress."
            );
            delete _storage.attackers[farmAttackers[attacker]];
        }
        _transferFromThis(farmAttackers, owner);
    }

    function getAttacker(uint256 wolfId)
        external
        view
        returns (
            uint16,
    ...
    {
        LandAttackStorage storage _storage = landAttackStorage();
        Attacker storage attacker = _storage.attackers[wolfId];
        return (attacker.farmId, attacker.alpha, attacker.healTime);
    }

    function getProtector(uint256 wolfId)
        external
        view
        returns (
            uint16,
    ...
    {
        LandAttackStorage storage _storage = landAttackStorage();
        Protector storage protector = _storage.protectors[wolfId];
        return (protector.farmId, protector.alpha, protector.defence);
    }

    function getAcreProtector(uint16 farmId, uint256 acre)
        external
        view
        returns (uint256)
    {
        LandAttackStorage storage _storage = landAttackStorage();
        uint256 wolfId = _storage.acreProtectors[farmId][acre];
        require(wolfId > 0, "Non-existing protection reference.");
        return wolfId;
    }

    function getFarmAttackerSize(uint16 farmId)
        external
        view
        returns (uint256)
    {
        LandAttackStorage storage _storage = landAttackStorage();
        return _storage.farmAttackers[farmId].length;
    }

    function getFarmAttacker(uint16 farmId, uint256 index)
        external
        view
        returns (uint256)
    {
        LandAttackStorage storage _storage = landAttackStorage();
        return _storage.farmAttackers[farmId][index];
    }

    function setBabyCreature(address babyCreature, bool old)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _setBabyCreature(babyCreature, old);
    }

    function setHealBase(uint256 healBase)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _healBase = healBase;
    ...
    function getHealBase() external view returns (uint256) {
        return _healBase;
    }

    function setWolfBattle(address wolfBattle)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _wolfBattle = wolfBattle;
    ...
    function getWolfBattle() external view returns (address) {
        return _wolfBattle;
    }

    function _shearFrom(uint16 farmId, uint256 acre) private {
        LandStakerStorage storage ls = landStakerStorage();
        uint256 stakables = ls.stakables.length;
        for (uint256 stakable = 0; stakable < stakables; stakable++) {
            Stake[] memory stakes = ls.stakes[farmId][acre][stakable];
            for (uint256 _stake = 0; _stake < stakes.length; _stake++) {
                if (stakes[_stake].stakable.length > 0) {
                    _shear(farmId, acre, stakable, _stake, msg.sender);
                }
            }
        }
    }

    function _shearFrom(uint16 farmId) private {
        (, uint256 acres) = abi.decode(
            _land.delegateCall(address(this), 1, abi.encode(farmId)),
            (address, uint256)
        );
        for (uint256 acre = 0; acre < acres; acre++) {
            _shearFrom(farmId, acre);
        }
    }

    function _steal(
        Attacker memory attacker,
        Protector memory protector,
        uint16 farmId,
        uint256 acre
    ) private returns (bool, uint256) {
        LandStakerStorage storage ls = landStakerStorage();

        (bool transfer, address to, uint256[3] memory params) = Attacks
            .getStealSheep(farmId, acre, attacker.alpha, protector.alpha);
        ...
        if (transfer) {
            transfer = !abi.decode(
                _creatureAttributesDelegator.delegateCall(
                    ...
                    2,
                    abi.encode(
                        ls.stakes[farmId][acre][params[0]][params[1]].stakable[
                            params[2]
                        ]
                    )
                ),
        ...
        if (transfer) {
            uint256[] memory _stakable = new uint256[](1);
            _stakable[0] = params[2];

            stolenId = ls.stakes[farmId][acre][params[0]][params[1]].stakable[
                params[2]
            ];

            _withdraw(farmId, acre, params[0], params[1], _stakable, to);
        }

        return (transfer, stolenId);
    }
    ...
    function _stealHQ(
        ...
        if (
            protector.alpha == 0 ||
            protector.alpha == MAX_ALPHA ||
            attacker.alpha != MAX_ALPHA
        ) {
            return false;
        }

        (bool transfer, address to) = Attacks.getStealWolf(
            attacker.alpha,
            protector.defence
        );

        if (transfer) {
            _withdrawProtection(farmId, acre, to);
        }

        return transfer;
    }

    function _withdrawProtection(
        uint16 farmId,
        uint256 acre,
        address owner
    ) private {
        LandAttackStorage storage _storage = landAttackStorage();
        uint256 protector = _storage.acreProtectors[farmId][acre];

        if (_storage.protectors[protector].alpha > 0) {
            delete _storage.acreProtectors[farmId][acre];
            delete _storage.protectors[protector];

            _transferFromThis(protector, owner);
        }
    }
    ...
    function _attack(
        ...
        Attacker storage attackerRef = _storage.attackers[wolfId];
        Protector storage protectorRef = _storage.protectors[
            _storage.acreProtectors[toFarmId][acre]
        ];
        ...
        if (protectorRef.alpha > 0) {
            uint256[] memory param = new uint256[](5);
            param[0] = fromFarmId;
            param[1] = toFarmId;
            param[2] = acre;
            param[3] = wolfId;
            param[4] = _storage.acreProtectors[toFarmId][acre];
            won = abi.decode(
                Address.functionCall(
                    _wolfBattle,
                    bytes.concat(
                        bytes4(keccak256(bytes("battle(uint256[])"))),
                        abi.encode(param)
                    )
                ),
                (bool)
        ...
        if (hq) {
            if (shear) {
                ...
                stolenId = _storage.acreProtectors[toFarmId][acre];
                stolen = _stealHQ(attackerRef, protectorRef, toFarmId, acre);
            }
        } else {
            if (shear) {
                _shearFrom(toFarmId, acre);
            } else {
                (stolen, stolenId) = _steal(
                    attackerRef,
                    protectorRef,
                    toFarmId,
                    acre
    ...
    function _getAlpha(uint256 wolfId) private returns (uint64) {
        return
            abi.decode(
                _creatureAttributesDelegator.delegateCall(
                    address(this),
...
// File: contracts/facades/LandAttackFacade.sol
...
contract LandAttackFacade is _LandAttackStorage {
    Land private immutable _land;
    LandAttack private immutable _landAttack;
    _ERC721 private immutable _babyCreatureOld;
    _ERC721 private immutable _babyCreatureNew;
    uint256 public immutable _tokenIdShift;
    ...
    constructor(
        ...
        _land = Land(land);
        _landAttack = LandAttack(landAttack);
        _babyCreatureOld = _ERC721(babyCreatureOld);
        _babyCreatureNew = _ERC721(babyCreatureNew);
        _tokenIdShift = tokenIdShift;
    }

    function getFarmProtectorsDelegate(uint16 farmId, uint256 acres)
        external
        view
        returns (uint256[2][] memory protectors, uint256 size)
    {
        LandAttackStorage storage la = landAttackStorage();

        protectors = new uint256[2][](acres);
        size = 0;

        for (uint256 i = 0; i < acres; i++) {
            uint256 id = la.acreProtectors[farmId][i];
            Protector storage protector = la.protectors[id];
            if (protector.alpha > 0) {
                protectors[i][0] = id;
                protectors[i][1] = 1;
                size++;
            }
    ...
    function getFarmAttackersDelegate(uint16 farmId)
        external
        view
        returns (uint256[] memory attackers, uint256[] memory healTime)
    {
        LandAttackStorage storage la = landAttackStorage();

        attackers = la.farmAttackers[farmId];
        healTime = new uint256[](attackers.length);
        for (uint256 i = 0; i < attackers.length; i++) {
            healTime[i] = la.attackers[attackers[i]].healTime;
        }
    }

    function getFarmProtectors(uint16 farmId)
        external
        returns (string[] memory protectors)
    {
        (, uint256 acres, ) = _land.getFarmInfo(farmId);

        (uint256[2][] memory _protectors, ) = abi.decode(
            _landAttack.delegateCall(
                address(this),
                0,
                abi.encode(farmId, acres)
            ),
            (uint256[2][], uint256)
        );

        protectors = new string[](acres);

        for (uint256 i = 0; i < _protectors.length; i++) {
            if ( _protectors[i][1] == 0) {
                continue;
            }
            uint256 tokenId = _protectors[i][0];
            if (tokenId < _tokenIdShift) {
                protectors[i] = _babyCreatureOld.tokenURI(tokenId);
            } else {
                protectors[i] = _babyCreatureNew.tokenURI(tokenId);
            }
        }
    }

    function getFarmAttackers(
        uint16 farmId,
        uint256 from,
        uint256 to
    )
        external
        returns (
            string[] memory attackers,
            uint256[] memory healTime,
            uint256 last
        )
    {
        (uint256[] memory _attackers, uint256[] memory _healTime) = abi.decode(
            _landAttack.delegateCall(address(this), 1, abi.encode(farmId)),
            (uint256[], uint256[])
        );

        healTime = _healTime;
        last = _attackers.length;

        if (from < last) {
            attackers = new string[](to < last ? to - from : last - from);

            uint256 index = 0;
            for (uint256 i = from; i < (to < last ? to : last); i++) {
                uint256 tokenId = _attackers[i];
                if (tokenId < _tokenIdShift) {
                    attackers[index] = _babyCreatureOld.tokenURI(tokenId);
                } else {
                    attackers[index] = _babyCreatureNew.tokenURI(tokenId);
                }
                index++;
            }
        }
    ...
    function getParameters() external view returns (bool paused, uint256 healBase) {
        paused = _landAttack.paused();
        healBase = _landAttack.getHealBase();
    }
}