...
contract SecondRewarder is IRewarder, OwnableUpgradeable {
    ...
    /// `rewardDebt` The amount of RING entitled to the user.
    struct UserInfo {
        uint256 amount;
        uint256 rewardDebt;
    ...
    /// Also known as the amount of RING to distribute per block.
    struct PoolInfo {
        uint128 accRingPerShare;
    ...
    function onRingReward(
        ...
        uint256 pending;
        if (user.amount > 0) {
            pending = (user.amount.mul(pool.accRingPerShare) /
        ...
        user.amount = lpToken;
        user.rewardDebt =
            lpToken.mul(pool.accRingPerShare) /
            ACC_TOKEN_PRECISION;
    ...
    /// @notice Sets the ring per second to be distributed. Can only be called by the owner.
    ...
    /// @notice Update the given pool's RING allocation point and `IRewarder` contract. Can only be called by the owner.
    ...
    /// @return pending RING reward for a given user.
        ...
        returns (uint256 pending)
    {
        ...
        uint256 lpSupply = MiniChefV2(MINICHEF_V2).lpToken(_pid).balanceOf(
            MINICHEF_V2
        );
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 time = block.timestamp.sub(pool.lastRewardTime);
            uint256 ringReward = time.mul(rewardPerSecond).mul(
                pool.allocPoint
            ) / totalAllocPoint;
            accRingPerShare = accRingPerShare.add(
                ringReward.mul(ACC_TOKEN_PRECISION) / lpSupply
            );
        }
        pending = (user.amount.mul(accRingPerShare) / ACC_TOKEN_PRECISION)
            .sub(user.rewardDebt)
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            uint256 lpSupply = MiniChefV2(MINICHEF_V2).lpToken(pid).balanceOf(
                MINICHEF_V2
            );
            ...
            if (lpSupply > 0) {
                ...
                pool.accRingPerShare = pool.accRingPerShare.add(
                    (ringReward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128()
                );
            }
...
contract MiniChefV2 is OwnableUpgradeable {
    ...
    function deposit(
        ...
        if (address(_rewarder) != address(0)) {
            _rewarder.onRingReward(pid, to, to, 0, user.amount);
        }
    ...
    function withdraw(
        ...
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onRingReward(pid, msg.sender, to, 0, user.amount);
        }
    ...
    function emergencyWithdraw(uint256 pid, address to) public {
        ...
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onRingReward(pid, msg.sender, to, 0, 0);
        }
