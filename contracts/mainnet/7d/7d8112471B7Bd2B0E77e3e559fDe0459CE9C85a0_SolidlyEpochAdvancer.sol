...
/**
 * @title SolidlyEpochAdvancer
 * @author beepidibop.eth
 * @notice Contract for advancing Solidly epochs with Gelato
 */

contract SolidlyEpochAdvancer is OpsReady, Ownable {
    using SafeERC20 for IERC20;

    IMinter public constant minter =
        IMinter(0xC4209c19b183e72A037b2D1Fb11fbe522054A90D);

    uint256 public tMinus;
    ...
    function gelatoAdvanceSolidlyEpoch() external onlyOps notPaused {
        require(tx.gasprice < maxGasPriceInGwei * 1 gwei, "Gas price too high");
        uint256 activePeriod = minter.active_period();
        require(
            block.timestamp > activePeriod + 1 weeks - tMinus,
            "Not time to advance yet"
        );
        _advanceSolidlyEpoch();
        (uint256 feeAmount, ) = IOps(ops).getFeeDetails();
        _payTxFee(feeAmount);
    }

    function advanceSolidlyEpoch() external {
        _advanceSolidlyEpoch();
    }

    function _advanceSolidlyEpoch() internal {
        // Low-level call for Gelato resolver
        address(minter).call(abi.encodeWithSignature("update_period()"));
    }

    //Settings
    function setTMinus(uint256 _tMinus) external onlyOwner {
        tMinus = _tMinus;
    }
    ...
    function createGelatoTask() external onlyOwner {
        ...
        bytes4 _execSelector = bytes4(
            abi.encodeWithSignature("gelatoAdvanceSolidlyEpoch()")
        );
    ...
    {
        canExec = true;
        execPayload = abi.encodeWithSignature("gelatoAdvanceSolidlyEpoch()");
    }
