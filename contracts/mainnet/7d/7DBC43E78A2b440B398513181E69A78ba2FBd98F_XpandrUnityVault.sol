...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant lqdr = address(0x10b620b2dbAC4Faa7D7FFD71Da486f5D44cd86f9);
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR LQDR-FTM EQUALIZER")),
            string(abi.encodePacked("XpE-LQDR-FTM"))
        )
        {
        gauge = _gauge;
        router = _router;
    ...
    function _addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmt, wftm, lqdr, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(wftm, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(lqdr, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(wftm, lqdr, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(wftm, lqdr, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyOwner {
        if(_token == equal || _token == wftm || _token == lqdr){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(lqdr, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(lqdr, router, type(uint).max);
    }
