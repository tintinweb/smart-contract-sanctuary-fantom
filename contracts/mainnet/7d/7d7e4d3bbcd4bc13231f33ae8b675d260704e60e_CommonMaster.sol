...
contract CommonMaster is ICommonMaster, Pausable, Ownable {
    ...
    if (mintedAmount.add(tokenReward) > totalToBeMintAmount) {
        tokenReward = totalToBeMintAmount.sub(mintedAmount);
    }
    mintedAmount = mintedAmount.add(tokenReward);

    // mint token reward for users
    require(token.mint(address(this), tokenReward), 'TOKEN: MINTING_FAILED');
