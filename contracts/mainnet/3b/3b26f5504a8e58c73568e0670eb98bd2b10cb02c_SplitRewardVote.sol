...
contract SplitRewardVote is Ownable {

    IERC721 trident;

    uint256 DEADLINE;
    
    int256 public voteResult;
    

    mapping(uint256=>bool) public voted;

    constructor(IERC721 trident_) {
        trident = trident_;
        voteResult = 0;
        DEADLINE = block.timestamp + 86400*7;
    }

    /**
    * If the result is negative then the rewards will not be halved. 
    * Otherwise, they will be.
    * In case of draw, a second vote will be carried out on discord within the community.
    * https://discord.gg/poseidonfinance
    * @param tokenId_ of the owner use for voting
    * @param value_ of the vote (true or false)
    */
    function vote(uint256 tokenId_, bool value_) external {
        require(block.timestamp < DEADLINE, "vote are closed");
        require(!voted[tokenId_], "can not vote twice.");
        
        if (trident.ownerOf(tokenId_) == msg.sender) {
            voteCount(value_);
            voted[tokenId_] = true;
        }
    }

    function voteCount(bool value_) private {
        if (value_) {
            voteResult += 1;
        } else {
            voteResult -= 1;
