...
contract HumanPalm is ERC721Enumerable, ERC721Pausable, ERC721Burnable, Ownable {
    ...
    uint256 public maxTokenIds = 1554;
    uint256 public maxDuringPresale = 3;
    uint256 public maxDuringSale = 9;
    uint256 public presaleDuration = 20 minutes;
    ...
    function give(address to, uint256 quantity) external onlyOwner {
        ...
        emit TokenMint(to, tokenId, (tokenId - 1) + quantity);
    }

    function giveMany(address[] calldata addresses, uint256 quantity) external onlyOwner {
        uint256 tokenId;
        for (uint256 j = 0; j < addresses.length; j++) {
            tokenId = _tokenIds.current();
            for (uint256 i = 0; i < quantity; i++) {
                _safeMint(addresses[j], tokenId + i);
                _tokenIds.increment();
            }
            emit TokenMint(addresses[j], tokenId, (tokenId - 1) + quantity);
        }
    }

    function startPresale() external onlyOwner {
        presaleStarted = true;
        presaleEnded = block.timestamp + presaleDuration;
    }
    ...
    function setMaxTokenIds(uint256 newMaxTokenIds) external onlyOwner {
        maxTokenIds = newMaxTokenIds;
    }

    function setMaxDuringPresale(uint256 newMaxDuringPresale) external onlyOwner {
        maxDuringPresale = newMaxDuringPresale;
    }

    function setMaxDuringSale(uint256 newMaxDuringSale) external onlyOwner {
        maxDuringSale = newMaxDuringSale;
    }

    function setPresaleDuration(uint256 newDuration) external onlyOwner {
        presaleDuration = newDuration;
    }
