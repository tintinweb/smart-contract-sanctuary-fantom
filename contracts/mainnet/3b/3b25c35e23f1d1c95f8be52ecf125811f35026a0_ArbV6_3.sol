...
contract ArbV6_3 is Operator {
    ...
    function getNumerator (uint256 kp) external view returns (
        ...
        numerator = this.sqrt_binarysearch2(kp);
    }

    function getFactor (uint8 _decimalBase, uint8 _decimalAsset) public pure returns (uint256 factor) {
        factor = 10 ** ((_decimalAsset + _decimalAsset - _decimalBase)-_decimalAsset / 2);
    }
    ...
    function getNewReserve(uint256 _pricePublic, uint256 _kPrivate, uint8 _decimalBase, uint8 _decimalAsset) 
        ...
        uint256 factor = this.getFactor(_decimalBase, _decimalAsset);
        newReserve = num.div(factor);
    }
