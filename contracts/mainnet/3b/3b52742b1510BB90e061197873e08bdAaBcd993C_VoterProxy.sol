    ...
    ProxyImplementation
{
        ...
        onlyOxPool
    {
        ...
        bool solidClaimed = true;
        if (oogSyncLimit(gaugeAddress, solidAddress)) {
            solidClaimed = claimSolid(msg.sender);
        }

        // deposit all LP if solid is claimed
        if (solidClaimed) {
            amount = solidPool.balanceOf(address(this));
        ...
        onlyOxPoolOrLegacyOxPool
    {
        ...
        uint256 lpBalanceHere = solidPool.balanceOf(address(this));

        require(
            solidClaimed || lpBalanceHere >= amount,
            "Gauge supply checkpoint not sync'd"
        );
        uint256 amountToWithdrawFromGauge;
        if (amount > lpBalanceHere) {
            amountToWithdrawFromGauge = amount.sub(lpBalanceHere);
        }
        if (amountToWithdrawFromGauge > 0) {
            require(solidClaimed, "Gauge supply checkpoint not sync'd");
            ISolidGauge(gaugeAddress).withdraw(amountToWithdrawFromGauge);
        }
        ...
        returns (bool _claimSolid)
    {
        ...
        if (_claimSolid) {
            ...
            uint256 _balanceBefore = IERC20(solidAddress).balanceOf(
                address(this)
            );
            ...
            solidStoredForOxPool[oxPoolAddress] = solidStoredForOxPool[
                oxPoolAddress
            ].add(IERC20(solidAddress).balanceOf(voterProxyAddress)).sub(
                    _balanceBefore
                );
    ...
    function getRewardFromBribe(
        ...
        //record solid balance
        uint256 solidBefore = IERC20(solidAddress).balanceOf(voterProxyAddress);
        ...
        for (
            ...
            if (claimableAddresses[tokenIndex] == solidAddress) {
                amount = IERC20(solidAddress).balanceOf(voterProxyAddress).sub(
                    solidBefore
                );
            }
            if (amount != 0) {
    ...
    /**
     * @notice Fetch solid directly from gauge
     * @param gaugeAddress Gauge address
     */
    function getSolidFromGauge(address gaugeAddress) public onlyGovernance {
        address[] memory tokensAddresses = new address[](1);
        tokensAddresses[0] = solidAddress;
        IGauge(gaugeAddress).getReward(address(this), tokensAddresses);
    }
