...
contract VaultUtils is IVaultUtils, Governable {
    ...
    function getBuyUsdeFeeBasisPoints(address _token, uint256 _usdeAmount) public view override returns (uint256) {
        return getFeeBasisPoints(_token, _usdeAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), true);
    }

    function getSellUsdeFeeBasisPoints(address _token, uint256 _usdeAmount) public view override returns (uint256) {
        return getFeeBasisPoints(_token, _usdeAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), false);
    }
    ...
    function getSwapFeeBasisPoints(
        ...
        uint256 feesBasisPoints0 = getFeeBasisPoints(_tokenIn, _usdeAmount, baseBps, taxBps, true);
        uint256 feesBasisPoints1 = getFeeBasisPoints(_tokenOut, _usdeAmount, baseBps, taxBps, false);
    ...
    function getFeeBasisPoints(
        ...
        uint256 initialAmount = vault.usdeAmounts(_token);
        uint256 nextAmount = initialAmount.add(_usdeDelta);
        if (!_increment) {
            nextAmount = _usdeDelta > initialAmount ? 0 : initialAmount.sub(_usdeDelta);
        }

        uint256 targetAmount = vault.getTargetUsdeAmount(_token);
        if (targetAmount == 0) {
            return _feeBasisPoints;
        }
