...
import "./LotteryAbstract.sol";

contract Lottery is Ownable, Initializable, Testable, LotteryAbstract {
    ...
    constructor(
        ...
        require(_bucketOneMaxNumber != 0 && _bucketTwoMaxNumber != 0, "e01");
        require(_bucketOneMaxNumber < _bucketTwoMaxNumber, "e02");
        require(
            _discountForBucketOne < _discountForBucketTwo &&
                _discountForBucketTwo < _discountForBucketThree,
            "e03"
        );
        require(_utilityToken != address(0), "e04");
        require(_sizeOfLotteryNumbers != 0 && _maxValidNumberRange != 0, "e05");
    ...
    //     require(sizeOfLottery_ != _newSize, "e07");
    //     require(sizeOfLottery_ != 0, "e08");
    ...
    //     require(maxValidRange_ != _newMaxRange, "e09");
    //     require(maxValidRange_ != 0, "e10");
    ...
    function updateBuckets(
        ...
        require(_bucketOneMax != 0 && _bucketTwoMax != 0, "e11");
        require(_bucketOneMax < _bucketTwoMax, "e12");
        require(
            _discountForBucketOne < _discountForBucketTwo &&
                _discountForBucketTwo < _discountForBucketThree,
            "e13"
        ...
        onlyOwner
    {
        ...
        require(allLotteries_[_lotteryId].lotteryStatus == Status.Open, "e15");
        ...
        randomGenerator_.getRandomNumber(_lotteryId, _seed);
        // Emits that random number has been requested
        emit RequestNumbers(_lotteryId, 0);
    }
    ...
    function numbersDrawn(
        ...
        for (uint256 i = 0; i < sizeOfLottery_; i++) {
            ...
            allLotteries_[_lotteryId].winningNumbers[i] = uint16(
     ...
     * @param   _prizePoolInUtilityToken The amount of utility token available to win in this
     *          lottery.
     ...
     * @param   _percentageBurn Percentage of the received tokens to burn
     *
     */
    function createNewLotto(
        uint8[] calldata _prizeDistribution,
        uint256 _prizePoolInUtilityToken,
        uint256 _costPerTicket,
        uint256 _startingTimestamp,
        uint256 _closingTimestamp,
        uint256 _percentageBurn
    ) external onlyOwner returns (uint256 lotteryId) {
        require(_prizeDistribution.length == sizeOfLottery_, "e17");
        ...
        require(prizeDistributionTotal == 100, "e18");
        require(_prizePoolInUtilityToken != 0 && _costPerTicket != 0, "e19");
        require(
            _startingTimestamp != 0 && _startingTimestamp < _closingTimestamp,
            "e20"
        );
        require(_percentageBurn >= 0 && _percentageBurn < 100, "e21");
    ...
    function withdrawUtilityToken(uint256 _amount) external onlyOwner {
        utilityToken.transfer(msg.sender, _amount);
    }
    ...
    function batchBuyLottoTicket(
        ...
        require(allLotteries_[_lotteryId].lotteryStatus == Status.Open, "e24");
        require(_numberOfTickets <= 50, "e25");
        ...
        require(_chosenNumbersForEachTicket.length == numberCheck, "e26");
        ...
        (
            ...
            uint256 toBurn
        ) = this.costToBuyTicketsWithDiscountAndBurn(
                _lotteryId,
                _numberOfTickets
            );
        // Transfers the required utility token to this contract
        utilityToken.transferFrom(msg.sender, address(this), costWithDiscount);
        ...
        allLotteries_[_lotteryId].totalBurnt = allLotteries_[_lotteryId]
            .totalBurnt
            .add(toBurn);
        ...
        notContract
    {
        ...
        require(nft_.getOwnerOfTicket(_tokenId) == msg.sender, "e29");
        // Sets the claim of the ticket to true (if claimed, will revert)
        require(nft_.claimTicket(_tokenId, _lotteryId), "e30");
        ...
        allLotteries_[_lotteryId].prizePoolInUtilityToken = allLotteries_[
            _lotteryId
        ].prizePoolInUtilityToken.sub(prizeAmount);
        // Transfering the user their winnings
        utilityToken.safeTransfer(address(msg.sender), prizeAmount);
    }
    ...
    function batchClaimRewards(uint256 _lotteryId, uint256[] calldata _tokeIds)
        ...
        notContract
    {
        require(_tokeIds.length <= 50, "e35");
        // Checking the lottery is in a valid time for claiming
        require(
            allLotteries_[_lotteryId].closingTimestamp <= getCurrentTime(),
            "31"
        ...
        for (uint256 i = 0; i < _tokeIds.length; i++) {
            ...
            require(nft_.getOwnerOfTicket(_tokeIds[i]) == msg.sender, "e33");
            ...
            require(nft_.claimTicket(_tokeIds[i], _lotteryId), "e34");
            // Getting the number of matching tickets
            uint8 matchingNumbers = _getNumberOfMatching(
        ...
        utilityToken.safeTransfer(address(msg.sender), totalPrize);
    }
}
...
abstract contract LotteryAbstract {
    using SafeMath for uint256;
    using SafeMath16 for uint16;
    using SafeMath8 for uint8;
    using SafeERC20 for IERC20;
    ...
    // State variables
    // Instance of utility token (collateral currency for lotto)
    IERC20 internal utilityToken;
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInUtilityToken; // The amount of utility token for prize money
        uint256 costPerTicket; // Cost per ticket in $utility token
        ...
        uint256 totalBurnt; // Total number of burned tokens
        uint256 burnAmount; // Amount of tokens to burn in percentage points, e,g 1
    }
    ...
    event NewBurn(uint256 amountBurnt);
    ...
    modifier onlyRandomGenerator() {
        require(msg.sender == address(randomGenerator_), "e36");
        _;
    }

    modifier notContract() {
        require(!address(msg.sender).isContract(), "e37");
        require(msg.sender == tx.origin, "e38");
        _;
    }
     ...
     * @return  uint256: The prize amount in utility token the user is entitled to
     */
    ...
    function costToBuyTickets(uint256 _lotteryId, uint256 _numberOfTickets)
        ...
        returns (uint256)
    {
        return allLotteries_[_lotteryId].costPerTicket.mul(_numberOfTickets);
    }

    function costToBuyTicketsWithDiscountAndBurn(
        uint256 _lotteryId,
        ...
        )
    {
        ...
        toBurn = costWithDiscount.mul(allLotteries_[_lotteryId].burnAmount).div(
                100
            );
    }
