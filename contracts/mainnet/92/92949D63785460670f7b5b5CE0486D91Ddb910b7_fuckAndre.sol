...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75);
...
contract fuckAndre is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   fuckAndreDividendTracker public dividendTracker;
    ...
    uint256 private _buyDeadFee;


    uint256 private _sellRewardsFee;
    uint256 private _sellLiquidityFee;
	uint256 private _sellDeadFee;
    ...
    event DeadWalletUpdated(address indexed newDeadWallet, address indexed oldDeadWallet);
    ...
    constructor() ERC20 ("Fuck Andre", "FUANDRE") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 2;
		_buyDeadFee = 0;
        ...
        _maxBuy = 10000000 * (10**18);
        _maxSell = 10000000 * (10**18);

        totalBuyFees = _buyRewardsFee.add(_buyLiquidityFee).add(_buyDeadFee);
        totalSellFees = _sellRewardsFee.add(_sellLiquidityFee).add(_sellDeadFee);

    	dividendTracker = new fuckAndreDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Fuck Andre: The dividend tracker already has that address");

        fuckAndreDividendTracker newDividendTracker = fuckAndreDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Fuck Andre: The new dividend tracker must be owned by the Fuck Andre token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Fuck Andre: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Fuck Andre: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Fuck Andre: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Fuck Andre: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Fuck Andre: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Fuck Andre: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Fuck Andre: The liquidity wallet is already this address");
        excludeFromFees(newLiquidityWallet, true);
        emit LiquidityWalletUpdated(newLiquidityWallet, liquidityWallet);
        liquidityWallet = newLiquidityWallet;
    }

    function updateDeadWallet(address newDeadWallet) public onlyOwner {
        require(newDeadWallet != deadWallet, "Fuck Andre: The dead wallet is already this address");
        excludeFromFees(newDeadWallet, true);
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Fuck Andre: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Fuck Andre: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != deadWallet && to != deadWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
		    ...
		    uint256 swapTokens;
			uint256 swapAmount = _sellDeadFee.add(_sellRewardsFee).add(_sellLiquidityFee);
            ...
            uint256 deadAmount = address(this).balance.mul(_sellDeadFee).div(swapAmount);
            transferToWallet(payable(deadWallet), deadAmount);

            emit Transfer(address(this), deadWallet, address(this).balance.mul(_sellDeadFee).div(swapAmount));
            }
    ...
    function updateBuyFees(uint8 newBuyLiquidityFee, uint8 newBuyDeadFee, uint8 newBuyRewardsFee) public onlyOwner {
        _buyLiquidityFee = newBuyLiquidityFee;
        _buyDeadFee = newBuyDeadFee;
    ...
    function updateSellFees(uint8 newSellLiquidityFee, uint8 newSellDeadFee, uint8 newSellRewardsFee) public onlyOwner {
        _sellLiquidityFee = newSellLiquidityFee;
        _sellDeadFee = newSellDeadFee;
...
contract fuckAndreDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("FUANDRE_Dividend_Tracker", "FUANDRE_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "FUANDRE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Fuck Andre contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "FUANDRE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FUANDRE_Dividend_Tracker: Cannot update claimWait to same value");
