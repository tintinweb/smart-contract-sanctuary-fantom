...
contract Golden is ERC20Burnable, Operator {
    ...
    // Initial distribution for the GOLDEN-WFTM LP -> GOLDEN pool
    uint256 public constant INITIAL_GOLDEN_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the GOLDEN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Golden Finance", "GOLDEN") {
        // Mints 1 GOLDEN to contract creator for initial pool setup
    ...
    function _getGoldenPrice() internal view returns (uint256 _goldenPrice) {
        try IOracle(goldenOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Golden: failed to fetch GOLDEN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _goldenPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_goldenPrice >= taxTiersTwaps[tierId]) {
    ...
    function setGoldenOracle(address _goldenOracle) public onlyOperatorOrTaxOffice {
        require(_goldenOracle != address(0), "oracle address cannot be 0 address");
        goldenOracle = _goldenOracle;
    }
    ...
    /**
     * @notice Operator mints GOLDEN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of GOLDEN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentGoldenPrice = _getGoldenPrice();
            currentTaxRate = _updateTaxRate(currentGoldenPrice);
            if (currentGoldenPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_goldenPool != address(0), "!_goldenPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_goldenPool, INITIAL_GOLDEN_POOL_DISTRIBUTION);
