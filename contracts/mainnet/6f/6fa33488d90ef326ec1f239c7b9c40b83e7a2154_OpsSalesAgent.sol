...
// File contracts/SalesAgent/SalesAgent.sol

contract OpsSalesAgent is Ownable {
    ...
    function authorizeToken(
        ...
        address token1 = pairToken.token1();
        require(token0 == address(token) || token1 == address(token), "Invalid pair for token");
        tokenSwapSettings[token] = SwapSettings({ pairToken: pairToken, swapRouter: swapRouter });
    }

    function swapToken(IERC20 token) public {
        require(
    ...
    function withdraw(address token) public onlyOwner {
        if (address(token) != address(0)) {
            ...
            require(sent, "Could not complete payment");
        }
    }

    function withdraw(address token, address to) public onlyOwner {
        require(to != address(0), "Cannot withdraw to null address");
        if (address(token) != address(0)) {
