...
// File contracts/EqualizerUsdcFxsvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualizerUsdcFxsvAMM is Ownable, Pausable {
    ...
    IEqualizerRouter.Routes[] public wrappedToUsdcPath;
    IEqualizerRouter.Routes[] public wrappedToFxsPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToUsdcPath,
        IEqualizerRouter.Routes[] memory _wrappedToFxsPath
    ) {
        ...
        for (uint i; i < _wrappedToUsdcPath.length; ++i) {
            wrappedToUsdcPath.push(_wrappedToUsdcPath[i]);
        }

        for (uint i; i < _wrappedToFxsPath.length; ++i) {
            wrappedToFxsPath.push(_wrappedToFxsPath[i]);
        }
    ...
    function addLiquidity() internal {
        uint256 wrappedBal = IERC20(wrapped).balanceOf(address(this)) / 2;
        IEqualizerRouter(router).swapExactTokensForTokens(wrappedBal, 0, wrappedToUsdcPath, address(this), block.timestamp);
        IEqualizerRouter(router).swapExactTokensForTokens(wrappedBal, 0, wrappedToFxsPath, address(this), block.timestamp);
        uint256 t1Bal = IERC20(usdc).balanceOf(address(this));
        uint256 t2Bal = IERC20(fxs).balanceOf(address(this));

        IEqualizerRouter(router).addLiquidity(usdc, fxs, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);      
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(fxs).safeApprove(router, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(fxs).safeApprove(router, type(uint).max);
    }
