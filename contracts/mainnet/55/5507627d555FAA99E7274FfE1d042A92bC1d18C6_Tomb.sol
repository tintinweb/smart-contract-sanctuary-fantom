...
contract Tomb is Context, IERC20, Operator {
    ...
    uint256 maxTxAmountDuringGenesis = 25 ether;

    uint256 maxTxAmount = 999999999999999999999999 ether;
    ...
    bool isSysAddingLiquidity;

    uint256 burnFee = 200;
    uint256 liquidityFee = 200;
    uint256 burnFeeWhileGenesis = 500;
    uint256 liquidityFeeWhileGenesis = 1000;

    address public daoFund;

    //---//
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    uint256 public constant INITIAL_TOMB_POOL_DISTRIBUTION = 24000 ether;
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 0 ether;
    uint256 public constant INITIAL_MINT = 100 ether;
    ...
    modifier updateMaxAmount() {
        if (!isFirstTx) {
            if (now < tokenLaunchTs) {
                maxTxAmount = 0;
            } else if (now >= tokenLaunchTs && now <= genesisEndTs) {
                maxTxAmount = maxTxAmountDuringGenesis;
            } else {
                maxTxAmount = 99999999999999999999999999 ether;
            }
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private updateMaxAmount {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(amount < maxTxAmount, "ERC20: exceed maxTxAmount");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if (isDuringGeneis && takeFee && isSelling) {
            burnAmount = amount.mul(burnFeeWhileGenesis).div(10000);
            liquidityAmount = amount.mul(liquidityFeeWhileGenesis).div(10000);
        } else if (isDuringGeneis && !isSelling) {
            burnAmount = 0;
            liquidityAmount = 0;
        } else {
        ...
        uint256 amountAfterTax = amount.sub(burnAmount).sub(liquidityAmount);
        ...
        if (liquidityAmount > 0) {
            _standardTransfer(sender, daoFund, liquidityAmount);
        }
    ...
    function swapAndLiquify() private {
        ...
        uint256 pegBalance = pegERC20.balanceOf(address(this));
        ...
        //        uint256 newBalance = pegERC20.balanceOf(address(this)).sub(pegBalance);
        //
        //        // add liquidity to uniswap
        //        addLiquidity(otherHalf, newBalance);
        //        isSysAddingLiquidity = false;
    ...
    //    function addLiquidity(uint256 tokenAmount, uint256 pegTokenAmount) private {
    //        // approve token transfer to cover all possible scenarios
    //        _approve(address(this), address(uniswapV2Router), tokenAmount);
    //        _approve(pegToken, address(uniswapV2Router), pegTokenAmount);
    //
    //
    //        // add the liquidity
     ...
     */
    constructor(string memory name_, string memory symbol_, address _router, address _pegToken) public {
        _name = name_;
        _symbol = symbol_;
        _decimals = 18;
        ...
        pegToken = address(_pegToken);
        pegERC20 = IERC20(pegERC20);
