...
contract Distributor is Ownable {
  ...
  constructor(address _doggo, address[] memory _teamWallets, address[] memory _devWallets) {
    doggo = IERC20(_doggo);

    for (uint256 i = 0; i < _teamWallets.length; i ++) {
      wallets.push(_teamWallets[i]);
      allocation[_teamWallets[i]] = 100;
      totalAllocation += allocation[_teamWallets[i]];
    }

    for (uint256 i = 0; i < _devWallets.length; i ++) {
      wallets.push(_devWallets[i]);
      allocation[_devWallets[i]] = 200;
      totalAllocation += allocation[_devWallets[i]];
    }
  }
  ...
  function removeWallet(address _address) external onlyOwner {
    uint256 index;
    bool walletFound;
  ...
  function distribute() external {
    uint256 balance = doggo.balanceOf(address(this));

    for (uint256 i = 0; i < wallets.length; i ++) {
      if (allocation[wallets[i]] == 0) {
        continue;
      }
      uint256 amount = balance.mul(allocation[wallets[i]]).div(totalAllocation);
      doggo.safeTransfer(wallets[i], amount);
    }
  }
