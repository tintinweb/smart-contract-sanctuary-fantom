...
contract StrategyEqualizerAnkrRewardPoolLP is StratFeeManagerInitializable, GasFeeThrottler {
    ...
    address public beethovenRouter;
    bytes32 public balancerSwapPoolId;
    ...
    ISolidlyRouter.Routes[] public ankrFTMToLp0Route;
    ISolidlyRouter.Routes[] public ankrFTMToLp1Route;
    ...
    function initialize(
        ...
        bytes32[] calldata _balancerSwapPoolId,
        ISolidlyRouter.Routes[] calldata _ankrFTMToLp0Route,
        ISolidlyRouter.Routes[] calldata _ankrFTMToLp1Route
    ) public initializer {
        ...
        for (uint i; i < _ankrFTMToLp0Route.length; ++i) {
            ankrFTMToLp0Route.push(_ankrFTMToLp0Route[i]);
        }

        for (uint i; i < _ankrFTMToLp1Route.length; ++i) {
            ankrFTMToLp1Route.push(_ankrFTMToLp1Route[i]);
        }

        output = outputToNativeRoute[0].from;
        native = outputToNativeRoute[outputToNativeRoute.length - 1].to;
        lpToken0 = ankrFTMToLp0Route[ankrFTMToLp0Route.length - 1].to;
        lpToken1 = ankrFTMToLp1Route[ankrFTMToLp1Route.length - 1].to;
        rewards.push(output);

        ankrFTM = address(0xCfC785741Dc0e98ad4c9F6394Bb9d43Cd1eF5179);
        beethovenRouter = address(0x20dd72Ed959b6147912C2e529F0a0C651c33c9ce);
        balancerSwapPoolId = _balancerSwapPoolId[0];
    ...
    function addLiquidity() internal {
        uint256 nativeBal = IERC20(native).balanceOf(address(this));
        if (nativeBal > 0) {
            balancerSwap(balancerSwapPoolId, native, ankrFTM, nativeBal);
        }

        uint256 ankrFTMBal = IERC20(ankrFTM).balanceOf(address(this));
        uint256 lp0Amt = ankrFTMBal / 2;
        uint256 lp1Amt = ankrFTMBal - lp0Amt;

        if (stable) {
            uint256 lp0Decimals = 10 ** IERC20Extended(lpToken0).decimals();
            uint256 lp1Decimals = 10 ** IERC20Extended(lpToken1).decimals();
            uint256 out0 = (ISolidlyRouter(unirouter).getAmountsOut(lp0Amt, ankrFTMToLp0Route)[
                ankrFTMToLp0Route.length
            ] * 1e18) / lp0Decimals;
            uint256 out1 = (ISolidlyRouter(unirouter).getAmountsOut(lp1Amt, ankrFTMToLp1Route)[
                ankrFTMToLp1Route.length
            ] * 1e18) / lp1Decimals;
            ...
            lp0Amt = (ankrFTMBal * 1e18) / (ratio + 1e18);
            lp1Amt = ankrFTMBal - lp0Amt;
        }
    ...
    function balancerSwap(
        ...
        return IBalancerVault(beethovenRouter).swap(singleSwap, funds, 1, block.timestamp);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(ankrFTM).safeApprove(beethovenRouter, type(uint256).max);
    ...
    function _removeAllowances() internal {
        ...
        IERC20(ankrFTM).safeApprove(beethovenRouter, 0);
    ...
    function balancerSwapPool() external view returns (bytes32) {
        return balancerSwapPoolId;
    }

    function ankrFTMToLp0() external view returns (address[] memory) {
        ISolidlyRouter.Routes[] memory _route = ankrFTMToLp0Route;
        return _solidlyToRoute(_route);
    }

    function ankrFTMToLp1() external view returns (address[] memory) {
        ISolidlyRouter.Routes[] memory _route = ankrFTMToLp1Route;
        return _solidlyToRoute(_route);
    }
}
