...
import "../SolidlyPairPriceOracle/SolidlyPairPriceOracleModule.sol";
...
contract UniswapAnchoredView is UniswapConfig, SolidlyPairPriceOracleModule {
    using FixedPoint for *;
    ...
    constructor(OpenOraclePriceData priceData_,
        ...
        for (uint i = 0; i < configs.length; i++) {
            ...
            if (config.priceSource == PriceSource.REPORTER || config.priceSource == PriceSource.UNISWAP) {
                ...
                } else if (config.pairType == PairType.SolidlyLike) {
                    initializeSolidlyPair(config.uniswapMarket);
                }
            } else {
...
//    }
    ...
    function fetchAnchorPrice(string memory symbol, TokenConfig memory config, uint conversionFactor) internal virtual returns (uint) {
        ...
        if (config.pairType == PairType.UniV2Like) {
            ...
            return fetchAnchorPriceSolidlyLikePair(symbol, config, conversionFactor);
        } else {
    ...
    /**
     * @dev support only solidly like pairs with wNative as one of the tokens
     */
    function fetchAnchorPriceSolidlyLikePair(string memory symbol, TokenConfig memory config, uint nativePrice) internal virtual returns (uint) {
        require(config.pairType == PairType.SolidlyLike,"only solidly like pairs");

        (uint224 anchorPriceRaw, uint T) = getResultForSolidlyPair(config.uniswapMarket);
...
contract UniswapConfig {
    ...
    enum PairType {
        ...
        SolidlyLike // implies a Solidly like pair (using 'SolidlyPairPriceOracleModule')
    }
...
contract SolidlyPairPriceOracleModule {
    ...
    function initializeSolidlyPair(address pair) internal {
        ReserveInfo storage reserveInfoStorage = getReserveInfo[pair];
    ...
    function getResultForSolidlyPair(address pair) internal returns (uint224 price, uint32 T) {
