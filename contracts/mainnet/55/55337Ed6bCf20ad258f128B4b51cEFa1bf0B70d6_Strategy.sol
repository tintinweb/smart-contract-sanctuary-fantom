...
contract Strategy is BaseStrategy {
    ...
    IERC20 public rewardToken = IERC20(0x4cdF39285D7Ca8eB3f090fDA0C069ba5F4145B37); 
    ...
    uint256 public swapSecurityFactor = 9500;
    uint256 public sellSecurityFactor = 9500;
    uint256 public profitLiquidationFactor = 10000;

    address public router = address(0x6D0176C5ea1e44b08D3dd001b0784cE42F47a3A7); 
    address public swapRouter = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29); 
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('TombSwap FTM-TOMB Strategy v2'));
    }
    ...
    function estimatedHarvest() public view returns (uint256) {
        ...
        route[0] = address(rewardToken);
        route[1] = address(want);
        uint256[] memory amounts = IUni(swapRouter).getAmountsOut(_rewards, route);
        return amounts[1];
    }
    ...
    function balanceOfPooled() public view returns (uint256 _amount) {
        ...
        uint256 ftmPerTOMB = IUni(swapRouter).getAmountOut(1e18, reserveB, reserveA);
    ...
    function _withdrawFromMasterChefAndTransfer(address _to) internal {
        if (abandonRewards) {
            masterChef.emergencyWithdraw(masterChefPoolId); // withdrawFromMasterChef();
        } else {
            _claimRewards();
            withdrawFromMasterChef();
    ...
    function getSwapAmount(
        ...
        uint256 nominator = IUni(swapRouter).getAmountOut(halfInvestment, reserveA, reserveB);
        uint256 denominator =
            IUni(swapRouter).quote(
                halfInvestment,
    ...
    function giveSwapRouterAllowance() internal {
        IERC20(rewardToken).safeApprove(swapRouter, 0);
        IERC20(rewardToken).safeApprove(swapRouter, max);
    ...
    function removeSwapRouterAllowance() internal {
        IERC20(rewardToken).safeApprove(swapRouter, 0);
        want.safeApprove(swapRouter, 0);
        IERC20(tomb).safeApprove(swapRouter, 0);
    }
