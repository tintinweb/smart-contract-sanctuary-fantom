...
contract HpToken is BEP20('HP Token', 'HP') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "HP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "HP::delegateBySig: invalid nonce");
        require(now <= expiry, "HP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "HP::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying HP (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "HP::_writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChefHp is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt;     // Reward debt. See explanation below.
        //
        // We do some fancy math here. Basically, any point in time, the amount of HPs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accHpPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accHpPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accHpPerShare;   // Accumulated HPs per share, times 1e12. See below.
    ...
    event UpdateEmissionRate(address indexed user, uint256 hpPerBlock);
    ...
    // Update the given pool's hp allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 hpReward = multiplier.mul(hpPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        hp.mint(devaddr, hpReward.div(12));
        hp.mint(address(this), hpReward);
        pool.accHpPerShare = pool.accHpPerShare.add(hpReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for hp allocation.
    ...
    // Safe hp transfer function, just in case if rounding error causes pool to not have enough HPs.
    function safeHpTransfer(address _to, uint256 _amount) internal {
        uint256 hpBal = hp.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > hpBal) {
            transferSuccess = hp.transfer(_to, hpBal);
        } else {
            transferSuccess = hp.transfer(_to, _amount);
        }
        require(transferSuccess, "safeHpTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _hpPerBlock) public onlyOwner {
        massUpdatePools();
        hpPerBlock = _hpPerBlock;
        emit UpdateEmissionRate(msg.sender, _hpPerBlock);
    }
