...
contract ThoreumITOv4 is IThoreumITOv4, ReentrancyGuard, Ownable, Whitelisted {
    using SafeMath for uint256;
    ...
    address public thoreumToken = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;
    ...
    struct PoolCharacteristics {
        ...
        uint256 maxCommitRatio;  //max commit base on Thoreum token holding
        uint256 minThoreumToJoin;
    ...
    function depositPool(uint256 _amount, uint8 _pid, address _referrer) external override nonReentrant notContract {
        ...
        uint256 thoreumHolding = IBEP20(thoreumToken).balanceOf(msg.sender);
        uint256 limitPerUserInLP = pool.limitPerUserInLP;

        require(thoreumHolding >= pool.minThoreumToJoin,"Not meet min THOREUM");
        ...
        require(pool.hasOverflow || pool.totalAmountPool.add(_amount) <= pool.raisingAmountPool, "Pool is full");
        ...
        if(pool.maxCommitRatio > 0){
            uint256 newLimit = thoreumHolding.mul(pool.maxCommitRatio).div(10000);
    ...
    function emergencyRefund(uint8 _pid) external override nonReentrant notContract{
        ...
        IBEP20(_poolInformation[_pid].lpToken).safeTransfer(address(msg.sender), userFund);
    ...
    function setThoreumToken(address _token) external onlyOwner {
        thoreumToken = _token;
    }
    ...
    function setPool(
        ...
        _poolInformation[_pid].minThoreumToJoin = _minThoreumToJoin;
    ...
    function setEmergencyRefund() public onlyOwner {
    ...
    )
    {
        ...
        minThoreumToJoin = pool.minThoreumToJoin;
    ...
    event EmergencyRefund(address indexed user,uint256 userFund, uint8 indexed _pid);
