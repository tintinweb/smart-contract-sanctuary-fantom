...
contract CommonMaster is ICommonMaster, Pausable, Ownable {
    ...
    uint256 rewardPerBlock; // Reward tokens to distribute per block.
    uint256 rewardPerLP; // Reward tokens to distribute per LP token.
    bool exists; //
}
    ...
    function updatePool(address _pair) public override{
        ...
        pool.accTokenPerShare = pool.accTokenPerShare.add(
            tokenReward.mul(accTokenPerShareMultiple).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function stake(address _pair, uint256 _amount) external override whenNotPaused {
        ...
        if (_amount != 0) {
            // Mint token rewards and transfer the staked LP tokens to the contract
            uint256 rewardAmount = _amount.mul(pool.rewardPerLP).div(10**uint256(ERC20(_pair).decimals()));
