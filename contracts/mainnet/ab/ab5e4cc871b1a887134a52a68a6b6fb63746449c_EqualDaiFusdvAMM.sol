...
// File contracts/EqualDaiFusdvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract EqualDaiFusdvAMM is Ownable, Pausable {
    ...
    address public constant fusd = address(0xAd84341756Bf337f5a0164515b1f6F993D194E1f);
    address public want;
    ...
    IEqualizerRouter.Routes[] public wrappedToDaiPath;
    IEqualizerRouter.Routes[] public wrappedToFusdPath;
    ...
    constructor(
        ...
        IEqualizerRouter.Routes[] memory _wrappedToDaiPath,
        IEqualizerRouter.Routes[] memory _wrappedToFusdPath
    ) {
        ...
        for (uint i; i < _wrappedToFusdPath.length; ++i) {
            wrappedToFusdPath.push(_wrappedToFusdPath[i]);
        }

        for (uint i; i < _wrappedToDaiPath.length; ++i) {
            wrappedToDaiPath.push(_wrappedToDaiPath[i]);
        }
    ...
    function addLiquidity() internal {
        ...
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToFusdPath, address(this), block.timestamp);  
        IEqualizerRouter(unirouter).swapExactTokensForTokens(wrappedHalf, 0, wrappedToDaiPath, address(this), block.timestamp);   


        uint256 lp0Bal = IERC20(fusd).balanceOf(address(this));
        uint256 lp1Bal = IERC20(dai).balanceOf(address(this));

        IEqualizerRouter(unirouter).addLiquidity(fusd, dai, stable, lp0Bal, lp1Bal, 1, 1, address(this), block.timestamp);        
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(fusd).safeApprove(unirouter, 0);
    }
    ...
    function _giveAllowances() internal {
        ...
        IERC20(dai).safeApprove(unirouter, type(uint).max);
        IERC20(fusd).safeApprove(unirouter, type(uint).max);
    }
