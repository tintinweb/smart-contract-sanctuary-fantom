...
contract Oracle is IOracle, ReentrancyGuard
{
  ...
  {
    bool isBASE = _BASEFeed[token] != address(0);


    return (isBASE ? _BASEFeed[token] : _USDFeed[token], isBASE);
  }

  function setFeeds (address[] calldata tokens, address[] calldata feeds, bool[] calldata isUSDStates) external nonReentrant
  {
    require(tokens.length == feeds.length && feeds.length == isUSDStates.length, "!=");
    require(_USDFeed[_BASE] == address(0), "set");
    ...
    for (uint256 i; i < tokens.length; i++)
    {
      ...
      _decimal[token] = uint256(ERC20(token).decimals());
    }
  }
