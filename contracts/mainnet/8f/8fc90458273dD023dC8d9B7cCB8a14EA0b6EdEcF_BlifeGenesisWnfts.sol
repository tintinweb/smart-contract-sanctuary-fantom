...
contract BlifeGenesisWnfts is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    mapping(address => UserNfts) private stakedTokens;
    ...
    function getStakedNfts(address _user) public view returns (uint256[] memory) {
            uint256[] memory stakedNfts = new uint[](stakedTokens[_user].totalNftsStaked);
            uint iter  = 0;
            for(uint i = 0; i < stakedTokens[_user].ids.length; i++)
            {
                if(stakedTokens[_user].idStaked[stakedTokens[_user].ids[i]] == true)
                {
                 stakedNfts[iter] = stakedTokens[_user].ids[i];
                 iter++;
                }
    ...
    function StakeNft(uint256 _pid, uint256[] calldata _tokenIds) external nonReentrant {
        ...
        if (user.amount > 0) {
         ...
         if (_tokenIds.length > 0) {
            ...
            stakedTokens[_sender].idStaked[_tokenIds[i]] = true;
            stakedTokens[_sender].totalNftsStaked++;
    ...
    function UnstakeNft(uint256 _pid, uint256[] calldata _tokenIds) external nonReentrant  {
        ...
        if (_tokenIds.length > 0) {
            ...
            stakedTokens[_sender].idStaked[_tokenIds[i]] = false;
            stakedTokens[_sender].totalNftsStaked--;
