...
contract MvxPresaleMVD is Ownable, ReentrancyGuard {
  ...
  function presale(uint256 _mvdAmount) external nonReentrant {
    address account = msg.sender;
    require(presaleWhitelist[account], "Presale: forbidden");
    require(isPresaleActive, "Presale: presale is no longer active");
    require(_mvdAmount > 0, "Presale: invalid _mvdAmount");

    // calculate MVX amount
    uint256 mvdPrice = getMvdPrice();
    uint256 usdcAmount = _mvdAmount.mul(mvdPrice).div(10**21);
    uint256 mvxAmount = _mvdAmount.mul(mvdPrice).mul(10**9).div(mvxPresalePrice);

    usdcReceived = usdcReceived.add(usdcAmount);
    require(usdcReceived <= usdcHardCap, "Presale: usdcHardCap exceeded");

    presaleAmounts[account] = presaleAmounts[account].add(usdcAmount);
    require(presaleAmounts[account] <= usdcSlotCap, "Presale: usdcSlotCap exceeded");

    //burn mvd from sender
    mvdBurned = mvdBurned.add(_mvdAmount);
    IERC20Burnable(mvd).burnFrom(account, _mvdAmount);

    //store sender achieved mvx amount
    mvxBalances[account] = mvxBalances[account].add(mvxAmount);
  }
  ...
  function getMvdPrice() public view returns (uint256) {
    IUniswapV2Pair pair = IUniswapV2Pair(mvdDaiPair);
    (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
    return reserve1.mul(10**9).div(reserve0);
  }
}
