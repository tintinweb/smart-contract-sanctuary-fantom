...
contract SimplePriceOracle is OwnableUpgradeSafe, PriceOracle {
  ...
  modifier onlyFeeder() {
    require(
      msg.sender == feeder,
      "SimplePriceOracle::onlyFeeder:: only feeder"
    );
    _;
  }
  ...
  function setPrices(
    ...
    require(
      token1s.length == len,
      "SimplePriceOracle::setPrices:: bad token1s length"
    );
    require(
      prices.length == len,
      "SimplePriceOracle::setPrices:: bad prices length"
    );
    ...
    returns (uint256 price, uint256 lastUpdate)
  {
    ...
    require(
      ...
      "SimplePriceOracle::getPrice:: bad price data"
    );
