...
contract GasMonetization is AccessControl {
    ...
    event WithdrawalCompleted(uint256 indexed projectId, uint256 epochNumber, uint256 amount);
    event WithdrawalCanceled(uint256 indexed projectId, uint256 epochNumber);
    ...
    */
    function requestWithdrawal(uint256 projectId) external {
        ...
        uint256 epoch = _sfc.currentSealedEpoch();
        uint256 lastProjectWithdrawal = _projects[projectId].lastWithdrawalEpoch;
        require(
            lastProjectWithdrawal < _last_epoch_funds_added
            && epoch - lastProjectWithdrawal > _withdrawal_epochs_frequency_limit,
            "GasMonetization: must wait to withdraw"
        );
        // prepare new withdrawal
        _pending_withdrawals[projectId].requestedOnEpoch = epoch;
        emit WithdrawalRequested(projectId, epoch);
    }
    ...
    */
    function completeWithdrawal(uint256 projectId, uint256 epochNumber, uint256 amount) external {
        ...
        if (request.receivedConfirmationsCount + 1 >= _confirmations_to_make_withdrawal) {
            delete _pending_withdrawals[projectId];
            _projects[projectId].lastWithdrawalEpoch = epochNumber;
    ...
    * @param projectId Id of project.
    * @param epochNumber Epoch number of withdrawal.
    */
    function cancelWithdrawal(uint256 projectId, uint256 epochNumber) external {
        require(hasPendingWithdrawal(projectId, epochNumber), "GasMonetization: no withdrawal request");
        PendingWithdrawalRequest storage request = _pending_withdrawals[projectId];
        // only owner or data provider can cancel withdrawal
        if (_projects[projectId].owner == _msgSender()) {
            // also owner must wait to cancel withdrawal until withdrawal epoch limit is reached
            require(
                _sfc.currentEpoch() - request.requestedOnEpoch > _withdrawal_epochs_frequency_limit,
                "GasMonetization: must wait to cancel"
            );
        } else {
            require(hasRole(REWARDS_DATA_PROVIDER_ROLE, _msgSender()), "GasMonetization: not data provider or owner");
        }
        delete _pending_withdrawals[projectId];
        emit WithdrawalCanceled(projectId, epochNumber);
    }
    ...
    */
    function updateProjectRewardsRecipient(uint256 projectId, address recipient) external {
        require(
            _projects[projectId].owner == _msgSender() || hasRole(PROJECTS_MANAGER_ROLE, _msgSender()),
    ...
    */
    function addFunds() public payable {
        ...
        _last_epoch_funds_added = _sfc.currentSealedEpoch();
    ...
    */
    ...
    /**
    * @notice Get withdrawal epochs frequency limit.
    */
    function getWithdrawalEpochsFrequencyLimit() external view returns(uint256) {
        return _withdrawal_epochs_frequency_limit;
    }

    /**
    * @notice Get withdrawal confirmations limit.
    */
    function getWithdrawalConfirmationsLimit() external view returns(uint256) {
    ...
    */
    ...
    /**
    * @notice Get epoch of pending withdrawal for given project id.
    * @param projectId Project id.
    */
    function getPendingWithdrawalEpoch(uint256 projectId) external view returns(uint256) {
