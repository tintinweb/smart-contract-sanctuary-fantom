...
import "../core/interfaces/IEquityManager.sol";
...
contract Timelock is ITimelock {
    ...
    event SignalRedeemUsde(address vault, address token, uint256 amount);
    ...
    function setTokenConfig(
        ...
        IVault(_vault).setUsdeAmount(_token, _usdeAmount);
    }
    ...
    function setUsdeAmounts(address _vault, address[] memory _tokens, uint256[] memory _usdeAmounts) external onlyKeeperAndAbove {
        for (uint256 i = 0; i < _tokens.length; i++) {
            IVault(_vault).setUsdeAmount(_tokens[i], _usdeAmounts[i]);
        }
    }

    function updateUsdeSupply(uint256 usdeAmount) external onlyKeeperAndAbove {
        address usde = IEquityManager(equityManager).usde();
        uint256 balance = IERC20(usde).balanceOf(equityManager);

        IUSDE(usde).addVault(address(this));

        if (usdeAmount > balance) {
            uint256 mintAmount = usdeAmount.sub(balance);
            IUSDE(usde).mint(equityManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdeAmount);
            IUSDE(usde).burn(equityManager, burnAmount);
        }

        IUSDE(usde).removeVault(address(this));
    }

    function setEquityCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        IEquityManager(equityManager).setCooldownDuration(_cooldownDuration);
    }
    ...
    function signalRedeemUsde(address _vault, address _token, uint256 _amount) external onlyAdmin {
        bytes32 action = keccak256(abi.encodePacked("redeemUsde", _vault, _token, _amount));
        _setPendingAction(action);
        emit SignalRedeemUsde(_vault, _token, _amount);
    }
    ...
    function redeemUsde(address _vault, address _token, uint256 _amount) external onlyAdmin {
        ...
        address usde = IVault(_vault).usde();
        IVault(_vault).setManager(address(this), true);
        IUSDE(usde).addVault(address(this));

        IUSDE(usde).mint(address(this), _amount);
        IERC20(usde).transfer(address(_vault), _amount);

        IVault(_vault).sellUSDE(_token, mintReceiver);
