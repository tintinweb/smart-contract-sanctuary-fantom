...
interface IBalanceOfDegenano {
    function balanceOfDegenano(address _address) external view returns (uint256);
}
...
contract Degenano is ERC20Detailed, Ownable {
    ...
    uint256 public rewardYield = 4244079140000; 
    ...
    address[] public degenanoGamesContracts;
    address[] public partyArray;
    mapping(address => bool) public automatedMarketMakerPairs;
    mapping(address => bool) public subContractCheck;
    mapping(address => bool) public degenanoGamesCheck;
    ...
    uint256 private constant maxBracketTax = 10; 
    ...
    constructor() ERC20Detailed('Degenano', 'DGN', uint8(DECIMALS)) {
    ...
    function getUserTotalOnDifferentContractsDegenano(address sender)
    public
    view
    returns (uint256)
    {
        ...
        uint256 balanceOfAllDegenanoGamesContracts;
        ...
        balanceOfAllDegenanoGamesContracts = getBalanceOfAllDegenanoGamesContracts(
            sender
        );

        userTotal.add(balanceOfAllSubContracts).add(
            balanceOfAllDegenanoGamesContracts
        );
        return userTotal;
    }
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < subContracts.length; i++) {
            userTotal += IBalanceOfDegenano(subContracts[i]).balanceOfDegenano(
                sender
            );
        }
    ...
    function getBalanceOfAllDegenanoGamesContracts(address sender)
    public
    view
    returns (uint256)
    {
        uint256 degenanoGamesTotal;

        for (uint256 i = 0; i < degenanoGamesContracts.length; i++) {
            degenanoGamesTotal += IPublicBalance(degenanoGamesContracts[i])
            .balanceOf(sender);
        }

        return degenanoGamesTotal;
    }
    ...
    returns (uint256)
    {
        ...
        uint256 userTotal = getUserTotalOnDifferentContractsDegenano(_address);
    ...
    function _swapTokensForFTM(uint256 tokenAmount, address receiver)
    private
    {
    ...
    function addDegenanoGamesAddies(address _degenanoGamesAddy, bool _value)
    public 
    onlyOwner
    {
        require(
            degenanoGamesCheck[_degenanoGamesAddy] != _value,
            'Value already set'
        );
        ...
        if (_value) {
            degenanoGamesContracts.push(_degenanoGamesAddy);
        } else {
            require(degenanoGamesContracts.length > 1, 'Required 1 pair');
            for (uint256 i = 0; i < degenanoGamesContracts.length; i++) {
                if (degenanoGamesContracts[i] == _degenanoGamesAddy) {
                    degenanoGamesContracts[i] = degenanoGamesContracts[
                    degenanoGamesContracts.length - 1
                    ];
                    degenanoGamesContracts.pop();
                    break;
                }
            }
        }

        emit SetDegenanoGamesAddresses(_degenanoGamesAddy, _value);
    }
    ...
    function setFees(
        ...
        emit SetFees(_liquidityFee, _riskFreeValue, _treasuryFee, _sellFeeTreasuryAdded, _sellFeeRFVAdded, totalBuyFee);
    }
    ...
    function setIsLiquidityInFTM(bool _value) external onlyOwner {
        ...
        emit SetIsLiquidityInFTM(_value, block.timestamp);
    }
    ...
    event SetDegenanoGamesAddresses(address indexed pair, bool indexed value);
