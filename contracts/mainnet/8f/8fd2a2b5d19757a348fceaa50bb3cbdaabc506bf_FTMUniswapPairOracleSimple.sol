/**
 *Submitted for verification at FtmScan.com on 2022-09-05
*/
...
// File contracts/Oracle/FTMUniswapPairOracle.sol
...
contract FTMUniswapPairOracleSimple is Owned {
    using FixedPoint for *;
    using SafeMath for uint256;
    
    address timelock_address;

    uint public PERIOD = 300; // 5 minutes TWAP (time-weighted average price)
    ...
    //spa
    address public constant token0 = 0x5602df4A94eB6C680190ACCFA2A475621E0ddBdc;
    //dai
    address public constant token1 = 0x8D11eC38a3EB5E956B052f67Da8Bdc9bef8Abf3E;
    ...
    // uint256 public PRICE_PRECISION = 1e9;
    ...
    function consult(address token, uint amountIn) public view returns (uint amountOut) {
        ...
        if(timeElapsed < PERIOD){
            ...
            (uint price0Cumulative, uint price1Cumulative, ) = UniswapV2OracleLibrary.currentCumulativePrices(address(pair));
            // uint32 timeElapsed = blockTimestamp - blockTimestampLast; // Overflow is desired
    ...
    function latestRoundData() external view returns (
        ...
        // int256 consult_result = int256(consult(token0, 1e9));
        uint256 consult_result = consult(token0, 1e9);
        ...
        uint256 price = consult_result.mul(uint256(_price)).mul(1e6).div(1e26);
