...
// File contracts/Cronjetomb.sol

pragma solidity 0.6.12;


contract Cronjetomb is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the Cronjetomb ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Cronjetomb", "Cronjetomb") {
        // Mints 1 Cronjetomb to contract creator for initial pool setup
    ...
    function _getCronjetombPrice() internal view returns (uint256 _CronjetombPrice) {
        try IOracle(CronjetombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Cronjetomb: failed to fetch Cronjetomb price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _CronjetombPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_CronjetombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCronjetombOracle(address _CronjetombOracle) public onlyOperatorOrTaxOffice {
        require(_CronjetombOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints Cronjetomb to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Cronjetomb to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCronjetombPrice = _getCronjetombPrice();
            currentTaxRate = _updateTaxRate(currentCronjetombPrice);
            if (currentCronjetombPrice < burnThreshold) {
                burnTax = true;
            }
        }
