...
contract OneRingCurveRouterFE is Ownable {
  ...
  error OneRingCurveRouter_Too_Much_Slippage();
  error OneRingCurveRouter_Incorrect_Fee();
  error OneRingCurveRouter_Incorrect_Slippage();
  ...
  function swap(address from, address to, uint256 amount, uint256 minAmountToReceive, uint256 slippage) external returns(uint256) {
    if(from == address(0)) {
    ...
    if(slippage > 1000) {
      revert OneRingCurveRouter_Incorrect_Slippage();
    }
    ...
    if(poolToUse != address(0)) {
      amountReceived = _swap(from, to, poolToUse, amount, slippage);
    }
      ...
      uint256 amountUSDCReceived = _swap(from, USDC, poolToGoToUSDC, amount, slippage);
      // Then we go from USDC to ``to''
      amountReceived = _swap(USDC, to, poolFromUSDCtoExpectedToken, amountUSDCReceived, slippage);
    }

    if(amountReceived <  minAmountToReceive) {
      revert OneRingCurveRouter_Too_Much_Slippage();
    }
   
    uint256 amountToSend = (amountReceived*oneRingFee)/1000;
  ...
  function _swap(address from, address to, address poolToUse, uint256 amount, uint256 slippage) internal returns(uint256) {
    ...
    if(isMetaPool[poolToUse]) {
      ...
      uint256 minimumToReceive = (amountExpectedToReceive*slippage)/1000;
  ...
  function changeOneRingFee(uint256 newFee) external onlyOwner {
    if(newFee > 1000 || newFee < 998) {
      revert OneRingCurveRouter_Incorrect_Fee();
    }
    oneRingFee = newFee;
