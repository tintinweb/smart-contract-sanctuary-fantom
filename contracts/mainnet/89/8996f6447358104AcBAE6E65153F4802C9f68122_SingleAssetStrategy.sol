...
interface IMasterChef {
    ...
    function pendingREAPER(uint256 _pid, address _user) external view returns (uint256);
...
contract SingleAssetStrategy is Ownable, Pausable {
    ...
    function callReward() public view returns (uint256) {
        uint256 outputBal = IMasterChef(masterchef).pendingREAPER(poolId, address(this));
        uint256 reward;
        if (output != wrapped) {
