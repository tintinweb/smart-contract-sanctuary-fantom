...
contract OutputReceiverProxy is Ownable, IOutputReceiverV3, ERC165 {
    ...
    function getValue(uint fnftId) external view override returns (uint) {
        (uint principalId,,,bytes32 poolId) = IResonate(resonate).activated(fnftId);
        if(fnftId == principalId) {
            (,,,,, uint256 packetSize) = IResonate(resonate).pools(poolId);
            return packetSize;
        } else {
            (,,,,,, uint residual) = IResonate(resonate).getPositionInfo(fnftId);
            return residual;
        }

        // TODO: This function should return either the amount of principal attached to the FNFT or the amount of interest pending in residuals (+ interest?)
    }

    function getAsset(uint fnftId) external view override returns (address asset) {
        (,,,, address vault,,) = IResonate(resonate).getPositionInfo(fnftId);
    ...
    function getOutputDisplayValues(uint fnftId) external view override returns (bytes memory output) {
        ...
        (address paymentAsset, address vault, uint80 rate,,, uint packetSize) = instance.pools(poolId);
        address vaultAdapter = instance.vaultAdapters(vault);
        ...
        output = abi.encode(accruedInterest, poolId, _formatAsset(farmedAsset), _formatAsset(paymentAsset), vault, vaultAdapter, wallet);
        output = bytes.concat(output, abi.encode(packetSize, rate, principalId, principalId+1, IERC20Detailed(farmedAsset).decimals()));
    }

    function updateTokenVault() external onlyOwner {
        TOKEN_VAULT = IAddressRegistry(ADDRESS_REGISTRY).getTokenVault();
    }
