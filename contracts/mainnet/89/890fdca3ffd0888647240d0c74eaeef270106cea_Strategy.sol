...
// File interfaces/ISolidlyRouter.sol
...
// File interfaces/ISolidlyV1Pair.sol
...
// File interfaces/ISolidexLP.sol
...
contract Strategy is BaseStrategy {
    ...
    address public SOLID = address(0x888EF71766ca594DED1F0FA3AE64eD2941740A20); // SOLID
    address public SEX = address(0xD31Fcd1f7Ba190dBc75354046F6024A9b86014d7); // SEX

    address public lpPairToken1;
    IV1Pair public solidlyLpToken; //  VariableV1 AMM - Ftm/Tomb
    ISolidexLP public solidexLP; // Solidex vAMM-Ftm/Tomb Deposit
    ...
    constructor(
        ...
        address _masterChef,
        address _solidexLP,
        address _solidlyLpToken,
        bool _isStablePair,
        uint256 _maxSlippageIn,
        uint256 _maxSlippageOut
    ) public BaseStrategy(_vault) {
        solidlyLpToken = IV1Pair(_solidlyLpToken);
        require(solidlyLpToken.token0() == address(want));
        lpPairToken1 = solidlyLpToken.token1();
        solidexLP = ISolidexLP(_solidexLP);

        maxSlippageIn = _maxSlippageIn;
        maxSlippageOut = _maxSlippageOut;

        dust = uint256(10).mul(10**uint256(ERC20(address(want)).decimals()));
    ...
    function name() external view override returns (string memory) {
        return string(abi.encodePacked('Solidex Tomb-wFTM Strategy'));
    }
    ...
    function balanceOfPairToken1() public view returns (uint256) {
        return IERC20(lpPairToken1).balanceOf(address(this));
    }

    function balanceOfLpTokens() public view returns (uint256) {
        return solidlyLpToken.balanceOf(address(this));
    }

    function balanceOfLPInMasterChef() public view returns (uint256 _amount) {
        _amount = solidexLP.balanceOf(address(this));
    }

    function balanceOfReward() public view returns (uint256 _solidRewards, uint256 _sexRewards) {
        _solidRewards = IERC20(SOLID).balanceOf(address(this));
        _sexRewards = IERC20(SEX).balanceOf(address(this));
    }

    function pendingRewards() public view returns (uint256 _solid, uint256 _sex) {
        address[] memory pools = new address[](1);
        pools[0] = address(solidlyLpToken);
        Amounts[] memory amounts = masterChef.pendingRewards(address(this), pools);
        _solid = amounts[0].solid;
        _sex = amounts[0].sex;
    }
    ...
    {
        (_amountWant, _amountPairToken1) = ISolidlyRouter(router).quoteRemoveLiquidity(
            address(want),
            lpPairToken1,
            isStablePair,
            liquidity
    ...
    function balanceOfPooled() public view returns (uint256 _amount) {
        ...
        uint256 totalPairTokens = amountPairToken.add(balanceOfPairToken1());
        uint256 pairTokenAsWant = pairToken1ToWant(totalPairTokens);
        return pairTokenAsWant.add(amountWant);
    }

    function estimateHarvest() external view returns (uint256 _profit) {
        (uint256 _solid, uint256 _sex) = pendingRewards();

        if (_solid > 0) {
            (uint256 fromSOLID, ) =
                ISolidlyRouter(router).getAmountOut(_solid, SOLID, address(want));
            _profit += fromSOLID;
        }

        if (_sex > 0) {
            (uint256 fromSEX, ) = ISolidlyRouter(router).getAmountOut(_sex, SEX, address(want));
            _profit += fromSEX;
        }
    }
    ...
    function wantToLpTokens(uint256 _amountWant) internal view returns (uint256) {
    ...
    function pairToken1ToWant(uint256 _amount) internal view returns (uint256) {
        ...
        return solidlyLpToken.getAmountOut(_amount, lpPairToken1);
    }
    ...
    function getSwapAmount(uint256 investmentA) internal view returns (uint256 _swapAmount) {
        (uint256 reserveA, uint256 reserveB, ) = solidlyLpToken.getReserves();
        uint256 halfInvestment = investmentA.div(2);
        uint256 nominator = solidlyLpToken.getAmountOut(halfInvestment, address(want));
    ...
    function _swap(uint256 _fullInvestment) internal returns (uint256[] memory _swapedAmounts) {
        ...
        (, bool isStable) =
            ISolidlyRouter(router).getAmountOut(swapAmountIn, address(want), lpPairToken1);

        route[] memory lpPairPath = new route[](1);
        lpPairPath[0] = route(address(want), lpPairToken1, isStable);

        _swapedAmounts = ISolidlyRouter(router).swapExactTokensForTokens(
            swapAmountIn,
            swapAmountIn.mul(swapSecurityFactor).div(basisOne),
            lpPairPath,
            address(this),
            block.timestamp
        );
    ...
    function sellAllLpPairToken() internal {
        uint256 lpToken1Balance = IERC20(lpPairToken1).balanceOf(address(this));

        (, bool isStable) =
            ISolidlyRouter(router).getAmountOut(lpToken1Balance, lpPairToken1, address(want));

        route[] memory lp1TokenToLp0Route = new route[](1);
        lp1TokenToLp0Route[0] = route(lpPairToken1, address(want), isStable);

        ISolidlyRouter(router).swapExactTokensForTokens(
            lpToken1Balance,
            lpToken1Balance.mul(swapSecurityFactor).div(basisOne),
            lp1TokenToLp0Route,
            address(this),
            block.timestamp
        );
    ...
    function sellAllRewards() internal {
        ...
        sexToWantRoutes[0] = route(SEX, address(want), false);

        route[] memory solidToWantRoutes = new route[](1);
        solidToWantRoutes[0] = route(SOLID, address(want), false);
    ...
    function removeLiquidity(uint256 _liquidityToExit) internal {
        ...
        if (toExit > 0) {
            ISolidlyRouter(router).removeLiquidity(
                address(want),
                lpPairToken1,
                isStablePair,
                toExit,
    ...
    function removeAllLiquidity() internal {
        removeLiquidity(balanceOfLpTokens());
    }
    ...
    function depositLpIntoMasterChef() internal {
        if (balanceOfLpTokens() > 0) {
            masterChef.deposit(address(solidlyLpToken), balanceOfLpTokens());
        }
    }
    ...
    function withdrawFromMasterChef() internal {
        ...
        if (masterChefBal > 0) {
            masterChef.withdraw(address(solidlyLpToken), masterChefBal);
        }
    }
    ...
    function _withdrawFromMasterChefAndTransfer(address _to) internal {
        if (abandonRewards) {
            ...
            withdrawFromMasterChef();
            (uint256 _solidRewards, uint256 _sexRewards) = balanceOfReward();
            IERC20(SOLID).transfer(_to, _solidRewards);
            IERC20(SEX).transfer(_to, _sexRewards);
        }
        uint256 lpTokensBalance = balanceOfLpTokens();
        if (lpTokensBalance > 0) {
            solidlyLpToken.transfer(_to, lpTokensBalance);
        }
    }
    ...
    function giveAllowances() internal {
        IERC20(address(solidlyLpToken)).safeApprove(address(masterChef), max);
        IERC20(address(solidlyLpToken)).safeApprove(router, max);

        IERC20(SOLID).safeApprove(router, max);
        IERC20(SEX).safeApprove(router, max);

        IERC20(want).safeApprove(router, 0);
        IERC20(want).safeApprove(router, max);

        IERC20(lpPairToken1).safeApprove(router, 0);
        IERC20(lpPairToken1).safeApprove(router, max);
    }
    ...
    function setIStablePair(bool _isStablePair) external onlyVaultManagers {
        isStablePair = _isStablePair;
    }
