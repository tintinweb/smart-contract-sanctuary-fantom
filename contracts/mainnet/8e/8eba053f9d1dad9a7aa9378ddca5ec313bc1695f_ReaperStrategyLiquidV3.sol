...
import "./interfaces/IMCV2.sol";
import "./interfaces/ISolidlyPair.sol";
import "./interfaces/ISolidlyRouter.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol";

contract ReaperStrategyLiquidV3 is ReaperBaseStrategyv3 {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IMCV2 public constant MASTER_CHEF = IMCV2(0x6e2ad6527901c9664f016466b8DA1357a004db0f);
     ...
     * {LQDR} - Reward token for depositing into MASTER_CHEF.
    ...
    function _deposit() internal override {
        ...
        if (wantBalance != 0) {
            IERC20Upgradeable(address(want)).safeIncreaseAllowance(address(MASTER_CHEF), wantBalance);
            MASTER_CHEF.deposit(poolId, wantBalance, address(this));
        }
    }
    ...
    function _withdraw(uint256 _amount) internal override {
        ...
        if (wantBal < _amount) {
            MASTER_CHEF.withdraw(poolId, _amount - wantBal, address(this));
        }
     ...
     *      1. Claims {LQDR} from the {MASTER_CHEF}.
     *      2. Claims fees by swapping {LQDR} to {USDC} using {SPIRIT_ROUTER_V2}.
     *      3. Makes new LP using the remaining {LQDR}.
     *      4. Re-deposits.
     */
    function _harvestCore() internal override returns (uint256 callerFee) {
        MASTER_CHEF.harvest(poolId, address(this));
    ...
    function _addLiquidity() internal {
        if (lqdrToLpTokenPath.length >= 2) {
            // {LQDR} is not one of the LP tokens
            ...
            _swap(baseToOtherToken, IERC20Upgradeable(baseToken).balanceOf(address(this)) / 2);
        } else {
            // {LQDR} itself is one of the LP tokens
            address otherToken = address(lpToken0) == address(LQDR) ? address(lpToken1) : address(lpToken0);
            address[] memory lqdrToOtherToken = new address[](2);
            lqdrToOtherToken[0] = address(LQDR);
            lqdrToOtherToken[1] = otherToken;
            _swap(lqdrToOtherToken, LQDR.balanceOf(address(this)) / 2);
        }
    ...
    function balanceOf() public view override returns (uint256) {
        (uint256 amount, ) = MASTER_CHEF.userInfo(poolId, address(this));
    ...
    function _reclaimWant() internal override {
        MASTER_CHEF.emergencyWithdraw(poolId, address(this));
    }
}
