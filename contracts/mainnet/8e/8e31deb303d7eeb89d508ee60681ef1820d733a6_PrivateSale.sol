...
contract PrivateSale is Ownable {
  ...
  function purchase() public payable inProgress {
    ...
    // Kiểm tra whitelist chỉ cho tier 1 và tier 2
    if (tier == 1 || tier == 2) {
        require(whitelisted[_msgSender()], "User is not whitelisted");
    }
  ...
  function addAllocation(address[] memory users, uint8 tier) external onlyOwner {
    ...
    for (uint256 i = 0; i < users.length; i++) {
        ...
        if ((tier == 1 || tier == 2) || (tier == 3 && isTier3Enabled)) {
...
function toggleTier3(bool enabled) external onlyOwner {
    isTier3Enabled = enabled;
}
