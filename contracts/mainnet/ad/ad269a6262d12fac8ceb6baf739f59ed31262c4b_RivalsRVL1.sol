...
import "./helpers/IRivalsTokenURIProcessor.sol";
import "../cryptoblock/CryptoBlocksERC721Upgradeable.sol";
import "./lib/RivalsConstants.sol";

/** @title RivalsRVL1: Aliens vs Astronauts
 * @dev Part of the Rivals project
 */

contract RivalsRVL1 is Initializable, UUPSUpgradeable, CryptoBlocksERC721Upgradeable {
    ...
    function initialize() initializer public {
        ...
        __ERC721_init_unchained("Rivals: Aliens vs Astronauts", "RVL1");
        ...
        __RivalsRVL1_init_unchained();
    }

    function __RivalsRVL1_init_unchained() internal initializer {
        _uintMapping[CryptoBlocksConstants.UINT_MAPPING_ROYALTY_AMOUNT] = 75;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return IRivalsTokenURIProcessor(_addressMapping[CryptoBlocksConstants.ADDR_MAPPING_TOKEN_URI_PROCESSOR]).getRivalsTokenURI(tokenId, _baseURI());
    }

    function resetWinsLosses(uint256 min_, uint256 max_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        for(uint256 i = min_; i < max_; i++) {
            delete _attributes[i][RivalsConstants.UINT_WINS];
            delete _attributes[i][RivalsConstants.UINT_LOSSES];
        }
    }
}
...
interface IRivalsTokenURIProcessor {
    function getRivalsTokenURI(uint256, string memory) external view returns(string memory);
}
