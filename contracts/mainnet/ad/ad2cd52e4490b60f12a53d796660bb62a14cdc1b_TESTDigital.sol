...
contract TESTDigital is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 34500 ether;
    // Initial distribution for the day 2-5 DIGITAL-WFTM LP -> DIGITAL pool
    uint256 public constant INITIAL_DIGITAL_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the DIGITAL ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("TESTDIGITAL", "TESTDigital Token") {
        // Mints 1 DIGITAL to contract creator for initial pool setup
    ...
    function _getDigitalPrice() internal view returns (uint256 _digitalPrice) {
        try IOracle(digitalOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Digital: failed to fetch DIGITAL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _digitalPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_digitalPrice >= taxTiersTwaps[tierId]) {
    ...
    function setDigitalOracle(address _digitalOracle) public onlyOperatorOrTaxOffice {
        require(_digitalOracle != address(0), "oracle address cannot be 0 address");
    ...
    function setBlacklistAddress(address _address, bool value) public onlyOperator returns (bool) {
        isBlacklisted[_address] = value;
        return true;
    }

    /**
     * @notice Operator mints DIGITAL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DIGITAL to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDigitalPrice = _getDigitalPrice();
            currentTaxRate = _updateTaxRate(currentDigitalPrice);
            if (currentDigitalPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        //address _digitalPool,
        //address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        //require(_digitalPool != address(0), "!_digitalPool");
        //require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        //_mint(_digitalPool, INITIAL_DIGITAL_POOL_DISTRIBUTION);
