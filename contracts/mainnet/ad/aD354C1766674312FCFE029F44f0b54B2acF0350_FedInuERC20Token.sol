...
contract FedInuERC20Token is ERC20Permit, Ownable {
    ...
    uint48 public buyTotalFees;
    uint48 public buyDevFee = 2;
    uint48 public buyLiquidityFee = 3;
    uint48 public buyBurnFee;
    uint48 public buyTreasuryFee = 5;

    uint48 public sellTotalFees;
    uint48 public sellDevFee = 4;
    uint48 public sellLiquidityFee = 6;
    uint48 public sellBurnFee;
    uint48 public sellTreasuryFee = 10;
    ...
    constructor(address _devWallet, address _treasuryWallet, address _uniswapRouter) ERC20("FedInu", "COKEN") ERC20Permit("FedInu") {
        uniswapV2Router = IUniswapV2Router02(_uniswapRouter);
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        _setAutomatedMarketMakerPair(address(uniswapV2Pair), true);

        initialSupply = 2000000000000 * 1e9;
        maxTransactionAmount = (initialSupply * 5) / 1000; // 0.5% maxTransactionAmountTxn
        maxWallet = (initialSupply * 15) / 1000; // 1.5% maxWallet
        _mint(owner(), initialSupply);

        buyTotalFees = buyDevFee + buyLiquidityFee + buyBurnFee + buyTreasuryFee;
        sellTotalFees = sellDevFee + sellLiquidityFee + sellBurnFee + sellTreasuryFee;
        ...
        excludeFromLimits(treasuryWallet, true);
        excludeFromLimits(address(this), true);
        excludeFromLimits(deadAddress, true);
        excludeFromLimits(0x568E1e52E78b1799D92ed663EdE1f8Dd4b474cA7, true); // team wallet
        excludeFromLimits(0x54E9C1Dddd92C656e7304844fdF07ED229cf60a4, true); // 8Ball wallet
    }
    ...
    function updateBuyFees(
        uint48 _devFee,
        uint48 _liquidityFee,
        uint48 _burnFee,
        uint48 _treasuryFee
    ) external onlyOwner {
        require(_devFee >= 2, "Dev fee cannot be lower than 2%");
        buyDevFee = _devFee;
    ...
    function swapBack() private {
        ...
        uint256 ethForLiquidity = ethBalance - ethForDev - ethForTreasury;

        tokensForLiquidity = 0;
        tokensForDev = 0;
        tokensForTreasury = 0;

        (success, ) = treasuryWallet.call{value: ethForTreasury}("");
