...
contract StrategyTarot is SimpleStratManager {
    ...
    address[] public bTarots; // addresses that the strategy will ever use. It for tracking of all underlying assets.
    address[] public bestbTarots; // addresses that the strategy is currently using to generate yields.
    ...
    ) SimpleStratManager(_vault, _strategist, _lunariaFeeRecipient) {}

    function init(address _vault, address[] memory _bTarots) public onlyManager {
        require(initialized == false, "init: already initialised");
        vault = _vault;
        want = IVault(_vault).token();
        setbTarots(_bTarots);
        setBestbTarots(_bTarots);
        _giveAllowances();
        initialized = true;
    }
    ...
    */
    ...
    function chargeFee() internal {
        ...
        IERC20(want).safeTransfer(strategist, toLunaria);
    ...
    /**
     * @dev In case something messed up, withdraw all funds to vault if no errors. else manually take out x amount.
     */
