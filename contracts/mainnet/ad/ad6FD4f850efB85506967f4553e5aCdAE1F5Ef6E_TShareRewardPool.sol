...
contract TShareRewardPool {
    ...
    function updateRewardPerSecond(uint256 _newRewardPerSecond) public onlyOperator {
        require(_newRewardPerSecond < 0.0025 ether, "To fast");
        massUpdatePools();
        tSharePerSecond = _newRewardPerSecond;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (pool.isTaxed) {
                depositFee = _amount.mul(feeParam).div(10000);
                pool.token.safeTransferFrom(_sender, daoFUnd, depositFee);
            }
