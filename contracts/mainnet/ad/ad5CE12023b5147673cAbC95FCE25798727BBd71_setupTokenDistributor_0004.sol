...
import {ITokenDitributor} from "@solarprotocol/solar-diamond/contracts/modules/token-distributor/ITokenDitributor.sol";
import {LibTokenDistributor} from "@solarprotocol/solar-diamond/contracts/modules/token-distributor/LibTokenDistributor.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

// solhint-disable-next-line contract-name-camelcase
contract setupTokenDistributor_0004 {
    function migrate(
        address treasury,
        address growthFund,
        ...
        require(growthFund != address(0), "growthFund is zero address");
        // solhint-disable-next-line reason-string
        require(
            stakingRewardReceiver != address(0),
            "stakingRewardReceiver is zero address"
        );
        require(stableCoin != address(0), "stableCoin is zero address");

        setupRegularTax(
            treasury,
            growthFund,
            stakingRewardReceiver,
            stableCoin
        );

        setupLaunchTax(treasury, growthFund, stakingRewardReceiver, stableCoin);

        IERC20(stableCoin).approve(address(this), type(uint256).max);
    }

    /**
     * @dev Distribution for the regular buy/sell tax.
     * Breakdown:
     *  41.20% distributed to the holders as $DAI
     *  17.64% zapIn liquidity and send LP token to treasury
     *  29.40% Sent to treasury as $DAI
     *  11.76% Sent to the Growth Fund as $DAI
     *
     * To achieve less token sell transactions, we will swap to $DAI before the distribution.
     * When adding LP, it will swap half of the distribution back to VOID and add the liquidity.
     */
    function setupRegularTax(
        address treasury,
        address growthFund,
        address stakingRewardReceiver,
        address stableCoin
        ...
        // 41.2% distributed to the holders as $DAI
        distributions[0] = ITokenDitributor.Distribution({
            destination: stakingRewardReceiver,
            proportion: 4120,
            swapTo: address(0),
            addLiquidity: false
        });
        ...
        distributions[3] = ITokenDitributor.Distribution({
            destination: growthFund,
            proportion: 1176,
            swapTo: address(0),
            addLiquidity: false
        });

        ITokenDitributor.Strategy memory strategy = ITokenDitributor.Strategy({
            token: address(this),
            swapTo: stableCoin,
            distributions: distributions
        });

        LibTokenDistributor.add(keccak256("TRADE_TAX"), strategy);
    }

    /**
     * @dev Distribution for the additional tax on launch.
     * Breakdown:
     *  33,33% distributed to the holders as $DAI
     *  33,33% Sent to treasury as $DAI
     *  33,34% Sent to the Growth Fund as $DAI
     */
    function setupLaunchTax(
        address treasury,
        address growthFund,
        address stakingRewardReceiver,
        address stableCoin
        ...
        ITokenDitributor.Distribution[]
            memory distributions = new ITokenDitributor.Distribution[](3);
        ...
        LibTokenDistributor.add(keccak256("LAUNCH_TAX"), strategy);
    }
}