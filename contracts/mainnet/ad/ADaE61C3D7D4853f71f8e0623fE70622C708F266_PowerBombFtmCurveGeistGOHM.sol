...
import "./PowerBombFtmCurveGeist.sol";

contract PowerBombFtmCurveGeistGOHM is PowerBombFtmCurveGeist {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IRouter constant spiritRouter = IRouter(0x16327E3FbDaCA3bcF7E38F5Af2599D2DDc33aE52);
    ...
    function initialize(
        ...
        WFTM.safeApprove(address(spiritRouter), type(uint).max);
    ...
    function _harvest(bool isDeposit) internal override {
        ...
        if (WFTMAmt > minSwapAmt) {
            ...
            // Swap WFTM to reward token with Spiritswap
            address[] memory path = getPath(address(WFTM), address(rewardToken));
            uint rewardTokenAmt = spiritRouter.swapExactTokensForTokens(WFTMAmt, 0, path, address(this), block.timestamp)[1];
