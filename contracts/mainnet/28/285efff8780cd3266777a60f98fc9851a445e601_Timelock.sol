...
interface IVaultUtils {
    ...
    function getBuyUsdaFeeBasisPoints(address _token, uint256 _usdaAmount)
        external
        view
        returns (uint256);

    function getSellUsdaFeeBasisPoints(address _token, uint256 _usdaAmount)
        external
        view
        returns (uint256);
...
interface IVault {
    ...
    function tokenToUsdain(address _token, uint256 _tokenAmount) external view returns (uint256);
...
contract Timelock is ITimelock {
    ...
    event SignalRedeemUsda(address vault, address token, uint256 amount);
    ...
    function setTokenConfig(
        ...
        IVault(_vault).setTokenConfig(_token, tokenDecimals, _tokenWeight, _minProfitBps, _maxUsdaAmount, isStable, isShortable);

        IVault(_vault).setBufferAmount(_token, _bufferAmount);

        IVault(_vault).setUsdaAmount(_token, _usdaAmount);
    }
    ...
    function signalRedeemUsda(
        address _vault,
        address _token,
        uint256 _amount
    ) external onlyAdmin {
        bytes32 action = keccak256(abi.encodePacked("redeemUsda", _vault, _token, _amount));
        _setPendingAction(action);
        emit SignalRedeemUsda(_vault, _token, _amount);
    }
    ...
    function redeemUsda(
        ...
        address usda = IVault(_vault).usda();
        IVault(_vault).setManager(address(this), true);
        IUSDA(usda).addVault(address(this));

        IUSDA(usda).mint(address(this), _amount);
        IERC20(usda).transfer(address(_vault), _amount);

        IVault(_vault).sellUSDA(_token, mintReceiver);

        IVault(_vault).setManager(address(this), false);
        IUSDA(usda).removeVault(address(this));
    }
    ...
    function signalVaultSetTokenConfig(
        ...
        bytes32 action = keccak256(abi.encodePacked("vaultSetTokenConfig", _vault, _token, _tokenDecimals, _tokenWeight, _minProfitBps, _maxUsdaAmount, _isStable, _isShortable));

        _setPendingAction(action);

        emit SignalVaultSetTokenConfig(_vault, _token, _tokenDecimals, _tokenWeight, _minProfitBps, _maxUsdaAmount, _isStable, _isShortable);
    }
    ...
    function vaultSetTokenConfig(
        ...
        IVault(_vault).setTokenConfig(_token, _tokenDecimals, _tokenWeight, _minProfitBps, _maxUsdaAmount, _isStable, _isShortable);
    }
