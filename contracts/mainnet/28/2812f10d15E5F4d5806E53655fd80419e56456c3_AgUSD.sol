...
contract AgUSD is ERC20, ERC20Burnable, Ownable, AGUSDErrors {
    ...
    constructor() ERC20("AggregatedUSD", "AgUSD") {
      IERC20(dai).approve(yvdai, (2**256)-1);
      IERC20(fusdt).approve(yvfusdt, (2**256)-1);
      IERC20(usdc).approve(yvusdc, (2**256)-1);
      IERC20(mim).approve(yvmim, (2**256)-1);
      IERC20(frax).approve(yvfrax, (2**256)-1);
    }
    ...
    function mintFromDAI(uint256 amount) public payable {
        ...
        YvVault(yvdai).deposit(amount/2, multisig);
    ...
    function mintFromUSDC(uint256 amount) public payable {
        ...
        YvVault(yvusdc).deposit(amount/2, multisig);
        _mint(msg.sender, amount);
    }
    ...
    function mintFromFUSDT(uint256 amount) public payable {
        ...
        YvVault(yvfusdt).deposit(amount/2, multisig);
        _mint(msg.sender, amount);
    }
    ...
    function mintFromMIM(uint256 amount) public payable {
        ...
        YvVault(yvmim).deposit(amount/2, multisig);
        _mint(msg.sender, amount);
    }
    ...
    function mintFromFRAX(uint256 amount) public payable {
        ...
        YvVault(yvfrax).deposit(amount/2, multisig);
        _mint(msg.sender, amount);
    }
