...
/**
    @dev This is the public presale contract, derived from Presale.
        The authorization mechanism is a hash check against an off-chain api.

        Upon success, the timestamp of the transaction is mapped to caller's hash. If the current blocktime is within COOLDOWN
        of this time, the transaction is reverted.
 */
...
/**
 * @dev Public Presale
 * Common interface with private presale
 */
contract PublicPresale is Presale {
    // timelock on hash and address
    mapping(bytes32 => uint256) private lastBuy;
    mapping(address => uint256) private lastAddressBuy;

    // Time between transactions from the same wallet
    uint64 private constant COOLDOWN = 60 seconds;
    ...
    function verify(
        ...
        require(proof.length > 2, "Invalid Proof");

        // Validate Hashes
        bytes32 h1 = _efficientHash(proof[0], rootHash);
        if (h1 != proof[1]) {
            return false;
        }
        bytes32 h2 = _efficientHash(keccak256(abi.encodePacked(_address)), rootHash);
        if (h2 != proof[2]) {
            return false;
        }

        // Check to ensure there hasn't been a very recent transaction from same hash
        uint256 last = Math.max(lastBuy[proof[0]], lastAddressBuy[_address]);
    ...
    function checkAccess(uint8 whitelistIndex, bytes32[] memory proof) internal view override returns (uint256) {
        require(verify(msg.sender, whitelistIndex, proof), "Validation Failed");

        return presaleMaximum;
    }

    /**
     * @dev accessor to return last buy time
     */
    function getLastBuy(address _address, bytes32[] calldata proof) external view returns (uint256) {
        return Math.max(lastBuy[proof[0]], lastAddressBuy[_address]);
    }
    ...
    /**
     * @dev The buy function calls back here on success so we can mark the last buy time.
     */
    function successHook(
        ...
        lastBuy[proof[0]] = block.timestamp;
        lastAddressBuy[_address] = block.timestamp;
    }

    /**
     * @dev Concatenating these values is more efficient in assembly.
     */
