 ...
 */
contract Context {
    // Empty internal constructor, to prevent people from ivakenly deploying
...
contract Iv is ERC20, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 IV-WFTM LP -> IV pool
    uint256 public constant INITIAL_IV_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the IV ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("IV", "IV") {
        // Mints 1 IV to contract creator for initial pool setup
    ...
    function _getIvPrice() internal view returns (uint256 _ivPrice) {
        try IOracle(ivOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Iv: failed to fetch IV price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ivPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ivPrice >= taxTiersTwaps[tierId]) {
    ...
    function setIvOracle(address _ivOracle) public onlyOperatorOrTaxOffice {
        require(_ivOracle != address(0), "oracle address cannot be 0 address");
        ivOracle = _ivOracle;
    }
    ...
    /**
     * @notice Operator mints IV to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of IV to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentIvPrice = _getIvPrice();
            currentTaxRate = _updateTaxRate(currentIvPrice);
            if (currentIvPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_ivPool != address(0), "!_ivPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_ivPool, INITIAL_IV_POOL_DISTRIBUTION);
