...
contract BasketLedgerv4 is Ownable {
  ...
  function depositv4(uint256 _basketId, address _account, address _vault, uint256 _amount) public {
    xlpSupply[_vault][_account] = xlpSupply[_vault][_account] + _amount;
    emit Deposit(_basketId, _vault, _account, _amount);
  }

  function withdrawv4(uint256 _basketId, address _account, address _vault, uint256 _amount) public onlyManager returns(uint256) {
    if (_amount > xlpSupply[_vault][_account]) {
      _amount = xlpSupply[_vault][_account];
    }
    if (_amount > 0) {
