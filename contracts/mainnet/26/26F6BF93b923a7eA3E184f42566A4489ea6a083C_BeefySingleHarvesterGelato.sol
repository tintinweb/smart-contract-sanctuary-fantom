...
contract BeefySingleHarvesterGelato is BeefySingleHarvesterBase, IResolver {
    ...
    function checker(address vault_) external override returns (bool canExec_, bytes memory execPayload_) {
        (bool willHarvestVault, uint256 estimatedTxCost, uint256 estimatedCallRewards, uint256 estimatedProfit, bool isDailyHarvest) = checkUpkeep(
            tx.gasprice,
            IBeefyVault(vault_)
        );
    ...
    function performUpkeep(
        ...
        _performUpkeep(IBeefyVault(vault_), checkerGasprice_, estimatedTxCost_, estimatedCallRewards_, estimatedProfit_, isDailyHarvest_);
    }
}
...
abstract contract BeefySingleHarvesterBase is ManageableUpgradeable, PausableUpgradeable {
    ...
    {
        require(gasPrice_ <= maxGasPrice, "tx.gasprice too high.");
    ...
    {
        (bool canHarvestVault, uint256 callReward, uint256 gasOverhead, uint256 lastHarvest) = _canHarvestVault(vault_);
        console.log("canHarvestVault", canHarvestVault);

        (bool shouldHarvestVault, uint256 estimatedTxCost, bool isDailyHarvest) = _shouldHarvestVault(gasPrice_, callReward, gasOverhead, lastHarvest);
        ...
        )
    {
        ...
        // Get lastHarvest before the harvest simulation, as the harvest simulation will overwrite lastHarvest.
    ...
    function _performUpkeep(
        ...
        require(tx.gasprice <= maxGasPrice, "tx.gasprice too high.");
        /* solhint-disable not-rely-on-time */
        require(!(vault_.strategy().lastHarvest() > block.timestamp - 1 days) == isDailyHarvest_, "Racing keepers detected.");
        /* solhint-enable not-rely-on-time */
        ...
        if (nativeBalance > depositFundsThreshold) {
            console.log("Depositing", nativeBalance);
            TASK_TREASURY.depositFunds(RECEIVER, address(NATIVE), nativeBalance);
        }
