...
contract XpandrUnityVault is ERC4626, AccessControl, Pauser {
    ...
    address internal constant bay = address(0xE5a4c0af6F5f7Ab5d6C1D38254bCf4Cc26d688ed);
    ...
    constructor(
       ...
       ERC4626(
            _asset,
            string(abi.encodePacked("XPANDR BAY-FTM EQUALIZER")),
            string(abi.encodePacked("XpE-BAY-FTM"))
        )
        {
        gauge = _gauge;
        router = _router;
    ...
    function _addLiquidity() internal {
        ...
        IEqualizerRouter(router).swapExactTokensForTokensSimple(wftmHalf, minAmt, wftm, bay, false, address(this), lastHarvest);

        uint t1Bal = SafeTransferLib.balanceOf(wftm, address(this));
        uint t2Bal = SafeTransferLib.balanceOf(bay, address(this));
        (uint t1Min, uint t2Min,) = IEqualizerRouter(router).quoteAddLiquidity(wftm, bay, false, t1Bal, t2Bal);
        IEqualizerRouter(router).addLiquidity(wftm, bay, false, t1Bal, t2Bal, t1Min * slippage / slippageDiv, t2Min * slippage / slippageDiv, address(this), lastHarvest);
    }
    ...
    function customTx(address _token, uint _amount, IEqualizerRouter.Routes[] memory _path) external onlyAdmin {
        if(_token == equal || _token == wftm || _token == bay){revert XpandrErrors.InvalidTokenOrPath();}
        uint bal;
    ...
    function _subAllowance() internal {
        ...
        SafeTransferLib.safeApprove(bay, router, 0);
    }
    ...
    function _addAllowance() internal {
        ...
        SafeTransferLib.safeApprove(bay, router, type(uint).max);
    }
