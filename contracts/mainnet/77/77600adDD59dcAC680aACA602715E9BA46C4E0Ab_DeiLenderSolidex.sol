...
contract DeiLenderSolidex is BoringOwnable {
    ...
    uint256 public BORROW_OPENING_FEE_PRECISION = 1e2;

    uint256 public LIQUIDATION_RATIO;
    uint256 public LIQUIDATION_RATIO_PRECISION = 1e2;
    ...
    mapping(address => Holder) public userHolder;
    ...
    /// convert debt to borrowed amount (debt is borrowed amount + interest)
    ...
    function getLiquidationPrice(address user) external view returns (uint256) {
        ...
        uint256 liquidationPrice = (getDebt(user) *
            LIQUIDATION_RATIO_PRECISION *
            1e18) / (userCollateralAmount * LIQUIDATION_RATIO);
        ...
        returns (uint256)
    {
        ...
        uint256 withdrawableCollateralAmount = userCollateralAmount -
            (getDebt(user) * LIQUIDATION_RATIO_PRECISION * 1e18) /
            (oracle.getPrice() * LIQUIDATION_RATIO);
    ...
    function isSolvent(address user) internal view returns (bool) {
        ...
        return
            userCollateralAmount.mul(oracle.getPrice()).mul(LIQUIDATION_RATIO) /
                (LIQUIDATION_RATIO_PRECISION * 1e18) >
            getDebt(user);
    }
    ...
    function addCollateral(address to, uint256 amount) public {
        ...
        collateral.safeTransferFrom(msg.sender, address(holder), amount);
        userHolder[to] = holder;
    ...
    function removeCollateral(address to, uint256 amount) public {
        ...
        userHolder[msg.sender].withdrawERC20(address(collateral), to, amount);
    ...
    function borrow(
        ...
        accrue();
        uint256 fee = amount.mul(BORROW_OPENING_FEE) /
    ...
    function liquidate(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (!isSolvent(user)) {
                ...
                userHolder[msg.sender].withdrawERC20(
                    address(collateral),
                    receiver,
                    userCollateral[user]
                );
    ...
    function claim(address[] calldata pools) public {
        userHolder[msg.sender].claim(pools);
    }

    function claimAndWithdraw(address[] calldata pools, address to) public {
        Holder holder = userHolder[msg.sender];
        holder.claim(pools);
        holder.withdrawERC20(
            address(solid),
            to,
            solid.balanceOf(address(holder))
        );
        holder.withdrawERC20(
            address(solidex),
            to,
            solidex.balanceOf(address(holder))
        );
    }
    ...
    function emergencyHolderWithdraw(
        ...
        holder.withdrawERC20(address(token), to, amount);
    }
