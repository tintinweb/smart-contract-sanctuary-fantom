...
contract FeeManagerFacet is Modifiers, IFees {
    using FeeManagerLib for FeeManagerContract;

    /// @notice get the fee permillion amount
    function fee(string memory feeLabel) external view override returns (Fee memory) {
        FeeManagerContract storage ds = s().fees[address(this)];
        return FeeManagerLib.fee(ds, feeLabel);
    }

    /// @notice calculate a fee amount given a fee name and an amount
    function calculateFee(string memory feeLabel, uint256 amount) external view override returns (uint256) {
        FeeManagerContract storage ds = s().fees[address(this)];
        return FeeManagerLib.calculateFee(ds, feeLabel, amount);
    }
