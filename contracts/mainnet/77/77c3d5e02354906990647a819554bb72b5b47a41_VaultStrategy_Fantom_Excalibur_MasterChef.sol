...
// File: contracts/VaultStrategy/Biswap/SmartChef/ISmartChef_BSC_Biswap.sol

interface IMasterChef_Fantom_Excalibur
{
    function deposit(uint256 pid, uint256 amount) external;
    function depositOnLockSlot(uint256 pid, uint256 amount, uint256 lockDurationSeconds, bool fromRegularDeposit) external;    
    ...
    function pendingRewardsOnLockSlot(uint256 pid, address userAddress, uint256 slotId) external view returns (uint256 pending, uint256 bonusRewards, bool canHarvestBonusRewards);

    function excToken() external view returns (address);
    function grailToken() external view returns (address);

    function getPoolInfo(uint256 pid) external view returns (address lpToken, uint256 allocPoint, uint256 lastRewardTime, uint256 accRewardsPerShare, uint256 depositFeeBP, bool isGrailRewards, uint256 lpSupply, uint256 lpSupplyWithMultiplier);
    function getUserInfo(uint256 pid, address userAddress) external view returns (uint256 amount, uint256 rewardDebt);
    function getUserLockSlotInfo(uint256 pid, address userAddress, uint256 slotId) external view returns (uint256 amount, uint256 rewardDebt, uint256 lockDurationSeconds, uint256 depositTime, uint256 multiplier, uint256 amountWithMultiplier, uint256 bonusRewards);
...
abstract contract VaultStrategyV1 is IVaultStrategy, VaultAccessManager
{
    ...
    //swap path
    IToken public alternativeSwapToken;
    ...
    function setAlternativeSwapToken(IToken _alternativeSwapToken) external
    {
        //check
        requireManager();

        //set
        alternativeSwapToken = _alternativeSwapToken;

        //events
        emit ConfigChanged("AlternativeSwapToken", uint256(uint160(address(alternativeSwapToken))));
    }    
	...
	{
	    address[] memory path;
        IToken swapPathToken = (address(alternativeSwapToken) == address(0) ? wrappedCoin : alternativeSwapToken);
		if (_from == swapPathToken
			|| _to == swapPathToken)
		{
            //direct
			path = new address[](2);
...
// File: contracts/VaultStrategy/Biswap/SmartChef/VaultStrategy_BSC_Biswap_SmartChef.sol

contract VaultStrategy_Fantom_Excalibur_MasterChef is VaultStrategyV1
{
    //========================
    // LIBS
    //========================
    ...
    IToken public constant REWARD_TOKEN = IToken(0x6e99e0676A90b2a5a722C44109db22220382cc9F);
    address public constant MASTERCHEF = 0x70B9611f3cd33e686ee7535927cE420C2A111005;
    address public constant ROUTER = 0xc8Fe105cEB91e485fb0AC338F2994Ea655C78691;
    ...
    {
        (address depositTokenAddr, , , , , , , ) = IMasterChef_Fantom_Excalibur(MASTERCHEF).getPoolInfo(_poolID);

        //init      
        depositToken = IToken(depositTokenAddr);
    ...
    function balanceOfPool() public view virtual override returns (uint256)
    {
        (uint256 _amount, ) = IMasterChef_Fantom_Excalibur(masterChef).getUserInfo(poolID, address(this));
        return _amount;
    }

    function poolAllocPoints() public view virtual override returns (uint256)
    {
        (, uint256 allocPoints, , , , , , ) = IMasterChef_Fantom_Excalibur(masterChef).getPoolInfo(poolID);
        return allocPoints;
    }

    function poolDepositFee() external view virtual override returns (uint256)
    {
        (, , , , uint256 depositFeeBP, , , ) = IMasterChef_Fantom_Excalibur(masterChef).getPoolInfo(poolID);
        return depositFeeBP.mul(PERCENT_FACTOR).div(1e4);
    }

    function poolPending() public view virtual override returns (uint256)
    {
        return IMasterChef_Fantom_Excalibur(masterChef).pendingRewards(poolID, address(this));
    }
    ...
    {
        IMasterChef_Fantom_Excalibur(masterChef).deposit(poolID, _amount);
    }

    function poolWithdraw(uint256 _amount) internal override
    {
        IMasterChef_Fantom_Excalibur(masterChef).withdraw(poolID, _amount);
    }

    function emergencyWithdraw() internal override
    {
        IMasterChef_Fantom_Excalibur(masterChef).emergencyWithdraw(poolID);
    }
}