...
// File contracts/interfaces/IWigoSingleChef.sol

pragma solidity 0.8.17;

interface IWigoSingleChef {
...
// File contracts/WigoSingleStrat.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract WigoSingleStrat is Ownable, Pausable {
    ...
    address public grimFeeRecipient = address(0x39a64CA2892485F22ee6140795c50C28a281AD97);
    ...
    function _deposit() internal whenNotPaused {
        ...
        IWigoSingleChef(masterchef).enterStaking(wantBal);
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IWigoSingleChef(masterchef).leaveStaking(_amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        if (caller != vault){
         ...
         IWigoSingleChef(masterchef).leaveStaking(0);
    ...
    function chargeFees(address caller) internal {
        ...
        uint256 grimFeeAmount = wrappedBal * RECIPIENT_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        ...
        uint256 wigoBal = IERC20(wigo).balanceOf(address(this));
        IWigoSingleChef(masterchef).enterStaking(wigoBal);
    }

     /** @dev Returns rewards unharvested */
    function rewardBalance() public view returns (uint256) {
        return IWigoSingleChef(masterchef).pendingWigo(pid, address(this));
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        (uint256 _amount, ) = IWigoSingleChef(masterchef).userInfo(pid, address(this));
        return _amount;
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        IWigoSingleChef(masterchef).emergencyWithdraw(pid);
        uint256 wantBal = IERC20(want).balanceOf(address(this));
    ...
    function setFees(uint256 newCallFee, uint256 newStratFee, uint256 newRecipientFee, uint256 newWithdrawFee, uint256 newTreasuryFee) external 
onlyAdmin {
