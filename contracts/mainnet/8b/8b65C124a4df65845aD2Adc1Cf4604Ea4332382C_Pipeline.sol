...
contract Pipeline is Ownable {
	...
	struct CallData {
		address inToken;
		uint256[] amount; // Amount for first call, amountPos for next calls
		address target; 
		bytes callData;
	}
	address pipelineProxy;
	address ETH_ADDRESS = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE; 

	event PipelineProxyChanged(address indexed newPipelineProxy);

	/// @dev Change word at pos <valuePos> with <value>. And then call _contract with this data.
	/// @param _contract Contract address to be called
	/// @param data Bytes data of contract call 
	/// @param valuesPos Array of positions of value in callData
	/// @param value Value of word to which it will change 
	/// @param ethValue How much ether we should send with call
	/// @return success - standart return from call
	/// @return result - standart return from call
	function callFunctionWithFullSignature(
		address _contract, 
		bytes memory data, 
		uint256[] memory valuesPos, 
		uint256 value,
		uint256 ethValue
	) 
		public // Should be internal. Public for testing.
		returns (bool success, bytes memory result) 
	{
		for (uint256 i = 0; i < valuesPos.length; i++) {
			uint256 pos = valuesPos[i];
			assembly {
				mstore(add(data, pos), value)
			}
		}

		(success, result) = _contract.call{value: ethValue}(data);
	}
	...
	///		 CallData structure:
	///			inToken: Address of token that will be used in contract
	///			amount: For first call is amount of token, for others is position of amount in callData
	///			target: Address of contract to be called
	///			callData: callData to call contract with
	/// @param swapData data to call swapper
	/// @param targetData data to call target
	function run(
		CallData calldata swapData,
		CallData memory targetData,
		address distToken
	) external payable {
		...
		if (swapData.inToken != ETH_ADDRESS) {
			// IERC20(initialToken).safeTransferFrom(msg.sender, address(this), amount);
			PipelineProxy(pipelineProxy).transfer(swapData.inToken, msg.sender, swapData.amount[0]);
			IERC20(swapData.inToken).safeApprove(swapData.target, swapData.amount[0]);
		}
		...
		if (targetData.inToken != ETH_ADDRESS) {
			...
			IERC20(targetData.inToken).safeApprove(targetData.target, erc20Balance);
		} else {
			ethBalance = address(this).balance;
			require(ethBalance > 0, "Zero eth balance after swap");
		}

		(success,) = callFunctionWithFullSignature(targetData.target, targetData.callData, targetData.amount, erc20Balance, ethBalance);
		require(success, "Can't mint");

		uint256 distTokenAmount = IERC20(distToken).balanceOf(address(this));
	...
	function runWithPool(
		CallData memory swapData,
		CallData memory poolData,
		CallData memory targetData,
		address distToken
	) external payable {
		...
		if (poolData.inToken != ETH_ADDRESS) {
			...
			require(erc20Balance > 0, "Zero token balance after swap");
			IERC20(poolData.inToken).safeApprove(poolData.target, erc20Balance);
		} else {
			ethBalance = address(this).balance;
			require(ethBalance > 0, "Zero eth balance after swap");
		}

		(success, ) = callFunctionWithFullSignature(poolData.target, poolData.callData, poolData.amount, erc20Balance, ethBalance); 
		require(success, "Can't call pool");

		if (targetData.inToken != ETH_ADDRESS) { // Should we allow eth for target?
			erc20Balance = IERC20(targetData.inToken).balanceOf(address(this));
			ethBalance = 0;
	...
	function setPipelineProxy(address newPipelineProxy) external onlyOwner {
		_setPipelineProxy(newPipelineProxy);
	}

	function _setPipelineProxy(address newPipelineProxy) internal {
		pipelineProxy = newPipelineProxy;
		emit PipelineProxyChanged(newPipelineProxy);
	}
...
contract PipelineProxy is Ownable {
	using SafeERC20 for IERC20;
	address trusted;
	...
	/// @dev Change main contract address
	/// @param _trusted New main contract address
	function setTrusted(address _trusted) onlyOwner external {
		_setTrusted(_trusted);
	}
