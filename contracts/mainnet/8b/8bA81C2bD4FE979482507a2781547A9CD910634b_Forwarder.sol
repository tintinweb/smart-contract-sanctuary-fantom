...
contract Forwarder is EIP712, Ownable {
  ...
  event Fee_Token_Updated(address indexed tokenAddress, address chainLinkFeed, bool isAllowed);
  ...
  constructor(
    ...
    feeTokenData[address(0)] = FeeTokenEntry(false, AggregatorV3Interface(chainlink_ETH_USDC));
  ...
  function execute(ForwardRequest calldata req, bytes calldata sig) public payable returns (bool success, bytes memory returnData) {
    _verifySignature(req, sig);
  ...
  function updateFeeToken(address tokenAddress, address chainLinkFeed, bool isAllowed) public onlyOwner {
    require(tokenAddress != address(0), "Can not set address zero");
    feeTokenData[tokenAddress] = FeeTokenEntry(isAllowed, AggregatorV3Interface(chainLinkFeed));
    emit Fee_Token_Updated(tokenAddress, chainLinkFeed, isAllowed);
  }
  ...
  function _verifySignature(ForwardRequest calldata req, bytes calldata signature) internal view {
    ...
    require(req.validUntilTime > block.timestamp || req.validUntilTime == 0, "Forwarder: request expired");
    ...
    require(signer.allowedAddress != address(0), "Forwarder: signature mismatch"); // signer is not set
