...
contract DelayedWithdrawalToolEth is DelayedWithdrawalTool {
    ...
    // request belongs to msg.sender
    // beneficiary is msg.sender
    function claimEth() external {
        claimEthFor(msg.sender);
    }

    // request belongs to _onBehalfOf
    // beneficiary is _onBehalfOf
    function claimEthFor(address _onBehalfOf) public {
        (uint256 underlyingAmount, uint256 index) = _setClaimed(_onBehalfOf);
        IWETH(underlying).withdraw(underlyingAmount);
        payable(_onBehalfOf).transfer(underlyingAmount);
        emit Claimed(_onBehalfOf, _onBehalfOf, index, underlyingAmount);
    }

    // request belongs to msg.sender
    // beneficiary is _onBehalfOf
    function claimEthTo(address _onBehalfOf) public {
        (uint256 underlyingAmount, uint256 index) = _setClaimed(msg.sender);
        IWETH(underlying).withdraw(underlyingAmount);
        payable(_onBehalfOf).transfer(underlyingAmount);
        emit Claimed(msg.sender, _onBehalfOf, index, underlyingAmount);
