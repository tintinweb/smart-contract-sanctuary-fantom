...
contract ZestLottery is Ownable, Pausable {
    ...
    //When there are many entries, the maximum chainlink callback gas limit of 25,000 is exceeded
    // ... In these cases we need a way to draw manually.
    // ... We can observe the randomWords from the block explorer and call it manually.
    function drawManually(uint256 requestId, uint256[] memory _randomWords) public onlyOwner {
        require(endTimes[lotteryID] < block.timestamp && !paused(), "Lottery not finished yet.");
        require(players.length > 0, "Nobody is playing!");
        drawLottery(requestId, _randomWords);
    }

    function drawLottery(uint256 reqestId, uint256[] memory _randomWords) public {
        require(
            msg.sender == vrfConsumer || msg.sender == address(this),
            "Only the VRF consumer, or drawManually can execute the draw"
        );
