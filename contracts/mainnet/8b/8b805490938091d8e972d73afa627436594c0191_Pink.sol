...
contract Pink is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 PINK-WFTM LP -> PINK pool
    uint256 public constant INITIAL_PINK_POOL_DISTRIBUTION = 150000 ether;
    ...
    /**
     * @notice Constructs the PINK ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Pink Panther", "PINK") {
        // Mints 1 PINK to contract creator for initial pool setup
    ...
    function _getPinkPrice() internal view returns (uint256 _pinkPrice) {
        try IOracle(pinkOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Pink: failed to fetch PINK price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _pinkPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_pinkPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPinkOracle(address _pinkOracle) public onlyOperatorOrTaxOffice {
        require(_pinkOracle != address(0), "oracle address cannot be 0 address");
        pinkOracle = _pinkOracle;
    }
    ...
    /**
     * @notice Operator mints PINK to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PINK to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPinkPrice = _getPinkPrice();
            currentTaxRate = _updateTaxRate(currentPinkPrice);
            if (currentPinkPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_pinkPool != address(0), "!_pinkPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_pinkPool, INITIAL_PINK_POOL_DISTRIBUTION);
