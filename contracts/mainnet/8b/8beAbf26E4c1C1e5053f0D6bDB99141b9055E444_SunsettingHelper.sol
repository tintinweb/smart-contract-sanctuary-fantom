...
/**
 * @title Helper contract for sunsetting 0xDAO on Fantom
 * @author 0xDAO
 */

contract SunsettingHelper is OxDAODeployer {
    function upgradeProcotol(
        address rewardsDistributorImpAddress,
        address voterProxyImpAddress,
        address voterProxyAssetsImpAddress,
        address oxdV1RedeemImpAddress
    ) external {
        IOxLens oxLens = IOxLens(
            address(0xDA00137c79B30bfE06d04733349d98Cf06320e69)
        );
        address voterProxyAddress = oxLens.voterProxyAddress();
        address voterProxyAssetsAddress = IVoterProxy(voterProxyAddress)
        ...
        address beepAddress = 0xDa00C4Fec58DC0accE8FbDCd52428a7f66dcc433;
        address c30Address = 0xC300051bae03A3d1596Da8Bb07030c07Fb5D7d5e;

        // link proxy to new implementations
        IProxy(voterProxyAddress).updateImplementationAddress(
            voterProxyImpAddress
        );

        IProxy(voterProxyAssetsAddress).updateImplementationAddress(
            voterProxyAssetsImpAddress
        );

        IProxy(rewardsDistributorAddress).updateImplementationAddress(
            rewardsDistributorImpAddress
        );

        IProxy(oxdV1RedeemAddress).updateImplementationAddress(
            oxdV1RedeemImpAddress
        );

        // set operators for VoterProxyAssets
        IOperator voterProxyAssets = IOperator(voterProxyAssetsAddress);
        voterProxyAssets.setOperator(beepAddress, true);
        voterProxyAssets.setOperator(c30Address, true);

        // set operators for oxdV1Redeem
        IOperator oxdV1Redeem = IOperator(oxdV1RedeemAddress);
        oxdV1Redeem.setOperator(beepAddress, true);
        oxdV1Redeem.setOperator(c30Address, true);
    }
}
