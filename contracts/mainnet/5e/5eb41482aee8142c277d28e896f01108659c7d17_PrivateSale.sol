...
contract PrivateSale is Ownable {
  ...
  uint256 public publicHardcapOfTier;
  ...
  function addAllocation(address[] memory users, uint8 tier) external onlyOwner {
    ...
    for (uint256 i = 0; i < users.length; i++) {
        if (!whitelisted[users[i]]) {
            ...
            // Gán giới hạn riêng biệt cho tier 3
            if (tier == 3) {
                hardcapOfTier[3] = publicHardcapOfTier;
            }
        }
    }
}
  ...
  function allocationFor(address user) public view returns (uint256) {
    if (tierOf[user] == 3 && !whitelisted[user]) {
        return publicHardcapOfTier.sub(purchaseOf[user]);
    }
