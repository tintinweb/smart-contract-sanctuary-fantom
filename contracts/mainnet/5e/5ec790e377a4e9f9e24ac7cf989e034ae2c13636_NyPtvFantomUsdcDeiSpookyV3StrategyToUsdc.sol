 ...
 * out quoting reference 'NyPtvFantomUsdcDeiSpookyV3StrategyToUsdc'
 *
...
contract NyPtvFantomUsdcDeiSpookyV3StrategyToUsdc is NYBaseProfitTakingStrategyV3 {
  ...
  function _swapFarmEmissionTokens() internal {
    ...
    if (deusToUsdcPath.length >= 2 && deusBalance > 0) {
      deus.safeIncreaseAllowance(SPOOKY_ROUTER, deusBalance);

      uint256[] memory amounts = IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(deusBalance, deusToUsdcPath);
      uint256 amountOutMin = (amounts[amounts.length - 1] * maxSlippage) / PERCENT_DIVISOR;
  ...
  function estimateHarvest() external view override returns (uint256 profit, uint256 callFeeToUser) {
    ...
    if (totalRewards != 0) {
      uint256[] memory harvestAmounts = IUniswapV2Router02(SPOOKY_ROUTER).getAmountsOut(totalRewards, booToUsdcPath);
