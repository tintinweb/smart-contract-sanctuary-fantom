...
contract RebateTreasury is Ownable {
    ...
    constructor(address rebel, address rebelOracle, address treasury) {
        Rebel = IERC20(rebel);
        RebelOracle = IOracle(rebelOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Rebel at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 rebelAmount = getRebelReturn(token, amount);
        require(rebelAmount <= Rebel.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient rebel balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + rebelAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += rebelAmount;
    }

    // Claim available Rebel rewards from bonding
    ...
    function setRebel(address rebel) external onlyOwner {
        Rebel = IERC20(rebel);
    }

    // Set Rebel oracle

    function setRebelOracle(address oracle) external onlyOwner {
        RebelOracle = IOracle(oracle);
    }
    ...
    function _claimVested(address account) internal {
        ...
        Rebel.transfer(account, claimable);
    }
    ...
    // Calculate Rebel return of bonding amount of token

    function getRebelReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 rebelPrice = getRebelPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / rebelPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 rebelPrice = getRebelPrice();
        if (rebelPrice < 1e18) return 0;

        uint256 rebelPremium = rebelPrice * DENOMINATOR / 1e18 - DENOMINATOR;
        if (rebelPremium < bondThreshold) return 0;
        if (rebelPremium <= secondaryThreshold) {
            return (rebelPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (rebelPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get REBEL price from Oracle

    function getRebelPrice() public view returns (uint256) {
        return RebelOracle.consult(address(Rebel), 1e18);
    }
    ...
    // Get claimable vested Rebel for account

    function claimableRebel(address account) external view returns (uint256) {
