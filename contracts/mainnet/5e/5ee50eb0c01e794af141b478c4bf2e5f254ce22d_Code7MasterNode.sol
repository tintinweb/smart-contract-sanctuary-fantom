...
contract Code7MasterNode {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    IERC20 public constant CODE7 = IERC20(0xF77864FCFfeC4598813E3378681c9330B771cA88);
    ...
    address public deadAdd;

    bool public enabled;
    uint256 public constant MULTIPLIER = 10e18;

    constructor(address _daoFund, uint256 _startTime) public {
        maxReturnPercent = 400; // 400% roi
        dripRate = 2880000; // 3% daily
        treasuryFeePercent = 10; // treasury allocation
        burnRatio = 13; // burn baby burn

        if (_daoFund != address(0)) daoFund = _daoFund;
        deadAdd = 0x000000000000000000000000000000000000dEaD;
    ...
    function setDaoFund(address _daoFund) external onlyDev {
        require(msg.sender == dev, "!dev");
        daoFund = _daoFund;
    }

    function setBurnRatio(uint256 rate) external onlyDev {
        burnRatio = rate;
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        uint256 daoFee = tierPrice.mul(treasuryFeePercent).div(100);
        uint256 burnFee = tierPrice.mul(burnRatio).div(100);
        uint256 netTierPrice = tierPrice.sub(daoFee).sub(burnFee);

        require(CODE7.balanceOf(_sender) >= tierPrice, "Insufficient balance");
        require(CODE7.allowance(_sender, address(this)) >= tierPrice, "Insufficient allowance");    
        
        CODE7.safeTransferFrom(_sender, address(this), netTierPrice);
        CODE7.safeTransferFrom(_sender, daoFund, daoFee);
        CODE7.safeTransferFrom(_sender, deadAdd, burnFee);
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            ...
            IERC20(CODE7).safeTransfer(_sender, _rewards);
    ...
    function getRewardDrip() public view returns (uint256) {
        if (lastDripTime < block.timestamp) {
            uint256 poolBalance = getCode7BalancePool();
    ...
    function getCode7Balance() public view returns (uint256) {
        return IERC20(CODE7).balanceOf(address(this));
    }

     function getCode7BalancePool() public view returns (uint256) {
        return getCode7Balance().sub(total_rewards);
