...
// File: contracts/facades/BabyCreatureBreedingFacade.sol



pragma solidity ^0.8.14;

contract BabyCreatureBreedingFacade is _BabyCreatureStorage {
    BabyCreature private immutable _babyCreatureOld;
    BabyCreature private immutable _babyCreatureNew;
    ...
    constructor(
        ...
        _babyCreatureOld = BabyCreature(babyCreatureOld);
        _babyCreatureNew = BabyCreature(babyCreatureNew);
        _tokenIdShift = tokenIdShift;
    }

    function getBreedingStakesDelegate(
        address owner,
        uint256 from,
        uint256 to
    ) external view returns (uint256[4][] memory breedingStakes, uint256 last) {
        BabyCreatureStorage storage _storage = babyCreatureStorage();
        Stake[] memory stakes = _storage.stakes[owner];
        last = stakes.length;
        to = to > last ? last : to;
        if (from < to) {
            breedingStakes = new uint256[4][](to - from);
            uint256 index = 0;
            for (uint256 i = from; i < to; i++) {
                breedingStakes[index][0] = stakes[i].start;
                breedingStakes[index][1] = stakes[i].end;
                breedingStakes[index][2] = stakes[i].male;
                breedingStakes[index][3] = stakes[i].female;
                index++;
            }
        }
    }

    function getCurrenciesDelegate()
        external
        view
        returns (
            uint256[] memory wolfCurrencies,
            uint256[] memory sheepCurrencies
        )
    {
        BabyCreatureStorage storage _storage = babyCreatureStorage();
        wolfCurrencies = _storage.breedingCurrency[0];
        sheepCurrencies = _storage.breedingCurrency[1];
    }
    ...
    function getCurrencies()
        ...
        returns (
            bool pause,
            uint256[3][] memory wolfCurrencies,
            uint256[3][] memory sheepCurrencies,
            string[] memory wolfCurrencyNames,
            string[] memory sheepCurrencyNames
        )
    {
        pause = _babyCreatureNew.paused();

        (
            uint256[] memory _wolfCurrencies,
            uint256[] memory _sheepCurrencies
        ) = abi.decode(
                _babyCreatureNew.delegateCall(address(this), 0, ""),
                (uint256[], uint256[])
            );
        (wolfCurrencies, wolfCurrencyNames) = _getDataFor(_wolfCurrencies);
        (sheepCurrencies, sheepCurrencyNames) = _getDataFor(_sheepCurrencies);
    }

    function getBreedingStakes(
        address owner,
        uint256 from,
        uint256 to
    ) external returns (uint256[4][] memory breedingStakes, uint256 last, string[] memory uri) {
        (breedingStakes, last) =  abi.decode(
                _babyCreatureNew.delegateCall(address(this), 1, abi.encode(owner, from, to)),
                (uint256[4][], uint256)
            );
        uri = new string[](breedingStakes.length * 2);
        uint256 j = 0;
        for (uint256 i = 0; i < breedingStakes.length; i++) {     
            uint256 maleId = breedingStakes[i][2];
            if (maleId < _tokenIdShift) {
                uri[j] = 
                    _babyCreatureOld.tokenURI(maleId);
            } else {
                uri[j] = 
                    _babyCreatureNew.tokenURI(maleId);
            }
            j++;

            uint256 femaleId = breedingStakes[i][3];
            if (femaleId < _tokenIdShift) {
                uri[j] = 
                    _babyCreatureOld.tokenURI(femaleId);
            } else {
                uri[j] = 
                    _babyCreatureNew.tokenURI(femaleId);
            }
            j++;
        }
    }

    function _getDataFor(uint256[] memory currencies)
        private
        view
        returns (uint256[3][] memory data, string[] memory names)
    {
        data = new uint256[3][](currencies.length);
        names = new string[](currencies.length);
        for (uint256 i = 0; i < currencies.length; i++) {
            (string memory name, uint256 price, address erc20, ) = _babyCreatureNew.getCurrency(
                currencies[i]
            );
