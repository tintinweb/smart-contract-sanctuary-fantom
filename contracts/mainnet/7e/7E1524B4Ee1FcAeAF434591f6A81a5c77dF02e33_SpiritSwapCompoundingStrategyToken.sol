...
// File contracts/SpiritSwapCompoundingStrategyToken.sol
...
/**
 * @notice This contract implements a compounding strategy for SpiritSwap MasterChef.
 *         It basically deposits and withdraws funds from MasterChef and collects the
 *         reward token (LQDR). The compounding happens by calling the gulp function;
 ...
 */
contract SpiritSwapCompoundingStrategyToken is ERC20, Ownable, ReentrancyGuard, DelayedActionGuard
{
	...
	{
		return _calcNetDepositAmount(_amount) * totalSupply() / totalReserve();
	}
	...
	{
		uint256 _poolLength = MasterChefV2(_masterChef).poolLength();
		require(_pid < _poolLength, "invalid pid");
		_reserveToken = MasterChefV2(_masterChef).lpToken(_pid);
		_rewardToken = MasterChefV2(_masterChef).LQDR();
	...
	{
		return MasterChefV2(masterChef).pendingLqdr(pid, address(this));
	}
	...
	{
		(_reserveAmount,) = MasterChefV2(masterChef).userInfo(pid, address(this));
	...
	{
		(,,,uint256 _depositFee) = MasterChefV2(masterChef).poolInfo(pid);
		uint256 _fee = _amount * _depositFee / 10000;
	...
	function _deposit(uint256 _amount) internal
	{
		...
		MasterChefV2(masterChef).deposit(pid, _amount, address(this));
	}

	/// @dev Harvests the MasterChef pool
	function _harvest() internal
	{
		MasterChefV2(masterChef).harvest(pid, address(this));
	}

	/// @dev Performs an withdrawal from the MasterChef pool
	function _withdraw(uint256 _amount) internal
	{
		MasterChefV2(masterChef).withdraw(pid, _amount, address(this));
	}

	/// @dev Performs an emergency withdrawal from the MasterChef pool
	function _emergencyWithdraw() internal
	{
		MasterChefV2(masterChef).emergencyWithdraw(pid, address(this));
	}
