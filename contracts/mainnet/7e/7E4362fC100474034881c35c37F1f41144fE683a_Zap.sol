...
//custom zapper start
    function zapInNativeToToken1ThenToLP( address _token1,address _toLP, address routerAddr, address _recipient)external payable{
        estimateZapIn(_toLP, routerAddr, msg.value);
        uint token1Amount = _swapNativeForToken(_token1, msg.value, address(this), routerAddr);
        _approveTokenIfNeeded(_token1, routerAddr);
        _zapInToken(_token1,token1Amount,_toLP,routerAddr,_recipient);
    }
    function SetApproveTokenIfNeeded(address token, address router) external {
...
//custom zapper end
    ...
    /* ========== Private Functions ========== */
//custom zapper start
    function _zapInToken(address _from, uint amount, address _to, address routerAddr, address _recipient) private {
        // From an ERC20 to an LP token, through specified router, going through base asset if necessary
        IERC20(_from).safeTransferFrom(msg.sender, address(this), amount);
        // we'll need this approval to add liquidity
        _approveTokenIfNeeded(_from, routerAddr);
   ...
   function _approveTokenIfNeeded_new(address token, address router) private {
        if (IERC20(token).allowance(address(this), router) == 0) {
            IERC20(token).safeIncreaseAllowance(router, type(uint).max);
        }
    }    
//custom zapper end
    ...
    function _swap(address _from, uint amount, address _to, address recipient, address routerAddr) private returns (uint) {
        ...
        if (fromBridge == _to)
        {
                ...
                path[0] = _from;
                path[1] = _to;
        } else if (fromBridge != address(0) && toBridge != address(0)) {
        ...
        uint[] memory amounts = router.swapExactTokensForTokens(amount, 0, path, recipient, block.timestamp+300);
        return amounts[amounts.length - 1];
    }
    ...
    function gettokenBridgeForRouter(address _token, address routerAddr) private view returns (address) {
        return tokenBridgeForRouter[_token][routerAddr];
    }
