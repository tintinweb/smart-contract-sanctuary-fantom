...
contract Omega is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the OMEGA ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("O-MEGA", "O-MEGA") {
        // Mints 1 OMEGA to contract creator for initial pool setup
    ...
    function _getOmegaPrice() internal view returns (uint256 _omegaPrice) {
        try IOracle(omegaOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Omega: failed to fetch OMEGA price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _omegaPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_omegaPrice >= taxTiersTwaps[tierId]) {
    ...
    function setOmegaOracle(address _omegaOracle) public onlyOperatorOrTaxOffice {
        require(_omegaOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints OMEGA to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of OMEGA to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentOmegaPrice = _getOmegaPrice();
            currentTaxRate = _updateTaxRate(currentOmegaPrice);
            if (currentOmegaPrice < burnThreshold) {
                burnTax = true;
            }
        }
