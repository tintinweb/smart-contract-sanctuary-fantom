...
//		(uint amount1,uint amount2)=getAmounts(tokenTo);
//		I(router).addLiquidity(mainToken,tokenTo,amount1,amount2,0,0,address(this),2**256-1);
		//if(I(tokenTo).balanceOf(address(this))>0){
		//	address p = I(factory).getPair(mainToken,tokenTo);
		//	I(tokenTo).transfer(p,balanceOf(address(this)));
		//	I(p).sync();
		//}		
		//nextSwapBlock+=block.number+86400;
		//uint leftover = I(mainToken).balanceOf(address(this));
		//if(leftover>0){ I(mainToken).transfer(_treasury, leftover); I(mainToken).addLeftover(leftover); }
	}

//	function getAmounts(address tTo) internal view returns (uint amountA,uint amountB){
//		address p = I(factory).getPair(mainToken,tTo);
//        uint reserveA = I(mainToken).balanceOf(p); uint reserveB = I(tTo).balanceOf(p);
//        uint ADesired = I(mainToken).balanceOf(address(this)); uint BDesired = I(tTo).balanceOf(address(this));
//        if (reserveA == 0 && reserveB == 0) {
//            (amountA, amountB) = (ADesired, BDesired);
//        } else {
//            uint BOptimal = ADesired*reserveB/reserveA;
//            if (BOptimal <= BDesired) {
//                (amountA, amountB) = (ADesired, BOptimal);
//            } else {
//                uint AOptimal = BDesired*reserveA/reserveB;
//                if(AOptimal > ADesired){
//                	BDesired = BDesired*ADesired/AOptimal-1000;
//                	AOptimal = ADesired;
//                }
//                (amountA, amountB) = (AOptimal, BDesired);
//            }
//        }
//	}

	function manualAdd(uint amount1, uint amount2,address tTo) public {
		require(msg.sender==liqMan);
		if(amount1==0){amount1 = I(mainToken).balanceOf(address(this));}
		if(amount2==0){amount2 = I(tTo).balanceOf(address(this));}
		I(router).addLiquidity(mainToken,tTo,amount1,amount2,0,0,address(this),2**256-1);
	}

	function withdraw() public {
		I(mainToken).transfer(liqMan,I(mainToken).balanceOf(address(this)));
		I(defTokenTo).transfer(liqMan,I(defTokenTo).balanceOf(address(this)));
	}

    //function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
    //    address token0 = tokenA < tokenB ? tokenA : tokenB;
    //    (uint reserve0, uint reserve1,) = I(I(factory).getPair(tokenA,tokenB)).getReserves();
