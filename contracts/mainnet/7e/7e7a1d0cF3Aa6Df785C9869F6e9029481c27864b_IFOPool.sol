...
// File: VOTING/contracts/utils/Address.sol
...
// File: VOTING/contracts/token/ERC20/IERC20.sol
...
// File: VOTING/contracts/math/SafeMath.sol
...
// File: VOTING/contracts/token/ERC20/SafeERC20.sol
...
// File: VOTING/contracts/utils/Pausable.sol
...
contract IFOPool is Ownable, Pausable {
    ...
    struct UserIFOInfo {
        ...
        uint256 lastActionBalance; // staked bar numbers (not include compoud bar) at last action
        uint256 lastValidActionBalance; // staked bar numbers in ifo valid period
    ...
    IERC20 public immutable receiptToken; // Raid token
     ...
     * @param _receiptToken: Raid token contract
    ...
    /**
     * @notice Deposits funds into the Bar Vault
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.barAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
     ...
     * @param _lastValidActionBalance: staked bar number at last action.
     ...
     * @param _amount:the bar amount that need be add or sub
    ...
    /**
     * @notice original Withdraws implementation from funds, the logic same as Bar Vault withdraw
    ...
    /**
     * @notice Reinvests BAR tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Bar Vault
     */
     ...
     * @return Expected reward to collect in BAR
     */
    function calculateHarvestBarRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingBar(0, address(this));
     ...
     */
    function calculateTotalPendingBarRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingBar(0, address(this));
