...
contract PEPE_MUSK is ERC20, ERC20Burnable, Ownable, DividendTrackerFunctions {
    
    address public walettAddress;
    uint16[3] public walettFees;

    uint16[3] public autoBurnFees;

    uint256 public swapThreshold;
    
    uint256 private _walettPending;
    ...
    event walettAddressUpdated(address walettAddress);
    event walettFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event walettFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20("PEPE MUSK", "PPMK") 
    {
        address supplyRecipient = 0x3A6031105fB1a6eD2Bbfc786BBDF6d73Fd82cFe3;
        
        walettAddressSetup(0x436ac5D09775cC987Af311da1e1547543D6698fC);
        walettFeesSetup(200, 200, 20);

        autoBurnFeesSetup(200, 200, 20);

        updateSwapThreshold(5 * (10 ** decimals()));

        lpTokensReceiverSetup(0x0000000000000000000000000000000000000000);
        liquidityFeesSetup(200, 200, 20);

        _deployDividendTracker(120, 1000 * (10 ** decimals()), address(this));

        gasForProcessingSetup(200000);
        rewardsFeesSetup(100, 100, 10);
        ...
        _transferOwnership(0x3A6031105fB1a6eD2Bbfc786BBDF6d73Fd82cFe3);
    }
    ...
    function walettAddressSetup(address _newAddress) public onlyOwner {
        walettAddress = _newAddress;

        excludeFromFees(_newAddress, true);

        emit walettAddressUpdated(_newAddress);
    }

    function walettFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        walettFees = [_buyFee, _sellFee, _transferFee];

        totalFees[0] = 0 + walettFees[0] + autoBurnFees[0] + liquidityFees[0] + rewardsFees[0];
        totalFees[1] = 0 + walettFees[1] + autoBurnFees[1] + liquidityFees[1] + rewardsFees[1];
        totalFees[2] = 0 + walettFees[2] + autoBurnFees[2] + liquidityFees[2] + rewardsFees[2];
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        emit walettFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                if (walettFees[txType] > 0) {
                    walettPortion = fees * walettFees[txType] / totalFees[txType];
                    _sendInTokens(from, walettAddress, walettPortion);
                    emit walettFeeSent(walettAddress, walettPortion);
                }
                ...
                fees = fees - walettPortion - autoBurnPortion;
            }
