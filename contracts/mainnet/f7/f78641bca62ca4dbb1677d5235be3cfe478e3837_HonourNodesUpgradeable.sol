...
contract HonourNodesUpgradeable is
  Initializable,
  IHonourNode,
  ...
  ReentrancyGuardUpgradeable
{
  ...
  function mintBuke(uint256 amount, string memory name) external onlyOwner {
  ...
  function mintMononofu(uint256 amount, string memory name) external onlyOwner {
    require(minted + amount <= nodeCap, "Node Amount Exceeded");
  ...
  function mintMusha(uint256 amount, string memory name) external onlyOwner {
    require(minted + amount <= nodeCap, "Node Amount Exceeded");
  ...
  function resolveRPC(string memory chain, uint256 tokenId)
    ...
    returns (string memory)
  {
    return string(abi.encodePacked(rpcURI, chain, "/", tokenId));
  }
  ...
  function claimRewards(uint256 tokenId) external {
    ...
    require(
      sender != address(0),
      "HonourNode: Cannot cashout from the zero address"
    );
    uint256 rewardAmount = syncNodeReward(tokenId);
    require(rewardAmount > 0, "HonourNode: Not enough reward to cash out");
