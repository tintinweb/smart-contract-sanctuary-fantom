...
// Note that this pool has no minter key of OBOL (rewards).
// Instead, the governance will call OBOL distributeReward method and send reward to this pool at the beginning.
contract ObolGenesisWnfts is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. OBOL to distribute.
        uint256 lastRewardTime; // Last time that OBOL distribution occurs.
        uint256 accOBolPerShare; // Accumulated OBOL per share, times 1e18. See below.
    ...
    // The time when OBOL mining starts.
    uint256 public poolStartTime;

    // The time when OBOL mining ends.
    uint256 public poolEndTime;

    address public protocolFundAddress;


    uint256 public oBolPerSecond = 0.10416 ether; // 90000 OBOL / (240h * 60min * 60s)
    ...
    constructor(
        ...
        if (_oBol != address(0)) oBol = IERC20(_oBol);
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "OBolGenesisPool: not operator");
        _;
    }
    
    function setRewardToken (address _rewardToken) public onlyOperator {
        require (block.timestamp <= poolStartTime, "pool started");
        oBol = IERC20(_rewardToken);
    } 

    function checkPoolDuplicate(IERC20 _token) internal view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "OBolGenesisPool: existing pool?");
        }
    }
    function nftCheckPoolDuplicate(IERC721 _nft) internal view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].nft != _nft, "OBolGenesisPool: existing pool?");
        }
    }

    // Add a new pool. Can only be called by the owner.
    // @ _allocPoint - amount of oBol this pool will emit
    ...
    // Update the given pool's OBOL allocation point. Can only be called by the owner.
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(oBolPerSecond);
            return poolEndTime.sub(_fromTime).mul(oBolPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(oBolPerSecond);
            return _toTime.sub(_fromTime).mul(oBolPerSecond);
        }
    }

    // View function to see pending OBOL on frontend.
    function pendingOBOL(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accOBolPerShare = pool.accOBolPerShare;
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _oBolReward = _multiplyHelper.div(totalAllocPoint);
            accOBolPerShare = accOBolPerShare.add(_oBolReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accOBolPerShare).div(1e18).sub(user.rewardDebt);
    }
    // View function to see pending OBOL on frontend.
    function pendingOBOLNft(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accOBolPerShare = pool.accOBolPerShare;
        ...
        if (block.timestamp > pool.lastRewardTime && nftSupply != 0) {
            ...
            uint256 _multiplyHelper =  _generatedReward.mul(pool.allocPoint); // intermidiate var to avoid multiply and division calc errors
            uint256 _oBolReward = _multiplyHelper.div(totalAllocPoint);
            accOBolPerShare = accOBolPerShare.add(_oBolReward.mul(1e18).div(nftSupply));
        }
        return user.amount.mul(accOBolPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 nftSupply = pool.nft.balanceOf(address(this));
         ...
         if (totalAllocPoint > 0) {
            ...
            uint256 _oBolReward = multiplyHelper.div(totalAllocPoint);
            pool.accOBolPerShare = pool.accOBolPerShare.add(_oBolReward.mul(1e18).div(nftSupply));
        } 
         pool.lastRewardTime = block.timestamp;
        }  else  {
            uint256 tokenSupply = pool.token.balanceOf(address(this));
            ...
            uint256 multiplyHelper = _generatedReward.mul(pool.allocPoint);
            uint256 _oBolReward = multiplyHelper.div(totalAllocPoint);
            pool.accOBolPerShare = pool.accOBolPerShare.add(_oBolReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant  {
        ...
        if (user.amount > 0) {
            ...
            uint256 _pending = user.amount.mul(pool.accOBolPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                // send pending reward to user, if rewards accumulating in _pending
                safeOBolTransfer(_sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accOBolPerShare).div(1e18);
    ...
    // Safe OBOL transfer function, in case if rounding error causes pool to not have enough OBOLs.
    function safeOBolTransfer(address _to, uint256 _amount) internal {
        uint256 _oBolBalance = oBol.balanceOf(address(this));
        if (_oBolBalance > 0) {
            if (_amount > _oBolBalance) {
                oBol.safeTransfer(_to, _oBolBalance);
            } else {
                oBol.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function setOperator(address _operator) external onlyOperator {
     ...
     function governanceRecoverUnsupported(
        ...
        require(address(_token) == address(oBol), "reward token only");
