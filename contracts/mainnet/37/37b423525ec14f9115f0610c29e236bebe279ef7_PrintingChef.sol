...
// File: contracts/PrintingChef.sol

contract PrintingChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
   ...
   // The pool limit (0 if none)
    ...
    constructor(
        ...
        poolInfo.push(PoolInfo({lpToken: _mprint, allocPoint: 1000, lastRewardBlock: startBlock, accRewardPerShare: 0}));
    ...
    // Stake mprint tokens to PrintingChef
    ...
    // Withdraw mprint tokens from STAKING.
    ...
    function sweepExtraToken(IERC20 token) external onlyOwner {
        require(address(token) != address(rewardToken), "can not sweep stake token");
        require(address(token) != address(mprint), "can not sweep reward token");
