...
contract TokenStaking is Ownable {
    ...
    constructor(
        ...
        uint256 _startTime,
        uint256 _bonusEndSec
    ) public {
        stakeToken = _stakeToken;
        rewardToken = _rewardToken;
        rewardPerSec = _rewardPerSec;
        startTime = _startTime;
        bonusEndSec = _bonusEndSec;
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        } else if (_from >= bonusEndSec) {
            return 0;
        } else {
            return bonusEndSec.sub(_from);
        }
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardBlock && totalStaked != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.timestamp);
            uint256 tokenReward = multiplier.mul(rewardPerSec).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    /// @param _rewardPerSec The amount of reward tokens to be given per block
    function setRewardPerSec(uint256 _rewardPerSec) external onlyOwner {
        rewardPerSec = _rewardPerSec;
    }

    /// @param  _bonusEndSec The block when rewards will end
    function setBonusEndSec(uint256 _bonusEndSec) external onlyOwner {
        require(_bonusEndSec > bonusEndSec, 'new bonus end block must be greater than current');
        bonusEndSec = _bonusEndSec;
    }
