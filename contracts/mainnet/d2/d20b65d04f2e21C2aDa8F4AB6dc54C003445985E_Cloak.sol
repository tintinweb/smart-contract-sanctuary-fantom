...
contract Cloak is ERC20Burnable, Operator {
    ...
    // Initial distribution for the CLOAK-WFTM LP -> CLOAK pool
    uint256 public constant INITIAL_CLOAK_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the CLOAK ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Cloak Finance", "CLOAK") {
        // Mints 1 CLOAK to contract creator for initial pool setup
    ...
    function _getCloakPrice() internal view returns (uint256 _cloakPrice) {
        try IOracle(cloakOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Cloak: failed to fetch CLOAK price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cloakPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cloakPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCloakOracle(address _cloakOracle) public onlyOperatorOrTaxOffice {
        require(_cloakOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CLOAK to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CLOAK to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCloakPrice = _getCloakPrice();
            currentTaxRate = _updateTaxRate(currentCloakPrice);
            if (currentCloakPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_cloakPool != address(0), "!_cloakPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_cloakPool, INITIAL_CLOAK_POOL_DISTRIBUTION);
