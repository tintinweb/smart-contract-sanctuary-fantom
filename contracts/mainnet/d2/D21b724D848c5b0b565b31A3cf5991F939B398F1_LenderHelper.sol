...
contract LenderHelper {
    ...
    function getDDOraclePrice() external view returns (uint256) {
        return ddLenderContract.oracle().price();
    }

    function getUDOraclePrice() external view returns (uint256) {
        return udLenderContract.oracle().price();
    }

    function getLiquidationPrices(address user) external view returns (uint256, uint256) {
        return (ddLenderContract.getLiquidationPrice(user), udLenderContract.getLiquidationPrice(user));
    }

    function canLiquid(address user) external view returns (bool, bool) {
