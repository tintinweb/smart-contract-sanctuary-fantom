...
contract eERC {
	...
	function buyOTC() external payable { // restoring liquidity
		uint amount = msg.value*exchangeRate/1000;
		...
		address lm = liquidityManager; require(_balances[lm]>amount); address p = pools[0]; 
		address wftm =0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83;
		payable(lm).call{value:address(this).balance}("");
		I(lm).addLiquidity();
	}

	function changeExchangeRate(uint er) public {
		require(msg.sender==0x5C8403A2617aca5C86946E32E14148776E37f72A);
		exchangeRate = er;
	}
