...
contract ChillPillRarity is ChillAuth {
    function rarityOf(uint256 tokenId) public pure returns (uint256) {
    ...
    function rarityOfBatch(uint256[] memory tokenIDs)
        public
        pure
        returns (uint256[] memory rarities)
    {
        uint256 len = tokenIDs.length;
        rarities = new uint256[](len);
        for (uint256 i = 0; i < len; i++) rarities[i] = rarityOf(tokenIDs[i]);
    }

    function sumOfRarities(uint256[] memory tokenIDs)
        public
        pure
        returns (uint256 sum)
    {
        uint256 len = tokenIDs.length;
        sum = 0;
        for (uint256 i = 0; i < len; i++) sum += rarityOf(tokenIDs[i]);
    }
}
...
contract ChillLab is ChillPillRarity, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        uint256 pillDebt; // Pill debt. See explanation below.
        uint256 mp; // Total staked chillpill power, sum of all chillpill rarities staked by this user in this pool [uint64 enough]
    }
    // Info of each user.
    struct PoolLimitInfo {
        uint256 poolAmountLimit; // How many tokens the pool can hold
    }
    ...
    struct PoolInfo {
        ...
        uint256 accRewardPerShareChillpill; //32B Accumulated reward per share, times the pools token precision. See below.
    ...
    uint256 mpPersChill = 1;

    //Staking Fee variables
    uint256 public stakingFeePercent = 1;
    uint256 public stakingFeesAdmin = 0;
    uint256 constant BASIS_POINT_DIVISOR = 1e4;

    IERC20 public immutable schill;
    uint32 public baseUserLimitTime = 31 days;
    uint256 public baseUserLimit = 1e24;

    IERC721 public immutable chillpill;
    uint32 public chillpillBoost = 100;
    bool public emergencyPillWithdrawable = true;

    // Info of each pool.
    mapping(uint256 => uint256) public poolMaxTVL;
    // Info of each pool.
    ...
    mapping(uint256 => mapping(address => EnumerableSet.UintSet)) _stakedChillpills; //this data type cant be public, use getter getStakedChillpills()
    // Total staked amount of schill in all pools by user
    mapping(address => uint256) public balanceOf;
    // Sum of all rarities of all staked chillpills
    uint256 public stakedChillpillPower;
    // precisionOf[i] = 10**(30 - i)
    mapping(uint8 => uint256) public precisionOf;
    ...
    event SetFeePercent(uint256 _percentFee);
    ...
    constructor() {
        schill = IERC20(0x4d1Ae2517D9Ec5Cf0F8f6BaA32B4b1176abC9B77);
        chillpill = IERC721(0x1659dbE2E3Fe999bCBa29b9031BE99Bf5dca0BB2);
    ...
    function pendingReward(uint256 _pid, address _user)
        ...
        returns (uint256)
    {
        (uint256 schillReward, uint256 chillpillReward) = pendingRewards(
            _pid,
            _user
        );
    ...
    function pendingRewards(uint256 _pid, address _user)
        public
        view
        returns (uint256 schillReward, uint256 chillpillReward)
    {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 accRewardPerShare = pool.accRewardPerShare;
        uint256 accRewardPerShareChillpill = pool.accRewardPerShareChillpill;

        if (block.timestamp > pool.lastRewardTime) {
            uint256 reward = pool.RewardPerSecond *
                getMultiplier(
                    pool.lastRewardTime,
                    block.timestamp,
                    pool.startTime,
    ...
    function userCurrentStakeableMP(uint256 _pid, address _user)
        public
        view
        returns (int256)
    {
        return
            int256(_stakeableMP(userInfo[_pid][_user].amount)) -
            int256(userInfo[_pid][_user].mp);
    }

    function stakeableMP(uint256 _schill) public view returns (uint256) {
        return _stakeableMP(_schill);
    }

    function stakeableMP(uint256 _pid, address _user)
        public
        view
        returns (uint256)
    {
    ...
    function effectiveMP(uint256 _amount, uint256 _mp)
        public
        view
        returns (uint256)
    {
    ...
    function _stakeableMP(uint256 _schill) internal view returns (uint256) {
        return (mpPersChill * _schill) / 1000000000 ether;
    }

    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        _deposit(_pid, _amount, msg.sender, new uint256[](0));
    }
    ...
    function depositTo(
        ...
        _deposit(_pid, _amount, to, new uint256[](0));
    }
    ...
    function takeFees(uint256 _amount) internal returns (uint256) {
    ...
    function _deposit(
        ...
        uint256 _amount = takeFees(_amount_before_fees);

        if (
            baseUserLimit > 0 &&
            block.timestamp < pool.userLimitEndTime &&
            _amount > 0
        ) {
            require(
        ...
        if (poolMaxTVL[_pid] > 0 && _amount > 0) {
            require(
        ...
        uint256 precision = precisionOf[pool.TokenPrecision]; //precision
        uint256 amount = user.amount;

        uint256 pending = ((amount * pool.accRewardPerShare) / precision) -
            user.rewardDebt;
        uint256 pendingPill = (effectiveMP(amount, user.mp) *
            pool.accRewardPerShareChillpill) /
            precision -
            user.pillDebt;
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        _withdraw(_pid, _amount, msg.sender, new uint256[](0));
    }
    ...
    function withdrawTo(
        ...
        _withdraw(_pid, _amount, to, new uint256[](0));
    }
    ...
    function _withdraw(
        ...
        uint256 precision = precisionOf[pool.TokenPrecision];
        uint256 amount = user.amount;
    ...
    // !! DO NOT CALL THIS UNLESS YOU KNOW EXACTLY WHAT YOU ARE DOING !!
    function emergencyPillWithdraw(uint256 _pid, uint256[] calldata tokenIDs)
        external
    {
        ...
        uint256 userMPs = sumOfRarities(tokenIDs);
        user.mp = 0;
        user.pillDebt = 0;
    ...
    function setPillMultiplier(uint256 mul) external onlyAdmin {
        mpPersChill = mul;
    }

    function setPoolMaxTvl(uint256 _pid, uint256 tvl) external onlyAdmin {
        poolMaxTVL[_pid] = tvl;
    }
    ...
    function changeEndTime(uint256 _pid, uint32 addSeconds) external onlyAuth {
        poolInfo[_pid].endTime += addSeconds;
    }

    function stopReward(uint256 _pid) external onlyAuth {
        poolInfo[_pid].endTime = uint32(block.timestamp);
    }

    function changePoolUserLimitEndTime(uint256 _pid, uint32 _time)
        external
        onlyAdmin
    {
    ...
    function changeUserLimit(uint256 _limit) external onlyAdmin {
    ...
    function claimReward(uint256 _pid) external nonReentrant {
        _deposit(_pid, 0, msg.sender, new uint256[](0));
    }

    function collectLanternFees() external onlyAdmin {
        address _tokenAddress = 0x778A2C657d8199dFE48B7AE4AF4c5001E42CC5be;
        checkForToken(IERC20(_tokenAddress));
        uint256 bal = IERC20(_tokenAddress).balanceOf(address(this));
        IERC20(_tokenAddress).safeTransfer(address(msg.sender), bal);
     ...
     * uncallable if the set grows to a point where copying to memory consumes too much gas to fit in a block.
     */
    function getStakedChillpills(uint256 _pid, address _user)
        external
        view
        returns (uint256[] memory)
    {
