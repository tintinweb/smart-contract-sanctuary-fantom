...
contract StrategyBeethovenXComposableMultiRewardChefUniV2 is StratFeeManagerInitializable, GasFeeThrottler {
    ...
    function initialize(
        ...
        uniswapRouterV2 = address(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
        rewardToNativeRoute = _rewardToNativeRoute;
        secondOutput = _rewardToNativeRoute[0];
    ...
    function swapRewardsToNative() internal {
        ...
        if (rewardBal > 0) {
            IUniswapRouterETH(uniswapRouterV2).swapExactTokensForTokens(
                rewardBal,
                0,
                rewardToNativeRoute,
                address(this),
    ...
    // native secondOutput amount for calling harvest
    function callReward() public view returns (uint256) {
        IFeeConfig.FeeCategory memory fees = getFees();
        (, uint256 rewardBal) = rewardsAvailable();
        uint256 nativeOut;
        if (rewardBal > 0) {
            uint256[] memory amountOut = IUniswapRouterETH(uniswapRouterV2).getAmountsOut(
                rewardBal,
                rewardToNativeRoute
            );
    ...
    function _giveAllowances() internal {
        ...
        IERC20(secondOutput).safeApprove(uniswapRouterV2, 0);
        IERC20(secondOutput).safeApprove(uniswapRouterV2, type(uint).max);
        if (secondOutput != native) {
            IERC20(native).safeApprove(uniswapRouterV2, type(uint).max);
        }
    }
    ...
    function _removeAllowances() internal {
        ...
        IERC20(secondOutput).safeApprove(uniswapRouterV2, 0);
        IERC20(native).safeApprove(uniswapRouterV2, 0);
    }
}