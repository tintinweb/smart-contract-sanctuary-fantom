...
contract BitGemFacet is Modifiers
{
    using UInt256Set for UInt256Set.Set;
    using ClaimLibrary for Claim;
    using BitGemLib for BitGemContract;
    using ERC721ALib for ERC721AContract;

    uint256 internal constant GEMHASH = uint256(keccak256("gem"));

    event BitGemTokenCreated(address indexed contractAddress, address indexed owner, BitGemSettings settings);

    /// @notice return all the mined tokens
    function setStakingState(bool state) external onlyOwner {
        BitGemContract storage ds = s().bitgems[address(this)];
        ds.settings.enabled = state;
    }

    function setAllowedMinter(address a, bool b) external onlyOwner {
        BitGemContract storage ds = s().bitgems[address(this)];
        ds.allowedMinters[a] = b;
    }

    function isAllowedMinter(address addr) private view returns (bool) {
        bool am = s().bitgems[address(this)].allowedMinters[addr];
        am = am || LibDiamond.contractOwner() == msg.sender;
        am = am || (addr == address(this));
        return am;
    }

    /// @notice mint a bitgem to the given address
    function mintBitGem(address to)
        external
        returns (uint256 tokenId) {
        // ensure we can mint
        require(isAllowedMinter(msg.sender), "not allowed to mint");
        BitGemContract storage ds = s().bitgems[address(this)];
        AttributeContract storage attr = s().attributes[address(this)];
        SaltStorage storage salt = s().salts[address(this)];
        ERC721AContract storage erc721Contract = s().erc721Contracts[address(this)];
        tokenId = ds._mintBitGem(attr, salt, erc721Contract, address(this), to);

        generateEvent("BitGemMinted", abi.encode(
            msg.sender,
            address(this),
            s().bitgems[address(this)].settings,
            tokenId
        ));  
    ...
    function price() external view returns (VariablePriceContract memory) {
        return s().variablePrices[address(this)];
    }
}
