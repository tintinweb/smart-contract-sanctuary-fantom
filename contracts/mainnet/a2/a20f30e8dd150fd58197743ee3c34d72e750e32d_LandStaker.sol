    ...
    _LandStakerStorage
{
    ...
    uint256 private _shearBase;
    uint256 private _fee;
    mapping(address => string[]) private _delegate;
    Land private _land;
    BabyWool private _babyWool;
    ...
    constructor(
        ...
        _grantRole(PAUSER_ROLE, msg.sender);
        _land = Land(land);
        _babyWool = BabyWool(babyWool);
        _shearBase = shearBase;
        _fee = fee;
    }
    ...
    function stake(
        ...
        (address owner, uint256 capacity, uint256 fertility) = abi
            .decode(
        ...
        Stake[] memory stakes = ls.stakes[farmId][acre][stakableType];
        for (uint256 i = 0; i < stakes.length; i++) {
            staked += stakes[i].stakable.length;
        }
        require(
    ...
    function _shear(
        ...
        if (block.timestamp > _stake.start && size > 0) {
            ...
            _babyWool.mint(
                owner,
                (duration * size * _shearBase * _stake.fertility) / DENOMINATOR,
                _fee
            );
        }
    }
    ...
    function _withdraw(
        ...
        LandStakerStorage storage ls = landStakerStorage();
        Stake[] storage stakes = ls.stakes[farmId][acre][stakableType];
