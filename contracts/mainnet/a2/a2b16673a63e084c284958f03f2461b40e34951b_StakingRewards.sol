...
library Address {
    ...
    address target, bytes memory data, uint256 weiValue, string memory errorMessage) private returns (bytes memory) {
        require(isContract(target), "Address: call to non-contract");
    ...
    bool success, bytes memory returndata, string memory errorMessage) private pure returns(bytes memory) {
        if (success) {
            return returndata;
        } else {
            if (returndata.length > 0) {
...
contract StakingRewards is RewardsDistributionRecipient, ReentrancyGuard, Pausable {
    ...
    IERC20 public rewardsToken; // DAI: 0x8D11eC38a3EB5E956B052f67Da8Bdc9bef8Abf3E
    IERC20 public stakingToken; // SOR: 0xEFFd4874AcA3Acd19a24dF3281b5cdAdD823801A
