...
contract Deca711 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    function refund() external nonReentrant {
        for (uint256 i = 1; i <= betRequestSize; i++) {
            if (betRequest[i].timestamp != 0 && block.timestamp >= betRequest[i].timestamp + 15 minutes) {
                delete betRequest[i].timestamp;
                rewardPool[betRequest[i].sender] += betRequest[i].betAmount;
            }
        }
    }
    ...
    function fulfillRandomWords(
        ...
        require(betRequest[betRequestSize].timestamp != 0, "timestamp should exist");
        uint256 sortedFace = randomWords[0] % 2;
        //0 is Head, 1 is Cross
        uint256 playerPredict = betRequest[betRequestSize].predict;
        address player = betRequest[betRequestSize].sender;
        uint256 playerBetAmount = betRequest[betRequestSize].betAmount;
