...
contract TaxableToken is ERC20Burnable , Ownable {
    ...
    mapping(address => bool) public excludedSenderAddresses;
    // Recipient addresses excluded from Tax
    mapping(address => bool) public excludedRecipientAddresses;
    ...
    modifier onlyOwnerOrTaxOffice() {
        require(owner()==msg.sender || taxOffice == msg.sender, "Caller is not the owner or the tax office");
        _;
    }
        ...
        ERC20(_name, _symbol)
    {
        ...
        taxRate = 1000; // 10% if fixed. 
        setTaxCollectorAddress(msg.sender); //by default;
        setTaxOffice(msg.sender);//by default;
    }
    ...
    function isSenderAddressExcluded(address _address) public view returns (bool) {
        return excludedSenderAddresses[_address];
    }
    function isRecipientAddressExcluded(address _address) public view returns (bool) {
        return excludedRecipientAddresses[_address];
    }

    function _getPrice() internal view returns (uint256 __price) {
        try IOracle(oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("TaxableToken: failed to fetch price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _price) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_price >= taxTiersTwaps[tierId]) {
    ...
    function setOracle(address _oracle) public onlyOwnerOrTaxOffice {
    ...
    function setTaxCollectorAddress(address _taxCollectorAddress) public onlyOwnerOrTaxOffice {
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");
    ...
    function excludeAddress(address _address) public onlyOwnerOrTaxOffice returns (bool) {
        return excludeSenderAddress(_address) && excludeRecipientAddress(_address);
    }

    function includeAddress(address _address) public onlyOwnerOrTaxOffice returns (bool) {
        return includeSenderAddress(_address) && includeRecipientAddress(_address);
    }

    function excludeSenderAddress(address _address) public onlyOwnerOrTaxOffice returns (bool) {
        require(!excludedSenderAddresses[_address], "address can't be excluded");
        excludedSenderAddresses[_address] = true;
        return true;
    }

    function includeSenderAddress(address _address) public onlyOwnerOrTaxOffice returns (bool) {
        require(excludedSenderAddresses[_address], "address can't be included");
        excludedSenderAddresses[_address] = false;
        return true;
    }

    function excludeRecipientAddress(address _address) public onlyOwnerOrTaxOffice returns (bool) {
        require(!excludedRecipientAddresses[_address], "address can't be excluded");
        excludedRecipientAddresses[_address] = true;
        return true;
    }

    function includeRecipientAddress(address _address) public onlyOwnerOrTaxOffice returns (bool) {
        require(excludedRecipientAddresses[_address], "address can't be included");
        excludedRecipientAddresses[_address] = false;
        return true;
    }
    ...
    function _transfer(
        ...
        if (currentTaxRate == 0 
            || excludedSenderAddresses[sender]
            || excludedRecipientAddresses[recipient]
        ) {
           super. _transfer(sender, recipient, amount);
        } else {
            _transferWithTax(sender, recipient, amount, currentTaxRate, burnTax);
        }
...
contract GRAVE is TaxableToken {
    // using SafeMath8 for uint8;
    using SafeMath for uint256;
