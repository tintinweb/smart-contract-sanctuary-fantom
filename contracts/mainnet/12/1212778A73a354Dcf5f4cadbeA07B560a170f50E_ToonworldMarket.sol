...
// File: contracts\SkeletoonMarket.sol
...
contract ToonworldMarket is ReentrancyGuard {
    ...
    address public  glyphs;
    address public  skeletoons;
    address public  toonBucks;

    IBucks public immutable toonBucksContract;

    constructor(address _ctraits, address _cglyphs, address _cbucks, address _cskeletoons, address _addressReward, address _uniswapRouter) 
    {
        owner = payable(msg.sender);

        traits = _ctraits;
        glyphs = _cglyphs;
        skeletoons = _cskeletoons;
        toonBucks = _cbucks;

        IBucks _toonBucks = IBucks(_cbucks);        
        toonBucksContract = _toonBucks;

        rewardAddress = _addressReward;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).getPair(_cbucks, _uniswapV2Router.WETH());
    ...
    function createMarketItem(
        uint256 nftContractSelect,
        uint256 tokenId,
        uint256 price) public payable nonReentrant{
         require(price > 10000000000000000, "Price must be above 0.01 Fantom");
        ...
        if (nftContractSelect == 0){
            nftContract = skeletoons;
        } else if (nftContractSelect == 1){
            nftContract = traits;
        } else if (nftContractSelect == 2){
            nftContract = glyphs;
        }
        ...
        function createMarketSale(
           ...
           idMarketItem[itemId].seller.transfer((msg.value/100)*95);
            ...
            swapWFTMForToonBuvk( (msg.value/1000)*40 ,  rewardAddress);
            swapWFTMForToonBuvk( (msg.value/1000)*10 ,  address(this));
            toonBucksContract.burn(address(this), toonBucksContract.balanceOf(address(this)));




            idMarketItem[itemId].owner = payable(msg.sender); //mark buyer as new owner
            idMarketItem[itemId].state = 1; //mark that it has been sold
        ...
        function cancelMarketSale(
            ...
            uint256 itemId
                ...
                uint tokenId = idMarketItem[itemId].tokenId;
                require((msg.sender) == idMarketItem[itemId].seller );
               // require(msg.value == price, "Please submit the asking price in order to complete purchase");
           ...
           //idMarketItem[itemId].seller.transfer(msg.value);
            ...
            IERC721(nftContract).transferFrom(address(this), idMarketItem[itemId].seller, tokenId);

            idMarketItem[itemId].state = 2;
        ...
        function fetchSingle(uint256 itemId) public view returns (MarketItem memory) {
            return idMarketItem[itemId];
        }

        function swapWFTMForToonBuvk(uint256 ethAmount, address _to) private {
