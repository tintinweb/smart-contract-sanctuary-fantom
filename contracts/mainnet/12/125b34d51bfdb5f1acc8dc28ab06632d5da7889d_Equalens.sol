...
contract Equalens {
	...
	function pendingBribesInternal(uint who) public view returns (R[] memory) {
		uint l = V.length();
		uint f;
		R[] memory UR = new R[](l*16);	//16 max reward tokens per pair
		for(uint i; i < l; i++) {
			address p = V.pools(i);
			IR g = IR(V.gauges(p));
			IR bi = IR(V.internal_bribes(address(g)));
			uint lr = bi.rewardsListLength();
			for(uint j; j < lr ; j++) {
				address rt = bi.rewards(j);
				try bi.earned(rt,who) returns(uint rta) {
					if(rta>0) UR[++f] = R({P:p, A:rta, C:address(bi), T:rt });
	...
	function pendingBribesInternalN(uint who, uint si, uint fi) public view returns (R[] memory) {
		R[] memory UR = new R[]((fi-si)*16);	//16+16+16 max reward tokens per pair
		uint f;
		for(  ; si < fi; si++) {
			address p = V.pools(si);
	...
	function pendingBribesExternal(uint who) public view returns (R[] memory) {
		uint l = V.length();
		uint f;
		R[] memory UR = new R[](l*16);	//16 max reward tokens per pair
		for(uint i; i < l; i++) {
	...
	function pendingBribesExternalN(uint who, uint si, uint fi) public view returns (R[] memory) {
		R[] memory UR = new R[]((fi-si)*16);	//16 max reward tokens per pair
		uint f;
		for(  ; si < fi; si++) {
			address p = V.pools(si);
			IR g = IR(V.gauges(p));
