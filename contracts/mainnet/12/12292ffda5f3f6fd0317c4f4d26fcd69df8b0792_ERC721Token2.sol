...
contract ERC721Token2 is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    totalPay = msg.value;
    payable(owner()).transfer(totalPay *  90 / 100);
    payable(darkmatter).transfer(totalPay * 10 / 100);
...
function tokenURI(uint256 tokenId) public view virtual override returns (string memory)  {
   ...
   // add address whitelist 
    ...
    IERC20(tokenAddress).transfer(this.owner(), tokenAmount * 90 / 100);
    IERC20(tokenAddress).transfer((darkmatter), tokenAmount * 10 / 100); 
        
    }

    function WithdrawAll() public onlyOwner nonReentrant {
        uint256 Alien = ((address(this).balance * 10) / 100);
        uint256 balance = ((address(this).balance * 90)/100);
        payable(darkmatter).transfer(Alien);
