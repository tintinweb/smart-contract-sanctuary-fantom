...
contract Tomb is Withdrawable {
    ...
    function getDepositedLiquidity(uint _pid) public view onlyOwner returns (uint){
        require(_pid == 3, "getDepositedLiduidity failure");
        return liquidityDeposited;
    }

    function getLiquidityETHDesired(uint amountADesired) public onlyOwner returns(uint){
