...
contract StrategyBasedTripool is Ownable, Pausable {
    ...
    address public constant TRICRYPTO_POOL = 0x3a1659Ddcf2339Be3aeA159cA010979FB49155FF;

    address public constant SPOOKY_ROUTER = 0xF491e7B69E4244ad4002BC14e878a34207E38c29;

    // tokens
    address public constant TRICRYPTO_TOKEN = 0x58e57cA18B7A47112b877E31929798Cd3D703b0f;

    address public constant TRICRYPTO_GAUGE = 0x00702BbDEaD24C40647f235F15971dB0867F6bdB;
    ...
    constructor(address _vault) public {
        ...
        totalProfit = 0;
        triPoolWant = WETH;
        crvToWantRoute = [CRV, WFTM, WETH];
        bShareToWantRoute = [BSHARE, WFTM, WETH];
        wftmToWantRoute = [WFTM, WETH];
        useBshare = true;
        harvestCrv = true;
        _giveAllowances();
    }
    ...
    function withdrawGauge(uint256 _amount) public ownerOrVault {
        uint256 triBal = balanceOfTripool();
        if (triBal < _amount) {
            IMasterChef(CHEF).withdraw(PID, _amount.sub(triBal));
        }
    }
    ...
    function stake() public whenNotPaused {
        uint256 triBal = balanceOfTripool();
        if (triBal > 0) {
            IRewardsGauge(TRICRYPTO_GAUGE).deposit(triBal, address(this), true);
        }
    }
    ...
    function unstake() public ownerOrVault {
        uint256 gaugeBal = balanceOfGauge();
        if (gaugeBal > 0) {
            IRewardsGauge(TRICRYPTO_GAUGE).withdraw(gaugeBal, true);
        }
    }
    ...
    function withdraw(uint256 _amount) external ownerOrVault {
        ...
        IERC20(TRICRYPTO_TOKEN).safeTransfer(vault, balanceOfTripool());
    ...
    function managerHarvestCrv() public onlyOwner {
        IRewardsGauge(TRICRYPTO_GAUGE).claim_rewards();
    ...
    function addLiquidityTripool() internal {
        uint256 usdtBal = IERC20(USDT).balanceOf(address(this));
        uint256 btcBal = IERC20(WBTC).balanceOf(address(this));
        uint256 ethBal = IERC20(WETH).balanceOf(address(this));
        if (usdtBal > 0 || btcBal > 0 || ethBal > 0) {
            uint256[3] memory amounts = [usdtBal, btcBal, ethBal];
            ICurveSwap3(TRICRYPTO_POOL).add_liquidity(amounts, 0);
        }
    }
    ...
    function balanceOf() public view returns (uint256) {
        return balanceOfTripool().add(balanceOfGauge()).add(balanceOfPool());
    }

    function balanceOfTripool() public view returns (uint256) {
        return IERC20(TRICRYPTO_TOKEN).balanceOf(address(this));
    }

    function balanceOfGauge() public view returns (uint256) {
        return IERC20(TRICRYPTO_GAUGE).balanceOf(address(this));
    }
    ...
    {
        token0 = IERC20(USDT).balanceOf(TRICRYPTO_POOL);
        token1 = IERC20(WBTC).balanceOf(TRICRYPTO_POOL);
        token2 = IERC20(WETH).balanceOf(TRICRYPTO_POOL);

        uint256 totalSupply = IERC20(TRICRYPTO_TOKEN).totalSupply();
    ...
    function balanceInUsdt() public view returns (uint256) {
        return ICurveSwap(TRICRYPTO_POOL).calc_withdraw_one_coin(balanceOf(), 0);
    }

    function profitInUsdt() public view returns (uint256) {
        return ICurveSwap(totalProfit).calc_withdraw_one_coin(balanceOf(), 0);
    }
    ...
    function crvAvailable() public view returns (uint256) {
        return IRewardsGauge(TRICRYPTO_GAUGE).claimable_reward(address(this), CRV);
    }

    function wftmAvailable() public view returns (uint256) {
        return IRewardsGauge(TRICRYPTO_GAUGE).claimable_reward(address(this), WFTM);
    }
    ...
    function setTripoolWant(address _triPoolWant) external onlyOwner {
        require(
            _triPoolWant == WETH || _triPoolWant == USDT || _triPoolWant == WBTC,
            "not tripool want"
        );
        triPoolWant = _triPoolWant;
    }
    ...
    function setBshareToWantRoute(address[] memory _bShareToWantRoute) external onlyOwner {
        ...
        require(
            _bShareToWantRoute[_bShareToWantRoute.length - 1] == triPoolWant,
    ...
    function setCrvToWantRoute(address[] memory _crvToWantRoute) external onlyOwner {
        ...
        require(
            _crvToWantRoute[_crvToWantRoute.length - 1] == triPoolWant,
    ...
    function setWftmToWantRoute(address[] memory _wftmToWantRoute) external onlyOwner {
        ...
        require(
            _wftmToWantRoute[_wftmToWantRoute.length - 1] == triPoolWant,
    ...
    function emergencyUnstake() public onlyOwner {
        uint256 gaugeBal = balanceOfGauge();
        if (gaugeBal > 0) {
            IRewardsGauge(TRICRYPTO_GAUGE).withdraw(gaugeBal);
        }
    }
    ...
    function emergencyStake() external onlyOwner {
        uint256 triBal = balanceOfTripool();
        if (triBal > 0) {
            IRewardsGauge(TRICRYPTO_GAUGE).deposit(triBal);
        }
    }
    ...
    function retireStrat() public ownerOrVault {
        ...
        emergencyUnstake();
        uint256 triBal = balanceOfTripool();
        IERC20(TRICRYPTO_TOKEN).transfer(vault, triBal);
    }
    ...
    function _giveAllowances() internal {
        IERC20(WETH).safeApprove(TRICRYPTO_POOL, uint256(-1));
        IERC20(WBTC).safeApprove(TRICRYPTO_POOL, uint256(-1));
        IERC20(USDT).safeApprove(TRICRYPTO_POOL, uint256(-1));
        ...
        IERC20(TRICRYPTO_TOKEN).safeApprove(TRICRYPTO_GAUGE, 0);
        IERC20(TRICRYPTO_TOKEN).safeApprove(TRICRYPTO_GAUGE, uint256(-1));
        IERC20(TRICRYPTO_GAUGE).safeApprove(CHEF, 0);
        IERC20(TRICRYPTO_GAUGE).safeApprove(CHEF, uint256(-1));
    }

    function _removeAllowances() internal {
        IERC20(WETH).safeApprove(TRICRYPTO_POOL, 0);
        IERC20(WBTC).safeApprove(TRICRYPTO_POOL, 0);
        IERC20(USDT).safeApprove(TRICRYPTO_POOL, 0);
