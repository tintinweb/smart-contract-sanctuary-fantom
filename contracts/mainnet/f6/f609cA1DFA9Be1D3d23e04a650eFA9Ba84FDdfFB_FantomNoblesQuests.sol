...
contract FantomNoblesQuests is ERC1155Drop {

    address _companion;

    modifier onlyCompanion() {
        if (msg.sender != _companion) {
            revert("Not authorized");
        }
        _;
    }
    ...
    function setCompanion(address _newCompanion) external onlyOwner {
        _companion = _newCompanion;
    }

    function mintAlmostNoble(address _receiver) external virtual onlyCompanion{
        _mint(_receiver, 1, 1, "");
    }

    function mintCrossChainNoble(address _receiver) external virtual onlyCompanion {
        _mint(_receiver, 2, 1, "");
    }
