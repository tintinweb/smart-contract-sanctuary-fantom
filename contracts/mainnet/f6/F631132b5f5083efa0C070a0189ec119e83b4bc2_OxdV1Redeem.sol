...
contract OxdV1Redeem is GovernableImplementation, ProxyImplementation {
    ...
    modifier onlyGovernanceOrOperator() {
        require(
            msg.sender == governanceAddress() || operator[msg.sender],
            "Only governance or operator"
        );
        _;
    }
    ...
    function redeem(uint256 amount) external {
        ...
        if (block.timestamp > nextClaimRewardsTimestamp) {
    ...
    function execute(
        ...
        bytes calldata data,
        Operation operation
    ) external onlyGovernanceOrOperator returns (bool success) {
