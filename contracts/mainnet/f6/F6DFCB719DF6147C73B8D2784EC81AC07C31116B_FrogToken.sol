...
contract FrogToken is ERC20 {
    ...
    function setStartingProtections(uint8 _block) external onlyOwner {
        require (_snipeBlockAmt == 0 && _block <= 10 && tradingEnabled);
        _snipeBlockAmt = _block;
    }

    function setSniperProtection(bool value) external onlyOwner {
        sniperProtection = value;
    }
    ...
    function _beforeTransfer(address from, address to) internal returns (bool) {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        if (!tradingEnabled) {
            revert("trading not available yet");
        }
        require(_beforeTransfer(from, to), "sniper");
