...
contract WorkerANT is ERC721, Ownable {
    ...
    function getUnHousedWorkers() public view returns(uint256[] memory) {
        uint256[] memory _workersList = getWorkers();
    ...
    function reduceHP (uint256 _index) public {
        if (idToHealthPoints[_index] > 0){
            ...
            require(playerToWorkers[tx.origin].length > 0 , "index out of bound");
            playerToWorkers[tx.origin][_index] =  playerToWorkers[tx.origin][ playerToWorkers[tx.origin].length - 1];
            playerToWorkers[tx.origin].pop();
            burn(_index);

            uint256[] memory _bbList = IBuildingBlock(buildingblock).getBuildingBlocks();
    ...
    function burn(uint256 _index) public {
        require(playerToWorkers[tx.origin].length > 0 , "index out of bound");
        for (uint i = 0; i < playerToWorkers[tx.origin].length - 1; i++) {
            playerToWorkers[tx.origin][i] = playerToWorkers[tx.origin][i + 1];
        }
        playerToWorkers[tx.origin].pop();
        _burn(_index);
    }
    ...
    function mint() public onlyOwner {        
        ...
        playerToWorkers[tx.origin].push(counter);
        counter++;
    }
