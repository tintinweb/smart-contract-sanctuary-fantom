...
// Join our Telegram community: https://t.me/moonswapfarm
...
contract MoonSwapToken is ERC20("Moon Swap Token", "MOON") {
    ...
    */
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying MOON (not scaled);
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MOON
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;  // Last block number that MOON distribution occurs.
        uint256 accMoonPerShare;   // Accumulated MOON per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The MOON TOKEN!
    MoonSwapToken public moon;
    address public devAddress;
    address public feeAddress;
    // address public vaultAddress;

    // MOON tokens created per block.
    uint256 public moonPerBlock = 0.005 ether;
    ...
    event UpdateEmissionRate(address indexed user, uint256 moonPerBlock);
    ...
    function pendingMoon(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accMoonPerShare = accMoonPerShare.add(moonReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 moonReward = multiplier.mul(moonPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        moon.mint(devAddress, moonReward.div(10));
        moon.mint(address(this), moonReward);
        pool.accMoonPerShare = pool.accMoonPerShare.add(moonReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMoonPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    // Safe MOON transfer function, just in case if rounding error causes pool to not have enough.
    ...
    function updateEmissionRate(uint256 _moonPerBlock) external onlyOwner {
        massUpdatePools();
