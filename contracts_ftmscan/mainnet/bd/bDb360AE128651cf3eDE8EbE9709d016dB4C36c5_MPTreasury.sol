...
contract MPTreasury is Ownable, ReentrancyGuard {
    ...
    uint public constant oneDay = 3600;//3600 * 24;

    // 14 days of mayhem
    // TODO: REVERT TEST VALUE ONLY!!!
    uint public constant phaseOneDuration = 3600 * 2;// oneDay * 14;
    ...
    function liquidateTreasury( uint percentLiquidation ) public {
        ...
        for( uint256 i = 0; i < reserveTokens.length; i++ ) {
            if ( isReserveToken[ reserveTokens[ i ] ] ) {
                if ( reserveTokens[ i ] == stableTokenAddress )
                    continue;
        ...
        // make the swap
        MPSwapRouter.swapETHForExactTokens{value: address(this).balance}(
            0, // accept any amount of tokens
            path,
            address(this),
            block.timestamp
        ...
        IMPERC20( MP ).burnFrom( address(this), MPBurnt );
