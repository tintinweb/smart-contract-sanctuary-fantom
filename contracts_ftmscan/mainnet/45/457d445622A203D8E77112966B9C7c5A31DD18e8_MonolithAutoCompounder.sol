...
interface IBeethovenxMasterChef {
    ...
    function pendingBeets(uint256 _pid, address _user) external returns (uint256 pending);
...
contract MonolithAutoCompounder {
    using SafeERC20 for IERC20;

    address public owner;
    modifier onlyOwner() {
        require(msg.sender == owner, "access policty violation");
        _;
    }

    uint256 public immutable pid = 37;
    IBeethovenxMasterChef public immutable beetsChef = IBeethovenxMasterChef(0x8166994d9ebBe5829EC86Bd81258149B87faCfd3);
    IBeethovenVault public immutable beetsVault = IBeethovenVault(0x20dd72Ed959b6147912C2e529F0a0C651c33c9ce);
    IERC20 public immutable beets = IERC20(0xF24Bcf4d1e507740041C9cFd2DddB29585aDCe1e);
    IERC20 public immutable wftm = IERC20(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83);
    IERC20 public immutable bptmnlt = IERC20(0xa216AA5d67Ef95DdE66246829c5103C7843d1AAB);
    bytes32 public immutable beetsSwapPoolId = 0xcde5a11a4acb4ee4c805352cec57e236bdbc3837000200000000000000000019; // BEETS-wFTM pool
    bytes32 public immutable monolithPoolId = 0xa216aa5d67ef95dde66246829c5103c7843d1aab000100000000000000000112; // MAI-wsEXOD-gOHM-EXOD-wFTM
    ...
    function swapBeetsForwFTM() public {
        uint256 balance = beetsBalance();

        if (balance != 0) {
            approveBeets();

            // swap the beets to wFTM
            beetsVault.swap(
    ...
    function joinPool() public {
        ...
        if (balance != 0) {
            ...
            // deposit the wFTM back into the pool
            beetsVault.joinPool(
                monolithPoolId,
                address(this),
                address(this),
    ...
    function exitPool() public {
        ...
        beetsVault.exitPool(
            ...
            IBeethovenVault.ExitPoolRequest({
                assets: poolAssets,
    ...
    function farmBeets() public {
        uint256 balance = lpBalance();

        if (balance != 0) {
            approveLP();

            beetsChef.deposit(pid, balance, address(this));
        }
    }

    function unfarmBeets() public {
        IBeethovenxMasterChef.UserInfo memory info = beetsChef.userInfo(pid, address(this));

        if (info.amount != 0) {
            beetsChef.withdrawAndHarvest(pid, info.amount, address(this));
        }
    }
