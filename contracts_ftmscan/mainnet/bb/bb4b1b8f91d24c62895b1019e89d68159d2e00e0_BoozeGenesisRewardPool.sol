...
contract BoozeGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accboozePerShare; // Accumulated PLAT per share, times 1e18. See below.
    ...
    // TESTNET
    //uint256 public boozePerSecond = 5.5555555 ether; // 20000 booze / (1h * 60min * 60s)
    //uint256 public runningTime = 1 hours; // 1 hours
    //uint256 public constant TOTAL_REWARDS = 20000 ether;
    // END TESTNET

    // MAINNET
     uint256 public boozePerSecond = 0.2314814 ether; // 20000 booze / (24h * 60min * 60s) 0.2314814
    ...
    function pendingbooze(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accboozePerShare = accboozePerShare.add(_platReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _platReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accboozePerShare = pool.accboozePerShare.add(_platReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
