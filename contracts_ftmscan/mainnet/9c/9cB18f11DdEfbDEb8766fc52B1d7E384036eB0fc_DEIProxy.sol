...
contract DEIProxy is Ownable {
	...
	address[] public usdc2deusPath;
	address[] public deus2deiPath;
	address[] public usdc2deiPath;
	...
	constructor(
		...
		address[] memory _usdc2deusPath,
		address[] memory _deus2deiPath,
		address[] memory _usdc2deiPath
	) {
		...
		usdc2deusPath = _usdc2deusPath;
		deus2deiPath = _deus2deiPath;
		usdc2deiPath = _usdc2deiPath;
	...
	function USDC2DEI(ProxyInput memory proxyInput) external returns (uint deiAmount) {
		...
		// buy needed deus
		uint usdcNeededAmount = IUniswapV2Router02(uniswapRouter).getAmountsIn(proxyInput.deusNeededAmount, usdc2deusPath)[0];
		IUniswapV2Router02(uniswapRouter).swapTokensForExactTokens(proxyInput.deusNeededAmount, usdcNeededAmount, usdc2deusPath, address(this), deadline);
		...
		if (residualDeus > 0) {
			uint residualDei = IUniswapV2Router02(uniswapRouter).swapExactTokensForTokens(residualDeus, 0, deus2deiPath, address(this), deadline)[deus2deiPath.length -1];
			deiAmount += residualDei;
		}else{
			// swap residual usdc to dei
			uint residualUsdc = IERC20(usdcAddress).balanceOf(address(this));
			if (residualUsdc > 0) {
				uint residualDei = IUniswapV2Router02(uniswapRouter).swapExactTokensForTokens(residualUsdc, 0, usdc2deiPath, address(this), deadline)[usdc2deiPath.length -1];
				deiAmount += residualDei;
			}
		}
	...
	function ERC202DEI(ProxyInput memory proxyInput, address[] memory path) external returns (uint deiAmount) {
		...
		if (IERC20(path[0]).allowance(address(this), uniswapRouter) < proxyInput.amountIn) {IERC20(path[0]).approve(uniswapRouter, type(uint).max);}
		...
		if (residualDeus > 0) {
			...
			uint residualUsdc = IERC20(usdcAddress).balanceOf(address(this));
			if (residualUsdc > 0) {
				uint residualDei = IUniswapV2Router02(uniswapRouter).swapExactTokensForTokens(residualDeus, 0, usdc2deiPath, address(this), deadline)[usdc2deiPath.length -1];
				deiAmount += residualDei;
			}
		}
	...
	function getAmountsOut(uint amountIn, uint deusPriceUSD, uint colPriceUSD, address[] memory path) public view returns (uint amountOut, uint usdcForMintAmount, uint deusNeededAmount) {
		...
		uint usdcNeededAmount = IUniswapV2Router02(uniswapRouter).getAmountsIn(deusNeededAmount, usdc2deusPath)[0];
		...
		uint deiAmount;
		if(deusUsedAmount > deusNeededAmount){
			uint newDeusUsedAmount;
			usdcNeededAmount = IUniswapV2Router02(uniswapRouter).getAmountsIn(deusUsedAmount, usdc2deusPath)[0];
			usdcForMintAmount = amountIn - usdcNeededAmount;
			(amountOut, newDeusUsedAmount) = calcMintFractionalDEI(MintFD_Params(deusPriceUSD, colPriceUSD, usdcForMintAmount * collateralMissingDecimalsD18, globalCollateralRatio));
			uint deusExtraAmount = deusUsedAmount - newDeusUsedAmount;
			deusNeededAmount = deusUsedAmount;
			deiAmount = IUniswapV2Router02(uniswapRouter).getAmountsOut(deusExtraAmount, deus2deiPath)[deus2deiPath.length-1];
		}else{
			usdcNeededAmount = IUniswapV2Router02(uniswapRouter).getAmountsIn(deusUsedAmount, usdc2deusPath)[0];
			uint usdcExtraAmount = amountIn - (usdcForMintAmount + usdcNeededAmount);
			deiAmount = IUniswapV2Router02(uniswapRouter).getAmountsOut(usdcExtraAmount, usdc2deiPath)[usdc2deiPath.length - 1];
			deusNeededAmount = deusUsedAmount;
		}
