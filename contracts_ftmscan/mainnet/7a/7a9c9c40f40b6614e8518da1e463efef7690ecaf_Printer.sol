...
contract Printer is IBEP20, Auth {
    ...
    PrinterDistributor distributor;
    ...
    constructor (address distributorAddress) Auth(msg.sender) {
        ...
        distributor = PrinterDistributor(distributorAddress);
        router = IPancakeRouter02(_router);
        pair = IPancakeFactory(router.factory()).createPair(WFTM, address(this));
    ...
    function setPair(address pairAddress) external onlyOwner { // remove me local testing
        pair = pairAddress;
    } 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {         
        ...
        // take fees
        if (isSell) {
            amountAfterFees = takeFee(sender, amount);
        }
        ...
        if (!isDividendProcessorExempt[recipient]) {
    ...
    function setFeePercentages(uint256 newMarketingFeePercent, uint256 newReflectionFeePercent, uint256 newPercentageMultiplyer) external authorized {
    ...
    function setIsDividendProcessorExempt(address holder, bool exempt) external authorized {
    ...
    function checkFree(address holder) external view returns(bool) {
