...
contract Archery is Context, IERC20, Ownable {
    ...
    uint256 public MAX_TAX_FEE;
    uint256 public MAX_LIQUIDITY_FEE;
    uint256 public MAX_MARKETING_FEE;
    uint256 public MIN_MAX_TX_AMOUNT;
    uint256 private _tTotal;
    uint256 private _rTotal;
    uint256 private _tFeeTotal;

    string private constant _name = "Archery";
    ...
    constructor (uint256 _supply, uint256 _txFee, uint256 _lpFee, uint256 _mFee, uint256 _MAXAMOUNT, uint256 _SELLMAXAMOUNT, address routerAddress, address tokenOwner, address _marketingAddress) public {
        ...
        MAX_MARKETING_FEE = _marketingFee;
        MAX_LIQUIDITY_FEE = _lpFee;
        MAX_TAX_FEE = _txFee;
        _maxTxAmount = _MAXAMOUNT * 10 ** _decimals;
        MIN_MAX_TX_AMOUNT = _maxTxAmount;
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= MAX_TAX_FEE,"Tax fee can`t be more than MAX TAX FEE");
        _taxFee = taxFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= MAX_LIQUIDITY_FEE,"Liqudity fee can`t be more than MAX LIQUIDITY FEE");
        _liquidityFee = liquidityFee;
    }


     function setMarketingFeePercent(uint256 marketingFee) external onlyOwner() {
        require(marketingFee <= MAX_MARKETING_FEE,"Marketing fee can`t be more than MAX MARKETING FEE");
    ...
    function setMaxTxPercent(uint256 maxTxPercent) public onlyOwner {
        require(maxTxPercent * 10 ** _decimals >= MIN_MAX_TX_AMOUNT,"Max tx amount cant be less than MIN MAX TX AMOUNT");
