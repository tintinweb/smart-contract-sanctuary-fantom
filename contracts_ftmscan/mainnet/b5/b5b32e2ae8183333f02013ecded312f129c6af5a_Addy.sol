...
contract Addy is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the ADDY ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ADDY", "ADDY") {
        // Mints 1 ADDY to contract creator for initial pool setup
    ...
    function _getAddyPrice() internal view returns (uint256 _addyPrice) {
        try IOracle(addyOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Addy: failed to fetch ADDY price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _addyPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_addyPrice >= taxTiersTwaps[tierId]) {
    ...
    function setAddyOracle(address _addyOracle) public onlyOperatorOrTaxOffice {
        require(_addyOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ADDY to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ADDY to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentAddyPrice = _getAddyPrice();
            currentTaxRate = _updateTaxRate(currentAddyPrice);
            if (currentAddyPrice < burnThreshold) {
                burnTax = true;
            }
        }
