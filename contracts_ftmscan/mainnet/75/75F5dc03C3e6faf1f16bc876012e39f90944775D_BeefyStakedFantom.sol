...
contract BeefyStakedFantom is ERC20Upgradeable, ReentrancyGuardUpgradeable, LockedAssetManager {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMathUpgradeable for uint256;

    // Important Variables
    address public want;
    ISRC public stakingContract;
    ...
    function initialize(
        ...
        want = _want;
        stakingContract = ISRC(_stakingContract);

        withdrawEnabled = false;
        withdrawalPeriodTime = 60 * 60 * 24 * 7;
    ...
    function depositAll() external {
        uint256 _amount = IERC20Upgradeable(want).balanceOf(msg.sender);
        IERC20Upgradeable(want).safeTransferFrom(msg.sender, address(this), _amount);
        IWrappedNative(want).withdraw(_amount);
    ...
    function _deposit(address _user, uint256 _amount) internal nonReentrant whenNotPaused {
        if (_amount > 0) {
            ...
            if (balanceOfLocked() > 0) {
                if (validatorUnlockTime() > currentUnlockTime()) {
                    stakingContract.relockStake(validatorID, lockTime(), _amount);
                } else {
                    uint256 lock = currentUnlockTime().sub(block.timestamp);
                    stakingContract.relockStake(validatorID, lock, _amount);
                }
            } else {
    ...
    function withdraw(uint256 _amount) external {
        require (withdrawEnabled == true, "Withdraw is not enabled");
    ...
    function currentUnlockTime() public view returns (uint256 time) {
    ...
    function lockTime() internal view returns (uint256) {
        return validatorUnlockTime().sub(block.timestamp);
    }
    ...
    function enableWithdraw() external onlyOwner {
        _pause();
        uint256 wrAmount = balanceOfLocked();
        stakingContract.unlockStake(validatorID, wrAmount);
        stakingContract.undelegate(validatorID, wrID, wrAmount);
    ...
    function claimStakingReward() external onlyRewardPool {
        ...
        IWrappedNative(want).deposit{value: _balance}();
        IERC20Upgradeable(want).safeTransfer(msg.sender, _balance);
...
contract LockedAssetManager is Initializable, OwnableUpgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
