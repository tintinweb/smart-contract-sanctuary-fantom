...
contract RaidRewards is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public claimedBasic; //round => id => value
    mapping(uint256 => mapping(uint256 => uint256)) public claimedWinnings;
    mapping(uint256 => mapping(uint256 => uint256)) public claimedLoot;

    mapping(uint256 => mapping(address => uint256)) public addressRewards;
    mapping(uint256 => mapping(address => uint256)) public addressClaimed;
    ...
    function claim(uint256 _raiderID, uint256 _gameRound) internal {
        ...
        uint256 claimableBasic = basicRewards - claimedBasic[_gameRound][_raiderID];
        uint256 claimableWinnings = winnings - claimedWinnings[_gameRound][_raiderID];
        uint256 claimableGold = gold - claimedLoot[_gameRound][_raiderID];

        claimedBasic[_gameRound][_raiderID] += claimableBasic;
        claimedBasic[_gameRound][_raiderID] += claimableWinnings;
