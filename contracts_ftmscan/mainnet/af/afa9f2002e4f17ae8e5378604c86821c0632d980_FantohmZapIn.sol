...
contract FantohmZapIn is ZapInBaseV3_1 {
   ...
   constructor(address _DAO)
    ...
    function _fillQuote1(
        address _fromTokenAddress,
        address _toTokenAddress,
        uint256 _amount,
        address _swapTarget,
        bytes calldata swapData
    ) public returns (uint256 amountBought) {
        if (_swapTarget == wftmTokenAddress) {
            IWETH(wftmTokenAddress).deposit{ value: _amount }();
            return _amount;
        }
    ...
    function _fillQuoteLp1(
        address _fromTokenAddress,
        address _pairAddress,
        uint256 _amount,
        address _swapTarget,
        bytes calldata swapData
    ) public returns (uint256 amountBought, address intermediateToken) {
        if (_swapTarget == wftmTokenAddress) {
            IWETH(wftmTokenAddress).deposit{ value: _amount }();
            return (_amount, wftmTokenAddress);
        }
