...
contract XodStaking is Ownable {
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( IMemo( sXOD ).gonsForBalance( _amount ) ),
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            IWarmup( warmupContract ).retrieve( _recipient, IMemo( sXOD ).balanceForGons( info.gons ) );
        }
    }
    ...
    function forfeit() external {
        ...
        IWarmup( warmupContract ).retrieve( address(this), IMemo( sXOD ).balanceForGons( info.gons ) );
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            IMemo( sXOD ).rebase( epoch.distribute, epoch.number );
            ...
            uint staked = IMemo( sXOD ).circulatingSupply();
