...
contract TokenSwap is GelatoTask, TokenHandler {
    ...
    function swap(address wallet, address router, address[] calldata path) external validGasPrice {
        address token = path[0];
        uint amount = _transferableAmount(wallet, token);
        require(amount > 0, "nothing to swap");
        if (amount > 0) {
            IERC20(token).safeTransferFrom(wallet, address(this), amount);
            _approveMaxIfNecessary(token, router, amount);
            IRouter(router).swapExactTokensForTokens(amount, 0, path, wallet, block.timestamp);
        }
    }
}
