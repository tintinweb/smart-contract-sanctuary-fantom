...
// File: contracts/Fantom Heaven.sol
...
contract FantomHeaven is ERC721Enumerable, Ownable, RandomlyAssigned {
  using Strings for uint256;
  
  string public baseExtension = ".json";
  uint256 public cost = 10 ether; 
  uint256 public maxFTH = 777; 
  uint256 public maxFTHMintAmount = 10; 
  bool public paused = false;
  
  string public baseURI = "https://ipfs.io/ipfs/QmfQKcqspLU5cx87DwtL6jKEcRQZaGyq9jvgy9DsCLQcV3/";
  
  constructor(
  ) ERC721("FantomHeaven", "FTH")
  RandomlyAssigned(777, 1) {}
  ...
  // public
    function mintFTHS(uint256 _FTHMintAmount) public payable {
    require(!paused);
    require(_FTHMintAmount > 0);
    require(_FTHMintAmount <= maxFTHMintAmount);
    require(totalSupply() + _FTHMintAmount <= maxFTH);
    require(msg.value >= cost * _FTHMintAmount);

    for (uint256 i = 1; i <= _FTHMintAmount; i++) {
        uint256 mintIndex = nextToken();
     if (totalSupply() < maxFTH) {
