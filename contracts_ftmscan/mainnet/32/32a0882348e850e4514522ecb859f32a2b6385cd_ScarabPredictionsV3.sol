...
contract ScarabPredictionsV3 {
  ...
  /* New */
  uint public actualMarketParticipantId = 0;
  mapping(uint => address) public marketParticipants;
  uint[] public marketParticipantsArrayId;
  /* New */
  ...
  */
  function placeBet(Option _option) external payable {
    ...
    /* New */
    marketParticipants[actualMarketParticipantId] = msg.sender;
    marketParticipantsArrayId.push(actualMarketParticipantId);
    actualMarketParticipantId++;
    /* New */
  ...
  */
  function restartMarket() external onlyOperator {
    ...
    /* NEW */
    distributeRewards();
    actualMarketParticipantId = 0;
    delete marketParticipantsArrayId;
    /* NEW */
  ...
  */
  function calculateWinnings(uint _marketId, address _user) public returns (uint winnings) {
    winnings = 0;
    ...
    //users[msg.sender].totalAmountToClaim += winnings; /* New */
    }
    return winnings;
  }
  ...
  /*NEW*/
    function distributeRewards() internal {
        // Set funds for people
        for (uint i = 0; i < marketParticipantsArrayId.length; i++) {
            uint idToCheck = marketParticipantsArrayId[i];
            address participantUserAddress = marketParticipants[idToCheck];

            Market storage m = markets[marketCount-1];
            uint winningBet = m.users[participantUserAddress].amountStaked[m.winningOption];
            uint winnings = calculateWinnings(marketCount-1, participantUserAddress);

            if(winningBet != 0 && winnings == winningBet) {
              winnings = winningBet.add(m.users[participantUserAddress].comission);
              commissionAmount = commissionAmount.sub(m.users[participantUserAddress].comission);
            }
            if(m.totalBets[m.winningOption] == 0 && m.amountStaked[participantUserAddress] != 0 && winningBet == 0){
                winnings = winnings + m.amountStaked[participantUserAddress];
                winnings = winnings.add(m.users[participantUserAddress].comission);
                commissionAmount = commissionAmount.sub(m.users[participantUserAddress].comission);
            }

            users[participantUserAddress].totalAmountToClaim = users[participantUserAddress].totalAmountToClaim + winnings;
            
            m.users[msg.sender].claimedWinnings = true;
            m.users[participantUserAddress].amountStaked[m.winningOption] = 0;  
        }
    }
  /*NEW*/
  ...
  */
  function calculateAllWinnings(address user) external view returns (uint winnings) { /* New */
    winnings = users[user].totalAmountToClaim;
    return winnings;
  }
  ...
  * @param _marketId The id of the prediction market instance on which this function is called.
  */
  function withdrawAllWinnings() external { /* NEW */
      require(users[msg.sender].totalAmountToClaim > 0, 'You have no winnings to claim');
