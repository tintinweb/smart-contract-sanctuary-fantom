...
contract Bomkis is ERC721Enumerable, Ownable, RandomlyAssigned {
  using Strings for uint256;
  
  string public baseExtension = ".json";
  uint256 public cost = 0.02 ether; 
  uint256 public maxBNO = 45; 
  uint256 public maxBNOMintAmount = 10; 
  ...
  // public
    function mintBNOS(uint256 _BNOMintAmount) public payable {
    require(!paused);
    require(_BNOMintAmount > 0);
    require(_BNOMintAmount <= maxBNOMintAmount);
    require(totalSupply() + _BNOMintAmount <= maxBNO);
    require(msg.value >= cost * _BNOMintAmount);

    for (uint256 i = 1; i <= _BNOMintAmount; i++) {
        uint256 mintIndex = nextToken();
     if (totalSupply() < maxBNO) {
