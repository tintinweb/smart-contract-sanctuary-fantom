...
import "../lib/RivalsRVL2Constants.sol";
import "../lib/RivalsDbConstants.sol";
import "../lib/RivalsMinterConstants.sol";
...
import "../../cryptoblock/ICryptoBlocksMintable.sol";
import "../../cryptoblock/minters/CryptoBlocksMinterUpgradeable.sol";
import "../../cryptoblock/currencyblocks/lib/CurrencyBlocksConstants.sol";

contract RivalsRVL2Minter is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, CryptoBlocksMinterUpgradeable {
    ...
    address[] internal _adminAddress;                       // Admin(s) address
    uint256[] internal _adminAmount;                        // Admin(s) amount / 1000. ex: 1 = 0.1%
    address private _rivals;
    address private _rivalsRVL1;
    ...
    function initialize() initializer public {
        ...
        __RivalsRVL2Minter_init_unchained();
    }

    function __RivalsRVL2Minter_init_unchained() internal initializer {
        maxSupply = 1200;
        price =  40 ether;
        _dividendsAmount = 0;
        _referralAmount = 0;
        _adminAddress.push(_msgSender());
        _adminAmount.push(500);
        _pause();
    }

    function _authorizeUpgrade(address) internal override onlyRole(AccessControlConstants.ADMIN_ROLE) {}

    function setAdminAddress(uint256 index_, address address_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _adminAddress[index_] = address_;
    }

    function getAdminAddress(uint256 index_) external view returns(address) {
        return _adminAddress[index_];
    }

    function setAdminAmount(uint256 index_, uint256 amount_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _adminAmount[index_] = amount_;
    }

    function getAdminAmount(uint256 index_) external view returns(uint256) {
        return _adminAmount[index_];
    }

    function addAdmin(address account_, uint256 amount_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        // make sure it doesn't exist
        bool exist = false;
        for(uint256 i = 0; i < _adminAddress.length; i++) {
            if(_adminAddress[i] == account_) {
                exist = true;
            }
        }
        if(!exist) {
            _adminAddress.push(account_);
            _adminAmount.push(amount_);
        }
    }

    function removeAdmin(address account_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        uint256 index;
        for(uint256 i = 0; i < _adminAddress.length; i++) {
            if(account_ == _adminAddress[i]) {
                index = i;
            }
        }

        uint256 lastIndex = _adminAddress.length - 1;
        address lastAddress = _adminAddress[lastIndex];
        uint256 lastAmount = _adminAmount[lastIndex];
        _adminAddress[index] = lastAddress;
        _adminAmount[index] = lastAmount;
        _adminAddress.pop();
        _adminAmount.pop();
    }

    function withdraw() external override onlyRole(AccessControlConstants.WITHDRAW_ROLE) {
        uint256 totalBalance = address(this).balance;
        for(uint256 i; i < _adminAddress.length; i++) {
            uint256 balance = (totalBalance * _adminAmount[i]) / 1000;
            if(balance > address(this).balance) {
                balance = address(this).balance;
            }
            (bool sent,) = payable(_adminAddress[i]).call{value: balance}("");
            require(sent, "Failed to send balance");
        }
    }

    /**
     * @dev Emergency withdraw in case withdraw fails
     */
    function emergencyWithdraw() external onlyRole(AccessControlConstants.ADMIN_ROLE) {
     ...
     */
    function buy(uint256 amount_, address) external payable whenNotPaused nonReentrant {
        require(msg.value >= price * amount_, "Insufficient fee paid");

        uint256 rareBoost = _getRareBoost(_msgSender());
        for (uint256 i; i < amount_; i++) {
            safeMint(_msgSender(), rareBoost);
        }
    }

    function safeMint(address to_, uint256 rareBoost_) internal {
        require(IERC721EnumerableUpgradeable(_rivals).totalSupply() <= maxSupply, "Insufficient supply");
        uint256 id_ = ICryptoBlocksMintable(_rivals).mint(to_);
        _setAttributes(id_, to_, rareBoost_);
    }

    /**
     * @dev Buy using special tickets to get discount, class, element, and badge
     */
    function mintWithTicket(uint256 amount_, uint256 discountTicketId_) external payable whenNotPaused nonReentrant {
        require(amount_ > 0 && discountTicketId_ == CurrencyBlocksConstants.ID_RIVALS_RVL2_MINT_TICKET, "A");
        IERC1155BurnableUpgradeable(_currencyBlocks).burn(msg.sender, discountTicketId_, amount_);
        uint256 rareBoost = _getRareBoost(msg.sender);
        for(uint256 i; i < amount_; i++) {
            safeMint(msg.sender, rareBoost);
        }
    }
     ...
     */
    function freeMint(address to_, uint256 amount_, bool rareBoostOn) external onlyRole(AccessControlConstants.MINTER_ROLE) {
        uint256 rareBoost = _getRareBoost(to_);
        if(!rareBoostOn) {
            rareBoost = 0;
        }
        for(uint256 i; i < amount_; i++) {
            safeMint(to_, rareBoost);
        }
    }
     ...
     */
    function extraMint(address to_, uint256 amount_) external onlyRole(AccessControlConstants.MINTER_ROLE) {
        require((maxSupply + amount_) <= type(uint256).max, "Max supply at limit");
        uint256 rareBoost = _getRareBoost(to_);
        for(uint256 i; i < amount_; i++) {
            uint256 id_ = ICryptoBlocksMintable(_rivals).mint(to_);
            _setAttributes(id_, to_, rareBoost);
            maxSupply++;
        }
    }

    function _getRareBoost(address to_) internal view returns(uint256) {
        IERC721EnumerableUpgradeable rivalsRVL1 = IERC721EnumerableUpgradeable(_rivalsRVL1);
        ICryptoBlocks rivals = ICryptoBlocks(_rivalsRVL1);
        uint256 boost;
        uint256 balance = IERC721Upgradeable(rivalsRVL1).balanceOf(to_);
        for(uint256 i; i < balance; i++) {
            uint256 tokenId = rivalsRVL1.tokenOfOwnerByIndex(to_, i);
            if(to_ == rivals.getBlockAttributeAddress(tokenId, RivalsConstants.ADDRESS_MINTER)) {
                boost += 3;
                if(boost >= 12) {
                    break;
                }
     ...
     * @dev Set attributes
     */
    function _setAttributes(uint256 id_, address to_, uint256 rareBoost_) internal {
        ICryptoBlocksAttributes rivals = ICryptoBlocksAttributes(_rivals);
        rivals.setBlockAttributeAddress(id_, RivalsConstants.ADDRESS_MINTER, to_);
        rivals.setBlockAttribute(id_, RivalsConstants.UINT_DBID, id_ + 800);
        rivals.setBlockAttribute(id_, RivalsConstants.UINT_LEVEL, 1);
        rivals.setBlockAttributeString(id_, RivalsConstants.STRING_NAME, string(abi.encodePacked("RVL2",id_.toString())));

        // set unique values
        rivals.setBlockAttribute(id_, RivalsConstants.UINT_CP_UV, _random(1 + rareBoost_ * 5, 100));
        uint256 classId = _random(1, 2);
        uint256 rarity = _random(1, 100);
        rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_CLASS, classId);
        // bg
        if(rarity > 10 + rareBoost_) {
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BG, _random(1, 7));
        } else {
            // rare bg
            if(classId == 1) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BG, _random(8, 9));
            } else {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BG, _random(10, 11));
            }
            rivals.setBlockAttribute(id_, RivalsConstants.UINT_RARITY, 1);
        }
        // body
        uint256 random = _random(1, 100);
        // 90%
        if(random > 10) {
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BODY, _random(1, 4));
        } else if(random >= 7) {
            // black 4%
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BODY, 5);
        } else if(random >= 3) {
            // white 4%
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BODY, 6);
        } else {
            // blue 2%
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BODY, 7);
        }
        // eyes
        random = _random(1, 100);
        // 90%
        if(random > 10) {
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_EYES, _random(1, 3));
        } else {
            // special 10%
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_EYES, 4);
        }
        // mouth
        rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_MOUTH, _random(1, 4));
        ...
        if(classId == 1) {
            ...
            random = _random(1, 100);
            if(random <= 20) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_MOLE, 1);
            }
            // color for facial hair and hair
            uint256 color = _random(1, 3);

            // goatee 30%
            random = _random(1, 100);
            if(random <= 40) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_GOATEE, color);
            }
            // mustache 30%
            random = _random(1, 100);
            if(random <= 30) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_MUSTACHE, color);
            }
            // pipe 10%
            random = _random(1, 100);
            if(random <= 10) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_PIPE, 1);
            }
            // head
            // hair
            uint256 hair = _random(1, 100);
            // hair 50%
            if(hair > 50) {
                hair = _random(1, 4);
                if(hair != 4) {
                    rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_HAIR, hair * color);
                } else {
                    rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_HAIR, 4);
                }
                // crown
                random = _random(1, 100);
                if(random <= 20) {
                    rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_CROWN, 1);
                }
            } else {
                // helmet
                random = _random(0, 9);
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_HELMET, random);
                if(random == 0) {
                    // crown
                    random = _random(1, 100);
                    if(random <= 20) {
            ...
            // clothes
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_CLOTHES, _random(0, 10));
            // weapon
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_WEAPON, _random(0, 4));
            // back
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BACK, _random(0, 1));
        } else {
            // face
            // battle paint 30%
            random = _random(1, 100);
            if(random <= 30) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BATTLE_PAINT, 1);
            }
            // nose ring 30%
            random = _random(1, 100);
            if(random <= 30) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_NOSE_RING, 1);
            }
            // eye patch 30%
            random = _random(1, 100);
            if(random <= 30) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_EYE_PATCH, 1);
            }
            // tattoo 30%
            random = _random(1, 100);
            if(random <= 30) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_TATTOO, 1);
            }
            // mud 30%
            random = _random(1, 100);
            if(random <= 30) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_MUD, 1);
            }
            // beard
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BEARD, _random(0, 7));
            // necklace 10%
            random = _random(1, 100);
            if(random <= 10) {
                rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_NECKLACE, 1);
            }
            // head
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_HEAD, _random(0, 17));
            // clothes
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_CLOTHES, _random(0, 11));
            // weapon
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_WEAPON, _random(0, 3));
            // back
            rivals.setBlockAttribute(id_, RivalsRVL2Constants.UINT_BACK, _random(0, 2));
        }
    }

    function setRivals(address rivals_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _rivals = rivals_;
    }

    function getRivals() external view returns(address) {
        return _rivals;
    }

    function setRivalsRVL1(address rivalsRVL1_) external onlyRole(AccessControlConstants.ADMIN_ROLE) {
        _rivalsRVL1 = rivalsRVL1_;
    }

    function getRivalsRVL1() external view returns(address) {
        return _rivalsRVL1;
    }
    ...
    function _random(uint256 min_, uint256 max_) internal view returns (uint256) {
        max_++;
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, _msgSender(), gasleft()))) % (max_ - min_);
...
library RivalsRVL2Constants {
    /**
     * Attribute Integer IDs
     */
    uint256 public constant UINT_CLASS = 200;
    uint256 public constant UINT_BG = 201;
    uint256 public constant UINT_BODY = 202;
    uint256 public constant UINT_MOLE = 203;
    uint256 public constant UINT_GOATEE = 204;
    uint256 public constant UINT_MUSTACHE = 205;
    uint256 public constant UINT_PIPE = 206;
    uint256 public constant UINT_BATTLE_PAINT = 207;
    uint256 public constant UINT_NOSE_RING = 208;
    uint256 public constant UINT_EYE_PATCH = 209;
    uint256 public constant UINT_TATTOO = 210;
    uint256 public constant UINT_MUD = 211;
    uint256 public constant UINT_BEARD = 212;
    uint256 public constant UINT_EYES = 213;
    uint256 public constant UINT_MOUTH = 214;
    uint256 public constant UINT_HAIR = 215;
    uint256 public constant UINT_HELMET = 216;
    uint256 public constant UINT_CROWN = 217;
    uint256 public constant UINT_HEAD = 218;
    uint256 public constant UINT_CLOTHES = 219;
    uint256 public constant UINT_WEAPON = 220;
    uint256 public constant UINT_BACK = 221;
    uint256 public constant UINT_NECKLACE = 222;
}
...
library RivalsMinterConstants {
    ...
    uint256 public constant RANDOM_MAX_EYES = 11;
    uint256 public constant RANDOM_MAX_MOUTH = 12;
    uint256 public constant RANDOM_MAX_HAIR = 13;
    uint256 public constant RANDOM_MAX_HELMET = 14;
    uint256 public constant RANDOM_MAX_CLOTHES = 15;
    uint256 public constant RANDOM_MAX_BODY = 16;
    uint256 public constant RANDOM_MAX_WEAPON = 17;
    uint256 public constant RANDOM_MAX_BACK = 18;
    uint256 public constant RANDOM_MAX_NECKLACE = 19;
}
...
library RivalsDbConstants {
     ...
     */
    uint256 public constant STRING_TOKEN_IMG_URI = 100;
    uint256 public constant STRING_TOKEN_IMG_URL_SUFFIX = 110;
}
