...
import "./EditableERC20.sol";

// escrowed Qi Time (eQiTime)
...
// Update Balance so it makes it a % of the maxlock, so we can change the max lock
// max lock needs to be synced with X number of seconds.
// let the update contract be given to another oracle contract later.

contract eQiTime is ReentrancyGuard, EditableERC20("escrowed Qi", "eQi", 18) {
    ...
    uint8 multiplier = 3;
    uint256 maxLock = 126144000; // 4 years in seconds
    ...
    event Enter(address user, uint256 amount, uint256 endTime);
    event Leave(address user, uint256 amount, uint256 endTime);
    ...
    function enter(uint256 _amount, uint256 _amountSeconds) public nonReentrant {

        if(userInfo[msg.sender].endTime == 0 || userInfo[msg.sender].endTime <= block.timestamp){
            userInfo[msg.sender].endTime = block.timestamp;
        }

        require(
                userInfo[msg.sender].endTime.add(_amountSeconds) <= maxLock.add(block.timestamp), 
                    "enter: timeStamp cannot be more than 4 years from now.");
        ...
        if(_amountSeconds != 0){
            userInfo[msg.sender].endTime = userInfo[msg.sender].endTime.add(_amountSeconds);
        }

        emit Enter(msg.sender, userInfo[msg.sender].amount, userInfo[msg.sender].endTime);
    }

    function leave() public nonReentrant {
        require(userInfo[msg.sender].endTime <= block.timestamp, "leave: tokens are still locked.");

        Qi.safeTransfer(msg.sender, userInfo[msg.sender].amount);

        emit Leave(msg.sender, userInfo[msg.sender].amount, userInfo[msg.sender].endTime);

        delete userInfo[msg.sender].amount;
        delete userInfo[msg.sender].endTime;
    }

    function endTime() public view returns (uint256) {
        return userInfo[msg.sender].endTime;
    }

    function currentMultiplier(address user) public view returns (uint256) {
        return userInfo[user].endTime.sub(block.timestamp).mul(userInfo[user].amount).mul(multiplier).mul(1000);
    }

    function balanceOf(address user) public view returns (uint256){
        // it starts as *3+1 for a 4x-max boost
        if (userInfo[user].endTime <= block.timestamp || userInfo[user].amount == 0){
    ...
    function timeLeft(address user) public view returns (uint256) {
        if (userInfo[user].endTime <= block.timestamp) {
            return 0;
        } else{
            return userInfo[user].endTime.sub(block.timestamp);
        }
    }
...
contract EditableERC20 is Ownable, ERC20, ERC20Detailed {
    ...
    constructor (string memory _tempName, string memory _tempSymbol, uint8 _decimals) ERC20Detailed(_tempName, _tempSymbol, _decimals) public {
        _name = _tempName;
        _symbol = _tempSymbol;
    }
