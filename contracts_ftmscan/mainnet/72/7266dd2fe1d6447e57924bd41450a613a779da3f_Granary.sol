...
interface ITVL {
	//Using Version = 6
	function p_lpt_tt_coin_usd(address, address, address) external view returns(uint256);
}
...
contract Granary
{
	...
	constructor (address _w, address _m, address _e, uint8 _p, address _R, address[] memory _r, string memory _id, address _v, address _tvlp)
	{
		want=IERC20(_w);
		mc=IMasterchef(_m);
		earn=IERC20(_e);
	...
	*/
	...
	uint256[7] public allnums = [
		...
		1e15,//mw		4       config, default 1 (near zero)
