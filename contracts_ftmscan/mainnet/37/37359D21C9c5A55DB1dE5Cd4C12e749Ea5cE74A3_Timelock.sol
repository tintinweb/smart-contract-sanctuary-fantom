...
import {TimelockTax} from "./TimelockTax.sol";
...
abstract contract TimelockTax is Context {
    ...
    // Get the amount of tax to be claimed
    function taxClaimAvailable(address token_) public view returns (uint256) {
    ...
    function claimTax(address token_) external onlyTaxAccount returns (uint256) {
        require(block.timestamp >= lastTax.add(taxCooldown), "TimelockTax: Too early to claim tax");
        uint256 tax = taxClaimAvailable(token_);
