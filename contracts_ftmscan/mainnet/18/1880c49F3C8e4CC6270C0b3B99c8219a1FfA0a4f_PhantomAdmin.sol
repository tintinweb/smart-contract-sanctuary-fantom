...
contract PhantomAdmin is PhantomStorageMixin, DSAuth {
    ...
    //function deployBondPricingTWAP(address[] memory path) external auth {
        //PhantomBondPricing twap = new PhantomBondPricing(path);
        //PhantomStorage().registerContract(abi.encodePacked(phantom.contracts.bondpricing, path[0]), twap);
    //}

    //function deployTWAP(address pair) external auth {
        //PhantomTWAP twap = new PhantomTWAP(pair);
        //PhantomStorage().registerContract(abi.encodePacked(phantom.contracts.twap, pair), twap);
    //}
...
import {IPhantomPayments} from "IPhantomPayments.sol";
...
import {IPhantomTWAP} from "IPhantomTWAP.sol";
