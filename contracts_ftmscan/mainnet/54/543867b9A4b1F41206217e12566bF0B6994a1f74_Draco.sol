...
/*
    http://draco.finance
*/
contract Draco is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the DRACO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("DRACO", "DRACO") {
        // Mints 1 DRACO to contract creator for initial pool setup
    ...
    function _getDracoPrice() internal view returns (uint256 _dracoPrice) {
        try IOracle(dracoOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Draco: failed to fetch DRACO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _dracoPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_dracoPrice >= taxTiersTwaps[tierId]) {
    ...
    function setDracoOracle(address _dracoOracle) public onlyOperatorOrTaxOffice {
        require(_dracoOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints DRACO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DRACO to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDracoPrice = _getDracoPrice();
            currentTaxRate = _updateTaxRate(currentDracoPrice);
            if (currentDracoPrice < burnThreshold) {
                burnTax = true;
            }
        }
