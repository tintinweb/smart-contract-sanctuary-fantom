...
interface IMasterChef {
    ...
    function pendingSpirit(uint256 _pid, address _user) external view returns (uint rewards);
}
...
// TODO: Configure proxy voting via InSpirit
contract Testing is IOutputReceiver, Ownable, ERC165, ERC721Holder {
    ...
    uint private constant MAX_TIME = (365 days * 4 - 1 days);
    ...
    address private constant SPIRIT_FTM_GAUGE = 0xEFe02cB895b6E061FA227de683C04F3Ce19f3A62;
    ...
    // A mapping for handling MasterChef and its odd nature of using arrays
    mapping(address => uint) public lpTokenToPID;
    ...
    mapping(address => mapping(address => NFTDetails)) public nftAssetMappigns;
    ...
    constructor(address _provider, address _spirit, string memory _meta, address _master) {
    ...
    function lockSpiritLPs(uint amount, uint nftsDesired, address asset, address nftDesired) external returns (uint fnftId) {
        NFTDetails storage nft = nftAssetMappigns[asset][nftDesired];
        ...
        require(amount >= nft.pricePerNFT * nftsDesired, "Must pay in full!");
    ...
    function receiveRevestOutput(
        ...
        // Forward LPs to custodian wallet
        IERC20(asset).safeTransfer(lpCustodian, amount);
        
        // Mint preset NFT to caller
        // O(n) ftw
        for(uint i = 0; i < tracker.nftIds.length; i++) {
            IERC_721(tracker.nft).safeMint(payee, tracker.nftIds[i]);
        }
    ...
    function deposit(address asset, address farm, uint amount) internal {
        ...
        // Farm exists, check whether it is MasterChef or GaugeProxy
        if(farm == MASTER) {
            // Farm is MasterChef, use PID and deposit method
            IMasterChef(farm).deposit(lpTokenToPID[asset], amount);
        } else {
    ...
    function withdraw(address asset, address farm, uint amount) internal {
        if(farm == MASTER) {
            // Farm is MasterChef, use PID and deposit method
            IMasterChef(farm).withdraw(lpTokenToPID[asset], amount);
        ...
        if(!withdrawSpirit && (PRECISION * IGauge(SPIRIT_FTM_GAUGE).derivedBalance(address(this)) / bal) < MAX_MUL) {
            // Should lock up for inSPIRIT
            IInSpirit(INSPIRIT).create_lock(spiritReward, block.timestamp + MAX_TIME);
        } else {
            // Forward to dev wallet
            IERC20(spirit).safeTransfer(owner(), spiritReward);
        }
    }
    ...
    function addPIDMappings(address[] memory tokenPairs, uint[] memory pids) external onlyOwner {
        for(uint i = 0; i < tokenPairs.length; i++) {
            lpTokenToPID[tokenPairs[i]] = pids[i];
        }
    }
    ...
    function mapNFTToAsset(uint pricePerNFT, uint maxSupply, address asset, address nft) external onlyOwner {
        nftAssetMappigns[asset][nft] = NFTDetails(0, pricePerNFT, maxSupply);
    }
