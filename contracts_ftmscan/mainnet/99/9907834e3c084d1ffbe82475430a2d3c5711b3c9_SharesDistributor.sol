...
contract SharesDistributor is Administrable {
    ...
    function distribute() external onlyAdmin {
        uint256 balance = TwoShare.balanceOf(address(this));
        for (uint256 a = 0; a < allocations.length; a ++) {
            TwoShare.transfer(allocations[a].account, balance * allocations[a].points / totalPoints);
        }
    }

    function setTwoShare(address share) external onlyOwner {
        TwoShare = IERC20(share);
    }
