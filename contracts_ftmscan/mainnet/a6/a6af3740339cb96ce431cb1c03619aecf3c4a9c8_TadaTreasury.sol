...
contract TadaTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, STADA }
    ...
    address public sTADA;
    uint public sTADAQueue; // Delays change to sTADA address
    ...
    constructor (
       ...
       isLiquidityToken[ _TADABUSD ] = true;
       liquidityTokens.push( _TADABUSD );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for TADA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint TADA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( TADA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn TADA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ITADAERC20( TADA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sTADA ).balanceOf( msg.sender ); // Can only borrow against sTADA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with TADA
        @param _amount uint
     */
    function repayDebtWithTADA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ITADAERC20( TADA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, TADA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( TADA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( TADA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns TADA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match TADA decimals
            value_ = _amount.mul( 10 ** IERC20( TADA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.STADA ) { // 9
            sTADAQueue = block.number.add( blocksNeededForQueue );
        } else return false;
