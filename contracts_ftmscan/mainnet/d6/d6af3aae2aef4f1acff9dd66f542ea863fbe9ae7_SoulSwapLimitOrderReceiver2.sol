...
library SoulSwapLibrary {
    using SafeMathSoulSwap for uint;
...
// File: contracts/interfaces/ICoffinBox.sol
...
// File: contracts/SoulSwapLimitOrderReceiver2.sol

pragma solidity >=0.6.12;

contract SoulSwapLimitOrderReceiver2 is ILimitOrderReceiver {
    using BoringERC20 for IERC20;
    using BoringMath for uint256;
    ...
    constructor (address _factory, ICoffinBox _coffinBox, bytes32 _pairCodeHash) public {
        factory = _factory;
    ...
    function onLimitOrder (IERC20 tokenIn, IERC20 tokenOut, uint256 amountIn, uint256 amountMinOut, bytes calldata data) override external {
        ...
        coffinBox.withdraw(tokenIn, address(this), address(this), amountIn, 0);
        ...
        if (keepTokenIn) {
            ...
            uint256 profit = amountIn.sub(_swapTokensForExactTokens(amountMinOut, amountExternal, path, address(coffinBox)));
            coffinBox.deposit(tokenOut, address(coffinBox), msg.sender, amountMinOut, 0);
            tokenIn.safeTransfer(address(coffinBox), profit);
            coffinBox.deposit(tokenIn, address(coffinBox), to, profit, 0);

        } else {
            // amountExternal is the minimum amount of tokenOut we are prepared to recieve in the trade
            uint256 amountOut = _swapExactTokensForTokens(amountIn, amountExternal, path, address(coffinBox));
            coffinBox.deposit(tokenOut, address(coffinBox), msg.sender, amountMinOut, 0);
            coffinBox.deposit(tokenOut, address(coffinBox), to, amountOut.sub(amountMinOut), 0);
    ...
    function _swapExactTokensForTokens(
        ...
        IERC20(path[0]).safeTransfer(SoulSwapLibrary.pairFor(factory, path[0], path[1], pairCodeHash), amountIn);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            ISoulSwapPair(SoulSwapLibrary.pairFor(factory, input, output, pairCodeHash)).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
