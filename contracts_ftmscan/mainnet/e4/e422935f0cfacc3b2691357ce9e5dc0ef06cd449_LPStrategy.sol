...
// File contracts/libraries/CUniswapV2Library.sol
...
// File contracts/strategy/BaseStrategy.sol
...
abstract contract BaseStrategy is IStrategy, Ownable {
    ...
    */
    ...
    function swapExactTokensForUnderlying(uint256 amountOutMin, address inputToken) public onlyExecutor returns (uint256 amountOut) {
        ...
        uint256[] memory amounts = CUniswapV2Library.getAmountsOut(factory, amountIn, path, pairCodeHash);

        amountOut = amounts[amounts.length - 1];

        require(amountOut >= amountOutMin, "BentoBox Strategy: insufficient output");

        IERC20(path[0]).safeTransfer(CUniswapV2Library.pairFor(factory, path[0], path[1], pairCodeHash), amounts[0]);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? CUniswapV2Library.pairFor(factory, output, path[i + 2], pairCodeHash) : _to;
            IUniswapV2Pair(CUniswapV2Library.pairFor(factory, input, output, pairCodeHash)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
}


// File contracts/interfaces/ISushiSwap.sol
...
// File contracts/strategy/LPStrategy.sol
...
contract LPStrategy is BaseStrategy {
    ...
    uint256 private constant FEE = 0; // 0% fees on minted LP
    ...
    */
    ...
    function swapToLP(uint256 amountOutMin) public onlyExecutor returns (uint256 amountOut) {
        ...
        uint256[] memory amounts = CUniswapV2Library.getAmountsOut(factory, swapAmountIn, path, pairCodeHash);
