...
// Simple printer token that prints 2SHARES from 2omb.finance
// The value of this token is the fact that emissions end in 
// a couple of months, and keep utility with 3omb.finance!!!
...
// Dependency file: contracts/baby/TWOSHARESPRINTERDividendTracker.sol
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function distributeTWOSHARESDividends(uint256 amount) public onlyOwner {
...
contract TWOSHARESPRINTERDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TWOSHARESPRINTER contract."
        );
    }
...
// Root file: contracts/baby/TWOSHARESPRINTER.sol
...
// import "contracts/interfaces/IUniswapV2Factory.sol";
// import "contracts/interfaces/IUniswapV2Router02.sol";
// import "contracts/baby/TWOSHARESPRINTERDividendTracker.sol";
// import "contracts/BaseToken.sol";

contract TWOSHARESPRINTER is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    TWOSHARESPRINTERDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = TWOSHARESPRINTERDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require (
            ...
            "TWOSHARESPRINTER: The dividend tracker already has that address"
        );

        TWOSHARESPRINTERDividendTracker newDividendTracker = TWOSHARESPRINTERDividendTracker(
            payable(newAddress)
        );

        require (
            newDividendTracker.owner() == address(this),
            "TWOSHARESPRINTER: The new dividend tracker must be owned by the TWOSHARESPRINTER token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "TWOSHARESPRINTER: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "TWOSHARESPRINTER: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "TWOSHARESPRINTER: The PanTWOSHARESSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "TWOSHARESPRINTER: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "TWOSHARESPRINTER: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "TWOSHARESPRINTER: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialTWOSHARESBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapTokensForTWOSHARES(tokens);
        uint256 newBalance = (IERC20(rewardToken).balanceOf(address(this))).sub(
            initialTWOSHARESBalance
        );
    ...
    function swapTokensForTWOSHARES(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributeTWOSHARESDividends(dividends);
