...
contract Scar is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;    

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 SCAR-WFTM LP -> SCAR pool
    uint256 public constant INITIAL_SCAR_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the SCAR ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SCAR", "SCAR") {
        // Mints 1 SCAR to contract creator for initial pool setup
    ...
    function _getScarPrice() internal view returns (uint256 _scarPrice) {
        try IOracle(scarOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Scar: failed to fetch SCAR price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _scarPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_scarPrice >= taxTiersTwaps[tierId]) {
    ...
    function setScarOracle(address _scarOracle) public onlyOperatorOrTaxOffice {
        require(_scarOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints SCAR to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SCAR to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentScarPrice = _getScarPrice();
            currentTaxRate = _updateTaxRate(currentScarPrice);
            if (currentScarPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_scarPool != address(0), "!_scarPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_scarPool, INITIAL_SCAR_POOL_DISTRIBUTION);
