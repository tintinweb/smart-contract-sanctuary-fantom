...
contract HadesTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SHADE }
    ...
    address public sHADE;
    uint public sHADEQueue; // Delays change to sHADE address
    ...
    /**
        @notice allow approved address to deposit an asset for HADE
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint HADE needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( HADE ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn HADE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IHADEERC20( HADE ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHADE ).balanceOf( msg.sender ); // Can only borrow against sHADE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with HADE
        @param _amount uint
     */
    function repayDebtWithHADE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHADEERC20( HADE ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, HADE, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( HADE ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( HADE ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns HADE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match HADE decimals
            value_ = _amount.mul( 10 ** IERC20( HADE ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SHADE ) { // 9
            sHADEQueue = block.number.add( blocksNeededForQueue );
        } else return false;
