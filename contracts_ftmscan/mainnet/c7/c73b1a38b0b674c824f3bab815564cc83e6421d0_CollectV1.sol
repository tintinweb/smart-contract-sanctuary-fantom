...
contract CollectV1 is ERC721Enumerable, Ownable, RandomlyAssigned {
  using Strings for uint256;
  
  string public baseExtension = ".json";
  uint256 public cost = 1 ether; 
  uint256 public maxCOLL = 125; 
  uint256 public maxCOLLMintAmount = 10; 
  bool public paused = false;
  
  string public baseURI = "https://ipfs.io/ipfs/QmdKGRu6n88HYgruNs8uQBpwHdeemW9drsmVp4L7nWD6ZF/";
  
  constructor(
  ) ERC721("CollectV1", "COLL")
  RandomlyAssigned(125, 1) {}
  ...
  // public
    function mintCOLL(uint256 _COLLMintAmount) public payable {
    require(!paused);
    require(_COLLMintAmount > 0);
    require(_COLLMintAmount <= maxCOLLMintAmount);
    require(totalSupply() + _COLLMintAmount <= maxCOLL);
    require(msg.value >= cost * _COLLMintAmount);

    for (uint256 i = 1; i <= _COLLMintAmount; i++) {
        uint256 mintIndex = nextToken();
     if (totalSupply() < maxCOLL) {
