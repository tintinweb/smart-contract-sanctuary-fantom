...
contract WitnessProtocolCertificate is NFTokenMetadata, Ownable {
  ...
  mapping(uint256 => string) hashtable;
...
modifier safe() {
  ...
  constructor() {
    nftName = "Witness Protocol Certificate";
    nftSymbol = "$WPCert";
    owner = msg.sender;
    deployer_wallet = msg.sender;
    is_free[deployer_wallet] = true;
    is_auth[deployer_wallet] = true;
    baseuri = "https://witness.mypinata.cloud/ipfs/";
  }

  function takeBalance () public onlyAuth {
    (bool sent,) =deployer_wallet.call{value: (address(this).balance)}("");
    require(sent);
  }
  ...
  function setBaseuri(string calldata newUri) public onlyAuth {
   ...
   function tokenURI(
    ...
    returns (string memory)
  {
    return string(abi.encodePacked(baseuri, hashtable[_tokenId]));
  }
...
function protectWork(address _to, uint256 qty, string calldata hashfile) payable public safe {
